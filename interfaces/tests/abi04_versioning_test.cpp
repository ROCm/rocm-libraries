...
static int run_same_node(const char* p5, const char* p6, const char* p7) {
  ...
  if (!h5 || !h6 || !h7) { fprintf(stderr, "dlopen failed: %s\n", dlerror()); return 2; }
  ...
  dlclose(h5); dlclose(h6); dlclose(h7);
...
static int run_bsymbolic(const char* pa6, const char* pb7, const char* pplain6) {
  ...
  if (!A || !B || !P) { fprintf(stderr, "dlopen failed: %s\n", dlerror()); return 2; }
  ...
  printf("[bsymbolic] co-residency + DF_SYMBOLIC delta: %s\n",
         bad ? "FAIL"
             : "PASS (own-node resolves, cross-node nil, DF_SYMBOLIC present on bsym / absent on plain)");
  printf("[bsymbolic] -Bsymbolic proven genuine via DT_FLAGS while the co-residency "
...
int main(int argc, char** argv) {
  ...
  if (!strcmp(mode, "same_node") && argc == 5)
    return run_same_node(argv[2], argv[3], argv[4]);
  if (!strcmp(mode, "bsymbolic") && argc == 5)
    return run_bsymbolic(argv[2], argv[3], argv[4]);
  fprintf(stderr,
