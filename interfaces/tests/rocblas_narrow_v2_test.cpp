...
int main() {
    ...
    if (rocblas_sgemv(h,rocblas_operation_none,4,4,&alpha,a,4,x,1,&beta,y,1)!=rocblas_status_success) return 5;
    if (rocblas_sger(h,4,4,&alpha,x,1,y,1,a,4)!=rocblas_status_success) return 6;
    if (rocblas_sgemm(h,rocblas_operation_none,rocblas_operation_none,4,4,4,&alpha,a,4,b,4,&beta,c,4)!=rocblas_status_success) return 7;
    if (rocblas_ssymm(h,rocblas_side_left,rocblas_fill_upper,4,4,&alpha,a,4,b,4,&beta,c,4)!=rocblas_status_success) return 8;
