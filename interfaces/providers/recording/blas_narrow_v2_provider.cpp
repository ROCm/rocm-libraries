...
namespace {
struct Context { rocm_blas_v2_context_options options; };
...
void destroy_context(void* p) { delete static_cast<Context*>(p); }
template<class T> rocblas_status record(void* p, const T* r, const char* operation) {
...
#define CALLBACK(name, Type) rocblas_status name(void* p,const Type* r){return record(p,r,#name);}
...
#undef CALLBACK
rocblas_status matmul_query(void* p,const rocm_blas_v2_matmul_request* r,
...
const rocm_blas_v2_provider table = {
    rocm::interfaces::recording::header(sizeof(table)), create_context, destroy_context,
    vector_transform, vector_reduce, vector_rotate, matrix_vector, rank_update,
    matmul_query, matmul, structured_matrix, triangular_matrix, matrix_transform};
}
