...
**A flag-on run builds everything itself.** Nothing that carries artifacts between runs is keyed on cmake flags, so every such mechanism has to be switched off by hand or a flag-on run inherits stages that were built flag-off. The release workflow does this in two places whenever a flag is active: `stage_reuse_mode` goes to `dry-run`, and the `prebuilt_stages` / `baseline_run_id` seeding it would normally take from the ci-env baseline is dropped. Passing `prebuilt_stages` or `baseline_run_id` explicitly together with a gated label is a hard error rather than a silently wrong build. Expect a labeled pull request to take a full build's wall time, not an incremental one.
