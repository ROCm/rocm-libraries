...
.. glossary::
    ...
    Warp ID
        The index of the warp within a block. A warp's warp ID is guaranteed to be unique.

    Thread ID
        The index of the thread within a block.

    Lane ID
        The index of the thread within the warp. The lane ID can be obtained using ``ThreadID % WarpSize``.
