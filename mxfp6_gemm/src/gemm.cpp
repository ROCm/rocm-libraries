...
void gemm(OutType ot, int M, int N, int Kp,
    ...
    switch (ot) {
        case OutType::F32:
            detail::dispatch_gemm<float>(M, N, Kp, dA, dBsh, dsA, dsB,
                                         static_cast<float*>(dD), A_row_bytes, B_row_bytes);
            break;
        case OutType::F16:
            detail::dispatch_gemm<__half>(M, N, Kp, dA, dBsh, dsA, dsB,
                                          static_cast<__half*>(dD), A_row_bytes, B_row_bytes);
            break;
        case OutType::BF16:
            detail::dispatch_gemm<__hip_bfloat16>(M, N, Kp, dA, dBsh, dsA, dsB,
                                                  static_cast<__hip_bfloat16*>(dD), A_row_bytes, B_row_bytes);
            break;
    }
}
