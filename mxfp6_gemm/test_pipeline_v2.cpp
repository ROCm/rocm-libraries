    ...
    int k_iters, int A_row_stride)
{
    ...
    // ---- Prologue: load A[0] to buffer 0 ----
    {
        ...
        ds_write_b128_plain(lds, static_cast<uint32_t>(row * LDS_ROW_BYTES + seg), tmp);
        if (tid < 128) {
            tmp = *reinterpret_cast<const v4i*>(A_base + tid * A_row_stride + 32);
            ds_write_b128_plain(lds, static_cast<uint32_t>(tid * LDS_ROW_BYTES + 32), tmp);
        }
    }
    __syncthreads();   // compiler emits lgkmcnt(0) for the LDS writes above
    ...
    for (int ki = 0; ki < k_iters; ki++) {
        ...
        // Plain loads: compiler manages vmcnt automatically.
        ...
        // ---- Prefetch next A to other buffer ----
        // Plain LDS store; __syncthreads below carries the lgkmcnt fence.
        if (ki < k_iters - 1) {
            ...
            ds_write_b128_plain(lds, static_cast<uint32_t>(nxt_buf + row * LDS_ROW_BYTES + seg), tmp);
            if (tid < 128) {
                tmp = *reinterpret_cast<const v4i*>(
                    A_base + tid * A_row_stride + nki * 48 + 32);
                ds_write_b128_plain(lds, static_cast<uint32_t>(nxt_buf + tid * LDS_ROW_BYTES + 32), tmp);
            }
        }

        // ---- MFMA ----
        // No manual waitcnt: the backend inserts relative lgkmcnt/vmcnt before
        // these MFMAs based on the a0/a1/b_reg/sa/sb register dependencies,
        // and can overlap the still-in-flight loads with compute.
        ...
        // ---- Sync for next iteration (cross-wave LDS dep: compiler-blind) ----
        if (ki < k_iters - 1) {
            __syncthreads();
        }
    }
