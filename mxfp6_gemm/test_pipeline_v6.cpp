    ...
    int k_iters, int A_row_stride)
{
    ...
    // Coalesced cooperative A load: 256 threads, 2 threads/row, 24B each.
    // thread t → row t/2, K-half t%2. Adjacent threads read consecutive 24B of
    // the same row (within-row coalesced) vs v2's uncoalesced 1-thread-per-row.
    // Writes straight into the LDS slot ds_read expects (row*52 + half*24).
    int co_row = tid >> 1;          // 0..127
    int co_half = tid & 1;          // K-half 0/1
    auto load_A_coalesced = [&](int ki, int buf) {
        using v6i_a = int __attribute__((__vector_size__(24), __aligned__(4)));
        const char* g = A_base + (size_t)co_row * A_row_stride + ki * 48 + co_half * 24;
        v6i_a x = *reinterpret_cast<const v6i_a*>(g);
        char* l = reinterpret_cast<char*>(lds) + buf + co_row * LDS_ROW_BYTES + co_half * 24;
        *reinterpret_cast<v6i_a*>(l) = x;
    };

    // ---- Prologue: load A[0] to buffer 0 ----
    load_A_coalesced(0, 0);
    ...
    for (int ki = 0; ki < k_iters; ki++) {
        ...
        // ---- Prefetch next A to other buffer (coalesced) ----
        if (ki < k_iters - 1) {
            load_A_coalesced(ki + 1, (1 - (ki & 1)) * LDS_BUF);
        }
