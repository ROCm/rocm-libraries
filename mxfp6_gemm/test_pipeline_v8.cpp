...
// v8: v7's coalesced K-slab + DOUBLE BUFFER with explicit register-staged
// prefetch. The next slab's global loads are issued to registers and overlap the
// current slab's MFMA (software pipeline), then written to the other LDS buffer.
// Coalescing made the load fast, so the LDS round-trip + barrier serialization
// became the exposed cost (v7=792 vs direct-coalesced v4=1252); overlapping it
// is the fix. Occupancy may drop (LDS 2× + staging regs) — fine if stall is low.
    ...
    int k_iters, int A_row_stride)
{
    ...
    constexpr int KSLAB = 4;                   // K64 steps per slab (K=256)
    constexpr int LROW = 49 * 4;               // LDS row bytes: 49 dw, gcd(49,64)=1
    constexpr int LBUF = M_TILE * LROW;        // one slab buffer (bytes)
    constexpr int MAXCH = M_TILE * KSLAB * 3 / 256;  // max 16B chunks/thread = 6

    using v4i_a = int __attribute__((__vector_size__(16), __aligned__(4)));

    __shared__ uint32_t lds[2 * LBUF / 4];
    ...
    // Direct coalesced slab load (prologue): global → LDS buffer.
    auto load_slab = [&](int slab, int cur_steps, int buf) {
        int cpr = cur_steps * 3, total = M_TILE * cpr, koff = slab * (KSLAB * 48);
        for (int c = tid; c < total; c += 256) {
            int row = c / cpr, cir = c % cpr;
            const char* g = A_base + (size_t)row * A_row_stride + koff + cir * 16;
            char* l = reinterpret_cast<char*>(lds) + buf + row * LROW + cir * 16;
            *reinterpret_cast<v4i_a*>(l) = *reinterpret_cast<const v4i_a*>(g);
        }
    };

    int n_slabs = (k_iters + KSLAB - 1) / KSLAB;
    int steps0 = k_iters < KSLAB ? k_iters : KSLAB;
    load_slab(0, steps0, 0);
    __syncthreads();

    for (int slab = 0; slab < n_slabs; slab++) {
        int cur_buf = (slab & 1) * LBUF;
        int cur_steps = k_iters - slab * KSLAB;
        if (cur_steps > KSLAB) cur_steps = KSLAB;

        // ---- Prefetch NEXT slab's global loads into registers (overlap MFMA) ----
        bool has_next = (slab + 1) < n_slabs;
        v4i_a nreg[MAXCH];
        int n_cpr = 0, n_total = 0, n_buf = 0;
        if (has_next) {
            int nsteps = k_iters - (slab + 1) * KSLAB;
            if (nsteps > KSLAB) nsteps = KSLAB;
            n_cpr = nsteps * 3; n_total = M_TILE * n_cpr;
            n_buf = (1 - (slab & 1)) * LBUF;
            int koff = (slab + 1) * (KSLAB * 48);
            #pragma unroll
            for (int j = 0; j < MAXCH; j++) {
                int c = j * 256 + tid;
                if (c < n_total) {
                    int row = c / n_cpr, cir = c % n_cpr;
                    nreg[j] = *reinterpret_cast<const v4i_a*>(
                        A_base + (size_t)row * A_row_stride + koff + cir * 16);
                }
            }
        }

        // ---- Compute current slab (overlaps the prefetch loads above) ----
        for (int s = 0; s < cur_steps; s++) {
            int ki = slab * KSLAB + s;
            uint32_t off0 = cur_buf + ((wave_m * M_PER_WAVE + 0) * 32 + dim) * LROW + s * 48 + khalf * 24;
            uint32_t off1 = cur_buf + ((wave_m * M_PER_WAVE + 1) * 32 + dim) * LROW + s * 48 + khalf * 24;
        ...
        // ---- Store prefetched regs → other LDS buffer, then barrier ----
        if (has_next) {
            #pragma unroll
            for (int j = 0; j < MAXCH; j++) {
                int c = j * 256 + tid;
                if (c < n_total) {
                    int row = c / n_cpr, cir = c % n_cpr;
                    char* l = reinterpret_cast<char*>(lds) + n_buf + row * LROW + cir * 16;
                    *reinterpret_cast<v4i_a*>(l) = nreg[j];
                }
            }
            __syncthreads();
        }
