    ...
    int k_iters, int A_row_stride)
{
    ...
    // Precompute per-thread coalesced load offsets ONCE (slab-independent).
    // cpr=12 (full slab) mapping; div/mod done here, not in the hot loop (v7's
    // 2.1× VALU came from c/12, c%12 per chunk per slab).
    constexpr int CPR = KSLAB * 3;          // 12 chunks/row, full slab
    constexpr int MAXCH = M_TILE * CPR / 256; // 6
    int gA[MAXCH], lL[MAXCH], cir3[MAXCH];
    #pragma unroll
    for (int j = 0; j < MAXCH; j++) {
        int c = j * 256 + tid;
        int row = c / CPR, cir = c % CPR;
        gA[j]   = row * A_row_stride + cir * 16;   // global offset (− koff)
        lL[j]   = row * LROW + cir * 16;           // LDS offset
        cir3[j] = cir;
    }
    auto load_slab = [&](int slab, int cur_steps) {
        using v4i_a = int __attribute__((__vector_size__(16), __aligned__(4)));
        int koff = slab * (KSLAB * 48);
        int valid = cur_steps * 3;          // valid chunks/row
        #pragma unroll
        for (int j = 0; j < MAXCH; j++) {
            if (cir3[j] < valid) {
                char* l = reinterpret_cast<char*>(lds) + lL[j];
                *reinterpret_cast<v4i_a*>(l) =
                    *reinterpret_cast<const v4i_a*>(A_base + gA[j] + koff);
            }
        }
    };

    // Per-tile LDS read base (s-independent); ds_read offset = base + s*48.
    uint32_t rbase0 = ((wave_m * M_PER_WAVE + 0) * 32 + dim) * LROW + khalf * 24;
    uint32_t rbase1 = ((wave_m * M_PER_WAVE + 1) * 32 + dim) * LROW + khalf * 24;
    ...
    for (int slab = 0; slab < n_slabs; slab++) {
        ...
        // ds_read register prefetch: issue step s+1's A read, then MFMA step s,
        // so the LDS read latency overlaps the MFMAs (hides the 1.1B LDS wait).
        v6i a_cur0 = ds_read_fp6x32_plain(lds, rbase0);
        v6i a_cur1 = ds_read_fp6x32_plain(lds, rbase1);
        for (int s = 0; s < cur_steps; s++) {
            int ki = slab * KSLAB + s;
            v6i a_nxt0, a_nxt1;
            if (s + 1 < cur_steps) {
                a_nxt0 = ds_read_fp6x32_plain(lds, rbase0 + (s + 1) * 48);
                a_nxt1 = ds_read_fp6x32_plain(lds, rbase1 + (s + 1) * 48);
            }
            ...
            mfma_scale_f32_32x32x64_fp6<0>(acc00, a_cur0, b_reg[0], sa[0], sb[0]);
            mfma_scale_f32_32x32x64_fp6<0>(acc01, a_cur0, b_reg[1], sa[0], sb[1]);
            mfma_scale_f32_32x32x64_fp6<0>(acc10, a_cur1, b_reg[0], sa[1], sb[0]);
            mfma_scale_f32_32x32x64_fp6<0>(acc11, a_cur1, b_reg[1], sa[1], sb[1]);
