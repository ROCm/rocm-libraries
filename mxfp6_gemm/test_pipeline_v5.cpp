...
// v5: A is a RUNTIME activation (row-major, cannot be pre-shuffled). Load it
// directly from global in raw [M][K] layout — each lane reads its 24-byte MFMA
// slice (uncoalesced across rows), no LDS, no barrier, no host preprocessing.
// Hypothesis: with L2 ~95% hot, the uncoalesced direct load still beats the LDS
// round-trip. B stays pre-shuffled (weights, valid to preprocess offline).
    ...
    int k_iters, int A_row_stride)
{
    ...
    // aligned(4): khalf*24 is only 8-aligned, so steer the compiler off b128.
