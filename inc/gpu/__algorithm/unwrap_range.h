...
struct __unwrap_range_impl {
  __device__ _LIBGPU_HIDE_FROM_ABI static constexpr auto __unwrap(_Iter __first, _Sent __sent)
  ...
  __device__ _LIBGPU_HIDE_FROM_ABI static constexpr auto __unwrap(_Iter __first, _Sent __last) {
    return gpu::pair{std::move(__first), std::move(__last)};
  }

  __device__ _LIBGPU_HIDE_FROM_ABI static constexpr auto
  ...
  __device__ _LIBGPU_HIDE_FROM_ABI static constexpr auto __rewrap(const _Iter&, _Iter __iter)
    requires (!(random_access_iterator<_Iter> && sized_sentinel_for<_Sent, _Iter>))
  {
    return __iter;
  }
...
struct __unwrap_range_impl<_Iter, _Iter> {
  __device__ _LIBGPU_HIDE_FROM_ABI static constexpr auto __unwrap(_Iter __first, _Iter __last) {
    return gpu::pair{gpu::__unwrap_iter(std::move(__first)), gpu::__unwrap_iter(std::move(__last))};
  }
...
template <class _Iter, class _Sent>
__device__ _LIBGPU_HIDE_FROM_ABI constexpr auto __unwrap_range(_Iter __first, _Sent __last) {
