...
#ifndef __GPU___ALGORITHM_COPY_BACKWARD_H
#define __GPU___ALGORITHM_COPY_BACKWARD_H
...
struct __copy_backward_loop {
  ...
  operator()(_InIter __first, _InIter __last, _OutIter __result) const {
    ...
    if (__sfirst == __slast) {
      auto __iters =
          gpu::__copy_backward<_AlgPolicy>(_Traits::__local(__first), _Traits::__local(__last), std::move(__result));
      return gpu::make_pair(__last, __iters.second);
    }

    __result =
        gpu::__copy_backward<_AlgPolicy>(_Traits::__begin(__slast), _Traits::__local(__last), std::move(__result))
            .second;
    --__slast;
    while (__sfirst != __slast) {
      __result =
          gpu::__copy_backward<_AlgPolicy>(_Traits::__begin(__slast), _Traits::__end(__slast), std::move(__result))
              .second;
      --__slast;
    }
    __result = gpu::__copy_backward<_AlgPolicy>(_Traits::__local(__first), _Traits::__end(__slast), std::move(__result))
                   .second;
  ...
  operator()(_InIter __first, _InIter __last, _OutIter __result) {
    ...
    while (true) {
      ...
      auto __iter        = gpu::__copy_backward<_AlgPolicy>(__last - __size, __last, __local_last).second;
      __last -= __size;
              ...
              _BidirectionalIterator2 __result)
{
  ...
  return gpu::__copy_backward<_ClassicAlgPolicy>(
      std::move(__first), std::move(__last), std::move(__result)).second;
}

} // namespace gpu

#endif // __GPU___ALGORITHM_COPY_BACKWARD_H
