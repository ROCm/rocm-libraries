...
#ifndef __GPU___ALGORITHM_SORT_H
#define __GPU___ALGORITHM_SORT_H
...
template <class _Tp>
struct __is_simple_comparator<__less<_Tp>&> : std::true_type {};
template <class _Tp>
struct __is_simple_comparator<less<_Tp>&> : std::true_type {};
template <class _Tp>
struct __is_simple_comparator<greater<_Tp>&> : std::true_type {};
#if _LIBGPU_STD_VER >= 20
template <>
struct __is_simple_comparator<ranges::less&> : std::true_type {};
template <>
struct __is_simple_comparator<ranges::greater&> : std::true_type {};
#endif

template <class _Compare, class _Iter, class _Tp = typename std::iterator_traits<_Iter>::value_type>
using __use_branchless_sort =
    std::integral_constant<bool, __is_cpp17_contiguous_iterator<_Iter>::value && sizeof(_Tp) <= sizeof(void*) &&
...
_LIBGPU_CONSTEXPR_SINCE_CXX14 void __selection_sort(_BidirectionalIterator __first, _BidirectionalIterator __last,
  ...
  for (--__lm1; __first != __lm1; ++__first) {
    _BidirectionalIterator __i = gpu::__min_element<_Compare>(__first, __last, __comp);
    if (__i != __first)
...
__device__ inline _LIBGPU_HIDE_FROM_ABI void __swap_bitmap_pos(
  ...
  using _Ops = _IterOps<_AlgPolicy>;
  typedef typename std::iterator_traits<_RandomAccessIterator>::difference_type difference_type;
  // Swap one gpu::pair on each iteration as long as both bitsets have at least one
  // element for swapping.
  while (__left_bitset != 0 && __right_bitset != 0) {
    difference_type __tz_left  = __LIBGPU_ctz(__left_bitset);
    __left_bitset              = __LIBGPU_blsr(__left_bitset);
    difference_type __tz_right = __LIBGPU_ctz(__right_bitset);
    __right_bitset             = __LIBGPU_blsr(__right_bitset);
...
__device__ inline _LIBGPU_HIDE_FROM_ABI void __swap_bitmap_pos_within(
  ...
  if (__left_bitset) {
    ...
    while (__left_bitset != 0) {
      difference_type __tz_left = __detail::__block_size - 1 - __LIBGPU_clz(__left_bitset);
    ...
    while (__right_bitset != 0) {
      difference_type __tz_right = __detail::__block_size - 1 - __LIBGPU_clz(__right_bitset);
...
__bitset_partition(_RandomAccessIterator __first, _RandomAccessIterator __last, _Compare __comp) {
  ...
  *__pivot_pos = std::move(__pivot);
  return gpu::make_pair(__pivot_pos, __already_partitioned);
}
...
void __introsort(_RandomAccessIterator __first,
  ...
  _LIBGPU_CONSTEXPR difference_type __limit = 24;
  // Lower bound for using Tuckey's ninther technique for median computation.
  _LIBGPU_CONSTEXPR difference_type __ninther_threshold = 128;
  while (true) {
    ...
    if (__depth == 0) {
      ...
      gpu::__partial_sort<_AlgPolicy, _Compare>(__first, __last, __last, __comp);
      return;
    }
    --__depth;
    {
...
__device__ inline _LIBGPU_HIDE_FROM_ABI _Number __log2i(_Number __n) {
  ...
  if (sizeof(__n) <= sizeof(unsigned))
    return sizeof(unsigned) * CHAR_BIT - 1 - __LIBGPU_clz(static_cast<unsigned>(__n));
  if (sizeof(__n) <= sizeof(unsigned long))
    return sizeof(unsigned long) * CHAR_BIT - 1 - __LIBGPU_clz(static_cast<unsigned long>(__n));
  if (sizeof(__n) <= sizeof(unsigned long long))
    return sizeof(unsigned long long) * CHAR_BIT - 1 - __LIBGPU_clz(static_cast<unsigned long long>(__n));
...
extern template _LIBGPU_FUNC_VIS void __sort<__less<char>&, char*>(char*, char*, __less<char>&);
#ifndef _LIBGPU_HAS_NO_WIDE_CHARACTERS
extern template _LIBGPU_FUNC_VIS void __sort<__less<wchar_t>&, wchar_t*>(wchar_t*, wchar_t*, __less<wchar_t>&);
#endif
extern template _LIBGPU_FUNC_VIS void __sort<__less<signed char>&, signed char*>(signed char*, signed char*, __less<signed char>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<unsigned char>&, unsigned char*>(unsigned char*, unsigned char*, __less<unsigned char>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<short>&, short*>(short*, short*, __less<short>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<unsigned short>&, unsigned short*>(unsigned short*, unsigned short*, __less<unsigned short>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<int>&, int*>(int*, int*, __less<int>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<unsigned>&, unsigned*>(unsigned*, unsigned*, __less<unsigned>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<long>&, long*>(long*, long*, __less<long>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<unsigned long>&, unsigned long*>(unsigned long*, unsigned long*, __less<unsigned long>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<long long>&, long long*>(long long*, long long*, __less<long long>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<unsigned long long>&, unsigned long long*>(unsigned long long*, unsigned long long*, __less<unsigned long long>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<float>&, float*>(float*, float*, __less<float>&);
extern template _LIBGPU_FUNC_VIS void __sort<__less<double>&, double*>(double*, double*, __less<double>&);
...
void __sort_impl(_RandomAccessIterator __first, _RandomAccessIterator __last, _Comp& __comp) {
  ...
  if (__builtin_is_constant_evaluated()) {
    gpu::__partial_sort<_AlgPolicy>(
        gpu::__unwrap_iter(__first), gpu::__unwrap_iter(__last), gpu::__unwrap_iter(__last), __comp);
  } else {
    std::__sort_dispatch<_AlgPolicy>(gpu::__unwrap_iter(__first), gpu::__unwrap_iter(__last), __comp);
  }
}
...
#endif // __GPU___ALGORITHM_SORT_H
