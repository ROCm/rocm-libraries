...
#ifndef __GPU___ITERATOR_COUNTED_ITERATOR_H
#define __GPU___ITERATOR_COUNTED_ITERATOR_H
...
{
public:
  _LIBGPU_NO_UNIQUE_ADDRESS _Iter __current_ = _Iter();
  ...
  constexpr counted_iterator& operator-=(iter_difference_t<_Iter> __n)
    requires random_access_iterator<_Iter>
  {
    _LIBGPU_ASSERT(-__n <= __count_, "Attempt to subtract too large of a size: "
  ...
  {
    _LIBGPU_ASSERT(__x.__count_ > 0 && __y.__count_ > 0,
                   "Iterators must not be past end of range.");
    return ranges::iter_swap(__x.__current_, __y.__current_);
  }
};
_LIBGPU_CTAD_SUPPORTED_FOR_TYPE(counted_iterator);

template<input_iterator _Iter>
  requires same_as<_ITER_TRAITS<_Iter>, std::iterator_traits<_Iter>>
struct std::iterator_traits<counted_iterator<_Iter>> : std::iterator_traits<_Iter> {
  using pointer = std::conditional_t<contiguous_iterator<_Iter>,
                                std::add_pointer_t<iter_reference_t<_Iter>>, void>;
};
...
#endif // __GPU___ITERATOR_COUNTED_ITERATOR_H
