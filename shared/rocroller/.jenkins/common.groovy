...
def runCompileCommand(platform, project, jobName, boolean codeCoverage=false, boolean enableTimers=false, String target='', boolean useYamlCpp=true, boolean staticAnalysis=false)
{
    ...
    withSSH(platform) {
        ...
        def command = """#!/usr/bin/env bash
                ...
                echo Using \$numThreads out of \$(nproc) threads for compiling. Total \$numExecutors executors.
...
def runTestCommand (platform, project)
{
    ...
    def command = """#!/usr/bin/env bash
                ...
                numExecutors=${executorCount}
                numThreads=\$((\$(nproc) / numExecutors))
                echo Using \$numThreads out of \$(nproc) threads for testing. Total of \$numExecutors executors.
