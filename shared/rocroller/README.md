...
Additionally, there are tests that run against guidepost kernels that have been generated by Tensile. See the [README](https://github.com/ROCm/rocm-libraries/shared/rocroller/blob/develop/test/unit/GemmGuidePost/README.md) for more information.
...
- Set `ROCROLLER_RANDOM_SEED` to an unsigned integer to control the seed for the `RandomGenerator` used in unit tests.
...
- Set `ROCROLLER_ARCHITECTURE_FILE` to override the default GPU architecture file (`source/rocRoller/GPUArchitecture_def.msgpack`). Supported formats are YAML and Msgpack.
...
To profile rocRoller on Linux, you can use the `perf` tool to sample the call graph and generate a flame graph visualization.

**FlameGraph Setup:**
First, clone [Brendan Gregg's FlameGraph](https://github.com/brendangregg/FlameGraph) repository.

**Build with Timers:**
Configure CMake with timers enabled:
```bash
cmake -DROCROLLER_ENABLE_TIMERS=ON ...
make -j
```

**Collect and Visualize Profiling Data:**
Follow the steps below (see also [scripts/flamegraph.py](scripts/flamegraph.py)):
```bash
...
```
This produces an interactive SVG flame graph (`kernel.svg`) showing call stack hot spots.

**Omniperf Profiling with Docker:**
If using the trace Dockerfile, you can profile rocRoller or Tensile guideposts with Omniperf via `rrperf`. For usage details, run:
```bash
