...
| `InsertClusterBarrierPass` | Inserts cluster-barrier (`s_barrier_signal/wait -3`) handshakes at four rules covering the main and tail loops; under `StreamKMulticast` it suppresses those rules and inserts producer-side `s_wait_tensorcnt` drains instead. See [Insert Cluster Barrier Pass](cluster-barrier.md) |
