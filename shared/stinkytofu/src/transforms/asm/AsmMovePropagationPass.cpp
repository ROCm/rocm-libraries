...
struct ReadWriteDestKeys {
    ...
    // nothing on the supported architectures marks more than two, so four leaves
    // room for one that does. An instruction that marked more would be recorded
    // only in part, and answering from the keys that happened to fit is how a
    // move gets propagated through an unrecorded tie -- the bug this pass exists
    // to prevent. So overflow refuses the whole instruction instead.
