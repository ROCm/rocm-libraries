...
struct ReadWriteDestKeys {
    ...
    // nothing on the supported architectures marks more than two. The slack is
    // deliberate: collectReadWriteDestKeys drops anything past capacity, so a
    // future opcode with three would silently lose one.
