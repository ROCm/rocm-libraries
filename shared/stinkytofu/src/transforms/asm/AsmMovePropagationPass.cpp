...
struct ReadWriteDestKeys {
    ...
    // room for one that does. Past capacity collectReadWriteDestKeys stops
    // collecting, which would let a move propagate through a tied operand -- the
    // bug this pass exists to prevent -- so treat it as a hard limit, not a hint.
...
ReadWriteDestKeys collectReadWriteDestKeys(const StinkyInstruction& inst) {
    ...
    for (const HwInstDesc::OperandFieldDesc& field : desc->operandFields) {
        ...
        if (result.count == ReadWriteDestKeys::kCapacity) break;
