...
// Whiten then run i_proj (Linear,ReLU,Linear) -> item embedding. `norm` and
// `scratch` are caller-owned scratch buffers reused across candidates (resized
// in place, so no per-candidate heap allocation in the scoring loop).
...
// `norm` and `scratch` are caller-owned scratch buffers reused across
// candidates (resized in place, so no per-candidate heap allocation).
...
// FNV-1a hash over the 8-int kernel signature, so the smart_K whitelist can be
// looked up in O(1) (see rank_configs_impl).
struct SigHash {
...
std::vector<Result> rank_configs_impl(const LoadedModel& model, const Problem& problem,
  ...
  // Build an O(1)-lookup set once so the per-config membership test in scan()
  // is O(1) instead of a linear scan over every whitelisted signature
  // (previously O(configs * signatures) for the whole call).
  const bool have_sk = !cm.smart_k_signatures.empty();
  std::unordered_set<std::array<int, 8>, SigHash> sk_set;
  if (have_sk) {
    sk_set.reserve(cm.smart_k_signatures.size() * 2);
    sk_set.insert(cm.smart_k_signatures.begin(), cm.smart_k_signatures.end());
  }
  ...
  // i_emb / norm scratch buffers are declared once and reused across candidates
  // (resized in place) so the scoring loop performs no per-config heap churn.
  std::vector<std::pair<std::uint32_t, float>> scored;
  scored.reserve(cand.size());
  std::vector<float> i_emb, norm;
  std::array<float, kInterDim> x_feat;
  for (std::uint32_t ci : cand) {
    const Config& cc = configs[ci];
    MlParams ml = ml_from(cc);
    get_or_compute_ie(cm, cc, ml, i_dim, norm, scratch, i_emb);
    float emb_score = score_from_embeds(cm, q_emb.data(), i_emb.data(), cm.embed_dim);
    build_inter_features(problem, cc, ml, hw, x_feat.data());
    float inter_score = compute_inter_score(cm, x_feat.data(), x_dim, norm, scratch);
