...
struct StructuredSparsityProblem {
    StructuredSparsityProblem(TensorView inputTensor,
                              MutableTensorView prunedTensor,
    ...
    StructuredSparsityProblem(TensorView inputTensor,
                              ...
                              StructuredSparsityPattern sparsityPattern)
        : StructuredSparsityProblem(std::move(inputTensor),
                                    std::move(prunedTensor),
                                    std::move(compressedTensor),
                                    std::move(sparsityPattern)) {
...
struct TwoOfFourMetadataProblem {
    TwoOfFourMetadataProblem(TensorView retainedIndexTensor,
                             MutableTensorView metadataTensor,
                             size_t sparsityAxis)
        : retainedIndices(std::move(retainedIndexTensor)),
...
inline void enumerateRetainedPositionSets(size_t groupSize,
    ...
    for (size_t position = nextPosition; position + remaining <= groupSize; ++position) {
        ...
        enumerateRetainedPositionSets(
            groupSize, retainedElements, position + 1, current, result);
        current.pop_back();
    }
}

inline StructuredSparsityPlan validateStructuredSparsity(
    ...
    if (inputShape[problem.pattern.axis] == 0)
        throw std::invalid_argument(
            "Structured sparsity axis extent must be nonzero.");
    ...
    std::vector<size_t> compressedDimensions(
        inputShape.dimensions().begin(), inputShape.dimensions().end());
    compressedDimensions[problem.pattern.axis] =
        inputShape[problem.pattern.axis] / problem.pattern.groupSize *
    ...
    if (problem.twoOfFourMetadata) {
        ...
        if (problem.twoOfFourMetadata->shape() !=
            Shape(std::move(metadataDimensions)))
            throw std::invalid_argument(
                "Structured sparsity two-of-four metadata shape mismatch.");
    }
    ...
    if (problem.pattern.selection == StructuredSparsitySelection::Fixed) {
        ...
        if (std::adjacent_find(positions.begin(), positions.end()) != positions.end())
            throw std::invalid_argument(
                "Structured sparsity fixed positions must be unique.");
        if (positions.back() >= problem.pattern.groupSize)
            throw std::out_of_range(
                "Structured sparsity fixed position exceeds group size.");
        plan.retainedPositionSets.push_back(std::move(positions));
    } else {
        std::vector<size_t> current;
        current.reserve(problem.pattern.retainedElements);
        enumerateRetainedPositionSets(problem.pattern.groupSize,
                                      problem.pattern.retainedElements,
                                      0,
                                      current,
                                      plan.retainedPositionSets);
        std::sort(
            plan.retainedPositionSets.begin(),
            plan.retainedPositionSets.end(),
            [](const std::vector<size_t>& left,
               const std::vector<size_t>& right) {
                return std::lexicographical_compare(left.rbegin(),
                                                    left.rend(),
                                                    right.rbegin(),
                                                    right.rend());
            });
...
inline void coordinatesForLine(size_t line,
                               const Shape& shape,
                               size_t excludedAxis,
...
inline void copyEncodedElement(ScalarType type,
                               std::span<const std::byte> source,
                               ptrdiff_t sourceElement,
    ...
    if (bits % 8 == 0) {
        ...
        std::memcpy(destination.data() + destinationBit / 8,
                    source.data() + sourceBit / 8,
                    bytes);
        return;
    }

    for (uint64_t bit = 0; bit < bits; ++bit)
        setBit(destination, destinationBit + bit,
               bitAt(source, sourceBit + bit));
}

inline void zeroEncodedElement(ScalarType type,
                               std::span<std::byte> destination,
                               ptrdiff_t destinationElement) {
    const uint64_t bits = scalarTypeInfo(type).storageBits;
    ...
    const uint64_t destinationBit = static_cast<uint64_t>(destinationElement) * bits;
    if (bits % 8 == 0) {
        std::memset(destination.data() + destinationBit / 8,
                    0,
                    static_cast<size_t>(bits / 8));
        return;
    }
    for (uint64_t bit = 0; bit < bits; ++bit)
        setBit(destination, destinationBit + bit, false);
}

inline size_t retainedPositionSetIndexForGroup(
    const StructuredSparsityProblem& problem,
    const StructuredSparsityPlan& plan,
    size_t groupLinearIndex) {
    if (problem.pattern.selection == StructuredSparsitySelection::Fixed)
        return 0;
    const int selected = indexedUniformInteger(
        problem.pattern.seed,
        problem.pattern.stream,
        groupLinearIndex,
        0,
...
inline const std::vector<size_t>& retainedPositionsForGroup(
    ...
    const StructuredSparsityPlan& plan,
    size_t groupLinearIndex) {
    return plan.retainedPositionSets[
        retainedPositionSetIndexForGroup(problem, plan, groupLinearIndex)];
}
...
inline std::vector<TwoOfFourPositionSet> makeTwoOfFourPositionSets(
    ...
    for (const std::vector<size_t>& retained : plan.retainedPositionSets) {
        ...
        positionSet.metadataNibble =
            twoOfFourMetadataNibble(static_cast<uint8_t>(retained[0]),
...
inline StructuredSparsityRunInfo applyTwoOfFourByteAligned(
    const StructuredSparsityProblem& problem,
    const StructuredSparsityPlan& plan,
    size_t firstSlice,
    size_t endSlice) {
    const size_t axis = problem.pattern.axis;
    const std::vector<TwoOfFourPositionSet> positionSets =
        makeTwoOfFourPositionSets(plan);
    const TwoOfFourPositionSet& fixedPositionSet = positionSets.front();
    const uint8_t fullMetadataByte = static_cast<uint8_t>(
        fixedPositionSet.metadataNibble |
        static_cast<uint8_t>(fixedPositionSet.metadataNibble << 4));
    const size_t metadataBytesPerLine = (plan.groupsPerLine + 1) / 2;
    const size_t inputBytesPerLine =
        plan.groupsPerLine * 4 * BytesPerElement;

    std::vector<size_t> coordinates(problem.input.shape().rank(), 0);
    for (size_t line = firstSlice; line < endSlice; ++line) {
        coordinatesForLine(line,
                           problem.input.shape(),
                           axis,
                           problem.pattern.indexOrder,
                           coordinates);
        const ptrdiff_t inputBase =
            problem.input.layout().elementOffset(coordinates);
        const ptrdiff_t prunedBase =
            problem.pruned.layout().elementOffset(coordinates);
        const ptrdiff_t compressedBase =
            problem.compressed.layout().elementOffset(coordinates);
        const ptrdiff_t retainedIndexBase =
            problem.retainedIndices
                ? problem.retainedIndices->layout().elementOffset(coordinates)
                : 0;
        ...
        uint8_t* metadataPointer =
            problem.twoOfFourMetadata
                ? reinterpret_cast<uint8_t*>(
                      problem.twoOfFourMetadata->storage().data()) +
                      static_cast<size_t>(metadataBase)
                : nullptr;
        if constexpr (!RandomSelection) {
            if (metadataPointer) {
                std::memset(metadataPointer,
                            fullMetadataByte,
                            plan.groupsPerLine / 2);
                if (plan.groupsPerLine % 2 != 0)
                    metadataPointer[metadataBytesPerLine - 1] =
                        fixedPositionSet.metadataNibble;
            }
        }
        ...
        uint8_t* retainedIndexPointer =
            problem.retainedIndices
                ? reinterpret_cast<uint8_t*>(
                      problem.retainedIndices->storage().data()) +
                      static_cast<size_t>(retainedIndexBase)
                : nullptr;
        for (size_t group = 0; group < plan.groupsPerLine; ++group) {
            const TwoOfFourPositionSet& positionSet = [&]()
                -> const TwoOfFourPositionSet& {
                if constexpr (RandomSelection) {
                    return positionSets[retainedPositionSetIndexForGroup(
                        problem,
                        plan,
                        line * plan.groupsPerLine + group)];
                } else {
                    return fixedPositionSet;
                }
            }();
            const std::byte* inputGroup =
                inputPointer + group * 4 * BytesPerElement;
            std::byte* prunedGroup =
                prunedPointer + group * 4 * BytesPerElement;
            std::byte* compressedGroup =
                compressedPointer + group * 2 * BytesPerElement;

            std::memcpy(compressedGroup,
                        inputGroup +
                            positionSet.retained[0] * BytesPerElement,
                        BytesPerElement);
            std::memcpy(compressedGroup + BytesPerElement,
                        inputGroup +
                            positionSet.retained[1] * BytesPerElement,
                        BytesPerElement);
            std::memset(prunedGroup +
                            positionSet.dropped[0] * BytesPerElement,
                        0,
                        BytesPerElement);
            std::memset(prunedGroup +
                            positionSet.dropped[1] * BytesPerElement,
                        0,
                        BytesPerElement);

            if (retainedIndexPointer) {
                retainedIndexPointer[group * 2] =
                    static_cast<uint8_t>(positionSet.retained[0]);
                retainedIndexPointer[group * 2 + 1] =
    ...
    return StructuredSparsityRunInfo{
        ...
        .retainedIndicesWritten =
            problem.retainedIndices ? groups * 2 : 0,
        .metadataBytesWritten =
            problem.twoOfFourMetadata
                ? slices * metadataBytesPerLine
                : 0,
    };
}
...
inline StructuredSparsityRunInfo applyTwoOfFourByteAlignedStrided(
    ...
    const ptrdiff_t inputAxisStride =
        problem.input.layout().strides()[axis];
    const ptrdiff_t prunedAxisStride =
        problem.pruned.layout().strides()[axis];
    const ptrdiff_t compressedAxisStride =
        problem.compressed.layout().strides()[axis];
    const ptrdiff_t retainedIndexAxisStride =
        problem.retainedIndices
            ? problem.retainedIndices->layout().strides()[axis]
            : 0;
    const ptrdiff_t metadataAxisStride =
        problem.twoOfFourMetadata
            ? problem.twoOfFourMetadata->layout().strides()[axis]
            : 0;
    const bool inPlace =
        problem.input.storage().data() == problem.pruned.storage().data();
    ...
    for (size_t line = firstSlice; line < endSlice; ++line) {
        ...
        for (size_t group = 0; group < plan.groupsPerLine; ++group) {
            ...
            const ptrdiff_t inputGroup =
                inputBase + static_cast<ptrdiff_t>(group * 4) *
                                inputAxisStride;
            const ptrdiff_t prunedGroup =
                prunedBase + static_cast<ptrdiff_t>(group * 4) *
                                 prunedAxisStride;
            const ptrdiff_t compressedGroup =
                compressedBase + static_cast<ptrdiff_t>(group * 2) *
                                     compressedAxisStride;

            const ptrdiff_t firstInput =
                inputGroup +
                static_cast<ptrdiff_t>(positionSet.retained[0]) *
                                 inputAxisStride;
            const ptrdiff_t secondInput =
                inputGroup +
                static_cast<ptrdiff_t>(positionSet.retained[1]) *
                                 inputAxisStride;
            std::memcpy(compressedPointer(compressedGroup),
                        inputPointer(firstInput),
                        BytesPerElement);
            std::memcpy(
                compressedPointer(compressedGroup + compressedAxisStride),
                inputPointer(secondInput),
                BytesPerElement);
            ...
            std::memset(
                prunedPointer(
                    prunedGroup +
                    static_cast<ptrdiff_t>(positionSet.dropped[0]) *
                        prunedAxisStride),
                0,
                BytesPerElement);
            std::memset(
                prunedPointer(
                    prunedGroup +
                    static_cast<ptrdiff_t>(positionSet.dropped[1]) *
                        prunedAxisStride),
                0,
                BytesPerElement);
            ...
            if (problem.retainedIndices) {
                ...
                const ptrdiff_t retainedGroup =
                    retainedIndexBase +
                    static_cast<ptrdiff_t>(group * 2) *
                        retainedIndexAxisStride;
                retainedIndexPointer[static_cast<size_t>(retainedGroup)] =
                    static_cast<uint8_t>(positionSet.retained[0]);
                retainedIndexPointer[static_cast<size_t>(
                    retainedGroup + retainedIndexAxisStride)] =
                    static_cast<uint8_t>(positionSet.retained[1]);
            }
            if (problem.twoOfFourMetadata) {
                uint8_t* metadataPointer =
                ...
                const ptrdiff_t metadataOffset =
                    ...
                    static_cast<ptrdiff_t>(group / 2) *
                        metadataAxisStride;
                uint8_t& metadataByte =
                    metadataPointer[static_cast<size_t>(metadataOffset)];
                if (group % 2 == 0)
                    metadataByte = positionSet.metadataNibble;
                else
                    metadataByte = static_cast<uint8_t>(
...
inline StructuredSparsityRunInfo applyTwoOfFourByteAlignedBySelection(
    ...
    if (problem.pattern.selection == StructuredSparsitySelection::Random) {
        return contiguousAxis
                   ? applyTwoOfFourByteAligned<BytesPerElement, true>(
                         problem, plan, firstSlice, endSlice)
                   : applyTwoOfFourByteAlignedStrided<
                         BytesPerElement,
                         true>(problem, plan, firstSlice, endSlice);
    }
    return contiguousAxis
               ? applyTwoOfFourByteAligned<BytesPerElement, false>(
                     problem, plan, firstSlice, endSlice)
               : applyTwoOfFourByteAlignedStrided<
                     BytesPerElement,
                     false>(problem, plan, firstSlice, endSlice);
}

inline std::optional<StructuredSparsityRunInfo>
    tryTwoOfFourByteAligned(const StructuredSparsityProblem& problem,
                            const StructuredSparsityPlan& plan,
                            size_t firstSlice,
                            size_t endSlice) {
    if (problem.pattern.groupSize != 4 ||
        problem.pattern.retainedElements != 2 ||
    ...
    const bool contiguousAxis =
        ...
        (!problem.retainedIndices ||
         problem.retainedIndices->layout().strides()[axis] == 1) &&
        (!problem.twoOfFourMetadata ||
         problem.twoOfFourMetadata->layout().strides()[axis] == 1);

    switch (scalarTypeInfo(problem.input.type()).storageBits / 8) {
        case 1:
            return applyTwoOfFourByteAlignedBySelection<1>(
                problem, plan, firstSlice, endSlice, contiguousAxis);
        case 2:
            return applyTwoOfFourByteAlignedBySelection<2>(
                problem, plan, firstSlice, endSlice, contiguousAxis);
        case 4:
            return applyTwoOfFourByteAlignedBySelection<4>(
                problem, plan, firstSlice, endSlice, contiguousAxis);
        case 8:
            return applyTwoOfFourByteAlignedBySelection<8>(
                problem, plan, firstSlice, endSlice, contiguousAxis);
        case 16:
            return applyTwoOfFourByteAlignedBySelection<16>(
                problem, plan, firstSlice, endSlice, contiguousAxis);
        default:
            return std::nullopt;
    }
...
inline StructuredSparsityRunInfo applyStructuredSparsity(
    ...
    const detail::StructuredSparsityPlan plan =
        detail::validateStructuredSparsity(problem);
    const auto [firstSlice, endSlice] =
        detail::validateStructuredSparsitySliceRange(plan, sliceRange);
    if (const auto fastRun =
            detail::tryTwoOfFourByteAligned(
                problem, plan, firstSlice, endSlice))
        return *fastRun;
    const size_t axis = problem.pattern.axis;
    ...
    for (size_t line = firstSlice; line < endSlice; ++line) {
        detail::coordinatesForLine(line,
                                   problem.input.shape(),
                                   axis,
                                   problem.pattern.indexOrder,
                                   inputCoordinates);
        const ptrdiff_t inputBase =
            problem.input.layout().elementOffset(inputCoordinates);
        const ptrdiff_t prunedBase =
            problem.pruned.layout().elementOffset(inputCoordinates);
        const ptrdiff_t compressedBase =
        ...
        for (size_t group = 0; group < plan.groupsPerLine; ++group) {
            ...
            for (size_t retainedIndex = 0; retainedIndex < retainedElements;
                 ++retainedIndex) {
                const size_t position = retainedPositions[retainedIndex];
                const ptrdiff_t sourceOffset =
                    inputBase +
                    static_cast<ptrdiff_t>(group * groupSize + position) *
                        inputAxisStride;
                const ptrdiff_t compressedOffset =
                    compressedBase +
                    static_cast<ptrdiff_t>(
                        group * retainedElements + retainedIndex) *
                        compressedAxisStride;
                detail::copyEncodedElement(problem.input.type(),
                                           problem.input.storage(),
                                           sourceOffset,
                                           problem.compressed.storage(),
                                           compressedOffset);
                if (problem.retainedIndices) {
                    ...
                    detail::encodeScalarKnown<ScalarType::UInt8>(
                        problem.retainedIndices->storage(),
                        retainedIndexOffset,
            ...
            if (problem.twoOfFourMetadata) {
                const uint8_t nibble = detail::twoOfFourMetadataNibble(
                    static_cast<uint8_t>(retainedPositions[0]),
                    static_cast<uint8_t>(retainedPositions[1]));
                const ptrdiff_t metadataOffset =
                    metadataBase +
                    static_cast<ptrdiff_t>(group / 2) * metadataAxisStride;
                if (group % 2 == 0) {
                    detail::encodeScalarKnown<ScalarType::UInt8>(
                        problem.twoOfFourMetadata->storage(),
                        metadataOffset,
                        nibble);
                } else {
                    const uint8_t previous = detail::decodeScalarKnown<
                        ScalarType::UInt8,
                        uint8_t>(problem.twoOfFourMetadata->storage(),
                                 metadataOffset);
                    detail::encodeScalarKnown<ScalarType::UInt8>(
                        problem.twoOfFourMetadata->storage(),
                        metadataOffset,
                        static_cast<uint8_t>(
                            previous | static_cast<uint8_t>(nibble << 4)));
                }
            }
            ...
            for (size_t position = 0; position < groupSize; ++position) {
                ...
                if (retained[position]) {
                    detail::copyEncodedElement(problem.input.type(),
                                               problem.input.storage(),
                                               sourceOffset,
                                               problem.pruned.storage(),
                                               prunedOffset);
                } else {
                    detail::zeroEncodedElement(
                        problem.pruned.type(),
                        problem.pruned.storage(),
                        prunedOffset);
                }
    ...
    return {
        ...
        .retainedIndicesWritten =
            problem.retainedIndices ? groups * retainedElements : 0,
        .metadataBytesWritten =
            problem.twoOfFourMetadata
                ? slices * ((plan.groupsPerLine + 1) / 2)
                : 0,
...
inline TwoOfFourMetadataRunInfo encodeTwoOfFourMetadata(
    const TwoOfFourMetadataProblem& problem) {
    ...
    const size_t lineCount =
        problem.retainedIndices.shape().elementCount() /
        problem.retainedIndices.shape()[problem.axis];

    std::vector<size_t> retainedCoordinates(
        problem.retainedIndices.shape().rank(), 0);
    const ptrdiff_t retainedAxisStride =
        problem.retainedIndices.layout().strides()[problem.axis];
    const ptrdiff_t metadataAxisStride =
        problem.metadata.layout().strides()[problem.axis];
    for (size_t line = 0; line < lineCount; ++line) {
        detail::coordinatesForLine(line,
                                   problem.retainedIndices.shape(),
                                   problem.axis,
        ...
        const ptrdiff_t metadataBase =
            problem.metadata.layout().elementOffset(retainedCoordinates);

        for (size_t metadataGroup = 0; metadataGroup < metadataGroups;
             ++metadataGroup) {
            const size_t firstGroup = metadataGroup * 2;
            const ptrdiff_t firstOffset =
                retainedBase +
                static_cast<ptrdiff_t>(firstGroup * 2) * retainedAxisStride;
            const uint8_t first0 = detail::decodeScalarKnown<
                ScalarType::UInt8,
                uint8_t>(problem.retainedIndices.storage(), firstOffset);
            const uint8_t first1 = detail::decodeScalarKnown<
                ScalarType::UInt8,
                uint8_t>(problem.retainedIndices.storage(),
                         firstOffset + retainedAxisStride);
            uint8_t encoded = detail::twoOfFourMetadataNibble(first0, first1);

            if (firstGroup + 1 < sparsityGroups) {
                const ptrdiff_t secondOffset =
                    firstOffset + 2 * retainedAxisStride;
                const uint8_t second0 = detail::decodeScalarKnown<
                    ScalarType::UInt8,
                    uint8_t>(problem.retainedIndices.storage(), secondOffset);
                const uint8_t second1 = detail::decodeScalarKnown<
                    ScalarType::UInt8,
                    uint8_t>(problem.retainedIndices.storage(),
                ...
                encoded = static_cast<uint8_t>(
                    encoded |
                    static_cast<uint8_t>(
                        detail::twoOfFourMetadataNibble(second0, second1) << 4));
            }

            detail::encodeScalarKnown<ScalarType::UInt8>(
                problem.metadata.storage(),
                metadataBase +
                    static_cast<ptrdiff_t>(metadataGroup) *
                        metadataAxisStride,
                encoded);
        }
    }
