...
enum class UlpComparisonMode {
    ...
    RelativeSpacing,
    /// Distance between ordered encodings after conversion to `ulpType`; integer types use
    /// absolute difference, and types without an encoded-distance implementation use
    /// `RelativeSpacing`.
    EncodedDistance,
};
...
/// `strictTolerance` changes only the tolerance test to `difference < tolerance`; exact equality
/// still passes. Opposite signed zeros fail when `equalSignedZero` is false. NaNs pass only when
/// both values are NaN and `equalNaNs` is true. Infinities pass only when they are equal.
///
...
/// - `relativeFrobeniusTolerance`: `||observed - expected||F / ||expected||F` must be less than or
///   equal to the configured tolerance. A zero expected norm produces 0 for a zero difference and
///   infinity otherwise. This criterion requires `computeFrobenius`.
...
struct ComparisonResult {
    ...
    // Frobenius evidence over selected finite values. Matched non-finite values are excluded.
    // A non-finite mismatch makes `frobeniusDifference` and `relativeFrobeniusError` infinite.
    ...
    // Matched NaNs and infinities count as compared components with zero distance.
...
/// Legacy host-validation policy: type-specific symmetric-relative tolerance, strict inequality
