...
int main() {
    ...
    const Tensor runtimeTensor = Tensor::fromNative(std::span<const float>(native));
    const Tensor reversed = runtimeTensor.alias(Layout(Shape{3}, {-1}, 2));
    return tensor.size() == 6 && tensor.loadAs<float>({1, 2}) == 4.0f &&
