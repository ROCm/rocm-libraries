...
void testSeedAndComplexPolicies() {
    ...
    for (size_t index = 0; index < 16; ++index) {
        const double expectedReal =
            indexedUniformInteger(seed, generation_random_domain_version_1::realComponent, index,
                                  parameters.lower, parameters.upper);
        const double expectedImaginary =
            indexedUniformInteger(seed, generation_random_domain_version_1::imaginaryComponent,
                                  index, parameters.lower, parameters.upper);
        ...
        require(realOnlyValue == std::complex<double>(expectedReal, 0.0),
                "Real-only generation did not zero the imaginary component.");
        require(replicatedValue == std::complex<double>(expectedReal, expectedReal),
                "Replicated generation did not use one value for both components.");
        require(cartesianValue == std::complex<double>(expectedReal, expectedImaginary),
    ...
    for (size_t index = 0; index < 16; ++index) {
        const int expected =
            indexedUniformInteger(seed, generation_random_domain_version_1::realComponent, index,
                                  parameters.lower, parameters.upper);
        require(realDestination.loadAs<int32_t>({index}) == expected,
                "Cartesian generation used the imaginary recipe for a real destination.");
    }
}

void testLegacyCompatibility() {
    GenerationOptions numericalLegacy;
    numericalLegacy.seed = 0x1020304050607080ULL;
    numericalLegacy.real.pattern = GenerationPattern::CandidateSet;
    numericalLegacy.real.candidates = {-6.0, -1.5, 0.0, 4.0};
    numericalLegacy.real.transform = GenerationTransform::Absolute;
    numericalLegacy.real.valueScale = 2.0;
    numericalLegacy.real.valueOffset = -1.0;
    numericalLegacy.real.alternatingDimensions = {0};
    numericalLegacy.real.negativeParity = 1;

    const GenerationRecipe numericalTyped = GenerationRecipe::realOnly(
        GenerationRecipe::candidateSet({.values = numericalLegacy.real.candidates})
            .withAbsoluteTransform()
            .withAffineValueMapping({.scale = 2.0, .offset = -1.0})
            .withAlternatingSign({.dimensions = {0}, .negativeWhenOdd = true}),
        {.seed = numericalLegacy.seed});
    requireEqualStorage(generate(ScalarType::Float32, Shape{32}, numericalLegacy),
                        generate(ScalarType::Float32, Shape{32}, numericalTyped),
                        "Typed numerical generation changed legacy-compatible bytes.");

    GenerationOptions complexLegacy;
    complexLegacy.seed = 91;
    complexLegacy.real.pattern = GenerationPattern::UniformInteger;
    complexLegacy.real.parameter0 = -9;
    complexLegacy.real.parameter1 = 9;
    complexLegacy.real.stream = generation_random_domain_version_1::realComponent;
    complexLegacy.imaginary.pattern = GenerationPattern::Normal;
    complexLegacy.imaginary.parameter0 = 1.5;
    complexLegacy.imaginary.parameter1 = 0.25;
    complexLegacy.imaginary.stream = generation_random_domain_version_1::imaginaryComponent;

    const GenerationRecipe complexTyped = GenerationRecipe::cartesian(
        GenerationRecipe::uniformInteger({.lower = -9, .upper = 9}),
        GenerationRecipe::normal({.mean = 1.5, .standardDeviation = 0.25}),
        {.seed = complexLegacy.seed});
    requireEqualStorage(generate(ScalarType::ComplexFloat64, Shape{32}, complexLegacy),
                        generate(ScalarType::ComplexFloat64, Shape{32}, complexTyped),
                        "Typed Cartesian generation changed legacy-compatible bytes.");
    requireEqualStorage(generate(ScalarType::ComplexFloat64, Shape{32}, complexLegacy),
                        generate(ScalarType::ComplexFloat64, Shape{32},
                                 legacyOptionsFromGenerationRecipe(
                                     generationRecipeFromLegacyOptions(complexLegacy))),
                        "Legacy-to-typed-to-legacy conversion changed generated bytes.");
    requireEqualStorage(generate(ScalarType::ComplexFloat64, Shape{32}, complexTyped),
                        generate(ScalarType::ComplexFloat64, Shape{32},
                                 legacyOptionsFromGenerationRecipe(complexTyped)),
                        "Typed-to-legacy conversion changed generated bytes.");
    ...
    GenerationOptions rawLegacy;
    rawLegacy.real.pattern = GenerationPattern::RawConstant;
    rawLegacy.real.parameter0 = -1.0;
    const Tensor rawLegacyValues = generate(ScalarType::UInt64, Shape{2}, rawLegacy);
    const Tensor rawCompatible = generate(ScalarType::UInt64, Shape{2},
                                          GenerationRecipe::realOnly(GenerationRecipe::rawConstant(
                                              {.bits = std::numeric_limits<uint64_t>::max()})));
    requireEqualStorage(rawLegacyValues, rawCompatible,
                        "Typed raw generation changed representable legacy bytes.");
}
...
void testValidationFailures() {
    ...
    requireThrows<std::invalid_argument>(
        [] {
            (void)legacyOptionsFromGenerationRecipe(GenerationRecipe::realOnly(
                GenerationRecipe::rawConstant({.bits = 0x0020000000000001ULL})));
        },
        "An inexact typed raw constant was converted to the legacy double field.");
...
int main() {
    try {
        static_assert(counterRandomAlgorithmVersion == 1);
