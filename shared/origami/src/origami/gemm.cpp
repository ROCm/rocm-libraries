...
double calculate_work_utilization(size_t M, size_t N, size_t K,
...
inline const char* rtype_to_string(streamk::reduction_type r) {
    switch (r) {
        case streamk::reduction_type::Tree:     return "Tree";
        case streamk::reduction_type::Parallel: return "Parallel";
        case streamk::reduction_type::None:     return "None";
...
std::tuple<size_t, size_t, size_t> compute_CU_occupancy(
    const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch, bool transA,
    bool transB, size_t MT_M, size_t MT_N, size_t MT_K, size_t MI_M, size_t MI_N, size_t MI_K,
    size_t element_size_A, size_t element_size_B, size_t element_size_out, data_type_t mi_datatype,
    int WGM, size_t workspace_size, size_t workspace_size_per_elem_c, int occupancy,
    int dynamic_grid_version, bool debug, size_t split) {
    ...
    {
        streamk::reduction_type rt = streamk::select_reduction(M, N, K, batch, MT_M, MT_N, MT_K,
                                                               hardware, dynamic_grid_version);
        numWGs = streamk::select_grid(
            M, N, K, batch, transA, transB, element_size_A, element_size_B, element_size_out,
            mi_datatype, workspace_size, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, WGM,
            workspace_size_per_elem_c, occupancy, hardware, dynamic_grid_version, rt);
...
static inline double compute_cvt_overhead(const hardware_t& hardware, size_t MT_M, size_t MT_N,
                                          size_t MT_K, size_t MI_M, size_t MI_N, size_t MI_K,
...
size_t compute_mt_compute_latency(const hardware_t& hardware, size_t M, size_t N, size_t K,
    ...
    //     either contiguous
    //     //dimesion from LDS then we will get poor LDS utilization. This actually happens as more
    //     like
    //     //a quantization effect where if either contiguous dimension of the tile is not evenly
    ...
    //     if(MT_K * safe_ceil_div(element_size_A, 8) % 128 != 0)
    //     {
    //         L_MT = L_MT * lds128_penalty(MT_K,element_size_A);
    //     }
 ...
 */
double estimate_l2_hit(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                       size_t MT_M, size_t MT_N, size_t MT_K, size_t element_size, int WGM,
                       size_t splittingFactor,bool debug) {
    ...
    // Concurrently executing workgroups are limited by the number of CUs.
    const size_t concurrent_workgroups = std::min(total_workgroups, hardware.N_CU);
    if (concurrent_workgroups == 0) return 0.0;
...
double estimate_mall_hit(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                         size_t MT_M, size_t MT_N, size_t MT_K,
                         size_t element_size,  // CRITICAL: Added parameter
                         int WGM, size_t numActiveCUs, size_t splittingFactor) {
    const size_t workgroups_m = safe_ceil_div(M, MT_M);
    const size_t workgroups_n = safe_ceil_div(N, MT_N);

    if (numActiveCUs == 0) return 0.0;
    ...
    if (hardware_t::is_debug_enabled()) {
        ...
        hardware.log_debug("MallFootprint_Bytes", calculate_footprint(mall_tile_m, mall_tile_n));
    }
 ...
 */
double compute_l2_hit_rate_global(size_t M, size_t N, size_t MT_M, size_t MT_N, size_t MT_K,
                                  size_t element_size, size_t l2_capacity_bytes) {
    ...
    if (grid_m == 0 || grid_n == 0) return 0.0;
...
double compute_memory_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, bool transA,
                              ...
                              size_t element_size_A, size_t element_size_B, size_t mx_block_size,
                              int WGM, size_t numActiveCUs, size_t splittingFactor, bool debug) {
    // 1) Estimate L2 hit-rate
    double H_mem1 = estimate_l2_hit(hardware, M, N, K, batch, MT_M, MT_N, MT_K, element_size_A, WGM,
                                    splittingFactor,debug);

    double H_mem1_global = compute_l2_hit_rate_global(M, N, MT_M, MT_N, MT_K, element_size_A,
                                                      hardware.L2_capacity * 1024);
    ...
    double H_mem2 = estimate_mall_hit(hardware, M, N, K, batch, MT_M, MT_N, MT_K, element_size_A,
                                      WGM, numActiveCUs, splittingFactor);
    ...
    //     if ((MT_K * safe_ceil_div(element_size_A, 8)) % (128) != 0) {
    //         L_mem = L_mem * lds128_penalty(MT_K, element_size_A);
    //     }
    // }
    ...
    //     // quantify the true impact of this Effect on the efficiency of computation.
    //     if ((MT_M * safe_ceil_div(element_size_A, 8)) % (128) != 0) {
    //         L_mem = L_mem * lds128_penalty(MT_M, element_size_A);
    //     }

    //     if ((MT_N * safe_ceil_div(element_size_B, 8)) % (128) != 0) {
    //         L_mem = L_mem * lds128_penalty(MT_N, element_size_A);
    //     }
    ...
    //     if (MT_M * safe_ceil_div(element_size_B, 8) % 128 != 0) {
    //         L_mem = L_mem * lds128_penalty(MT_M, element_size_A);
    //     }
    ...
    if (debug || hardware_t::is_debug_enabled()) {
        ...
        hardware.log_debug("MT_N % 128 + MT_M % 128 bytes", (MT_M * safe_ceil_div(element_size_A, 8) % 128) + MT_N * safe_ceil_div(element_size_B, 8) % 128);
        hardware.log_debug("MT_N % 64 + MT_M % 64 bytes", (MT_M * safe_ceil_div(element_size_A, 8) % 64) + MT_N * safe_ceil_div(element_size_B, 8) % 64);
 ...
 */
double compute_lds_latency_cycles(size_t K, size_t MT_M, size_t MT_N, size_t MT_K,
                                  size_t element_size_A, size_t element_size_B,
    ...
    const double effective_lds_bw_gbps = LDS_BANDWIDTH_THEORETICAL_GBPS * LDS_EFFECTIVE_BW_PERCENT;
    ...
    const double total_bytes_read_from_lds = num_k_slices * (bytes_A_per_slice + bytes_B_per_slice);
...
double compute_tile_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                            ...
                            size_t mx_block_size, int WGM, size_t occupancy, size_t numActiveCUs,
                            size_t splittingFactor, bool debug) {
    ...
    double L_mem = compute_memory_latency(hardware, M, N, K, transA, transB, batch, MT_M, MT_N,
                                          MT_K, element_size_A, element_size_B, mx_block_size, WGM,
                                          numActiveCUs, splittingFactor, debug);

    double L_LDS = compute_lds_latency_cycles(K, MT_M, MT_N, MT_K, element_size_A, element_size_B,
                                              splittingFactor);
    ...
    size_t MT_M_rounded_128bytes = round_elements_to_128B(MT_M,element_size_A);
    size_t MT_M_rounded_128bytes = round_elements_to_128B(MT_N,element_size_A);

    double L_epilogue = (static_cast<double>(numActiveCUs / splittingFactor) * MT_M_rounded_128bytes * MT_N *
    ...
    if (splittingFactor > 1) {
        ...
        double partial_read_bytes = n_partials * MT_M_rounded_128bytes * MT_N * safe_ceil_div(element_size_out, 8);
    ...
    if (tf32_emu) {
        L_cvt = compute_cvt_overhead(hardware, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, element_size_A,
                                     element_size_B, debug);
    }
    ...
    double effective_tile_penalty = (utilization > 1e-9) ? (1.0 / (utilization * 2)) : 1.0;
...
double compute_wave_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
    ...
    double L_wave =
        compute_tile_latency(hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N,
                             MI_K, element_size_A, element_size_B, element_size_out, mi_datatype,
                             mx_block_size, WGM, occupancy, numActiveCUs, splittingFactor, debug);
...
double compute_total_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                             ...
                             data_type_t mi_datatype, size_t mx_block_size, int WGM,
                             size_t non_temporal_a, size_t non_temporal_b, size_t occupancy,
                             size_t split, bool debug) {
    if (debug || hardware_t::is_debug_enabled()) {
    ...
    auto [numActiveCUs, numWaves, splittingFactor] = compute_CU_occupancy(
        hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K,
        element_size_A, element_size_B, element_size_out, mi_datatype, WGM,
    ...
    double L_wave =
        compute_wave_latency(hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N,
                             MI_K, element_size_A, element_size_B, element_size_out, mi_datatype,
                             mx_block_size, WGM, occupancy, numActiveCUs, splittingFactor, debug);
    ...
    if (heuristics && mixed_medium_gemms) {
        ...
        if (!transA && transB && MT_M == 128 && MT_N == 64 && MT_K == 128 &&
            grid_m * grid_n > hardware.N_CU) {
            total_latency = total_latency * 2;
        }
        if (!transA && transB && MT_M == 64 && MT_N == 64 && MT_K == 256 &&
            grid_m * grid_n > hardware.N_CU) {
            total_latency = total_latency * 2;
        }
        ...
        if (!transA && !transB && MT_M == 64 && MT_N == 128 && MT_K == 64 &&
            grid_m * grid_n > hardware.N_CU) {
            total_latency = total_latency * 2;
        }

        if (!transA && !transB && MT_M == 64 && MT_N == 128 && MT_K == 32 &&
            grid_m * grid_n > hardware.N_CU) {
            total_latency = total_latency * 2;
        }
    ...
    if (heuristics && largek_panel_panel) {
        ...
        long long tilesM = (M + MT_M - 1) / MT_M;  // ceil(M / MT_M)
        long long tilesN = (N + MT_N - 1) / MT_N;  // ceil(N / MT_N)
        long long launched = tilesM * tilesN * MT_M * MT_N;
        long long actual = M * N;

        if (launched == 0) return 0.0;  // avoid divide-by-zero

        long long waste = launched - actual;
        double waste_pct = static_cast<double>(waste) / static_cast<double>(launched);

        total_latency = total_latency / (1.0 - waste_pct);
    }
    ...
    if (heuristics && largen_block_panel) {
        ...
        if (transA && !transB) {
            if (MT_M == 16 && MT_N == 288 && MT_K == 32) {
                total_latency = total_latency * 4;
            }
        }
    }

    if (heuristics && !tf32_emu && !largek_panel_panel && !largen_block_panel &&
        !mixed_medium_gemms) {
    ...
    if (debug || hardware_t::is_debug_enabled()) {
        ...
        hardware.log_debug("Peak theoretical GFLOPs based on occupancy",1300 * occupancy_percent);
        if (hardware_t::is_debug_enabled()) {
...
double compute_perf_gflops(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
    ...
    double latency_cycles =
        compute_total_latency(hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M,
                              MI_N, MI_K, element_size_A, element_size_B, element_size_out,
                              mi_datatype, mx_block_size, WGM, 0, 0, debug);
