...
double calculate_work_utilization(size_t M, size_t N, size_t K, size_t MT_M, size_t MT_N,
                                  size_t MT_K) {
...
double calculate_output_utilization(size_t M, size_t N, size_t MT_M, size_t MT_N,
...
std::tuple<size_t, size_t, size_t> compute_CU_occupancy(
    ...
    int dynamic_grid_version, size_t split) {
...
static inline double compute_cvt_overhead_x1(const hardware_t& hardware, size_t MT_M, size_t MT_N,
                                             size_t MT_K, size_t MI_M, size_t MI_N, size_t MI_K,
...
size_t compute_mt_compute_latency(const hardware_t& hardware, size_t M, size_t N, size_t K,
                                  ...
                                  size_t element_size_B, data_type_t mi_datatype) {
...
double estimate_l2_hit(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
    ...
    const size_t cu_per_xcd =
        std::max(safe_ceil_div(effective_cus, hardware.NUM_XCD), static_cast<size_t>(1));
...
double estimate_mall_hit(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
                         size_t MT_M, size_t MT_N, size_t MT_K, size_t element_size, int WGM,
                         size_t numActiveCUs, size_t splittingFactor) {
    const size_t workgroups_m = safe_ceil_div(M, MT_M);
    const size_t workgroups_n = safe_ceil_div(N, MT_N);

    if (numActiveCUs == 0) throw std::runtime_error("Number of Active CUs was 0");
...
double compute_tile_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
    ...
    double L_compute =
        compute_mt_compute_latency(hardware, M, N, K, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N,
                                   MI_K, element_size_A, element_size_B, mi_datatype);
    ...
    int real_occupancy =
        std::min(occupancy, safe_ceil_div(grid_m * grid_n * batch * splittingFactor,
    ...
    if (splittingFactor > 1) {
        ...
        double partial_write_bytes =
            grid_m * grid_n * MT_M_rounded_128bytes * MT_N * safe_ceil_div(element_size_out, 8);
    ...
    {
        L_cvt = compute_cvt_overhead_x1(hardware, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K,
                                        element_size_A, element_size_B, mi_datatype);
    }
    ...
    if (K % MT_K != 0) {
        ...
        L_epilogue += problem_k_quant * 50000;  // Scale by remainder proportion of problem. 50k
                                                // cycle penalty if have to zero pad all except 1.
...
double compute_wave_latency(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
    ...
    double L_wave =
        compute_tile_latency(hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N,
                             ...
                             mx_block_size, WGM, occupancy, numActiveCUs, splittingFactor);
...
double compute_perf_gflops(const hardware_t& hardware, size_t M, size_t N, size_t K, size_t batch,
    ...
    double latency_cycles = compute_total_latency(
        hardware, M, N, K, batch, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K,
        element_size_A, element_size_B, element_size_out, mi_datatype, mx_block_size, WGM, 0, 0, 1);
