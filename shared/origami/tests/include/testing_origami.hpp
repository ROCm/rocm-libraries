...
void ComputeLoads(
    int MT_M, int MT_N, int MT_K, const std::optional<int> expected)
{
...
{
    auto NumberMatrixInstructions = origami::compute_number_matrix_instructions(
...
void ComputeMTComputeLatency(const origami::hardware_t& hardware,
                                size_t          M,
                                          ...
                                          const std::optional<int>                 expected_gt)
{
    auto latency = origami::compute_mt_compute_latency(
        hardware, M, N, K, transA, transB, MT_M, MT_N, MT_K, MI_M, MI_N, MI_K, element_size_A, element_size_B, origami::data_type_t::BFloat16);
                                      ...
                                      int             splittingFactor)
{
    auto mem_latency_small = origami::compute_memory_latency(
        hardware, M, N, K, transA, transB, batch, MT_M, MT_N, MT_K, element_size_A, element_size_B, mx_block_size, wgm, numActiveCUs, splittingFactor);

    auto mem_latency_large = origami::compute_memory_latency(
        hardware, M, N, K, transA, transB, batch, MT_M * 2, MT_N * 2, MT_K * 2, element_size_A, element_size_B, mx_block_size, wgm, numActiveCUs, splittingFactor);
...
void CheckLDSCapacity(const origami::hardware_t& hardware,
                                 int             MT_M,
                                 int             MT_N,
                                 int             MT_K,
                                 size_t          element_size)
{
    auto fit_lds_memory
        = origami::check_lds_capacity(hardware, MT_M, MT_N, MT_K, element_size);
...
{
    auto tflops = origami::compute_tflops_from_latency(
        latency_cycles, M, N, K, clock_GHz);
