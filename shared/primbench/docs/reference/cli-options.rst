...
.. note::
   ...
   The full option table below is reconstructed from the ``settings`` struct
   fields declared in the public header. The README describes suffix support
   for ``--size`` (``KiB``, ``MiB``, ``GiB``) and documents every option
   listed here; however, the complete prose descriptions for each option from
   the README were not available in the source material provided. The
   descriptions below are derived from the Doxygen comments on the
   corresponding ``settings`` fields in ``primbench.hpp``.
...
.. list-table::
   ...
   * - ``--batch-window-size``
     - ``size_t`` / ``10``
     - Number of recent batches used in the noise-reduction sliding window for early stopping.
