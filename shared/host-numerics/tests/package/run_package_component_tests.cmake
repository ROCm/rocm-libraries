...
function(_install_project build_dir install_dir label)
    ...
    set(_private_include_dir
        "${install_dir}/include/roc/host_numerics/detail"
    )
...
function(
    ...
    set(
        ...
        "-DROCHostNumerics_TEST_COMPONENT=${component}"
        "-DROCHostNumerics_TEST_EXPECTED_COMPONENTS=${expected_components}"
        ${ARGN}
    )
    execute_process(
...
function(
    ...
    set(
        ...
        "-DCMAKE_PREFIX_PATH=${install_dir}"
        "-DROCHostNumerics_TEST_COMPONENT=${component}"
        "-DROCHostNumerics_TEST_EXPECTED_COMPONENTS=${component}"
        ${ARGN}
    )
    execute_process(
        COMMAND ${_configure_command}
...
# Configure, build, and execute the default component-less package consumer.
function(_run_default_package install_dir case_name)
    set(_build_dir "${PACKAGE_WORK_DIR}/${case_name}")
    file(REMOVE_RECURSE "${_build_dir}")
    execute_process(
        COMMAND
...
_build_project("${COMPONENT_BINARY_DIR}" "ROCHostNumerics producer")
_install_project(
    "${COMPONENT_BINARY_DIR}"
    "${_full_install_dir}"
    "ROCHostNumerics producer"
)
...
_run_default_package("${_full_install_dir}" full-default)
_run_component(
    "${_full_install_dir}"
    full-amd-gpu-layout
    AMDGPULayout
...
file(
    GLOB_RECURSE
    _blas_target_files
    "${_full_install_dir}/*ROCHostNumericsBLASTargets.cmake"
)
...
file(
    ...
    "${_full_install_dir}/*ROCHostNumericsOperationsTargets.cmake"
)
...
set(
    ...
    ${_generator_arguments}
    ${_configure_cache_arguments}
    "-DROCHostNumerics_SOURCE_DIR=${COMPONENT_SOURCE_DIR}"
...
file(
    ...
    "${_source_install_dir}/*ROCHostNumericsBLASTargets.cmake"
)
...
_run_default_package("${_source_install_dir}" source-default)

# Configure a minimal producer. MX remains part of Operations; BLAS remains the
# intentionally optional component.
...
set(
    ...
    ${_generator_arguments}
    ${_configure_cache_arguments}
    -DHOST_NUMERICS_BUILD_TESTING=OFF
    -DHOST_NUMERICS_BUILD_PYTHON=OFF
    -DHOST_NUMERICS_BUILD_BLAS_BACKEND=OFF
)
execute_process(
...
_build_project("${_minimal_build_dir}" "Minimal ROCHostNumerics producer")
_install_project(
    "${_minimal_build_dir}"
    "${_minimal_install_dir}"
    "Minimal ROCHostNumerics producer"
)
_run_default_package("${_minimal_install_dir}" minimal-default)
_expect_component_failure(
    "${_minimal_install_dir}"
...
message(STATUS "ROCHostNumerics package component tests passed.")
