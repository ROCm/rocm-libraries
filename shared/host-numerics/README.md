...
`host-numerics` owns product-independent CPU generation, reference
...
- `roc::host-numerics-core`
  ...
  - Its primary entry points are `roc/host_numerics/scalar.hpp` and
    `roc/host_numerics/tensor.hpp`; `index_order.hpp` is a shared enum and
...
- `roc::host-numerics-amd-gpu-layout`
  ...
  - Exports `roc/host_numerics/amd_gpu_layout/mx.hpp`.
  ...
  - Uses private optional OpenMP parallelism, disabled with
    `HOST_NUMERICS_AMD_GPU_LAYOUT_ENABLE_OPENMP=OFF`. Large transforms use at
...
- `roc::host-numerics-blas`
  ...
  - Built with `HOST_NUMERICS_BUILD_BLAS_BACKEND=ON`.
...
Every file and target owned by `shared/host-numerics` is product-independent.
...
```text
private product adapter -> roc::host-numerics -> roc::host-numerics-core
private product adapter -> roc::host-numerics-amd-gpu-layout
```

The dependency may never point in the opposite direction. The
`host-numerics-component-boundary` test scans the complete component source
...
Tensor::deepCopy();
Tensor::copyLogicalElementsToEncodedStorage(destination);
Tensor::copySelectedElementsToEncodedStorage(destination, linearIndices, indexOrder);
...
Tensor::copyConvertedTo(ScalarType, Layout);
Tensor::copyConvertedTo(ScalarType, Layout, ScalarConversionOptions);
```
...
`allocateUninitialized()` is limited to byte-addressable scalar types because
packed element stores must preserve neighboring bits in the same byte.
...
complete backing range. `copyLogicalElementsFrom()` and the encoded-storage copy-out
operations transfer only logical coordinates and preserve layout gaps and unrelated
packed boundary bits. Overlapping source and destination storage is staged through an
independent copy. Selected copy-out requires an explicit logical index order.
...
preserving shape, strides, and offset. Destination-layout overloads preserve the
logical shape and, when changing layout, require provably non-overlapping element
offsets. Same-type
conversion preserves exact logical encodings; cross-type conversion decodes and
re-encodes logical values through the core scalar codecs. Option-bearing overloads make
...
distribution terms rather than numbered fields. Recipe settings carry the
seed, logical index order, and an optional identifier for an independent
...
generateAt(output, logicalIndex, recipe);
```
...
effective random-stream identifier, and logical index. Loop order and thread
count do not affect the result.

Product adapters that need stable, independent seed sequences use
`deriveDeterministicSeed(baseSeed, streamIdentifier, sequenceIndex)`. The
counter algorithm and distribution primitives remain private implementation
details.
...
required. AXPBY and GEMM coefficients use the same runtime-typed `Scalar`
representation; assigning a native arithmetic value constructs the corresponding
`Scalar` value.
...
- relative maximum-norm evidence, strict threshold selection, and explicit
  zero-denominator/non-finite norm policy;
...
request.epilogue.alpha = std::complex<double>{1.0, 0.0};
request.epilogue.beta = std::complex<double>{0.0, 0.0};
request.epilogue.scaleC = std::complex<double>{1.0, 0.0};

GemmBackend backend = GemmBackend::Automatic;

GemmSupportInfo support = queryGemmSupport(request, backend);
GemmRunInfo run = referenceGemm(request, backend);
```
...
GemmResult owned = referenceGemm(problem, output, backend);
```
...
Destination logical elements must have distinct storage offsets. D cannot
overlap A, B, bias, or scale tensors; exact C=D aliasing with the same type and
layout is supported, while differently mapped C/D overlap is rejected.
The `GemmBackend` argument selects execution policy, while an optional backend
implementation object makes BLAS available without becoming part of the
numerical request. Automatic execution evaluates that BLAS implementation,
the built-in Blocked strategy, and finally Pointwise. Support and cost policy
live in this component; product adapters do not select by tensor dimensions,
strides, or output density. Automatic is the only policy that falls back;
explicit Pointwise, Blocked, and BLAS requests either use that implementation or
report that it is unsupported. `GemmBackend::Mixed` is reporting-only and
describes an aggregate whose independent requests used different concrete
implementations.
...
- alpha/beta and an explicit scalar C scale;
- explicit row- or column-axis bias and scale-alpha bindings;
- row scale-A and column scale-B, with length-one tensors selecting scalar broadcast;
- independently optional tensor-backed A/B block scales with independent block sizes;
...
- automatic Pointwise/Blocked execution, an optional BLAS implementation,
...
The optional BLAS component exposes `queryGemmSupportWithBlasBackend` and
`referenceGemmWithBlasBackend` for dense F32/F64/complex GEMM. It sends directly
compatible requests to CBLAS without staging. For runtime-typed, scaled, or
compute-input-quantized operands, it materializes accumulator-typed scratch and
performs component-owned output scaling/conversion. This preserves accelerated
large mixed-type references without placing conversion loops in product
adapters. Under Automatic policy, block-scaled requests fall back to Pointwise
because folding the scales into individual operands would not preserve the
requested reduction-block scale boundaries.
Automatic execution always prefers directly consumable BLAS requests. A
transformed request must meet conservative operation-count and arithmetic-
intensity gates so skinny matrices do not spend more time staging than
computing.
...
ABI. Because the backend is a static archive, an installed consumer may
legitimately resolve a different conforming provider.

The BLAS conformance executable is registered in separate one-thread and
multi-thread CTest processes. It uses only the host-numerics API and common
...
when both block sizes and K align to its eight-element reduction tile. Each
scale is applied after its complete logical reduction segment, even when that
segment spans multiple backend tiles. The strategy preserves partial-output
...
The Operations target generates packed data, natural-layout scales, explicit
per-element scale indices, and the decoded F32 reference tensor:

```cpp
MxDataGeneration data = MxDataGeneration::preserveRange(
    GenerationRecipe::realOnly(
        GenerationRecipe::uniformReal({.lower = -1.0, .upper = 1.0}), {.seed = 17}),
    {.lower = -1.0, .upper = 1.0});
MxGenerationProblem problem(Shape{64, 128}, std::move(data));
...
`MxGenerationProblem` requires its rank-two shape and data-generation policy at
construction, so there is no invalid default problem or implicit random recipe.

MX generation composes the same immutable `GenerationRecipe` as ordinary
tensor generation with a small `MxDataGeneration` quantization policy. It
selects nearest quantization, range-preserving quantization, or preservation of
generated encoded values. `MxScaleGenerationMode`
independently selects derived, random finite, minimum/zero, one, two, maximum,
or NaN scales.
...
selection and swizzling are intentionally outside this target. The optional
`AMDGPULayout` component exposes one scale-layout conversion entry point,
`copyMxScaleStorageToPhysicalLayout`, whose `MxScaleStorageLayout` argument
selects natural, GFX950, or GFX1250 storage. Architecture-specific conversion
helpers remain implementation details. The `scaleIndices` tensor makes the
blocked-axis mapping explicit and lets NumPy independently verify
...
Standalone epilogue scales and activation parameters also use `Scalar`, matching
the fused GEMM epilogue and keeping scalar decoding and conversion in one path.
...
- component-owned parallel scheduling for independent output slices.
...
axis. The operation parallelizes provably independent output slices and uses a
deterministic serial traversal for overlapping layouts. Product adapters make
one operation call; pruning, compression, random selection, metadata arithmetic,
and scheduling remain in the shared component. The sparsity operation has no HIP
or AMDGPU dependency; MX generation uses the bounded OpenMP
...
```cpp
#include <roc/host_numerics/tensor.hpp>      // stable tensor core
#include <roc/host_numerics/comparison.hpp>  // host comparison plan/report
#include <roc/host_numerics/validation.hpp>  // all validation operations
```
...
target_link_libraries(app PRIVATE roc::host-numerics)
```

With no explicit components, the package loads `Operations` and its `Core`
dependency. The installed package also accepts explicit `Core`, `Operations`,
`BLAS`, and `AMDGPULayout` components. Blocked GEMM and MX generation are part
of `Operations`; `BLAS` also requires `Operations`, while `Operations` requires
`Core`. An explicit `Core`-only lookup does not search for CBLAS or OpenMP. The
`BLAS` component locates a conforming
...
cmake -S shared/host-numerics \
  -B build/host-numerics-python \
  -DHOST_NUMERICS_BUILD_TESTING=ON \
  -DHOST_NUMERICS_BUILD_PYTHON=ON
cmake --build build/host-numerics-python
ctest --test-dir build/host-numerics-python --output-on-failure
```

The `roc_host_numerics` package currently provides:
...
```bash
cmake -S shared/host-numerics -B build/host-numerics \
  -DHOST_NUMERICS_BUILD_TESTING=ON
cmake --build build/host-numerics
ctest --test-dir build/host-numerics --output-on-failure
```
...
```bash
cmake -S shared/host-numerics -B build/host-numerics-benchmark \
  -DHOST_NUMERICS_BUILD_TESTING=ON \
  -DHOST_NUMERICS_BUILD_PYTHON=OFF \
  -DHOST_NUMERICS_BUILD_BENCHMARKS=ON \
  -DCMAKE_BUILD_TYPE=Release
cmake --build build/host-numerics-benchmark
OMP_NUM_THREADS=24 \
  build/host-numerics-benchmark/tests/host-numerics-structured-sparsity-benchmark \
  2048 4096 7
```
