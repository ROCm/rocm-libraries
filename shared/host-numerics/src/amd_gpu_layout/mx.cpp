...
GFX950ScalePlan makeGFX950ScalePlan(size_t inputElementCount, const std::vector<size_t>& sizes) {
    ...
    const size_t inputElements =
        checkedMultiply(plan.rowCount, plan.columnCount, "gfx950 MX scale layout");
    ...
    plan.outputElementCount =
        checkedMultiply(plan.paddedRowCount, plan.paddedColumnCount, "gfx950 MX scale layout");
    plan.shuffle.sizes = {plan.paddedRowCount / 32, 2, 16, plan.paddedColumnCount / 8, 2, 4};
    plan.shuffle.sourceStrides = {
        checkedMultiply(32, plan.paddedColumnCount, "gfx950 MX scale layout strides"),
        checkedMultiply(16, plan.paddedColumnCount, "gfx950 MX scale layout strides"),
...
GFX1250ScalePlan makeGFX1250ScalePlan(size_t inputElementCount, size_t slowDimension,
    ...
    const size_t expectedInput =
        checkedMultiply(slowDimension, fastDimension, "gfx1250 MX scale layout input");
    ...
    plan.outputElementCount =
        checkedMultiply(slowDimension, paddedFastDimension, "gfx1250 MX scale layout output");
    return plan;
}
...
static size_t copyGfx950ScaleStorageBytes(const std::byte* input, size_t inputElementCount,
...
static size_t copyGfx1250ScaleStorageBytes(const std::byte* input, size_t inputElementCount,
                                           size_t elementSize, size_t slowDimension,
                                           size_t fastDimension, size_t mxBlock,
                                           std::byte* output) {
