...
inline void validateAxpbyRequest(const AxpbyRequest& request) {
    ...
    requireProvablyDistinctDestinationElementOffsets(request.output, "Reference AXPBY", "output");
    if (request.x)
        rejectOverlappingTensorStorageUnlessIdenticallyMapped(
            request.output, *request.x,
            "Reference AXPBY output overlaps X with a different storage mapping.");
    if (request.y)
        rejectOverlappingTensorStorageUnlessIdenticallyMapped(
            request.output, *request.y,
            "Reference AXPBY output overlaps Y with a different storage mapping.");
}
