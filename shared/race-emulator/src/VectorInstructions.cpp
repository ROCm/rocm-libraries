...
public:
  ...
  std::function<int()> getExecutor(Wave &wave,
                                   std::string_view line) const final {
    auto partitioned = getPartitioned(line);
    assert(partitioned.size() >= 4 && "VOPC requires Dst, Src0, Src1");
...
static Register<VOP1_VConvert<uint32_t, float>> v_cvt_u_f("v_cvt_u32_f32");
...
static RegisterVOP1<float> v_rcp_f32("v_rcp_iflag_f32",
                                     [](float a) { return 1.0f / a; });
