...
TEST(GpuEmulatorUnitTests, MemoryRoundTrip) {
  Wave regs(/*vgprCount*/ 10, /*sgprCount*/ 10, /*waveSize*/ 1);
  regs.enableRaceChecks(true);
...
TEST(GpuEmulatorUnitTests, GlobalLoadStore_WithInstructionOffsets) {
...
TEST(GpuEmulatorUnitTests, DS_Write_B32_Direct) {
  ...
  Wave regs(4, 4, 0, 1, id, &lds, &labels, &macros); // 4 VGPRs, 4 SGPRs

  regs.setDsPreserve(true);
  ...
  uint32_t actual = 0;
  actual = lds.readWithoutChecks<uint32_t>(116); // Clear first

  EXPECT_EQ(actual, 0xCAFEBABE);
}

TEST(GpuEmulatorUnitTests, DS_Write_AllVariants_Combined) {
  // 1. Setup (One-time cost)
  LDS lds;
  lds.resize(1024); // Init with "unset" pattern
  int id = 0;

  std::map<std::string, int> labels;
  std::map<std::string, Macro> macros;
  Wave regs(16, 16, 0, 1, id, &lds, &labels,
  ...
  // Logic: Write 0xDEADBEEF. Should only write 0xEF (low byte).
  {
    ...
    // Verify written byte
    uint8_t actual = 0;
    actual = lds.readWithoutChecks<uint8_t>(10);
    // std::memcpy(&actual, lds.getBase() + 10, 1);
    ...
    // byte pattern)
    uint8_t neighbor = 0;
    neighbor = lds.readWithoutChecks<uint8_t>(11);
    // std::memcpy(&neighbor, lds.getBase() + 11, 1);
  ...
  // Logic: Write 0xCAFEBABE. Should write 0xBABE (low 16 bits).
  {
    ...
    uint16_t actual = 0;
    actual = lds.readWithoutChecks<uint16_t>(20);
    // std::memcpy(&actual, lds.getBase() + 20, 2);
    EXPECT_EQ(actual, 0xBABE);

    // Verify it didn't write 4 bytes
    uint16_t upperPart = 0;
    upperPart = lds.readWithoutChecks<uint16_t>(22);
    // std::memcpy(&upperPart, lds.getBase() + 22, 2);
  ...
  // Logic: Write v[4:5] (0xA..A, 0xB..B) to Address 32.
  {
    ...
    val1 = lds.readWithoutChecks<uint32_t>(32);
    val2 = lds.readWithoutChecks<uint32_t>(36);
    // std::memcpy(&val1, lds.getBase() + 32, 4);
    // std::memcpy(&val2, lds.getBase() + 36, 4);
  ...
  // Logic: Write v[8:11] to Address 64.
  {
    ...
    for (int i = 0; i < 4; ++i) {
      uint32_t val = 0;
      val = lds.readWithoutChecks<uint32_t>(64 + (i * 4));
      // std::memcpy(&val, lds.getBase() + 64 + (i*4), 4);
      EXPECT_EQ(val, 0x10101010 * (i + 1));
    }
  }
}

TEST(GpuEmulatorUnitTests, DS_Read_Variants) {
  LDS lds;
  lds.resize(1024);
  int id = 0;
  std::map<std::string, int> labels;
  std::map<std::string, Macro> macros;
  Wave regs(10, 10, 0, 1, id, &lds, &labels, &macros);

  // Setup Memory
  // Addr 10: 0xFF (Represents -1 in i8, or 255 in u8)
  uint8_t valByte = 0xFF;
  lds.writeWithoutChecks<uint8_t>(10, valByte);
  // std::memcpy(lds.getBase() + 10, &valByte, 1);

  // Addr 20: 0xDEADBEEF (for b32 test)
  uint32_t val32 = 0xDEADBEEF;
  lds.writeWithoutChecks<uint32_t>(20, val32);
  // std::memcpy(lds.getBase() + 20, &val32, 4);
...
TEST(GpuEmulatorUnitTests, BufferLoad_Offen_Success) {
  Wave regs(/*vgprCount*/ 64, /*sgprCount*/ 32, /*waveSize*/ 1);
...
TEST(GpuEmulatorUnitTests, BufferLoad_Offen_Extended) {
  Wave regs(/*vgprCount*/ 64, /*sgprCount*/ 32, /*waveSize*/ 1);
...
TEST(GpuEmulatorUnitTests, BufferStore_Offen_Extended) {
  Wave regs(/*vgprCount*/ 64, /*sgprCount*/ 32, /*waveSize*/ 1);
...
TEST(GpuEmulatorUnitTests, BufferLoad_NegativeOffsets) {
  Wave regs(10, 32, 64);
...
TEST(GpuEmulatorUnitTests, DS_Write_B8_D16_HI) {
  LDS lds;
  lds.resize(1024);
  int id = 0;
  std::map<std::string, int> labels;
  std::map<std::string, Macro> macros;
  Wave regs(16, 16, 0, 1, id, &lds, &labels, &macros);
  ...
  // Bits [07:00] = 0x44 (Low byte of Low word)
  {
    ...
    uint8_t actual = lds.readWithoutChecks<uint8_t>(300);
...
TEST(GpuEmulatorUnitTests, DS_Write_Extended_Features) {
  // Setup
  LDS lds;
  lds.resize(1024);
  int id = 0;
  ...
  // d16_hi should write 0xAABB (bits [31:16]).
  {
    ...
    uint16_t actual = lds.readWithoutChecks<uint16_t>(100);
  ...
  // Logic: Write to Address 200 with offset:16 -> Effective Address 216
  {
    ...
    uint32_t baseVal = lds.readWithoutChecks<uint32_t>(200);
    // Assuming memory was zeroed or didn't contain this pattern
    EXPECT_NE(baseVal, 0x12345678);

    // Check Effective Address (200 + 16 = 216)
    uint32_t offsetVal = lds.readWithoutChecks<uint32_t>(216);
    EXPECT_EQ(offsetVal, 0x12345678);
  }
}

TEST(GpuEmulatorUnitTests, DS_Read_AllVariants_Extended) {
  LDS lds;
  lds.resize(1024);
  int id = 0;
  std::map<std::string, int> labels;
