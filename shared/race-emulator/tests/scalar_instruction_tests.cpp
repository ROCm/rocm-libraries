...
#define TEST_SCALAR_BINARY_OP(TEST_NAME, ASM_OP, ...)                          \
  TEST(GpuEmulatorTestsPartTwo, TEST_NAME) {                                   \
    Wave regs(8, 8, 8);                                                        \
...
#define TEST_SCALAR_CMP(TEST_NAME, ASM_OP, ...)                                \
  TEST(GpuEmulatorTests, TEST_NAME) {                                          \
    Wave regs(8, 8, 8);                                                        \
    struct Case {                                                              \
      uint32_t s0;                                                             \
      uint32_t s1;                                                             \
...
TEST(GpuEmulatorUnitTests, SAndB32) {

  Wave regs(/*vgprCount*/ 1, /*sgprCount*/ 4, /*waveSize*/ 1);
...
// A test of s_cmp_eq_u32
TEST(GpuEmulatorUnitTests, SCmpEqU32) {
  Wave regs(/*vgprCount*/ 1, /*sgprCount*/ 4, /*waveSize*/ 1);
...
TEST(GpuEmulatorUnitTests, SBfeU32) {
...
TEST(GpuEmulatorUnitTests, SAddI32) {
  Wave regs(1, 5, 1);
...
TEST(GpuEmulatorUnitTests, SCSelectB32) {
  // 5 SGPRs.
  Wave regs(1, 5, 1);
...
TEST(GpuEmulatorUnitTests, SXorB64) {
...
TEST(GpuEmulatorUnitTests, SMovkI32_SignExtension) {
  Wave regs(1, 1, 1);
...
TEST(GpuEmulatorUnitTests, SAdd) {
...
TEST(GpuEmulatorTests, SAddCU32) {
  Wave regs(8, 8, 8);
...
TEST(GpuEmulatorUnitTestsPartTwoPartTwo, S_LSHL_B64) {
  // Needs s0-s4 (5 registers)
  Wave regs(/*vgpr*/ 0, /*sgpr*/ 6, /*wave*/ 64);
...
TEST(GpuEmulatorUnitTestsPartTwo, MOV_Ops_V2) {
  Wave regs(4, 4, 64);
