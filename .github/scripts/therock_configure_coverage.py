...
# This branch covers the HEADER-ONLY libraries (rocPRIM / hipCUB / rocThrust):
# they have no shared object to instrument, so coverage is collected from their
# instrumented test binaries (see each project's test_categories_coverage.yaml).
# The shared-object components (hipRAND/rocRAND/rocFFT/rocBLAS) are handled in a
# separate PR. cmake_options pin the build to the PRIM group only so the coverage
# job does not inherit unrelated components. coverage_config is the per-project
# coverage metadata file, kept next to the project's test_categories.yaml.
