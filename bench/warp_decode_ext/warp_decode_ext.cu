...
// FP8 block-scale MoE benchmark. Three entry points:
//
                             ...
                             torch::Tensor& intermediate)
{
    ...
    ck_tile::launch_warp_decode_gate_up<GateUpKernelFP8>(kargs, s);
}
                              ...
                              torch::Tensor& intermediate)
{
    ...
    auto s = make_stream_cfg();
    ck_tile::launch_warp_decode_gate_up<GateUpKernelBF16>(kargs, s);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("warp_decode_gate_up_fp8",
          &warp_decode_gate_up_fp8,
          "Warp-decode gate+up (fp8 activation, Block2D<1,128> x scale, "
    ...
    m.def("warp_decode_gate_up_bf16",
          &warp_decode_gate_up_bf16,
          "Warp-decode gate+up (bf16 activation, no x scale, Block2D<128,128> "
    ...
    m.def("warp_decode_down_reduce",
          &warp_decode_down_reduce,
          "Warp-decode down projection with in-register weighted reduction "
