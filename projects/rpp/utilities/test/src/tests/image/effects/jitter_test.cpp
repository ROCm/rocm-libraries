...
// Full name:
// Image_Effects/JitterTest.Correctness/<Backend>_<DType>to<DType>_<Layout>_<Roi>_<Size>_k1
// Image_Effects_Window/JitterWindowTest.ReachableWithinWindow/<Backend>_<DType>to<DType>_<Layout>_<Roi>_<Size>_k{3,5}
//
// Correctness and ReachableWithinWindow use SEPARATE fixture classes (rather than two TEST_P bodies
// sharing one fixture) so each INSTANTIATE_TEST_SUITE_P only cross-instantiates its own test: a
// shared fixture would otherwise run Correctness's identity check against kernelSize={3,5} (fails,
// since jitter is genuinely randomized there) and ReachableWithinWindow's ASSERT_GT(r,0) against
// kernelSize=1 (fails by construction) -- a self-inflicted test bug, not a kernel defect.
...
INSTANTIATE_TEST_SUITE_P(Image_Effects, JitterTest,
                         ::testing::ValuesIn(with_params<JitterParams>(
...
class JitterWindowTest : public ::testing::TestWithParam<WithParams<JitterParams>> {};

TEST_P(JitterWindowTest, ReachableWithinWindow) {
    const auto& p = GetParam();
    switch (p.cfg.dtype) {
        case DType::U8:
        ...
        case DType::F32:
            ...
            break;
        default:
            FAIL() << "unsupported dtype for jitter window check";
    }
}

INSTANTIATE_TEST_SUITE_P(Image_Effects_Window, JitterWindowTest,
                         ::testing::ValuesIn(with_params<JitterParams>(
...
// from std::random_device and is therefore never reproducible -- see
// .notes/jira-tickets/rain-nondeterministic-seed.md). A real per-call seed parameter must make
// repeated calls with the same seed bit-identical, and different seeds should (with overwhelming
// probability, over this many pixels) produce a divergent result somewhere in the ROI.

class JitterSeedTest : public ::testing::TestWithParam<TestConfig> {};

TEST_P(JitterSeedTest, SeedInvariant) {
    const TestConfig& cfg = GetParam();
    using T = Rpp8u;
    const Rpp32u kernelSizeVal = 5;
...
INSTANTIATE_TEST_SUITE_P(Image_Effects, JitterSeedTest,
