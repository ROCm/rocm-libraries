...
double saturation_tolerance(DType dt) {
    switch (dt) {
        ...
        // some pixels (same systemic HIP I8 HSV defect as hue; see section 13 of the plan) -- which
        // 1.0 catches.
        case DType::I8:
            return 1.0;
        case DType::F32:
...
void run_saturation(const TestConfig& cfg, const SaturationParams& op) {
    ...
    PinnedArray<Rpp32f> sat(cfg.backend, shape.n);
    ...
    golden = input;
    saturation_reference<T>(input.data(), golden.data(), desc, cfg.dtype, roi.data(), XYWH,
                            op.saturation);
    ...
    ASSERT_EQ(rppt_saturation(src.ptr(), &desc, dst.ptr(), &desc, sat.data(), roi.data(), XYWH,
