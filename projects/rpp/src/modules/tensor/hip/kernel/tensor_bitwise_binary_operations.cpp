...
template <typename T>
struct BitwiseLoadStoreExecute;

template <>
struct BitwiseLoadStoreExecute<uchar> {
    using VectorType = d_uchar8;

    __device__ __forceinline__ static void rpp_hip_load8(uchar* src, uchar* dst) {
        rpp_hip_load8_to_uchar8(src, dst);
    }
    __device__ __forceinline__ static void rpp_hip_pack_and_store8(uchar* dst, d_uchar8* dst_u8) {
        rpp_hip_pack_uchar8_and_store8(dst, dst_u8);
    };
};

template <>
struct BitwiseLoadStoreExecute<ushort> {
    using VectorType = d_ushort8;

    __device__ __forceinline__ static void rpp_hip_load8(ushort* src, ushort* dst) {
        rpp_hip_load8_to_ushort8(src, dst);
    }
    __device__ __forceinline__ static void rpp_hip_pack_and_store8(ushort* dst, d_ushort8* dst_u8) {
        rpp_hip_pack_ushort8_and_store8(dst, dst_u8);
    };
};

template <>
struct BitwiseLoadStoreExecute<uint> {
    using VectorType = d_uint8;

    __device__ __forceinline__ static void rpp_hip_load8(uint* src, uint* dst) {
        rpp_hip_load8_to_uint8(src, dst);
    }
    __device__ __forceinline__ static void rpp_hip_pack_and_store8(uint* dst, d_uint8* dst_u8) {
        rpp_hip_pack_uint8_and_store8(dst, dst_u8);
    };
};

// Structures used to dispatch execution of bitwise operations (AND, OR, XOR)
template <typename VectorType, typename Operation>
struct BitwiseOperationExecute;

template <typename VectorType>
struct BitwiseOperationExecute<VectorType, BitwiseOr> {
    __device__ __forceinline__ static void rpp_hip_math_bitwiseOp8(VectorType* a, VectorType* b,
                                                                   VectorType* c) {
...
template <typename VectorType>
struct BitwiseOperationExecute<VectorType, BitwiseXor> {
    __device__ __forceinline__ static void rpp_hip_math_bitwiseOp8(VectorType* a, VectorType* b,
                                                                   VectorType* c) {
...
template <typename VectorType>
struct BitwiseOperationExecute<VectorType, BitwiseAnd> {
    __device__ __forceinline__ static void rpp_hip_math_bitwiseOp8(VectorType* a, VectorType* b,
                                                                   VectorType* c) {
...
__global__ void tensor_or_tensor_1d_hip_tensor(T* srcPtr1, T* srcPtr2, uint* srcStrides1,
                                               uint* srcStrides2, uint* src1BeginOffsets,
                                               uint* src2BeginOffsets, T* dstPtr, uint* dstStrides,
                                               uint* dstDims, Operation op) {
    ...
    if (id_x >= dstSampleDims[0]) return;
...
#pragma unroll
        ...
        VectorType dst_vec8;
        BitwiseOperationExecute<VectorType, Operation>::rpp_hip_math_bitwiseOp8(
            (VectorType*)srcArr1, (VectorType*)srcArr2, &dst_vec8);
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_non_broadcast_1d_hip_tensor(T* src1Ptr, T* src2Ptr, T* dstPtr,
                                                             uint strides, uint* roiTensor1,
                                                             uint* roiTensor2, Operation op) {
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_2d_hip_tensor(T* srcPtr1, T* srcPtr2, uint* srcStrides1,
                                               uint* srcStrides2, uint* src1BeginOffsets,
                                               uint* src2BeginOffsets, T* dstPtr, uint* dstStrides,
                                               uint* dstDims, Operation op) {
    uint id_x = (hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x) * 8;  // width
    ...
    if (id_x >= dstSampleDims[1] || id_y >= dstSampleDims[0]) return;

    uint numRows = dstSampleDims[1] - id_x;

    if (numRows >= 8) {
        uint srcBaseIdx1 =
            (id_z * src1SampleStrides[0]) + ((id_y)*src1SampleStrides[1]) + src1BeginOffsets[id_z];
        uint srcBaseIdx2 =
            (id_z * src2SampleStrides[0]) + ((id_y)*src2SampleStrides[1]) + src2BeginOffsets[id_z];
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_non_broadcast_2d_hip_tensor(T* src1Ptr, T* src2Ptr, T* dstPtr,
                                                             uint2 stridesNH, uint* roiTensor1,
                                                             uint* roiTensor2, Operation op) {
    uint id_x = (hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x) * 8;  // width
    uint id_y = hipBlockIdx_y * hipBlockDim_y + hipThreadIdx_y;        // height
    uint id_z = hipBlockIdx_z * hipBlockDim_z + hipThreadIdx_z;        // batchsize
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_3d_hip_tensor(T* srcPtr1, T* srcPtr2, uint* srcStrides1,
                                               uint* srcStrides2, uint src1BeginOffset,
                                               uint src2BeginOffset, T* dstPtr, uint* dstStrides,
                                               uint* dstDims, Operation op) {
    ...
    if (id_x >= dstDims[2] || id_y >= dstDims[1] || id_z >= dstDims[0]) return;
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_non_broadcast_3d_hip_tensor(T* src1Ptr, T* src2Ptr, T* dstPtr,
                                                             uint2 stridesDH, uint* roiTensor1,
                                                             uint* roiTensor2, Operation op) {
    uint id_x = (hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x) * 8;  // lengthX
    uint id_y = hipBlockIdx_y * hipBlockDim_y + hipThreadIdx_y;        // lengthY
    uint id_z = hipBlockIdx_z * hipBlockDim_z + hipThreadIdx_z;        // lengthZ
    ...
    if (id_x >= lengthX2 || id_y >= lengthY2 || id_z >= lengthZ2) return;

    uint srcIdx1 =
        ((id_z + beginZ1) * stridesDH.x) + ((id_y + beginY1) * stridesDH.y) + id_x + beginX1;
    uint srcIdx2 =
        ((id_z + beginZ2) * stridesDH.x) + ((id_y + beginY2) * stridesDH.y) + id_x + beginX2;
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_nd_hip_tensor(T* srcPtr1, T* srcPtr2, uint* srcStrides1,
                                               uint* srcStrides2, uint* src1BeginOffsets,
                                               uint* src2BeginOffsets, uint numDims, T* dstPtr,
                                               uint* dstStrides, uint* dstDims, Operation op) {
    ...
    uint* src2SampleStrides = srcStrides2 + id_z * RPPT_MAX_DIMS;
    uint* dstSampleStrides = dstStrides + id_z * RPPT_MAX_DIMS;
    if (id_x >= dstSampleStrides[0]) return;
...
template <typename T, typename Operation>
__global__ void tensor_or_tensor_non_broadcast_nd_hip_tensor(T* src1Ptr, T* src2Ptr,
                                                             d_uint5_s src1Dims, uint numDims,
                                                             T* dstPtr, d_uint5_s strides,
    ...
    uint totalElements = strides.data[0];

    if (id_x >= totalElements) return;
    ...
    if (remaining >= 8) {
        ...
        for (uint e = 0; e < remaining; e++) {
            ...
            for (int i = 0; i < numDims; i++) {
                uint coord = (flatIdx / dimStrides[i]) % src1Dims.data[i];
                if (coord >= length[i]) {
                    skipElement = true;
                    break;
                }
...
template <typename T, typename Operation>
RppStatus hip_exec_tensor_binary_bitwise_generic_tensor(
    T* srcPtr1, T* srcPtr2, RpptGenericDescPtr srcGenericDescPtr1,
    RpptGenericDescPtr srcGenericDescPtr2, T* dstPtr, RpptGenericDescPtr dstGenericDescPtr,
    Operation op, uint* roiTensor1, uint* roiTensor2, rpp::Handle& handle) {
    Rpp32u batchSize = dstGenericDescPtr->dims[0];  // Number of samples in batch
    Rpp32u src1NDim =
        srcGenericDescPtr1->numDims - 1;  // Omitting batchSize here to get tensor dimension
    Rpp32u src2NDim =
        srcGenericDescPtr2->numDims - 1;  // Omitting batchSize here to get tensor dimension
    ...
    for (int i = 0; i < minDim; i++) {
        if (srcGenericDescPtr1->dims[src1NDim - i] != srcGenericDescPtr2->dims[src2NDim - i]) {
            if ((srcGenericDescPtr1->dims[src1NDim - i] != 1) &&
                (srcGenericDescPtr2->dims[src2NDim - i] != 1))
                return RPP_ERROR_INVALID_DIM_LENGTHS;
        }
    }

    // Allocate pinned buffers for broadcast dims/strides - Strides and Dims for each sample in
    // batch
    Rpp32u* src1BroadcastDims =
        reinterpret_cast<Rpp32u*>(handle.GetInitHandle()->mem.mgpu.scratchBufferPinned.floatmem);
...
#pragma omp parallel for num_threads(batchSize)
    for (int i = 0; i < batchSize; i++) {
        ...
        // Compute begin offsets based on ROIs & check incompatibility of dimensions and compute
        // dstSampleDims
        for (int j = 0; j < dstDim; j++) {
            if ((src1SampleDims[j] != src2SampleDims[j]) && (src1SampleDims[j] != 1) &&
                (src2SampleDims[j] != 1))
        ...
        for (int j = 0; j < src1NDim; j++) src1BeginOffsets[i] += src1Begin[j] * src1Strides[j + 1];
        for (int j = 0; j < src2NDim; j++) src2BeginOffsets[i] += src2Begin[j] * src2Strides[j + 1];

        // Source strides for sample set to zero if corresponding axis shape = 1 for broadcasting
        // purposes Setting stride to zero will allow for repetition of values operated required for
        // broadcasting
        for (int j = 0; j < dstDim; j++) {
    ...
    // Copy to device
    RPP_HIP_RETURN_IF_ERROR(hipMemcpyAsync(d_dstBroadcastDims, dstBroadcastDims,
                                           batchSize * RPPT_MAX_DIMS * sizeof(Rpp32u),
                                           hipMemcpyHostToDevice, handle.GetStream()));
    RPP_HIP_RETURN_IF_ERROR(hipMemcpyAsync(d_src1BeginOffsets, src1BeginOffsets,
                                           batchSize * sizeof(Rpp32u), hipMemcpyHostToDevice,
                                           handle.GetStream()));
    RPP_HIP_RETURN_IF_ERROR(hipMemcpyAsync(d_src2BeginOffsets, src2BeginOffsets,
                                           batchSize * sizeof(Rpp32u), hipMemcpyHostToDevice,
                                           handle.GetStream()));
    RPP_HIP_RETURN_IF_ERROR(hipMemcpyAsync(d_src1BroadcastStrides, src1BroadcastStrides,
                                           batchSize * RPPT_MAX_DIMS * sizeof(Rpp32u),
                                           hipMemcpyHostToDevice, handle.GetStream()));
    RPP_HIP_RETURN_IF_ERROR(hipMemcpyAsync(d_src2BroadcastStrides, src2BroadcastStrides,
                                           batchSize * RPPT_MAX_DIMS * sizeof(Rpp32u),
                                           hipMemcpyHostToDevice, handle.GetStream()));
    RPP_HIP_RETURN_IF_ERROR(hipMemcpyAsync(d_dstBroadcastStrides, dstBroadcastStrides,
                                           batchSize * RPPT_MAX_DIMS * sizeof(Rpp32u),
                                           hipMemcpyHostToDevice, handle.GetStream()));
    ...
    if (dstDim == 1) {
        ...
        for (int batchCount = 0; batchCount < batchSize; batchCount++) {
            hipLaunchKernelGGL(
                tensor_or_tensor_3d_hip_tensor,
                dim3(ceil((float)globalThreads_x / LOCAL_THREADS_X),
                     ceil((float)globalThreads_y / LOCAL_THREADS_Y),
                     ceil((float)globalThreads_z / LOCAL_THREADS_Z)),
                ...
                d_src2BroadcastStrides + batchCount * RPPT_MAX_DIMS, src1BeginOffsets[batchCount],
...
// Contains kernel launches specific to the non broadcast version, cannot be used for the broadcast
// cases
template <typename T, typename Operation>
RppStatus hip_exec_tensor_non_broadcast_binary_bitwise_generic_tensor(
    T* srcPtr1, T* srcPtr2, RpptGenericDescPtr srcGenericDescPtr1,
    RpptGenericDescPtr srcGenericDescPtr2, T* dstPtr, RpptGenericDescPtr dstGenericDescPtr,
    Operation op, uint* roiTensor1, uint* roiTensor2, rpp::Handle& handle) {
    ...
    if (numDims == 1) {
        ...
        for (int batchCount = 0; batchCount < dstGenericDescPtr->dims[0]; batchCount++) {
            hipLaunchKernelGGL(
                tensor_or_tensor_non_broadcast_3d_hip_tensor,
                dim3(ceil((float)globalThreads_x / LOCAL_THREADS_X),
                     ceil((float)globalThreads_y / LOCAL_THREADS_Y),
                     ceil((float)globalThreads_z / LOCAL_THREADS_Z)),
                dim3(LOCAL_THREADS_X, LOCAL_THREADS_Y, LOCAL_THREADS_Z), 0, handle.GetStream(),
...
template RppStatus tensor_binary_bitwise_op_dispatch_gpu_tensor<Rpp8u>(
...
template RppStatus tensor_binary_bitwise_op_dispatch_gpu_tensor<Rpp16u>(
    Rpp16u*, Rpp16u*, RpptGenericDescPtr, RpptGenericDescPtr, Rpp16u*, RpptGenericDescPtr,
    RpptBitwiseOp, RpptBroadcastMode, Rpp32u*, Rpp32u*, rpp::Handle&);

template RppStatus tensor_binary_bitwise_op_dispatch_gpu_tensor<Rpp32u>(
