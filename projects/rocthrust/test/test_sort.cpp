  ...
  for (auto size : get_sizes())
  {
    ...
    if (std::is_floating_point<key_type>::value)
    {
      for (auto seed : get_seeds())
      {
        ...
        h_keys = get_random_data<key_type>(size, (key_type) -1000, (key_type) + 1000, seed);
      }
    }
    else
    {
      for (auto seed : get_seeds())
      {
        ...
        h_keys = get_random_data<key_type>(
          size, get_default_limits<key_type>::min(), get_default_limits<key_type>::max(), seed);
      }
    }
...
TYPED_TEST(SortTests, SortByKey)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    std::shuffle(h_keys.begin(), h_keys.end(), std::default_random_engine(std::random_device{}()));
    ...
    thrust::sort_by_key(expected_keys.begin(), expected_keys.end(), expected_values.begin(), compare_function());
...
TYPED_TEST(SortTests, StableSortByKey)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    thrust::host_vector<key_type> expected_keys(h_keys);
    thrust::host_vector<value_type> expected_values(h_values);
    thrust::stable_sort_by_key(expected_keys.begin(), expected_keys.end(), expected_values.begin(), compare_function());

    thrust::device_vector<key_type> d_keys(h_keys);
    thrust::device_vector<value_type> d_values(h_values);
    thrust::stable_sort_by_key(d_keys.begin(), d_keys.end(), d_values.begin(), compare_function());
