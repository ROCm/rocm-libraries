...
void TestBoundsWithBigIndexesHelper(int magnitude)
{
  ...
  _THRUST_STD::intmax_t distance_low_value =
    _THRUST_STD::distance(begin, thrust::lower_bound(thrust::device, begin, end, 17));

  _THRUST_STD::intmax_t distance_high_value =
    _THRUST_STD::distance(begin, thrust::lower_bound(thrust::device, begin, end, (1ll << magnitude) - 17));

  ASSERT_EQ(distance_low_value, 16);
  ASSERT_EQ(distance_high_value, (1ll << magnitude) - 18);

  distance_low_value = _THRUST_STD::distance(begin, thrust::upper_bound(thrust::device, begin, end, 17));

  distance_high_value =
    _THRUST_STD::distance(begin, thrust::upper_bound(thrust::device, begin, end, (1ll << magnitude) - 17));
