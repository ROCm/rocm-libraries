...
TYPED_TEST(MinElementTests, TestMinElementSimple)
{
  ...
  ASSERT_EQ(*thrust::min_element(data.begin(), data.end(), _THRUST_STD::greater<T>()), 5);
  ASSERT_EQ(thrust::min_element(data.begin(), data.end(), _THRUST_STD::greater<T>()) - data.begin(), 1);
}
...
TYPED_TEST(MinElementVectorUnitTests, TestMinElementWithTransform)
{
  ...
  ASSERT_EQ(*thrust::min_element(thrust::make_transform_iterator(data.begin(), _THRUST_STD::negate<T>()),
...
void TestMinElementWithBigIndexesHelper(int magnitude)
{
  ...
  ASSERT_EQ(*thrust::min_element(thrust::device, begin, end, _THRUST_STD::greater<long long>()), (1ll << magnitude));
}
