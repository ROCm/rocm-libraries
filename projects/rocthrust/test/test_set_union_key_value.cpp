...
TYPED_TEST(SetUnionKeyValuePrimitiveTests, TestSetUnionKeyValueDescending)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      thrust::stable_sort(h_a.begin(), h_a.end(), _THRUST_STD::greater<T>());
      thrust::stable_sort(h_b.begin(), h_b.end(), _THRUST_STD::greater<T>());
      ...
      h_end =
        thrust::set_union(h_a.begin(), h_a.end(), h_b.begin(), h_b.end(), h_result.begin(), _THRUST_STD::greater<T>());
      h_result.erase(h_end, h_result.end());

      d_end =
        thrust::set_union(d_a.begin(), d_a.end(), d_b.begin(), d_b.end(), d_result.begin(), _THRUST_STD::greater<T>());
      d_result.erase(d_end, d_result.end());
