...
TYPED_TEST(UniqueByKeyIntegralTests, TestUniqueCopyByKeyLargeInput)
{
  ...
  ASSERT_EQ(num_selected_out, _THRUST_STD::distance(values_out.begin(), selected_aut_end.second));
...
TYPED_TEST(UniqueByKeyIntegralTests, TestUniqueCopyByKeyLargeOutCount)
{
  ...
  ASSERT_EQ(num_selected_out, _THRUST_STD::distance(values_out, selected_aut_end.second));
}
