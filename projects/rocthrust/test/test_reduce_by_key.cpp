...
TYPED_TEST(ReduceByKeysIntegralTests, TestReduceByKey)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      HostIteratorPair h_last = thrust::reduce_by_key(
        h_keys.begin(), h_keys.end(), h_vals.begin(), h_keys_output.begin(), h_vals_output.begin());
      DeviceIteratorPair d_last = thrust::reduce_by_key(
...
ReduceByKeyKernel(int const N, int* in_keys, int* in_values, int* out_keys, int* out_values, int* out_sizes)
{
  if (threadIdx.x == 0)
  {
    ...
    auto end_pair = thrust::reduce_by_key(
      thrust::hip::par, in_keys_begin, in_keys_end, in_values_begin, out_keys_begin, out_values_begin);
...
TEST(ReduceByKeyTests, TestReduceByKeyDevice)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      auto end_pair = thrust::reduce_by_key(
        h_keys.begin(), h_keys.end(), h_values.begin(), h_keys_result.begin(), h_values_result.begin());
...
TEST(ReduceByKeyTests, TestReduceByKeyWithBigIndexes)
{
  ...
  std::vector<size_t> magnitudes = {30, 31, 32, 33};
