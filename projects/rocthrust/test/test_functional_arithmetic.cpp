...
#define INSTANTIATE_INTEGER_TYPES(Macro, vector_type, operator_name)                         \
  Macro(vector_type, operator_name, int8_t) Macro(vector_type, operator_name, uint8_t)       \
    Macro(vector_type, operator_name, int16_t) Macro(vector_type, operator_name, uint16_t)   \
      Macro(vector_type, operator_name, int32_t) Macro(vector_type, operator_name, uint32_t) \
        Macro(vector_type, operator_name, int64_t) Macro(vector_type, operator_name, uint64_t)
...
#define DECLARE_BINARY_ARITHMETIC_FUNCTIONAL_UNITTEST(operator_name, OperatorName)                      \
  TEST(FunctionalArithmeticTests, Test##OperatorName##FunctionalHost)                                   \
  {                                                                                                     \
    SCOPED_TRACE(testing::Message() << "with device_id= " << test::set_device_from_ctest());            \
                                                                                                        \
    INSTANTIATE_ALL_TYPES(INSTANTIATE_BINARY_ARITHMETIC_FUNCTIONAL_TEST, host_vector, operator_name);   \
  }                                                                                                     \
  TEST(FunctionalArithmeticTests, Test##OperatorName##FunctionalDevice)                                 \
  {                                                                                                     \
    SCOPED_TRACE(testing::Message() << "with device_id= " << test::set_device_from_ctest());            \
                                                                                                        \
