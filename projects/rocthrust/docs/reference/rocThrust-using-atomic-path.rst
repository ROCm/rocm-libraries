...
This behavior can be changed through the ``ROCPRIM_USE_ATOMIC_BLOCK_ID`` environment variable. Set ``ROCPRIM_USE_ATOMIC_BLOCK_ID`` to 0, to never use the atomic path. Set ``ROCPRIM_USE_ATOMIC_BLOCK_ID`` to 2 to always use the atomic path. Set ``ROCPRIM_USE_ATOMIC_BLOCK_ID`` to 1 to return to the default behavior.
