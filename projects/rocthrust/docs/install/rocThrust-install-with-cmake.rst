...
* ``LINK_HIP_DEVICE_LIBS``. Set to ``OFF`` to prevent linking against device dependencies (including rocPRIM). In this case, the build can only be used for host-side computation. Default is ``ON``.
