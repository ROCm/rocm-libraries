...
void TestTransformReduce(const size_t n)
{
  ...
  T cpu_result = thrust::transform_reduce(h_data.begin(), h_data.end(), _THRUST_STD::negate<T>(), init, _THRUST_STD::plus<T>());
  T gpu_result = thrust::transform_reduce(d_data.begin(), d_data.end(), _THRUST_STD::negate<T>(), init, _THRUST_STD::plus<T>());
...
void TestTransformReduceFromConst(const size_t n)
{
  ...
  T cpu_result = thrust::transform_reduce(h_data.cbegin(), h_data.cend(), _THRUST_STD::negate<T>(), init, _THRUST_STD::plus<T>());
  T gpu_result = thrust::transform_reduce(d_data.cbegin(), d_data.cend(), _THRUST_STD::negate<T>(), init, _THRUST_STD::plus<T>());
