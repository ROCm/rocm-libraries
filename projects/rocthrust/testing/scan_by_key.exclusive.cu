...
void TestExclusiveScanByKeySimple()
{
  ...
  thrust::exclusive_scan_by_key(
    keys.begin(), keys.end(), vals.begin(), output.begin(), T(10), _THRUST_STD::equal_to<T>(), _THRUST_STD::multiplies<T>());

  ref = {10, 10, 20, 60, 10, 10, 60};
  ASSERT_EQUAL(output, ref);

  thrust::exclusive_scan_by_key(keys.begin(), keys.end(), vals.begin(), output.begin(), T(10), _THRUST_STD::equal_to<T>());
...
void TestScanByKeyDiscardOutput(std::size_t n)
{
  ...
  thrust::exclusive_scan_by_key(
    d_keys.cbegin(), d_keys.cend(), d_vals.cbegin(), out, T{}, _THRUST_STD::equal_to<T>{}, _THRUST_STD::multiplies<T>{});
}
