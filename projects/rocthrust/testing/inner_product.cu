...
void TestInnerProductWithOperator()
{
  ...
  T init   = 3;
  T result = thrust::inner_product(v1.begin(), v1.end(), v2.begin(), init, _THRUST_STD::multiplies<T>(), _THRUST_STD::minus<T>());
...
void TestInnerProductWithBigIndexesHelper(int magnitude)
{
  ...
  *has_executed                         = false;
  ...
  ASSERT_EQUAL(thrust::inner_product(thrust::device, begin, end, begin, 0ll, _THRUST_STD::plus<long long>(), fn),
               (1ll << magnitude));
...
{ // Regression test for NVIDIA/thrust#1178
  ...
  auto result = thrust::inner_product(v1.begin(), v1.end(), v2.begin(), 0.0f, _THRUST_STD::plus<float>{}, _1 * _2 + 1.0f);
