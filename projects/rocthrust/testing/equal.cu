...
void TestEqual(const size_t n)
{
  ...
  if (n > 0)
  {
    ...
    ASSERT_EQUAL(thrust::equal(h_data1.begin(), h_data1.begin() + 1, h_data2.begin(), _THRUST_STD::greater<T>()),
                 false);
    ASSERT_EQUAL(thrust::equal(d_data1.begin(), d_data1.begin() + 1, d_data2.begin(), _THRUST_STD::greater<T>()),
                 false);
  }
}
