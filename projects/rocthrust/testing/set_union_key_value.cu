...
void TestSetUnionKeyValueDescending(size_t n)
{
  ...
  h_end = thrust::set_union(h_a.begin(), h_a.end(), h_b.begin(), h_b.end(), h_result.begin(), _THRUST_STD::greater<T>());
  h_result.erase(h_end, h_result.end());

  d_end = thrust::set_union(d_a.begin(), d_a.end(), d_b.begin(), d_b.end(), d_result.begin(), _THRUST_STD::greater<T>());
  d_result.erase(d_end, d_result.end());
