...
void TestRemoveCopyIfToDiscardIterator(const size_t n)
{
  ...
  size_t num_false = thrust::count_if(h_data.begin(), h_data.end(), ::internal::not_fn(is_true<T>()));
...
void TestRemoveCopyIfStencilToDiscardIterator(const size_t n)
{
  ...
  size_t num_false = thrust::count_if(h_stencil.begin(), h_stencil.end(), ::internal::not_fn(is_true<T>()));
