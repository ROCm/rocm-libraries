...
void TestTransformUnaryDevice(ExecutionPolicy exec)
{
  ...
  transform_kernel<<<1, 1>>>(exec, input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), iter_vec.begin());
...
void TestTransformIfUnaryNoStencilDevice(ExecutionPolicy exec)
{
  ...
  transform_if_kernel<<<1, 1>>>(
    exec, input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), ::cuda::std::identity{}, iter_vec.begin());
  cudaError_t const err = cudaDeviceSynchronize();
  ASSERT_EQUAL(cudaSuccess, err);
...
void TestTransformUnaryCudaStreams()
{
  ...
  iter = thrust::transform(thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>());
