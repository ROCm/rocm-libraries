...
void TestScanDevice(ExecutionPolicy exec, const size_t n)
{
  ...
  inclusive_scan_kernel<<<1, 1>>>(exec, d_input.begin(), d_input.end(), d_output.begin(), (T) 11, _THRUST_STD::plus<T>{});
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
...
void TestScanCudaStreams()
{
  ...
  iter = thrust::inclusive_scan(thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
  ...
  iter =
    thrust::inclusive_scan(thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), 3, _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
  ...
  iter =
    thrust::exclusive_scan(thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), 3, _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
