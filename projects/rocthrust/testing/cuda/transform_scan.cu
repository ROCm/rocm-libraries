...
void TestTransformScanDevice(ExecutionPolicy exec)
{
  ...
  transform_inclusive_scan_kernel<<<1, 1>>>(
    exec, input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), _THRUST_STD::plus<T>(), iter_vec.begin());
  {
  ...
  transform_inclusive_scan_init_kernel<<<1, 1>>>(
    exec, input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), 3, _THRUST_STD::plus<T>(), iter_vec.begin());
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
  ...
  transform_exclusive_scan_kernel<<<1, 1>>>(
    exec, input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), 0, _THRUST_STD::plus<T>(), iter_vec.begin());
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
  ...
  input = input_copy;
  transform_inclusive_scan_kernel<<<1, 1>>>(
    exec, input.begin(), input.end(), input.begin(), _THRUST_STD::negate<T>(), _THRUST_STD::plus<T>(), iter_vec.begin());
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
  ...
  input = input_copy;
  transform_inclusive_scan_init_kernel<<<1, 1>>>(
    exec, input.begin(), input.end(), input.begin(), _THRUST_STD::negate<T>(), 3, _THRUST_STD::plus<T>(), iter_vec.begin());
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
...
void TestTransformScanCudaStreams()
{
  ...
  iter = thrust::transform_inclusive_scan(
    thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), _THRUST_STD::plus<T>());
  ...
  // inclusive scan with nonzero init
  iter = thrust::transform_inclusive_scan(
    thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), 3, _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
  ...
  iter = thrust::transform_exclusive_scan(
    thrust::cuda::par.on(s), input.begin(), input.end(), output.begin(), _THRUST_STD::negate<T>(), 0, _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
  ...
  input = input_copy;
  iter  = thrust::transform_inclusive_scan(
    thrust::cuda::par.on(s), input.begin(), input.end(), input.begin(), _THRUST_STD::negate<T>(), _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
  ...
  input = input_copy;
  iter  = thrust::transform_inclusive_scan(
    thrust::cuda::par.on(s), input.begin(), input.end(), input.begin(), _THRUST_STD::negate<T>(), 3, _THRUST_STD::plus<T>());
  cudaStreamSynchronize(s);
...
void TestTransformScanConstAccumulator()
{
  ...
  thrust::transform_inclusive_scan(
    input.begin(), input.end(), output.begin(), ::cuda::std::identity{}, _THRUST_STD::plus<T>());
  thrust::inclusive_scan(input.begin(), input.end(), reference.begin(), _THRUST_STD::plus<T>());
