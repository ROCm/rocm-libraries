...
void TestMinElementDevice(ExecutionPolicy exec)
{
  ...
  typename thrust::host_vector<int>::iterator h_max =
    thrust::min_element(h_data.begin(), h_data.end(), _THRUST_STD::greater<int>());

  min_element_kernel<<<1, 1>>>(exec, d_data.begin(), d_data.end(), _THRUST_STD::greater<int>(), d_result.begin());
  {
    cudaError_t const err = cudaDeviceSynchronize();
    ASSERT_EQUAL(cudaSuccess, err);
  }
...
void TestMinElementCudaStreams()
{
  ...
  ASSERT_EQUAL(*thrust::min_element(thrust::cuda::par.on(s), data.begin(), data.end(), _THRUST_STD::greater<T>()), 5);
  ASSERT_EQUAL(
    thrust::min_element(thrust::cuda::par.on(s), data.begin(), data.end(), _THRUST_STD::greater<T>()) - data.begin(), 1);
...
void TestMinElementDevicePointer()
{
  ...
  ASSERT_EQUAL(thrust::min_element(thrust::device, raw_ptr, raw_ptr + n, _THRUST_STD::greater<T>()) - raw_ptr, 1);
}
