...
void TestMaxElementSimple()
{
  ...
  ASSERT_EQUAL(*thrust::max_element(data.begin(), data.end(), _THRUST_STD::greater<T>()), 1);
  ASSERT_EQUAL(thrust::max_element(data.begin(), data.end(), _THRUST_STD::greater<T>()) - data.begin(), 2);
}
...
void TestMaxElementWithTransform()
{
  ...
  ASSERT_EQUAL(*thrust::max_element(thrust::make_transform_iterator(data.begin(), _THRUST_STD::negate<T>()),
...
void TestMaxElement(const size_t n)
{
  ...
  typename thrust::host_vector<T>::iterator h_min =
    thrust::max_element(h_data.begin(), h_data.end(), _THRUST_STD::greater<T>());
  typename thrust::device_vector<T>::iterator d_min =
    thrust::max_element(d_data.begin(), d_data.end(), _THRUST_STD::greater<T>());
