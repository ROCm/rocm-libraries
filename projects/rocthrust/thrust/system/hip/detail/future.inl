...
{
  THRUST_HOST void operator()(hipEvent_t e) const
  {
    if (nullptr != e)
    {
...
public:
  ...
  THRUST_HOST unique_marker()
      : handle_(nullptr, marker_deleter())
  {
    native_handle_type e;
    thrust::hip_rocprim::throw_on_error(hipEventCreateWithFlags(&e, hipEventDisableTiming));
    handle_.reset(e);
  }
  ...
  THRUST_HOST auto get() const THRUST_DECLTYPE_RETURNS(native_handle_type(handle_.get()));
  THRUST_HOST auto native_handle() const THRUST_DECLTYPE_RETURNS(native_handle_type(handle_.get()));

  THRUST_HOST bool valid() const noexcept
  {
    return bool(handle_);
  }

  THRUST_HOST bool ready() const
  {
  ...
  THRUST_HOST void wait() const
  {
    thrust::hip_rocprim::throw_on_error(hipEventSynchronize(handle_.get()));
  }

  THRUST_HOST bool operator==(unique_marker const& other) const
  {
...
{
  THRUST_HOST void operator()(hipStream_t s) const
  {
    if (nullptr != s)
    {
...
public:
  THRUST_HOST constexpr stream_conditional_deleter() noexcept
      : cond_(true)
  {}

  THRUST_HOST explicit constexpr stream_conditional_deleter(nonowning_t) noexcept
      : cond_(false)
  {}
...
public:
  /// \brief Create a new stream and construct a handle to it. When the handle
  ///        is destroyed, the stream is destroyed.
  THRUST_HOST unique_stream()
      : handle_(nullptr, stream_conditional_deleter())
  {
    native_handle_type s;
    thrust::hip_rocprim::throw_on_error(hipStreamCreateWithFlags(&s, hipStreamNonBlocking));
    handle_.reset(s);
  }

  /// \brief Construct a non-owning handle to an existing stream. When the
  ///        handle is destroyed, the stream is not destroyed.
  THRUST_HOST explicit unique_stream(nonowning_t, native_handle_type handle)
  ...
  THRUST_EXEC_CHECK_DISABLE
  THRUST_HOST unique_stream(unique_stream&& o) noexcept
  ...
  THRUST_HOST void wait() const
  {
    thrust::hip_rocprim::throw_on_error(hipStreamSynchronize(handle_.get()));
  }

  THRUST_HOST void depend_on(unique_marker& e)
  {
    thrust::hip_rocprim::throw_on_error(hipStreamWaitEvent(handle_.get(), e.get(), 0));
  }

  THRUST_HOST void depend_on(unique_stream& s)
  {
    if (s != *this)
    {
      unique_marker e;
  ...
  THRUST_HOST void record(unique_marker& e)
  {
    thrust::hip_rocprim::throw_on_error(hipEventRecord(e.get(), handle_.get()));
  }

  THRUST_HOST bool operator==(unique_stream const& other) const
  {
    return other.handle_ == handle_;
  }
...
template <typename... Dependencies>
THRUST_HOST unique_eager_event make_dependent_event(std::tuple<Dependencies...>&& deps);

template <typename X, typename XPointer, typename ComputeContent, typename... Dependencies>
THRUST_HOST unique_eager_future_promise_pair<X, XPointer>
...
public:
  ...
  THRUST_HOST explicit async_signal(unique_stream&& stream)
      : stream_(std::move(stream))
  {}

  THRUST_HOST virtual ~async_signal() {}
...
public:
  ...
  // destroyed.
  THRUST_HOST explicit async_keep_alives(unique_stream&& stream, keep_alives_type&& keep_alives)
      : async_signal(std::move(stream))
      , keep_alives_(std::move(keep_alives))
  {}

  THRUST_HOST virtual ~async_keep_alives() {}
};
...
struct async_value : virtual async_signal
{
  ...
  THRUST_HOST explicit async_value(unique_stream stream)
      : async_signal(std::move(stream))
  {}

  THRUST_HOST virtual ~async_value() {}

  THRUST_HOST virtual bool valid_content() const noexcept
  {
    return false;
  }

  THRUST_HOST virtual value_type get()
  {
    throw thrust::event_error(event_errc::no_state);
  }

  THRUST_HOST virtual value_type extract()
  {
    throw thrust::event_error(event_errc::no_state);
  }
...
public:
  ...
  THRUST_HOST bool valid_content() const noexcept final override
  {
    return nullptr != content_;
  }

  // Precondition: `true == valid_content()`.
  THRUST_HOST pointer data()
  {
    if (!valid_content())
    {
  ...
  // Precondition: `true == valid_content()`.
  THRUST_HOST const_pointer data() const
  {
    if (!valid_content())
    {
      throw thrust::event_error(event_errc::no_content);
  ...
  // Blocks.
  // Precondition: `true == valid_content()`.
  THRUST_HOST value_type get() final override
  {
    this->stream().wait();
    return *data();
  }
  ...
  // Blocks.
  // Precondition: `true == valid_content()`.
  THRUST_HOST value_type extract() final override
  {
    this->stream().wait();
...
public:
  THRUST_HOST_DEVICE weak_promise()
      : device_(0)
      , content_{}
  {}
  ...
  template <typename U>
  THRUST_HOST_DEVICE void set_value(U&& value) &&
  {
    *content_ = THRUST_FWD(value);
  }

  template <typename X, typename XPointer, typename ComputeContent, typename... Dependencies>
  friend THRUST_HOST unique_eager_future_promise_pair<X, XPointer>
  thrust::system::hip::detail::make_dependent_future(ComputeContent&& cc, std::tuple<Dependencies...>&& deps);
};
...
struct ready_event final
{
  ...
  template <typename U>
  THRUST_HOST_DEVICE explicit ready_event(ready_future<U>)
  {}

  THRUST_HOST_DEVICE static constexpr bool valid_content() noexcept
  {
    return true;
  }

  THRUST_HOST_DEVICE static constexpr bool ready() noexcept
  {
    return true;
  }
};
...
public:
  THRUST_HOST_DEVICE ready_future()
      : value_{}
  {}
  ...
  template <typename U>
  THRUST_HOST_DEVICE explicit ready_future(U&& u)
      : value_(THRUST_FWD(u))
  {}
  ...
  THRUST_HOST_DEVICE value_type get() const
  {
    return value_;
  }

  THRUST_NODISCARD THRUST_HOST_DEVICE value_type extract()
  {
...
protected:
  ...
  THRUST_HOST explicit unique_eager_event(int device_id, std::unique_ptr<detail::async_signal> async_signal)
      : device_(device_id)
...
public:
  ...
  template <typename U>
  THRUST_HOST explicit unique_eager_event(unique_eager_future<U>&& other)
  ...
  THRUST_HOST bool valid_stream() const noexcept
  {
    return bool(async_signal_);
  }

  THRUST_HOST bool ready() const noexcept
  {
    if (valid_stream())
    {
      return stream().ready();
  ...
  THRUST_HOST detail::unique_stream& stream()
  {
    if (!valid_stream())
    {
      throw thrust::event_error(event_errc::no_state);
  ...
  THRUST_HOST int where() const noexcept
  {
    return device_;
  }

  // Precondition: `true == valid_stream()`.
  THRUST_HOST void wait()
  {
    stream().wait();
  }
  ...
  thrust::system::hip::detail::make_dependent_event(std::tuple<Dependencies...>&& deps);
};

template <typename T>
struct unique_eager_future final
{
  THRUST_STATIC_ASSERT_MSG((!std::is_same<T, ::std::remove_cv_t<::std::remove_reference_t<void>>>::value),
...
public:
  THRUST_HOST unique_eager_future()
      : device_(0)
      , async_signal_()
  {}
  ...
  THRUST_HOST bool valid_content() const noexcept
  {
    if (!valid_stream())
    {
      return false;
  ...
  THRUST_HOST detail::unique_stream const& stream() const
  {
    if (!valid_stream())
    {
      throw thrust::event_error(event_errc::no_state);
  ...
  // Blocks.
  // Precondition: `true == valid_content()`.
  THRUST_NODISCARD THRUST_HOST value_type extract()
  {
    if (!valid_content())
    {
      throw thrust::event_error(event_errc::no_content);
...
template <typename X>
THRUST_HOST optional<unique_stream> try_acquire_stream(int, ready_future<X>&) noexcept
{
  // There's no stream to acquire!
  return {};
}

THRUST_HOST optional<unique_stream> try_acquire_stream(int device_id, unique_eager_event& parent) noexcept
{
...
template <typename X>
THRUST_HOST optional<unique_stream> try_acquire_stream(int device_id, unique_eager_future<X>& parent) noexcept
{
  // We have unique ownership, so we can always steal the stream if the future
  // has one as long as they are on the same device as us.
  if (parent.valid_stream())
...
template <typename... Dependencies>
THRUST_HOST acquired_stream acquire_stream_impl(int, std::tuple<Dependencies...>&, index_sequence<>) noexcept
{
...
template <typename... Dependencies>
THRUST_HOST acquired_stream acquire_stream(int device_id, std::tuple<Dependencies...>& deps) noexcept
{
...
template <typename X>
THRUST_HOST void create_dependency(unique_stream& child, unique_eager_future<X>& parent)
{
  child.depend_on(parent.stream());
}

template <typename... Dependencies>
THRUST_HOST void create_dependencies_impl(acquired_stream&, std::tuple<Dependencies...>&, index_sequence<>)
{}
...
template <typename... Dependencies>
THRUST_HOST void create_dependencies(acquired_stream& as, std::tuple<Dependencies...>& deps)
{
