...
sort(thrust::execution_policy<DerivedPolicy>& exec, RandomAccessIterator first, RandomAccessIterator last)
{
  ...
  thrust::sort(exec, first, last, _THRUST_STD::less<value_type>());
} // end sort()
  ...
  RandomAccessIterator2 values_first)
{
  ...
  thrust::sort_by_key(exec, keys_first, keys_last, values_first, _THRUST_STD::less<value_type>());
} // end sort_by_key()
...
{
  using value_type = thrust::detail::it_value_t<RandomAccessIterator>;
  thrust::stable_sort(exec, first, last, _THRUST_STD::less<value_type>());
} // end stable_sort()
...
{
  using value_type = thrust::detail::it_value_t<RandomAccessIterator1>;
  thrust::stable_sort_by_key(exec, keys_first, keys_last, values_first, _THRUST_STD::less<value_type>());
...
is_sorted_until(thrust::execution_policy<DerivedPolicy>& exec, ForwardIterator first, ForwardIterator last)
{
  ...
  return thrust::is_sorted_until(exec, first, last, _THRUST_STD::less<InputType>());
} // end is_sorted_until()
...
{
  if (_THRUST_STD::distance(first, last) < 2)
  {
    return last;
  }
  ...
  _THRUST_STD::advance(first_plus_one, 1);
