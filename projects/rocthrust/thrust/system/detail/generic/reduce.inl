...
reduce(thrust::execution_policy<ExecutionPolicy>& exec, InputIterator first, InputIterator last, T init)
{
  ...
  return thrust::reduce(exec, first, last, init, _THRUST_STD::plus<T>());
} // end reduce()

template <typename ExecutionPolicy, typename InputIterator, typename T, typename BinaryFunction>
THRUST_HOST_DEVICE T reduce(thrust::execution_policy<ExecutionPolicy>&, InputIterator, InputIterator, T, BinaryFunction)
{
...
THRUST_HOST_DEVICE void reduce_into(
...
{
  // use plus<T> by default
  thrust::reduce_into(exec, first, last, output, init, _THRUST_STD::plus<T>());
} // end reduce_into()
