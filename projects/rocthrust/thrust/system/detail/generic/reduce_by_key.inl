  ...
  BinaryFunction binary_op)
{
  ...
  thrust::transform(
    exec, keys_first, keys_last - 1, keys_first + 1, head_flags.begin() + 1, ::internal::not_fn(binary_pred));
  head_flags[0] = 1;

  // compute tail flags
  thrust::detail::temporary_array<FlagType, ExecutionPolicy> tail_flags(exec, n); // COPY INSTEAD OF TRANSFORM
  thrust::transform(exec, keys_first, keys_last - 1, keys_first + 1, tail_flags.begin(), ::internal::not_fn(binary_pred));
  tail_flags[n - 1] = 1;
