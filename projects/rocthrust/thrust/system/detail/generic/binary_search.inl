...
struct lbf
{
  ...
  THRUST_HOST_DEVICE thrust::detail::it_difference_t<RandomAccessIterator>
  ...
  BinarySearchFunction func)
{
  thrust::for_each(
    ...
    thrust::make_zip_iterator(thrust::make_tuple(values_end, output + _THRUST_STD::distance(values_begin, values_end))),
    detail::binary_search_functor<ForwardIterator, StrictWeakOrdering, BinarySearchFunction>(begin, end, comp, func));

  return output + _THRUST_STD::distance(values_begin, values_end);
}
...
template <typename OutputType,
          ...
          typename StrictWeakOrdering,
          typename BinarySearchFunction>
THRUST_HOST_DEVICE OutputType binary_search(
  thrust::execution_policy<DerivedPolicy>& exec,
  ForwardIterator begin,
  ForwardIterator end,
  const T& value,
...
// this functor differs from _THRUST_STD::less<T>
// because it allows the types of lhs & rhs to differ
// which is required by the binary search functions
// XXX use C++14 _THRUST_STD::less<> when it's ready
struct binary_search_less
{
  template <typename T1, typename T2>
  THRUST_HOST_DEVICE bool operator()(const T1& lhs, const T2& rhs) const
  {
    return lhs < rhs;
  }
};
