...
{
  return thrust::inclusive_scan_by_key(exec, first1, last1, first2, result, _THRUST_STD::equal_to<>());
}
...
{
  return thrust::inclusive_scan_by_key(exec, first1, last1, first2, result, binary_pred, _THRUST_STD::plus<>());
}
  ...
  AssociativeOperator binary_op)
{
  ...
  if (n != 0)
  {
    ...
    flags[0] = 1;
    thrust::transform(exec, first1, last1 - 1, first1 + 1, flags.begin() + 1, _THRUST_STD::not_fn(binary_pred));
...
THRUST_HOST_DEVICE OutputIterator exclusive_scan_by_key(
  ...
  T init)
{
  return thrust::exclusive_scan_by_key(exec, first1, last1, first2, result, init, _THRUST_STD::equal_to<>());
}
...
THRUST_HOST_DEVICE OutputIterator exclusive_scan_by_key(
  ...
  BinaryPredicate binary_pred)
{
  return thrust::exclusive_scan_by_key(exec, first1, last1, first2, result, init, binary_pred, _THRUST_STD::plus<>());
}
  ...
  AssociativeOperator binary_op)
{
  ...
  if (n != 0)
  {
    ...
    thrust::replace_copy_if(
      exec, first2, last2 - 1, flags.begin() + 1, temp.begin() + 1, _THRUST_STD::negate<HeadFlagType>(), init);
    temp[0] = init;
