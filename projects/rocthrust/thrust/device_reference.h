...
public:
   ...
   *  \endcode
   */
  THRUST_HOST_DEVICE explicit device_reference(const pointer& ptr)
      : super_t(ptr)
  {}

  THRUST_HOST_DEVICE const device_reference& operator=(const device_reference& other) const
  {
   ...
   */
  template <typename OtherT>
  THRUST_HOST_DEVICE const device_reference& operator=(const device_reference<OtherT>& other) const
  {
    return super_t::operator=(other);
  }
   ...
   *  \return <tt>*this</tt>
   */
  THRUST_HOST_DEVICE const device_reference& operator=(const value_type& x) const
  {
...
template <typename T>
THRUST_HOST_DEVICE void swap(device_reference<T> x, device_reference<T> y) noexcept(noexcept(x.swap(y)))
{
  x.swap(y);
}
