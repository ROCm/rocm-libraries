...
struct allocator_delete final
{
  ...
  allocator_delete(allocator_delete<U, UAllocator>&& other) noexcept
      : alloc_(_THRUST_STD::move(other.get_allocator()))
  {}
  ...
  {
    using traits = detail::allocator_traits<::internal::remove_cvref_t<Allocator>>;
...
{
  THRUST_HOST_DEVICE tagged_deleter(Lambda&& l)
      : Lambda(THRUST_FWD(l))
  {}
...
template <typename Pointer, typename Lambda>
THRUST_HOST_DEVICE tagged_deleter<Pointer, Lambda> make_tagged_deleter(Lambda&& l)
{
...
THRUST_HOST std::unique_ptr<
  T,
  allocator_delete<
    T,
    typename detail::allocator_traits<::internal::remove_cvref_t<Allocator>>::template rebind_traits<T>::allocator_type>>
allocate_unique(Allocator const& alloc, Args&&... args)
{
  ...
  return std::unique_ptr<T, decltype(deleter)>(hold.release(), _THRUST_STD::move(deleter));
}
