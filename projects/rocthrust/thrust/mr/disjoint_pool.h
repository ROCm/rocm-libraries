...
private:
  ...
  struct oversized_block_descriptor
  {
    ...
    THRUST_HOST_DEVICE bool operator==(const oversized_block_descriptor& other) const
    {
  ...
  {
  public:
    THRUST_HOST_DEVICE equal_pointers(void_ptr p)
        : p(p)
    {}

    THRUST_HOST_DEVICE bool operator()(const oversized_block_descriptor& desc) const
    {
  ...
  {
  public:
    THRUST_HOST_DEVICE matching_alignment(std::size_t requested)
        : requested(requested)
    {}
  ...
  struct pool
  {
    THRUST_HOST pool(const pointer_vector& free)
        : free_blocks(free)
        , previous_allocated_count(0)
    {}

    THRUST_HOST pool(const pool& other)
...
public:
  ...
  THRUST_NODISCARD void_ptr do_allocate_impl(std::size_t bytes, std::size_t alignment)
  {
    ...
    if (bucket.free_blocks.empty())
    {
      ...
      if (n == 0)
      {
        n = _THRUST_STD::max(m_options.min_blocks_per_chunk, //
                              m_options.min_bytes_per_chunk >> bytes_log2);
      }
      else
      {
        n = _THRUST_STD::min({n * 3 / 2, //
  ...
  virtual void do_deallocate(void_ptr p, std::size_t n, std::size_t alignment = THRUST_MR_DEFAULT_ALIGNMENT) override
  {
    ...
    assert(reinterpret_cast<_THRUST_STD::intmax_t>(detail::pointer_traits<void_ptr>::get(p)) % alignment == 0);
