...
public:
   ...
   */
  THRUST_HOST_DEVICE allocator(MR* resource)
      : mem_res(resource)
  {}

  /*! Copy constructor. Copies the resource pointer. */
  template <typename U>
  THRUST_HOST_DEVICE allocator(const allocator<U, MR>& other)
   ...
   */
  THRUST_HOST void deallocate(pointer p, size_type n) noexcept
  {
   ...
   */
  THRUST_HOST_DEVICE MR* resource() const
  {
    return mem_res;
  }
...
THRUST_HOST_DEVICE bool operator==(const allocator<T, MR>& lhs, const allocator<T, MR>& rhs) noexcept
{
...
public:
   ...
   */
  THRUST_EXEC_CHECK_DISABLE
  THRUST_HOST_DEVICE stateless_resource_allocator()
      : base(get_global_resource<Upstream>())
  {}

  /*! Copy constructor. Copies the memory resource pointer. */
  THRUST_HOST_DEVICE stateless_resource_allocator(const stateless_resource_allocator& other)
      : base(other)
  {}

  /*! Conversion constructor from an allocator of a different type. Copies the memory resource pointer. */
  template <typename U>
  THRUST_HOST_DEVICE stateless_resource_allocator(const stateless_resource_allocator<U, Upstream>& other)
      : base(other)
  {}
