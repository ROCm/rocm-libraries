...
class iterator_core_access
{
  ...
  inline THRUST_HOST_DEVICE friend bool
  ...
  THRUST_HOST_DEVICE static typename Facade::reference dereference(Facade const& f)
  {
    return f.dereference();
  }

  template <typename Facade>
  THRUST_HOST_DEVICE static void increment(Facade& f)
  {
    f.increment();
  }

  template <typename Facade>
  THRUST_HOST_DEVICE static void decrement(Facade& f)
  {
    f.decrement();
  }

  template <class Facade1, class Facade2>
  THRUST_HOST_DEVICE static bool equal(Facade1 const& f1, Facade2 const& f2)
  {
    return f1.equal(f2);
  }
  ...
  THRUST_HOST_DEVICE static void advance(Facade& f, typename Facade::difference_type n)
  {
    f.advance(n);
  }
  ...
  distance_from(Facade1 const& f1, Facade2 const& f2)
  {
    ...
    // Facade2 is convertible to Facade1
    return distance_from(f1, f2, typename ::internal::is_convertible<Facade2, Facade1>::type());
  }
  ...
  //
  template <typename Derived, typename Value, typename System, typename Traversal, typename Reference, typename Difference>
  THRUST_HOST_DEVICE static Derived&
  ...
  template <typename Derived, typename Value, typename System, typename Traversal, typename Reference, typename Difference>
  THRUST_HOST_DEVICE static Derived const&
...
template <typename Derived,
          ...
          typename Traversal,
          typename Reference,
          typename Difference = ::internal::ptrdiff_t>
class iterator_facade
{
private:
  /*! \cond
  ...
  // Curiously Recurring Template interface.
  //
  THRUST_HOST_DEVICE Derived& derived()
  {
    return *static_cast<Derived*>(this);
  }

  THRUST_HOST_DEVICE Derived const& derived() const
  {
...
public:
   ...
   */
  using value_type = ::internal::remove_const_t<Value>;
   ...
   */
  THRUST_HOST_DEVICE reference operator*() const
  {
   ...
   */
  THRUST_HOST_DEVICE reference operator[](difference_type n) const
  {
   ...
   *  \return <tt>*this</tt>
   */
  THRUST_HOST_DEVICE Derived& operator++()
  {
   ...
   */
  THRUST_HOST_DEVICE Derived operator++(int)
  {
   ...
   */
  THRUST_HOST_DEVICE Derived& operator+=(difference_type n)
  {
   ...
   */
  THRUST_HOST_DEVICE Derived operator-(difference_type n) const
  {
...
// Iterator addition
template <typename Derived, typename Value, typename System, typename Traversal, typename Reference, typename Difference>
inline THRUST_HOST_DEVICE Derived
