...
// we cannot use _THRUST_STD::integral_constant, because it has a conversion operator to T that causes an ambiguity
...
class THRUST_DECLSPEC_EMPTY_BASES strided_iterator
...
public:
  ...
  static_assert(_THRUST_STD::random_access_iterator<RandomAccessIterator>,
                "The iterator underlying a strided_iterator must be a random access iterator.");
  static_assert(_THRUST_STD::is_same_v<iterator_traversal_t<RandomAccessIterator>, random_access_traversal_tag>);
  static_assert(_THRUST_STD::is_convertible_v<decltype(StrideHolder::value), difference_type>,
  ...
  THRUST_HOST_DEVICE strided_iterator(RandomAccessIterator it, StrideHolder stride = {})
      : super_t(it)
      , StrideHolder(stride)
  {}
  ...
  THRUST_HOST_DEVICE const auto& stride_holder() const
  {
    return static_cast<const StrideHolder&>(*this);
  }

  //! Returns the stride's value
  THRUST_HOST_DEVICE auto stride() const -> difference_type
  {
...
private:
  ...
  THRUST_HOST_DEVICE bool equal(strided_iterator<RandomAccessIterator, OtherStrideHolder> const& other) const
  {
    return this->base() == other.base();
  }

  THRUST_HOST_DEVICE difference_type distance_to(strided_iterator const& other) const
  {
...
THRUST_HOST_DEVICE auto make_strided_iterator(Iterator it, Stride stride)
{
...
THRUST_HOST_DEVICE auto make_strided_iterator(Iterator it)
{
