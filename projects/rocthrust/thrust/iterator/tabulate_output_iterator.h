...
{
public:
  THRUST_HOST_DEVICE tabulate_output_iterator_proxy(BinaryFunction fun, DifferenceT index)
      : fun(fun)
      , index(index)
  {}
  ...
  template <typename T>
  THRUST_HOST_DEVICE tabulate_output_iterator_proxy operator=(const T& x)
  {
    fun(index, x);
    return *this;
  }
...
public:
  ...
  THRUST_HOST_DEVICE tabulate_output_iterator(BinaryFunction fun)
      : fun(fun)
  {}
...
tabulate_output_iterator<BinaryFunction> THRUST_HOST_DEVICE make_tabulate_output_iterator(BinaryFunction fun)
{
