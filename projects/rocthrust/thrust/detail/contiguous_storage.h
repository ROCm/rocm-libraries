...
public:
  ...
  THRUST_HOST_DEVICE explicit contiguous_storage(const allocator_type& alloc = allocator_type());

  THRUST_EXEC_CHECK_DISABLE
  THRUST_HOST_DEVICE explicit contiguous_storage(size_type n, const allocator_type& alloc = allocator_type());

  THRUST_EXEC_CHECK_DISABLE
  THRUST_HOST_DEVICE explicit contiguous_storage(copy_allocator_t, const contiguous_storage& other);
  ...
  THRUST_HOST_DEVICE reference operator[](size_type n);

  THRUST_HOST_DEVICE const_reference operator[](size_type n) const;

  THRUST_HOST_DEVICE allocator_type get_allocator() const;
  ...
  THRUST_HOST_DEVICE void swap(contiguous_storage& other)
  {
    ...
    else if constexpr (!allocator_traits<Alloc>::is_always_equal::value)
    {
      _THRUST_IF_TARGET(_THRUST_IS_DEVICE,
                        ...
                        (if (m_allocator != other.m_allocator) { throw allocator_mismatch_on_swap(); }));
    }
  }
  ...
  THRUST_HOST_DEVICE iterator uninitialized_copy(InputIterator first, InputIterator last, iterator result);

  template <typename System, typename InputIterator>
  THRUST_HOST_DEVICE iterator uninitialized_copy(
    thrust::execution_policy<System>& from_system, InputIterator first, InputIterator last, iterator result);

  template <typename InputIterator, typename Size>
  THRUST_HOST_DEVICE iterator uninitialized_copy_n(InputIterator first, Size n, iterator result);
