...
#else
  using value_type =
    typename thrust::detail::eval_if<::std::is_void<::std::remove_cv_t<::std::remove_reference_t<Element>>>::value,
#endif
                                                      thrust::detail::identity_<void>,
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
                                                      ::cuda::std::remove_cv<Element>>::type;
#else
                                     ::std::remove_cv<Element>>::type;
#endif
...
#else
    typename thrust::detail::eval_if<::std::is_same<Derived, use_default>::value,
#endif
...
#else
    ::std::is_void<::std::remove_cv_t<::std::remove_reference_t<Element>>>::value,
#endif
    thrust::detail::identity_<void>,
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
    thrust::detail::eval_if<::cuda::std::is_same<Reference, use_default>::value,
#else
    thrust::detail::eval_if<::std::is_same<Reference, use_default>::value,
#endif
...
public:
  ...
  THRUST_HOST_DEVICE pointer();

  // NOTE: This is needed so that Thrust smart pointers can be used in
  // `std::unique_ptr`.
  THRUST_HOST_DEVICE pointer(std::nullptr_t);
  ...
  // OtherPointer's system shall be convertible to Tag
  template <typename OtherPointer>
  THRUST_HOST_DEVICE explicit pointer(
    const OtherPointer& other,
  ...
  // NOTE: This is needed so that Thrust smart pointers can be used in
  // `std::unique_ptr`.
  THRUST_HOST_DEVICE derived_type& operator=(std::nullptr_t);
  ...
  THRUST_HOST_DEVICE Element* get() const;

  THRUST_HOST_DEVICE Element* operator->() const;

  // NOTE: This is needed so that Thrust smart pointers can be used in
  // `std::unique_ptr`.
  THRUST_HOST_DEVICE explicit operator bool() const;

  THRUST_HOST_DEVICE static derived_type
