...
public:
  ...
  THRUST_HOST_DEVICE std::uint64_t size() const
  {
...
private:
  ...
  static_assert(_THRUST_STD::is_convertible_v<IndexType, typename Bijection::index_type>,
...
public:
  ...
  template <class URBG>
  THRUST_HOST_DEVICE random_bijection(IndexType n, URBG&& g)
      : bijection(n, _THRUST_STD::forward<URBG>(g))
      , n(n)
  {}

  THRUST_HOST_DEVICE IndexType operator()(IndexType i) const
  {
  ...
  THRUST_HOST_DEVICE IndexType size() const
  {
    return n;
  }
};
