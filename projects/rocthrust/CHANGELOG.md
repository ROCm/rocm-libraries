...
* Added `enum_devices.cpp` to the test directory and built as a new target by CMake. It generates the resource spec file required by CTest when running tests in parallel.
...
* Removed the `GenerateResourceSpec.cmake` script - it is replaced by the added `enum_devices.cpp` code above.
