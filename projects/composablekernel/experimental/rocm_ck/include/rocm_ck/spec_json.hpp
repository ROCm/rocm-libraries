...
//
// Runtime to_json() functions for GemmSpec and ElementwiseSpec.
...
inline std::string tensor_name_str(const TensorName& tn)
{
    return std::string(tn.data, static_cast<std::size_t>(tn.len));
}

inline std::string quoted(const char* s) { return std::string("\"") + s + "\""; }
...
/// Serialize an elementwise variant to JSON for the build-time extractor.
inline std::string to_json(const char* name, const ElementwiseSpec& spec, TargetSet targets)
{
    std::string j;
    j += "{\n";
    j += "  \"name\": " + detail::quoted(name) + ",\n";
    j += "  \"spec_type\": \"ElementwiseSpec\",\n";
    j += "  \"targets\": " + detail::targets_json(targets) + ",\n";
    j += "  \"spec\": {\n";
    ...
    j += "    \"block_tile\": " + std::to_string(spec.block_tile) + ",\n";
    j += "    \"workgroup_size\": " + std::to_string(spec.workgroup_size) + ",\n";
    j += "    \"block_waves\": " + std::to_string(spec.block_waves) + ",\n";
    j += "    \"wave_tile\": " + std::to_string(spec.wave_tile) + ",\n";
    j += "    \"pad\": " + std::string(spec.pad ? "true" : "false") + "\n";
