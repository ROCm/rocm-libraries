...
{
    // CK Tile's KernelComponentFactoryGfx950 reuses the gfx9 rows verbatim for
    // non-trload, across every supported symmetric head dim.
...
{
    // The TrLoad-eligibility tag is the single source of truth for the
    // gfx950-vs-gfx9 distinction: only gfx950 may request tr_load == true.
    ...
    // It is a refinement of the gfx9 family.
...
{
    // The default tr_load argument is false, so gfx950 resolves to the shared
    // gfx9 non-trload table whether or not the caller passes it explicitly.
