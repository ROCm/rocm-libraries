...
class TestSoCacheAbiKey:
    ...
    def test_abi_is_versioned(self):
        # _SO_ABI is a plain literal, not derived at import time: a regex over the
        # ctypes source could not read multi-token or namespaced return types and
        # dropped those exports silently, which froze the cache key instead of
        # moving it. test_so_abi_token.py does the parse where it can fail loudly
        # and pins this literal to the export set. All that matters here is that it
        # is a non-empty, filename-safe token.
