...
def _run_one(label: str, config, M: int, N: int, K: int,
    ...
    C_ref = _reference_gemm(A_f32, B_f32, BQ, problem, c_dtype)
    ...
    result_timed = runner.run(A=A_raw, B=B_raw, BQ=BQ, problem=problem,
                              c_dtype=c_dtype)
    # BQuantGpuGemmRunner.run() always requests timing internally via BQuantDispatcherLib.run()
    # time_ms=0 would indicate the GPU timer was not engaged.
...
def _make_bf16_inputs(M, N, K, gK, gN, seed=42):
    ...
    QK_B = math.ceil(K / gK)
    QN_B = math.ceil(N / gN)
    A = rng.uniform(-1.0, 1.0, (M, K)).astype(np.float32).astype(np.float32)
    B = rng.uniform(-1.0, 1.0, (K, N)).astype(np.float32).astype(np.float32)
    BQ = rng.uniform(0.5, 1.5, (QK_B, QN_B)).astype(np.float32)
    # bf16 round-trip for reference accuracy
    A_bf16 = A.astype(np.float32)
    B_bf16 = B.astype(np.float32)
    # Numpy doesn't have bf16 natively; pass as uint16 (bfloat16 view)
    # The ctypes lib receives void* and casts to bf16_t, so we pass a float32 view
    # scaled to bf16 precision via a round-trip through np.float32 truncation.
    import struct
    def to_bf16_raw(x):
        # Pack as float32, take upper 2 bytes (bf16 bit pattern), unpack as uint16
        packed = np.frombuffer(x.astype(np.float32).tobytes(), dtype=np.uint16)
        # bf16 is upper 16 bits of float32 (little-endian: indices 1,3,5,...)
        return packed[1::2].reshape(x.shape)
    A_raw = to_bf16_raw(A_bf16)
    B_raw = to_bf16_raw(B_bf16)
    # Decode bf16 back to float32 for reference
    def from_bf16_raw(x):
        u16 = x.flatten()
        # Interleave with zeros: bf16 uint16 → float32 by padding lower 2 bytes
        words = np.zeros(len(u16) * 2, dtype=np.uint16)
        words[1::2] = u16
        return words.view(np.float32).reshape(x.shape)
    A_dec = from_bf16_raw(A_raw)
    B_dec = from_bf16_raw(B_raw)
    return A_raw, A_dec, B_raw, B_dec, BQ
...
def test_h3_mx_bf16bf16(out_dir: Path, gfx_arch: str) -> tuple[str, str]:
    ...
    A_raw, A_dec, B_raw, B_dec, BQ = _make_bf16_inputs(M, N, K, gK, gN)
    return _run_one("H3/mx_bf16bf16", cfg, M, N, K,
                    A_raw, A_dec, B_raw, B_dec, BQ,
                    out_dir, c_dtype=np.float32, gfx_arch=gfx_arch)
...
def test_h3_mx_bf16bf8(out_dir: Path, gfx_arch: str) -> tuple[str, str]:
    ...
    A_raw, A_dec, B_raw_bf8, B_dec, BQ = _make_bf16_inputs(M, N, K, gK, gN)
    ...
    return _run_one("H3/mx_bf16bf8", cfg, M, N, K,
                    A_raw, A_dec, B_raw_bf8, B_dec_bf8, BQ,
                    out_dir, c_dtype=np.float32, gfx_arch=gfx_arch)
...
def test_h3_mx_bf16fp4(out_dir: Path, gfx_arch: str) -> tuple[str, str]:
    ...
    A_raw, A_dec, _, _, BQ = _make_bf16_inputs(M, N, K, gK, gN)
    ...
    return _run_one("H3/mx_bf16fp4", cfg, M, N, K,
                    A_raw, A_dec, B_raw, B_f32_approx, BQ,
                    out_dir, c_dtype=np.float32, gfx_arch=gfx_arch)
