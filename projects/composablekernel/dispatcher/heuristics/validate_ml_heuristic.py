...
def validate_ml_heuristic(dtype: str, layout: str, model_dir: str, data_dir: str):
    ...
    dtype_specific = Path(data_dir) / f'{dtype}_original' / f'{dtype}_training_data.parquet'
    ...
    if dtype_specific.exists():
        ...
        training_data = training_data[(training_data['dtype'] == dtype) & (training_data['layout'] == layout)]
            ...
            if ranked:
                ...
                if len(ml_kernel_row) > 0:
                    ...
                    if (shape_idx + 1) % 20 == 0:
                        ...
                        print(f"  [{shape_idx + 1:3d}/{len(shape_groups)}] "
    ...
    if results:
        ...
        print()
        print(f"Oracle-best matches: {oracle_matches}/{len(results)} ({100*oracle_matches/len(results):.1f}%)")
        print()

        # Classify by M size
        df_results['m_class'] = pd.cut(df_results['m'],
                                       bins=[0, 8, 128, 1024, float('inf')],
                                       labels=['Tiny (M<8)', 'Small (8≤M<128)',
                                              'Medium (128≤M<1024)', 'Large (M≥1024)'])

        print("Efficiency by M size:")
        for m_class in ['Tiny (M<8)', 'Small (8≤M<128)', 'Medium (128≤M<1024)', 'Large (M≥1024)']:
            subset = df_results[df_results['m_class'] == m_class]
            if len(subset) > 0:
                print(f"  {m_class:25s}: {subset['efficiency_pct'].mean():6.2f}% "
        ...
        top5 = df_results.nlargest(5, 'efficiency_pct')[['m', 'n', 'k', 'efficiency_pct', 'oracle_best_tflops', 'is_oracle_best']]
        for idx, row in top5.iterrows():
            match = "✓" if row['is_oracle_best'] else " "
            print(f"  {match} M={row['m']:5d} N={row['n']:5d} K={row['k']:5d}: "
                  f"{row['efficiency_pct']:.2f}% ({row['oracle_best_tflops']:.2f} TFLOPS)")

        print()
        print("Bottom 5 shapes (worst efficiency):")
        bottom5 = df_results.nsmallest(5, 'efficiency_pct')[['m', 'n', 'k', 'efficiency_pct', 'oracle_best_tflops', 'is_oracle_best']]
...
def main():
    ...
    parser.add_argument('--layout', default='rcr', choices=['rcr', 'rrr', 'crr', 'ccr'],
                        help='Matrix layout')
    ...
    if args.model_dir is None:
        ...
        if args.model_dir is None:
            ...
            print(f"   Please specify --model_dir explicitly")
            return 1
