...
def model_dir(tmp_path):
    ...
    spec = {"feature_names": fe.get_feature_names(), "categorical_features": fe.get_categorical_features()}
...
class TestRandomSearch:
    ...
    def test_different_problems_produce_results(self, predictor):
        ...
        r1 = searcher.search({"m": 16, "n": 1536, "k": 7168, "dtype": "fp8", "layout": "rcr", "split_k": 1}, budget=50, top_k=3)
        searcher2 = SurrogateSearch(predictor, strategy="random", seed=42)
        r2 = searcher2.search({"m": 20480, "n": 7168, "k": 256, "dtype": "fp8", "layout": "rcr", "split_k": 1}, budget=50, top_k=3)
        assert len(r1) > 0
        assert len(r2) > 0
    ...
    def test_m1_corner_case(self, predictor):
        ...
        results = searcher.search(
            {"m": 1, "n": 4096, "k": 4096, "dtype": "fp8", "layout": "rcr", "split_k": 1},
            budget=50, top_k=5,
        )
        assert len(results) > 0
