...
def load_kernel_list(build_dir: Path, dtype: str, layout: str) -> List[KernelConfig]:
    ...
    kernel_list_path = build_dir / "tile_engine" / "ops" / "gemm" / "gemm_universal" / dtype / layout / "gemm_universal_kernel_list.txt"
...
def run_benchmark(
    ...
    try:
        ...
        if json_match:
            ...
            time_match = re.search(r'(?:avg[_\s]?time|latency)[:\s]+(\d+\.?\d*)\s*(?:ms)?', output, re.I)
            if time_match:
...
def main():
    parser = argparse.ArgumentParser(description='Generate GEMM benchmark data for ML training')
    parser.add_argument('--build_dir', type=str, default='/tmp/build',
                        help='CK build directory')
    parser.add_argument('--output_dir', type=str, default='/tmp/benchmark_data',
    ...
    parser.add_argument('--num_build_jobs', type=int, default=4,
                        help='Number of parallel build jobs')
    parser.add_argument('--num_benchmark_jobs', type=int, default=1,
                        help='Number of parallel benchmark jobs (use 1 for accurate timing)')
    parser.add_argument('--max_kernels', type=int, default=None,
        ...
        with ProcessPoolExecutor(max_workers=args.num_build_jobs) as executor:
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                if success:
                    ...
                    build_results['failed_kernels'].append({'name': kernel_name, 'error': error})
                
                if (i + 1) % 10 == 0:
                    print(f"  Built {i + 1}/{len(kernels)} ({build_results['success']} success, {build_results['failed']} failed)")
    ...
    for kernel in kernels:
        ...
        intermediate_file = output_dir / f'benchmark_results_{args.dtype}_{args.layout}_partial.json'
    ...
    # Print summary
    valid_count = sum(
        1 for kr in all_results 
        for br in kr['benchmarks'] 
        if br['is_valid']
    )
