...
class MLHeuristic
{
    ...
    double predict_tflops(const Problem& prob, const KernelKey& key) const
    {
        ...
        int64_t ol  = 0;
        double pred = 0;
        // data_type=1 == C_API_DTYPE_FLOAT64 (f is std::array<double>); passing
        // 0 (FLOAT32) makes LightGBM reinterpret the double bytes as floats and
        // return a near-constant garbage prediction.
