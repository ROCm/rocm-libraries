...
// For packed types PackedSize=2; for all other types PackedSize=1. TensorQuant
// only supports fp8/bf8 (PackedSize=1) but we keep this helper generic.
template <typename T>
                                     ...
                                     float* time_ms)
{
    ...
    if(hipMalloc(&AQ_dev, elements_to_bytes<QDataType>(1)) != hipSuccess)
    {
        cleanup();
        return -1;
    }
    if(hipMalloc(&BQ_dev, elements_to_bytes<QDataType>(1)) != hipSuccess)
    {
        cleanup();
        return -1;
    }
    ...
    if(hipMemcpy(AQ_dev, AQ_host, elements_to_bytes<QDataType>(1), hipMemcpyHostToDevice) !=
       hipSuccess)
    {
        cleanup();
        return -1;
    }
    if(hipMemcpy(BQ_dev, BQ_host, elements_to_bytes<QDataType>(1), hipMemcpyHostToDevice) !=
       hipSuccess)
    {
        cleanup();
        return -1;
