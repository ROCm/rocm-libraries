 ...
 */
int dispatcher_run_grouped_abquant_gemm(const void* A,
                                        const void* B,
                                        const void* AQ,
                                        const void* BQ,
                                        void* C,
                                        ...
                                        float* time_ms)
{
    ...
    const char* kFn = "dispatcher_run_grouped_abquant_gemm";
    ...
    return launch_and_copyback<SelectedKernel, CDataType>(
        kFn, args, C, C_dev, static_cast<std::size_t>(M) * static_cast<std::size_t>(N), time_ms);
}
