...
Naming convention (byte-exact with ABQuantKernelConfig.name in gemm_abquant_utils.py):
    ...
    {PadM}_{PadN}_{PadK}_{APreshuffle}_{BPreshuffle}_gsn{GroupN}_{tile}

Reference:
    tile_engine/ops/gemm/block_scale_gemm/gemm_abquant/gemm_abquant_instance_builder.py
"""
...
from codegen_common import make_abquant_kernel_name, BQUANT_DTYPE_MAP
...
# AQDataType and BQDataType are always float32 per gemm_abquant_instance_builder.py
# =============================================================================
...
# ABQuant only supports compv3 pipeline (from gemm_abquant_instance_builder.py)
...
class ABQuantKernelSpec:
    ...
    quant_group_k: int = 128   # A and B K-group size
    quant_group_n: int = 1     # B N-group size (always included in name)
    preshuffle_a: bool = False  # APreshuffleQuant
    preshuffle_b: bool = False  # BPreshuffleQuant
    pad_m: bool = False
    pad_n: bool = False
    pad_k: bool = True
    block_size: int = 256
    ...
    def name(self) -> str:
        ...
        return make_abquant_kernel_name(
            ...
            warp_tile_m=t.warp_tile_m, warp_tile_n=t.warp_tile_n,
...
class ABQuantKernelHeaderGenerator:
    def generate(self, spec: ABQuantKernelSpec) -> str:
        ...
        # AQ is always RowMajor; BQ is always ColumnMajor per gemm_abquant_instance_builder.py
        ...
        preshuffle_a = str(spec.preshuffle_a).lower()
        preshuffle_b = str(spec.preshuffle_b).lower()

        return f"""\
// SPDX-License-Identifier: MIT
// Auto-generated ABQuant GEMM kernel header.
// DO NOT EDIT — regenerate via unified_gemm_abquant_codegen.py
#pragma once
...
struct {struct} {{
    ...
    static constexpr ck_tile::index_t GroupSizeN    = {spec.quant_group_n};

    static constexpr bool kPadM            = {pad_m};
    static constexpr bool kPadN            = {pad_n};
    static constexpr bool kPadK            = {pad_k};
    static constexpr bool APreshuffleQuant = {preshuffle_a};
    static constexpr bool BPreshuffleQuant = {preshuffle_b};
    ...
    using GemmPipelineProblem = ck_tile::GemmPipelineProblemBase<
        ADataType, BDataType, AccDataType, TileShape, Traits, BDataType>;
...
constexpr ck_tile::index_t GroupSizeN = {ns}::{struct}::GroupSizeN;
...
def _build_specs(config: dict) -> List[ABQuantKernelSpec]:
    ...
    # quant_group_n can be a single value or a list for sweep
    qgn_raw = config.get("quant_group_n", 1)
    quant_group_n_vals = qgn_raw if isinstance(qgn_raw, list) else [qgn_raw]

    # preshuffle flags can be bool or list for sweep
    pqa_raw = config.get("preshuffle_a", False)
    pqa_vals = pqa_raw if isinstance(pqa_raw, list) else [pqa_raw]
    pqb_raw = config.get("preshuffle_b", False)
    pqb_vals = pqb_raw if isinstance(pqb_raw, list) else [pqb_raw]

    for variant_key, layout, tile_dict, qgn, pqa, pqb in itertools.product(
...
def generate_kernels(
    ...
    if not specs:
        log.warning("No ABQuant kernel specs produced")
        return []
...
def main() -> int:
    parser = argparse.ArgumentParser(description="ABQuant GEMM kernel header generator")
