...
class ABQuantKernelHeaderGenerator:
    ...
    def generate(self, spec: ABQuantKernelSpec) -> str:
        ...
        return f"""\
// SPDX-License-Identifier: MIT
// Auto-generated ABQuant (A+B block-scale) GEMM kernel header.
...
constexpr ck_tile::index_t AGroupSizeK = {ns}::{struct}::AGroupSizeK;
constexpr ck_tile::index_t BGroupSizeK = {ns}::{struct}::BGroupSizeK;
constexpr ck_tile::index_t BGroupSizeN = {ns}::{struct}::BGroupSizeN;
...
def generate_kernels(
    ...
    """Generate all ABQuant kernel headers into output_dir.
