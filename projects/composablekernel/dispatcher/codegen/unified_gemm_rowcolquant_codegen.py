...
def generate_kernels(
    ...
    """Generate all RowColQuant kernel headers into output_dir.
    ...
    log.info("Generating %d RowColQuant kernel headers into %s", len(specs), output_dir)

    gen = RowColQuantKernelHeaderGenerator()
    generated: List[Path] = []

    def _generate_one(spec: RowColQuantKernelSpec) -> Path:
