...
"""
RowColQuant GEMM dispatcher utilities.

Three-layer Python bridge for row/col quantized GEMM:
  RowColQuantKernelConfig  — describes one kernel; .name is byte-exact with codegen
  RowColQuantDispatcherLib — thin ctypes wrapper
  RowColQuantGpuGemmRunner — high-level runner

HostArgs mapping (QuantGemmHostArgs):
  aq_ptr    = A row-scale vector [M], float32; QK_A=1, stride_AQ=1
  bq_ptr    = B col-scale vector [N], float32; QK_B=1, stride_BQ=N
"""
...
class RowColQuantKernelConfig:
    ...
    def name(self) -> str:
        return make_rowcolquant_kernel_name(
            ...
            persistent=False,  # always False for RowColQuant
        )
...
class RowColQuantGemmProblem:
    ...
    # RowColQuant: one scale per row (A) and per col (B)
    @property
    def QK_A(self) -> int:
        return 1
...
class RowColQuantDispatcherLib:
    ...
    int dispatcher_run_rowcolquant_gemm(A, AQ, B, BQ, C,
    ...
    def run(self, A, AQ, B, BQ, C, M, N, K, stride_A, stride_B, stride_C,
            k_batch=1) -> Tuple[int, float]:
        import numpy as np
...
class RowColQuantGpuGemmRunner:
    ...
    def run(self, A, AQ, B, BQ, problem: RowColQuantGemmProblem, c_dtype=None) -> RowColQuantGemmResult:
        """
        Run RowColQuant GEMM.

        A   shape (M, K), row-major, dtype: fp8/bf8
        AQ  shape (M,), float32 — per-row A scale (one value per row)
        B   shape (K, N), col-major, dtype: fp8/bf8
        BQ  shape (N,), float32 — per-col B scale (one value per col)
        """
        import numpy as np
        ...
        stride_A = K   # A row-major
        stride_B = K   # B col-major: leading dim = K
        stride_C = N   # C row-major

        rc, time_ms = self._lib.run(
            A=A, AQ=AQ, B=B, BQ=BQ, C=C,
            M=M, N=N, K=K,
...
def _generate_rowcolquant_kernel(config: RowColQuantKernelConfig, output_dir: Path) -> Optional[Path]:
...
def _compile_rowcolquant_kernel(hpp_path, so_path, gfx_arch, hipcc=_DEFAULT_HIPCC,
...
def setup_multiple_rowcolquant_dispatchers(
    ...
    def _build_one(idx, cfg):
        ...
        if so.exists():
            return idx, so
        ok = _compile_rowcolquant_kernel(hpp, so, arch, hipcc, extra_include_dirs)
...
def default_fp8_config(gfx_arch: str = _DEFAULT_GFX_ARCH) -> RowColQuantKernelConfig:
    """Default fp8 RowColQuant config (GemmConfigQuantDecode<fp8_t>).
...
def default_bf8_config(gfx_arch: str = _DEFAULT_GFX_ARCH) -> RowColQuantKernelConfig:
    """Default bf8 RowColQuant config."""
    return RowColQuantKernelConfig(
