...
"""
AQuant GEMM dispatcher utilities.

Three-layer Python bridge for the dispatcher's AQuant GEMM path
(A-matrix quantized, B unquantized):
  ...
  AQuantGpuGemmRunner      -- high-level runner: accepts numpy arrays, calls the kernel

Build helper (self-contained):
  setup_multiple_aquant_dispatchers(configs, ...) : codegen -> hipcc -> .so paths

HostArgs mapping (QuantGemmHostArgs):
  aq_ptr    = A scale tensor [M, QK_A], float32, row-major
  bq_ptr    = nullptr (unused for AQuant)
  QK_A      = ceil(K / quant_group_k)
  QK_B      = 0
  stride_AQ = QK_A (row-major)
  stride_BQ = 0
"""
...
class AQuantKernelConfig:
    """Complete description of one AQuant GEMM kernel.
...
class AQuantDispatcherLib:
    ...
    Expected .so exports:
      ...
      int  dispatcher_run_aquant_gemm(A, AQ, B, C,
                                       M, N, K,
    ...
    def run(
        ...
        # B is col-major (rcr layout): stride_B=K means leading dim = K.
        # asfortranarray ensures the right memory layout for 2D B.
...
class AQuantGpuGemmRunner:
    ...
    def run(self, A, AQ, B, problem: AQuantGemmProblem, c_dtype=None) -> AQuantGemmResult:
        ...
        A   shape: (M, K) or packed, dtype: fp8/bf8/pk_int4
        AQ  shape: (M, QK_A), dtype: float32 — per-row A scale factors
        B   shape: (K, N) col-major, dtype: fp8/bf8/pk_int4
        c_dtype: numpy dtype for output C (defaults to np.float16)
        """
        import numpy as np
        ...
        # Strides: A row-major [M,K] -> stride=K; AQ row-major [M,QK_A] -> stride=QK_A;
        # B col-major [K,N] -> stride=K; C row-major [M,N] -> stride=N
        stride_A  = K
        stride_AQ = QK_A
        stride_B  = K
        stride_C  = N
...
def _generate_aquant_kernel(config: AQuantKernelConfig, output_dir: Path) -> Optional[Path]:
    """Run unified_gemm_aquant_codegen.py for one config; return .hpp path or None."""
...
def _compile_aquant_kernel(
    ...
    """Compile a generated .hpp into a .so via hipcc (two-step: compile then link)."""
...
# =============================================================================
# Default configs (matching GemmConfigQuantDecodeInterwave tile from gemm_utils.hpp)
# =============================================================================
...
def default_fp8_config(
    ...
    """Default fp8 AQuant config (GemmConfigQuantDecodeInterwave<fp8_t>).

    WarpTileK=128: get_k_warp_tile<fp8_t, 16>() on gfx950 returns 128.
    """
...
def default_fp8i4_config(
    ...
    """Default fp8i4 AQuant config (A=fp8, B=pk_int4).

    WarpTileK=16: pk_int4 is not an 8-bit float type -> get_k_warp_tile returns 16 on gfx950.
    """
    return AQuantKernelConfig(
        variant_key="fp8i4",
        layout="rcr",
...
def default_bf8i4_config(
    ...
    """Default bf8i4 AQuant config (A=bf8, B=pk_int4)."""
    return AQuantKernelConfig(
        variant_key="bf8i4",
        layout="rcr",
        pipeline="compv3",
