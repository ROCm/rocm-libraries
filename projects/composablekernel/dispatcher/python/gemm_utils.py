...
def _ctypes_source_name(config: GemmKernelConfig) -> str:
    ...
    The grouped kernel has a multi-problem launch signature, and the multi_d
    kernel fuses extra D operands and exposes a GemmMultiDArgs launch signature;
    neither is expressible by the single-problem ``gemm_ctypes_lib.cpp``, so each
    compiles against its dedicated ctypes source.
    """
...
def expand_sweep(
    ...
    arguments (defaults to fp16 / rcr / standard) and is applied to every emitted
