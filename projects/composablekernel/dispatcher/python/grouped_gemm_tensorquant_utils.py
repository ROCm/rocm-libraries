...
class TensorQuantDispatcherLib:
    ...
    Expected .so exports:
      ...
      int  dispatcher_run_grouped_tensorquant_gemm(A, B, AQ, BQ, C,
                               M, N, K,
    ...
    def _setup(self):
        ...
        lib.dispatcher_run_grouped_tensorquant_gemm.restype  = ctypes.c_int
        lib.dispatcher_run_grouped_tensorquant_gemm.argtypes = [
            ctypes.c_void_p,   # A
            ctypes.c_void_p,   # B
            ctypes.c_void_p,   # AQ
            ctypes.c_void_p,   # BQ
    ...
    def run(
        ...
        """Call dispatcher_run_grouped_tensorquant_gemm with ctypes-wrapped pointers.
        ...
        rc = self._lib.dispatcher_run_grouped_tensorquant_gemm(
...
class TensorQuantGpuGemmRunner:
    ...
    def run(self, A, B, AQ, BQ, problem: TensorQuantGemmProblem, c_dtype=None) -> TensorQuantGemmResult:
        ...
        if problem.k_batch != 1:
            raise ValueError(
                f"k_batch={problem.k_batch} is not supported by the grouped_gemm_tensorquant "
        ...
        if rc != 0:
            raise RuntimeError(
                f"dispatcher_run_grouped_tensorquant_gemm failed with code {rc} "
...
def setup_multiple_tensorquant_dispatchers(
    ...
    def _compile_fn(hpp: Path, so: Path, a: str) -> bool:
        return _compile_tensorquant_kernel(
    ...
    return build_dispatchers(
        configs,
        arch=arch,
        tmp_prefix="tensorquant_dispatcher_",
        log_label="TensorQuant",
        generate_fn=_generate_tensorquant_kernel,
