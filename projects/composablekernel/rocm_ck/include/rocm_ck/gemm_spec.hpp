...
///
/// TODO(#7280): consolidate with ck_common::Pipeline (Mem, CompV1..V5,
/// PreShuffleV1/V2). The two enumerations cover overlapping but not
/// identical use cases -- pt.2 should extend ck_common::Pipeline to cover
/// the rocm_ck strategies and migrate this enum to a thin alias.
enum class Pipeline
{
    V1,
    V3,
...
/// Convenience overload: build a single-target TargetSet from a GpuTarget.
/// Provided because TargetSet(GpuTarget) is explicit (to prevent silent
/// conversions in mixed comparisons), but a single-target makeSpec call is
/// common and self-evident at the call site.
consteval GemmSpec makeSpec(const Signature& sig, const GemmAlgorithm& algo, GpuTarget target)
{
    return makeSpec(sig, algo, TargetSet::of(target));
}
