...
inline __host__ __device__ constexpr bhalf_t type_convert<bhalf_t, float>(float x)
{
    ...
    u.fp32 = x;
    return bit_cast<bhalf_t>(uint16_t(static_cast<uint32_t>(u.int32) >> 16));
#endif
}
...
#endif // defined(__gfx950__) || defined(__gfx125__) || defined(__gfx13__)
...
#if defined(__gfx120__) || defined(__gfx13__)
...
#endif // defined(__gfx120__) || defined(__gfx13__)
#else
...
template <>
inline __host__ __device__ f8_fnuz_t type_convert<f8_fnuz_t, bf8_fnuz_t>(bf8_fnuz_t x)
{
    float fvalue = type_convert<float, bf8_fnuz_t>(x);
    return type_convert<f8_fnuz_t, float>(fvalue);
}

template <>
inline __host__ __device__ bf8_fnuz_t type_convert<bf8_fnuz_t, f8_fnuz_t>(f8_fnuz_t x)
{
    float fvalue = type_convert<float, f8_fnuz_t>(x);
    return type_convert<bf8_fnuz_t, float>(fvalue);
}
 ...
 * formats when hardware intrinsics are unavailable (CK_MX_TO_FP6_CVT_FAST_PATH = 0). It supports
 ...
 * formats when hardware intrinsics are unavailable (CK_MX_FROM_FP6_CVT_FAST_PATH = 0).
 *
