...
#if(defined(__gfx950__) || defined(__gfx12__) || defined(__gfx13__)) && __HIP_DEVICE_COMPILE__
...
static __device__ fp8x2_storage_t cast_to_f8_from_f16(half2_t v, unsigned int rng = 0)
{
    ...
    if constexpr(saturate)
    {
        if((val.i16_vec[0] & 0x7C00) != 0x7C00)
        {
            if constexpr(interpret == ck_fp8_interpretation_t::CK_E4M3_FNUZ)
            {
                val.vhalf[0] = __builtin_amdgcn_fmed3h(val.vhalf[0], 240.0, -240.0);
            }
            else if constexpr(interpret == ck_fp8_interpretation_t::CK_E4M3_OCP)
            {
                val.vhalf[0] = __builtin_amdgcn_fmed3h(val.vhalf[0], 448.0, -448.0);
            }
            else
            {
                val.vhalf[0] = __builtin_amdgcn_fmed3h(val.vhalf[0], 57344.0, -57344.0);
            }
        }
        if((val.i16_vec[1] & 0x7C00) != 0x7C00)
        {
            if constexpr(interpret == ck_fp8_interpretation_t::CK_E4M3_FNUZ)
            {
                val.vhalf[1] = __builtin_amdgcn_fmed3h(val.vhalf[1], 240.0, -240.0);
            }
            else if constexpr(interpret == ck_fp8_interpretation_t::CK_E4M3_OCP)
            {
                val.vhalf[1] = __builtin_amdgcn_fmed3h(val.vhalf[1], 448.0, -448.0);
            }
            else
            {
                val.vhalf[1] = __builtin_amdgcn_fmed3h(val.vhalf[1], 57344.0, -57344.0);
            }
        }
    }
    ...
    out.vi16 = (interpret == ck_fp8_interpretation_t::CK_E4M3_FNUZ ||
                interpret == ck_fp8_interpretation_t::CK_E4M3_OCP)
                   ? __builtin_amdgcn_cvt_pk_fp8_f16(val.vhalf)
                   : __builtin_amdgcn_cvt_pk_bf8_f16(val.vhalf);
...
template <ck_fp8_interpretation_t interp,
          ...
          bool stochastic_rounding = false>
#if CK_FP8_CVT_FAST_PATH || CK_USE_OCP_FP8
__host__ __device__ static inline fp8_storage_t cvt_bhalf_t_to_fp8(const __bf16 x)
#else
__host__ static inline fp8_storage_t cvt_bhalf_t_to_fp8(const __bf16 x)
#endif
{
    {
        __is_interpret_supported(interp);
        ...
        ignore = rng;
        return cvt_float_to_fp8<interp, ck_saturation_t::CK_SATFINITE, stochastic_rounding>(
            bit_cast<float>(uint32_t{bit_cast<ushort>(x)} << 16)); // convert value to float
...
template <ck_fp8_interpretation_t interp,
          ...
          bool stochastic_rounding = false>
#if CK_FP8_CVT_FAST_PATH || CK_USE_OCP_FP8
__host__ __device__ static inline fp8x2_storage_t cvt_bhalf_t_to_fp8(const bf16x2_t x)
#else
__host__ static inline fp8x2_storage_t cvt_bhalf_t_to_fp8(const bf16x2_t x)
#endif
{
#if CK_WORKAROUND_BF16_TO_FP8_CONVERSION
    return cvt_float_to_fp8<interp, ck_saturation_t::CK_SATFINITE, stochastic_rounding>(float2_t{
        bit_cast<float>(uint32_t{bit_cast<ushort>(x[0])} << 16),
        bit_cast<float>(uint32_t{bit_cast<ushort>(x[1])} << 16)}); // convert values to float
#else // CK_WORKAROUND_BF16_TO_FP8_CONVERSION
    {
        ...
        ignore = rng;
        return cvt_float_to_fp8<interp, ck_saturation_t::CK_SATFINITE, stochastic_rounding>(
            float2_t{bit_cast<float>(uint32_t{bit_cast<ushort>(x[0])} << 16),
                     bit_cast<float>(uint32_t{bit_cast<ushort>(x[1])}
                                     << 16)}); // convert values to float
#endif // defined(__gfx950__)
    }
#endif // CK_WORKAROUND_BF16_TO_FP8_CONVERSION
}

#else // (__clang_major__ < 21) where there is no gfx13 support
