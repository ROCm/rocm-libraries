                ...
                for(index_t i_ytilde = 0; i_ytilde < YTilde; ++i_ytilde)
                {
                    for(index_t i_xtilde = 0; i_xtilde < XTilde; ++i_xtilde)
                    {
                        ...
                        if constexpr(NDimSpatial == 2 && !CTranspose)
                        {
                            ...
                            // N must not be split: the packed descriptors describe the
                            // whole batch, so when the batch is chunked across
                            // workgroups the flat path reads the wrong slice. This is
                            // the same quantity assigned to num_workgroups_per_Conv_N_
                            // below, written inline because that member is not yet set
