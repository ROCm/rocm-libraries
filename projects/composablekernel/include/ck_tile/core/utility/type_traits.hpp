...
// Usage: if_select_v<T, int, float, double> evaluates to float if T==int, else double
template <typename Cond, typename Match, typename TrueType, typename FalseType>
using if_select_v = std::conditional_t<std::is_same_v<Cond, Match>, TrueType, FalseType>;
