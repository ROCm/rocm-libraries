...
struct SpargeBlockMapPipeline
{
                                   ...
                                   bool attention_sink) const
    {
        ...
        // CLI/trek (which include 01_fmha/mask.hpp) decode
        // mask_enum::mask_top_left -> bool, so this header avoids that header.
        // Multiplicative causal form below handles BLKQ=64,BLKK=128 (kM0<kN0)
        // and the kM0>=kN0 case uniformly.
        ...
        if(!sim_q)
        {
            // Only fill causal-valid prefix when active.
            ...
            // Sink force. Under top-left causal, kb=0 always causal-valid for
            // qb>=0 -> no-op; meaningful for mask=no + sink=1.
        ...
        {
            // Top-left causal at block grain.
            ...
            if(tid == 0)
            {
                ...
                // causal_killed gates the force-on so past-diagonal blocks are
        ...
        // ==================================================================
        // Force smem_bmap[0]=1 BEFORE LUT collation reads it (don't manually
        // insert into the delta stream — reuses existing LUT-build loop).
        // Causal post-multiply unnecessary: D.2 sets killed scores to -inf;
        // selection gate L490 `bv > 0` excludes them, so smem_bmap[bi]=1 never
        // fires for killed blocks.
        if(attention_sink && tid == 0)
            smem_bmap[0] = 1;
        __syncthreads();
