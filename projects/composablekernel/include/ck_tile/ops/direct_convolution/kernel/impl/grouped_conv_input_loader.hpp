...
CK_TILE_DEVICE void buffer_load16_to_lds(CK_TILE_LDS_ADDR ElementType* lds_dest,
                                         const __amdgpu_buffer_rsrc_t& rsrc,
                                         ck_tile::index_t voffset,
                                         ck_tile::index_t is_valid)
{
#if defined(__gfx950__)
    ck_tile::amd_async_buffer_load<ElementType,
                                   8,
    ...
    const ck_tile::int32x4_t data = __builtin_amdgcn_raw_buffer_load_b128(rsrc, v, 0, 0);
...
template <typename TC,
          auto cfg,
          typename InputType   = ck_tile::fp16x4_t,
          bool Padded          = true,
...
struct InputLoader
{
    ...
    using MfmaBuf =
        ck_tile::buffer_view<ck_tile::address_space_enum::lds, ElementType, ck_tile::index_t, true>;
    using MfmaViewType =
        ck_tile::tensor_view<MfmaBuf, ck_tile::remove_cvref_t<decltype(mfma_desc)>>;

    using MfmaWindowType = decltype(ck_tile::make_tile_window(
        MfmaViewType{},
        ck_tile::make_tuple(
            ck_tile::number<TC::BLOCK_Q>{}, ck_tile::number<TC::BLOCK_C4>{}, ck_tile::number<4>{}),
        {0, 0, 0},
        mfma_dist));
    ...
    CK_TILE_LDS_ADDR ElementType*
        store_input_lds;               // per-thread LDS write destination (lane-0 address)
        ...
        : input_lds_ptr(input_lds)
    {
        ...
        constexpr auto tile_lengths    = ck_tile::make_tuple(ck_tile::number<1>{},
                                                          ck_tile::number<TC::TOTAL_SPATIAL>{},
                                                          ck_tile::number<TC::BLOCK_C8>{},
                                                          ck_tile::number<8>{});
        ...
        auto compute_load_active = [&](const auto& dram_window) {
            ...
            auto warp_bottom_idx =
                dram_window
                    .get_pre_computed_warp_coords()[ck_tile::number<0>{}][ck_tile::number<0>{}]
                    .get_bottom_index();
            auto lds_offset =
                ck_tile::make_tensor_coordinate(lds_store_desc, warp_bottom_idx).get_offset();
            const int lane_id = ck_tile::get_lane_id();
        ...
        auto init_unpadded = [&]() {
            ...
            const auto input_dram_view =
                ck_tile::make_tensor_view<ck_tile::address_space_enum::global>(input_base,
                                                                               input_dram_desc);
            ...
            auto dram_elem_offset =
                tmp_dram_window
                    .get_pre_computed_coords()[ck_tile::number<0>{}][ck_tile::number<1>{}]
                    .get_offset();
            ...
            input_rsrc           = ck_tile::make_builtin_buffer_resource(
                input_base, static_cast<uint32_t>(elem_space_size * sizeof(ElementType)));
            ...
            auto warp_adaptor_bottom_idx =
                tmp_dram_window
                    .get_pre_computed_warp_coords()[ck_tile::number<0>{}][ck_tile::number<0>{}]
                    .get_bottom_index();
            auto warp_lds_offset =
                ck_tile::make_tensor_coordinate(lds_store_desc, warp_adaptor_bottom_idx)
                    .get_offset();
        ...
        if constexpr(Padded)
        {
            if(c_per_group != TC::GROUP_SIZE)
            {
                ...
                padded_state_.input_base_padded_ =
                    in + static_cast<size_t>(bc.block_n) * hi * wi * bc.C_in + bc.block_k_in;
        ...
        requires(Padded)
    {
        ...
        auto padded_dram_window =
            ck_tile::make_tile_window(padded_dram_view,
                                      ck_tile::make_tuple(ck_tile::number<1>{},
                                                          ck_tile::number<TC::TOTAL_SPATIAL>{},
                                                          ck_tile::number<TC::BLOCK_C8>{},
                                                          ck_tile::number<8>{}),
        ...
        auto lds_write_view =
            ck_tile::make_tensor_view<ck_tile::address_space_enum::lds>(lds_base, lds_write_desc);

        auto lds_write_window =
            ck_tile::make_tile_window(lds_write_view,
                                      ck_tile::make_tuple(ck_tile::number<1>{},
                                                          ck_tile::number<TC::TOTAL_SPATIAL>{},
                                                          ck_tile::number<TC::BLOCK_C8>{},
                                                          ck_tile::number<8>{}),
