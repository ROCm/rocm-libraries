...
// MXFlatmmPipelineProblem is defined in the baseline mx pipeline header.
// Rule 4 (plan v3): do NOT redefine it here — include the baseline header instead.
...
template <typename Problem, typename PipelinePolicy = MXFlatmmPipelineAgBgCrPolicySync>
struct MXFlatmmPipelineAGmemBGmemCRegV1Sync : FlatmmPipelineAGmemBGmemCRegV1<Problem, PipelinePolicy>
{
    ...
    // Aload_num: actual buffer_load count per thread for one A tile.
    // = total_A_bytes_per_tile / threads / dram_vector_bytes.
    // dwordx4 (16B) for Phase 2; dwordx3 (12B) for Phase 1.
    static constexpr index_t ADramVecBytes =
        std::is_same_v<ADataType, pk_fp6x16_t> ? 16 : 16; // dwordx4 for all types
    static constexpr index_t Aload_num =
        kMPerBlock * (kKPerBlock * sizeof(ADataType) / APackedSize) / BlockSize / ADramVecBytes;
    static constexpr index_t Aload_num_perK = Aload_num / KIterPerWarp;
    static constexpr index_t Aload_rep      = (Aload_num_perK + MIterPerWarp - 1) / MIterPerWarp;
                             ...
                             void* __restrict__ p_smem_pong) const
    {
        ...
        // Sync path: byte DRAM window (no XOR — see sync policy MakeMX_ASyncLoadBytesDramWindow).
        auto a_dram_window = PipelinePolicy::template MakeMX_ASyncLoadBytesDramWindow<Problem>(
        ...
        // Sync helper: buffer_load A tile to VGPR, then ds_write to LDS.
        auto sync_load_store_A_ = [](auto& lds, auto& dram) {
            auto a_vgpr = load_tile(dram);
            store_tile(lds, a_vgpr);
        };

        // HEAD
        // Prefetch A0
        {
            auto a_vgpr_head = load_tile(a_dram_window);
            store_tile(a_store_lds_window_ping, a_vgpr_head);
        }
        ...
        {
            sync_load_store_A_(a_store_lds_window_pong, a_dram_window);
        ...
        // D2: A is now sync (buffer_load→VGPR), so Aload_num enters vmcnt count.
        // Wait for all buffer_loads (A + B + Scale) to complete, then barrier
        // so all waves' ds_writes are visible before any wave issues ds_reads.
        ...
        auto main_body_implx2 = [&]() mutable {
            // Issue A buffer_load BEFORE B/Scale loads so A DRAM latency overlaps with GEMM 2i.
            // store_tile (VGPR→LDS) is deferred until after s_waitcnt+block_sync_lds.
            auto a_vgpr_ping = load_tile(a_dram_window);
            move_tile_window(a_dram_window, {0, kKPerBlock * sizeof(ADataType) / APackedSize});
            ...
            static_for_product<number<KPackIterPerWarp>,
                ...
                [&](auto ikpack, auto impack, auto inpack, auto ikxdl, auto imxdl, auto inxdl) {
                    ...
                    // preload next A from lds: advance (impack, ikpack) in product-loop order
                    constexpr auto next_im = (impack + 1) % MPackIterPerWarp;
                    constexpr auto next_ik = ikpack + (impack + 1) / MPackIterPerWarp;
                    if constexpr(next_ik < KPackIterPerWarp &&
                                 (n_iter == NIterPerWarp - 1))
                    {
                        constexpr auto next_m = next_im * MXdlPack + imxdl;
                        constexpr auto next_k = next_ik * KXdlPack + ikxdl;
                        a_warp_tensor(number<APackIter>{}) = load_tile_with_offset( //
                            a_warp_window_ping,
                            tuple<number<next_m * WG::kM>,
                                  number<sizeof(ADataType) * next_k * WG::kK / APackedSize>>{});
                    }
                });
            // barrier as ds_load A(2i) and buffer_load_lds A(2i + 1) finished
            // D2: Aload_num covers the A buffer_loads issued before this GEMM.
            s_waitcnt< // vmcnt
                Aload_num + Bload_num + ScaleAload_num + ScaleBload_num>();
            block_sync_lds();

            // A VGPRs are ready (waited above). Store to LDS ping with no extra stall.
            store_tile(a_store_lds_window_ping, a_vgpr_ping);
            ...
            // Issue A buffer_load BEFORE B/Scale so A DRAM latency overlaps with GEMM 2i+1.
            auto a_vgpr_pong = load_tile(a_dram_window);
            move_tile_window(a_dram_window, {0, sizeof(ADataType) * kKPerBlock / APackedSize});
            ...
            // A buffer_loads (issued above) + B(2i+2) + Scale(2i+2) all complete.
            s_waitcnt< // vmcnt
                Aload_num + Bload_num + ScaleAload_num + ScaleBload_num>();
            block_sync_lds();

            // A VGPRs ready. Store to LDS pong with no extra stall.
            store_tile(a_store_lds_window_pong, a_vgpr_pong);
            // move B window to next flat K
            move_tile_window(scale_a_dram_window, {0, kKPerBlock / (32 * KXdlPack)});
            move_tile_window(scale_b_dram_window, {0, kKPerBlock / (32 * KXdlPack)});
        ...
        if constexpr(TailNum == TailNumber::Even)
        {
            ...
            // barrier as ds_load A(2i) and buffer_load_lds A(2i + 1) finished
            // D2: Aload_num added — A sync loads are now counted in vmcnt.
            s_waitcnt< // vmcnt
                Aload_num + Bload_num + ScaleAload_num + ScaleBload_num>();
            block_sync_lds();
