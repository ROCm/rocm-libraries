...
struct MXFp6FlatmmPipelineV1Policy : UniversalFlatmmPipelineAgBgCrPolicy
{
    ...
    // MXFP6-dedicated policy: A and B must both be pk_fp6x16_t.
    // FP4/FP8 traffic stays on MXFlatmmPipelineAgBgCrPolicy.
    static_assert(std::is_same_v<ADataType, pk_fp6x16_t>,
                  "MXFp6FlatmmPipelineV1Policy requires ADataType = pk_fp6x16_t");
    static_assert(std::is_same_v<BDataType, pk_fp6x16_t>,
                  "MXFp6FlatmmPipelineV1Policy requires BDataType = pk_fp6x16_t");
    ...
    {
        constexpr index_t K2 = DWORDx3; // FP6: 12B (vs. 16B = DWORDx4 for FP4/FP8)
    ...
    MakeMX_AAsyncLoadBytesDramWindow(const WindowTmp& window_tmp)
    {
        ...
        constexpr index_t K2 = DWORDx3; // FP6: 12B
        constexpr index_t K1 = kDramLoadPackBytes / DWORDx4; // fp8/fp6/fp4 K1 equal to 8
    ...
    {
        constexpr index_t K2     = DWORDx3; // FP6: 12B (zero-padded to K2_Pad=16 in LDS)
        constexpr index_t K2_Pad = 16;
        constexpr index_t K1     = kDramLoadPackBytes / DWORDx4; // 8
        constexpr index_t K0     = KPerBlock / (K1 * K2 / sizeof(ADataType) * APackedSize);
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeMX_ALDSBytes_TileDistribution()
    {
        ...
        // FP6-only branch (K_Lane=4, K_Thread=32, DWORDx3=12).
        return make_static_tile_distribution(
            tile_distribution_encoding< //
                sequence<NWarps>,
    ...
    {
        // FP6 LDS descriptor element-space is already counted in bytes.
...
struct MXFp6FlatmmPipelineV1Policy
{
...
#define FORWARD_METHOD_(method)                                                                    \
    ...
    {                                                                                              \
        return detail::MXFp6FlatmmPipelineV1Policy<Problem>::method(std::forward<Args>(args)...); \
    }
