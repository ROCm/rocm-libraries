...
// Reuses MXFlatmmPipelineProblem (defined in the original MX pipeline header)
// so existing dispatch code and instance generation work unchanged.
#include "ck_tile/ops/flatmm/pipeline/mx_flatmm_pipeline_agmem_bgmem_creg_v1.hpp"
#include "ck_tile/ops/flatmm/pipeline/mx_fp6_flatmm_pipeline_v1_policy.hpp"

namespace ck_tile {

// MXFP6-dedicated flat-matmul pipeline (Phase-2 scaffold).
//
// This class starts as a behaviorally identical copy of
// MXFlatmmPipelineAGmemBGmemCRegV1 but lives in its own translation chain so
// we can iterate FP6-specific optimizations (single-B-buffer, AGPR direct
// write, custom tile shape, occupancy=2 scheduling) without touching FP4.
//
// FP4 dispatch continues to use MXFlatmmPipelineAGmemBGmemCRegV1 unchanged.
template <typename Problem, typename PipelinePolicy = MXFp6FlatmmPipelineV1Policy>
struct MXFp6FlatmmPipelineV1 : FlatmmPipelineAGmemBGmemCRegV1<Problem, PipelinePolicy>
{
                             ...
                             void* __restrict__ p_smem) const
    {
        ...
        auto b_flat_dram_offsets = generate_tuple(
            ...
            number<NIterPerWarp>{});
        // [Task #7] True single B buffer: ONE declaration, no pong alias, no second
        // storage. All historical b_warp_tensor_ping(...) refs in main_body / TAIL are
        // renamed to b_warp_tensor_ping(...). B is loaded → vmcnt(0) drain → consumed
        // within each K-iter pair; no overlap between load and consume of same buffer.
        ...
        auto main_body_implx2 = [&]() mutable {
            // [Mutation 1] B prefetch for K-iter 2i+1 is relocated to AFTER GEMM 2i
            // (search "INSERTED B prefetch (2i+1)" below). At entry the single B
            // tile already holds B(2i) (from prior body call's end prefetch, or from
            // HEAD on the first call), so GEMM 2i below reads the correct data.
            ...
            // INSERTED B prefetch (2i+1) — must come AFTER GEMM 2i (which consumed
            // the previous-iter B(2i) from the same physical buffer) and BEFORE
            // GEMM 2i+1 (which will consume B(2i+1)). The compiler enforces vmcnt
            // via RAW dep between this load and the next GEMM's bit_cast read.
            ...
            // [Task #10 attempt 2] removed s_waitcnt<0> — RAW dep through bit_cast<BWarpTensor>(b_warp_tensor_ping(...)) will insert vmcnt naturally before next GEMM
            block_sync_lds();
            ...
            __builtin_amdgcn_iglp_opt(1); // [Task #26] IGLP retune: MFMA-priority

            ////////////////////////////// Next K //////////////////////////////

            // [Mutation 1] B prefetch for K-iter 2i+2 is relocated to AFTER GEMM 2i+1
            // (search "INSERTED B prefetch (2i+2)" below). Single B tile still holds
            // B(2i+1) (loaded above) which GEMM 2i+1 below consumes.
            ...
            // INSERTED B prefetch (2i+2) — must come AFTER GEMM 2i+1 (which just
            // consumed B(2i+1)) and BEFORE the NEXT body-call's GEMM 2(i+1)=GEMM 2i+2
            // (which will consume B(2i+2)). On exit of this body call, the single
            // B tile holds B(2i+2), restoring the invariant "single = B(2j) at entry".
            static_ford<sequence<KIterPerWarp, NIterPerWarp>>{}([&](auto kn) {
                constexpr auto kIter             = number<kn[number<0>{}]>{};
                constexpr auto nIter             = number<kn[number<1>{}]>{};
                b_warp_tensor_ping(nIter)(kIter) = load_tile_with_offset(
        ...
        {
            // [Mutation 1] B prefetch for K-iter loopK is relocated to AFTER
            // GEMM(loopK-1) below. At entry the single B tile already holds B(loopK-1)
            // (from prior body's end prefetch, or from HEAD if HasHotLoop=false).
            ...
            // INSERTED B prefetch (loopK) — TAIL Even consumes B(loopK-1) in the
            // GEMM above and B(loopK) in the final GEMM below. With the single
            // buffer the B(loopK) load must come AFTER GEMM(loopK-1).
            static_ford<sequence<KIterPerWarp, NIterPerWarp>>{}([&](auto kn) {
                constexpr auto kIter             = number<kn[number<0>{}]>{};
                constexpr auto nIter             = number<kn[number<1>{}]>{};
                b_warp_tensor_ping(nIter)(kIter) = load_tile_with_offset(
