...
struct MXGemmKernel : UniversalGemmKernel<TilePartitioner_, MXGemmPipeline_, EpiloguePipeline_>
{
                                   ...
                                   int partition_idx = get_block_id()) const
    {
        ...
        // alignas(16): the MXFP6 LDS path uses a 16-byte padded per-element
        // stride (lds_padded_element<pk_fp6x16_t>), so the LDS base must be
        // 16-byte aligned rather than relying on the default LDS symbol
        // alignment.
