...
struct FmhaN128AffineKProbe
{
    template <index_t IAccess>
    CK_TILE_HOST_DEVICE static constexpr index_t ByteDelta()
    {
        static_assert(IAccess >= 0 && IAccess < 16);
        return (IAccess / 8) * 16 * 136 * 2 + (IAccess % 8) * 32;
    }

    template <index_t Stage, index_t IAccess, typename TileWindow>
    CK_TILE_HOST_DEVICE static constexpr bool ValidateStageDelta()
    {
        ...
        for(index_t warp = 0; warp < 4; ++warp)
        {
            for(index_t lane = 0; lane < 32; ++lane)
            {
                ...
                // QK stages load SU1/2/3; PV stage3 primes the next tile's SU0.
    ...
    template <index_t Stage, index_t IAccess, typename DistributedTensor, typename TileWindow>
    CK_TILE_DEVICE static void Load(DistributedTensor& dst,
                                    const TileWindow& window,
                                    const FmhaN128PreparedKProbe::Address& base)
    {
        static_assert(ValidateStageDelta<Stage, IAccess, TileWindow>());
        ...
        FmhaN128PreparedKProbe::Load<IAccess>(dst, window, address);
    }
};
