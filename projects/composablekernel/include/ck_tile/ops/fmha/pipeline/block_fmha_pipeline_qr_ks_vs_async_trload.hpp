            ...
            sweep_tile_span(p_spans[I0], [&](auto idx0) {
                ...
                sweep_tile_span(p_spans[I1], [&](auto idx1) {
                    ...
                    if constexpr(BiasEnum == BlockAttentionBiasEnum::ELEMENTWISE_BIAS ||
                                 BiasEnum == BlockAttentionBiasEnum::ALIBI)
                    {
                        p_compute(i_j_idx) = fmha_fast_exp2(s_new[i_j_idx] - get_validated_m(m_stab[i_idx]));
                    }
                    else
                    {
                        if constexpr(kHasLogitsSoftCap)
                        {
                            p_compute(i_j_idx) =
                                fmha_fast_exp2(s_new[i_j_idx] - get_validated_m(m_stab[i_idx]));
                        }
                        else
                        {
                            p_compute(i_j_idx) = fmha_fast_exp2(scale_s * s_new[i_j_idx] - row_max);
                        }
                    }
                });
            });
            ...
            sweep_tile_span(p_spans[I0], [&](auto idx0) {
                ...
                sweep_tile_span(p_spans[I1], [&](auto idx1) {
                    ...
                    if constexpr(BiasEnum == BlockAttentionBiasEnum::ELEMENTWISE_BIAS ||
                                 BiasEnum == BlockAttentionBiasEnum::ALIBI)
                    {
                        p_compute(i_j_idx) = fmha_fast_exp2(s_new[i_j_idx] - get_validated_m(m[i_idx]));
                    }
                    else
                    {
                        if constexpr(kHasLogitsSoftCap)
