...
struct BlockFmhaBatchPrefillPipelineQRKSVSAsync
{
               ...
               const float* p_scale_ptr = nullptr) const
    {
        ...
        do
        {
            ...
            // s_acc[i,j] *= q_descale[q_origin+i, qo_head] * k_descale[k_page(j), k_slot(j),
            // kv_head]
            //
            ...
            else if constexpr(QScaleEnum == BlockAttentionQuantScaleEnum::PER_TOKEN_HEAD)
            {
                ...
                else
                {
                    ...
                    static_for<0, kPagesPerTile, 1>{}([&](auto p) {
                        const index_t gp =
                            (k_origin_n + p.value * kPageBlockSize) >> kLog2PageBlockSize;
                        tile_k_pages[p.value] = page_idx[ck_tile::min(gp, max_page_table_idx)];
                    });
                }
                ...
                const index_t tid_in_block = static_cast<index_t>(
                ...
                {
                    lds_q_descale[off] =
                        q_descale_per_token_ptr[(q_row_base + off) * stride_q_descale_token +
                ...
                {
                    const index_t k_page =
                        tile_k_pages[(kPagesPerTile == 1) ? index_t{0}
                                                          : (off >> kLog2PageBlockSize)];
                    const index_t k_slot = off & kPageSlotMask;
                    lds_k_descale[off]   = k_descale_ptr[k_page * nblock_stride_k_descale_page +
