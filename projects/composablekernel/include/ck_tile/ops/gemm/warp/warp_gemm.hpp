...
// clang-format off
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaF16F16F32M32N32K16                                  = typename UnificationDispatcher<half_t, half_t, float, 32, 32, 16, false, false, false, NumAccess>::Type;
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaF16F16F32M32N32K16TransposedCDistribution           = typename UnificationDispatcher<half_t, half_t, float, 32, 32, 16, true, false, false, NumAccess>::Type;
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaF16F16F32M16N16K32                                  = typename UnificationDispatcher<half_t, half_t, float, 16, 16, 32, false, false, false, NumAccess>::Type;
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaF16F16F32M16N16K32TransposedCDistribution           = typename UnificationDispatcher<half_t, half_t, float, 16, 16, 32, true, false, false, NumAccess>::Type;
using WarpGemmMfmaF16F16F32M32N32K8                                   = typename UnificationDispatcher<half_t, half_t, float, 32, 32,  8, false>::Type;
using WarpGemmMfmaF16F16F32M32N32K8TransposedCDistribution            = typename UnificationDispatcher<half_t, half_t, float, 32, 32,  8,  true>::Type;
using WarpGemmMfmaF16F16F32M4N64K16                                   = typename UnificationDispatcher<half_t, half_t, float,  4, 64, 16, false>::Type;
using WarpGemmMfmaF16F16F32M64N4K16                                   = typename UnificationDispatcher<half_t, half_t, float, 64,  4, 16, false>::Type;
using WarpGemmMfmaF16F16F32M16N16K16                                  = typename UnificationDispatcher<half_t, half_t, float, 16, 16, 16, false>::Type;
using WarpGemmMfmaF16F16F32M16N16K16TransposedCDistribution           = typename UnificationDispatcher<half_t, half_t, float, 16, 16, 16,  true>::Type;
// using WarpGemmMfmaF16F16F32M32N32K8SwizzleA                           = typename UnificationDispatcher<half_t, half_t, float, 32, 32,  8, false, true>::Type;
// using WarpGemmMfmaF16F16F32M32N32K16SwizzleA                          = typename UnificationDispatcher<half_t, half_t, float, 32, 32, 16, false, true>::Type;
// using WarpGemmMfmaF16F16F32M32N32K8SwizzleBTransposedCDistribution    = typename UnificationDispatcher<half_t, half_t, float, 32, 32,  8,  true, true>::Type;
// using WarpGemmMfmaF16F16F32M32N32K16SwizzleBTransposedCDistribution   = typename UnificationDispatcher<half_t, half_t, float, 32, 32, 16,  true, true>::Type;

// bf16 named WarpGemms (no WMMA or StructuredSparsity)
using WarpGemmMfmaBf16Bf16F32M32N32K8                                 = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32,  8, false>::Type;
using WarpGemmMfmaBf16Bf16F32M32N32K8TransposedCDistribution          = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32,  8,  true>::Type;
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaBf16Bf16F32M32N32K16                                = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32, 16, false, false, false, NumAccess>::Type;
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaBf16Bf16F32M32N32K16TransposedCDistribution         = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32, 16,  true, false, false, NumAccess>::Type;
template<WGAttrNumAccessEnum NumAccessA = ESingle, WGAttrNumAccessEnum NumAccessB = ESingle>
using WarpGemmMfmaBf16Bf16F32M16N16K32                                = typename UnificationDispatcher<bf16_t, bf16_t, float, 16, 16, 32, false, false, false, NumAccessA, NumAccessB>::Type;
template<WGAttrNumAccessEnum NumAccess = ESingle>
using WarpGemmMfmaBf16Bf16F32M16N16K32TransposedCDistribution         = typename UnificationDispatcher<bf16_t, bf16_t, float, 16, 16, 32,  true, false, false, NumAccess>::Type;
template<WGAttrNumAccessEnum NumAccessA = ESingle, WGAttrNumAccessEnum NumAccessB = ESingle>
using WarpGemmMfmaBf16Bf16F32M16N16K64                                = typename UnificationDispatcher<bf16_t, bf16_t, float, 16, 16, 64, false, false, false, NumAccessA, NumAccessB>::Type;
using WarpGemmMfmaBf16Bf16F32M4N64K16                                 = typename UnificationDispatcher<bf16_t, bf16_t, float,  4, 64, 16, false>::Type;
using WarpGemmMfmaBf16Bf16F32M64N4K16                                 = typename UnificationDispatcher<bf16_t, bf16_t, float, 64,  4, 16, false>::Type;
using WarpGemmMfmaBf16Bf16F32M16N16K16                                = typename UnificationDispatcher<bf16_t, bf16_t, float, 16, 16, 16, false>::Type;
using WarpGemmMfmaBf16Bf16F32M16N16K16TransposedCDistribution         = typename UnificationDispatcher<bf16_t, bf16_t, float, 16, 16, 16,  true>::Type;
// using WarpGemmMfmaBf16Bf16F32M32N32K8SwizzleA                         = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32,  8, false, true>::Type;
// using WarpGemmMfmaBf16Bf16F32M32N32K16SwizzleA                        = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32, 16, false, true>::Type;
// using WarpGemmMfmaBf16Bf16F32M32N32K8SwizzleBTransposedCDistribution  = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32,  8,  true, true>::Type;
// using WarpGemmMfmaBf16Bf16F32M32N32K16SwizzleBTransposedCDistribution = typename UnificationDispatcher<bf16_t, bf16_t, float, 32, 32, 16,  true, true>::Type;
//clang-format on
