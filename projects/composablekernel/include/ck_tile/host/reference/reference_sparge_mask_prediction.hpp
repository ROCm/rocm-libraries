...
template <typename T>
HostTensor<float> compute_block_means(const HostTensor<T>& data_bhsd,
                                      index_t batch,
                                      index_t nhead,
                                      index_t num_blocks,
                                      index_t block_size,
...
template <typename T>
HostTensor<float>
compute_block_similarity(const HostTensor<T>& data_bhsd,
                         ...
                         index_t hdim)
{
    auto kmv = [&](index_t b, index_t h, index_t d) { return km ? (*km)(b, h, d) : 0.0f; };
    ...
    for(index_t b = 0; b < batch; ++b)
        for(index_t h = 0; h < nhead; ++h)
            for(index_t blk = 0; blk < num_blocks; ++blk)
            {
                ...
                sim(b, h, blk) =
                    u_norm_sq / (static_cast<float>(count) * static_cast<float>(count));
            }
    return sim;
}

template <typename T>
HostTensor<float> compute_global_k_mean(
    const HostTensor<T>& k_bhsd, index_t batch, index_t nhead_k, index_t seqlen_k, index_t hdim)
{
                ...
                for(index_t g = 0; g < scales_per_blk; ++g)
                {
                    ...
                    for(index_t tt = 0; tt < tokens_per_scale; ++tt)
                    {
                        ...
                        for(index_t d = 0; d < hdim; ++d)
                        {
                            ...
                            int8_out(b, h, s, d) = type_convert<int8_t>(saturates<int8_t>{}(r));
                        }
                    }
                }
            }
    ...
    for(index_t b = 0; b < batch; ++b)
        for(index_t h = 0; h < nhead; ++h)
        {
            ...
            for(index_t s = 0; s < seqlen; ++s)
                for(index_t d = 0; d < hdim; ++d)
                    amax = std::max(
                        amax, std::abs(type_convert<float>(data_bhsd(b, h, s, d)) - kmv(b, h, d)));
            ...
            for(index_t s = 0; s < seqlen; ++s)
                ...
                {
                    float v = type_convert<float>(data_bhsd(b, h, s, d)) - kmv(b, h, d);
                    int8_out(b, h, s, d) = type_convert<int8_t>(saturates<int8_t>{}(v / scale));
                }
        }
}
...
template <typename T>
HostTensor<uint8_t> reference_sparge_mask_prediction(const HostTensor<T>& q_bhsd,
                                                     ...
                                                     const sparge_causal_params& causal)
{
    ...
    float scale =
        (params.scale != 0.0f) ? params.scale : 1.0f / std::sqrt(static_cast<float>(hdim));

    // Q side is never centered by the device (km_ptr == nullptr): pass null.
    auto q_means =
        compute_block_means(q_bhsd, batch, nhead, num_q_blocks, block_size_q, seqlen_q, hdim);
    auto q_sim = compute_block_similarity(
        q_bhsd, nullptr, batch, nhead, num_q_blocks, block_size_q, seqlen_q, hdim);
    ...
    if(have_km)
        km_storage.push_back(compute_global_k_mean(k_bhsd, batch, nhead_k, seqlen_k, hdim));
    const HostTensor<float>* km = have_km ? &km_storage.front() : nullptr;

    auto k_means =
        compute_block_means(k_bhsd, batch, nhead_k, num_k_blocks, block_size_k, seqlen_k, hdim);
    auto k_sim = compute_block_similarity(
        k_bhsd, km, batch, nhead_k, num_k_blocks, block_size_k, seqlen_k, hdim);
    if(have_km)
    {
        for(index_t b = 0; b < batch; ++b)
            for(index_t h = 0; h < nhead_k; ++h)
    ...
    for(index_t b = 0; b < batch; ++b)
    {
        for(index_t h = 0; h < nhead; ++h)
        {
            const float head_cdfthreshd =
                detail::lookup_per_head_host(params.cdfthreshd_per_head, h, cdf_scalar);
            const float head_topk =
                detail::lookup_per_head_host(params.topk_per_head, h, topk_scalar);
            const float head_simthreshold =
                detail::lookup_per_head_host(params.simthreshold_per_head, h, sim_scalar);
            ...
            for(index_t qi = 0; qi < num_q_blocks; ++qi)
            {
                ...
                index_t causal_max_kj =
                    causal_type ? std::min(num_k_blocks - 1,
                                           (last_q + right_ext + causal_delta) / block_size_k)
                                : (num_k_blocks - 1);
                index_t causal_min_kj =
                ...
                if(head_simthreshold > 0.0f && q_sim(b, h, qi) <= head_simthreshold)
                {
                    for(index_t kj = 0; kj < num_k_blocks; ++kj)
                        mask(b, h, qi, kj) = (kj >= causal_min_kj && kj <= causal_max_kj) ? 1 : 0;
                ...
                const index_t n_target =
                    topk_mode ? std::max(index_t{1},
                                         static_cast<index_t>(head_topk *
                                                              static_cast<float>(num_k_blocks)))
                              : num_k_blocks;
                float cum          = 0.0f;
