...
static void ssd_forward_cpu_ref(
    ...
    int B, int G, int EH, int C, int L, int D, int N)
{
    auto idx5 = [](int a, int b, int c, int d, int e,
                   int B_, int C_, int D_, int E_) -> size_t {
        return static_cast<size_t>(((a * B_ + b) * C_ + c) * D_ + d) * E_ + e;
    };
    auto idx4 = [](int a, int b, int c, int d,
                   int B_, int C_, int D_) -> size_t {
    ...
    auto idx2 = [](int a, int b, int B_) -> size_t {
        return static_cast<size_t>(a) * B_ + b;
    };
    ...
    for (int bi = 0; bi < B; ++bi) {
        for (int eh = 0; eh < EH; ++eh) {
            ...
            for (int ci = 0; ci < C; ++ci) {
                ...
                for (int i = 0; i < L; ++i)
                    for (int j = 0; j < L; ++j) {
                        if      (j < i)  segsum_mat[i*L+j] = expf(cum[i]-cum[j]);
                        else if (j == i) segsum_mat[i*L+j] = 1.0f;
                        else             segsum_mat[i*L+j] = 0.0f;
                    }
                ...
                for (int i = 0; i < L; ++i)
                    for (int j = 0; j < L; ++j)
                        pre_intra2[i*L+j] = segsum_mat[i*L+j] *
                            Delta[idx4(bi, eh, ci, j, EH, C, L)] * intra_bmm1[i*L+j];
                    ...
                    for (int d_ = 0; d_ < D; ++d_) {
                        ...
                        for (int lp = 0; lp < L; ++lp)
                            acc += pre_intra2[l*L+lp] *
                                   X[idx5(bi, eh, d_, ci, lp, EH, D, C, L)];
...
int main(int argc, char* argv[])
{
    ...
    if (do_verify) {
        ...
        std::cout << "[Y]      " << (y_mismatch == 0 ? "PASSED" : "FAILED")
                  << " (" << y_mismatch << "/" << sz_y << " mismatches)" << std::endl;
        std::cout << "[Fstate] " << (f_mismatch == 0 ? "PASSED" : "FAILED")
                  << " (" << f_mismatch << "/" << sz_f << " mismatches)" << std::endl;
