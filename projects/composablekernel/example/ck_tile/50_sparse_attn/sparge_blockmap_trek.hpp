...
struct sparge_blockmap_args
{
    ...
    // Pipeline only honours mask_enum::mask_top_left; CLI warns on other types.
    // Defaults preserve back-compat for callers not yet setting these.
                                            ...
                                            const void* sim_k_ws_ptr)
{
    ...
    // Decode mask_enum -> bool at the trek boundary. The kernel-tree header
    // takes bool is_causal_tl and never sees mask_enum.
    const bool is_causal_tl = (args.mask_type == mask_enum::mask_top_left);
