...
#
# sparge_sage codegen: i8fp8bf16 x {perwarp,blockscale,perthread} x batch x {no-mask,causal} x
# no-bias x hdim128. Generates one kernel translation unit per (qscale, mask) variant (3-callable
# launch chain: fused preprocess with INT8 Q/K quant -> mask prediction -> quantized sage
# attention) plus the dispatch API. The qscale enum drives both the preprocess per-token-group
# absmax granularity (TileSageAttnTraits kBlockScaleSizeQ/K) and the attention descale stride.
...
# Tile: bm0=128, bn0=64, bk0=32, bn1=128, bk1=32, bk0max=128; gemm warps 4x1x1 / 4x1x1;
# warp tiles 32x32x32 (int8 MFMA). PERWARP: Q scale block 32, K scale block 64.
# PERTENSOR: one global scale per (batch,head); preprocess runs means-only (NO_SCALE) and a
# dedicated per-(b,h) global-absmax Q/K quant kernel produces INT8 Q/K + a single q/k scale.
...
// Preprocess (fused K+Q with PERWARP INT8 quant) + mask prediction kernels run on the original
// bf16 Q/K. Block size 256 matches the sparge preprocess/mask pipelines.
// Q/K quant target dtype = cfg::QDataType (int8 for i8fp8bf16, fp8 for fp8bf16). Scale divisor is
// chosen inside the pipeline (127 for int8, fp8_max for fp8).
                                         ...
                                         fmha_sparge_sage_fwd_args a)
{
    ...
    mp_kargs.k_sim               = nullptr;
    mp_kargs.q_sim               = nullptr;
    ...
    mp_kargs.simthreshold        = 0.0f;
    mp_kargs.cdfthreshd_per_head = nullptr;
    mp_kargs.topk_per_head       = nullptr;
    mp_kargs.simthreshold_per_head = nullptr;
    mp_kargs.causal_type         = static_cast<ck_tile::index_t>(a.causal_type);
    mp_kargs.attention_sink      = false;
    ...
    // a per-(b,h) scale (nhead stride 1, batch stride = nhead). Built unconditionally; only
    ...
    if constexpr(kIsGroup)
    {
        ...
        // PERTENSOR group scale is per-(b,h) batch-major [batch, nhead]; override the group
        // packed-block stride defaults the kernel set for the block-scale schemes.
                           ...
                           const ck_tile::stream_config& s)
{
       ...
       t.hdim_q <= 128 && t.hdim_v <= 128)
    {
        ...
        SPARGE_SAGE_DISPATCH("blockscale", BLOCKSCALE, false)
       ...
       t.hdim_q <= 128 && t.hdim_v <= 128)
    {
        ...
        SPARGE_SAGE_DISPATCH("blockscale", BLOCKSCALE, true)
...
# (dtype_name, cfg, qkfp8_bool). i8fp8bf16: INT8 Q/K. fp8bf16: FP8 Q/K (fp8 MFMA gemm0).
DTYPE_VARIANTS = [
    ("i8fp8bf16", "SpargeSageI8Fp8Bf16", "false"),
    ("fp8bf16", "SpargeSageFp8Bf16", "true"),
]
# (qscale_name, qscale_enum)
QSCALE_VARIANTS = [
    ("perwarp", "PERWARP"),
...
# (mask_suffix, F_mask_bool, F_log_suffix)
MASK_VARIANTS = [
    ("", "false", ""),
    ("_mask", "true", "_mask"),
]
# (mode_name, F_group_bool)
MODE_VARIANTS = [
    ("batch", "false"),
    ("group", "true"),
]
# (bias_suffix, F_bias_enum, F_bias_suffix). Bias is added to the descaled fp32 s_acc: ALIBI via
# position-encoding (row/col), ELEMENTWISE via a DRAM bias tile loaded into the *same* gemm0 C
# (s_acc) distribution -- so the int8-MFMA SwizzleB/TransposedC layout aligns by construction.
BIAS_VARIANTS = [
    ("", "NO_BIAS", ""),
    ("_alibi", "ALIBI", "_alibi"),
...
def _render_kernel(dtype_name: str, cfg: str, qkfp8_bool: str, qscale_name: str, qscale_enum: str,
    ...
    # PERTENSOR runs the preprocess means-only (NO_SCALE) and quantizes Q/K in a dedicated
    # global-absmax kernel; all other modes fuse per-block quant into the preprocess.
...
def write_blobs(output_dir: Path, kernel_filter, receipt, optdim_list, mask_impl) -> None:
    ...
    if optdim_list != [-1] and 128 not in optdim_list:
        # still emit so the TU exists; sparge_sage only supports hdim128.
        pass
