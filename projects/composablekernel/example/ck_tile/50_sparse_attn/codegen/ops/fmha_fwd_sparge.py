...
# NOTE: Sparge sparse attention kernel has the following restrictions enforced by static_assert:
...
void fmha_sparge_fwd_oneshot(fmha_sparge_fwd_traits t, fmha_sparge_fwd_args a, const ck_tile::stream_config& s){{

    const bool has_load_tr = ck_tile::is_load_tr_supported();

{F_dispatch}
    std::cerr << "fmha_sparge_fwd_oneshot: no matching dispatch (dtype=" << t.data_type
...
class KernelComponentFactory:
    ...
    def get_pipelines(dtype, hdim, hdim_v, receipt, mask_impl) -> List[FmhaFwdPipeline]:
        ...
        # NOTE: logits soft-cap is NOT supported by Sparge sparse attention (enforced by static_assert)
        pipelines = []
        if dtype in ["fp16", "bf16"]:
            for logits, mask in itertools.product(
                ...
                if hdim == 256 and hdim_v == 256:
                    # sparge fmha only supports dim <= 192 for now.
                    continue
                pipelines.append(
                    FmhaFwdPipeline(
                        "qr_async_sparge",
...
def get_fwd_blobs(
    ...
    # NOTE: Sparge sparse attention only supports batch mode (group mode NOT supported, enforced by static_assert)
        ...
        for ((hdim, hdim_v), tiles), mode in itertools.product(d.items(), ["batch"]):
            for tile, pipeline in itertools.product(
                ...
                if tile.F_bm0 not in (64, 128) or tile.F_bn0 != 128:
                    continue
                if pipeline.tag != "qr_async_sparge":
                    continue
                k = FmhaFwdKernel(
                    F_idx=1,
                    F_hdim=hdim,
