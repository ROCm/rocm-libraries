                           ...
                           bool is_v_rowmajor)
{
    ...
    s.stride_v =
        is_v_rowmajor ? (i_perm ? hdim_v : nhead_k * hdim_v) : (i_perm ? sk : nhead_k * sk);
    s.stride_o = (o_perm ? hdim_v : nhead * hdim_v);

    s.nhead_stride_q = (i_perm ? sq * hdim_q : hdim_q);
    s.nhead_stride_k = (i_perm ? sk * hdim_q : hdim_q);
    s.nhead_stride_v =
        is_v_rowmajor ? (i_perm ? sk * hdim_v : hdim_v) : (i_perm ? hdim_v * sk : sk);
                             ...
                             const std::vector<int32_t>& mask_batch_offsets)
{
    ...
    const std::string data_type = std::is_same_v<DataType_, ck_tile::bf16_t> ? "bf16" : "fp16";
    ...
    const float scale_s =
        (scale_s_user != 0.0f) ? scale_s_user : 1.0f / ck_tile::sqrt(static_cast<float>(hdim_q));
    mask_info mask = mask_info::decode(mask_str, total_q, total_k);
    auto st        = compute_strides(
        nhead, nhead_k, total_q, total_k, hdim_q, hdim_v, i_perm, o_perm, is_v_rowmajor);
    ...
    fill_common_traits(
        fmha_traits, hdim_q, hdim_v, data_type, is_v_rowmajor, mask, bias, logits_soft_cap);
    fmha_traits.is_group_mode = is_group_mode;

    fmha_jenga_fwd_args args;
    assert(nhead % nhead_k == 0);
    fill_common_args(args,
                     q_buf,
                     k_buf,
                     v_buf,
                     o_buf,
                              ...
                              std::vector<int32_t>* out_vbn)
{
    ...
    {
        std::cerr << tag << " error: bias.type=" << bias.type << " but bias.ptr==nullptr.\n";
        return -1.0f;
    }
    ...
    {
        std::cerr << tag
                  << " warning: simthreshold_per_head_ptr set but scalar "
    ...
    // beyond that no variant exists, so reject here.
    {
            ...
            for(int b = 0; b < batch; ++b)
            {
                ...
                if(num_k_blocks_b > kMaxKBlocks)
                {
                    std::cerr << tag << " error: seqlen_k[" << b << "]=" << seqlen_k_b
                              << " (=" << num_k_blocks_b << " K-blocks @ block_size=" << block_size
                              << ") exceeds kMaxKBlocksPow2=" << kMaxKBlocks << " (max "
                              << kMaxKBlocks * block_size << " tokens per seq).\n";
                    return -1.0f;
                }
            }
        }
        else
        {
            ...
            {
                std::cerr << tag << " error: seqlen_k=" << seqlen_k << " (=" << num_k_blocks
                          << " K-blocks @ block_size=" << block_size
                          << ") exceeds kMaxKBlocksPow2=" << kMaxKBlocks << " (max "
                          << kMaxKBlocks * block_size << " tokens).\n";
                return -1.0f;
            }
        }
    }
    ...
    if(max_seqlen_q == 0)
        max_seqlen_q = total_q_tokens;
    if(max_seqlen_k == 0)
        max_seqlen_k = total_k_tokens;
    (void)max_seqlen_k;
...

    {
        ...
        if(out_lut != nullptr)
        {
            ...
            HIP_CHECK_ERROR(
                hipMemcpy(out_lut->data(),
                          static_cast<const char*>(workspace.GetDeviceBuffer()) + ws.lut_off,
                          ws.lut_bytes,
        ...
        if(out_vbn != nullptr)
        {
            ...
            HIP_CHECK_ERROR(
                hipMemcpy(out_vbn->data(),
                          static_cast<const char*>(workspace.GetDeviceBuffer()) + ws.vbn_off,
                          ws.vbn_bytes,
                          hipMemcpyDeviceToHost));
        }
        return ave_time;
                                   ...
                                   std::vector<int32_t>* out_vbn)
{
    ...
    // TileSageAttnTraits so a trait change can't silently desync the descale stride.
    {
        ...
        static_assert(
            ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERWARP>::
                        kBlockScaleSizeQ == 32 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERWARP>::
                        kBlockScaleSizeK == 64 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTHREAD>::
                        kBlockScaleSizeQ == 4 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTHREAD>::
                        kBlockScaleSizeK == 16 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::BLOCKSCALE>::
                        kBlockScaleSizeQ == 128 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::BLOCKSCALE>::
                        kBlockScaleSizeK == 128 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTENSOR>::
                        kBlockScaleSizeQ == 128 &&
                ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTENSOR>::
                        kBlockScaleSizeK == 128,
        ...
        static_assert(ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERWARP>::
                                  kBlockScaleSizeQ <= kPpBlockSize &&
                          ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERWARP>::
                                  kBlockScaleSizeK <= kPpBlockSize &&
                          ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTHREAD>::
                                  kBlockScaleSizeQ <= kPpBlockSize &&
                          ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTHREAD>::
                                  kBlockScaleSizeK <= kPpBlockSize &&
                          ...
                          ck_tile::TileSageAttnTraits<false, false, false, false, QSE::PERTENSOR>::
                                  kBlockScaleSizeK <= kPpBlockSize,
                      "block_scale_size must be <= pp_block_size (=128) or workspace "
                      "scales-per-block (spb) collapses to 0");
    }
    ...
    // smooth_k: host-compute per-channel global K-mean km = k.mean(dim=-2) per (b, head_k,
    // channel), fed to BOTH device and reference K quant. Q is never centered; fp32 accumulation in
    // the same element order as the host reference's compute_global_k_mean. Disabled -> km_ptr
    // nullptr.
    ...
    if(hp.smooth_k)
    {
        ...
        for(int b = 0; b < batch; ++b)
        {
            ...
            const int64_t b_tok_off = is_group_mode ? static_cast<int64_t>(seqstart_k_host[b]) : 0;
            const int64_t b_base    = is_group_mode ? (b_tok_off * st.stride_k)
            ...
            for(int h = 0; h < nhead_k; ++h)
                for(int d = 0; d < hdim_q; ++d)
                {
                    ...
                    for(int s = 0; s < sk; ++s)
                    {
                        const int64_t off = b_base + static_cast<int64_t>(h) * st.nhead_stride_k +
    ...
    if(out_lut != nullptr)
    {
        ...
        HIP_CHECK_ERROR(
            hipMemcpy(out_lut->data(),
                      static_cast<const char*>(workspace.GetDeviceBuffer()) + ws.base.lut_off,
                      ws.base.lut_bytes,
                      hipMemcpyDeviceToHost));
    }
    if(out_vbn != nullptr)
    {
        ...
        HIP_CHECK_ERROR(
            hipMemcpy(out_vbn->data(),
                      static_cast<const char*>(workspace.GetDeviceBuffer()) + ws.base.vbn_off,
                      ws.base.vbn_bytes,
                      hipMemcpyDeviceToHost));
    }
    return ave_time;
...
#define INSTANTIATE_SPARGE_SAGE(T)                                                             \
    template float sparge_sage_sparse_attention<T>(const ck_tile::HostTensor<T>&,              \
                                                   const ck_tile::HostTensor<T>&,              \
                                                   const ck_tile::HostTensor<ck_tile::fp8_t>&, \
                                                   const ck_tile::HostTensor<float>&,          \
