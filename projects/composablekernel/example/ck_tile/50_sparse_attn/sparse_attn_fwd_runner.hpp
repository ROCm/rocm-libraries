...
parse_csv_floats(const std::string& csv, const char* field_name, ck_tile::index_t expected_len)
{
    ...
    if(expected_len <= 0)
        throw std::invalid_argument(
            std::string("parse_csv_floats: expected_len must be > 0 (field '") + field_name + "')");
    ...
    while(std::getline(ss, token, ','))
    {
        ...
        catch(const std::exception&)
        {
            throw std::invalid_argument(std::string("-") + field_name +
                                        " contains non-float token '" + token + "'");
        }
    }
    if(static_cast<ck_tile::index_t>(out.size()) != expected_len)
        throw std::invalid_argument(std::string("-") + field_name + " has " +
                                    std::to_string(out.size()) + " values but expected " +
                                    std::to_string(expected_len) + " (= nhead_q)");
    return out;
}
                                  ...
                                  const std::string& pvthreshd_per_head_csv)
{
    ...
    auto upload =
        [&](bool used, std::vector<float>& host, ck_tile::DeviceMem& buf, const void*& hp_ptr) {
            if(!used)
                return;
    ...
    const bool use_pv =
        fill_field(out.h_pvthreshd, csv_pvthreshd, scalar_pvthreshd, /*clamp=*/false);
...
// CPU-reference mask-prediction params from the same scalars + per-head vectors as the device
// launch (so device and reference select identical blocks).
inline ck_tile::sparge_block_predict_params make_sparge_predict_params(float scalar_cdf,
                                                                       float scalar_topk,
                                                                       float scalar_sim,
...
// [batch,nhead_k,hdim_v]. outer index ob = packed?0:b (batch V is [B,..]; group V is packed
// [1,..]).
template <typename T>
                                ...
                                const std::vector<int32_t>& tok_len)
{
    ...
    auto wr = [&](int ob, ck_tile::index_t h, ck_tile::index_t tok, ck_tile::index_t dv)
        -> decltype(auto) { return i_perm ? v_fp8(ob, h, tok, dv) : v_fp8(ob, tok, h, dv); };
                      ...
                      bool print_result                         = true)
{
    ...
    for(size_t i = 0; i < gpu_bhsd.mData.size(); ++i)
    {
        ...
        if(diff > atol + rtol * std::abs(ref_val))
        {
            ...
            if(mismatch_msgs.size() < kMaxMismatchPrint)
            {
                ...
                oss << "  Mismatch at " << i << ": GPU=" << gpu_val << ", Ref=" << ref_val
                    << ", Diff=" << diff;
    ...
    if(!pass)
    {
        ...
        std::cout << label << ": max_abs_diff=" << max_diff << ", max_rel_diff=" << max_rel_diff
...
bool validate_vs_blocked_ref(const ck_tile::HostTensor<T>& q_host,
...
inline ck_tile::HostTensor<BiasT> generate_random_bias(int rank,
                                                       ck_tile::index_t batch,
                                                       ck_tile::index_t nhead,
                                                       ck_tile::index_t seqlen_q,
...
bool validate_vs_blocked_ref_with_bias(const ck_tile::HostTensor<T>& q_host,
                                       const ck_tile::HostTensor<T>& k_host,
                                       const ck_tile::HostTensor<T>& v_host,
                                       const ck_tile::HostTensor<T>& output_host,
                                       const ck_tile::HostTensor<uint8_t>& block_mask,
                                       ...
                                       const std::vector<float>* pvthreshd_per_head = nullptr)
{
    ...
    auto gpu_out = to_bhsd(output_host, o_perm);
    ck_tile::HostTensor<T> ref_out({batch, nhead, seqlen_q, hdim_v});
    ck_tile::reference_blocked_attention<T, uint8_t, BiasT>(q_ref,
                                                            k_ref,
                                                            v_ref,
                                                            block_mask,
                                                            ref_out,
                                   ...
                                   const char* api_label)
{
    ...
    {
        s.elementwise_host.emplace(
            generate_random_bias<BiasT>(bi.rank_info, batch, nhead, seqlen_q, seqlen_k, seed));
    ...
    for(ck_tile::index_t b = 0; b < outer; ++b)
        s.alibi_slopes_host.insert(
            s.alibi_slopes_host.end(), base_slopes.begin(), base_slopes.end());
...
slice_elementwise_bias_to_b1(const ck_tile::HostTensor<BiasT>& full_bias,
                             int bias_rank,
                             ck_tile::index_t b,
                             ck_tile::index_t nhead,
                             ck_tile::index_t sq,
...
// Mode mirrors make_alibi_from_lr_mask: is_causal (left<0 && right==0) -> VERTICAL; else
// causal_type.
template <typename BiasT>
inline ck_tile::HostTensor<BiasT> alibi_to_dense(const std::vector<float>& slopes_host,
                                                 ck_tile::index_t nhead,
                                                 ck_tile::index_t seqlen_q,
                                                 ck_tile::index_t seqlen_k,
                                                 int window_left,
                                                 ...
                                                 int causal_type)
{
    ...
    for(ck_tile::index_t h = 0; h < nhead; ++h)
    {
        ...
        {
            const ck_tile::long_index_t zp =
                (mode == 0) ? srd : (static_cast<ck_tile::long_index_t>(i) + srd);
...
inline ck_tile::HostTensor<uint8_t> generate_random_block_mask(ck_tile::index_t batch,
                                                               ck_tile::index_t nhead,
...
inline std::vector<int32_t> generate_seqlens_group(
    int32_t batch, int32_t seqlen_avg, int32_t seqlen_min, int32_t seqlen_max, uint32_t seed)
{
...
generate_random_block_mask_group(const std::vector<int32_t>& q_blocks_per_b,
                                 ...
                                 std::vector<int32_t>& mask_batch_offsets_out)
{
    ...
    // 2D alloc for sizing only; data written via the packed linear index (kernel reads flat
    // buffer).
    ...
    for(ck_tile::index_t h = 0; h < nhead; ++h)
    {
        for(int32_t b = 0; b < batch; ++b)
        {
            ...
            const int64_t blk0 =
                static_cast<int64_t>(base) * nhead + static_cast<int64_t>(h) * qb_n * kb_n;
            for(int32_t qb = 0; qb < qb_n; ++qb)
                for(int32_t kb = 0; kb < kb_n; ++kb)
                {
                    const bool is_diag               = ensure_diagonal && (qb == kb);
                    mask_data[blk0 + qb * kb_n + kb] = (is_diag || dist(rng) > sparsity) ? 1 : 0;
                }
        }
    }
    return mask;
...
// Shared per-(b,h,q) row helpers for LUT delta-encode/decode. Both encoders (batch + group) and
// both decoders (batch + group) call these on a contiguous row; only the base-offset math differs
// upstream.

// Encode one row: block-map[num_block_k] (0/1, >0.5f test) -> delta-encoded LUT row, zero-padded to
// num_block_k, returns valid_block_num. First selected block is stored absolute (delta-from-0);
// each subsequent one is (kb - prev). Mirrors the original inner loops exactly.
...
inline void
decode_lut_row(const int32_t* lut_row, int32_t vbn, uint8_t* map_row, int32_t num_block_k)
{
    int32_t acc = 0;
...
inline void block_map_to_lut_group(const ck_tile::HostTensor<uint8_t>& packed_mask,
                                   const std::vector<int32_t>& q_blocks_per_b,
                                   const std::vector<int32_t>& k_blocks_per_b,
                                   const std::vector<int32_t>& mask_batch_offsets,
                                   ...
                                   ck_tile::HostTensor<int32_t>& vbn_packed)
{
    ...
    for(ck_tile::index_t h = 0; h < nhead; ++h)
    {
        for(int32_t b = 0; b < batch; ++b)
        {
            ...
            const int64_t lut_blk0 =
                static_cast<int64_t>(mask_base) * nhead + static_cast<int64_t>(h) * qb_n * kb_n;
            const int64_t vbn_blk0 =
                static_cast<int64_t>(vbn_base) * nhead + static_cast<int64_t>(h) * qb_n;
            for(int32_t qb = 0; qb < qb_n; ++qb)
            {
                const int64_t row       = lut_blk0 + static_cast<int64_t>(qb) * kb_n;
                vbn_data[vbn_blk0 + qb] = encode_lut_row(mask_data + row, lut_data + row, kb_n);
            }
        }
    }
}
...
// Decode device delta-encoded LUT + valid-block counts into a dense [b,h,qb,kb] 0/1 map, so the
// sage validation references the kernel's actual selection instead of re-predicting on the host.
inline ck_tile::HostTensor<uint8_t> device_lut_to_block_map(const std::vector<int32_t>& lut,
...
// Group variant of device_lut_to_block_map. Packed batch-outer/head-mid (see
// block_map_to_lut_group): mask_base = prefix sum of q_blocks*k_blocks, vbn_base = prefix sum of
// q_blocks. Decodes sub-batch b into [1, nhead, qb_b, kb_b].
inline ck_tile::HostTensor<uint8_t> device_lut_to_block_map_group(const std::vector<int32_t>& lut,
                                                                  const std::vector<int32_t>& vbn,
                                                                  ck_tile::index_t nhead,
                                                                  ...
                                                                  ck_tile::index_t kb_b)
{
    ...
    for(ck_tile::index_t h = 0; h < nhead; ++h)
        ...
        {
            const size_t vidx =
                static_cast<size_t>(vbn_base) * nhead + static_cast<size_t>(h) * qb_b + qb;
            ...
            decode_lut_row(
                lut.data() + lbase, vbn[vidx], bm_data + mbase, static_cast<int32_t>(kb_b));
        }
    return bm;
}
                                      ...
                                      const std::string& label)
{
    ...
    const bool gross = (rate < 0.5) || (host_sel > 0 && (double(dev_sel) / double(host_sel) < 0.5 ||
...
inline std::size_t compute_sparse_attn_flop(ck_tile::index_t batch,
                                            ck_tile::index_t nhead,
                                            ck_tile::index_t seqlen_q,
                                            ck_tile::index_t seqlen_k,
                                            ...
                                            const mask_info& mask)
{
    ...
    {
        effective_area =
            static_cast<std::size_t>(static_cast<double>(seqlen_q) * seqlen_k * (1.0 - sparsity));
    }
    else
    {
        effective_area =
            static_cast<std::size_t>(static_cast<double>(mask.get_unmaskarea()) * (1.0 - sparsity));
    }
...
template <typename T>
std::size_t compute_sparse_attn_num_byte(ck_tile::index_t batch,
                                         ck_tile::index_t nhead,
                                         ck_tile::index_t nhead_k,
                                         ck_tile::index_t seqlen_q,
                                         ck_tile::index_t seqlen_k,
...
inline std::size_t compute_sparse_attn_flop_group(ck_tile::index_t nhead,
    ...
    for(size_t b = 0; b < seqlen_qs.size(); ++b)
    {
        ...
        const std::size_t mask_area =
            (m.type == mask_enum::no_mask)
                ...
                : m.get_unmaskarea();
        const auto area =
            static_cast<std::size_t>(static_cast<double>(mask_area) * (1.0 - sparsity));
...
template <typename T>
std::size_t compute_sparse_attn_num_byte_group(ck_tile::index_t nhead,
                                               ck_tile::index_t nhead_k,
                                               const std::vector<int32_t>& seqlen_qs,
                                               const std::vector<int32_t>& seqlen_ks,
                                               ck_tile::index_t hdim_q,
                                               ...
                                               float sparsity)
{
    ...
    for(auto v : seqlen_qs)
        total_q += v;
    for(auto v : seqlen_ks)
        total_k += v;

    std::size_t num_byte =
        static_cast<std::size_t>(total_q) * nhead * (sizeof(T) * hdim_q + sizeof(T) * hdim_v);
    const auto kv_dense =
        static_cast<std::size_t>(total_k) * nhead_k * (sizeof(T) * hdim_q + sizeof(T) * hdim_v);
    num_byte += static_cast<std::size_t>(static_cast<double>(kv_dense) *
                                         (1.0 - static_cast<double>(sparsity)));
    return num_byte;
}

inline void print_perf(double avg_time_ms, std::size_t flop, std::size_t num_byte)
{
                              ...
                              const std::vector<int32_t>& seqlen_ks)
{
    ...
    std::ostringstream oss;
    oss << std::fixed;
    oss << "{\"api\":\"" << api << "\"," << "\"mode\":\"" << mode << "\"," << "\"prec\":\"" << prec
        << "\"," << "\"batch\":" << batch << "," << "\"nhead\":" << nhead << ","
        << "\"nhead_k\":" << nhead_k << "," << "\"seqlen_q\":" << seqlen_q << ","
        << "\"seqlen_k\":" << seqlen_k << "," << "\"hdim_q\":" << hdim_q << ","
        << "\"hdim_v\":" << hdim_v << "," << "\"sparsity\":" << std::setprecision(4) << sparsity
        << "," << "\"mask_type\":\"" << mask_str << "\"," << "\"iperm\":" << (i_perm ? 1 : 0) << ","
        << "\"operm\":" << (o_perm ? 1 : 0) << "," << "\"latency_ms\":" << std::setprecision(4)
        << latency_ms << "," << "\"tflops\":" << std::setprecision(2) << tflops << ","
...
sparse_attn_result sparse_attn_fwd_run(
    ...
    const std::string& sparsity_per_head_csv, // sparge only: CSV per-Q-head sparsity; routed to
                                              // topk/cdf field by sparge_mode
    const std::string& sim_per_head_csv,
    const std::string& pvthreshd_per_head_csv,
    bool smooth_k,
    bool print_sparsity,
    ...
    const std::string& qkdtype   = "int8")      // sparge_sage Q/K quant dtype: int8 | fp8
{
    ...
    float scale =
        (scale_s_user != 0.0f) ? scale_s_user : 1.0f / std::sqrt(static_cast<float>(hdim_q));
    ...
    std::cout << "[" << api << "] b=" << batch << ", h=" << nhead << "(" << nhead_k << ")"
              << ", s=" << seqlen_q << "x" << seqlen_k << ", d=" << hdim_q << "(" << hdim_v << ")"
              << ", sparsity=" << sparsity << ", mask=" << mask_str << ", sink=" << attention_sink
              << ", perm=" << i_perm << "/" << o_perm << std::flush;

    std::size_t flop = compute_sparse_attn_flop(
        batch, nhead, seqlen_q, seqlen_k, hdim_q, hdim_v, sparsity, mask_decoded);
    std::size_t num_byte = compute_sparse_attn_num_byte<T>(
        batch, nhead, nhead_k, seqlen_q, seqlen_k, hdim_q, hdim_v, sparsity);
    ...
    if(api == "jenga" && mode == sparse_attn_mode::batch)
    {
        ...
        auto bs      = setup_bias<BiasT>(
            bi, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "jenga");
        if(bs.args.type < 0)
            return sparse_attn_result::failure;

        try
        {
            ave_time = jenga_sparse_attention<T>(q_host,
                                                 k_host,
                                                 v_host,
                                                 block_mask,
                                                 output_host,
        ...
        if(do_validation)
        {
            ...
            {
                auto alibi_dense = alibi_to_dense<BiasT>(bs.alibi_slopes_host,
                                                         nhead,
                                                         seqlen_q,
                                                         seqlen_k,
                                                         mask_decoded.left,
            ...
            {
                pass = validate_vs_blocked_ref<T, DataTypeConfig>(q_host,
                                                                  k_host,
                                                                  v_host,
                                                                  output_host,
                                                                  block_mask,
    ...
    else if(api == "jenga" && mode == sparse_attn_mode::group)
    {
        ...
        bias_info bi = bias_info::decode(bias_str);
        auto bs      = setup_bias<BiasT>(
            bi, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "jenga group");
        if(bs.args.type < 0)
            return sparse_attn_result::failure;

        try
        {
            ave_time = jenga_sparse_attention<T>(q_packed,
                                                 k_packed,
                                                 v_packed,
                                                 mask_packed,
                                                 o_packed,
        ...
        std::cout << ", sq:" << format_int_vec(seqlen_qs) << ", sk:" << format_int_vec(seqlen_ks)
                  << std::flush;
        if(stream_config.time_kernel_)
        ...
        // Validate by slicing each sequence into a B=1 sub-batch; one combined valid:y/n at the
        // end.
        if(do_validation)
        {
            pass = true;
            for(int32_t b = 0; b < batch; ++b)
            {
                ...
                auto q_b =
                    slice_packed_to_b1(q_packed, seqstart_q_host[b], sq, nhead, hdim_q, i_perm);
                auto k_b =
                    slice_packed_to_b1(k_packed, seqstart_k_host[b], sk, nhead_k, hdim_q, i_perm);
                auto v_b =
                    slice_packed_to_b1(v_packed, seqstart_k_host[b], sk, nhead_k, hdim_v, i_perm);
                auto o_b =
                    slice_packed_to_b1(o_packed, seqstart_q_host[b], sq, nhead, hdim_v, o_perm);
    ...
    else if(api == "vsa" && mode == sparse_attn_mode::batch)
    {
        ...
        bias_info bi = bias_info::decode(bias_str);
        auto bs =
            setup_bias<BiasT>(bi, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "vsa");
        if(bs.args.type < 0)
            return sparse_attn_result::failure;

        try
        {
            ave_time = vsa_sparse_attention<T>(q_host,
                                               k_host,
                                               v_host,
                                               lut,
                                               valid_block_num,
    ...
    else if(api == "vsa" && mode == sparse_attn_mode::group)
    {
        ...
        block_map_to_lut_group(mask_packed,
                               q_blocks,
                               k_blocks,
                               mask_batch_offsets,
                               seqstart_q_block_host,
        ...
        bias_info bi = bias_info::decode(bias_str);
        auto bs      = setup_bias<BiasT>(
            bi, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "vsa group");
        if(bs.args.type < 0)
            return sparse_attn_result::failure;

        try
        {
            ave_time = vsa_sparse_attention<T>(q_packed,
                                               k_packed,
                                               v_packed,
                                               lut_packed,
                                               vbn_packed,
    ...
    else if(api == "sparge" && mode == sparse_attn_mode::batch)
    {
        ...
        {
            std::cerr << "error: -sparge_mode must be 'topk' or 'cdf', got '" << sparge_mode
                      << "'\n";
            return sparse_attn_result::failure;
        }
        ...
        bias_info bi = bias_info::decode(bias_str);
        auto bs      = setup_bias<BiasT>(
            bi, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "sparge");
        if(bs.args.type < 0)
            return sparse_attn_result::failure;
        ...
        // Opt-in actual sparsity (extra hipMemcpy + host sum); else TFlops uses the -sparsity
        // input, which diverges from the realised ratio when CDF / sim / sink kick in.
        ...
        try
        {
            ave_time = sparge_sparse_attention<T>(q_host,
                                                  k_host,
                                                  v_host,
                                                  output_host,
                                                  batch,
        ...
        }
        {
            ...
            const std::size_t flop_used     = have_actual ? compute_sparse_attn_flop(batch,
                                                                                 nhead,
                                                                                 seqlen_q,
                                                                                 seqlen_k,
                                                                                 hdim_q,
            ...
            const std::size_t num_byte_used = have_actual
                                                  ? compute_sparse_attn_num_byte<T>(batch,
                                                                                    nhead,
                                                                                    nhead_k,
                                                                                    seqlen_q,
                                                                                    seqlen_k,
        ...
        if(do_validation)
        {
            ...
            auto rp = make_sparge_predict_params(
            ...
            // Numeric reference = the device's actual selection (decoded LUT); host prediction is
            // the selection soft-check oracle. See check_selection_agreement.
            ...
            auto host_mask    = ck_tile::reference_sparge_mask_prediction<T>(q_ref,
                                                                          k_ref,
                                                                          batch,
                                                                          nhead,
                                                                          nhead_k,
            ...
            else if(bi.type == bias_enum::alibi)
            {
                ...
                auto alibi_dense = alibi_to_dense<float>(bs.alibi_slopes_host,
                                                         nhead,
                                                         seqlen_q,
                                                         seqlen_k,
                                                         mask_decoded.left,
    ...
    else if(api == "sparge_sage" && mode == sparse_attn_mode::batch)
    {
        ...
        else
        {
            ...
            {
                std::cout << ", sparge_sage: alibi requires a causal mask (-mask=t/b)" << std::flush
                          << std::endl;
                return sparse_attn_result::skipped;
            }
            auto bs_sage = setup_bias<float>(
                bi_sage, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "sparge_sage");
            ...
            // Host V prequant: v_fp8 follows i_perm (device reads same layout as Q/K); v_dequant
            // BHSD.
            ck_tile::HostTensor<ck_tile::fp8_t> v_fp8 =
            ...
            host_prequant_v_fp8<T>(
                ...
                std::vector<int32_t>(static_cast<size_t>(batch), static_cast<int32_t>(seqlen_k)));

            float ave = -1.0f;
            std::vector<int32_t> dev_lut, dev_vbn; // device's selected blocks (delta-encoded)
            try
            {
                ave = sparge_sage_sparse_attention<T>(q_host,
                                                      k_host,
                                                      v_fp8,
                                                      v_descale,
                                                      output_host,
            ...
            if(do_validation)
            {
                ...
                const ck_tile::index_t nkb = (seqlen_k + block_size - 1) / block_size;
                auto device_mask =
                    device_lut_to_block_map(dev_lut, dev_vbn, batch, nhead, nqb, nkb);
                ...
                if(smooth_k)
                    k_km = ck_tile::compute_global_k_mean(k_ref, batch, nhead_k, seqlen_k, hdim_q);
                if(qs_pertensor)
                {
                            ...
                            for(ck_tile::index_t h = 0; h < nhead; ++h)
                                for(ck_tile::index_t s = 0; s < seqlen_q; ++s)
                                    for(ck_tile::index_t d = 0; d < hdim_q; ++d)
                                        q_deq(b, h, s, d) =
                                            ck_tile::type_convert<T>(q_qf(b, h, s, d) * q_sc(b, h));
                        for(ck_tile::index_t b = 0; b < batch; ++b)
                            for(ck_tile::index_t h = 0; h < nhead_k; ++h)
                                for(ck_tile::index_t s = 0; s < seqlen_k; ++s)
                                    for(ck_tile::index_t d = 0; d < hdim_q; ++d)
                                        k_deq(b, h, s, d) =
                                            ck_tile::type_convert<T>(k_qf(b, h, s, d) * k_sc(b, h));
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if(sage_qk_fp8)
                    {
                        ...
                        ck_tile::reference_sparge_rowwise_quant_fp8<T>(k_ref,
                                                                       batch,
                                                                       nhead_k,
                                                                       seqlen_k,
                                                                       hdim_q,
                    ...
                    else
                    {
                        ...
                        ck_tile::reference_sparge_rowwise_quant<T>(k_ref,
                                                                   batch,
                                                                   nhead_k,
                                                                   seqlen_k,
                                                                   hdim_q,
                ...
                {
                    using BiasT      = float;
                    auto alibi_dense = alibi_to_dense<BiasT>(bs_sage.alibi_slopes_host,
                                                             nhead,
                                                             seqlen_q,
                                                             seqlen_k,
                                                             mask_decoded.left,
                ...
                // int8 QK + fp8 V -> atol ~0.07; fp8 QK (3-bit mantissa) is coarser, relax to
                // ~0.18.
                const double q_rtol = sage_qk_fp8 ? 0.15 : 0.1;
                const double q_atol = sage_qk_fp8 ? 0.18 : 0.07;
                pass                = validate_tensors(gpu_out,
                                        ref_out,
                                        q_rtol,
                                        q_atol,
    ...
    else if(api == "sparge" && mode == sparse_attn_mode::group)
    {
        ...
        bias_info bi = bias_info::decode(bias_str);
        auto bs      = setup_bias<BiasT>(
            bi, batch, nhead, seqlen_q, seqlen_k, causal_type, seed + 500, "sparge group");
        if(bs.args.type < 0)
            return sparse_attn_result::failure;
        ...
        try
        {
            ave_time = sparge_sparse_attention<T>(q_packed,
                                                  k_packed,
                                                  v_packed,
                                                  o_packed,
                                                  batch,
        ...
        if(do_validation)
        {
            ...
            for(int32_t b = 0; b < batch; ++b)
            {
                ...
                // Numeric reference = device's actual selection (decoded packed LUT for this
                // sub-batch); host prediction is the selection soft-check oracle.
                auto device_mask_b =
                    device_lut_to_block_map_group(dev_lut,
                                                  dev_vbn,
                                                  nhead,
                                                  /*mask_base=*/mask_batch_offsets[b],
                                                  /*vbn_base=*/seqstart_q_block_host[b],
                                                  q_blocks_g[b],
                                                  k_blocks_g[b]);
                auto host_mask_b    = ck_tile::reference_sparge_mask_prediction<T>(q_ref_b,
                                                                                k_ref_b,
                                                                                /*batch=*/1,
                                                                                nhead,
                                                                                nhead_k,
    ...
    else if(api == "sparge_sage" && mode == sparse_attn_mode::group)
    {
        ...
        else
        {
            ...
            auto seqlen_qs = generate_seqlens_group(batch, sq_avg, sq_min, seqlen_q, seed + 300);
            auto seqlen_ks =
                (seqlen_q == seqlen_k && sq_min == sk_min)
                    ? seqlen_qs
                    : generate_seqlens_group(batch, sk_avg, sk_min, seqlen_k, seed + 301);
            const auto seqstart_q_host = to_seqstarts(seqlen_qs);
            const auto seqstart_k_host = to_seqstarts(seqlen_ks);
            ...
            try
            {
                ave = sparge_sage_sparse_attention<T>(q_packed,
                                                      k_packed,
                                                      v_fp8,
                                                      v_descale,
                                                      o_packed,
            ...
            if(do_validation)
            {
                ...
                for(int32_t b = 0; b < batch; ++b)
                {
                    ...
                    auto q_b =
                        slice_packed_to_b1(q_packed, seqstart_q_host[b], sq, nhead, hdim_q, i_perm);
                    auto k_b = slice_packed_to_b1(
                        k_packed, seqstart_k_host[b], sk, nhead_k, hdim_q, i_perm);
                    auto o_b =
                        slice_packed_to_b1(o_packed, seqstart_q_host[b], sq, nhead, hdim_v, o_perm);

                    auto q_ref_b = to_bhsd(q_b, i_perm);
                    auto k_ref_b = to_bhsd(k_b, i_perm);
                    // Numeric reference = the kernel's decoded LUT (truth); top-k diverges at
                    // near-tied block scores, corrupting ALIBI rows (see batch path).
                    auto device_mask_b =
                        device_lut_to_block_map_group(dev_lut,
                                                      dev_vbn,
                                                      nhead,
                                                      /*mask_base=*/lut_off_g[b],
                                                      /*vbn_base=*/q_block_off_g[b],
                                                      q_blocks_g[b],
                                                      k_blocks_g[b]);
                    ...
                    if(smooth_k)
                        k_km_b = ck_tile::compute_global_k_mean(k_ref_b, 1, nhead_k, sk, hdim_q);
                    if(qs_pertensor)
                    {
                        ...
                        if(sage_qk_fp8)
                        {
                            ...
                            for(ck_tile::index_t h = 0; h < nhead; ++h)
                                for(int32_t s = 0; s < sq; ++s)
                                    for(ck_tile::index_t d = 0; d < hdim_q; ++d)
                                        q_deq(0, h, s, d) =
                                            ck_tile::type_convert<T>(q_qf(0, h, s, d) * q_sc(0, h));
                            for(ck_tile::index_t h = 0; h < nhead_k; ++h)
                                for(int32_t s = 0; s < sk; ++s)
                                    for(ck_tile::index_t d = 0; d < hdim_q; ++d)
                                        k_deq(0, h, s, d) =
                                            ck_tile::type_convert<T>(k_qf(0, h, s, d) * k_sc(0, h));
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if(sage_qk_fp8)
                        {
                            ...
                            ck_tile::reference_sparge_rowwise_quant_fp8<T>(k_ref_b,
                                                                           1,
                                                                           nhead_k,
                                                                           sk,
                                                                           hdim_q,
                        ...
                        else
                        {
                            ...
                            ck_tile::reference_sparge_rowwise_quant<T>(k_ref_b,
                                                                       1,
                                                                       nhead_k,
                                                                       sk,
                                                                       hdim_q,
                    ...
                    for(ck_tile::index_t h = 0; h < nhead_k; ++h)
                        for(int32_t s = 0; s < sk; ++s)
                            for(ck_tile::index_t dv = 0; dv < hdim_v; ++dv)
                                v_deq_b(0, h, s, dv) = v_dequant(0, h, seqstart_k_host[b] + s, dv);
                    ...
                    bool sub_pass       = validate_tensors(gpu_out_b,
                                                     ref_out,
                                                     g_rtol,
                                                     g_atol,
                                                     std::string("sparge_sage group sub-batch ") +
                                                         std::to_string(b));
                    pass                = pass && sub_pass && sel_ok_b;
                }
                std::cout << ", valid:" << (pass ? "y" : "n") << std::flush << std::endl;
