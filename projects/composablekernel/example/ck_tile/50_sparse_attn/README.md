...
This folder contains examples for sparse attention using ck_tile tile-programming. Three sparse-mask variants are exposed through one executable.
...
```
    ...
    -qkdtype    sparge_sage Q/K quant dtype: int8 (i8fp8bf16) | fp8 (fp8bf16); V always fp8 (default:int8)
       ...
       -mode    kernel mode. 0:batch, 1:group (jenga + vsa + sparge) (default:0)
          ...
          -h    nhead for q (default:4)
        -h_k    nhead for k/v, -1 means equal to h (default:-1)
   ...
   -sparsity    target sparsity ratio [0,1). 0=dense (default:0.5)
                jenga / vsa: random-mask activation probability (skip ratio)
                sparge: passed to the algorithm chosen by -sparge_mode
-sparge_mode    sparge: block-selection algorithm (default:topk)
                  topk: deterministic 1-sparsity ratio of K-blocks per Q-block
                  cdf:  CDF threshold; greedy until cumulative softmax prob
                        reaches 1-sparsity (realised sparsity floats)
-simthreshold  sparge: cosine similarity threshold for fix blocks (default:0.0)
...
-{sparsity,sim,pvthreshd}_per_head
                sparge: comma-separated per-Q-head value, length=nhead_q
...
-logits_soft_cap Gemma-style logits soft cap; 0 ⇒ disabled (default:0)
       ...
       -bias    n or 0, no bias (default:n)
                e or 1, elementwise rank=0 (1*1*sq*sk, broadcast b/h)
                e:1 or 1:1, elementwise rank=1 (1*h*sq*sk, broadcast b)
                e:2 or 1:2, elementwise rank=2 (b*h*sq*sk, per-batch)
                a or 2, ALIBI rank=0 (1 slope per head, shared)
                a:1 or 2:1, ALIBI rank=1 (per-batch slope buffer)
...
Example 3: `./bin/tile_example_sparse_attn_fwd -api=sparge -b=1 -h=4 -s=4096 -d=128 -sparsity=0.6 -sink=1` runs sparge with deterministic 60% K-block skipping per Q-block (topk is the default `-sparge_mode`) plus an attention sink (always include the first K block).
...
All three variants support both `b*h*s*d` (`-iperm=1`, BHSD) and `b*s*h*d` (`-iperm=0`, BSHD). Output permutation via `-operm=0/1` is also supported. Internally the kernels accept arbitrary `stride_q/k/v/o`, so other layouts can be exercised by editing the host args directly.

### attention bias
Attention bias is supported by all three APIs in batch + group mode, with the single exclusion being **jenga + ALIBI** — jenga's compute path ignores the mask, so ALIBI's distance encoding diverges from the CPU reference; use vsa or sparge instead.
...
### alibi
ALiBi (Attention Linear Bias) is supported via `-bias=a[:rank]` for vsa and sparge. Slopes are computed via the standard `get_alibi_slopes(nhead)` formula and require a causal mask (`-mask=t/b`). `a:0` uses `[1,h]` slopes shared across batch; `a:1` uses `[b,h]` per-batch slopes.

### scale_s and logits soft cap
`-scale_s=X` overrides the default `1/sqrt(d)` softmax scale (RoPE-aware models, fixed-scale eval, ablations). `-logits_soft_cap=C` enables Gemma-2/3 style soft cap `s = C * tanh(s * scale / C)` pre-softmax, required by the Gemma family. Soft cap is **NO_BIAS only** (kernel + pipeline static_assert) — pair with `-bias=n`.
...
VSA applies the mask at element level. Sparge additionally excludes blocks entirely outside the mask boundary at the prediction stage. **Jenga batch mode rejects `t/b/swa`** — its pipeline ignores the mask in compute, so kernel ↔ CPU reference would diverge; use `-api=vsa` or `-api=sparge` for masked workloads (jenga group + mask is fine).
...
At the C++ API level, `ck_tile::sparge_hyperparam_args` still exposes both `cdfthreshd` and `topk` fields independently (set the active one and leave the other at 0); the CLI's `-sparge_mode` only affects how the example wires the unified `-sparsity` value into those fields.

### sparge — similarity threshold
...
### sparge — per-head hyperparameters
At the C++ API level, `ck_tile::sparge_hyperparam_args` carries one device pointer per Q-head (length `nhead_q`, float) for `cdfthreshd` / `topk` / `simthreshold` / `pvthreshd`. `nullptr` falls back to the scalar. From the CLI, `-perhead_test=1` synthesizes a deterministic per-head pattern (requires `-h >= 2`), and `-{sparsity,sim,pvthreshd}_per_head=v0,v1,...` supplies per-head values explicitly (length must equal `nhead_q`). `-sparsity_per_head` is converted to `1 - sparsity[h]` and routed to either the `topk` or the `cdfthreshd` field according to `-sparge_mode`; the other field stays at 0. Per-field precedence: explicit CSV > `-perhead_test` > scalar.
