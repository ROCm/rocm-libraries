...
template <typename InOutDataType,
          ...
          // infer_h8_opt: sched_group_barrier MFMA clustering (no s_setprio).
          bool kUseSchedGroup    = false,
          // Round-3 #1: TrLoad opt-in (xor-swizzled LDS +
          // `ds_read_tr*` + buffer_load). See sibling jagged-fwd
          // dispatcher for the gfx942 caveat (silent zero-V reads).
          bool kUseTrLoad        = false,
          // Round-3 #2: per-CU block budget. -1 = pipeline default.
...
struct jagged_forward_splitkv_causal_softmax_bias_dropout_dispatch
{
    ...
    // No-softmax path uses the parametric ...TileSettingW (see jagged
    // non-splitkv dispatcher for rationale); softmax path keeps the
    // original 2-arg tile setting.
    ...
    // Round-3 #1: opt-in TrLoad enable (see jagged non-splitkv
    // sibling dispatcher for caveat — gfx942 silently reads zero
    // from `ds_read_tr*` paths because `amd_transpose_load_to_vgpr`
    // is `#if defined(__gfx950__)`-gated).
    ...
    static void RunWithFwdSplitKVKernel(HstuAttentionNoGroupFwdParams& param, hipStream_t stream)
    {
        ...
        // the legacy auto heuristic. num_splits is a runtime param, so this does
        // not require a new compiled kernel — only the launch grid / combine
        // bucket change.
        param.num_splits =
