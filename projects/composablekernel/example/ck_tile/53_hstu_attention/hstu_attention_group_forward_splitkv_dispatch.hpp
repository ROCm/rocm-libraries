...
struct group_forward_splitkv_causal_softmax_bias_dropout_dispatch
{
    ...
    {
        // Sweep-axis override: a positive force_num_splits forces that split
        // count (clamped to the 64 hardware cap from MAX_SPLITS_SWITCH); 0 keeps
        // the legacy auto heuristic.
        param.num_splits =
            (param.force_num_splits > 0)
                ? ck_tile::min(param.force_num_splits, 64)
