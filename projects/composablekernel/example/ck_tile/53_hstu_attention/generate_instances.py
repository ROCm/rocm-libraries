...
def create_forward_instances(instance_dir: Path, headdims: List, modes=None) -> None:
    if modes is None:
        modes = ["jagged"]
    for mode in modes:
        for dtype in ["fp16", "bf16"]:
            for has_causal in [True, False]:
                for use_softmax in [False]:
...
def create_forward_instances_ref(instance_dir: Path, headdims: List, modes=None) -> None:
    if modes is None:
        modes = ["jagged"]
    for mode in modes:
        for dtype in ["fp16", "bf16"]:
...
if __name__ == "__main__":
    # Harness-focused subset: jagged fwd, bf16/fp16, causal variants, no softmax/bias,
    # head dims 64 and 128 (prod sweep uses D=128; d=64 supported for smaller models).
    headdims_fwd = [64, 128]
    harness_modes = ["jagged"]
    ...
    create_forward_instances(output_dir, headdims_fwd, harness_modes)
    create_forward_instances_ref(output_dir, headdims_fwd, harness_modes)
