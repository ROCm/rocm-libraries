...
def main():
    ...
    if args.compile_only:
        ...
        print(f"{'=' * 70}")
        # exhaustive mode probes which tiles compile, so failures are the signal there
        if failed > 0 and args.tiles != "exhaustive":
            sys.exit(1)
        return
    ...
    print(f"{'=' * 70}")
    if (failed > 0 and args.tiles != "exhaustive") or any(
        r.get("status") == "FAIL" for r in all_results
    ):
        sys.exit(1)
