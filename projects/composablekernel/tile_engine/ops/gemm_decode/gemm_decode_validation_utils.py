...
def is_tile_config_valid(tile: Dict[str, Any]) -> bool:
    """Return True iff the per-config entry satisfies the P0 invariants.
    ...
    if tile.get("output_axis", "smallM") != "smallM":
        return False
    if bool(tile.get("use_dot2", False)) or bool(tile.get("use_packed_fp32", False)):
        return False  # P0b features.
    return True
...
def is_trait_combination_valid(trait: Dict[str, Any]) -> bool:
    ...
    if bool(trait.get("has_bias", False)):
        return False  # bias epilogue lands in P0b.
    return True
