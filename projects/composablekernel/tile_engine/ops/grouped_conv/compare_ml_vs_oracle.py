...
def run_end_to_end_workflow(args):
    ...
    if args.shapes:
        ...
        for i, p in enumerate(problems):
            print(f"  {i}: N={p['N']} C={p['C']} K={p['K']} Hi={p['Hi']}x{p['Wi']} Y={p['Y']}x{p['X']}")
        ...
        try:
            ...
            problem_attr = args.problem_set.upper().replace('_', '_').replace('FORWARD', 'PROBLEMS_FORWARD')
            if not hasattr(problem_module, problem_attr):
                # Try alternate naming
                problem_attr = [attr for attr in dir(problem_module) if 'PROBLEM' in attr.upper()][0]
            problems_list = getattr(problem_module, problem_attr)
            problems = []
            for prob in problems_list:
                problems.append({
                    'N': prob.N, 'C': prob.C, 'K': prob.K, 'G': prob.G,
                    'Hi': prob.Hi, 'Wi': prob.Wi, 'Y': prob.Y, 'X': prob.X,
                    'stride_h': prob.stride_h, 'stride_w': prob.stride_w,
                    'pad_h': prob.pad_h, 'pad_w': prob.pad_w,
...
Examples:
  ...
  # Use manual workflow instead - see error message when attempting Mode 2
    ...
    parser.add_argument('--shapes', nargs='+', help='Custom shapes (e.g., "N=1,C=64,K=64,Hi=28,Wi=28,Y=3,X=3,stride_h=1,stride_w=1")')
    parser.add_argument('--problem-set', help='Problem set module name (e.g., forward_validation_300)')
    parser.add_argument('--variant', default='forward', choices=['forward', 'bwd_data', 'bwd_weight'])
    ...
    parser.add_argument('--plot', default=None, help='Generate scatter plot PNG (optional)')
    ...
    print(f"{'Prob':<6} {'Oracle Best':<30} {'ML Top-1':<30} {'Oracle TFLOPS':<15} {'ML Actual TFLOPS':<18} {'Efficiency':<12}")
    print("-" * 135)
    ...
    for prob_idx in sorted(oracle.keys()):
        ...
        oracle_short = oracle_best_name.split('_')[-2] + '_' + oracle_best_name.split('_')[-1]
        ml_short = ml_picked_name.split('_')[-2] + '_' + ml_picked_name.split('_')[-1]

        print(f"{prob_idx:<6} {oracle_short:<30} {ml_short:<30} "
    ...
    if efficiencies:
        ...
        p10_eff = sorted_eff[len(sorted_eff) // 10] if len(sorted_eff) >= 10 else sorted_eff[0]
        ...
        print()
        print(f"Top-1 Accuracy:        {top1_matches}/{total_problems} ({100.0*top1_matches/total_problems:.1f}%)")
        print(f"Top-5 Hit Rate:        {top5_matches}/{total_problems} ({100.0*top5_matches/total_problems:.1f}%)")
        ...
        if args.output:
            with open(args.output, 'w', newline='') as f:
                ...
                writer.writerow(['top1_accuracy', f"{100.0*top1_matches/total_problems:.1f}"])
                writer.writerow(['top5_hit_rate', f"{100.0*top5_matches/total_problems:.1f}"])
        ...
        if args.plot:
            try:
                ...
                # Color by efficiency
                scatter = ax.scatter(oracle_tflops_list, ml_tflops_list,
                                    c=efficiencies_arr,
                                    cmap='RdYlGn',
                                    vmin=60, vmax=100,
                                    alpha=0.7,
                ...
                ax.plot([min_val, max_val], [min_val, max_val],
                       'r--', linewidth=2.5, label='Perfect Prediction (Y=X)', alpha=0.8, zorder=5)

                # Add efficiency lines
                ax.plot([min_val, max_val], [0.9*min_val, 0.9*max_val],
                       'orange', linestyle=':', linewidth=2, label='90% Efficiency', alpha=0.7, zorder=4)
                ax.plot([min_val, max_val], [0.8*min_val, 0.8*max_val],
                       'gold', linestyle=':', linewidth=2, label='80% Efficiency', alpha=0.7, zorder=4)
                ax.plot([min_val, max_val], [0.7*min_val, 0.7*max_val],
                       'yellow', linestyle=':', linewidth=1.5, label='70% Efficiency', alpha=0.6, zorder=4)

                # Labels and title
                ax.set_xlabel('Oracle TFLOPS (Best Kernel)', fontsize=13, fontweight='bold')
                ax.set_ylabel('ML Heuristic TFLOPS (Top-1 Prediction)', fontsize=13, fontweight='bold')
                ax.set_title('ML Heuristic vs Oracle Performance\nGrouped Convolution Forward (bf16, gfx950)',
