...
def main():
    ...
    with open(all_configs, "r") as f:
        for i, line in enumerate(f):
            if i % 50000 == 0:
                print(
                    f"  Processed {i} lines, found {len(valid_shapes)} valid shapes..."
                )
    ...
    print(
        f"  Unique C values: {len(channels_c)} (range: {min(channels_c)} to {max(channels_c)})"
    )
    print(
        f"  Unique K values: {len(channels_k)} (range: {min(channels_k)} to {max(channels_k)})"
    )
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"# {len(sampled_shapes)} diverse problems sampled from MIOpen ALL_CONFIGS_FULL.txt\n"
        )
        f.write(
        ...
        f.write("]\n\n")
        f.write(
            f"assert len(TRAINING_PROBLEMS_FORWARD_MIOPEN) == {len(sampled_shapes)}, "
        )
        f.write(
            f'"Expected {len(sampled_shapes)} problems, got {{len(TRAINING_PROBLEMS_FORWARD_MIOPEN)}}"\n'
        )
