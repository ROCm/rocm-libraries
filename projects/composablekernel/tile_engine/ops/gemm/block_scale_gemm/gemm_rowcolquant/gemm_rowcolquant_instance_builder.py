...
class GemmRowColQuantKernelBuilder(GemmKernelBuilder):
    ...
    def _list_kernels(self):
        ...
        print(f"Found {len(tile_configs)} valid tile configurations after filtering")
        trait_combos = self._generate_trait_combinations()
        print(f"Found {len(trait_combos)} valid trait combinations after filtering")
        ...
        for tile_config in tile_configs:
            for trait_combo in trait_combos:
                kernels.append(
                    (
                        self._build_kernel_name(tile_config, trait_combo),
                        tile_config,
                        trait_combo,
                    )
                )
        ...
        with open(
            ...
            f.write(str(len(kernels)))

        with open(
            self.working_path / f"{self.kernel_name_prefix}_kernel_list.txt", "w"
        ) as f:
            for kernel_name, tile_config, trait_combo in kernels:
                tile_str = self._tile_config_string(tile_config)
                trait_str = (
                    f"{trait_combo[0]}_{trait_combo[1]}_{trait_combo[2]}_"
                    + "_".join(str(x) for x in trait_combo[3:])
                )
                f.write(f"{kernel_name}|{tile_str}|{trait_str}\n")

        print(f"Listed {len(kernels)} rowcolquant kernel configurations")
