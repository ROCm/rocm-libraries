...
class FlatmmBenchmark:
    ...
    def discover_kernels(self) -> List[Path]:
        """Find all benchmark_flatmm_* executables in the build directory"""
        bin_dir = self.build_dir / "bin"
        if not bin_dir.exists():
            print(f"Error: Binary directory {bin_dir} does not exist")
            return []

        kernels = list(bin_dir.glob("benchmark_flatmm_*"))
        if self.verbose:
    ...
    def extract_kernel_info(self, kernel_path: Path) -> Dict[str, str]:
        ...
        # benchmark_flatmm_fp16_rcr_mem_default_intrawave_False_False_False_False_False_256x256x32_2x2x1_4x64x16
        parts = name.split("_")

        if len(parts) >= 3:
            # Extract data type (3rd part after benchmark_flatmm_)
...
def main():
    ...
    parser.add_argument(
        "--csv", default="flatmm_benchmark_results.csv", help="CSV output filename"
    )
    parser.add_argument(
    ...
    # Run benchmark sweep
    print("Starting Flatmm kernel benchmark sweep...")
