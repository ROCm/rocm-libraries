...
struct GemmProblem
{
    ...
    int batch_stride_a_, batch_stride_b_, batch_stride_c_;
    int batch_count_;
    ...
    friend std::ostream& operator<<(std::ostream& os, const GemmProblem& problem)
    {
        os << "{\n"
           ...
           << "   \"batch_stride_a\":" << problem.batch_stride_a_ << ",\n"
           << "   \"batch_stride_b\":" << problem.batch_stride_b_ << ",\n"
           << "   \"batch_stride_c\":" << problem.batch_stride_c_ << ",\n"
           << "   \"batch_count\":" << problem.batch_count_ << ",\n"
...
inline bool compare(std::string instanceName,
                    ck_tile::index_t K,
                                ...
                                ck_tile::index_t batch_count)
{
    if(verify == 1)
    {
        ...
        ck_tile::reference_batched_gemm<ADataType, BDataType, AccDataType, CDataType>(
            a_b_m_k, b_b_n_k, c_b_m_n_host_result);
    }
    else if(verify == 2)
    {
        ...
        ADataType* d_A = static_cast<ADataType*>(a_b_m_k_dev_buf.GetDeviceBuffer());
        BDataType* d_B = static_cast<BDataType*>(b_b_k_n_dev_buf.GetDeviceBuffer());
        CDataType* d_C = static_cast<CDataType*>(c_b_m_n_gpu_buf_ref.GetDeviceBuffer());
        ...
        c_b_m_n_gpu_buf_ref.FromDevice(c_b_m_n_host_result.data());
    }
}
