...
// CSV-driven CK Tile forward dataset test (AICK-1277).
//
// This mirrors test_grouped_convnd_fwd_tile.cpp, but instead of hardcoding the
// convolution shapes it loads them from the same CSV catalogue consumed by the
// Old CK test_grouped_convnd_fwd_dataset_xdl test
...
class TestGroupedConvndFwdDatasetTile : public ::testing::Test
{
    protected:
    ...
    void Run()
    {
        ...
        for(size_t i = 0; i < conv_args.size(); i++)
        {
            ...
            [[maybe_unused]] auto&& [case_passed, avg_time, op_name] =
...
template <typename SignatureDetailsType>
class TestGroupedConvndFwdDatasetTile2d
    : public TestGroupedConvndFwdDatasetTile<SignatureDetailsType>
{
};

template <typename SignatureDetailsType>
class TestGroupedConvndFwdDatasetTile3d
    : public TestGroupedConvndFwdDatasetTile<SignatureDetailsType>
{
};

TYPED_TEST_SUITE(TestGroupedConvndFwdDatasetTile2d, KernelTypes2d);
TYPED_TEST_SUITE(TestGroupedConvndFwdDatasetTile3d, KernelTypes3d);

TYPED_TEST(TestGroupedConvndFwdDatasetTile2d, Test2D)
{
...
TYPED_TEST(TestGroupedConvndFwdDatasetTile3d, Test3D)
{
    this->conv_args.clear();
