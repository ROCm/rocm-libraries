// Workload A (simple) -- V4 experimental, placeholder.
// Mirrors test_simple_v3_placeholder.cpp exactly. Same kernel shape:
//   * 1024 threads (4 blocks x 256), 10 mapCoord calls per loop iteration,
//     LOOP_ITERS=10000 default, N_TRIALS=100 default.
//
// V4 vs V3 surface mapping:
//   v3::runtime_dim<Id> (index_t)  ->  v4::placeholder<Id>{} (typed object)
//   v3::make_graph_bindings<g>(args...)  ->  v4::applyBindings<g>(RuntimeBindings)
//   v3::calculateOffset<g>(coords, gb)  ->  v4::mapCoord<g>(out, in, gb)
// One applyBindings per kernel launch; per-coord mapCoord in the hot loop.
...
constexpr auto make_simple_v4_runtime()
{
    // Shape constants live function-local (mirrors V3's `make_simple_v3_runtime()`
    // placement -- keeps `g`'s constexpr materialization at the same scope across
    // V3 and V4 for a fair compile-time comparison).
    using namespace v4;
    constexpr placeholder<0> K_DIV_p{};
    constexpr placeholder<1> MPB_p{};
    constexpr placeholder<2> K_MOD_p{};
    constexpr placeholder<3> s0_p{};
    constexpr placeholder<4> s1_p{};
    ...
    return make_transform_graph(
        outputs(read(OFF)),
        make_embed(dims(K_DIV_p, MPB_p, K_MOD_p),
                             strides(s0_p, s1_p, 1), read(S_KD, S_MP, S_KM), write(OFF)),
        make_merge(dims(K_DIV_p, K_MOD_p), read(S_K), write(S_KD, S_KM)),
...
{
    constexpr auto g = make_simple_v4_runtime();   // function-local g (mirrors V3)
...
int main()
{
    ...
    index_t h_args[NA];
    h_args[0] = KPerBlock / 8;
    h_args[1] = MPerBlock;
    h_args[2] = 8;
    h_args[3] = (MPerBlock + 1) * 8;
    h_args[4] = 8;
    h_args[5] = KPerBlock;
    ...
    for(int trial = 1; trial <= n_trials; ++trial)
    {
        ...
        std::fprintf(stderr, "simple v4 trial %d: %.4f ms\n", trial_base + trial, ms);
    }
