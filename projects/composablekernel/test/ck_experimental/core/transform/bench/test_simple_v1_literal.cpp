...
//
// Workload A (simple) — V1 type-based descriptor.
// 3-transform GEMM A-side LDS chain (Embed + PassThrough + Merge), 3D base.
...
CK_TILE_HOST_DEVICE constexpr auto make_a_lds_desc_v1()
{
...
{
    constexpr auto desc = make_a_lds_desc_v1<M, K>();
...
int main()
{
    ...
    for(int trial = 1; trial <= n_trials; ++trial)
    {
        ...
        std::fprintf(stderr, "simple v1 literal trial %d: %.4f ms\n", trial_base + trial, ms);
    }
