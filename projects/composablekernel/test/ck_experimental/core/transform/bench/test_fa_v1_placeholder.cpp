                            ...
                            const index_t* n_iters_ptr)
{
    ...
    for(index_t i=0;i<ni;++i){
        ...
        s += desc.calculate_offset(make_multi_index(b, h, sx, d)); s += desc.calculate_offset(make_multi_index(b, h, sx, d + 1)); s += desc.calculate_offset(make_multi_index(b, h + 1, sx, d)); s += desc.calculate_offset(make_multi_index(b + 1, h, sx, d)); s += desc.calculate_offset(make_multi_index(b, h, sx + 1, d));
    }
    out[tid]=s;
}
} // namespace
...
int main()
{
    ...
    for(int t=1;t<=nt;++t){
        ...
        std::fprintf(stderr,"fa v1 placeholder trial %d: %.4f ms\n",tb+t,ms);
    }
