                            ...
                            const index_t* n_iters_ptr)
{
    ...
    for(index_t i=0;i<ni;++i){
        ...
        s += v4::calculateOffset<g_lit>(static_array<index_t,4>{b, h, sx, d}); s += v4::calculateOffset<g_lit>(static_array<index_t,4>{b, h, sx, d + 1}); s += v4::calculateOffset<g_lit>(static_array<index_t,4>{b, h + 1, sx, d}); s += v4::calculateOffset<g_lit>(static_array<index_t,4>{b + 1, h, sx, d}); s += v4::calculateOffset<g_lit>(static_array<index_t,4>{b, h, sx + 1, d});
    }
    out[tid]=s;
}
} // namespace
...
int main()
{
    ...
    for(int t=1;t<=nt;++t){
        ...
        std::fprintf(stderr,"fa v4 literal trial %d: %.4f ms\n",tb+t,ms);
    }
