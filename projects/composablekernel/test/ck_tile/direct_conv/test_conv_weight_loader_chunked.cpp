...
class ConvWeightLoaderChunkedTest : public ::testing::Test
{
    ...
    static float
    read_weight(const std::vector<_Float16>& w, int kw, int C, int k, int r, int s, int c)
    {
    ...
    void run_and_verify()
    {
        ...
        ck_tile::hip_check_error(
            hipMemcpy(d_wei, wei.data(), wei.size() * sizeof(_Float16), hipMemcpyHostToDevice));

        test_chunked_weight_kernel<CfgIdx><<<dim3(1, 1, 1), BLOCK_SIZE_T>>>(d_wei, d_out, C);
        ck_tile::hip_check_error(hipDeviceSynchronize());

        std::vector<_Float16> out_host(out_elems);
        ck_tile::hip_check_error(
            hipMemcpy(out_host.data(), d_out, out_elems * sizeof(_Float16), hipMemcpyDeviceToHost));
                ...
                for(int S = 0; S < KW; ++S)
                {
                    ...
                    for(int CS = 0; CS < N_CSPW; ++CS)
                    {
                        ...
                        for(int j = 0; j < 8; ++j)
                        {
                            ...
                            const float expected = read_weight(wei, KW, C, k_out, R, S, c_abs);
                            const float actual =
                                static_cast<float>(out_host[(tid * SLOTS + slot) * 8 + j]);
                            EXPECT_EQ(actual, expected)
                                << "cfg=" << CfgIdx << " tid=" << tid << " wave=" << wave_id
                                << " R=" << R << " S=" << S << " CS=" << CS << " k_out=" << k_out
                                << " c_grp=" << c_grp_local << " c_abs=" << c_abs;
                        }
                    }
                }
            }
...
TEST_F(ConvWeightLoaderChunkedTest, Cfg49_CspwIs2) { run_and_verify<CFG_CSPW2_FPROP>(); }
