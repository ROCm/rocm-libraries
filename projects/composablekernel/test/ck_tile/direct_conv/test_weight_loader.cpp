...
//
// KernelConfigurations<>::configs_map.get(9)  — Fprop, vector_size=8 (default, full GROUP_SIZE)  → unpadded
// KernelConfigurations<>::configs_map.get(46) — Fprop, CyclicShift, vector_size=2               → c%2==0
// KernelConfigurations<>::configs_map.get(47) — Fprop, CyclicShift, vector_size=1               → any c
// ============================================================================
...
protected:
    ...
    static constexpr int BLOCK_GROUPS = KernelConfigurations<>::configs_map.get(CFG_VEC1).block_groups();
...
{
    auto p = make_params(4);
    p.groups = KernelConfigurations<>::configs_map.get(CFG_VEC1).block_groups() + 1; // not a multiple
    p.c_tot  = p.groups * 4;
    p.k_tot  = p.groups * 4;
...
protected:
    ...
    void run_and_verify(int c_per_group, int k_per_group)
    {
        using TC = ns_16c::TileConstants<ns_16c::KernelConfigurations<>::configs_map.get(CfgIdx)>;
        constexpr int LDS_FP16 = TC::Weight::WEIGHT_LDS_SIZE_UINT4 * 8;
        constexpr int BLOCK_SIZE = ns_16c::KernelConfigurations<>::configs_map.get(CfgIdx).block_size();
        constexpr int groups = ns_16c::KernelConfigurations<>::configs_map.get(CfgIdx).block_groups();
...
protected:
    ...
    void run_and_verify(int c_per_group, int k_per_group)
    {
        using TC = ns_8c::TileConstants<ns_8c::KernelConfigurations<>::configs_map.get(CfgIdx)>;
        constexpr int LDS_FP16 = TC::Weight::WEIGHT_LDS_SIZE_UINT4 * 8;
        constexpr int BLOCK_SIZE = ns_8c::KernelConfigurations<>::configs_map.get(CfgIdx).block_size();
        constexpr int groups = ns_8c::KernelConfigurations<>::configs_map.get(CfgIdx).block_groups();
