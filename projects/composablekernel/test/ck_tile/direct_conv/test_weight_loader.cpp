...
{
    protected:
    static constexpr int KH         = KernelConfigurations<>::configs_map.get(CFG_VEC1).kh;
    ...
    static float read_weight(const std::vector<_Float16>& wei,
                             ...
                             int c)
    {
        int idx = g * (k_per_group * KH * KW * c_per_group) + k * (KH * KW * c_per_group) +
                  (y * KW + x) * c_per_group + c;
    ...
    void run_and_verify(int c_per_group, int k_per_group)
    {
        ...
        test_weight_load_kernel<CfgIdx, Padding>
            <<<dim3(1, 1, 1), BLOCK_SIZE>>>(d_wei, d_lds_out, groups, c_per_group, k_per_group);
        ...
        for(int g = 0; g < groups; g++)
            for(int k = 0; k < GROUP_SIZE; k++)
                for(int yx = 0; yx < kh_kw; yx++)
                    ...
                    {
                        int lds_idx = g * GROUP_SIZE * kh_kw * GROUP_SIZE + k * kh_kw * GROUP_SIZE +
                                      yx * GROUP_SIZE + c;

                        float actual = static_cast<float>(lds_host[lds_idx]);
                        float expected =
                            (k < k_per_group && c < c_per_group)
                                ? read_weight(
                                      wei_host, k_per_group, c_per_group, g, k, yx / KW, yx % KW, c)
                                : 0.0f;

                        EXPECT_EQ(actual, expected) << "cfg=" << CfgIdx << " g=" << g << " k=" << k
                                                    << " yx=" << yx << " c=" << c;
                    }
...
class ValidConfigTest : public ::testing::Test
{
    ...
    static Conv2dParams make_params(int c_per_group, int k_per_group = 4)
    {
        ...
        p.q         = 32;
        p.groups    = groups;
        p.kh        = KernelConfigurations<>::configs_map.get(CFG_VEC1).kh;
        p.kw        = KernelConfigurations<>::configs_map.get(CFG_VEC1).kw;
        return p;
    }
};
...
TEST_F(ValidConfigTest, C4_valid_all)
{
    ...
    EXPECT_TRUE(is_valid_config(p, KernelConfigurations<>::configs_map.get(CFG_UNPADDED)));
    EXPECT_TRUE(is_valid_config(p, KernelConfigurations<>::configs_map.get(CFG_VEC2)));
...
TEST_F(ValidConfigTest, C3_only_vec1)
{
    ...
    EXPECT_FALSE(is_valid_config(p, KernelConfigurations<>::configs_map.get(CFG_UNPADDED)));
    EXPECT_FALSE(is_valid_config(p, KernelConfigurations<>::configs_map.get(CFG_VEC2)));
    EXPECT_TRUE(is_valid_config(p, KernelConfigurations<>::configs_map.get(CFG_VEC1)));
}
...
TEST_F(ValidConfigTest, NonAlignedGroups_rejected)
{
    auto p = make_params(4);
    p.groups =
        KernelConfigurations<>::configs_map.get(CFG_VEC1).block_groups() + 1; // not a multiple
    p.c_tot = p.groups * 4;
    p.k_tot = p.groups * 4;
    EXPECT_FALSE(is_valid_config(p, KernelConfigurations<>::configs_map.get(CFG_VEC1)));
}
...
{
    protected:
    static constexpr int KH = ns_16c::KernelConfigurations<>::configs_map.get(CFG_16C_VEC1).kh;
    static constexpr int KW = ns_16c::KernelConfigurations<>::configs_map.get(CFG_16C_VEC1).kw;
    ...
    void run_and_verify(int c_per_group, int k_per_group)
    {
        ...
        constexpr int BLOCK_SIZE =
            ns_16c::KernelConfigurations<>::configs_map.get(CfgIdx).block_size();
        constexpr int groups =
            ns_16c::KernelConfigurations<>::configs_map.get(CfgIdx).block_groups();
        ...
        test_weight_load_kernel_16c<CfgIdx, Padding>
            <<<dim3(1, 1, 1), BLOCK_SIZE>>>(d_wei, d_lds_out, groups, c_per_group, k_per_group);
        ck_tile::hip_check_error(hipDeviceSynchronize());
...
{
    protected:
    static constexpr int KH = ns_8c::KernelConfigurations<>::configs_map.get(CFG_8C_VEC1).kh;
    static constexpr int KW = ns_8c::KernelConfigurations<>::configs_map.get(CFG_8C_VEC1).kw;
    ...
    void run_and_verify(int c_per_group, int k_per_group)
    {
        ...
        constexpr int LDS_FP16 = TC::Weight::WEIGHT_LDS_SIZE_UINT4 * 8;
        constexpr int BLOCK_SIZE =
            ns_8c::KernelConfigurations<>::configs_map.get(CfgIdx).block_size();
        constexpr int groups =
            ns_8c::KernelConfigurations<>::configs_map.get(CfgIdx).block_groups();
        ...
        test_weight_load_kernel_8c<CfgIdx, Padding>
            <<<dim3(1, 1, 1), BLOCK_SIZE>>>(d_wei, d_lds_out, groups, c_per_group, k_per_group);
        ck_tile::hip_check_error(hipDeviceSynchronize());
