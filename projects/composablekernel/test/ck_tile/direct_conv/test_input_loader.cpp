                                       ...
                                       int k_per_group = 0)
{
    ...
    ck_tile::direct_conv::InputLoader<TC, cfg, ck_tile::fp16x4_t, Padding> il(
        bc, lds_buf, in, hi, wi, px, 0, 1, 1, 1, 1, c_per_group);
...
class InputLoaderTest : public ::testing::Test
{
    ...
    static float
    read_input(const std::vector<_Float16>& inp, int wi, int C_total, int h, int w, int c)
    {
            ...
            for(int c8 = 0; c8 < BLOCK_C8; c8++)
            {
                for(int c = 0; c < 8; c++)
                {
                    ...
                    EXPECT_EQ(actual, expected)
                        ...
                        << " c8_phys=" << c8_phys << " group=" << group_idx
                        << " gc=" << group_local_c << " w_actual=" << w_actual;
                }
            }
        }
