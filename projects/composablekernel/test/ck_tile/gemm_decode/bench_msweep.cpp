...
//
// For each M in [1, Mmax] and a fixed (N, K) it sweeps three levers
...
// instance. M, N, K are runtime, so every M reuses the same compiled kernels.
template <index_t kMPerWarp, index_t kNPerWarp, bool kChipletSwizzle,
          index_t kChipletNumXcds, index_t kChipletChunk>
float bench_one(index_t M, index_t N, index_t K, int warmup, int repeat,
...
// Sweep the chiplet chunk sizes (plus the swizzle-off point) for a fixed
// compile-time (kMPerWarp, kNPerWarp) register tile and fold every cell into
// `best`.
               ...
               const DeviceMem& a_buf, const DeviceMem& b_buf, DeviceMem& c_buf, Best& best)
{
    auto consider = [&](float t, bool swz, index_t chunk) {
        if(t < best.t_us)
            best = Best{t, kMPerWarp, kNPerWarp, swz, chunk};
    };
    consider(bench_one<kMPerWarp, kNPerWarp, false, 8, 8>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf),
             false, 8);
    consider(bench_one<kMPerWarp, kNPerWarp, true, 8, 4>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf),
             true, 4);
    consider(bench_one<kMPerWarp, kNPerWarp, true, 8, 8>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf),
             true, 8);
    consider(bench_one<kMPerWarp, kNPerWarp, true, 8, 16>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf),
             true, 16);
    consider(bench_one<kMPerWarp, kNPerWarp, true, 8, 32>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf),
             true, 32);
    consider(bench_one<kMPerWarp, kNPerWarp, true, 8, 64>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf),
             true, 64);
}
...
int main(int argc, char** argv)
{
    ...
    std::fprintf(stderr,
                 ...
                 "    sweeping (kMPerWarp,kNPerWarp) with mp*np<=16 x swizzle{off,chunk 4..64}\n",
                 int(N), int(K));

    // Clean CSV to stdout for the FlyDSL join.
    std::printf("impl,M,N,K,time_us,tflops,gbytes_s,mp,np,swizzle,chunk\n");

    for(index_t M = 1; M <= Mmax; ++M)
    {
        const float base_us =
            bench_one<1, 1, false, 8, 8>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf) * 1000.0f;

        // (mp, np) register-tile grid, pruned to mp*np <= 16 to cap the
        // accumulator/register footprint. mp>1 is the new B-reuse lever (A4),
        // np>1 the existing A-reuse lever (A1).
        Best best;
        sweep_cfg<1, 1>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<1, 2>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<1, 4>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<2, 1>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<2, 2>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<2, 4>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<4, 1>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<4, 2>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<4, 4>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<8, 1>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        sweep_cfg<8, 2>(M, N, K, warmup, repeat, a_buf, b_buf, c_buf, best);
        const float best_us = best.t_us * 1000.0f;

        std::fprintf(stderr,
                     "M=%2d  base=%8.2fus  best=%8.2fus (mp%d np%d %s chunk=%2d) "
                     "spd=%5.3fx  %6.2f TF/s  %7.1f GB/s\n",
                     int(M), base_us, best_us, int(best.m_per_warp), int(best.n_per_warp),
                     best.swizzle ? "swz" : "off", int(best.chunk),
                     base_us / best_us, tflops(M, best_us), gbps(M, best_us));

        std::printf("gemm_decode_base,%d,%d,%d,%.3f,%.3f,%.2f,1,1,off,0\n",
                    int(M), int(N), int(K), base_us, tflops(M, base_us), gbps(M, base_us));
        std::printf("gemm_decode_best,%d,%d,%d,%.3f,%.3f,%.2f,%d,%d,%s,%d\n",
                    int(M), int(N), int(K), best_us, tflops(M, best_us), gbps(M, best_us),
                    int(best.m_per_warp), int(best.n_per_warp),
                    best.swizzle ? "on" : "off", int(best.chunk));
        std::fflush(stdout);
    }
