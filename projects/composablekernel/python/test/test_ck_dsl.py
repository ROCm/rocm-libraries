...
class TestHelpers(unittest.TestCase):
    ...
    # NOTE: ``test_ck_dsl_provider_unified_compile_path`` was relocated to the
    # provider-owned, torch-free suite at
    # ``dnn-providers/ck-dsl-provider/python/tests/test_sdpa_generation.py``
    # (as ``test_relocated_2c_unified_compile_path`` plus the full generation
    # matrix) so the provider's SDPA-fwd coverage no longer lives in this
    # torch-importing DSL test module.
