...
static int make_spec(int idx, ckc_gemm_universal_spec_t *spec) {
    *spec = ckc_gemm_universal_spec_default();
    ...
    case 0:
        spec->name = "test1";
        spec->tile = (ckc_gemm_tile_spec_t){.tile_m = 128, .tile_n = 128, .tile_k = 32,
                                            .warp_m = 2, .warp_n = 2, .warp_k = 1,
                                            .warp_tile_m = 16, .warp_tile_n = 16,
                                            .warp_tile_k = 16};
        spec->trait.pipeline = "compv3"; spec->trait.epilogue = "default";
        spec->data.dtype_a = "fp16"; spec->data.dtype_b = "fp16";
        spec->data.dtype_c = "fp16"; spec->data.dtype_acc = "fp32";
        spec->wave_size = 64; spec->block_size = 256; spec->batched = false; break;
    case 3:
        spec->name = "test4";
        spec->tile = (ckc_gemm_tile_spec_t){.tile_m = 128, .tile_n = 256, .tile_k = 64,
                                            .warp_m = 4, .warp_n = 1, .warp_k = 1,
                                            .warp_tile_m = 16, .warp_tile_n = 16,
                                            .warp_tile_k = 32};
        spec->trait.pipeline = "mem"; spec->trait.epilogue = "default";
        spec->data.dtype_a = "fp16"; spec->wave_size = 64; spec->block_size = 256;
        spec->batched = false; break;
    case 4:
        spec->name = "test5";
        spec->tile = (ckc_gemm_tile_spec_t){.tile_m = 64, .tile_n = 64, .tile_k = 64,
                                            .warp_m = 1, .warp_n = 1, .warp_k = 1,
                                            .warp_tile_m = 16, .warp_tile_n = 16,
                                            .warp_tile_k = 16};
        spec->trait.pipeline = "compv3"; spec->trait.epilogue = "cshuffle";
        spec->trait.chiplet_swizzle = true; spec->data.dtype_a = "fp16";
        spec->wave_size = 64; spec->block_size = 64; spec->batched = false; break;
    case 5:
        spec->name = "test6";
        spec->tile = (ckc_gemm_tile_spec_t){.tile_m = 256, .tile_n = 256, .tile_k = 128,
                                            .warp_m = 4, .warp_n = 4, .warp_k = 1,
                                            .warp_tile_m = 32, .warp_tile_n = 32,
                                            .warp_tile_k = 16};
        spec->trait.pipeline = "compv4"; spec->trait.epilogue = "default";
        spec->trait.direct_to_lds = true; spec->data.dtype_a = "fp16";
        spec->wave_size = 64; spec->block_size = 1024; spec->batched = true; break;
    default:
        return -1;
    }
...
static int run_one(int idx) {
    ...
    /* Build the original kernel. ckc_build_universal_gemm_new is the
     * init-from-spec wrapper: it calls ckc_ir_builder_init on b0 itself, so the
    ...
    ckc_ir_builder_t b0;
    ckc_kernel_def_t *k0 = ckc_build_universal_gemm_new(&b0, &spec, "gfx950");
    if (!k0 || !ckc_ir_builder_ok(&b0)) {
        fprintf(stderr, "[%d] build failed: %s\n", idx, ckc_ir_builder_error(&b0));
        ckc_ir_builder_free(&b0);
        return 1;
    }

    /* serialize(k0) */
    char *s0 = NULL;
    if (ckc_ir_serialize(k0, &s0) != CKC_OK || !s0) {
    ...
    /* lower(k0) */
    char *ll0 = NULL;
    if (ckc_lower_kernel_to_llvm(k0, CKC_LLVM_FLAVOR_AUTO, "gfx950", &ll0) != CKC_OK || !ll0) {
    ...
    ckc_ir_builder_t b1;
    if (ckc_ir_builder_init(&b1, "parsed") != CKC_OK) {
        fprintf(stderr, "[%d] parse builder init failed\n", idx);
        free(s0); free(ll0); ckc_ir_builder_free(&b0);
        return 1;
    }
    ckc_kernel_def_t *k1 = NULL;
    if (ckc_ir_parse(s0, &b1, &k1) != CKC_OK || !k1) {
        fprintf(stderr, "[%d] parse failed: %s\n", idx, ckc_ir_builder_error(&b1));
        free(s0); free(ll0); ckc_ir_builder_free(&b0); ckc_ir_builder_free(&b1);
        return 1;
    }
    ...
    char *s1 = NULL;
    if (ckc_ir_serialize(k1, &s1) != CKC_OK || !s1) {
    ...
    char *ll1 = NULL;
    if (ckc_lower_kernel_to_llvm(k1, CKC_LLVM_FLAVOR_AUTO, "gfx950", &ll1) != CKC_OK || !ll1) {
    ...
    if (ckc_verify(k1, &diags, &ndiags) != CKC_OK) {
        ...
        for (size_t i = 0; i < ndiags; ++i) {
            if (diags[i].severity == CKC_DIAG_ERROR) {
                ++nerr;
    ...
    ckc_diags_free(diags, ndiags);

    if (rc == 0) {
        printf("PASS  [%d] %s  roundtrip+idempotent+verify-clean (.ll %zu bytes)\n",
               idx, spec.name, strlen(ll0));
    }

    free(s0); free(s1); free(ll0); free(ll1);
    ckc_ir_builder_free(&b0);
    ckc_ir_builder_free(&b1);
    return rc;
}
