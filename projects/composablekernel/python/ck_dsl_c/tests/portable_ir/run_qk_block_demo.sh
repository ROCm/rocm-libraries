...
python3 -m ck_dsl.portable_ir.examples.qk_block --emit recipe > "$OUT/qk.recipe.json"
echo ""
...
for D in 64 128 256; do
    ...
    python3 -m ck_dsl.portable_ir.examples.qk_block --emit ll --D "$D" --dtype f16 --arch "$ARCH" \
