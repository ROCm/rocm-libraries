...
# (ck.dsl.recipe/v1) automatically.
#
...
class RecipeRecorder:
    ...
    def param(self, name, type_json, **opts):
        self._emit({"op": "param", "name": name, "type": type_json, "bind": name, "attrs": opts})
        return V(name, type_json)

    # -- constants / ids --
    def const_i32(self, val):
        r = self._fresh("c")
        self._emit({"op": "emit", "opcode": "arith.constant", "out": {"bind": r, "type": "i32"},
                    "attrs": {"ity": _s("i32"), "value": {"t": "i", "v": _to_intexpr(val)}}})
        return V(r, "i32")

    def const_f32(self, val):
        r = self._fresh("c")
        self._emit({"op": "emit", "opcode": "arith.constant", "out": {"bind": r, "type": "f32"},
    ...
    def _emit_op(self, opcode, ins, rtype, attrs=None, hint="v"):
        r = self._fresh(hint)
        node = {"op": "emit", "opcode": opcode, "in": [v.reg for v in ins],
    ...
    def load_vN(self, ptr, idx, elem, vec, align=16):
        ...
        return self._emit_op("memref.global_load_vN", [ptr, idx], rtype,
    ...
    def load(self, ptr, idx, elem="f32", align=4):
        return self._emit_op("memref.global_load_typed", [ptr, idx], elem,
                             {"align": _i(align), "elem_type": _s(elem)})

    def store(self, ptr, idx, val, elem="f32", align=4):
        self._emit({"op": "emit", "opcode": "memref.global_store_typed",
                    "in": [ptr.reg, idx.reg, val.reg],
                    "attrs": {"align": _i(align), "elem_type": _s(elem)}})

    def yield_(self, *vals):
        self._emit({"op": "emit", "opcode": "scf.yield", "in": [v.reg for v in vals],
    ...
    def static_for(self, var, lo, hi, step=1):
        ...
        try:
            ...
            self._stack.pop()
            self._emit({"op": "static_for", "var": var, "lo": _to_intexpr(lo),
                        "hi": _to_intexpr(hi), "step": _to_intexpr(step), "body": body})
    ...
    def static_if(self, pred, then_fn, else_fn):
        ...
        self._stack.pop()
        self._emit({"op": "static_if", "pred": _to_intexpr(pred), "then": tbody, "else": ebody})
...
class _StaticForRec:
    ...
    def __exit__(self, *exc):
        ...
        self.rec._stack.pop()
        self.rec._emit({"op": "static_for", "var": self.var, "lo": _to_intexpr(self.lo),
                        "hi": _to_intexpr(self.hi), "step": _to_intexpr(self.step),
                        "body": self._body})
...
class _ForRec:
    def __init__(self, rec, lo, hi, step, iters, iv):
        self.rec, self.lo, self.hi, self.step, self.iters, self.iv = rec, lo, hi, step, iters, iv
        self.results = None
    ...
    def __exit__(self, *exc):
        ...
        self.rec._emit({
            "op": "scf_for", "iv": self.iv, "lo": self.lo.reg, "hi": self.hi.reg,
            "step": self.step.reg,
            "iter": [{"name": name, "init": init.reg} for name, init in self.iters],
            "results": res, "unroll": False, "elide_trailing_barrier": True, "body": self._body,
        })
...
def record_mini_attn():
    rec = RecipeRecorder(
        "ckdsl_mini_attn_norm{use_norm}_{dtype}",
    ...
    accf = loop.results[2]
    lf = loop.results[1]
    out = rec.static_if(rec.spec("use_norm"),
