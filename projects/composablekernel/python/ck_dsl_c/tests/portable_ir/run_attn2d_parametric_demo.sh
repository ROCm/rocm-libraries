...
python3 -m ck_dsl.portable_ir.roll_recipe --emit recipe > "$OUT/attn2d_param.recipe.json" 2> "$OUT/roll.err" || {
...
for D in 64 128 256; do
    ...
    python3 -m ck_dsl.portable_ir.kerneldef_to_recipe --emit ll --D "$D" --dtype fp16 --arch "$ARCH" \
    ...
    ckb=$(python3 -m ck_dsl.portable_ir.kerneldef_to_recipe --emit recipe --D "$D" --dtype fp16 2>/dev/null | wc -c)
