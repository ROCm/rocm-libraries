...
from ck_dsl.core.ir_export import export_kernel_ir_json
...
def build(dtype: str, head_size: int, seqlen: int, batch: int, num_heads: int, gqa: int):
...
def main() -> int:
    ...
    ap.add_argument("--gqa", type=int, default=1, help="query heads per kv head (1 = MHA)")
    ap.add_argument("--arch", default="gfx950")
    ap.add_argument("--ll", action="store_true", help="print Python-lowered .ll instead of IR JSON")
    args = ap.parse_args()

    kernel = build(args.dtype, args.head_size, args.seqlen, args.batch, args.num_heads, args.gqa)
    if args.ll:
