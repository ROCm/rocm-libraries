...
python3 -m ck_dsl.portable_ir.src.kerneldef_to_recipe --emit recipe --D "$D" --dtype "$DTYPE" --arch "$ARCH" \
...
python3 -m ck_dsl.portable_ir.src.kerneldef_to_recipe --emit ll --D "$D" --dtype "$DTYPE" --arch "$ARCH" \
