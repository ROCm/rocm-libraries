...
echo ">> building C++ core (libckc_core.a, includes the C++20 portable-IR tooling) -> libckc.so"
...
c++ -shared -fPIC -Wl,--whole-archive "$OUT/core/libckc_core.a" -Wl,--no-whole-archive -lm -o "$OUT/libckc.so" || {
