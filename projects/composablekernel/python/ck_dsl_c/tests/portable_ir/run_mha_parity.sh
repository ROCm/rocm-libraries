...
for dt in fp16 bf16; do
  for hd in 64 128 256; do
    tag="${dt}_d${hd}"
    python3 "$HERE/export_mha.py" --dtype "$dt" --head-size "$hd" --arch "$ARCH" \
        > "$OUT/$tag.ir.json" 2> "$OUT/$tag.exp.err" || { echo "EXPORT FAIL $tag"; cat "$OUT/$tag.exp.err"; rc=1; continue; }
    python3 "$HERE/export_mha.py" --dtype "$dt" --head-size "$hd" --arch "$ARCH" --ll \
...
for s in 2048 4096 8192; do
  python3 "$HERE/export_mha.py" --dtype fp16 --head-size 128 --seqlen "$s" --arch "$ARCH" > "$OUT/poly_s$s.json" 2>/dev/null
...
python3 "$HERE/export_mha.py" --dtype fp16 --head-size 128 --seqlen 2048 --num-heads 32 --gqa 1 --arch "$ARCH" > "$OUT/poly_mha.json" 2>/dev/null
python3 "$HERE/export_mha.py" --dtype fp16 --head-size 128 --seqlen 2048 --num-heads 32 --gqa 8 --arch "$ARCH" > "$OUT/poly_gqa.json" 2>/dev/null
