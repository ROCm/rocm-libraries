...
python3 -m ck_dsl.portable_ir.roll_recipe --emit recipe > "$OUT/param.recipe.json" 2>/dev/null

for D in 64 128 256; do
  echo ""
  echo "== fp16 MHA D$D =="
  python3 -m ck_dsl.portable_ir.export_mha --dtype fp16 --head-size "$D" --gqa 1 --num-heads 32 --arch "$ARCH" > "$OUT/pir_$D.json" 2>/dev/null
