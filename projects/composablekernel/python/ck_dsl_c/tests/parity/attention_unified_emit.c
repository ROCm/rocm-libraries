...
static int make_problem(int idx, ckc_unified_attention_problem_t *p) {
    *p = ckc_unified_attention_problem_default();
    switch (idx) {
    case 0:
        p->head_size = 64;  p->block_size = 32;  p->dtype = "bf16";
        p->num_query_heads = 64; p->num_kv_heads = 8;
        p->total_q = 128; p->max_seqlen_q = 512; p->max_seqlen_k = 512;
        p->use_sinks = true; p->softcap = 0.0;
        break;
    case 1:
        p->head_size = 128; p->block_size = 32;  p->dtype = "bf16";
        p->num_query_heads = 64; p->num_kv_heads = 8;
        p->total_q = 256; p->max_seqlen_q = 1024; p->max_seqlen_k = 1024;
        p->use_sinks = true; p->softcap = 0.0;
        break;
    case 2:
        p->head_size = 256; p->block_size = 16;  p->dtype = "fp16";
        p->num_query_heads = 32; p->num_kv_heads = 4;
        p->total_q = 64; p->max_seqlen_q = 256; p->max_seqlen_k = 256;
        p->use_sinks = false; p->softcap = 0.0;
        break;
    case 3:
        p->head_size = 64;  p->block_size = 16;  p->dtype = "bf16";
        p->num_query_heads = 16; p->num_kv_heads = 2;
        p->total_q = 512; p->max_seqlen_q = 2048; p->max_seqlen_k = 2048;
        p->use_sinks = true; p->sliding_window = 128; p->softcap = 0.0;
        break;
    case 4:
        p->head_size = 128; p->block_size = 32;  p->dtype = "bf16";
        p->num_query_heads = 64; p->num_kv_heads = 8;
        p->total_q = 1; p->max_seqlen_q = 1; p->max_seqlen_k = 4096;
        p->use_sinks = false; p->softcap = 0.0;
        break;
    case 5:
        p->head_size = 64;  p->block_size = 32;  p->dtype = "bf16";
        p->num_query_heads = 128; p->num_kv_heads = 16;
        p->total_q = 1024; p->max_seqlen_q = 4096; p->max_seqlen_k = 4096;
        p->use_sinks = true; p->softcap = 50.0;
        break;
    default:
        return -1;
    }
...
int main(int argc, char **argv) {
    ...
    kname[0] = 0;
    ckc_status_t nst =
        ckc_unified_attention_2d_scalar_kernel_name(&p, NULL, kname, sizeof kname);
    if (nst != CKC_OK) {
