...
static int make_spec(int idx, ckc_add_rmsnorm2d_rdquant_spec_t *spec) {
    *spec = ckc_add_rmsnorm2d_rdquant_spec_default();
    ...
    switch (idx) {
    case 0:
        spec->n_per_block = 4096; spec->dtype = "f16";
        spec->out_dtype = "i8"; spec->block_size = 256; spec->vec = 4;
        spec->save_residual = true; spec->save_yscale = true;
        spec->wave_size = 64;
        break;
    case 1:
        spec->n_per_block = 8192; spec->dtype = "f16";
        spec->out_dtype = "fp8e4m3"; spec->block_size = 256; spec->vec = 8;
        spec->save_residual = true; spec->save_yscale = true;
        spec->wave_size = 64;
        break;
    case 2:
        spec->n_per_block = 2048; spec->dtype = "bf16";
        spec->out_dtype = "bf8e5m2"; spec->block_size = 128; spec->vec = 4;
        spec->save_residual = false; spec->save_yscale = false;
        spec->wave_size = 64;
        break;
    case 3:
        spec->n_per_block = 16384; spec->dtype = "f16";
        spec->out_dtype = "i8"; spec->block_size = 256; spec->vec = 4;
        spec->save_residual = true; spec->save_yscale = true;
        spec->wave_size = 64;
        break;
    case 4:
        spec->n_per_block = 1024; spec->dtype = "bf16";
        spec->out_dtype = "i8"; spec->block_size = 64; spec->vec = 4;
        spec->save_residual = true; spec->save_yscale = true;
        spec->wave_size = 64;
        break;
    case 5:
        spec->n_per_block = 6144; spec->dtype = "f16";
        spec->out_dtype = "fp8e4m3"; spec->block_size = 256; spec->vec = 2;
        spec->save_residual = true; spec->save_yscale = false;
        spec->wave_size = 64;
        break;
    default:
        return -1;
...
int main(int argc, char **argv) {
    ...
    reason[0] = 0;
    if (!ckc_add_rmsnorm2d_rdquant_is_valid_spec(
            &spec, arch, reason, sizeof reason)) {
    ...
    ckc_kernel_def_t *kernel =
        ckc_build_add_rmsnorm2d_rdquant_new(&b, &spec, arch);
    if (!kernel) {
