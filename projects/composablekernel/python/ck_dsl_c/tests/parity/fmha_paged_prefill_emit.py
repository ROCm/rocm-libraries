...
from ck_dsl.instances.common.fmha_paged_prefill import (
...
def main() -> int:
    if len(sys.argv) < 2:
        sys.stderr.write("usage: fmha_paged_prefill_emit.py <config_index 0..5>\n")
        return 2
    idx = int(sys.argv[1])
    spec = _spec(idx)
    kernel = build_fmha_fwd_paged_prefill(spec, arch="gfx950")
