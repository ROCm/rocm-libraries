...
def main():
    ...
    ap.add_argument(
        "--cli",
        default=str(
            Path(os.environ.get("TMPDIR", "/tmp")) / "ckc_irart" / "ir_lower_cli"
        ),
    ...
    print(f"IR-artifact lowering parity  families={len(fams)}  cli={cli}")
    print("-" * 78)
    ...
    print("\n" + "=" * 78)
    print("SUMMARY -- Python-IR -> C-parse -> C-lower vs Python-lower")
    print("=" * 78)
