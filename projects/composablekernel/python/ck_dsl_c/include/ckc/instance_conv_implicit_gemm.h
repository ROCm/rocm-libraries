 ...
 *   -----------------------------------   --------------------------------------
 *   @dataclass ConvProblem                ckc_conv_problem_t
 *                                           (already ported: helper_ck_dsl.
 *                                            instances.common.conv_implicit_gemm.h)
 *   @dataclass ConvAccumulatorEpilogue    ckc_conv_acc_epilogue_t
 *                                           (value type defined here; the deep-fused
 *                                            peer carries a narrower slice)
 *   @dataclass ImplicitGemmConvSpec       ckc_implicit_gemm_conv_spec_t
 *   spec.* @property / methods            ckc_implicit_gemm_conv_spec_*(...)
 *   is_valid_spec(spec, arch)             ckc_implicit_gemm_conv_is_valid_spec(...)
 *   make_a_descriptor / _b_ / _d_         ckc_conv_make_{a,b,d}_descriptor(...)
 *   build_implicit_gemm_conv(spec, arch)  ckc_build_implicit_gemm_conv(...)
 *   (+ convenience: build -> lower .ll)   ckc_conv_implicit_gemm_lower_to_llvm(...)
 *
 ...
 * caller fills a ckc_implicit_gemm_conv_spec_t (problem + tile geometry are the
 * only required fields); ckc_implicit_gemm_conv_spec_default() returns a struct
 ...
 * (helper_ck_dsl.instances.common.conv_implicit_gemm.h); this header includes it
 ...
 * the sibling PRIVATE header ckc/instance_conv_implicit_gemm_internal.h; public
 ...
 * are exposed via the ckc_conv_build_overrides_t struct: pass NULL to get the
 ...
 */
#ifndef CKC_INSTANCE_CONV_IMPLICIT_GEMM_H
#define CKC_INSTANCE_CONV_IMPLICIT_GEMM_H
 ...
 * derivation. */
typedef struct ckc_implicit_gemm_conv_spec
{
    ckc_conv_problem_t problem;
 ...
 * CKC_OK or CKC_ERR_VALUE (buffer too small). */
 ...
 * ckc_tensor_descriptor_t into the supplied builder's arena (transforms are
 ...
 * the concrete ckc_tensor_descriptor_t is declared in transforms.h, which the
 ...
 * deep-fusion prototype. Bundled here: a NULL ckc_conv_build_overrides_t*
 ...
 * that hook.
 *
 * NOTE: the override signatures intentionally take a ckc_warp_grid_t* (the bound
 * WarpGrid the closures read) and the descriptor-derived offset Values, matching
 * the Python callable signatures. ckc_warp_grid_t is declared in the epilogues
 * helper header, included by the body TUs. */
struct ckc_warp_grid; /* fwd (full decl in epilogues helper header) */

typedef struct ckc_conv_build_overrides
{
 ...
 * kernel_name(). Use ckc_build_implicit_gemm_conv_new() for the init-from-spec
 * convenience. */
 ...
 * LLVM .ll text. `arch` NULL => "gfx950". On CKC_OK *out_ll receives a malloc'd
...
#endif /* CKC_INSTANCE_CONV_IMPLICIT_GEMM_H */
