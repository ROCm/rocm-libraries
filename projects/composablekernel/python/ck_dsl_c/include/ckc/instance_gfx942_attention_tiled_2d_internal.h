 ...
 *
 * ckc/instance_gfx942_attention_tiled_2d_internal.h -- PRIVATE shared state +
 * phase-function contract for the C99 port of build_unified_attention_2d_tiled
 * (ck_dsl/instances/gfx942/attention_tiled_2d.py, build body lines 1104-5287).
 *
 ...
 *   struct, ckc_gfx942_attn2d_build_ctx_t, holding EXACTLY the set of variables
 *   the closures share. The driver (ckc_build_unified_attention_2d_tiled_scalar
 ...
 *   mirror the Python closure names with a ``ckc_gfx942_attn2d_`` prefix (so they
 ...
 * ckc/instance_gfx942_attention_tiled_2d.h.
 */
#ifndef CKC_INSTANCE_GFX942_ATTENTION_TILED_2D_INTERNAL_H
#define CKC_INSTANCE_GFX942_ATTENTION_TILED_2D_INTERNAL_H
...
#define CKC_GFX942_ATTN2D_MAX_ACCS 64
/* Online-softmax iter-args: 2*SOFTMAX_STATE_SLOTS (m,l per slot) + ACCS. */
#define CKC_GFX942_ATTN2D_MAX_ITER_ARGS 96
/* Per-lane register slots (REGS_PER_LANE): 4 (M=16), 8 (M=32), 16 (32x32). */
#define CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE 16
/* QK / PV K-iters (HD/16 or T/16); HD<=256, T<=128 -> <=16. */
#define CKC_GFX942_ATTN2D_MAX_K_ITERS 16
/* QK / PV N-tiles (T/16 or HD/16); <=16. */
#define CKC_GFX942_ATTN2D_MAX_N_TILES 16

/* ============================================================ *
 *  ckc_gfx942_attn2d_build_ctx_t
 *
...
typedef struct ckc_gfx942_attn2d_build_ctx
{
    ...
    ckc_value_t* q_regs[CKC_GFX942_ATTN2D_MAX_K_ITERS * CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    int q_regs_count;
    ckc_value_t* q32_regs[CKC_GFX942_ATTN2D_MAX_K_ITERS];
    int q32_regs_count;
    ...
    ckc_value_t* hoist_in_warp_row[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ckc_value_t* hoist_state_row[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ckc_value_t* hoist_q_pos[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ckc_value_t* hoist_q_head[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ckc_value_t* hoist_row_mask[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ...
    ckc_value_t* acc_cur[CKC_GFX942_ATTN2D_MAX_ACCS];
    ckc_value_t* out_carry[CKC_GFX942_ATTN2D_MAX_ITER_ARGS];
    int out_carry_count;

    /* ---- final results read by the epilogue (lines 5064-5080) ---- */
    ckc_value_t* l_final[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ckc_value_t* acc_final[CKC_GFX942_ATTN2D_MAX_ACCS];
    ckc_value_t* rcp_l[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ckc_value_t* l_nonzero[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
} ckc_gfx942_attn2d_build_ctx_t;
...
void ckc_gfx942_attn2d_cfvst_block_coords(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                          ...
                                          ckc_value_t** out_a,
                                          ckc_value_t** out_b);
void ckc_gfx942_attn2d_cfvst_store_v_regs(ckc_gfx942_attn2d_build_ctx_t* ctx, void* payload);
void ckc_gfx942_attn2d_issue_v_transposed_store(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                                ckc_value_t* kv_tile_idx);

/* ----- fp8 K/V cache loaders (lines 2905-3346) ----- */
void ckc_gfx942_attn2d_issue_kv_fp8_async_load(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                               ckc_value_t* kv_tile_idx,
                                               ckc_value_t* buf_idx);
void ckc_gfx942_attn2d_dequant_fp8_lds_to_bf16(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                               ckc_value_t* src,
                                               ckc_value_t* dst,
                                               ckc_value_t* scale,
                                               ckc_value_t* buf_idx);
void ckc_gfx942_attn2d_issue_fp8_dequant_loads(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                               ckc_value_t* kv_tile_idx,
                                               ckc_value_t* buf_idx);
void ckc_gfx942_attn2d_issue_k_fp8_mfma_async(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                              ckc_value_t* kv_tile_idx,
                                              ckc_value_t* buf_idx);
void ckc_gfx942_attn2d_issue_v_fp8_mfma_async(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                              ckc_value_t* kv_tile_idx);
void ckc_gfx942_attn2d_issue_v_fp8_mfma_stripe(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                               ckc_value_t* kv_tile_idx);
...
int ckc_gfx942_attn2d_kslot(const ckc_gfx942_attn2d_build_ctx_t* ctx, int group_idx);
...
#endif /* CKC_INSTANCE_GFX942_ATTENTION_TILED_2D_INTERNAL_H */
