 ...
 *
 * ckc/helper_instance_gfx950_deep_fused_conv_pool.h.h -- PUBLIC C99 API for the
 * gfx950 (CDNA, wave64, MFMA 32x32x16) arch shim over the family-agnostic
 * deep-fused conv0 -> conv1 -> maxpool kernel, ported from
 ...
 *   Python module is a 66-LOC wrapper that:
 ...
 *   ckc_deep_fused_conv_pool_* prefix).
 *
 ...
 *   body is needed: the arch string "gfx950" selects the MFMA emitter path
 *   inside the shared builder via the MmaOp resolver, so the gfx950 entries are
 *   thin name+default wrappers that stamp the geometry then call the common
 *   driver (build_deep_fused_conv_pool is the COMMON entry re-exported under the
 *   gfx950 name -- unlike gfx1201 there is NO gfx950-specific closure body).
 *
 * Error model mirrors the rest of the C port: build/lower routes errors through
 * the sticky-error IRBuilder (ckc_i_*); the validity gate returns a bool +
 * reason; the convenience lower returns a ckc_status_t.
 */
#ifndef CKC_HELPER_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_H_H
#define CKC_HELPER_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_H_H
 ...
 *
 * This is a thin arch shim: the gfx950 Python re-exports the COMMON
 * build_deep_fused_conv_pool verbatim, so this entry forwards &spec->base and
 * the normalised arch straight to the family-agnostic common driver
 * ckc_build_deep_fused_conv_pool(...), which resolves the MFMA MmaOp (wave64,
 * 32x32x16) when arch == "gfx950" and emits the MFMA codegen path. The caller
 * owns `b` and frees it with ckc_ir_builder_free(). `b_unused` is named to flag
 * that, like the Python, the caller-supplied builder is the surface emitted into
 * via the wrapped conv driver; it is NOT re-initialised here. */
ckc_kernel_def_t*
...
#endif /* CKC_HELPER_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_H_H */
