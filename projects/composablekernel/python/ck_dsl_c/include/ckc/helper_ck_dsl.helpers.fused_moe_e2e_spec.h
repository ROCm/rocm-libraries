 ...
 *
 * ckc/helper_ck_dsl.helpers.fused_moe_e2e_spec.h -- C99 port of the spec /
 * tile-policy surface of
 * ck_dsl/instances/common/fused_moe_e2e.py (the module the task references as
 * ck_dsl.helpers.fused_moe_e2e_spec; the real symbols live in
 ...
 *   -------------------------------------   ------------------------------------
 *   _default_gemm_tile()                    ckc_fmoe_default_gemm_tile()
 *   _default_bf16_gemm_tile()               ckc_fmoe_default_bf16_gemm_tile()
 *   _default_gemm_tile_gfx942()             ckc_fmoe_default_gemm_tile_gfx942()
 *   _default_bf16_gemm_tile_gfx942()        ckc_fmoe_default_bf16_gemm_tile_gfx942()
 *   _large_batch_gemm_tile()                ckc_fmoe_large_batch_gemm_tile()
 *   _sparse_batch_gemm_tile()               ckc_fmoe_sparse_batch_gemm_tile()
 *   _sparse_batch_gemm_tile_gfx942()        ckc_fmoe_sparse_batch_gemm_tile_gfx942()
 *   _resolve_launch_arch(arch)              ckc_fmoe_resolve_launch_arch(arch)
 *   _gemm_dtype_to_universal(dtype)         ckc_fmoe_gemm_dtype_to_universal(dtype)
 *   _ensure_2byte_dtype(dtype)              ckc_fmoe_ensure_2byte_dtype(dtype)
 *   class FusedMoeForwardSpec               ckc_fmoe_forward_spec_t
 *     .total_pairs                          ckc_fmoe_forward_spec_total_pairs()
 *     .to_topk_softmax_spec()               ckc_fmoe_forward_spec_to_topk_softmax_spec()
 *     .to_gemm_spec(name_suffix=)           ckc_fmoe_forward_spec_to_gemm_spec()
 *     .to_batched_gemm_spec()               ckc_fmoe_forward_spec_to_batched_gemm_spec()
 *     .to_batched_gemm_spec_preshuffle_b()
 * ckc_fmoe_forward_spec_to_batched_gemm_spec_preshuffle_b()
 *
 * Tiles are returned by value as ckc_gemm_tile_spec_t (the gemm_universal C
 ...
 * (NULL / -1) on the Python ValueError path; the ckc_b_* builder variants route
 * the same failure through the sticky-error IRBuilder with the Python message.
 */
#ifndef CKC_HELPER_FUSED_MOE_E2E_SPEC_H
#define CKC_HELPER_FUSED_MOE_E2E_SPEC_H
...
/* ----------------------------------------------------------- GEMM tile policy *
 *
 * Each returns a fully-populated ckc_gemm_tile_spec_t (warp_k defaults to 1 as
 ...
 * ValueError message recorded on `b` (CKC_ERR_VALUE). A dead builder is a
 * NULL no-op. */
 ...
 * ValueError message recorded on `b` (CKC_ERR_VALUE). */
 ...
 * ckc_fmoe_default_gemm_tile() (Python field(default_factory=_default_gemm_tile)).
 * `arch` is Optional[str]: NULL == Python None. Every bool/int knob carries its
 * Python default; ckc_fmoe_forward_spec_default() materialises them. */
 ...
 * returned spec. Returns CKC_OK, CKC_ERR_VALUE (name buffer too small / NULL
 ...
 * name_cap); the returned spec's `name` points at it. Returns CKC_OK or
 * CKC_ERR_VALUE. */
...
#endif /* CKC_HELPER_FUSED_MOE_E2E_SPEC_H */
