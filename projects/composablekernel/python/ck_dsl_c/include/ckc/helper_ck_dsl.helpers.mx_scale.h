 ...
 *
 * ckc/helper_ck_dsl.helpers.mx_scale.h -- C99 port of ck_dsl.helpers.mx_scale.
 *
 ...
 * on a non-i8 input it records CKC_ERR_VALUE + a Python-matching message on the
 * builder and returns NULL. If the builder is already in an error state it is a
 * no-op returning NULL, like every other ckc_b_* call.
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_MX_SCALE_H
#define CKC_HELPER_CK_DSL_HELPERS_MX_SCALE_H
 ...
 * (CKC_ERR_VALUE) is set with the Python-matching message and NULL is returned.
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_MX_SCALE_H */
