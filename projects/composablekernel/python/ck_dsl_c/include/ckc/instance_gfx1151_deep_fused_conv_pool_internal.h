 ...
 *
 * ckc/instance_gfx1151_deep_fused_conv_pool_internal.h -- PRIVATE shared state +
 ...
 * builder (ck_dsl/instances/gfx1151/deep_fused_conv_pool.py, 3238 LOC).
 *
 ...
 *   helper calls into ONE context struct, ckc_gfx1151_dfcp_build_ctx_t, and turns
 *   each module-level Python function into a free function taking a pointer to it.
 *   The driver (ckc_build_gfx1151_deep_fused_conv_pool) populates the ctx in the
 ...
 *     _setup_input_cache          -> ckc_gfx1151_dfcp_stage_input_footprint[_int]
 *     _setup_specialized_a_loader -> ckc_gfx1151_dfcp_stage_conv0_a[_int]
 *     _load_a_tile_from_cache     -> ckc_gfx1151_dfcp_wmma_gemm_from_lds[_int]
 *     _load_a_tile_specialized    -> ckc_gfx1151_dfcp_wmma_gemm_conv0_direct[_int]
 *     _load_a_operand_from_cache  -> ckc_gfx1151_dfcp_load_conv0_a_frag_from_footprint*
 ...
 *   mirror the Python module-level names with a ckc_gfx1151_dfcp_ prefix.
 *
 ...
 */
#ifndef CKC_INSTANCE_GFX1151_DEEP_FUSED_CONV_POOL_INTERNAL_H
#define CKC_INSTANCE_GFX1151_DEEP_FUSED_CONV_POOL_INTERNAL_H
...
#include "ckc/ir.h"
/* ckc_ir_builder_t / ckc_value_t / ckc_kernel_def_t / ckc_status_t. */
#include "ckc/helper_ck_dsl.instances.common.deep_fused_conv_pool.h"
/* opaque ckc_warp_grid_t / ckc_mma_op_t. */
 ...
 * ===================================================================== */
typedef struct ckc_gfx1151_dfcp_build_ctx
{
    /* =============================================================== *
     ...
     * hints for "mem". Opaque to phases (passed to ckc_gfx1151_dfcp_emit_wmma_k_sched). */
    const void* policy;

    /* ---- derived geometry cached from the spec (Python prologue locals) ----
     * kpad = ckc_gfx1151_dfcp_kpad(spec). block_size, conv_tile_h/w, foot_h/w are
    ...
    ckc_value_t* accs0[CKC_GFX1151_DFCP_MAX_ACCS];
    size_t num_accs0;
    ckc_value_t* accs1[CKC_GFX1151_DFCP_MAX_ACCS];
    size_t num_accs1;
    ...
    ckc_value_t* a_frags[CKC_GFX1151_DFCP_MAX_AFRAGS];
    size_t num_a_frags;
    ...
    ckc_value_t* h_blk; /* readfirstlane(tile_idx) / w_tiles */
    ckc_value_t* w_blk; /* readfirstlane(tile_idx) % w_tiles */
 ...
 * left zeroed for the phase walk. Returns CKC_OK or a status mirroring the Python
 * ValueError paths (message in b->err). */
ckc_status_t ckc_gfx1151_dfcp_build_ctx_init(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                             ckc_ir_builder_t* b,
...
/* 0 - x. */
ckc_value_t* ckc_gfx1151_dfcp_neg_i32(ckc_ir_builder_t* b, ckc_value_t* x);
/* smin(smax(x, lo), hi). */
ckc_value_t* ckc_gfx1151_dfcp_clamp_i32(ckc_ir_builder_t* b, ckc_value_t* x, int lo, int hi);
/* smax(x, 0). */
ckc_value_t* ckc_gfx1151_dfcp_relu_i32(ckc_ir_builder_t* b, ckc_value_t* x);
/* RNE round x / 2**shift using integer ops only (sign-aware, ties-to-even). */
ckc_value_t* ckc_gfx1151_dfcp_round_shift_rne_i32(ckc_ir_builder_t* b, ckc_value_t* x, int shift);
/* trunc(clamp_i32(round_shift_rne_i32(x, shift), -127,127), I8). */
ckc_value_t* ckc_gfx1151_dfcp_quant_i8_shift(ckc_ir_builder_t* b, ckc_value_t* x, int shift);
/* trunc(clamp_i32(round_shift_rne_i32(x, shift), -8,7), I8). */
ckc_value_t* ckc_gfx1151_dfcp_quant_i4_shift(ckc_ir_builder_t* b, ckc_value_t* x, int shift);

/* Vector (<N x i32>) twins of the scalar primitives (specialized_rne path). */
ckc_value_t* ckc_gfx1151_dfcp_splat_i32(ckc_ir_builder_t* b, int value, int n);
ckc_value_t* ckc_gfx1151_dfcp_neg_i32_vec(ckc_ir_builder_t* b, ckc_value_t* x);
 ...
 * USES the NEW helper ckc_unpack_i4_byte_to_pair_i32 (ck_dsl.helpers.i4_dequant). */
 ...
 * for the iu8 conv1 atom. USES ckc_unpack_i4_byte_to_pair_i32. */
...
ckc_value_t* ckc_gfx1151_dfcp_load_frag_iu8_from_lds(ckc_ir_builder_t* b,
                                                     ckc_value_t* smem,
                                                     ckc_value_t* frag_rc,
                                                     ckc_value_t* atom_off,
                                                     ckc_value_t* k_tile_base);
/* _pack_i4_codes_to_i32: <8 x i8> int4 codes -> one i32, low nibble first. */
ckc_value_t* ckc_gfx1151_dfcp_pack_i4_codes_to_i32(ckc_ir_builder_t* b, ckc_value_t* codes);
/* _load_frag_iu4_codes_from_lds: byte-per-code C0 -> <2 x i32> iu4 A-fragment. */
ckc_value_t* ckc_gfx1151_dfcp_load_frag_iu4_codes_from_lds(ckc_ir_builder_t* b,
                                                           ckc_value_t* smem,
                                                           ckc_value_t* frag_rc,
                                                           ckc_value_t* atom_off,
                                                           ckc_value_t* k_tile_base);
/* _load_frag_iu4_packed_from_lds: packed-byte W1 -> <2 x i32> iu4 B-fragment. */
ckc_value_t* ckc_gfx1151_dfcp_load_frag_iu4_packed_from_lds(ckc_ir_builder_t* b,
                                                            ckc_value_t* smem,
                                                            ckc_value_t* frag_rc,
                                                            ckc_value_t* atom_off,
                                                            ckc_value_t* k_tile_base);
/* _load_conv0_a_frag_from_footprint: gather f16 conv0 A-fragment from footprint. */
ckc_value_t* ckc_gfx1151_dfcp_load_conv0_a_frag_from_footprint(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                               ckc_value_t* inp_smem,
                                                               ckc_value_t* m_row,
                                                               ckc_value_t* k_base,
                                                               int frag_len);
 ...
 *  into the provided out-array (out[CKC_GFX1151_DFCP_MAX_ACCS], *out_count set).
...
void ckc_gfx1151_dfcp_emit_wmma_k_sched(ckc_ir_builder_t* b,
                                        const void* policy,
                                        int n_ds,
                                        int n_mma);

/* _wmma_gemm_from_lds: generic f16 WMMA GEMM from two LDS tiles. */
void ckc_gfx1151_dfcp_wmma_gemm_from_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                         const ckc_mma_op_t* op,
                                         ckc_value_t* a_smem,
                                         ckc_value_t* b_smem,
                                         int k_total,
...
void ckc_gfx1151_dfcp_wmma_gemm_from_lds_int(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                             const ckc_mma_op_t* op,
                                             ckc_value_t* a_smem,
                                             ckc_value_t* b_smem,
                                             int k_total,
...
void ckc_gfx1151_dfcp_wmma_gemm_conv0_direct(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                             const ckc_mma_op_t* op,
                                             ckc_value_t* inp_smem,
                                             ckc_value_t* w0_smem,
                                             const void* policy,
...
void ckc_gfx1151_dfcp_wmma_gemm_conv0_direct_int(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                 const ckc_mma_op_t* op,
                                                 ckc_value_t* inp_smem,
                                                 ckc_value_t* w0_smem,
                                                 const void* policy,
...
void ckc_gfx1151_dfcp_wmma_gemm_conv1_i4_from_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                  const ckc_mma_op_t* op,
                                                  ckc_value_t* c0_smem,
                                                  ckc_value_t* w1_smem,
                                                  int k_total,
...
void ckc_gfx1151_dfcp_wmma_gemm_conv1_i4_packed_from_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                         const ckc_mma_op_t* op,
                                                         ckc_value_t* c0_smem,
                                                         ckc_value_t* w1_smem,
                                                         int k_total,
...
void ckc_gfx1151_dfcp_wmma_gemm_conv1_i4_from_regs(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                   const ckc_mma_op_t* op,
...
void ckc_gfx1151_dfcp_wmma_gemm_conv1_i8_from_regs(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                   const ckc_mma_op_t* op,
                                                   ckc_value_t* const* a_frags,
                                                   int a_rows,
                                                   int a_cols,
 ...
 * the body switches on; see ckc_gfx1151_dfcp_code_fn_t). `int8` => unsqueezed
...
void ckc_gfx1151_dfcp_fuse_c0_to_conv1_a_regs(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                              const ckc_mma_op_t* op0,
...
void ckc_gfx1151_dfcp_scatter_codes_to_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                           const ckc_mma_op_t* op,
...
void ckc_gfx1151_dfcp_scatter_codes_to_i8_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                              const ckc_mma_op_t* op,
                                              ckc_value_t* const* accs,
                                              size_t num_accs,
                                              ckc_value_t* dst_smem,
 ...
 * (specialized_rne). */
void ckc_gfx1151_dfcp_scatter_vec_codes_to_i8_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                  const ckc_mma_op_t* op,
                                                  ckc_value_t* const* accs,
                                                  size_t num_accs,
                                                  ckc_value_t* dst_smem,
...
void ckc_gfx1151_dfcp_scatter_packed_i4_codes_to_lds(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                     const ckc_mma_op_t* op,
                                                     ckc_value_t* const* accs,
                                                     size_t num_accs,
                                                     ckc_value_t* dst_smem,
...
void ckc_gfx1151_dfcp_repack_c0_lds_to_packed(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                              ckc_value_t* c0_smem,
                                              ckc_value_t* c0_packed_smem);
...
void ckc_gfx1151_dfcp_emit_maxpool_finalquant(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                              ckc_value_t* c1_smem,
                                              ckc_value_t* y_ptr);
/* _emit_maxpool_finalpack_i8: native path (i8 int4 codes; mf=1 pack is no-op);
 * persistent coords via h_blk/w_blk. */
void ckc_gfx1151_dfcp_emit_maxpool_finalpack_i8(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                ckc_value_t* c1_smem,
                                                ckc_value_t* y_ptr,
                                                ckc_value_t* h_blk,
                                                ckc_value_t* w_blk);
...
void ckc_gfx1151_dfcp_emit_persistent_body(ckc_gfx1151_dfcp_build_ctx_t* ctx);
...
void ckc_gfx1151_dfcp_emit_single_tile_body(ckc_gfx1151_dfcp_build_ctx_t* ctx);

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* CKC_INSTANCE_GFX1151_DEEP_FUSED_CONV_POOL_INTERNAL_H */
