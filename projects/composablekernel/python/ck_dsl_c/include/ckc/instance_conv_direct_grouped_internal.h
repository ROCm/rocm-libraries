 ...
 *
 * ckc/instance_conv_direct_grouped_internal.h -- PRIVATE shared state + phase-
 ...
 *   (ckc_dconv_16c_ctx_t / ckc_dconv_4c_ctx_t) that holds EXACTLY the variables
 ...
 *   functions mirror the Python closure names with a `ckc_dconv16c_` /
 *   `ckc_dconv4c_` prefix.
 *
 ...
 * ckc/instance_conv_direct_grouped.h.
 */
#ifndef CKC_INSTANCE_CONV_DIRECT_GROUPED_INTERNAL_H
#define CKC_INSTANCE_CONV_DIRECT_GROUPED_INTERNAL_H
 ...
 * ===================================================================== */
#define CKC_DCONV16C_MAX_PASSES 16
...
#define CKC_DCONV_MAX_QTILES 8
/* KH for a 3x3 conv; the circular accumulator depth. Sized to the max KH the
 * builders unroll. */
#define CKC_DCONV_MAX_ACC_SLOTS 8

/* ===================================================================== *
 *  ckc_dconv_16c_ctx_t  --  shared state for build_direct_conv_16c.
 *
...
typedef struct ckc_dconv_16c_ctx
{
    ...
    ckc_value_t* A_smem; /* smem_alloc lds_a [1, lds_total_fp16] */
    ckc_value_t* B_smem; /* smem_alloc lds_b (or == A_smem)      */
    ckc_value_t* a_rsrc; /* buffer_rsrc(A, A_bytes)              */
    ckc_value_t* b_rsrc; /* buffer_rsrc(Bp, B_bytes)            */
    ckc_value_t* d_rsrc; /* buffer_rsrc(D, D_bytes)             */
    ...
    ckc_value_t* acc_tiles[CKC_DCONV_MAX_QTILES][CKC_DCONV_MAX_ACC_SLOTS];
} ckc_dconv_16c_ctx_t;

/* ===================================================================== *
 *  ckc_dconv_4c_ctx_t  --  shared state for build_direct_conv_4c.
 *
 *  Field order follows the Python prologue (lines 837-966).
 * ===================================================================== */
typedef struct ckc_dconv_4c_ctx
{
    /* ---- inputs / resolved environment -- */
    ckc_ir_builder_t* b;
...
#endif /* CKC_INSTANCE_CONV_DIRECT_GROUPED_INTERNAL_H */
