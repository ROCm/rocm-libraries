 ...
 *
 * helper_ck_dsl.helpers.sweep.h -- C99 port of the
 * ``ck_dsl.helpers.sweep`` module.
 *
 ...
 * via the C builder (ckc_b_*, ckc/ir.h) and the TileWindow load/store-as-f32
 * peers (ckc/helper_ck_dsl.helpers.tensor_view.h). The builder-call sequence
 ...
 * error on the builder (ckc_i_set_err) and returns an empty result / void.
 */
#ifndef CKC_HELPER_SWEEP_H
#define CKC_HELPER_SWEEP_H
...
/* Bookkeeping returned by ckc_sweep_row_chunks (Python RowChunkSweepResult).
 *
 ...
 */
typedef struct ckc_row_chunk_sweep_result
{
    ckc_value_t** cached;
    int num_cached;
    int chunks_per_thread;
} ckc_row_chunk_sweep_result_t;

/* ---------------------------------------------------------------- callbacks */

/* Pass-1 body callback for ckc_sweep_row_chunks.
 *
 ...
 */
typedef void (*ckc_pass2_row_body_fn)(ckc_ir_builder_t* b,
                                      ckc_value_t* n_off,
                                      int k,
...
/* -------------------------------------------------------- TileWindow peers *
 *
 * Provided by the tensor_view port (helper_ck_dsl.helpers.tensor_view.c).
 * Declared here so this translation unit compiles standalone; resolved at
 * link time once the peer lands.
 *
 * ckc_tile_window_load_vec_as_f32: TileWindow.load_vec_as_f32 -- vector load
 *   + per-lane f32 promotion. Writes the ``n`` f32 SSA scalars to out[0..n)
 *   (caller-provided, length >= n).
 *
 * ckc_tile_window_store_vec_from_f32: TileWindow.store_vec_from_f32 -- f32
 ...
 *     by ckc_sweep_row_chunks; when non-empty, chunk k receives the slice
