 ...
 *
 * ckc/helper_ck_dsl.instances.common._matmul_nbits_large_n_opt.h -- C99 port of
 ...
 *   -------------------------------------------  ---------------------------------
 *   build_large_n_opt_matmul_nbits(spec, arch)   ckc_build_large_n_opt_matmul_nbits()
 *
 ...
 *   - ckc_matmul_nbits_spec_t / ckc_matmul_nbits_kernel_name /
 *     ckc_matmul_nbits_scale_wire_dtype  (_matmul_nbits_common.h)
 *   - ckc_wmma_params / ckc_wmma_params_t (_matmul_nbits_large_n.h)
 *   - ckc_unpack_i4_byte_to_pair_f16      (i4_dequant.h)
 *
 * Lifetime: every node is arena-owned (ckc_ir_builder_t.arena); nothing is freed
 * individually, mirroring the Python GC lifetime.
 */
#ifndef CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_LARGE_N_OPT_H
#define CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_LARGE_N_OPT_H
 ...
 * ckc_build_large_n_opt_matmul_nbits_new() for the init-from-spec convenience.
 *
 ...
 * (CKC_ERR_VALUE) and return NULL, matching the Python ValueErrors. */
...
#endif /* CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_LARGE_N_OPT_H */
