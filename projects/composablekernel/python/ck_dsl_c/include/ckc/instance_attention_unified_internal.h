 ...
 *
 * ckc/instance_attention_unified_internal.h -- PRIVATE shared state + phase-
 ...
 *   one shared context struct, ckc_attn_unified_build_ctx_t, which holds EXACTLY
 ...
 * ckc/instance_attention_unified.h.
 */
#ifndef CKC_INSTANCE_ATTENTION_UNIFIED_INTERNAL_H
#define CKC_INSTANCE_ATTENTION_UNIFIED_INTERNAL_H
...
typedef struct ckc_attn_unified_build_ctx
{
     ...
     * ckc_unified_attn_problem_t*) can be called without re-deriving it. The
    ...
    ckc_value_t* init_m;
    ckc_value_t* init_l;
    ckc_value_t* init_acc;
    ckc_value_t* sink_h; /* 2D + use_sinks: global_load(sinks, q_head)   */
    ...
    ckc_unified_attn_paged_kv_descriptor_t kv_desc; /* value type, not a pointer  */
    ckc_tensor_descriptor_t* ml_desc;
    ckc_tensor_descriptor_t* out_desc;
 ...
 * narrower declaration -- see ckc_attn_unified_declare_reduce_params.) */
...
#endif /* CKC_INSTANCE_ATTENTION_UNIFIED_INTERNAL_H */
