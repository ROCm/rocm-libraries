 ...
 *
 * ckc/instance_gfx950_attention_tiled_2d.h -- PUBLIC C99 surface for the gfx950
 * (CDNA4 / MI355X) WIDE-ATOM tiled-2D unified-attention kernel INSTANCE BUILDER
 * (ck_dsl/instances/gfx950/attention_tiled_2d.py, 3818 LOC).
 *
 *   Python (gfx950/attention_tiled_2d.py)            C99 (this header)
 *   ------------------------------------------- --------------------------------------------------
 *   class UnifiedAttention2DTiledSpec                ckc_attention_tiled_2d_spec_t  (ALREADY
 * PORTED: (frozen dataclass + __post_init__)               helper_helper_ck_dsl.instances.gfx942.
 *                                                       attention_tiled_2d.h -- arch-parameterised,
 *                                                       reused verbatim here)
 *   spec.@property accessors                         ckc_attention_tiled_2d_spec_*  (same helper
 * hdr) _mfma_32x32_c_row / _mfma_32x32_c_col ckc_gfx950_attention_tiled_2d_mfma_32x32_c_row/_col
 *   supports_tiled_2d(..., arch="gfx950")            ckc_gfx950_attention_tiled_2d_supports
 *   build_unified_attention_2d_tiled(spec, arch)     ckc_gfx950_build_unified_attention_2d_tiled
 *   (+ convenience: build -> lower .ll)              ckc_gfx950_attention_tiled_2d_lower_to_llvm
 *
 ...
 *   ported once (helper_helper_ck_dsl.instances.gfx942.attention_tiled_2d.h).
 *   The KERNEL EMITTER, however, is NOT interchangeable:
 *
 *     - gfx950 uses the WIDE-K MFMA atoms mfma_f32_16x16x32 / mfma_f32_32x32x16
 *       (ckc_mfma_32x32x16_for_dtype, Python lines 813-814); gfx942 uses the
 ...
 *   ``ckc_gfx950_attention_tiled_2d_`` prefix so it never clashes with the gfx942
 ...
 *   ckc/instance_gfx950_attention_tiled_2d_internal.h; public callers only ever
 *   touch THIS header.
 *
 ...
 *   - ckc_mfma_32x32x16_for_dtype, ckc_mfma_16x16x32_for_dtype, the warp xor
 *       reduces, ckc_dequant_fp8x8_to_dtype, pv32_v_load_paired
 *       (ckc/helper_ck_dsl.helpers.attention.h, helper_ck_dsl.helpers.mfma_attention.h)
 *   - ckc_transpose_lds_reader_* (TransposeLdsReader)
 *       (ckc/helper_ck_dsl.helpers.layouts.h)
 *   - the arch gate require_tiled_attention_arch / validate_tiled_attention_arch
 *       (instances/common/attention_arch.py -> ckc/helper_ck_dsl.core.arch.h)
 *   - make_static_tile_distribution / make_c_warp_dstr_encoding
 *       (ckc/helper_ck_dsl.helpers.distribution.h, helper_ck_dsl.helpers.atoms.h)
 *
 ...
 *
 * ENTRY RETURN. ckc_gfx950_build_unified_attention_2d_tiled returns the built
 ...
 */
#ifndef CKC_INSTANCE_GFX950_ATTENTION_TILED_2D_H
#define CKC_INSTANCE_GFX950_ATTENTION_TILED_2D_H
...
typedef struct ckc_gfx950_attention_tiled_2d_supports_args
{
    int head_size;
    int block_size;
 ...
 * ckc/instance_gfx950_attention_tiled_2d_internal.h. It reproduces the Python
...
#endif /* CKC_INSTANCE_GFX950_ATTENTION_TILED_2D_H */
