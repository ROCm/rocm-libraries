 ...
 *
 * ckc/instance_gemm_internal.h -- PRIVATE shared state + phase-function
 ...
 *   struct, ckc_gemm_build_ctx_t, which holds EXACTLY the set of variables the
 *   closures shared. The driver (ckc_build_universal_gemm in
 *   ckc/instance_gemm_universal.h) populates the ctx in the same order the
 ...
 * ckc/instance_gemm_universal.h.
 */
#ifndef CKC_INSTANCE_GEMM_INTERNAL_H
#define CKC_INSTANCE_GEMM_INTERNAL_H
...
typedef struct ckc_gemm_build_ctx
{
    ...
    ckc_value_t* sorted_token_ids;
    ckc_value_t* slot_size_p;
    ...
    ckc_value_t* batch_off_a;
    ckc_value_t* batch_off_b;
    ckc_value_t* batch_off_c;

    /* ---- per-CTA tile origins (SGPR-pinned SSA) -- */
    ckc_value_t* block_m_off; /* row tile base (chiplet-swizzle aware)     */
    ...
    /* ---- accumulators -- */
    ckc_value_t* acc_init; /* _emit_zero_acc_op(b, op)          */
    ...
    ckc_value_t* for_results[CKC_GEMM_MAX_ACCS];
    int num_for_results;
} ckc_gemm_build_ctx_t;
...
/* _emit_mma(b, op, a, bb, c): target-neutral D = a*bb + c via ckc_b_mma. */
...
ckc_value_t* ckc_gemm_swz_col(ckc_gemm_build_ctx_t* ctx, ckc_value_t* col, ckc_value_t* row);
...
void ckc_gemm_emit_load_phase(ckc_gemm_build_ctx_t* ctx,
                              ckc_value_t* A_dst,
                              ckc_value_t* B_dst,
                              ckc_value_t* k_off,
                              int parity_imm,
...
ckc_value_t* ckc_gemm_emit_frag_smem_load(ckc_gemm_build_ctx_t* ctx,
                                          ckc_value_t* src,
...
void ckc_gemm_emit_wmma_phase(ckc_gemm_build_ctx_t* ctx,
                              ckc_value_t* A_src,
                              ckc_value_t* B_src,
 ...
 * step. Delegates to ckc_gemm_emit_wmma_phase on RDNA. Emits the compv3/compv4
...
typedef void (*ckc_gemm_per_cell_fn)(
...
#endif /* CKC_INSTANCE_GEMM_INTERNAL_H */
