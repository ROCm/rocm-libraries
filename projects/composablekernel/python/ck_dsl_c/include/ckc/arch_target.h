 ...
 *
 * ckc/arch_target.h -- PUBLIC API for the C99 port of
 * ck_dsl.core.arch.target (the polymorphic-core arch SSOT).
 *
 * This is the single CK_DSL-owned description of *what a gfx target supports*:
 ...
 *   ------------------------------    --------------------------------------
 *   class LayoutMap (frozen)          ckc_layout_map_t
 *     .fn closure                       ckc_lane_coord_fn (fn ptr + builder)
 *     .coord(builder, lane, slot)       ckc_layout_map_coord()
 *   class MmaOp (frozen)              ckc_mma_op_t
 *     .shape / .a_layout()/...          ckc_mma_op_shape() / *_layout() getters
 *   class MemoryCapabilities          ckc_memory_caps_t
 *   class ResourceLimits              ckc_resource_limits_t
 *   class MmaCatalog                  ckc_mma_catalog_t + ckc_mma_catalog_* ()
 *   class ArchTarget (frozen)         ckc_arch_target_t + ckc_arch_* () getters
 *   normalize_dtype()                 ckc_normalize_dtype()
 *   ArchTarget.from_gfx(gfx)          ckc_arch_target_from_gfx()
 *   known_arches()                    ckc_known_arches()
 *   arch_from_isa(isa)                ckc_arch_from_isa()
 *
 ...
 * fail.
 */
#ifndef CKC_ARCH_TARGET_H
#define CKC_ARCH_TARGET_H
 ...
 * coordinate meaning depends on the role (see ckc_mma_role_t). On a failed
 * builder it is a no-op leaving *out0/*out1 = NULL. */
typedef void (*ckc_lane_coord_fn)(
 ...
 * ValueError); on out-of-range it sets the builder error (CKC_ERR_VALUE) and
...
typedef struct ckc_mma_op
{
    ...
    int c_frag_len;
    int wave_size;
    const ckc_layout_map_t* a_layout; /* may be NULL */
    const ckc_layout_map_t* b_layout; /* may be NULL */
    const ckc_layout_map_t* c_layout; /* may be NULL */
} ckc_mma_op_t;
 ...
 * error (CKC_ERR_NOTIMPL) with the same message text, so callers can either
...
typedef struct ckc_memory_caps
{
    bool has_async_lds;
    bool has_ds_read_tr;
...
typedef struct ckc_resource_limits
{
    int max_threads_per_block;
    int vgprs;
    int agprs;
    int sgprs;
} ckc_resource_limits_t;
...
typedef struct ckc_mma_catalog
{
    const ckc_mma_op_t* ops; /* static array */
    int num_ops;
} ckc_mma_catalog_t;
...
const ckc_mma_op_t* ckc_mma_catalog_by_op_id(const ckc_mma_catalog_t* cat, const char* op_id);
 ...
 * ArchTarget. */
typedef struct ckc_arch_target
{
    const char* gfx;
    const char* family;
    ...
    int vmcnt_bits;
    ckc_mma_catalog_t mma;
    ckc_memory_caps_t memory;
    ckc_resource_limits_t limits;
} ckc_arch_target_t;
...
bool ckc_arch_supports_dtype_combo(
    const ckc_arch_target_t* t, const char* a, const char* b, const char* c, const char* family);
