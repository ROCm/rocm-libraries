 ...
 *
 * ckc/lower_hip_internal.h -- PRIVATE shared declarations for the C99 port of
 * ck_dsl.core.lower_hip. NOT a public API: only the lower_hip_*.c translation
 * units include this. The public contract is ckc/lower_hip.h.
 *
 ...
 * call them. Per-op handlers (`ckc_h_op_<dotted_with_underscores>`) are NOT
 * declared here individually -- each bucket defines its own static handlers and
 * registers them through the dispatch table that ckc_h_lower_op() consults
 * (see ckc_h_dispatch below). Cross-cutting handlers reachable from another
 * bucket (e.g. tile.mma re-dispatching to a concrete mfma handler) go through
 * ckc_h_lower_op(), so no per-handler externs are needed.
 *
 * Naming: internal helpers are prefixed ckc_h_ (h = hip lowerer) to keep them
 * out of the public ckc_ / ckc_b_ / ckc_lower_ namespace.
 */
#ifndef CKC_LOWER_HIP_INTERNAL_H
#define CKC_LOWER_HIP_INTERNAL_H
...
/* printf-style ckc_h_emit: format into the arena, then emit with indent. The
 ...
 * handler in another bucket. Sets CKC_ERR_NOTIMPL for an unhandled opcode. */
 ...
 * lowerer status (CKC_OK or the error it just set). */
typedef ckc_status_t (*ckc_h_handler_fn)(ckc_h_lowerer_t* lw, const ckc_op_t* op);

/* Look up the handler for an opcode, or NULL if none. The table is assembled in
 * bucket 0 from each bucket's exported registration array (ckc_h_handlers_*),
 ...
 * the dispatch table at first use. (opcode==CKC_OP_INVALID terminates a table.)
 */
typedef struct ckc_h_handler_entry
{
    ckc_opcode_t opcode;
    ckc_h_handler_fn handler;
} ckc_h_handler_entry_t;
...
/* The two raw encoders behind ckc_h_encode_waitcnt, exposed for any direct
 ...
 * WMMA instruction (CDNA/MFMA). Returns CKC_OK if allowed, else sets the sticky
 * error (CKC_ERR_NOTIMPL) and returns it. `op_id` is the atom id for the message.
 ...
 * smem ops can index it. In C, op.attrs is the frozen ckc_attr_map_t which we
 ...
 *
 * ckc_h_smem_set_storage records "<name>_storage" for the smem alloc result;
 * ckc_h_smem_storage looks it up for a consuming op's smem operand. A lookup
 * miss is the Python "smem op before smem_alloc was lowered" RuntimeError case:
 * the caller should ckc_h_fail. Returns NULL on miss. */
