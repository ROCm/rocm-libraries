 ...
 *
 * ckc/helper_instance_gfx950_attention_tiled_2d_fastkv_regp.h -- C99 port of the
 ...
 * ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_t (spec proxy: use_register_pv property + +
 * ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_make kernel_name() suffix) +
 * ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_use_register_pv
 *                                                       +
 * ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_kernel_name make_fastkv_register_p_spec(...)
 ...
 *     - ckc_attention_tiled_2d_spec_t / @property accessors / validator / config
 *         (ckc/helper_helper_ck_dsl.instances.gfx942.attention_tiled_2d.h)
 *     - ckc_gfx942_attention_tiled_2d_supports                (supports_tiled_2d)
 *     - ckc_build_unified_attention_2d_tiled_scalar           (build tiled)
 ...
 *   a value struct holding the wrapped ckc_attention_tiled_2d_spec_t plus the two
 ...
 *   ckc_build_unified_attention_2d_fastkv_register_p reproduces the 3 Python
 ...
 *   ckc_build_unified_attention_2d_tiled_scalar port; nothing long is duplicated
 ...
 * not copied.
 */
#ifndef CKC_HELPER_INSTANCE_GFX950_ATTENTION_TILED_2D_FASTKV_REGP_H
#define CKC_HELPER_INSTANCE_GFX950_ATTENTION_TILED_2D_FASTKV_REGP_H
 ...
 * as helpers below. */
typedef struct ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy
{
...
bool ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_use_register_pv(
    const ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_t* p);
 ...
 * NUL-terminated into ``out`` (capacity ``out_cap``). On success returns CKC_OK
 * and, if out_len != NULL, sets *out_len to the byte length (excluding NUL); a
 * too-small buffer (or NULL base/out) returns CKC_ERR_VALUE and writes nothing
 * usable. */
ckc_status_t ckc_gfx950_attention_tiled_2d_fastkv_regp_spec_proxy_kernel_name(const char* base,
                                                                              char* out,
                                                                              size_t out_cap,
                                                                              size_t* out_len);
 ...
 * supports_tiled_2d (here ckc_gfx942_attention_tiled_2d_supports) with the
...
typedef struct ckc_gfx950_supports_fastkv_register_p_2d_args
{
    int head_size;
    int block_size;
...
} ckc_gfx950_supports_fastkv_register_p_2d_args_t;

bool ckc_gfx950_supports_fastkv_register_p_2d(
    const ckc_gfx950_supports_fastkv_register_p_2d_args_t* args, char* reason, size_t reason_cap);
 ...
 *
 * On a failing guard it records the Python ValueError text on ``b`` (CKC_ERR_VALUE)
 * and returns NULL. Otherwise it forms the proxy (use_register_pv -> true) and
 * calls ckc_build_unified_attention_2d_tiled_scalar with arch threaded through;
...
#endif /* CKC_HELPER_INSTANCE_GFX950_ATTENTION_TILED_2D_FASTKV_REGP_H */
