 ...
 *
 * ckc/instance_fused_moe_e2e.h -- PUBLIC API for the C99 port of the end-to-end
 * fused-MoE forward orchestrator
 * ck_dsl/instances/common/fused_moe_e2e.py (2678 LOC).
 *
 *   Python (fused_moe_e2e.py)               C99 (this header)
 *   -------------------------------------   ------------------------------------
 *   @dataclass FusedMoeForwardSpec          ckc_fmoe_forward_spec_t
 *     (+ tile factories / dtype helpers)      (reused: helper_ck_dsl.helpers.
 *                                              fused_moe_e2e_spec.h)
 *   class FusedMoeForward                    ckc_fmoe_forward_t
 *   FusedMoeForward.__init__(spec)           ckc_fmoe_forward_init(...)
 *   FusedMoeForward.forward(...)             (KernelDef.forward_fn -- see below)
 *   build entry (no direct Python peer)      ckc_build_fused_moe_forward(...)
 *   (+ convenience: build -> lower .ll)      ckc_fused_moe_forward_lower_to_llvm(...)
 *
 * WHAT THIS MODULE IS.
 ...
 *   ckc_build_fused_moe_forward(spec, arch) returns a ckc_kernel_def_t* that
 *   ENCAPSULATES the full orchestrator. Mirroring the task's contract:
 *
 *       kernel_def = ckc_build_fused_moe_forward(&spec, "gfx950");
 *       forward    = kernel_def->forward_fn;   // see ckc_fmoe_forward_fn_t
 ...
 *   marked TODO(port) and the forward_fn returns CKC_ERR_NOTIMPL until a HIP
 ...
 * ckc/instance_fused_moe_e2e_internal.h. Public callers only ever touch THIS
 * header.
 *
 * Error model mirrors the rest of the C port: builders route failures through
 * the sticky-error IRBuilder (ckc_b_* / NULL return); pure helpers return a
 * sentinel; the convenience lower returns a ckc_status_t; the runtime forward
 * entry returns a ckc_status_t (CKC_ERR_NOTIMPL for the stubbed launch path).
 */
#ifndef CKC_INSTANCE_FUSED_MOE_E2E_H
#define CKC_INSTANCE_FUSED_MOE_E2E_H
 ...
 * them verbatim: ckc_fmoe_forward_spec_t, ckc_fmoe_forward_spec_default(), the
 * seven tile factories, ckc_fmoe_resolve_launch_arch(),
 * ckc_fmoe_gemm_dtype_to_universal(), ckc_fmoe_ensure_2byte_dtype(), and the
 ...
 * ckc_fmoe_forward_t*). Pointers are device pointers (uint64). Returns CKC_OK
 * on a completed launch chain, or CKC_ERR_NOTIMPL while the launch path is a
 * TODO(port) stub. */
typedef ckc_status_t (*ckc_fmoe_forward_fn_t)(void* self,
...
/* ============================================================ *
 * ckc_fmoe_forward_t   (FusedMoeForward host instance)
 * ============================================================ *
 *
 ...
 * internal header (ckc_fmoe_build_ctx_t); this public type is an opaque-ish
...
typedef struct ckc_fmoe_forward
{
     ...
     * Defined in ckc/instance_fused_moe_e2e_internal.h. */
    void* ctx;

    ckc_fmoe_forward_fn_t forward_fn; /* bound dispatch entry (static/dyn) */
} ckc_fmoe_forward_t;
 ...
 * On a tile-policy / dtype ValueError returns CKC_ERR_VALUE. */
 ...
 * and return a ckc_kernel_def_t* whose forward_fn drives the full MoE pipeline.
 ...
 * its forward_fn `self`; free both with ckc_fused_moe_forward_free(). On
 * failure returns NULL (allocation / tile-policy ValueError).
 *
 * Usage (task contract):
 *     ckc_kernel_def_t* kd = ckc_build_fused_moe_forward(&spec, "gfx950");
 *     ckc_fmoe_forward_fn_t forward = kd->forward_fn;       // see note below
 *     status = forward(self, rl, X, Wg, Wu, Wd, Y, stream);
 *
 * NOTE: ckc_kernel_def_t (ir.h) carries the lowerable IR fields only. The
 ...
 * provided by ckc_build_fused_moe_forward_simple(). */
 ...
 * the instance. Caller frees with ckc_fused_moe_forward_free(kd). */
 ...
 * runs (see ckc_fmoe_stage_t); the spec's tile + trait policy is applied
 * exactly as the live orchestrator would for that stage.
 *
 * `arch` NULL => resolved per the spec. On CKC_OK *out_ll receives a malloc'd
...
#endif /* CKC_INSTANCE_FUSED_MOE_E2E_H */
