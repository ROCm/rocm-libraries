 ...
 *
 * ckc/helper_ck_dsl.instances.common.sparse_attention.h -- C99 port of the
 * LDS bitmap primitives from ck_dsl/instances/common/sparse_attention.py
 ...
 *   -------------------------------------   ------------------------------------
 *   _BLOCK_SIZE = 64                        CKC_SPARSE_ATTN_BLOCK_SIZE
 *   _const_i8(b, value)                     ckc_sparse_attn_const_i8
 *   _cooperative_iter(b, tid, total, body)  ckc_sparse_attn_cooperative_iter
 *   _stage_jenga_mask_to_lds(...)           ckc_sparse_attn_stage_jenga_mask_to_lds
 *   _stage_vsa_bitmap_to_lds(...)           ckc_sparse_attn_stage_vsa_bitmap_to_lds
 *   _lds_bitmap_predicate(b, lds, idx)      ckc_sparse_attn_lds_bitmap_predicate
 *
 * Each routine reproduces its Python counterpart's ckc_b_* builder-call
 ...
 * sticky-error IRBuilder (ckc_b_*); a first failure latches builder->status
 * and turns subsequent calls into no-ops.
 *
 * Lifetime: every node returned is arena-owned (ckc_ir_builder_t.arena).
 * Nothing is freed individually.
 */
#ifndef CKC_HELPER_CK_DSL_INSTANCES_COMMON_SPARSE_ATTENTION_H
#define CKC_HELPER_CK_DSL_INSTANCES_COMMON_SPARSE_ATTENTION_H
...
typedef void (*ckc_sparse_attn_slot_body_fn)(ckc_ir_builder_t* b, ckc_value_t* slot, void* user);
...
void ckc_sparse_attn_cooperative_iter(ckc_ir_builder_t* b,
                                      ckc_value_t* tid,
                                      int total,
                                      ckc_sparse_attn_slot_body_fn body,
                                      void* user);
 ...
 * ``ckc_b_sync``. NULL on a builder error. */
 ...
 * handle; the caller issues the trailing ``ckc_b_sync``. NULL on a builder
 * error. */
...
#endif /* CKC_HELPER_CK_DSL_INSTANCES_COMMON_SPARSE_ATTENTION_H */
