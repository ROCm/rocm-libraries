 ...
 *
 * ckc/isa_backend.h -- C99 port of ck_dsl.core.isa.backend.
 *
 ...
 * Python -> C99 mapping
 * ---------------------
 *   class ISABackend (base, CDNA)        ckc_isa_backend_t + CKC_ISA_GFX9_MFMA
 *   class Gfx950Backend                  CKC_ISA_GFX950 (byte-identical baseline)
 *   class Gfx9MfmaBackend                CKC_ISA_GFX9_MFMA
 *   class Gfx11RdnaBackend               CKC_ISA_GFX11_RDNA
 *   class Gfx12RdnaBackend               CKC_ISA_GFX12_RDNA
 *   backend_for(arch)                    ckc_backend_for()
 *   backend.triple/.datalayout           ckc_isa_triple()/ckc_isa_datalayout()
 *   backend.module_preamble()            ckc_isa_module_preamble()
 *   backend.buffer_rsrc_word3            ckc_isa_buffer_rsrc_word3()
 *   backend.encode_waitcnt(...)          ckc_isa_encode_waitcnt(...)
 *   backend.emit_wmma(lowerer, op)       ckc_isa_emit_wmma_call(...) (text only)
 *   _RDNA_WMMA / _RDNA_WMMA_INT /        ckc_isa_wmma_lookup() / *_int / *_gfx12
 ...
 * ported as ckc_isa_emit_wmma_call(), writing into a ckc_strbuf. The MMA op
 * resolution table (op.name -> intrinsic/decl/element-type spec) is fully
 * ported as the lookup functions below.
 */
#ifndef CKC_ISA_BACKEND_H
#define CKC_ISA_BACKEND_H
...
#define CKC_BUFFER_RSRC_WORD3_CDNA 0x00027000u
/* RDNA (gfx10/11/12) "raw" SRD word3. */
#define CKC_BUFFER_RSRC_WORD3_RDNA 0x31014000u
...
typedef struct ckc_wmma_int_spec
{
    const char* op_name;
    const char* decl_key;
 ...
 * CKC_ERR_NOTIMPL if the spec needs a bitcast but a cast name is missing, or
 * -1 on OOM. */
