 ...
 *
 * ckc/instance_conv_implicit_gemm_internal.h -- PRIVATE shared state + phase-
 ...
 *   struct, ckc_conv_build_ctx_t, which holds EXACTLY the set of variables the
 *   closures shared. The driver (ckc_build_implicit_gemm_conv in
 *   ckc/instance_conv_implicit_gemm.h) populates the ctx in the same order the
 ...
 *   functions mirror the Python closure / module-helper names with a `ckc_conv_`
 *   prefix.
 *
 ...
 * ckc/instance_conv_implicit_gemm.h.
 */
#ifndef CKC_INSTANCE_CONV_IMPLICIT_GEMM_INTERNAL_H
#define CKC_INSTANCE_CONV_IMPLICIT_GEMM_INTERNAL_H
...
#define CKC_CONV_MAX_K_ITERS 4096

/* ============================================================ *
 * LdsLayout (value-type slice)
 * ============================================================ *
 *
 * The LdsLayout C port (ck_dsl/helpers/layouts.py) is a peer that is not yet a
...
typedef struct ckc_conv_build_ctx
{
    ...
    ckc_value_t* block_n_off_v; /* grid.block_n_off OR swizzled col*tile_n */
    ...
    /* ---- accumulators ---- */
    ckc_value_t* acc_init; /* zero_vec_f32(c_per_lane)    */
    ...
    ckc_tensor_descriptor_t* A_desc; /* make_a_descriptor(p, decompose_m)     */
    ckc_tensor_descriptor_t* B_desc; /* make_b_descriptor(p)                  */
    ...
    ckc_tensor_descriptor_t* D_desc;
...
/* _choose_load_vec(spec) -> width. Thin adapter over ckc_choose_load_vec. */
 ...
 * ckc_loads_descriptor_fn the loaders call; ctx is passed as `user`. */
 ...
 * ckc_conv_emit_wmma_phase on RDNA. Honours the a_operand_override hook and
 ...
 *   unroll_k                -> ckc_conv_emit_kloop_unroll
...
#endif /* CKC_INSTANCE_CONV_IMPLICIT_GEMM_INTERNAL_H */
