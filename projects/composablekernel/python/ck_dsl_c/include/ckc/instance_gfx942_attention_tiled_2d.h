 ...
 *
 * ckc/instance_gfx942_attention_tiled_2d.h -- PUBLIC C99 surface for the gfx942
 * (CDNA3) narrow-atom tiled-2D unified-attention kernel INSTANCE BUILDER
 * (ck_dsl/instances/gfx942/attention_tiled_2d.py, 5287 LOC).
 *
 *   Python (attention_tiled_2d.py)                C99 (this header)
 *   -------------------------------------------   ------------------------------------------------
 *   class UnifiedAttention2DTiledSpec             ckc_attention_tiled_2d_spec_t   (already ported:
 *     (frozen dataclass + __post_init__)            helper_helper_ck_dsl.instances.gfx942.
 *                                                   attention_tiled_2d.h -- reused verbatim here)
 *   spec.@property accessors                       ckc_attention_tiled_2d_spec_* (same helper hdr)
 *   supports_tiled_2d(...)                         ckc_gfx942_attention_tiled_2d_supports
 *   build_unified_attention_2d_tiled(spec, arch)  ckc_build_unified_attention_2d_tiled_scalar
 *   (+ convenience: build -> lower .ll)           ckc_gfx942_attention_tiled_2d_lower_to_llvm
 *
 * SCOPE NOTE / NAMING.
 ...
 *   helpers in ckc/helper_helper_ck_dsl.instances.gfx942.attention_tiled_2d.h.
 *   This header REUSES those by include and does NOT redeclare them. The entry
 *   point ckc_build_unified_attention_2d_tiled_scalar keeps the exact name the
 ...
 *   surface) carries the ``ckc_gfx942_attention_tiled_2d_`` prefix so it never
 ...
 *   ckc/instance_gfx942_attention_tiled_2d_internal.h; public callers only ever
 *   touch THIS header.
 *
 ...
 *      transposed-x8 path).
 *
 * ENTRY RETURN. ckc_build_unified_attention_2d_tiled_scalar returns the built
 ...
 * the convenience lower returns a ckc_status_t. Every IR node is arena-owned.
 */
#ifndef CKC_INSTANCE_GFX942_ATTENTION_TILED_2D_H
#define CKC_INSTANCE_GFX942_ATTENTION_TILED_2D_H
...
typedef struct ckc_gfx942_attention_tiled_2d_supports_args
{
    int head_size;
    int block_size;
 ...
 * supplied (already ckc_ir_builder_init'd, named spec.kernel_name()) builder
 ...
 * ckc/instance_gfx942_attention_tiled_2d_internal.h. It reproduces the Python
...
#endif /* CKC_INSTANCE_GFX942_ATTENTION_TILED_2D_H */
