 ...
 *
 * ckc/online.h -- one-call convenience entry points for the ONLINE portable-IR
 ...
 * (recipe VM | IR import) + ckc_lower_kernel_to_llvm so a Python/ctypes binding
 * only ever deals with char* in / char* out -- no need to expose the
 * ckc_ir_builder_t / ckc_kernel_def_t layout across the FFI boundary.
 *
 ...
 *
 * The returned *out_ll is malloc'd; free it with ckc_online_free().
 */
#ifndef CKC_ONLINE_H
#define CKC_ONLINE_H
...
/* portable-IR JSON (schema "ck.dsl.ir/v1", concrete graph) -> .ll */
