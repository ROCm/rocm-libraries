 ...
 *
 * ckc/helper_instance_gfx950_deep_fused_conv_pool_internal.h.h -- PRIVATE shared
 * state + phase / closure-function contract for the C99 port of the gfx950 (CDNA,
 * wave64, MFMA 32x32x16) arch shim over build_deep_fused_conv_pool
 *   (ck_dsl/instances/gfx950/deep_fused_conv_pool.py  ->
 *    re-exported common ck_dsl/instances/common/deep_fused_conv_pool.py
 *    build_deep_fused_conv_pool, Python lines 1212-1401).
 *
 * WHY THIS HEADER EXISTS.
 *   The gfx950 module is a thin shim that pins MFMA geometry (wave64, 32x32x16)
 *   and re-exports the common builder under the historical gfx950 public names
 *   (kernel name "ck_dsl_gfx950_deep_fused_conv_pool"). The common
 ...
 *   `spec` (here the gfx950-pinned spec), the derived `conv_spec`
 *   (spec.conv_spec()), the resolved MMA `op` (_resolve_conv_op(conv_spec,
 *   "gfx950") -> the MFMA 32x32x16 op), the `arch` string ("gfx950"), and --
 ...
 *   ckc_gfx950_dfcp_build_ctx_t, which holds EXACTLY the variables the closures
 *   share. The gfx950 public driver stamps the MFMA geometry/name onto the spec,
 *   populates the ctx in the order the Python prologue computes its locals
 *   (is_valid -> conv_spec -> op -> ctx), then hands the closure phase functions
 *   + the ctx (as the callback `user`) to the wrapped conv builder.
 *
 ...
 *   ckc_dfcp_* phases). This gfx950 surface is a SEPARATE, parallel set of
 *   symbols carrying the ckc_gfx950_dfcp_ prefix so the gfx950 shim translation
 *   units never clash with the common ones when both land in one program. The
 *   gfx950 phase bodies are byte-faithful to the common closures driven by the
 *   MFMA-resolved op; they reuse the ported common emit/pure helpers (ckc_dfcp_*
 *   from ckc/helper_ck_dsl.instances.common.deep_fused_conv_pool.h) and the
 *   shared helper closures over the common spec view (== &spec->base).
 *
 *   The gfx950 maxpool routing differs from gfx1201: gfx950 (MFMA 32x32) takes
 *   the intra-lane register-resident fast path (_maxpool_is_intra_lane) when its
 *   exact 32x32 geometry holds, and the WMMA intra-lane fast path is naturally
 *   geometry-gated off; otherwise it falls through to the layout-agnostic
 *   cshuffle-LDS gather + maxpool.
 *
 * CONTRACT STABILITY (bucket note).
 *   This header is the ONE shared surface every gfx950 body-implementing agent
 ...
 *   the Python closure names with a `ckc_gfx950_dfcp_` prefix.
 *
 * THIS HEADER EMITS NO IR AND DECLARES NO PUBLIC API. It is included only by the
 * helper_instance_gfx950_deep_fused_conv_pool_internal.h.c translation unit (and
 * any sibling gfx950 shim phase .c files).
 */
#ifndef CKC_HELPER_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_INTERNAL_H_H
#define CKC_HELPER_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_INTERNAL_H_H
 ...
 * ckc_implicit_gemm_conv_spec_t + the emit/pure phase helper prototypes
 * (ckc_dfcp_*) the gfx950 closures reuse, plus the common spec value type
 * ckc_deep_fused_conv_pool_spec_t the gfx950 spec wraps. */
...
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_NAME "ck_dsl_gfx950_deep_fused_conv_pool"

/* The pinned CDNA MFMA geometry the gfx950 shim stamps (Python class defaults:
 * wave_size=64, warp_tile 32x32x16). */
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_WAVE_SIZE 64
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_WARP_TILE_M 32
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_WARP_TILE_N 32
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_WARP_TILE_K 16

/* The arch string this shim targets; the common builder routes to MFMA codegen
 * when handed this. */
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_ARCH "gfx950"

/* The per-warp conv1 accumulator count is mfmas_per_warp_m * mfmas_per_warp_n.
 * For the gfx950 MFMA deep-fusion tile space (one CTA owns all channels,
 * tile_n<=32, warp_tile 32x32) this is tiny (1 for the MFMA-32x32 intra-lane
 * register-resident path, a small handful otherwise). 64 is generous headroom
 * and matches the common internal-header convention. */
#define CKC_GFX950_DFCP_MAX_ACCS 64

/* ------------------------------------------------------------------ *
 * Gfx950DeepFusedConvPoolSpec
 * ------------------------------------------------------------------ *
 *
 * The common spec pinned to the MFMA geometry. Identical fields to the common
 * ckc_deep_fused_conv_pool_spec_t (embedded as `base`); only the gfx950 defaults
 * differ (kernel name; the wave64 32x32x16 geometry is already the common
 * default). The struct is a single-member wrapper so `&spec->base` is the common
 * spec the shared builder consumes (mirroring the Python subclass relationship
 * `Gfx950DeepFusedConvPoolSpec(DeepFusedConvPoolSpec)`).
 *
 * NOTE: this type is also published by the gfx950 PUBLIC header when that exists;
 * it is declared here (guarded) so this internal surface is self-contained for
 * translation units that include only the internal header. */
#ifndef CKC_GFX950_DEEP_FUSED_CONV_POOL_SPEC_DEFINED
#define CKC_GFX950_DEEP_FUSED_CONV_POOL_SPEC_DEFINED
typedef struct ckc_gfx950_deep_fused_conv_pool_spec
{
    ckc_deep_fused_conv_pool_spec_t base;
} ckc_gfx950_deep_fused_conv_pool_spec_t;
#endif /* CKC_GFX950_DEEP_FUSED_CONV_POOL_SPEC_DEFINED */

/* ===================================================================== *
 *  ckc_gfx950_dfcp_build_ctx_t
 *
 *  The single shared state object for the gfx950 shim. Holds every
 *  enclosing-function local that the build_deep_fused_conv_pool closures
 *  capture (with `op` resolved to the MFMA 32x32x16 op, `arch` == "gfx950"),
 ...
 *  builder hands in.
 * ===================================================================== */
typedef struct ckc_gfx950_dfcp_build_ctx
{
    /* =============================================================== *
     * (A) BUILD-TIME CONSTANTS -- set once by the gfx950 driver before calling
    ...
    /* op = _resolve_conv_op(conv_spec, "gfx950"); the resolved MFMA MMA op
     * (wave64, m=n=32, k=16) that drives the numeric core
    ...
    /* ---- MFMA register-residency decision (gfx950 maxpool routing) ----
     * The gfx950 MFMA path picks between the register-resident intra-lane
     * maxpool fast path and the generic cshuffle-LDS gather maxpool. Staged so
     * the epilogue dispatch never re-derives the geometry predicate.
     *   use_mfma_register_maxpool =
     *       _maxpool_is_intra_lane(common_spec, grid)
     * (The WMMA intra-lane fast path is geometry-gated off for MFMA warp_tile
     *  32x32, so it is never selected here.) */
    bool use_mfma_register_maxpool;
     ...
     * thread/lane/warp decode + tile geometry the emit helpers read (wave64
     * geometry for gfx950). */
     ...
     * per-warp tile count. */
    ckc_value_t* conv0_accs[CKC_GFX950_DFCP_MAX_ACCS];
    size_t num_conv0_accs;
    ckc_value_t* conv1_accs[CKC_GFX950_DFCP_MAX_ACCS];
    size_t num_conv1_accs;
} ckc_gfx950_dfcp_build_ctx_t;

/* Zero-initialise the ctx and populate the (A) build-time constants from the
 * gfx950 spec + arch: normalises arch to "gfx950", sets common_spec =
 * &spec->base, computes conv_spec / op (op is the MFMA 32x32x16 op via the peer
 * conv ports + _resolve_conv_op), derives `defer` / `deferred_epi`, resolves the
 * MFMA register-residency decision and the A-load routing flags. The caller owns
 ...
 *  build_deep_fused_conv_pool, driven by the gfx950 (MFMA) op. Each reads/
 ...
 * ctx->defer), then route to the MFMA register-resident intra-lane maxpool
 * (ckc_dfcp_emit_inline_maxpool_from_registers when
 *  ctx->use_mfma_register_maxpool) OR the generic cshuffle-LDS maxpool (stage
...
#endif /* CKC_HELPER_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_INTERNAL_H_H */
