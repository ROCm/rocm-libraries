...
/*
 * bindings/ckc_engine_fmha_extra.cpp -- bindings for the remaining FMHA-fwd
 * variant families and the fused-MoE end-to-end orchestrator, kept in their own
 * translation unit for the same reason as ckc_engine_attention.cpp: the FMHA
 * instance headers re-use a set of struct tags (ckc_fmha_*) across several
 ...
 * (ckc_engine_fused_moe_e2e.cpp): its internal header drags in the
 * moe_gemm_fused / tensor_view headers, whose ckc_tensor_descriptor tag clashes
...
/* Build a ckc_fmha_common_spec_t from the common sub-keys of the dict. Mirrors
 * the make_spec helpers in the standalone emitters: when block_size_q /
 * block_size_k are present the shape is built via ckc_fmha_shape_make, else
 * ckc_fmha_shape_default. */
ckc_fmha_common_spec_t common_of(const py::dict& d, Store& st)
{
    ...
    if(d.contains("block_size_q") || d.contains("block_size_k"))
        shape = ckc_fmha_shape_make(x_int(d, "head_size", 0),
 ...
 * spec.kernel_name(), then calls ckc_build_fmha_fwd_appendkv(&b, &spec, arch),
 * then lowers via ckc_lower_kernel_to_llvm_ex. */
...
ckc_fmha_appendkv_spec_t appendkv_build(const py::dict& d, Store& st, bool* ok, std::string* why)
{
    *ok                        = true;
    ...
    if(x_bool(d, "has_rotary", false))
    {
        ...
        ckc_rotary_layout_t l = (lay == "interleaved") ? CKC_ROTARY_INTERLEAVED : CKC_ROTARY_HALF;
        ckc_rotary_spec_t rot;
        ckc_status_t rs = ckc_rotary_spec_init(&rot,
...
ckc_kernel_def_t* appendkv_make(const py::dict& d, Store& st, ckc_ir_builder_t* b, const char* arch)
{
    bool ok = true;
    std::string why;
    ckc_fmha_appendkv_spec_t s = appendkv_build(d, st, &ok, &why);
    if(!ok)
        throw std::runtime_error("ckc_engine.fmha_appendkv: " + why);
    char namebuf[256];
    namebuf[0] = '\0';
    if(ckc_fmha_appendkv_kernel_name(&s, namebuf, sizeof namebuf) != CKC_OK)
        throw std::runtime_error("ckc_engine.fmha_appendkv kernel_name failed");
    ckc_ir_builder_init(b, namebuf);
    return ckc_build_fmha_fwd_appendkv(b, &s, arch);
}
...
std::string appendkv_lower(const py::dict& d, const std::string& arch)
{
    ...
    ckc_ir_builder_t b;
    ckc_kernel_def_t* k = appendkv_make(d, st, &b, a);
    if(!k || !ckc_ir_builder_ok(&b))
    {
        std::string m = std::string("fmha_appendkv build failed: ") + ckc_ir_builder_error(&b);
...
std::string appendkv_serialize(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = ser_kernel(k, "ckc_engine.fmha_appendkv_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
/* ========================== fmha_paged_prefill ========================== */
/* Single KernelDef -> one .ll. kb-based build + ckc_lower_kernel_to_llvm_ex. */
...
std::string paged_lower(const py::dict& d, const std::string& arch)
{
    ...
    ckc_kernel_def_t* k = ckc_build_fmha_fwd_paged_prefill(&kb, &s, a);
    if(!k)
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_paged_prefill_lower_llvm build failed");
    }
    char* ll = nullptr;
    char err[CKC_ERR_MSG_CAP];
    err[0]          = '\0';
...
std::string paged_serialize(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_paged_prefill_serialize_ir build failed");
    }
    std::string out = ser_kernel(k, "ckc_engine.fmha_paged_prefill_serialize_ir");
    ckc_fmha_kernel_builder_free(&kb);
    return out;
}
std::vector<std::string> paged_verify(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_paged_prefill_verify build failed");
    }
    std::vector<std::string> out = ver_kernel(k);
    ckc_fmha_kernel_builder_free(&kb);
    return out;
...
/* ============================== fmha_varlen ============================= */
/* Single KernelDef -> one .ll. ckc_build_fmha_fwd_varlen takes an err buffer. */

ckc_fmha_fwd_varlen_spec_t varlen_build(const py::dict& d, Store& st)
{
    ckc_fmha_common_spec_t c = common_of(d, st);
    return ckc_fmha_fwd_varlen_spec_default(
...
std::string varlen_serialize(const py::dict& d, const std::string& arch)
{
    ...
    try
    {
        out = ser_kernel(k, "ckc_engine.fmha_varlen_serialize_ir");
    }
    catch(...)
    {
        ckc_fmha_kernel_builder_free(&kb);
...
std::string splitkv_lower_one(const ckc_fmha_splitkv_decode_spec_t* s, const char* a, bool reduce)
{
    char* ll = nullptr;
    char err[CKC_ERR_MSG_CAP];
    err[0]          = '\0';
    ckc_status_t s2 = reduce ? ckc_fmha_splitkv_decode_reduce_lower_to_llvm(
                             ...
                             : ckc_fmha_splitkv_decode_segment_lower_to_llvm(
                                   s, a, CKC_LLVM_FLAVOR_AUTO, &ll, err, sizeof err);
    return take_ll(s2, ll, err, "ckc_engine.fmha_splitkv_decode_lower_llvm");
}
...
std::string splitkv_serialize(const py::dict& d, const std::string& arch)
{
    ...
    ckc_fmha_kernel_builder_t kb;
    std::memset(&kb, 0, sizeof kb);
    ckc_kernel_def_t* k = reduce ? ckc_build_fmha_fwd_splitkv_decode_reduce(&kb, &s, a)
                                 : ckc_build_fmha_fwd_splitkv_decode_segment(&kb, &s, a);
    if(!k)
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_splitkv_decode_serialize_ir build failed");
    }
    std::string out = ser_kernel(k, "ckc_engine.fmha_splitkv_decode_serialize_ir");
    ckc_fmha_kernel_builder_free(&kb);
    return out;
}
std::vector<std::string> splitkv_verify(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_splitkv_decode_verify build failed");
    }
    std::vector<std::string> out = ver_kernel(k);
    ckc_fmha_kernel_builder_free(&kb);
    return out;
