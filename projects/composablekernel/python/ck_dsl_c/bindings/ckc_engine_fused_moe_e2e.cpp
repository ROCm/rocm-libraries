...
/*
 * bindings/ckc_engine_fused_moe_e2e.cpp -- binding for the fused-MoE end-to-end
 ...
 * in the moe_gemm_fused / tensor_view headers, whose ckc_tensor_descriptor tag
 * clashes with the transforms.h definition pulled in by the FMHA common header
 * in ckc_engine_fmha_extra.cpp -- the same isolation reason as
 * ckc_engine_moe_gemm_fused.cpp.
 *
 * fused_moe_e2e is NOT a single bound lowering unit. It is a host-launch
 * orchestrator: ckc_build_fused_moe_forward returns a kernel whose forward_fn
 ...
 * builder shape) -- via ckc_fused_moe_forward_lower_to_llvm and prints them
 ...
 * at the dataclass default (ckc_fmoe_forward_spec_default), so the tile-swap
 ...
 * same tile-swap policy the lower path runs (ckc_fmoe_build_ctx_init). Returns
 * NULL on failure; the caller owns *b. Mirrors the standalone emitter's
 * build_stage_kernel(). */
ckc_kernel_def_t* fmoe_build_stage_kernel(ckc_ir_builder_t* b,
                                          ...
                                          ckc_fmoe_stage_t stage)
{
    ...
    if(!ctx)
        return nullptr;
    if(ckc_fmoe_build_ctx_init(ctx, spec, spec->arch ? spec->arch : "gfx950") != CKC_OK)
    {
...
std::string fmoe_serialize(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_ir_builder_t b;
        ckc_kernel_def_t* k = fmoe_build_stage_kernel(&b, &s, FMOE_STAGES[i].stage);
        if(!k)
        {
        ...
        try
        {
            text = ser_kernel(k, "ckc_engine.fused_moe_e2e_serialize_ir");
        }
        catch(...)
        {
            ckc_ir_builder_free(&b);
