 ...
 * helper_ck_dsl.helpers.transforms.h that the WMMA-FMHA / fmha attention headers
 ...
 * bound in a separate translation unit (ckc_engine_img2col.cpp) and registered
 * via register_img2col(). */
}

namespace py = pybind11;

/* Defined in ckc_engine_img2col.cpp (separate TU to dodge the ckc_conv_problem
 * tag clash described above). Adds the img2col_* entry points to the module. */
void register_img2col(py::module_& m);

/* The attention families share a set of struct tags (ckc_fmha_*,
 * ckc_attention_tiled_2d_spec_t, the 3d spec tags) across many instance headers;
...
/* Fill a ckc_gemm_universal_spec_t (already default-initialised) from a spec
 ...
 * verifier run generically on the returned ckc_kernel_def_t*.
 * ------------------------------------------------------------------------ */

/* Wrap a `*_lower_to_llvm` result: raise on failure, else return the .ll. */
std::string take_lowered(ckc_status_t st, char* llvm_text, const char* err, const char* fn)
{
    if(st != CKC_OK || !llvm_text)
    {
        if(llvm_text)
...
ckc_batched_gemm_spec_t bg_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_batched_gemm_spec_t s = ckc_batched_gemm_spec_default();
...
ckc_matmul_nbits_spec_t mn_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_matmul_nbits_spec_t s = ckc_matmul_nbits_spec_default();
    {
        std::string v;
        if(dict_str(d, "name", v))
            s.name = keep(v);
...
ckc_d_load_kind_t parse_d_load_kind(const std::string& s)
{
    if(s == "stock")
...
ckc_gemm_multi_d_spec_t md_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_gemm_multi_d_spec_t s = ckc_gemm_multi_d_spec_default();
    ...
    ckc_gemm_universal_spec_finalize(&s.base);
                                         ...
                                         std::vector<ckc_gemm_abd_b_operand_t>& b_ops)
{
    ...
    ckc_gemm_multi_abd_spec_t s = ckc_gemm_multi_abd_spec_default();
...
/* Fill a ckc_conv_problem_t from a "problem" sub-dict (used by
...
ckc_direct_conv_16c_spec_t dg16_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_direct_conv_16c_spec_t s = ckc_direct_conv_16c_spec_default();
    if(d.contains("problem") && py::isinstance<py::dict>(d["problem"]))
...
ckc_direct_conv_4c_spec_t dg4_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_direct_conv_4c_spec_t s = ckc_direct_conv_4c_spec_default();
    if(d.contains("problem") && py::isinstance<py::dict>(d["problem"]))
        fill_direct_conv_problem(&s.problem, d["problem"].cast<py::dict>());
    {
        std::string v;
...
std::string conv_direct_grouped_serialize_ir(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = serialize_kernel(k, "ckc_engine.conv_direct_grouped_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
ckc_deep_fused_conv_pool_spec_t dfcp_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    return ckc_make_deep_fused_conv_pool_spec(dict_int(d, "n", 1),
                                              dict_int(d, "h", 0),
                                              dict_int(d, "w", 0),
                                              dict_int(d, "c", 0),
                                              dict_int(d, "k0", 0),
...
std::string rmsnorm2d_lower_llvm(const py::dict& d, const std::string& arch)
{
    ...
    ckc_ir_builder_free(&b);
    return take_lowered(st, ll, err, "ckc_engine.rmsnorm2d_lower_llvm");
}
...
std::string smoothquant_lower_llvm(const py::dict& d, const std::string& arch)
{
    ...
    ckc_status_t st = ckc_lower_kernel_to_llvm_ex(
        k, CKC_LLVM_FLAVOR_AUTO, arch_or_default(arch), &ll, err, sizeof err);
    ckc_ir_builder_free(&b);
    return take_lowered(st, ll, err, "ckc_engine.smoothquant_lower_llvm");
}
...
std::string moe_smoothquant_lower_llvm(const py::dict& d, const std::string& arch)
{
    ...
    ckc_status_t st = ckc_lower_kernel_to_llvm_ex(
        k, CKC_LLVM_FLAVOR_AUTO, arch_or_default(arch), &ll, err, sizeof err);
    ckc_ir_builder_free(&b);
    return take_lowered(st, ll, err, "ckc_engine.moe_smoothquant_lower_llvm");
}
...
ckc_moe_fused_mega_kernel_spec_t mfm_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_moe_fused_mega_kernel_spec_finalize(&s);
    return s;
}
...
std::string moe_fused_mega_fp8_serialize_ir(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = serialize_kernel(k, "ckc_engine.moe_fused_mega_fp8_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
ckc_kernel_def_t* fmoe_build_phase(const py::dict& d,
                                   ckc_ir_builder_t* b,
                                   const ckc_fused_moe_spec_t* s,
                                   const char* arch)
{
    std::string phase = "gather";
    dict_str(d, "phase", phase);
    if(phase == "silu_mul")
        return ckc_build_moe_silu_mul_new(b, s, arch);
    if(phase == "silu_mul_packed")
        return ckc_build_moe_silu_mul_packed_new(b, s, arch);
    if(phase == "static_scatter_gather")
        return ckc_build_moe_static_scatter_gather_new(b, s, arch);
    if(phase == "topk_weighted_reduce")
        return ckc_build_moe_topk_weighted_reduce_new(b, s, arch);
    return ckc_build_moe_gather_new(b, s, arch);
}
...
ckc_kernel_def_t* msort_build_phase(const py::dict& d,
                                    ckc_ir_builder_t* b,
                                    const ckc_moe_sorting_spec_t* s,
                                    const char* arch)
{
    std::string phase = "histogram";
    dict_str(d, "phase", phase);
    if(phase == "scan")
        return ckc_build_moe_sort_scan_new(b, s, arch);
    if(phase == "scatter")
        return ckc_build_moe_sort_scatter_new(b, s, arch);
    if(phase == "persistent")
        return ckc_build_moe_sort_persistent_new(b, s, arch);
    return ckc_build_moe_sort_histogram_new(b, s, arch);
}
...
ckc_wmma_gemm_int8_spec_t w1151i8_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_wmma_gemm_int8_spec_t s = ckc_wmma_gemm_int8_spec_default();
    {
        std::string v;
        if(dict_str(d, "dtype", v))
            s.dtype = keep(v);
...
ckc_wmma_gemm_iu8_spec_t w1151iu8_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_wmma_gemm_iu8_spec_t s = ckc_wmma_gemm_iu8_spec_default();
    {
        std::string v;
        if(dict_str(d, "name", v))
            s.name = keep(v);
                                                       ...
                                                       std::deque<std::string>& store)
{
    ...
    ckc_wmma_gemm_iu8_dequant_spec_t s = ckc_wmma_gemm_iu8_dequant_spec_default();
    {
        std::string v;
        if(dict_str(d, "name", v))
            s.name = keep(v);
...
ckc_fmha_mask_mode_t parse_fmha_mask(const std::string& s)
{
    if(s == "causal")
...
std::string gfx1151_wmma_fmha_fwd_lower_llvm(const py::dict& d, const std::string& arch)
{
    ...
    char kname[512];
    kname[0] = '\0';
    ckc_wmma_fmha_fwd_kernel_name(&s, kname, sizeof kname);
...
std::string gfx1151_wmma_fmha_fwd_serialize_ir(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = serialize_kernel(k, "ckc_engine.gfx1151_wmma_fmha_fwd_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
ckc_wmma_gemm_gfx1201_spec_t w1201_build_spec(const py::dict& d, std::deque<std::string>& store)
{
    ...
    ckc_wmma_gemm_gfx1201_spec_t s = ckc_wmma_gemm_gfx1201_spec_default();
    {
        std::string v;
        if(dict_str(d, "dtype", v))
            s.dtype = keep(v);
