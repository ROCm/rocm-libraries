                                    ...
                                    const char* arch)
{
            ...
            for (int ni = 0; ni < ctx->mfmas_n; ++ni)
            {
                if (idx >= CKC_MOE_MAX_ACCS)
                {
                    ckc_i_set_err(b, CKC_ERR_VALUE,
                                  "build_moe_gate_up_silu_gemm: too many accumulators");
                    return false;
                }
                char nm[48];
    ...
    if (!ckc_moe_make_lds_view2(b, &ctx->a_lds_view, ctx->A_smem, ctx->block_m, ctx->block_k,
                                ctx->storage_dtype) ||
        !ckc_moe_make_lds_view2(b, &ctx->bg_lds_view, ctx->Bg_smem, ctx->block_n, ctx->block_k,
                                ctx->storage_dtype) ||
        !ckc_moe_make_lds_view2(b, &ctx->bu_lds_view, ctx->Bu_smem, ctx->block_n, ctx->block_k,
                                ctx->storage_dtype))
    {
        return false;
    }
