...
ckc_value_t* ckc_moe_interleaved_load_wgateup(ckc_ir_builder_t* bb,
                                              int e,
                                              ckc_value_t* k_off,
                                              ckc_value_t* row,
                                              ckc_value_t* col,
                                              void* user)
{
    ...
    ckc_value_t* glob_off =
        ckc_b_add(bb, base_off, ckc_b_mul(bb, vec_idx, ctx->plan.c_load_vec));
    if (load_vec == 1)
    {
                                        ...
                                        const char* arch)
{
    ...
    /* ok, why = is_valid_gemm_spec(u, arch); raise on reject */
    {
        ...
        if (!ckc_gemm_universal_is_valid_spec(u, ctx->arch, why, sizeof(why)))
        {
            ...
            snprintf(msg, sizeof(msg),
                     "invalid interleaved gate/up GEMM spec: %s", why);
    ...
    op.noalias = true; op.noalias_set = true;
    op.readonly = true; op.readonly_set = true;
    op.align = 16; op.align_set = true;
    ...
    memset(&op, 0, sizeof(op));
    op.noalias = true; op.noalias_set = true;
    op.writeonly = true; op.writeonly_set = true;
    op.align = 16; op.align_set = true;
    ...
    if (ctx->block_n % 2)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "interleaved gate/up requires even tile_n");
        return false;
    }
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "interleaved ctx_init: acc group exceeds CKC_MOE_MAX_ACCS");
        return false;
    }
    {
        int idx = 0;
    ...
    {
        ckc_value_t* bucket_head = ckc_b_add(
            b, ckc_b_mul(b, ckc_b_block_id_z(b), ctx->slot_size_p), ctx->block_m_off);
        ckc_value_t* first_token =
            ckc_b_global_load_i32(b, ctx->sorted_token_ids, bucket_head, 0);
