...
static void ckc__set_unknown_arch_reason(char* out, size_t out_cap, const char* gfx)
{
    ...
    wrote = snprintf(out + pos, out_cap - pos,
                     "\"unknown gfx target '%s'; known: [", gfx);
    if (wrote < 0)
    {
        out[0] = '\0';
        return;
    ...
    for (i = 0; i < count; ++i)
    {
        wrote = snprintf(out + pos, out_cap - pos, "%s'%s'",
                         (i == 0) ? "" : ", ", arches[i]);
        if (wrote < 0)
        {
            out[out_cap - 1] = '\0';
...
bool ckc_validate_fmha_mfma_atom(const char* dtype,
                                 const char* arch,
                                 char* out,
                                 size_t out_cap)
{
    ...
    if (ckc_archtarget_op_for_shape(target, "mma", a_name, a_name, "fp32",
                                    blk, blk, blk) == NULL)
    {
        if (out != NULL && out_cap != 0)
        {
