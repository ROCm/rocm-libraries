 ...
 *
 * lower_hip_lower_hip_cast.c -- C99 port of ck_dsl.core.lower_hip, BUCKET 1:
 ...
 * ckc_h_emitf / ckc_h_name / ckc_h_type_to_hip / ...) are DEFINED in bucket 0
 * (lower_hip_core.c) and only called here. The registration table is exported
 * via ckc_h_handlers_cast(), which bucket 0 stitches into the dispatch table.
 *
 ...
 * here ckc_h_emit/ckc_h_emitf does the same).
 */
#include "ckc/ir.h"
...
static ckc_status_t ckc_h_op_arith_zext(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
...
static ckc_status_t ckc_h_op_arith_sext(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *t = ckc_h_type_to_hip(lw, r->type);
    ckc_h_emitf(lw, "%s %s = (%s)%s;", t, ckc_h_name(lw, r), t, ckc_h_name(lw, v));
...
static ckc_status_t ckc_h_op_arith_trunc(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *t = ckc_h_type_to_hip(lw, r->type);
    ckc_h_emitf(lw, "%s %s = (%s)%s;", t, ckc_h_name(lw, r), t, ckc_h_name(lw, v));
...
static ckc_status_t ckc_h_op_arith_trunc_f32_to_f16(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    ckc_h_emitf(lw, "fp16 %s = (fp16)%s;", ckc_h_name(lw, r), ckc_h_name(lw, v));
    return lw->status;
}
...
static ckc_status_t ckc_h_op_arith_rint_f32(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    ckc_h_emitf(lw, "float %s = rintf(%s);", ckc_h_name(lw, r), ckc_h_name(lw, v));
    return lw->status;
}
...
static ckc_status_t ckc_h_op_arith_bitcast(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
...
static ckc_status_t ckc_h_op_arith_cast_to_f32(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    ckc_h_emitf(lw, "float %s = (float)%s;", ckc_h_name(lw, r), ckc_h_name(lw, v));
    return lw->status;
}
...
static ckc_status_t ckc_h_op_arith_cast_f32_to(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
 ...
 *     (v,) = op.operands
 *     self._emit(f"float {_name(op.result)} = (float){_name(v)};") */
static ckc_status_t ckc_h_op_arith_sitofp_f32(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    ckc_h_emitf(lw, "float %s = (float)%s;", ckc_h_name(lw, r), ckc_h_name(lw, v));
    return lw->status;
...
static ckc_status_t ckc_h_op_arith_cvt_fp8_to_f32(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    ckc_h_emitf(lw,
                "float %s = __builtin_amdgcn_cvt_f32_fp8("
                "(unsigned int)(unsigned char)%s, 0);",
                ckc_h_name(lw, r), ckc_h_name(lw, v));
    return lw->status;
}

/* def _op_arith_cvt_bf8_to_f32(self, op): -- e5m2 sibling. */
static ckc_status_t ckc_h_op_arith_cvt_bf8_to_f32(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    ckc_h_emitf(lw,
...
static ckc_status_t ckc_h_op_arith_cvt_f32_to_fp8(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *rn = ckc_h_name(lw, r);
    ckc_h_emitf(lw,
                ...
                "%s, 0.0f, 0u, false);",
                rn, ckc_h_name(lw, v));
    ckc_h_emitf(lw, "fp8e4m3 %s = (fp8e4m3)(%s_pk & 0xffu);", rn, rn);
    return lw->status;
}

/* def _op_arith_cvt_f32_to_bf8(self, op): -- e5m2 sibling. */
static ckc_status_t ckc_h_op_arith_cvt_f32_to_bf8(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *rn = ckc_h_name(lw, r);
    ckc_h_emitf(lw,
                ...
                "%s, 0.0f, 0u, false);",
                rn, ckc_h_name(lw, v));
    ckc_h_emitf(lw, "bf8e5m2 %s = (bf8e5m2)(%s_pk & 0xffu);", rn, rn);
    return lw->status;
}
...
static ckc_status_t ckc_h_op_arith_cvt_f32_to_i8_sat(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *rn = ckc_h_name(lw, r);
...
static ckc_status_t ckc_h_op_arith_cvt_pk_f32_fp8x4(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    ...
    ckc_h_emitf(lw, "unsigned int %s_p; __builtin_memcpy(&%s_p, &%s, sizeof(%s_p));",
                nice, nice, vn, nice);
    ckc_h_emitf(lw, "%s %s_lo = __builtin_amdgcn_cvt_pk_f32_fp8(%s_p, false);",
                pair_t, nice, nice);
    ckc_h_emitf(lw, "%s %s_hi = __builtin_amdgcn_cvt_pk_f32_fp8(%s_p, true);",
                pair_t, nice, nice);
    ckc_h_emitf(lw, "%s %s;", res_t, nice);
    ckc_h_emitf(lw, "%s[0] = %s_lo.x;", nice, nice);
    ckc_h_emitf(lw, "%s[1] = %s_lo.y;", nice, nice);
    ckc_h_emitf(lw, "%s[2] = %s_hi.x;", nice, nice);
    ckc_h_emitf(lw, "%s[3] = %s_hi.y;", nice, nice);
    return lw->status;
}
...
static ckc_status_t ckc_h_op_arith_cvt_pk_f32_bf8x4(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    ...
    ckc_h_emitf(lw, "unsigned int %s_p; __builtin_memcpy(&%s_p, &%s, sizeof(%s_p));",
                nice, nice, vn, nice);
    ckc_h_emitf(lw, "%s %s_lo = __builtin_amdgcn_cvt_pk_f32_bf8(%s_p, false);",
                pair_t, nice, nice);
    ckc_h_emitf(lw, "%s %s_hi = __builtin_amdgcn_cvt_pk_f32_bf8(%s_p, true);",
                pair_t, nice, nice);
    ckc_h_emitf(lw, "%s %s;", res_t, nice);
    ckc_h_emitf(lw, "%s[0] = %s_lo.x;", nice, nice);
    ckc_h_emitf(lw, "%s[1] = %s_lo.y;", nice, nice);
...
static ckc_status_t ckc_h_op_arith_cvt_pk_fp8_f32x4(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *nice = ckc_h_name(lw, r);
    const char *vn = ckc_h_name(lw, v);
    ...
    ckc_h_emitf(lw,
                ...
                "%s[2], %s[3], %s_lo, true);",
                nice, vn, vn, nice);
    ckc_h_emitf(lw, "%s %s; __builtin_memcpy(&%s, &%s_p, sizeof(%s));",
                res_t, nice, nice, nice, nice);
    return lw->status;
}

/* def _op_arith_cvt_pk_bf8_f32x4(self, op): -- e5m2 sibling. */
static ckc_status_t ckc_h_op_arith_cvt_pk_bf8_f32x4(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *nice = ckc_h_name(lw, r);
    const char *vn = ckc_h_name(lw, v);
...
static ckc_status_t ckc_h_op_arith_cvt_pk_i8_f32x4(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    const ckc_value_t *v = op->operands[0];
    const ckc_value_t *r = h_res(op);
    const char *nice = ckc_h_name(lw, r);
    const char *vn = ckc_h_name(lw, v);
    ...
    ckc_h_emitf(lw, "%s %s;", res_t, nice);
    for (i = 0; i < 4; i++) {
        ckc_h_emitf(lw, "float %s_r%d = rintf(%s[%d]);", nice, i, vn, i);
        ckc_h_emitf(lw, "int %s_i%d = (int)%s_r%d;", nice, i, nice, i);
        ckc_h_emitf(lw, "%s_i%d = (%s_i%d < -128) ? -128 : ((%s_i%d > 127) ? 127 : %s_i%d);",
                    nice, i, nice, i, nice, i, nice, i);
        ckc_h_emitf(lw, "%s[%d] = (int8_t)%s_i%d;", nice, i, nice, i);
    }
    return lw->status;
}
...
static ckc_status_t ckc_h_op_gpu_thread_id(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    ...
    ckc_h_emitf(lw, "int %s = (int)threadIdx.%s;", ckc_h_name(lw, r), axis);
    return lw->status;
}
...
static ckc_status_t ckc_h_op_gpu_block_id(ckc_h_lowerer_t *lw, const ckc_op_t *op) {
    ...
    ckc_h_emitf(lw, "int %s = (int)blockIdx.%s;", ckc_h_name(lw, r), axis);
    return lw->status;
}

/* ----------------------------- registration table -------------------------- */

const ckc_h_handler_entry_t *ckc_h_handlers_cast(void) {
    static const ckc_h_handler_entry_t table[] = {
        { CKC_OP_ARITH_ZEXT,             ckc_h_op_arith_zext },
        { CKC_OP_ARITH_SEXT,             ckc_h_op_arith_sext },
        { CKC_OP_ARITH_TRUNC,            ckc_h_op_arith_trunc },
        { CKC_OP_ARITH_TRUNC_F32_TO_F16, ckc_h_op_arith_trunc_f32_to_f16 },
        { CKC_OP_ARITH_RINT_F32,         ckc_h_op_arith_rint_f32 },
        { CKC_OP_ARITH_BITCAST,          ckc_h_op_arith_bitcast },
        { CKC_OP_ARITH_CAST_TO_F32,      ckc_h_op_arith_cast_to_f32 },
        { CKC_OP_ARITH_CAST_F32_TO,      ckc_h_op_arith_cast_f32_to },
        { CKC_OP_ARITH_SITOFP_F32,       ckc_h_op_arith_sitofp_f32 },
        { CKC_OP_ARITH_CVT_FP8_TO_F32,   ckc_h_op_arith_cvt_fp8_to_f32 },
        { CKC_OP_ARITH_CVT_BF8_TO_F32,   ckc_h_op_arith_cvt_bf8_to_f32 },
        { CKC_OP_ARITH_CVT_F32_TO_FP8,   ckc_h_op_arith_cvt_f32_to_fp8 },
        { CKC_OP_ARITH_CVT_F32_TO_BF8,   ckc_h_op_arith_cvt_f32_to_bf8 },
        { CKC_OP_ARITH_CVT_F32_TO_I8_SAT, ckc_h_op_arith_cvt_f32_to_i8_sat },
        { CKC_OP_ARITH_CVT_PK_F32_FP8X4, ckc_h_op_arith_cvt_pk_f32_fp8x4 },
        { CKC_OP_ARITH_CVT_PK_F32_BF8X4, ckc_h_op_arith_cvt_pk_f32_bf8x4 },
        { CKC_OP_ARITH_CVT_PK_FP8_F32X4, ckc_h_op_arith_cvt_pk_fp8_f32x4 },
        { CKC_OP_ARITH_CVT_PK_BF8_F32X4, ckc_h_op_arith_cvt_pk_bf8_f32x4 },
        { CKC_OP_ARITH_CVT_PK_I8_F32X4,  ckc_h_op_arith_cvt_pk_i8_f32x4 },
        { CKC_OP_GPU_THREAD_ID,          ckc_h_op_gpu_thread_id },
        { CKC_OP_GPU_BLOCK_ID,           ckc_h_op_gpu_block_id },
