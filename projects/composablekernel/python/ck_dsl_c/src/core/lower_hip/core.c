...
const char *ckc_h_type_to_hip(ckc_h_lowerer_t *lw, const ckc_type_t *t) {
    ...
    if (t->kind == CKC_TYPE_VECTOR) {
        ...
        if (strcmp(elem, "i1") == 0) {
            ...
            if (strcmp(elem, "f16") != 0 && strcmp(elem, "bf16") != 0 &&
                strcmp(elem, "f32") != 0 && strcmp(elem, "i32") != 0 &&
                strcmp(elem, "i16") != 0 && strcmp(elem, "i8") != 0 &&
    ...
    /* scalar: _HIP_TYPE[t.name] */
    {
        ...
        if (!s) {
            ckc_h_fail(lw, CKC_ERR_KEY, "type_to_hip: unmappable type '%s'",
                       t->name ? t->name : "(null)");
            return "";
        }
        return s;
...
const char *ckc_h_f32_literal(ckc_h_lowerer_t *lw, double val) {
    ...
    if (isinf(val)) {
        out = ckc_arena_strdup(&lw->b->arena,
...
ckc_status_t ckc_h_require_wmma_arch(ckc_h_lowerer_t *lw, const char *op_id) {
    ...
    if (!lw->arch.has_wmma) {
        return ckc_h_fail(lw, CKC_ERR_NOTIMPL,
...
void ckc_h_smem_set_storage(ckc_h_lowerer_t *lw, const ckc_value_t *smem_result,
