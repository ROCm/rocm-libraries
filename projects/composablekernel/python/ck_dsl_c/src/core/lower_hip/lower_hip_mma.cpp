...
static ckc_status_t ckc_h_op_tile_mma(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    ...
    if(!fn)
    {
        ...
        return ckc_h_fail(lw, CKC_ERR_NOTIMPL, "no HIP lowering for op '%s'", dotted);
    }
...
static ckc_status_t ckc_h_op_tile_register_p_from_qk_c(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    ...
    if(!target)
    {
        return ckc_h_fail(lw, CKC_ERR_KEY, "register_p_from_qk_c: missing 'target_dtype' attr");
    }
...
static ckc_status_t ckc_h_op_tile_inline_asm(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    if(!ckc_h_live(lw))
    {
        return lw->status;
...
static ckc_status_t ckc_h_op_tile_readfirstlane(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
...
static ckc_status_t ckc_h_op_tile_ds_bpermute_b64(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    const ckc_value_t* addr = op->operands[0];
    const ckc_value_t* data = op->operands[1];
    const ckc_value_t* r    = h_res(op);
    ...
    ckc_h_emitf(lw, "int %s_phi = __builtin_amdgcn_ds_bpermute(%s, %s_hi);", nice, an, nice);
    ckc_h_emitf(
...
static ckc_status_t ckc_h_op_tile_permlane32_swap(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
...
static ckc_status_t h_ds_read_tr16(ckc_h_lowerer_t* lw,
                                   const ckc_op_t* op,
                                   const char* op_id,
                                   const char* raw_ty,
                                   const char* shim,
...
static ckc_status_t ckc_h_op_tile_ds_read_tr16_b64(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    return h_ds_read_tr16(
        lw, op, "ds_read_tr16_b64", "i16x4_raw", "_llvm_amdgcn_ds_read_tr16_b64", 4, 8);
}

static ckc_status_t ckc_h_op_tile_ds_read_tr16_b128(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    return h_ds_read_tr16(
...
static ckc_status_t ckc_h_op_tile_ds_read_tr_b8(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    if(!ckc_h_live(lw))
    {
        return lw->status;
...
static ckc_status_t ckc_h_op_tile_sync_half_block(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
...
static ckc_status_t ckc_h_op_tile_sync_lds_only(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    int mask;
    (void)op;
...
static ckc_status_t ckc_h_op_tile_s_barrier_bare(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    (void)op;
...
static ckc_status_t ckc_h_op_tile_sched_group_barrier(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    int64_t m = 0, c = 0, g = 0;
    ckc_attr_get_int(&op->attrs, "mask", &m);
    ckc_attr_get_int(&op->attrs, "count", &c);
    ckc_attr_get_int(&op->attrs, "group", &g);
    ckc_h_emitf(lw, "__builtin_amdgcn_sched_group_barrier(%d, %d, %d);", (int)m, (int)c, (int)g);
    return lw->status;
}
...
static ckc_status_t ckc_h_op_vector_select(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        {
            ckc_h_emitf(lw, "%s[%d] = %s[%d] ? %s[%d] : %s[%d];", nice, i, mn, i, ln, i, rn, i);
        }
    }
    return lw->status;
}
...
static ckc_status_t ckc_h_op_vector_trunc_f32_to_f16(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    const ckc_value_t* v = op->operands[0];
    const ckc_value_t* r = h_res(op);
    const char* nice     = ckc_h_name(lw, r);
...
static ckc_status_t ckc_h_op_vector_trunc_f32_to(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    const ckc_value_t* v = op->operands[0];
    const ckc_value_t* r = h_res(op);
    const char* res_t    = ckc_h_type_to_hip(lw, r->type);
...
static ckc_status_t ckc_h_op_scf_for(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    ...
    if(!iv_ty)
    {
        ...
        return ckc_h_fail(
            lw, CKC_ERR_KEY, "scf.for: no HIP type for iv_type '%s'", iv_type ? iv_type : "<none>");
    }
    ...
    for(i = 0; i < meta_n && i < op->num_results; i++)
    {
        ...
        ckc_h_emitf(lw, "%s %s;", h_for_cpp_type(lw, mtype), ckc_h_name(lw, op->results[i]));
    }
...
static const ckc_op_t* h_find_enclosing_for(const ckc_region_t* region, const ckc_op_t* target)
{
    int i, r, k;
    if(!region)
    {
...
static ckc_status_t ckc_h_op_scf_yield(ckc_h_lowerer_t* lw, const ckc_op_t* op)
{
    ...
    {
        return ckc_h_fail(
            lw, CKC_ERR_VALUE, "scf.yield: %d values vs %d iter_args", op->num_operands, meta_n);
    }
    for(i = 0; i < meta_n; i++)
    {
        const char* mname = ckc_attr_get_str(items[i], "name");
