...
static void diag_push(diag_buf_t *db, ckc_diag_severity_t sev, const ckc_op_t *op,
                      const char *msg) {
    if (db->count >= db->cap) {
...
static int is_addr_space(const char *s) {
    return s && (strcmp(s, "global") == 0 || strcmp(s, "constant") == 0 ||
                 strcmp(s, "shared") == 0 || strcmp(s, "lds") == 0 ||
                 strcmp(s, "private") == 0);
}
...
static void check_type(verifier_t *v, const ckc_type_t *t, const char *where,
                       const ckc_op_t *op) {
    if (!t) {
        return;
    }
        ...
        for (int i = 0; i < t->rank; ++i) {
            if (t->shape[i] <= 0) {
                ...
                for (int j = 0; j < t->rank && o < sizeof(shp); ++j) {
                    o += (size_t)snprintf(shp + o, sizeof(shp) - o, "%s%d",
                                          j ? ", " : "", t->shape[j]);
                }
...
static void check_contract(verifier_t *v, const ckc_op_t *op) {
    ...
    if (is_binary_same_type(o)) {
        if (op->num_operands != 2 || op->num_results != 1) {
            v_errf(v, op, "%s: expected 2 operands / 1 result, got %d / %d", name,
                   op->num_operands, op->num_results);
            return;
        }
        ...
        if (op->num_operands != 1 || op->num_results != 1) {
            v_errf(v, op, "%s: expected 1 operand / 1 result, got %d / %d", name,
                   op->num_operands, op->num_results);
            return;
        }
        if (!ckc_type_eq(op->operands[0]->type, op->results[0]->type)) {
            v_errf(v, op, "%s: result type %s != operand type %s", name,
                   tn(op->results[0]->type), tn(op->operands[0]->type));
        }
    } else if (is_cmp(o)) {
        if (op->num_operands != 2 || op->num_results != 1) {
        ...
        const ckc_type_t *b = op->operands[1]->type;
        if (!ckc_type_eq(a, b)) {
            v_errf(v, op, "%s: comparison operand types differ (%s vs %s)", name, tn(a),
                   tn(b));
        }
        ...
        if (!vt || vt->kind != CKC_TYPE_VECTOR) {
            ...
            if (!ckc_type_eq(op->results[0]->type, vt->elem)) {
                v_errf(v, op, "%s: result type %s != vector element %s", name,
                       tn(op->results[0]->type), tn(vt->elem));
            }
            int64_t idx;
            if (ckc_attr_get_int(&op->attrs, "index", &idx)) {
                if (!(idx >= 0 && idx < vt->count)) {
                    v_errf(v, op, "%s: extract index %lld out of range [0,%d)", name,
                           (long long)idx, vt->count);
                }
            }
        }
        ...
        if (op->num_operands < 3) {
            v_errf(v, op, "%s: expected at least 3 operands (a, b, c), got %d", name,
                   op->num_operands);
        }
...
static void check_scf_for(verifier_t *v, const ckc_op_t *op) {
    ...
    if (have_iter_meta) {
        if (iter_meta_count != num_iter_inits) {
            v_errf(v, op, "scf.for: %d iter_args declared but %d init operands",
                   iter_meta_count, num_iter_inits);
        }
        if (op->num_results != num_iter_inits) {
            v_errf(v, op, "scf.for: %d results but %d iter-args", op->num_results,
                   num_iter_inits);
        }
        ...
        for (int i = 0; i < lim; ++i) {
            ...
            if (!ckc_type_eq(res, init)) {
                v_errf(v, op, "scf.for: result type %s != iter init type %s", tn(res),
                       tn(init));
            }
        }
    }
        ...
        if (body->num_ops == 0 || body->ops[body->num_ops - 1]->opcode != CKC_OP_SCF_YIELD) {
            ...
            if (yld->num_operands != num_iter_inits) {
                v_errf(v, yld,
                       ...
                       yld->num_operands, num_iter_inits);
            } else {
                for (int i = 0; i < num_iter_inits; ++i) {
                    if (!ckc_type_eq(yld->operands[i]->type, op->operands[3 + i]->type)) {
                        v_errf(v, yld, "scf.yield type %s != iter-arg type %s",
                               tn(yld->operands[i]->type), tn(op->operands[3 + i]->type));
                    }
                }
            }
        }
...
static void check_op(verifier_t *v, const ckc_op_t *op) {
    ...
    for (int i = 0; i < op->num_operands; ++i) {
        ...
        if (!def) {
            ...
            v_errf(v, op, "operand '%s' type %s does not match its definition type %s",
                   o->name, tn(o->type), tn(def));
        }
    }
