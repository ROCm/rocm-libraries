...
{
    return s.constants_folded == 0 && s.common_subexpressions == 0 &&
           s.dead_ops_removed == 0;
}
...
static void repl_set(repl_map_t *m, const char *name, ckc_value_t *value)
{
    ...
    if (m->count == m->cap) {
        ...
        repl_entry_t *ne =
            (repl_entry_t *)ckc_arena_alloc(&m->b->arena, (size_t)nc * sizeof(*ne));
        if (!ne) return;  /* OOM: silently keep old map; arena sets no err here */
...
static void rewrite_region_operands(ckc_ir_builder_t *b, ckc_region_t *region,
                                    const repl_map_t *repl);

/* Python _rewrite_operands: op.operands = [repl.get(v.name, v) ...]; recurse. */
static void rewrite_operands(ckc_ir_builder_t *b, ckc_op_t *op,
                             const repl_map_t *repl)
{
    int i;
...
static void use_add(use_map_t *m, const char *name, int delta)
{
    ...
    if (m->count == m->cap) {
        ...
        use_entry_t *ne =
            (use_entry_t *)ckc_arena_alloc(&m->b->arena, (size_t)nc * sizeof(*ne));
        if (!ne) return;
        if (m->entries && m->count)
            memcpy(ne, m->entries, (size_t)m->count * sizeof(*ne));
        m->entries = ne;
...
static void count_uses_region(ckc_region_t *region, use_map_t *uses)
{
    ...
    for (i = 0; i < region->num_ops; i++) {
        ...
        for (j = 0; j < op->num_operands; j++) {
            ...
            if (operand) use_add(uses, operand->name, 1);
        }
...
static bool attr_value_eq(const ckc_attr_value_t *x, const ckc_attr_value_t *y)
{
    ...
    case CKC_ATTR_STR:
        if (x->u.s == y->u.s) return true;
        if (!x->u.s || !y->u.s) return false;
        return strcmp(x->u.s, y->u.s) == 0;
    case CKC_ATTR_BOOL:
        return x->u.b == y->u.b;
    case CKC_ATTR_LIST: {
        int i;
        if (x->u.list.count != y->u.list.count) return false;
...
static int attr_count_no_loc(const ckc_attr_map_t *m)
{
    int i, n = 0;
    for (i = 0; i < m->count; i++) {
        if (strcmp(m->entries[i].key, "loc") != 0) n++;
    }
    return n;
}
...
{
    int i, j;
    if (attr_count_no_loc(a) != attr_count_no_loc(b)) return false;
    for (i = 0; i < a->count; i++) {
        const char *key = a->entries[i].key;
        bool found = false;
        if (strcmp(key, "loc") == 0) continue;
        for (j = 0; j < b->count; j++) {
...
static void cse_insert(cse_table_t *t, ckc_op_t *op, ckc_value_t *result)
{
    if (t->count == t->cap) {
        ...
        cse_entry_t *ne =
            (cse_entry_t *)ckc_arena_alloc(&t->b->arena, (size_t)nc * sizeof(*ne));
        if (!ne) return;
...
{
    if (t && t->name) {
        if (strcmp(t->name, "i1") == 0) return "i1";
        if (strcmp(t->name, "i64") == 0) return "i64";
    }
    return "i32";
}
...
static bool const_int(const ckc_value_t *v, int64_t *out)
{
    ...
    if (!v) return false;
    op = v->op;
    if (op == NULL || op->opcode != CKC_OP_ARITH_CONSTANT) return false;
    ity = ckc_attr_get_str(&op->attrs, "ity");
    if (ity == NULL) ity = "i32";  /* op.attrs.get("ity", "i32") */
    if (strcmp(ity, "i1") != 0 && strcmp(ity, "i32") != 0 &&
        strcmp(ity, "i64") != 0)
        return false;
...
static bool try_fold(const ckc_op_t *op, int64_t *out)
{
    ...
    if (op->num_operands < 1 || op->num_operands > 3) return false;
    /* Python: ints = [_const_int(v) for v in op.operands]; if any None -> skip */
    for (i = 0; i < op->num_operands; i++) {
        if (!const_int(op->operands[i], &ints[i])) return false;
    }

    switch (op->opcode) {
    case CKC_OP_ARITH_ADD:
        if (op->num_operands != 2) return false;
        *out = ints[0] + ints[1];
        return true;
    ...
    case CKC_OP_ARITH_DIV:
        if (op->num_operands != 2 || ints[1] == 0) return false;
    ...
    case CKC_OP_ARITH_CMP: {
        ...
        if (pred == NULL) pred = "lt";  /* op.attrs.get("pred", "lt") */
        a = ints[0];
        bc = ints[1];
        if (strcmp(pred, "lt") == 0)      *out = (a < bc);
        else if (strcmp(pred, "le") == 0) *out = (a <= bc);
        else if (strcmp(pred, "gt") == 0) *out = (a > bc);
        else if (strcmp(pred, "ge") == 0) *out = (a >= bc);
        else if (strcmp(pred, "eq") == 0) *out = (a == bc);
        else if (strcmp(pred, "ne") == 0) *out = (a != bc);
        else return false;  /* Python would KeyError; conservatively skip fold */
        return true;
    }
    case CKC_OP_ARITH_SELECT:
        if (op->num_operands != 3) return false;
...
ckc_pass_stats_t ckc_canonicalize_region(ckc_ir_builder_t *b,
                                         ckc_region_t *region)
{
    ...
    for (i = 0; i < region->num_ops; i++) {
        ckc_op_t *op = region->ops[i];
        for (j = 0; j < op->num_regions; j++) {
            ckc_pass_stats_t nested =
                ckc_canonicalize_region(b, op->regions[j]);
...
ckc_pass_stats_t ckc_optimize_kernel(ckc_ir_builder_t *b,
                                     ...
                                     int max_iter)
{
    ...
    if (max_iter <= 0) max_iter = 3;  /* Python default max_iter=3 */
    if (!kernel || !kernel->body) return total;

    for (iter = 0; iter < max_iter; iter++) {
        ckc_pass_stats_t stats = ckc_canonicalize_region(b, kernel->body);
        total = ckc_pass_stats_add(total, stats);
        if (ckc_pass_stats_is_zero(stats)) break;
    }
    return total;
}
