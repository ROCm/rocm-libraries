...
ckc_value_t* ckc_b_readfirstlane(ckc_ir_builder_t* b, ckc_value_t* v)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "readfirstlane: NULL operand");
...
ckc_value_t* ckc_b_pin_sgpr(ckc_ir_builder_t* b, ckc_value_t* v)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "pin_sgpr: NULL operand");
...
ckc_value_t* ckc_b_wave_all(ckc_ir_builder_t* b, ckc_value_t* predicate)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!predicate)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "wave_all: NULL predicate");
...
ckc_value_t* ckc_b_wave_any(ckc_ir_builder_t* b, ckc_value_t* predicate)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!predicate)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "wave_any: NULL predicate");
...
ckc_value_t* ckc_b_wave_ballot(ckc_ir_builder_t* b, ckc_value_t* predicate)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!predicate)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "wave_ballot: NULL predicate");
...
ckc_value_t* ckc_b_ds_bpermute(ckc_ir_builder_t* b, ckc_value_t* addr, ckc_value_t* data)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!addr || !data)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "ds_bpermute: NULL operand");
    if(!ckc_flow_is_i32(addr->type) || !ckc_flow_is_i32(data->type))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "ds_bpermute requires i32 addr + i32 data");
    ops[0] = addr;
    ops[1] = data;
...
ckc_value_t* ckc_b_ds_bpermute_b64(ckc_ir_builder_t* b, ckc_value_t* addr, ckc_value_t* data)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!addr || !data)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "ds_bpermute_b64: NULL operand");
...
ckc_value_t* ckc_b_ds_swizzle_xor(ckc_ir_builder_t* b, ckc_value_t* data, int xor_mask)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!data)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "ds_swizzle_xor: NULL data");
...
ckc_b_mov_dpp(ckc_ir_builder_t* b, ckc_value_t* data, int row_shr, int row_shl, bool bound_ctrl)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!data)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "mov_dpp: NULL data");
    has_shr = (row_shr >= 0);
    has_shl = (row_shl >= 0);
    if(has_shr == has_shl)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "mov_dpp requires exactly one of row_shr / row_shl to be set");
                           ...
                           ckc_value_t** out_hi)
{
    ...
    if(!lo || !hi)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "permlane32_swap: NULL operand");
        return;
    }
...
ckc_b_perm_b32(ckc_ir_builder_t* b, ckc_value_t* src0, ckc_value_t* src1, ckc_value_t* sel)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!src0 || !src1 || !sel)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "perm_b32: NULL operand");
    if(!ckc_flow_is_i32(src0->type) || !ckc_flow_is_i32(src1->type) || !ckc_flow_is_i32(sel->type))
...
ckc_value_t* ckc_b_permlanex16(ckc_ir_builder_t* b, ckc_value_t* v)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "permlanex16: NULL operand");
...
ckc_value_t* ckc_b_byte_perm(ckc_ir_builder_t* b, ckc_value_t* a, ckc_value_t* bb, int64_t sel)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!a || !bb)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "byte_perm: NULL operand");
...
ckc_value_t* ckc_b_warp_shuffle_xor(ckc_ir_builder_t* b, ckc_value_t* v, int lane_xor)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "warp_shuffle_xor: NULL operand");
    ...
    if(lane_xor >= 1 && lane_xor <= 31)
    {
        ...
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "warp_shuffle_xor: unsupported type %s", v->type->name);
    }
...
static ckc_value_t* ckc_flow_ds_read_tr(ckc_ir_builder_t* b,
                                        ckc_opcode_t opcode,
                                        ckc_value_t* smem,
                                    ...
                                    const ckc_type_t* dtype)
{
    ...
    if(num_indices <= 0)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "ds_read_tr16_b64 needs at least one index");
    if(!dtype)
        dtype = ckc_f16();
    return ckc_flow_ds_read_tr(
        b, CKC_OP_TILE_DS_READ_TR16_B64, smem, indices, num_indices, dtype, 4, "tr16", true);
}
                                     ...
                                     const ckc_type_t* dtype)
{
    ...
    if(num_indices <= 0)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "ds_read_tr16_b128 needs at least one index");
    if(!dtype)
        dtype = ckc_f16();
    return ckc_flow_ds_read_tr(
        b, CKC_OP_TILE_DS_READ_TR16_B128, smem, indices, num_indices, dtype, 8, "tr16w", true);
}
                                 ...
                                 const ckc_type_t* dtype)
{
    ...
    if(num_indices <= 0)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "ds_read_tr_b8 needs at least one index");
    return ckc_flow_ds_read_tr(
        b, CKC_OP_TILE_DS_READ_TR_B8, smem, indices, num_indices, dtype, 8, "tr8", false);
}
...
ckc_value_t* ckc_b_smem_addr_of(ckc_ir_builder_t* b, ckc_value_t* smem)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!smem)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "smem_addr_of: NULL smem");
                                 ...
                                 int coherency)
{
    ...
    {
        ckc_i_set_err(
            b, CKC_ERR_VALUE, "async_buffer_load_lds dwords must be 1, 3, or 4 (got %d)", dwords);
        return;
    }
    if(!(coherency >= 0 && coherency <= 3))
    {
                               ...
                               int dwords)
{
    ...
    if(!(dwords == 1 || dwords == 2 || dwords == 4))
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "buffer_store dwords must be 1, 2, or 4 (got %d)", dwords);
        return;
    }
    attrs = ckc_i_attrs(b);
    ckc_attr_set_int(b, &attrs, "dwords", (int64_t)dwords);
...
void ckc_b_sync_half_block(ckc_ir_builder_t* b, ckc_value_t* half_selector)
{
    ...
    if(!half_selector)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "sync_half_block: NULL selector");
        return;
    }
...
void ckc_b_s_setprio(ckc_ir_builder_t* b, int level)
{
    ...
    if(level < 0 || level > 3)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "s_setprio level must be in 0..3");
        return;
    }
    attrs = ckc_i_attrs(b);
...
ckc_for_t ckc_b_scf_for(
    ckc_ir_builder_t* b, ckc_value_t* lo, ckc_value_t* hi, ckc_value_t* step, const char* iv_name)
{
    ckc_for_t out;
    ckc_value_t* ops[3];
    ckc_region_t* body;
    ...
    if(!lo || !hi || !step)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "scf_for: NULL bound/step");
        return out;
    }
                             ...
                             bool elide_trailing_barrier)
{
    ...
    if(!lo || !hi || !step)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "scf_for_iter: NULL bound/step");
        return out;
    }
    if(num_iter_args < 0)
        num_iter_args = 0;
    ...
    /* Assemble iter_args metadata + iter_inits + iter_vars + rtys. */
    {
        ...
        for(i = 0; i < num_iter_args; ++i)
        {
            ...
            if(!init)
            {
                ckc_i_set_err(b, CKC_ERR_VALUE, "scf_for_iter: NULL iter init");
                return out;
            }
...
void ckc_b_scf_yield(ckc_ir_builder_t* b, ckc_value_t* const* values, int num_values)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "scf_yield: NULL values");
        return;
    }
    attrs = ckc_i_attrs(b);
...
ckc_if_t ckc_b_scf_if(ckc_ir_builder_t* b, ckc_value_t* cond)
{
    ...
    if(!cond)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "scf_if: NULL cond");
        return out;
    }
