                                      ...
                                      const ckc_type_t *dtype, int align)
{
    ...
    if (!ptr || !idx || !mask || !other || !dtype)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "masked_global_load: null operand/dtype");
...
ckc_value_t *ckc_b_zero_vec_f16(ckc_ir_builder_t *b, int n)
{
    if (!ckc_i_live(b)) return NULL;
    if (n <= 0)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "zero_vec_f16 needs positive n, got %d", n);
                                  ...
                                  ckc_value_t *value, const char *ordering)
{
    ...
    if (!ops) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_OOM, "lds_atomic_add: OOM");
    ops[0] = smem;
                                             ...
                                             const char *ordering)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!ptr || !idx || !value)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "global_atomic_add_pk_bf16: null operand");
    if (ordering == NULL) ordering = "monotonic";
    if (!ckc_i_ordering_ok(ordering))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "unknown ordering '%s'", ordering);
                                        ...
                                        const char *hint)
{
    ...
    if (!ckc_i_is_vector(a->type, NULL, -1) || !ckc_type_eq(a->type, c->type))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_binary expects matching vector operands");
    ops[0] = a; ops[1] = c;
...
ckc_value_t *ckc_b_vector_sum(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *ops[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_sum: null operand");
...
ckc_value_t *ckc_b_vector_reduce_max(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *ops[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_reduce_max: null operand");
    if (!ckc_i_is_vector(v->type, NULL, -1))
...
ckc_value_t *ckc_b_vector_splat(ckc_ir_builder_t *b, ckc_value_t *scalar, int n)
{
    ...
    const ckc_type_t *vt;
    if (!ckc_i_live(b)) return NULL;
    if (!scalar) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_splat: null operand");
                              ...
                              ckc_value_t *a, ckc_value_t *c)
{
    ...
    if (!ckc_i_is_vector(a->type, NULL, -1) || !ckc_type_eq(a->type, c->type))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_cmp expects matching vector operands");
...
ckc_value_t *ckc_b_vector_trunc(ckc_ir_builder_t *b, ckc_value_t *v, const ckc_type_t *target)
{
    ...
    char hint[16];
    if (!ckc_i_live(b)) return NULL;
    if (!v || !target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_trunc: null operand/target");
    if (!ckc_i_is_vector(v->type, NULL, -1))
...
ckc_value_t *ckc_b_vector_sext(ckc_ir_builder_t *b, ckc_value_t *v, const ckc_type_t *target)
{
    ...
    char hint[16];
    if (!ckc_i_live(b)) return NULL;
    if (!v || !target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_sext: null operand/target");
    if (!ckc_i_is_vector(v->type, NULL, -1))
...
ckc_value_t *ckc_b_vec_extract(ckc_ir_builder_t *b, ckc_value_t *v, int i)
{
    ...
    const ckc_type_t *et;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_extract: null operand");
...
ckc_value_t *ckc_b_vec_insert(ckc_ir_builder_t *b, ckc_value_t *v, ckc_value_t *scalar, int i)
{
    ...
    ckc_attr_map_t a;
    if (!ckc_i_live(b)) return NULL;
    if (!v || !scalar) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_insert: null operand");
    if (!ckc_i_is_vector(v->type, NULL, -1))
                            ...
                            int num_components, const ckc_type_t *elem)
{
    ...
    int i;
    if (!ckc_i_live(b)) return NULL;
    if (!elem) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_pack: null elem");
    if (num_components <= 0 || !components)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_pack needs at least one component");
    for (i = 0; i < num_components; ++i) {
        if (!components[i] || !ckc_type_eq(components[i]->type, elem))
            return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_pack expected %s, got %s",
                                 elem->name,
...
ckc_value_t *ckc_b_vec_concat(ckc_ir_builder_t *b, ckc_value_t *a, ckc_value_t *bb)
{
    ...
    int n;
    if (!ckc_i_live(b)) return NULL;
    if (!a || !bb) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_concat: null operand");
...
ckc_value_t *ckc_b_vec_bitcast(ckc_ir_builder_t *b, ckc_value_t *v, const ckc_type_t *target)
{
    ...
    ckc_attr_map_t a;
    if (!ckc_i_live(b)) return NULL;
    if (!v || !target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_bitcast: null operand/target");
    ops[0] = v;
    a = ckc_i_attrs(b);
    ckc_attr_set_str(b, &a, "target", target->name);
...
ckc_value_t *ckc_b_vec_cast_f32_to(ckc_ir_builder_t *b, ckc_value_t *v, const ckc_type_t *target)
{
    ...
    char hint[16];
    if (!ckc_i_live(b)) return NULL;
    if (!v || !target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_cast_f32_to: null operand/target");
    ...
    if (!ckc_i_type_is(target, "f16") && !ckc_i_type_is(target, "bf16"))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vec_cast_f32_to unsupported target %s",
                             target->name);
    vt = ckc_vector_type(b, target, ckc_i_count_of(v->type));
    if (!vt) return NULL;
    ops[0] = v;
