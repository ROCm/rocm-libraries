...
static ckc_value_t* zero_vec_impl(
    ckc_ir_builder_t* b, const ckc_type_t* elem, const char* elem_name, int n, const char* hint)
{
    ckc_attr_map_t a;
    const ckc_type_t* vt;
...
static const char* cz_hint(ckc_ir_builder_t* b, int n)
{
    ...
    if(!out)
        return (const char*)ckc_i_set_err(b, CKC_ERR_OOM, "OOM cz hint");
    return out;
}
...
ckc_value_t* ckc_b_zero_vec_f32(ckc_ir_builder_t* b, int n)
{
    ...
    if(n <= 0)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "zero_vec_f32 needs positive n, got %d", n);
    hint = cz_hint(b, n);
    if(!hint)
        return NULL;
...
ckc_value_t* ckc_b_zero_vec(ckc_ir_builder_t* b, const ckc_type_t* elem, int n)
{
    ...
    return (ckc_value_t*)ckc_i_set_err(
        b, CKC_ERR_VALUE, "zero_vec unsupported elem %s", elem->name);
}
...
ckc_value_t* ckc_b_umul_hi_i32(ckc_ir_builder_t* b, ckc_value_t* a, ckc_value_t* c)
{
    ...
    operands[0] = a;
    operands[1] = c;
    return ckc_i_op1(b, CKC_OP_ARITH_UMUL_HI_I32, operands, 2, ckc_i32(), NULL, "umh");
}
...
ckc_value_t* ckc_b_clamp_f32(ckc_ir_builder_t* b, ckc_value_t* v, ckc_value_t* lo, ckc_value_t* hi)
{
    ...
    if(!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "clamp_f32 expects f32 input, got %s", v->type->name);
...
ckc_value_t* ckc_b_fcmp(ckc_ir_builder_t* b, const char* pred, ckc_value_t* a, ckc_value_t* c)
{
    ...
    if(!ok)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "unsupported fcmp predicate '%s'", pred);
    if(!a || !c)
...
ckc_value_t* ckc_b_zext(ckc_ir_builder_t* b, ckc_value_t* v, const ckc_type_t* target)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "zext NULL value");
    if(!target)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "zext NULL target");
    operands[0] = v;
...
ckc_value_t* ckc_b_sext(ckc_ir_builder_t* b, ckc_value_t* v, const ckc_type_t* target)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "sext NULL value");
    if(!target)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "sext NULL target");
    operands[0] = v;
...
ckc_value_t* ckc_b_trunc(ckc_ir_builder_t* b, ckc_value_t* v, const ckc_type_t* target)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "trunc NULL value");
    if(!target)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "trunc NULL target");
    operands[0] = v;
...
ckc_value_t* ckc_b_bitcast(ckc_ir_builder_t* b, ckc_value_t* v, const ckc_type_t* target)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "bitcast NULL value");
    if(!target)
...
ckc_value_t* ckc_b_trunc_f32_to_f16(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "trunc_f32_to_f16 NULL value");
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_TRUNC_F32_TO_F16, operands, 1, ckc_f16(), NULL, "t");
}
...
ckc_value_t* ckc_b_rint_f32(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "rint_f32 NULL value");
    if(!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "rint_f32 expects f32 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_RINT_F32, operands, 1, ckc_f32(), NULL, "rint");
}
...
ckc_value_t* ckc_b_cast_to_f32(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_to_f32 NULL value");
    if(ckc_i_type_is(v->type, "f32"))
        return v;
    if(!ckc_i_type_is(v->type, "f16") && !ckc_i_type_is(v->type, "bf16"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cast_to_f32 unsupported from %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CAST_TO_F32, operands, 1, ckc_f32(), NULL, "f32");
}
...
ckc_value_t* ckc_b_cast_f32_to(ckc_ir_builder_t* b, ckc_value_t* v, const ckc_type_t* target)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_f32_to NULL value");
    if(!target)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_f32_to NULL target");
    if(!ckc_i_type_is(v->type, "f32"))
...
ckc_value_t* ckc_b_sitofp_f32(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "sitofp_f32 NULL value");
    if(!ckc_i_type_is(v->type, "i32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "sitofp_f32 expects i32 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_SITOFP_F32, operands, 1, ckc_f32(), NULL, "sitof");
}
...
ckc_value_t* ckc_b_cvt_fp8_to_f32(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_fp8_to_f32 NULL value");
    if(!ckc_i_type_is(v->type, "fp8e4m3"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_fp8_to_f32 expects fp8e4m3 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_FP8_TO_F32, operands, 1, ckc_f32(), NULL, "dq8");
}
...
ckc_value_t* ckc_b_cvt_bf8_to_f32(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_bf8_to_f32 NULL value");
    if(!ckc_i_type_is(v->type, "bf8e5m2"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_bf8_to_f32 expects bf8e5m2 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_BF8_TO_F32, operands, 1, ckc_f32(), NULL, "dqb8");
}
...
ckc_value_t* ckc_b_cvt_pk_f32_fp8x4(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_f32_fp8x4 NULL value");
    ...
    if(!vt)
        return NULL;
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_PK_F32_FP8X4, operands, 1, vt, NULL, "dq8x4");
}
...
ckc_value_t* ckc_b_cvt_pk_f32_bf8x4(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_f32_bf8x4 NULL value");
    ...
    if(!vt)
        return NULL;
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_PK_F32_BF8X4, operands, 1, vt, NULL, "dqb8x4");
}
...
ckc_b_cvt_scalef32_pk_f32_fp8x4(ckc_ir_builder_t* b, ckc_value_t* v, ckc_value_t* scale)
{
    ...
    if(!v || !scale)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_scalef32_pk_f32_fp8x4 NULL operand");
    if(!ckc_i_is_vector(v->type, "fp8e4m3", 4))
        return (ckc_value_t*)ckc_i_set_err(
            b,
            CKC_ERR_VALUE,
    ...
    operands[0] = v;
    operands[1] = scale;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_SCALEF32_PK_F32_FP8, operands, 2, vt, NULL, "sdq8x4");
}
...
ckc_b_cvt_scalef32_pk_f32_bf8x4(ckc_ir_builder_t* b, ckc_value_t* v, ckc_value_t* scale)
{
    ...
    if(!v || !scale)
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_scalef32_pk_f32_bf8x4 NULL operand");
    if(!ckc_i_is_vector(v->type, "bf8e5m2", 4))
        return (ckc_value_t*)ckc_i_set_err(
            b,
            CKC_ERR_VALUE,
    ...
    if(!ckc_i_type_is(scale->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_scalef32_pk_f32_bf8x4 scale must be f32");
    vt = ckc_vector_type(b, ckc_f32(), 4);
    if(!vt)
        return NULL;
    operands[0] = v;
    operands[1] = scale;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_SCALEF32_PK_F32_BF8, operands, 2, vt, NULL, "sdqb8x4");
}
...
ckc_value_t* ckc_b_cvt_f32_to_fp8(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_f32_to_fp8 NULL value");
    if(!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_f32_to_fp8 expects f32 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_F32_TO_FP8, operands, 1, ckc_fp8e4m3(), NULL, "q8");
}
...
ckc_value_t* ckc_b_cvt_f32_to_bf8(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_f32_to_bf8 NULL value");
    if(!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_f32_to_bf8 expects f32 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_F32_TO_BF8, operands, 1, ckc_bf8e5m2(), NULL, "qb8");
}
...
ckc_value_t* ckc_b_cvt_f32_to_i8_sat(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_f32_to_i8_sat NULL value");
    if(!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_f32_to_i8_sat expects f32 input, got %s", v->type->name);
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_F32_TO_I8_SAT, operands, 1, ckc_i8(), NULL, "qi8");
}
...
ckc_value_t* ckc_b_cvt_pk_fp8_f32x4(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_fp8_f32x4 NULL value");
    if(!ckc_i_is_vector(v->type, "f32", 4))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_pk_fp8_f32x4 expects vec<f32x4> input, got %s", v->type->name);
    vt = ckc_vector_type(b, ckc_fp8e4m3(), 4);
    if(!vt)
        return NULL;
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_PK_FP8_F32X4, operands, 1, vt, NULL, "q8x4");
}
...
ckc_value_t* ckc_b_cvt_pk_bf8_f32x4(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_bf8_f32x4 NULL value");
    if(!ckc_i_is_vector(v->type, "f32", 4))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_pk_bf8_f32x4 expects vec<f32x4> input, got %s", v->type->name);
    vt = ckc_vector_type(b, ckc_bf8e5m2(), 4);
    if(!vt)
        return NULL;
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_PK_BF8_F32X4, operands, 1, vt, NULL, "qb8x4");
}
...
ckc_value_t* ckc_b_cvt_pk_i8_f32x4(ckc_ir_builder_t* b, ckc_value_t* v)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_i8_f32x4 NULL value");
    if(!ckc_i_is_vector(v->type, "f32", 4))
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "cvt_pk_i8_f32x4 expects vec<f32x4> input, got %s", v->type->name);
    vt = ckc_vector_type(b, ckc_i8(), 4);
    if(!vt)
        return NULL;
    operands[0] = v;
    return ckc_i_op1(b, CKC_OP_ARITH_CVT_PK_I8_F32X4, operands, 1, vt, NULL, "qi8x4");
}
