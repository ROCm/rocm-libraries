...
#define CKC_SCALAR_SINGLETON(fn, kindenum, cname)                   \
    ...
    {                                                               \
        static const ckc_type_t t = {/* kind    */ CKC_TYPE_SCALAR, \
                                     /* name    */ cname,           \
                                     /* scalar  */ kindenum,        \
                                     /* elem    */ NULL,            \
                                     /* count   */ 0,               \
...
const ckc_type_t* ckc_vector_type(ckc_ir_builder_t* b, const ckc_type_t* elem, int count)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!elem)
        return (const ckc_type_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "vector_type: NULL element type");
...
const ckc_type_t* ckc_ptr_type(ckc_ir_builder_t* b, const ckc_type_t* pointee, const char* space)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!pointee)
        return (const ckc_type_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "ptr_type: NULL pointee type");
    if(!space)
        space = "global";
    t = (ckc_type_t*)ckc_arena_calloc(&b->arena, sizeof(*t));
    if(!t)
...
ckc_smem_type(ckc_ir_builder_t* b, const ckc_type_t* elem, const int* shape, int rank)
{
    ...
    if(!ckc_i_live(b))
        return NULL;
    if(!elem)
        return (const ckc_type_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "smem_type: NULL element type");
    if(rank < 0)
        rank = 0;

    t = (ckc_type_t*)ckc_arena_calloc(&b->arena, sizeof(*t));
    if(!t)
        return (const ckc_type_t*)ckc_i_set_err(b, CKC_ERR_OOM, "smem_type: OOM");
...
static ckc_attr_entry_t* ckc_attr_reserve(ckc_ir_builder_t* b, ckc_attr_map_t* m, const char* key)
{
    ckc_attr_entry_t* e;
...
ckc_value_t* ckc_op_result(ckc_ir_builder_t* b, ckc_op_t* op)
{
    if(!ckc_i_live(b))
        return NULL;
    if(!op)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "op.result: NULL op");
    if(op->num_results != 1)
