...
ckc_value_t *ckc_b_zero_vec(ckc_ir_builder_t *b, const ckc_type_t *elem, int n)
{
    ...
    return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "zero_vec unsupported elem %s",
                         elem->name);
}
...
ckc_value_t *ckc_b_umul_hi_i32(ckc_ir_builder_t *b, ckc_value_t *a, ckc_value_t *c)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!a || !c)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "umul_hi_i32 NULL operand");
                       ...
                       ckc_value_t *d)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!a || !c || !d)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "fma NULL operand");
                         ...
                         ckc_value_t *d)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!a || !c || !d)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "fmax3 NULL operand");
    if (!ckc_type_eq(a->type, c->type) || !ckc_type_eq(c->type, d->type))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
                         ...
                         ckc_value_t *d)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!a || !c || !d)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "fmin3 NULL operand");
    if (!ckc_type_eq(a->type, c->type) || !ckc_type_eq(c->type, d->type))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
                             ...
                             ckc_value_t *lo, ckc_value_t *hi)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!v)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "clamp_f32 NULL value");
                        ...
                        ckc_value_t *c)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!pred)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "fcmp predicate is NULL");
    ...
    if (!ok)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "unsupported fcmp predicate '%s'",
                             pred);
    if (!a || !c)
                        ...
                        const ckc_type_t *target)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "zext NULL value");
    if (!target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "zext NULL target");
    operands[0] = v;
                        ...
                        const ckc_type_t *target)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "sext NULL value");
    if (!target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "sext NULL target");
    operands[0] = v;
                         ...
                         const ckc_type_t *target)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "trunc NULL value");
    if (!target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "trunc NULL target");
    operands[0] = v;
                           ...
                           const ckc_type_t *target)
{
    ...
    ckc_attr_map_t at;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "bitcast NULL value");
    if (!target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "bitcast NULL target");
    operands[0] = v;
    at = ckc_i_attrs(b);
                          ...
                          ckc_value_t *lhs, ckc_value_t *rhs)
{
    ...
    if (!ckc_i_live(b)) return NULL;
    if (!cond || !lhs || !rhs)
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "select NULL operand");
    operands[0] = cond;
    operands[1] = lhs;
    operands[2] = rhs;
...
ckc_value_t *ckc_b_trunc_f32_to_f16(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "trunc_f32_to_f16 NULL value");
    operands[0] = v;
...
ckc_value_t *ckc_b_rint_f32(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "rint_f32 NULL value");
    if (!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
...
ckc_value_t *ckc_b_cast_to_f32(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_to_f32 NULL value");
    if (ckc_i_type_is(v->type, "f32"))
        return v;
                               ...
                               const ckc_type_t *target)
{
    ...
    ckc_attr_map_t at;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_f32_to NULL value");
    if (!target) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_f32_to NULL target");
    if (!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cast_f32_to expects f32 input");
...
ckc_value_t *ckc_b_sitofp_f32(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "sitofp_f32 NULL value");
...
ckc_value_t *ckc_b_cvt_fp8_to_f32(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_fp8_to_f32 NULL value");
...
ckc_value_t *ckc_b_cvt_bf8_to_f32(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_bf8_to_f32 NULL value");
...
ckc_value_t *ckc_b_cvt_pk_f32_fp8x4(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ...
    const ckc_type_t *vt;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_f32_fp8x4 NULL value");
...
ckc_value_t *ckc_b_cvt_pk_f32_bf8x4(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ...
    const ckc_type_t *vt;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_f32_bf8x4 NULL value");
...
ckc_value_t *ckc_b_cvt_f32_to_fp8(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_f32_to_fp8 NULL value");
    if (!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
...
ckc_value_t *ckc_b_cvt_f32_to_bf8(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_f32_to_bf8 NULL value");
    if (!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
...
ckc_value_t *ckc_b_cvt_f32_to_i8_sat(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ckc_value_t *operands[1];
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_f32_to_i8_sat NULL value");
    if (!ckc_i_type_is(v->type, "f32"))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
...
ckc_value_t *ckc_b_cvt_pk_fp8_f32x4(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ...
    const ckc_type_t *vt;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_fp8_f32x4 NULL value");
...
ckc_value_t *ckc_b_cvt_pk_bf8_f32x4(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ...
    const ckc_type_t *vt;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_bf8_f32x4 NULL value");
    if (!ckc_i_is_vector(v->type, "f32", 4))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
...
ckc_value_t *ckc_b_cvt_pk_i8_f32x4(ckc_ir_builder_t *b, ckc_value_t *v)
{
    ...
    const ckc_type_t *vt;
    if (!ckc_i_live(b)) return NULL;
    if (!v) return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE, "cvt_pk_i8_f32x4 NULL value");
    if (!ckc_i_is_vector(v->type, "f32", 4))
        return (ckc_value_t*)ckc_i_set_err(b, CKC_ERR_VALUE,
