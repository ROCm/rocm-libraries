...
void ckc_ll_emit_wave_ballot(ckc_lower_t* L, const ckc_value_t* pred, const char* result_name)
{
    ...
    ckc_ll_emitf(L, "  %s = call i64 @llvm.amdgcn.ballot.i64(i1 %s)", result_name, cmp_name);
}
...
static void _op_tile_readfirstlane(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    {
        ckc_ll_fail(L, CKC_ERR_KEY, "tile.readfirstlane: unsupported type '%s'", tyname);
    }
...
static void _op_tile_pin_sgpr(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    {
        ckc_ll_fail(L, CKC_ERR_KEY, "tile.pin_sgpr: unsupported type '%s'", tyname);
    }
    ckc_ll_emitf(L,
...
static void _op_tile_ds_bpermute_b64(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L, "  %s = call i32 @llvm.amdgcn.ds.bpermute(i32 %s, i32 %s)", plo, addr_s, lo32);
    ckc_ll_emitf(L, "  %s = call i32 @llvm.amdgcn.ds.bpermute(i32 %s, i32 %s)", phi, addr_s, hi32);
...
static void _op_tile_mov_dpp(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    {
        ckc_ll_fail(L, CKC_ERR_KEY, "tile.mov_dpp: missing 'row_shr'/'row_shl'");
    }
...
static void _op_tile_permlane32_swap(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L,
                 ...
                 lo_s,
                 hi_s);
    ckc_ll_emitf(L, "  %s = extractvalue { i32, i32 } %s, 0", op->results[0]->name, tmp);
    ckc_ll_emitf(L, "  %s = extractvalue { i32, i32 } %s, 1", op->results[1]->name, tmp);
}
...
static void ll_ds_read_tr16(ckc_lower_t* L,
                            const ckc_op_t* op,
...
static void _op_tile_ds_read_tr_b8(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L,
                 ...
                 raw,
                 addr);
    ckc_ll_emitf(L, "  %s = bitcast <2 x i32> %s to <8 x i8>", ll_result_name(op), raw);
}
