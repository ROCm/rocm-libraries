...
static int ll_elem_bytes(const char *name) {
    ...
    if (strcmp(name, "i8") == 0 || strcmp(name, "fp8e4m3") == 0 ||
        strcmp(name, "bf8e5m2") == 0) {
        return 1;
    }
    if (strcmp(name, "i16") == 0 || strcmp(name, "f16") == 0 ||
        strcmp(name, "bf16") == 0) {
        return 2;
    }
...
static const char *ll_attr_str(const ckc_op_t *op, const char *key,
                               const char *dflt) {
...
static const char *ll_smem_gidx(ckc_lower_t *L, const ckc_op_t *op,
                                int lo, int hi) {
    /* "i32 0" + ", i32 <op>" per index. Build incrementally into the arena. */
    const char *acc = "i32 0";
    int i;
    for (i = lo; i < hi; i++) {
        acc = ckc_arena_printf(&L->arena, "%s, i32 %s", acc,
...
static void op_memref_global_load(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L, "  %s = load half, ptr addrspace(1) %s, align %lld",
                 ll_res(op), gep, (long long)align);
}
...
static void op_memref_global_load_typed(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        gep, elem_ty, ckc_ll_operand(L, ptr), ckc_ll_operand(L, idx));
    ckc_ll_emitf(L, "  %s = load %s, ptr addrspace(1) %s, align %lld",
                 ll_res(op), elem_ty, gep, (long long)align);
}
...
static void op_memref_global_store(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        "  %s = getelementptr inbounds half, ptr addrspace(1) %s, i32 %s",
        gep, ckc_ll_operand(L, ptr), ckc_ll_operand(L, idx));
    ckc_ll_emitf(L, "  store half %s, ptr addrspace(1) %s, align %lld",
                 ckc_ll_operand(L, val), gep, (long long)align);
}
...
static void op_memref_global_store_typed(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        "  %s = getelementptr inbounds %s, ptr addrspace(1) %s, i32 %s",
        gep, elem_ty, ckc_ll_operand(L, ptr), ckc_ll_operand(L, idx));
    ckc_ll_emitf(L, "  store %s %s, ptr addrspace(1) %s, align %lld",
                 elem_ty, ckc_ll_operand(L, val), gep, (long long)align);
}
...
static void op_memref_global_atomic_add(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        ll_res(op), rmw_op, gep, elem_ty, ckc_ll_operand(L, val), ordering, md);
}
...
static void op_memref_global_load_vN(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        gep, elem_ty, ckc_ll_operand(L, ptr), idx_ty, ckc_ll_operand(L, idx));
    align = ll_attr_int(op, "align", vec * 2);
    ckc_ll_emitf(L,
        "  %s = load <%lld x %s>, ptr addrspace(1) %s, align %lld",
        ll_res(op), (long long)vec, elem_ty, gep, (long long)align);
}
...
static void op_memref_global_store_vN(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    int64_t vec = ll_attr_int(op, "vec", 0);
    const char *gep = ckc_ll_fresh(L, "gep");
    const char *elem_ty = ll_is_vec(val->type)
                              ? ckc_ll_llvm_type(L, val->type->elem)
                              : ckc_ll_llvm_type(L, val->type);
    const char *elem_name = ll_is_vec(val->type) ? val->type->elem->name
                                                 : val->type->name;
    ...
    ckc_ll_emitf(L,
        ...
        gep, elem_ty, ckc_ll_operand(L, ptr), ckc_ll_operand(L, idx));
    ckc_ll_emitf(L, "  store %s %s, ptr addrspace(1) %s, align %lld",
                 ty, ckc_ll_operand(L, val), gep, (long long)align);
}

static void op_memref_cooperative_global_store(ckc_lower_t *L,
                                               const ckc_op_t *op) {
    const ckc_value_t *ptr = op->operands[0];
    ...
    addr_elem_ty = ll_is_vec(addrs->type)
                       ? ckc_ll_llvm_type(L, addrs->type->elem)
                       : "i32";
    addrs_ty = ckc_ll_llvm_type(L, addrs->type);
    values_ty = ckc_ll_llvm_type(L, values->type);
    for (i = 0; i < n; i++) {
        const char *ai = ckc_ll_fresh(L, ckc_arena_printf(&L->arena, "coop_a%lld",
                                                          (long long)i));
        const char *vi = ckc_ll_fresh(L, ckc_arena_printf(&L->arena, "coop_v%lld",
                                                          (long long)i));
        const char *gep;
        ckc_ll_emitf(L, "  %s = extractelement %s %s, i32 %lld",
                     ai, addrs_ty, ckc_ll_operand(L, addrs), (long long)i);
        ckc_ll_emitf(L, "  %s = extractelement %s %s, i32 %lld",
                     vi, values_ty, ckc_ll_operand(L, values), (long long)i);
        gep = ckc_ll_fresh(L, ckc_arena_printf(&L->arena, "coop_gep%lld",
                                               (long long)i));
        ckc_ll_emitf(L,
            "  %s = getelementptr inbounds %s, ptr addrspace(1) %s, %s %s",
            gep, elem_ty, ckc_ll_operand(L, ptr), addr_elem_ty, ai);
        ckc_ll_emitf(L, "  store %s %s, ptr addrspace(1) %s, align 4",
                     elem_ty, vi, gep);
    }
}
...
static void op_tile_smem_store(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        gep, agg_ty, gname, gidx);
    ckc_ll_emitf(L, "  store %s %s, ptr addrspace(3) %s, align %d",
                 ckc_ll_llvm_type(L, value->type), ckc_ll_operand(L, value),
                 gep, align);
}
...
static void op_tile_lds_atomic_add(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    const char *rmw_op =
        (val->type->name && strcmp(val->type->name, "f32") == 0) ? "fadd"
                                                                 : "add";
    ckc_ll_emitf(L,
        "  %s = getelementptr inbounds %s, ptr addrspace(3) %s, %s",
        gep, agg_ty, gname, gidx);
    ckc_ll_emitf(L,
        ...
        ll_res(op), rmw_op, gep, elem_ty, ckc_ll_operand(L, val), ordering);
}
...
static void op_tile_smem_store_vN(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        (long long)vec, elem_ty, ckc_ll_operand(L, value), gep,
        (long long)align);
}
...
static void op_tile_smem_load_v4(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        "i32 0, i32 %s, i32 %s",
        base, agg_ty, gname, ckc_ll_operand(L, row), ckc_ll_operand(L, col));
    for (i = 0; i < 4; i++) {
        const char *ep = ckc_ll_fresh(L, "smem.ep");
        const char *ld;
        ckc_ll_emitf(L,
            "  %s = getelementptr inbounds half, ptr addrspace(3) %s, i32 %d",
            ep, base, i);
        ld = ckc_ll_fresh(L, "smem.ld");
        ckc_ll_emitf(L, "  %s = load half, ptr addrspace(3) %s, align 2",
                     ld, ep);
        elems[i] = ld;
    }
    prev = "undef";
    for (i = 0; i < 4; i++) {
        ...
        ckc_ll_emitf(L,
            "  %s = insertelement <4 x half> %s, half %s, i32 %d",
            tmp, prev, elems[i], i);
        prev = tmp;
    }
}
...
static void op_tile_smem_load_vN(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        "  %s = getelementptr inbounds %s, ptr addrspace(3) %s, %s",
        base, agg_ty, gname, idx_strs);
    if (vec == 1) {
        const char *scalar = ckc_ll_fresh(L, "smem.s");
        ckc_ll_emitf(L, "  %s = load %s, ptr addrspace(3) %s, align %lld",
                     scalar, elem_ty, base, (long long)align);
        ckc_ll_emitf(L,
            "  %s = insertelement <1 x %s> undef, %s %s, i32 0",
            ll_res(op), elem_ty, elem_ty, scalar);
    } else {
        ckc_ll_emitf(L,
            "  %s = load <%lld x %s>, ptr addrspace(3) %s, align %lld",
            ll_res(op), (long long)vec, elem_ty, base, (long long)align);
    }
}
...
static void op_tile_smem_store_distributed(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    const char *gname = ckc_ll_smem_global_name(L, smem, &stype);
    const char *agg_ty = ckc_ll_smem_storage_type(L, stype);
    const char *elem_ty = ll_is_vec(values->type)
                              ? ckc_ll_llvm_type(L, values->type->elem)
    ...
    for (i = 0; i < n; i++) {
        ...
        ckc_ll_emitf(L, "  %s = extractelement %s %s, i32 %d",
                     ev, values_ty, ckc_ll_operand(L, values), i);
        ckc_ll_emitf(L,
            "  %s = getelementptr inbounds %s, ptr addrspace(3) %s, i32 0, i32 %d",
            gep, agg_ty, gname, i);
        ckc_ll_emitf(L, "  store %s %s, ptr addrspace(3) %s, align 2",
                     elem_ty, ev, gep);
    }
}
...
static void op_tile_smem_store_vN_f32(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    if (vec == 1) {
        if (ll_is_vec(value->type)) {
            ...
            ckc_ll_emitf(L, "  %s = extractelement %s %s, i32 0",
                         ext, ckc_ll_llvm_type(L, value->type),
                         ckc_ll_operand(L, value));
            ckc_ll_emitf(L,
                "  store float %s, ptr addrspace(3) %s, align %lld",
                ext, gep, (long long)align);
        } else {
            ckc_ll_emitf(L,
                "  store float %s, ptr addrspace(3) %s, align %lld",
                ckc_ll_operand(L, value), gep, (long long)align);
        }
    } else {
        ckc_ll_emitf(L,
            "  store <%lld x float> %s, ptr addrspace(3) %s, align %lld",
            (long long)vec, ckc_ll_operand(L, value), gep, (long long)align);
    }
}
...
static void op_tile_smem_load_vN_f32(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    if (vec == 1) {
        const char *scalar = ckc_ll_fresh(L, "smem.s");
        ckc_ll_emitf(L, "  %s = load float, ptr addrspace(3) %s, align %lld",
                     scalar, base, (long long)align);
        ckc_ll_emitf(L,
            "  %s = insertelement <1 x float> undef, float %s, i32 0",
            ll_res(op), scalar);
    } else {
        ckc_ll_emitf(L,
            "  %s = load <%lld x float>, ptr addrspace(3) %s, align %lld",
            ll_res(op), (long long)vec, base, (long long)align);
    }
}
...
static void op_tile_smem_addr_of(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L, "  %s = ptrtoint ptr addrspace(3) %s to i64",
                 ll_res(op), gname);
}
...
static void op_tile_smem_ptr_add(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L, "  %s = add i64 %s, %s",
                 ll_res(op), ckc_ll_operand(L, base), ckc_ll_operand(L, off));
}
...
static void op_tile_global_ptr_add(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    if (strcmp(off_ty, "i32") == 0) {
        ...
        ckc_ll_emitf(L, "  %s = zext i32 %s to i64",
    ...
    ckc_ll_emitf(L,
        ...
        ll_res(op), ckc_ll_operand(L, ptr), off64);
}
...
static void op_tile_buffer_rsrc(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    if (L->flavor == CKC_LLVM_FLAVOR_LLVM22) {
        ...
        nb_text = ckc_arena_printf(&L->arena, "i32 %s",
                                   ckc_ll_operand(L, num_bytes));
    }
    word3 = L->backend->buffer_rsrc_word3;
    ckc_ll_emitf(L,
        "  %s = call ptr addrspace(8) @%s("
        "ptr addrspace(1) %s, i16 0, %s, i32 %d)",
        ll_res(op), intrinsic, ckc_ll_operand(L, ptr), nb_text, word3);
}
...
static void op_tile_buffer_load_vN_f16(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    if (dwords == 1) {
        ...
        ckc_ll_emitf(L,
            ...
            tmp, ckc_ll_operand(L, rsrc), ckc_ll_operand(L, voffset),
            ckc_ll_operand(L, soffset));
        ckc_ll_emitf(L, "  %s = bitcast i32 %s to <2 x half>",
                     ll_res(op), tmp);
    } else {
        const char *intr = ckc_arena_printf(&L->arena,
            "raw.ptr.buffer.load.v%lldi32", (long long)dwords);
        const char *tmp;
        int64_t halves = dwords * 2;
        ckc_ll_need(L, intr);
        tmp = ckc_ll_fresh(L, ckc_arena_printf(&L->arena, "blv%lld",
                                               (long long)dwords));
        ckc_ll_emitf(L,
            "  %s = call <%lld x i32> @llvm.amdgcn.raw.ptr.buffer.load.v%lldi32("
            "ptr addrspace(8) %s, i32 %s, i32 %s, i32 0)",
            tmp, (long long)dwords, (long long)dwords,
            ckc_ll_operand(L, rsrc), ckc_ll_operand(L, voffset),
            ckc_ll_operand(L, soffset));
        ckc_ll_emitf(L,
            "  %s = bitcast <%lld x i32> %s to <%lld x half>",
            ll_res(op), (long long)dwords, tmp, (long long)halves);
    }
}
...
static void op_tile_buffer_store_vN_f16(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    if (dwords == 1) {
        ...
        ckc_ll_emitf(L, "  %s = bitcast <2 x half> %s to i32",
                     bc, ckc_ll_operand(L, val));
        ckc_ll_emitf(L,
            "  call void @llvm.amdgcn.raw.ptr.buffer.store.i32("
            "i32 %s, ptr addrspace(8) %s, i32 %s, i32 %s, i32 0)",
            bc, ckc_ll_operand(L, rsrc), ckc_ll_operand(L, voffset),
            ckc_ll_operand(L, soffset));
    } else {
        const char *intr = ckc_arena_printf(&L->arena,
            "raw.ptr.buffer.store.v%lldi32", (long long)dwords);
        const char *bc;
        ckc_ll_need(L, intr);
        bc = ckc_ll_fresh(L, "bsbc");
        ckc_ll_emitf(L,
            "  %s = bitcast <%lld x half> %s to <%lld x i32>",
            bc, (long long)halves, ckc_ll_operand(L, val), (long long)dwords);
        ckc_ll_emitf(L,
            "  call void @llvm.amdgcn.raw.ptr.buffer.store.v%lldi32("
            "<%lld x i32> %s, ptr addrspace(8) %s, i32 %s, i32 %s, i32 0)",
            (long long)dwords, (long long)dwords, bc,
            ckc_ll_operand(L, rsrc), ckc_ll_operand(L, voffset),
            ckc_ll_operand(L, soffset));
    }
}
...
static void op_tile_async_buffer_load_lds_addr(ckc_lower_t *L,
                                               const ckc_op_t *op) {
    const ckc_value_t *rsrc = op->operands[0];
    ...
    ckc_ll_emitf(L, "  %s = inttoptr i64 %s to ptr addrspace(3)",
                 ptr_name, ckc_ll_operand(L, lds_addr));
    ckc_ll_emitf(L,
        "  call void @llvm.amdgcn.raw.ptr.buffer.load.lds("
        "ptr addrspace(8) %s, ptr addrspace(3) %s, i32 %lld, i32 %s, i32 %s, "
        "i32 0, i32 %lld)",
        ckc_ll_operand(L, rsrc), ptr_name, (long long)size_bytes,
        ckc_ll_operand(L, voff), ckc_ll_operand(L, soff), (long long)aux);
}
...
static void op_tile_async_buffer_load_lds(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        "ptr addrspace(8) %s, ptr addrspace(3) %s, i32 %lld, i32 %s, i32 %s, "
        "i32 0, i32 %lld)",
        ckc_ll_operand(L, rsrc), ckc_ll_operand(L, lds_ptr),
        (long long)bytes_per_lane, ckc_ll_operand(L, voffset),
        ckc_ll_operand(L, soffset), (long long)aux);
}
...
static void op_tile_global_load_lds(ckc_lower_t *L, const ckc_op_t *op) {
    ...
    ckc_ll_emitf(L,
        ...
        gep, ckc_ll_operand(L, src_ptr), ckc_ll_operand(L, byte_off));
    ptr_name = ckc_ll_fresh(L, "lds_ptr");
    ckc_ll_emitf(L, "  %s = inttoptr i64 %s to ptr addrspace(3)",
                 ptr_name, ckc_ll_operand(L, lds_addr));
    ckc_ll_emitf(L,
        ...
        gep, ptr_name, (long long)size_bytes, (long long)aux);
}
...
void ckc_ll_register_mem(void) {
    ...
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_LOAD_TYPED,
                       op_memref_global_load_typed);
    ...
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_STORE_TYPED,
                       op_memref_global_store_typed);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_STORE_VN, op_memref_global_store_vN);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_ATOMIC_ADD,
                       op_memref_global_atomic_add);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_ATOMIC_ADD_F32,
                       op_memref_global_atomic_add_f32);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_ATOMIC_ADD_PK_BF16,
                       op_memref_global_atomic_add_pk_bf16);
    ckc_ll_set_handler(CKC_OP_MEMREF_COOPERATIVE_GLOBAL_STORE,
    ...
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_STORE_DISTRIBUTED,
                       op_tile_smem_store_distributed);
    ...
    ckc_ll_set_handler(CKC_OP_TILE_BUFFER_LOAD_VN_F16,
                       op_tile_buffer_load_vN_f16);
    ckc_ll_set_handler(CKC_OP_TILE_BUFFER_LOAD_F16, op_tile_buffer_load_f16);
    ckc_ll_set_handler(CKC_OP_TILE_BUFFER_STORE_VN_F16,
                       op_tile_buffer_store_vN_f16);
    ckc_ll_set_handler(CKC_OP_TILE_BUFFER_STORE_F16, op_tile_buffer_store_f16);

    ckc_ll_set_handler(CKC_OP_TILE_ASYNC_BUFFER_LOAD_LDS,
                       op_tile_async_buffer_load_lds);
    ckc_ll_set_handler(CKC_OP_TILE_ASYNC_BUFFER_LOAD_LDS_ADDR,
