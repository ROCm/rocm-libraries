...
void ckc_moe_ssg_claim(ckc_moe_ssg_ctx_t* ctx)
{
    ...
    ctx->local = ckc_b_global_atomic_add(b, ctx->Counter, ctx->eid,
...
ckc_kernel_def_t* ckc_moe_ssg_copy(ckc_moe_ssg_ctx_t* ctx)
{
     ...
     *         b.smem_load_vN(out_row_slot, b.const_i32(0), dtype=I32, n=1), 0)) */
    {
        ckc_value_t* idx[1] = {ckc_b_const_i32(b, 0)};
        ckc_value_t* loaded =
            ckc_b_smem_load_vN(b, ctx->out_row_slot, idx, 1, ckc_i32(), 1);
    ...
    for (int k = 0; k < ctx->chunks; ++k)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < VEC; ++i)
            {
                ...
                ckc_b_global_atomic_add(b, ctx->Y,
                                        ckc_b_add(b, dst_off, ckc_b_const_i32(b, i)),
                                        contrib, NULL);
            }
        }
    }
