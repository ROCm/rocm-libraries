 ...
 *
 *   ckc_build_img2col reproduces the DETERMINISTIC IR prologue byte-for-byte:
 ...
 *   transforms unmerge_magic + unmerge_lower, and the k_local_base / m_val /
 *   k_val_base / y_rsrc arithmetic.
 *
 *   The conv-source LOAD path and the wide STORE path of the Python body depend
 *   on helpers that are NOT yet ported to C:
 *     - conv_implicit_gemm.make_a_descriptor (uses transforms.pad, out of the
 *       transforms-port scope),
 *     - tensor_view.make_buffer_resource / view_from_transforms_descriptor,
 *     - distribution.load_tile / make_load_store_traits / TileDistributionEncoding
 *       feeding make_static_tile_distribution for the load tile.
 *   Those load/store statements are left as TODO(port) stubs so this module
 *   links; the byte-identical verify+fix loop resolves them once those helpers
 *   land. The stubbed region is bracketed and documented inline.
 */
    ...
    ckc_ir_builder_t* b, const ckc_conv_problem_t* p)
{
    ...
    ckc_transform_t* t_k;
    ckc_transform_t* t_pad_r;
    ckc_transform_t* t_pad_s;
    ...
    t_pad_r = ckc_pad(b, "r", 0, p->R);
    t_pad_s = ckc_pad(b, "s", 0, p->S);

    if (t_m == NULL || t_hi == NULL || t_wi == NULL || t_k == NULL ||
        t_pad_r == NULL || t_pad_s == NULL)
    {
        return NULL;
    }
