...
static bool ckc_dist_validate_target(
    ckc_ir_builder_t* b, const ckc_tile_distribution_encoding_t* e, const char* kind, int idx, int major, int minor)
{
    if(major == 0)
    {
...
ckc_tile_distribution_t* ckc_make_static_tile_distribution(ckc_ir_builder_t* b,
                                                           const ckc_tile_distribution_encoding_t* encoding)
{
    ...
    if(num_X > 0)
    {
        rows = (ckc_h_bucket_ref_t**)ckc_arena_alloc(
            &b->arena, (size_t)num_X * sizeof(ckc_h_bucket_ref_t*));
        if(rows == NULL)
        {
            return (ckc_tile_distribution_t*)ckc_dist_set_err(
                b, CKC_ERR_OOM, "distribution: arena alloc failed");
                                      ...
                                      int out_x_cap)
{
    ...
    if(num_ys != enc->num_Y || num_ps != enc->num_P || out_x_cap < enc->num_X)
    {
        ...
        ckc_dist_set_err(b, CKC_ERR_VALUE,
                         "calculate_x: rank mismatch (ys=%d/%d ps=%d/%d x_cap=%d/%d)",
                         num_ys, enc->num_Y, num_ps, enc->num_P, out_x_cap, enc->num_X);
        return 0;
    }
...
ckc_tile_distribution_encoding_t* ckc_make_reduce_tile_distribution_encoding(
    ckc_ir_builder_t* b,
    const ckc_tile_distribution_encoding_t* encoding,
    ...
    int num_reduce)
{
    ...
    int* rs_lengths_out = NULL; int rs_count = 0;
    ckc_h_row_t* hs_out = NULL; int hs_count = 0;
    ckc_p_seq_t* ps_out = NULL;
    int* ys_major_out = NULL; int* ys_minor_out = NULL; int ys_count = 0;
    int total_minor = 0;
    int i, r;
    int cnt_major_out;
    int cnt_r_out;
    ...
    if(cnt_major_out > 1)
    {
        hs_out = (ckc_h_row_t*)ckc_arena_alloc(
            &b->arena, (size_t)(cnt_major_out - 1) * sizeof(ckc_h_row_t));
        if(hs_out == NULL)
        {
            return (ckc_tile_distribution_encoding_t*)ckc_dist_set_err(
                b, CKC_ERR_OOM, "distribution: arena alloc failed");
    ...
    if(e->num_P > 0)
    {
        ps_out = (ckc_p_seq_t*)ckc_arena_alloc(
            &b->arena, (size_t)e->num_P * sizeof(ckc_p_seq_t));
        if(ps_out == NULL)
        {
            return (ckc_tile_distribution_encoding_t*)ckc_dist_set_err(
                b, CKC_ERR_OOM, "distribution: arena alloc failed");
...
ckc_static_distributed_tensor_t* ckc_make_static_distributed_tensor(
    ckc_ir_builder_t* b,
    const ckc_tile_distribution_t* distribution,
    const ckc_type_t* dtype)
{
    ...
    t = (ckc_static_distributed_tensor_t*)ckc_arena_calloc(
        &b->arena, sizeof(ckc_static_distributed_tensor_t));
    if(t == NULL)
    {
        return (ckc_static_distributed_tensor_t*)ckc_dist_set_err(
            b, CKC_ERR_OOM, "distribution: arena alloc failed");
    ...
    t->dtype        = dtype;
    t->num_storage  = n;
    t->storage      = (ckc_value_t**)ckc_arena_calloc(
        &b->arena, (size_t)n * sizeof(ckc_value_t*));
    if(t->storage == NULL)
    {
        return (ckc_static_distributed_tensor_t*)ckc_dist_set_err(
            b, CKC_ERR_OOM, "distribution: arena alloc failed");
                                     ...
                                     int cap)
{
     ...
     * contribution order). Then sort entries by r-level ascending. */
    {
        ...
        for(k = 0; k < cnt; ++k)
        {
            ...
            if(best != k)
            {
                int tr = rm[k]; rm[k] = rm[best]; rm[best] = tr;
                int td = dv[k]; dv[k] = dv[best]; dv[best] = td;
                int tl = ln[k]; ln[k] = ln[best]; ln[best] = tl;
            }
        }
        {
...
static ckc_value_t* ckc_dist_fold(ckc_ir_builder_t* b,
                                ...
                                int wave_size)
{
    ...
    if(reduced == NULL || reduced->distribution == NULL ||
       reduced->distribution->encoding == NULL)
    {
    ...
    {
        ckc_dist_set_err(b, CKC_ERR_VALUE,
                         "block_tile_reduce_sync requires at least one P dim");
        return;
    }
    ...
    for(off = 0; off < reduced->num_storage; ++off)
    {
        ...
        for(pi = 0; pi < n_lane; ++pi)
        {
            ...
            {
                ckc_value_t* remote =
                    ckc_b_warp_shuffle_xor(b, v_local, derivative << istage);
...

    {
        ...
        /* if lane == 0: publish thread buffer interleaved by warp. */
        {
            ...
            for(i = 0; i < thread_buf_size; ++i)
            {
                ...
                ckc_value_t* slot = ckc_b_add(b, warp,
                                              ckc_b_const_i32(b, i * num_reduce_warps));
                ckc_value_t* idx[1];
                idx[0] = slot;
        ...
        for(i = 0; i < thread_buf_size; ++i)
        {
            ...
            for(idx_i = 0; idx_i < num_reduce_warps; ++idx_i)
            {
                ckc_value_t* slot = ckc_b_add(
                    b, local_smem_os,
                    ckc_b_const_i32(b, i * num_reduce_warps + idx_i));
                ckc_value_t* sidx[1];
                ckc_value_t* vec;
                sidx[0] = slot;
