...
ckc_status_t ckc_direct_conv_problem_short(const ckc_direct_conv_problem_t* p,
                                           char* out,
                                           size_t out_cap)
{
    int n;
    ...
    n = snprintf(out, out_cap, "N%dH%dW%d_g%d_c%dk%d", p->N, p->H, p->W, p->groups,
                 p->cpg, p->kpg);
                                             ...
                                             size_t out_cap)
{
    ...
    return ckc_kernel_name_join(spec->name, parts, 4, flag_names, flag_on, 1, out,
                                out_cap, NULL);
}
                                          ...
                                          size_t reason_cap)
{
    ...
    if (p->cpg != 16 || p->kpg != 16)
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap,
                     "DirectConv16cSpec expects cpg=kpg=16 (got %d, %d)", p->cpg,
                     p->kpg);
        }
        return CKC_ERR_VALUE;
    }
    ...
    if (spec->block_groups == 0 || (p->groups % spec->block_groups) != 0)
    {
        ...
        {
            snprintf(reason, reason_cap,
                     "groups %d not divisible by block_groups %d", p->groups,
                     spec->block_groups);
        }
        return CKC_ERR_VALUE;
    }
...
#define CK_DCONV16C_REJECT(...)                          \
    ...
    if (p->cpg != 16 || p->kpg != 16)
    {
        CK_DCONV16C_REJECT("DirectConv16cSpec expects cpg=kpg=16 (got %d, %d)",
                           p->cpg, p->kpg);
    }
    /* if p.groups % spec.block_groups != 0: return False, ... */
    if (spec->block_groups == 0 || (p->groups % spec->block_groups) != 0)
    {
        CK_DCONV16C_REJECT("groups %d not divisible by block_groups %d", p->groups,
                           spec->block_groups);
    }
    ...
    if (spec->fold_k32 &&
        !ckc_mma_catalog_has_shape(mma, "mma", "f16", "f16", "fp32", 16, 16, 32))
    {
        CK_DCONV16C_REJECT(
            "fold_k32=True needs the 16x16x32 f16 MFMA atom, absent on %s; use "
                                            ...
                                            size_t out_cap)
{
    ...
    return ckc_kernel_name_join(spec->name, parts, 3, NULL, NULL, 0, out, out_cap,
                                NULL);
}
                                         ...
                                         size_t reason_cap)
{
    ...
    if (p->cpg != 4 || p->kpg != 4)
    {
        ...
        {
            snprintf(reason, reason_cap,
                     "DirectConv4cSpec expects cpg=kpg=4 (got %d, %d)", p->cpg,
                     p->kpg);
        }
        return CKC_ERR_VALUE;
    }
    ...
    if ((spec->block_groups % 16) != 0)
    {
        ...
        {
            snprintf(reason, reason_cap,
                     "DirectConv4cSpec block_groups must be a multiple of 16");
        }
        return CKC_ERR_VALUE;
    }
    ...
    if ((spec->block_q % 4) != 0)
    {
        ...
        {
            snprintf(reason, reason_cap,
                     "DirectConv4cSpec block_q must be a multiple of 4");
        }
        return CKC_ERR_VALUE;
    }
    /* if p.groups % self.block_groups != 0: raise ValueError(...) */
...
#define CK_DCONV4C_REJECT(...)                           \
    ...
    if (p->cpg != 4 || p->kpg != 4)
    {
        CK_DCONV4C_REJECT("DirectConv4cSpec expects cpg=kpg=4 (got %d, %d)", p->cpg,
                          p->kpg);
    }
    ...
    if (spec->block_groups == 0 || (p->groups % spec->block_groups) != 0)
    {
        CK_DCONV4C_REJECT("groups %d not divisible by block_groups %d", p->groups,
                          spec->block_groups);
    }
