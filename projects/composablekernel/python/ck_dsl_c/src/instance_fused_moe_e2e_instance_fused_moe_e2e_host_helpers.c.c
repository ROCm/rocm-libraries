                                      ...
                                      size_t* n_written)
{
    ...
    size_t to_copy = cap < (size_t)CKC_FMOE_NUM_WORKSPACE_SPECS
                         ? cap
                         : (size_t)CKC_FMOE_NUM_WORKSPACE_SPECS;
 ...
 * ------------------------------------------------------------------ */
ckc_status_t ckc_fmoe_host_preshuffle_b(int E, int N, int K,
 ...
 * block_k). */
ckc_tensor_t* ckc_fmoe_ensure_w_down_preshuffled(ckc_fmoe_build_ctx_t* ctx,
                                                 ckc_tensor_t* W_down)
{
                                                    ...
                                                    ckc_tensor_t* W_up)
{
    ...
    if (ctx->gu_concat_preshuffled != NULL &&
        ctx->gu_concat_preshuffled_key_valid &&
                                                         ...
                                                         ckc_tensor_t* W_up)
{
    ...
    if (ctx->gu_interleaved_preshuffled != NULL &&
        ctx->gu_interleaved_preshuffled_key_valid &&
...
ckc_tensor_t* ckc_fmoe_ensure_gu_concat(ckc_fmoe_build_ctx_t* ctx,
                                        ckc_tensor_t* W_gate,
                                        ckc_tensor_t* W_up)
{
    if (ctx == NULL || W_gate == NULL || W_up == NULL)
    ...
    if (ctx->gu_concat != NULL && ctx->gu_concat_key_valid &&
        ctx->gu_concat_key[0] == key[0] && ctx->gu_concat_key[1] == key[1])
    {
...
ckc_status_t ckc_fmoe_build_per_expert_problems(ckc_fmoe_build_ctx_t* ctx,
                                                ...
                                                size_t* n_written)
{
    if (ctx == NULL || counts_cpu == NULL || offsets_cpu == NULL ||
        b_base == NULL || out == NULL)
    {
        return CKC_ERR_VALUE;
    }
    ...
    for (int e = 0; e < experts; ++e)
    {
        ...
        p->K = b_inner_dim;
        p->A_ptr =
            a_base + (uint64_t)offset * (uint64_t)a_inner_dim * (uint64_t)elem_bytes;
        p->B_ptr = b_base[e];
        p->C_ptr =
            c_base + (uint64_t)offset * (uint64_t)c_inner_dim * (uint64_t)elem_bytes;
        ++written;
    }
