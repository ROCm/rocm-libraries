 ...
 */
ckc_status_t ckc_fmha_fwd_paged_prefill_kernel_name(
    const ckc_fmha_fwd_paged_prefill_spec_t* spec, char* out, size_t out_cap)
{
                                              ...
                                              size_t reason_cap)
{
    ...
    if (pg <= 0 || (pg & (pg - 1)) != 0)
    {
        ...
        (void)snprintf(buf,
                       sizeof buf,
                       "page_block_size must be a positive power of two (got %d)",
                       pg);
        ckc_fpp_copy_str(reason, reason_cap, buf);
        return false;
    }
                                                   ...
                                                   const char* arch)
{
    ...
    bs_loop = ckc_b_scf_for_iter(b,
                                 ckc_b_const_i32(b, 0),
                                 ckc_b_const_i32(b, bs_iters),
                                 ckc_b_const_i32(b, 1),
                                 bs_args,
                                 2,
                                 "bs_i",
...
ckc_status_t ckc_fmha_fwd_paged_prefill_lower_to_llvm(
    const ckc_fmha_fwd_paged_prefill_spec_t* spec,
    const char* arch,
