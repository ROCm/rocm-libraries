...
{
    if (ckc_mma_catalog_has_shape(&target->mma, "mma", "f16", "f16", "fp32",
                                  16, 16, 32))
        return true;
...
{
    return ckc_mma_catalog_has_shape(&target->mma, "mma", "f16", "f16", "fp32",
                                     16, 16, 16) &&
           ckc_mma_catalog_has_shape(&target->mma, "mma", "bf16", "bf16",
                                     "fp32", 16, 16, 16);
}
...
static bool ckc__validate_tiled_attention_arch(const char* arch,
                                               char* reason,
                                               size_t reason_cap)
{
...
bool ckc_gfx942_attention_tiled_2d_supports(
    const ckc_gfx942_attention_tiled_2d_supports_args_t* args,
    char* reason,
    size_t reason_cap)
{
    const char* arch;
    ...
    {
        ckc__reason(reason, reason_cap,
                    "gfx942 tiled 2D kernel does not support the fp8 K/V cache "
                    "(ds_read_tr_b8 is gfx950-only)");
        return false;
    }
    if (!(args->dtype != NULL && (strcmp(args->dtype, "fp16") == 0 ||
                                  strcmp(args->dtype, "bf16") == 0)))
    {
        snprintf(buf, sizeof(buf),
    ...
    if (!(args->head_size == 64 || args->head_size == 128 ||
          args->head_size == 256))
    {
        snprintf(buf, sizeof(buf),
    ...
    if (!(args->block_size == 16 || args->block_size == 32 ||
          args->block_size == 64))
    {
        snprintf(buf, sizeof(buf),
    ...
    if (args->kv_storage_dtype != NULL &&
        strcmp(args->kv_storage_dtype, "fp8e4m3") != 0)
    {
        snprintf(buf, sizeof(buf),
    ...
    {
        snprintf(buf, sizeof(buf),
                 "tiled 2D kernel: unsupported q_dtype '%s'", args->q_dtype);
        ckc__reason(reason, reason_cap, buf);
        return false;
    }
    if (args->has_tile_size)
    {
        ...
        if (args->tile_size * args->head_size < threads * halves_per_lane)
        {
            snprintf(buf, sizeof(buf),
                     ...
                     "num_warps=%d (need >= %d)",
                     args->tile_size * args->head_size, args->num_warps,
    ...
    if (!args->use_fp8)
    {
        ...
        if (args->use_mfma_32x32x8 && args->use_transposed_qk_32x32)
        {
            if (args->use_k_sliced_ring)
            {
                ...
                int _k_bytes =
                    _K_SLICE_SLOTS * _t_eff * _K_SLICE_HD * _BPE;
                int _v_kpack = 8;
                int _v_pixels = 64;
                ...
                int _v_bytes =
                    _v_kgroups * _v_ngroups * (_v_pixels + _v_kpack) * _BPE;
                ...
                if (_lds_x8 > _LDS_CAPACITY_BYTES)
                {
                    snprintf(buf, sizeof(buf),
                             ...
                             _lds_x8, arch, _LDS_CAPACITY_BYTES);
                    ckc__reason(reason, reason_cap, buf);
                    return false;
                }
            ...
            }
            {
                ...
                int q_lds = (_block_m <= 2 * _t_eff)
                                ? 0
                                : _block_m * args->head_size * _BPE;
                int v_pad = args->use_conflict_free_v_store ? 8 : 0;
                int _lds_x8 =
                    k_slots * _t_eff * args->head_size * _BPE +
        ...
        }
        {
            ...
            int _lds =
                ...
                + ((_block_m <= 2 * _t_eff)
                       ? 0
                       : _block_m * args->head_size * _BPE) /* Q_lds       */
            ...
            if (_lds > _LDS_CAPACITY_BYTES)
            {
                snprintf(buf, sizeof(buf),
                         ...
                         "CODEGEN would fail",
                         _lds, _block_m, _t_eff, args->head_size, arch,
                         _LDS_CAPACITY_BYTES);
                ckc__reason(reason, reason_cap, buf);
                return false;
            }
 ...
 * ===================================================================== */
static ckc_status_t ckc__attn2d_kernel_name(
...

    {
        const char* cfvnosplit =
            (s->use_conflict_free_v_store &&
             !s->use_conflict_free_v_store_split)
                ? "cfvnosplit"
                : "";
        const char* cfvplainv =
            (s->use_conflict_free_v_store &&
             !s->use_conflict_free_v_ck_vlds)
                ? "cfvplainv"
                : "";
        const char* parts[] = {
            d_buf,
 ...
 * ===================================================================== */
ckc_status_t ckc_gfx942_attention_tiled_2d_lower_to_llvm(
    const ckc_attention_tiled_2d_spec_t* spec,
    const char* arch,
    ...
    size_t err_cap)
{
    ...
    st = ckc_lower_kernel_to_llvm_ex(kernel, flavor, arch ? arch : "gfx942",
                                     out_ll, err, err_cap);
    ckc_ir_builder_free(&b);
    return st;
}
