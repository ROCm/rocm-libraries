...
static ckc_value_t* ckc__c_row(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, int reg)
{
...
void ckc_gfx942_attention_tiled_3d_emit_loop_init(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    ...
    ckc_gfx942_attention_tiled_3d_issue_k(ctx, ctx->tile_start, ckc_b_const_i32(b, 0));
 ...
 * ============================================================ */
void ckc_gfx942_attention_tiled_3d_emit_softmax_loop(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    ckc_ir_builder_t* b                          = ctx->b;
    const ckc_gfx942_attn_tiled_3d_config_t* cfg = &ctx->cfg;
    const ckc_type_t* dtype                      = cfg->dtype;
    ...
    ckc_b_region_enter(b, kvloop.body);
    {
        ...
        for(n = 0; n < cfg->PV_N_TILES; n++)
        {
            ...
            {
                ckc_value_t* v_n_n   = ckc_b_const_i32(b, n);
                ckc_value_t* v_n_16  = ckc_b_const_i32(b, 16);
                ckc_value_t* v_n_mul = ckc_b_mul(b, v_n_n, v_n_16);
