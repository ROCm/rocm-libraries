                                      ...
                                      const char* arch)
{
    ...
    const bool SWIZZLE_VLDS =
        true /* env HIPDNN_GFX942_SWIZZLE_VLDS default "1" */ && !FP8_MFMA_PV && !FP8_MFMA_QK &&
        !USE_MFMA_32X32 && !ctx->REGISTER_PV && !TRANSPOSED_V && ckc_type_eq(V_LDS_DTYPE, dtype) &&
        _V_ROW_PER_WAVE_CALL && (NUM_WARPS == 1 || NUM_WARPS == 2) &&
        (V_ROWS_PER_CALL != 0 && (T % V_ROWS_PER_CALL) == 0) && (HD == 64 || HD == 128) &&
        !ctx->FAST_PAGED_KV_DESC && _swz_fits;
...
ckc_value_t* ckc_gfx942_attn2d_in_warp_row(ckc_gfx942_attn2d_build_ctx_t* ctx, int r)
{
    ...
    ckc_value_t* rg4 = ckc_b_mul(ctx->b, ckc_a2d_lane_rg(ctx), ckc_b_const_i32(ctx->b, 4));
...
void ckc_gfx942_attn2d_emit_licm_hoist(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    for(reg = 0; reg < ctx->REGS_PER_LANE; ++reg)
    {
        ...
        if(ctx->USE_MFMA_32X32)
            row = ckc_b_add(b, ctx->wave_row_base, ckc__mfma_32x32_c_row(b, ctx->lane, reg));
        else
            row = ckc_b_add(b, ctx->wave_row_base, ckc_gfx942_attn2d_in_warp_row(ctx, reg));
