 ...
 * bucket of the C99 port of ck_dsl.instances.gfx942.attention_tiled_2d.
 *
 * Ports the three pieces of the Python emitter that previously had no C
 * counterpart, which is why ``ckc_build_unified_attention_2d_tiled_scalar`` was
 * calling ``ckc_gfx942_attn2d_emit_kv_body`` with a NULL ``ctx->kv_tile_iv``
 * (the very first ``b.mul(kv_tile_iv, ...)`` then aborted the build with
 * "binop: NULL operand"):
 *
 *   1. ckc_gfx942_attn2d_emit_loop_bounds_and_inits
 ...
 *   2. ckc_gfx942_attn2d_emit_preloop_prefetch
 *        _issue_k(tile_start, 0) (3591), the ("cur_buf", 0) carry append (3606-
 *        3607) and kv_step (3689).
 *   3. ckc_gfx942_attn2d_drive_kv_loop (defined in the kv_body_pv TU; declared
 ...
 * run yet. */
static ckc_value_t* sc_neg_inf(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    if (ctx->neg_inf_v != NULL)
        return ctx->neg_inf_v;
    return ckc_b_const_f32(B, -INFINITY);
}
static ckc_value_t* sc_one_f(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    if (ctx->one_f_v != NULL)
        return ctx->one_f_v;
    return ckc_b_const_f32(B, 1.0);
}
static ckc_value_t* sc_rcp_ln2(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
...
static ckc_value_t* sc_in_warp_row(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                   ckc_value_t* lane_rg, int r)
{
    ...
    ckc_value_t* rg4 = ckc_b_mul(B, lane_rg, ckc_b_const_i32(B, 4));
    return ckc_b_add(B, rg4, ckc_b_const_i32(B, atom_idx * 16 + in_atom));
}
 ...
 * _in_warp_row. */
static ckc_value_t* sc_state_row(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                 ckc_value_t* lane_rg, int r)
{
 ...
 * ===================================================================== */
void ckc_gfx942_attn2d_emit_loop_bounds_and_inits(
    ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    ckc_value_t* msp_raw =
        ckc_b_add(B, msp_inner, ckc_b_const_i32(B, bm1_div_nqk + 1));
    ckc_value_t* max_seq_prefix_len = ckc_b_select(
        B, ckc_b_cmp_lt(B, msp_raw, ctx->seq_len), msp_raw, ctx->seq_len);
    ...
    ckc_value_t* nt_inner =
        ckc_b_add(B, max_seq_prefix_len, ckc_b_const_i32(B, T - 1));
    ...
    if (SLIDING_WINDOW > 0)
    {
        ...
        ckc_value_t* qpos_hi_raw =
            ckc_b_add(B, ctx->qb_start_pos, ckc_b_const_i32(B, bm1_div_nqk));
        ckc_value_t* cur_q_minus1 =
            ckc_b_sub(B, ctx->cur_batch_q_len, ckc_b_const_i32(B, 1));
        ckc_value_t* qpos_hi = ckc_b_select(
            B, ckc_b_cmp_lt(B, qpos_hi_raw, cur_q_minus1), qpos_hi_raw,
            cur_q_minus1);
        ...
        ckc_value_t* fak_inner =
            ckc_b_sub(B, ckc_b_add(B, ctx->context_len, ctx->qb_start_pos),
                      sw_const);
        ckc_value_t* first_allowed_key =
            ckc_b_add(B, fak_inner, ckc_b_const_i32(B, 1));
        ckc_value_t* last_allowed_key =
            ckc_b_add(B, ctx->context_len, qpos_hi);
        ckc_value_t* tile_start_raw =
            ckc_b_div(B, first_allowed_key, ckc_b_const_i32(B, T));
        ...
        ckc_value_t* ts_lt =
            ckc_b_cmp_lt(B, tile_start_raw, ckc_b_const_i32(B, 0));
        ctx->tile_start = ckc_b_select(
            B, ts_lt, ckc_b_const_i32(B, 0), tile_start_raw);
        /* Same left-to-right ordering: build the div BEFORE the +1 const, and
         * the cmp_lt BEFORE the select operands. */
        ckc_value_t* tile_end_div =
            ckc_b_div(B, last_allowed_key, ckc_b_const_i32(B, T));
        ckc_value_t* tile_end_raw =
            ckc_b_add(B, tile_end_div, ckc_b_const_i32(B, 1));
    ...
    ctx->lane_col_div4_v =
        ckc_b_div(B, ctx->lane_col_v, ckc_b_const_i32(B, 4));
    ctx->lane_col_mod4_v =
        ckc_b_mod(B, ctx->lane_col_v, ckc_b_const_i32(B, 4));
    ...
    ckc_value_t* l_inits[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
    ...
    if (ctx->USE_SINKS)
    {
        ...
        {
            ckc_value_t* row =
                ckc_b_add(B, ctx->wave_row_base, sc_state_row(ctx, lane_rg, r));
            ...
            ckc_value_t* sink_h =
                ckc_b_global_load(B, ctx->sinks, qh, ctx->dtype, 2);
            ckc_value_t* sink_f =
                ckc_b_fmul(B, ckc_b_cast_to_f32(B, sink_h), rcp_ln2);
    ...
    for (int n = 0; n < ACC_N_TILES; ++n)
    {
        for (int atom = 0; atom < ACC_M_ATOMS; ++atom)
        {
            if (ACC_M_ATOMS > 1)
                snprintf(ctx->iter_args_name_buf[n_args],
                         sizeof(ctx->iter_args_name_buf[0]), "acc%da%d", n, atom);
            else
                snprintf(ctx->iter_args_name_buf[n_args],
                         sizeof(ctx->iter_args_name_buf[0]), "acc%d", n);
            ctx->iter_args_names[n_args] = ctx->iter_args_name_buf[n_args];
 ...
 * ===================================================================== */
void ckc_gfx942_attn2d_emit_preloop_prefetch(
    ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    if (ctx == NULL)
        return;
    ...
    ckc_gfx942_attn2d_issue_k(ctx, ctx->tile_start, ckc_b_const_i32(B, 0));

    /* cur_buf carry append (3606-3607). */
    int idx = ctx->iter_args_count;
    snprintf(ctx->iter_args_name_buf[idx], sizeof(ctx->iter_args_name_buf[0]),
             "cur_buf");
