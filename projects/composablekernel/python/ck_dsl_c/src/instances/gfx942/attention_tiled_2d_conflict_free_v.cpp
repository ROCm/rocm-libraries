                                          ...
                                          ckc_value_t* kv_tile_idx)
{
    ...
    for(call = 0; call < V_T_ITEMS_PER_THREAD; ++call)
    {
        ...
        for(j = 0; j < V_T_VEC; ++j)
        {
            ...
            ckc_value_t* linear_j =
                ckc_b_add(b, ckc_b_mul(b, token_j, ckc_b_const_i32(b, ctx->HD)), col);
            ...
            ckc_value_t* in_range =
                ckc_b_cmp_lt(b, ckc_b_add(b, tile_tok_base, token_j), max_seq_prefix_len);
            if(valid_j != NULL)
            ...
            ckc_value_t* safe_elem_j =
                (ctx->KV_BYTES != 1) ? ckc_b_div(b, safe_voff_j, ckc_b_const_i32(b, ctx->KV_BYTES))
                                     : safe_voff_j;
                                                   ...
                                                   ckc_value_t* d0)
{
    ...
    ckc_value_t* in_range = ckc_b_cmp_lt(b, ckc_b_add(b, tile_tok_base, t_row), max_seq_prefix_len);
    if(valid != NULL)
    ...
    if(ctx->CFV_STORE_SEPOFF)
    {
        ...
        ckc_value_t* linear1 = ckc_b_add(b,
                                         ckc_b_mul(b, t_row, ckc_b_const_i32(b, ctx->HD)),
                                         ckc_b_add(b, d0, ckc_b_const_i32(b, 1)));
        ...
        ckc_value_t* safe_elem1 = (ctx->KV_BYTES != 1)
                                          ...
                                          ckc_value_t* kv_tile_idx)
{
    ...
    payload->blocks = (ckc__cfvst_blk_t*)ckc_arena_alloc(
        &b->arena,
        sizeof(ckc__cfvst_blk_t) * (size_t)(V_T_ITEMS_PER_THREAD > 0 ? V_T_ITEMS_PER_THREAD : 1));
    payload->count = 0;
...
void ckc_gfx942_attn2d_cfvst_store_v_regs(ckc_gfx942_attn2d_build_ctx_t* ctx, void* payload_v)
{
    ...
    if(ctx->CFV_STORE_PREZERO)
    {
        ...
        ckc_for_t pz = ckc_b_scf_for(b,
                                     ctx->tid,
                                     ckc_b_const_i32(b, ctx->HD * ctx->T),
                                     ckc_b_const_i32(b, ctx->THREADS),
                                     "vpz");
