...
{
    return dt != NULL && dt->name != NULL && nm != NULL &&
           (strcmp(dt->name, nm) == 0);
}
                                                   ...
                                                   ckc_value_t* v_buf)
{
    ...
    for(int j = 0; j < 4; ++j)
    {
        ckc_value_t* v_row = ckc_b_add(B, ckc_b_const_i32(B, k_iter * 16 + j),
                                       v_k_chunk_base);
        ckc_value_t* loaded =
            ckc_gfx942_attn2d_v_load1(ctx, v_buf, v_row, v_n_col);
                                                        ...
                                                        int p_count)
{
        ...
        for(int k = 0; k < T / 8; ++k)
        {
            ...
            if(ctx->TRANSPOSED_V)
            {
                ...
                if(ctx->CFV_SCALAR_READ)
                {
                    ...
                    for(int kk = 0; kk < 4; ++kk)
                    {
                        ...
                        ckc_value_t* v1 =
                            ckc_gfx942_attn2d_v_t_load(ctx, v_dim32, t, 1);
            ...
            else
            {
                ...
                for(int kk = 0; kk < 4; ++kk)
                {
                    ...
                    ckc_value_t* v1 =
                        ckc_b_smem_load_vN(B, ctx->V_lds, idx, 3, DT, 1);
    ...
    for(int k = 0; k < T / 16; ++k)
    {
        ...
        {
            ckc_value_t* A_v_t =
                ckc_pv32_v_load_paired(B, ctx->V_lds, v_buf, n, k,
                                       lane_half32, lane_col32, DT);
            ...
            for(int kk = 0; kk < 8; ++kk)
            {
                ...
                b_p_elems[kk] =
                    ckc_b_cast_f32_to(B, p_regs[p_tile * RPL + preg], DT);
            }
...
static ckc_value_t* pack_p_a16(ckc_gfx942_attn2d_build_ctx_t* ctx,
                               ckc_value_t* const* regs4)
{
    return ckc_gfx942_attn2d_pack_p_a16(ctx, regs4, 4);
}
static ckc_value_t* pack_p_a32(ckc_gfx942_attn2d_build_ctx_t* ctx,
                               ckc_value_t* const* regs0_4, ckc_value_t* const* regs1_4)
{
        ...
        if(ctx->TRANSPOSED_QK_32X32)
        {
            ...
            for(int n = 0; n < ACC_N_TILES; ++n)
            {
                ...
                for(int reg = 0; reg < REGS_PER_LANE; ++reg)
                {
                    ...
                    ckc_value_t* a = ctx->TRANSPOSED_SCALAR_STATE ? alpha_t
                                                                  : in->alpha_regs[reg];
        ...
        else if(ctx->USE_MFMA_32X32X8)
        {
            ...
            for(int n = 0; n < ACC_N_TILES; ++n)
            {
                ...
                for(int k = 0; k < PV_K_ITERS; ++k)
                {
                    ...
                    {
                        ckc_value_t* v_row =
                            ckc_b_add(B, k_row_base, ckc_b_const_i32(B, j));
                        ckc_value_t* loaded =
                            ckc_gfx942_attn2d_v_load1(ctx, v_buf, v_row, v_col32);
                        ckc_value_t* elem = ckc_b_vec_extract(B, loaded, 0);
        ...
        else
        {
            ...
            for(int n = 0; n < ACC_N_TILES; ++n)
            {
                ...
                for(int k = 0; k < ctx->T / 16; ++k)
                {
                    ...
                    ckc_value_t* p_row32 =
                        ckc_b_add(B, ctx->wave_row_base, lane_col32);
                    ckc_value_t* pidx[2] = {p_row32, p_off32};
                    ckc_value_t* A_p32 =
                        ckc_b_smem_load_vN(B, ctx->P_lds, pidx, 2, DT, 8);
                    ...
                    ckc_value_t* B32_r0 =
                        ckc_b_ds_read_tr16_b64(B, ctx->V_lds, r0idx, 3, DT);
                    ckc_value_t* r1row =
                        ckc_b_add(B, tr_row_base32, ckc_b_const_i32(B, 4));
                    ckc_value_t* r1idx[3] = {v_buf, r1row, col0};
                    ckc_value_t* B32_r1 =
                        ckc_b_ds_read_tr16_b64(B, ctx->V_lds, r1idx, 3, DT);
        ...
        for(int n = 0; n < narrow_n; ++n)
        {
            ...
            for(int atom = 0; atom < M_ATOMS; ++atom)
            {
                ...
                for(int in_atom = 0; in_atom < 4; ++in_atom)
                {
                    ...
                    ckc_value_t* e =
                        ckc_b_vec_extract(B, ckc_gfx942_attn2d_acc_get(ctx, n, atom),
                                          in_atom);
            ...
            ckc_value_t* v_n_col =
                ckc_b_add(B, ckc_b_mul(B, vn_n, vn_16), lane_col);
            ckc_value_t* v_k_chunk_base =
            ...
            for(int k = 0; k < PV_K_ITERS; ++k)
            {
                ...
                for(int atom = 0; atom < M_ATOMS; ++atom)
                {
                    ...
                    if(ctx->REGISTER_PV)
                    {
                        ...
                        {
                            regs4[r] =
                                in->p_regs_f32[(atom * 4 + r) * in->p_regs_f32_stride + k];
                        }
                    ...
                    acc_per_atom[atom] =
                        mfma_16x16x16(ctx, A_p, B_v, acc_per_atom[atom]);
                }
            }
            for(int atom = 0; atom < M_ATOMS; ++atom)
            {
...
ckc_for_t ckc_gfx942_attn2d_drive_kv_loop(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    ckc_for_t kvloop = ckc_b_scf_for_iter(B, ctx->tile_start, ctx->tile_end,
                                          ctx->kv_step, iargs, ctx->iter_args_count,
                                          kIvName, /*unroll=*/false,
