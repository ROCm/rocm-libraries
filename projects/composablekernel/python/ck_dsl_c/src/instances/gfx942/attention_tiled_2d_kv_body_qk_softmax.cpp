...
static ckc_value_t* fh_qk_scale(ckc_gfx942_attn2d_build_ctx_t* ctx) { return ctx->qk_scale_v; }
...
static ckc_value_t* fh_max_seq_prefix_len(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    return ckc_b_select(ctx->b, ckc_b_cmp_lt(ctx->b, msp_raw, ctx->seq_len), msp_raw, ctx->seq_len);
}
...
static ckc_value_t* fh_warp_xor_reduce_max(ckc_gfx942_attn2d_build_ctx_t* ctx, ckc_value_t* v)
{
    ckc_value_t* cur = v;
    for(int k = 0; k < 4; ++k)
    {
...
static ckc_value_t* fh_warp_xor_reduce_sum(ckc_gfx942_attn2d_build_ctx_t* ctx, ckc_value_t* v)
{
...
static ckc_value_t* fh_apply_softcap(ckc_gfx942_attn2d_build_ctx_t* ctx, ckc_value_t* s)
{
...
static ckc_value_t* fh_in_warp_row(ckc_gfx942_attn2d_build_ctx_t* ctx, ckc_value_t* lane_rg, int r)
{
    int atom_idx = r / 4;
    int in_atom  = r % 4;
...
static ckc_value_t*
    p_regs_f32_buf[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE * CKC_GFX942_ATTN2D_MAX_N_TILES];
...
void ckc_gfx942_attn2d_emit_kv_body(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    ctx->nxt_buf_v =
        ctx->K_SINGLE_BUF ? ctx->cur_buf : ckc_b_sub(b, ckc_b_const_i32(b, 1), ctx->cur_buf);
    ...
    if(ctx->USE_MFMA_32X32 && ctx->TRANSPOSED_QK_32X32)
    {
        ...
        for(int n = 0; n < QK_N_TILES; ++n)
        {
            ...
            for(int k = 0; k < QK_K_ITERS; ++k)
            {
                ...
                ckc_value_t* k_off_t    = ckc_b_add(
                    b, k_off_base, ckc_b_mul(b, lane_half, ckc_b_const_i32(b, TQK_HALF_STRIDE)));
                ckc_value_t* A_k_t =
            ...
            for(int n = 0; n < QK_N_TILES; ++n)
            {
                ...
                for(int k = 0; k < QK_K_ITERS; ++k)
                {
                    ...
                    ckc_value_t* k_off_t =
                        ckc_b_add(b,
                                  k_off_base,
                                  ckc_b_mul(b, lane_half, ckc_b_const_i32(b, TQK_HALF_STRIDE)));
        ...
        for(int group_idx = 0; group_idx < n_groups; ++group_idx)
        {
            ...
            for(int n = 0; n < QK_N_TILES; ++n)
            {
                for(int reg = 0; reg < 16; ++reg)
                {
                    ...
                    ckc_value_t* k_local =
                        ckc_b_add(b, k_local_base, ckc__mfma_32x32_c_row(b, ctx->lane, reg));
                    ...
                    if(mask_fallback)
                    {
                        ckc_value_t* q_row_t = ckc_b_add(
                            b, ctx->wave_row_base, ckc__mfma_32x32_c_col(b, ctx->lane, 0));
                        qp_r = ckc_b_add(b,
                                         ctx->qb_start_pos,
                        ...
                        ckc_value_t* qh_mod = ckc_b_mod(b, q_row_t, ckc_b_const_i32(b, ctx->NQK));
                        ckc_value_t* qh_r   = ckc_b_add(b, qh_mul, qh_mod);
                        /* row_ok = land(qp<q_len, qh<NUM_QH) (qp cmp first). */
                        ckc_value_t* ok_pos = ckc_b_cmp_lt(b, qp_r, ctx->cur_batch_q_len);
                        ckc_value_t* ok_qh = ckc_b_cmp_lt(b, qh_r, ckc_b_const_i32(b, ctx->NUM_QH));
         ...
         * ============================================================ */
        for(int r = 0; r < SOFTMAX_STATE_SLOTS; ++r)
            alpha_regs[r] = ckc_b_exp2(b, ckc_b_fsub(b, ctx->m_cur[r], m_new[r]));
        for(int r = 0; r < SOFTMAX_STATE_SLOTS; ++r)
        {
    ...
    else if(ctx->USE_MFMA_32X32)
    {
        ...
        {
            ckc_value_t* acc32   = ckc_b_zero_vec_f32(b, 16);
            ckc_value_t* k_row32 = ckc_b_add(b, ckc_b_const_i32(b, n * 32), lane_col32_qk);
            for(int k = 0; k < QK_K_ITERS; ++k)
            {
                ...
                ckc_value_t* kc_off32 = ckc_b_add(
                    b, kc_base, ckc_b_mul(b, lane_half_qk, ckc_b_const_i32(b, B32_HALF_STRIDE)));
        ...
        ckc_value_t* nxt_buf =
            (ctx->nxt_buf_v != NULL)
                ? ctx->nxt_buf_v
                : (ctx->K_SINGLE_BUF ? cur_buf : ckc_b_sub(b, ckc_b_const_i32(b, 1), cur_buf));
        if(ctx->K_SINGLE_BUF)
        {
        ...
        for(int reg = 0; reg < REGS_PER_LANE; ++reg)
        {
            ...
            for(int n = 0; n < QK_N_TILES; ++n)
            {
                ...
                ckc_value_t* col_abs =
                    ckc_b_add(b, tile_off, ckc__mfma_32x32_c_col(b, ctx->lane, n));
                ckc_value_t* causal_ok = ckc_b_cmp_le(b, col_abs, causal_lim);
                ckc_value_t* in_prefix = ckc_b_cmp_lt(b, col_abs, max_seq_prefix_len);
                ckc_value_t* m_ok      = ckc_b_land(b, ckc_b_land(b, row_ok, causal_ok), in_prefix);
                if(ctx->SLIDING_WINDOW > 0)
        ...
        ckc_value_t* m_new[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE];
        ckc_value_t* s_local[CKC_GFX942_ATTN2D_MAX_REGS_PER_LANE][CKC_GFX942_ATTN2D_MAX_N_TILES];
        for(int reg = 0; reg < REGS_PER_LANE; ++reg)
        {
        ...
        for(int reg = 0; reg < REGS_PER_LANE; ++reg)
        {
            ...
            for(int n = 0; n < QK_N_TILES; ++n)
            {
                ckc_value_t* p      = ckc_b_exp2(b, ckc_b_fsub(b, s_local[reg][n], m_new[reg]));
    ...
    else
    {
        ...
        for(int n = 0; n < QK_N_TILES; ++n)
        {
            ...
            for(int k = 0; k < QK_K_ITERS; ++k)
            {
                ...
                ckc_value_t* kc_off =
                    ckc_b_add(b, kc_base, ckc_b_mul(b, lane_rg, ckc_b_const_i32(b, 4)));
                ckc_value_t* k_row = ckc_b_add(b, ckc_b_const_i32(b, n * 16), lane_col);
                ckc_value_t* idx[3];
                ...
                idx[1]           = k_row;
                idx[2]           = kc_off;
                ckc_value_t* B_v = ckc_b_smem_load_vN(b, ctx->K_lds, idx, 3, ctx->dtype, 4);
                for(int atom = 0; atom < M_ATOMS_PER_WARP; ++atom)
                {
        ...
        for(int reg = 0; reg < REGS_PER_LANE; ++reg)
        {
            ...
            for(int n = 0; n < QK_N_TILES; ++n)
            {
                ...
                ckc_value_t* col_abs;
                {
                    ...
                    col_abs = ckc_b_add(b, ckc_b_add(b, tile_off, ckc_b_mul(b, cn, c16)), lane_col);
                }
                ckc_value_t* causal_ok = ckc_b_cmp_le(b, col_abs, causal_lim);
                ckc_value_t* in_prefix = ckc_b_cmp_lt(b, col_abs, max_seq_prefix_len);
                ckc_value_t* m_ok      = ckc_b_land(b, ckc_b_land(b, row_ok, causal_ok), in_prefix);
            ...
            for(int n = 0; n < QK_N_TILES; ++n)
            {
                ...
                if(!ctx->REGISTER_PV)
                {
                    ...
                    ckc_value_t* col;
                    {
                        ckc_value_t* cn  = ckc_b_const_i32(b, n);
                        ckc_value_t* c16 = ckc_b_const_i32(b, 16);
                        col              = ckc_b_add(b, ckc_b_mul(b, cn, c16), lane_col);
                    }
                    ckc_value_t* idx[2] = {row, col};
                    if(ctx->FP8_MFMA_PV)
                    {
