...
static bool ckc_attn3d950_validate_arch(const char* eff_arch,
                                        char* rbuf,
                                        size_t rcap,
                                        const char** reason)
{
 ...
 * ===================================================================== */
bool ckc_gfx950_attn_tiled_3d_config_from_spec(
    ckc_ir_builder_t* b,
    ...
    ckc_gfx950_attn_tiled_3d_config_t* out)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_NOTIMPL, "%s",
    ...
    size_t err_cap)
{
    ...
    if (kernel == NULL)
    {
        ...
        ckc_attn3d950_set_err_buf(err, err_cap,
                                  (m != NULL && m[0] != '\0')
                                      ? m
                                      : "build_unified_attention_3d_tiled_gfx950 failed");
        ckc_ir_builder_free(&b);
        return (st == CKC_OK) ? CKC_ERR_VALUE : st;
    }

    st = ckc_lower_kernel_to_llvm_ex(kernel, flavor,
                                     (arch != NULL) ? arch : "gfx950", out_ll, err, err_cap);
    ckc_ir_builder_free(&b);
    return st;
}
    ...
    size_t err_cap)
{
    ...
    if (kernel == NULL)
    {
        ...
        ckc_attn3d950_set_err_buf(err, err_cap,
                                  (m != NULL && m[0] != '\0')
                                      ? m
                                      : "build_unified_attention_reduce_tiled_gfx950 failed");
        ckc_ir_builder_free(&b);
        return (st == CKC_OK) ? CKC_ERR_VALUE : st;
    }
