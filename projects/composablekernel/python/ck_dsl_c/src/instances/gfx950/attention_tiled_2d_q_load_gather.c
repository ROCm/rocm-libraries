 ...
 * ===================================================================== */
void ckc_gfx950_attn2d_emit_loop_bounds_and_inits(
        ...
        else
        {
            for (int r = 0; r < SOFTMAX_STATE_SLOTS; ++r)
            {
                ...
                ckc_value_t* row = ckc_b_add(
                    B, ctx->wave_row_base, ckc_gfx950_attn2d_state_row(ctx, r));
                ckc_value_t* qh_mul =
                    ckc_b_mul(B, ctx->kv_head_idx, ckc_b_const_i32(B, NQK));
                ckc_value_t* qh_mod = ckc_b_mod(B, row, ckc_b_const_i32(B, NQK));
                ckc_value_t* qh = ckc_b_add(B, qh_mul, qh_mod);
                ckc_value_t* qh_in =
                    ckc_b_cmp_lt(B, qh, ckc_b_const_i32(B, NUM_QH));
                ckc_value_t* sink_h =
                    ckc_b_global_load(B, ctx->sinks, qh, ctx->dtype, 2);
                ckc_value_t* sink_f =
                    ckc_b_fmul(B, ckc_b_cast_to_f32(B, sink_h), rcp_ln2);
            ...
            for (k = 0; k < ctx->QK_K_ITERS; ++k)
            {
                ...
                q_col_off =
                    ckc_b_add(b, q_col_base,
                              ckc_b_mul(b, lane_rg, ckc_b_const_i32(b, 8)));
 ...
 * ===================================================================== */
void ckc_gfx950_attn2d_emit_preloop_prefetch(
    ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    if (ctx == NULL)
        return;
