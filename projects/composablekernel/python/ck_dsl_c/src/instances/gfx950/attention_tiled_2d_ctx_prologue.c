 ...
 *   - ckc_gfx950_attn2d_build_ctx_init  (Python build body lines 711-1135):
 ...
 * shared ckc_gfx950_attn2d_build_ctx_t. The per-warp lane-decomposition scalars
...
static bool ckc_g950a2d_streq(const char* a, const char* c)
{
    if(a == c)
        return true;
    if(!a || !c)
...
static bool ckc_g950a2d_require_tiled_attention_arch(ckc_ir_builder_t* b, const char* arch)
{
...
/* ===================================================================== *
 *  ckc_gfx950_attn2d_build_ctx_init -- the prologue (lines 711-1135)
 * ===================================================================== */
                                      ...
                                      const char* arch)
{
    ...
    if(!ckc_g950a2d_require_tiled_attention_arch(b, arch))
        return false;

    /* ---- dtype gate (lines 750-751) ---- */
    if(!(ckc_g950a2d_streq(spec->dtype, "fp16") || ckc_g950a2d_streq(spec->dtype, "bf16")))
    {
        b->status = CKC_ERR_NOTIMPL;
    ...
    const int SOFTMAX_STATE_SLOTS =
        (USE_MFMA_32X32 && ctx->TRANSPOSED_QK_32X32 && ctx->TRANSPOSED_SCALAR_STATE) ? 1
                                                                                     : REGS_PER_LANE;
    ...
    const bool Q_ALIAS_K =
        ckc_type_eq(K_LDS_DTYPE, dtype) && (Q_BYTES <= K_TOTAL_BYTES);
    ...
    ctx->pv_tr_reader =
        ckc_transpose_lds_reader_bind(b, &ctx->pv_tr_reader_desc, ctx->lane);
...
static ckc_value_t* ckc_g950a2d_lane_rg(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
...
ckc_value_t* ckc_gfx950_attn2d_in_warp_row(ckc_gfx950_attn2d_build_ctx_t* ctx, int r)
{
    ...
    ckc_value_t* rg4 = ckc_b_mul(ctx->b, ckc_g950a2d_lane_rg(ctx), ckc_b_const_i32(ctx->b, 4));
    return ckc_b_add(ctx->b, rg4, ckc_b_const_i32(ctx->b, atom_idx * 16 + in_atom));
}
...
ckc_value_t* ckc_gfx950_attn2d_state_row(ckc_gfx950_attn2d_build_ctx_t* ctx, int r)
{
    if(ctx->USE_MFMA_32X32 && !ctx->TRANSPOSED_QK_32X32)
        return ckc_gfx950_attention_tiled_2d_mfma_32x32_c_row(ctx->b, ctx->lane, r);
    return ckc_gfx950_attn2d_in_warp_row(ctx, r);
}
                                              ...
                                              ckc_value_t* v3)
{
    ...
    ckc_value_t* is0 = ctx->lane_rg_is0_v;
    ckc_value_t* is1 = ctx->lane_rg_is1_v;
    ckc_value_t* is2 = ctx->lane_rg_is2_v;
...
ckc_value_t* ckc_gfx950_attn2d_acc_get(ckc_gfx950_attn2d_build_ctx_t* ctx, int n, int atom)
{
    ...
    return ctx->acc_cur[ckc_gfx950_attn2d_acc_idx(ctx, n, atom)];
}
...
ckc_value_t* ckc_gfx950_attn2d_acc_final_get(ckc_gfx950_attn2d_build_ctx_t* ctx, int n, int atom)
{
    ...
    return ctx->acc_final[ckc_gfx950_attn2d_acc_idx(ctx, n, atom)];
}
