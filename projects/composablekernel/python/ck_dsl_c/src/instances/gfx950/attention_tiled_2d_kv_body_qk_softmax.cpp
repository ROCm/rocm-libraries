...
static ckc_value_t* fh_warp_xor_reduce_max(ckc_gfx950_attn2d_build_ctx_t* ctx, ckc_value_t* v)
{
    ckc_value_t* cur = v;
    for(int k = 0; k < 4; ++k)
    {
...
static ckc_value_t* fh_warp_xor_reduce_sum(ckc_gfx950_attn2d_build_ctx_t* ctx, ckc_value_t* v)
{
...
__attribute__((unused)) static ckc_value_t* fh_mfma_16x16x16(ckc_gfx950_attn2d_build_ctx_t* ctx,
                                                             ckc_value_t* a,
                                                             ckc_value_t* bv,
                                                             ckc_value_t* c)
{
...
static ckc_value_t* fh_apply_softcap(ckc_gfx950_attn2d_build_ctx_t* ctx, ckc_value_t* s)
{
...
static ckc_value_t*
    p_regs_f32_buf[CKC_GFX950_ATTN2D_MAX_REGS_PER_LANE * CKC_GFX950_ATTN2D_MAX_N_TILES];
...
void ckc_gfx950_attn2d_emit_kv_body(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    ...
    else if(ctx->USE_MFMA_32X32)
    {
        ...
        ckc_value_t* lane_half_qg = (ctx->lane_half_qg_v != NULL) ? ctx->lane_half_qg_v : lane_half;
        ...
        /* per-row max via 32-lane butterfly + online update (Python 3043-3054). */
        {
            ...
            for(int reg = 0; reg < REGS_PER_LANE; ++reg)
            {
                ...
                for(int n = 0; n < QK_N_TILES; ++n)
                {
                    ckc_value_t* p = ckc_b_exp2(b, ckc_b_fsub(b, s_local[reg][n], m_new_out[reg]));
    ...
    else
    {
        ...
        ckc_value_t* m_new[CKC_GFX950_ATTN2D_MAX_REGS_PER_LANE];
        ckc_value_t* s_local[CKC_GFX950_ATTN2D_MAX_REGS_PER_LANE][CKC_GFX950_ATTN2D_MAX_N_TILES];
        for(int reg = 0; reg < REGS_PER_LANE; ++reg)
        {
            ckc_value_t* local_max = neg_inf;
            for(int n = 0; n < QK_N_TILES; ++n)
