                                           ...
                                           float mf)
{
    ...
    conv = ckc_conv_problem_make(n, h, w, c, k0, r, s,
                                 /*sH*/ 1, /*sW*/ 1,
                                 /*pH*/ 1, /*pW*/ 1,
                                 /*dH*/ 1, /*dW*/ 1);

    /* problem = FusedConvPoolProblem(conv=conv, conv1_k=k1)  (pool defaults). */
    spec.problem = ckc_fused_conv_pool_problem_make(conv, /*conv1_k*/ k1,
                                                    /*pool_y*/ 2, /*pool_x*/ 2,
                                                    /*pool_stride_h*/ 2,
                                                    /*pool_stride_w*/ 2);
    ...
    const ckc_gfx1151_deep_fused_conv_pool_spec_t* spec, char* out, size_t out_cap)
{
    ...
    /* problem.short() */
    st = ckc_fused_conv_pool_problem_short(&spec->problem, short_buf,
                                           sizeof(short_buf), NULL);
    if (st != CKC_OK)
    {
        return st;
    }
    ...
    snprintf(pt_buf, sizeof(pt_buf), "pt%dx%d", spec->pool_tile_h,
             spec->pool_tile_w);
    ...
    if (spec->persistent)
    {
        snprintf(persist_buf, sizeof(persist_buf), "persist%d",
                 spec->persistent_ctas);
        parts[np++] = persist_buf;
    }
    parts[np++] = "i8i4_realquant";

    /* kernel_name_join(name, *parts): prefix = spec->name, no flags. */
    return ckc_kernel_name_join(spec->name, parts, np,
                                /*flag_names*/ NULL, /*flag_on*/ NULL,
                                /*num_flags*/ 0, out, out_cap, NULL);
}
    ...
    size_t reason_cap)
{
    ...
    if (!ckc_mma_catalog_has_shape(&target->mma, "wmma", "f16", "f16", "fp32",
                                   GFX1151_WMMA, GFX1151_WMMA, GFX1151_WMMA))
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap, "WMMA 16x16x16 f16 atom absent on %s",
                     arch);
        }
        return false;
    }
    ...
    if (target->wave_size != GFX1151_WAVE)
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap, "this kernel is wave32; %s is wave%d",
                     arch, target->wave_size);
        }
        return false;
    }
    ...
    if (c->N != 1)
    {
        g1151_set_reasonf(reason, reason_cap,
                          "tiled schedule supports only N=1 (got N=%d)", c->N);
        return false;
    }
    ...
    {
        g1151_set_reason(reason, reason_cap,
    ...
    if (spec->tile_m != conv_tile_h * conv_tile_w)
    {
        ...
        {
            snprintf(reason, reason_cap,
                     "tile_m=%d must equal conv tile %dx%d=%d", spec->tile_m,
                     conv_tile_h, conv_tile_w, conv_tile_h * conv_tile_w);
        }
        return false;
    }
    ...
    if (k0 > spec->tile_n)
    {
        ...
        {
            snprintf(reason, reason_cap,
                     "one CTA owns all conv0 channels: K0=%d > tile_n=%d", k0,
                     spec->tile_n);
        }
        return false;
    }
    ...
    {
        g1151_set_reasonf(reason, reason_cap,
                          "conv0 channels K0=%d must be a multiple of 16 (conv1 K)",
                          c->K);
        return false;
    }
    ...
    if (!spec->direct_conv0)
    {
        ...
        {
            g1151_set_reason(reason, reason_cap,
                             "tile_m*kpad must divide block_size (A0 staging is untailed)");
            return false;
        }
    }
    ...
    if (spec->sched_policy == NULL ||
        (strcmp(spec->sched_policy, "mem") != 0 &&
         strcmp(spec->sched_policy, "compv3") != 0 &&
         strcmp(spec->sched_policy, "compv4") != 0 &&
         strcmp(spec->sched_policy, "intrawave") != 0))
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap, "unknown sched_policy '%s'",
    ...
    if (spec->native_int)
    {
        if (ckc_mma_catalog_by_op_id(&target->mma, GFX1151_OP_ID_IU8) == NULL)
        {
            if (reason != NULL && reason_cap > 0)
            {
                snprintf(reason, reason_cap, "%s atom absent on %s",
                         GFX1151_OP_ID_IU8, arch);
            }
            return false;
        }
        ...
        {
            g1151_set_reason(reason, reason_cap,
                             "native_int is incompatible with butterfly_conv01");
            return false;
        }
    }
    ...
    {
        g1151_set_reason(reason, reason_cap,
                         "interior_fastpath is only implemented for native direct conv0");
        return false;
    }
    ...
    if (spec->static_direct_kmap)
    {
        ...
        {
            g1151_set_reason(reason, reason_cap,
                             "static_direct_kmap assumes C=8 and R=S=3");
            return false;
        }
    }
    ...
    if (spec->packed_c0_handoff)
    {
        ...
        {
            g1151_set_reason(reason, reason_cap,
                             "packed_c0_handoff is only implemented for native_int");
            return false;
        }
        if (c->K % 2)
        {
            g1151_set_reason(reason, reason_cap,
                             "packed_c0_handoff requires even conv0 channels");
            return false;
        }
    }
    ...
    if (spec->repack_c0)
    {
        ...
        {
            g1151_set_reason(reason, reason_cap,
                             "repack_c0 is only implemented for native_int");
            return false;
        }
        if (spec->packed_c0_handoff)
        {
            g1151_set_reason(reason, reason_cap,
                             "repack_c0 and packed_c0_handoff are mutually exclusive");
            return false;
        }
        if (c->K % 2)
        {
            g1151_set_reason(reason, reason_cap,
                             "repack_c0 requires even conv0 channels");
            return false;
        }
    }
    ...
    if (spec->fused_c0a1)
    {
        ...
        {
            g1151_set_reason(reason, reason_cap,
                             "fused_c0a1 requires conv0 K0 divisible by 16");
            return false;
        }
    }
 ...
 * ===================================================================== */
ckc_kernel_def_t*
ckc_build_gfx1151_deep_fused_conv_pool(ckc_ir_builder_t* b,
                                       const ckc_gfx1151_deep_fused_conv_pool_spec_t* spec,
                                       const char* arch)
{
...
ckc_kernel_def_t*
ckc_build_gfx1151_deep_fused_conv_pool_new(ckc_ir_builder_t* b,
                                           const ckc_gfx1151_deep_fused_conv_pool_spec_t* spec,
                                           const char* arch)
{
    char name[256];
    ...
    if (ckc_gfx1151_deep_fused_conv_pool_kernel_name(spec, name, sizeof(name)) !=
        CKC_OK)
    {
        return NULL;
    }
