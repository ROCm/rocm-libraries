 ...
 *   ck_dsl/instances/gfx1151/deep_fused_conv_pool.py :: build_deep_fused_conv_pool
 ...
 * (ckc_gfx1151_dfcp_build_ctx_t): the ctx prologue (driver) has already resolved
 ...
 *
 *   ckc_gfx1151_dfcp_emit_persistent_body  <- Python 2780-2876
 ...
 *   ckc_gfx1151_dfcp_emit_single_tile_body <- Python 2878-3085
 ...
 * ckc_gfx1151_dfcp_code_fn_t selector enum: the scatter / fuse phases take the
...
#include "ckc/ir.h" /* ckc_b_* scaffolding (const_i32, scf_for, readfirstlane, div, mod) */
 ...
 *  CKC_GFX1151_DFCP_CODE_CONV0_I8 / CKC_GFX1151_DFCP_CODE_CONV1_I8 for the fuse /
...
void ckc_gfx1151_dfcp_emit_persistent_body(ckc_gfx1151_dfcp_build_ctx_t* ctx)
{
     ...
     *   b.sync() */
    ckc_gfx1151_dfcp_stage_conv0_w0_int(ctx, ctx->W0, ctx->w0_smem);
    if(spec->conv1_int8)
        ckc_gfx1151_dfcp_stage_conv1_w1_i8(ctx, ctx->W1, ctx->w1_smem);
    else
        ckc_gfx1151_dfcp_stage_conv1_w1_packed(ctx, ctx->W1, ctx->w1_smem);
    ckc_b_sync(b);
    ...
    loop = ckc_b_scf_for(b,
                         ckc_b_block_id_x(b),
                         ckc_b_const_i32(b, num_tiles),
                         ckc_b_const_i32(b, spec->persistent_ctas),
                         "tile_idx");
    ...
    ckc_b_region_enter(b, loop.body);
    {
        ...
        ckc_gfx1151_dfcp_stage_input_footprint_int(
            ctx, ctx->X, ctx->a0_smem, ctx->h_blk, ctx->w_blk);
        ckc_b_sync(b); /* footprint store -> conv0 GEMM read of a0_smem */

        /* conv0: direct native-int iu8 GEMM, reorient=True for the in-register
         * fused_c0a1 handoff (acc[k0,m]). */
        ctx->num_accs0 = 0;
        ckc_gfx1151_dfcp_wmma_gemm_conv0_direct_int(ctx,
                                                    ctx->op0,
                                                    ctx->a0_smem,
                                                    ctx->w0_smem,
                                                    ctx->policy,
        ...
        if(spec->conv1_int8)
        {
            ...
            ckc_gfx1151_dfcp_fuse_c0_to_conv1_a_regs(ctx,
                                                     ctx->op0,
                                                     ctx->accs0,
                                                     ctx->num_accs0,
                                                     CKC_GFX1151_DFCP_CODE_CONV0_I8,
                                                     /*int8=*/true,
                                                     ctx->a_frags,
                                                     &ctx->num_a_frags,
                                                     &ctx->a_frags_rows,
                                                     &ctx->a_frags_cols);
            ckc_gfx1151_dfcp_wmma_gemm_conv1_i8_from_regs(ctx,
                                                          ctx->op0,
                                                          ctx->a_frags,
                                                          ctx->a_frags_rows,
                                                          ctx->a_frags_cols,
        ...
        else
        {
            ...
            ckc_gfx1151_dfcp_fuse_c0_to_conv1_a_regs(ctx,
                                                     ...
                                                     &ctx->a_frags_rows,
                                                     &ctx->a_frags_cols);
            ckc_gfx1151_dfcp_wmma_gemm_conv1_i4_from_regs(ctx,
                                                          ctx->op1,
                                                          ctx->a_frags,
                                                          ctx->a_frags_rows,
                                                          ctx->a_frags_cols,
        ...
        ckc_gfx1151_dfcp_scatter_codes_to_i8_lds(ctx,
                                                 ctx->op1,
                                                 ctx->accs1,
                                                 ctx->num_accs1,
                                                 ctx->c1_smem,
                                                 CKC_GFX1151_DFCP_CODE_CONV1_I8);
        ckc_b_sync(b);
        ckc_gfx1151_dfcp_emit_maxpool_finalpack_i8(
...
void ckc_gfx1151_dfcp_emit_single_tile_body(ckc_gfx1151_dfcp_build_ctx_t* ctx)
{
    ...
    if(spec->native_int)
    {
        conv0_code = CKC_GFX1151_DFCP_CODE_CONV0_I8;
        conv1_code = CKC_GFX1151_DFCP_CODE_CONV1_I8;
    }
    else
    {
        conv0_code = CKC_GFX1151_DFCP_CODE_CONV0_F16;
        conv1_code = CKC_GFX1151_DFCP_CODE_CONV1_F16;
    }
    conv0_code_vec = CKC_GFX1151_DFCP_CODE_CONV0_VEC_I8;
    conv1_code_vec = CKC_GFX1151_DFCP_CODE_CONV1_VEC_I8;
    ...
    if(spec->native_int && spec->direct_conv0)
    {
        ...
        ckc_gfx1151_dfcp_stage_input_footprint_int(ctx,
                                                   ctx->X,
                                                   ctx->a0_smem,
                                                   /*h_blk=*/NULL,
                                                   /*w_blk=*/NULL);
    ...
    else if(spec->native_int)
    {
        ...
        ckc_gfx1151_dfcp_stage_conv0_a_int(ctx, ctx->X, ctx->a0_smem);
        ckc_gfx1151_dfcp_stage_conv0_w0_int(ctx, ctx->W0, ctx->w0_smem);
        ckc_b_sync(b);
        ckc_gfx1151_dfcp_wmma_gemm_from_lds_int(ctx,
                                                ctx->op0,
                                                ctx->a0_smem,
                                                ctx->w0_smem,
                                                ctx->kpad,
    ...
    else if(spec->direct_conv0)
    {
        ...
        ckc_gfx1151_dfcp_stage_input_footprint(ctx, ctx->X, ctx->a0_smem);
        ckc_gfx1151_dfcp_stage_conv0_w0(ctx, ctx->W0, ctx->w0_smem);
        ckc_b_sync(b);
        ckc_gfx1151_dfcp_wmma_gemm_conv0_direct(
    ...
    else
    {
        ...
        ckc_gfx1151_dfcp_stage_conv0_a(ctx, ctx->X, ctx->a0_smem);
        ckc_gfx1151_dfcp_stage_conv0_w0(ctx, ctx->W0, ctx->w0_smem);
        ckc_b_sync(b);
        ckc_gfx1151_dfcp_wmma_gemm_from_lds(ctx,
                                            ctx->op,
                                            ctx->a0_smem,
                                            ctx->w0_smem,
                                            ctx->kpad,
    ...
    else if(spec->native_int)
    {
        ...
        if(spec->repack_c0)
        {
            ckc_gfx1151_dfcp_repack_c0_lds_to_packed(ctx, ctx->c0_smem, ctx->c0_packed_smem);
            ckc_b_sync(b);
            ckc_gfx1151_dfcp_wmma_gemm_conv1_i4_packed_from_lds(ctx,
                                                                ctx->op1,
                                                                ctx->c0_packed_smem,
                                                                ctx->w1_smem,
                                                                c->K,
        ...
        {
            ckc_gfx1151_dfcp_wmma_gemm_conv1_i4_from_lds(ctx,
                                                         ctx->op1,
                                                         ctx->c0_smem,
                                                         ctx->w1_smem,
                                                         c->K,
    ...
    else
    {
        ...
        if(spec->early_w1)
        {
            ckc_gfx1151_dfcp_stage_conv1_w1(ctx, ctx->W1, ctx->w1_smem);
            ckc_gfx1151_dfcp_scatter_codes_to_lds(
                ctx, ctx->op0, ctx->accs0, ctx->num_accs0, ctx->c0_smem, conv0_code);
            ckc_b_sync(b);
        }
        else
    ...
    ctx->c_m1 = ckc_b_const_f32(b, (double)spec->m1);
     ...
     *      (Python 3079-3083) ---- */
    if(spec->native_int)
        ckc_gfx1151_dfcp_emit_maxpool_finalpack_i8(ctx,
                                                   ctx->c1_smem,
                                                   ctx->Y,
                                                   /*h_blk=*/NULL,
                                                   /*w_blk=*/NULL);
    else
        ckc_gfx1151_dfcp_emit_maxpool_finalquant(ctx, ctx->c1_smem, ctx->Y);
}
