 ...
 * ck_dsl/instances/common/deep_fused_conv_pool.py lines 1334-1375), driven here
 * by the WMMA-resolved op.
 *
 * SCOPE OF THIS TRANSLATION UNIT.
 *   Exactly one phase function: ckc_gfx1201_dfcp_epilogue_override -- the fused
 ...
 *       maxpool (stage conv1 accs to LDS + ckc_dfcp_emit_inline_maxpool_from_
 *       cshuffle).
 *
 ...
 *   ckc_gfx1201_dfcp_build_ctx_init).
 *
 ...
 *
 * Peers (ckc_gfx1201_dfcp_build_ctx_init, the other closures) live in sibling
                                        ...
                                        ckc_value_t* w1_rsrc)
{
    ...
    ctx->num_conv0_accs = (num_accs < (size_t)CKC_GFX1201_DFCP_MAX_ACCS)
                              ? num_accs
                              : (size_t)CKC_GFX1201_DFCP_MAX_ACCS;
     ...
     * the one ckc_gfx1201_dfcp_build_ctx_init computed from spec.conv1_epilogue. */
    defer = ctx->defer;

    st = ckc_dfcp_emit_conv1_1x1(b, spec, conv_spec_, op, ctx->c_smem,
                                 ctx->w1_smem, grid, /*defer_epilogue=*/defer,
                                 ctx->conv1_accs, (size_t)CKC_GFX1201_DFCP_MAX_ACCS,
                                 &ctx->num_conv1_accs);
    if (st != CKC_OK)
    {
