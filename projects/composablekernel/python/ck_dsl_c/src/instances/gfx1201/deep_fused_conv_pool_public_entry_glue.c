 ...
 * ===================================================================== */
ckc_status_t
ckc_gfx1201_dfcp_build_ctx_init(ckc_gfx1201_dfcp_build_ctx_t* ctx,
                                ckc_ir_builder_t* b,
...
static void* ckc_gfx1201_dfcp_tramp_input_cache_setup(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    const ckc_warp_grid_t* grid,
    ckc_value_t* a_rsrc,
    void* user)
{
    ...
    return (void*)ckc_gfx1201_dfcp_setup_specialized_a_loader(ctx, spec, grid,
                                                              a_rsrc);
}

static void ckc_gfx1201_dfcp_tramp_a_load_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* k_off,
    ckc_value_t* a_dst,
    ...
    void* user)
{
    ...
    if (ctx->use_input_cache)
    {
        ckc_gfx1201_dfcp_load_a_tile_from_cache(ctx, spec, k_off, a_dst, grid,
                                                cache);
    }
    else
    {
        ckc_gfx1201_dfcp_load_a_tile_specialized(ctx, spec, k_off, a_dst, grid,
                                                 cache);
    }
}

static ckc_value_t* ckc_gfx1201_dfcp_tramp_a_operand_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* a_row,
    ckc_value_t* k_off,
    ...
    void* user)
{
    ...
    ckc_value_t* cache = (ckc_value_t*)input_cache_context;
    (void)b;
    return ckc_gfx1201_dfcp_load_a_operand_from_cache(ctx, spec, a_row, k_off,
                                                      col_base, a_per_lane, grid,
                                                      cache);
}

static void ckc_gfx1201_dfcp_tramp_epilogue_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* const* accs,
    int num_accs,
    ...
    void* user)
{
    ...
    (void)b;
    ckc_gfx1201_dfcp_epilogue_override(ctx, spec, accs,
...
ckc_kernel_def_t*
ckc_build_gfx1201_deep_fused_conv_pool_new(ckc_ir_builder_t* b,
                                           const ckc_gfx1201_deep_fused_conv_pool_spec_t* spec,
                                           const char* arch)
{
    char name[256];
    ...
    if (ckc_gfx1201_deep_fused_conv_pool_kernel_name(spec, name, sizeof(name)) !=
        CKC_OK)
    {
        return NULL;
    }
