 ...
 *   ckc_conv_apply_accumulator_epilogue   <- _apply_accumulator_epilogue  (647-689)
 *   ckc_conv_emit_epilogue                <- epilogue dispatcher          (1349-1377)
 *   ckc_conv_emit_direct_epilogue         <- _emit_direct_epilogue        (1386-1414)
 *   ckc_conv_emit_direct_epilogue_wmma    <- _emit_direct_epilogue_wmma   (1417-1478)
 *   ckc_conv_emit_cshuffle_epilogue       <- _emit_cshuffle_epilogue      (1481-1523)
 *
...
/* ===================================================================== *
 * ckc_conv_apply_accumulator_epilogue   (Python lines 647-689)
 *
                                         ...
                                         ckc_value_t** out_accs)
{
    ...
    ckc_value_t* c_bias;
    ckc_value_t* c_scale;
    ckc_value_t* c_clamp_min;
    ckc_value_t* c_clamp_max;

    /* if epilogue.is_identity(): return list(accs) */
    if (ckc_conv_acc_epilogue_is_identity(epilogue))
    {
    ...
    for (a = 0; a < num_accs; ++a)
    {
        ...
        for (i = 0; i < count; ++i)
        {
            ...
            if (c_bias != NULL)
                v = ckc_b_fadd(b, v, c_bias);
            if (c_scale != NULL)
                v = ckc_b_fmul(b, v, c_scale);
            if (epilogue->relu)
                v = ckc_b_fmax(b, v, c_zero);
            if (c_clamp_min != NULL)
                v = ckc_b_fmax(b, v, c_clamp_min);
            if (c_clamp_max != NULL)
                v = ckc_b_fmin(b, v, c_clamp_max);
            elems[i] = v;
        }
        /* out.append(b.vec_pack(elems, elems[0].type)) */
        out_accs[a] = ckc_b_vec_pack(b, elems, count, elems[0]->type);
    }
}
 ...
 *
 * The ckc_epilogue_addr_fn contract returns off_elements directly and writes
 * the optional i1 validity through *out_valid. `user` carries the bound
 * D descriptor. */
static ckc_value_t* ckc_conv_d_addr(ckc_ir_builder_t* b,
                                    ...
                                    void* user)
{
    ...
    ckc_transforms_descriptor_offset(b, D_desc, names, values, 2, &off, &valid);
    if (out_valid != NULL)
        *out_valid = valid;
    return off;
}
...
/* ===================================================================== *
 * ckc_conv_emit_direct_epilogue   (Python lines 1386-1414)
 *
 ...
 * ===================================================================== */
void ckc_conv_emit_direct_epilogue(ckc_ir_builder_t* b,
                                   ...
                                   ckc_value_t* d_rsrc)
{
    ...
    epi.atom = ckc_mfma_atom("f16", spec->warp_tile_m, spec->warp_tile_n,
                             spec->warp_tile_k);
    epi.grid = *grid;

    {
        /* hoist bounds in Python's left-to-right order: M first, then N_gemm */
        ckc_value_t* bound_m = ckc_b_const_i32(b, ckc_conv_problem_m(p));
        ckc_value_t* bound_n = ckc_b_const_i32(b, ckc_conv_problem_n_gemm(p));
        ckc_direct_epilogue_store(b,
                                  &epi,
                                  accs,
                                  num_accs,
                                  ckc_conv_d_addr,
...
/* ===================================================================== *
 * ckc_conv_emit_direct_epilogue_wmma   (Python lines 1417-1478)
 *
 ...
 * ===================================================================== */
void ckc_conv_emit_direct_epilogue_wmma(ckc_ir_builder_t* b,
                                        const ckc_implicit_gemm_conv_spec_t* spec,
                                        ...
                                        ckc_value_t* c0)
{
    ...
    ckc_value_t* c_N = ckc_b_const_i32(b, ckc_conv_problem_n_gemm(p));
    /* D_desc = make_d_descriptor(p) */
    ckc_tensor_descriptor_t* D_desc = ckc_conv_make_d_descriptor(b, p);
    /* c_map = op.c_layout() */
    const ckc_arch_layout_map_t* c_map = ckc_mmaop_c_layout(op, b);
    ...
    for (mi = 0; mi < mfmas_m; ++mi)
    {
        for (ni = 0; ni < mfmas_n; ++ni)
        {
            ...
            for (i = 0; i < op->c_frag_len; ++i)
            {
                ...
                ckc_value_t* v_f32;
                ckc_value_t* v_f16;
                ckc_value_t* d_off_elems = NULL;
                ckc_value_t* d_off_bytes;
                ckc_value_t* safe_off;

                /* row_off, col_off = c_map.coord(b, lane, i) */
                ckc_arch_layout_map_coord(c_map, b, lane, i, &row_off, &col_off);
                ...
                m_ok = ckc_b_cmp_lt(b, m_val, c_M);
                n_ok = ckc_b_cmp_lt(b, n_val, c_N);
                /* ok = b.land(m_ok, n_ok) */
                ok = ckc_b_land(b, m_ok, n_ok);
                ...
                /* d_off_elems, _ = D_desc.offset(b, m=m_val, k_out=n_val) */
                {
                    ...
                    values[0] = m_val;
                    values[1] = n_val;
                    ckc_transforms_descriptor_offset(b, D_desc, names, values, 2,
                                                     &d_off_elems, &valid);
                }
                /* d_off_bytes = b.mul(d_off_elems, b.const_i32(2)) */
                d_off_bytes = ckc_b_mul(b, d_off_elems, ckc_b_const_i32(b, 2));
                /* safe_off = b.select(ok, d_off_bytes, b.const_i32((1<<31)-1)) */
                safe_off = ckc_b_select(b, ok, d_off_bytes,
                                        ckc_b_const_i32(b, (int64_t) ((1u << 31) - 1u)));
                /* b.buffer_store_f16(d_rsrc, safe_off, c0, v_f16) */
                ckc_b_buffer_store_f16(b, d_rsrc, safe_off, c0, v_f16);
            }
        }
    }
}
...
/* ===================================================================== *
 * ckc_conv_emit_cshuffle_epilogue   (Python lines 1481-1523)
 *
 ...
 * ===================================================================== */
void ckc_conv_emit_cshuffle_epilogue(ckc_ir_builder_t* b,
                                     const ckc_implicit_gemm_conv_spec_t* spec,
                                     ckc_value_t* const* accs,
                                     int num_accs,
                                     const ckc_warp_grid_t* grid,
...
/* ===================================================================== *
 * ckc_conv_emit_epilogue   (Python lines 1349-1377)
 *
...
void ckc_conv_emit_epilogue(ckc_conv_build_ctx_t* ctx)
{
    ...
    ckc_value_t* final_accs[CKC_CONV_MAX_ACCS];
    ckc_conv_apply_accumulator_epilogue(b, &spec->acc_epilogue, ctx->final_accs,
                                        num_accs, final_accs);
    ...
    {
        ckc_conv_emit_cshuffle_epilogue(b, spec, final_accs, num_accs, &ctx->grid,
                                        ctx->d_rsrc);
    }
    /* elif op.family == "wmma": */
    else if (ctx->op != NULL && ctx->op->family != NULL &&
             strcmp(ctx->op->family, "wmma") == 0)
    {
        ckc_conv_emit_direct_epilogue_wmma(b,
                                           spec,
                                           ctx->op,
                                           final_accs,
                                           num_accs,
    ...
    {
        ckc_conv_emit_direct_epilogue(b, spec, final_accs, num_accs, &ctx->grid,
                                      ctx->d_rsrc);
    }
}
