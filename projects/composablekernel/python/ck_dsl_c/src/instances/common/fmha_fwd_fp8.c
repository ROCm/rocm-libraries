...
ckc_status_t ckc_fmha_fwd_fp8_kernel_name(const ckc_fmha_fwd_fp8_spec_t* spec,
                                          char* out,
                                          size_t out_cap)
{
    char hbuf[32];
                                    ...
                                    size_t reason_cap)
{
    ...
    {
        fp8_fwd_set_reason(reason, reason_cap,
    ...
    if (!ckc_mma_catalog_has_shape(&target->mma, NULL, "f16", "f16", "fp32",
                                   CKC_MFMA_ATTN_BLOCK_M, CKC_MFMA_ATTN_BLOCK_M,
    ...
    if (!ckc_arch_fits_lds(target, bytes_lds))
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap, "LDS budget %ld > %d cap on %s",
                     bytes_lds, target->lds_capacity_bytes, arch);
        }
        return false;
    }
                                   ...
                                   const ckc_fmha_fwd_fp8_spec_t* spec)
{
    ...
    ckc_fmha_kernel_builder_add_tensor(kb, "K", kv, /*readonly*/ true,
                                       /*writeonly*/ false, /*align*/ 8);
    ckc_fmha_kernel_builder_add_tensor(kb, "V", kv, /*readonly*/ true,
                                       /*writeonly*/ false, /*align*/ 8);
                                         ...
                                         const char* arch)
{
    ...
    /* kb.decode_grid() */
    ckc_fmha_kernel_builder_decode_grid(kb, /*num_queries_per_kv=None*/ -1,
    ...
    if (spec->has_waves_per_eu)
    {
        ...
        if (kernel != NULL)
        {
            ckc_attr_set_int(b, &kernel->attrs, "waves_per_eu",
                             (int64_t)spec->waves_per_eu);
        }
    }
                                        ...
                                        size_t* out_count)
{
    ...
    st = ckc_fmha_kernel_builder_init(&kb, "ck_dsl_fmha_fwd_fp8_sig_probe",
                                      &spec->common);
    if (st != CKC_OK)
    {
        return st;
