...
ckc_fmha_shape_t ckc_fmha_shape_make(int head_size,
                                     int num_query_heads,
                                     int num_kv_heads,
                                     int block_size_q,
                                     int block_size_k)
...
ckc_fmha_shape_t ckc_fmha_shape_default(int head_size,
                                        int num_query_heads,
                                        int num_kv_heads)
{
                                   ...
                                   const char** out_reason)
{
    ...
    if (s->head_size <= 0 || s->head_size > 256)
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false,
            ckc_arena_printf(arena, "head_size %d out of supported range (1..256)",
                             s->head_size),
            "head_size out of supported range (1..256)");
    }
    /* if s.head_size not in (32, 64, 128, 192, 256): */
    if (s->head_size != 32 && s->head_size != 64 && s->head_size != 128 &&
        s->head_size != 192 && s->head_size != 256)
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false,
        ...
        s->block_size_q != 128)
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false,
            ckc_arena_printf(arena, "block_size_q %d not in {16, 32, 64, 128}",
                             s->block_size_q),
        ...
        s->block_size_k != 128 && s->block_size_k != 256)
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false,
            ckc_arena_printf(arena, "block_size_k %d not in {16..256}",
                             s->block_size_k),
    ...
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false, NULL, "mask_mode not recognised");
    }
    ...
    if (spec->mask_mode == CKC_FMHA_MASK_SLIDING_WINDOW && spec->sliding_window <= 0)
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false,
            ckc_arena_printf(arena,
                             "sliding_window mask requires window > 0 (got %d)",
                             spec->sliding_window),
    ...
    {
        return ckc_i_fmha_set_reason(
            arena, out_reason, false, NULL,
            "use_sinks=True only supports f16 in v1");
    }
    /* return True, "ok" */
    if (out_reason != NULL)
    {
 ...
 * on success, 0 on OOM. */
static int ckc_i_reserve(ckc_arena_t* arena,
                         void** arr,
                         size_t* cap,
                         size_t count,
                         size_t elem_size)
                                   ...
                                   const char* dtype)
{
    ...
    if (!ckc_i_reserve(arena, (void**)&kb->sig_order, &kb->cap_sig_order,
                       kb->n_sig_order, sizeof(ckc_fmha_sig_order_entry_t)))
    {
        return;
    }
...
static ckc_value_t* ckc_i_named_lookup(const ckc_fmha_named_value_t* arr,
                                       size_t n,
                                       const char* name)
{
    size_t i;
                                                ...
                                                int align)
{
    ...
    ckc_i_named_append(&kb->b.arena, &kb->tensor_params, &kb->n_tensor_params,
                       &kb->cap_tensor_params, name, p);
                                             ...
                                             int align)
{
    ...
    ckc_i_named_append(&kb->b.arena, &kb->other_params, &kb->n_other_params,
                       &kb->cap_other_params, name, p);
                                         ...
                                         size_t n)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        if (ckc_i_reserve(arena, (void**)&kb->stride_params, &kb->cap_stride_params,
                          kb->n_stride_params, sizeof(ckc_fmha_stride_pair_t)))
        {
        ...
        ckc_i_named_append(arena, &kb->other_params, &kb->n_other_params,
                           &kb->cap_other_params, sn_token, tok);
        ckc_i_named_append(arena, &kb->other_params, &kb->n_other_params,
                           &kb->cap_other_params, sn_head, hd);
...
ckc_value_t* ckc_fmha_kernel_builder_tensor(const ckc_fmha_kernel_builder_t* kb,
                                            const char* name)
{
...
ckc_value_t* ckc_fmha_kernel_builder_scalar(const ckc_fmha_kernel_builder_t* kb,
                                            const char* name)
{
    if (kb == NULL || name == NULL)
    {
...
ckc_value_t* ckc_fmha_kernel_builder_ptr(const ckc_fmha_kernel_builder_t* kb,
                                         const char* name)
{
    if (kb == NULL || name == NULL)
    {
                                         ...
                                         ckc_value_t** out_kv_head_idx)
{
    ...
    else
    {
        ...
        kb->kv_head_idx = ckc_b_to_sgpr_u32(
            b, ckc_b_div(b, kb->head_idx, ckc_b_const_i32(b, nqkv)));
    }
    ...
    int n_lengths)
{
    ...
    coords = (coord_names != NULL) ? (const char**)coord_names
                                   : (const char**)default_coords;
    ...
    if (sp == NULL)
    {
        ...
        return ckc_tensor_descriptor_naive(&kb->b, tensor_name, lens_ptr, n_lengths,
                                           NULL /* strides */, coords, 3);
    }
     ...
     *   upper_names=coord_names) */
    {
        ...
        d = (ckc_tensor_descriptor_t*)ckc_arena_calloc(arena,
        ...
        for (i = 0; i < 3; ++i)
        {
            ...
            base_lengths[i] =
                (lens_ptr != NULL && i < n_lengths) ? lens_ptr[i] : 0;
        }
        /* base_strides=(0, 0, 1) */
        base_strides[0] = 0;
        base_strides[1] = 0;
...
ckc_value_t* ckc_fmha_kernel_builder_k_row_base(ckc_fmha_kernel_builder_t* kb,
                                                ckc_value_t* k_idx)
{
    if (kb == NULL)
    {
...
ckc_value_t* ckc_fmha_kernel_builder_v_row_base(ckc_fmha_kernel_builder_t* kb,
                                                ckc_value_t* k_idx)
{
    if (kb == NULL)
    {
