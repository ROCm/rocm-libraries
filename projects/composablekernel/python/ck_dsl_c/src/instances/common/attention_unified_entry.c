...
static const ckc_type_t* ckc__dtype_ir_2d3d(
...
static const ckc_type_t* ckc__dtype_ir_reduce(
    const ckc_unified_attention_problem_t* p)
{
    if (p->dtype && strcmp(p->dtype, "fp16") == 0)
        return ckc_f16();
...
bool ckc_unified_attention_supports_scalar(
    const ckc_unified_attention_problem_t* p,
    const char** out_reason)
{
    ...
    if (p->use_fp8 || p->q_dtype != NULL)
    {
        if (out_reason != NULL)
            *out_reason =
                "FP8 unified attention is not enabled in the scalar 2D path yet";
        return false;
    }
    if (p->use_alibi)
    {
    ...
    int num_segments)
{
    ...
    if (!ckc_attn_unified_ctx_init(&ctx, b, CKC_ATTN_UNIFIED_3D, p, dtype,
                                   num_segments))
        return NULL;
    ...
    const char* name)
{
    ...
    if (!ckc_attn_unified_ctx_init(&ctx, b, CKC_ATTN_UNIFIED_REDUCE, p, dtype,
                                   num_segments))
        return NULL;
    ...
    const char* name)
{
    return ckc::guard_builder(b, [&]() -> ckc_kernel_def_t* {
        ...
        if (b == NULL || p == NULL)
            return NULL;
        if (ckc_unified_attention_2d_scalar_kernel_name(p, name, kname,
                                                        sizeof(kname)) != CKC_OK)
            return NULL;
...
ckc_kernel_def_t* ckc_build_unified_attention_3d_scalar_new(
    ckc_ir_builder_t* b,
    const ckc_unified_attention_problem_t* p,
    const char* name,
    int num_segments)
{
    return ckc::guard_builder(b, [&]() -> ckc_kernel_def_t* {
        ...
        if (b == NULL || p == NULL)
            return NULL;
        if (ckc_unified_attention_3d_scalar_kernel_name(p, name, num_segments, kname,
                                                        sizeof(kname)) != CKC_OK)
            return NULL;
        if (ckc_ir_builder_init(b, kname) != CKC_OK)
            return NULL;
...
ckc_kernel_def_t* ckc_build_unified_attention_reduce_scalar_new(
    ckc_ir_builder_t* b,
    const ckc_unified_attention_problem_t* p,
    int num_segments,
    const char* name)
{
    return ckc::guard_builder(b, [&]() -> ckc_kernel_def_t* {
        ...
        if (b == NULL || p == NULL)
            return NULL;
        if (ckc_unified_attention_reduce_scalar_kernel_name(p, name, num_segments,
                                                            kname,
                                                            sizeof(kname)) != CKC_OK)
            return NULL;
        if (ckc_ir_builder_init(b, kname) != CKC_OK)
...
ckc_status_t ckc_unified_attention_2d_scalar_lower_to_llvm(
    const ckc_unified_attention_problem_t* p,
    const char* name,
...
ckc_status_t ckc_unified_attention_3d_scalar_lower_to_llvm(
    const ckc_unified_attention_problem_t* p,
    const char* name,
    int num_segments,
    ckc_llvm_flavor_t flavor,
...
ckc_status_t ckc_unified_attention_reduce_scalar_lower_to_llvm(
    const ckc_unified_attention_problem_t* p,
    int num_segments,
    const char* name,
    ckc_llvm_flavor_t flavor,
    ...
    size_t err_cap)
{
    ...
    kernel = ckc_build_unified_attention_reduce_scalar_new(&b, p, num_segments,
                                                           name);
    if (kernel == NULL)
    {
        st = ckc_ir_builder_status(&b);
