                                    ...
                                    size_t* out_len)
{
    ...
    if (p->is_3d)
    {
        written = snprintf(out, out_cap, "N%dD%dH%dW%dC%d_K%dZ%dY%dX%d", p->N, p->Di,
                           p->Hi, p->Wi, p->C, p->K, p->Z, p->Y, p->X);
    }
    else
    {
        written = snprintf(out, out_cap, "N%dH%dW%dC%d_K%dY%dX%d", p->N, p->Hi,
                           p->Wi, p->C, p->K, p->Y, p->X);
    }
