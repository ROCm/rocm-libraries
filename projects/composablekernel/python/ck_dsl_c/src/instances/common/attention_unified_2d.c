...
void ckc_attn_unified_emit_2d_softmax_loop(ckc_attn_unified_build_ctx_t* ctx)
{
    ...
    ckc_b_region_enter(b, loop.body);
    {
        ...
        (void)ckc_unified_attn_magic_div_mod(
            b, kpos, p->block_size, &block_idx, &token_in_block);
        ...
        ckc_value_t* physical = ckc_b_global_load_i32(
            ...
            ckc_b_add(b,
                      ckc_b_mul(b, ctx->seq_idx, ckc_b_const_i32(b, max_blocks_2d)),
                      block_idx),
            /*align=*/4);

        /* Vectorised QK dot product (vec8 main fold + scalar tail). */
        ckc_value_t* score = ctx->zero_f;
        ckc_value_t* q_off_base = ckc__q_offset(ctx, ctx->q_tok, ctx->q_head, ckc_b_const_i32(b, 0));
        ckc_value_t* k_off_base = ckc_unified_attn_paged_kv_offset(b,
                                                                   &ctx->kv_desc,
                                                                   physical,
                                                                   token_in_block,
                                                                   ctx->kv_head,
        ...
        for (d = (p->head_size / VEC) * VEC; d < p->head_size; ++d)
        {
            ...
            ckc_value_t* qv_s =
                ckc_b_cast_to_f32(b, ckc_b_global_load(b, ctx->abi_query, q_off, dtype, /*align=*/2));
            ckc_value_t* kv_s =
                ckc_b_cast_to_f32(b, ckc_b_global_load(b, ctx->abi_key, k_off, dtype, /*align=*/2));
        ...
        {
            ckc_value_t* dist =
                ckc_b_sub(b, ckc_b_add(b, ctx->context_len, ctx->query_pos), kpos);
...
void ckc_attn_unified_emit_2d_epilogue(ckc_attn_unified_build_ctx_t* ctx)
{
    ...
    ckc_value_t* valid = ckc_b_land(
        b, ctx->active, ckc_b_cmp_lt(b, ctx->dim, ckc_b_const_i32(b, p->head_size)));
