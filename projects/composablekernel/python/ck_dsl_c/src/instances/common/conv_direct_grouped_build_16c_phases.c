...
void ckc_dconv16c_load_weights(ckc_dconv_16c_ctx_t* ctx)
{
     ...
     *                        coord_names=("k_out","r","s","c")) */
    {
        ...
        lengths[3] = ctx->p.cpg;
        ctx->b_desc =
            ckc_tensor_descriptor_naive(b, "B", lengths, 4, NULL, coord_names, 4);
    }
    ...
    if (ctx->spec->fold_k32)
    {
        ...
        for (r_const = 0; r_const < ctx->p.KH; ++r_const)
        {
             ...
             *   s_lane_k32*cpg + ch_lane_k32 */
            {
                ...
                ckc_transforms_descriptor_offset(b, ctx->b_desc, in_names, in_values, 4,
                                                 &w_off_k32, &valid);
            }
...
void ckc_dconv16c_build_chunk_meta(ckc_dconv_16c_ctx_t* ctx)
{
    ...
    for (pass_idx = 0; pass_idx < ctx->PASSES; ++pass_idx)
    {
        ...
        n_out = ckc_tensor_descriptor_unmerge_lower(b, ctx->chunk_desc, in_names, in_values, 1,
...
void ckc_dconv16c_build_descriptors(ckc_dconv_16c_ctx_t* ctx)
{
    ...
    /* ---- input descriptor A[N,H,W,total_c] + 2 embeds ---- */
    {
        ...
        /* embed(upper=("y_iter",), into="h", strides=(1,), offset=-PAD, lo=0, hi=H) */
        {
            ...
            int h_strides[1] = {1};
            xforms[0] = ckc_embed_bounded(b, h_upper, 1, "h", h_strides, -ctx->p.PAD,
                                          0, ctx->p.H);
        }
         ...
         *       offset=-PAD, lo=0, hi=W) */
        {
            ...
            int w_strides[2] = {1, 1};
            xforms[1] = ckc_embed_bounded(b, w_upper, 2, "w", w_strides, -ctx->p.PAD,
                                          0, ctx->p.W);
        }
