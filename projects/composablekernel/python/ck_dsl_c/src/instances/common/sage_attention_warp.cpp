...
bool ckc_sage_warp_prologue(ckc_sage_warp_ctx_t* ctx)
{
    ...
    if(ctx->spec->quant_mode == CKC_SAGE_QUANT_I4_FP8_BF16)
    {
        ...
        if(ctx->ept_pairs != 1)
        {
            ...
            ckc_i_set_err(
                b, CKC_ERR_VALUE, "i4 sage v1 supports head_size == 128 (one byte per lane)");
            return false;
        }
    }
                                              ...
                                              void* user)
{
    ...
    ckc_value_t* k_scale_v = ckc_b_load_k_scale_for_block(
...
ckc_kernel_def_t* ckc_sage_warp_emit_body(ckc_sage_warp_ctx_t* ctx)
{
    ...
    if(ctx->s.mask_mode == CKC_FMHA_MASK_CAUSAL || ctx->s.mask_mode == CKC_FMHA_MASK_SLIDING_WINDOW)
