...
ckc_kernel_def_t* ckc_build_decode_gemv_matmul_nbits(
    ckc_ir_builder_t* b,
    const ckc_matmul_nbits_decode_gemv_spec_t* spec,
    const char* arch);
#ifdef __cplusplus
}
#endif
...
static ckc_value_t* ckc_mnb_wmma(ckc_ir_builder_t* b,
                                 const char* wmma_op,
                                 ckc_value_t* a,
                                 ckc_value_t* bb,
                                 ckc_value_t* c)
                                                 ...
                                                 const char* arch)
{
    ...
    ckc_value_t** a_row_off = (ckc_value_t**)ckc_arena_alloc(&b->arena, (size_t)n_sub_m * sizeof(*a_row_off));
    ...
    ckc_for_t loop =
        ckc_b_scf_for_iter(b, c0, cK, cwtk, iter_args, n_acc, "k0", false, true);
    ckc_b_region_enter(b, loop.body);
    {
        ...
        ckc_value_t** a_frag = (ckc_value_t**)ckc_arena_alloc(&b->arena, (size_t)n_sub_m * sizeof(*a_frag));
        if (a_frag == NULL)
        {
            return NULL;
        }
        {
            ...
            {
                ckc_value_t* idx =
                    ckc_b_add(b, ckc_b_add(b, a_row_off[sm], k0), half_k_elem);
                a_frag[sm] = ckc_b_global_load_vN_f16(b, A, idx, wp.frag_k, 0);
            }
        }

        int n_bytes = wp.frag_k / 2; /* packed B bytes feeding one fragment */
        ckc_value_t** b_frag = (ckc_value_t**)ckc_arena_alloc(&b->arena, (size_t)n_sub_n * sizeof(*b_frag));
        if (b_frag == NULL)
        {
            return NULL;
        }
        {
            ...
            for (sn = 0; sn < n_sub_n; ++sn)
            {
                ckc_value_t* packed = ckc_b_global_load_vN(
                    b, Bp, ckc_b_add(b, ckc_b_add(b, b_byte_off[sn], k_half), half_k_byte),
                    I8t, n_bytes, 0);
                ckc_value_t* scale =
                    ckc_b_global_load(b, Sp, ckc_b_add(b, b_scale_off[sn], k_grp), scale_t, 1);
                ckc_value_t* scale_f32 = ckc_b_cast_to_f32(b, scale);
                ckc_value_t** comps =
                    (ckc_value_t**)ckc_arena_alloc(&b->arena, (size_t)(2 * n_bytes) * sizeof(*comps));
                if (comps == NULL)
                {
                    return NULL;
                }
                {
                    ...
                    for (j = 0; j < n_bytes; ++j)
                    {
                        ...
                        ckc_value_t* lo = NULL;
                        ckc_value_t* hi = NULL;
                        if (ckc_dequant_i4_byte_to_f16_pair(b, byte, scale_f32, &lo, &hi) !=
                            CKC_OK)
                        {
                            return NULL;
                        }
            ...
            for (sn = 0; sn < n_sub_n; ++sn)
            {
                ...
                for (i = 0; i < 8; ++i)
                {
                    ...
                    if (wp.split_k_by_half)
                    {
                        ...
                        out_row = ckc_b_add(b, r0, ckc_b_add(b, half_k_elem, ckc_b_const_i32(b, i)));
                    }
                    else
                    {
                    ...
                    }
                    {
                        ...
                        ckc_b_global_store(b, C, ckc_b_add(b, ckc_b_mul(b, out_row, cN), out_col),
                                           h, 1);
                        ckc_b_region_leave(b);
                    }
                }
