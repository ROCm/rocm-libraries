                                     ...
                                     const ckc_mma_op_t* op)
{
    ...
    ctx->use_specialized =
        (!ctx->use_input_cache) &&
        ckc_dfcp_can_use_specialized_conv0_a_loader(spec);
 ...
 * ===================================================================== */
ckc_value_t* ckc_dfcp_m_index_fn(ckc_dfcp_build_ctx_t* ctx,
                                 ckc_value_t* row,
                                 const ckc_warp_grid_t* grid)
{
    ckc_ir_builder_t* b;
                                        ...
                                        ckc_value_t* a_rsrc)
{
    ...
    cache =
        ckc_dfcp_setup_input_footprint_cache(ctx->b, ctx->spec, a_rsrc, grid);
 ...
 * ===================================================================== */
ckc_value_t* ckc_dfcp_setup_specialized_a_loader(
                                     ...
                                     ckc_value_t* cache)
{
    ...
    ckc_dfcp_load_conv0_a_tile_from_input_cache(ctx->b, ctx->spec, conv_spec_,
                                                k_off, a_dst, grid, cache);
}
                                      ...
                                      ckc_value_t* a_rsrc)
{
    ...
    ckc_dfcp_load_conv0_a_tile_specialized(ctx->b, ctx->spec, conv_spec_, k_off,
                                           a_dst, grid, a_rsrc);
}
 ...
 *                                                 frag_len, cache)
 * ===================================================================== */
ckc_value_t* ckc_dfcp_load_a_operand_from_cache(
    ckc_dfcp_build_ctx_t* ctx,
    const ckc_implicit_gemm_conv_spec_t* conv_spec_,
    ckc_value_t* row,
    ckc_value_t* k_off,
                                ...
                                ckc_value_t* w1_rsrc)
{
    ...
    ctx->num_conv0_accs = (num_accs < (size_t)CKC_DFCP_MAX_ACCS)
                              ? num_accs
    ...
    st = ckc_dfcp_emit_conv1_1x1(b, spec, conv_spec_, op, ctx->c_smem,
                                 ...
                                 ctx->conv1_accs, (size_t)CKC_DFCP_MAX_ACCS,
                                 &ctx->num_conv1_accs);
    if (st != CKC_OK)
    {
...
static void* ckc_dfcp_tramp_input_cache_setup(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    const ckc_warp_grid_t* grid,
    ckc_value_t* a_rsrc,
...
static void ckc_dfcp_tramp_a_load_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* k_off,
    ckc_value_t* a_dst,
...
static ckc_value_t* ckc_dfcp_tramp_a_operand_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* a_row,
    ckc_value_t* k_off,
    ...
    void* user)
{
    ...
    ckc_value_t* cache = (ckc_value_t*)input_cache_context;
    (void)b;
    return ckc_dfcp_load_a_operand_from_cache(ctx, spec, a_row, k_off, col_base,
                                              a_per_lane, grid, cache);
}

static void ckc_dfcp_tramp_epilogue_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* const* accs,
    int num_accs,
    ...
    void* user)
{
    ...
    (void)b;
    ckc_dfcp_epilogue_override(ctx, spec, accs,
                              (num_accs < 0) ? 0u : (size_t)num_accs, grid,
                              d_rsrc, w1_rsrc);
}
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_deep_fused_conv_pool(
    ckc_ir_builder_t* b_unused,
    const ckc_deep_fused_conv_pool_spec_t* spec,
    const char* arch)
{
    ...
    reason[0] = '\0';
    if (!ckc_deep_fused_conv_pool_is_valid_spec(spec, arch, reason,
                                                sizeof(reason)))
    {
...
ckc_kernel_def_t* ckc_build_deep_fused_conv_pool_new(
    ckc_ir_builder_t* b,
    const ckc_deep_fused_conv_pool_spec_t* spec,
    const char* arch)
{
 ...
 * ===================================================================== */
ckc_status_t ckc_deep_fused_conv_pool_lower_to_llvm(
    const ckc_deep_fused_conv_pool_spec_t* spec,
    const char* arch,
