...
static void ckc_conv_set_final_accs(ckc_conv_build_ctx_t* ctx,
...
void ckc_conv_emit_kloop_unroll(ckc_conv_build_ctx_t* ctx)
{
    ...
    for (it = 0; it < K_iters; ++it)
    {
        ...
        if (it + 1 < K_iters)
        {
            ...
            ckc_conv_emit_load_phase(ctx,
                                     ckc_b_const_i32(b, (int64_t)(it + 1) * block_k),
                                     buf_a[nxt],
                                     buf_b[nxt]);
        }
        ...
        ckc_conv_emit_mfma_phase(ctx, buf_a[cur], buf_b[cur],
                                 current_accs, num_accs, new_accs);
        for (i = 0; i < num_accs; ++i)
...
void ckc_conv_emit_kloop_simple(ckc_conv_build_ctx_t* ctx)
{
    ...
    ckc_b_region_enter(b, for_op.body);
    {
        ...
        ckc_conv_emit_mfma_phase(ctx, ctx->A_smem, ctx->B_smem,
                                 iter_vars, for_op.num_iter_vars, new_accs);
        ckc_b_sync(b);
...
void ckc_conv_emit_kloop_async(ckc_conv_build_ctx_t* ctx)
{
     ...
     *       issue_load(p, buffers[p % nb]) */
    {
        ...
        for (pp = 0; pp < prologue_n; ++pp)
        {
            int slot = pp % nb;
            ckc_conv_emit_load_phase(ctx,
                                     ckc_b_const_i32(b, (int64_t)pp * block_k),
                                     buf_a[slot],
                                     buf_b[slot]);
        }
    }
    ...
    for (it = 0; it < num_iters; ++it)
    {
        ...
        if (has_next)
        {
            ...
            ckc_conv_emit_load_phase(ctx,
                                     ckc_b_const_i32(b, (int64_t)issue_idx * block_k),
                                     buf_a[nxt],
                                     buf_b[nxt]);
        }
