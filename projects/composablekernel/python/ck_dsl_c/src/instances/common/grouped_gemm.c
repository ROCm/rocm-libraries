 ...
 * ===================================================================== */
ckc_gemm_universal_spec_t ckc_grouped_gemm_to_universal_spec(
 ...
 * ===================================================================== */
ckc_status_t ckc_grouped_gemm_kernel_name(const ckc_grouped_gemm_spec_t* spec,
                                          char* out,
                                          size_t out_cap)
{
                                         ...
                                         const char* arch)
{
    ...
    if (!ckc_gemm_universal_is_valid_spec(&u, arch, reason, sizeof(reason)))
    {
        return (ckc_kernel_def_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "invalid grouped_gemm spec for %s: %s", arch,
            reason);
    }
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_grouped_gemm_single_launch(
    ckc_ir_builder_t* b, const ckc_grouped_gemm_spec_t* spec, const char* arch)
{
    ...
    if (!ckc_gemm_universal_is_valid_spec(&u, arch, reason, sizeof(reason)))
    {
        return (ckc_kernel_def_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE,
            "invalid grouped_gemm_single_launch spec for %s: %s", arch, reason);
    }
    return ckc_build_universal_gemm(b, &u, arch);
}
...
static const char* grouped_gemm_ptr_dt(const ckc_grouped_gemm_spec_t* spec)
{
    ...
    if (dt != NULL &&
        (strcmp(dt, "f16") == 0 || strcmp(dt, "fp16") == 0 ||
         strcmp(dt, "bf16") == 0))
    {
        return dt;
    }
...
ckc_status_t ckc_grouped_gemm_single_launch_signature(
    const ckc_grouped_gemm_spec_t* spec,
    ckc_arena_t* arena,
    const ckc_sig_entry_t** out_items,
    size_t* out_count)
