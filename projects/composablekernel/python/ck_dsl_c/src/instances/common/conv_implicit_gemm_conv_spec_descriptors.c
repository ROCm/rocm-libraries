 ...
 * ckc_conv_lds_layout_validate / _validate_for_async /
 * ckc_implicit_gemm_conv_spec_effective_lds_layout reproduce
 * ck_dsl/helpers/layouts.py + ImplicitGemmConvSpec.effective_lds_layout, and
...
/* ===================================================================== *
 *  LdsLayout port   (ck_dsl/helpers/layouts.py lines 66-203)
 *
...
bool ckc_conv_lds_layout_validate(const ckc_conv_lds_layout_t* l,
                                  char* reason,
                                  size_t reason_cap)
{
    /* LdsLayout.validate():
    ...
    if (l == NULL)
    {
        CKC_LDS_REJECT("lds layout is NULL");
    }
    if (l->logical_cols <= 0)
    {
        CKC_LDS_REJECT("logical_cols must be positive");
    }
    if (l->k_pad < 0)
    {
        CKC_LDS_REJECT("k_pad must be >= 0");
    }
    if (l->swizzle != NULL && strcmp(l->swizzle, "xor") != 0 &&
        strcmp(l->swizzle, "cyclic") != 0)
    {
        CKC_LDS_REJECT("unsupported LDS swizzle %s", l->swizzle);
    }
    return true;
}
                                            ...
                                            size_t reason_cap)
{
    ...
    if (l->k_pad != 0)
    {
        CKC_LDS_REJECT("async LDS layout must be packed: k_pad must be 0");
    }
    if (l->swizzle != NULL)
    {
        CKC_LDS_REJECT(
...
bool ckc_implicit_gemm_conv_spec_effective_lds_layout(
    const ckc_implicit_gemm_conv_spec_t* s,
    ckc_conv_lds_layout_t* out,
    char* reason,
    size_t reason_cap)
{
    ckc_conv_lds_layout_t l;

    if (s == NULL || out == NULL)
    {
        CKC_LDS_REJECT("spec or out is NULL");
    }
    ...
    {
        CKC_LDS_REJECT(
            "spec.lds_layout override is not supported in the C port "
    ...
    if (!ckc_conv_lds_layout_validate(&l, reason, reason_cap))
    {
        return false;
    }
...
#define CKC_CSPEC_REJECT(...)                          \
    ...
    {
        ckc_conv_lds_layout_t layout;
        if (!ckc_implicit_gemm_conv_spec_effective_lds_layout(s, &layout, reason,
                                                              reason_cap))
        {
            return false;
        }
        if (s->async_dma &&
            !ckc_conv_lds_layout_validate_for_async(&layout, reason, reason_cap))
        {
            return false;
        }
    }
                                                         ...
                                                         bool decompose_m)
{
    ...
    int strides_wo[2];
    int dims_k[4];
    const ckc_transform_t* xforms[8];
    int n_x = 0;
                                                         ...
                                                         const ckc_conv_problem_t* p)
{
    ...
    const char* into_k[4];
    int dims_k[4];
    const ckc_transform_t* xforms[4];
    int n_x = 0;
    ckc_tensor_descriptor_t* desc;
                                                         ...
                                                         const ckc_conv_problem_t* p)
{
    ...
    if (p->is_3d)
    {
        ...
        dims_m[2] = Ho;
        dims_m[3] = Wo;
        xforms[0] = ckc_unmerge_magic(b, "m", into_m, 4, dims_m);
        if (xforms[0] == NULL)
        {
            return NULL;
        }
