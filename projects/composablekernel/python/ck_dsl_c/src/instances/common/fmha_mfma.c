...
{
    ckc_fmha_shape_t shape = ckc_fmha_shape_default(
        spec->head_size, spec->num_query_heads, spec->num_kv_heads);
 ...
 * --------------------------------------------------------------------------- */
ckc_status_t ckc_fmha_mfma_kernel_name(const ckc_fmha_mfma_spec_t* spec,
                                       char* out,
                                       size_t out_cap)
{
    const char* name;
                                 ...
                                 size_t reason_cap)
{
    ...
    if (!ok)
    {
        fmha_set_reason(reason, reason_cap,
    ...
    if (spec->head_size % 16 != 0)
    {
        snprintf(buf, sizeof(buf),
                 "tiled FMHA needs head_size %% 16 == 0 (got %d)", spec->head_size);
        fmha_set_reason(reason, reason_cap, buf);
        return false;
    }
    /* seqlen_q % BLOCK_M != 0 */
    ...
    if (!ckc_mma_catalog_has_shape(ckc_archtarget_mma(target), family, "f16", "f16", "fp32",
                                   CKC_MFMA_ATTN_BLOCK_M, CKC_MFMA_ATTN_BLOCK_M,
                                   CKC_MFMA_ATTN_BLOCK_K))
    {
        snprintf(buf, sizeof(buf),
                 "unsupported f16 %s warp_tile (%d,%d,%d) on %s",
                 family, CKC_MFMA_ATTN_BLOCK_M, CKC_MFMA_ATTN_BLOCK_M,
                 CKC_MFMA_ATTN_BLOCK_K, arch);
        fmha_set_reason(reason, reason_cap, buf);
        return false;
    }
                                             ...
                                             size_t err_cap)
{
    ...
    k_batch_offset = ckc_b_mul(b, ckc_b_mul(b, kb.batch_idx, seqlen_k),
                               ckc_fmha_kernel_builder_stride_token(&kb, "k"));
    v_batch_offset = ckc_b_mul(b, ckc_b_mul(b, kb.batch_idx, seqlen_k),
                               ckc_fmha_kernel_builder_stride_token(&kb, "v"));
