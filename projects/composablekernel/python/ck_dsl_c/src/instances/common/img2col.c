                               ...
                               size_t reason_cap)
{
    ...
    {
        snprintf(buf, sizeof(buf), "unsupported dtype '%s' (only f16 in v1)",
    ...
    {
        snprintf(buf, sizeof(buf), "vec_k must be one of {1, 2, 4, 8} (got %d)",
                 spec->vec_k);
        ckc_img2col_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    if (spec->block_tile_k % spec->vec_k != 0)
    {
        snprintf(buf, sizeof(buf),
                 "block_tile_k %d not divisible by vec_k %d", spec->block_tile_k,
                 spec->vec_k);
        ckc_img2col_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    if (block_size > 1024)
    {
        snprintf(buf, sizeof(buf),
                 ...
                 block_size, spec->block_tile_m, spec->block_tile_k, spec->vec_k);
        ckc_img2col_set_reason(reason, reason_cap, buf);
        return false;
    }
...
static ckc_tensor_descriptor_t* ckc_i_img2col_make_a_descriptor(
    ckc_ir_builder_t* b, const ckc_conv_problem_t* p)
{
    ...
    naive_desc = ckc_tensor_descriptor_naive(b, "A_nhwc", base_lengths, 4, NULL,
                                             base_names, 4);
    if (naive_desc == NULL)
    {
        return NULL;
    ...
    if (t_m == NULL || t_hi == NULL || t_wi == NULL || t_k == NULL ||
        t_pad_y == NULL || t_pad_x == NULL)
    {
        return NULL;
    }
                                   ...
                                   ckc_value_t** out_valid)
{
    ...
    vals[0]  = m_v;
    vals[1]  = k_v;
    return ckc_transforms_descriptor_offset(b, a_desc, names, vals, 2, out_off,
                                            out_valid);
}
...
ckc_kernel_def_t* ckc_build_img2col(ckc_ir_builder_t* b,
                                    const ckc_img2col_spec_t* spec,
                                    const char* arch)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "invalid img2col spec for %s: %s", arch,
                      reason);
        return NULL;
    }
    ...
    tid = ckc_b_thread_id_x(b);
    {
        ...
        naive_desc = ckc_tensor_descriptor_naive(b, "img2col_tid", lengths, 2,
                                                 NULL, coord_names, 2);
        if (naive_desc == NULL)
        {
            return NULL;
        ...
        }
        {
            ...
            chain[0] = xform;
            tid_unmerge_desc =
                ckc_tensor_descriptor_transform(b, naive_desc, chain, 1);
        }
        ...
        in_names[0] = "tid";
        in_values[0] = tid;
        n_out = ckc_tensor_descriptor_unmerge_lower(b, tid_unmerge_desc, in_names,
                                                    in_values, 1, out_names,
                                                    out_values, 8);
        if (n_out < 0)
        {
            return NULL;
        ...
        if (m_local == NULL || k_chunk_local == NULL)
        {
            ckc_i_set_err(b, CKC_ERR_VALUE,
                          "build_img2col: tid decode missing m_local/k_chunk_local");
            return NULL;
        }
    }
    ...
    k_val_base =
        ckc_b_add(b, ckc_b_mul(b, ckc_b_block_id_x(b), c_block_k), k_local_base);
    ...
    /* A_desc = make_a_descriptor(p) -- built here (emits no IR; .offset does). */
    {
        ...
        ckc_tensor_descriptor_t* a_desc =
            ckc_i_img2col_make_a_descriptor(b, p);
        ckc_value_t* loaded;
         ...
         * in the parity set). Reproduce build_img2col's vector branch. */
        {
            ...
            {
                x1 = ckc_b_add(b, x1,
                               ckc_b_mul(b, k_chunk_local, ckc_b_const_i32(b, V)));
            }
            ...
            if (!ckc_i_img2col_a_offset(b, a_desc, glob_m, glob_k, &elem_off,
                                        &valid_unused))
            {
                return NULL;
            }
            ...
            if (!ckc_i_img2col_a_offset(b, a_desc, glob_m, glob_k, &elem_off2,
                                        &valid_off))
            {
                return NULL;
            }
            ...
            byte_off = ckc_b_select(b, mask, byte_off,
                                    ckc_b_const_i32(b, (int64_t)0x7fffffff));
            ...
            if (V == 1)
            {
                ...
                ckc_value_t* raw0 =
                    ckc_b_buffer_load_f16(b, x_rsrc, byte_off, x_soffset);
                ckc_value_t* scalar0 = ckc_b_cast_to_f32(b, raw0);
                loaded = ckc_b_cast_f32_to(b, scalar0, F16t);
            }
            else
            {
                ckc_value_t* vec = ckc_b_buffer_load_vN_f16(
                    b, x_rsrc, byte_off, x_soffset, V / 2);
                ckc_value_t* raw[8];
        ...
        {
            ckc_value_t* out_off_elems =
                ckc_b_add(b, ckc_b_mul(b, m_val, c_K), k_val_base);
            ckc_value_t* out_off_bytes =
                ckc_b_mul(b, out_off_elems, c_half_bytes);
            ...
            ckc_value_t* safe_out_off =
                ckc_b_select(b, in_bounds, out_off_bytes, oob_sentinel);
            if (V == 1)
            {
            ...
            {
                ckc_b_buffer_store_vN_f16(b, y_rsrc, safe_out_off, c0, loaded,
                                          V / 2);
            }
        }
    }
...
ckc_kernel_def_t* ckc_build_img2col_new(ckc_ir_builder_t* b,
                                        const ckc_img2col_spec_t* spec,
                                        const char* arch)
{
