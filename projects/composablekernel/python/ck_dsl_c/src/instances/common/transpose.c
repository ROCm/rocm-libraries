 ...
 *
 * ckc/instance_transpose.c -- C99 port of
 * ck_dsl/instances/common/transpose.py.
 *
 ...
 * (ckc_tensor_descriptor_naive + ckc_unmerge_magic + unmerge_lower) so its
 ...
 */
ckc_status_t ckc_transpose2d_kernel_name(const ckc_transpose2d_spec_t* spec,
                                         char* out,
                                         size_t out_cap)
{
    char tile_part[32];
    ...
    return ckc_kernel_name_join(spec->name, parts, 5, NULL, NULL, 0, out, out_cap,
                                NULL);
}
                                   ...
                                   size_t reason_cap)
{
    ...
    if (!ckc_arch_fits_lds(target, lds_bytes))
    {
        snprintf(buf, sizeof(buf), "LDS staging %ld > %d cap on %s", lds_bytes,
                 target->lds_capacity_bytes, arch);
        tx_copy_msg(reason, reason_cap, buf);
        return false;
    }
 ...
 * ===================================================================== */
static bool tx_decode_thread_layout(ckc_ir_builder_t* b,
                                    ckc_value_t* tid,
                                    const char* major,
                                    const char* minor,
                                    int dims0,
                                    int dims1,
                                    ckc_value_t** out_major,
                                    ckc_value_t** out_minor)
{
    int lengths[2];
    ...
    xf = ckc_unmerge_magic(b, "tid", lowers, 2, dims);
    if (xf == NULL)
    {
        return false;
    }
    ...
    in_names[0] = "tid";
    in_values[0] = tid;
    n_out = ckc_tensor_descriptor_unmerge_lower(b, desc2, in_names, in_values, 1,
                                                out_names, out_values, 8);
    if (n_out < 0)
    {
        return false;
    }
 ...
 * ===================================================================== */
static ckc_value_t* tx_morton_decode_bits(ckc_ir_builder_t* b,
                                          ckc_value_t* val,
                                          int shift,
                                          int width)
{
    ...
    if (shift)
    {
        v = ckc_b_div(b, v, ckc_b_const_i32(b, 1 << shift));
    }
    v = ckc_b_land(b, v, ckc_b_const_i32(b, 0x55555555 & mask_lim));
                                        ...
                                        const char* arch)
{
    ...
    int TM, TN, vec, BS;
    ckc_param_opts_t opts_x, opts_y;
    ckc_value_t* X;
    ckc_value_t* Y;
    ckc_value_t* H;
    ckc_value_t* W;
    ...
    if (!ckc_transpose2d_is_valid_spec(spec, "gfx950", reason, sizeof(reason)))
    {
        char msg[224];
    ...
    {
        tile_x = ckc_b_block_id_x(b);
        tile_y = ckc_b_block_id_y(b);
    }

    /* h0 = tile_y * TM; w0 = tile_x * TN */
    h0 = ckc_b_mul(b, tile_y, ckc_b_const_i32(b, TM));
    w0 = ckc_b_mul(b, tile_x, ckc_b_const_i32(b, TN));
     ...
     * byte-identical. */
    lds_org0 = ckc_b_const_i32(b, 0);
    lds_org1 = ckc_b_const_i32(b, 0);

    /* Phase 1 thread layout: (row1, col1_chunk), dims=(TM, TN//vec). */
    if (!tx_decode_thread_layout(b, tid, "row1", "col1_chunk", TM, TN / vec, &row1,
                                 &col1_chunk))
    {
        return NULL;
    }
     ...
     */
    {
        ...
        for (i = 0; i < vec; ++i)
        {
            casts[i] = ckc_b_cast_f32_to(b, scalars[i], io_ty);
        }
    ...
    if (!tx_decode_thread_layout(b, tid, "col2", "row2_chunk", TN, TM / vec, &col2,
                                 &row2_chunk))
    {
        return NULL;
    }
    /* row2_base = row2_chunk * c_vec */
    row2_base = ckc_b_mul(b, row2_chunk, c_vec);
     ...
     */
    {
        ...
        for (i = 0; i < vec; ++i)
        {
            ckc_value_t* row_i = ckc_b_add(b, row2_base, ckc_b_const_i32(b, i));
            /* TileWindow.load_scalar -> _global_indices: add(lds_origin, idx)
             * reusing the lds_tile origin consts (no fresh const_i32(0)). */
            g0 = ckc_b_add(b, lds_org0, row_i);
            g1 = ckc_b_add(b, lds_org1, col2);
            ...
            (void)ckc_b_add(
                b, ckc_b_mul(b, g0, ckc_b_const_i32(b, TN + spec->lds_pad)), g1);
            idx2[0] = g0;
            idx2[1] = g1;
                                            ...
                                            const char* arch)
{
    return ckc::guard_builder(b, [&]() -> ckc_kernel_def_t* {
        ...
        if (ckc_transpose2d_kernel_name(spec, name, sizeof(name)) != CKC_OK)
        {
            return NULL;
        }
        ...
        return ckc_build_transpose2d(b, spec, arch);
    });
}
 ...
 * ===================================================================== */
ckc_status_t ckc_transpose2d_grid(int h,
                                  int w,
                                  const ckc_transpose2d_spec_t* spec,
                                  int out[3])
{
...
/* ===================================================================== *
 *  ckc_transpose2d_lower_to_llvm -- build + lower to .ll convenience.
                                           ...
                                           size_t err_cap)
{
    ...
    kernel = ckc_build_transpose2d_new(&b, spec, arch);
    if (kernel == NULL)
    {
