                                      ...
                                      size_t reason_cap)
{
    ...
    {
        sage_set_reason(reason, reason_cap,
    ...
    atom_reason[0] = '\0';
    if (!ckc_validate_fmha_mfma_atom(sage_common_dtype(spec), arch, atom_reason,
                                     sizeof(atom_reason)))
    {
        sage_set_reason(reason, reason_cap,
                        atom_reason[0] != '\0' ? atom_reason : "invalid MFMA atom");
        return false;
    }
    ...
    if (spec->seqlen_q <= 0 || spec->seqlen_k <= 0)
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap,
                     "seqlen_q / seqlen_k must be > 0 (got %d, %d)", spec->seqlen_q,
                     spec->seqlen_k);
        }
        return false;
    }
    ...
    if (spec->quant_mode == CKC_SAGE_QUANT_I4_FP8_BF16 &&
        (cs->shape.head_size % 2) != 0)
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap,
    ...
    if (cs->shape.head_size % CKC_FMHA_WARP_SIZE != 0)
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap,
                     ...
                     CKC_FMHA_WARP_SIZE, cs->shape.head_size);
        }
        return false;
    }
 ...
 * ===================================================================== */
void ckc_sage_declare_params(ckc_fmha_kernel_builder_t* kb,
                             const ckc_sage_attention_spec_t* spec)
{
    ...
    ckc_fmha_kernel_builder_add_tensor(kb, "K", kv_dtype, /*readonly*/ true,
                                       /*writeonly*/ false, /*align*/ 8);
    ckc_fmha_kernel_builder_add_tensor(kb, "V", kv_dtype, /*readonly*/ true,
                                       /*writeonly*/ false, /*align*/ 8);
    ...
    {
        ckc_fmha_kernel_builder_add_ptr(kb, "codebook_k", "f32", /*readonly*/ true,
                                        /*align*/ 4);
        ckc_fmha_kernel_builder_add_ptr(kb, "codebook_v", "f32", /*readonly*/ true,
                                        /*align*/ 4);
    }
                                          ...
                                          size_t* out_count)
{
    ...
    st = ckc_fmha_kernel_builder_init(&kb, "ck_dsl_sage_attention_sig_probe",
                                      &spec->common);
    if (st != CKC_OK)
    {
        return st;
    ...
    if (kernel == NULL)
    {
        ...
        if (err != NULL && err_cap > 0)
        {
            ...
            snprintf(err, err_cap, "%s",
                     m != NULL ? m : "build_sage_attention failed");
        }
        ckc_fmha_kernel_builder_free(&kb);
