                                         ...
                                         ckc_value_t** out_accs)
{
    ...
    c_clamp_min = epilogue->has_clamp_min ? ckc_b_const_f32(b, epilogue->clamp_min) : NULL;
    /* c_clamp_max = b.const_f32(epilogue.clamp_max) if clamp_max is not None else None */
    c_clamp_max = epilogue->has_clamp_max ? ckc_b_const_f32(b, epilogue->clamp_max) : NULL;
                                   ...
                                   ckc_value_t* d_rsrc)
{
    ...
    epi.atom = ckc_mfma_atom("f16", spec->warp_tile_m, spec->warp_tile_n, spec->warp_tile_k);
    epi.grid = *grid;
    ...
    for(mi = 0; mi < mfmas_m; ++mi)
    {
        for(ni = 0; ni < mfmas_n; ++ni)
        {
            ...
            for(i = 0; i < op->c_frag_len; ++i)
            {
                ...
                safe_off = ckc_b_select(
                    b, ok, d_off_bytes, ckc_b_const_i32(b, (int64_t)((1u << 31) - 1u)));
                                     ...
                                     ckc_value_t* d_rsrc)
{
     ...
     *        bounds=(b.const_i32(p.M), b.const_i32(p.N_gemm))) */
    {
        ...
        ckc_value_t* bound_m = ckc_b_const_i32(b, ckc_conv_problem_m(p));
        ckc_value_t* bound_n = ckc_b_const_i32(b, ckc_conv_problem_n_gemm(p));
        ckc_cshuffle_epilogue_store(
            b, &epi, accs, num_accs, ckc_conv_d_addr, (void*)D_desc, d_rsrc, bound_m, bound_n);
    }
}
...
void ckc_conv_emit_epilogue(ckc_conv_build_ctx_t* ctx)
{
    ...
    ckc_conv_apply_accumulator_epilogue(
    ...
    {
        ckc_conv_emit_cshuffle_epilogue(b, spec, final_accs, num_accs, &ctx->grid, ctx->d_rsrc);
    }
    /* elif op.family == "wmma": */
    ...
    {
        ckc_conv_emit_direct_epilogue(b, spec, final_accs, num_accs, &ctx->grid, ctx->d_rsrc);
    }
}
