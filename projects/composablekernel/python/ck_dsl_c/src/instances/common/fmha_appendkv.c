...
ckc_fmha_appendkv_spec_t ckc_fmha_appendkv_spec_default(ckc_fmha_common_spec_t common,
                                                        int batch)
{
...
ckc_status_t ckc_fmha_appendkv_kernel_name(const ckc_fmha_appendkv_spec_t* spec,
                                           char* out,
                                           size_t out_cap)
{
    const char* parts[6];
    ...
    return ckc_kernel_name_join(ckc_fmha_appendkv_spec_name(spec), parts, 6,
                                     ...
                                     size_t reason_cap)
{
    ...
    if (spec->has_rotary && spec->rotary.head_size != spec->common.shape.head_size)
    {
        snprintf(buf, sizeof(buf),
                 ...
                 spec->rotary.head_size, spec->common.shape.head_size);
        ckc_appendkv_set_reason(reason, reason_cap, buf);
        return false;
    }
...
{
    ckc_b_vector_row_copy(b, src_ptr, dst_ptr, src_row_base, dst_row_base, H,
                          dtype, CKC_FMHA_APPENDKV_VEC * 2);
}
                                        ...
                                        ckc_value_t* cache_row_base)
{
    ...
    if (!spec->has_rotary)
    {
        ckc_appendkv_copy_row_vec(b, H, dtype, K_new, K_cache, in_row_base,
                                  cache_row_base);
        return CKC_OK;
    }
    ...
    if (layout == CKC_ROTARY_HALF && (H / 2) % VEC == 0)
    {
        ...
        for (c = 0; c < n_chunks; ++c)
        {
            ...
            lo_vec = ckc_b_global_load_vN(b, K_new,
                                          ckc_b_add(b, in_row_base, ckc_b_const_i32(b, d_lo)),
                                          ty, VEC, VEC * elem_bytes);
            hi_vec = ckc_b_global_load_vN(b, K_new,
            ...
            for (j = 0; j < VEC; ++j)
            {
                ...
                ckc_rotary_load_cos_sin(b, cos_table, sin_table, dst_pos,
                                        ckc_b_const_i32(b, pair), &spec->rotary,
                                        &cos_v, &sin_v);
                lo_f32 = ckc_b_cast_to_f32(b, ckc_b_vec_extract(b, lo_vec, j));
                hi_f32 = ckc_b_cast_to_f32(b, ckc_b_vec_extract(b, hi_vec, j));
                ckc_rotary_apply_pair_f32(b, lo_f32, hi_f32, cos_v, sin_v,
                                          &new_lo, &new_hi);
                out_lo_f32[j] = new_lo;
                out_hi_f32[j] = new_hi;
            }
    ...
    if (layout == CKC_ROTARY_INTERLEAVED && H % VEC == 0 && VEC % 2 == 0)
    {
        ...
        for (c = 0; c < n_chunks; ++c)
        {
            ...
            vec = ckc_b_global_load_vN(b, K_new,
    ...
    /* Catch-all fallback: scalar pair loop. */
    {
        ...
        for (pair = 0; pair < pair_count; ++pair)
        {
            ...
            k_lo = ckc_b_load_scalar_as_f32(b, K_new,
                                            ckc_b_add(b, in_row_base, ckc_b_const_i32(b, lo_d)),
                                            dtype);
            k_hi = ckc_b_load_scalar_as_f32(b, K_new,
                                            ckc_b_add(b, in_row_base, ckc_b_const_i32(b, hi_d)),
                                            dtype);
            ckc_rotary_load_cos_sin(b, cos_table, sin_table, dst_pos,
                                    ckc_b_const_i32(b, pair), &spec->rotary,
                                    &cos_v, &sin_v);
            ckc_rotary_apply_pair_f32(b, k_lo, k_hi, cos_v, sin_v, &new_lo, &new_hi);
            ckc_b_store_scalar_from_f32(b, K_cache,
                                        ckc_b_add(b, cache_row_base, ckc_b_const_i32(b, lo_d)),
                                        new_lo, dtype);
            ckc_b_store_scalar_from_f32(b, K_cache,
                                        ckc_b_add(b, cache_row_base, ckc_b_const_i32(b, hi_d)),
                                        new_hi, dtype);
        }
    }
    return CKC_OK;
...
static void ckc_appendkv_copy_v(ckc_ir_builder_t* b,
                                ...
                                ckc_value_t* cache_row_base)
{
    ckc_appendkv_copy_row_vec(b, H, dtype, V_new, V_cache, in_row_base,
                              cache_row_base);
}
                              ...
                              ckc_value_t* stride_cache_head)
{
    ...
    ckc_appendkv_copy_k(b, spec, H, dtype, K_new, K_cache, cos_table, sin_table,
                        dst_pos, in_row_base, cache_row_base);
...
ckc_status_t ckc_fmha_appendkv_grid(const ckc_fmha_appendkv_spec_t* spec,
                                    int total_new_q,
                                    int out[3])
{
    int totals[1];
