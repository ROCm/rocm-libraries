...
ckc_status_t ckc_matmul_nbits_scale_wire_dtype(const char* scale_dtype,
                                               const char** out)
{
...
void ckc_matmul_nbits_spec_finalize(ckc_matmul_nbits_spec_t* spec)
{
    ...
    spec->block_size = ckc_warp_tile_init_block_size(
        spec->block_size, spec->tile.warp_m, spec->tile.warp_n, spec->tile.warp_k,
        spec->wave_size);
}

ckc_status_t ckc_matmul_nbits_kernel_name(const ckc_matmul_nbits_spec_t* spec,
                                          char* out,
                                          size_t out_cap)
{
    char part_nk[64];
    ...
    return ckc_kernel_name_join(spec->name, parts, 8, flag_names, flag_on, 1, out,
                                out_cap, NULL);
}
                                           ...
                                           size_t reason_cap)
{
    ...
    if (!ckc_matmul_nbits_arch_supported(arch))
    {
        ...
        CKC_NBITS_REJECT(
            "matmul_nbits v1 supports ['gfx1151', 'gfx1201'] only (got '%s')",
            arch);
    }
    if (!ckc_matmul_nbits_family_known(spec->family))
    {
        CKC_NBITS_REJECT(
            "unknown family '%s'; expected one of ('large_n', 'skinny_n', "
            "'decode_gemv')",
    ...
    {
        CKC_NBITS_REJECT(
            "matmul_nbits v1 fixes group_size=%d (got %d)",
            CKC_MATMUL_NBITS_V1_GROUP_SIZE, spec->group_size);
    }
    if (spec->zero_points)
    {
        CKC_NBITS_REJECT(
    ...
    {
        CKC_NBITS_REJECT(
            "matmul_nbits v1 ships packing='row_k_contiguous' only (got '%s')",
    ...
    {
        CKC_NBITS_REJECT("N / K must be positive (got N=%d, K=%d)", spec->N,
                         spec->K);
    }
    if (spec->K % spec->group_size)
    {
        CKC_NBITS_REJECT("K (%d) must be divisible by group_size (%d)", spec->K,
                         spec->group_size);
    }
    ...
    {
        CKC_NBITS_REJECT("spec wave_size %d != %s wave_size %d", spec->wave_size,
    ...
    if (strcmp(family_str, "decode_gemv") != 0)
    {
        ...
        if (!ckc_archtarget_supports_dtype_combo(target, CKC__WMMA_AB_DTYPE,
                                                 CKC__WMMA_AB_DTYPE,
                                                 CKC__WMMA_C_DTYPE, mma_family))
        {
            CKC_NBITS_REJECT("unsupported matmul_nbits dtype fp16 on %s", arch);
        }
        if (ckc_archtarget_op_for_shape(target, mma_family, CKC__WMMA_AB_DTYPE,
                                        CKC__WMMA_AB_DTYPE, CKC__WMMA_C_DTYPE,
                                        t->warp_tile_m, t->warp_tile_n,
                                        t->warp_tile_k) == NULL)
        {
            CKC_NBITS_REJECT("unsupported fp16 warp_tile (%d, %d, %d) on %s",
                             t->warp_tile_m, t->warp_tile_n, t->warp_tile_k, arch);
        }
        ...
        if (spec->N % t->tile_n)
        {
            CKC_NBITS_REJECT("N (%d) must be divisible by tile_n (%d)", spec->N,
                             t->tile_n);
        }
        if (spec->K % t->tile_k)
        {
            CKC_NBITS_REJECT("K (%d) must be divisible by tile_k (%d)", spec->K,
                             t->tile_k);
        }
    }
    ...
    expected_bs =
        ckc_warp_tile_block_size(t->warp_m, t->warp_n, t->warp_k, spec->wave_size);
    if (expected_bs != spec->block_size)
    {
        CKC_NBITS_REJECT(
            "block_size %d != warp_m*warp_n*warp_k*wave_size = %d",
            spec->block_size, expected_bs);
    }
    ...
    {
        CKC_NBITS_REJECT("seq_len_tile must be positive (got %d)",
                         spec->seq_len_tile);
    }
...
ckc_status_t ckc_matmul_nbits_grid(int M,
