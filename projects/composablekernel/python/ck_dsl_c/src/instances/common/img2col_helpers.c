...
ckc_status_t ckc_img2col_conv_problem_short(const ckc_conv_problem_t* p,
                                            char* out,
                                            size_t out_cap)
{
    int wrote;
    ...
    wrote = snprintf(out, out_cap, "N%dH%dW%dC%d_K%dY%dX%d", p->N, p->Hi, p->Wi,
                     p->C, p->K, p->Y, p->X);
...
ckc_status_t ckc_img2col_kernel_name(const ckc_img2col_spec_t* spec,
                                     char* out,
                                     size_t out_cap)
{
    char short_part[128];
    ...
    wrote = snprintf(tile_part, sizeof(tile_part), "t%dx%d", spec->block_tile_m,
                     spec->block_tile_k);
    ...
    return ckc_kernel_name_join(spec->name, parts, 4, NULL, NULL, 0, out,
                                out_cap, NULL);
}
