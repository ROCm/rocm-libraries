                                     ...
                                     const char* arch)
{
    ...
    const char* resolved_arch =
        ckc_fmoe_resolve_launch_arch(arch != NULL ? arch : s.arch);
     ...
     * downstream anyway). */
    const bool is_bf16 =
        (s.dtype != NULL && strcmp(s.dtype, "bf16") == 0);
    ...
    else if (ckc_fmoe_tile_eq(&s.gemm_tile, &default_tile) &&
             s.hidden >= 1024 && s.tokens >= 32)
    {
    ...
    ctx->use_static_offsets =
        (s.tokens * s.topk * s.experts) <= 512;
