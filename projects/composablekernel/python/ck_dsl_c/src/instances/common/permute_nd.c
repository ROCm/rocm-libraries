...
{
    int d;
    if (spec == NULL || out == NULL) return 0;
...
int ckc_permute_vec_width(const ckc_permute_spec_t* spec)
{
    ...
    if (spec == NULL || spec->rank == 0) return 1;
    /* perm[-1] == rank - 1 */
    if (spec->perm[spec->rank - 1] == spec->rank - 1)
    {
        ...
        {
            int v = kVecs[i];
            if (inner % v == 0 && total % v == 0) return v;
        }
        return 1;
    }
...
ckc_status_t ckc_permute_kernel_name(const ckc_permute_spec_t* spec,
                                     char* out,
                                     size_t out_cap)
{
    /* Python kernel_name():
    ...
    if (spec == NULL || out == NULL || out_cap == 0) return CKC_ERR_VALUE;
    ...
    for (d = 0; d < spec->rank; ++d)
    {
        int w = snprintf(shape_part + off, sizeof(shape_part) - off, "%s%d",
                         d == 0 ? "" : "x", spec->x_shape[d]);
        if (w < 0 || (size_t)w >= sizeof(shape_part) - off) return CKC_ERR_VALUE;
        off += (size_t)w;
    }
    ...
    for (d = 0; d < spec->rank; ++d)
    {
        ...
        if (w < 0 || (size_t)w >= sizeof(perm_part) - off) return CKC_ERR_VALUE;
        off += (size_t)w;
    }
    ...
    return ckc_kernel_name_join(spec->name, parts, n_parts, NULL, NULL, 0,
                                out, out_cap, NULL);
}
...
{
    size_t n;
    if (reason == NULL || reason_cap == 0) return;
    n = strlen(msg);
    if (n >= reason_cap) n = reason_cap - 1;
    memcpy(reason, msg, n);
    reason[n] = '\0';
}
                               ...
                               size_t reason_cap)
{
    ...
    {
        snprintf(buf, sizeof(buf), "unsupported dtype %s",
                 spec->dtype ? spec->dtype : "(null)");
        permute_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    /* legal block sizes = {64,128,256,512,1024} clamped to max_threads_per_block. */
    {
        ...
        for (i = 0; i < 5; ++i)
        {
            if (kBs[i] <= max_tpb) legal_bs[n_legal++] = kBs[i];
        }
    }
    bs_ok = false;
    for (i = 0; i < n_legal; ++i)
    {
        if (spec->block_size == legal_bs[i]) { bs_ok = true; break; }
    }
    if (!bs_ok)
    {
    ...
    {
        snprintf(buf, sizeof(buf), "rank %d > %d (CK Tile cap)", spec->rank,
                 CKC_PERMUTE_MAX_RANK);
        permute_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    for (d = 0; d < spec->rank; ++d) seen[d] = 0;
    for (d = 0; d < spec->rank; ++d)
    {
        int p = spec->perm[d];
...
permute_build_offset_descriptor(ckc_ir_builder_t* b, const ckc_permute_spec_t* spec)
{
    ...
    for (d = 0; d < n; ++d)
    {
        ...
        if (in_names[d] == NULL || out_names[d] == NULL) return NULL;
...
ckc_kernel_def_t* ckc_build_permute(ckc_ir_builder_t* b,
...
ckc_status_t ckc_permute_grid(const ckc_permute_spec_t* spec, int out[3])
{
    ...
    int totals[1];
    int tiles[1];
    if (spec == NULL || out == NULL) return CKC_ERR_VALUE;
    vec = ckc_permute_vec_width(spec);
    total = ckc_permute_total_elements(spec);
                                   ...
                                   size_t* out_count)
{
    ...
    if (arena == NULL || spec == NULL || out == NULL) return CKC_ERR_VALUE;
    ...
    if (st != CKC_OK) return st;
    if (count > out_cap) return CKC_ERR_VALUE;
    for (i = 0; i < count; ++i) out[i] = items[i];
