 ...
 * (ck_dsl/instances/common/sage_attention.py, lines 464-591): the MFMA-tiled
 ...
 * ckc/instance_sage_attention_internal.h:
 *
 *   ckc_sage_mfma_prologue                 (Python lines 477-496)
 *   ckc_sage_mfma_fold_scales              (Python lines 498-555)
 *   ckc_sage_mfma_k_block_scale_transform  (Python lines 534-553, the
 *                                           extra_score_transform closure)
 *   ckc_sage_mfma_emit_body                (Python lines 557-591)
 *
 * Each phase reads / writes only the carried ckc_sage_mfma_ctx_t (+ the builder
 * it carries) and emits IR in byte-identical Python order. Peer phases /
 * primitives (ckc_sage_declare_params, ckc_sage_magic_div, the qk_scale loaders,
 * ckc_mfma_attention_fwd_inner_body) are called through the shared headers.
 */
#include <stddef.h>
 ...
 * The common-spec mask mode (ckc_fmha_mask_mode_t) lowers to the attention
 * helper's narrower enum (ckc_attn_mask_mode_t) used by the params struct.
 * ---------------------------------------------------------------------- */
static ckc_attn_mask_mode_t sage_mfma_to_attn_mask(ckc_fmha_mask_mode_t m)
{
    switch (m)
    {
 ...
 * ===================================================================== */
void ckc_sage_mfma_prologue(ckc_sage_mfma_ctx_t* ctx)
{
    ckc_ir_builder_t* b;
    ...
    /* kb.block_size(64) */
    ckc_fmha_kernel_builder_block_size(ctx->kb, 64);

    /* _declare_params(kb, spec) */
    ckc_sage_declare_params(ctx->kb, ctx->spec);

    /* kb.decode_grid() */
    ckc_fmha_kernel_builder_decode_grid(ctx->kb, /*num_queries_per_kv*/ -1,
                                        /*has_batch_axis*/ false, NULL, NULL, NULL);
    ...
    ctx->q_tile_base =
        ckc_b_mul(b, ctx->q_tile_idx, ckc_b_const_i32(b, CKC_MFMA_ATTN_BLOCK_M));
 ...
 *  (the closure constant for ckc_sage_mfma_k_block_scale_transform).
 * ===================================================================== */
void ckc_sage_mfma_fold_scales(ckc_sage_mfma_ctx_t* ctx)
{
    ckc_ir_builder_t* b;
    ...
    {
        ctx->q_block_idx =
            ckc_sage_magic_div(b, ctx->q_tile_base, ctx->spec->q_scale.scale_block);
    }
    else
    {
    ...
    ctx->q_scale_v = ckc_b_load_q_scale_for_block(b, ctx->q_scale_ptr, &ctx->spec->q_scale,
                                                  ctx->batch_idx, ctx->head_idx,
                                                  ctx->q_block_idx);

    if (ctx->spec->k_scale.layout == CKC_QK_SCALE_PER_HEAD)
    {
        /* k_scale_const = load_k_scale_for_block(..., k_block_idx=const_i32(0)) */
        ctx->k_scale_const = ckc_b_load_k_scale_for_block(
            b, ctx->k_scale_ptr, &ctx->spec->k_scale, ctx->batch_idx, ctx->kv_head_idx,
            ckc_b_const_i32(b, 0));

        /* scale_log2 = raw * (q_scale_v * k_scale_const) */
        ctx->scale_log2 = ckc_b_fmul(b, ctx->scale_log2_raw,
                                     ckc_b_fmul(b, ctx->q_scale_v, ctx->k_scale_const));
    ...
    else
    {
        ...
        ctx->scale_log2 = ckc_b_fmul(b, ctx->scale_log2_raw, ctx->q_scale_v);

        /* c_block_k = const_i32(MFMA_ATTN_BLOCK_K) -- closure constant. */
        ctx->c_block_k = ckc_b_const_i32(b, CKC_MFMA_ATTN_BLOCK_K);
 ...
 *
 *  Used only on the per_block k_scale path. `user` is the ckc_sage_mfma_ctx_t*
 *  (it carries c_block_k / k_scale_ptr / batch_idx / kv_head_idx / spec).
 * ===================================================================== */
ckc_value_t* ckc_sage_mfma_k_block_scale_transform(ckc_ir_builder_t* b,
                                                   ckc_value_t* score_log2,
                                                   ckc_value_t* kt,
                                                   int row_in_atom,
                                                   void* user)
{
    ckc_sage_mfma_ctx_t* ctx = (ckc_sage_mfma_ctx_t*)user;
    ckc_value_t* k_pos;
    ckc_value_t* k_block_idx;
    ckc_value_t* k_scale_v;
    ...
    /* k_pos = b.mul(kt, c_block_k) */
    k_pos = ckc_b_mul(b, kt, ctx->c_block_k);

    /* k_block_idx = _magic_div(b, k_pos, spec.k_scale.scale_block) */
    k_block_idx = ckc_sage_magic_div(b, k_pos, ctx->spec->k_scale.scale_block);

    /* k_scale_v = load_k_scale_for_block(...) */
    k_scale_v = ckc_b_load_k_scale_for_block(b, ctx->k_scale_ptr, &ctx->spec->k_scale,
                                             ctx->batch_idx, ctx->kv_head_idx, k_block_idx);

    /* return b.fmul(score, k_scale_v) */
    return ckc_b_fmul(b, score_log2, k_scale_v);
}
 ...
 *      return kb.kernel
 * ===================================================================== */
ckc_kernel_def_t* ckc_sage_mfma_emit_body(ckc_sage_mfma_ctx_t* ctx)
{
    ckc_ir_builder_t* b;
    ...
    ctx->kv_dtype = (ctx->spec->quant_mode == CKC_SAGE_QUANT_FP8_BF16) ? "fp8e4m3" : NULL;

    /* Populate ckc_mfma_attn_params_t (Python keyword-only argument list). */
    ...
    /* return kb.kernel */
    kernel = ckc_fmha_kernel_builder_kernel(ctx->kb);
