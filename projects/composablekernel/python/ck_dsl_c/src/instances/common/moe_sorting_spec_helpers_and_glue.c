...
#define CK_MOE_SORT_REJECT(...)                        \
    ...
    {
        CK_MOE_SORT_REJECT(
            "tokens / topk / experts must be > 0 (got %d, %d, %d)",
            spec->tokens, spec->topk, spec->experts);
    }
    /* if experts > 1024: ... */
    ...
    {
        CK_MOE_SORT_REJECT(
            "experts (%d) > block_size (%d); pick a larger block_size or wait for "
            "multi-pass scan",
            spec->experts, spec->block_size);
    }
                                                    ...
                                                    ckc_value_t* lane_id)
{
    ...
    for (stride = 1; stride < length; stride *= 2)
    {
        ...
        ckc_value_t* src_lane =
            ckc_b_select(b, do_add, src_sub, ckc_b_const_i32(b, 0));
...
static ckc_status_t ckc_i_moe_sort_lower(ckc_kernel_def_t* (*build_new)(ckc_ir_builder_t*,
                                         ...
                                         size_t err_cap)
{
    ...
    if (kernel == NULL)
    {
        ...
        ckc_i_moe_sort_set_err(err, err_cap,
                               (m != NULL && m[0] != '\0') ? m : build_fail_msg);
                                                        ...
                                                        size_t err_cap)
{
    return ckc_i_moe_sort_lower(ckc_build_moe_sort_histogram_new,
                                "build_moe_sort_histogram failed", spec, arch, flavor,
                                out_ll, err, err_cap);
}
...
ckc_status_t ckc_build_moe_sort_scan_lower_to_llvm(const ckc_moe_sorting_spec_t* spec,
                                                   ...
                                                   size_t err_cap)
{
    return ckc_i_moe_sort_lower(ckc_build_moe_sort_scan_new, "build_moe_sort_scan failed",
                                spec, arch, flavor, out_ll, err, err_cap);
}
                                                      ...
                                                      size_t err_cap)
{
    return ckc_i_moe_sort_lower(ckc_build_moe_sort_scatter_new,
                                "build_moe_sort_scatter failed", spec, arch, flavor,
                                out_ll, err, err_cap);
}
                                                         ...
                                                         size_t err_cap)
{
    return ckc_i_moe_sort_lower(ckc_build_moe_sort_persistent_new,
                                "build_moe_sort_persistent failed", spec, arch, flavor,
                                out_ll, err, err_cap);
}
