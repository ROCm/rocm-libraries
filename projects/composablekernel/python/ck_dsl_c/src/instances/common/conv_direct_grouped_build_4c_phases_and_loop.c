...
bool ckc_dconv4c_prologue(ckc_dconv_4c_ctx_t* ctx)
{
    ...
    if (!ok)
    {
        CKC_ERR_SNPRINTF(b->err, sizeof b->err, "invalid direct_conv_4c spec for %s: %s",
                 ctx->arch ? ctx->arch : "gfx950", reason);
    ...
    ckc_attr_set_int(b, &b->kernel->attrs, "max_workgroup_size",
                     ckc_direct_conv_4c_threads_per_block(spec));
    ...
    ctx->group_in_wg =
        ckc_b_add(b, ckc_b_mul(b, ctx->wave_id, ckc_b_const_i32(b, 16)), ctx->batch);
    ctx->g = ckc_b_add(b, ckc_b_mul(b, ctx->by, ckc_b_const_i32(b, spec->block_groups)),
                       ctx->group_in_wg);
...
void ckc_dconv4c_load_weights(ckc_dconv_4c_ctx_t* ctx)
{
    ...
    /* Lines 883-887: b_desc = TensorDescriptor.naive("B", ...). */
    {
        ...
        lengths[2] = p->KW;
        lengths[3] = p->cpg;
        ctx->b_desc = ckc_tensor_descriptor_naive(b, "B", lengths, 4, NULL,
                                                  coord_names, 4);
    }

    /* Line 888: k_out_val = b.add(b.mul(g, c_kpg), lane_q). */
    ctx->k_out_val =
        ckc_b_add(b, ckc_b_mul(b, ctx->g, ctx->c_kpg), ctx->lane_q);
    ...
    for (r_const = 0; r_const < p->KH; ++r_const)
    {
        for (s_const = 0; s_const < p->KW; ++s_const)
        {
            ...
            ckc_transforms_descriptor_offset(b, ctx->b_desc, in_names, in_values,
                                             4, &w_off, &w_valid);

            /* b.buffer_load_vN_f16(b_rsrc, b.mul(w_off, c_half_bytes), c0, 2). */
            w = ckc_b_buffer_load_vN_f16(b, ctx->b_rsrc,
                                         ckc_b_mul(b, w_off, ctx->c_half_bytes),
                                         ctx->c0, 2);
...
void ckc_dconv4c_build_descriptors(ckc_dconv_4c_ctx_t* ctx)
{
    ...
    /* Lines 925-946: a_desc = naive("A", ...).transform(embed, embed). */
    {
        ...
        a_naive = ckc_tensor_descriptor_naive(b, "A", lengths, 4, NULL,
                                              coord_names, 4);

        /* embed(upper=("y_iter",), into="h", strides=(1,), offset=-PAD,
         *       lo=0, hi=H). */
        xforms[0] = ckc_embed_bounded(b, up_h, 1, "h", strides_h, -p->PAD, 0,
                                      p->H);
        /* embed(upper=("wo","s"), into="w", strides=(1,1), offset=-PAD,
         *       lo=0, hi=W). */
        xforms[1] = ckc_embed_bounded(b, up_w, 2, "w", strides_w, -p->PAD, 0,
                                      p->W);
    ...
    /* Lines 953-957: d_desc = naive("D", [N,H,W,total_k], ...). */
    {
        ...
        ctx->d_desc = ckc_tensor_descriptor_naive(b, "D", lengths, 4, NULL,
                                                  coord_names, 4);
    }
...
ckc_kernel_def_t* ckc_dconv4c_stream_h_loop(ckc_dconv_4c_ctx_t* ctx)
{
    ...
    for (y = 0; y < ctx->n_iters; ++y)
    {
        ...
        for (qt = 0; qt < ctx->q_tiles_per_wave; ++qt)
        {
            ...
            ckc_value_t* q_base =
                ckc_b_add(b, ctx->q_tile_start, ckc_b_const_i32(b, qt * 4));
            ...
            for (s_idx = 0; s_idx < ctx->n_s_consts; ++s_idx)
            {
                ...
                ckc_transforms_descriptor_offset(b, ctx->a_desc, in_names,
                                                 in_values, 5, &a_off, &valid);

                /* Line 984: safe_a = select(valid, a_off*2, oob_sentinel). */
                safe_a = ckc_b_select(b, valid,
                                      ckc_b_mul(b, a_off, ctx->c_half_bytes),
                                      ctx->oob_sentinel);
            ...
            for (r_const = 0; r_const < p->KH; ++r_const)
            {
                ...
                for (s_const = 0; s_const < p->KW; ++s_const)
                {
                    acc = ckc_b_mfma_f32_4x4x4_f16(
                        b, ctx->weights[r_const * p->KW + s_const],
                        inputs[s_const], acc);
                }
                accs[p_idx] = acc;
            }
        ...
        if (0 <= p_flush && p_flush < p->H)
        {
            ...
            for (qt = 0; qt < ctx->q_tiles_per_wave; ++qt)
            {
                ...
                ckc_transforms_descriptor_offset(b, ctx->d_desc, in_names,
                                                 in_values, 4, &d_base,
                                                 &d_valid);

                /* Line 1020: safe_d = select(out_q_ok, d_base*2, oob_sentinel). */
                safe_d = ckc_b_select(b, out_q_ok,
                                      ckc_b_mul(b, d_base, ctx->c_half_bytes),
                                      ctx->oob_sentinel);
                ...
                ckc_b_buffer_store_vN_f16(b, ctx->d_rsrc, safe_d, ctx->c0, acc_h,
                                          2);
            }
        }
