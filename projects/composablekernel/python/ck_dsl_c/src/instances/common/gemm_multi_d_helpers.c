...
ckc_status_t ckc_gemm_multi_d_kernel_name(const ckc_gemm_multi_d_spec_t* spec,
                                          char* out,
                                          size_t out_cap)
{
    char base_name[256];
    ...
    for (i = 0; i < spec->num_d_operands; ++i)
    {
        ...
        wrote = snprintf(d_suffix + pos, sizeof(d_suffix) - pos, "%s%s",
                         spec->d_operands[i].param_name, opname);
 ...
 * ===================================================================== */
ckc_fused_epilogue_t* ckc_gemm_multi_d_build_fused_epilogue(
    ckc_arena_t* arena, const ckc_gemm_multi_d_spec_t* spec, bool* out_is_mde)
{
    ...
    ops = (ckc_epilogue_op_t*)ckc_arena_alloc(
        arena, sizeof(ckc_epilogue_op_t) * (num ? num : 1));
    if (ops == NULL)
    {
        return NULL;
    }
    ...
    if (spec->d_load_kind == CKC_D_LOAD_STOCK)
    {
        ckc_fused_epilogue_t* fe =
            (ckc_fused_epilogue_t*)ckc_arena_alloc(arena, sizeof(*fe));
        if (fe == NULL)
        {
            return NULL;
        }
    ...
    {
        ckc_multi_d_epilogue_t* md =
            (ckc_multi_d_epilogue_t*)ckc_arena_alloc(arena, sizeof(*md));
 ...
 *  gemm_multi_d_grid
 * ===================================================================== */
ckc_status_t ckc_gemm_multi_d_grid(const ckc_gemm_multi_d_spec_t* spec,
                                   int m,
                                   int n,
                                   int batch,
                                   int out[3])
