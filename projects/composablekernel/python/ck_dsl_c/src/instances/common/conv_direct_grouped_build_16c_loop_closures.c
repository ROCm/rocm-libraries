                                 ...
                                 int out_cap)
{
    ...
    for (i = 0; i < ctx->n_chunk_meta; ++i)
    {
        ...
        {
            ckc_value_t* mul_ag = ckc_b_mul(b, ctx->chunk_meta[i].abs_group,
                                            ctx->c_cpg);
            ckc_value_t* mul_cb = ckc_b_mul(b, ctx->chunk_meta[i].ch_block,
                                            ckc_b_const_i32(b, 4));
        ...
        off_names[4] = "c";
        off_vals[4] = c_val;
        if (!ckc_transforms_descriptor_offset(b, ctx->a_desc, off_names, off_vals, 5,
                                              &a_off_elems, &addr_valid))
        {
            return -1;
        }
                                         ...
                                         ckc_value_t* lds)
{
    ...
    W_lds_idx = ckc_b_add(b, ctx->q_in_lane,
                          ckc_b_const_i32(b, q_subtile * 16 + s_const));
 ...
 * ===================================================================== */
ckc_value_t* ckc_dconv16c_lds_read_input_k32(ckc_dconv_16c_ctx_t* ctx,
                                             int q_subtile,
                                             ckc_value_t* lds)
{
    ckc_ir_builder_t* b = ctx->b;
     ...
     *                   s_lane_k32) */
    W_lds_idx = ckc_b_add(b,
                          ckc_b_add(b, ctx->q_in_lane,
                                    ckc_b_const_i32(b, q_subtile * 16)),
                          ctx->s_lane_k32);
    /* lds_idx = b.add(b.add(b.mul(W_lds_idx, c_BG_cpg), b.mul(wave_id, c_cpg)),
     *                 ch_lane_k32)
     * Force Python left-to-right SSA emission (C arg eval order unspecified). */
...
void ckc_dconv16c_prologue_prefetch(ckc_dconv_16c_ctx_t* ctx)
{
    ...
    n = ckc_dconv16c_issue_dram_load(ctx, ctx->c0, vecs, lds_idx,
...
ckc_kernel_def_t* ckc_dconv16c_stream_h_loop(ckc_dconv_16c_ctx_t* ctx)
{
    ...
    for (y = 0; y < n_iters; ++y)
    {
        ...
        if (y + 1 < n_iters)
        {
            n_loads_next = ckc_dconv16c_issue_dram_load(
                ctx, ckc_b_const_i32(b, y + 1), loads_next_vecs, loads_next_lds,
                CKC_DCONV16C_MAX_PASSES);
            if (n_loads_next < 0)
            {
                return NULL;
        ...
        if (has_loads_next)
        {
            ckc_dconv16c_store_to_lds(ctx, loads_next_vecs, loads_next_lds,
                                      n_loads_next, nxt);
        }
        /* b.sync() */
        ckc_b_sync(b);
        ...
        if (0 <= p_flush_val && p_flush_val < p->H)
        {
            for (qt = 0; qt < q_subtiles; ++qt)
            {
                ...
                out_q = ckc_b_add(b,
                                  ckc_b_add(b, ctx->q_tile_start,
                                            ckc_b_const_i32(b, qt * 16)),
                                  ctx->q_in_lane);
                 ...
                 * is unspecified). */
                {
                    ...
                    ckc_value_t* mul_c4 = ckc_b_mul(b, ctx->c4,
                                                    ckc_b_const_i32(b, 4));
                ...
                off_names[3] = "k";
                off_vals[3] = k_val;
                if (!ckc_transforms_descriptor_offset(b, ctx->d_desc, off_names,
                                                      off_vals, 4, &d_base, &d_valid))
                {
                    return NULL;
                }
                ...
                safe_d_off = ckc_b_select(b, out_q_valid, d_base_bytes,
                                          ctx->oob_sentinel);
                ...
                ckc_b_buffer_store_vN_f16(b, ctx->d_rsrc, safe_d_off, ctx->c0,
                                          acc_h, 2);
            }
        }
