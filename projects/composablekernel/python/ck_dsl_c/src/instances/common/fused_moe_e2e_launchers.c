 ...
 *   _ensure_batched_gemm_preshuffle_b_launcher       -> ckc_fmoe_ensure_batched_gemm_preshuffle_b_launcher
 *   _ensure_silu_mul_packed_launcher                 -> ckc_fmoe_ensure_silu_mul_packed_launcher
 *   _ensure_gate_up_silu_launcher                    -> ckc_fmoe_ensure_gate_up_silu_launcher
 *   _ensure_interleaved_gate_up_silu_launcher        -> ckc_fmoe_ensure_interleaved_gate_up_silu_launcher
 *   _ensure_interleaved_gate_up_silu_preshuffle_launcher
 *                                                    -> ckc_fmoe_ensure_interleaved_gate_up_silu_preshuffle_launcher
 *   _ensure_down_reduce_launcher                     -> ckc_fmoe_ensure_down_reduce_launcher
 *   _ensure_static_scatter_gather_launcher           -> ckc_fmoe_ensure_static_scatter_gather_launcher
 *   _moe_batched_gemm_launcher                        -> ckc_fmoe_moe_batched_gemm_launcher
 *   _moe_interleaved_gate_up_silu_launcher            -> ckc_fmoe_moe_interleaved_gate_up_silu_launcher
 *   _grouped_gate_up_silu_launcher                    -> ckc_fmoe_grouped_gate_up_silu_launcher
 *   _grouped_down_reduce_launcher                     -> ckc_fmoe_grouped_down_reduce_launcher
 *   _grouped_gate_up_spec                             -> ckc_fmoe_grouped_gate_up_spec
 *   _grouped_down_spec                                -> ckc_fmoe_grouped_down_spec
 *   _ensure_compiled                                  -> ckc_fmoe_ensure_compiled
 *
...
static bool ckc_fmoe_drive_build(const char* kernel_name,
                                 ckc_fmoe_build_thunk_fn thunk,
                                 void* user)
{
    ckc_ir_builder_t b;
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_topk_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->topk_launcher == NULL)
    {
        ...
        ckc_topk_softmax_spec_t spec =
            ckc_fmoe_forward_spec_to_topk_softmax_spec(&ctx->spec);
...
ckc_grouped_gemm_launcher_t* ckc_fmoe_ensure_gemm_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->gemm_launcher == NULL)
    {
        ...
        if (ckc_fmoe_forward_spec_to_gemm_spec(&ctx->spec, "gemm",
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_batched_gemm_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->batched_gemm_launcher == NULL)
    {
        ...
        if (ckc_fmoe_forward_spec_to_batched_gemm_spec(&ctx->spec,
                                                       name_buf, sizeof(name_buf),
                                                       &spec) != CKC_OK)
        {
            return NULL;
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_batched_gemm_preshuffle_b_launcher(
    ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->batched_gemm_preshuffle_b_launcher == NULL)
    {
        ...
        ctx->batched_gemm_preshuffle_b_launcher =
            CKC_FMOE_SENTINEL(g_batched_preb_compiled);
    }
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_silu_mul_packed_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->silu_mul_packed_launcher == NULL)
    {
        ...
        if (!ckc_fmoe_drive_build("silu_mul_packed", ckc_fmoe_thunk_silu_mul_packed,
                                  &fmoe_spec))
        {
            return NULL;
        }
...
static ckc_gemm_trait_spec_t ckc_fmoe_gemm_trait(bool preshuffle_b,
                                                 bool active_tile_skip)
{
    /* The gate_up_silu spec default already carries epilogue="default". */
    ckc_moe_gate_up_silu_gemm_spec_t tmpl =
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_gate_up_silu_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->gate_up_silu_launcher == NULL)
    {
        ...
        char name_buf[256];
        (void)snprintf(name_buf, sizeof(name_buf), "%s_gate_up_silu",
...
{
    (void)snprintf(name_buf, name_cap, "%s_interleaved_gate_up_silu",
                   ctx->spec.name != NULL ? ctx->spec.name : "");
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_interleaved_gate_up_silu_launcher(
    ckc_fmoe_build_ctx_t* ctx)
{
    if (ctx == NULL)
    {
    ...
    if (ctx->interleaved_gate_up_silu_launcher == NULL)
    {
        ...
        char name_buf[256];
        ckc_moe_interleaved_gate_up_silu_gemm_spec_t spec =
            ckc_fmoe_make_interleaved_spec(ctx, name_buf, sizeof(name_buf),
                                           /*preshuffle_b=*/false, universal_dtype);
        ...
        ctx->interleaved_gate_up_silu_launcher =
            CKC_FMOE_SENTINEL(g_interleaved_compiled);
    }
    return ctx->interleaved_gate_up_silu_launcher;
}

ckc_kernel_launcher_t* ckc_fmoe_ensure_interleaved_gate_up_silu_preshuffle_launcher(
    ckc_fmoe_build_ctx_t* ctx)
{
    if (ctx == NULL)
    {
    ...
    if (ctx->interleaved_gate_up_silu_preshuffle_launcher == NULL)
    {
        ...
        ckc_moe_interleaved_gate_up_silu_gemm_spec_t spec =
            ckc_fmoe_make_interleaved_spec(ctx, name_buf, sizeof(name_buf),
                                           /*preshuffle_b=*/true, universal_dtype);
        ckc_ir_builder_t b;
        if (ckc_ir_builder_init(&b, "interleaved_gate_up_silu") != CKC_OK)
        {
            return NULL;
...
ckc_fmoe_make_down_reduce_spec(const ckc_fmoe_build_ctx_t* ctx,
                               ...
                               const char* universal_dtype)
{
    (void)snprintf(name_buf, name_cap, "%s_down_reduce",
                   ctx->spec.name != NULL ? ctx->spec.name : "");
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_down_reduce_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    if (ctx->down_reduce_launcher == NULL)
    {
        ...
        ckc_moe_down_reduce_gemm_spec_t spec =
            ckc_fmoe_make_down_reduce_spec(ctx, name_buf, sizeof(name_buf),
                                           /*grouped=*/false, universal_dtype);
...
ckc_kernel_launcher_t* ckc_fmoe_ensure_static_scatter_gather_launcher(
    ckc_fmoe_build_ctx_t* ctx)
{
    if (ctx == NULL)
    {
    ...
    if (ctx->static_scatter_gather_launcher == NULL)
    {
         ...
         *   arch=self.arch) */
        if (!ckc_fmoe_drive_build("static_scatter_gather", ckc_fmoe_thunk_static_sg,
                                  &spec))
        {
            return NULL;
        }
...
ckc_moe_gate_up_silu_gemm_spec_t ckc_fmoe_grouped_gate_up_spec(
    const ckc_fmoe_build_ctx_t* ctx)
{
...
ckc_moe_down_reduce_gemm_spec_t ckc_fmoe_grouped_down_spec(
    const ckc_fmoe_build_ctx_t* ctx)
{
                                                 ...
                                                 bool active_tile_skip)
{
    const bool keyed_on_traits =
        (kind == CKC_FMOE_GEMM_BATCHED ||
         kind == CKC_FMOE_GEMM_INTERLEAVED_GATE_UP);
                                                          ...
                                                          bool active_tile_skip)
{
    ...
    char name_buf[256];
    (void)snprintf(name_buf, sizeof(name_buf), "%s_batched_gemm",
                   ctx->spec.name != NULL ? ctx->spec.name : "");
    ...
    return ckc_fmoe_cache_put(ctx, CKC_FMOE_GEMM_BATCHED, preshuffle_b,
                              active_tile_skip, launcher);
}

ckc_kernel_launcher_t* ckc_fmoe_moe_interleaved_gate_up_silu_launcher(
    ckc_fmoe_build_ctx_t* ctx,
    bool preshuffle_b,
    bool active_tile_skip)
{
     ...
     * cached = self._moe_gemm_launcher_cache.get(key); if cached: return cached */
    ckc_kernel_launcher_t* cached =
        ckc_fmoe_cache_get(ctx, CKC_FMOE_GEMM_INTERLEAVED_GATE_UP,
                           preshuffle_b, active_tile_skip);
    if (cached != NULL)
    {
        return cached;
    }
     ...
     *   trait=trait, dtype=_gemm_dtype_to_universal(dtype)) */
    char name_buf[256];
    (void)snprintf(name_buf, sizeof(name_buf), "%s_interleaved_gate_up_silu",
                   ctx->spec.name != NULL ? ctx->spec.name : "");
    ckc_moe_interleaved_gate_up_silu_gemm_spec_t spec =
        ckc_moe_interleaved_gate_up_silu_gemm_spec_default();
    spec.name = name_buf;
    ...
    /* launcher = KernelLauncher(...); self._moe_gemm_launcher_cache[key] = launcher */
    ckc_kernel_launcher_t* launcher = CKC_FMOE_SENTINEL(g_cache_compiled);
    return ckc_fmoe_cache_put(ctx, CKC_FMOE_GEMM_INTERLEAVED_GATE_UP,
                              preshuffle_b, active_tile_skip, launcher);
}
...
ckc_kernel_launcher_t* ckc_fmoe_grouped_gate_up_silu_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    /* launcher = KernelLauncher(...); self._moe_gemm_launcher_cache[key] = launcher */
    ckc_kernel_launcher_t* launcher = CKC_FMOE_SENTINEL(g_cache_compiled);
    return ckc_fmoe_cache_put(ctx, CKC_FMOE_GEMM_GROUPED_GATE_UP, false, false,
                              launcher);
}
...
ckc_kernel_launcher_t* ckc_fmoe_grouped_down_reduce_launcher(ckc_fmoe_build_ctx_t* ctx)
{
    ...
    /* launcher = KernelLauncher(...); self._moe_gemm_launcher_cache[key] = launcher */
    ckc_kernel_launcher_t* launcher = CKC_FMOE_SENTINEL(g_cache_compiled);
    return ckc_fmoe_cache_put(ctx, CKC_FMOE_GEMM_GROUPED_DOWN_REDUCE, false, false,
                              launcher);
}
