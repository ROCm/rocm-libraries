...
static void ckc_wsp3_producer_load(ckc_wsp3_ctx_t* w,
                                   ckc_value_t* k_off,
                                   ckc_value_t* a_buf_row,
                                   ckc_value_t* b_buf_row)
{
    ckc_ir_builder_t* b = w->b;
...
static void ckc_wsp3_producer_load_async(ckc_wsp3_ctx_t* w,
                                         ckc_value_t* k_off,
                                         ckc_value_t* a_buf,
                                         ckc_value_t* b_buf)
{
    ...
    for(p = 0; p < w->a_passes; ++p)
    {
        ...
        ckc_b_async_buffer_load_lds_addr(
            b, w->a_rsrc, lds, ckc_b_mul(b, off, w->c2), w->zsoff, w->dtl_dwords, 2);
    }
    for(p = 0; p < w->b_passes; ++p)
    {
        ...
        ckc_b_async_buffer_load_lds_addr(
            b, w->b_rsrc, lds, ckc_b_mul(b, off, w->c2), w->zsoff, w->dtl_dwords, 2);
    }
}

/* _consumer_mfma(accs, a_buf_row, b_buf_row, out_new[num_accs]): one K-tile of
 * MFMAs for this consumer warp. */
static void ckc_wsp3_consumer_mfma(ckc_wsp3_ctx_t* w,
                ...
                if(mi + 1 < w->mfmas_m)
                {
                    a_rows[mi + 1] = ckc_gemm_emit_smem_load(
                        ...
                        ckc_b_add(
                            b,
                            a_base,
                            ckc_b_add(b, ckc_b_const_i32(b, (mi + 1) * w->wtm), w->m_in_atom)),
                        col_base,
                        w->a_per_lane,
                        w->storage_dtype);
                }
        ...
        else
        {
            for(mi = 0; mi < w->mfmas_m; ++mi)
            {
                a_rows[mi] = ckc_gemm_emit_smem_load(
                    ...
                    w->A_smem,
                    ckc_b_add(
                        b, a_base, ckc_b_add(b, ckc_b_const_i32(b, mi * w->wtm), w->m_in_atom)),
                    col_base,
                    w->a_per_lane,
                    w->storage_dtype);
            }
            for(ni = 0; ni < w->mfmas_n; ++ni)
            {
                b_cols[ni] = ckc_gemm_emit_smem_load(
                    ...
                    w->B_smem,
                    ckc_b_add(
                        b, b_base, ckc_b_add(b, ckc_b_const_i32(b, ni * w->wtn), w->n_in_atom)),
                    col_base,
                    w->b_per_lane,
                    w->storage_dtype);
            }
