 ...
 *
 * instance_streamk_gemm.c -- C99 port of ck_dsl/instances/common/streamk_gemm.py
 * (CK Tile 40_streamk_gemm parity).
 *
 * The builder-call sequence in ckc_build_streamk_gemm mirrors build_streamk_gemm
...
bool ckc_streamk_gemm_partition(const ckc_streamk_gemm_spec_t* spec,
                                ckc_streamk_partition_t* out)
{
    ...
    if ((spec->M % spec->tile_m) || (spec->N % spec->tile_n) ||
        (spec->K % spec->tile_k))
    {
        return false;
    }
...
const ckc_mfma_atom_t* ckc_streamk_gemm_atom(const ckc_streamk_gemm_spec_t* spec)
{
    ...
    {
        return ckc_mfma_atom("f16", 16, 16, 16);
    }
    if (spec->tile_m == 32 && spec->tile_n == 32)
    {
        return ckc_mfma_atom("f16", 32, 32, 8);
    }
    return NULL;
}
...
{
    ckc_streamk_partition_t part;
    if (!ckc_streamk_gemm_partition(spec, &part))
    {
        return -1;
    }
    return ckc_compute_streamk_grid_size(&part, spec->num_cus, spec->blocks_per_cu,
                                         NULL);
}
...
int ckc_streamk_gemm_persistent_max_iters(const ckc_streamk_gemm_spec_t* spec)
{
    ...
    nm = ckc_streamk_partition_num_macro_tiles(&part);
 ...
 * ===================================================================== */
ckc_status_t ckc_streamk_gemm_kernel_name(const ckc_streamk_gemm_spec_t* spec,
                                          char* out,
                                          size_t out_cap)
{
    char part_mnk[64];
    ...
    snprintf(part_t, sizeof(part_t), "t%dx%dx%d", spec->tile_m, spec->tile_n,
             spec->tile_k);
                                    ...
                                    size_t reason_cap)
{
    ...
    if (spec->reduction != CKC_STREAMK_REDUCTION_ATOMIC)
    {
        return set_reason(reason, reason_cap,
                          "v1 ships the Atomic reduction strategy only");
    }
    /* Python: MFMA path supports tile (16,16) or (32,32). */
    if (!((spec->tile_m == 16 && spec->tile_n == 16) ||
          (spec->tile_m == 32 && spec->tile_n == 32)))
    {
        return set_reason(reason, reason_cap,
                          "MFMA path supports tile (16,16) or (32,32)");
    }
    /* Python: M/N/K must be divisible by their tile sizes. */
    if (spec->tile_m == 0 || spec->tile_n == 0 || spec->tile_k == 0 ||
        (spec->M % spec->tile_m) || (spec->N % spec->tile_n) ||
        (spec->K % spec->tile_k))
    {
        return set_reason(reason, reason_cap,
    ...
    atom = ckc_streamk_gemm_atom(spec);
    if (atom == NULL)
    {
    ...
    {
        return set_reason(reason, reason_cap,
                          "tile_k must be a multiple of atom.k");
    }
    ...
    if (!ckc_mma_catalog_has_shape(mma, NULL, "fp16", "fp16", "fp32", atom->m,
                                   atom->n, atom->k))
    {
        return set_reason(reason, reason_cap,
                          "f16 MFMA atom not available on target");
    }

    /* Python: reject persistent=True when persistent_max_iters > 1 (P35 bug). */
    if (spec->persistent)
    {
        int mi = ckc_streamk_gemm_persistent_max_iters(spec);
        if (mi > 1)
        {
            return set_reason(reason, reason_cap,
 ...
 *  into ckc_streamk_body_env_t. _process_macro_tile is the body; the two
 ...
 * ===================================================================== */
typedef struct ckc_streamk_body_env
{
    const ckc_streamk_gemm_spec_t* spec;
    ckc_streamk_partition_t partition;
    const ckc_mfma_atom_t* atom;
    ...
    ckc_value_t* k_macro_base;
} ckc_streamk_body_env_t;
...
static ckc_value_t* streamk_load_a(ckc_ir_builder_t* b, ckc_value_t* kt, void* user)
{
    ...
    ckc_value_t* k_tile_base = ckc_b_add(
        b, env->k_macro_base, ckc_b_mul(b, kt, ckc_b_const_i32(b, env->atom->k)));
    return ckc_load_a_row_major_contiguous(b, env->A, env->atom, &env->lane_decode,
                                           env->m_tile_base, k_tile_base,
                                           env->spec->K);
}
...
static ckc_value_t* streamk_load_b(ckc_ir_builder_t* b, ckc_value_t* kt, void* user)
{
    ...
    ckc_value_t* k_tile_base = ckc_b_add(
        b, env->k_macro_base, ckc_b_mul(b, kt, ckc_b_const_i32(b, env->atom->k)));
    return ckc_load_b_col_strided_scalars(b, env->B, env->atom, &env->lane_decode,
                                          env->n_tile_base, k_tile_base,
                                          env->spec->N);
}

/* Python _process_macro_tile(linear_id). */
static void streamk_process_macro_tile(ckc_ir_builder_t* b,
                                       ...
                                       void* user)
{
    ...
    decoded = ckc_emit_streamk_decode(b, linear_id, &env->partition);

    /* SGPR-pin m_tile / n_tile / k_iter (CTA-uniform). */
    m_tile = ckc_b_to_sgpr_u32(b, decoded.m_tile);
    n_tile = ckc_b_to_sgpr_u32(b, decoded.n_tile);
    k_iter = ckc_b_to_sgpr_u32(b, decoded.k_iter);
    ...
    env->m_tile_base =
        ckc_b_to_sgpr_u32(b, ckc_b_mul(b, m_tile, ckc_b_const_i32(b, spec->tile_m)));
    env->n_tile_base =
        ckc_b_to_sgpr_u32(b, ckc_b_mul(b, n_tile, ckc_b_const_i32(b, spec->tile_n)));
    env->k_macro_base =
    ...
    acc_final = ckc_mfma_k_loop(b, spec->tile_k, atom, streamk_load_a, streamk_load_b,
                                NULL, NULL, "kt", "acc", env);

    /* store_acc_to_global(b, C=Cf32, atom=atom, lane_decode=..., m_tile_base=...,
     *     n_tile_base=..., acc=acc_final, N=spec.N, out_dtype="f32",
     *     atomic_add=True) */
    ckc_store_acc_to_global(b, env->Cf32, atom, &env->lane_decode, env->m_tile_base,
                            env->n_tile_base, acc_final, spec->N, "f32",
                            /*atomic_add=*/true, /*epilogue=*/NULL,
                            /*epilogue_user=*/NULL);
}
                                         ...
                                         const char* arch)
{
    char reason[256];
    int BS;
    ckc_streamk_body_env_t env;
    ckc_param_opts_t opts;
    const ckc_type_t* ptr_f16;
    const ckc_type_t* ptr_f32;
    const ckc_type_t* ptr_i32;
    ckc_value_t* A;
    ckc_value_t* Bp;
    ckc_value_t* Cf32;
    ckc_value_t* Counter;
    ...
    if (!ckc_streamk_gemm_is_valid_spec(spec, arch, reason, sizeof(reason)))
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "invalid streamk_gemm spec for %s: %s",
                      arch, reason);
        return NULL;
    }
    ...
    env.spec = spec;
    if (!ckc_streamk_gemm_partition(spec, &env.partition))
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "streamk_gemm: degenerate partition");
        return NULL;
    }
    BS = ckc_streamk_gemm_block_size(spec);
    ...
    ptr_f16 = ckc_ptr_type(b, ckc_f16(), "global");
    ptr_f32 = ckc_ptr_type(b, ckc_f32(), "global");
    ...
    if (spec->persistent)
    {
        ...
        ckc_persistent_tile_for_each(b, Counter, ckc_b_const_i32(b, nm), max_iters,
                                     streamk_process_macro_tile, &env,
                                     /*counter_idx=*/NULL, /*cooperative=*/true,
                                     /*wave_size=*/64, /*block_size=*/64);
    }
    else
    {
        /* _ = Counter  (kept in ABI); _process_macro_tile(b.block_id_x()) */
        (void)Counter;
        streamk_process_macro_tile(b, ckc_b_block_id_x(b), &env);
    }
                                             ...
                                             const char* arch)
{
    return ckc::guard_builder(b, [&]() -> ckc_kernel_def_t* {
        ...
        if (ckc_streamk_gemm_kernel_name(spec, name, sizeof(name)) != CKC_OK)
        {
            return NULL;
        }
        ...
        return ckc_build_streamk_gemm(b, spec, arch);
...
ckc_kernel_def_t* ckc_build_streamk_gemm_block_tile(ckc_ir_builder_t* b,
                                                    ...
                                                    const char* arch)
{
    ckc_streamk_gemm_spec_t block_tile_spec;
    if (b == NULL || spec == NULL)
    {
        return NULL;
    }
...
ckc_status_t ckc_streamk_gemm_grid(const ckc_streamk_gemm_spec_t* spec, int out[3])
{
    ...
    if (spec->persistent)
    {
        int gs = ckc_streamk_gemm_grid_size(spec);
        if (gs < 0)
        {
            return CKC_ERR_VALUE;
        }
    ...
    else
    {
        ...
        out[0] = ckc_streamk_partition_num_macro_tiles(&part);
        out[1] = 1;
        out[2] = 1;
        return CKC_OK;
    }
...
/* ===================================================================== *
 *  ckc_streamk_gemm_lower_to_llvm -- build + lower to .ll convenience.
                                            ...
                                            size_t err_cap)
{
    ...
    kernel = ckc_build_streamk_gemm_new(&b, spec, arch);
    if (kernel == NULL)
    {
