...
static void
ckc_mega_mfma_atom_widths(const ckc_gemm_universal_spec_t* u, int* a_per, int* b_per, int* c_per)
{
                                         ...
                                         const ckc_gemm_universal_spec_t* u_down)
{
    ...
    ctx->elem_bytes_b = ckc_b_const_i64(b, 2);
    {
        ...
        ckc_value_t* off_g = ckc_b_mul(b, ckc_b_mul(b, eg, sg), ctx->elem_bytes_b);
        ...
        ckc_value_t* off_u = ckc_b_mul(b, ckc_b_mul(b, eu, su), ctx->elem_bytes_b);
        ...
        ckc_value_t* off_d = ckc_b_mul(b, ckc_b_mul(b, ed, sd), ctx->elem_bytes_b);
