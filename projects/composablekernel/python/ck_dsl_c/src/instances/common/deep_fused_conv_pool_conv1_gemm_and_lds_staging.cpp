                                                       ...
                                                       int valid_k)
{
...

    {
        ...
        {
            (void)ckc_i_set_err(
                b, CKC_ERR_VALUE, "deep_fused_conv_pool: frag_len %d out of range", frag_len);
            return NULL;
        }
                                                         ...
                                                         bool sync)
{
    ...
    if((int)num_accs != mfmas_m * mfmas_n)
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "expected %d accs, got %d", mfmas_m * mfmas_n, (int)num_accs);
    }
    ...
    shape[0] = grid->tile_m;
    shape[1] = grid->tile_n;
    if(ckc_make_lds_view(b, &c_view, ckc_f16(), shape, 2, "DeepFusionC_smem", NULL) != CKC_OK)
    {
        return NULL;
    }
    c_smem = c_view.base;
    ...
    for(mi = 0; mi < mfmas_m; ++mi)
    {
        for(ni = 0; ni < mfmas_n; ++ni)
        {
            ...
            {
                ckc_i_set_err(
                    b, CKC_ERR_VALUE, "cshuffle stage: c_frag_len=%d exceeds cap", c_frag_len);
                return NULL;
            }
            ...
            tile_m_base = ckc_b_add(b, warp_m_off, ckc_b_const_i32(b, mi * op->m));
            tile_n_base = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * op->n));
            ...
            for(i = 0; i < c_frag_len; ++i)
            {
                ...
                if(!ckc_arch_layout_map_coord(c_map, b, grid->lane, i, &row_in_atom, &col_in_atom))
                {
                    return NULL;
                }
 ...
 *   off = row * c_k0 + col */
static ckc_value_t* dfcp_w1_descriptor(
                                     ...
                                     size_t* out_n)
{
    ...
    if(!ckc_arch_layout_map_coord(a_map, b, grid->lane, 0, &a_mn_in_atom, &a_k_base))
    {
        return ckc_ir_builder_status(b);
    }
    if(!ckc_arch_layout_map_coord(b_map, b, grid->lane, 0, &b_k_base, &b_mn_in_atom))
    {
        return ckc_ir_builder_status(b);
    }
    ...
    {
        return ckc_i_set_err(b, CKC_ERR_VALUE, "deep_fused_conv_pool: bad conv1 tile_k") == NULL
                   ? CKC_ERR_VALUE
                   : CKC_ERR_VALUE;
    }
        ...
        for(kk = 0; kk < k_atoms; ++kk)
        {
            ...
            for(mi = 0; mi < mfmas_m; ++mi)
            {
                ckc_value_t* a_row = ckc_b_add(
                    b, warp_m_off, ckc_b_add(b, ckc_b_const_i32(b, mi * op->m), a_mn_in_atom));
            ...
            for(ni = 0; ni < mfmas_n; ++ni)
            {
                ckc_value_t* b_row = ckc_b_add(
                    b, warp_n_off, ckc_b_add(b, ckc_b_const_i32(b, ni * op->n), b_mn_in_atom));
    ...
    else
    {
        ...
        ckc_conv_apply_accumulator_epilogue(b, &spec->conv1_epilogue, accs, num_accs, out_accs);
        if(!ckc_ir_builder_ok(b))
        {
            return ckc_ir_builder_status(b);
        }
