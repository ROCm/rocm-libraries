                              ...
                              ckc_value_t** out_accs)
{
    ...
    for (int kk = 0; kk < ctx->k_atoms; ++kk)
    {
        ...
        for (int mi = 0; mi < ctx->mfmas_m; ++mi)
        {
            ...
            a_rows[mi] = ckc_conv_emit_frag_smem_load(b,
                                                      A_src,
                                                      a_row_in_atom,
                                                      a_k_in_atom,
                                                      atom_row,
        ...
        for (int ni = 0; ni < ctx->mfmas_n; ++ni)
        {
            ...
            b_cols[ni] = ckc_conv_emit_frag_smem_load(b,
                                                      B_src,
                                                      b_col_in_atom,
                                                      b_k_in_atom,
                                                      atom_row,
                              ...
                              ckc_value_t** out_accs)
{
    ...
    for (int kk = 0; kk < ctx->k_atoms; ++kk)
    {
        ...
        ckc_value_t* col_mul =
            ckc_b_mul(b, k_blk, ckc_b_const_i32(b, ctx->a_per_lane));
        ...
        for (int mi = 0; mi < ctx->mfmas_m; ++mi)
        {
            ...
            ckc_value_t* a_row =
                ckc_b_add(b,
                          warp_m_off,
                          ckc_b_add(b, ckc_b_const_i32(b, mi * spec->warp_tile_m), m_in_atom));
        ...
        for (int ni = 0; ni < ctx->mfmas_n; ++ni)
        {
            ...
            ckc_value_t* b_row =
                ckc_b_add(b,
                          warp_n_off,
                          ckc_b_add(b, ckc_b_const_i32(b, ni * spec->warp_tile_n), n_in_atom));
        ...
        for (int mi = 0; mi < ctx->mfmas_m; ++mi)
        {
            for (int ni = 0; ni < ctx->mfmas_n; ++ni)
            {
                out_accs[flat] = ckc_conv_emit_mfma(b, atom, a_rows[mi], b_cols[ni], out_accs[flat]);
                ++flat;
            }
        }
        ...
        ckc_schedule_policy_emit_after_mfma_step(&ctx->schedule,
                                                 b,
                                                 ctx->mfmas_m + ctx->mfmas_n,
                                                 ctx->mfmas_m * ctx->mfmas_n);
    }
}
