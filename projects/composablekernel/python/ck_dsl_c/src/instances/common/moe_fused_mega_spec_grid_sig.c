...
static void ckc_moe_mega_fill_universal_common(
    const ckc_moe_fused_mega_kernel_spec_t* spec, ckc_gemm_universal_spec_t* out)
{
    const char* dt;
...
void ckc_moe_fused_mega_gate_up_universal_spec(
    const ckc_moe_fused_mega_kernel_spec_t* spec, ckc_gemm_universal_spec_t* out)
{
    ...
    if (spec->name != NULL)
    {
        snprintf(ckc_moe_mega_gu_name, sizeof(ckc_moe_mega_gu_name), "%s_gu",
                 spec->name);
    }
    else
    {
...
void ckc_moe_fused_mega_down_universal_spec(
    const ckc_moe_fused_mega_kernel_spec_t* spec, ckc_gemm_universal_spec_t* out)
{
    if (spec == NULL || out == NULL)
    {
    ...
    if (spec->name != NULL)
    {
        snprintf(ckc_moe_mega_down_name, sizeof(ckc_moe_mega_down_name), "%s_down",
                 spec->name);
    }
    else
    {
...
ckc_status_t ckc_moe_fused_mega_kernel_name(
    const ckc_moe_fused_mega_kernel_spec_t* spec, char* out, size_t out_cap)
{
...
ckc_status_t ckc_moe_fused_mega_signature(
    const ckc_moe_fused_mega_kernel_spec_t* spec,
    ckc_sig_entry_t* out,
    size_t out_cap,
    size_t* out_count)
{
    ...
    out[i].name = "A";              out[i].type = ptr_dt_type;        ++i;
    out[i].name = "WGate";          out[i].type = ptr_dt_type;        ++i;
    out[i].name = "WUp";            out[i].type = ptr_dt_type;        ++i;
    out[i].name = "WDown";          out[i].type = ptr_dt_type;        ++i;
    out[i].name = "SortedTokenIds"; out[i].type = "ptr<i32, global>"; ++i;
    out[i].name = "SortedWeights";  out[i].type = "ptr<f32, global>"; ++i;
    out[i].name = "BlockExpertIds"; out[i].type = "ptr<i32, global>"; ++i;
    out[i].name = "Y";              out[i].type = "ptr<f32, global>"; ++i;

    /* 11 scalar params (Python order), all i32. */
    out[i].name = "M";             out[i].type = "i32"; ++i;
    out[i].name = "N";             out[i].type = "i32"; ++i;
    out[i].name = "K";             out[i].type = "i32"; ++i;
    out[i].name = "H_out";         out[i].type = "i32"; ++i;
    out[i].name = "stride_a";      out[i].type = "i32"; ++i;
    out[i].name = "stride_b_gate"; out[i].type = "i32"; ++i;
    out[i].name = "stride_b_up";   out[i].type = "i32"; ++i;
    out[i].name = "stride_b_down"; out[i].type = "i32"; ++i;
    out[i].name = "slot_size";     out[i].type = "i32"; ++i;
    out[i].name = "tokens";        out[i].type = "i32"; ++i;
