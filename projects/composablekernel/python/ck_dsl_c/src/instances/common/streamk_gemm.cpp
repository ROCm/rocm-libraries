...
int ckc_streamk_gemm_grid_size(const ckc_streamk_gemm_spec_t* spec)
{
    ...
    return ckc_compute_streamk_grid_size(&part, spec->num_cus, spec->blocks_per_cu, NULL);
}
                                    ...
                                    size_t reason_cap)
{
    ...
    if(!ckc_mma_catalog_has_shape(mma, NULL, "fp16", "fp16", "fp32", atom->m, atom->n, atom->k))
    {
...
ckc_build_streamk_gemm(ckc_ir_builder_t* b, const ckc_streamk_gemm_spec_t* spec, const char* arch)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "invalid streamk_gemm spec for %s: %s", arch, reason);
        return NULL;
    }
