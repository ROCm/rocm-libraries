...
bool ckc_elementwise_is_unary(const ckc_elementwise_spec_t* spec)
{
    ...
    op = spec->op;
    return ckc_ew_streq(op, "copy") || ckc_ew_streq(op, "neg") ||
           ckc_ew_streq(op, "abs") || ckc_ew_streq(op, "relu") ||
           ckc_ew_streq(op, "gelu_tanh") || ckc_ew_streq(op, "quick_gelu") ||
           ckc_ew_streq(op, "silu") || ckc_ew_streq(op, "swish") ||
           ckc_ew_streq(op, "tanh") || ckc_ew_streq(op, "sigmoid") ||
...
bool ckc_elementwise_is_binary(const ckc_elementwise_spec_t* spec)
{
    ...
    op = spec->op;
    return ckc_ew_streq(op, "add") || ckc_ew_streq(op, "sub") ||
           ckc_ew_streq(op, "mul") || ckc_ew_streq(op, "max") ||
           ckc_ew_streq(op, "min") || ckc_ew_streq(op, "swiglu") ||
...
ckc_status_t ckc_elementwise_kernel_name(const ckc_elementwise_spec_t* spec,
                                         char* out,
                                         size_t out_cap)
{
                                   ...
                                   size_t reason_cap)
{
    ...
    if (!(spec->block_size == 64 || spec->block_size == 128 ||
          spec->block_size == 256 || spec->block_size == 512 ||
          spec->block_size == 1024))
    {
        snprintf(buf, sizeof(buf),
                 "block_size %d not in {64, 128, 256, 512, 1024}", spec->block_size);
        return ckc_ew_reason(reason, reason_cap, buf);
    }
...
ckc_kernel_def_t* ckc_build_elementwise(ckc_ir_builder_t* b,
        ...
        ckc_b_region_enter(b, gate.then_region);
        {
            ...
            for (i = 0; i < spec->vec; ++i)
            {
                ...
                ckc_b_region_enter(b, ib.then_region);
                {
                    ...
                    a_s = ckc_b_cast_to_f32(
                        b, ckc_tensor_view_load_scalar(b, &a_view, indices, 1));
                    if (is_binary)
                    {
