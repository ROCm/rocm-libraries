...
ckc_status_t ckc_fmoe_forward_static(ckc_fmoe_build_ctx_t* ctx)
{
     ...
     * (and any spec ValueError) matches the Python prepare() call. */
    {
        ...
        size_t n_ws = 0;
        ckc_status_t st = ckc_fmoe_workspace_specs(
            ctx, ws_specs, CKC_FMOE_NUM_WORKSPACE_SPECS, &n_ws);
        if (st != CKC_OK)
        {
            return st;
        }
    ...
    ckc_kernel_launcher_t* batched_gemm_launcher =
        ckc_fmoe_ensure_batched_gemm_launcher(ctx);
     ...
     * (lines 2303-2309). */
    const bool use_experimental_fused =
        s->use_experimental_fused_gate_up_silu;
    const bool use_experimental_interleaved =
        s->use_experimental_interleaved_gate_up_silu && !use_experimental_fused;
    const bool use_experimental_down_reduce =
        s->use_experimental_fused_down_reduce;
    const bool use_experimental_static_sg =
        s->use_experimental_static_scatter_gather;
    ...
    if (use_experimental_interleaved)
    {
        if (s->active_tile_skip_gemms)
        {
            interleaved_gate_up_silu_launcher =
                ckc_fmoe_moe_interleaved_gate_up_silu_launcher(
                    ctx,
    ...
    ckc_kernel_launcher_t* down_reduce_launcher =
        use_experimental_down_reduce ? ckc_fmoe_ensure_down_reduce_launcher(ctx)
                                     : NULL;
    ...
    ckc_kernel_launcher_t* silu_mul_packed_launcher =
        use_experimental_fused ? NULL
                               : ckc_fmoe_ensure_silu_mul_packed_launcher(ctx);
    ...
    if (!(use_experimental_fused || use_experimental_interleaved))
    {
        gu_concat = ckc_fmoe_ensure_gu_concat(
            ctx, (ckc_tensor_t*)(uintptr_t)ctx->W_gate,
    ...
    else
    {
        ...
        if (s->active_tile_skip_gemms)
        {
            gate_up_b_launcher = ckc_fmoe_moe_batched_gemm_launcher(
                ctx, /*preshuffle_b=*/s->preshuffle_w_gate_up_packed,
                /*active_tile_skip=*/true);
            gate_up_b_tensor =
                s->preshuffle_w_gate_up_packed
                    ? ckc_fmoe_ensure_gu_concat_preshuffled(
                          ctx, (ckc_tensor_t*)(uintptr_t)ctx->W_gate,
                          (ckc_tensor_t*)(uintptr_t)ctx->W_up)
                    : gu_concat;
        }
        ...
        {
            gate_up_b_launcher =
                ckc_fmoe_ensure_batched_gemm_preshuffle_b_launcher(ctx);
    ...
    else
    {
        ...
        if (s->active_tile_skip_gemms)
        {
            down_b_launcher = ckc_fmoe_moe_batched_gemm_launcher(
                ctx, /*preshuffle_b=*/s->preshuffle_w_down,
                /*active_tile_skip=*/true);
            down_b_tensor =
                s->preshuffle_w_down
                    ? ckc_fmoe_ensure_w_down_preshuffled(
        ...
        else if (s->preshuffle_w_down)
        {
            down_b_launcher =
                ckc_fmoe_ensure_batched_gemm_preshuffle_b_launcher(ctx);
            down_b_tensor = ckc_fmoe_ensure_w_down_preshuffled(
                ctx, (ckc_tensor_t*)(uintptr_t)ctx->W_down);
        }
        else
        {
...
ckc_status_t ckc_fmoe_capture_graph(ckc_fmoe_build_ctx_t* ctx,
                                    uint64_t routing_logits, uint64_t X,
                                    uint64_t W_gate, uint64_t W_up, uint64_t W_down,
                                    uint64_t Y, int warmup_iters)
{
    if (ctx == NULL)
    {
        return CKC_ERR_VALUE;
