...
static const char *pipeline_to_cktile(const char *p)
{
    ...
    if (strcmp(p, "mem") == 0)    return "ck_tile::GemmPipeline::MEMORY";
    if (strcmp(p, "compv3") == 0) return "ck_tile::GemmPipeline::COMPUTE_V3";
    if (strcmp(p, "compv4") == 0) return "ck_tile::GemmPipeline::COMPUTE_V4";
    return NULL;
}
...
static const char *scheduler_to_cktile(const char *s)
{
    ...
    if (strcmp(s, "intrawave") == 0) return "ck_tile::GemmPipelineScheduler::Intrawave";
    if (strcmp(s, "interwave") == 0) return "ck_tile::GemmPipelineScheduler::Interwave";
    return NULL;
}
...
static const char *dtype_to_cktile(const char *d)
{
    ...
    if (strcmp(d, "fp16") == 0)     return "ck_tile::half_t";
    if (strcmp(d, "f16") == 0)      return "ck_tile::half_t";
    if (strcmp(d, "bf16") == 0)     return "ck_tile::bf16_t";
    if (strcmp(d, "bfloat16") == 0) return "ck_tile::bf16_t";
    if (strcmp(d, "fp32") == 0)     return "float";
    if (strcmp(d, "f32") == 0)      return "float";
    if (strcmp(d, "fp8e4m3") == 0)  return "ck_tile::fp8_t";
    return NULL;
}
...
static bool layout_3(const char *layout, const char *out[3],
...
static bool is_fp16_path(const ckc_cktile_data_spec_t *d)
{
    ...
    return d->dtype_acc &&
           (strcmp(d->dtype_acc, "fp32") == 0 || strcmp(d->dtype_acc, "f32") == 0);
}
...
static void gemm_kernel_name(const ckc_cktile_gemm_spec_t *spec,
                             char *buf, size_t cap)
{
...
static void conv_kernel_name(const ckc_cktile_conv_spec_t *spec,
                             char *buf, size_t cap)
{
    ...
    n += snprintf(buf + n, (n < (int)cap) ? cap - (size_t)n : 0,
                  ...
                  spec->name,
                  p->N, p->Hi, p->Wi, p->C, p->K, p->R, p->S,
                  spec->tile_m, spec->tile_n, spec->tile_k,
                  spec->warp_m, spec->warp_n,
                  spec->warp_tile_m, spec->warp_tile_n, spec->warp_tile_k,
                  spec->pipeline, spec->epilogue);
    /* acc_epilogue.tag(): empty for identity. */
    if (spec->acc_epilogue_tag && spec->acc_epilogue_tag[0]) {
        n += snprintf(buf + n, (n < (int)cap) ? cap - (size_t)n : 0,
                      "_%s", spec->acc_epilogue_tag);
    }
    if (spec->async_dma) {
...
ckc_status_t ckc_lower_universal_gemm_to_cktile(
    const ckc_cktile_gemm_spec_t *spec,
    ...
    char err[CKC_ERR_MSG_CAP])
{
    ...
    if (!pipe) {
        ckc__err(err, "unsupported pipeline '%s'; supported: %s",
                 trait->pipeline ? trait->pipeline : "", PIPELINE_CHOICES_STR);
        return CKC_ERR_NOTIMPL;
    }
    if (!sched) {
        ckc__err(err, "unsupported scheduler '%s'; supported: %s",
                 trait->scheduler ? trait->scheduler : "", SCHEDULER_CHOICES_STR);
        return CKC_ERR_NOTIMPL;
    }
    ...
    const char *double_smem_buffer =
        (strcmp(trait->pipeline, "compv4") == 0) ? "true" : "false";
    ...
    L(out, line);
    snprintf(line, sizeof line, "//     tile_m=%d, tile_n=%d, tile_k=%d,",
             t->tile_m, t->tile_n, t->tile_k);
    L(out, line);
    snprintf(line, sizeof line, "//     warp_m=%d, warp_n=%d, warp_k=%d,",
             t->warp_m, t->warp_n, t->warp_k);
    L(out, line);
    snprintf(line, sizeof line, "//     warp_tile_m=%d, warp_tile_n=%d,",
             t->warp_tile_m, t->warp_tile_n);
    L(out, line);
    snprintf(line, sizeof line, "//     warp_tile_k=%d,", t->warp_tile_k);
    L(out, line);
    snprintf(line, sizeof line, "//     pipeline='%s', scheduler='%s',",
             trait->pipeline, trait->scheduler);
    L(out, line);
    snprintf(line, sizeof line, "//     epilogue='%s', pad=(%s,%s,%s),",
             trait->epilogue, py_bool(trait->pad_m), py_bool(trait->pad_n),
             py_bool(trait->pad_k));
    L(out, line);
    ...
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t M_Warp_Tile = %d;", t->warp_tile_m);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t N_Warp_Tile = %d;", t->warp_tile_n);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t K_Warp_Tile = %d;", t->warp_tile_k);
    L(out, line);
    L(out, "");
    ...
    L(out, line);
    L(out, "");
    snprintf(line, sizeof line, "    static constexpr bool DoubleSmemBuffer = %s;", double_smem_buffer);
    L(out, line);
    ...
    L(out, "");
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t NumWaveGroups = %d;", num_wave_groups);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr int kBlockPerCu = %d;", block_per_cu);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t TileParitionerGroupNum = %d;", tp_group_num);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t TileParitionerM01 = %d;", tp_m01);
    L(out, line);
    L(out, "");
    ...
    L(out, "    ck_tile::sequence<GemmConfig::M_Warp_Tile, GemmConfig::N_Warp_Tile, GemmConfig::K_Warp_Tile>,");
    ...
    L(out, "    typename ::PipelineTypeTraits<GemmConfig::Pipeline>::template GemmPipeline<UniversalGemmProblem>;");
    L(out, "");
...
ckc_status_t ckc_lower_implicit_gemm_conv_to_cktile(
    const ckc_cktile_conv_spec_t *spec,
    const char *kernel_name_override,
    ckc_strbuf_t *out,
    char err[CKC_ERR_MSG_CAP])
{
    ...
    if (!pipe) {
        ckc__err(err, "conv pipeline '%s'; supported %s",
                 spec->pipeline ? spec->pipeline : "", PIPELINE_CHOICES_STR);
        return CKC_ERR_NOTIMPL;
    }
    if (!spec->epilogue ||
        (strcmp(spec->epilogue, "cshuffle") != 0 &&
         strcmp(spec->epilogue, "default") != 0)) {
        ckc__err(err, "conv epilogue '%s'; supported 'cshuffle' / 'default'",
    ...
    const char *double_smem_buffer =
        (strcmp(spec->pipeline, "compv4") == 0) ? "true" : "false";
    ...
    snprintf(line, sizeof line,
             ...
             spec->name, p->N, p->Hi, p->Wi, p->C, p->K, p->R, p->S,
             p->sH, p->sW, p->pH, p->pW, p->dH, p->dW);
    L(out, line);
    snprintf(line, sizeof line, "//     tile_m=%d, tile_n=%d, tile_k=%d,",
             spec->tile_m, spec->tile_n, spec->tile_k);
    L(out, line);
    snprintf(line, sizeof line, "//     warp_m=%d, warp_n=%d,",
             spec->warp_m, spec->warp_n);
    L(out, line);
    snprintf(line, sizeof line, "//     warp_tile=(%d,%d,%d),",
             spec->warp_tile_m, spec->warp_tile_n, spec->warp_tile_k);
    L(out, line);
    snprintf(line, sizeof line, "//     pipeline='%s', epilogue='%s')",
             spec->pipeline, spec->epilogue);
    L(out, line);
    L(out, "//");
    ...
    L(out, "// Mirrors example/ck_tile/20_grouped_convolution/grouped_convolution_forward_invoker.hpp.");
    L(out, "// =========================================================");
    L(out, "");
    L(out, "#include <ck_tile/core.hpp>");
    L(out, "#include <ck_tile/host.hpp>");
    ...
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t M_Warp_Tile = %d;", spec->warp_tile_m);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t N_Warp_Tile = %d;", spec->warp_tile_n);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t K_Warp_Tile = %d;", spec->warp_tile_k);
    L(out, line);
    L(out, "");
    snprintf(line, sizeof line, "    static constexpr bool DoubleSmemBuffer = %s;", double_smem_buffer);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t NumWaveGroups = %d;", num_wave_groups);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t NumGroupsToMerge = %d;", num_groups_to_merge);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr int kBlockPerCu = %d;", block_per_cu);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t VectorSizeA = %d;", vector_size_a);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t VectorSizeB = %d;", vector_size_b);
    L(out, line);
    snprintf(line, sizeof line, "    static constexpr ck_tile::index_t VectorSizeC = %d;", vector_size_c);
    L(out, line);
    L(out, "");
    ...
    L(out, "    ck_tile::sequence<ConvConfig::M_Warp_Tile, ConvConfig::N_Warp_Tile, ConvConfig::K_Warp_Tile>>;");
    L(out, "");
    ...
    L(out, "");
    L(out, "using GemmPipeline =");
    L(out, "    typename ::PipelineTypeTraits<ConvConfig::Pipeline>::template GemmPipeline<UniversalGemmProblem>;");
    L(out, "");
    ...
    L(out, "// ``example/ck_tile/20_grouped_convolution/grouped_convolution_forward_invoker.hpp`` does.");
    snprintf(line, sizeof line, "extern \"C\" float launch_%s(", name);
    L(out, line);
...
ckc_status_t ckc_lower_spec_to_cktile(
    const ckc_cktile_spec_t *spec,
    const char *kernel_name_override,
    ckc_strbuf_t *out,
    char err[CKC_ERR_MSG_CAP])
{
    ...
    case CKC_CKTILE_SPEC_GEMM:
        return ckc_lower_universal_gemm_to_cktile(spec->u.gemm,
                                                  kernel_name_override, out, err);
    case CKC_CKTILE_SPEC_CONV:
        return ckc_lower_implicit_gemm_conv_to_cktile(spec->u.conv,
                                                      kernel_name_override, out, err);
    default:
        ckc__err(err,
