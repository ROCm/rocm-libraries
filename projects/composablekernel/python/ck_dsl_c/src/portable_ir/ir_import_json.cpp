 ...
 * (schema "ck.dsl.ir/v1", produced by ck_dsl.core.ir_export). See
 * ckc/ir_import.h for the contract.
 *
 * Two parts:
 *   1. a small dependency-free recursive-descent JSON parser -> tagged DOM;
 *   2. a walker that re-drives the DOM through the public ckc_b_* builder API.
 *
 ...
 * ckc_b_op path keyed by ckc_opcode_from_name.
 */
...
{
    jval_t* v = (jval_t*)ckc_arena_calloc(jp->arena, sizeof(jval_t));
    if (v)
        v->kind = k;
    return v;
}
...
static char* jp_string_raw(jparser_t* jp)
{
    ...
    char* out = (char*)ckc_arena_alloc(jp->arena, cap);
    if (!out) {
...
static jval_t* jp_array(jparser_t* jp)
{
    ...
    while (jp->p < jp->end) {
        ...
        if (v->arr_len == cap) {
            cap = cap ? cap * 2 : 8;
            jval_t** na = (jval_t**)ckc_arena_alloc(jp->arena, (size_t)cap * sizeof(jval_t*));
            if (!na) {
                jval_free(item);
                jval_free(v);
...
static jval_t* jp_object(jparser_t* jp)
{
    ...
    while (jp->p < jp->end) {
        ...
        if (v->obj_len == cap) {
            cap = cap ? cap * 2 : 8;
            jmember_t* nm =
                (jmember_t*)ckc_arena_alloc(jp->arena, (size_t)cap * sizeof(jmember_t));
            if (!nm) {
                jval_free(val);
                jval_free(v);
...
static jval_t* json_parse(const char* text, ckc_arena_t* arena, char* err, size_t err_cap)
{
    jparser_t jp;
    jp.p = text;
...
static void vmap_put(importer_t* im, const char* name, ckc_value_t* val)
{
    if (im->failed)
        return;
...
static ckc_value_t* vmap_get(importer_t* im, const char* name)
{
...
static void rename_value(importer_t* im, ckc_value_t* v, const char* exported_id)
{
    if (!v || !exported_id)
        return;
    char* copy = ckc_arena_strdup(&im->b->arena, exported_id);
    if (copy)
        v->name = copy;
}

/* Build a ckc_type_t from a JSON type node: scalar -> name string; composite
 * -> {"kind": ...}. */
static const ckc_type_t* import_type(importer_t* im, const jval_t* t)
{
    if (!t) {
    ...
    if (t->kind == J_OBJ) {
        ...
        if (strcmp(kind, "smem") == 0) {
            ...
            return ckc_smem_type(im->b, elem, dims, rank);
        }
...
/* Populate a ckc_attr_map_t from a JSON typed-attr object (keys -> {"t","v"}).
...
static void import_attrs(importer_t* im, const jval_t* attrs, ckc_attr_map_t* m)
{
    ckc_attr_map_init(m);
    if (!attrs)
        return;
    ...
    for (int i = 0; i < attrs->obj_len && !im->failed; i++) {
        ...
        if (strcmp(t, "i") == 0)
            ckc_attr_set_int(im->b, m, key, (int64_t)v->num);
        else if (strcmp(t, "f") == 0)
            ckc_attr_set_float(im->b, m, key, v->num);
        else if (strcmp(t, "b") == 0)
            ckc_attr_set_bool(im->b, m, key, v->b);
        else if (strcmp(t, "s") == 0)
            ckc_attr_set_str(im->b, m, key, v->str ? v->str : "");
        else
...
static void import_scf_for(importer_t* im, const jval_t* op)
{
    ...
    ckc_value_t* lo = vmap_get(im, jstr(operands->arr[0]));
    ckc_value_t* hi = vmap_get(im, jstr(operands->arr[1]));
    ckc_value_t* step = vmap_get(im, jstr(operands->arr[2]));
    if (!lo || !hi || !step) {
    ...
    if (num_iter > 0) {
        ...
        ckc_iter_arg_t* ia = (ckc_iter_arg_t*)calloc((size_t)num_iter, sizeof(ckc_iter_arg_t));
        ...
        f = ckc_b_scf_for_iter(im->b, lo, hi, step, ia, num_iter, strip_pct(iv_attr), unroll, elide);
        if (iv_attr) {
        ...
        f = ckc_b_scf_for(im->b, lo, hi, step, strip_pct(iv_attr));
        if (iv_attr) {
            rename_value(im, f.iv, iv_attr);
            vmap_put(im, iv_attr, f.iv);
        }
    }
    if (!ckc_ir_builder_ok(im->b)) {
        imp_fail(im, "scf.for build: %s", ckc_ir_builder_error(im->b));
        return;
    }
    ...
    ckc_b_region_enter(im->b, f.body);
    import_region_ops(im, regions->arr[0]);
    ckc_b_region_leave(im->b);
...
static void import_scf_if(importer_t* im, const jval_t* op)
{
    ...
    ckc_value_t* cond = vmap_get(im, jstr(operands->arr[0]));
    if (!cond) {
    ...
    int mark = im->len;
    ckc_if_t s = ckc_b_scf_if(im->b, cond);
    if (regions && regions->kind == J_ARR && regions->arr_len >= 1) {
        ckc_b_region_enter(im->b, s.then_region);
        import_region_ops(im, regions->arr[0]);
        ckc_b_region_leave(im->b);
    }
...
static void import_generic_op(importer_t* im, const jval_t* op, const char* opcode_name)
{
    ...
    ckc_value_t** ops = NULL;
    const ckc_type_t** rtypes = NULL;
    if (n_ops) {
        ops = (ckc_value_t**)calloc((size_t)n_ops, sizeof(ckc_value_t*));
        if (!ops) {
    ...
    if (n_res) {
        rtypes = (const ckc_type_t**)calloc((size_t)n_res, sizeof(ckc_type_t*));
        if (!rtypes) {
            free(ops);
    ...
    ckc_op_t* built = ckc_b_op(im->b, opcode, ops, n_ops, rtypes, n_res, &m, NULL, 0,
                               /*result_name_hint=*/NULL, /*loc=*/NULL);
    free(ops);
    free(rtypes);
    if (!built || !ckc_ir_builder_ok(im->b)) {
        imp_fail(im, "op '%s' build failed: %s", opcode_name,
                 ckc_ir_builder_ok(im->b) ? "null op" : ckc_ir_builder_error(im->b));
        return;
    }
...
static void import_params(importer_t* im, const jval_t* params)
{
    ...
    for (int i = 0; i < params->arr_len && !im->failed; i++) {
        ...
        const ckc_type_t* type = import_type(im, jobj_get(p, "type"));
        if (!name || !type) {
        ...
        ckc_value_t* pv = ckc_b_param(im->b, name, type, &opts);
        if (!pv) {
...
static void import_kernel_attrs(importer_t* im, const jval_t* attrs)
{
    if (!attrs || attrs->kind != J_OBJ)
        return;
    ckc_kernel_def_t* k = ckc_ir_builder_kernel(im->b);
    if (!k)
        return;
    for (int i = 0; i < attrs->obj_len && !im->failed; i++) {
        const char* key = attrs->obj[i].key;
        ...
        if (!t || !v)
            continue;
        if (strcmp(t, "i") == 0)
            ckc_attr_set_int(im->b, &k->attrs, key, (int64_t)v->num);
        else if (strcmp(t, "f") == 0)
            ckc_attr_set_float(im->b, &k->attrs, key, v->num);
        else if (strcmp(t, "b") == 0)
            ckc_attr_set_bool(im->b, &k->attrs, key, v->b);
        else if (strcmp(t, "s") == 0)
            ckc_attr_set_str(im->b, &k->attrs, key, v->str ? v->str : "");
    }
}
                                         ...
                                         size_t err_cap)
{
    ...
    ckc_arena_t parse_arena;
    if (ckc_arena_init(&parse_arena, 0) != 0) {
        if (err && err_cap)
    ...
    if (!root) {
        ...
        ckc_arena_destroy(&parse_arena);
        return CKC_ERR_VALUE;
    }

    const char* schema = jstr(jobj_get(root, "schema"));
    if (!schema || strcmp(schema, "ck.dsl.ir/v1") != 0) {
        if (err && err_cap)
            snprintf(err, err_cap, "bad/missing schema (want ck.dsl.ir/v1)");
        jval_free(root);
        ckc_arena_destroy(&parse_arena);
        return CKC_ERR_VALUE;
    }
    ...
    if (opts && opts->expected_kernel_name &&
        strcmp(opts->expected_kernel_name, kname) != 0) {
        if (err && err_cap)
            snprintf(err, err_cap, "kernel name '%s' != expected '%s'", kname,
