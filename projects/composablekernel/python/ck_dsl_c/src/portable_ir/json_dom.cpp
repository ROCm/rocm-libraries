...
{
    jd_val_t* v = (jd_val_t*)ckc_arena_calloc(jp->arena, sizeof(jd_val_t));
    if (v)
        v->kind = k;
    return v;
}
...
static jd_val_t* jdp_array(jdp_t* jp)
{
    ...
    while (jp->p < jp->end) {
        ...
        if (v->arr_len == cap) {
            int nc = cap ? cap * 2 : 8;
            jd_val_t** na = (jd_val_t**)ckc_arena_alloc(jp->arena, (size_t)nc * sizeof(jd_val_t*));
            if (!na) {
...
static jd_val_t* jdp_object(jdp_t* jp)
{
    ...
    while (jp->p < jp->end) {
        ...
        if (v->obj_len == cap) {
            int nc = cap ? cap * 2 : 8;
            jd_member_t* nm =
                (jd_member_t*)ckc_arena_alloc(jp->arena, (size_t)nc * sizeof(jd_member_t));
            if (!nm) {
