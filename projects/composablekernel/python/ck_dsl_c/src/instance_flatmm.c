...
ckc_status_t ckc_flatmm_kernel_name(const ckc_flatmm_spec_t* spec,
                                    char* out,
                                    size_t out_cap)
{
    ckc_gemm_universal_spec_t u;
                              ...
                              size_t reason_cap)
{
    ...
    if (spec->preshuffle_b)
    {
        ckc__copy_str(reason, reason_cap,
...
ckc_kernel_def_t* ckc_build_flatmm(ckc_ir_builder_t* b,
...
ckc_kernel_def_t* ckc_build_flatmm_new(ckc_ir_builder_t* b,
                                       const ckc_flatmm_spec_t* spec,
                                       const char* arch)
{
    char name[256];
 ...
 * (batch,1)). */
ckc_status_t ckc_flatmm_grid(const ckc_flatmm_spec_t* spec,
                             int batch,
                             int m,
                             int n,
                             int out[3])
...
ckc_status_t ckc_flatmm_preshuffle_b_layout(const ckc_flatmm_spec_t* spec,
                                            int n,
                                            int k,
                                            int out_shape[4],
                                            int out_strides[4])
