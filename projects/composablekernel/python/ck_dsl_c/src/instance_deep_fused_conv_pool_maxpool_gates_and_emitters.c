                                    ...
                                    const ckc_warp_grid_t* grid)
{
    ...
    return grid->warp_tile_m == 32 && grid->warp_tile_n == 32 &&
           ...
           ckc_warp_grid_mfmas_per_warp_n(NULL, grid) == 1 &&
           grid->warp_n == 1 && grid->warp_m == 2 && p->pool_stride_h == 2 &&
           p->pool_stride_w == 2 && conv_tile_h == 8 && conv_tile_w == 8 &&
           spec->tile_m == 64 &&
    ...
    const ckc_mma_op_t* op)
{
    ...
    return grid->wave_size == 32 && op->m == 16 && op->n == 16 &&
           grid->warp_tile_m == 16 && grid->warp_tile_n == 16 &&
           ckc_warp_grid_mfmas_per_warp_m(NULL, grid) == 2 && grid->warp_n == 1 &&
           p->pool_stride_h == 2 && p->pool_stride_w == 2 && conv_tile_w == 16 &&
           grid->warp_m == spec->pool_tile_h &&
           ckc_fused_conv_pool_problem_conv1_channels(p) <= 32;
}
    ...
    const ckc_conv_acc_epilogue_t* epilogue)
{
    ...
    for (ci = 0; ci < 3; ++ci)
    {
        ...
        if (out_k % cand == 0 &&
            (spec->pool_tile_h * spec->pool_tile_w * (out_k / cand)) >=
                block_size / 2)
        {
            kvec = cand;
            break;
    ...
    for (e = 0; e < elems_per_thread; ++e)
    {
        ...
        vec_idx = ckc_b_add(
            b, ckc_b_mul(b, ckc_b_const_i32(b, e), ckc_b_const_i32(b, block_size)),
            grid->tid);
        ...
        for (yy = 0; yy < 2; ++yy)
        {
            ...
            for (xx = 0; xx < 2; ++xx)
            {
                ...
                idx[1] = k0;
                v_vec =
                    ckc_b_smem_load_vN_f16(b, c_smem, idx, 2, kvec);
                for (j = 0; j < kvec; ++j)
                {
                    accs[j] = ckc_b_fmax(
                        b, accs[j],
                        ckc_b_cast_to_f32(b, ckc_b_vec_extract(b, v_vec, j)));
                }
            }
        }
        ...
        y_base_elems = ckc_b_add(
            ...
            ckc_b_mul(
                b,
                ckc_b_add(
                    b, ckc_b_mul(b, global_pho, ckc_b_const_i32(b, pool_wo)),
                    global_pwo),
                c_out_k),
            k0);
        for (j = 0; j < kvec; ++j)
        ...
        if (kvec >= 2)
        {
            ...
            ckc_b_buffer_store_vN_f16(b, y_rsrc, safe_base, ckc_b_const_i32(b, 0),
                                      y_vec, kvec / 2);
        }
        else
        {
            ckc_b_buffer_store_f16(b, y_rsrc, safe_base, ckc_b_const_i32(b, 0),
                                   halves[0]);
        }
    }
}
    ...
    const ckc_conv_acc_epilogue_t* epilogue)
{
    ...
    pwo_base =
        ckc_b_add(b, block_pool_w, ckc_b_mul(b, half, ckc_b_const_i32(b, 4)));
    ...
    {
        ckc_value_t* gpwo =
            ckc_b_add(b, pwo_base, ckc_b_const_i32(b, w_local));
        int s0 = 2 * w_local;
        int s1 = 2 * w_local + 1;
        ckc_value_t* pix_off = ckc_b_mul(
            b, ckc_b_add(b, row_off, gpwo), c_out_k); /* (gpho*pool_wo+gpwo)*out_k */
        int ni;
    ...
    const ckc_warp_grid_t* grid, const ckc_conv_acc_epilogue_t* epilogue)
{
    ...
    pwo_base =
        ckc_b_add(b, block_pool_w, ckc_b_mul(b, m_blk, ckc_b_const_i32(b, 2)));
    ...
    for (pho_l = 0; pho_l < 2; ++pho_l)
    {
        ...
        for (pwo_l = 0; pwo_l < 2; ++pwo_l)
        {
            ...
            ckc_value_t* acc = ckc_b_fmax(
                b,
                ckc_b_fmax(b, ckc_b_vec_extract(b, acc_vec, s0),
                           ckc_b_vec_extract(b, acc_vec, s1)),
                ckc_b_fmax(b, ckc_b_vec_extract(b, acc_vec, s2),
                           ckc_b_vec_extract(b, acc_vec, s3)));
            ckc_value_t* y_h;
            ...
            y_off_elems = ckc_b_add(
                ...
                ckc_b_mul(
                    b,
                    ckc_b_add(b, ckc_b_mul(b, gpho, c_pool_wo), gpwo),
                    c_out_k),
                channel);
