...
ckc_llvm_flavor_t ckc_llvm_flavor_from_name(const char *name) {
    if (name) {
        if (strcmp(name, "llvm20") == 0) return CKC_LLVM_FLAVOR_LLVM20;
        if (strcmp(name, "llvm22") == 0) return CKC_LLVM_FLAVOR_LLVM22;
    }
...
static ckc_ll_block_t *ll_make_block(ckc_lower_t *L, const char *label) {
    ckc_ll_block_t *blk = ckc_arena_calloc(&L->arena, sizeof(*blk));
    if (!blk) {
...
ckc_ll_block_t *ckc_ll_new_block(ckc_lower_t *L, const char *base) {
    ...
    L->block_counter += 1;
    char *label = ckc_arena_printf(&L->arena, "%s.%d",
...
void ckc_ll_block_emitf(ckc_lower_t *L, ckc_ll_block_t *blk,
                        const char *fmt, ...) {
    if (!L || !blk) {
        return;
    }
...
const char *ckc_ll_fresh(ckc_lower_t *L, const char *hint) {
    ...
    L->tmp_counter += 1;
    char *s = ckc_arena_printf(&L->arena, "%%%s.%d",
...
const char *ckc_ll_llvm_type(ckc_lower_t *L, const ckc_type_t *t) {
    ...
    if (t->kind == CKC_TYPE_PTR) {
        ...
        if (sp && strcmp(sp, "global") == 0)   return "ptr addrspace(1)";
        if (sp && strcmp(sp, "lds") == 0)       return "ptr addrspace(3)";
        if (sp && strcmp(sp, "constant") == 0)  return "ptr addrspace(4)";
        return "ptr";
    }
    ...
    const char *n = t->name;
    if (n) {
        if (strcmp(n, "i1") == 0)       return "i1";
        if (strcmp(n, "i8") == 0)       return "i8";
        if (strcmp(n, "i16") == 0)      return "i16";
        if (strcmp(n, "i32") == 0)      return "i32";
        if (strcmp(n, "i64") == 0)      return "i64";
        if (strcmp(n, "f16") == 0)      return "half";
        if (strcmp(n, "bf16") == 0)     return "bfloat";
        if (strcmp(n, "fp8e4m3") == 0)  return "i8";
        if (strcmp(n, "bf8e5m2") == 0)  return "i8";
...
const char *ckc_ll_llvm_type_from_name(ckc_lower_t *L, const char *name) {
    ...
    if (strcmp(name, "i8") == 0)       return "i8";
    if (strcmp(name, "f16") == 0)      return "half";
    if (strcmp(name, "bf16") == 0)     return "bfloat";
    if (strcmp(name, "f32") == 0)      return "float";
    if (strcmp(name, "fp8e4m3") == 0)  return "i8";
    if (strncmp(name, "vec<", 4) == 0) {
        ...
        if (xpos && end && end > xpos) {
            ...
            if (strcmp(elem, "f32") == 0)       lelem = "float";
            else if (strcmp(elem, "f16") == 0)  lelem = "half";
            else if (strcmp(elem, "bf16") == 0) lelem = "bfloat";
            else if (strcmp(elem, "i32") == 0)  lelem = "i32";
            else {
...
const char *ckc_ll_fp32_hex(ckc_lower_t *L, double x) {
    ...
    return ckc_arena_printf(&L->arena, "0x%016llX",
...
const char *ckc_ll_escape_asm_string(ckc_lower_t *L, const char *s) {
    ...
    size_t n = strlen(s);
    size_t cap = n * 4 + 1;
    char *out = ckc_arena_alloc(&L->arena, cap);
    if (!out) {
...
void ckc_ll_binop(ckc_lower_t *L, const ckc_op_t *op, const char *llvm_op) {
    if (!ckc_ll_live(L) || !op || op->num_results < 1 ||
        op->num_operands < 2) {
        return;
    }
    ...
    ckc_ll_emitf(L, "  %s = %s %s %s, %s",
                 res->name, llvm_op, ckc_ll_llvm_type(L, res->type),
                 ckc_ll_operand(L, a), ckc_ll_operand(L, b));
}

void ckc_ll_vector_binop(ckc_lower_t *L, const ckc_op_t *op,
                         const char *llvm_op) {
    if (!ckc_ll_live(L) || !op || op->num_results < 1 ||
        op->num_operands < 2) {
        return;
    ...
    const ckc_value_t *b = op->operands[1];
    ckc_ll_emitf(L, "  %s = %s %s %s, %s",
                 res->name, llvm_op, ckc_ll_llvm_type(L, a->type),
                 ckc_ll_operand(L, a), ckc_ll_operand(L, b));
}
...
void ckc_ll_collect_smem(ckc_lower_t *L, const ckc_region_t *region) {
    ...
    for (int i = 0; i < region->num_ops; i++) {
        ...
        if (op->opcode == CKC_OP_TILE_SMEM_ALLOC && op->num_results > 0) {
            ...
            char *gname = ckc_arena_printf(&L->arena, "@%s.%s",
...
const char *ckc_ll_smem_global_name(ckc_lower_t *L, const ckc_value_t *smem,
                                    const ckc_type_t **out_stype) {
    if (out_stype) {
        *out_stype = NULL;
    }
    if (!L || !smem) {
...
#define LL_APPEND_ATTR(txt)                                              \
    do {                                                                 \
        int _n = snprintf(buf + w, sizeof buf - w, "%s%s",              \
                          w ? " " : "", (txt));                          \
        if (_n > 0) { w += (size_t)_n; }                                 \
    } while (0)
    if (ckc_attr_get_bool(&p->attrs, "noalias", false))   LL_APPEND_ATTR("noalias");
    if (ckc_attr_get_bool(&p->attrs, "readonly", false))  LL_APPEND_ATTR("readonly");
    if (ckc_attr_get_bool(&p->attrs, "writeonly", false)) LL_APPEND_ATTR("writeonly");
    if (ckc_attr_get_bool(&p->attrs, "nocapture", true))  LL_APPEND_ATTR("nocapture");
    if (ckc_attr_get_bool(&p->attrs, "nonnull", false))   LL_APPEND_ATTR("nonnull");
    int64_t align = 0;
...
void ckc_ll_finalize(ckc_lower_t *L, ckc_strbuf_t *out) {
    ...
    for (size_t i = 0; i < L->smem_globals.len; i++) {
        ...
        const char *elem_name = (g->stype && g->stype->elem)
                                    ? g->stype->elem->name : NULL;
        bool elem_is_byte = elem_name &&
            (strcmp(elem_name, "i8") == 0 ||
    ...
    for (size_t i = 0; i < L->dyn_decls.len; i++) {
        ...
        if (ll_resolve_decl(L, k) == L->dyn_decls.data[i].decl &&
            ll_resolve_decl(L, k) != NULL) {
    ...
    /* Function header. */
    ckc_strbuf_appendf(out, "define amdgpu_kernel void @%s(",
                       L->kernel ? L->kernel->name : "");
    if (L->kernel) {
        for (int i = 0; i < L->kernel->num_params; i++) {
            ...
            ckc_strbuf_appendf(out, "%s%s%s %%%s",
                               i ? ", " : "", tstr, attrs, p->name);
        }
    }
    ...
    if (L->kernel) {
        ...
        if (ckc_attr_get_int(&L->kernel->attrs, "waves_per_eu", &wpe)) {
            ckc_strbuf_appendf(out,
                ...
                (long long)wpe, (long long)wpe);
        }
        const ckc_attr_value_t *agpr =
            ckc_attr_get(&L->kernel->attrs, "agpr_alloc");
        if (agpr) {
...
ckc_status_t ckc_lower_kernel_to_llvm_ex(const ckc_kernel_def_t *kernel,
    ...
    L.kernel = kernel;
    L.status = CKC_OK;
    L.err = ckc_arena_calloc(&L.arena, CKC_ERR_MSG_CAP);
...
ckc_status_t ckc_lower_kernel_to_llvm(const ckc_kernel_def_t *kernel,
                                      ...
                                      const char *arch,
                                      char **out_text) {
    return ckc_lower_kernel_to_llvm_ex(kernel, flavor, arch, out_text,
                                       NULL, 0);
}
