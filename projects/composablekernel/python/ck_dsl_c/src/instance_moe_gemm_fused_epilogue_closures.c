...
void ckc_moe_emit_gate_up_silu_epilogue_default(
    ckc_ir_builder_t* b,
    ...
    ckc_value_t* batch_off_c)
{
    ...
    for (int e = 0; e < vecs_per_thread; ++e)
    {
        ...
        ckc_value_t* in_bounds =
            ckc_moe_pad_in_bounds(b, c_m, c_n, M, N, pad_m, pad_n, store_vec);
...
void ckc_moe_emit_interleaved_silu_epilogue(
    ckc_ir_builder_t* b,
    const ckc_gemm_universal_spec_t* spec,
    ckc_value_t* const* accs,
    int num_accs,
    ...
    ckc_value_t* batch_off_c)
{
    ...
    int vec_h = 4;
    while (vec_h > 1
           && (hidden_cols_per_tile % vec_h != 0 || total_hidden % (threads * vec_h) != 0))
    {
        vec_h /= 2;
    }
    ...
    for (int u = 0; u < units_per_thread; ++u)
    {
        ...
        ckc_value_t* off =
            ckc_b_add(b, batch_off_c, ckc_b_add(b, ckc_b_mul(b, c_m, N), c_n_start));
