...
static ckc_value_t* ckc__lane_half_base(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    if (ctx->lane_half_base_v == NULL)
        ctx->lane_half_base_v =
            ckc_b_mul(ctx->b, ctx->tid, ckc_b_const_i32(ctx->b, 8));
                                            ...
                                            ckc_value_t* buf_idx)
{
    ...
    for (int call = 0; call < kv_calls_per_tile; ++call)
    {
        ...
        ckc_b_async_buffer_load_lds_addr(b, k_rsrc, k_dst, voff, zero_soff, 4,
                                         CKC_CACHE_STREAM);
    }
}
                                            ...
                                            ckc_value_t* buf_idx)
{
    ...
    for (int call = 0; call < kv_calls_per_tile; ++call)
    {
        ...
        ckc_b_async_buffer_load_lds_addr(b, v_rsrc, v_dst, voff, zero_soff, 4,
                                         CKC_CACHE_STREAM);
    }
}
