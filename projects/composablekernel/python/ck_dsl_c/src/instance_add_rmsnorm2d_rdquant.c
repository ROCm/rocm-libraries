...
static ckc_status_t ckc_x_make_naive_tensor_view_packed(ckc_tensor_view_t* out,
                                                        ckc_value_t* base,
                                                        const int* shape,
                                                        int rank,
                                                     ...
                                                     const char* out_dtype)
{
    ...
    if ((strcmp(out_dtype, "fp8e4m3") == 0 || strcmp(out_dtype, "bf8e5m2") == 0) &&
        (n % 4 == 0))
    {
        ...
        for (off = 0; off < n; off += 4)
        {
            ...
            packed_chunks[num_chunks++] =
                is_fp8 ? ckc_b_cvt_pk_fp8_f32x4(b, quad)
    ...
    /* i8 path (or VEC=2 fp8/bf8): per-element saturating cast + vec_pack. */
    {
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            if (strcmp(out_dtype, "i8") == 0)
            {
                ...
                ckc_value_t* clamped = ckc_b_clamp_f32(
                    b, sf, ckc_b_const_f32(b, -127.0), ckc_b_const_f32(b, 127.0));
            ...
            {
                (void)ckc_i_set_err(b, CKC_ERR_VALUE, "unsupported out_dtype %s",
...
static void ckc_x_store_packed_chunk_local(ckc_ir_builder_t* b,
                                           ckc_value_t* qy_ptr,
...
int ckc_add_rmsnorm2d_rdquant_elems_per_thread(
...
ckc_status_t ckc_add_rmsnorm2d_rdquant_kernel_name(
    const ckc_add_rmsnorm2d_rdquant_spec_t* spec, char* out, size_t out_cap)
{
    char nbuf[32];
    char bbuf[32];
    char vbuf[32];
    ...
    return ckc_kernel_name_join(spec->name, parts, 5, flag_names, flag_on, 2, out,
                                out_cap, NULL);
}
...
bool ckc_add_rmsnorm2d_rdquant_is_valid_spec(
    const ckc_add_rmsnorm2d_rdquant_spec_t* spec,
    const char* arch,
    char* reason,
    size_t reason_cap)
{
    ...
    /* if spec.out_dtype not in ("i8", "fp8e4m3", "bf8e5m2"): ... */
    {
        ...
        if (!ok_od)
        {
            snprintf(buf, sizeof(buf), "unsupported out_dtype %s%s%s",
                     od ? "'" : "", od ? od : "None", od ? "'" : "");
            ckc_arn_set_reason(reason, reason_cap, buf);
            return false;
        }
    ...
    is_fp8 = (strcmp(spec->out_dtype, "fp8e4m3") == 0 ||
              strcmp(spec->out_dtype, "bf8e5m2") == 0);
    if (is_fp8 && (target->family == NULL || strcmp(target->family, "cdna") != 0))
    {
        snprintf(buf, sizeof(buf),
                 ...
                 "out_dtype='i8'",
                 spec->out_dtype, arch, target->family ? target->family : "None");
        ckc_arn_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    if (spec->block_size > ckc_arch_max_threads_per_block(target))
    {
        snprintf(buf, sizeof(buf),
                 ...
                 spec->block_size, ckc_arch_max_threads_per_block(target), arch);
        ckc_arn_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    /* bytes_lds = 2 * block_size * 4; if not target.fits_lds(bytes_lds): ... */
    {
        ...
        {
            snprintf(buf, sizeof(buf), "LDS budget %ld > %d cap on %s",
...
static ckc_value_t* ckc_arn_fadd_cb(ckc_ir_builder_t* b, ckc_value_t* a,
                                    ckc_value_t* c, void* user)
{
    (void)user;
    return ckc_b_fadd(b, a, c);
}
static ckc_value_t* ckc_arn_fmax_cb(ckc_ir_builder_t* b, ckc_value_t* a,
                                    ckc_value_t* c, void* user)
{
    (void)user;
...
static void ckc_arn_pass1_body(ckc_ir_builder_t* b,
                               ckc_value_t* n_off,
                               ckc_value_t* const* a_scalars,
                               int vec,
                               void* user)
{
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_add_rmsnorm2d_rdquant(
    ckc_ir_builder_t* b,
    const ckc_add_rmsnorm2d_rdquant_spec_t* spec,
    const char* arch)
{
    ...
    {
        (void)ckc_i_set_err(b, CKC_ERR_VALUE,
                            "invalid add_rmsnorm2d_rdquant spec: %s", reason);
        return NULL;
    }
    ...
    (void)ckc_sweep_row_chunks(b, &a_tile, tid, BS, VEC, elems_per_thread,
                               NULL /* row */, ckc_arn_pass1_body, &p1,
                               false /* cache */);

    /* total_sq, total_amax_g = block_lds_reduce_pair(
     *     b, s_sq, s_amax_g, lds_sum, lds_max, tid, block_size=BS,
     *     combine_a="sum", combine_c="max") */
    (void)ckc_block_lds_reduce_pair(b, p1.s_sq, p1.s_amax_g, lds_sum, lds_max,
                                    tid, BS, CKC_REDUCE_SUM, CKC_REDUCE_MAX,
                                    &total_sq, &total_amax_g);
    ...
    for (k = 0; k < chunks; ++k)
    {
        ...
        if (use_packed_store)
        {
            ...
            packed = ckc_x_pack_quant_chunk_local_f32(b, scaled_f32, VEC, q_ty,
                                                      spec->out_dtype);
        ...
        else
        {
            ...
            for (i = 0; i < VEC; ++i)
            {
                ...
                ckc_value_t* q = ckc_x_quantize_scalar_f32(b, y_f32, inv_yscale,
                                                           spec->out_dtype, q_ty);
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_add_rmsnorm2d_rdquant_new(
    ckc_ir_builder_t* b,
    const ckc_add_rmsnorm2d_rdquant_spec_t* spec,
    const char* arch)
{
 ...
 * ===================================================================== */
ckc_status_t ckc_add_rmsnorm2d_rdquant_grid(
    int m, const ckc_add_rmsnorm2d_rdquant_spec_t* spec, int out[3])
{
    int totals[2];
    int tiles[2];
    (void)spec;
 ...
 * ===================================================================== */
ckc_status_t ckc_add_rmsnorm2d_rdquant_lower_to_llvm(
    const ckc_add_rmsnorm2d_rdquant_spec_t* spec,
    const char* arch,
