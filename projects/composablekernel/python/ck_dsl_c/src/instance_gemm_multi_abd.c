...
static void ckc_abd_fail(ckc_ir_builder_t* b, ckc_status_t st, const char* msg)
{
    ...
    b->status = st;
    snprintf(b->err, CKC_ERR_MSG_CAP, "%s", msg ? msg : "");
}
                                            ...
                                            size_t out_cap)
{
    ...
    else
    {
        ...
        for (i = 0; i < spec->num_d_operands; ++i)
        {
            ...
            n = snprintf(d_suffix + off, sizeof(d_suffix) - off, "%s%s",
                         d->param_name ? d->param_name : "", ckc_abd_d_op_str(d));
                                      ...
                                      size_t reason_cap)
{
    ...
    if (spec->num_a_operands > CKC_GEMM_ABD_MAX_A)
    {
        if (reason && reason_cap)
        {
            snprintf(reason, reason_cap, "num_a %zu > MAX_A %d", spec->num_a_operands,
                     CKC_GEMM_ABD_MAX_A);
        }
        return false;
    }
    if (spec->num_b_operands > CKC_GEMM_ABD_MAX_B)
    {
        if (reason && reason_cap)
        {
            snprintf(reason, reason_cap, "num_b %zu > MAX_B %d", spec->num_b_operands,
                     CKC_GEMM_ABD_MAX_B);
        }
        return false;
    }
    if (spec->num_d_operands > CKC_GEMM_MULTI_D_MAX_D)
    {
        if (reason && reason_cap)
        {
            snprintf(reason, reason_cap, "num_d %zu > MAX_D %d", spec->num_d_operands,
                     CKC_GEMM_MULTI_D_MAX_D);
        }
        return false;
    }
    if (spec->base.trait.epilogue == NULL ||
        strcmp(spec->base.trait.epilogue, "cshuffle") != 0)
    {
        if (reason && reason_cap)
        {
    ...
    for (i = 0; i < spec->num_d_operands; ++i)
    {
        ...
        if (name &&
            (strcmp(name, "M") == 0 || strcmp(name, "N") == 0 ||
             strcmp(name, "K") == 0 || strcmp(name, "C") == 0))
        {
            if (reason && reason_cap)
            {
        ...
        for (ai = 0; ai < spec->num_a_operands; ++ai)
        {
            ...
            for (bi = 0; bi < spec->num_b_operands; ++bi)
            {
                ...
                if (an && bn && strcmp(an, bn) == 0)
                {
                    if (reason && reason_cap)
                    {
                        snprintf(reason, reason_cap, "A and B share param_names ['%s']",
                                 an);
                    }
                    return false;
                }
        ...
        for (ai = 0; ai < spec->num_a_operands; ++ai)
        {
            ...
            for (di = 0; di < spec->num_d_operands; ++di)
            {
                ...
                if (an && dn && strcmp(an, dn) == 0)
                {
                    if (reason && reason_cap)
                    {
                        snprintf(reason, reason_cap, "A and D share param_names ['%s']",
                                 an);
                    }
                    return false;
                }
        ...
        for (bi = 0; bi < spec->num_b_operands; ++bi)
        {
            ...
            for (di = 0; di < spec->num_d_operands; ++di)
            {
                ...
                if (bn && dn && strcmp(bn, dn) == 0)
                {
                    if (reason && reason_cap)
                    {
                        snprintf(reason, reason_cap, "B and D share param_names ['%s']",
                                 bn);
                    }
                    return false;
                }
    ...
    /* Arch-aware base GEMM validation (delegates to gemm_universal). */
    {
        ...
        bool ok_base = ckc_gemm_universal_is_valid_spec(&spec->base, arch, base_reason,
                                                        sizeof(base_reason));
        if (!ok_base)
        {
            if (reason && reason_cap)
                                           ...
                                           const char* arch)
{
    ...
    if (spec->num_a_operands > 1 || spec->num_b_operands > 1)
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 spec->num_a_operands, spec->num_b_operands);
    ...
    if (arena == NULL)
    {
        ckc_abd_fail(b, CKC_ERR_VALUE,
                     "build_gemm_multi_abd: num_d > 0 requires a non-NULL arena");
        return NULL;
    }
                                          ...
                                          size_t* out_count)
{
    ...
    {
        ckc_signature_builder_ptr(&sb, spec->a_operands[i].name,
                                  spec->a_operands[i].dtype, NULL);
    }
    /* for name, dtype in b_operands: sb.ptr(name, dtype) */
    for (i = 0; i < spec->num_b_operands; ++i)
    {
        ckc_signature_builder_ptr(&sb, spec->b_operands[i].name,
                                  spec->b_operands[i].dtype, NULL);
    }
    /* sb.ptr("C", base.data.dtype_c).scalar("M","i32").scalar("N","i32")
     *   .scalar("K","i32") */
    ...
    for (i = 0; i < spec->num_d_operands; ++i)
    {
        ckc_signature_builder_ptr(&sb, spec->d_operands[i].param_name, spec->d_dtype,
                                  NULL);
    }
 ...
 *       m, n, batch)
 */
ckc_status_t ckc_gemm_multi_abd_grid(const ckc_gemm_multi_abd_spec_t* spec,
                                     int m,
                                     int n,
                                     int batch,
                                     int out[3])
