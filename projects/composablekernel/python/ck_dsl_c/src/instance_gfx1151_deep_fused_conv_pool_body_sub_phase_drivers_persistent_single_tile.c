...
void ckc_gfx1151_dfcp_emit_persistent_body(ckc_gfx1151_dfcp_build_ctx_t* ctx)
{
    ...
    ckc_b_region_enter(b, loop.body);
    {
        ...
        ckc_gfx1151_dfcp_stage_input_footprint_int(ctx, ctx->X, ctx->a0_smem,
                                                   ctx->h_blk, ctx->w_blk);
        ...
        ckc_b_sync(b);
        ckc_gfx1151_dfcp_emit_maxpool_finalpack_i8(ctx, ctx->c1_smem, ctx->Y,
                                                   ctx->h_blk, ctx->w_blk);
    }
...
void ckc_gfx1151_dfcp_emit_single_tile_body(ckc_gfx1151_dfcp_build_ctx_t* ctx)
{
    ...
    else if (spec->direct_conv0)
    {
        ...
        ckc_b_sync(b);
        ckc_gfx1151_dfcp_wmma_gemm_conv0_direct(ctx,
                                                ctx->op,
                                                ctx->a0_smem,
                                                ctx->w0_smem,
                                                ctx->policy,
