...
static ckc_value_t* ckc_sq_tree_fmax(ckc_ir_builder_t* b,
                                     ckc_value_t** values,
                                     int n,
                                     ckc_value_t** scratch)
{
...
static int ckc_sq_pick_traits(const ckc_tile_distribution_encoding_t* enc,
                              int* vector_dim_y)
{
    int num_Y = enc->num_Y;
...
static int ckc_sq_y_to_linear(const ckc_tile_distribution_encoding_t* enc,
                              const int* y)
{
    int off = 0;
    int i;
                                            ...
                                            const char* qname)
{
        ...
        (n % 4) == 0)
    {
        ...
        {
            ckc_value_t* quad =
                ckc_b_vec_pack(b, &scaled[off], 4, ckc_f32());
...
ckc_status_t ckc_smoothquant_kernel_name(const ckc_smoothquant_spec_t* spec,
                                         char* out,
                                         size_t out_cap)
{
    char nbuf[32];
...
static ckc_status_t ckc_sq_make_packed_view(ckc_tensor_view_t* out,
                                            ckc_value_t* base,
                                            int n,
