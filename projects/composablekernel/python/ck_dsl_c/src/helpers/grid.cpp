...
/*
 * helper_ck_dsl.helpers.grid.c -- C99 port of ck_dsl.helpers.grid.
 *
 ...
 * ------------------------------------------------------------------------ */
static ckc_value_t* ckc_i_chiplet_transform_chunked_dynamic(
    ckc_ir_builder_t* b, ckc_value_t* wgid, ckc_value_t* num_wgs, int num_xcds, int chunk_size)
{
    int block;
    ckc_value_t* c_num_xcds;
    ckc_value_t* c_chunk_size;
    ckc_value_t* c_block;
    ckc_value_t* limit;
    ckc_value_t* xcd;
    ckc_value_t* local_pid;
    ckc_value_t* chunk_idx;
    ckc_value_t* pos_in_chunk;
    ckc_value_t* new_wgid;
    ckc_value_t* in_full_block;
    ...
    return ckc_b_select(b, in_full_block, new_wgid, wgid);
}
 ...
 * ------------------------------------------------------------------------ */
static ckc_super_tile_swizzle_result_t ckc_i_super_tile_swizzle_dynamic(
    ckc_ir_builder_t* b, ckc_value_t* wgid, ckc_value_t* num_pid_m, ckc_value_t* num_pid_n, int wgm)
{
    ckc_super_tile_swizzle_result_t res;
    ckc_value_t* c_wgm;
    ckc_value_t* num_wgid_in_group;
    ckc_value_t* group_id;
    ckc_value_t* first_pid_m;
    ckc_value_t* rem;
    ckc_value_t* use_wgm;
    ckc_value_t* group_size_m;
    ckc_value_t* local_id;
    ...
    if(wgm <= 0)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "super_tile_swizzle_dynamic: wgm=%d must be > 0", wgm);
        return res;
    }
 ...
 *   return b.select(in_full_block, new_wgid, wgid)
 * ------------------------------------------------------------------------ */
static ckc_value_t* ckc_i_chiplet_transform_chunked(
    ckc_ir_builder_t* b, ckc_value_t* wgid, int num_wgs, int num_xcds, int chunk_size)
{
    int block;
    int limit;
    ckc_value_t* c_num_xcds;
 ...
 *   pid_n        = b.div(local_id, group_size_m)
 * ------------------------------------------------------------------------ */
static ckc_super_tile_swizzle_result_t ckc_i_super_tile_swizzle(
    ckc_ir_builder_t* b, ckc_value_t* wgid, int num_pid_m, int num_pid_n, int wgm)
{
    ckc_super_tile_swizzle_result_t res;
    int num_wgid_in_group;
    ckc_value_t* c_wgm;
    ckc_value_t* c_num_pid_m;
    ckc_value_t* c_num_wgid_in_group;
    ckc_value_t* group_id;
    ckc_value_t* first_pid_m;
    ckc_value_t* rem;
    ckc_value_t* use_wgm;
