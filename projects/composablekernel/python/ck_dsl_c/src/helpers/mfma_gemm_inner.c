 ...
 *
 * helper_ck_dsl.helpers.mfma_gemm_inner.c -- C99 port of nine symbols from
 * ck_dsl/helpers/mfma_gemm_inner.py. See the header for the symbol map and the
...
static const ckc_type_t* ckc_i_ir_type_for_dtype(const char* dtype_in)
{
    if (dtype_in == NULL)
    {
        return NULL;
...
static bool ckc_i_dtype_align2(const char* dtype_in)
{
    return dtype_in != NULL
           && (strcmp(dtype_in, "f16") == 0 || strcmp(dtype_in, "bf16") == 0);
}
 ...
 */
ckc_lane_decode_t ckc_decode_mfma_lanes(ckc_ir_builder_t* b,
 ...
 * lookup ckc_mfma_atom(dtype, m, n, k). The (dtype, m, n, k) tuples come straight
...
ckc_mfma_atom_for_dtype(const char* dtype_in, int m, int n, bool prefer_packed_k)
{
    ...
    if (strcmp(dtype_in, "f16") == 0 || strcmp(dtype_in, "fp16") == 0)
    {
        if (m == 16 && n == 16)
        {
            return prefer_packed_k ? ckc_mfma_atom("f16", 16, 16, 32)
                                   : ckc_mfma_atom("f16", 16, 16, 16);
        }
        if (m == 32 && n == 32)
        {
            return prefer_packed_k ? ckc_mfma_atom("f16", 32, 32, 16)
                                   : ckc_mfma_atom("f16", 32, 32, 8);
        }
    }
    /* if dtype_in == "bf16": */
    if (strcmp(dtype_in, "bf16") == 0)
    {
        if (m == 16 && n == 16)
        {
            return prefer_packed_k ? ckc_mfma_atom("bf16", 16, 16, 32)
                                   : ckc_mfma_atom("bf16", 16, 16, 16);
        }
        if (m == 32 && n == 32)
        {
            return ckc_mfma_atom("bf16", 32, 32, 16);
        }
    }
    /* if dtype_in == "fp8e4m3": */
    if (strcmp(dtype_in, "fp8e4m3") == 0)
    {
        ...
        if (m == 32 && n == 32)
        {
            return ckc_mfma_atom("fp8e4m3", 32, 32, 16);
        }
    }
    /* if dtype_in == "bf8e5m2": */
    if (strcmp(dtype_in, "bf8e5m2") == 0)
    {
        ...
        if (m == 32 && n == 32)
        {
            return ckc_mfma_atom("bf8e5m2", 32, 32, 16);
        }
    }
    /* raise ValueError(...) */
    return NULL;
    ...
    ckc_ir_builder_t* b, const char* dtype_in, int m, int n, bool prefer_packed_k)
{
    ...
    atom = ckc_mfma_atom_for_dtype(dtype_in, m, n, prefer_packed_k);
    if (atom == NULL)
    {
        ...
        return (const ckc_mfma_atom_t*)ckc_i_set_err(
            ...
            dtype_in ? "'" : "", dtype_in ? dtype_in : "None", dtype_in ? "'" : "",
            m, n);
    }
    return atom;
}
                                             ...
                                             int K)
{
    ...
    if (!ckc_i_live(b) || atom == NULL || lane_decode == NULL)
    {
        return NULL;
    }

    dtype_ir = ckc_i_ir_type_for_dtype(atom->dtype_in);
    if (dtype_ir == NULL)
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "unsupported dtype %s%s%s",
            atom->dtype_in ? "'" : "", atom->dtype_in ? atom->dtype_in : "None",
            atom->dtype_in ? "'" : "");
    }
    ...
    {
        return ckc_b_global_load_vN(b, A, a_addr, dtype_ir, atom->a_per_lane,
                                    atom->a_per_lane * 2);
    }

    /* fp8 / bf8: no vec load helper -- fall back to scalar loads. */
    out = ckc_b_zero_vec(b, dtype_ir, atom->a_per_lane);
                                            ...
                                            int N)
{
    ...
    out = ckc_b_zero_vec(b, dtype_ir, atom->b_per_lane);
                             ...
                             void* user)
{
    ...
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "mfma_k_loop: atom/load_a/load_b must be non-NULL");
    }
    ...
    if (atom->k == 0 || (K % atom->k) != 0)
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE,
            "mfma_k_loop: K=%d must be divisible by atom.k=%d", K, atom->k);
    }
    n_tiles = K / atom->k;

    /* acc0 = initial_acc if initial_acc is not None else atom.zero_acc(b) */
    acc0 = (initial_acc != NULL) ? initial_acc : ckc_b_zero_vec_f32(b, atom->c_per_lane);
    ...
    kloop = ckc_b_scf_for_iter(b, lb, ub, step, loop_args, 1,
    ...
    ckc_b_region_enter(b, kloop.body);
    {
        ...
        new_acc = ckc_b_mma(b, atom->name, a_vec, b_vec, acc_v, NULL, 0);
    ...
    if (!ckc_i_live(b) || kloop.op == NULL || kloop.op->num_results < 1)
    {
        return NULL;
    }
...
static bool ckc_i_lane_to_output(ckc_ir_builder_t* b,
                                 const ckc_mfma_atom_t* atom,
                                 ckc_value_t* lane,
                                 int i,
                                 ckc_value_t** out_row,
                                 ckc_value_t** out_col)
{
    ...
    ckc_i_set_err(b, CKC_ERR_NOTIMPL,
                  "no lane_to_output dispatch for atom %dx%d", atom->m, atom->n);
    return false;
}
                                     ...
                                     void* epilogue_user)
{
    ...
    if (atom == NULL || lane_decode == NULL)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "store_acc_to_global: atom/lane_decode must be non-NULL");
        return b->status;
    }
    ...
    if (is_f32)
    {
        out_dtype_ir = ckc_f32();
    }
    else
    {
        out_dtype_ir = ckc_i_ir_type_for_dtype(out_dtype);
        if (out_dtype_ir == NULL)
        {
            ckc_i_set_err(b, CKC_ERR_VALUE, "unsupported dtype '%s'", out_dtype);
            return b->status;
        }
    }
    ...
    for (i = 0; i < atom->c_per_lane; ++i)
    {
        ...
        if (!ckc_i_lane_to_output(b, atom, lane_decode->lane, i, &row_in, &col_in))
        {
            return b->status;
        }
        ...
        {
            val = ckc_b_cast_f32_to(b, c_f32, out_dtype_ir);
        }
        ...
        if (atomic_add)
        {
            ...
            {
                ckc_i_set_err(b, CKC_ERR_VALUE,
                              "atomic_add output requires out_dtype='f32'");
                return b->status;
            }
            ckc_b_global_atomic_add(b, C, addr, val, NULL);
        }
        else
        {
            ckc_b_global_store(b, C, addr, val, is_f32 ? 4 : 2);
        }
    }
    return b->status;
}
                                      ...
                                      const ckc_archtarget_t** out_target)
{
    ...
    if (target == NULL)
    {
        if (out_reason != NULL)
        {
            ...
            {
                int wrote;
                wrote = snprintf(known + pos, sizeof(known) - pos, "%s'%s'",
                                 (k == 0) ? "" : ", ", arches[k]);
                if (wrote < 0)
                {
                    break;
            ...
            if (b != NULL)
            {
                *out_reason = ckc_arena_printf(
                    &b->arena,
                    "\"unknown gfx target '%s'; known: %s. Add a row to "
                    "arch_specs.json.\"",
                    arch ? arch : "None", known);
            }
            else
            {
    ...
    if (block_size > ckc_archtarget_max_threads_per_block(target))
    {
        ...
        if (out_reason != NULL)
        {
            ...
            {
                *out_reason = ckc_arena_printf(
                    &b->arena, "block_size %d > %d (hardware cap) on %s",
                    block_size, ckc_archtarget_max_threads_per_block(target),
                    arch ? arch : "None");
            }
            else
            {
                                               ...
                                               const char* where)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "validate_mfma_atom_in_catalog: atom must be non-NULL");
        return b->status;
    }
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "unknown gfx target '%s'", arch ? arch : "None");
        return b->status;
    }
    mma = ckc_archtarget_mma(target);

    /* if not target.mma.has_shape(...): raise NotImplementedError(...) */
    if (!ckc_mma_catalog_has_shape(mma, /*family=*/NULL, atom->dtype_in,
                                   atom->dtype_in, atom->dtype_out, atom->m,
                                   atom->n, atom->k))
    {
        ckc_i_set_err(
            b, CKC_ERR_NOTIMPL,
            ...
            where ? where : "None", atom->name ? atom->name : "None",
            atom->dtype_in ? atom->dtype_in : "None", atom->m, atom->n, atom->k,
            arch ? arch : "None");
        return b->status;
    }
    return CKC_OK;
