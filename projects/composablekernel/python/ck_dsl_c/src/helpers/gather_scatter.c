 ...
 *
 * helper_ck_dsl.helpers.gather_scatter.c -- C99 port of
 * ck_dsl.helpers.gather_scatter.load_sorted_token_id and
...
#include "ckc/helper_ck_dsl.helpers.gather_scatter.h"
                                        ...
                                        ckc_value_t* bucket_idx)
{
    ...
    if (pty->pointee == NULL || pty->pointee->name == NULL ||
        strcmp(pty->pointee->name, "i32") != 0)
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "SortedTokenIds must be ptr<i32>, got ptr<%s>",
            (pty->pointee && pty->pointee->name) ? pty->pointee->name : "None");
    }

    /* return b.global_load_i32(sorted_token_ids, bucket_idx)
     * Python passes no align kwarg -> default; 0 matches global_load_i32. */
    return ckc_b_global_load_i32(b, sorted_token_ids, bucket_idx, 0);
}
                                           ...
                                           ckc_value_t* bucket_idx)
{
    ...
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "SortedWeights must be ptr<f32>, got ptr<%s>",
            (pty->pointee && pty->pointee->name) ? pty->pointee->name : "None");
    }

    /* return b.global_load_f32(sorted_weights, bucket_idx)
     * Python passes no align kwarg -> default; 0 matches global_load_f32. */
    return ckc_b_global_load_f32(b, sorted_weights, bucket_idx, 0);
}
