                                             ...
                                             int K)
{
    ...
    {
        return ckc_b_global_load_vN(b, A, a_addr, dtype_ir, atom->a_per_lane, atom->a_per_lane * 2);
    }
                             ...
                             void* user)
{
    ...
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "mfma_k_loop: K=%d must be divisible by atom.k=%d", K, atom->k);
    }
    n_tiles = K / atom->k;
                                     ...
                                     void* epilogue_user)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "store_acc_to_global: atom/lane_decode must be non-NULL");
        return b->status;
    }
    ...
    for(i = 0; i < atom->c_per_lane; ++i)
    {
        ...
        if(atomic_add)
        {
            ...
            if(!is_f32)
            {
                ckc_i_set_err(b, CKC_ERR_VALUE, "atomic_add output requires out_dtype='f32'");
                return b->status;
            }
                                      ...
                                      const ckc_archtarget_t** out_target)
{
    ...
    if(target == NULL)
    {
        if(out_reason != NULL)
        {
            ...
            if(b != NULL)
            {
                *out_reason = ckc_arena_printf(&b->arena,
                                               ...
                                               const char* where)
{
    ...
    if(atom == NULL)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "validate_mfma_atom_in_catalog: atom must be non-NULL");
        return b->status;
    }
    ...
    if(target == NULL)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "unknown gfx target '%s'", arch ? arch : "None");
        return b->status;
    }
    mma = ckc_archtarget_mma(target);

    /* if not target.mma.has_shape(...): raise NotImplementedError(...) */
    if(!ckc_mma_catalog_has_shape(mma,
                                  /*family=*/NULL,
                                  atom->dtype_in,
                                  atom->dtype_in,
                                  atom->dtype_out,
