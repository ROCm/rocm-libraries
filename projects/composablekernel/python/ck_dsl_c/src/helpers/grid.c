 ...
 * ------------------------------------------------------------------------ */
static ckc_value_t* ckc_i_chiplet_transform_chunked_dynamic(ckc_ir_builder_t* b,
                                                            ckc_value_t* wgid,
                                                            ckc_value_t* num_wgs,
                                                            int num_xcds,
                                                            int chunk_size)
 ...
 * ------------------------------------------------------------------------ */
static ckc_super_tile_swizzle_result_t
ckc_i_super_tile_swizzle_dynamic(ckc_ir_builder_t* b,
                                 ckc_value_t* wgid,
                                 ...
                                 int wgm)
{
    ...
    {
        ckc_i_set_err(
            b, CKC_ERR_VALUE, "super_tile_swizzle_dynamic: wgm=%d must be > 0", wgm);
        return res;
    }
 ...
 *   return b.select(in_full_block, new_wgid, wgid)
 * ------------------------------------------------------------------------ */
static ckc_value_t* ckc_i_chiplet_transform_chunked(ckc_ir_builder_t* b,
                                                    ckc_value_t* wgid,
                                                    int num_wgs,
                                                    int num_xcds,
                                                    int chunk_size)
 ...
 *   pid_n        = b.div(local_id, group_size_m)
 * ------------------------------------------------------------------------ */
static ckc_super_tile_swizzle_result_t ckc_i_super_tile_swizzle(ckc_ir_builder_t* b,
                                                                ckc_value_t* wgid,
                                                                int num_pid_m,
                                                                int num_pid_n,
                                                                int wgm)
                                                            ...
                                                            int chunk_size)
{
    ...
    ckc_value_t* remapped =
        ckc_i_chiplet_transform_chunked(b, wgid, num_wgs, num_xcds, chunk_size);
