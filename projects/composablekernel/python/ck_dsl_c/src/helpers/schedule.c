...
ckc_hotloop_inst_list_t ckc_hotloop_inst_list_from_geometry(
    ckc_ir_builder_t* b,
    ...
    int b_packed_size)
{
    ...
    il.c_mfma_inst_num = m_per_block * n_per_block * k_per_block
...
{
    return ckc_hlil_a_read16(il) ? il->a_lds_read_inst_num
                                 : il->a_lds_read_inst_num / 2;
}

int ckc_hlil_num_ds_read_inst_b(const ckc_hotloop_inst_list_t* il)
{
    return ckc_hlil_b_read16(il) ? il->b_lds_read_inst_num
                                 : il->b_lds_read_inst_num / 2;
}
...
void ckc_schedule_policy_emit_mfma_setprio_bookend(
    const ckc_schedule_policy_t* p,
    ckc_ir_builder_t* b,
    void (*emit_mfma_fn)(ckc_ir_builder_t* b, void* user),
    void* user)
{
    if (strcmp(p->mode, "interwave") == 0)
    {
                                             ...
                                             bool force)
{
    ...
    for (i = 0; i < num_buffer_load_inst; ++i)
    {
        ...
        ckc_b_sched_group_barrier(b, CKC_SCHED_VMEM_READ, 1, 0);
        ckc_b_sched_group_barrier(
            b, CKC_SCHED_MFMA, il->c_mfma_inst_num / num_issue - 3, 0);
    }
