...
static ckc_value_t* ckc_red_emit_combine(ckc_ir_builder_t* b,
    ...
    ckc_ir_builder_t* b, ckc_combine_fn combine, void* user, ckc_value_t* const* xs, int n)
{
    ...
    if(n < 1)
    {
        return (ckc_value_t*)ckc_red_set_err(
            b, CKC_ERR_VALUE, "tree_reduce requires at least one value");
    }
...
static ckc_value_t* ckc_red_tree_reduce_scalars(ckc_ir_builder_t* b,
                                                ckc_reduce_combine_t combine,
                                                ckc_value_t* const* parts,
                                                int n)
{
...
static ckc_value_t* ckc_red_warp_xor_reduce(ckc_ir_builder_t* b,
                                            ckc_value_t* val,
                                            ckc_reduce_combine_t combine,
                                            int wave_size)
{
                                    ...
                                    ckc_value_t** out_idx)
{
    ...
    {
        ckc_red_set_err(
            b, CKC_ERR_VALUE, "unknown index combine %d; expected argmax or argmin", (int)combine);
        return 0;
    }
    if(!ckc_red_is_f32(val))
    {
    ...
    for(shift = 0; shift < cluster_len_shift; ++shift)
    {
        ...
        ckc_b_region_enter(b, iff.then_region);
        {
            ...
            i_b = ckc_b_bitcast(
                b,
                ckc_b_vec_extract(b, ckc_b_smem_load_vN_f32(b, lds_idx, j_idx, 1, 1), 0),
                ckc_i32());
