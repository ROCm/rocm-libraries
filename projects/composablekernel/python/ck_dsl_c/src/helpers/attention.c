...
[[noreturn]] static void* ckc_attn_set_err(ckc_ir_builder_t* b, ckc_status_t st, const char* fmt, ...)
{
    (void)b;
                                      ...
                                      ckc_value_t* neg_inf)
{
    ...
    else
    {
        ...
        return (ckc_value_t*)ckc_attn_set_err(b, CKC_ERR_VALUE, "unknown mask_mode %d", (int)mask_mode);
    }
                                        ...
                                        const ckc_type_t* dtype)
{
    ...
    if(dtype == NULL)
    {
        return (ckc_value_t*)ckc_attn_set_err(b, CKC_ERR_VALUE, "dequant_fp8x8_to_dtype: dtype is NULL");
    }
