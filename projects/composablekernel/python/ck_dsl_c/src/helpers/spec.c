...
int ckc_warp_tile_init_block_size(int current_block_size,
                                  int warp_m,
                                  int warp_n,
                                  int warp_k,
                                  int wave_size)
...
void ckc_io_spec_rule_init(ckc_io_spec_rule_t* rule,
                           const char* dtype,
                           int block_size,
                           int vec)
{
...
static const char* ckc_format_int_set(ckc_arena_t* arena,
                                      const int* vals,
                                      size_t n)
{
...
int ckc_validate_io(ckc_arena_t* arena,
                    const ckc_io_spec_rule_t* rule,
                    const char** out_reason)
{
    ...
    {
        if (rule->dtype != NULL && dtypes[i] != NULL &&
            strcmp(rule->dtype, dtypes[i]) == 0)
        {
            found = 1;
            break;
        }
    ...
    if (!found)
    {
        if (out_reason != NULL)
        {
            ...
            const char* r = ckc_arena_printf(arena, "unsupported dtype '%s'",
    ...
    if (!found)
    {
        if (out_reason != NULL)
        {
            ...
            {
                const char* r =
                    ckc_arena_printf(arena, "vec %d not in %s", rule->vec, set_str);
    ...
    if (rule->n_per_block_set)
    {
        ...
        if (rule->max_elems_per_thread_set)
        {
            ...
            int elems =
                (rule->block_size != 0) ? (rule->n_per_block / rule->block_size) : 0;
            ...
            if (elems > rule->max_elems_per_thread)
            {
                if (out_reason != NULL)
                {
                    const char* r = ckc_arena_printf(
                        ...
                        "a multi-pass kernel",
                        elems, rule->max_elems_per_thread);
...
ckc_status_t ckc_choose_load_vec(int tile_m,
                                 int tile_n,
                                 int tile_k,
                                 int block_size,
                                 int* out_vec)
...
const char* ckc_ptr_type_str(ckc_arena_t* arena,
                             const char* dtype,
...
ckc_status_t ckc_sig_scalar(ckc_arena_t* arena,
                            const char* name,
                            const char* ty,
                            ckc_sig_entry_t* out)
{
    ...
    if (ty == NULL ||
        (strcmp(ty, "i32") != 0 && strcmp(ty, "i64") != 0 && strcmp(ty, "f32") != 0))
    {
...
static int sb_reserve_one(ckc_signature_builder_t* sb)
{
    ...
    grown = (ckc_sig_entry_t*)ckc_arena_alloc(sb->arena,
                                              new_cap * sizeof(ckc_sig_entry_t));
    if (grown == NULL)
    {
        return -1;
    }
...
ckc_status_t ckc_signature_builder_init(ckc_signature_builder_t* sb,
                                        ckc_arena_t* arena)
{
...
ckc_signature_builder_t* ckc_signature_builder_scalar(ckc_signature_builder_t* sb,
                                                      const char* name,
                                                      const char* ty)
{
    ckc_sig_entry_t entry;
...
ckc_status_t ckc_ceil_div_grid(const int* totals,
                               const int* tiles,
                               size_t num_dims,
                               int out[3])
{
