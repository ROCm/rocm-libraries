                                  ...
                                  int hazard_nop)
{
    ...
    op = ckc_b_inline_asm(b, ckc_strbuf_cstr(&tmpl), "=v,0,a,a", operands, 3,
                          result_types, 1, &opts);
    ckc_strbuf_free(&tmpl);
                                 ...
                                 ckc_value_t** out_accs)
{
    ...
    src_a = (ckc_value_t**)ckc_arena_alloc(&b->arena,
                                           (size_t)n * sizeof(ckc_value_t*));
    src_b = (ckc_value_t**)ckc_arena_alloc(&b->arena,
                                           (size_t)n * sizeof(ckc_value_t*));
    operands = (ckc_value_t**)ckc_arena_alloc(
        &b->arena, (size_t)(3 * n) * sizeof(ckc_value_t*));
    result_types = (const ckc_type_t**)ckc_arena_alloc(
        &b->arena, (size_t)n * sizeof(const ckc_type_t*));
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        ckc_strbuf_appendf(&tmpl,
                           "v_mfma_f32_16x16x128_f8f6f4 $%d, $%d, $%d, $%d",
                           vdst, srcA, srcB, vsrc);
        if (inter_nop && (i + 1 < n))
        {
    ...
    op = ckc_b_inline_asm_multi(b, ckc_strbuf_cstr(&tmpl),
                                ckc_strbuf_cstr(&cons), operands, 3 * n,
                                result_types, n, &opts);
    ckc_strbuf_free(&tmpl);
    ckc_strbuf_free(&cons);
