...
/*
 * helper_ck_dsl.helpers.schedule.c -- C99 port of ck_dsl/helpers/schedule.py.
 *
 ...
 * (ckc_b_s_setprio / ckc_b_sched_group_barrier / ckc_b_sched_barrier) so the
...
ckc_hotloop_inst_list_t ckc_hotloop_inst_list_from_geometry(ckc_ir_builder_t* b,
                                                            ...
                                                            int b_packed_size)
{
    ...
    /* LDS widths default to k_pack when None (CKC_HLIL_UNSET). */
    if(a_lds_write_width == CKC_HLIL_UNSET)
        a_lds_write_width = k_pack;
    if(b_lds_write_width == CKC_HLIL_UNSET)
        b_lds_write_width = k_pack;
    if(a_lds_read_width == CKC_HLIL_UNSET)
        a_lds_read_width = k_pack;
    if(b_lds_read_width == CKC_HLIL_UNSET)
        b_lds_read_width = k_pack;

    m_per_xdl = ckc_mfma_atom_m(atom);
    n_per_xdl = ckc_mfma_atom_n(atom);
    k_per_xdl = ckc_mfma_atom_k_per_xdlops(atom);
    ...
    b_db = ckc_schedule_dtype_bytes(b_dtype);
    if(a_db < 0 || b_db < 0)
    {
...
{
    ckc_schedule_policy_t p = ckc_schedule_policy_default();
    ...
    (void)b;
    return ckc_schedule_policy_default();
}

void ckc_schedule_policy_emit_prologue(const ckc_schedule_policy_t* p, ckc_ir_builder_t* b)
{
    if(p->setprio_level != CKC_SCHED_SETPRIO_NONE)
        ckc_b_s_setprio(b, p->setprio_level);
}
                                              ...
                                              int mfma_count)
{
    if(!p->emit_hints)
        return;
    ckc_b_sched_group_barrier(b, CKC_SCHED_DS_READ, ds_read_count, 0);
    ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, mfma_count, 0);
}

void ckc_schedule_policy_emit_mfma_valu_pairs(
    const ckc_schedule_policy_t* p, ckc_ir_builder_t* b, int pairs, int valu_per_pair, int group)
{
    int i;
    if(!p->emit_hints)
        return;
    for(i = 0; i < pairs; ++i)
    {
        ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, 1, group);
        ckc_b_sched_group_barrier(b, CKC_SCHED_VALU, valu_per_pair, group);
    }
}

void ckc_schedule_policy_emit_mfma_trans_pairs(
    const ckc_schedule_policy_t* p, ckc_ir_builder_t* b, int pairs, int trans_per_pair, int group)
{
    int i;
    if(!p->emit_hints)
        return;
    ...
    {
        ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, 1, group);
        ckc_b_sched_group_barrier(b, CKC_SCHED_TRANS, trans_per_pair, group);
    }
}
...
void ckc_schedule_policy_emit_hotloop_v3(const ckc_schedule_policy_t* p,
                                         ...
                                         bool force)
{
    const ckc_hotloop_inst_list_t* il = inst_list;
    ...
    for(li = 0; li < num_buffer_load_inst_a; ++li)
    {
        ...
        {
            ckc_b_sched_group_barrier(b, CKC_SCHED_DS_WRITE, 1, 0);
            ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, 1, 0);
        }
        ckc_b_sched_group_barrier(b, CKC_SCHED_VMEM_READ, 1, 0);
        ckc_b_sched_group_barrier(
            b, CKC_SCHED_MFMA, num_mfma_per_issue - num_dswrite_per_issue_a, 0);
    }
    for(li = 0; li < num_buffer_load_inst_b; ++li)
    {
        ...
        ckc_b_sched_group_barrier(b, CKC_SCHED_VMEM_READ, 1, 0);
        ckc_b_sched_group_barrier(
            b, CKC_SCHED_MFMA, num_mfma_per_issue - num_dswrite_per_issue_b, 0);
    }
    ...
    for(i = 0; i < num_dsread_a_mfma; ++i)
    {
        ...
        {
            ckc_b_sched_group_barrier(b, CKC_SCHED_DS_READ, ds_read_a_mfma_rate, 0);
        }
        else
        {
    ...
    for(i = 0; i < num_dsread_b_mfma; ++i)
    {
        ...
        {
            ckc_b_sched_group_barrier(b, CKC_SCHED_DS_READ, ds_read_b_mfma_rate, 0);
        }
        else
        {
            ckc_b_sched_group_barrier(b,
...
void ckc_schedule_policy_emit_compv3_hotloop(const ckc_schedule_policy_t* p,
                                             ...
                                             bool force)
{
    ckc_schedule_policy_emit_hotloop_v3(p, b, inst_list, force);
}
                                             ...
                                             bool force)
{
    ...
    {
        ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, 1, 0);
        ckc_b_sched_group_barrier(b, CKC_SCHED_DS_READ, num_ds_read_inst / num_issue, 0);
        ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, 1, 0);
        ckc_b_sched_group_barrier(b, CKC_SCHED_DS_WRITE, num_ds_write_inst / num_issue, 0);
        ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, 1, 0);
        ckc_b_sched_group_barrier(b, CKC_SCHED_VMEM_READ, 1, 0);
        ckc_b_sched_group_barrier(b, CKC_SCHED_MFMA, il->c_mfma_inst_num / num_issue - 3, 0);
    }
