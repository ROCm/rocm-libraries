...
bool ckc_fmoe_tile_eq(const ckc_gemm_tile_spec_t* a,
                      const ckc_gemm_tile_spec_t* b)
{
    if (a == NULL || b == NULL)
    {
        return a == b;
    ...
    return a->tile_m == b->tile_m && a->tile_n == b->tile_n &&
           a->tile_k == b->tile_k && a->warp_m == b->warp_m &&
           a->warp_n == b->warp_n && a->warp_k == b->warp_k &&
...
const char* ckc_b_fmoe_gemm_dtype_to_universal(ckc_ir_builder_t* b,
                                               const char* dtype)
{
    const char* u;
    ...
    if (u == NULL)
    {
        ...
        return (const char*)ckc_i_set_err(
            b, CKC_ERR_VALUE,
...
ckc_topk_softmax_spec_t ckc_fmoe_forward_spec_to_topk_softmax_spec(
...
static ckc_status_t ckc_fmoe_compose_name(char* out,
                                          size_t cap,
                                          const char* base,
                                          const char* suffix)
{
...
ckc_status_t ckc_fmoe_forward_spec_to_batched_gemm_spec(
    const ckc_fmoe_forward_spec_t* spec,
    char* name_out,
    size_t name_cap,
    ckc_batched_gemm_spec_t* out_spec)
...
ckc_status_t ckc_fmoe_forward_spec_to_batched_gemm_spec_preshuffle_b(
    const ckc_fmoe_forward_spec_t* spec,
    char* name_out,
    size_t name_cap,
    ckc_batched_gemm_spec_t* out_spec)
