                                                  ...
                                                  bool cache)
{
    ...
    scratch = (ckc_value_t**)ckc_arena_alloc(
    ...
    for (k = 0; k < chunks_per_thread; ++k)
    {
        ...
        mul_kbs = ckc_b_mul(
            b, ckc_b_const_i32(b, (int64_t)k * (int64_t)block_size), c_vec);
                          ...
                          int num_cached_f32)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "pass2_row_chunks requires a body callback");
        return;
    }
    if (vec != 0 && (elems_per_thread % vec) != 0)
    {
    ...
    out = (ckc_value_t**)ckc_arena_alloc(
        &b->arena, (size_t)(vec > 0 ? vec : 1) * sizeof(ckc_value_t*));
