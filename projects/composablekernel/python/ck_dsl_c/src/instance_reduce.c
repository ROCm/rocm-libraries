 ...
 * ===================================================================== */
ckc_status_t ckc_reduce2d_kernel_name(const ckc_reduce2d_spec_t* spec,
                                      char* out,
                                      size_t out_cap)
{
    char nbuf[32];
    ...
    return ckc_kernel_name_join(spec->name, parts, 5,
                                NULL, NULL, 0, /* no flags */
                                out, out_cap, NULL);
}
...
{
    return op != NULL &&
           (strcmp(op, "sum") == 0 || strcmp(op, "max") == 0 ||
            strcmp(op, "min") == 0 || strcmp(op, "mean") == 0 ||
            strcmp(op, "prod") == 0);
}

bool ckc_reduce2d_is_valid_spec(const ckc_reduce2d_spec_t* spec,
                                char* reason,
                                size_t reason_cap)
{
    ...
    if (!ckc_reduce2d_op_ok(spec->op))
    {
        ...
        {
            snprintf(reason, reason_cap, "unsupported op '%s'",
                     spec->op ? spec->op : "(null)");
        }
        return false;
    }
 ...
 * ===================================================================== */
static const ckc_tile_distribution_t* ckc_reduce2d_make_row_reduce_distribution(
    ckc_ir_builder_t* b,
...
static ckc_value_t* ckc_reduce2d_tree_combine_cb(ckc_ir_builder_t* b,
                                                ckc_value_t* a,
                                                ckc_value_t* c,
                                                void* user)
{
 ...
 */
static void ckc_reduce2d_sweep_body(ckc_ir_builder_t* b,
                                    ckc_value_t* n_off,
                                    ...
                                    void* user)
{
    ...
    chunk_partial = ckc_tree_reduce(b, ckc_reduce2d_tree_combine_cb, ctx,
                                    x_scalars, vec);
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_reduce2d(ckc_ir_builder_t* b,
                                     const ckc_reduce2d_spec_t* spec,
                                     const char* arch)
{
    ...
    /* return b.kernel  (Python build_reduce2d returns b.kernel; the C port's
     * peers append ret() before returning -- match the existing instances). */
    ckc_b_ret(b);
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_reduce2d_new(ckc_ir_builder_t* b,
                                         const ckc_reduce2d_spec_t* spec,
                                         const char* arch)
{
    char name[256];
