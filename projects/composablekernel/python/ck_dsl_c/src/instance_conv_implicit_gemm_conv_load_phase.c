...
int ckc_conv_choose_load_vec(const ckc_implicit_gemm_conv_spec_t* spec)
{
    ...
    ckc_status_t st = ckc_choose_load_vec(
        spec->tile_m, spec->tile_n, spec->tile_k, block_size, &out_vec);
 ...
 * ===================================================================== */
ckc_value_t* ckc_conv_emit_smem_load(ckc_ir_builder_t* b,
                                     ckc_value_t* smem,
                                     ckc_value_t* row,
                                     ckc_value_t* col,
                                     int n)
                                   ...
                                   void* ctx_user)
{
    ...
    if (ov != NULL && ov->a_mhw_index_fn != NULL)
    {
        ...
        ckc_transforms_descriptor_offset(
            b, ctx->A_desc, names, vals, 4, &off, &valid);
        *out_valid = valid;
        return off;
    }
    ...
    /* A_desc.offset(b_, m=m_val, k=k_val) */
    {
        ...
        ckc_value_t* valid = NULL;
        ckc_transforms_descriptor_offset(
            b, ctx->A_desc, names, vals, 2, &off, &valid);
        *out_valid = valid;
        return off;
    }
}
                                   ...
                                   void* ctx_user)
{
    ...
    ckc_value_t* valid = NULL;
    ckc_transforms_descriptor_offset(
        b, ctx->B_desc, names, vals, 2, &off, &valid);
    *out_valid = valid;
    return off;
}
                              ...
                              ckc_value_t* B_dst)
{
    ...
    if (spec->async_dma)
    {
        ...
        ckc_async_tile_loader_slot_issue(
            b,
            &a_slot,
            ctx->tid,
            ctx->a_rsrc,
    ...
    {
        ov->a_load_override(
            b, spec, k_off, A_dst, &ctx->grid, ctx->input_cache_context, ov->user);
    }
    else
    {
