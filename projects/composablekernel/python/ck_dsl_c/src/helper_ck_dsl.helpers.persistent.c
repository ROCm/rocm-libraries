 ...
 *
 * helper_ck_dsl.helpers.persistent.c -- C99 port of ck_dsl.helpers.persistent.
 *
 ...
 */
#include "ckc/helper_ck_dsl.helpers.persistent.h"
 ...
 * ------------------------------------------------------------------------ */
ckc_value_t* ckc_build_persistent_counter_init(ckc_ir_builder_t* b,
                                               ckc_value_t* counter,
                                               ckc_value_t* counter_idx,
                                               int increment,
                                               bool cooperative,
                                               ckc_value_t* broadcast_slot,
                                               int wave_size,
                                               int block_size)
{
    ...
    if (counter_idx == NULL)
        counter_idx = ckc_b_const_i32(b, 0);
    ...
    if (block_size <= wave_size)
    {
        ckc_value_t* inc_per_lane = ckc_b_select(
            b, is_lead, ckc_b_const_i32(b, (int64_t)increment), ckc_b_const_i32(b, 0));
        ckc_value_t* fetched =
            ckc_b_global_atomic_add(b, counter, counter_idx, inc_per_lane, NULL);
    ...
    }
    {
        ...
        {
            ckc_value_t* v = ckc_b_global_atomic_add(
                b, counter, counter_idx, ckc_b_const_i32(b, (int64_t)increment), NULL);
            ckc_value_t* store_idx[1] = {ckc_b_const_i32(b, 0)};
            ckc_b_smem_store_vN(b, broadcast_slot, store_idx, 1, v, 1);
        }
 ...
 * ------------------------------------------------------------------------ */
static void ckc_i_persistent_tile_loop(ckc_ir_builder_t* b,
                                       ckc_value_t* counter,
                                       ckc_value_t* num_tiles,
                                       int max_iters,
                                       ckc_value_t* tile_idx_init,
                                       ckc_value_t* counter_idx,
                                       bool cooperative,
                                       ckc_value_t* broadcast_slot,
                                       int wave_size,
                                       int block_size,
                                       ckc_persistent_tile_body_fn body,
                                       void* user_data)
{
    ...
    for_op = ckc_b_scf_for_iter(b,
                                ckc_b_const_i32(b, 0),
                                ckc_b_const_i32(b, (int64_t)max_iters),
                                ckc_b_const_i32(b, 1),
                                loop_args,
                                1,
                                "pers_iter",
    ...
    {
        ckc_value_t* tile_idx = for_op.iter_vars[0];
        ckc_value_t* in_range = ckc_b_cmp_lt(b, tile_idx, num_tiles);
        ckc_value_t* next_tile;
        ...
        {
            ckc_if_t gate = ckc_b_scf_if(b, in_range);
            ckc_b_region_enter(b, gate.then_region);
 ...
 *           body(tile_idx)
 * ------------------------------------------------------------------------ */
void ckc_persistent_tile_for_each(ckc_ir_builder_t* b,
                                  ckc_value_t* counter,
                                  ckc_value_t* num_tiles,
                                  int max_iters,
                                  ckc_persistent_tile_body_fn body,
                                  ...
                                  int block_size)
{
    ...
    tile_idx0 = ckc_build_persistent_counter_init(b,
                                                  counter,
                                                  counter_idx,
                                                  /*increment=*/1,
                                                  cooperative,
