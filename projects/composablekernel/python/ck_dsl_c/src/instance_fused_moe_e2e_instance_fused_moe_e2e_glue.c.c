                                             ...
                                             uint64_t stream)
{
    ...
    return ckc_fmoe_forward_dispatch(ctx, routing_logits, X, W_gate, W_up,
                                     W_down, Y, stream);
}
    ...
    switch (stage)
    {
        case CKC_FMOE_STAGE_ROUTER:
        {
            ...
            ckc_topk_softmax_spec_t s =
                ckc_fmoe_forward_spec_to_topk_softmax_spec(&adj_spec);
            st = ckc_topk_softmax_lower_to_llvm(&s, resolved_arch, flavor,
                                                out_ll, err, err_cap);
            break;
        }
        ...
        case CKC_FMOE_STAGE_DOWN_GEMM:
        {
            ...
            st = ckc_fmoe_forward_spec_to_batched_gemm_spec(&adj_spec, name_buf,
                                                            sizeof(name_buf), &s);
            if (st != CKC_OK)
            {
                ckc_fmoe_set_err(err, err_cap,
                                 "lower_to_llvm: to_batched_gemm_spec failed");
                break;
            }
            st = ckc_batched_gemm_lower_to_llvm(&s, resolved_arch, flavor,
                                                out_ll, err, err_cap);
            break;
        }
        ...
        case CKC_FMOE_STAGE_TOPK_REDUCE:
            ...
            ckc_fmoe_set_err(err, err_cap,
                             "lower_to_llvm: stage not yet wired (TODO port)");
            st = CKC_ERR_NOTIMPL;
            break;
        default:
