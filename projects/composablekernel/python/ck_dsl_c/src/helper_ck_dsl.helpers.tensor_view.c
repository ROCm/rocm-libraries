 ...
 * this phase) but kept for parity with the module surface. */
static int ckc_tv_dtype_elem_bytes(const ckc_type_t* dtype)
{
    if (ckc_tv_name_is(dtype, "f16") || ckc_tv_name_is(dtype, "bf16")) return 2;
    if (ckc_tv_name_is(dtype, "f32")) return 4;
    if (ckc_tv_name_is(dtype, "i32")) return 4;
    if (ckc_tv_name_is(dtype, "i64")) return 8;
                                          ...
                                          const ckc_type_t* dtype)
{
    ...
    if (out == NULL || shape == NULL || dtype == NULL) return CKC_ERR_VALUE;
    if (rank <= 0 || rank > CKC_TV_MAX_RANK) return CKC_ERR_VALUE;
    prod = 1;
    for (i = rank - 1; i >= 0; --i)
    {
                                          ...
                                          int num_indices)
{
    ...
    if (d == NULL || indices == NULL) return NULL;
                                         ...
                                         int num_indices)
{
    ...
    /* global */
    if (ckc_tv_name_is(dt, "f16")) return ckc_b_global_load_f16(b, v->base, off, 0);
    if (ckc_tv_name_is(dt, "bf16")) return ckc_b_global_load_bf16(b, v->base, off, 0);
    if (ckc_tv_name_is(dt, "f32")) return ckc_b_global_load_f32(b, v->base, off, 0);
    if (ckc_tv_name_is(dt, "i32")) return ckc_b_global_load_i32(b, v->base, off, 0);
    if (ckc_tv_name_is(dt, "i64")) return ckc_b_global_load_i64(b, v->base, off, 0);
                                      ...
                                      int n)
{
    ...
    if (ckc_tv_name_is(dt, "f32"))
    {
        ...
        int i;
        if (n < 1 || n > CKC_TV_MAX_VEC) return NULL;
        for (i = 0; i < n; ++i)
        {
            ckc_value_t* eoff =
                ckc_b_add(b, off, ckc_b_const_i32(b, i));
                                      ...
                                      int n)
{
    ...
    if (b == NULL || w == NULL || w->view == NULL) return NULL;
...
{
    ckc_value_t* gidx[CKC_TV_MAX_RANK];
    if (b == NULL || w == NULL || w->view == NULL) return;
    if (!ckc_tile_window_global_indices(b, w, local_indices, num_indices, gidx))
        return;
                                     ...
                                     ckc_value_t** out)
{
     ...
     */
    int i;
    if (b == NULL || w == NULL || w->view == NULL || out == NULL) return;
    if (n == 1)
    {
        ckc_value_t* scalar =
            ckc_tile_window_load_scalar(b, w, local_indices, num_indices);
                                        ...
                                        int num_values)
{
    ...
    casts = (ckc_value_t**)ckc_arena_alloc(
        &b->arena, (size_t)num_values * sizeof(ckc_value_t*));
                                     ...
                                     ckc_value_t** out)
{
    ...
    const ckc_type_t* dt;
    int i;
    if (b == NULL || v == NULL || out == NULL) return;
    dt = v->desc.dtype;
                                  ...
                                  const ckc_stride_t* strides)
{
     ...
     */
    ckc_status_t st;
    if (out == NULL) return CKC_ERR_VALUE;
    if (strides == NULL)
...
ckc_status_t ckc_make_naive_tensor_view_packed(ckc_tensor_view_t* out,
                                               ckc_value_t* base,
                                               const int* shape,
                                               int rank,
                                               const ckc_type_t* dtype)
                               ...
                               const ckc_stride_t* strides /* NULL => packed */)
{
    ...
    ckc_status_t st;
    ckc_value_t* smem;
    if (b == NULL || out == NULL) return CKC_ERR_VALUE;
