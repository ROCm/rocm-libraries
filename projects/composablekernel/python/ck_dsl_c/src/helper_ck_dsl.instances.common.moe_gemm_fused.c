...
static void ckc_moe_plan_rowcol(const ckc_moe_kloop_plan_t* plan,
                                ...
                                ckc_value_t** out_col)
{
    ckc_moe_vec_rowcol(plan->b, e, plan->tid, plan->c_threads, plan->block_k_div_vec,
                       plan->c_load_vec, plan->load_vec, out_row, out_col);
}
                             ...
                             ckc_value_t** out_groups_flat)
{
    ...
    for (int kk = 0; kk < plan->k_atoms; ++kk)
    {
        ...
        ckc_value_t* col_mul =
            ckc_b_mul(b, k_blk, ckc_b_const_i32(b, plan->a_per_lane));
        ckc_value_t* col_base =
            ckc_b_add(b, col_mul, ckc_b_const_i32(b, kk * t->warp_tile_k));
        ...
        for (int mi = 0; mi < plan->mfmas_m; ++mi)
        {
            ...
            a_rows[mi] =
                ckc_gemm_emit_smem_load(b, a_smem, a_row, col_base, plan->a_per_lane, plan->storage_dtype);
        }
                                ...
                                ckc_value_t** out_groups_flat)
{
    ...
    ckc_moe_emit_global_load(plan, a_view, a_mn_origin, operands, num_operands, b_mn_origin, c0,
                             a_pre0, b_pre0);
    ...
    int n_ia = 0;
    char names[CKC_MOE_MAX_ITER_ARGS][24];
    ...
    ckc_for_t for_op =
        ckc_b_scf_for_iter(b, c0, K, c_block_k, iter_args, n_ia, "k0", false, false);
    ckc_b_region_enter(b, for_op.body);
    {
        ...
        ckc_moe_emit_global_load(plan, a_view, a_mn_origin, operands, num_operands, b_mn_origin,
                                 k_clamped, a_next, b_next);

        ckc_value_t* new_groups_flat[2 * CKC_MOE_MAX_MFMAS];
        ckc_moe_emit_mfma_phase(plan, a_smem, operands, num_operands, cur_groups, group_sizes,
                                warp_m_idx, warp_n_idx, lane, sched_groups, new_groups_flat);
        ckc_b_sync(b);
...
static void ckc_moe_finalize_block_size(int* block_size,
...
ckc_gemm_universal_spec_t ckc_moe_gate_up_silu_gemm_spec_to_universal(
    const ckc_moe_gate_up_silu_gemm_spec_t* spec)
{
    return ckc_moe_to_universal(spec->name, &spec->tile, &spec->trait, spec->wave_size,
                                spec->block_size, spec->dtype);
}
...
ckc_gemm_universal_spec_t ckc_moe_down_reduce_gemm_spec_to_universal(
...
ckc_status_t ckc_moe_down_reduce_gemm_spec_kernel_name(
    const ckc_moe_down_reduce_gemm_spec_t* spec, char* out, size_t out_cap)
{
