 ...
 * ===================================================================== */
const ckc_mfma_atom_t* ckc_fused_mega_fp8_spec_gate_up_atom(
...
const ckc_mfma_atom_t* ckc_fused_mega_fp8_spec_down_atom(
    const ckc_fused_mega_kernel_spec_fp8_t* spec)
{
    if (spec == NULL)
    {
                                                 ...
                                                 size_t out_cap)
{
    ...
    n = snprintf(out, out_cap, "%s_moe_fused_mega_fp8_m%dn%dk%d",
                 spec->name, spec->tile_m, spec->tile_n_inter, spec->tile_k_gu);
...
ckc_value_t* ckc_moe_fp8_b_base(ckc_moe_fp8_build_ctx_t* ctx,
                                ckc_value_t* ptr, ckc_value_t* stride_b, ckc_value_t* expert_idx)
{
...
ckc_value_t* ckc_moe_fp8_scale_base(ckc_moe_fp8_build_ctx_t* ctx,
                                    ckc_value_t* ptr, ckc_value_t* stride_e, ckc_value_t* expert_idx)
{
...
void ckc_moe_fp8_select_item(ckc_moe_fp8_build_ctx_t* ctx,
                             ckc_value_t* m_block_idx, ckc_value_t* bx_block)
{
    ckc_value_t* bx;
...
void ckc_moe_fp8_emit_body(ckc_moe_fp8_build_ctx_t* ctx)
{
    ...
    if (ctx->spec->use_dtla)
    {
        ...
        ckc_value_t* wave_lds_base = ckc_b_smem_ptr_add(
            b, bstage_base_i64,
            ckc_b_sext(b, ckc_b_mul(b, ctx->warp_id, ckc_b_const_i32(b, warp_wave_bytes)),
                       ckc_i64()));
    ...
    for (mi = 0; mi < mfmas_m; ++mi)
    {
        ...
        ckc_value_t* m_tile_base = ckc_b_add(
            b, ctx->block_m_off,
            ckc_b_add(b, ctx->warp_m_off, ckc_b_const_i32(b, mi * atom->m)));

        for (ni = 0; ni < mfmas_n; ++ni)
        {
            n_tile_bases[ni] = ckc_b_add(
                b, ctx->gu_n_off,
                ckc_b_add(b, ctx->warp_n_off, ckc_b_const_i32(b, ni * atom->n)));
        }

        snprintf(tag, sizeof(tag), "%d", mi);
        ckc_moe_fp8_emit_fp8_gateup_fused_kloop(
            ctx, ctx->A, ctx->WGate, ctx->WUp, ctx->AScale, ctx->WGateScale, ctx->WUpScale,
            m_tile_base, n_tile_bases, mfmas_n, ctx->K, ctx->stride_a_scale,
            ctx->stride_gate_scale, ctx->stride_up_scale, tag,
            ctx->dtla.present ? &ctx->dtla : NULL, ctx->cadence, g_dqs, u_dqs);
    ...
    {
        ckc_value_t* amax_lane = ckc_moe_fp8_store_hidden_f32_pass(
            ctx, gate_list, up_list, &ctx->f32_view, ctx->warp_m_off, ctx->warp_n_off,
            ctx->lane, mfmas_m, mfmas_n, ctx->one_f32, ctx->c_neg_log2e, ctx->c_floor);

        /* 64-lane butterfly max over the warp (xor 1,2,4,8,16,32). */
        ckc_value_t* amax_warp = amax_lane;
        for (xm = 0; xm < 6; ++xm)
        {
            amax_warp = ckc_b_fmax(
                b, amax_warp, ckc_b_warp_shuffle_xor(b, amax_warp, kButterfly[xm]));
        }
    ...
    {
        ckc_for_t sweep = ckc_b_scf_for_iter(
            b, ctx->tid, ckc_b_const_i32(b, n_blocks), ctx->c_threads, NULL, 0, "cell",
            false, false);
        ckc_b_region_enter(b, sweep.body);
        {
            ...
            row_bc = ckc_b_scf_for_iter(
                b, ctx->c0, ctx->c_block_m, ckc_b_const_i32(b, 1), NULL, 0, "rb", false, false);
    ...
    /* ---- STAGE 1b Pass C: quantize f32 Hidden -> fp8 LDS (PACKED) ---- */
    {
        ...
        ckc_for_t qsweep = ckc_b_scf_for_iter(
            b, ctx->tid, ckc_b_const_i32(b, total_q4), ctx->c_threads, NULL, 0, "qcell",
            false, false);
    ...
    /* ---- STAGE 2: down fp8 GEMM (LDS-A) -> dequant -> weighted atomic Y ---- */
    {
        ...
        ckc_for_t down_for = ckc_b_scf_for_iter(
            b, ctx->c0, ctx->H_out, ckc_b_const_i32(b, ctx->spec->tile_n_down), NULL, 0, "ho",
            false, false);
        ckc_b_region_enter(b, down_for.body);
        {
            ...
            for (mi = 0; mi < mfmas_m_down; ++mi)
            {
                for (ni = 0; ni < mfmas_n_down; ++ni)
                {
                    ...
                    d_dq = ckc_moe_fp8_emit_fp8_down_group_gemm(
                        ctx, &ctx->fp8_view, ctx->WDown, ctx->WDownScale, n_tile_base,
                        &ctx->scale_view, tile_n, ctx->N, inter_blk_base,
                        ctx->stride_down_scale, m_row_base, dtag, ctx->cadence);
                    down_list[down_count++] = d_dq;
                }
            }

            ckc_moe_fp8_emit_down_atomic_reduce(
                ctx, down_list, down_warp_m_off, down_warp_n_off, ctx->lane, mfmas_m_down,
                mfmas_n_down, ctx->block_m_off, ho, ctx->H_out, ctx->SortedTokenIds,
                ctx->SortedWeights, ctx->Y, ctx->tokens);
            ckc_b_scf_yield(b, NULL, 0);
        }
        ckc_b_region_leave(b);
    }
...
static ckc_value_t* moe_fp8_param(ckc_ir_builder_t* b,
                                  const char* name,
                                  const ckc_type_t* t,
                                  bool noalias, bool readonly, int align)
{
                                                    ...
                                                    const ckc_fused_mega_fp8_levers_t* levers)
{
    ...
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "invalid fp8 fused-mega spec for %s: %s",
                      arch, (reason != NULL) ? reason : "");
        return NULL;
    }
    ...
    if (spec->warp_m != 1
        || warp_n_cols * spec->warp_n != tile_n
        || CKC_MOE_FP8_GROUP_K % warp_n_cols != 0
        || warps_per_block * n_blocks != spec->warp_n)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      ...
                      tile_n, spec->warp_n, warp_n_cols);
        return NULL;
    }
    ...
    ctx.warp_m_off =
        ckc_b_mul(b, ctx.warp_m_idx, ckc_b_const_i32(b, ctx.mfmas_m * ctx.atom->m));
    ctx.warp_n_off =
        ckc_b_mul(b, ctx.warp_n_idx, ckc_b_const_i32(b, ctx.mfmas_n * ctx.atom->n));
    ...
    else
    {
        ...
        ckc_for_t wloop = ckc_b_scf_for_iter(
            b, p, ctx.p_total_work, ctx.p_P, NULL, 0, "witem", false, false);
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_moe_fused_mega_gemm_fp8_new(ckc_ir_builder_t* b,
                                                        const ckc_fused_mega_kernel_spec_fp8_t* spec,
                                                        const char* arch,
                                                        bool persistent,
                                                        const ckc_fused_mega_fp8_levers_t* levers)
                                                  ...
                                                  size_t err_cap)
{
    ...
    if (kernel == NULL)
    {
        ...
        ckc_moe_fp8_set_err(err, err_cap,
                            (m != NULL && m[0] != '\0') ? m
                                                        : "build_moe_fused_mega_gemm_fp8 failed");
