 ...
 * ===================================================================== */
ckc_status_t ckc_mx_gemm_kernel_name(const ckc_mx_gemm_spec_t* spec,
                                     char* out,
                                     size_t out_cap)
{
    char shape[64];
                               ...
                               size_t reason_cap)
{
    ...
    {
        snprintf(buf, sizeof(buf), "block_size %d > %d (hardware cap) on %s",
                 block_size, ckc_archtarget_max_threads_per_block(target), arch);
        ckc_mxg_setreason(reason, reason_cap, buf);
        return false;
    }
    ...
    {
        snprintf(buf, sizeof(buf), "K (%d) must be divisible by group_k (%d)",
                 spec->K, spec->group_k);
        ckc_mxg_setreason(reason, reason_cap, buf);
        return false;
    }
    ...
    {
        ckc_mxg_setreason(reason, reason_cap,
                          "M / N must divide their tile sizes (v1 no partial tiles)");
        return false;
    }
    if (spec->block_tile_m != 16 || spec->block_tile_n != 16)
    {
        snprintf(buf, sizeof(buf),
                 ...
                 spec->block_tile_m, spec->block_tile_n);
        ckc_mxg_setreason(reason, reason_cap, buf);
        return false;
    }
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_mx_gemm(ckc_ir_builder_t* b,
                                    const ckc_mx_gemm_spec_t* spec,
                                    const char* arch)
{
    char reason[160];
    ...
    {
        return (ckc_kernel_def_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "mx_gemm: no MFMA atom for mantissa_dtype %s",
            spec->mantissa_dtype);
    }
    ...
    ckc_b_region_enter(b, outer.body);
    {
        ...
        b_scale_off =
            ckc_b_add(b, ckc_b_mul(b, kg, ckc_b_const_i32(b, spec->N)), n_global_col);

        /* a_scale = decode_mx_scale_e8m0(b, b.global_load(AScale, a_scale_off, I8, align=1)) */
        a_scale = ckc_decode_mx_scale_e8m0(
            b, ckc_b_global_load(b, AScale, a_scale_off, ckc_i8(), 1));
        /* b_scale = decode_mx_scale_e8m0(b, b.global_load(BScale, b_scale_off, I8, align=1)) */
        b_scale = ckc_decode_mx_scale_e8m0(
            b, ckc_b_global_load(b, BScale, b_scale_off, ckc_i8(), 1));
    ...
    ckc_store_acc_to_global(b, C, atom, &lane_decode, m_tile_base, n_tile_base,
                            acc_final, spec->N, "f32",
                            /*atomic_add=*/false, /*epilogue=*/NULL, /*user=*/NULL);
 ...
 * ===================================================================== */
ckc_kernel_def_t* ckc_build_mx_gemm_new(ckc_ir_builder_t* b,
                                        const ckc_mx_gemm_spec_t* spec,
                                        const char* arch)
{
    char name[256];
 ...
 * ===================================================================== */
void ckc_mx_gemm_grid(const ckc_mx_gemm_spec_t* spec,
                      int* out_gx,
                      int* out_gy,
                      int* out_gz)
{
