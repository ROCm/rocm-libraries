    ...
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* lane, int reg)
{
    ...
    if(!(0 <= reg && reg < 4))
    {
        ...
        {
            snprintf(B->err, (size_t)CKC_ERR_MSG_CAP,
                     "mfma_16x16 reg must be 0..3, got %d", reg);
    ...
    if(!ckc_tile_distribution_calculate_x(B, ctx->C16_DIST, ys, 2, ps, ps_counts, 1,
                                          out_x, 2))
    {
        return NULL;
    }
...
static ckc_value_t* ckc__ml_offset(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx,
                                   ckc_value_t* token, ckc_value_t* head, ckc_value_t* seg)
{
...
static ckc_value_t* ckc__seg_acc_offset(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx,
                                        ckc_value_t* token, ckc_value_t* head,
                                        ckc_value_t* seg, ckc_value_t* dim)
{
    ...
    in_values[2] = seg;
    in_values[3] = dim;
    if(!ckc_transforms_descriptor_offset(B, ctx->seg_acc_desc, in_names, in_values, 4, &off,
                                         &valid))
    {
        return NULL;
    }
...
static ckc_value_t* ckc__q_offset(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx,
                                  ckc_value_t* token, ckc_value_t* head, ckc_value_t* dim)
{
...
static ckc_value_t* ckc__paged_kv_offset(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx,
                                         ckc_value_t* tile_idx, ckc_value_t* linear_half,
                                         ckc_value_t* kv_head)
{
    ...
    in_values[2] = kv_head;
    if(!ckc_transforms_descriptor_offset(B, ctx->paged_kv_desc, in_names, in_values, 3, &off,
                                         &valid))
    {
        return NULL;
    }
 ...
 * ============================================================ */
void ckc_gfx942_attention_tiled_3d_declare_params(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    ...
    ctx->segm_output_ptr = ckc_b_param(B, "segm_output_ptr",
                                       ckc_ptr_type(B, ckc_f32(), "global"), &o);
    ...
    ctx->segm_max_ptr = ckc_b_param(B, "segm_max_ptr",
                                    ckc_ptr_type(B, ckc_f32(), "global"), &o);
    ...
    o.writeonly = true; o.writeonly_set = true;
    o.align = 4;        o.align_set = true;
    ctx->segm_expsum_ptr = ckc_b_param(B, "segm_expsum_ptr",
                                       ckc_ptr_type(B, ckc_f32(), "global"), &o);
    ...
    o.readonly = true; o.readonly_set = true;
    o.align = 4;       o.align_set = true;
    ctx->block_tables = ckc_b_param(B, "block_tables_ptr",
                                    ckc_ptr_type(B, ckc_i32(), "global"), &o);

    /* seq_lens_ptr: I32* readonly align4 */
    memset(&o, 0, sizeof(o));
    o.readonly = true; o.readonly_set = true;
    o.align = 4;       o.align_set = true;
    ctx->seq_lens = ckc_b_param(B, "seq_lens_ptr",
                                ckc_ptr_type(B, ckc_i32(), "global"), &o);
    ...
    o.readonly = true; o.readonly_set = true;
    o.align = 4;       o.align_set = true;
    ctx->alibi_slopes_ptr = ckc_b_param(B, "alibi_slopes_ptr",
                                        ckc_ptr_type(B, ckc_f32(), "global"), &o);
    ...
    o.readonly = true; o.readonly_set = true;
    o.align = 4;       o.align_set = true;
    ctx->qq_bias_ptr = ckc_b_param(B, "qq_bias_ptr",
                                   ckc_ptr_type(B, ckc_f32(), "global"), &o);
    ...
    o.readonly = true; o.readonly_set = true;
    o.align = 4;       o.align_set = true;
    ctx->cu_q = ckc_b_param(B, "query_start_len_ptr",
                            ckc_ptr_type(B, ckc_i32(), "global"), &o);
...
static void ckc__build_paged_kv_desc(ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
     ...
     *   coord_names=("physical_block","token","kv_head","dim")) (lines 570-575) */
    {
        ...
        const int strides[4] = {CFG.kv_stride_blk_b, CFG.kv_stride_tok_b, CFG.kv_stride_h_b,
                                CFG.KV_BYTES};
    ...
    if (T == BS)
    {
        ...
        chain[0] = ckc_indirect(B, "tile_idx", "physical_block",
                                ctx->block_tables, ctx->seq_base, NULL, 0);
        chain[1] = ckc_unmerge(B, "linear_half", into_td, 2, dims_td);
        ctx->paged_kv_desc =
            ckc_tensor_descriptor_transform(B, ctx->kv_base_desc, chain, 2);
    }
    else
    {
        ...
        chain[1] = ckc_indirect(B, "linear_block_idx", "physical_block",
                                ctx->block_tables, ctx->seq_base, NULL, 0);
        chain[2] = ckc_unmerge(B, "linear_half", into_ttd, 2, dims_ttd);
        chain[3] = ckc_embed(B, emb_up, 2, "token", emb_str, 0);
        ctx->paged_kv_desc =
            ckc_tensor_descriptor_transform(B, ctx->kv_base_desc, chain, 4);
    }
}
 ...
 * ckc_gfx942_attention_tiled_3d_emit_prologue -- lines 331-483.
 * ============================================================ */
void ckc_gfx942_attention_tiled_3d_emit_prologue(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    const int HD      = CFG.HD;
    const int T       = CFG.T;
    ...
    ctx->seq_idx = ckc_binary_search_seq_idx(B, ctx->cu_q, ctx->q_block_global_idx,
                                             ctx->num_seqs_p, BLOCK_Q,
                                             CFG.binary_search_iters, false);

    /* ---- cu_q bounds / per-sequence geometry (lines 344-350) ---- */
    ctx->cu_q_start = ckc_b_global_load_i32(B, ctx->cu_q, ctx->seq_idx, 0);
    ctx->cu_q_stop  = ckc_b_global_load_i32(B, ctx->cu_q,
                                            ckc_b_add(B, ctx->seq_idx, ckc_b_const_i32(B, 1)), 0);
    ...
    ctx->lane_half_base =
        ckc_b_mul(B, ctx->tid, ckc_b_const_i32(B, CFG.HALVES_PER_LANE));
 ...
 * ckc_gfx942_attention_tiled_3d_emit_q_to_lds -- lines 437-467.
 * ============================================================ */
void ckc_gfx942_attention_tiled_3d_emit_q_to_lds(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    const int NQK     = CFG.NQK;
    const int NUM_QH  = CFG.NUM_QH;
...
void ckc_gfx942_attention_tiled_3d_issue_k_load(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* kv_tile_idx, ckc_value_t* buf_idx)
{
    ...
    for(call = 0; call < kv_calls_per_tile; ++call)
    {
        ...
        ckc_b_async_buffer_load_lds_addr(B, ctx->key_rsrc, k_dst, voff, ctx->zero_soff,
                                         ASYNC_LDS_DWORDS, CKC_CACHE_ALL);
    }
}

/* _issue_v_load(kv_tile_idx, buf_idx) -- lines 619-632. */
void ckc_gfx942_attention_tiled_3d_issue_v_load(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* kv_tile_idx, ckc_value_t* buf_idx)
{
    const int KV_HALVES_PER_CALL = CFG.KV_HALVES_PER_CALL;
    const int kv_calls_per_tile  = CFG.kv_calls_per_tile;
    ...
    for(call = 0; call < kv_calls_per_tile; ++call)
    {
        ...
        ckc_b_async_buffer_load_lds_addr(B, ctx->value_rsrc, v_dst, voff, ctx->zero_soff,
                                         ASYNC_LDS_DWORDS, CKC_CACHE_ALL);
    }
}
...
void ckc_gfx942_attention_tiled_3d_issue_wide_load(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, bool is_value, ckc_value_t* kv_tile_idx,
    ckc_value_t* buf_idx)
{
    const int HD        = CFG.HD;
    const int THREADS   = CFG.THREADS;
    ...
    for(call = 0; call < wide_chunks_per_thread; ++call)
    {
        ...
        ckc_value_t* vec =
            ckc_b_global_load_vN(B, src, elem_off, dtype, WIDE_ELEMS, 16);
    ...
    ckc_value_t* buf_idx)
{
    ...
    for(call = 0; call < fp8_chunks_per_thread; ++call)
    {
        ...
        ckc_value_t* fp8_vec = ckc_b_global_load_vN(B, src, voff, ckc_fp8e4m3(),
                                                    fp8_elems_per_chunk, fp8_elems_per_chunk);
        ...
        for(i = 0; i < 4; ++i)
        {
            deq[i] = ckc_b_cast_f32_to(B, ckc_b_fmul(B, ckc_b_vec_extract(B, lo_f32, i), scale),
                                       dtype);
        }
        for(i = 0; i < 4; ++i)
        {
...
void ckc_gfx942_attention_tiled_3d_issue_k(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* tile_idx, ckc_value_t* buf_idx)
{
    if(CFG.KV_FP8)
    {
...
void ckc_gfx942_attention_tiled_3d_issue_v(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* tile_idx, ckc_value_t* buf_idx)
{
    if(CFG.KV_FP8)
    {
