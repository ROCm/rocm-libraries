...
static ckc_value_t* dfcp_spec_descriptor(ckc_ir_builder_t* b_,
                                         ckc_value_t* row,
                                         ckc_value_t* col,
                                         ...
                                         void* user)
{
    ...
    off_elems = ckc_b_add(
        b_,
        ckc_b_mul(b_, ckc_b_add(b_, ckc_b_mul(b_, hi, st->c_wi), wi), st->c_c),
        ci);
    ...
    ckc_value_t* a_dst, const ckc_warp_grid_t* grid, ckc_value_t* a_rsrc)
{
    ...
    if (ckc_coalesced_tile_loader_choose_vec(
            ...
            ckc_deep_fused_conv_pool_spec_block_size(spec), 8,
            &load_vec) != CKC_OK)
    {
     ...
     *             rsrc=a_rsrc). */
    ckc_coalesced_tile_loader_load(b, &loader, grid->tid, a_dst,
                                   dfcp_spec_descriptor, &st, a_rsrc,
                                   /*ptr=*/NULL);
}
    ...
    ckc_value_t* a_rsrc, const ckc_warp_grid_t* grid)
{
    ...
    shape[1] = c->C;
    input_smem =
        ckc_b_smem_alloc(b, ckc_f16(), shape, 2, "InputFoot_smem");
    ...
    for (e = 0; e < elems_per_thread; e++)
    {
        ...
        idx = ckc_b_add(b,
                        ckc_b_mul(b, ckc_b_const_i32(b, e),
                                  ckc_b_const_i32(b, block_size)),
                        grid->tid);
        ...
        {
            ckc_value_t* h_ge =
                ckc_b_cmp_ge(b, global_h, ckc_b_const_i32(b, 0));
            ckc_value_t* h_lt =
                ckc_b_cmp_lt(b, global_h, ckc_b_const_i32(b, c->Hi));
            h_ok = ckc_b_land(b, h_ge, h_lt);
        }
        {
            ckc_value_t* w_ge =
                ckc_b_cmp_ge(b, global_w, ckc_b_const_i32(b, 0));
            ckc_value_t* w_lt =
                ckc_b_cmp_lt(b, global_w, ckc_b_const_i32(b, c->Wi));
        ...
        off_elems = ckc_b_add(
            b,
            ckc_b_mul(b,
                      ckc_b_add(b, ckc_b_mul(b, global_h,
                                             ckc_b_const_i32(b, c->Wi)),
                                global_w),
                      c_c),
            ci);
    ...
    ckc_value_t* a_dst, const ckc_warp_grid_t* grid, ckc_value_t* input_smem)
{
    ...
    c_sc = ckc_b_const_i32(b, c->S * c->C);
    c_c = ckc_b_const_i32(b, c->C);
    c_foot_w = ckc_b_const_i32(b, foot_w);
    ...
    for (e = 0; e < elems_per_thread; e++)
    {
        ...
        foot_row = ckc_b_add(b, ckc_b_mul(b, ih, c_foot_w), iw);
        {
            ...
            lidx[1] = ci;
            v = ckc_b_vec_extract(
                b, ckc_b_smem_load_vN_f16(b, input_smem, lidx, 2, /*n=*/1), 0);
        }
    ...
    ckc_value_t* input_smem)
{
    ...
    if (b == NULL || spec == NULL || frag_len <= 0 ||
        frag_len > CKC_DFCP_MAX_ACCS)
    {
        return NULL;
    }
    p = &spec->problem;
    ...
    for (i = 0; i < frag_len; i++)
    {
        ...
        kg = ckc_b_add(b, k_off,
                       ckc_b_add(b, col_base, ckc_b_const_i32(b, i)));
        kg_ok = ckc_b_cmp_lt(b, kg, c_k_gemm);
        r = ckc_b_div(b, kg, c_sc);
        rem = ckc_b_mod(b, kg, c_sc);
