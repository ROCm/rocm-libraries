 ...
 * of build_moe_fused_mega_gemm (ck_dsl/instances/common/moe_fused_mega.py, lines
 * 636-738), split into the STAGE phase functions over the build context
 * (ckc/instance_moe_fused_mega_internal.h).
 *
 * SCOPE OF THIS PART-FILE (the STAGE phase functions only):
 *   ckc_moe_mega_emit_stage1_gate_up        (Python STAGE 1, lines 637-652)
 *   ckc_moe_mega_emit_stage2_silu_to_lds    (Python STAGE 2, lines 654-673)
 *   ckc_moe_mega_emit_stage3_reshape        (Python STAGE 3, lines 675-684; no-op)
 *   ckc_moe_mega_emit_stage45_down_reduce   (Python STAGE 4+5, lines 686-734)
 *   ckc_moe_mega_emit_body                   (the closure walk, lines 636-738)
 *
 * The two file-level STAGE-4 helpers (_emit_moe_down_mfma_phase_lds_a /
 * _emit_moe_down_kloop_lds_a -> ckc_moe_mega_emit_down_mfma_phase_lds_a /
 * ckc_moe_mega_emit_down_kloop_lds_a) live in a SEPARATE part-file; this file
 * calls ckc_moe_mega_emit_down_kloop_lds_a through the internal header.
 *
 ...
 * ===================================================================== */
void ckc_moe_mega_emit_stage1_gate_up(ckc_moe_mega_build_ctx_t* ctx,
                                      ...
                                      ckc_value_t** out_up_res)
{
    ...
    char gu_name_buf[2 * CKC_MOE_MEGA_MAX_ACCS][24];
    const char* acc_names_flat[2 * CKC_MOE_MEGA_MAX_ACCS];
    for (int mi = 0; mi < ctx->mfmas_m; ++mi)
    {
        for (int ni = 0; ni < ctx->mfmas_n; ++ni)
        {
            const int flat = mi * ctx->mfmas_n + ni;
            snprintf(gu_name_buf[flat], sizeof(gu_name_buf[0]),
                     "gate_acc_m%d_n%d", mi, ni);
            acc_names_flat[flat] = gu_name_buf[flat];
            snprintf(gu_name_buf[n + flat], sizeof(gu_name_buf[0]),
                     "up_acc_m%d_n%d", mi, ni);
            acc_names_flat[n + flat] = gu_name_buf[n + flat];
        }
    }

    /* a_mn_origin / b_mn_origin are 2-elem (batch_off, block_mn_off) arrays. */
    ckc_value_t* a_mn_origin[2] = {ctx->a_mn_origin[0], ctx->a_mn_origin[1]};
    ckc_value_t* b_mn_origin[2] = {ctx->b_mn_origin[0], ctx->b_mn_origin[1]};

    const int sched_groups = 2 * ctx->mfmas_m * ctx->mfmas_n;

    /* The reused k-loop returns the final groups flat (sum(group_sizes)). */
    ckc_value_t* out_flat[2 * CKC_MOE_MEGA_MAX_ACCS];
    ckc_moe_emit_prefetch_kloop(&ctx->plan,
                                ctx->a_view,
                                ctx->a_lds_view,
                                ctx->A_smem,
                                a_mn_origin,
...
typedef struct ckc_moe_mega_silu_cell_ctx
{
    ckc_ir_builder_t* b;
    int mfmas_n;
    ckc_value_t* const* gate_res;
    ckc_value_t* const* up_res;
    ckc_value_t* one_f32;
    ckc_value_t* c_neg_log2e;
    const ckc_type_t* storage_dtype;
} ckc_moe_mega_silu_cell_ctx_t;
...
static ckc_value_t* ckc_moe_mega_silu_cell(int mi, int ni, int i, void* user)
{
    ...
    ckc_value_t* silu =
        ckc_moe_silu_mul_f32(cc->b, g, up, cc->one_f32, cc->c_neg_log2e);
    return ckc_b_cast_f32_to(cc->b, silu, cc->storage_dtype);
}

void ckc_moe_mega_emit_stage2_silu_to_lds(ckc_moe_mega_build_ctx_t* ctx,
                                          ckc_value_t* const* gate_res,
                                          ckc_value_t* const* up_res)
{
    ...
    ckc_value_t* warp_m_off =
        ckc_b_mul(b, ctx->warp_m_idx,
                  ckc_b_const_i32(b, (int64_t)ctx->mfmas_m * ctx->t.warp_tile_m));
    ckc_value_t* warp_n_off =
        ckc_b_mul(b, ctx->warp_n_idx,
                  ckc_b_const_i32(b, (int64_t)ctx->mfmas_n * ctx->t.warp_tile_n));

    ckc_moe_cwarp_decode_t cdec;
    if (!ckc_moe_cwarp_decode_init(&cdec, b, ctx->u_gu, warp_m_off, warp_n_off,
                                   ctx->lane))
    {
    ...
    ckc_moe_emit_cshuffle_stage(b, ctx->u_gu, &cdec, ctx->Hidden_smem,
                                ctx->storage_dtype, ctx->c_per_lane,
                                ckc_moe_mega_silu_cell, &cell_ctx);
    ckc_b_sync(b);
}
 ...
 * ===================================================================== */
void ckc_moe_mega_emit_stage3_reshape(ckc_moe_mega_build_ctx_t* ctx)
{
 ...
 * ===================================================================== */
void ckc_moe_mega_emit_stage45_down_reduce(ckc_moe_mega_build_ctx_t* ctx)
{
    ckc_ir_builder_t* b = ctx->b;

    /* scf_for_iter(c0, H_out, c_block_n_down, [], iv_name="ho"). No carried
     * iter-args (Python passes []). */
    ckc_for_t down_for = ckc_b_scf_for_iter(b, ctx->c0, ctx->H_out,
                                            ctx->c_block_n_down,
                                            /*iter_args=*/NULL, /*num=*/0, "ho",
                                            /*unroll=*/false,
    ...
    ckc_b_region_enter(b, down_for.body);
    {
        ...
        ckc_value_t* down_accs[CKC_MOE_MEGA_MAX_ACCS];
        for (int j = 0; j < nd; ++j)
        {
        ...
        ckc_value_t* b_mn_origin[2] = {ctx->batch_off_b, ho};

        const int down_sched_groups = ctx->down_mfmas_m * ctx->down_mfmas_n;

        ckc_value_t* down_res[CKC_MOE_MEGA_MAX_ACCS];
        ckc_moe_mega_emit_down_kloop_lds_a(&ctx->plan_down,
                                           ctx->Hidden_smem,
                                           &ctx->down_operand,
                                           down_accs,
                                           nd,
 ...
 * ===================================================================== */
void ckc_moe_mega_emit_body(ckc_moe_mega_build_ctx_t* ctx)
{
    /* STAGE 1: gate + up GEMM -> f32 register groups. */
    ckc_value_t* gate_res[CKC_MOE_MEGA_MAX_ACCS];
    ckc_value_t* up_res[CKC_MOE_MEGA_MAX_ACCS];
    ckc_moe_mega_emit_stage1_gate_up(ctx, gate_res, up_res);

    /* STAGE 2: SiLU(gate)*up -> PERSISTENT LDS Hidden_smem. */
    ckc_moe_mega_emit_stage2_silu_to_lds(ctx, gate_res, up_res);

    /* STAGE 3: reshape note -- Option A identity (no IR). */
    ckc_moe_mega_emit_stage3_reshape(ctx);

    /* STAGE 4 + 5: DOWN GEMM -> weighted atomic reduce into Y. */
    ckc_moe_mega_emit_stage45_down_reduce(ctx);
}
