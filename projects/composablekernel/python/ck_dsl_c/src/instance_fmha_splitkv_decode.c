...
ckc_fmha_splitkv_decode_spec_t ckc_fmha_splitkv_decode_spec_default(
    ckc_fmha_common_spec_t common, int batch, int num_segments)
{
...
ckc_status_t ckc_fmha_splitkv_decode_kernel_name(
    const ckc_fmha_splitkv_decode_spec_t* spec,
    const char* phase,
    char* out,
    size_t out_cap)
{
    ...
    return ckc_kernel_name_join(name, parts, 7, NULL, NULL, 0, out, out_cap,
                                NULL);
}
...
bool ckc_fmha_splitkv_decode_is_valid_spec(
    const ckc_fmha_splitkv_decode_spec_t* spec,
    const char* arch,
    char* reason,
    size_t reason_cap)
{
    ...
    /* if spec.num_segments not in (1, 2, 4, 8, 16, 32, 64, 128): ... */
    {
        ...
        bool good = (n == 1 || n == 2 || n == 4 || n == 8 || n == 16 ||
                     n == 32 || n == 64 || n == 128);
        if (!good)
        {
            snprintf(buf, sizeof(buf),
                     "num_segments %d not in {1, 2, ..., 128}", n);
            ckc_splitkv_set_reason(reason, reason_cap, buf);
            return false;
        }
    }
    ...
    }
    {
        ...
        for (k = 0; k < ept; ++k)
        {
            ckc_b_store_scalar_from_f32(
                b, ptr,
                ckc_b_add(b, row_base,
                          ckc_b_add(b, lane_d_base, ckc_b_const_i32(b, k))),
                values_f32[k], dtype);
        }
    }
}
    ...
    const char* arch)
{
    ...
    if (!ckc_fmha_splitkv_decode_is_valid_spec(spec, arch, reason, sizeof(reason)))
    {
        ...
        ckc_i_set_err(ckc_fmha_kernel_builder_builder(kb), CKC_ERR_VALUE,
                      "invalid splitkv_decode spec: %s", reason);
        return NULL;
    }
...

    {
         ...
         * Python op-emission (and SSA-numbering) order exactly. */
        {
            ...
            ckc_value_t* seg_ge = ckc_b_cmp_ge(b, seg_p1,
        ...
        kv_head_k_off = ckc_b_add(
            b, ckc_b_mul(b, kv_head_idx, ckc_fmha_kernel_builder_stride_head(kb, "k")),
            lane_d_base);
        kv_head_v_off = ckc_b_add(
            b, ckc_b_mul(b, kv_head_idx, ckc_fmha_kernel_builder_stride_head(kb, "v")),
            lane_d_base);
        ...
        if (!ckc_b_load_lane_slice_f32(b, Q, q_row, lane_d_base, s->dtype, ept,
                                       q_lane))
        {
            return NULL;
        }
         ...
         *                         iv_name="k_idx") */
        k_loop = ckc_b_scf_for_iter(b, seg_start, seg_end, ckc_b_const_i32(b, 1),
                                    iter_args, num_iter_args, "k_idx", false,
                                    false);
        ckc_b_region_enter(b, k_loop.body);
        {
            ...
            if (!ckc_b_load_lane_slice_f32(b, K, k_row, ckc_b_const_i32(b, 0),
                                           s->dtype, ept, k_lane))
            {
                ckc_b_region_leave(b);
                return NULL;
            }
            if (!ckc_b_load_lane_slice_f32(b, V, v_row, ckc_b_const_i32(b, 0),
                                           s->dtype, ept, v_lane))
            {
                ckc_b_region_leave(b);
                return NULL;
            ...
            score_log2 = ckc_apply_attention_mask(
                b, score_log2, ckc_splitkv_attn_mask_mode(s->mask_mode), k_idx,
                ckc_b_const_i32(b, 0), s->sliding_window, NULL, NULL);
            if (score_log2 == NULL)
            {
                ckc_b_region_leave(b);
                return NULL;
        ...
        seg_stride = ckc_b_mul(b, ckc_b_const_i32(b, s->shape.num_query_heads),
                               ckc_b_const_i32(b, spec->batch));
        ...
        for (k = 0; k < ept; ++k)
        {
            ...
            ckc_b_global_store(b, ws_acc, ckc_b_add(b, ws_idx_acc_base, d),
                               acc_final[k], 4);
        }
    ...
    const char* arch)
{
...

    {
        ...
        base_ml = ckc_b_add(
            b, ckc_b_mul(b, seq_idx, ckc_b_const_i32(b, s->shape.num_query_heads)),
            head_idx);
        ...
        /* Pass 1: overall_max. */
        {
            ...
            ia[0].name = "mx";
            ia[0].init = neg_inf;
            mx_loop = ckc_b_scf_for_iter(b, ckc_b_const_i32(b, 0),
                                         ckc_b_const_i32(b, spec->num_segments),
                                         ckc_b_const_i32(b, 1), ia, 1, "s_mx",
                                         false, false);
        ...
        /* Pass 2: overall_expsum. */
        {
            ...
            ia[0].name = "den";
            ia[0].init = zero_f;
            sum_loop = ckc_b_scf_for_iter(b, ckc_b_const_i32(b, 0),
                                          ckc_b_const_i32(b, spec->num_segments),
                                          ckc_b_const_i32(b, 1), ia, 1, "s_sum",
                                          false, false);
        ...
        for (k = 0; k < ept; ++k)
        {
            ...
            ia[0].name = accname;
            ia[0].init = zero_f;
            acc_loop = ckc_b_scf_for_iter(b, ckc_b_const_i32(b, 0),
                                          ckc_b_const_i32(b, spec->num_segments),
                                          ckc_b_const_i32(b, 1), ia, 1, ivname,
                                          false, false);
            ckc_b_region_enter(b, acc_loop.body);
            {
                ...
                ckc_value_t* acc_base = H_pow2
                    ? ckc_b_shl(b, acc_base_idx, ckc_b_const_i32(b, H_shift))
                    : ckc_b_mul(b, acc_base_idx, ckc_b_const_i32(b, H));
                ckc_value_t* ov = ckc_b_global_load_f32(b, ws_acc,
                                                        ckc_b_add(b, acc_base, d), 0);
                ckc_value_t* yld = ckc_b_fadd(b, ac, ckc_b_fmul(b, ov, factor));
                ckc_b_scf_yield(b, &yld, 1);
            }
            ckc_b_region_leave(b);
            /* acc_per_lane.append(fmul(acc_loop.results[0], inv_l)) */
            acc_per_lane[k] = ckc_b_fmul(
        ...
        ckc_splitkv_store_lane_slice_f32_packed(b, O, o_row, lane_d_base,
                                                acc_per_lane, s->dtype, ept);
...
ckc_status_t ckc_fmha_splitkv_decode_segment_grid(
    const ckc_fmha_splitkv_decode_spec_t* spec, int out[3])
{
...
ckc_status_t ckc_fmha_splitkv_decode_reduce_grid(
    const ckc_fmha_splitkv_decode_spec_t* spec, int out[3])
{
    if (spec == NULL || out == NULL)
    {
 ...
 * kernel to build. */
static ckc_status_t ckc_splitkv_lower_to_llvm(
    const ckc_fmha_splitkv_decode_spec_t* spec,
    const char* arch,
    ...
    size_t err_cap)
{
    ...
    kernel = is_segment
                 ? ckc_build_fmha_fwd_splitkv_decode_segment(&kb, spec, arch)
...
ckc_status_t ckc_fmha_splitkv_decode_segment_lower_to_llvm(
    const ckc_fmha_splitkv_decode_spec_t* spec,
    const char* arch,
    ckc_llvm_flavor_t flavor,
    char** out_ll,
...
ckc_status_t ckc_fmha_splitkv_decode_reduce_lower_to_llvm(
    const ckc_fmha_splitkv_decode_spec_t* spec,
    const char* arch,
    ckc_llvm_flavor_t flavor,
    char** out_ll,
