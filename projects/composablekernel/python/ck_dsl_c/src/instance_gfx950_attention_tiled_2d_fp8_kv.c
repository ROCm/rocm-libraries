...
ckc_attn2d_fp8_dma_pick(const ckc_gfx950_attn2d_build_ctx_t* ctx)
{
...
void ckc_gfx950_attn2d_emit_fp8_async_preloop_setup(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    ...
    ctx->lane_fp8_base_v =
        ckc_b_mul(b, ctx->tid, ckc_b_const_i32(b, dma.bytes_per_lane));
                                               ...
                                               bool is_v)
{
    ...
    for(call = 0; call < chunks_per_thread; ++call)
    {
        ...
        ckc_value_t* voff =
            ckc_attn2d_fp8_kv_voff(ctx, kv_tile_idx, linear_half_first, NULL);
                                              ...
                                              ckc_value_t* buf_idx)
{
    ...
    K_lds_addr = (ctx->K_lds_addr_v != NULL) ? ctx->K_lds_addr_v
                                             : ckc_b_smem_addr_of(b, ctx->K_lds);
                                              ...
                                              ckc_value_t* kv_tile_idx)
{
    ...
    V_lds_addr = (ctx->V_lds_addr_v != NULL) ? ctx->V_lds_addr_v
                                             : ckc_b_smem_addr_of(b, ctx->V_lds);
