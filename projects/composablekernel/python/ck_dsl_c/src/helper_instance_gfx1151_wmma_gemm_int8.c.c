 ...
 *
 * helper_instance_gfx1151_wmma_gemm_int8.c.c -- C99 port of
 * ck_dsl/instances/gfx1151/wmma_gemm_int8.py.
 *
 * INT8-storage / f16-compute WMMA GEMM for gfx1151 (RDNA3.5 / Strix Halo): one
 * wave32 per 16x16 output tile, no LDS, RCR layout (C = A @ B.T). Operands are
 * stored int8 and converted to f16 on-chip (lossless for |x| <= 127), then fed
 * to the hardware-verified wmma_f32_16x16x16_f16 path with f32 accumulate. The
 * combined dequant scale (scale_a * scale_b) is folded into the epilogue. The
...
#include "ckc/helper_instance_gfx1151_wmma_gemm_int8.c.h"
...
#define CKC_WMMA_INT8_M 16    /* _WMMA_M */
#define CKC_WMMA_INT8_N 16    /* _WMMA_N */
#define CKC_WMMA_INT8_K 16    /* _WMMA_K */
#define CKC_WMMA_INT8_WAVE 32 /* _WAVE */
                                      ...
                                      size_t reason_cap)
{
     ...
     * catalog gate queries the *f16 compute* atom. op_for_shape returns NULL
     * when the shape/dtype combo is absent. */
    if (ckc_archtarget_op_for_shape(target, "wmma", "fp16", "fp16", "fp32",
                                    CKC_WMMA_INT8_M, CKC_WMMA_INT8_N, CKC_WMMA_INT8_K) == NULL)
    {
        snprintf(buf, sizeof(buf),
                 "WMMA %dx%dx%d f16 compute atom absent on %s "
                 "(WMMA is an RDNA/gfx11 instruction)",
                 CKC_WMMA_INT8_M, CKC_WMMA_INT8_N, CKC_WMMA_INT8_K, arch);
        ckc_wmma_gemm_int8_set_reason(reason, reason_cap, buf);
        return false;
    }

    /* if target.wave_size != _WAVE: ... */
    if (target->wave_size != CKC_WMMA_INT8_WAVE)
    {
        snprintf(buf, sizeof(buf),
...
/* ===================================================================== *
 *  _i8_to_f16(b, vec, i): extract slot i (int8), sign-extend -> f32 -> f16.
 *
 *    b.cast_f32_to(b.sitofp_f32(b.sext(b.vec_extract(vec, i), I32)), F16)
 * ===================================================================== */
static ckc_value_t* ckc_wmma_gemm_int8_i8_to_f16(ckc_ir_builder_t* b, ckc_value_t* vec, int i)
{
    ckc_value_t* slot = ckc_b_vec_extract(b, vec, i);
    ckc_value_t* ext = ckc_b_sext(b, slot, ckc_i32());
    ckc_value_t* f32v = ckc_b_sitofp_f32(b, ext);
    return ckc_b_cast_f32_to(b, f32v, ckc_f16());
}
                                           ...
                                           const char* arch)
{
    ...
    {
        (void)ckc_i_set_err(b, CKC_ERR_VALUE, "invalid WMMA int8 GEMM spec: %s", reason);
        return NULL;
    }
    ...
    ckc_attr_set_int(b, &b->kernel->attrs, "max_workgroup_size", CKC_WMMA_INT8_WAVE);
    ...
    c16 = ckc_b_const_i32(b, CKC_WMMA_INT8_K);
    c32 = ckc_b_const_i32(b, CKC_WMMA_INT8_WAVE);
    ...
    ckc_b_region_enter(b, loop.body);
    {
        ...
        ckc_value_t* a_i8;
        ckc_value_t* b_i8;
        ckc_value_t* a_comp[16];
        ckc_value_t* b_comp[16];
        ckc_value_t* a_frag;
        ckc_value_t* b_frag;
        ckc_value_t* nacc;
        ...
        for (j = 0; j < 16; ++j)
        {
            a_comp[j] = ckc_wmma_gemm_int8_i8_to_f16(b, a_i8, j);
        }
        a_frag = ckc_b_vec_pack(b, a_comp, 16, f16);
        /* b_frag = b.vec_pack([_i8_to_f16(b, b_i8, i) for i in range(16)], F16) */
        for (j = 0; j < 16; ++j)
        {
            b_comp[j] = ckc_wmma_gemm_int8_i8_to_f16(b, b_i8, j);
        }
        b_frag = ckc_b_vec_pack(b, b_comp, 16, f16);
...
ckc_status_t ckc_wmma_gemm_int8_grid(int M, int N, int out[3])
{
    ...
    out[0] = (M + CKC_WMMA_INT8_M - 1) / CKC_WMMA_INT8_M;
    out[1] = (N + CKC_WMMA_INT8_N - 1) / CKC_WMMA_INT8_N;
    out[2] = 1;
    return CKC_OK;
}
