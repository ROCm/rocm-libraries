 ...
 *
 * SCOPE (this TU only):
 *   * ckc_gemm_build_populate_ctx -- the build prologue that POPULATES the
 *     shared ckc_gemm_build_ctx_t. Mirrors the Python enclosing-function body
 ...
 *   * ckc_gemm_emit_load_phase    (Python closure emit_load_phase, ~1112)
 *   * ckc_gemm_emit_frag_smem_load(Python closure _emit_frag_smem_load, ~1372)
 *   * ckc_gemm_emit_wmma_phase    (Python closure _emit_wmma_phase, ~1409)
 *
 * Peers (the module-level helpers + the other phase closures) are CALLED via
 * ckc/instance_gemm_internal.h and are defined in sibling part-files; this TU
 ...
 * in ckc/helper_ck_dsl.helpers.tensor_view.h, which themselves reproduce the
...
#include "ckc/helper_ck_dsl.helpers.grid.h" /* ckc_chiplet_aware_super_tile_dynamic */
...
void ckc_gemm_build_populate_ctx(ckc_gemm_build_ctx_t* ctx);

/* ===================================================================== *
 *  build prologue -- POPULATE ckc_gemm_build_ctx_t
 *
...
void ckc_gemm_build_populate_ctx(ckc_gemm_build_ctx_t* ctx)
{
    ...
    {
        ckc_value_t* n_pid_m = ckc_b_div(
            b, ckc_b_add(b, ctx->M, ckc_b_const_i32(b, ctx->block_m - 1)), ctx->c_block_m);
        ckc_value_t* n_pid_n = ckc_b_div(
            b, ckc_b_add(b, ctx->N, ckc_b_const_i32(b, ctx->block_n - 1)), ctx->c_block_n);
        ckc_value_t* wgid_flat = ckc_b_add(
            b, ckc_b_mul(b, ckc_b_block_id_y(b), n_pid_n), ckc_b_block_id_x(b));
        ckc_super_tile_swizzle_result_t swz = ckc_chiplet_aware_super_tile_dynamic(
            b,
            wgid_flat,
            n_pid_m,
            n_pid_n,
    ...
    if (ctx->prefetch && !spec->trait.direct_to_lds)
    {
        ...
        ctx->b->status = CKC_ERR_VALUE;
        snprintf(ctx->b->err, CKC_ERR_MSG_CAP, "dtl_prefetch requires direct_to_lds=True");
        return;
    }
    ...
    ctx->num_accs = ctx->mfmas_m * ctx->mfmas_n;
    {
        ...
        for (int mi = 0; mi < ctx->mfmas_m; ++mi)
        {
            for (int ni = 0; ni < ctx->mfmas_n; ++ni)
            {
                ...
                /* Persist the name: ckc_b_fresh duplicates into the arena and
                ...
                const char* owned = ckc_b_fresh(b, namebuf);
                if (owned && owned[0] == '%')
                    ++owned;
     ...
     * offset formula.) */
    {
        ...
        ckc_stride_t a_strides[3] = {
            ckc_stride_imm(1), ckc_stride_value(ctx->K), ckc_stride_imm(1)};
        ckc_stride_t b_strides[3] = {
            ckc_stride_imm(1), ckc_stride_value(ctx->K), ckc_stride_imm(1)};
        ckc_make_global_view(&ctx->a_view, ctx->A, view_shape, 3, ctx->storage_dtype, a_strides);
        ckc_make_global_view(&ctx->b_view, ctx->Bp, view_shape, 3, ctx->storage_dtype, b_strides);
    }
    ...
    /* LDS views: 2D (block_*, block_k); padded => with_strides((_lds_k,1)). */
    {
        ...
        if (ctx->lds_pad)
        {
            ckc_stride_t a_strides[2] = {ckc_stride_imm(ctx->lds_k), ckc_stride_imm(1)};
            ckc_stride_t b_strides[2] = {ckc_stride_imm(ctx->lds_k), ckc_stride_imm(1)};
            ckc_tensor_descriptor_with_strides(
        ...
        {
            ckc_tensor_descriptor_packed(&ctx->a_lds_desc, a_shape, 2, ctx->storage_dtype);
            ckc_tensor_descriptor_packed(&ctx->b_lds_desc, b_shape, 2, ctx->storage_dtype);
        }
    ...
    if (ctx->dtl)
    {
        ...
        {
            ctx->b->status = CKC_ERR_VALUE;
            snprintf(ctx->b->err, CKC_ERR_MSG_CAP,
                     "direct_to_lds requires block_k %% %d == 0 (got %d)",
                     ctx->dtl_halves, ctx->block_k);
            return;
        }
 ...
 *  pre-materialised in ckc_gemm_build_populate_ctx above.
 * ===================================================================== */
ckc_value_t* ckc_gemm_swz_col(ckc_gemm_build_ctx_t* ctx,
                              ckc_value_t* col,
                              ckc_value_t* row)
{
    ckc_ir_builder_t* b = ctx->b;
    ...
    return ckc_b_xor(
        b, col,
        ckc_b_shl(b, ckc_b_mod(b, ckc_b_lshr(b, row, ctx->c_swr), ctx->c_swmod),
                  ctx->c_swl));
}
                              ...
                              ckc_value_t* parity_v)
{
    ...
    if (spec->trait.direct_to_lds)
    {
        ...
        ckc_value_t* a_lds_par_base;
        ckc_value_t* b_lds_par_base;
        if (ctx->prefetch && parity_is_value)
        {
            ckc_value_t* a_par_v = ckc_b_zext(
                b, ckc_b_mul(b, parity_v, ckc_b_const_i32(b, a_half_bytes)), I64);
            ckc_value_t* b_par_v = ckc_b_zext(
                b, ckc_b_mul(b, parity_v, ckc_b_const_i32(b, b_half_bytes)), I64);
            a_lds_par_base = ckc_b_smem_ptr_add(b, ctx->dtl_a_lds_base, a_par_v);
            b_lds_par_base = ckc_b_smem_ptr_add(b, ctx->dtl_b_lds_base, b_par_v);
        }
        else
        {
        ...
        ckc_value_t* a_lds_wave_base;
        ckc_value_t* b_lds_wave_base;
        if (t->warp_m * t->warp_n * t->warp_k > 1)
        {
            ckc_value_t* warp_id = ckc_b_div(b, ctx->tid, ctx->c_wave);
            ckc_value_t* wave_par_off = ckc_b_zext(
                b, ckc_b_mul(b, warp_id, ckc_b_const_i32(b, wave_bytes)), I64);
            a_lds_wave_base = ckc_b_smem_ptr_add(b, a_lds_par_base, wave_par_off);
            b_lds_wave_base = ckc_b_smem_ptr_add(b, b_lds_par_base, wave_par_off);
        }
        else
        {
        ...
        for (int p = 0; p < ctx->dtl_a_passes; ++p)
        {
            ...
            ckc_value_t* pass_lds_a =
                (pass_off_bytes > 0)
                    ? ckc_b_smem_ptr_add(
                          b, a_lds_wave_base,
                          ckc_b_zext(b, ckc_b_const_i32(b, pass_off_bytes), I64))
                    : a_lds_wave_base;
            ckc_value_t* chunk_idx =
                ckc_b_add(b, ctx->tid, ckc_b_const_i32(b, p * spec->block_size));
            ckc_value_t* row = ckc_b_div(b, chunk_idx, ctx->dtl_c_chunks_per_row);
            ckc_value_t* col_v = ckc_b_mod(b, chunk_idx, ctx->dtl_c_chunks_per_row);
            ckc_value_t* col = ckc_b_mul(b, col_v, ctx->dtl_c_halves_per_chunk);
            ...
            ckc_value_t* a_row_term =
                ckc_b_mul(b, ckc_b_add(b, ctx->block_m_off, row), ctx->K);
            ckc_value_t* a_k_term = ckc_b_add(b, k_off, ckc_gemm_swz_col(ctx, col, row));
            ckc_value_t* off_elems =
                ckc_b_add(b, ctx->batch_off_a, ckc_b_add(b, a_row_term, a_k_term));
            ckc_value_t* off_bytes = ckc_b_mul(b, off_elems, c2);
            ckc_b_async_buffer_load_lds_addr(
                b, ctx->dtl_a_rsrc, pass_lds_a, off_bytes, ctx->dtl_zero_soff,
                ctx->dtl_dwords, spec->trait.dtl_cache_a);
        }
        for (int p = 0; p < ctx->dtl_b_passes; ++p)
        {
            int pass_off_bytes = p * ctx->dtl_pass_bytes + b_parity_bytes_static;
            ckc_value_t* pass_lds_b =
                (pass_off_bytes > 0)
                    ? ckc_b_smem_ptr_add(
                          b, b_lds_wave_base,
                          ckc_b_zext(b, ckc_b_const_i32(b, pass_off_bytes), I64))
            ...
            ckc_value_t* b_row_term =
                ckc_b_mul(b, ckc_b_add(b, ctx->block_n_off, row), ctx->K);
            ckc_value_t* b_k_term = ckc_b_add(b, k_off, ckc_gemm_swz_col(ctx, col, row));
            ckc_value_t* off_elems =
                ckc_b_add(b, ctx->batch_off_b, ckc_b_add(b, b_row_term, b_k_term));
            ckc_value_t* off_bytes = ckc_b_mul(b, off_elems, c2);
            ckc_b_async_buffer_load_lds_addr(
                b, ctx->dtl_b_rsrc, pass_lds_b, off_bytes, ctx->dtl_zero_soff,
                ctx->dtl_dwords, spec->trait.dtl_cache_b);
        }
        return;
    }
    ...
    ckc_tile_window_t a_global_tile, b_global_tile, a_lds_tile, b_lds_tile;
    {
        int a_glen[3] = {1, ctx->block_m, ctx->block_k};
        int b_glen[3] = {1, ctx->block_n, ctx->block_k};
        ckc_value_t* a_gorigin[3] = {ctx->batch_off_a, ctx->block_m_off, k_off};
        ckc_value_t* b_gorigin[3] = {ctx->batch_off_b, ctx->block_n_off, k_off};
        ckc_make_tile_window(&a_global_tile, &ctx->a_view, a_glen, a_gorigin, 3);
        ckc_make_tile_window(&b_global_tile, &ctx->b_view, b_glen, b_gorigin, 3);

        int a_llen[2] = {ctx->block_m, ctx->block_k};
        int b_llen[2] = {ctx->block_n, ctx->block_k};
        ckc_value_t* zero_a = ckc_b_const_i32(b, 0);
        ckc_value_t* zero_b = ckc_b_const_i32(b, 0);
        ckc_value_t* a_lorigin[2] = {zero_a, zero_b};
        ckc_value_t* zero_c = ckc_b_const_i32(b, 0);
        ckc_value_t* zero_d = ckc_b_const_i32(b, 0);
        ckc_value_t* b_lorigin[2] = {zero_c, zero_d};
        ckc_make_tile_window(&a_lds_tile, &ctx->a_lds_view, a_llen, a_lorigin, 2);
        ckc_make_tile_window(&b_lds_tile, &ctx->b_lds_view, b_llen, b_lorigin, 2);
    }
...
#define LD_ROW(rvar, blk)                                                                      \
    (!db ? (rvar)                                                                              \
         : (parity_v != NULL                                                                   \
                ? ckc_b_add(b, (rvar), ckc_b_mul(b, parity_v, ckc_b_const_i32(b, (blk))))      \
    ...
    {
        ckc_value_t* vec_idx = ckc_b_add(b, ckc_b_mul(b, ckc_b_const_i32(b, e), ctx->c_threads),
                                         ctx->tid);
        /* _vec_rc(vec_idx). */
        ...
        {
            ckc_value_t* gidx[3] = {ckc_b_const_i32(b, 0), row, col};
            ckc_value_t* a_val = ckc_tile_window_load_vec(b, &a_global_tile, gidx, 3, load_vec);
            ckc_value_t* lidx[2] = {LD_ROW(row, ctx->block_m), ckc_gemm_swz_col(ctx, col, row)};
    ...
    if (spec->trait.preshuffle_b)
    {
        ...
        {
            ckc_value_t* vec_idx = ckc_b_add(
                b, ckc_b_mul(b, ckc_b_const_i32(b, e), ctx->c_threads), ctx->tid);
            ckc_value_t* glob_off = ckc_b_add(b, base_off, ckc_b_mul(b, vec_idx, ctx->c_load_vec));
            ckc_value_t* row = ckc_b_div(b, vec_idx, ctx->c_block_k_div_vec);
            ckc_value_t* col_v = ckc_b_mod(b, vec_idx, ctx->c_block_k_div_vec);
            ckc_value_t* col = (load_vec > 1) ? ckc_b_mul(b, col_v, ctx->c_load_vec) : col_v;
            if (load_vec == 1)
            ...
            {
                ckc_value_t* b_val = ckc_b_global_load_vN(
                    b, ctx->Bp, glob_off, ctx->storage_dtype, load_vec, 0);
                ckc_value_t* lidx[2] = {LD_ROW(row, ctx->block_n), ckc_gemm_swz_col(ctx, col, row)};
                                          ...
                                          int frag_len)
{
    ...
    bool is_half = ckc_type_eq(ctx->storage_dtype, ckc_f16())
                   || ckc_type_eq(ctx->storage_dtype, ckc_bf16());
    int max_vec = is_half ? 8 : frag_len;
    if (frag_len <= max_vec)
    {
        return ckc_gemm_emit_smem_load(b, src, lds_row, lds_col, frag_len, ctx->storage_dtype);
    }
    ckc_value_t* frag = NULL;
    for (int off = 0; off < frag_len; off += max_vec)
    {
        ckc_value_t* chunk = ckc_gemm_emit_smem_load(
            b, src, lds_row, ckc_b_add(b, lds_col, ckc_b_const_i32(b, off)), max_vec,
            ctx->storage_dtype);
                              ...
                              ckc_value_t** out_accs)
{
    ...
    const ckc_arch_layout_map_t* a_map = ckc_mmaop_a_layout(ctx->op, b);
    const ckc_arch_layout_map_t* b_map = ckc_mmaop_b_layout(ctx->op, b);
    ...
    ckc_value_t* warp_m_off = ckc_b_mul(
        b, ctx->warp_m_idx, ckc_b_const_i32(b, ctx->mfmas_m * t->warp_tile_m));
    ckc_value_t* warp_n_off = ckc_b_mul(
        b, ctx->warp_n_idx, ckc_b_const_i32(b, ctx->mfmas_n * t->warp_tile_n));
    ...
    {
        ckc_value_t* k_tile_base = ckc_b_const_i32(b, kk * t->warp_tile_k);
        for (int mi = 0; mi < ctx->mfmas_m; ++mi)
        {
            ckc_value_t* atom_row = ckc_b_add(
                b, warp_m_off, ckc_b_const_i32(b, mi * t->warp_tile_m));
            a_rows[mi] = ckc_gemm_emit_frag_smem_load(
                ctx, A_src, a_row_in_atom, a_k_in_atom, atom_row, k_tile_base, ctx->a_per_lane);
        }
        for (int ni = 0; ni < ctx->mfmas_n; ++ni)
        {
            ckc_value_t* atom_row = ckc_b_add(
                b, warp_n_off, ckc_b_const_i32(b, ni * t->warp_tile_n));
            b_cols[ni] = ckc_gemm_emit_frag_smem_load(
