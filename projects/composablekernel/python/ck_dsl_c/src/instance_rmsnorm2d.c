...
ckc_status_t ckc_rmsnorm2d_kernel_name(const ckc_rmsnorm2d_spec_t* spec,
                                       char* out,
                                       size_t out_cap)
{
                                 ...
                                 size_t reason_cap)
{
    ...
    if (!ckc_archtarget_fits_lds(target, bytes_lds))
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap, "LDS budget %ld over cap on %s", bytes_lds,
                     arch);
        }
        ok = false;
        goto done;
 ...
 * we ignore). */
static ckc_value_t* ckc_rms_fadd_combine(ckc_ir_builder_t* b,
                                         ckc_value_t* a,
                                         ckc_value_t* c,
                                         void* user)
{
...
static void ckc_rms_pass1_body(ckc_ir_builder_t* b,
                               ckc_value_t* n_off,
                               ...
                               void* user)
{
    ...
    /* chunk_sq = [b.fmul(xi, xi) for xi in x_scalars] */
    chunk_sq =
        (ckc_value_t**)ckc_arena_alloc(&b->arena, (size_t)(vec > 0 ? vec : 1) *
                                                      sizeof(ckc_value_t*));
    for (i = 0; i < vec; ++i)
    {
                               ...
                               void* user)
{
    ...
    if (ctx->two_pass)
    {
        ...
        ckc_tile_window_load_vec_as_f32(b, ctx->x_tile, local_indices, 2, vec,
                                        xs_local);
        xs = xs_local;
    }
    else
...
ckc_kernel_def_t* ckc_build_rmsnorm2d(ckc_ir_builder_t* b,
...
ckc_status_t ckc_rmsnorm2d_grid(int m,
                                const ckc_rmsnorm2d_spec_t* spec,
                                int out[3])
{
    int totals[2];
