 ...
 * (status + err). */
static void* ckc_red_set_err(ckc_ir_builder_t* b, ckc_status_t st, const char* fmt, ...)
{
    if(b == NULL)
    {
        return NULL;
...
ckc_red_emit_combine(ckc_ir_builder_t* b, ckc_reduce_combine_t combine, ckc_value_t* a, ckc_value_t* c)
{
    ...
    default:
        return ckc_red_set_err(b, CKC_ERR_VALUE, "unknown combine %d", (int)combine);
    }
}
                             ...
                             int n)
{
    ...
    if(n < 1)
    {
        return ckc_red_set_err(b, CKC_ERR_VALUE, "tree_reduce requires at least one value");
    }
    ...
    {
        return ckc_red_set_err(b, CKC_ERR_OOM, "tree_reduce arena alloc failed");
    }
    for(i = 0; i < n; ++i)
    {
        cur[i] = xs[i];
                                                     ...
                                                     int wave_size)
{
    ...
    if(parts == NULL)
    {
        return ckc_red_set_err(b, CKC_ERR_OOM, "wave prologue parts alloc failed");
    }
