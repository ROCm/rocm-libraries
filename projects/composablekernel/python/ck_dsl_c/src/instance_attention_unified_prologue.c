...
void ckc_attn_unified_emit_prologue(ckc_attn_unified_build_ctx_t* ctx)
{
    ...
    if(ctx->kind == CKC_ATTN_UNIFIED_3D)
    {
        ...
        if(!ckc_unified_attn_magic_div_mod(b, ctx->zd, ctx->p->head_size,
                                           &ctx->segm_idx, &ctx->dim))
        {
            return;
        }
    }
    ...
    if(ctx->kind == CKC_ATTN_UNIFIED_3D)
    {
        ...
        ctx->tiles_per_segment = ckc_unified_attn_magic_div(
            b,
            ckc_b_add(b, ctx->kv_len, ckc_b_const_i32(b, seg_block - 1)),
            seg_block);
        ...
        seg_stop_raw = ckc_b_mul(
            b, ckc_b_add(b, ctx->segm_idx, ckc_b_const_i32(b, 1)),
            ckc_b_mul(b, ctx->tiles_per_segment, ckc_b_const_i32(b, ctx->p->block_size)));
    ...
    if(ctx->kind == CKC_ATTN_UNIFIED_3D)
    {
        if(!ckc_unified_attn_segm_descriptors(b, &ctx->sel_p, ctx->num_segments,
                                              &ctx->ml_desc, &ctx->out_desc))
        {
            return;
        }
    }
