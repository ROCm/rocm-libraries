...
ckc_status_t ckc_coalesced_tile_loader_vecs_per_thread(
    const ckc_coalesced_tile_loader_t* self, int* out)
{
    int total_vecs;
                                    ...
                                    ckc_value_t* ptr)
{
    ...
    if (self->use_buffer_rsrc && rsrc == NULL)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "CoalescedTileLoader: use_buffer_rsrc=True requires rsrc");
        return;
    }
    if (!self->use_buffer_rsrc && ptr == NULL)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "CoalescedTileLoader: use_buffer_rsrc=False requires ptr");
        return;
    }
    ...
    if (ckc_coalesced_tile_loader_vecs_per_thread(self, &vecs_per_thread) != CKC_OK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      ...
                      "block_size %d",
                      self->tile_rows, self->tile_cols, self->load_vec,
                      self->block_size);
        return;
    }
    ...
    for (e = 0; e < vecs_per_thread; ++e)
    {
        ...
        else
        {
            ...
            else
            {
                 ...
                 */
                ckc_value_t* v =
                    ckc_b_global_load_vN_f16(b, ptr, off_elems, self->load_vec, 0);
                ckc_b_smem_store_vN_f16(b, smem_dst, indices, 2, v, self->load_vec);
            }
        }
    }
                                      ...
                                      int coherency)
{
    ...
    for (p = 0; p < L->passes; ++p)
    {
        ...
        if (p > 0)
        {
            pass_base = ckc_b_smem_ptr_add(
                b, self->per_wave_lds_base,
...
int ckc_async_tile_loader_slot_required_lds_bytes(
    const ckc_async_tile_loader_slot_t* self)
{
