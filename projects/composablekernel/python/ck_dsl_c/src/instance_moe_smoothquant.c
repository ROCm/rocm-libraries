...
static ckc_value_t* ckc_moe_sq_tree_fmax(ckc_ir_builder_t* b,
                                         ckc_value_t** values,
                                         int n,
                                         ckc_value_t** scratch)
{
...
static int ckc_moe_sq_pick_traits(const ckc_tile_distribution_encoding_t* enc,
                                  int* vector_dim_y)
{
    int num_Y = enc->num_Y;
...
static int ckc_moe_sq_y_to_linear(const ckc_tile_distribution_encoding_t* enc,
                                  const int* y)
{
    int off = 0;
    int i;
...
{
    int off, i;
    if ((strcmp(qname, "fp8e4m3") == 0 || strcmp(qname, "bf8e5m2") == 0) &&
        (n % 4) == 0)
    {
        ...
        for (off = 0; off < n; off += 4)
        {
            ...
            ckc_value_t* chunk = (strcmp(qname, "fp8e4m3") == 0)
                                     ? ckc_b_cvt_pk_fp8_f32x4(b, quad)
    ...
    }
    {
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            {
                ckc_value_t* cl = ckc_b_clamp_f32(
                    b, scaled[i], ckc_b_const_f32(b, -127.0), ckc_b_const_f32(b, 127.0));
                                       ...
                                       size_t reason_cap)
{
         ...
         strcmp(canon, "bf8e5m2") != 0))
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason, reason_cap, "unsupported out_dtype %s",
                                            ...
                                            const char* arch)
{
    ...
    if (VEC == 4 || VEC == 8)
    {
        ...
        ckc_tile_distribution_t* qy_dist =
            ckc_moe_sq_make_row_x_distribution(b, BS, VEC, EPT);
    ...
    else
    {
        ...
        for (k = 0; k < chunks; ++k)
        {
            ...
            for (i = 0; i < VEC; ++i)
            {
                ckc_value_t* x_f32 = cached[k * VEC + i];
                ckc_value_t* y_f32 = ckc_b_fmul(b, x_f32, sm_scalars[i]);
                ckc_value_t* q = ckc_moe_sq_quantize_scalar_f32(
                    b, y_f32, inv_yscale, spec->out_dtype);
...
ckc_status_t ckc_moe_smoothquant_grid(int tokens,
                                      const ckc_moe_smoothquant_spec_t* spec,
                                      int out[3])
{
    int totals[1];
