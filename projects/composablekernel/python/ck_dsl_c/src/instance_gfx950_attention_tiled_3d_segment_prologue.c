                                        ...
                                        ckc_value_t* dim)
{
    ...
    in_values[2] = seg;
    in_values[3] = dim;
    if(!ckc_transforms_descriptor_offset(B, ctx->seg_acc_desc, in_names, in_values, 4, &off, &valid))
    {
        return NULL;
    }
    return off;
...
void ckc_gfx950_attention_tiled_3d_emit_prologue(ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx)
{
    ...
    ctx->seq_idx = ckc_binary_search_seq_idx(B, ctx->cu_q, ctx->q_block_global_idx,
                                             ctx->num_seqs_p, BLOCK_Q, CFG.binary_search_iters,
                                             false);
 ...
 * ============================================================ */
void ckc_gfx950_attention_tiled_3d_emit_async_infra(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx)
{
     ...
     * )  (lines 593-601) */
    {
        ...
        chain[0] = ckc_indirect(B, "tile_idx", "physical_block", ctx->block_tables, ctx->seq_base,
                                NULL, 0);
        ...
        /* seg_acc zero-fill (lines 436-460) */
        {
            ...
            for(n = 0; n < PV_N_TILES; ++n)
            {
                for(reg = 0; reg < 4; ++reg)
                {
                    ...
                    ckc_value_t* row_ok_b = ckc_b_cmp_lt(B, qh_r, ckc_b_const_i32(B, NUM_QH));
                    ckc_value_t* row_ok   = ckc_b_land(B, row_ok_a, row_ok_b);
                    ckc_value_t* qp_r_safe =
                        ckc_b_select(B, row_ok, qp_r, ckc_b_const_i32(B, 0));
                    ckc_value_t* qh_r_safe =
                        ckc_b_select(B, row_ok, qh_r, ckc_b_const_i32(B, 0));
                    ckc_value_t* qtoken = ckc_b_add(B, ctx->cu_q_start, qp_r_safe);
...
void ckc_gfx950_attention_tiled_3d_emit_q_to_lds(ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx)
{
    ...
    {
        ckc_value_t* q_vid_mul =
            ckc_b_mul(B, ckc_b_const_i32(B, li), ckc_b_const_i32(B, THREADS));
...
void ckc_gfx950_attention_tiled_3d_issue_k_load(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* kv_tile_idx, ckc_value_t* buf_idx)
{
    ...
    for(call = 0; call < kv_calls_per_tile; ++call)
    {
        ...
        ckc_value_t* voff =
            ckc__paged_kv_offset(ctx, kv_tile_idx, linear_half, ctx->kv_head_idx);
        ckc_value_t* k_dst =
...
void ckc_gfx950_attention_tiled_3d_issue_v_load(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* kv_tile_idx, ckc_value_t* buf_idx)
{
    const int KV_HALVES_PER_CALL = CFG.KV_HALVES_PER_CALL;
    const int kv_calls_per_tile  = CFG.kv_calls_per_tile;
...
void ckc_gfx950_attention_tiled_3d_issue_fp8_dequant_loads(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx, bool is_value, ckc_value_t* kv_tile_idx,
    ckc_value_t* buf_idx)
{
    const int HD                    = CFG.HD;
    const int THREADS               = CFG.THREADS;
...
void ckc_gfx950_attention_tiled_3d_issue_k(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* tile_idx, ckc_value_t* buf_idx)
{
    if(CFG.KV_FP8)
    {
...
void ckc_gfx950_attention_tiled_3d_issue_v(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx, ckc_value_t* tile_idx, ckc_value_t* buf_idx)
{
    if(CFG.KV_FP8)
    {
