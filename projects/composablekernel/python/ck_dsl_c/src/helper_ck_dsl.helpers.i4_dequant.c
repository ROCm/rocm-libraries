                                            ...
                                            ckc_value_t** out_high)
{
    ...
    if (packed_byte == NULL || packed_byte->type == NULL ||
        packed_byte->type->name == NULL ||
        strcmp(packed_byte->type->name, "i8") != 0)
    {
        const char* got = (packed_byte != NULL && packed_byte->type != NULL &&
                           packed_byte->type->name != NULL)
                              ? packed_byte->type->name
                              : "(null)";
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "unpack_i4_byte_to_pair_i32 expects i8 input, got %s",
                      got);
    ...
    high_unsigned =
        ckc_b_land(b, ckc_b_lshr(b, byte_i32, ckc_b_const_i32(b, 4)), mask_lo);
