...
static const char *mem_idx_join(ckc_h_lowerer_t *lw,
                                ckc_value_t *const *vals, int n)
{
    ckc_strbuf_t sb;
    const char *out;
    int i;
...
static const char *mem_attr_str(const ckc_op_t *op, const char *key,
                                const char *dflt)
{
...
static ckc_status_t _op_tile_smem_alloc(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_alloc: missing result");
    }
    res = op->results[0];
    st = res->type;
    if (!st || st->kind != CKC_TYPE_SMEM) {
        return ckc_h_fail(lw, CKC_ERR_TYPE,
                          "tile.smem_alloc: result is not an smem type");
    }
    ...
    ckc_strbuf_appendf(&decl, "    __shared__ %s %s_storage[%s];",
                       elem, nice, ckc_strbuf_cstr(&dims));
...
static ckc_status_t _op_tile_smem_store(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem store before smem_alloc was lowered");
    }
...
static ckc_status_t _op_tile_smem_store_vN(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_operands < 2) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_store_vN: too few operands");
    }
    smem = op->operands[0];
    value = op->operands[op->num_operands - 1];
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem store_vN before smem_alloc was lowered");
    }
    idx_str = mem_idx_join(lw, &op->operands[1], op->num_operands - 2);
    elem_name = mem_attr_str(op, "elem_type", "f16");
    prefix = ckc_h_vec_prefix(elem_name, /*full_map=*/true);
    ckc_h_emitf(lw, "*reinterpret_cast<%s%lld*>(&%s[%s]) = %s;",
                prefix, (long long)vec, storage, idx_str,
                ckc_h_name(lw, value));
    return lw->status;
}

/* Python _op_tile_smem_store_vN_f32 */
static ckc_status_t _op_tile_smem_store_vN_f32(ckc_h_lowerer_t *lw,
                                               const ckc_op_t *op)
{
    ckc_value_t *smem, *value;
    const char *storage, *idx_str;
    ...
    if (op->num_operands < 2) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_store_vN_f32: too few operands");
    }
    smem = op->operands[0];
    value = op->operands[op->num_operands - 1];
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem store_vN_f32 before smem_alloc was lowered");
    }
    idx_str = mem_idx_join(lw, &op->operands[1], op->num_operands - 2);
    ckc_h_emitf(lw, "*reinterpret_cast<f32x%lld*>(&%s[%s]) = %s;",
                (long long)n, storage, idx_str, ckc_h_name(lw, value));
    return lw->status;
}

/* Python _op_tile_smem_store_distributed (P42 debug shim) */
static ckc_status_t _op_tile_smem_store_distributed(ckc_h_lowerer_t *lw,
                                                    const ckc_op_t *op)
{
    ...
    if (op->num_operands < 2) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_store_distributed: too few operands");
    }
    smem = op->operands[0];
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem_store_distributed before smem_alloc was lowered");
    }
    n = (values->type && values->type->kind == CKC_TYPE_VECTOR)
            ? values->type->count
            : 1;
    for (i = 0; i < n; i++) {
        ckc_h_emitf(lw, "%s[%d] = %s[%d];",
                    storage, i, ckc_h_name(lw, values), i);
    }
    return lw->status;
}
...
static ckc_status_t _op_tile_smem_load_v4(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_operands < 3 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_load_v4: bad operand/result count");
    }
    smem = op->operands[0];
    row = op->operands[1];
    col = op->operands[2];
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem load before smem_alloc was lowered");
    }
    nice = ckc_h_name(lw, op->results[0]);
    ckc_h_emitf(lw, "f16x4 %s;", nice);
    for (i = 0; i < 4; i++) {
        ckc_h_emitf(lw, "%s[%d] = %s[%s][%s + %d];",
                    nice, i, storage, ckc_h_name(lw, row),
                    ckc_h_name(lw, col), i);
    }
    return lw->status;
}
...
static ckc_status_t _op_tile_smem_load_vN(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_operands < 1 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_load_vN: bad operand/result count");
    }
    smem = op->operands[0];
    n = mem_attr_int(op, "vec", 0);
    elem_name = mem_attr_str(op, "elem_type", "f16");
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem load_vN before smem_alloc was lowered");
    }
    ...
    ckc_h_emitf(lw,
                ...
                prefix, (long long)n, res, prefix, (long long)n,
                storage, idx_str);
    return lw->status;
}

/* Python _op_tile_smem_load_vN_f32 */
static ckc_status_t _op_tile_smem_load_vN_f32(ckc_h_lowerer_t *lw,
                                              const ckc_op_t *op)
{
    ckc_value_t *smem;
    ...
    if (op->num_operands < 1 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_load_vN_f32: bad operand/result count");
    }
    smem = op->operands[0];
    n = mem_attr_int(op, "vec", 0);
    storage = mem_storage_of(lw, smem);
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem load_vN_f32 before smem_alloc was lowered");
    }
    idx_str = mem_idx_join(lw, &op->operands[1], op->num_operands - 1);
    res = ckc_h_name(lw, op->results[0]);
    ckc_h_emitf(lw,
                "f32x%lld %s = *reinterpret_cast<const f32x%lld*>(&%s[%s]);",
                (long long)n, res, (long long)n, storage, idx_str);
    return lw->status;
}
...
static ckc_status_t _op_tile_smem_addr_of(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_operands < 1 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_addr_of: bad operand/result count");
    }
    smem = op->operands[0];
    storage = mem_storage_of(lw, smem);
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "smem_addr_of before smem_alloc was lowered");
    }
    ckc_h_emitf(lw, "int64_t %s = (int64_t)(&%s[0]);",
                ckc_h_name(lw, op->results[0]), storage);
    return lw->status;
}
...
static ckc_status_t _op_tile_smem_ptr_add(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.smem_ptr_add: bad operand/result count");
    }
    base = op->operands[0];
    off = op->operands[1];
    ckc_h_emitf(lw, "int64_t %s = %s + %s;",
                ckc_h_name(lw, op->results[0]),
                ckc_h_name(lw, base), ckc_h_name(lw, off));
    return lw->status;
}
...
static ckc_status_t _op_tile_lds_atomic_add(ckc_h_lowerer_t *lw,
                                            const ckc_op_t *op)
{
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.lds_atomic_add: bad operand/result count");
    }
    smem = op->operands[0];
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "lds_atomic_add before smem_alloc was lowered");
    }
    idx_expr = mem_idx_join(lw, &op->operands[1], op->num_operands - 2);
    ckc_h_emitf(lw, "%s %s = atomicAdd(&%s[%s], %s);",
                cpp_t, ckc_h_name(lw, op->results[0]),
                storage, idx_expr, ckc_h_name(lw, val));
    return lw->status;
}
...
static ckc_status_t _op_memref_global_load(ckc_h_lowerer_t *lw,
                                           const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx;
    if (!ckc_h_live(lw)) {
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_load: bad operand/result count");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    ckc_h_emitf(lw, "fp16 %s = %s[%s];",
                ckc_h_name(lw, op->results[0]),
                ckc_h_name(lw, ptr), ckc_h_name(lw, idx));
    return lw->status;
}

/* Python _op_memref_global_load_typed */
static ckc_status_t _op_memref_global_load_typed(ckc_h_lowerer_t *lw,
                                                 const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx;
    const char *cpp_t;
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_load_typed: bad operand/result count");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    cpp_t = ckc_h_type_to_hip(lw, op->results[0]->type);
    ckc_h_emitf(lw, "%s %s = %s[%s];",
                cpp_t, ckc_h_name(lw, op->results[0]),
                ckc_h_name(lw, ptr), ckc_h_name(lw, idx));
    return lw->status;
}
...
static ckc_status_t _op_memref_global_load_vN(ckc_h_lowerer_t *lw,
                                              const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx;
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_load_vN: bad operand/result count");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    vec = mem_attr_int(op, "vec", 0);
    ...
    ckc_h_emitf(lw,
                "%s%lld %s = *reinterpret_cast<const %s%lld*>(%s + %s);",
                prefix, (long long)vec, res, prefix, (long long)vec,
                ckc_h_name(lw, ptr), ckc_h_name(lw, idx));
    return lw->status;
}
...
static ckc_status_t _op_memref_global_store(ckc_h_lowerer_t *lw,
                                            const ckc_op_t *op)
{
    ...
    if (op->num_operands < 3) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_store: too few operands");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    val = op->operands[2];
    ckc_h_emitf(lw, "%s[%s] = %s;",
                ckc_h_name(lw, ptr), ckc_h_name(lw, idx),
                ckc_h_name(lw, val));
    return lw->status;
}
...
static ckc_status_t _op_memref_global_store_typed(ckc_h_lowerer_t *lw,
                                                  const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx, *val;
    if (!ckc_h_live(lw)) {
    ...
    if (op->num_operands < 3) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_store_typed: too few operands");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    val = op->operands[2];
...
static ckc_status_t _op_memref_global_store_vN(ckc_h_lowerer_t *lw,
                                               const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx, *val;
    ...
    if (op->num_operands < 3) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_store_vN: too few operands");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    val = op->operands[2];
    ...
    prefix = ckc_h_vec_prefix(elem_name, /*full_map=*/false);
    ckc_h_emitf(lw, "*reinterpret_cast<%s%lld*>(%s + %s) = %s;",
                prefix, (long long)n, ckc_h_name(lw, ptr),
                ckc_h_name(lw, idx), ckc_h_name(lw, val));
    return lw->status;
}
...
static ckc_status_t _op_memref_global_atomic_add(ckc_h_lowerer_t *lw,
                                                 const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx, *val;
    const char *cpp_t;
    ...
    if (op->num_operands < 3 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_atomic_add: bad operand/result count");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    val = op->operands[2];
...
static ckc_status_t _op_memref_global_atomic_add_f32(ckc_h_lowerer_t *lw,
                                                     const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx, *val;
    if (!ckc_h_live(lw)) {
    ...
    if (op->num_operands < 3) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.global_atomic_add_f32: too few operands");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    val = op->operands[2];
    ckc_h_emitf(lw, "atomicAdd(%s + %s, %s);",
                ckc_h_name(lw, ptr), ckc_h_name(lw, idx),
                ckc_h_name(lw, val));
    return lw->status;
}
...
static ckc_status_t _op_memref_global_atomic_add_pk_bf16(ckc_h_lowerer_t *lw,
                                                         const ckc_op_t *op)
{
    ckc_value_t *ptr, *idx, *val;
    if (!ckc_h_live(lw)) {
    ...
    if (op->num_operands < 3 || op->num_results < 1) {
        return ckc_h_fail(
            lw, CKC_ERR_VALUE,
            "memref.global_atomic_add_pk_bf16: bad operand/result count");
    }
    ptr = op->operands[0];
    idx = op->operands[1];
    val = op->operands[2];
...
static ckc_status_t _op_memref_cooperative_global_store(ckc_h_lowerer_t *lw,
                                                        const ckc_op_t *op)
{
    ...
    if (op->num_operands < 3) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "memref.cooperative_global_store: too few operands");
    }
    ptr = op->operands[0];
    addrs = op->operands[1];
    ...
    n = mem_attr_int(op, "vec", 0);
    for (i = 0; i < (int)n; i++) {
        ckc_h_emitf(lw, "%s[%s[%d]] = %s[%d];",
                    ckc_h_name(lw, ptr), ckc_h_name(lw, addrs), i,
                    ckc_h_name(lw, values), i);
    }
    return lw->status;
}
...
static ckc_status_t _op_tile_global_ptr_add(ckc_h_lowerer_t *lw,
                                            const ckc_op_t *op)
{
    ckc_value_t *ptr, *off;
    if (!ckc_h_live(lw)) {
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.global_ptr_add: bad operand/result count");
    }
    ptr = op->operands[0];
    off = op->operands[1];
    ckc_h_emitf(lw,
                ...
                ckc_h_name(lw, op->results[0]),
                ckc_h_name(lw, ptr), ckc_h_name(lw, off));
    return lw->status;
}
...
static ckc_status_t _op_tile_buffer_rsrc(ckc_h_lowerer_t *lw, const ckc_op_t *op)
{
    ...
    if (op->num_operands < 2 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.buffer_rsrc: bad operand/result count");
    }
    ptr = op->operands[0];
    ...
    ckc_h_emitf(lw,
                ...
                ckc_h_name(lw, op->results[0]),
                ckc_h_name(lw, ptr), ckc_h_name(lw, num_bytes));
    return lw->status;
}
...
static ckc_status_t _op_tile_buffer_load_f16(ckc_h_lowerer_t *lw,
                                             const ckc_op_t *op)
{
    ...
    if (op->num_operands < 3 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.buffer_load_f16: bad operand/result count");
    }
    rsrc = op->operands[0];
    ...
    ckc_h_emitf(lw,
                ...
                tmp, ckc_h_name(lw, rsrc), ckc_h_name(lw, voffset),
...
static ckc_status_t _op_tile_buffer_load_vN_f16(ckc_h_lowerer_t *lw,
                                                const ckc_op_t *op)
{
    ckc_value_t *rsrc, *voffset, *soffset;
    ...
    if (op->num_operands < 3 || op->num_results < 1) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.buffer_load_vN_f16: bad operand/result count");
    }
    rsrc = op->operands[0];
    voffset = op->operands[1];
    soffset = op->operands[2];
    ...
    ckc_h_emitf(lw,
                ...
                raw_t, tmp, b_suffix, ckc_h_name(lw, rsrc),
                ckc_h_name(lw, voffset), ckc_h_name(lw, soffset));
    ckc_h_emitf(lw,
                "f16x%lld %s; __builtin_memcpy(&%s, &%s, %lld);",
                halves, res, res, tmp, (long long)dwords * 4);
    return lw->status;
}
...
static ckc_status_t _op_tile_buffer_store_f16(ckc_h_lowerer_t *lw,
                                              const ckc_op_t *op)
{
    ...
    if (op->num_operands < 4) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.buffer_store_f16: too few operands");
    }
    rsrc = op->operands[0];
    voffset = op->operands[1];
    soffset = op->operands[2];
    ...
    ckc_h_emitf(lw,
                ...
                tmp, tmp, vname, tmp, ckc_h_name(lw, rsrc),
                ckc_h_name(lw, voffset), ckc_h_name(lw, soffset));
    return lw->status;
}

/* Python _op_tile_buffer_store_vN_f16 */
static ckc_status_t _op_tile_buffer_store_vN_f16(ckc_h_lowerer_t *lw,
                                                 const ckc_op_t *op)
{
    ckc_value_t *rsrc, *voffset, *soffset, *val;
    ...
    if (op->num_operands < 4) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.buffer_store_vN_f16: too few operands");
    }
    rsrc = op->operands[0];
    voffset = op->operands[1];
    soffset = op->operands[2];
    ...
    if (dwords == 1) {
        ...
        ckc_h_emitf(lw,
                    ...
                    (long long)dwords, tmp, tmp, vname,
                    (long long)dwords * 4, b_suffix, tmp,
                    ckc_h_name(lw, rsrc), ckc_h_name(lw, voffset),
                    ckc_h_name(lw, soffset));
    } else {
        return ckc_h_fail(lw, CKC_ERR_KEY,
...
static ckc_status_t _op_tile_async_buffer_load_lds_addr(ckc_h_lowerer_t *lw,
                                                        const ckc_op_t *op)
{
    ...
    if (op->num_operands < 4) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.async_buffer_load_lds_addr: too few operands");
    }
    rsrc = op->operands[0];
    ...
    ckc_h_emitf(lw,
                ...
                "%lld, %s, %s, 0, 0);",
                ckc_h_name(lw, rsrc), ckc_h_name(lw, lds_addr),
                (long long)size_bytes, ckc_h_name(lw, voff),
                ckc_h_name(lw, soff));
    return lw->status;
}

/* Python _op_tile_async_buffer_load_lds (typed-LDS variant) */
static ckc_status_t _op_tile_async_buffer_load_lds(ckc_h_lowerer_t *lw,
                                                   const ckc_op_t *op)
{
    ...
    if (op->num_operands < 4) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "tile.async_buffer_load_lds: too few operands");
    }
    rsrc = op->operands[0];
    ...
    if (!storage) {
        return ckc_h_fail(lw, CKC_ERR_VALUE,
                          "async_buffer_load_lds before smem_alloc was lowered");
    }
    ckc_h_emitf(lw,
                "_llvm_amdgcn_raw_ptr_buffer_load_lds(%s, "
                "(__attribute__((address_space(3))) void*)&%s[0], "
                "%lld, %s, %s, 0, %lld);",
                ckc_h_name(lw, rsrc), storage, (long long)size_bytes,
                ckc_h_name(lw, voff), ckc_h_name(lw, soff), (long long)aux);
    return lw->status;
}
