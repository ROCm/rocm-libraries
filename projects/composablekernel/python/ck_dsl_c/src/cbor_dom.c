 ...
 *
 * ckc/cbor_dom.c -- minimal CBOR (RFC 8949) decoder into the json_dom jd_val_t.
...
{
    jd_val_t* v = (jd_val_t*)ckc_arena_calloc(r->arena, sizeof(jd_val_t));
    if (!v)
...
static char* cbor_rd_text(cbor_rd_t* r, uint64_t n)
{
    ...
    char* s = (char*)ckc_arena_alloc(r->arena, (size_t)n + 1);
    if (!s)
    {
        cbor_fail(r, "out of memory");
        return NULL;
...
static jd_val_t* cbor_value(cbor_rd_t* r, int depth)
{
    ...
    case 4: /* array */
    {
        ...
        if (arg > 0)
        {
            v->arr = (jd_val_t**)ckc_arena_calloc(r->arena, sizeof(jd_val_t*) * (size_t)arg);
            if (!v->arr)
            {
                cbor_fail(r, "out of memory");
                return NULL;
    ...
    case 5: /* map */
    {
        ...
        if (arg > 0)
        {
            v->obj = (jd_member_t*)ckc_arena_calloc(r->arena, sizeof(jd_member_t) * (size_t)arg);
            if (!v->obj)
            {
                cbor_fail(r, "out of memory");
                return NULL;
