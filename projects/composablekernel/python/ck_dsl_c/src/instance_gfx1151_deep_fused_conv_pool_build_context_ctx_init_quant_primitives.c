 ...
 *   - ckc_gfx1151_dfcp_build_ctx_init        (the Python prologue, lines
...
ckc_value_t* ckc_gfx1151_dfcp_quant_i8(ckc_ir_builder_t* b, ckc_value_t* vf32, ckc_value_t* inv_scale)
{
    ckc_value_t* scaled;
    ckc_value_t* clamped;
    if (b == NULL)
    ...
    scaled = ckc_b_fmul(b, vf32, inv_scale);
    clamped = ckc_b_clamp_f32(b, scaled, ckc_b_const_f32(b, -127.0), ckc_b_const_f32(b, 127.0));
 ...
 *     return b.cvt_f32_to_i8_sat(clamped) */
ckc_value_t* ckc_gfx1151_dfcp_quant_i4(ckc_ir_builder_t* b, ckc_value_t* vf32, ckc_value_t* inv_scale)
{
    ckc_value_t* scaled;
    ckc_value_t* clamped;
    if (b == NULL)
    ...
    scaled = ckc_b_fmul(b, vf32, inv_scale);
    clamped = ckc_b_clamp_f32(b, scaled, ckc_b_const_f32(b, -8.0), ckc_b_const_f32(b, 7.0));
    return ckc_b_cvt_f32_to_i8_sat(b, clamped);
}
...
ckc_value_t* ckc_gfx1151_dfcp_i8_to_f32(ckc_ir_builder_t* b, ckc_value_t* qi8)
{
    ...
    return ckc_b_sitofp_f32(b, ckc_b_sext(b, qi8, ckc_i32()));
}
...
ckc_value_t* ckc_gfx1151_dfcp_neg_i32(ckc_ir_builder_t* b, ckc_value_t* x)
{
    ...
    return ckc_b_sub(b, ckc_b_const_i32(b, 0), x);
}
...
ckc_value_t* ckc_gfx1151_dfcp_clamp_i32(ckc_ir_builder_t* b, ckc_value_t* x, int lo, int hi)
{
    ...
    return ckc_b_smin(b, ckc_b_smax(b, x, ckc_b_const_i32(b, lo)), ckc_b_const_i32(b, hi));
}
...
ckc_value_t* ckc_gfx1151_dfcp_relu_i32(ckc_ir_builder_t* b, ckc_value_t* x)
{
    ...
    return ckc_b_smax(b, x, ckc_b_const_i32(b, 0));
}
...
ckc_value_t* ckc_gfx1151_dfcp_quant_i8_shift(ckc_ir_builder_t* b, ckc_value_t* x, int shift)
{
    ...
    q = ckc_gfx1151_dfcp_clamp_i32(b, ckc_gfx1151_dfcp_round_shift_rne_i32(b, x, shift), -127, 127);
    return ckc_b_trunc(b, q, ckc_i8());
}
...
ckc_value_t* ckc_gfx1151_dfcp_quant_i4_shift(ckc_ir_builder_t* b, ckc_value_t* x, int shift)
{
    ...
    q = ckc_gfx1151_dfcp_clamp_i32(b, ckc_gfx1151_dfcp_round_shift_rne_i32(b, x, shift), -8, 7);
    return ckc_b_trunc(b, q, ckc_i8());
}
...
ckc_value_t* ckc_gfx1151_dfcp_splat_i32(ckc_ir_builder_t* b, int value, int n)
{
    ...
    return ckc_b_vector_splat(b, ckc_b_const_i32(b, value), n);
}

/* Helper: x.type.count for an <N x i32> vector operand. */
static int ckc_gfx1151_dfcp_vec_count(const ckc_value_t* x)
{
...
ckc_value_t* ckc_gfx1151_dfcp_neg_i32_vec(ckc_ir_builder_t* b, ckc_value_t* x)
{
    ...
    return ckc_b_vector_sub(b, ckc_b_zero_vec(b, ckc_i32(), ckc_gfx1151_dfcp_vec_count(x)), x);
}
...
ckc_value_t* ckc_gfx1151_dfcp_clamp_i32_vec(ckc_ir_builder_t* b, ckc_value_t* x, int lo, int hi)
{
    ...
    return ckc_b_vector_smin(b,
                             ckc_b_vector_smax(b, x, ckc_gfx1151_dfcp_splat_i32(b, lo, n)),
                             ckc_gfx1151_dfcp_splat_i32(b, hi, n));
}
...
ckc_value_t* ckc_gfx1151_dfcp_relu_i32_vec(ckc_ir_builder_t* b, ckc_value_t* x)
{
    ...
    return ckc_b_vector_smax(b, x, ckc_b_zero_vec(b, ckc_i32(), ckc_gfx1151_dfcp_vec_count(x)));
}
...
ckc_value_t* ckc_gfx1151_dfcp_round_shift_rne_i32_vec(ckc_ir_builder_t* b, ckc_value_t* x, int shift)
{
    int n;
    ckc_value_t* sign;
    ckc_value_t* ax;
    ...
    q = ckc_b_vector_lshr(b,
                          ckc_b_vector_add(b, ax, bias),
                          ckc_gfx1151_dfcp_splat_i32(b, shift, n));
...
ckc_value_t* ckc_gfx1151_dfcp_quant_i8_shift_vec_i32(ckc_ir_builder_t* b, ckc_value_t* x, int shift)
{
    if (b == NULL)
    {
        return NULL;
    }
    return ckc_gfx1151_dfcp_clamp_i32_vec(
        b, ckc_gfx1151_dfcp_round_shift_rne_i32_vec(b, x, shift), -127, 127);
}

/* def _quant_i4_shift_vec_i32(b, x, shift):
 *     return _clamp_i32_vec(b, _round_shift_rne_i32_vec(b, x, shift), -8, 7) */
ckc_value_t* ckc_gfx1151_dfcp_quant_i4_shift_vec_i32(ckc_ir_builder_t* b, ckc_value_t* x, int shift)
{
    if (b == NULL)
    {
        return NULL;
    ...
    return ckc_gfx1151_dfcp_clamp_i32_vec(
        b, ckc_gfx1151_dfcp_round_shift_rne_i32_vec(b, x, shift), -8, 7);
}
 ...
 *  (ckc_gfx1151_dfcp_code_fn_t) and apply the matching transform to each acc
 ...
 * (Python 2933-2934). */
static ckc_value_t* ckc_gfx1151_dfcp_conv0_code_f16(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                    ckc_value_t* p0)
{
    ckc_ir_builder_t* b = ctx->b;
...
static ckc_value_t* ckc_gfx1151_dfcp_conv0_code_vec_i8(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                       ckc_value_t* acc)
{
    ckc_ir_builder_t* b = ctx->b;
                                                   ...
                                                   ckc_value_t* p1)
{
    ...
    if (spec->native_int)
    {
        ...
        ckc_value_t* q1 = ckc_gfx1151_dfcp_quant_i4_shift(b, p1, 2);
        return ckc_b_trunc(b, ckc_gfx1151_dfcp_relu_i32(b, ckc_b_sext(b, q1, ckc_i32())), ckc_i8());
    }
    else
    {
 ...
 * (Python 3060-3061). */
static ckc_value_t* ckc_gfx1151_dfcp_conv1_code_f16(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                    ckc_value_t* p1)
{
    ckc_ir_builder_t* b = ctx->b;
...
static ckc_value_t* ckc_gfx1151_dfcp_conv1_code_vec_i8(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                                       ckc_value_t* acc)
{
    ckc_ir_builder_t* b = ctx->b;
                                            ...
                                            ckc_value_t* slot)
{
    ...
    default:
        ...
        return (ckc_value_t*)ckc_i_set_err(
            ctx->b, CKC_ERR_TYPE, "gfx1151 dfcp: vector code fn %d applied as scalar", code_fn);
    }
}
...
/* ===================================================================== *
 *  ckc_gfx1151_dfcp_build_ctx_init -- the Python build_deep_fused_conv_pool
 *  prologue (lines 2688-2778).
 * ===================================================================== */
ckc_status_t
ckc_gfx1151_dfcp_build_ctx_init(ckc_gfx1151_dfcp_build_ctx_t* ctx,
                                ckc_ir_builder_t* b,
                                ...
                                const char* arch)
{
    ...
    if (!ckc_gfx1151_deep_fused_conv_pool_is_valid_spec(spec, arch, reason, sizeof(reason)))
    {
        (void)ckc_i_set_err(
            b, CKC_ERR_VALUE, "invalid gfx1151 deep fused conv/pool spec: %s", reason);
        return CKC_ERR_VALUE;
    }
    ...
    target = ckc_archtarget_from_gfx(ctx->arch);
    if (target == NULL)
    {
        (void)ckc_i_set_err(b, CKC_ERR_KEY, "unknown gfx target '%s'", ctx->arch);
        return CKC_ERR_KEY;
    }
    ctx->op = ckc_archtarget_op_for_shape(target,
                                          "wmma",
                                          "f16",
                                          "f16",
                                          "fp32",
                                          CKC_GFX1151_DFCP_WMMA,
                                          CKC_GFX1151_DFCP_WMMA,
                                          CKC_GFX1151_DFCP_WMMA);
    ...
    if (spec->native_int)
    {
        ctx->op0 = ckc_archtarget_by_op_id(target, CKC_GFX1151_DFCP_OP_ID_IU8);
        ctx->op1 = ckc_archtarget_by_op_id(target, CKC_GFX1151_DFCP_OP_ID_IU4);
    }
    else
    {
        ctx->op0 = ctx->op;
    ...
    {
        ckc_warp_grid_t* grid = (ckc_warp_grid_t*)ckc_arena_alloc(&b->arena, sizeof(*grid));
        ckc_value_t* wave;
        ckc_value_t* c_warps_n;
        ckc_value_t* c_warps_n_warp_m;
        ckc_value_t* c_tile_m;
        ckc_value_t* c_tile_n;
        ckc_value_t* c_tile_k;
        ckc_value_t* tid_v;
        ckc_value_t* lane_v;
        ckc_value_t* warp_id_v;
        ckc_value_t* warp_m_idx_v;
        ckc_value_t* warp_n_idx_v;
        ckc_value_t* warp_k_idx_v;
        ckc_value_t* block_m_off_v;
        ckc_value_t* block_n_off_v;
        ckc_value_t* block_k_off_v;
        ...
        /* warp_k == 1 path */
        warp_m_idx_v = ckc_b_div(b, warp_id_v, c_warps_n);
        warp_n_idx_v = ckc_b_mod(b, warp_id_v, c_warps_n);
        warp_k_idx_v = ckc_b_const_i32(b, 0);

        /* block_m_axis="y", block_n_axis="x"; block_k_axis=None. */
        block_m_off_v = ckc_b_mul(b, ckc_b_block_id_y(b), c_tile_m);
    ...
    {
        ckc_attr_set_int(
            b, &b->kernel->attrs, "max_workgroup_size", ckc_gfx1151_dfcp_block_size(spec));
    ...
    {
        const ckc_type_t* a0_t = spec->native_int ? ckc_i8() : ckc_f16();
        const ckc_type_t* c0_t = a0_t;
        const ckc_type_t* w1_t = a0_t;
        const ckc_type_t* c1_t = a0_t;

        foot_h = ckc_gfx1151_dfcp_foot_h(spec);
        foot_w = ckc_gfx1151_dfcp_foot_w(spec);
    ...
    return ckc_ir_builder_ok(b) ? CKC_OK : ckc_ir_builder_status(b);
}
