...
ckc_value_t* ckc_b_load_scalar(ckc_ir_builder_t* b,
                               ckc_value_t* ptr,
                               ckc_value_t* idx,
                               const char* dtype)
{
    ...
    return (ckc_value_t*)ckc_i_set_err(
        b, CKC_ERR_VALUE, "unsupported I/O dtype %s%s%s", dtype ? "'" : "",
        dtype ? dtype : "None", dtype ? "'" : "");
}
...
ckc_value_t* ckc_b_load_vec(ckc_ir_builder_t* b,
                            ckc_value_t* ptr,
                            ckc_value_t* idx,
                            const char* dtype,
                            int n)
{
    ...
    if (!ckc_io_is_vec_width(n))
    {
        return (ckc_value_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE,
            "load_vec n must be 2/4/8 (got %d); use load_scalar for n=1", n);
    }
                              ...
                              ckc_value_t** out)
{
    ...
    {
        return ckc_b_load_vec_as_f32(
            b, ptr, ckc_b_add(b, row_base, lane_d_base), dtype, ept, out);
    }
...
void ckc_b_store_vec(ckc_ir_builder_t* b,
                     ckc_value_t* ptr,
                     ckc_value_t* idx,
                     ckc_value_t* value,
                     int n)
{
    ...
    if (!ckc_io_is_vec_width(n))
    {
        (void)ckc_i_set_err(
            b, CKC_ERR_VALUE,
            "store_vec n must be 2/4/8 (got %d); use store_scalar for n=1", n);
        return;
    }
                           ...
                           int vec_bytes)
{
    ...
    for (c = 0; c < n_chunks; ++c)
    {
        ...
        ckc_value_t* v =
            ckc_b_global_load_vN(b, src, src_addr, ty, vec, vec * elem_bytes);
