 ...
 * (ck_dsl/instances/gfx942/attention_tiled_2d.py lines 2905-3346).
 *
 * SCOPE.
 *   The fp8 K/V staging / dequant / native-fp8-MFMA loaders:
 *     _issue_kv_fp8_async_load   -> ckc_gfx942_attn2d_issue_kv_fp8_async_load
 *     _dequant_fp8_lds_to_bf16   -> ckc_gfx942_attn2d_dequant_fp8_lds_to_bf16
 *     _issue_fp8_dequant_loads   -> ckc_gfx942_attn2d_issue_fp8_dequant_loads
 *     _issue_k_fp8_mfma_async    -> ckc_gfx942_attn2d_issue_k_fp8_mfma_async
 *     _issue_v_fp8_mfma_async    -> ckc_gfx942_attn2d_issue_v_fp8_mfma_async
 *     _issue_v_fp8_mfma_stripe   -> ckc_gfx942_attn2d_issue_v_fp8_mfma_stripe
 *
 ...
 *
 * BINDING.
 *   Reads/writes only ckc_gfx942_attn2d_build_ctx_t fields + the builder it
...
static ckc_attn2d_fp8_dma_t
ckc_attn2d_fp8_dma_pick(const ckc_gfx942_attn2d_build_ctx_t* ctx)
{
...
static ckc_value_t* ckc_attn2d_zero_soff(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    return ckc_b_const_i32(ctx->b, 0);
}
 ...
 * ===================================================================== */
static ckc_value_t* ckc_attn2d_fp8_kv_voff(ckc_gfx942_attn2d_build_ctx_t* ctx,
                                           ...
                                           ckc_value_t** out_valid)
{
    ...
    if(ctx->kv_desc != NULL)
    {
        (void)ckc_transforms_descriptor_offset(
                                               ...
                                               ckc_value_t* buf_idx)
{
    ...
    {
        ckc_value_t* wave_fp8_offset_i32 = ckc_b_to_sgpr_u32(
            b, ckc_b_mul(b, ctx->wave_id, ckc_b_const_i32(b, FP8_WAVE_BYTES)));
    ...
    for(call = 0; call < dma.calls_per_tile; ++call)
    {
        ...
        ckc_value_t* lds_dst =
            ckc_b_smem_ptr_add(b, wave_base, ckc_b_const_i64(b, (int64_t)call * dma.bytes_per_call));
        ckc_b_async_buffer_load_lds_addr(b,
                                         ctx->k_rsrc,
                                         lds_dst,
                                         voff,
                                         zero_soff,
                                               ...
                                               ckc_value_t* buf_idx)
{
    ...
    for(c = 0; c < chunks_per_thread; ++c)
    {
        ckc_value_t* chunk_id =
            ckc_b_add(b, ckc_b_mul(b, ckc_b_const_i32(b, c), ckc_b_const_i32(b, ctx->THREADS)), ctx->tid);
        ...
        ckc_value_t* fp8_vec =
            ckc_b_smem_load_vN(b, src, idx_load, 3, ckc_fp8e4m3(), CHUNK);
                                              ...
                                              ckc_value_t* buf_idx)
{
    ...
    K_lds_addr = ckc_b_smem_addr_of(b, ctx->K_lds);
    ...
    for(call = 0; call < dma.calls_per_tile; ++call)
    {
        ...
        ckc_value_t* voff = ckc_attn2d_fp8_kv_voff(ctx, kv_tile_idx, linear_elem, NULL);
        ckc_value_t* k_dst =
            ckc_b_smem_ptr_add(b, K_wave_base, ckc_b_const_i64(b, (int64_t)call * dma.bytes_per_call));
        ckc_b_async_buffer_load_lds_addr(b,
                                         ctx->k_rsrc,
                                         k_dst,
                                         voff,
                                              ...
                                              ckc_value_t* kv_tile_idx)
{
    ...
    V_lds_addr = ckc_b_smem_addr_of(b, ctx->V_lds);
    ...
    for(call = 0; call < dma.calls_per_tile; ++call)
    {
        ...
        ckc_value_t* voff = ckc_attn2d_fp8_kv_voff(ctx, kv_tile_idx, linear_elem, NULL);
        ckc_value_t* v_dst =
            ckc_b_smem_ptr_add(b, V_wave_base, ckc_b_const_i64(b, (int64_t)call * dma.bytes_per_call));
        ckc_b_async_buffer_load_lds_addr(b,
                                         ctx->v_rsrc,
                                         v_dst,
                                         voff,
                                               ...
                                               ckc_value_t* kv_tile_idx)
{
    ...
    for(call = 0; call < chunks_per_thread; ++call)
    {
        ...
        ckc_value_t* fp8_vec = ckc_b_global_load_vN(b, ctx->value, voff, ckc_fp8e4m3(), CHUNK, CHUNK);
