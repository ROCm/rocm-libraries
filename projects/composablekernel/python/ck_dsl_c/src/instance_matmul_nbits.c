                                                 ...
                                                 const char* arch)
{
    ...
    ckc_value_t** a_row_off = ckc_arena_alloc(&b->arena, (size_t)n_sub_m * sizeof(*a_row_off));
    ckc_value_t** b_byte_off = ckc_arena_alloc(&b->arena, (size_t)n_sub_n * sizeof(*b_byte_off));
    ckc_value_t** b_scale_off = ckc_arena_alloc(&b->arena, (size_t)n_sub_n * sizeof(*b_scale_off));
    ...
    ckc_iter_arg_t* iter_args = ckc_arena_alloc(&b->arena, (size_t)n_acc * sizeof(*iter_args));
    if (iter_args == NULL)
    {
        return NULL;
    }
    {
        ...
        for (i = 0; i < n_acc; ++i)
        {
            char* nm = ckc_arena_alloc(&b->arena, 24);
            if (nm == NULL)
            {
                return NULL;
            }
    ...
    ckc_for_t loop =
        ckc_b_scf_for_iter(b, c0, cK, cwtk, iter_args, n_acc, "k0", false, false);
    ckc_b_region_enter(b, loop.body);
    {
        ...
        ckc_value_t** a_frag = ckc_arena_alloc(&b->arena, (size_t)n_sub_m * sizeof(*a_frag));
        if (a_frag == NULL)
        {
            return NULL;
        }
        ...
        ckc_value_t** b_frag = ckc_arena_alloc(&b->arena, (size_t)n_sub_n * sizeof(*b_frag));
        if (b_frag == NULL)
        {
            return NULL;
        }
        {
            ...
            for (sn = 0; sn < n_sub_n; ++sn)
            {
                ...
                ckc_value_t** comps =
                    ckc_arena_alloc(&b->arena, (size_t)(2 * n_bytes) * sizeof(*comps));
                if (comps == NULL)
                {
                    return NULL;
                }
        ...
        ckc_value_t** nacc = ckc_arena_alloc(&b->arena, (size_t)n_acc * sizeof(*nacc));
        if (nacc == NULL)
        {
            return NULL;
        }
