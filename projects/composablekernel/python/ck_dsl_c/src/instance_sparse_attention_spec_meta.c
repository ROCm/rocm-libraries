...
ckc_jenga_sparse_spec_t ckc_jenga_sparse_spec_default(ckc_fmha_common_spec_t common,
                                                      int seqlen_q,
                                                      int seqlen_k)
{
...
ckc_status_t ckc_jenga_sparse_kernel_name(const ckc_jenga_sparse_spec_t* spec,
                                          char* out,
                                          size_t out_cap)
{
    const char* name;
...
ckc_vsa_sparse_spec_t ckc_vsa_sparse_spec_default(ckc_fmha_common_spec_t common,
                                                  int seqlen_q,
                                                  int seqlen_k)
{
...
ckc_status_t ckc_vsa_sparse_kernel_name(const ckc_vsa_sparse_spec_t* spec,
                                        char* out,
                                        size_t out_cap)
{
    const char* name;
                                            ...
                                            size_t reason_cap)
{
    ...
    if (!ok)
    {
        sparse_set_reason(reason, reason_cap,
                             ...
                             size_t reason_cap)
{
    ...
    if (spec->block_k % CKC_MFMA_ATTN_BLOCK_K != 0)
    {
        snprintf(buf, sizeof(buf),
                 ...
                 spec->block_k, CKC_MFMA_ATTN_BLOCK_K);
        sparse_set_reason(reason, reason_cap, buf);
        return false;
    }
                           ...
                           size_t reason_cap)
{
    ...
    {
        snprintf(buf, sizeof(buf),
                 "max_blocks_per_q must be > 0 (got %d)", spec->max_blocks_per_q);
        sparse_set_reason(reason, reason_cap, buf);
        return false;
    }
    /* seqlen_k % block_k != 0 */
