 ...
 * ck_dsl/instances/common/sage_attention.py (lines 104-456).
 *
 ...
 *
 *   ckc_sage_magic_div              <- _magic_div                 (104-117)
 *   ckc_sage_is_lds_handle          <- _is_lds_handle             (364-367)
 *   ckc_sage_stage_codebook_to_lds  <- _stage_codebook_to_lds     (298-326)
 *   ckc_sage_codebook_lds_lookup_f32<- _codebook_lds_lookup_f32   (329-331)
 *   ckc_sage_codebook_i8_to_f32     <- _codebook_i8_to_f32        (334-346)
 *   ckc_sage_codebook_i4_pair_to_f32<- _codebook_i4_pair_to_f32   (349-361)
 *   ckc_sage_vectorised_byte_slice  <- _vectorised_byte_slice     (370-406)
 *   ckc_sage_load_kv_lane_f32       <- _load_kv_lane_f32          (409-441)
 *   ckc_sage_load_q_lane_f32        <- _load_q_lane_f32           (444-456)
 *
 ...
 *
 * In C the type kind discriminant carries this; SmemType <-> CKC_TYPE_SMEM.
 */
bool ckc_sage_is_lds_handle(const ckc_value_t* v)
{
    return v != NULL && v->type != NULL && v->type->kind == CKC_TYPE_SMEM;
}
 ...
 *         return cb_lds
 */
ckc_value_t* ckc_sage_stage_codebook_to_lds(ckc_ir_builder_t* b,
                                            ckc_value_t* cb_global,
                                            int n_entries,
                                            ckc_value_t* tid,
                                            const char* name_hint)
{
    ...
    for(base = 0; base < n_entries; base += CKC_FMHA_WARP_SIZE)
    {
        ...
        if(base + CKC_FMHA_WARP_SIZE <= n_entries)
        {
            ckc_value_t* val = ckc_b_global_load_f32(b, cb_global, slot, 0);
            ckc_value_t* idx[1];
            idx[0] = slot;
...
ckc_sage_codebook_i8_to_f32(ckc_ir_builder_t* b, ckc_value_t* cb_ptr, ckc_value_t* i8_value)
{
    ...
    if(ckc_sage_is_lds_handle(cb_ptr))
    {
        return ckc_sage_codebook_lds_lookup_f32(b, cb_ptr, idx);
    }
    return ckc_b_global_load_f32(b, cb_ptr, idx, 0);
}
 ...
 *         return lo, hi
 */
void ckc_sage_codebook_i4_pair_to_f32(ckc_ir_builder_t* b,
                                      ckc_value_t* cb_ptr,
                                      ckc_value_t* packed_byte_i8,
                                      ckc_value_t** out_lo,
                                      ckc_value_t** out_hi)
{
    ...
    ckc_value_t* c8;
    ckc_unpack_i4_byte_to_pair_i32(b, packed_byte_i8, &lo_i32, &hi_i32);
    c8 = ckc_b_const_i32(b, 8);
    if(ckc_sage_is_lds_handle(cb_ptr))
    {
        *out_lo = ckc_sage_codebook_lds_lookup_f32(b, cb_ptr, ckc_b_add(b, lo_i32, c8));
        *out_hi = ckc_sage_codebook_lds_lookup_f32(b, cb_ptr, ckc_b_add(b, hi_i32, c8));
        return;
    }
    *out_lo = ckc_b_global_load_f32(b, cb_ptr, ckc_b_add(b, lo_i32, c8), 0);
    *out_hi = ckc_b_global_load_f32(b, cb_ptr, ckc_b_add(b, hi_i32, c8), 0);
}
 ...
 *         ]
 */
void ckc_sage_vectorised_byte_slice(ckc_ir_builder_t* b,
                                    ckc_value_t* KV,
                                    ckc_value_t* base,
                                    ckc_value_t* lane_d_base,
                                    int ept,
                                    const ckc_type_t* elem_ty,
                                    ckc_value_t** out)
{
    ckc_value_t* addr_base = ckc_b_add(b, base, lane_d_base);
    bool is_f16_like;
    bool is_fp8_like;
    int k;
    ...
    if(ept == 1)
    {
        out[0] = ckc_b_global_load(b, KV, addr_base, elem_ty, 0);
        return;
    }
       ...
       (is_fp8_like && (ept == 2 || ept == 4 || ept == 8 || ept == 16)))
    {
        ...
        ckc_value_t* vec =
            ckc_b_global_load_vN(b, KV, addr_base, elem_ty, ept, ept * elem_bytes);
        for(k = 0; k < ept; ++k)
        {
            out[k] = ckc_b_vec_extract(b, vec, k);
        }
        return;
    }
 ...
 */
void ckc_sage_load_kv_lane_f32(ckc_ir_builder_t* b,
                               ckc_value_t* KV,
                               ckc_value_t* base,
                               ckc_value_t* lane_d_base,
                               int ept,
                               ...
                               ckc_value_t* cb_ptr,
                               const ckc_type_t* kv_ty,
                               const char* dtype,
                               ckc_value_t** out)
{
    ckc_value_t* elems[CKC_SAGE_MAX_EPT];
    int k;
    (void)dtype;

    if(quant_mode == CKC_SAGE_QUANT_FP16_BF16 || quant_mode == CKC_SAGE_QUANT_FP8_BF16)
    {
        ckc_sage_vectorised_byte_slice(b, KV, base, lane_d_base, ept, kv_ty, elems);
        if(quant_mode == CKC_SAGE_QUANT_FP16_BF16)
        {
            for(k = 0; k < ept; ++k)
            {
                out[k] = ckc_b_cast_to_f32(b, elems[k]);
            }
        }
        else
        {
            for(k = 0; k < ept; ++k)
            {
                out[k] = ckc_b_cvt_fp8_to_f32(b, elems[k]);
            }
        }
        return;
    }
    if(quant_mode == CKC_SAGE_QUANT_I8_FP8_BF16)
    {
        ckc_sage_vectorised_byte_slice(b, KV, base, lane_d_base, ept, ckc_i8(), elems);
        for(k = 0; k < ept; ++k)
        {
            out[k] = ckc_sage_codebook_i8_to_f32(b, cb_ptr, elems[k]);
        }
        return;
    }
    ...
    snprintf(b->err,
             ...
             ckc_sage_quant_mode_name(quant_mode));
}
 ...
 *         ]
 */
void ckc_sage_load_q_lane_f32(ckc_ir_builder_t* b,
                              ckc_value_t* Q,
                              ...
                              ckc_value_t** out)
{
    ...
    {
        ckc_value_t* vec = ckc_b_global_load_vN(b, Q, addr_base, kv_ty, ept, ept * elem_bytes);
        for(k = 0; k < ept; ++k)
        {
            out[k] = ckc_b_cast_to_f32(b, ckc_b_vec_extract(b, vec, k));
        }
        return;
    }
    for(k = 0; k < ept; ++k)
