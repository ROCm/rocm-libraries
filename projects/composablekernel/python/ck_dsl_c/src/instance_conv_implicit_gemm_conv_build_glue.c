 ...
 *     path regardless.
 *   - spec.effective_lds_layout() has no public accessor yet; the documented
 *     derivation (sync +8 when block_k>=16, async/packed +0) is reproduced
 *     inline into ctx->lds_layout (TODO(port) for the LdsLayout peer).
                             ...
                             const ckc_conv_build_overrides_t* overrides)
{
    ...
    /* ---- LDS plan ---- (894-913). effective_lds_layout() derivation:
     * sync path pads each K-row by +8 halves (when block_k >= 16) to dodge LDS
     * bank conflicts; the async / packed path uses +0 (lane-contiguous LDS).
     * TODO(port): no spec.effective_lds_layout()/LdsLayout accessor yet -- the
     * documented derivation is reproduced inline; validate_for_async() (the
     * async-only assert) is a no-op stub until the LdsLayout peer lands. */
    ctx->lds_layout.logical_cols = ctx->block_k;
    if (spec->has_lds_k_pad)
    {
        ctx->lds_layout.k_pad = spec->lds_k_pad;
    }
    ...
    {
        ctx->lds_layout.k_pad = (ctx->block_k >= 16) ? 8 : 0;
    }
    ctx->lds_layout.row_stride = ctx->lds_layout.logical_cols + ctx->lds_layout.k_pad;
    ctx->lds_layout.swizzle    = NULL;
    /* if spec.async_dma: lds_layout.validate_for_async()  -- TODO(port) */
    ...
    else
    {
        ckc_status_t sa = ckc_coalesced_tile_loader_from_tile(
            ctx->block_m, ctx->block_k, ctx->threads, ctx->load_vec, true, &ctx->a_sync_loader);
        ckc_status_t sb = ckc_coalesced_tile_loader_from_tile(
            ctx->block_n, ctx->block_k, ctx->threads, ctx->load_vec, true, &ctx->b_sync_loader);
        if (sa != CKC_OK || sb != CKC_OK)
        {
