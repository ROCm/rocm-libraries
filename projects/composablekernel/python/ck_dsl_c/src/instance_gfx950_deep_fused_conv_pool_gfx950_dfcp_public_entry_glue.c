 ...
 *   (ck_dsl/instances/gfx950/deep_fused_conv_pool.py -- the 67-LOC thin shim that
 ...
 *       ckc_gfx950_deep_fused_conv_pool_make_spec / _spec_default
 *       ckc_gfx950_deep_fused_conv_pool_is_valid_spec / _signature / _grid /
 *       _kernel_name.
 *   - ckc_gfx950_dfcp_build_ctx_init  (the Python prologue: arch normalise,
 ...
 *   - ckc_build_gfx950_deep_fused_conv_pool / _new   (the gfx950 public build
 ...
 *     driver).
 *   - ckc_gfx950_deep_fused_conv_pool_lower_to_llvm   (build -> lower .ll).
 *
 ...
 * ckc_conv_acc_epilogue_t typedef collides with the deep-fused helper's same-named
 ...
 * ckc/instance_conv_implicit_gemm.h). The gfx950 spec embeds the common spec as
...
extern const ckc_implicit_gemm_conv_spec_t* ckc_deep_fused_conv_pool_spec_conv_spec(
 ...
 * ValueError path. */
extern const ckc_mma_op_t* ckc_conv_resolve_op(
    ckc_ir_builder_t* b, const ckc_implicit_gemm_conv_spec_t* conv_spec,
    const char* arch);
...
typedef struct ckc_gfx950_dfcp_conv_overrides
{
    ...
    void (*epilogue_override)(ckc_ir_builder_t* b,
                              ...
                              void* user);
    void* user;
} ckc_gfx950_dfcp_conv_overrides_t;
...
/* ckc_gfx950_dfcp_build_ctx_init is defined in the sibling part-file
 ...
 * in ckc/instance_gfx950_deep_fused_conv_pool_internal.h. */

/* ===================================================================== *
 * Conv-builder callback trampolines.
 *
 * The conv builder calls the override hooks with its own ABI
 * (ckc_gfx950_dfcp_conv_overrides_t) + an opaque `user` cookie. We thread the
 * shared ckc_gfx950_dfcp_build_ctx_t through `user` and forward to the gfx950 ctx
...
static void* ckc_gfx950_dfcp_tramp_extra_params(ckc_ir_builder_t* b, void* user)
{
    ckc_gfx950_dfcp_build_ctx_t* ctx = (ckc_gfx950_dfcp_build_ctx_t*)user;
    (void)b;
    return (void*)ckc_gfx950_dfcp_extra_params(ctx);
}

static ckc_value_t* ckc_gfx950_dfcp_tramp_m_index_fn(ckc_ir_builder_t* b,
                                                     ckc_value_t* row,
                                                     const ckc_warp_grid_t* grid,
                                                     void* user)
{
    ckc_gfx950_dfcp_build_ctx_t* ctx = (ckc_gfx950_dfcp_build_ctx_t*)user;
    (void)b;
    return ckc_gfx950_dfcp_m_index_fn(ctx, row, grid);
}

static void ckc_gfx950_dfcp_tramp_a_mhw_index_fn(ckc_ir_builder_t* b,
                                                 ckc_value_t* row,
                                                 const ckc_warp_grid_t* grid,
                                                 ckc_value_t** out_n,
                                                 ckc_value_t** out_ho,
                                                 ...
                                                 void* user)
{
    ckc_gfx950_dfcp_build_ctx_t* ctx = (ckc_gfx950_dfcp_build_ctx_t*)user;
    (void)b;
    ckc_gfx950_dfcp_a_mhw_index_fn(ctx, row, grid, out_n, out_ho, out_wo);
}

static void* ckc_gfx950_dfcp_tramp_input_cache_setup(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    const ckc_warp_grid_t* grid,
    ckc_value_t* a_rsrc,
    void* user)
{
    ...
    {
        return (void*)ckc_gfx950_dfcp_setup_input_cache(ctx, spec, grid, a_rsrc);
    }
    return (void*)ckc_gfx950_dfcp_setup_specialized_a_loader(ctx, spec, grid,
                                                             a_rsrc);
}

static void ckc_gfx950_dfcp_tramp_a_load_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* k_off,
    ckc_value_t* a_dst,
    ...
    void* user)
{
    ...
    if (ctx->use_input_cache)
    {
        ckc_gfx950_dfcp_load_a_tile_from_cache(ctx, spec, k_off, a_dst, grid,
                                               cache);
    }
    else
    {
        ckc_gfx950_dfcp_load_a_tile_specialized(ctx, spec, k_off, a_dst, grid,
                                                cache);
    }
}

static ckc_value_t* ckc_gfx950_dfcp_tramp_a_operand_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* a_row,
    ckc_value_t* k_off,
    ...
    void* user)
{
    ...
    ckc_value_t* cache = (ckc_value_t*)input_cache_context;
    (void)b;
    return ckc_gfx950_dfcp_load_a_operand_from_cache(ctx, spec, a_row, k_off,
                                                     col_base, a_per_lane, grid,
                                                     cache);
}

static void ckc_gfx950_dfcp_tramp_epilogue_override(
    ckc_ir_builder_t* b,
    const ckc_implicit_gemm_conv_spec_t* spec,
    ckc_value_t* const* accs,
    int num_accs,
    ...
    void* user)
{
    ...
    (void)b;
    ckc_gfx950_dfcp_epilogue_override(ctx, spec, accs,
                                      (num_accs < 0) ? 0u : (size_t)num_accs,
                                      grid, d_rsrc, w1_rsrc);
}
                                      ...
                                      const char* arch)
{
    ...
    ckc_gfx950_dfcp_conv_overrides_t ov;
    ckc_status_t st;
    ...
    reason[0] = '\0';
    if (!ckc_deep_fused_conv_pool_is_valid_spec(cs, arch, reason,
                                                sizeof(reason)))
    {
        ckc_i_set_err(b, CKC_ERR_VALUE,
                      "invalid deep fused conv/pool spec for %s: %s", arch,
                      reason);
        return NULL;
    }
    ...
    st = ckc_gfx950_dfcp_build_ctx_init(&ctx, b, spec, arch, conv_spec, op);
    if (st != CKC_OK)
    {
    ...
    ov.user = &ctx;
    ov.extra_params = ckc_gfx950_dfcp_tramp_extra_params;
    ov.m_index_fn = ckc_gfx950_dfcp_tramp_m_index_fn;
    ov.a_mhw_index_fn = ckc_gfx950_dfcp_tramp_a_mhw_index_fn;

    if (ctx.use_input_cache || ctx.use_specialized)
    {
        ov.input_cache_setup = ckc_gfx950_dfcp_tramp_input_cache_setup;
        ov.a_load_override = ckc_gfx950_dfcp_tramp_a_load_override;
    }
    if (ctx.use_operand_ovr)
    {
        ov.a_operand_override = ckc_gfx950_dfcp_tramp_a_operand_override;
    }
    ov.epilogue_override = ckc_gfx950_dfcp_tramp_epilogue_override;
    ...
    return ckc_build_implicit_gemm_conv(
        b, conv_spec, arch, (const ckc_conv_build_overrides_t*)&ov);
}
