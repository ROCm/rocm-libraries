...
static ckc_buffer_resource_t* ckc_make_buffer_resource(ckc_ir_builder_t* b,
                                                       ckc_value_t* ptr,
...
ckc_status_t ckc_pooling_problem_short(const ckc_pooling_problem_t* p,
                                       char* out,
                                       size_t out_cap)
{
    int n;
    ...
    n = snprintf(out, out_cap, "N%dH%dW%dC%d_Y%dX%d_s%dx%d_p%dx%d", p->N, p->H, p->W,
                 p->C, p->Y, p->X, p->sH, p->sW, p->pH, p->pW);
...
ckc_status_t ckc_pooling2d_kernel_name(const ckc_pooling2d_spec_t* spec,
                                       char* out,
                                       size_t out_cap)
{
    char shortbuf[128];
...
#define POOL_REASON(...)                                                          \
    ...
    ckc_io_spec_rule_init(&rule, spec->dtype, spec->block_size,
                          spec->vec >= 2 ? spec->vec : 2);
    ...
    if (strcmp(spec->op, "max") != 0 && strcmp(spec->op, "avg") != 0 &&
        strcmp(spec->op, "sum") != 0)
    {
...
static ckc_tensor_descriptor_t* pool_make_input_descriptor(
    ckc_ir_builder_t* b, const ckc_pooling_problem_t* p)
{
    int lengths[4];
    ...
    base = ckc_tensor_descriptor_naive(b, "X_nhwc", lengths, 4, /*strides*/ NULL,
                                       coord_names, 4);
    if (base == NULL)
    {
        return NULL;
...
static ckc_value_t* pool_combine(ckc_ir_builder_t* b,
                                 const char* op,
                                 ckc_value_t* acc,
                                 ckc_value_t* x)
{
...
static int ckc_pool_dtype_elem_bytes(const ckc_type_t* dtype)
{
    ...
    if (n == NULL) return 0;
    if (strcmp(n, "f16") == 0 || strcmp(n, "bf16") == 0) return 2;
    if (strcmp(n, "f32") == 0 || strcmp(n, "i32") == 0) return 4;
    if (strcmp(n, "i64") == 0) return 8;
    return 0;
}
                                                       ...
                                                       ckc_value_t* num_bytes)
{
    ...
    r = (ckc_buffer_resource_t*)ckc_arena_calloc(&b->arena,
                                                 sizeof(ckc_buffer_resource_t));
...
{
    ckc_buffer_view_t* v;
    if (b == NULL || rsrc == NULL) return NULL;
                                  ...
                                  int n_origin)
{
    ...
    if (n_lengths != view->rank || n_origin != view->rank ||
        n_lengths > CKC_POOL_TV_MAX_RANK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "TensorView.tile: rank mismatch");
        return NULL;
    }
    w = (ckc_buffer_window_t*)ckc_arena_calloc(&b->arena,
                                               sizeof(ckc_buffer_window_t));
                                           ...
                                           int num_indices)
{
    ...
    if (b == NULL || v == NULL || num_indices != v->rank) return NULL;
    for (i = 0; i < v->rank; ++i)
    {
        ckc_value_t* term;
        if (v->strides[i] == 1)
...
static int ckc_pool_y_to_linear(const ckc_tile_distribution_encoding_t* e,
                                const int* y, int n_y)
{
    int off = 0;
    int i;
                                              ...
                                              ckc_value_t* value)
{
    int off;
    (void)b;
    if (dt == NULL || dt->distribution == NULL ||
        dt->distribution->encoding == NULL)
        return;
                           ...
                           int n_ps)
{
    ...
    if (b == NULL || w == NULL || w->view == NULL || dt == NULL ||
    ...
    if (tyname == NULL ||
        (strcmp(tyname, "f16") != 0 && strcmp(tyname, "bf16") != 0))
    {
    ...
    if (!ckc_tile_distribution_calculate_x(b, dist, ys, enc->num_Y,
                                           /*ps*/ NULL, /*ps_counts*/ NULL,
                                           /*num_ps*/ 0, x_coords,
    ...
    for (i = 0; i < spv; ++i)
    {
        ...
        scalars[i] = (off >= 0 && off < dt->num_storage) ? dt->storage[off]
                                                         : NULL;
    }
    ...
    if (spv == 1)
    {
        ...
        {
            ckc_b_buffer_store_f16(b, w->view->rsrc->rsrc, byte_off,
                                   w->view->rsrc->soffset, casts[0]);
        }
        else
        {
            ckc_i_set_err(b, CKC_ERR_NOTIMPL,
                          "buffer scalar store not yet wired for dtype %s",
                          tyname);
        }
        return;
    }
     ...
     * _global_indices / desc.offset / byte_off / raw_ptr_buffer_store_vN. */
    {
        ...
        if (strcmp(tyname, "f16") == 0)
        {
            ckc_b_buffer_store_vN_f16(b, w->view->rsrc->rsrc, byte_off,
                                      w->view->rsrc->soffset, packed, spv / 2);
        }
        else
        {
            ckc_i_set_err(b, CKC_ERR_NOTIMPL,
                          "buffer vec store not yet wired for dtype %s",
                          tyname);
        }
    }
}
...
ckc_kernel_def_t* ckc_build_pooling2d(ckc_ir_builder_t* b,
    ...
    for (y_i = 0; y_i < p->Y; ++y_i)
    {
        ...
        for (x_i = 0; x_i < p->X; ++x_i)
        {
            ...
            if (strcmp(spec->op, "avg") == 0)
            {
                ...
                if (valid != NULL)
                {
                    contrib = ckc_b_select(b, valid, ckc_b_const_f32(b, 1.0),
