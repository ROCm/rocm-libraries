 ...
 *  reject, "ok" on accept.
 * ===================================================================== */
bool ckc_fused_moe_is_valid_spec(const ckc_fused_moe_spec_t* spec,
                                 char* reason,
                                 size_t reason_cap)
{
    const char* dtype;
    ...
    if (spec->hidden % spec->vec != 0)
    {
        if (reason != NULL && reason_cap > 0)
        {
            snprintf(reason,
                     reason_cap,
                     "hidden %d not divisible by vec %d",
                     spec->hidden,
                     spec->vec);
        }
        return false;
...
const ckc_tile_distribution_t* ckc_moe_chunk_distribution(ckc_ir_builder_t* b,
                                                          int block_size,
                                                          int vec)
{
    int h_levels[2];
