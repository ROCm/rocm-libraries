                                       ...
                                       ckc_value_t* seg)
{
    ...
    in_values[1]       = head;
    in_values[2]       = seg;
    if(!ckc_transforms_descriptor_offset(ctx->b, ctx->ml_desc_red, in_names, in_values, 3, &off,
                                         &valid))
    {
        return NULL;
    }
                                            ...
                                            ckc_value_t* dim)
{
    ...
    in_values[2]       = seg;
    in_values[3]       = dim;
    if(!ckc_transforms_descriptor_offset(ctx->b, ctx->seg_acc_desc_red, in_names, in_values, 4, &off,
                                         &valid))
    {
        return NULL;
    }
                                        ...
                                        ckc_value_t* dim)
{
    ...
    in_values[1]       = head;
    in_values[2]       = dim;
    if(!ckc_transforms_descriptor_offset(ctx->b, ctx->out_desc_red, in_names, in_values, 3, &off,
                                         &valid))
    {
        return NULL;
    }
 ...
 * ============================================================ */
void ckc_gfx942_attention_tiled_3d_reduce_max_pass(
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    ckc_ir_builder_t* b = ctx->b;
    const ckc_gfx942_attn_tiled_3d_config_t* cfg = &ctx->cfg;
    ...
    ckc_gfx942_attention_tiled_3d_build_ctx_t* ctx)
{
    ...
    for(li = 0; li < HALFS_PER_THREAD; ++li)
    {
        ...
        d = ckc_b_add(b, ckc_b_mul(b, ckc_b_const_i32(b, li), ckc_b_const_i32(b, THREADS)),
                      ctx->tid);
        ...
        acc_loop = ckc_b_scf_for_iter(b, ckc_b_const_i32(b, 0), ckc_b_const_i32(b, NUM_SEG),
                                      ckc_b_const_i32(b, 1), iter_args, 1, ivname, false, false);
        ...
        scalar_out_f32 =
            ckc_b_fmul(b, (acc_loop.op != NULL && acc_loop.op->num_results >= 1)
                              ? acc_loop.op->results[0]
                              : NULL,
                       ctx->inv_l);
