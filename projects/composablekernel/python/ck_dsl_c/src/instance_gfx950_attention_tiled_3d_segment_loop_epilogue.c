 ...
 * ============================================================ */
void ckc_gfx950_attention_tiled_3d_emit_loop_init(
    ...
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx)
{
    ...
    ckc_for_t kvloop = ckc_b_scf_for_iter(
        b, ctx->tile_start, ctx->tile_end, ckc_b_const_i32(b, 1), iter_args, num_iter, "kv_tile",
        false, false);
    ...
    ckc_b_region_enter(b, kvloop.body);
    {
        ...
        if (acc_vals == NULL || new_acc == NULL || A_kits == NULL || S_n == NULL ||
            masked == NULL)
        {
        ...
        for (n = 0; n < cfg->PV_N_TILES; n++)
        {
            ...
            n_col_base = ckc_b_add(
                b, ckc_b_mul(b, ckc_b_const_i32(b, n), ckc_b_const_i32(b, 16)), ctx->tr_col_lane);
 ...
 * ============================================================ */
void ckc_gfx950_attention_tiled_3d_emit_epilogue(
    ckc_gfx950_attention_tiled_3d_build_ctx_t* ctx)
{
    ckc_ir_builder_t* b = ctx->b;
    const ckc_gfx950_attn_tiled_3d_config_t* cfg = &ctx->cfg;
    ...
    for (n = 0; n < cfg->PV_N_TILES; n++)
    {
        for (reg = 0; reg < 4; reg++)
        {
            ...
            qtoken = ckc_b_add(b, ctx->cu_q_start, qp_r);
            {
                ...
                names[3] = "dim";
                vals[3] = col;
                ckc_transforms_descriptor_offset(b, ctx->seg_acc_desc, names, vals, 4,
                                                 &seg_acc_idx, NULL);
            }
