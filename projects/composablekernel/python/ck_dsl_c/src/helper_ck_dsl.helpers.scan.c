...
void ckc_lds_zero_i32(ckc_ir_builder_t* b,
                      ckc_value_t* lds_buf,
                      ckc_value_t* tid,
                      int block_size,
                      int length)
...
void ckc_block_exclusive_scan_i32(ckc_ir_builder_t* b,
                                  ckc_value_t* lds_buf,
                                  ckc_value_t* tid,
                                  int block_size,
                                  int length)
{
    ...
    left_idx =
        ckc_b_select(b, in_range_left, ckc_b_sub(b, tid, ckc_b_const_i32(b, 1)), ckc_b_const_i32(b, 0));
    /* left_vec = b.smem_load_vN(lds_buf, left_idx, dtype=I32, n=1) */
    {
        ckc_value_t* lidx2[1];
        lidx2[0] = left_idx;
