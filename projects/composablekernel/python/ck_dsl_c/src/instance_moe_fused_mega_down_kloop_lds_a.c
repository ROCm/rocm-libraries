...
#define CKC_MOE_MEGA_DK_MAX_ITER_ARGS \
    (CKC_MOE_MEGA_DK_MAX_MFMAS + CKC_MOE_MEGA_DK_MAX_VECS)
                                             ...
                                             ckc_value_t** out_accs)
{
    ...
    for (int kk = 0; kk < plan->k_atoms; ++kk)
    {
        ...
        ckc_value_t* b_col_mul =
            ckc_b_mul(b, k_blk, ckc_b_const_i32(b, plan->b_per_lane));
        ckc_value_t* b_col_base =
            ckc_b_add(b, b_col_mul, ckc_b_const_i32(b, kk * t->warp_tile_k));
        ...
        ckc_value_t* a_col_mul =
            ckc_b_mul(b, k_blk, ckc_b_const_i32(b, plan->a_per_lane));
        ckc_value_t* a_col_inner =
            ckc_b_add(b, a_col_mul, ckc_b_const_i32(b, kk * t->warp_tile_k));
        ...
        for (int mi = 0; mi < plan->mfmas_m; ++mi)
        {
            ckc_value_t* a_row = ckc_b_add(
                b, warp_m_off,
                ckc_b_add(b, ckc_b_const_i32(b, mi * t->warp_tile_m), m_in_atom));
            a_rows[mi] = ckc_gemm_emit_smem_load(b, a_smem, a_row, a_col, plan->a_per_lane,
                                                 plan->storage_dtype);
        }
        ...
        for (int ni = 0; ni < plan->mfmas_n; ++ni)
        {
            ...
            b_cols[ni] = ckc_gemm_emit_smem_load(b, operand->smem, b_row, b_col_base,
                                                 plan->b_per_lane, plan->storage_dtype);
        }
        int flat = 0;
        for (int mi = 0; mi < plan->mfmas_m; ++mi)
        {
                                       ...
                                       ckc_value_t** out_accs)
{
    ...
    char names[CKC_MOE_MEGA_DK_MAX_ITER_ARGS][24];
    int n_ia = 0;
    ...
    ckc_for_t for_op =
        ckc_b_scf_for_iter(b, c0, K, c_block_k, iter_args, n_ia, "dk0", false, false);
    ckc_b_region_enter(b, for_op.body);
    {
        ...
        ckc_moe_mega_emit_down_mfma_phase_lds_a(plan, a_smem_persistent, k0, operand, cur_accs,
                                                n_acc, warp_m_idx, warp_n_idx, lane, sched_groups,
                                                new_accs);
        ckc_b_sync(b);
