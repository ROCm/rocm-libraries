...
const ckc_type_t* ckc_fuse_b_dtype_to_ir_str(ckc_ir_builder_t* b, const char* dtype)
{
    ...
    if (ty == NULL)
    {
        ...
        return (const ckc_type_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "unsupported epilogue dtype %s%s%s",
...
ckc_value_t* ckc_fuse_ir_dtype_zero(ckc_ir_builder_t* b, const ckc_type_t* dtype)
{
    ...
    return (ckc_value_t*)ckc_i_set_err(
        b, CKC_ERR_NOTIMPL, "ir_dtype_zero: unsupported %s",
...
ckc_value_t* ckc_fuse_ir_dtype_const(ckc_ir_builder_t* b, const ckc_type_t* dtype, double value)
{
    ...
    return (ckc_value_t*)ckc_i_set_err(
        b, CKC_ERR_NOTIMPL, "ir_dtype_const: unsupported %s",
        (dtype && dtype->name) ? dtype->name : "None");
}
                                           ...
                                           ckc_value_t* idx)
{
    ...
    return (ckc_value_t*)ckc_i_set_err(
        b, CKC_ERR_NOTIMPL, "ir_dtype_global_load: unsupported %s",
        (dtype && dtype->name) ? dtype->name : "None");
}
    ...
    if (!vectorize)
    {
        ...
        for (i = 0; i < n_elems; ++i)
        {
            ...
            for (op_idx = 0; op_idx < mde->base.num_ops; ++op_idx)
            {
                ...
                ckc_value_t* ptr = ckc_fe_params_get(&mde->base.params, mde->base.ops[op_idx].param_name);
