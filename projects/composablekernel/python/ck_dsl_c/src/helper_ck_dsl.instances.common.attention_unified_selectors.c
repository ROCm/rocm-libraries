...
int ckc_unified_attn_num_queries_per_kv(ckc_ir_builder_t* b,
                                        const ckc_unified_attn_problem_t* p)
{
    if(p->num_kv_heads == 0 || (p->num_query_heads % p->num_kv_heads) != 0)
    {
        if(b != NULL && b->status == CKC_OK)
        {
            ...
            (void)snprintf(b->err, (size_t)CKC_ERR_MSG_CAP,
...
static bool enable_transposed_qk_32x32(const ckc_unified_attn_problem_t* p)
{
    ...
    bool single_seq_hd64 = (p->head_size == 64) && (p->block_size == 16) &&
                           (p->num_seqs <= 1) && (nqpk(p) >= 4) &&
                           (p->max_seqlen_q > 768);
...
{
    return arch_is("gfx942") &&
           (strcmp(p->dtype, "fp16") == 0 || strcmp(p->dtype, "bf16") == 0) &&
           !p->use_fp8 && (p->head_size == 64 || p->head_size == 128) &&
           (p->max_seqlen_q > 1 && p->max_seqlen_q <= 768) &&
           p->sliding_window == 0 && !p->use_sinks && p->softcap == 0 &&
...
static bool enable_gfx942_fp16_flash(const ckc_unified_attn_problem_t* p)
{
    return arch_is("gfx942") && (p->head_size == 64 || p->head_size == 128) &&
           strcmp(p->dtype, "fp16") == 0 && !p->use_fp8 &&
           p->sliding_window == 0 && !p->use_sinks && p->softcap == 0 &&
           !p->use_alibi && !p->use_qq_bias && !enable_gfx942_small_q_narrow(p);
}
...
static int select_2d_tile_size(const ckc_unified_attn_problem_t* p)
{
    ...
    if(p->head_size == 64 && p->block_size == 16 && p->num_seqs <= 1 &&
       !p->use_fp8 && strcmp(p->dtype, "bf16") == 0 && nqpk(p) >= 4)
    {
...

    {
        ...
        while(target > 1)
        {
            ...
            lds_bytes = BLOCK_M * HD * WORK_BYTES + 2 * T * HD * WORK_BYTES +
                        2 * T * HD * WORK_BYTES + BLOCK_M * T * WORK_BYTES +
                        BLOCK_M * HD * 4;
...
int ckc_unified_attn_select_2d_block_m_per_warp(const ckc_unified_attn_problem_t* p)
{
    ...
    if(p->head_size == 64 && p->block_size == 16 && p->num_seqs <= 1 &&
       !p->use_fp8 && strcmp(p->dtype, "bf16") == 0 && nqpk(p) >= 4 &&
       p->max_seqlen_q > 768 && p->sliding_window == 0 && p->softcap == 0 &&
       !p->use_sinks && !p->use_alibi && !p->use_qq_bias)
    {
        return 32;
    }
    return 16;
...
ckc_value_t* ckc_unified_attn_magic_div(ckc_ir_builder_t* b,
...
ckc_value_t*
ckc_unified_attn_paged_kv_offset(ckc_ir_builder_t* b,
                                            ...
                                            ckc_value_t* rcp_ln2)
{
    ...
    if(!ckc_unified_attn_physical_block_and_token(b, p, block_tables, seq_idx, kpos,
    ...
    /* q_off_base, _ = q_desc.offset(b, token=q_tok, head=q_head, dim=const_i32(0)) */
    {
        ...
        if(!ckc_transforms_descriptor_offset(b, q_desc, in_names, in_values, 3,
                                             &q_off_base, &valid))
        {
            return NULL;
        }
    }
    k_off_base = ckc_unified_attn_paged_kv_offset(b, &kv_desc, physical, token_in_block,
                                                 kv_head, ckc_b_const_i32(b, 0));
    ...
    for(d8 = 0; d8 < n_vec; ++d8)
    {
        ...
        for(i = 0; i < VEC; ++i)
        {
            score = ckc_b_fadd(
                b, score,
                ckc_b_fmul(b, ckc_b_cast_to_f32(b, ckc_b_vec_extract(b, qv, i)),
    ...
    for(d = n_vec * VEC; d < p->head_size; ++d)
    {
        ...
        k_off = ckc_unified_attn_paged_kv_offset(b, &kv_desc, physical, token_in_block,
                                                kv_head, d_v);
                                          ...
                                          ckc_value_t* dim)
{
    ...
    kv_desc = ckc_unified_attn_paged_kv_descriptor(p);
    v_off = ckc_unified_attn_paged_kv_offset(b, &kv_desc, physical, token_in_block,
                                            kv_head, dim);
