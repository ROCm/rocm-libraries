...
The `ck_dsl/examples/` tree (~40k LOC) is where CK DSL proves its kernels
...
... -m ck_dsl.examples.run_all --check --only gemm,attention
```
...
A nonzero example exit code is a real **FAIL**, except `rc=2` (the ck_dsl
...
2. Inputs are seeded -- ck_dsl examples use `np.random.default_rng(0xC0FFEE)`
...
| `numeric_differential_lane` | differential | `ck_dsl_c.tests.differential.numeric` | `--arch gfx950` | the full L6 numeric lane (GEMM/elementwise/norm/reduce/attention) as one repeatable example; per-config GREEN/XFAIL table digested |
...
Example(
    ...
    module="ck_dsl.examples.gfx950.my_driver",
