...
"""Empirically verify the RDNA4 (gfx1201) WMMA fragment lane map on hardware.

The RDNA4 WMMA 16x16x16 ABI differs from RDNA3/3.5: A/B operands are
``<8 x half>`` per lane (no cross-half duplication) and the accumulator is
column-distributed. The lane maps encoded in
``core/arch/target.py::_wmma_gfx12_*`` and the epilogue in
``instances/gfx1201/wmma_gemm.py`` are a *hypothesis* until proven on silicon.

This probe builds that WMMA GEMM and runs ``ck_dsl.run_manifest --verify``
...
at multiple tile counts uniquely confirms the mapping (an identity×constant
input cannot distinguish row from col — random input can).

Must run on a gfx1201 device (alola Navi 48 / MARKHAM node):
