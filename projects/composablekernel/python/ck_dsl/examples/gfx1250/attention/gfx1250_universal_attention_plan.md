...
This page captures the implementation plan for porting CK_DSL universal
...
The gfx1250-class features below are **not implemented in CK_DSL
...
- **Compile path** — Phase 1 uses the **HIP path**
  ...
  CK_DSL core does not yet emit. Hot kernels migrate to the **LLVM-direct path**
...
- **A5.** Fix stale `known_arches()` expectation in `test_ck_dsl_multiarch.py`
...
A cross-arch comparison of the **same ck_dsl decode path** on the qwen3-30B-A3B
