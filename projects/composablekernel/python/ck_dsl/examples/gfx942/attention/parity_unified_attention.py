...
The harness:
  ...
  1. Loads the canonical shapes we validate from the shipped ``shapes.json``
     (``--scenario`` selects subsets); these mirror the ck-dsl-provider
...
def _gfx942_spec_class():
    ...
    ``ck_dsl.instances.UnifiedAttention2DTiledSpec`` re-exports the gfx950 spec
...
def _run_ck_dsl(s: Shape, data, launcher, spec, *, warmup: int, attempts: int):
