...
six scenarios in `examples/moe/test_fused_moe_preshuffle.py`).

The lever is now an env-toggleable knob in
`examples/moe/fused_moe_e2e_perf.py`:
...
Standalone BatchedGemm validation (`examples/moe/test_active_tile_skip.py`):
...
on every test scenario in `examples/moe/test_fused_moe_preshuffle.py`.

The lever is exposed via:

- Spec: `FusedMoeForwardSpec.active_tile_skip_gemms=True`.
- Env (in `examples/moe/fused_moe_e2e_perf.py`):
