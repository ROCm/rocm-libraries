...
Drives :class:`ck_dsl.instances.FusedMoeForward` and three reference
...
Backends
--------
1. **CK DSL** -- :class:`ck_dsl.instances.FusedMoeForward`. Composes
   ...
   :func:`~ck_dsl.runtime.launcher.launch_kernel` calls.
...
ROOT = Path(__file__).resolve().parents[4]  # .../composablekernel
    ...
    *,
    ...
    if getattr(fwd, "_use_static_offsets", False):
        try:
            ...
            print(
                f"  ck_dsl: graph capture failed ({type(exc).__name__}: {exc!r}); "
...
def _isolate_lane() -> None:
    ...
    torch.cuda.synchronize()
    try:
        from ck_dsl.runtime.launcher import synchronize_and_release
    ...
    *,
    ...
    if not skip_ck_dsl:
        try:
            ...
            res.results["ck_dsl"] = BackendResult(
                backend="ck_dsl", ok=True, ms=ms, max_abs=mx, mean_abs=mn, rel_max=rl
            )
            print(f"  ck_dsl       : {ms:8.3f} ms   max_abs={mx:.4g}  rel={rl:.4g}")
        except Exception as exc:
            res.results["ck_dsl"] = BackendResult(
                backend="ck_dsl", ok=False, note=f"{type(exc).__name__}: {exc}"
            )
            print(f"  ck_dsl       : FAIL   {exc!r}")
        _isolate_lane()
...
def print_summary(results: List[ScenarioResult]) -> None:
    ...
    for r in results:
        ck_ms = r.results.get("ck_dsl", BackendResult("", False)).ms
...
def main() -> int:
    ...
    parser.add_argument("--skip-ck-dsl", action="store_true")
    ...
    for r in results:
        ck = r.results.get("ck_dsl")
        if ck is not None and ck.ok and ck.max_abs is not None and ck.max_abs > 0.5:
            print(
                f"\nFAIL: ck_dsl correctness regressed on {r.scenario.name} "
