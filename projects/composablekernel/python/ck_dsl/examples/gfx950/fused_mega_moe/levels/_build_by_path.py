...
``ck_dsl.instances.common.moe_fused_mega_fp8`` on disk is never touched or
...
def load_level_module(path: str | Path, *, mod_name: str | None = None) -> ModuleType:
    ...
    ``ck_dsl.instances.common._level_<stem>``).
    """
    path = Path(path).resolve()
    if not path.is_file():
        raise FileNotFoundError(f"level snapshot not found: {path}")
    if mod_name is None:
        mod_name = f"ck_dsl.instances.common._level_{path.stem}"
...
def build_level_kernel(
    path: str | Path,
    *,
    name: str = "ck_dsl_fused_moe_mega_fp8",
    arch: str = "gfx950",
    **spec_kwargs,
):
