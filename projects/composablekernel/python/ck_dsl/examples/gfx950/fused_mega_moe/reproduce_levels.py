...
from ck_dsl.examples.gfx950.moe.fused_moe_e2e_perf import (  # noqa: E402
    Scenario,
    make_inputs,
    time_callable_ms,
    _compare,
...
from ck_dsl.runtime.launcher import (  # noqa: E402
    KernelLauncher,
    LaunchConfig,
)
from ck_dsl.examples.gfx950.fused_mega_moe.levels._build_by_path import (  # noqa: E402
    load_level_module,
)

# Production kernel (the FINAL best; all levers default-on).
from ck_dsl.instances.common import moe_fused_mega_fp8 as PROD  # noqa: E402
...
def _materialize_module(lvl: LevelDef):
    ...
    if lvl.mechanism in ("snapshot", "sub-diff(L5)"):
        ...
        # Use a name under a REAL package (ck_dsl.instances.common.*) so the
        ...
        mod = load_level_module(
            path,
            mod_name=f"ck_dsl.instances.common._repro_level_{lvl.idx}_{path.stem}",
        )
        # Snapshot specs are self-contained; only tile_m is a knob they all carry.
        spec = mod.FusedMegaKernelSpecFp8(
            name=f"ck_dsl_fused_moe_mega_fp8_L{lvl.idx}", tile_m=lvl.tile_m
        )
        return mod, spec
    # flag mechanism -> production module.
    spec = PROD.FusedMegaKernelSpecFp8(
        name=f"ck_dsl_fused_moe_mega_fp8_L{lvl.idx}",
        tile_m=lvl.tile_m,
