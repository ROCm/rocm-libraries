...
The seven scripts each isolate one of these pieces, benchmark the `ck_dsl`
...
is lower and the kernel goes bandwidth-bound sooner. Both the `ck_dsl` 3D kernel
...
the indices). The `ck_dsl` `topk_softmax` kernel does the whole thing in **one
...
slot. The `ck_dsl` `MoeSortingLauncher` implements exactly this as a **3-kernel
...
```
`silu(x) = x · σ(x)`. The `ck_dsl` pipeline is: `topk_softmax` → (sort, **skipped**)
