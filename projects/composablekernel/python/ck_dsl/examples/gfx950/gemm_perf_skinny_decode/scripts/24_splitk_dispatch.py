...
o_proj / gate_up / down -- pick it up automatically:
  ...
  2. ``dispatch_gemm_bf16`` selects a candidate and the split-K heuristic
     (``ck_dsl.helpers.split_k``) folds a valid split degree into the spec when
...
``CK_DSL_GEMM_SPLIT_K`` (``auto`` | ``off`` | ``<n>``). The dry-run path (no GPU)
...
def report_dispatch():
    ...
    # forced env degree (CK_DSL_GEMM_SPLIT_K=<n>) intentionally overrides it.
    auto = os.environ.get("CK_DSL_GEMM_SPLIT_K", "auto").strip().lower() in ("", "auto")
    print(
