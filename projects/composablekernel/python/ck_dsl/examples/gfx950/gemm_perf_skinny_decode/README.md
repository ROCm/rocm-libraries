...
```bash
cd <ck_dsl>/examples/gfx950/gemm_perf_skinny_decode    # this folder
...
> **Re-measured 2026-05-29 serially (exclusive GPU, 50 warmup × 1000
> timed × 20 attempts × 3 interleaved rounds, N=60).** chiplet/baseline =
> 0.976× (the chiplet swizzle is a real 2.4 % lift, confirmed serially);
> chiplet/rocBLAS = 1.01× best / 0.99× median — DSL is within the noise
> floor of rocBLAS and slightly faster than it on the median.

The honest reference is the standalone rocBLAS probe: **DSL 10.30 µs
(median 10.32) vs rocBLAS 10.18 µs (median 10.46) — 1.01× best, within
the noise floor.** DSL is **more bytes-per-µs efficient** than rocBLAS
...
```
ck_dsl/examples/gfx950/gemm_perf_skinny_decode/
├── README.md                              # this file
├── scripts/
...
Best correct variant (step 21, confirmed step 22 N=60):
  ...
  latency:     10.30 µs best / 10.32 µs median (N=60, serial 2026-05-29, spread ~1%)
  throughput:  6.51 TFLOPS, 3263 GB/s (44% HBM)
  correctness: max_abs_diff vs fp32 reference = 0.0 (bit-exact)
  vs rocBLAS:  1.01× best / 0.99× median — within noise of rocBLAS (10.18 µs standalone)
