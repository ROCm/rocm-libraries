...
> split-K support that lives in `ck_dsl/instances/common/gemm_universal.py`
...
selection lives in `ck_dsl.helpers.split_k.select_split_k`: it engages only when
...
opt-in / default-safe and overridable via `CK_DSL_GEMM_SPLIT_K`
(`auto` | `off` | `<n>`).
