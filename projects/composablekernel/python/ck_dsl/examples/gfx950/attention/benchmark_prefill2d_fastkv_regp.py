...
class CkDslFastKvRegPBench:
    ...
    def benchmark(
        ...
        *,
        ...
        return {
            "framework": "ckdsl_ua_tiled_2d_fastkv_regp",
            "variant": variant,
            "policy": policy,
...
def _print_summary(rows: list[dict[str, Any]], joined: list[dict[str, Any]]) -> None:
    ...
    for row in joined:
        ratio = row.get("ckdsl_over_triton_latency")
        if ratio != "":
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("/tmp/ckdsl_prefill2d_fastkv_regp.json"),
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("/tmp/ckdsl_prefill2d_fastkv_regp.csv"),
    )
    parser.add_argument(
        "--joined-csv",
        type=Path,
