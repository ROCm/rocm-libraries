...
the published `ck_dsl vs ck_tile_cpp` gap by ~1 % on the H=4096
...
in `run_ck_dsl()`. The previous hardcoded `streaming_block_size=64`
...
| Scenario | ck_dsl vs torch (pre-tighten) | (post-tighten) |
|---|---:|---:|
...
```text
decode_T1_E8_K2_H4096_I7168 : ck_dsl baseline 0.388 ms (0.31x of cktile)
                              ck_dsl preshuf  0.355 ms (0.34x of cktile)
...
- `CK_DSL_PRESHUFFLE_W_DOWN=1`
- `CK_DSL_PRESHUFFLE_W_GATE_UP_INTERLEAVED=1` (production default
  path uses the interleaved kernel)
- `CK_DSL_PRESHUFFLE_W_GATE_UP_PACKED=1` (forces the packed
