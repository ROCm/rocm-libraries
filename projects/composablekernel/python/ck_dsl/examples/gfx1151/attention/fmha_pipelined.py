...
addressing uses :func:`~ck_dsl.helpers.make_global_view` 3D ``(head, token, dim)``
views + :func:`~ck_dsl.helpers.make_tile_window`; the score/accumulator carry as
packed :class:`~ck_dsl.helpers.WmmaTensor` tiles, QK fragments come from
:func:`~ck_dsl.helpers.load_wmma_tile` and the matmul is
:func:`~ck_dsl.helpers.wmma_mma` over :class:`~ck_dsl.helpers.WmmaAtom`; the
...
through a :func:`~ck_dsl.helpers.make_lds_view`; and the O epilogue uses
:func:`~ck_dsl.helpers.store_wmma_tile`. The ``p_xpose="shuffle"`` path is the one
...
from ck_dsl.core.ir import F16, F32, I16, I32, IRBuilder, KernelDef, PtrType, VectorType
...
from ck_dsl.helpers.attention import (
...
class PipelinedCfg:
    ...
    def kernel_name(self) -> str:
        from ck_dsl.helpers.spec import kernel_name_join
