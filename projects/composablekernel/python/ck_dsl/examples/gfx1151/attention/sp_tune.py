...
"""Driver for the software-pipelined WMMA FMHA kernel (``fmha_pipelined``)."""
...
def verify_and_time(cfg: PipelinedCfg, shape: Shape, *, warmup=15, iters=100, tol=2e-2,
                    objdump=None, arch="gfx1151"):
    import numpy as np

    art = compile_kernel(build_wmma_fmha_pipelined(cfg, arch=arch), arch=arch)
    ...
    grid = pipelined_grid(cfg, seqlen_q=Sq, batch=B)
...
def _fmt(r):
    c = r["cfg"]
    return (
        f"sched={int(c.sched)} xp={c.p_xpose:7s} | {'Y' if r['ok'] else 'N'} {r['max_abs']:.2e} "
...
def main():
    ...
    ap.add_argument("--sched", type=int, nargs="+", default=[0, 1])
    ap.add_argument("--pxpose", nargs="+", default=["lds"],
                    choices=["lds", "shuffle"])
    ap.add_argument("--fusek", nargs="+", default=["auto"],
                    choices=["auto", "0", "1"])
    args = ap.parse_args()
    _fk = {"auto": None, "0": False, "1": True}
    ...
    for fk in args.fusek:
        for xp in args.pxpose:
            for sc in args.sched:
                cfg = PipelinedCfg(
                    head_size=shape.head_size, num_query_heads=shape.heads,
                    num_kv_heads=shape.kv_heads,
                    mask_mode="causal" if shape.causal else "none", sched=bool(sc),
                    p_xpose=xp, fuse_k=_fk[fk],
                )
                try:
                    r = verify_and_time(cfg, shape, objdump=objdump)
                except Exception as e:  # noqa: BLE001
                    print(f"sched={sc} xp={xp} fk={fk}: BUILD/RUN FAIL: {e}")
                    continue
                print(f"fk={fk:4s} " + _fmt(r))
