...
added address-arithmetic op costs TFLOP/s). The helper primitives it reuses:
  ...
  * :func:`~ck_dsl.helpers.make_global_view` + :func:`~ck_dsl.helpers.make_tile_window`
  ...
  * :class:`~ck_dsl.helpers.WmmaAtom` (``wmma_f32_16x16x16_f16``) for the WMMA
    contract, with its hardware-verified ``a_layout``/``b_layout``/``c_layout``
    maps driving every lane decode.
  * :class:`~ck_dsl.helpers.WmmaTensor` — a packed distributed tensor carrying
    one lane's fragment/accumulator as a single SSA vector — together with
    :func:`~ck_dsl.helpers.load_wmma_tile` (one packed ``global_load_dwordx8``
    per ``<16 x half>`` fragment, no f32 cast) and :func:`~ck_dsl.helpers.wmma_mma`
  ...
  * :func:`~ck_dsl.helpers.make_lds_view` for the P-transpose (and the optional
    V-transpose) LDS staging.
  * :func:`~ck_dsl.helpers.store_wmma_tile` for the O epilogue.
  * :mod:`ck_dsl.helpers.attention` (``wave_reduce_max``/``wave_reduce_sum`` /
