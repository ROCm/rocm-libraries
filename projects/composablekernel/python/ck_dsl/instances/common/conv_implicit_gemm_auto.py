...
class ImplicitGemmConvSpec:
    ...
    tile_m: int = 64
    tile_n: int = 64
    # tile_k=64 + 32x32x16 atom: measured-best default on gfx950 (ties the old
    # tk128/16x16x32 on the bake-off shape, ~1.4x faster on compute-bound
    # shapes). Mirrors the conv_implicit_gemm.py default. All shipped callers
    # already override to this config.
    tile_k: int = 64
...
def is_valid_spec(spec: ImplicitGemmConvSpec, arch: str = "gfx950") -> Tuple[bool, str]:
    ...
    The MFMA warp-tile atom and the per-WG LDS capacity are sourced from
    :class:`ck_dsl.core.arch.ArchTarget`, so a warp-tile atom that exists
    on gfx950 but not gfx942 (e.g. the default wide ``16x16x32`` f16
    atom) is rejected for gfx942 with a structured reason instead of
    crashing comgr at lower time.
    """
...
def build_implicit_gemm_conv_auto(
    ...
    :func:`is_valid_spec`) before lowering so a gfx950-only atom
    requested for gfx942 fails cleanly rather than crashing comgr.
    """
    spec.validate()
