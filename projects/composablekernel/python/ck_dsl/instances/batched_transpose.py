...
Same per-batch algorithm as :mod:`ck_dsl.instances.transpose`:
...
def is_valid_spec(spec: BatchedTranspose2DSpec) -> Tuple[bool, str]:
    ok, why = validate_io(
        IOSpecRule(
            dtype=spec.dtype,
...
def build_batched_transpose2d(spec: BatchedTranspose2DSpec) -> KernelDef:
