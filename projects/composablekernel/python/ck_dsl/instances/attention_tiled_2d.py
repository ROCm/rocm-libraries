    ...
    with kvloop as (kv_tile_iv, carry):
        ...
        if USE_MFMA_32X32:
            if TRANSPOSED_QK_32X32:
                ...
                # already invariant across (n, reg) in this lane); we
                # reuse the hoisted ``st_alibi_slope`` (or the per-iter
                # ``st_alibi_slope_iter``) so the computation is a single
                # ``v_pk_mul_f32`` chain per (n, reg). Without this, the
                # transposed-32x32 path skipped ALiBi / QQ-bias entirely
                # — that was the root cause of the 2D regressions on
                # ``alibi_decode_d128_b16``, ``alibi_mixed_d128_b16``, and
                # ``qq_bias_prefill_d128_b16`` (max_abs values 2.113,
                # 2.425, 0.2686 vs the fp16 ULP gate).
                if USE_ALIBI:
                ...
                for n in range(QK_N_TILES):
                    for reg in range(16):
                        ...
                        s_raw = b.vec_extract(ST32_n[n], reg)
                        ...
                        if USE_ALIBI:
                            ...
                            add_term = b.fmul(b.fmul(slope_for_lane, pos_f), rcp_ln2)
                ...
                PT32_n = [[None] * 16 for _ in range(QK_N_TILES)]
                st_l_local = zero_f
                for n in range(QK_N_TILES):
                    for reg in range(16):
                        p_t = b.exp2(b.fsub(st_scores[(n, reg)], st_m_new))
                        PT32_n[n][reg] = p_t
                ...
                for n in range(ACC_N_TILES):
                    ...
                    for k in range(T // 16):
                        ...
                        for kk in range(8):
                            ...
                            p0 = PT32_n[p_tile0][reg0]
                            p1 = PT32_n[p_tile1][reg1]
                            # PT32_n stores P^T[k, q] in lanes whose
                            ...
                            # Therefore cross-half exchange is only needed
                            # when the K-row owner half differs from this
                            # lane's half.
                            if owner_half0 == 1:
