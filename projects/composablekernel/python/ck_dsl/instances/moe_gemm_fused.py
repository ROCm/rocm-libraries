...
def build_moe_gate_up_silu_gemm(spec: FusedGateUpSiluGemmSpec) -> KernelDef:
    """Build the fused gate+up+silu MFMA kernel."""

    u = spec.to_universal_spec()
    ok, why = is_valid_gemm_spec(u)
    if not ok:
    ...
    def emit_load_phase(k_off: Value) -> None:
        ...
        for e in range(a_vecs_per_thread):
            ...
            if load_vec == 1:
                av = a_global.load_scalar(b, b.const_i32(0), row, col)
                a_lds.store_scalar(b, row, col, value=av)
            else:
                av = a_global.load_vec(b, b.const_i32(0), row, col, n=load_vec)
                a_lds.store_vec(b, row, col, value=av, n=load_vec)
        ...
        for e in range(b_vecs_per_thread):
            ...
            col = b.mul(col_v, c_load_vec) if load_vec > 1 else col_v
            if load_vec == 1:
                gv = wg_global.load_scalar(b, b.const_i32(0), row, col)
                uv = wu_global.load_scalar(b, b.const_i32(0), row, col)
                bg_lds.store_scalar(b, row, col, value=gv)
                bu_lds.store_scalar(b, row, col, value=uv)
            else:
                gv = wg_global.load_vec(b, b.const_i32(0), row, col, n=load_vec)
                uv = wu_global.load_vec(b, b.const_i32(0), row, col, n=load_vec)
                bg_lds.store_vec(b, row, col, value=gv, n=load_vec)
                bu_lds.store_vec(b, row, col, value=uv, n=load_vec)
    ...
    for_op = b.scf_for_iter(c0, K, c_block_k, gate_accs + up_accs, iv_name="k0")
    n_accs = len(gate_accs)
    with for_op as (k0, iter_vars):
        emit_load_phase(k0)
        b.sync()
        gate_vars = list(iter_vars[:n_accs])
        up_vars = list(iter_vars[n_accs:])
...
def build_moe_down_reduce_gemm(spec: FusedDownReduceGemmSpec) -> KernelDef:
    ...
    def emit_load_phase(k_off: Value) -> None:
        ...
        for e in range(b_vecs_per_thread):
            ...
            col = b.mul(col_v, c_load_vec) if load_vec > 1 else col_v
            if load_vec == 1:
                bv = b_global.load_scalar(b, b.const_i32(0), row, col)
                b_lds.store_scalar(b, row, col, value=bv)
            else:
                bv = b_global.load_vec(b, b.const_i32(0), row, col, n=load_vec)
                b_lds.store_vec(b, row, col, value=bv, n=load_vec)
