...
:func:`~ck_dsl.runtime.launcher.launch_kernel` chained launches plus
a per-expert grouped-GEMM dispatch loop. Composes the existing single-
purpose instance launchers:

* :class:`~ck_dsl.instances.topk_softmax.TopkSoftmaxSpec` (single
  launch) for the router.
* :class:`~ck_dsl.instances.moe_sorting.MoeSortingLauncher` for the
  3-phase sort (histogram -> scan -> scatter).
* :class:`~ck_dsl.instances.fused_moe.FusedMoeLauncher` for the
  3-phase MoE-specific chain (gather -> silu_mul -> topk_reduce).
* :class:`~ck_dsl.instances.grouped_gemm.GroupedGemmLauncher` for
...
:func:`~ck_dsl.runtime.launcher.launch_kernel` call -- one
:func:`~ck_dsl.runtime.launcher.make_kernel` closure per kernel,
plus two :class:`~ck_dsl.runtime.launcher.make_kernel`-shape lambdas
that ``hipMemset`` the histogram + counter buffers between sort
phases. The per-expert GEMMs go through
:class:`~ck_dsl.instances.GroupedGemmLauncher`, which loops in Python
...
def _default_gemm_tile() -> TileSpec:
    ...
    Sweep result on MI355X (``examples/moe/tune_gate_up_silu.py``):
...
class FusedMoeForward:
    ...
    :func:`~ck_dsl.runtime.launcher.launch_kernel` calls plus a
    ...
    def _ensure_topk_launcher(self) -> KernelLauncher:
        if self._topk_launcher is None:
            ...
            artifact = compile_kernel(build_topk_softmax(spec), capture_ir_text=False)
    ...
    def _ensure_gemm_launcher(self) -> GroupedGemmLauncher:
        if self._gemm_launcher is None:
            ...
            artifact = compile_kernel(
                build_grouped_gemm(gemm_spec), capture_ir_text=False
            )
    ...
    def _ensure_silu_mul_packed_launcher(self) -> KernelLauncher:
        ...
        if self._silu_mul_packed_launcher is None:
            ...
            artifact = compile_kernel(
                build_moe_silu_mul_packed(fmoe_spec), capture_ir_text=False
            )
    ...
    def _ensure_static_scatter_gather_launcher(self) -> KernelLauncher:
        ...
        if self._static_scatter_gather_launcher is None:
            ...
            artifact = compile_kernel(
                build_moe_static_scatter_gather(spec), capture_ir_text=False
            )
    ...
    def capture_graph(
        ...
        *,
        ...
        :class:`~ck_dsl.runtime.launcher.WorkspacePool` and let the
    ...
    def _forward_static(
        ...
        *,
        ...
        :func:`~ck_dsl.runtime.launcher.launch_kernel` chain (no
