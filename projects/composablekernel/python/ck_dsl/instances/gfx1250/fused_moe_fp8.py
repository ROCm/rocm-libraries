...
class Gfx1250Fp8MoeSpec:
    ...
    dtype: str = "bf16"
    name: str = "ck_dsl_gfx1250_fp8_moe"
...
class Gfx1250Fp8Moe:
    ...
    def __init__(self, spec: Gfx1250Fp8MoeSpec, *, arch: str = "gfx1250") -> None:
        ...
        self._gather_spec = FusedMoeSpec(
            tokens=rows, experts=1, topk=1, hidden=H, intermediate=I,
            dtype=spec.dtype, block_size=bcommon, vec=8, name=f"{spec.name}_gather",
        )
        self._silu_spec = FusedMoeSpec(
            tokens=rows, experts=1, topk=1, hidden=H, intermediate=I,
            dtype=spec.dtype, block_size=bcommon, vec=8, name=f"{spec.name}_silu",
        )
        self._reduce_spec = FusedMoeSpec(
            tokens=rows, experts=1, topk=1, hidden=H, intermediate=I,
            dtype=spec.dtype, block_size=bcommon, vec=8, name=f"{spec.name}_reduce",
        )
        ...
        self._aq_spec = SmoothQuantSpec(
            n_per_block=H, dtype=spec.dtype, out_dtype=spec.lowbit,
            block_size=aq_bs, vec=aq_vec, save_yscale=True, wave_size=32,
            name=f"{spec.name}_actq",
        )
        self._hq_spec = SmoothQuantSpec(
            n_per_block=I, dtype=spec.dtype, out_dtype=spec.lowbit,
            block_size=hq_bs, vec=hq_vec, save_yscale=True, wave_size=32,
        ...
        self._gu_spec = BlockScaledGemmSpec(
            name=f"{spec.name}_gu", M=slot, N=I, K=H,
            dtype_a=spec.lowbit, dtype_b=spec.lowbit, dtype_c=spec.dtype,
            scale_dtype="fp32", block_k=H,
        )
        self._down_spec = BlockScaledGemmSpec(
            name=f"{spec.name}_down", M=slot, N=H, K=I,
            dtype_a=spec.lowbit, dtype_b=spec.lowbit, dtype_c=spec.dtype,
            scale_dtype="fp32", block_k=I,
        )
    ...
    # ------------------------------------------------------------------
    def forward_numpy(self, rt, *, X, topk_ids, topk_weights,
                      Wg, Wu, Wd, return_debug=False):
        ...
        self.bind(rt)
        s = self.spec
        H, I, E = s.hidden, s.intermediate, s.experts
        slot, rows = s.slot_size, s.rows
        bf16 = ml_dtypes.bfloat16
        fp8 = ml_dtypes.float8_e4m3fn if s.lowbit == "fp8e4m3" else ml_dtypes.float8_e5m2
        ...
        dSmH, dSmI = _up(rt, u8(ones_h), ones_h.nbytes), _up(rt, u8(ones_i), ones_i.nbytes)
        ...
        self._launch(self._gather, moe_gather_grid(self._gather_spec),
                     self._gather_spec.block_size,
                     struct.pack("<QQQii", dX, dSids, dGI, rows, H))
        # 2. dynamic per-row activation quant.
        self._launch(self._actq, smoothquant_grid(rows, self._aq_spec),
        ...
        for e in range(E):
            ...
            c_u = dUp + e * slot * I * 2
            self._launch(self._gu, gu_grid, self._gu_spec.block_size,
                         struct.pack("<QQQQQiii", a, dWg + e * I * H, asc,
                                     dWgS + e * I * 4, c_g, slot, I, H))
            self._launch(self._gu, gu_grid, self._gu_spec.block_size,
                         struct.pack("<QQQQQiii", a, dWu + e * I * H, asc,
                                     dWuS + e * I * 4, c_u, slot, I, H))
        # 4. SwiGLU activation.
        self._launch(self._silu, moe_silu_mul_grid(self._silu_spec),
                     self._silu_spec.block_size,
                     struct.pack("<QQQii", dGate, dUp, dHid, rows, I))
        # 5. dynamic per-row hidden quant.
        self._launch(self._hidq, smoothquant_grid(rows, self._hq_spec),
        ...
        for e in range(E):
            self._launch(self._down, down_grid, self._down_spec.block_size,
                         struct.pack("<QQQQQiii",
                                     dHq + e * slot * I, dWd + e * H * I,
                                     dHs + e * slot * 4, dWdS + e * H * 4,
                                     dDown + e * slot * H * 2, slot, H, I))
        # 7. topk-weighted scatter back to token rows.
        self._launch(self._reduce, moe_topk_weighted_reduce_grid(self._reduce_spec),
        ...
        for d in (dX, dSids, dSw, dWg, dWgS, dWu, dWuS, dWd, dWdS, dSmH, dSmI,
                  dGI, dAq, dAs, dGate, dUp, dHid, dHq, dHs, dDown, dY):
            rt.free(d)
