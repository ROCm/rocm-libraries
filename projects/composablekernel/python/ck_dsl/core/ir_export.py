...
"""Portable CK-DSL IR exporter (schema ``ck.dsl.ir/v1``).

Serializes an already-built :class:`~ck_dsl.core.ir.KernelDef` graph into a
structured, dependency-free dict (JSON-able) that the pure-C ``ck_dsl_c``
backend can import and lower without embedding CPython.

This is the offline half of the "shape-polymorphic portable IR" path described
in ``dsl_docs/architecture/portable_ir_schema.md``: Python remains the kernel
authoring surface; the *result* of running a builder (the SSA graph) is
exported here and re-driven through the C IRBuilder by ``ckc_import_kernel``.
...
- Types are serialized structurally (scalars as their canonical name string,
  composites as objects) mirroring the C ``ckc/ir.h`` type model.
...
def _type_to_json(t: Type) -> Any:
    ...
    if isinstance(t, SmemType):
        return {"kind": "smem", "elem": _type_to_json(t.elem), "shape": [int(x) for x in t.shape]}
...
def _attr_to_json(value: Any) -> Dict[str, Any]:
    ...
    Kinds mirror the C ``ckc_attr_kind_t``: i (int64), f (double), b (bool),
    ...
    if isinstance(value, (list, tuple)):
        ...
        return {"t": "l", "v": items}
    raise TypeError(f"ir_export: unsupported attr value type {type(value).__name__!r}: {value!r}")
...
def _param_to_json(p: Param) -> Dict[str, Any]:
    return {"name": p.name, "type": _type_to_json(p.type), "attrs": dict(sorted(p.attrs.items()))}
...
def export_kernel_ir(
    ...
    *,
    ...
    """Return the portable-IR dict for ``kernel`` (schema ``ck.dsl.ir/v1``)."""
    opcodes = sorted(_collect_opcodes(kernel.body))
    payload: Dict[str, Any] = {
        "schema": SCHEMA,
        "producer": {"name": "ck_dsl_python", "version": "0.1"},
        "requires": {"min_ckc_ir": 1, "opcodes": opcodes},
        "kernel": {
            "name": kernel.name,
...
def export_kernel_ir_json(kernel: KernelDef, *, indent: int | None = 2, **kw: Any) -> str:
