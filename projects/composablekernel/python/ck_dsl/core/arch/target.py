...
"""CK_DSL architecture target metadata (the polymorphic-core SSOT).

``ArchTarget`` is the single CK_DSL-owned description of *what a gfx target
...
in this module imports from ``dispatcher/`` — CK_DSL is standalone and must be
...
# A callable that, given an :class:`~ck_dsl.core.ir.IRBuilder`, a runtime lane
...
class LayoutMap:
    ...
    def coord(self, builder: Any, lane: Any, slot: int) -> Tuple[Any, Any]:
        ...
        ``builder`` is an :class:`~ck_dsl.core.ir.IRBuilder`; ``lane`` is a
