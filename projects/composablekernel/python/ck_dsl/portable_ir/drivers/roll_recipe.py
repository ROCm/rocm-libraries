...
# D64/D128/D256 by run_attn2d_parametric_demo.sh.
import argparse
import copy
import json
import sys
...
def _roll_body(body0, body1):
    ...
    for s in range(0, len(body1) - 2 * L):
        if (_is_const_int(body1[s]) and _const_val(body1[s]) == 0
                and _is_const_int(body1[s + L]) and _const_val(body1[s + L]) == VEC
                and _is_const_int(body1[s + 2 * L]) and _const_val(body1[s + 2 * L]) == 2 * VEC):
            run_start = s
            break
    ...
    rolled.append({
        "op": "static_for", "var": "d8", "lo": 0,
        "hi": {"div": [{"spec": "D"}, VEC]}, "step": 1, "body": template,
    })
        ...
        if op == "emit" and i1.get("opcode") == "arith.constant":
            ...
            if v0a.get("t") == "i" and v1a.get("t") == "i":
                ...
                if isinstance(a, int) and isinstance(b, int) and a != b:
                    ...
                    assert a % 64 == 0 and b == (a // 64) * 128, f"unexpected const {a},{b}"
                    coeff = a // 64
                    i1["attrs"]["value"] = {"t": "i", "v": {"mul": [{"spec": "D"}, coeff]}}
...
def _roll_body_d0(body0, body1):
    ...
    for s in range(0, len(body0) - 2 * L):
        if (_is_const_int(body0[s]) and _const_val(body0[s]) == 0
                and _is_const_int(body0[s + L]) and _const_val(body0[s + L]) == VEC):
            run_start = s
            break
    ...
    rolled.append({"op": "alias", "bind": ACC, "from": carry_in0})
    rolled.append({"op": "static_for", "var": "d8", "lo": 0,
                   "hi": {"div": [{"spec": "D"}, VEC]}, "step": 1, "body": template})
