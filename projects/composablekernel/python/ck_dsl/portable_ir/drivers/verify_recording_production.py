...
def _cases():
    ...
    from ck_dsl.instances.common import (
        attention_unified,
        elementwise,
    ...
    yield ("transpose f16 64x64",
           transpose,
           lambda: transpose.build_transpose2d(
               transpose.Transpose2DSpec(tile_m=64, tile_n=64, vec=8, dtype="f16",
                                         lds_pad=8, grid_order="row")))
...
def main() -> int:
    ...
    for label, module, build in _cases():
        ...
        print(f"{label:<26} {nops:>6}  {'PASS' if match_walk else 'FAIL':>9}  "
              f"{('PASS' if match_legacy else ('N/A*' if multi else 'FAIL')):>9}  "
              f"{','.join(multi) if multi else '-'}")

    print("-" * 78)
    print("* N/A: recorder emits richer 'outs' for multi-result ops; legacy walk keeps single 'out'.")
    if failures:
