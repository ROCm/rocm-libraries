...
#
# Needs a shared libckc (CKC_LIB or auto-built) and, for comgr timing, a comgr
# tool path in $COMGR (else comgr is reported as n/a). Drive it with
# ck_dsl_c/tests/portable_ir/run_online_bench.sh.
import hashlib
import os
import statistics
import subprocess
...
def _comgr_ms(ll: str) -> Optional[float]:
    ...
    try:
        def _run():
            subprocess.run([COMGR, llpath, hpath, ARCH], check=True,
...
def _online_split(cbor: bytes, ints: Dict[str, int], strs: Dict[str, str]
...
def _row(name: str, build_fn: Callable, *, ints: Optional[Dict] = None,
         strs: Optional[Dict] = None,
         recipe: Optional[dict] = None) -> Dict[str, float]:
    ints = ints or {}
    strs = strs or {}
    if recipe is None:
    ...
    if COMGR:
        assert _hsaco_sha(ll) == _hsaco_sha(py_ll), f"{name}: online HSACO != python HSACO"
    comgr = _comgr_ms(ll)
    return {"name": name, "lines": ll.count("\n") + 1, "build": build_ms,
            "serialize": serialize_ms, "py_lower": py_lower_ms, "c_build": c_build,
            "c_lower": c_lower, "comgr": comgr or float("nan"), "cbor_bytes": len(cbor)}


def _fmt_table(rows: List[Dict[str, float]]) -> None:
    cols = [("kernel", "name", "%-30s"), ("ll", "lines", "%6d"),
            ("build", "build", "%8.3f"), ("serialize", "serialize", "%9.3f"),
            ("py_lower", "py_lower", "%8.3f"), ("c_build", "c_build", "%8.3f"),
            ("c_lower", "c_lower", "%8.3f"), ("comgr", "comgr", "%9.2f")]
    head = "  ".join(("%-30s" if k == "name" else
                      ("%6s" if k == "lines" else "%9s")) % h for h, k, _ in cols)
...
def _summary(rows: List[Dict[str, float]]) -> None:
    ...
    for r in rows:
        ...
        print("%-30s  %9.1f%%  %9.1f%%  %9.1f%%" % (
            r["name"], 100 * r["c_build"] / total, 100 * r["c_lower"] / total,
            100 * comgr / total))
    ...
    for r in rows:
        ...
        print("%-30s  %9.3f  %9.3f  %6.1fx" % (r["name"], r["py_lower"], r["c_lower"], sp))
...
def main() -> int:
    ...
    print(f"== online portable-IR compile timeline (arch={ARCH}, N={N}, "
