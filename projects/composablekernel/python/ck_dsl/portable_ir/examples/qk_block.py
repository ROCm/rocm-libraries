...
# every D. Run via run_qk_block_demo.sh.
import argparse
import json
import sys
...
def kernel_name(D: int, dtype: str) -> str:
    return f"ckdsl_qk_block_d{D}_{dtype}"
...
def record_qk_block():
    from ck_dsl.portable_ir.src.recipe_recorder import RecipeRecorder, IExpr
    ...
    C = rec.param("C", {"kind": "ptr", "pointee": "f32", "space": "global"},
    ...
    loop = rec.static_for_acc("d8", 0, rec.idiv(rec.spec("D"), VEC), [("score", score0)])
...
def main() -> int:
    ...
    if args.emit == "recipe":
        ...
        sys.stdout.write(lower_kernel_to_llvm(build_qk_block(args.D, args.dtype), arch=args.arch))
    return 0
