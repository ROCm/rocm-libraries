...
# exactly that kernel for any D. The two are compared (HSACO byte-identical) by
# run_recipe_demo.sh.
#
...
def kernel_name(D: int, dtype: str) -> str:
    return f"ckdsl_recipe_toy_d{D}_{dtype}"
...
def make_recipe() -> dict:
    ...
    body = [
        ...
        {"op": "emit", "opcode": "memref.global_load_typed", "in": ["A", "i"],
         "out": {"bind": "a", "type": "f32"}, "attrs": f32load},
        {"op": "emit", "opcode": "memref.global_load_typed", "in": ["B", "i"],
         "out": {"bind": "bb", "type": "f32"}, "attrs": f32load},
    ...
    return {
        ...
        "kernel_name_fmt": "ckdsl_recipe_toy_d{D}_{dtype}",
...
def main() -> int:
    ...
    if args.emit == "recipe":
        ...
        sys.stdout.write(lower_kernel_to_llvm(build_toy(args.D, args.dtype), arch=args.arch))
    return 0
