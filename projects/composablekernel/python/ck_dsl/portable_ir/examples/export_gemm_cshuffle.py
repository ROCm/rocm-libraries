...
# production (run_gemm_cshuffle_demo.sh).
#
...
def make_recipe() -> dict:
    r = roll(build, axis="TN", sample_points=[32, 64], holdout_points=[128, 256],
...
def main() -> int:
    ...
    if args.emit == "recipe":
        ...
        sys.stdout.write(lower_kernel_to_llvm(build(args.TN, args.arch), arch=args.arch))
    return 0
