...
# (schema ck.dsl.recipe/v1), mirroring the C recipe VM (recipe_vm.c) semantics:
...
def eval_intexpr(node: Any, ivars: Dict[str, int], spec_int: Dict[str, int],
                 spec_str: Dict[str, str]) -> int:
    ...
    if isinstance(node, dict):
        ...
        for k, fn in _BIN.items():
            if k in node:
                a, b = node[k]
                return fn(eval_intexpr(a, ivars, spec_int, spec_str),
...
class _Expander:
    ...
    def _eval_type(self, t: Any) -> Any:
        ...
        if isinstance(t, dict):
            ...
            if any(k in t for k in ("spec", "var", "spec_str_eq", "add", "sub",
                                    "mul", "div", "mod")):
                return self._eval(t)
    ...
    def _instr(self, instr: Dict[str, Any]) -> None:
        ...
        if op == "param":
            ...
            self.out.append({"op": "param", "name": name, "type": instr["type"],
                             "bind": name, "attrs": dict(instr.get("attrs", {}))})
            return
        if op == "const_i32":
            r = self._fresh("c")
            self.env[instr["bind"]] = r
            self.out.append({"op": "emit", "opcode": "arith.constant",
                             "out": {"bind": r, "type": "i32"},
                             "attrs": {"ity": {"t": "s", "v": "i32"},
        ...
        if op == "scf_for":
            iv = self._fresh("iv")
            lo, hi, step = (self._resolve(instr["lo"]), self._resolve(instr["hi"]),
            ...
            for m in self._expand_iter_list(instr.get("iter", [])):
                nm = self._fresh("it")
                iters.append({"name": nm, "init": self._resolve(m["init"]),
                              "_tmpl": m["name"]})
            ...
            self.out.append({
                ...
                "elide_trailing_barrier": bool(instr.get("elide_trailing_barrier", True)),
        ...
        if op == "emit":
            ...
            ins = [self._resolve(n) for n in self._expand_name_list(instr.get("in", []))]
    ...
    def instr(self, a: Dict[str, Any], b: Dict[str, Any]) -> bool:
        ...
        if op == "emit":
            ...
            if a.get("attrs", {}) != b.get("attrs", {}):
                self.why = f"attrs of {a['opcode']}: {a.get('attrs')} != {b.get('attrs')}"
                return False
            ...
            if "outs" in a:
                ...
                for oa, ob in zip(a["outs"], b["outs"]):
                    if oa["type"] != ob["type"] or not self._bind(oa["bind"], ob["bind"]):
                        return False
            return True
        if op == "scf_for":
            ...
            for ma, mb in zip(ia, ib):
                if not self._use(ma["init"], mb["init"]) or not self._bind(ma["name"], mb["name"]):
                    return False
