...
# (ck_dsl_c/tests/parity/*_emit.py): for each, it pulls `_spec(0)` and the
...
def _find_build_fn(mod):
    ...
    for k, v in vars(mod).items():
        if not callable(v) or not k.startswith("build"):
            continue
        if getattr(v, "__module__", "").startswith("ck_dsl.instances"):
            cands.append((k, v))
...
def _kernel_thunks(mod):
    ...
    label = getattr(mod, "__name__", "").replace("ckdsl_parity_", "")
    arch_cands = []
...
def main() -> int:
    ...
    paths = sorted(p for p in (os.path.join(_PARITY_DIR, f)
                               for f in os.listdir(_PARITY_DIR))
                   if p.endswith("_emit.py"))
    results = []
    for path in paths:
        ...
        try:
            ...
            status, detail = "FAIL", "unexpected: " + traceback.format_exc().splitlines()[-1]
    ...
    print("-" * 70)
    print(f"recorded OK: {npass}   skipped: {nskip}   FAILED: {nfail}   "
