...
Subclass of :class:`ck_dsl.core.ir.IRBuilder` that records every emitted op into
a recipe (schema ``ck.dsl.recipe/v1``) *as the kernel is built*. Any production
...
from ck_dsl.core.ir import IRBuilder, KernelDef, Op
...
def _result_fields(op: Op) -> Dict[str, Any]:
    ...
    return {"outs": [{"bind": _reg(r), "type": _type_to_json(r.type)} for r in op.results]}
...
def shallow_instr(op: Op) -> Tuple[Dict[str, Any], List[List]]:
    ...
    if op.name == "scf.for":
        ...
        instr = {
            ...
            "iter": [{"name": _bare(m["name"]), "init": _reg(inits[i])}
    ...
    instr: Dict[str, Any] = {"op": "emit", "opcode": op.name,
...
class RecordingIRBuilder(IRBuilder):
    ...
    def param(self, name: str, t, **attrs: Any):
        ...
        self._rec_params.append({"op": "param", "name": name, "type": _type_to_json(t),
                                 "bind": name, "attrs": dict(attrs)})
        return v
    ...
    def recipe(self) -> Dict[str, Any]:
        """The recorded concrete recipe (schema ck.dsl.recipe/v1)."""
        return {
            "schema": "ck.dsl.recipe/v1",
...
def _ckdsl_irbuilder_modules() -> List[Any]:
    ...
    *is* the real class are touched, and they are restored afterward."""
    ...
    for mod in list(sys.modules.values()):
        try:
            if getattr(mod, "IRBuilder", None) is IRBuilder:
                mods.append(mod)
...
def record_kernel(build_callable: Callable[[], KernelDef], *modules: Any
    ...
    With no ``modules`` given, every imported ``ck_dsl`` module that bound the
    ...
    """
    targets = list(modules) if modules else _ckdsl_irbuilder_modules()
    if not targets:
