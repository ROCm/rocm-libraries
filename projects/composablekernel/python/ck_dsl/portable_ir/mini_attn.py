...
def kernel_name(use_norm: int, dtype: str) -> str:
    return f"ckdsl_mini_attn_norm{int(use_norm)}_{dtype}"
...
def build_mini_attn(use_norm: int, dtype: str = "f32"):
    ...
    lo = b.const_i32(0)
    step = b.const_i32(1)
    loop = b.scf_for_iter(lo, n, step, [("m", m0), ("l", l0), ("acc", acc0)],
                          iv_name="k", unroll=False, elide_trailing_barrier=True)
...
def _const(bind, ity, value):
    return {"op": "emit", "opcode": "arith.constant",
...
def make_recipe() -> dict:
    ...
    body = [
        {"op": "emit", "opcode": "arith.add", "in": ["tid", "k"], "out": {"bind": "idx", "type": "i32"}},
        {"op": "emit", "opcode": "memref.global_load_typed", "in": ["Q", "idx"],
         "out": {"bind": "x", "type": "f32"}, "attrs": f32load},
        {"op": "emit", "opcode": "arith.fmax", "in": ["m", "x"], "out": {"bind": "mnew", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fsub", "in": ["x", "mnew"], "out": {"bind": "d1", "type": "f32"}},
        {"op": "emit", "opcode": "math.exp2", "in": ["d1"], "out": {"bind": "p", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fsub", "in": ["m", "mnew"], "out": {"bind": "d2", "type": "f32"}},
        {"op": "emit", "opcode": "math.exp2", "in": ["d2"], "out": {"bind": "c", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fmul", "in": ["l", "c"], "out": {"bind": "lc", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fadd", "in": ["lc", "p"], "out": {"bind": "lnew", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fmul", "in": ["acc", "c"], "out": {"bind": "ac", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fmul", "in": ["p", "x"], "out": {"bind": "px", "type": "f32"}},
        {"op": "emit", "opcode": "arith.fadd", "in": ["ac", "px"], "out": {"bind": "accnew", "type": "f32"}},
        {"op": "emit", "opcode": "scf.yield", "in": ["mnew", "lnew", "accnew"], "attrs": {"num": _i(3)}},
    ]
    program = [
        {"op": "param", "name": "Q", "type": ptr_f32, "bind": "Q",
         "attrs": {"noalias": True, "readonly": True, "align": 16}},
        {"op": "param", "name": "O", "type": ptr_f32, "bind": "O",
        ...
        {"op": "scf_for", "iv": "k", "lo": "lo", "hi": "n", "step": "step",
         "iter": [{"name": "m", "init": "m0"}, {"name": "l", "init": "l0"},
                  {"name": "acc", "init": "acc0"}],
         "results": ["mf", "lf", "accf"], "unroll": False, "elide_trailing_barrier": True,
         "body": body},
        {"op": "static_if", "pred": {"spec": "use_norm"},
         "then": [
             {"op": "emit", "opcode": "math.rcp", "in": ["lf"], "out": {"bind": "rl", "type": "f32"}},
             {"op": "emit", "opcode": "arith.fmul", "in": ["accf", "rl"], "out": {"bind": "out", "type": "f32"}},
         ],
         "else": [{"op": "alias", "bind": "out", "from": "accf"}]},
        _const("z", "i32", _i(0)),
        {"op": "emit", "opcode": "arith.cmp", "in": ["n", "z"], "out": {"bind": "cond", "type": "i1"},
         "attrs": {"pred": _s("gt")}},
        {"op": "scf_if", "cond": "cond",
         "then": [{"op": "emit", "opcode": "memref.global_store_typed", "in": ["O", "tid", "out"],
                   "attrs": f32load}]},
        {"op": "ret"},
    ]
    return {
        "schema": "ck.dsl.recipe/v1",
        "kernel_name_fmt": "ckdsl_mini_attn_norm{use_norm}_{dtype}",
...
def main() -> int:
    ...
    if args.emit == "recipe":
        ...
        sys.stdout.write(lower_kernel_to_llvm(build_mini_attn(args.use_norm, args.dtype), arch=args.arch))
    return 0
