...
def _op_to_instr(op):
    if op.name == "scf.for":
        ...
        iters = [{"name": _bare(m["name"]), "init": _reg(inits[i])}
                 for i, m in enumerate(iter_meta)]
        return {
            "op": "scf_for",
            "iv": _bare(op.attrs["iv"]),
            "lo": _reg(lo), "hi": _reg(hi), "step": _reg(step),
            "iter": iters,
            "results": [_reg(r) for r in op.results],
            "unroll": bool(op.attrs.get("unroll", False)),
            "elide_trailing_barrier": bool(op.attrs.get("elide_trailing_barrier", True)),
            "body": [_op_to_instr(o) for o in op.regions[0].ops],
        }
    if op.name == "scf.if":
        return {"op": "scf_if", "cond": _reg(op.operands[0]),
    ...
    if op.results:
        instr["out"] = {"bind": _reg(op.results[0]), "type": _type_to_json(op.results[0].type)}
    if op.attrs:
...
def kerneldef_to_recipe(kernel) -> dict:
    params = [{"op": "param", "name": p.name, "type": _type_to_json(p.type),
               "bind": p.name, "attrs": dict(p.attrs)} for p in kernel.params]
