...
#
# A bundle (schema "ck.dsl.bundle/v1") packs many concrete (or rolled) recipes,
...
BUNDLE_SCHEMA = "ck.dsl.bundle/v1"
...
def bundle_lookup(bundle: Dict[str, Any], key: str,
                  arch: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
def record_concrete_bundle(cases: List[Tuple[Any, str]]) -> List[Dict[str, Any]]:
    ...
    for build, arch in cases:
        ...
        entries.append({"key": kernel.name, "arch": arch,
...
def _demo() -> int:
    ...
    print(f"CBOR bundle: {len(blob)} bytes   vs JSON recipes total: {json_size} bytes "
...
def _main(argv: Optional[List[str]] = None) -> int:
    ...
    bnd = sub.add_parser("bundle", help="pack recipes (json[:key[:arch]]) -> CBOR bundle")
    bnd.add_argument("cbor_out")
    bnd.add_argument("recipes", nargs="+",
                     help="path/to/recipe.json[:key[:arch]] (key defaults to kernel_name_fmt)")

    rd = sub.add_parser("record-demo",
    ...
    if args.cmd == "bundle":
        ...
        for spec in args.recipes:
            ...
            with open(path) as f:
                recipe = json.load(f)
            key = parts[1] if len(parts) > 1 and parts[1] else recipe.get(
                "kernel_name_fmt", path)
            arch = parts[2] if len(parts) > 2 and parts[2] else "gfx950"
            entries.append({"key": key, "arch": arch,
                            "family": recipe.get("kernel_name_fmt", key), "recipe": recipe})
        n = write_bundle(args.cbor_out, entries)
        print(f"wrote {args.cbor_out}: {n} bytes, {len(entries)} entries "
