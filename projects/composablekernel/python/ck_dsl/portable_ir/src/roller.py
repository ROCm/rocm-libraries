...
def _use_slots(instr: Dict[str, Any]):
    ...
    if op == "emit":
        for i in range(len(instr.get("in", []))):
            yield (lambda i=i: instr["in"][i], lambda v, i=i: instr["in"].__setitem__(i, v))
    elif op == "scf_for":
        for key in ("lo", "hi", "step"):
            yield (lambda key=key: instr[key], lambda v, key=key: instr.__setitem__(key, v))
...
def _is_const_int(instr: Dict[str, Any]) -> bool:
    return (instr.get("op") == "emit" and instr.get("opcode") == "arith.constant"
...
def _sig(instr: Dict[str, Any]) -> Tuple:
    ...
    if op == "emit":
        ...
        return (op, instr["opcode"], len(instr.get("in", [])),
                ("out" in instr), len(instr.get("outs", [])), akeys)
    if op == "scf_for":
...
# --------------------------------------------------------------------------
def _linear_expr(axis: str, a0: int, v0: int, a1: int, v1: int, var: bool = False) -> Optional[Any]:
...
def _merge_instr(a: Dict[str, Any], b: Dict[str, Any], axis: str, a0: int, a1: int,
                 align):
        ...
        if aa != ba:
            ...
            for key in aa:
                ...
                if (av.get("t") == "i" and bv.get("t") == "i"
                        and isinstance(av.get("v"), int) and isinstance(bv.get("v"), int)):
...
def _addr_keyer(body: List[Dict[str, Any]]):
    ...
    def key(name, depth=0):
        ...
        return (instr.get("opcode", instr["op"]),
...
def lane_label_body(body: List[Dict[str, Any]], yields: List[str],
                    n_lanes: int) -> Dict[int, Any]:
...
def _divergence_reason(la: List[Dict[str, Any]], lb: List[Dict[str, Any]]) -> str:
    ...
    if a.get("op") == "scf_for" and b.get("op") == "scf_for":
        ...
        if na != nb:
            return (base + f": runtime scf.for iter-arg arity scales with axis "
...
def _run_candidates(la: List[Dict[str, Any]], lb: List[Dict[str, Any]]):
    ...
    for L in range(1, max(f, len(lb) - f) + 1):
        ...
        run_start = f
        while (run_start - L >= 0 and sa[run_start - L:run_start] == blockpat
...
def _roll_run(mid_a: List[Dict[str, Any]], mid_b: List[Dict[str, Any]],
              axis: str, a0: int, a1: int,               varname: str,
              L: int, n_a: int, n_b: int,
    ...
    b1_def_at = {(j, k): d for j, instr in enumerate(block1)
                 for k, d in enumerate(_defs(instr))}
    last_def_at = {(j, k): d for j, instr in enumerate(last)
                   for k, d in enumerate(_defs(instr))}
    ...
    prologue = [{"op": "alias", "bind": info["name"], "from": info["init"]}
                for info in carries.values()]
    hi = count_expr if start == 0 else {"add": [count_expr, start]}
    static_for = {"op": "static_for", "var": varname, "lo": start, "hi": hi,
                  "step": 1, "body": template}
...
class _Roller:
    ...
    def align(self, la: List[Dict[str, Any]], lb: List[Dict[str, Any]]
            ...
            for a, b in zip(la, lb):
                ...
                if m is None:
                    if (_is_const_int(a) and _is_const_int(b)
                            ...
                            and _linear_expr(self.axis, self.a0, _const_int(a),
                                             self.a1, _const_int(b)) is None):
                        self._fail(
                        ...
                        self._fail(f"merge {a.get('op')}/{a.get('opcode','')} vs "
        ...
        if len(lb) < len(la):
            self._fail(f"shorter-at-larger-axis (|la|={len(la)} > |lb|={len(lb)}); "
        ...
        while f < m and _sig(la[f]) == _sig(lb[f]):
            f += 1
        if (f < m and la[f].get("op") == "scf_for" and lb[f].get("op") == "scf_for"
        ...
        for run_start, a_end, run_end_b, L, n_a, n_b in cands:
            ...
            def _restore():
                self.lane_refs, self.result_refs, self._run, self._fan = (
                    dict(snap[0]), dict(snap[1]), snap[2], snap[3])
            ...
            rolled = _roll_run(la[run_start:a_end], mid_b,
                               self.axis, self.a0, self.a1, varname, L, n_a, n_b,
        ...
        self._fail(f"no run candidate rolled at level: |la|={len(la)} |lb|={len(lb)} "
                   f"({len(cands)} candidates tried); first ops="
                   + ",".join(i.get("opcode", i["op"]) for i in lb[:4]))
        return None

    def _lane_families(self, mid_b: List[Dict[str, Any]], suffix_b: List[Dict[str, Any]],
                       L: int, n_b: int, varname: str) -> Dict[Tuple[int, int], str]:
    ...
    def _roll_fan_body(self, full_a: List[Dict[str, Any]], full_b: List[Dict[str, Any]],
                       n_a: int, n_b: int) -> Optional[List[Dict[str, Any]]]:
        ...
        for (ka, pa, _sa, _ea), (kb, pb, _sb, eb) in zip(segs_a, segs_b):
            ...
            if kb == "shared":
                ...
                rolled = _roll_run(mid_a, mid_b, self.axis, self.a0, self.a1,
                                   varname, L, n_a, n_b, lane_refs=self.lane_refs)
                if rolled is None:
                    return None
                run_instrs, suffix_rewrite = rolled
    ...
    def _roll_fan(self, a: Dict[str, Any], b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not body_a or not body_b:
            return None
        if body_a[-1].get("opcode") != "scf.yield" or body_b[-1].get("opcode") != "scf.yield":
            return None
        yield_b = body_b[-1]
        ...
        new_yield = {"op": "emit", "opcode": "scf.yield",
...
def roll_two(recipe_a: Dict[str, Any], recipe_b: Dict[str, Any], axis: str,
             a0: int, a1: int, spec_decl: List[Dict[str, str]],
             name_fmt: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "kernel_name_fmt": name_fmt or _parameterize_name(
            recipe_a.get("kernel_name_fmt", ""), recipe_b.get("kernel_name_fmt", ""),
            axis, a0, a1),
        "spec": spec_decl,
