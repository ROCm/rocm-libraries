...
# HSACO equivalence to the C VM is covered by run_cbor_bundle_demo.sh.
#
#   python3 -m unittest ck_dsl.portable_ir.tests.test_recipe_bundle
import math
import unittest
...
class TestCborCodec(unittest.TestCase):
    ...
    def test_scalars(self):
        for v in [None, True, False, 0, 1, 23, 24, 255, 256, 65535, 65536,
                  2**31, 2**32, -1, -24, -256, -(2**32)]:
            self._roundtrip(v)
    ...
    def test_strings_unicode(self):
        for s in ["", "f32", "ckdsl_mini_attn_norm0_f32", "Δμ→λ", "a" * 300]:
            self._roundtrip(s)
    ...
    def test_recipe_like(self):
        recipe = {
            ...
            "program": [
                {"op": "param", "name": "Q", "type": {"kind": "ptr", "pointee": "f32",
                                                      "space": "global"}, "bind": "Q",
                 "attrs": {"noalias": True, "align": 16}},
                {"op": "emit", "opcode": "tile.inline_asm",
                 "outs": [{"bind": "r0", "type": "i32"}, {"bind": "r1", "type": "i32"}]},
                {"op": "emit", "opcode": "arith.constant",
                 "out": {"bind": "c", "type": "f32"}, "attrs": {"value": {"t": "f", "v": -1e30}}},
                {"op": "ret"},
            ],
        }
...
class TestBundle(unittest.TestCase):
    def test_build_and_lookup(self):
        entries = [
            {"key": "k_a", "arch": "gfx950", "recipe": {"schema": "ck.dsl.recipe/v1", "x": 1}},
            {"key": "k_a", "arch": "gfx942", "recipe": {"schema": "ck.dsl.recipe/v1", "x": 2}},
            {"key": "k_b", "arch": "gfx950", "recipe": {"schema": "ck.dsl.recipe/v1", "x": 3}},
        ]
...
class TestConcreteRecordBundle(unittest.TestCase):
    def test_record_concrete_bundle_roundtrip(self):
        ...
        self.assertEqual([e["key"] for e in entries],
                         ["ckdsl_mini_attn_norm0_f32", "ckdsl_mini_attn_norm1_f32"])
        bundle = cbor_decode(cbor_encode(build_bundle(entries)))
        r = bundle_lookup(bundle, "ckdsl_mini_attn_norm1_f32", "gfx950")
        self.assertEqual(r["schema"], "ck.dsl.recipe/v1")
