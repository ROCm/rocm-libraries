...
# wrong.
#
#   python3 -m unittest ck_dsl.portable_ir.tests.test_recording_builder
...
def build_forloop(b):
    ...
    loop = b.scf_for_iter(lo, hi, st, [("acc", acc0)], iv_name="k0",
...
def build_inline_asm_multi(b):
    ...
    tid = b.thread_id_x()
    a = b.const_i32(7)
    outs = b.inline_asm_multi("v_swap_b32 $0, $1, $2, $2", "=v,=v,v",
                              [a], result_types=[I32, I32])
...
class TestRecordingBuilder(unittest.TestCase):
    def test_non_invasive(self):
        ...
        for name, fn in KERNELS.items():
            with self.subTest(kernel=name):
                ...
                fn(rec)
                self.assertEqual(print_ir(plain.kernel), print_ir(rec.kernel),
        ...
        for name, fn in KERNELS.items():
            with self.subTest(kernel=name):
                ...
                self.assertEqual(recipe, expected,
                                 f"{name}: recorded recipe diverged from KernelDef")
    ...
    def test_no_ops_dropped(self):
        ...
        for name, fn in KERNELS.items():
            with self.subTest(kernel=name):
                rb = RecordingIRBuilder(name)
                fn(rb)
                body_instrs = [i for i in rb.recipe()["program"] if i.get("op") != "param"]
                self.assertEqual(_count_instrs(body_instrs), _count_ops(rb.kernel.body.ops),
    ...
    def test_params_captured(self):
        ...
        self.assertEqual(params[0]["attrs"], {"noalias": True, "readonly": True, "align": 16})
    ...
    def test_multi_result_captured(self):
        ...
        asm = [i for i in rb.recipe()["program"]
    ...
    def test_region_nesting(self):
        ...
        self.assertTrue(any(i["op"] == "scf_if" for i in scf_for["body"]),
...
def _production_cases():
    ...
    return [
        ...
        ("elementwise_silu", elementwise,
         lambda: elementwise.build_elementwise(
             elementwise.ElementwiseSpec(op="silu", dtype="bf16", block_size=64, vec=8))),
        ("reduce_sum_f16", reduce_mod,
         lambda: reduce_mod.build_reduce2d(
             reduce_mod.Reduce2DSpec(n_per_block=4096, op="sum", block_size=256,
                                     vec=4, dtype="f16", wave_size=64))),
    ]
...
class TestRecordingProductionKernels(unittest.TestCase):
    ...
    def test_records_production_kernels(self):
        ...
        for name, module, build in _production_cases():
            with self.subTest(kernel=name):
                ...
                self.assertEqual(recorded, kernel_to_recipe(kernel),
    ...
    def test_matches_legacy_recipe(self):
        ...
        for name, module, build in _production_cases():
            with self.subTest(kernel=name):
                kernel, recorded = record_kernel(build, module)
                self.assertEqual(recorded, kerneldef_to_recipe(kernel),
...
class TestRecordCoverage(unittest.TestCase):
    ...
    def test_no_recorder_failures_across_surface(self):
        ...
        paths = sorted(p for p in (os.path.join(record_coverage._PARITY_DIR, f)
                                   for f in os.listdir(record_coverage._PARITY_DIR))
