...
"""Static config for the CK_DSL remote-test orchestrator.

Kept as plain Python (no PyYAML dep) so it imports cleanly inside the
ck_dsl package. Override any field on the CLI with --set key=value.
"""
...
CK_PY_ROOT = Path(__file__).resolve().parents[3]  # .../composablekernel/python
REPO_ROOT = CK_PY_ROOT.parents[2]                 # rocm-libraries/
...
class ArchProfile:
    ...
    arch: str                       # e.g. "gfx942"
    example_module: str             # ck_dsl.examples.<arch>.<mod>
...
# site-specific path.
_SSH_KEY = os.environ.get("CKDSL_REMOTE_KEY", "" if _ALIAS_MODE else
                          str(Path(os.environ.get("HOME", "")) / ".ssh/id_ed25519"))

REMOTE = RemoteHost(
    user=_REMOTE_USER,
    host=_REMOTE_HOST,
    stage_root=os.environ.get("CKDSL_REMOTE_STAGE", "~/ckdsl_remote"),
    ssh_key=_SSH_KEY,
)
...
ARCHES: Dict[str, ArchProfile] = {
    ...
    "gfx942": ArchProfile(
        arch="gfx942",
        example_module="ck_dsl.examples.gfx942.gemm_demo",
        example_args=["--m", "256", "--n", "256", "--k", "256"],
        slurm_constraint="GFX942&MARKHAM",
    ),
    "gfx1151": ArchProfile(
        arch="gfx1151",
    ...
    "gfx1201": ArchProfile(
        arch="gfx1201",
        example_module="ck_dsl.examples.gfx1201.matmul_nbits_verify",
        example_args=[
            "--m", "128",
            "--n", "4096",
            "--k", "4096",
        ...
        slurm_constraint="GFX1201&MARKHAM",
