...
"""Push staged artifacts + a slim copy of the ck_dsl python tree to the
...
# ck_dsl/__init__ pulls in helpers->analysis->benchmark->... so it's easier
...
def push_ck_dsl_tree() -> str:
    """Rsync ck_dsl/ to <stage_root>/ck_pkg/ck_dsl/ on the login node."""
    remote_pkg = f"{config.remote_stage_root()}/ck_pkg"
    transport.ssh_run(["mkdir", "-p", f"{remote_pkg}/ck_dsl"])
    transport.rsync_push(
        config.CK_PY_ROOT / "ck_dsl",
        f"{remote_pkg}/ck_dsl",
        delete=True,
        extra=_PKG_EXCLUDES,
    )
