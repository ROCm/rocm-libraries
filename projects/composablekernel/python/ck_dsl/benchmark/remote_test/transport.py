...
_CTRL_DIR = Path(tempfile.gettempdir()) / f"ckdsl_ssh_{os.getuid()}"
...
def _target() -> str:
    if not config.REMOTE.host:
        raise RuntimeError(
            "Remote host not configured. Set CKDSL_REMOTE_HOST to either an "
            "SSH alias from ~/.ssh/config (recommended) or a real hostname. "
            "If passing a real hostname, also set CKDSL_REMOTE_USER."
        )
