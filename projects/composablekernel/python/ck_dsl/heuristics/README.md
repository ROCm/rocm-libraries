# ck_dsl Heuristics: ML-Based Kernel Selection

Fast, accurate kernel selection for `ck_dsl` operations using LightGBM
...
models used with `ck_dsl` -- a self-contained toolkit to **train, retrain,
...
- **Run scripts from this directory** (flat intra-package imports); it is a
  standalone tool dir, not imported as a deep `ck_dsl.heuristics` sub-package.

## Quick Start

### 1. Generate benchmark data with the ck_dsl sweep

Generate training data natively via `ck_dsl` -- no CK Tile / CMake / ninja
...
| File | Purpose |
|---|---|
| `gen_gemm_sweep_data.py` | ck_dsl-native generator: enumerate shapes + `UniversalGemmSpec` variants, build (`build_all_instances`) + run (`sweep_run`), write training parquet |
...
| `DATA_GENERATION.md` | Detailed guide for generating training data via the ck_dsl sweep |
...
- **[DATA_GENERATION.md](DATA_GENERATION.md)**: Complete guide for generating training
  data via the ck_dsl sweep (`gen_gemm_sweep_data.py`), managing datasets, and troubleshooting
