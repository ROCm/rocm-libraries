...
"""
ck_dsl-native GEMM heuristics training-data generator (GEMM shim).

The op-parameterized superset lives in :mod:`ck_dsl.heuristics.gen_sweep_data`
...
and keeps the ``python3 -m ck_dsl.heuristics.gen_gemm_sweep_data`` CLI working
unchanged.

This is the ck_dsl replacement for the old CK-Tile ``generate_benchmark_data.py``
...
the ck_dsl sweep ecosystem end-to-end:
  ...
  2. Enumerate *kernel-config variants* as :class:`UniversalGemmSpec` objects via
     :func:`ck_dsl.instances.all_dispatcher_configs` (validity-filtered by
     ``is_valid_spec``).
  3. Build every variant on the fly with
     :func:`ck_dsl.sweep.build_all_instances` (LLVM IR -> HSACO, cached).
  4. Run each ``(variant, shape)`` through
     :func:`ck_dsl.sweep_bench.sweep_run` to record per-shape TFLOPS +
     correctness.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "ck_dsl-native GEMM heuristics training-data generator "
            "(drives ck_dsl.sweep + ck_dsl.sweep_bench, emits training parquet)."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=Path("/tmp/ck_dsl_sweep_cache"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional C++ launcher; omit to use python -m ck_dsl.run_manifest.",
    )
