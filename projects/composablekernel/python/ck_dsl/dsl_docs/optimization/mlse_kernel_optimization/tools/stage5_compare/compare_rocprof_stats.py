...
def parse_kernel_trace(filepath):
    ...
    with open(filepath) as f:
        ...
        rows = [r for r in reader if r['Kind'] == 'KERNEL_DISPATCH' and 'ckdsl' in r.get('Kernel_Name', '').lower() or 'grouped_conv' in r.get('Kernel_Name', '').lower()]
        ...
        return {
            ...
            'workgroup_size': int(row['Workgroup_Size_X']) * int(row['Workgroup_Size_Y']) * int(row['Workgroup_Size_Z']),
...
cktile = parse_kernel_trace('/workspace/FlyDSL_CKTile_conv_comparison/comparison/ck_tile/rocprof_stats/run_kernel_trace.csv')
...
# LDS
print(f"{'LDS (bytes)':<20} {ckdsl['lds_bytes']:<20} {cktile['lds_bytes']:<20} ", end="")
lds_diff_pct = (ckdsl['lds_bytes'] / cktile['lds_bytes'] - 1) * 100 if cktile['lds_bytes'] > 0 else 0
print(f"({lds_diff_pct:+.1f}%)")

# Workgroup size
print(f"{'Workgroup size':<20} {ckdsl['workgroup_size']:<20} {cktile['workgroup_size']:<20} ", end="")
...
ckdsl_occ, ckdsl_vgpr_lim, ckdsl_lds_lim = calculate_occupancy(ckdsl['vgpr'], ckdsl['agpr'], ckdsl['lds_bytes'])
cktile_occ, cktile_vgpr_lim, cktile_lds_lim = calculate_occupancy(cktile['vgpr'], cktile['agpr'], cktile['lds_bytes'])

print(f"{'Framework':<20} {'VGPR Limit':<15} {'LDS Limit':<15} {'Occupancy':<15} {'Percentage':<15}")
print("-" * 90)
print(f"{'CK DSL':<20} {ckdsl_vgpr_lim:<15} {ckdsl_lds_lim:<15} {ckdsl_occ:<15} {ckdsl_occ/16*100:.1f}%")
print(f"{'CK Tile C++':<20} {cktile_vgpr_lim:<15} {cktile_lds_lim:<15} {cktile_occ:<15} {cktile_occ/16*100:.1f}%")
...
if ckdsl['vgpr'] < cktile['vgpr']:
    ...
    print(f"  → CK DSL should have HIGHER occupancy ({ckdsl_occ} vs {cktile_occ} waves/CU)")
    if ckdsl_occ > cktile_occ:
        print(f"  → Yet CK DSL is SLOWER! This confirms: occupancy is NOT the bottleneck")
        print(f"  → The gap must be from: LDS bank conflicts, memory access patterns, or instruction scheduling")
else:
    print(f"⚠ CK DSL has similar or worse VGPR usage ({ckdsl['vgpr']} vs {cktile['vgpr']})")

if ckdsl['lds_bytes'] > cktile['lds_bytes']:
    print(f"⚠ CK DSL uses {lds_diff_pct:+.1f}% MORE LDS ({ckdsl['lds_bytes']} vs {cktile['lds_bytes']} bytes)")
    print(f"  → Potential LDS pressure or different allocation strategy")
elif ckdsl['lds_bytes'] < cktile['lds_bytes']:
    print(f"✓ CK DSL uses {abs(lds_diff_pct):.1f}% LESS LDS ({ckdsl['lds_bytes']} vs {cktile['lds_bytes']} bytes)")
