...
OUT_DIR = Path("/workspace/mlse-tools-internal/performance/kernel_optimization/comparison/ck_dsl_profile")
...
def profile_config(name, pipeline, async_dma):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = make_conv_manifest(
            ...
            atoms=[f"tile.mfma_f32_{spec.warp_tile_m}x{spec.warp_tile_n}x{spec.warp_tile_k}_f16"],
            notes=f"{name}",
        )
        ...
        cmd = [
            ...
            '--',
            'python3',
            '/workspace/rocm-libraries/projects/composablekernel/python/ck_dsl/run_manifest.py',
            str(manifest_path),
        ]

        result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
        if result.returncode != 0:
            print(f"  ERROR: rocprof failed")
            print(result.stderr)
            return None
        ...
        if not stats_csv.exists():
            print(f"  ERROR: No stats CSV found")
            return None
        ...
        stats = {
            ...
            'occupancy': float(row.get('OccupancyPct', 0)) if row.get('OccupancyPct') else None,
            'avg_time_ns': float(row.get('AverageNs', 0)) if row.get('AverageNs') else None,
        }
...
if __name__ == "__main__":
    ...
    print("="*60)
    print(f"{'Config':<15} {'VGPRs':<8} {'SGPRs':<8} {'AGPRs':<8} {'LDS (KB)':<10} {'Occ %':<8}")
    print("-"*60)

    for s in results:
        lds_kb = s['lds_bytes'] / 1024 if s['lds_bytes'] else 0
        print(f"{s['name']:<15} {s['vgpr'] or 'N/A':<8} {s['sgpr'] or 'N/A':<8} "
    ...
    if results:
        ...
        print(f"  VGPRs: {s['vgpr']} → allows up to {512 // (((s['vgpr']-1)//4 + 1)*4)} waves")
        print(f"  SGPRs: {s['sgpr']} → allows up to {800 // (((s['sgpr']-1)//8 + 1)*8)} waves")
        print(f"  AGPRs: {s['agpr']} → allows up to {256 // (((s['agpr']-1)//4 + 1)*4) if s['agpr'] else 'unlimited'} waves")
        print(f"  LDS: {s['lds_bytes']} → allows up to {163840 // s['lds_bytes']} workgroups/CU")
        ...
        print(f"  Estimated VGPRs: {est_vgpr} → allows up to {512 // (((est_vgpr-1)//4 + 1)*4)} waves")
        print(f"  AGPRs: {est_agpr} → allows up to {256 // (((est_agpr-1)//4 + 1)*4) if est_agpr else 'unlimited'} waves")

        if 512 // (((est_vgpr-1)//4 + 1)*4) < 6:
            print(f"  ⚠️  WARNING: Estimated VGPR usage may reduce occupancy below 6 waves/CU")
            print(f"      Consider partial unrolling instead of full unrolling")
