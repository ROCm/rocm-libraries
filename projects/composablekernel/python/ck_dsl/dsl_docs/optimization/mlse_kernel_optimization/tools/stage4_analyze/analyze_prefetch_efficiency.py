...
class VMemCounters:
    ...
    def vmem_stall_pct(self) -> float:
        ...
        return 100.0 * self.wait_inst_vmem / self.total_cycles if self.total_cycles else 0.0
...
class BufferLoadStats:
    ...
    prefetch_distance: Optional[int] = None  # Detected prefetch distance in loop iterations

    @property
    def total_loads(self) -> int:
        return (self.buffer_load_dword + self.buffer_load_dwordx2 +
                self.buffer_load_dwordx3 + self.buffer_load_dwordx4 +
                self.global_load_dword + self.global_load_dwordx2 +
...
def parse_isa_for_loads(isa_path: Path) -> Tuple[BufferLoadStats, bool]:
    ...
    for i, line in enumerate(lines):
        ...
        if 'async' in line_lower and ('buffer_load' in line_lower or 'global_load' in line_lower):
    ...
    if len(loop_blocks) >= 3:
        ...
        for i in range(len(loop_blocks)):
            ...
            loads_per_block[i] = sum(1 for line in block_lines if 'buffer_load' in line.lower() or 'global_load' in line.lower())
    ...
    if loop_blocks:
        ...
        for i, line in enumerate(first_block):
            if first_load_idx is None and ('buffer_load' in line.lower() or 'global_load' in line.lower()):
                first_load_idx = i
        ...
        if first_load_idx is not None and first_mfma_idx is not None and first_mfma_idx > first_load_idx:
            stats.prefetch_distance = first_mfma_idx - first_load_idx

    return stats, has_pipelining


def calculate_efficiency_score(vmem_counters: VMemCounters, buffer_stats: BufferLoadStats) -> float:
...
def estimate_performance_gain(bottleneck: str, current_efficiency: float, arch: str) -> float:
...
def generate_recommendations(
    ...
    if efficiency_score >= 80:
        recs.append(f"✅ Excellent prefetch efficiency ({efficiency_score:.1f}/100) - well optimized!")
    elif efficiency_score >= 60:
        recs.append(f"✅ Good prefetch efficiency ({efficiency_score:.1f}/100) - minor improvements possible")
    elif efficiency_score >= 40:
        recs.append(f"⚠️ Moderate prefetch efficiency ({efficiency_score:.1f}/100) - optimization recommended")
    else:
        recs.append(f"🔴 Poor prefetch efficiency ({efficiency_score:.1f}/100) - significant optimization needed!")
    ...
    if bottleneck == "latency-bound":
        ...
        recs.append(
            f"💡 Memory bandwidth is limiting performance - optimize data movement"
        )
        ...
        recs.append("✅ Compute-bound - memory latency is well hidden. Focus on compute optimizations.")
    ...
    if estimated_gain > 0:
        recs.append(f"📈 Estimated performance gain: ~{estimated_gain:.1f}% from prefetch optimization")
...
def print_analysis(analysis: PrefetchAnalysis):
    ...
    print(f"  Avg wait/VMEM inst:    {analysis.vmem_counters.avg_wait_per_vmem:.1f} cycles")
    print()
    ...
    print(f"  global_load_*:         {stats.global_load_dword + stats.global_load_dwordx2 + stats.global_load_dwordx4}")
    print(f"  Total loads:           {stats.total_loads}")
    print(f"  Avg load width:        {stats.avg_load_width_bytes:.1f} bytes")
    print(f"  Wide load %:           {stats.wide_load_pct:.1f}% (128-bit)")
    print(f"  Async copy:            {'✅ Yes' if stats.async_copy_detected else '❌ No'}")
  ...
  %(prog)s --rocprof kernel_stats.csv --arch gfx950
    ...
    try:
        ...
        if args.isa:
            ...
            buffer_stats = BufferLoadStats()
            has_pipelining = False
            print("Warning: No ISA file provided - load pattern analysis will be limited", file=sys.stderr)
        ...
        estimated_gain = estimate_performance_gain(bottleneck, efficiency_score, args.arch)

        # Generate recommendations
        recommendations = generate_recommendations(
            vmem_counters, buffer_stats, efficiency_score, bottleneck, args.arch, has_pipelining
        )
