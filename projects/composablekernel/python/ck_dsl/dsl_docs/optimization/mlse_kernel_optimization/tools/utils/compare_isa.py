...
def count_instructions(isa_file):
    ...
    counts['s_sched_group_barrier'] = len(re.findall(r'\bs_sched_group_barrier', isa_text))
...
def main():
    ck_dsl_isa = Path("/workspace/mlse-tools-internal/performance/kernel_optimization/comparison/ck_dsl_isa/mem_sync.s")
    flydsl_isa = Path("/workspace/FlyDSL_CKTile_conv_comparison/comparison/flydsl/isa_latest/igemm_conv2d_kernel_0/21_final_isa.s")
    ...
    # MFMA ratio
    mfma_ratio = fly_counts['v_mfma_total'] / ck_counts['v_mfma_total'] if ck_counts['v_mfma_total'] > 0 else 0
    ...
    # Scheduling hints
    if fly_counts['s_sched_group_barrier'] > 0 and ck_counts['s_sched_group_barrier'] == 0:
    ...
    # LDS pattern
    lds_reads_per_mfma_ck = ck_counts['ds_read'] / ck_counts['v_mfma_total'] if ck_counts['v_mfma_total'] > 0 else 0
    lds_reads_per_mfma_fly = fly_counts['ds_read'] / fly_counts['v_mfma_total'] if fly_counts['v_mfma_total'] > 0 else 0
    print(f"\n3. LDS reads per MFMA: CK DSL {lds_reads_per_mfma_ck:.2f}, FlyDSL {lds_reads_per_mfma_fly:.2f}")

    # Wait counts
    waits_per_mfma_ck = ck_counts['s_waitcnt'] / ck_counts['v_mfma_total'] if ck_counts['v_mfma_total'] > 0 else 0
    waits_per_mfma_fly = fly_counts['s_waitcnt'] / fly_counts['v_mfma_total'] if fly_counts['v_mfma_total'] > 0 else 0
    ...
    # Code size
    size_ratio = fly_counts['total_lines'] / ck_counts['total_lines'] if ck_counts['total_lines'] > 0 else 0
    print(f"\n5. Code size: FlyDSL is {size_ratio:.1f}× larger ({fly_counts['total_lines']} vs {ck_counts['total_lines']} lines)")
    print(f"   MFMA density: CK DSL {ck_counts['v_mfma_total']/ck_counts['total_lines']*100:.1f}% MFMAs/line")
    print(f"   MFMA density: FlyDSL {fly_counts['v_mfma_total']/fly_counts['total_lines']*100:.1f}% MFMAs/line")
    ...
    for i, rec in enumerate(sorted(recommendations, key=lambda x: x['priority']), 1):
        print(f"{i}. **{rec['name']}** (Impact: {rec['impact']}, Effort: {rec['effort']})")
        print(f"   {rec['desc']}\n")
