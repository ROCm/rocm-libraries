...
class ISAInstructionCounter:
    ...
    def find_mfma_contexts(self, context_lines: int = 10) -> List[Tuple[int, List[str]]]:
...
Examples:
  ...
  %(prog)s kernel.s --mfma-context 15
    ...
    parser.add_argument('isa_file2', type=Path, nargs='?', help='Second ISA file for comparison')
    ...
    parser.add_argument('--mfma-context', type=int, metavar='N',
                       help='Show N lines of context around each MFMA instruction')
    parser.add_argument('--loop-structure', action='store_true',
                       help='Show loop labels and branches')
