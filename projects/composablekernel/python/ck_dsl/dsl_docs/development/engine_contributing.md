...
`ck_dsl/helpers/`, `ck_dsl/instances/`, or `ck_dsl_c/`, read this first — it
...
| **C++ engine** | `ck_dsl_c/` | A faithful port of the same lowering, compiled into `libckc_core.a`. It is what the hipDNN provider links so kernels can be served with **no Python at runtime**. |
...
cmake -S "$PY/ck_dsl_c" -B /tmp/ckc_verify -DCMAKE_BUILD_TYPE=Release
cmake --build /tmp/ckc_verify -j

# 2. Run the differential .ll gate (default archive is /tmp/ckc_verify/libckc_core.a).
PYTHONPATH="$PY" python3 -m ck_dsl_c.tests.differential.run_diff --mode ll
```
...
PYTHONPATH="$PY" python3 -m ck_dsl_c.tests.differential.run_diff --mode ll --only gemm

# See *structural* (real) drift vs incidental SSA-numbering drift.
PYTHONPATH="$PY" python3 -m ck_dsl_c.tests.differential.run_diff --mode ir --canonical --only gemm
```
...
```bash
ck_dsl_c/tools/check_byte_identity.sh --ref-pyroot <reference-tree-python> --ref-shim <stubs>
```
...
PYTHONPATH="$PY" python3 -m ck_dsl_c.tests.differential.run_diff --check-golden

# Re-bless — ONLY from a green state, as part of a reviewed, intended change.
PYTHONPATH="$PY" python3 -m ck_dsl_c.tests.differential.run_diff --record-golden
```
...
(`ck_dsl_c/bindings/`), which exposes each family's lowering to Python. If you add
...
mirrored path in `ck_dsl_c/src/` (the directory layout mirrors the Python module
...
```bash
ck_dsl_c/tools/build.sh        # see ck_dsl_c/BUILD.md for options
```
