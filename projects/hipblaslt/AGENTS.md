...
- **The build never probes the local GPU; the runtime decides per device.**
  Whenever the targets can include gfx1250 (which `all`, the default, does),
  `invoke build` builds *both* revisions' GEMM trees — `library/gfx1250/` for
  the shipping part and `library/gfx1250v0/` — and the loader picks one from
  `asicRevision`. One install therefore runs on either silicon and the same
  command produces the same package on any machine. `--gfx1250-revision v0|v1`
  prunes to a single tree.
- **v0 and v1 are independent revisions, not a subset relationship** — each has
  its own tuned GEMM tree; neither is guaranteed loadable on the other's silicon.
  A v0 part loads `library/gfx1250v0/` only and does **not** fall back to the
  shipping `library/gfx1250/`; pinning `v1` therefore leaves v0 silicon with no
  GEMM library (no library beats v1 kernels a v0 part cannot run).
- **Only the directory carries the stepping.** Every file inside
  `library/gfx1250v0/` — master, Mapping, shards, helper kernels — is named for
  the compiler target, `gfx1250`, because that is what the loader forms
  filenames from once it has chosen the directory. The stepping appears in the
  directory name and nowhere else.
- ExtOp and matrix-transform are **not** split by revision: the runtime resolves
  them from `gcnArchName`, which is `gfx1250` on both parts, so both revisions
  read them from `library/gfx1250/`.
...
- The two trees are generated by two `hipblaslt_create_device_library()` calls
  with **separate output directories** (`Tensile/` and `Tensile-gfx1250v0/`),
  because a TensileCreateLibrary run writes `Kernels.cpp` and a `build_tmp`
  scratch tree directly under its output directory and skips assembling a kernel
  whose `.o` is already there. They are merged back into one `library/` root by
  the install rules.
