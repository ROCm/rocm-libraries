...
- Device generation builds and force-installs the canonical TensileLite wheel into the selected Python with `--no-deps`, exposes only its in-tree raw rocisa target through command-scoped `PYTHONPATH`, and configures the exact built client before generation. Host-only builds do not require this Python environment.
