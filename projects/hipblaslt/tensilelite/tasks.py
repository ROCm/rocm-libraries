...
def _find_amdclang(rocm_path=None):
    """Locate amdclang++ across Linux and Windows ROCm layouts."""
    # After rocm-sdk init, amdclang++ should be on PATH
    ...
    candidates = [
        ...
        os.path.join(rocm, "lib", "llvm", "bin", "amdclang++"),
        os.path.join(rocm, "bin", "amdclang++.exe"),
        os.path.join(rocm, "lib", "llvm", "bin", "amdclang++.exe"),
    ]
...
def asm(
    ...
    amdclang = _find_amdclang(rocm_path)
    if not amdclang:
