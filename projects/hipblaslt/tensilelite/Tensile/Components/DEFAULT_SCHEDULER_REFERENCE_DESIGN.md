...
With the SHADOW capture as reference (post-nmsx, post-g9fi):
- **Per-category counts** are equal by construction on data-flow categories (LR/LW/GR/MFMA/CVT_PACK/MIDDLE_PACK), modulo SYNC/SNOP which are scheduler-inserted. The MFMA parity holds because BOTH sides are per-leaf — see below.
- **MFMA per-leaf symmetry (g9fi)**: an earlier interpretation of CMS's `expand_cms_macro` claimed it walked `macro.items()` directly and therefore produced one MFMA tag per `mfmaIter` Module wrapper. Empirical inspection (BPG#11 probe) refutes that: CMS's `customMainLoopSchedule` flattens `MfmaCodeAllIters` via `removeComments(...)` (which is `.flatitems()`, `CustomSchedule/dispatch.py:73-78`) BEFORE the dispatch loop, then tags every leaf in the flat list as MFMA at `dispatch.py:235-240`. The `mfmaIter` Module wrapper is destroyed before tag assignment, so CMS emits 48 per-leaf MFMA tags on BPG#11 — not 1 per Module. The SHADOW side runs an explicit per-leaf walk over `macIterCode.flatitems()` that tags every leaf as MFMA (KernelWriter.py around the `_captureDefaultSchedule` idmap-build site). The walk MUST exist for fail-loud coverage of non-MFMAInstruction sub-leaves the `mfmaIter` Module may emit (SNop at KernelWriterAssembly.py:8367 when `s_nop != 0`, SWaitAlu at 8622, tail-loop / shiftK control ops); deleting it would surface a `CaptureCategoryMissingError` on any kernel whose `mfmaIter` is not pure MFMAInstructions. With both sides per-leaf, the MFMA count is equal by construction and needs NO count-parity exclusion (test at `test_capture_pipeline_checks.py::test_shadow_main_capture_categories_match_cms_subject`; explicit MFMA-count lock-in at `test_shadow_mfma_count_matches_cms_subject_on_bpg11`).
