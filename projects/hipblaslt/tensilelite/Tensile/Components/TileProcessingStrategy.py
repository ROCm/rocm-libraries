...
from ..Common import persistentSpatialCluster
...
class DataParallel(TileProcessingStrategy):
    ...
    def computeTotalTiles(self, writer, kernel, dstSgpr):
        if persistentSpatialCluster(kernel):
            return ClusterTileMapping.blockCount(writer, kernel, dstSgpr, paddedPeers=True)
        return super().computeTotalTiles(writer, kernel, dstSgpr)

    def tileIndexToWorkGroup(self, writer, kernel, sTmp):
        if persistentSpatialCluster(kernel):
            return ClusterTileMapping.materialize(writer, kernel, sTmp)
