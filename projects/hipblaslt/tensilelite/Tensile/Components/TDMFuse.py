...
# an integer can never be reachable while its preconditions are unwritten.
# `test_every_shipped_row_is_reachable` holds the other direction: the table
# carries no row without an integer and an entry here.
_GROUPING_ACCEPTED = {
    "MX_AB": _acceptAlways,
...
def _arrangedGroups(ks):
    ...
    if not ks.get("TDMCross", 0):
        return groups
    from .TDMCross import TDM_CROSS_CROSSED, crossGroups
    if ks["TDMCross"] != TDM_CROSS_CROSSED:
        raise ValueError("TDMCross=%r names no arrangement"
                         % (ks["TDMCross"],))
