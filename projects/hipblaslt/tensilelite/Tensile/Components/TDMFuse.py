...
is not an index into that table: TDM_FUSE_GROUPING maps it to a row name.
TDMFuse values are baked into
...
#
# tdmWaveComponents and tdmSoleWave below bound what a row may be -- a
...
TDM_GROUPS = {
    ...
    # No integer selects these and no acceptance entry covers them. They are
    # the negative case the resolver is tested against: every derived function
    # answers for a row nothing wired up, and tdmGroupingName still raises.
    "AB": TdmGrouping("AB", (("A", "B"), ("MXSA",), ("MXSB",))),
    "None": TdmGrouping("None", (("A",), ("B",), ("MXSA",), ("MXSB",))),
}
...
def tdmFusePaired(ks):
    """TDMFuse=1: {A,MXSA} and {MXSB,B}, crossed parity. NumWaves>1."""
...
def tdmWavePartition(ks, tc):
    ...
    The descriptor init, the stagger gate and the fill guards all read this, so
    they follow one arrangement automatically.
    """
