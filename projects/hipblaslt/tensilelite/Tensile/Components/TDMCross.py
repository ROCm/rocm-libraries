...
# two are implemented; TDMFuse._arrangedGroups raises on any other value.
TDM_CROSS_DEFAULT = 0
TDM_CROSS_CROSSED = 1
...
def tdmCross(ks):
    ...
    return ks.get("TDMCross", TDM_CROSS_DEFAULT)
...
def tdmCrossRejectReason(ks):
    ...
    inherits the right answer. Crossing preserves total LDS exactly and only
    moves which wave fills which part, but a divergent per-tensor prefetch
    depth is still refused below: that depth is applied by the pre-crossing
    pairing, which crossing then contradicts.
    """
    # Imported here so this module stays free of a load-time dependency on the
    # grouping it arranges.
