...
from .TDMFuse import (liveGroups, tdmGroupingSeparatesAB, tdmMemberIsLive,
                      tdmSeparateABDescriptors, tdmSetOwner)
...
def pgrAutoPairRequested(state):
    ...
    -1, or one key on -1 against a real depth. A pair-auto form with
    PrefetchGlobalRead 0 or 1 drops the keys instead of searching, so it is not
    auto here and has nothing to step to.
    """
...
def resolvePrefetchGlobalReadSpecialValues(state, skip=0):
    ...
    (-1, -1): pick the max-LDS pair starting at PrefetchGlobalRead. Auto is
    asked for per tensor, so a bare PrefetchGlobalRead=-1 requests nothing and
    leaves the legacy scalar path to derive -- a pair is never reached without
    a key naming one.
...
def divergentPairUnsupportedReason(ks):
    ...
    if ks["_ScheduleIterAlg"] != 0:
        # What "cannot be moved" means, concretely: _dcpScheduleSingleBufferedFillLate
        # lifts the thin tensor out of the unroll loop header by identity, which
        # only works while noSchedGlobalRead has put the whole globalReadA /
        # globalReadB modules there. Every other algorithm runs
        # prepareGRInstToSched instead, which leaves just globalRead*.header in
        # the header and hands each tensor_load_to_lds to schedGlobalRead
        # separately, so there is no fill group left to re-slot. The relocation
        # is what keeps the single-buffered tensor from refilling the block the
        # current iteration is still reading, so this is a correctness refusal
        # rather than a missed relaxation.
        return ("_ScheduleIterAlg=%u schedules the global reads one instruction at a "
                "time into the per-iteration slots, so the unroll loop header keeps "
                "no fill group to re-slot between the thin tensor's last local read "
                "and the pre-read sync; only _ScheduleIterAlg=0 leaves that group "
                "whole. ScheduleIterAlg=4 derives to it, ScheduleIterAlg=3 keeps "
                "_ScheduleIterAlg=3" % ks["_ScheduleIterAlg"])
    ...
    if ks["PrefetchLocalRead"] % loopIters == 0:
        # KernelWriter asserts on numItersPLR, which is PrefetchLocalRead %
        # LoopIters, so a multiple of LoopIters leaves 0 and puts lateIter on
        # the loop's last sub-iteration. It asserts unconditionally, so this
        # cannot be narrowed by ClusterLocalRead or ForceUnrollSubIter: both
        # are user-settable, and gating on them let ClusterLocalRead=0 walk
        # past here and raise the assert out of getSourceFileString.
        return ("PrefetchLocalRead=%u is a multiple of LoopIters=%u, and is rewritten to "
                "0 after this check, leaving no sub-iteration between the last local read "
...
def _dcpTokensGateSupported(_ks):
    """Separate descriptor sets take disjoint tensor tokens, so the thick
    tensor has only its own fill outstanding on its own counter."""
    return 1


def _dcpTextGateSupported(ks):
    """One per descriptor set a fill actually writes.

    A and B share a set on this path, so a scale-less problem issues a single
    tensor_load_to_lds per fill and one carrying MX scales issues a second for
    the scale set. A set is filled by whichever member owns it, so a set whose
    owner is absent is not filled and does not count.

    Counting a set that issues nothing is not a lost optimisation but a weaker
    gate: at one op per fill, s_wait_tensorcnt 2 retires with the previous thin
    fill and the current thick fill both still in flight, and the LDS reads
    below it read blocks that have not landed.
    """
    return sum(1 for group in liveGroups(ks)
               if tdmMemberIsLive(ks, tdmSetOwner(ks, group[0])))


# How many tensor ops one fill leaves outstanding under each mechanism. Not a
# tunable: a count above what a fill issues is a gate that retires with the
# previous fill outstanding too.
...
def decoupledThickGateRelaxation(ks):
    ...
    tensor's reads start while the thick tensor's last block is still landing.
      ...
      TEXT, N    one shared set means one token and only a full drain, which
                 KernelWriter._dcpRelaxThickTextGate relaxes afterwards to the
                 N tensor ops one fill on that grouping issues
...
def dcpThickGateFromTokenPasses(ks):
    """True when the relaxed thick gate is the token passes' to emit.

    Two passes produce that count together, and _StinkyTofuOptLevel 3 is the only
    level that turns both on: postMainLoopBarrierCheckAndReset rebuilds the
    barriers from per-tensor token transitions, and StinkyTofu's wait-count
    insertion then gates each rebuilt barrier on the tokens that barrier actually
    needs. Either one alone still emits a full drain -- without the rebuild the
    barrier still spans both tensors, and without the insertion the writer's own
    full-drain waits are what reaches the assembler.

    So both of those gates ask this instead of asking for level 3. The text
    mechanism is deliberately not included: its count comes from rewriting
    emitted text, and Solution refuses that at level 0 for its own reasons.
    """
    gate = decoupledThickGateRelaxation(ks)
    return gate is not None and gate.mechanism == DCP_THICK_GATE_TOKENS


# Public because KernelWriter._dcpRelaxThickTextGate reads it too: the
# rewriter and the audit below have to stop at the same line, or the audit
# judges a site by a gate the rewriter was never allowed to touch.
