...
from .TDMFuse import tdmGroupingSeparatesAB, tdmSeparateABDescriptors, tdmSetOwner
...
def _dcpSetsRespectLdsSides(ks):
    """True when no descriptor set straddles the two LDS sides.

    A fill is tagged with the token of the descriptor set it rides, while
    the ds_load that consumes it waits on the token of the LDS group the
    data was laid into. A set holding tensors from both sides therefore
    gates producer and consumer on different tokens.
    """
    return all(dcpLdsSide(tc) == dcpLdsSide(tdmSetOwner(ks, tc))
               for tc in DCP_LDS_SIDE)
...
def decoupledThickGateRelaxation(ks):
    ...
    tensor's reads start while the thick tensor's last block is still landing.
      ...
      None       no thin side to start early, or disjoint tokens the LDS
    ...
    if tdmGroupingSeparatesAB(ks):
        if not tdmSeparateABDescriptors(ks):
            return None
        # The thin side must be single-buffered: the relaxation skips past its
        # one refill, and a deeper thin side has no second stream to skip.
        if min(numLdsBlkA, numLdsBlkB) != 1:
            return None
        if not _dcpSetsRespectLdsSides(ks):
            return None
...
def dcpThickGateUncoveredSites(lines, marker, relaxed, accepted):
    ...
    count, or no gate and no read below it.
