...
def _grComputeAllOffsets_legacy(module, writer, tileInfo, colId, rowId, rowOffset):
  ...
  for i in range(1, len(tileInfo.sharedVgprGROffset)):
    ...
    loadWidth = tileInfo.loadWidthGR
    # The rotation below is one half of a pair: GR writes the second load of a subtile
    # at a rotated column and the reader rotates back. Under SourceSwap the swizzle is
    # off (interleaved row map), so only the FP8 path — which de-rotates unconditionally
    # — has a reader that undoes it. Off the SourceSwap path the behavior is unchanged
    # from the fold branch: rotate unless the tile is host-preshuffled.
    if writer.states.kernel.get("SourceSwap", False):
      _grDoRotate = tileInfo.loadRatioGR == 0.5 and (tileInfo.bpe == 1 or writer.states.subtileLdsSwizzle)
    else:
      _grDoRotate = tileInfo.loadRatioGR == 0.5 and not tileInfo.isPreShuffled
    if _grDoRotate:
...
def _graTileAssignment_legacy(writer, kernel, useSwizzling=True):
  ...
  if kernel.get("SourceSwap", False):
    # SourceSwap (ss1): the swizzle is replaced by the interleaved local-read map.
    # subtileLdsSwizzle is False under SourceSwap, so this takes the no-swizzle path.
    ...
    # Non-SourceSwap fold path unchanged: swizzle unless both tiles are host-preshuffled.
