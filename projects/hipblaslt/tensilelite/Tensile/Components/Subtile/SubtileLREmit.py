...
def _lraTileAssignment_legacy(writer, kernel):
  ...
  if kernel.get("SourceSwap", False):
    # SourceSwap (ss1) interleaved local-read map: each lane owns a group of VW rows
    # rather than one row, so its base advances by VW rows. A and B interleave by their
    # own VectorWidth, so each needs its own base; they share one register when the
    # factors agree.
