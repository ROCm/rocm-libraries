...
- **`Bulk-Snapshot-Update: yes` is an override marker, not a template requirement.** It exists for exactly one situation: a PR's `.ambr` diff spans more files than CI's blanket-regeneration guard (`characterization/tools/check_snapshot_diff.py`, AIHPBLAS-3876) allows, and the change is a genuine, reviewed mass update rather than an accidental or forced-green `--snapshot-update`. Add or update a numbered ADR (`NNNN-*.md`) in the *same PR* carrying this line on its own (case-insensitive, starting at column 0) in the header metadata block, next to `Status:` and before the first `##` section — the guard reads only numbered ADRs whose content changed in the PR diff (added, modified, or renamed with edits), as of the PR's head commit. A pre-existing ADR already on disk, unchanged in this PR, does not grant the override even if it already contains the marker. Neither does the field quoted in a code block, in a Nygard section's body, or in this README, so examples like the template below never grant it. Omit the line entirely for every other ADR; its mere presence on a *changed* ADR is what grants the override, so don't add it out of habit. This is a mechanics/process field like `Commit:`, not a Nygard section — it does not replace `Status`/`Context`/`Decision`/`Consequences`, which must still explain *why* the bulk change is correct.
