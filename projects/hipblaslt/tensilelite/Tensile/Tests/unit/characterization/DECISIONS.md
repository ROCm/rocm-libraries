...
**Context:** `test_emit_bigfiles_char.py`'s `test_bigfile_capped_emit` (Phase 3;
10 parametrized cases) read real production tuned-logic YAMLs directly out of
...
**Decision:** For each of the 10 `_BIG` entries, replace the live-file
reference with a vendored, trimmed, self-contained fixture checked into
...
**Verified no behavior change:** each of the 10 new fixtures was run against
the existing, untouched `.ambr` golden before the test module was updated —
zero snapshot diff in every case, confirming the golden's provenance changed
(vendored fixture vs. live file) but not its content.
