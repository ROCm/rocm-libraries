...
## D24 — Per-file baseline refresh (post `LdsPadding.py` rewrite); one file's floor lowered

**Context:** `f77c1c9ddc8` ([#11409](https://github.com/ROCm/rocm-libraries/pull/11409), "drop
odd-dword LDS padding and rerank tile-major candidate", merged 2026-09-07) rewrote
`SolutionStructs/LdsPadding.py` almost entirely after the baseline was last refreshed
(`c520f9d0`, 2026-08-13). The `TensileLite coverage / Linux` job started failing the per-file
ratchet on every `develop` run afterward — deterministically, not xdist noise — because
`LdsPadding.py`'s combined-suite line coverage moved 99.35% -> 98.27%, a drop of 1.08pp against
the gate's 1.0pp tolerance. Confirmed locally (native `amdclang++` toolchain via TheRock's
`build_tools/install_rocm_from_artifacts.py`, no docker needed): `tox -e coverage-unit` then
`tox -e coverage-gate` reproduces the identical 98.27% and the identical gate failure.

Two lines are missing out of 225 statements:
- **Line 565** (`_compute_fp32_config`, under `if best is None:`): structurally unreachable
  today. `_search_padding`'s no-padding candidate `(0, 0)` is always a legal candidate (its cost
  function never returns `None` for `B=0`), so `best` can never actually be `None`. Same
  already-accepted shape as the sibling `_b64_compute_config` / `_compute_fp16_config`, whose
  docstrings say so explicitly ("kept in case a future change narrows the candidate set").
- **Line 74** (`_even_dword_only`'s odd-dword zeroing branch): real, reachable behavior — it is
  the exact odd-dword-rejection logic #11409's motivation describes — just not yet exercised by
  a unit/characterization test.

**Decision:** Refresh `coverage-baseline.json` from a real `coverage-unit` run against current
`develop`, lowering only `LdsPadding.py`'s floor (`--allow-lower=Tensile/SolutionStructs/LdsPadding.py`)
to its current 98.27%. Accept line 74 as untested debt without opening a new tracker — a
single-line, single-branch gap in a small config helper is a credible no-tracker case; a follow-up
characterization test for the odd-dword-zeroing path is welcome but not blocking. Every other
file's floor only rises or gets newly pinned in this same refresh (real accumulated progress since
the 2026-08-13 baseline), consistent with the ratchet's add-only rule.

**Alternatives rejected:**
- Leave the floor at 99.35% and mark the job `continue-on-error` — rejected: masks a real,
  reviewable coverage change behind a weakened gate (the base PR-quality skill's M3: never weaken
  a test/gate solely to green CI).
- Write a new characterization test for line 74 in this same PR — rejected for scope: this PR is
  test-infra/baseline maintenance responding to an already-merged product change; adding new test
  coverage is better scoped to its own reviewed PR against the actual behavior it is testing.

**Consequences:** `LdsPadding.py` is an honest <99% module until a follow-up test covers line 74;
the ratchet still catches any further regression below 98.27% (minus tolerance). No production
