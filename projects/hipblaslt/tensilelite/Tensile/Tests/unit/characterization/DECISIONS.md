...
## D24 — `Contractions.py`: first arm of the deferred predicate/state matrix (isStreamK)

**Context:** ADR 0007 (D10) deferred the `ProblemPredicate.CompoundPredicates`
arms that only fire for other-problem-configurations — including the
`isStreamK` branch, which swaps the ordinary `WorkgroupNumberCheck` for
`StreamKWorkgroupNumberCheck` — for a future "Contractions matrix" slice, and
explicitly rejected hand-synthesizing a derived-solution state for that slice
("fragile, since they must match the full post-derivation key set exactly").
While fixing ROCM-31016 (a `StreamKWorkgroupNumberCheck` float-precision bug),
this branch initially got exactly that rejected treatment: a synthetic
`dict(solution_state, StreamK=1)` patched onto the vendored gfx942-HSS
fixture's derived state.

**Decision:** Replaced the synthetic patch with a second fixture chain
(`raw_streamk` / `problem_type_streamk` / `solution_state_streamk`) parsed via
the same `parseLibraryLogicFile` path, pointed at
`Tensile/Tests/unit/test_data/solution_pool_gfx950.yaml` — a real gfx950
static Stream-K (`StreamK: 3`) solution already vendored for the
BenchmarkProblems solution-pool tests (`test_LibraryIO.py`,
`test_benchmarkProblems_solution_pool.py`). `test_problem_predicate_compound_streamk`
now asserts the `isStreamK` swap against a real derived state instead of a
hand-edited one. Kept as a fully separate fixture chain from
`raw`/`problem_type`/`solution_state` (no shared helpers) so the addition
cannot change the existing fixtures' behavior.

**Why:** This reuses an already-vendored fixture rather than vendoring a new
logic YAML or hand-synthesizing state, so it avoids both alternatives ADR
0007 rejected, while covering the first of the deferred arms. It does not
close ADR 0007 — sparse, activation, bias, batched, and dtype arms remain
deferred — so ADR 0007 stays `Accepted` as-is.

**Alternatives rejected:**
- Keep the synthetic `dict(solution_state, StreamK=1)` patch — rejected per
  ADR 0007's own reasoning: a hand-edited state isn't guaranteed to match
  what `Solution` construction actually derives for a real Stream-K solution,
  so it could pass while a real one fails (or vice versa).
- Vendor a new, purpose-built gfx950 StreamK logic fixture for this test —
  rejected as unnecessary; a suitable one is already vendored and exercised
