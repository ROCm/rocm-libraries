...
from Tensile.Components.TDMCross import crossGroups
from Tensile.Components.TDMFuse import tdmMemberIsLive, liveGroups, tdmGrouping

from Tensile.Components.DecouplePGR import dcpThickThinIssueOrder
from Tensile.Components.TDMFuse import TDM_GROUPS, partitionedGroups, tdmFusePaired, tdmGroupPartner
from Tensile.Components.DecouplePGR import tdmWaveIssueOrder
from Tensile.Components.TDMCross import TDM_CROSS_CROSSED, TDM_CROSS_DEFAULT, tdmCrossRejectReason
...
def _everyShape():
    ...
    for fuse, numWaves, mxA, mxB, pgrA, pgrB, split, subtile, inst in itertools.product(
            (0, 1, 2, 3), (1, 2, 4, 8), (0, 32), (0, 32), (1, 2), (1, 2),
...
def test_a_declined_grouping_falls_back_before_the_cross_is_judged():
    ...
    grouping really is the default one and not the paired grouping the
    parameter asked for. The grouping settles before the arrangement is even
    asked about; a subtile solution is refused a cross on its own account."""
...
# ---------------------------------------------------------------------------
# Naming. The default must add no token: a token would rename all 204 kernels
# and shift dedup.
# ---------------------------------------------------------------------------
...
def test_default_adds_no_token_to_the_kernel_name():
    """A token at the default would rename all 204 kernels and shift dedup.
...
# ----------------------------------------------------------------------------
# The rest of this file reads operands off instructions the writer actually
# produced, rather than off the partition model the tests above state their
# assertions in terms of. The defect that motivated it lived entirely in the
# writer: descriptor initialisation resolved wave parity through the
# partition while increment selection resolved it from argument position, so
# at TDMCross=1 the model was right and the emitted code was wrong. The
# writer is driven through a stand-in: the functions under test reach only
# the parity helpers and the GlobalReadIncs accessor.
# ----------------------------------------------------------------------------


def _ksWriter(fuse=0, cross=TDM_CROSS_DEFAULT, numWaves=4, mxBlockA=32, mxBlockB=32,
        **overrides):
    ks = {
        "TDMFuse": fuse,
        "TDMCross": cross,
...
def test_increment_select_agrees_with_the_partition(cross, group):
    ...
    """
    kernel = _ksWriter(fuse=0, cross=cross)
...
def test_default_arrangement_pins_each_member_to_its_own_increment():
    ...
    """
    assert _selectSources(_ksWriter(fuse=0), "A", "B") == (_incs("B"), _incs("A"))
    assert _selectSources(_ksWriter(fuse=0), "MXSA", "MXSB") == \
...
def test_crossing_swaps_the_scale_sources_and_leaves_the_data_sources():
    ...
    default = _ksWriter(fuse=0, cross=TDM_CROSS_DEFAULT)
    crossed = _ksWriter(fuse=0, cross=TDM_CROSS_CROSSED)
...
def _assignment(ks):
    """{wave: (tc, ...)} built from the partition the writer itself reads.

    Group order is the arrangement's order, so the tuple per wave matches what
    the writer emits. Keeping the inversion here rather than in the module means
    no shipped helper exists only for these assertions.
    """
    groups = tdmGrouping(ks).groups
    if ks.get("TDMCross", 0):
        groups = crossGroups(groups)
    out = {w: [] for w in range(ks.get("NumWaves", 1))}
    for group in groups:
        for tc in group:
...
def _shippedWavePartition(ks, tc):
    ...
    rewritten, and rewriting it is the moment to notice that 242 kernels and
    the whole b8-3 evidence chain move with it.
    """
    numWaves = ks.get("NumWaves", 1)
    name = tdmGrouping(ks).name
    if name in ("A_MX", "B_MX"):
        # The row seats both scales on one data tensor; the other rides alone.
        fused = "A" if name == "A_MX" else "B"
        if tc == fused:
            return 2, (0, 1)
        if tc == "MXSA":
            return 1, (2,)
...
def test_default_assignment_is_the_inverse_of_the_shipped_partition():
    ...
    for ks in _everyShape():
        ...
        assert _assignment(ks) == expected, ks
...
def test_crossing_swaps_the_second_group_against_the_first(fuse, expected):
    ...
    assert _assignment(_ks(fuse=fuse, cross=TDM_CROSS_CROSSED)) == expected
...
def test_crossing_is_its_own_inverse():
    ...
    for grouping in TDM_GROUPS.values():
        assert crossGroups(crossGroups(grouping.groups)) == grouping.groups
    for fuse in (0, 1):
        assert (_assignment(_ks(fuse=fuse, cross=TDM_CROSS_CROSSED))
                != _assignment(_ks(fuse=fuse)))
...
def test_crossing_moves_no_tensor_off_the_waves_altogether():
    ...
    for fuse in (0, 1):
        plain = _assignment(_ks(fuse=fuse))
        crossed = _assignment(_ks(fuse=fuse, cross=TDM_CROSS_CROSSED))
...
def test_crossed_default_grouping_balances_every_wave():
    ...
    for members in _assignment(_ks(fuse=0, cross=TDM_CROSS_CROSSED)).values():
...
def test_live_groups_drop_dead_members_but_keep_the_wave_layout():
    ...
    assert "MXSB" not in {tc for m in _assignment(ks).values() for tc in m}
