...
################################################################################
# On-simulator (gfx1250) GPU test for the StreamK workgroup-cluster
...
# emits in Tensile/Components/StreamK.py, run on a gfx1250 simulation environment:
#
...
#     store/release/signal/wait/acquire/accumulate sequence runs on the
#     simulation environment, computes the right reduction, and does not hang; and
...
#
# Usage:
#   source <your gfx1250 simulation environment setup>
...
def _emit_reduction_module(C):
    ...
    dependence on the workgroup-id SGPR, whose value is unreliable on the
...
class TestStreamKClusterReductionOnSim:
    """gfx1250 execution of the cluster reduction fast-path handshake."""
