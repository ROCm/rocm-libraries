...
# Token-gate count is produced by passes, not by rewritten text; this emits.
...
def test_token_thick_gate_reaches_the_assembler_on_every_schedule_iter_alg(
    ...
    assert gate is not None and gate.mechanism == DCP_THICK_GATE_TOKENS, (
        "expected token-based thick gate, got %s"
        % (gate,))
...
def _thickGates(asm, thick):
    """First s_wait_tensorcnt under each thick fill label; stop at the next fill label."""
