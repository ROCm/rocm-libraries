...
def test_resolve_bare_scalar_auto_is_not_a_pair_request():
    """A pair is asked for per tensor, so a bare -1 asks for nothing.

    This shape reaches (2, 2) when both keys request it, so a scalar that still
    searched would decouple a solution whose yaml named no per-tensor depth
    anywhere. Nothing is added to the state, which leaves the legacy scalar
    path to derive it.
    """
...
def test_scalar_prefetch_global_read_does_not_accept_auto():
    """-1 is a per-tensor value, so the scalar's own list has to refuse it.

    Leaving it accepted there would keep a spelling that now resolves to no
    pair and to no prefetch depth either.
    """
...
# ---------------------------------------------------------------------------
# One-sided auto: -1 on one tensor, a real depth on the other. PGRA and PGRB
# each support -1 independently, so this runs the same max-LDS search narrowed
# to the pairs that keep the fixed tensor where the caller put it.
# ---------------------------------------------------------------------------
...
# The thick-wait tests further down drive _dcpRelaxThickTextGate with
# hand-written assembly. That is why a token-path regression could live through
# all of them: the token mechanism writes no text of its own, so a fixture can
# never show whether the passes that produce its count ran. This one emits.


def _dcpFillLabelsAndGates(asm):
    """([(line, side)] fill labels, [(line, count)] gates) of emitted assembly."""
    labels, gates = [], []
    for i, line in enumerate(asm.splitlines()):
        fill = re.match(r"\s*label_Dcp(?:Early|Late)Fill([AB])(?:End)?\s*:", line)
        if fill:
            labels.append((i, fill.group(1)))
...
@pytest.mark.parametrize("pgrA, pgrB", [(1, 2), (2, 1)])
def test_token_thick_gate_reaches_the_assembler_on_every_schedule_iter_alg(
        _gp_gfx1250, gfx1250_iim, assembler, capsys, scheduleIterAlg, pgrA, pgrB):
    """decoupledThickGateRelaxation answering ('tokens', 1) is a claim about text.

    ScheduleIterAlg=0 used to derive Valid, emit err=0 and warn about nothing
    while every gate came out a full drain. The two passes that produce the
    count between them -- the token-driven barrier rebuild and StinkyTofu's
    wait-count insertion -- were both keyed on _StinkyTofuOptLevel 3, which only
    ScheduleIterAlg=4 derives, and nothing asserted on emitted text.

    Read off the emission, and without a proximity window: the fill label names
    its own tensor, so it carries the issue order, while the gate itself lands
    at the dependence point rather than under the label.
    """
    ...
    assert sol.get("Valid") is True, out
    gate = decoupledThickGateRelaxation(sol)
    assert gate is not None and gate.mechanism == DCP_THICK_GATE_TOKENS, (
        "this fixture has to reach the token mechanism to prove anything, got %s"
        % (gate,))
    ...
    labels, gates = _dcpFillLabelsAndGates(asm)
    thick = tdmWaveIssueOrder(sol, "A", "B")[0]
    assert [side for _, side in labels] == [thick], (
        "the thick tensor is filled first and TDMFuse=1 labels only that fill, "
        "so the labels are the issue order; expected %s, got %s" % (thick, labels))

    below = [(line, count) for line, count in gates if line > labels[0][0]]
    assert below, ("the thick fill label at line %d is followed by no gate at all"
                   % labels[0][0])
    assert below[0][1] == gate.tensorcnt, (
        "thick %s at _StinkyTofuOptLevel=%s: the first gate below the fill label "
        "is s_wait_tensorcnt %d at line %d, not the relaxed %d that "
        "decoupledThickGateRelaxation promised. Every gate: %s"
        % (thick, sol.get("_StinkyTofuOptLevel"), below[0][1], below[0][0],
           gate.tensorcnt, gates))


# ---------------------------------------------------------------------------
# What the relaxed thick gate is allowed to be is what one fill issues. A and B
# share a descriptor set on the text arm, so a problem carrying MX scales fills
# two sets per fill and a scale-less one fills a single set -- and a gate that
# allows two where one is issued retires with the previous fill still in
# flight, which is an under-wait, not a missed optimisation.
# ---------------------------------------------------------------------------
...
def _scaleLessDivergent(pgrA, pgrB):
    """A scale-less divergent pair that fits the VGPR budget at both levels."""
...
def _thickGates(asm, thick):
    """The first s_wait_tensorcnt under each thick fill label, or None.

    Scanned the way _dcpRelaxThickTextGate scans -- stopping at the next fill
    label -- so a header copy with nothing under it reports None instead of
    borrowing the next region's gate.
    """
    ...
    for i, line in enumerate(lines):
        ...
        for candidate in lines[i + 1:]:
            if DP.dcpIsFillLabel(candidate):
                break
            gate = DP.DCP_TENSORCNT_RE.match(candidate)
            if gate:
                found = int(gate.group(1))
                break
        gates.append(found)
    return gates
...
def test_scale_less_divergent_pair_gates_on_its_single_fill(
        _gp_gfx1250, gfx1250_iim, assembler, capsys, pgrA, pgrB, thick):
    """One descriptor set means one tensor_load_to_lds per fill, so the gate is 1.

    Asserted on emitted assembly rather than on the helper alone: a count only
    means something if the fill it describes is the one the writer emits.
    """
    sol, out = _derive(gfx1250_iim, assembler, capsys, **_scaleLessDivergent(pgrA, pgrB))
    assert sol.get("Valid") is True, out
    gate = decoupledThickGateRelaxation(sol)
    ...
    assert gates and set(gates) - {None} == {1}, gates
...
def test_mx_divergent_pair_still_gates_on_two(
        _gp_gfx1250, gfx1250_iim, assembler, capsys, pgrA, pgrB, thick):
    """The MX scales ride a second descriptor set, so a fill issues two tensor
    ops and the gate stays at 2. Deriving the count narrows the scale-less
    case; it does not move this one."""
    ...
    gates = _thickGates(asm, thick)
    assert gates and set(gates) - {None} == {2}, gates


def test_solution_rejects_a_plr_that_leaves_no_late_sub_iteration(
        _gp_gfx1250, gfx1250_iim, assembler, capsys):
    """MatrixInstK=128 at DepthU=128 gives LoopIters=1, so PrefetchLocalRead=1
    leaves numItersPLR=0 and the late fill has no sub-iteration to move into.
    KernelWriter asserts that unconditionally, while ClusterLocalRead is
    user-settable, so a guard that tested it let the AssertionError out through
    getSourceFileString instead of rejecting the solution.
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"PrefetchGlobalRead": 1, "PrefetchGlobalReadA": 1, "PrefetchGlobalReadB": 0},
         "leave both tensors on one LDS block"),
        ({"ScheduleIterAlg": 3},
         "schedules the global reads one instruction at a time"),
    ],
)
...
# ---------------------------------------------------------------------------
# Divergent thick-wait post-pass (KernelWriter._dcpRelaxThickTextGate).
# A text pass over already-emitted assembly, so it needs no toolchain: the
# writer is stubbed down to the one field the pass touches.
# ---------------------------------------------------------------------------
...
def test_thick_wait_does_not_claim_a_wait_past_the_next_fill_label():
    """The next fill label ends this site's window, for the retag loop and for
    the coverage check alike.

    The drain below that label gates the region the label opens, so it is
    neither rewritten here nor read as this site's own gate. Nothing between
    the two labels reads LDS, so this site has nothing to hold back and
    nothing to refuse.
    """
...
def test_thick_wait_refuses_to_walk_past_the_thick_gate_to_the_thin_drain():
    ...
    assumes -- reject the kernel instead of relaxing a real dependency. Only a
    first gate weaker than the relaxation can be reaching for that drain."""
...
def test_thick_wait_accepts_a_gate_already_strong_enough(emitted):
    """A first gate at or below the relaxed count already holds the dependency
    this pass exists to loosen: the kernel is correct, it just did not need us.
    Rejecting it would cost a solution to save an optimisation."""
...
# ---------------------------------------------------------------------------
# TDMFuse=1 is not this pass's business: A and B hold disjoint tensor tokens,
# so the wait-count insertion pass computes the relaxed thick gate from
# dataflow and emits it directly. These pin the no-op against the layout the
# paired arm actually emits -- `[body..., label]`, label last.
# ---------------------------------------------------------------------------
...
def test_thick_wait_paired_is_a_no_op(asmArgs, asmKwargs):
    """The paired arm returns the assembly untouched before the scan, so the
    region clone, the body length, the label and any pre-existing gate are all
    the same code path; these ids are the shapes it has to survive."""
...
def test_auto_selection_steps_down_to_a_divergent_pair_when_the_equal_pair_will_not_fit():
    ...
    # The packed block, which a cap of 0 asks for: setLdsOffsets gives up the
    # power-of-two block rather than exceed MaxLDS, so a cap between the two
    # sizes still admits the equal pair -- and so does the derivation.
...
def test_no_candidate_can_strand_a_legal_pair():
    """A divergent pair above the two-block cap is never offered.

    It used to be: (3, 2) outranked (2, 2) on LDS, was rejected downstream for
    exceeding two blocks, and stranded the legal pair that fitted under the same
    cap, because nothing retried. The guarantee is now structural rather than a
    ...
    # A cap that admits (2, 2) but not (3, 3): the band the stranding happened in.
    cap = estimate((2, 2))
...
# ---------------------------------------------------------------------------
# The estimate against the layout assignDerivedParameters actually lays down.
# Auto ranks on this number and nothing else, so a divergence here is a
# divergence in which pair ships.
# ---------------------------------------------------------------------------
def _estimateProbe(**overrides):
    """Estimate input for the shape _postConversionState describes.

    MacroTile is resolved here the way pgrAutoPairSelectMaxLds resolves it
    before calling the estimate.
    """
...
def test_equal_pair_block_is_rounded_to_a_power_of_two_only_while_it_fits():
    """setLdsOffsets rounds the block up so the buffer swap is an inlined xor,
    and declines when the rounded-up pair would not fit -- which is the whole
    of StoreSwapAddr. Reading only the packed size understated an equal pair by
    up to a factor of two per block."""
...
def test_the_round_up_can_never_cost_a_pair_that_fitted():
    """The layout takes the power-of-two block only while the rounded-up pair
    still fits, so the estimate either reports the packed size or a size within
    MaxLDS. That is why matching the layout here changes no selection."""
...
def test_divergent_pair_is_packed_with_no_stride_round_up():
    """setLdsOffsetsDecoupled lays nBlkA x [A|MXSA] then nBlkB x [MXSB|B] and
    packs them: no power-of-two stride (it sets StoreSwapAddr instead), and no
    round-up to eight bytes, which neither real layout does."""
...
def _derivedLdsBytesAB(sol, pgrA, pgrB):
    ...
    assignDerivedParameters keeps it in a local. LdsNumBytes is the max of it
    and the occupancy, reduction and epilogue floors, which are the same for
    every candidate and so are not what auto ranks on.
    """
...
def test_estimate_matches_the_derivation_once_the_padding_is_resolved(
        _gp_gfx1250, gfx1250_iim, assembler, capsys, pgrA, pgrB):
    """The padding is the only thing the estimate cannot see.

    Auto runs before LdsPad* is derived, so the estimate reads it as 0 and
    comes out low. Hand it the pads the derivation resolved and the two agree
    byte for byte -- which is what says the rest, both layouts and the
    power-of-two block, is the derivation's own arithmetic and not a second
...
# ---------------------------------------------------------------------------
# (2, 1) against (1, 2). Byte-identical LDS, different kernels: (1, 2) makes B
# thick, which reverses local-read emission and turns off the backend ds_read
# ordering pass. Which one ships is a decision, not the order of two appends.
# ---------------------------------------------------------------------------
_F8F8 = {
    "MacDataTypeA": DataType("F8"),
...
def _tiedPairState(miWaveTile, miWaveGroup, **overrides):
    ...
    Equal element sizes with MacroTile0 == MacroTile1 is the whole tie: the two
    arms then differ only in which side holds the second block. MIWaveTile and
    MIWaveGroup vary while the tile does not, which is what the discriminator
    needs and what MacroTile could not have given.
    """
    state = {
        "PrefetchGlobalRead": 2,
        "DepthU": 256,
...
def _tiedPairCap(state):
    ...
    assert decouplePGRLdsBytesEstimate(probe, _F8F8) == tied, \
        "the fixture has to tie, or the tie-break is never reached"
    probe["PrefetchGlobalReadA"], probe["PrefetchGlobalReadB"] = 2, 2
    assert decouplePGRLdsBytesEstimate(probe, _F8F8) > tied, \
...
def test_equal_macro_tile_does_not_mean_equal_mi_wave_tile():
    """Why MIWaveTile can discriminate here at all.

    _macroTileFromMIGeometry folds MIWaveGroup and MIBlockBM/BN in beside it,
    so distinct MIWaveTile values reach the same tile -- MacroTile, the other
    key in state, is exactly the one that cannot tell these apart.
    """
...
def test_auto_makes_the_heavier_local_read_side_thick(miWaveTile, miWaveGroup, expected):
    """The thin tensor's refill is relocated behind a whole-workgroup
    write-after-read fence and cannot start until every ds_read retires, while
    the thick tensor keeps its fill at the loop head. So the side with more
    local reads to overlap is the side that should hold two blocks."""
...
def test_auto_tie_break_does_not_depend_on_the_candidate_order(
    ...
    """A tie used to go to whichever arm pgrAutoPairCandidates appended first,
    against a strict `>`; either the order or the operator could have flipped
    the shipped kernel without a test noticing. Reversing the list has to leave
...
def test_auto_weighs_the_wave_tile_by_its_own_input_count():
    """MIWaveTile alone is not the work: a side reading half as many elements
    per thread issues half the local reads at the same tile."""
...
def test_owner_over_the_full_row_by_pair_cross_product(
    ...
    """Presence follows the pair; mechanism follows the row."""
    monkeypatch.setattr(TF, "tdmGrouping", lambda _s: TDM_GROUPS[name])
...
def test_presence_follows_the_pair_not_the_fuse_integer(pgrA, pgrB, shape):
    """Presence is a structural answer, never a TDMFuse integer.

    Every pair here leaves the thin side on one block, so a divergent pair
    earns a relaxation whatever row it lands on. This is the mutation guard:
    re-gating presence on a fuse integer -- the defect the owner exists to
    prevent -- makes a row disagree here and fails loudly.
    """
    present = {
...
def test_token_path_requires_a_second_token_stream():
    """A separate-descriptor pair the TDM does not move on both tensors has no
    second stream to skip past."""
...
def test_a_thin_side_deeper_than_one_block_earns_nothing_on_any_row(
    ...
    """The thin-side rule belongs to both mechanisms, not just the token one.

    Each justifies its count by skipping past the thin side's single refill,
    so neither may hand one out once the thin side holds two blocks.
    DCP_MAX_LDS_BLOCKS_DIVERGENT refuses these pairs before a solution reaches
    the writer, so they are asked of the function directly.
    """
    monkeypatch.setattr(TF, "tdmGrouping", lambda _s: TDM_GROUPS[name])
...
def test_supported_counts_match_the_documented_mechanisms():
    """Disjoint tokens give the thick tensor its own counter, so one fill is
    always one op there. The shared token counts every descriptor set the fill
    writes, which is two with the MX scale set and one without it."""
...
def test_the_token_pass_predicate_names_exactly_the_tokens_mechanism(
        name, pgrA, pgrB, shape, monkeypatch):
    """One predicate for the two writer gates that have to move together.

    postMainLoopBarrierCheckAndReset and StinkyTofu's wait-count insertion
    produce the relaxed count between them and either alone still emits a full
    drain, so both read this instead of each naming an opt level. Answered
    against the row table rather than against decoupledThickGateRelaxation, so
    the predicate and the mechanism cannot agree by sharing a bug.
    """
    monkeypatch.setattr(TF, "tdmGrouping", lambda _s: TDM_GROUPS[name])
...
def uncovered(schedule, relaxed=TEXT_RELAXED):
    ...
    Mirrors _dcpRelaxThickTextGate on both halves of its decision: a full drain
    is rewritten to `relaxed`, and a gate already at or below `relaxed` is
    accepted where it stands. Modelling only the rewrite would let the check
    refuse a site production accepts. Doing it for every gate rather than the
    first below each label stands in for the real loop: in these shapes each
    gate is some site's own.
    """
...
def test_a_gate_weaker_than_the_relaxation_is_refused():
    ...
    Reaching a gate weaker than the relaxation means the emitted shape is not
    the one this pass assumes, so it may not claim the site is relaxed: the
    next gate down drains the thin tensor into the block its reads are about to
    touch.
    """
...
def test_a_gate_already_stronger_than_the_relaxation_is_accepted():
    """Production accepts a gate at or below the relaxed count where it stands
    -- the kernel is correct, it just did not need this pass -- so refusing one
    here would refuse a kernel over an instruction that already holds."""
...
def test_a_site_is_not_judged_by_a_gate_in_the_next_fill_region():
    """The audit's scan window is the rewriter's.

    _dcpRelaxThickTextGate stops at the next fill label, so the gate under
    that label belongs to the region it opens. Reading it as the first site's
    own gate refuses a kernel over an instruction that site never reaches.
    """
    weak = "s_wait_tensorcnt %d" % (TEXT_RELAXED + 1)
...
# ---------------------------------------------------------------------------
# Auto ranks on an estimate that cannot see LDS padding, so its best candidate
# can still be refused by the real LDS check. These pin that auto walks on to
# the next candidate instead of losing the solution, and that it only does so
# for that one rejection.
# ---------------------------------------------------------------------------
def test_auto_pair_ranking_head_is_the_selected_pair():
    """One ordering, so stepping the ranking cannot disagree with selection."""
    for maxLds in (327680, 90000, 1024):
        state = _postConversionState(MaxLDS=maxLds)
        ranking = pgrAutoPairRanking(2, state, _F8F4)
        selected = pgrAutoPairSelectMaxLds(2, state, _F8F4)
        assert (ranking[0] if ranking else None) == selected, maxLds
...
def test_auto_pair_ranking_drops_the_candidates_a_cap_excludes():
    """A tighter cap must shorten the ranking from the deep end, not reorder it."""
...
def test_resolve_auto_skip_walks_the_ranking_in_order():
    ...
    assert len(ranking) > 1, "the fixture has to offer a second candidate"
...
def test_resolve_auto_skip_past_the_ranking_reports_the_same_no_fit():
    """Exhausting the ranking may not degrade into some other pair or reason."""
    ranking = pgrAutoPairRanking(2, _postConversionState(), _F8F4)
...
# The witness: BBS at MacroTile (256, 128), DepthU 256, stock device LDS. The
# estimate ranks (2, 1) at exactly 327680, the derivation sizes it at 332800.
_LDS_WITNESSES = [
...
def test_solution_auto_steps_past_a_pair_the_lds_check_refuses(
    ...
    """Auto used to hand back the pair its estimate liked and stop there."""
...
def test_solution_auto_matches_the_same_pair_written_out(
        _gp_gfx1250, gfx1250_iim, assembler, capsys, mi, depthU, expected):
    """A stepped-to pair has to derive to the explicit pair, not to a hybrid.

    The pair pins the scalar PrefetchGlobalRead, which several parameters are
    derived from before the padding exists, so a retry that reused them would
    ship a kernel no explicit pair can produce.
    """
    auto, autoOut = _derive(gfx1250_iim, assembler, capsys, **_bbsWitness(
        MatrixInstruction=mi, DepthU=depthU,
        PrefetchGlobalReadA=-1, PrefetchGlobalReadB=-1))
...
def test_solution_auto_exhausting_the_ranking_still_rejects(
        _gp_gfx1250, gfx1250_iim, assembler, capsys):
    """The ladder may not turn a genuine no-fit into a valid solution."""
...
def test_solution_auto_does_not_retry_an_unrelated_rejection(
        _gp_gfx1250, gfx1250_iim, assembler, capsys):
    """Only the LDS-capacity rejection is retried.

    ClusterDim is refused for every divergent pair, so a ladder that stepped on
    any rejection would walk the whole ranking and report the last candidate's
...
def test_solution_derivation_leaves_no_capacity_marker_behind(
        _gp_gfx1250, gfx1250_iim, assembler, capsys):
    """The marker is derivation-internal and must not reach a solution."""
    from Tensile.SolutionStructs.Solution import Solution
...
# ---------------------------------------------------------------------------
# Which ScheduleIterAlg can carry a divergent decoupled pair, and why
# ---------------------------------------------------------------------------
#
# ScheduleIterAlg 0 and 4 both derive _ScheduleIterAlg=0 (4 additionally derives
# _StinkyTofuOptLevel=3), so both run noSchedGlobalRead, which puts the whole
# globalReadA / globalReadB modules into the unroll loop header.
# _dcpScheduleSingleBufferedFillLate lifts the thin tensor out of that header by
# identity, so the fill group has to still be there as a module.
#
# ScheduleIterAlg=3 keeps _ScheduleIterAlg=3 and runs prepareGRInstToSched
# instead, which puts only globalRead*.header into the header and hands each
# tensor_load_to_lds to schedGlobalRead one at a time. Nothing is left to
# re-slot, and the relocation is what keeps the single-buffered tensor from
# refilling the block the current iteration is still reading -- so the refusal
# is load-bearing for correctness, which is what the last test here pins.
...
def _dcpSolutionModule():
    """The Solution *module*, not the class of the same name.

    Tensile.SolutionStructs re-exports the Solution class under its submodule's
    own name, so ``import Tensile.SolutionStructs.Solution as m`` binds the
    class and a monkeypatch applied to it silently does nothing at all.
    """
    import importlib
...
def test_schedule_iter_alg_3_refuses_a_divergent_pair_over_the_fill_placement(
    ...
    """The refusal has to name the placement, not just the algorithm number.

    ScheduleIterAlg=3 is the algorithm the tuners reach for, so a reader who
    hits this rejection needs to know whether it is a real limitation or a
    conservative gate someone could widen. It is the former, and the message
    says which scheduling step removes the fill group.
    """
    sol, out = _derive(gfx1250_iim, assembler, capsys, TDMFuse=tdmFuse,
                       ScheduleIterAlg=3, PrefetchGlobalRead=max(pgrA, pgrB),
                       PrefetchGlobalReadA=pgrA, PrefetchGlobalReadB=pgrB)
    assert sol.get("Valid") is False, out
    # SIA=4 is remapped during derivation; 3 is not, which is the whole reason
    # the fill never reaches the header.
    ...
    assert "schedules the global reads one instruction at a time" in out, out
    assert "no fill group to re-slot" in out, out
...
def test_the_relaxed_gate_and_its_side_order_across_every_schedule_iter_alg(
        ...
        pgrA, pgrB):
    """Hero (1,2) and mirror (2,1) over the three algorithms the yaml sweeps.

    A cell either emits the relaxation it promised, in the issue order
    ``tdmWaveIssueOrder`` picked, or it is refused for one of two reasons this
    test names. Anything else -- a cell that derives Valid and then emits a full
    drain, or one refused for a reason nobody wrote down -- fails here.

    The gate is the first ``s_wait_tensorcnt`` anywhere below the first fill
    label, with no proximity window: on the token path the gate lands at the
    true dependence point, over a hundred lines below the label.
    """
    sol, out = _derive(gfx1250_iim, assembler, capsys, TDMFuse=tdmFuse,
                       ScheduleIterAlg=scheduleIterAlg,
                       PrefetchGlobalRead=max(pgrA, pgrB),
...
def test_the_schedule_iter_alg_3_refusal_is_correctness_not_conservatism(
    ...
    """Drop only the ScheduleIterAlg term and the writer stops being able to emit.

    This is the difference that decides whether the refusal may be widened: a
    conservative gate would let a correct kernel through, while this one is the
    only thing standing between ScheduleIterAlg=3 and a fill the relocation
    never moved. Pinned as a test because the gate is one line and the failure
    it prevents is silent everywhere except here.
    """
    ...
    assert sol.get("Valid") is True, (
        "the refusal under test is no longer the only thing refusing this "
        "solution, so the rest of this proves nothing: %s" % out)
    assert sol["_ScheduleIterAlg"] == 3, sol["_ScheduleIterAlg"]
