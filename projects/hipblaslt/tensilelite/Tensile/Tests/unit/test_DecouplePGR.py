...
# Whether every tensor in a row sits on the LDS side its descriptor set's
# owner sits on. The LDS layout is nBlkA x [A|MXSA] then nBlkB x [MXSB|B]
# whatever the grouping says, so a set that straddles the two would have the
# fill and the ds_load consuming it waiting on different tokens.
ROW_RESPECTS_LDS_PAIRING = {
    "MX_AB": False,   # {A,B} straddles, but shares one set and takes no tokens
    "paired": True,   # {A,MXSA} and {MXSB,B} are exactly the two LDS sides
    "A_MX": False,    # MXSB rides A's set while its LDS sits on B's side
    "B_MX": False,    # MXSA rides B's set while its LDS sits on A's side
}


def _rowEarnsRelaxation(name):
    """A row relaxes the gate unless it would take tokens the LDS cannot honour."""
    return not ROW_SEPARATES_AB[name] or ROW_RESPECTS_LDS_PAIRING[name]
...
def test_every_grouping_row_has_a_stated_expectation():
    ...
    assert set(ROW_RESPECTS_LDS_PAIRING) == set(TDM_GROUPS), (
        "a grouping row was added without stating whether its descriptor sets "
        "keep every tensor on its own LDS side")
...
def test_owner_over_the_full_row_by_pair_cross_product(
    ...
    """Presence follows the pair and the row's LDS pairing; mechanism follows
    the row."""
    monkeypatch.setattr(TF, "tdmGrouping", lambda _s: TDM_GROUPS[name])
    gate = decoupledThickGateRelaxation(ks(pgrA=pgrA, pgrB=pgrB))
    if shape != "divergent" or not _rowEarnsRelaxation(name):
        assert gate is None
        return
    assert gate is not None, (
        "a divergent pair earns a relaxation on every row whose descriptor sets "
        "keep each tensor on its own LDS side; the %s row returned None, which "
        "is the fuse-gated-presence bug" % name)
...
@pytest.mark.parametrize("pgrA,pgrB,shape", PAIRS)
def test_presence_follows_the_pair_and_the_rows_lds_pairing(pgrA, pgrB, shape):
    """Presence is a structural answer, never a TDMFuse integer.

    A divergent pair earns a relaxation on every row whose descriptor sets keep
    each tensor on the LDS side its set's owner sits on; where they do not, the
    disjoint tokens could not be honoured and the answer is a full drain. This
    is the mutation guard: re-gating presence on a fuse integer -- the defect
    the owner exists to prevent -- makes a row disagree here and fails loudly.
    """
    present = {
    ...
    expected = {
        fuse: shape == "divergent"
        and _rowEarnsRelaxation(tdmGrouping(ks(fuse=fuse)).name)
        for fuse in sorted(TDM_FUSE_GROUPING)
    }
