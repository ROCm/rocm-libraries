...
# What makes decoupledThickGateRelaxation answer DCP_THICK_GATE_TOKENS: a
# divergent decoupled pair on a grouping that gives A and B separate TDM
# descriptors. The pass reads that through dcpThickGateFromTokenPasses.
_TOKEN_GATE_KEYS = {
...
def test_the_pass_does_not_run_below_optlevel_3():
    """Without a decoupled token gate, ScheduleIterAlg=0 keeps the writer's barriers."""
...
def test_the_pass_runs_at_optlevel_0_for_a_decoupled_token_gate():
    """A TOKENS thick gate needs this rebuild whatever opt level it derives.

    The barriers it replaces span both tensors, so a wait placed on one of them
    can only be a full drain -- which is what ScheduleIterAlg=0 emitted while
    decoupledThickGateRelaxation reported a relaxed gate. The rebuild and
    StinkyTofu's wait-count insertion produce that count together.
    """
    kernel = _waveParityKernel(_StinkyTofuOptLevel=0, **_TOKEN_GATE_KEYS)
    assert dcpThickGateFromTokenPasses(kernel), \
        "the fixture stopped earning a TOKENS thick gate, so this proves nothing"
