...
def test_every_grouping_row_answers_without_a_new_branch(row, shares):
    """Each row inherits the right answer as data, with no branch per row.
...
def test_tdmfuse_own_guards_keep_precedence(_gp_gfx1250, gfx1250_iim, assembler, capsys):
    ...
    four waves, so the shared-scale row declines, the writer falls back to the
    default grouping, and nothing is aliased for PAP to complain about.
    """
    sol, out = _derive(gfx1250_iim, assembler, capsys, PrefetchAcrossPersistent=1,
...
def test_pap_never_names_an_unallocated_tdm_increment(
    ...
    tdmMXSAMXSBIncs is allocated only when no shared scale set is active, while
    the PAP tail reset builds the name unconditionally. Against the unfixed tree
    TDMFuse=2 + PAP emits `s[sgprtdmMXSAMXSBIncs]` with no .set, and the
    assembler refuses it with "expected absolute expression".
    """
    sol, asm = _emit_asm(gfx1250_iim, assembler, PrefetchAcrossPersistent=1, TDMFuse=fuse)
    capsys.readouterr()
    if asm is None:
