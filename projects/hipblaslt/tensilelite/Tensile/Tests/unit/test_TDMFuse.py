...
def test_b_mx_separates_a_from_b():
    """Both descriptor questions the decoupled PGR thick gate asks first."""
...
def test_an_issue_declares_every_lds_side_its_set_fills():
    """A_MX seats {A, MXSA, MXSB} on one set, and MXSB's LDS is on B's side.

    A's single tensor_load_to_lds therefore fills both LDS groups, so it has
    to carry both tokens: with side A's alone, MXSB's ds_load would wait on a
    token that fill never touched.
    """
...
def test_a_dead_set_member_contributes_no_token(monkeypatch):
    """A scale-less problem degenerates its groups, so what the issue fills is
    the set's live members rather than the row's full membership."""
...
def test_shipped_increment_literals_match_the_partition():
    ...
    This site is deliberately NOT routed through the partition. A group's member
    order is its wave order, so the row offers one arrangement and no reachable
    arrangement makes the literals wrong -- a structural finding, not a defect.
    Routing it would also make the dispatch read solution keys that stub-driven
    writer tests do not supply.
