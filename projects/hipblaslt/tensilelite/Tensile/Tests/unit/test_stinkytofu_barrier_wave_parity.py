...
tensor_load_to_lds it guards. A rebuild that recursed module by module could not
see that branch, so it put the barrier after it.

That guard is not in this tree yet: its emitter,
KernelWriter._dcpScheduleSingleBufferedFillLate, arrives in the next commit, so
the reference is forward and this fix is a prerequisite rather than a companion.
The cases below raise the split-guard shape from synthetic modules, so they hold
at this commit without the emitter.
