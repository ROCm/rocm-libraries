...
from Tensile.Components.Subtile.Kernel import (
    TileInfo, AB_B16, AB_B4, AB_B8, MXSA_B4, MXSB_B4, MXSA_B8, MXSB_B8, CD_F32,
)
...
def makeTileInfo(tc, kernel):
    ...
    if mx:
        ...
        mx_geo_a = MXSA_B8 if abbrev in ('fp8', 'bf8') else MXSA_B4
        mx_geo_b = MXSB_B8 if abbrev in ('fp8', 'bf8') else MXSB_B4
    else:
        ab_geo = AB_B16
        mx_geo_a = mx_geo_b = None
    _geo = {
        'A': ab_geo,
        'B': ab_geo,
        'MXSA': mx_geo_a,
...
def create_kernel(MT0=256, MT1=256, dtype='bf16', depthU=None, waveGroup=(2, 2)):
...
if __name__ == "__main__":
    ...
    kernel = create_kernel(args.mt0, args.mt1, dtype=args.dtype, depthU=args.du,
                           waveGroup=waveGroup)
    tiA = makeTileInfo('A', kernel)
    tiB = makeTileInfo('B', kernel)
