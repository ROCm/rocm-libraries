...
| Direct TensileLite API | Implemented in this layer | Explicit YAML and GEMM descriptors produce a checked algorithm through `tensilelite::getGemmAlgo`, then C/C++ APIs execute it |
| Direct sample and CI | TBD in the next basic layer | Sample `29_hipblaslt_jit_gemm` and the shared driver check direct C/C++ execution independently of the generic API |
