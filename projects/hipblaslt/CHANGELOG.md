...
* Support ReLU and Clamp activation functions with auxiliary output for f16/bf16 data types to capture intermediate results (enabled by ``HIPBLASLT_EPILOGUE_RELU_AUX``, ``HIPBLASLT_EPILOGUE_RELU_AUX_BIAS``, ``HIPBLASLT_EPILOGUE_CLAMP_AUX_EXT``, and ``HIPBLASLT_EPILOGUE_CLAMP_AUX_BIAS_EXT``)
...
* Added the log print for Grouped Gemm
* Supported LocalSplitU for swizzleA kernel
* Completed the support of `--api_method` mix and cpp with `--swizzleA` for hipblaslt-bench
