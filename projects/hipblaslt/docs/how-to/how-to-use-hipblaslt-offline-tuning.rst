...
#. Set the environment variable ``HIPBLASLT_TUNING_FILE=<file_name>`` and run the command. ``hipblaslt-bench`` then tunes the
   ...
   search has no time limit unless ``HIPBLASLT_TUNING_BUDGET_MS_PER_SHAPE`` sets one. A problem the file already holds is tuned
   again only by a run that searches more than the one that wrote it. Grouped GEMM and pointer-array batched problems are not
   tuned.
