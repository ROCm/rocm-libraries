...
static hipblasStatus_t runBf16TnFusedMatmul(hipblasLtHandle_t                  handle,
                                            ...
                                            int&                               algoCount)
{
    // TN bf16 GEMM with col-major A/B/C/D descriptors; A's leading dimension is k.
...
TEST(FusedEpilogueE2E, decomposedProducerConsumerMatchesReference)
{
    ...
    for(int64_t m = 0; m < mTok; ++m)
        for(int64_t n = 0; n < Nout; ++n)
        {
            ...
            expected[n + m * Nout] = bf16_to_f32(f32_to_bf16(acc * rstd[m])); // D2 col-major [Nout, Mtok]
        }
                                      ...
                                      void*                                     dResidualOut = nullptr)
{
    ...
    else
    {
        ASSERT_EQ(runTypedTnFusedMatmulFp8D(
                      handle, M, N, K, A, K, B, C, D, gemm1InType, prod, dWs, wsSize, algoCount),
                  HIPBLAS_STATUS_SUCCESS);
                                   ...
                                   size_t                                    wsSize)
{
    ...
    for(int64_t mt = 0; mt < d.mTok; ++mt)
        for(int64_t no = 0; no < d.nOut; ++no)
        {
            const float got   = bf16_to_f32(hD2[no + mt * d.nOut]);
...
TEST(FusedEpilogueE2E, partialRmsMxfp8InputMxfp8QuantMatchesReference)
{
    ...
    void *dMat1Scale = nullptr, *dMat2Scale = nullptr, *dResidualOut = nullptr, *dWs = nullptr;
    const size_t wsSize       = size_t(256) * 1024 * 1024;
    const size_t residualOutSz = static_cast<size_t>(mTok) * nHid * sizeof(uint16_t);
    ...
    ASSERT_EQ(hipMalloc(&dMat1Scale, mat1ScaleSz), hipSuccess);
    ASSERT_EQ(hipMalloc(&dMat2Scale, mat2ScaleSz), hipSuccess);
    ASSERT_EQ(hipMalloc(&dResidualOut, residualOutSz), hipSuccess);
    ASSERT_EQ(hipMalloc(&dWs, wsSize), hipSuccess);
    ...
    const std::vector<uint8_t> hMat1Scale(mat1ScaleSz, 127u);
    const std::vector<uint8_t> hMat2Scale(mat2ScaleSz, 127u);
    ASSERT_EQ(hipMemcpy(dMat1Scale, hMat1Scale.data(), mat1ScaleSz, hipMemcpyHostToDevice),
              hipSuccess);
    ASSERT_EQ(hipMemcpy(dMat2Scale, hMat2Scale.data(), mat2ScaleSz, hipMemcpyHostToDevice),
              hipSuccess);
    ...
    static_cast<void>(hipFree(dMat1Scale));
    static_cast<void>(hipFree(dMat2Scale));
    static_cast<void>(hipFree(dResidualOut));
    static_cast<void>(hipFree(dWs));
}
