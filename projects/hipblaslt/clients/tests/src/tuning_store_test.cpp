...
// Host-only tests for the tuning cache's file format and store: reading rows,
// which row wins, whether a key is tuned again, and how the environment picks
// the mode. They link the store directly, so they need neither a device nor the
// hooks hipblaslt-test uses, and they assert on the store's own decisions.
...
namespace
{
                          ...
                          const TuningSearch& search   = rankedSearch(16))
    {
        ...
        entry.source        = TuningEntrySource::TuneMode;
    ...
    TuningRowsLoaded loadInto(OverrideMap&       map,
                              const std::string& text,
                              TuningEntrySource  source = TuningEntrySource::ManagedCacheFile)
    {
        std::istringstream in(text);
        return loadTuningRows(in, map, source, kStamp);
    }
    ...
    std::string withCell(const std::string& row, const std::string& column, const std::string& value)
    {
    ...
    // A row in the historical hipblaslt-bench format: no schema version, and
    // only the problem columns it ever recorded.
    ...
    // from, including the fields that only exist in the widened format.
    TEST_F(TuningStore, WrittenRowReadsBackAsItsKeyAndEntry)
    {
        ...
        EXPECT_EQ(read.source, TuningEntrySource::ManagedCacheFile);
    ...
    {
        OverrideMap map;
        const auto  row = withCell(tunedRow(halfKey(), tunedEntry(7, "kernel")), "schema_version", "99");
    ...
    // Cut before the completion columns, or between them, the prefix that
    // survives is well formed and would otherwise read as a finished search.
    TEST_F(TuningStore, RowCutShortIsRejected)
    {
        const auto row = tunedRow(halfKey(), tunedEntry(7, "kernel", false, 1000));
        for(const char* column : {"complete", "budget_ms", "rotating_mb"})
        {
    ...
    TEST_F(TuningStore, RowDifferingInAKeyColumnServesOnlyItsOwnProblem)
    {
        ...
        for(const auto& change : changes)
        {
            ...
            OverrideMap map;
            EXPECT_EQ(loadInto(map, fileOf({withCell(row, change.column, change.value)})).accepted, 1u)
    ...
    // Files written before the widened key keep matching on the fields the old
    // format recorded, and only on those.
    TEST_F(TuningStore, LegacyRowMatchesOnTheHistoricalFields)
    {
        const ProblemOverride key = halfKey();
        OverrideMap           map;
        ASSERT_EQ(loadInto(map, fileOf({legacyRow(key, 7, "kernel")}), TuningEntrySource::LegacyOverrideFile)
                      .accepted,
                  1u);

        EXPECT_TRUE(map.find(key).empty()) << "a legacy row was filed under the widened key";
    ...
    // that validates, so the completed row has to come first, and it does
    ...
    // A hand-written override file has no completeness to rank, so its order is
    // the only intent it can express.
    TEST_F(TuningStore, LegacyRowsKeepFileOrder)
    {
        const ProblemOverride key = halfKey();
        OverrideMap           map;
        loadInto(map,
                 fileOf({legacyRow(key, 1, "first"), legacyRow(key, 2, "second")}),
    ...
    TEST_F(TuningStore, RepeatedRowRefreshesAndRenamedRowIsKept)
    {
        ...
        OverrideMap map;
        EXPECT_EQ(loadInto(map, fileOf({tunedRow(key, first), tunedRow(key, refreshed)})).accepted, 1u);
    ...
    TEST_F(TuningStore, FinishedSearchIsWidenedButNotRepeated)
    {
        ...
        OverrideMap narrow;
        loadInto(narrow, fileOf({tunedRow(key, tunedEntry(7, "kernel", true, 0, rankedSearch(2)))}));
    ...
    TEST_F(TuningStore, RowsWithoutARecordedSearchAreFinal)
    {
        ...
        OverrideMap legacy;
        loadInto(legacy, fileOf({legacyRow(key, 7, "kernel")}), TuningEntrySource::LegacyOverrideFile);
        EXPECT_FALSE(legacy.needsRetune(key, rankedSearch(16), 0));
    }

    TEST_F(TuningStore, BudgetComparison)
    {
        EXPECT_TRUE(tuningBudgetIsMoreGenerous(1000, -1)) << "a row from before the column";
    ...
    TEST_F(TuningStore, SearchCoverage)
    {
        ...
        EXPECT_FALSE(tuningSearchCovers(less([](TuningSearch& s) { s.workspaceBytes = 0; }),
                                        rankedSearch(16)));
        EXPECT_FALSE(tuningSearchCovers(less([](TuningSearch& s) { s.coldIters = 0; }),
                                        rankedSearch(16)));
        EXPECT_FALSE(tuningSearchCovers(less([](TuningSearch& s) { s.hotIters = 1; }),
                                        rankedSearch(16)));
        EXPECT_FALSE(tuningSearchCovers(less([](TuningSearch& s) { s.flushICache = false; }),
                                        rankedSearch(16)));
        EXPECT_FALSE(tuningSearchCovers(less([](TuningSearch& s) { s.rotatingMb = 0; }),
                                        rankedSearch(16)));
    }
    ...
    // Anything but cache or tune is off, and a mode with nowhere to keep
    ...
    TEST_F(TuningStore, AppendedRowsReadBackFromTheFile)
    {
        ...
        ASSERT_TRUE(appendTuningRow(path, tunedRow(halfKey(256, 256, 256), tunedEntry(1, "a")), kStamp));
        ASSERT_TRUE(appendTuningRow(path, tunedRow(halfKey(512, 256, 256), tunedEntry(2, "b")), kStamp));
        ...
        OverrideMap   map;
        EXPECT_EQ(loadTuningRows(again, map, TuningEntrySource::ManagedCacheFile, kStamp).accepted, 2u);
