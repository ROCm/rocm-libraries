# Generate a GEMM kernel with hipblaslt-bench
...
`HIPBLASLT_ENABLE_JIT_GEMM=ON`. Building `hipblaslt-bench` also requires
...
that failure instead of dropping the requested operation. C and D must share
a storage datatype because TensileLite represents both with one `DestDataType`
field; separate C/D types require extending the generated-library contract.
...
If Origami cannot model a request, or all its recipes fail validation, the
generator tries TensileLite defaults and native instruction recipes. The manifest
labels that path `tensile.defaults` and records no predicted latency. For MX
block-scaled inputs, default recipes also supply the target's required memory
transport and subtile parameters, with the reason recorded in the manifest.
The request separately retains the descriptor's physical scale layout; this is
recorded as `implementation_parameters`, not as a predicted tuning choice.
...
`HIPBLASLT_JIT_PYTHONPATH` is a colon-separated list of additional Python
import directories. Using `--jit-gemm` in a build without the feature reports
that `HIPBLASLT_ENABLE_JIT_GEMM` must be enabled.
...
The `hipblaslt-jit-gemm-ci.yml` workflow covers gfx90a, gfx942, gfx950, and
gfx1250 with the predicted route and explicit Stream-K/output-amax recipes.
Its shared driver, `.github/scripts/test_hipblaslt_jit.py`, accepts `--build`,
`--architecture`, and a fresh `--output` path; `--case` selects one route.
