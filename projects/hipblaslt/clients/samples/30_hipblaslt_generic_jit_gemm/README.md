# Run a generic JIT solution through the public GEMM APIs
...
```bash
cmake --build "$project_build" --target _rocisa hipblaslt-generic-jit-gemm --parallel
...
"$project_build/clients/staging/hipblaslt-generic-jit-gemm" \
...
The [direct sample](../29_hipblaslt_jit_gemm/README.md) keeps the one-call explicit TensileLite API. Both examples share the same runtime.
