...
"""Check normal C/extension helper failures before submission and state publication.
...
def write_generator(path, source, case, readobj):
    path.write_text(
        f"#!{sys.executable}\n"
        "import json, pathlib, shutil, subprocess, sys, yaml\n"
...
def main():
    ...
    parser.add_argument('bundle', type=Path, help='Existing local split-K bundle containing manifest.json')
    ...
    empty_library.mkdir()
    env = dict(os.environ, HIPBLASLT_TENSILE_LIBPATH=str(empty_library), PYTHONDONTWRITEBYTECODE='1')
    ...
    for case in ('unlisted-helper-modules', 'missing-helper-symbols'):
        ...
        command = [str(args.executable.resolve(strict=True)), str(valid), str(output), '',
                   'unused.yaml', str(output / case), manifest['architecture']['requested'],
                   'unused-compiler', '--normal-api', 'both', '--second-python', str(damaged),
                   '--expect-helper-failure', '1', '--m', str(args.m), '--n', str(args.n),
                   '--k', str(args.k)]
        (output / (case + '-command.json')).write_text(json.dumps(command, indent=2))
        proc = subprocess.run(command, env=env, text=True, capture_output=True, timeout=300)
        (output / (case + '.log')).write_text(proc.stdout + proc.stderr)
        if proc.returncode != 0:
            raise RuntimeError(f'{case} failed; inspect {output / (case + ".log")}')
        mutation = json.loads((output / (case + '-second') / 'mutation.json').read_text())
        ...
        print(f'PASS {case}: normal C/extension preflight and retained prior context', flush=True)
