...
namespace hipblaslt_ext::experimental::jit::tensilelite
{
    ...
    hipblasStatus_t detail::generateBundle(const Options&           options,
                                           const hipDeviceProp_t&   properties,
                                           int                      device,
                                           ...
                                           Diagnostics&             diagnostics)
    {
        ...
        if(configured.architecture.empty())
            configured.architecture = properties.gcnArchName;
        if(!targetMatchesDevice(configured.architecture, properties.gcnArchName))
        {
            diagnostics.message = "Requested TensileLite architecture does not match device";
            return HIPBLAS_STATUS_ARCH_MISMATCH;
        }
        generate(configured);
        bundle = loadGeneratedBundle(configured, properties, device);
