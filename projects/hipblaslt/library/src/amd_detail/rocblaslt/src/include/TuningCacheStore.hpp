...
// The tuning cache's file format and in-memory store: the problem key, what a
// row resolves to, reading and writing rows, which row wins, and whether a key
// is worth tuning again. Nothing here touches a device, the solution library or
// the logger, so the same code builds into hipBLASLt and into a host-only test.
...
namespace TensileLite
{
    ...
    struct TuningModeConfig
    {
        ...
        // an inherited environment must not be able to impose on it. Set when
        // that is why the mode is off, so the caller can say so.
        ...
        bool reads() const { return mode != TuningMode::Off && !cachePath.empty(); }
        bool writes() const { return mode == TuningMode::Tune && !cachePath.empty(); }
    };
     ...
     * exactly what a later lookup rebuilds. Any key field missing from the row
     * would parse back as its default and silently fail to match.
     */
