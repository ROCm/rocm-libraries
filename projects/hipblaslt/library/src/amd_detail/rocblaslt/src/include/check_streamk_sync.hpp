...
inline void hipblaslt_check_streamk_sync_reset(rocblaslt_handle handle, hipStream_t stream)
{
    ...
    static_cast<void>(hipMemsetAsync(
        handle->Synchronizer, 0, _rocblaslt_handle::c_syncGsuTotalElements * sizeof(int), stream));
}
                                              ...
                                              const char*      label)
{
    ...
    std::vector<int> host(_rocblaslt_handle::c_syncGsuTotalElements);
    static_cast<void>(hipMemcpy(host.data(),
                                handle->Synchronizer,
                                _rocblaslt_handle::c_syncGsuTotalElements * sizeof(int),
                                hipMemcpyDeviceToHost));

    size_t nonzero = 0, first = _rocblaslt_handle::c_syncGsuTotalElements;
    for(size_t i = 0; i < _rocblaslt_handle::c_syncGsuTotalElements; ++i)
        if(host[i] != 0)
        {
            if(nonzero == 0)
                first = i;
    ...
    if(nonzero)
        fprintf(stderr,
                ...
                label,
                nonzero,
                _rocblaslt_handle::c_syncGsuTotalElements,
                first);
}
