function partial_pass_3d(in_length, nbatch, pp_dim, pp_radices, in_batched, out_batched, test_mode, transform_type)
  ...
  if ~(strcmp(transform_type,transform_type_0) || ...
       strcmp(transform_type,transform_type_1) || ...
       strcmp(transform_type,transform_type_2))
      display(test_mode);
  ...
  if (strcmp(transform_type,transform_type_1))
    in_batched_tmp = zeros(prod(in_length),1);
    in_batched_tmp(1:2:end) = real(in_batched);
    in_batched_tmp(2:2:end) = imag(in_batched);
    in_batched = convert_1d_to_3d(in_batched_tmp, in_length(1), in_length(2), in_length(3), nbatch, ordering);

    out_batched = convert_1d_to_3d(out_batched, in_length(1), in_length(2), 1+in_length(3)/2, nbatch, ordering);

    N = in_length(1) * in_length(2) * (1+in_length(3)/2);
  else
  ...
  for ibatch=1:nbatch
    ...
    if (strcmp(test_mode,test_mode_1))
      ...
      if (strcmp(test_mode,test_mode_2))
        if (strcmp(transform_type,transform_type_1))
          out_3d_rc_1 = out_3d_rc_1(:,1:(1+size(out_3d_rc_1,2)/2),:);
        endif
      ...
      if (strcmp(test_mode,test_mode_3))
        if (strcmp(transform_type,transform_type_1))
          out_3d_rc_pp_1 = out_3d_rc_pp_1(:,1:(1+size(out_3d_rc_pp_1,2)/2),:);
        endif
