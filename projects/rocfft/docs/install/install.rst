...
:doc:`./building-installing-rocfft`.
...
rocFFT is included with the ROCm Core SDK on Linux and Windows. For the most
...
Alternatively, if you want to install rocFFT as part of the ROCm
...
This includes rocFFT and hipFFT.
...
2. Install the ROCm FFT package that matches your desired ROCm version,
   ...
   * ``<llvm_target>`` (starting with ``gfx``) is used if you are installating
   ...
   For example, to install the latest FFT development package release for
...
nightly builds for the ROCm Core SDK and its components, including rocFFT.
