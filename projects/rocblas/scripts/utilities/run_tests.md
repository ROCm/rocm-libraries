...
- Jobs whose recorded PID is **dead** are reset to `not_started` and re-run;
  the old log is preserved as `{job_id}.txt.prev`.
- **Failed** jobs are always re-run on resume.
- Already-passed jobs are skipped with a count shown in the summary.
...
| `{GROUP}.{variant}.txt.prev`| Previous log preserved on PID-dead recovery    |
