...
* ``ROCBLAS_TENSILE_ARCH_SKIPLIST``: GPU targets skipped during Tensile library generation. The rocBLAS library still compiles for all ``GPU_TARGETS`` (default: ``gfx1250``). If every target is skipped, Tensile builds stub ``gfx000`` when that logic exists. GEMM on a skipped architecture uses hipBLASLt or source kernels.
