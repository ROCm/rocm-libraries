...
* HIP and libhipcxx from an installed ROCm. Packaged hipThreads ships with ROCm 10.0 and later.
...
The GPU architecture is auto-detected on Linux, so you don't need to set ``-DCMAKE_HIP_ARCHITECTURES``.
...
The hipThreads tests are under ``test/``. Run the suite with `lit <https://llvm.org/docs/CommandGuide/lit.html>`_.
lit compiles each test with ``hipcc`` and runs it, so the suite works against a ``Debug`` or ``Release`` library build:
...
On Windows, use the same Ninja, clang, and ``-DCMAKE_HIP_ARCHITECTURES`` CMake options as the library build in :ref:`Build on Windows <hipthreads-build-windows>`.
...
Some examples need extra setup. For example, the sparse matrix multiply data is pulled with ``git lfs``, and llama3.c takes a model path as an argument. Check the ``CMakeLists.txt`` footer for the step you are building.
