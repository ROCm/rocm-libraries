...
constexpr int __host__ __device__ get_start_row(int arr_size, int tid, int num_threads) {
  ...
  return (tid < arr_size % num_threads) ? (tid * chunk_size) : (tid * chunk_size + arr_size % num_threads);
}
...
void __device__ rmsnorm(float *o, float *x, float *weight, int size) {
  // parallel reduction of sum of squares using atomics
  __shared__ hip::atomic<float, hip::thread_scope_block> atomic_ss;
  if (hip::this_thread::get_fiber_id() == 0) {
    atomic_ss.store(0.0f, hip::memory_order_relaxed);
  }
  __syncthreads();

  float ss = 0.0f;
  for (int j = hip::this_thread::get_fiber_id(); j < size; j += 32) {
    ss += x[j] * x[j];
  }

  // Atomic add to accumulate partial sums
  atomic_ss.fetch_add(ss, hip::memory_order_relaxed);
  __syncthreads();

  // serialization point to calculate normalization factor
  __shared__ float shared_ss;
  if (hip::this_thread::get_fiber_id() == 0) {
    ss = atomic_ss.load(hip::memory_order_relaxed);
    ss /= size;
    ss += 1e-5f;
    ss = 1.0f / sqrtf(ss);
    shared_ss = ss;
...
void __host__ __device__ softmax(float *x, int size) {
  ...
  int fid = hip::this_thread::get_fiber_id();

  // find max value (for numerical stability) using atomics
  __shared__ hip::atomic<float, hip::thread_scope_block> atomic_max;
  if (fid == 0) {
    atomic_max.store(-INFINITY, hip::memory_order_relaxed);
  }
  __syncthreads();

  float max_val = fid < size ? x[fid] : -INFINITY;
  for (int i = fid + 32; i < size; i += 32) {
    if (x[i] > max_val) {
      max_val = x[i];
    }
  }
  ...
  atomic_max.fetch_max(max_val, hip::memory_order_relaxed);
  __syncthreads();
  max_val = atomic_max.load(hip::memory_order_relaxed);

  // exp and sum using atomics
  __shared__ hip::atomic<float, hip::thread_scope_block> atomic_sum;
  if (fid == 0) {
    atomic_sum.store(0.0f, hip::memory_order_relaxed);
  }
  __syncthreads();

  float sum = 0.0f;
  for (int i = fid; i < size; i += 32) {
    x[i] = expf(x[i] - max_val);
    sum += x[i];
  }

  // Atomic add to accumulate sum
  atomic_sum.fetch_add(sum, hip::memory_order_relaxed);
  __syncthreads();
  sum = atomic_sum.load(hip::memory_order_relaxed);
...
void __device__ matmul(float *xout, float *x, float *w, int n, int d, int tid, int num_threads) {
  ...
  for (int i = start_row + hip::this_thread::get_fiber_id(); i < start_row + chunk_size; i += hip::this_thread::get_width()) {
    float val = 0.0f;
    for (int j = 0; j < n; ++j){
      val += w[i * n + j] * x[j];
    }
...
std::unique_ptr<float[]> forward(Transformer *transformer, int token, int pos, int vocab_size) {
  ...
  for (int tid = 0; tid < num_threads; ++tid) {
    threads[tid] = hip::thread(hip::thread::max_width(), [] __device__ (Transformer *t, int token, int pos, int tid) {
      ...
      const int kv_mul = p->n_heads / p->n_kv_heads; // integer multiplier of the kv sharing in multiquery
      ...
      for (unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        matmul(s->q.get(), s->xb.get(), w->wq + l * dim * dim, dim, dim, tid, num_threads);
        matmul(s->k, s->xb.get(), w->wk + l * dim * kv_dim, dim, kv_dim, tid, num_threads);
        matmul(s->v, s->xb.get(), w->wv + l * dim * kv_dim, dim, kv_dim, tid, num_threads);
        barrier_wait<num_threads>();

        // RoPE relative positional encoding: complex-valued rotate q and k in each head
        for (int i = tid; i < p->n_heads; i += num_threads) {
          for (int j = hip::this_thread::get_fiber_id() * 2; j < head_size; j += 2 * hip::this_thread::get_width()) {
        ...
        for (int h = tid; h < p->n_heads; h += num_threads) {
          ...
          for (int t = hip::this_thread::get_fiber_id(); t <= pos; t += hip::this_thread::get_width()) {
          ...
          // Sync all the fibers after the above loop, but not across threads
          __syncthreads();

          // softmax the scores to get attention weights, from 0..pos inclusively
          softmax(att, pos + 1);
          // Sync all the fibers after the above softmax call, but not across threads
          __syncthreads();

          // weighted sum of the values, store back into xb
          float *xb = s->xb.get() + h * head_size;
          for (int i = hip::this_thread::get_fiber_id(); i < head_size; i += hip::this_thread::get_width()) {
            float val = 0.0f;
            for (int t = 0; t <= pos; t++) {
        ...
        matmul(s->xb2.get(), s->xb.get(), w->wo + l * dim * dim, dim, dim, tid, num_threads);

        barrier_wait<num_threads>();
        // residual connection back into x
        if (tid == 0) {
          for (int i = hip::this_thread::get_fiber_id(); i < dim; i += hip::this_thread::get_width()) {
            x[i] += s->xb2[i];
          }
          __syncthreads();
        ...
        matmul(s->hb.get(), s->xb.get(), w->w1 + l * dim * hidden_dim, dim, hidden_dim, tid, num_threads);
        matmul(s->hb2.get(), s->xb.get(), w->w3 + l * dim * hidden_dim, dim, hidden_dim, tid, num_threads);
        ...
        // as we will for SwiGLU, thus each fiber is only reading from array entries it wrote to.
        // A simple __syncthreads() will therefore be sufficient.
        __syncthreads();
        ...
        matmul(s->xb.get(), s->hb.get(), w->w2 + l * dim * hidden_dim, hidden_dim, dim, tid, num_threads);
        barrier_wait<num_threads>();
  ...
  HIP_CHECK(hipMemcpyAsync(logits.get(), logits_device_ptr, vocab_size * sizeof(float), hipMemcpyDeviceToHost, g_stream));
  HIP_CHECK(hipStreamSynchronize(g_stream));
  return logits;
}
