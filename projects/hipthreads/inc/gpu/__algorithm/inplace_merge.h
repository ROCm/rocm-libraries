...
#ifndef __GPU___ALGORITHM_INPLACE_MERGE_H
#define __GPU___ALGORITHM_INPLACE_MERGE_H
                          ...
                          _OutputIterator __result, _Compare&& __comp)
{
    for (; __first1 != __last1; ++__result)
    {
        if (__first2 == __last2)
        {
            gpu::__move<_AlgPolicy>(__first1, __last1, __result);
            return;
        }
...
void __buffered_inplace_merge(
    ...
    _Compare&& __comp,
    typename std::iterator_traits<_BidirectionalIterator>::difference_type __len1,
    typename std::iterator_traits<_BidirectionalIterator>::difference_type __len2,
    typename std::iterator_traits<_BidirectionalIterator>::value_type* __buff) {
...
void __inplace_merge(
    ...
    while (true)
    {
        ...
        {   // __len >= 1, __len2 >= 2
            ...
            __m1 = gpu::__upper_bound<_AlgPolicy>(__first, __middle, *__m2, __comp, std::__identity());
        ...
        else
        {
            ...
            __m2 = gpu::lower_bound(__middle, __last, *__m1, __comp);
        ...
        __middle = gpu::__rotate<_AlgPolicy>(__m1, __middle, __m2).first;
        // __len12 and __len21 now have swapped meanings
        // merge smaller range with recursive call and larger with tail recursion elimination
        if (__len11 + __len21 < __len12 + __len22)
        {
            gpu::__inplace_merge<_AlgPolicy>(
              ...
              _Compare&& __comp)
{
    ...
    difference_type __buf_size = gpu::min(__len1, __len2);
// TODO: Remove the use of std::get_temporary_buffer
_LIBGPU_SUPPRESS_DEPRECATED_PUSH
    gpu::pair<value_type*, std::ptrdiff_t> __buf = std::get_temporary_buffer<value_type>(__buf_size);
_LIBGPU_SUPPRESS_DEPRECATED_POP
    unique_ptr<value_type, __return_temporary_buffer> __h(__buf.first);
    return gpu::__inplace_merge<_AlgPolicy>(
...
{
    gpu::inplace_merge(std::move(__first), std::move(__middle), std::move(__last),
                        __less<typename std::iterator_traits<_BidirectionalIterator>::value_type>());
}

} // namespace gpu

#endif // __GPU___ALGORITHM_INPLACE_MERGE_H
