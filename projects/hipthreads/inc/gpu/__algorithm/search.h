...
#ifndef __GPU___ALGORITHM_SEARCH_H
#define __GPU___ALGORITHM_SEARCH_H
...
gpu::pair<_Iter1, _Iter1> __search_forward_impl(_Iter1 __first1, _Sent1 __last1,
                                           ...
                                           _Proj2& __proj2) {
  if (__first2 == __last2)
    return gpu::make_pair(__first1, __first1); // Everything matches an empty sequence
  while (true) {
    // Find first element in sequence 1 that matchs *__first2, with a mininum of loop checks
    while (true) {
      if (__first1 == __last1) { // return __last1 if no element matches *__first2
        _IterOps<_AlgPolicy>::__advance_to(__first1, __last1);
        return gpu::make_pair(__first1, __first1);
      }
    ...
    while (true) {
      if (++__m2 == __last2) // If pattern exhausted, __first1 is the answer (works for 1 element pattern)
        return gpu::make_pair(__first1, ++__m1);
      if (++__m1 == __last1) { // Otherwise if source exhaused, pattern not found
        return gpu::make_pair(__m1, __m1);
      }
...
gpu::pair<_Iter1, _Iter1> __search_random_access_impl(_Iter1 __first1, _Sent1 __last1,
  ...
  while (true) {
    ...
    while (true) {
      if (++__m2 == __last2)
        return gpu::make_pair(__first1, __first1 + _DiffT1(__size2));
...
gpu::pair<_Iter1, _Iter1> __search_impl(_Iter1 __first1, _Sent1 __last1,
                                   ...
                                   _Proj1& __proj1,
                                   _Proj2& __proj2,
                                   std::enable_if_t<__is_cpp17_random_access_iterator<_Iter1>::value
  ...
  if (__size1 < __size2) {
    return gpu::make_pair(__last1, __last1);
  }
...
#endif // __GPU___ALGORITHM_SEARCH_H
