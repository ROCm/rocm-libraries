...
#ifndef __GPU___ALGORITHM_FIND_END_OF_H
#define __GPU___ALGORITHM_FIND_END_OF_H
...
_LIBGPU_HIDE_FROM_ABI inline _LIBGPU_CONSTEXPR_SINCE_CXX14 gpu::pair<_Iter1, _Iter1> __find_end_impl(
  ...
  if (__first2 == __last2)
    return gpu::pair<_Iter1, _Iter1>(__match_last, __match_last);
  while (true) {
    while (true) {
      if (__first1 == __last1) // if source exhausted return last correct answer (or __last1 if never found)
        return gpu::pair<_Iter1, _Iter1>(__match_first, __match_last);
...
_LIBGPU_HIDE_FROM_ABI _LIBGPU_CONSTEXPR_SINCE_CXX14 _Iter1 __find_end(
    ...
    std::random_access_iterator_tag) {
  typedef typename std::iterator_traits<_Iter1>::difference_type _D1;
...
#endif // __GPU___ALGORITHM_FIND_END_OF_H
