...
__device__ TEST_CONSTEXPR_CXX23 void test_reset_pointer() {
  typedef typename std::conditional<IsArray, A[], A>::type VT;
  const int expect_alive = IsArray ? 3 : 1;
#if TEST_STD_VER >= 11
  {
    using U = gpu::unique_ptr<VT>;
    U u;
    ((void)u);
...
#endif
  {
    gpu::unique_ptr<VT> p(newValue<VT>(expect_alive));
  ...
  if (!TEST_IS_CONSTANT_EVALUATED)
    assert(A::count == 0);
  {
    gpu::unique_ptr<const VT> p(newValue<const VT>(expect_alive));
    if (!TEST_IS_CONSTANT_EVALUATED)
      assert(A::count == expect_alive);
    const A* i = p.get();
    assert(i != nullptr);
...
template <bool IsArray>
__device__ TEST_CONSTEXPR_CXX23 void test_reset_nullptr() {
  typedef typename std::conditional<IsArray, A[], A>::type VT;
  const int expect_alive = IsArray ? 3 : 1;
#if TEST_STD_VER >= 11
  {
...
template <bool IsArray>
__device__ TEST_CONSTEXPR_CXX23 void test_reset_no_arg() {
  typedef typename std::conditional<IsArray, A[], A>::type VT;
  const int expect_alive = IsArray ? 3 : 1;
#if TEST_STD_VER >= 11
  {
