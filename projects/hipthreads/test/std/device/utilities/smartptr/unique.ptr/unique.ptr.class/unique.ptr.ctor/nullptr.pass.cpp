...
// __device__ TEST_CONSTINIT gpu::unique_ptr<int> global_static_unique_ptr_single(nullptr);
// __device__ TEST_CONSTINIT gpu::unique_ptr<int[]> global_static_unique_ptr_runtime(nullptr);
...
#if TEST_STD_VER >= 11
  {
    using U1 = gpu::unique_ptr<VT>;
    using U2 = gpu::unique_ptr<VT, Deleter<VT> >;
...
#endif
  {
    gpu::unique_ptr<VT> p(nullptr);
    assert(p.get() == 0);
  }
  {
    gpu::unique_ptr<VT, NCDeleter<VT> > p(nullptr);
    assert(p.get() == 0);
    assert(p.get_deleter().state() == 0);
  }
  {
    gpu::unique_ptr<VT, DefaultCtorDeleter<VT> > p(nullptr);
    assert(p.get() == 0);
    assert(p.get_deleter().state() == 0);
  }
}
...
__device__ TEST_CONSTEXPR_CXX23 void test_sfinae() {
  ...
  { // the constructor does not participate in overload resolution when
    ...
    using U = gpu::unique_ptr<VT, void (*)(void*)>;
    static_assert(!std::is_constructible<U, decltype(nullptr)>::value, "");
  }
  { // the constructor does not participate in overload resolution when
    // the deleter is not default constructible
    using Del = CDeleter<VT>;
    using U1  = gpu::unique_ptr<VT, NonDefaultDeleter>;
    using U2  = gpu::unique_ptr<VT, Del&>;
    using U3  = gpu::unique_ptr<VT, Del const&>;
