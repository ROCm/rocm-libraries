...
__global__ void gmain() {
  ...
  static_assert(SFINAEs_away<gpu::unique_ptr, int*>);
  // Cannot deduce from (array).
  static_assert(SFINAEs_away<gpu::unique_ptr, int[]>);
  // Cannot deduce from (ptr, Deleter&&).
  static_assert(SFINAEs_away<gpu::unique_ptr, int*, Deleter&&>);
  // Cannot deduce from (array, Deleter&&).
  static_assert(SFINAEs_away<gpu::unique_ptr, int[], Deleter&&>);
  // Cannot deduce from (ptr, const Deleter&).
  static_assert(SFINAEs_away<gpu::unique_ptr, int*, const Deleter&>);
