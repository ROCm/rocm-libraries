...
### Resolved issues
* Fixed `rocsparse_roti` and the generic `rocsparse_rot` routine, whose device kernel computed the element index with 32-bit arithmetic. For sparse vectors with `nnz` of `2^32` (~4.3 billion) or more entries, reachable with 64-bit indices, the block-index multiply overflowed, leaving elements past the overflow point unrotated while low-index elements were processed with the wrong data. The index is now computed in the 64-bit index type, and the kernel uses a grid-stride loop with the launch grid clamped to the device limit.
