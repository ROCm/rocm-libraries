...
### Resolved issues
* Fixed an integer overflow in `rocsparse_csr2hyb`, which computed the ELL element count `ell_width * m` in 32-bit arithmetic. For matrices whose padded ELL part exceeds `INT32_MAX` (~2.1 billion) elements the count wrapped to a small or negative value, which under-sized the ELL device allocations while the fill kernel still wrote the full element count, resulting in out-of-bounds device writes. The element count is now computed and stored in 64-bit arithmetic, and the ELL element index is widened to 64-bit across the CSR-to-ELL, CSR-to-HYB, ELL-to-CSR, and HYB-to-CSR device kernels.
