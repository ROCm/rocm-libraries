...
* Added the `rocsparse_diagonal_mode` enum and the `rocsparse_sptrsv_input_diagonal_mode` and `rocsparse_sptrsm_input_diagonal_mode` set-input values to enable diagonal-only solves in `rocsparse_sptrsv` and `rocsparse_sptrsm`. CSR and CSC formats are supported.
