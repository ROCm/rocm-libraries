...
### Added
* Added `rocsparse_handle_create` to create a handle associated with a user-provided stream. All internal device memory allocation and initialization is stream-ordered on that stream, so handle creation never blocks the calling thread or other GPU streams.
* Added `rocsparse_handle_destroy` to destroy a handle created with `rocsparse_handle_create`, with an optional error descriptor argument.
