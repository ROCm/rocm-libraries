...
### Resolved issues
* Fixed the COO atomic SpMV kernels used by `rocsparse_spmv`/`rocsparse_v2_spmv` (COO and COO AoS formats), whose device kernels computed the global element id in 32-bit arithmetic. For a COO matrix declared with a 64-bit index type and `nnz` of `2^32` (~4.3 billion) or more, the element-id multiply overflowed, leaving non-zeros past the overflow point unaccumulated. The id is now computed in the 64-bit index type, the kernels use a grid-stride loop, and the launch grid is clamped to the device limit.
