...
### Resolved issues
* Fixed `rocsparse_sptrsv_get_output` for ELL matrices, which returned `rocsparse_singularity_none` and position `-1` for the generic singularity outputs even when analysis or compute had detected a missing or exactly-zero diagonal pivot. The ELL triangular-solve info is now connected in the singularity type and position queries, matching CSR, COO, and CSC.
