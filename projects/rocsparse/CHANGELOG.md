...
* Fixed an out-of-bounds read of the row map in the block-row incomplete factorization kernels used by `rocsparse_spic0` and `rocsparse_spilu0`. Eight of the nine BSR kernels indexed the map with the raw block index and never compared it against the block row count, so a launch grid rounded up past that count read outside the map allocation. All nine now bounds check, which is what the CSR kernels in the same family already did.
* Fixed `rocsparse_spic0` and `rocsparse_spilu0` silently truncating the launch grid for matrices with more block rows than the hardware grid.x maximum of 2147483647. The block row count is taken from the sparse matrix descriptor as a 64-bit value and was narrowed into the 32-bit grid extent without a clamp. The general BSR kernels now clamp the extent and recover the remaining block rows inside the kernel.
* Fixed a signed integer overflow in the `rocsparse_csritilu0` async-inplace preprocessing, which computed four launch grid extents as `(target_size + BLOCKSIZE - 1) / BLOCKSIZE`. The addition overflowed 32-bit `rocsparse_int` before the division for target sizes above 2147482625, which is undefined behaviour. The extents are now computed as `(target_size - 1) / BLOCKSIZE + 1`, the form used everywhere else in the directory, which cannot overflow for any representable positive size.
