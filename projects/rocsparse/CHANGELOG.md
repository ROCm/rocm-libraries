...
### Resolved issues
* Fixed the ELL SpMV kernels used by `rocsparse_spmv` (ELL format) and the legacy `rocsparse_Xellmv` routines, whose device kernels computed the row index as the product of a 32-bit block index and block size, so the index wrapped at `2^32` (~4.3 billion) even when the matrix was declared with a 64-bit index type. For an ELL matrix with more than `2^32` rows, rows past the wrap point were mapped back onto low row ids and their results were left uncomputed. The row index is now computed in the 64-bit index type.
