...
// 32 channels per group, TF32 input/delta (fp32 in HBM, simulated via BF16
// MFMA).
//
// Macro structure (H-streaming + double LDS ping-pong + acc[kh*kw] held across
// the full input height + atomicAdd cross-workgroup reduction + 4 waves/group
// with input_half x delta_half quadrant tiling + optional wave_n N-tiling +
// padded staging) is identical to the BF16/FP16 sibling kernel
// grouped_32c_wgrad_fp16bf16.cpp. The 4-wave/group + wave_n + padded staging
// design is dtype-independent and reused verbatim.
//
// Two TF32-specific design points:
//
...
//    big / small halves of input and delta so each ds_read_tr16_b64 still
//    reads a contiguous bf16x4. The 3-term expansion (big*big + big*small +
//    small*big, small*small dropped) lives inside the
//    mfma_16x16x32(bf16_pair_x8, bf16_pair_x8, fp32x4) wrapper in
//    mfma_dispatch.h, shared with the tf32 path of grouped_32c.cpp
//    and grouped_16c_wgrad_tf32.cpp.
//
// 2) Two-stage HBM->LDS ("issue + commit"). FP16/BF16 wgrad uses
//    buffer_load_lds to stream HBM bytes straight into LDS. TF32 needs the
//    fp32 -> BF16 pair conversion in registers between HBM and LDS, so we
//    split the transfer:
//        load_*_global  -> __builtin_amdgcn_raw_buffer_load_b128 (HBM -> VGPR)
//        commit_*_lds   -> fp32xN_to_bf16_pair + 2x ds_write_b64 (VGPR -> LDS)
//
// LDS geometry, swizzle, wave grid (4 waves/group + wave_n), staging layout
// (padded STAGING_K_STRIDE = GROUP_SIZE + 1) are unchanged from
// grouped_32c_wgrad_fp16bf16.cpp - see that file for the streaming wgrad /
// 4-quadrant wave / padded staging rationale.
...
{
namespace grouped_32c_wgrad_tf32
{
...
// Mirrors the fp16/bf16 32c wgrad config ladder. TF32 LDS occupancy is ~2x
// fp16/bf16 wgrad (4 LDS halves at uint2 granularity vs 2 LDS regions at
// uint4 granularity); each combination of (groups_per_wg, wave_q32, wave_n)
// still fits within the 65 KB compute / 76 KB staging budget.
...
template <Config cfg>
__device__ void conv2d_grouped_32c_wgrad_tf32_cdna4_nhwc_impl(const float* __restrict__ input,
                                                              ...
                                                              int px)
{
    ...
    constexpr int INPUT_LDS_BUFFER_SIZE_U2 = BLOCK_C4 * BLOCK_W_TOTAL * cfg.wave_n;
    constexpr int DELTA_LDS_BUFFER_SIZE_U2 = BLOCK_C4 * BLOCK_Q_TOTAL * cfg.wave_n;
    ...
    constexpr int STAGING_K_STRIDE = GROUP_SIZE + 1; // padded by 1 to break the 32-bank alignment
    ...
    // Wave mapping: 4 waves per group, each computing a 16x16 quadrant.
    const int wave_group    = wave / WAVES_PER_GROUP;
    const int wave_in_group = wave % WAVES_PER_GROUP;
    const int input_half    = wave_in_group / 2;
    const int delta_half    = wave_in_group % 2;
    ...
    bf16_pair_x8 delta_regs[cfg.kh][cfg.wave_n * cfg.wave_q32];
    ...
    constexpr int INPUT_LOAD_PASSES =
        divup(BLOCK_W_TOTAL * cfg.wave_n * BLOCK_C4, cfg.block_size());
    constexpr int DELTA_LOAD_PASSES =
        divup(BLOCK_Q_TOTAL * cfg.wave_n * BLOCK_C4, cfg.block_size());

    // SwizzleT::offset_uint2 inverse, used to map a linear uint2 LDS slot to
    // (col_total, c4_idx) for global-memory addressing on the write side. A uint2 slot is
    ...
    // For 32c, each wave operates on a 16-channel half (input_half or
    // delta_half selects which half), so lane_c4 covers 16/4 = 4 c4 slots.
    ...
    auto load_delta_q32_lds = [&](int buf, int x_base) -> bf16_pair_x8 {
    ...
    {
        // Per-pass load info, invariant across rows. wave_n N-tiling stacks
        // n_local in the col_total dimension; the swizzle inverse recovers
        // (col_total, c4_idx) and then col_total / BLOCK_W_TOTAL splits into
        // (n_local, col).
            ...
            if(input_active[pass])
            {
                ...
                if(actual_n < N && global_col >= 0 && global_col < wi)
                    input_global_offsets[pass] =
                        sizeof(uint4) * ((size_t)actual_n * hi * wi * C4 + (size_t)global_col * C4 +
                                         block_c4 + c4_idx);
                else
        ...
        {
            int lds_idx_u2          = tid + pass * cfg.block_size();
            delta_active[pass]      = (lds_idx_u2 < BLOCK_Q_TOTAL * cfg.wave_n * BLOCK_C4);
            ...
            if(delta_active[pass])
            {
                ...
                if(actual_n < N && global_q >= 0 && global_q < wo)
                    delta_global_offsets[pass] =
                        sizeof(uint4) * ((size_t)actual_n * ho * wo * C4 + (size_t)global_q * C4 +
                                         block_c4 + c4_idx);
                else
    ...
    // Epilogue: LDS-staged coalesced atomicAdd with padded staging row.
    // Same structure as fp16/bf16 32c wgrad - the BF16-pair scheme only
    // affects the compute loop; here the accumulators are already plain
    // fp32x4. The 4-quadrant wave layout writes to disjoint (k, c) slots
    // in the staging tile so no atomic is needed within the workgroup.
...
template <Config cfg>
__global__ void conv2d_grouped_32c_wgrad_tf32_nhwc_cdna4(const float* __restrict__ input,
                                                         const float* __restrict__ delta,
                                                         float* __restrict__ wgrad,
                                                         int N,
                                                         int groups,
                                                         ...
                                                         int px)
{
    ...
    {
        conv2d_grouped_32c_wgrad_tf32_cdna4_nhwc_impl<cfg>(input,
                                                           delta,
                                                           wgrad,
                                                           N,
                                                           groups,
                 ...
                 hipStream_t stream)
{
    ...
    conv2d_grouped_32c_wgrad_tf32_nhwc_cdna4<cfg>
...
class Grouped_32C_Wgrad_TF32_ConvKernel : public GroupedWgradConvKernel
{
public:
    constexpr Grouped_32C_Wgrad_TF32_ConvKernel(const Config& cfg, LaunchFn launch_fn)
...
HIPCONV_DEFINE_KERNEL_TABLE(Grouped_32C_Wgrad_TF32_ConvKernel);
...
HIPCONV_EXPORT_KERNEL_TABLE(grouped_32c_wgrad_tf32_cdna4_kernels,
                            hipconv::cdna4::grouped_32c_wgrad_tf32);
#endif
