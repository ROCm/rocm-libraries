...
template <typename Tgpu_ /* the data type used in GPU computations (usually half) */,
          typename Tcheck_ /* the data type used in CPU checkings (usually double) */>
int mloLRNForwardRunHost_opt(bool do_scale,
                         int norm_region,
                         int pad,
                         int local_area,
                         Tcheck_ alphaoverarea,
                         ...
                         bool multi_threaded)
{
    ...
    {
        std::cout << "ERROR: Lrn kernel size is insufficient." << std::endl;
        std::ofstream("/data/Dev/LRN-OPT-" + std::string{typeid(Tgpu_).name()} + "-" + (multi_threaded ? "MT" : "ST") + "-Forward.csv", std::ofstream::app) << "ERROR: Lrn kernel size is insufficient." << std::endl;
        return -1;
    }
    ...
    const auto t1 = std::chrono::high_resolution_clock::now();
    const auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
    std::ofstream("/data/Dev/LRN-OPT-" + std::string{typeid(Tgpu_).name()} + "-" + (multi_threaded ? "MT" : "ST") + "-Forward.csv", std::ofstream::app) << ns << std::endl;
...
template <typename Tgpu_ /* the data type used in GPU computations (usually half) */,
          typename Tcheck_ /* the data type used in CPU checkings (usually double) */>
int mloLRNBackwardRunHost_opt(int norm_region,
                          int pad,
                          int local_area,
                          Tcheck_ /*alphaoverarea*/,
                          Tcheck_ alpha,
                          ...
                          bool multi_threaded)
{
    ...
    {
        std::cout << "ERROR: Lrn kernel size is insufficient." << std::endl;
        std::ofstream("/data/Dev/LRN-OPT-" + std::string{typeid(Tgpu_).name()} + "-" + (multi_threaded ? "MT" : "ST") + "-Backward.csv", std::ofstream::app) << "ERROR: Lrn kernel size is insufficient." << std::endl;
        return -1;
    }
    ...
    const auto t1 = std::chrono::high_resolution_clock::now();
    const auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
    std::ofstream("/data/Dev/LRN-OPT-" + std::string{typeid(Tgpu_).name()} + "-" + (multi_threaded ? "MT" : "ST") + "-Backward.csv", std::ofstream::app) << ns << std::endl;
