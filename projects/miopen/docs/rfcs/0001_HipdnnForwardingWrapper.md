...
>
> - The pre-wrapper `libMIOpen.so` snapshot captured for the abidiff gate (§4.6, §6) is taken from a library whose exported surface is the public C API rather than the public C API plus accumulated internals.
...
> - The header-leakage and abidiff CI gates added in Phase 1 task 8 are checking the intended surface (public C API only) rather than locking in today's polluted one.
>
...
6. Implement the failure-mode behavior defined in §6.1: load-time fallback to Private with a one-time warning, per-call hipDNN errors propagated (not silently absorbed), and the `miopenGetLastForwardedError()` thread-local + `[hipDNN-forwarded]` error-string prefix. *(Note: the Phase 2 forwarding prototype currently falls back silently on **any** hipDNN failure (investigation reference §6 concern 4), which is broader than this design. Reconciling the prototype's blanket fallback with §6.1's "decline-unsupported-up-front but propagate genuine per-call errors" split is part of this task.)*
