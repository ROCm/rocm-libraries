...
- **Forwarding shims** — #7253 left thin shims at the old `src/include/miopen/` paths for backward-compat; cleaner is to rewrite include sites directly (as #7254 then did). Prefer **direct rewrites from the start** to avoid a second pass, unless review rejects the churn as too large for one PR. Any shim used is tracked debt with a follow-up to remove it.
...
- **Forwarding-shim debt.** If shims are introduced for compat (as #7253 did), they must be cleaned up later (as #7254 started). Prefer direct includes to avoid the second pass — but weigh against PR size.
