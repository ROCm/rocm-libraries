...
// ============================================================================
// Regression repros for PR #5240 (softmax optimization) review.
//
// The default perf config sets vectorized = (stride == 1), so any stride-1
// problem (all INSTANCE mode, all NHWC-channel) takes the int4-vectorized
// load/store path with no tuning required.
// ============================================================================

// --- Bug 1: misaligned int4 vectorized load/store -------------------------
...
// 8 for fp16). For inner sizes that are NOT a multiple, the odd-o blocks issue
// a 16-byte-misaligned vector load/store -> UB (memory fault or wrong result).
// Reuses SoftmaxCommon's full CPU-vs-GPU comparison.
INSTANTIATE_TEST_SUITE_P(
    Repro_PR5240_Misalign,
    GPU_Softmax_FP32,
    testing::Values(
        // INSTANCE inner = C*H*W, none a multiple of 4:
        TestCase{{2, 50, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW},
        TestCase{{3, 10, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW},
        TestCase{{2, 6, 1, 1}, {0.5f, 0.5f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW},
        TestCase{{2, 50, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_LOG, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW},
        // NHWC-channel: inner = C, stride = 1, outer = N*H*W:
        TestCase{{2, 50, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_CHANNEL, miopenTensorNHWC}));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GPU_Softmax_FP16,
    testing::Values(
        TestCase{{2, 13, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW},
        TestCase{{2, 21, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW},
        TestCase{{3, 101, 1, 1}, {1.0f, 0.0f}, MIOPEN_SOFTMAX_ACCURATE, MIOPEN_SOFTMAX_MODE_INSTANCE, miopenTensorNCHW}));
...
// --- Bug 3: non-contiguous (non-packed) tensor ----------------------------
// GetStride/GetOuterSize/GetInnerSize derive geometry from lengths + layout
// enum only and ignore the descriptor's real strides. base develop passed the
...
struct GPU_Softmax_PR5240_NonContig_FP32 : public testing::Test
{
};

TEST_F(GPU_Softmax_PR5240_NonContig_FP32, ChannelPaddedNStride)
{
    auto&& handle = get_handle();
    ...
    EXPECT_LT(max_err, 1e-4) << "Non-packed tensor mis-addressed: GPU output does not "
                                "match strided CPU reference (max abs err " << max_err << ").";
}

// --- Extra coverage: non-zero x/y offsets (PRE-EXISTING issue, NOT this PR) --
// This PR moved x_offset/y_offset/dx_offset/dy_offset out of InvokeParams and
// into ProblemDescription (baked into the kernel as X_OFFSET/Y_OFFSET/...). No
// existing test passes a non-zero value, so the offset path is uncovered.
//
// Findings from the tests below (both DISABLED -- see caveats):
//   * The Softmax solver (this PR's kernel) DOES honor offsets correctly.
//   * But SoftmaxForward runs Find over {AttnSoftmax, Softmax}; AttnSoftmax
//     (a separate solver, NOT modified by this PR) ignores the offsets and can
...
//   * This is PRE-EXISTING and orthogonal to this PR, and the selection is
//     non-deterministic, so these are DISABLED to keep the suite deterministic
//     and avoid mis-attributing the failure. They are kept as evidence for a
//     separate AttnSoftmax-offset ticket.
struct GPU_Softmax_PR5240_Offset_FP32 : public testing::Test
{
};
...
TEST_F(GPU_Softmax_PR5240_Offset_FP32, DISABLED_ForwardNonZeroOffset_FindRace)
{
...
// --- Bug 2: divergent __syncthreads() deadlock ----------------------------
...
//       --gtest_filter='*PR5240_Deadlock*'
struct GPU_Softmax_PR5240_Deadlock_FP32 : public testing::Test
{
};

TEST_F(GPU_Softmax_PR5240_Deadlock_FP32, DISABLED_CsrStreamPartialBlock)
{
    auto&& handle = get_handle();
