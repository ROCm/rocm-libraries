...
struct conv_test_input // TODO: check all the fields if they're parameters
{
    ...
    bool do_backward_data    = false;
    bool do_backward_weights = false;
    int search               = 1;
    bool gen_float           = true;
...
struct conv_test_base : public ::testing::TestWithParam<conv_test_input>
{
    ...
    void Run()
    {
        if(!param.input_dims.empty())
            ...
            filter.spatialDim = param.filter_dims.size();
        bool is_int8 = (input.desc.GetType() == miopenInt8);

        filter.mode             = cmode_lookup.at(miopen::ToUpper(param.conv_mode));
        filter.paddingMode      = pmode_lookup.at(miopen::ToUpper(param.pad_mode));
        std::size_t spatial_dim = filter.GetSpatialDimension();
        filter.group_count      = std::max(static_cast<int>(param.groupCount), 1);

        miopenTensorLayout_t input_layout_t =
            StringToLayoutType(param.in_layout, param.tensor_vect, param.vector_length);
        miopenTensorLayout_t weight_layout_t =
            StringToLayoutType(param.fil_layout, param.tensor_vect, param.vector_length);
        ...
        else if(spatial_dim == 2)
        {
            ...
            input = tensor<T>{input_layout_t,
                              ...
                              param.spatial_dim_elements.at(0),
                              param.spatial_dim_elements.at(1)}
                        .generate(tensor_elem_gen_integer{17});
        }
        else if(spatial_dim == 3)
        {
            input = tensor<T>{param.batch_size,
                              param.input_channels,
                              param.spatial_dim_elements.at(0),
                              param.spatial_dim_elements.at(1),
                              param.spatial_dim_elements.at(2)}
                        .generate(tensor_elem_gen_integer{17});
        }

        if(!param.weight_tensor_dims.empty())
        {
            if(param.fil_layout == "CHWN")
            {
                weights = tensor<T>{weight_layout_t, param.weight_tensor_dims}.generate(
                    tensor_elem_gen_integer{17});
                param.output_channels = param.weight_tensor_dims.at(3);
                std::copy(param.weight_tensor_dims.begin() + 1,
                          param.weight_tensor_dims.end() - 1,
        ...
        {
            if(param.fil_layout == "NCHW")
            {
                weights = tensor<T>{weight_layout_t,
                                    param.output_channels,
                                    param.input_channels / filter.group_count,
                                    param.filter_dims.at(0),
                                    param.filter_dims.at(1)}
                              .generate(tensor_elem_gen_integer{17});
            }
            else if(param.fil_layout == "CHWN")
            {
                weights = tensor<T>{weight_layout_t,
                                    param.input_channels / filter.group_count,
                                    param.filter_dims.at(0),
            ...
            {
                weights = tensor<T>{param.output_channels,
                                    param.input_channels / filter.group_count,
                                    param.filter_dims.at(0),
                                    param.filter_dims.at(1)}
                              .generate(tensor_elem_gen_integer{17});
        ...
        else if(spatial_dim == 3)
        {
            weights = tensor<T>{param.output_channels,
                                ...
                                param.filter_dims.at(0),
                                param.filter_dims.at(1),
                                param.filter_dims.at(2)}
                          .generate(tensor_elem_gen_integer{17});
        }

        if(input.desc.GetNumDims() != param.in_layout.size() ||
           weights.desc.GetNumDims() != param.fil_layout.size() ||
           input.desc.GetNumDims() != param.out_layout.size())
        {
            std::cout << input.desc.GetNumDims() << "," << param.in_layout.size() << std::endl;
            std::cout << weights.desc.GetNumDims() << "," << param.fil_layout.size() << std::endl;
            std::cout << input.desc.GetNumDims() << "," << param.out_layout.size() << std::endl;
        ...
        if(param.in_layout != "NCHW" && param.in_layout != "NCDHW")
        {
        ...
        if(param.fil_layout != "NCHW" && param.fil_layout != "NCDHW" && param.fil_layout != "CHWN")
        {
        ...
        if(input.desc.GetNumDims() != 2 + spatial_dim ||
           ...
           param.pads_strides_dilations.size() != 3 * spatial_dim ||
           param.trans_output_pads.size() != spatial_dim)
        {
        ...
        std::copy_n(param.trans_output_pads.begin(), spatial_dim, filter.trans_output_pads.begin());
             ...
             (filter.group_count > 1 && in_c_len % wei_c_len == 0))))
        {
            ...
            {
                auto output = get_output_tensor<T, Tout>(filter, input, weights, param.out_layout);
                ...
                if(is_int8)
                {
                    ...
                    auto output_int8 =
                        get_output_tensor<T, Tout>(filter, input, weights, param.out_layout);
                ...
                {
                    miopen::debug::testing_find_db_enabled = param.enable_fdb;
                }

                conv_stats stats;

                if(param.do_forward && !skip_forward)
                {
                    if(is_int8)
                    {
                        ...
                        {
                            std::cout << "FAILED: bad output_type: '" << param.output_type << '\''
                                      << std::endl;
                            fail();
                        }
                    }
