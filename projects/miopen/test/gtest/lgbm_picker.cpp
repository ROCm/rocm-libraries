...
//     (LgbmForest) must reproduce LightGBM's own raw lambdarank scores within a
//     tight tolerance on committed (features -> score) vectors. The golden
//     scores are produced by the LightGBM Python API from the exact model
//     shipped in the tree, so this is the real correctness guard for the walker
//     that replaced the Treelite-generated C.
...
// ---------------------------------------------------------------------------
// Golden-vector parity: the in-house forest walker vs LightGBM's own scores
// ---------------------------------------------------------------------------
...
// categorical bitset membership, and the missing/default-left path -- the parts
// most at risk in a hand-written tree walk -- against ground truth generated by
// the LightGBM Python API from the exact model shipped in the tree.
