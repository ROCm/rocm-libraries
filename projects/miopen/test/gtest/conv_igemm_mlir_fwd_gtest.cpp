...
template <typename T>
struct conv_igemm_mlir_fwd_gtest : public conv2d_test_base<T, TestCase>
{
    void SetUp() override
    {
        prng::reset_seed();
...
using GPU_Conv2dMLIRTestIGemmFwd_FP16 = conv_igemm_mlir_fwd_gtest<half_float::half>;
using GPU_Conv2dMLIRTestIGemmFwd_FP32 = conv_igemm_mlir_fwd_gtest<float>;
using GPU_Conv2dMLIRTestIGemmFwd_I8   = conv_igemm_mlir_fwd_gtest<int8_t>;

TEST_P(GPU_Conv2dMLIRTestIGemmFwd_FP16, TestFloat16)
{
...
TEST_P(GPU_Conv2dMLIRTestIGemmFwd_FP32, TestFloat32)
{
    if(IsTestSupportedForDevice())
    {
        ScopedEnvironment<std::string> find_mode(MIOPEN_FIND_MODE, "normal");
...
TEST_P(GPU_Conv2dMLIRTestIGemmFwd_I8, TestInt8)
{
    if(IsTestSupportedForDevice())
    {
        ScopedEnvironment<std::string> find_mode(MIOPEN_FIND_MODE, "normal");
...
#define INSTANTIATE_MIOPEN_FULL_FWD_TEST_SUITE(id, ...)                                    \
    INSTANTIATE_MIOPEN_FULL_TEST(                                                          \
        id, GPU_Conv2dMLIRTestIGemmFwd_FP16, half_float::half, __VA_ARGS__);               \
    INSTANTIATE_MIOPEN_FULL_TEST(id, GPU_Conv2dMLIRTestIGemmFwd_FP32, float, __VA_ARGS__); \
    INSTANTIATE_MIOPEN_FULL_TEST(id, GPU_Conv2dMLIRTestIGemmFwd_I8, int8_t, __VA_ARGS__)
