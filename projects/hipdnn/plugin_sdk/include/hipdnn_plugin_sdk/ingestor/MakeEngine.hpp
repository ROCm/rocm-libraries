...
/// @param engineName The engine's scoped name, used only to locate its on-disk
///        winner-cache shard (see `KernelIngestorStateManager`'s @param engineName
///        note). Defaulted from @p set, same as @p describedBy -- a caller that already
///        moved `set.engine` out (as makeEngine() below does) must pass it explicitly,
///        or the state manager gets an empty name and disables its disk cache.
...
{
    // Every read of the UED happens in its own statement, fully sequenced before the
    // moves below: the arguments of one call are unsequenced with respect to each
    // other, so reading any of them inside the call could observe an already-moved-from
    // engine -- silently resolving no graph match, warning about an engine it cannot
    // name, and (per this task) writing every engine's winner cache to one shared,
    // unnamed shard. engineName is captured here for exactly the same reason
    // describedBy already was.
