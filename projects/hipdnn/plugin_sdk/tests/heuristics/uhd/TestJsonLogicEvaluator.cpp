...
using hipdnn_plugin_sdk::uhd::JsonLogicEvaluator;
using hipdnn_plugin_sdk::uhd::VariableContext;
...
TEST_F(TestJsonLogicEvaluator, ModeratelyNestedAllPredicateStillEvaluates)
{
    ...
    const VariableContext ctx;
    const JsonLogicEvaluator eval;
    EXPECT_DOUBLE_EQ(eval.evaluateDouble(JsonLogicEvaluator::parse(expr), ctx), 1.0);
}
