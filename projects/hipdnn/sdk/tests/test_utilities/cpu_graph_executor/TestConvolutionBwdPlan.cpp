...
TEST_F(TestConvolutionBwdPlan, ExecutePlan)
{
    ...
    ConvolutionBwdPlan<float, float, float> patient(std::move(params));
    ...
    CpuFpReferenceConvolutionImpl<float, float, float>::convBwdData(directTensorBundle.dxTensor,
...
TEST(TestConvolutionBwdPlanBuilder, PlanConstruction)
{
    ...
    ConvolutionBwdPlanBuilder<DataType::FLOAT, DataType::FLOAT, DataType::FLOAT> patient;

    auto builtPlan = patient.buildNodePlan(graphWrap, graphWrap.getNode(0));

    bool result
        = dynamic_cast<ConvolutionBwdPlan<float, float, float>*>(builtPlan.get()) != nullptr;
    EXPECT_TRUE(result);
}
...
TEST(TestConvolutionBwdPlanBuilder, IsApplicable)
{
    ...
    ConvolutionBwdPlanBuilder<DataType::FLOAT, DataType::FLOAT, DataType::FLOAT> floatPlanBuilder;

    EXPECT_TRUE(floatPlanBuilder.isApplicable(graphWrap.getNode(0), graphWrap.getTensorMap()));

    auto tensorMapCopy = graphWrap.getTensorMap();
    tensorMapCopy.erase(2);
    ConvolutionBwdPlanBuilder<DataType::FLOAT, DataType::FLOAT, DataType::HALF> badTypesPlanBuilder;
