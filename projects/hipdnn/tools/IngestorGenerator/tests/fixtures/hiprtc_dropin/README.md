...
`valid.yaml` is the baseline that loads and generates. Every rejection fixture
differs from it in exactly one place, so the test asserting the rejection is
asserting about that place and nothing else. `pack_discriminates.yaml` and
`installed_symbols_new_name.yaml` also load and generate: they are the two
optional `engine` keys, each exercised alone against the same baseline.

| Fixture | Rejected by | Because |
|---|---|---|
| `valid.yaml` | — | loads, generates, stages the bundle |
| `pack_discriminates.yaml` | — | loads and generates; the single-pack discriminator warning is silent because the author stated the fact the generator cannot see |
| `installed_symbols_new_name.yaml` | — | loads and generates; `native_symbol_namespace` keeps the installed pack's symbols while `name` is the drop-in's own, so the two engines have distinct ids |
...
| `multi_pack_discriminates.yaml` | load | `engine.pack_discriminates` on a two-pack engine: the key answers a question only a single-pack engine has, and a multi-pack engine discriminates in descriptors instead |
