...
def _require_real_profile_with_sources(profile_path: Path) -> dict:
    """Skip unless both the profile AND the provider sources it cites are present.

    A profile's ``cpp_mirror``/``test`` paths point into the pack it describes, so a
    checkout carrying the generator without that pack makes every structural check
    fail on absent files -- a fact about what this tree ships, not about whether the
    profile is sound. The existing profile-absent skip covers only half of that; this
    ...
    cited = {
        ...
        for key in ("cpp_mirror", "test")
        if surface.get(key) and surface[key] != "none"
    }
    missing = [path for path in cited if not (_REPO_ROOT / path).exists()]
    if missing:
        pytest.skip(f"the pack this profile describes is not in this checkout: {missing[0]}")
    return profile
...
class TestAgainstTheRealProfile:
    ...
    def test_the_real_profile_check_names_only_genuinely_unguarded_surfaces(self):
        profile = _require_real_profile_with_sources(_REAL_PROFILE)
        failures, unguarded = launch_surface.check(profile, _REPO_ROOT)
        assert failures == [], (
    ...
    def test_the_real_profile_passes_with_allow_unguarded(self):
        _require_real_profile_with_sources(_REAL_PROFILE)
        result = _run(
