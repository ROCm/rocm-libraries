...
// The server's stderr is free-form diagnostics and never protocol, so it is the
// only thing that explains a failure to start -- a missing dependency, an
...
async function openClient() {
  ...
  transport.stderr?.on("data", (chunk) => {
    serverStderr = (serverStderr + chunk.toString("utf8")).slice(-STDERR_KEEP);
  });

  const instance = new Client({ name: CLIENT_NAME, version: app.getVersion() });
  ...
  if (!connecting) {
    connecting = openClient().then(
      ...
      (err) => {
        ...
        reconnectAt =
          Date.now() +
          RECONNECT_BACKOFF_MS[Math.min(reconnectFailures - 1, RECONNECT_BACKOFF_MS.length - 1)];
        throw err;
      }
    );
  }
...
function handleClosed(instance) {
  ...
  // The poll drives the reconnect for as long as a run is live; a session with
  // nothing running reconnects on the next call instead.
  reconnectAt = Date.now() + RECONNECT_BACKOFF_MS[0];
}
...
// A new server knows nothing about the runs it did not start, so every live one
// is re-subscribed and re-read before the renderer is told it is back.
...
function registerNotifications(instance) {
  ...
  // Something appeared or finished; the runs we are tracking may be the ones it
  // happened to.
...
const handlers = {
  ...
  "flow:revealArtifact": ({ uri }) =>
    envelope(async () => {
      shell.showItemInFolder(await artifactPath(uri));
      return {};
    }),
};
