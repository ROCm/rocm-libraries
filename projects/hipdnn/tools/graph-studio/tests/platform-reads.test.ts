...
// `resolveAndContain` returns whatever `path.resolve` produces, which is
// drive-prefixed and backslash-separated on Windows. Anchor the base the same
// way and build the expectations with `join`, so these assert containment
// rather than one platform's spelling of a path.
const BASE = resolve("/home/user/reports");

test("resolves a plain relative path", () => {
  expect(resolveAndContain(BASE, "trace.pftrace")).toBe(join(BASE, "trace.pftrace"));
});

test("resolves a nested relative path", () => {
  expect(resolveAndContain(BASE, "traces/sample.pftrace")).toBe(
    join(BASE, "traces", "sample.pftrace"),
  );
});
...
test("allows a path that normalises back inside the base", () => {
  expect(resolveAndContain(BASE, "traces/../trace.pftrace")).toBe(join(BASE, "trace.pftrace"));
});
