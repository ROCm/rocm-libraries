...
checks belong at matching when available, otherwise at preparation. Keep the launch
arithmetic a pure function of descriptor metadata in a `<Pack>Geometry.hpp` beside the
pack, tested per shape family: `Gfx942AttentionDenseGeometry.hpp` and its test are the
worked pattern.
