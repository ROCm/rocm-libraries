...
- **Absent and explicitly null are both absence** — read them identically and
  explicitly at the read site (`Knowledge/hipdnn/dnn-benchmarking-null-attribute-defaults.md`).
...
- **Mask semantics (SDPA).** Deprecated `causal_mask` / `causal_mask_bottom_right`
  ...
  error (`Knowledge/hipdnn/sdpa-mask-attribute-precedence.md`). Read the precedence,
  do not infer it.
...
| "supported shapes" as an enumerated table | do not | this is the coverage model of existing engines, recorded as a limitation, not a design to copy (`Knowledge/hipdnn/rocke-gfx950-supported-shapes-are-a-descriptor-table.md`) |
