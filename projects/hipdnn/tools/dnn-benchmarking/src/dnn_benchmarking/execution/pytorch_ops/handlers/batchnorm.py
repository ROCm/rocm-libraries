...
def compile_batchnorm_inference(
    node: Dict[str, Any],
    graph_json: Dict[str, Any],
) -> CompiledOp:
    """Plan batchnorm inference with precomputed inverse variance."""
...
def compile_batchnorm_inference_variance(
    node: Dict[str, Any],
    graph_json: Dict[str, Any],
) -> CompiledOp:
    """Plan batchnorm inference with variance and epsilon.
...
def compile_batchnorm_training(
    node: Dict[str, Any],
    graph_json: Dict[str, Any],
) -> CompiledOp:
    ...
    save_mean_uid = _optional_uid(node, "mean_tensor_uid")
    save_inv_uid = _optional_uid(node, "inv_variance_tensor_uid")
    ...
    update_running = any(uid is not None for uid in running_present)
    if update_running and not all(uid is not None for uid in running_present):
        raise ValueError(
...
def compile_batchnorm_backward(
    node: Dict[str, Any],
    graph_json: Dict[str, Any],
) -> CompiledOp:
    ...
    saved_stats = mean_uid is not None
