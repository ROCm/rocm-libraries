...
def compile_reduction(
    ...
    """Plan hipDNN reduction attributes with PyTorch reductions."""
    ...
    in_uid = int(in_uid)
    out_uid = int(out_uid)
    out_shape = _tensor_shape(graph_json, out_uid)
    mode = _reduction_mode_name(_node_param(node, "mode", "NOT_SET"))

    def run(tensors: Dict[int, torch.Tensor]) -> None:
        x = _tensor(tensors, in_uid, node)
        dims, keepdim = _reduction_dims_for_output(x, out_shape)

        # Assume float32 accumulation: promote low-precision floating inputs so
        # the reduction accumulates in fp32, then cast the result back to the
        # graph dtype on store. Integer inputs are reduced in their own dtype.
        promote = x.is_floating_point() and x.dtype not in (
            torch.float32,
            torch.float64,
        )
