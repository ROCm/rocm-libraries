...
def _storage_view(storage: np.ndarray, tensor_info: TensorInfo) -> np.ndarray:
    ...
    if tensor_info.strides:
        byte_strides = tuple(stride * storage.dtype.itemsize for stride in tensor_info.strides)
...
def _dense_from_storage_bytes(data_bytes: bytes, tensor_info: TensorInfo, dtype: np.dtype) -> np.ndarray:
...
def _bfloat16_storage_bytes_to_ndarray(data_bytes: bytes, tensor_info: TensorInfo) -> np.ndarray:
    """Decode raw bfloat16 storage bytes into a dense float32 logical ndarray."""
    storage = np.frombuffer(data_bytes, dtype=np.uint16, count=tensor_info.storage_elements)
...
def _encode_bfloat16_dense_to_storage_bytes(data_f32: np.ndarray, tensor_info: TensorInfo) -> bytes:
    ...
    def fill_inputs_random(self, seed: Optional[int] = None) -> None:
        ...
        for tensor_info in self._tensor_infos:
            ...
            if tensor_info.is_pass_by_value:
                ...
                self._host_data[tensor_info.uid] = np.asarray([tensor_info.value], dtype=dtype)
                continue
            ...
            if dtype_key == "bfloat16":
                ...
                self._host_data[tensor_info.uid] = data
                if buffer:
                    buffer.copy_from_host(_encode_dense_to_storage_bytes(data, tensor_info))
