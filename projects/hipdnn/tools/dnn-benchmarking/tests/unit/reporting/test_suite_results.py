...
class TestTimingStats:
    ...
    def test_from_timings_correct_values(self):
        """Test 1: TimingStats.from_timings([1.0, 2.0, 3.0]) produces correct
        mean, std, min, max, p95, p99."""
        stats = TimingStats.from_timings([1.0, 2.0, 3.0])

        assert stats.mean_ms == pytest.approx(2.0)
        assert stats.std_ms == pytest.approx(1.0)
        assert stats.min_ms == pytest.approx(1.0)
        assert stats.max_ms == pytest.approx(3.0)
        assert stats.p95_ms == pytest.approx(2.9)
        assert stats.p99_ms == pytest.approx(2.98)

    def test_from_timings_single_value(self):
        """TimingStats with single value has zero std."""
        stats = TimingStats.from_timings([5.0])
    ...
    def test_from_timings_empty_raises(self):
        """TimingStats.from_timings raises on empty list."""
        with pytest.raises(ValueError, match="empty"):
            TimingStats.from_timings([])

    def test_to_dict(self):
        """TimingStats.to_dict includes all stat fields."""
        stats = TimingStats(
