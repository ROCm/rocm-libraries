...
A handle used with the ingestor **must** expose `hipStream_t getStream() const`: benchmarking brackets each candidate with HIP events on that stream. `GenericPlanBuilder` `static_assert`s the requirement, so a handle without it fails to compile with a message naming it rather than failing at plan-build time. The requirement sits on the ingestor templates, not on `validateHandleType()`, so a provider that never instantiates them is unaffected.
