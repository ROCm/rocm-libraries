...
A **pack** is one engine's contribution, and it is exactly two `.cpp` files plus its
kernel source:
...
| `<Op>Descriptors.cpp` | The descriptor set: the engine descriptor, its metadata schema (KMD), and one kernel definition per variant. |
...
2. Write `<Op>Descriptors.cpp`, returning a `DescriptorSet`. Each `MetadataField` in the
   KMD declares its `name`, its expected `type`, and either a `defaultValue` or nothing
   (making it mandatory). A kernel supplying a field of the wrong type is rejected at
   load.

3. Declare both entry points in
   [`IngestorPacks.hpp`](../../../dnn-providers/hip-kernel-provider/src/engines/kernel_ingestor_engine/IngestorPacks.hpp)
   and add **one row** to the table in `IngestorPacks.cpp`.

4. Add the three files to the engine's `CMakeLists.txt`, and add tests.
