...
Both `graph::set_preferred_engine_id_ext()` and `graph::deselect_engines()` also accept engine names, which is usually more readable than a numeric ID. A name is hashed to an engine ID, so it selects a plugin-supplied engine only when that plugin derived its engine ID from the same name. Run `hipdnn_list_engines` to see the names available on your system.
