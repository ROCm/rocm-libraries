...
Unzip it to a path with no spaces. E.g. After unzipped to `C:\dist\clang` you will have `C:\dist\clang\bin` as a folder.
...
You can use the table at https://github.com/ROCm/TheRock/blob/main/RELEASES.md#index-page-listing to determine the GFX Family for this card. E.g. GPU architecture `gfx1103` is GFX Family `gfx110X-dgpu`.

Download a recent nightly *Windows* build tarball of TheRock ROCm SDK for your GFX Family from https://therock-nightly-tarball.s3.amazonaws.com/index.html. E.g., for `gfx110X-dgpu`, the most recent tarball available (at the time of this writing) is `therock-dist-windows-gfx110X-dgpu-7.10.0a20251118.tar.gz`.
...
Unzip the downloaded tarball to a path with no spaces. E.g. after unzipped to `C:\dist\therock` you will have `c:\dist\therock\bin` as a folder.

#### 8. Setup Environment Variables

Add bin folders to your development environment `PATH` variable. E.g., assuming Clang and TheRock were unzipped to `c:\dist\clang` and `c:\dist\therock` respectively:
```cmd
PATH=c:\dist\clang\bin;c:\dist\therock\bin;%PATH%
```

Set the following environment varilables (replace `gfx1103`, `C:/dist/therock`, and `C:/dist/clang/bin` in the following with the respective values for your system):
```cmd
set HIP_PLATFORM=amd
set GPU_TARGETS=gfx1103
set ROCM_PATH=C:/dist/therock
set CMAKE_PROGRAM_PATH=C:/dist/clang/bin
```
...
```
With `CMAKE_GENERATOR` set, the Ninja project files can be generated by runing `cmake ..` from the `projects\hipDNN\build` folder. E.g.:
```cmd
projects\hipdnn\build>cmake ..
-- Building for: Ninja
-- Using ROCm Clang compilers from C:/dist/therock/lib/llvm/bin
-- Using ROCm Clang compilers from C:/dist/therock/lib/llvm/bin
```

(TIP: for convenience, the above environment variable settings can be added to a .bat file that is run whenever a command window is opened for building hipDNN.)
...
* When generating the project, CMake will warn about a clang-format mismatch.  That’s okay for now.
...
* **PATH Configuration**: To link with your build of hipDNN, add the location of your projects/hipDNN/build folder to your build environment PATH:
   ```cmd
   set PATH=<hipDNN_build_dir>\backend\src;%PATH%
   ```
   (Or alternatively run the hipDNN `install` target and ensure the install directory is added to your PATH.)
