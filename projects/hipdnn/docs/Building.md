...
| `hipdnn-graph-studio` | hipDNN core + all providers + the Graph Studio desktop app (built into `build/graph-studio/`, launched by `build/bin/start-graph-studio.bat\|.sh`) |
...
| hipdnn-graph-studio | `projects/hipdnn/tools/graph-studio/` | `hipdnn_graph_studio` | Bun, Node.js 20+, a C++ compiler; provider plugins at runtime |
