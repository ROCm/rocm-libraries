...
2. **If not loaded, ask the user which skill(s) to install and which host/scope.** Use `--codex` for Codex user-global skills (`$CODEX_HOME/skills` when set, otherwise `~/.codex/skills`), `--claude` for Claude user-global skills (`~/.claude/skills`), or `--target <dir>` for an explicit workspace or user scope. Then run one of:
   ```
   python3 tools/ai/link-skills.py --codex <skill-name> [<skill-name> ...]
   python3 tools/ai/link-skills.py --claude <skill-name> [<skill-name> ...]
   python3 tools/ai/link-skills.py --target <target-skills-dir> <skill-name> [<skill-name> ...]
   ```
   Use `python3 tools/ai/link-skills.py --list` to see what's available. The user must name each skill explicitly — there is no "link everything" mode, so a skill is never installed without consent. The installer links by default using symlinks on Linux/macOS and junctions on Windows so skills auto-update with the checkout; pass `--copy` for a snapshot install.
