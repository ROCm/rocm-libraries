...
- `tools/ai/skills/hipdnn-superbuild-test/SKILL.md`
  - Runs tests against an existing superbuild with per-component selection (`hipdnn`, `miopen`, `hipblaslt`, `hip-kernel`, `integration-tests`, or `all`), unit/integration scope, optional `--filter=<gtest_pattern>`, `--verbose`, and `--keep-going`. Handles Windows DLL PATH and the `hip-kernel-provider` target naming quirk automatically. For domain knowledge on the cross-provider `hipdnn_integration_tests` suite itself (bundle/sweep vs C++ tests, YAML tiers, per-engine TOML config, CMake/CTest wiring, and its known green-but-broken failure modes), pair it with `hipdnn-integration-testing`.
...
- `tools/ai/skills/hipdnn-integration-testing/SKILL.md`
  - Domain reference for `dnn-providers/integration-tests`: the bundle/sweep vs C++ test split, YAML tier filtering (three separate `test_categories*.yaml` scopes), per-engine `--test-config` TOML (tolerance overrides, test skips), CMake/CTest wiring (`add_external_integration_test_target()`), and how to invoke `hipdnn_integration_tests` directly against one engine. Documents the suite's known green-but-broken failure modes: a "zero tests ran" run is always a config bug, not a clean pass; and RFC 0015's per-graph engine-support-claim schema is parsed but not yet enforced, so an engine silently losing support for a graph shows up only as a `GTEST_SKIP`, never a failure.
  - Suggest this skill when the user asks how the integration test suite works, how to add/configure a bundle or TOML entry, or is triaging a suspicious integration-test run (0 tests, all-skip, "did engine X regress"). Pair it with `hipdnn-superbuild-test` when the user also needs to actually run something in this workspace.
