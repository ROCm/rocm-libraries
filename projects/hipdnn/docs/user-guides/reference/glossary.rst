...
- **Engine ID**: The 64-bit integer that identifies an engine throughout hipDNN, conventionally the FNV-1a hash of its name.
- **Engine Name**: The human-readable display label for an engine, reported by the plugin that provides it or supplied by hipDNN. Not required to be unique. See :ref:`engine-names`.
