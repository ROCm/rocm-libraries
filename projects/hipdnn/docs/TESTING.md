...
Detailed commands and test-authoring conventions remain in [Testing](Testing.md). Detailed layer
reference, release procedures, and the run-record template are consolidated below.
...
[Testing](Testing.md). The expected development sequence is:
...
The superbuild uses prefixed targets only. See [Testing: Running Tests](Testing.md#running-tests)
...
in [Testing: Quick Reference](Testing.md#quick-reference).
...
| Static analysis is manual, limited to hipDNN C++ code, excludes provider components and HIP-language code, and has no regular cadence | High | High | Manual `hipdnn-tidy` on eligible hipDNN sources | Untracked |
