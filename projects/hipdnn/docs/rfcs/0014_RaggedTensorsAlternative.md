# hipDNN: Ragged Tensors Design Document (Alternative)

- Contributors: hipDNN maintainers
- **Status**: Draft

> This document is an alternative to
> [`0014_RaggedTensors.md`](./0014_RaggedTensors.md). It drops the
> SDK-level connection between a ragged primary and its `seq_lens`
> tensor, and recommends a self-owning `RaggedTensor<T, IndexT>`
> instead of the `IrregularTensor<T>` + `RaggedView<T, IndexT>`
> split. The rationale: with no SDK-level `seq_lens` linkage, the
> reasons that pushed the original 0014 toward the storage/view
> split — deferred `seq_lens` attachment, the bundle-side aux
> ownership problem, output-validation `seq_lens` wraps — all
> dissolve, and the simpler one-type design becomes the better
> choice.
>
> `seq_lens` is still a real tensor in the graph; it remains the
> right primitive for ops that need to bound their work to valid
> sequence positions. The change here is that the *ragged-tensor
> abstraction itself* no longer knows about `seq_lens`. CPU
> references (and kernels) that consume `seq_lens` look it up from
> the variant pack the same way they look up any other input.
...
a `ragged_offset[B+1]` aux tensor: batch `b` occupies the contiguous
range `[ragged_offset[b], ragged_offset[b+1])`.
...
5. **Plan layer** wraps the variant-pack pointer in a
   ...
   reference as `TensorBase<T>&` — reference signatures are
   unchanged.
6. **Integration-test harness** accepts a *pre-supplied input
   bundle* — `unordered_map<int64_t, shared_ptr<ITensor>>` keyed by
   UID — so the structurally-constrained `ragged_offset` aux can
   carry deliberate values shared between the GPU and CPU paths via
   each `ITensor`'s built-in host/device `MigratableMemory`.

`seq_lens` is **not** part of the ragged-tensor abstraction. It
remains a tensor in the graph that ops reference through their own
node-level attribute APIs (e.g.
`SdpaAttributes::set_seq_len_q`); CPU references and kernels look
it up from the variant pack like any other input. This decoupling
is the primary departure from
[`0014_RaggedTensors.md`](./0014_RaggedTensors.md).
...
Some ops additionally need to distinguish *valid* sequence positions
within each batch's range from *trailing padding* positions kept for
alignment. That information is carried by a separate `seq_lens[B]`
tensor referenced through the op's node-level attributes (e.g.
`SdpaAttributes::set_seq_len_q`) — matching cuDNN frontend-API
conventions. `seq_lens` is independent of the ragged-tensor
abstraction: a ragged primary always reports the full per-batch
range from `ragged_offset` and never consults `seq_lens` itself.
Ops that care about per-batch valid lengths read `seq_lens` from
the variant pack directly.
...
and its last element must agree with the primary's physical buffer
size. The harness needs a way to consume deliberate values for
this aux tensor.
...
(modulo per-batch alignment, see [§4.2](#42-frontend-tensorattributes-additions)):
...
- **Packed with per-batch trailing padding.** Each batch's range
  ...
  the remaining `ragged_offset[b+1] - ragged_offset[b] - seq_lens[b]`
  rows are padding kept for alignment.

  Whether the trailing rows are padding is an op-level concern. The
  ragged tensor itself iterates the full per-batch range; the op's
  CPU reference or kernel queries `seq_lens` from the variant pack
  to decide what to do with positions `>= seq_lens[b]`.

A "padded, seq-lens-only" mode — in which the physical buffer is
exactly `prod(dims) = B * S_max * …` elements, every batch has the
same padded extent, and only `seq_lens` distinguishes valid from
padded entries — needs no new SDK type at all. The primary in that
mode has consistent strides and a non-ragged tensor shape, so an
ordinary `Tensor<T>` / `TensorBase<T>` represents it correctly; the
op's CPU reference looks up `seq_lens` from the variant pack the
same way the ragged case does. The mode is therefore in scope by
construction; this RFC's new SDK types only become necessary when
`physicalElementCount ≠ prod(dims)`.
...
Serialization). Schema evolution rules (RFC 0005) require appending
optional defaulted fields rather than reordering or repurposing
existing fields.
...
`CpuFpReferenceSdpa::forward(q, k, v, …)` typed as `TensorBase<T>&`).
...
2. **Data SDK** introduces a single new owning type
   ...
   `ShallowRaggedTensor<T, IndexT>`. Both expose ragged-aware
   iteration over `ragged_offset` ranges. `seq_lens` is not part of
   either type.
...
4. **Plan layer** wraps the variant-pack pointer in a
   ...
   `TensorBase<T>&`. CPU references and kernels that need `seq_lens`
   look it up from the variant pack themselves.
5. **Test harness** accepts a pre-supplied input bundle so the
   structurally-constrained `ragged_offset` aux carries deliberate
   values rather than meaningless random fill. The bundle stores
   tensors as `shared_ptr<ITensor>` to allow the same aux to be
   threaded into a ragged primary's constructor.
...
physical buffer, in elements. The default `1` (no alignment) matches
AITER FMHA's needs.
...
1. **The `ragged_offset` aux is
   `std::shared_ptr<TensorBase<IndexT>>`**, with `IndexT` templated
   (defaulting to `int32_t`, with `int64_t` also permitted).
2. **`dims()[1]` is required to be `S_max`** (the max padded
   ...
   verify it. `elementSpace()` equals the physical buffer size and
   is decoupled from `prod(dims)`.
3. **Polymorphic index-strategy hook on `ITensor`**: introduce
   `virtual std::unique_ptr<ITensorIndex> makeIndex(bool isEnd) const`
   so ragged iteration can supply a `RaggedCompositeIndex` that
   walks each batch's full `[ragged_offset[b], ragged_offset[b+1])`
   range in turn. The existing `LinearIndex` / `CompositeIndex`
   strategies move behind this interface.
4. **Iteration walks `ragged_offset` ranges, not `seq_lens`-bounded
   ranges.** Each batch's full per-batch range — including any
   trailing padding kept for alignment — is iterated as part of
   that batch. Padding never leaks into the wrong batch's range, so
   indexing is always semantically correct (every visited element
   belongs to the right batch and to the tensor's owned memory);
   it only over-reports by visiting padded positions for ops that
   could in principle skip them. Ops that must skip padding query
...
`RaggedTensor<T, IndexT, PinnedHostAllocator<T>, …>` — same pattern
`PinnedTensor<T>` uses for `Tensor<T>`.

**Immutability.** `ragged_offset` is set at construction and never
changed; the type has no setter for it. (Per
[§4.5](#45-data-sdk-shared-elements), the type also does not know
about `seq_lens`, so the post-construction-attach problem that the
original 0014 had to design around doesn't exist.)

### 4.7 Data SDK: `ShallowRaggedTensor<T, IndexT>` (non-owning peer)

A non-owning peer to `RaggedTensor<T, IndexT>`, used by the plan
layer at execute time when only a `void*` from the variant pack is
available.
...
public:
    ...
    //   dims()/strides()/elementSpace()/elementCount() as provided
    //   isPacked() -> false
    //   begin/end  -> RaggedCompositeIndex via makeIndex()
    //
...
`RaggedTensor` and `ShallowRaggedTensor` share the same
`RaggedCompositeIndex` implementation; only memory ownership
differs.
...
- **Node-level op attributes** (e.g. `SdpaAttributes`) carry
  `seq_lens` UIDs via existing accessors; these are referenced like
  any other tensor input — the SDK ragged types know nothing about
  them.
...
- **Plan layer (executor)**: when executing an op whose graph
  ...
  `ragged_offset` ref is obtained from
  `TensorAttributes::ragged_offset_tensor_uid()`. `seq_lens`, if
  the op consumes it, is fetched as a separate `TensorBase<IndexT>&`
  from the variant pack the same way any other input would be.

### 4.10 Plan-layer construction and CPU reference impact

```cpp
const auto qPhysicalElementCount =
    computePhysicalElementCount(_params.qTensor.raggedOffset,
...
The per-execute physical element count is computed from the current
variant-pack `ragged_offset` (see
[§4.11.2](#4112-bundle-allocation)) rather than cached at plan-build
time, so each execute is free to use different `ragged_offset`
values.
...
whether to skip indices `>= seq_lens[b]` within batch `b` — exactly
the same pattern as any other op input that controls work shape.
The reference does **not** need any new
`primary.validSeqLen(b)`-style API on the ragged primary itself.

### 4.11 Test-harness integration

#### 4.11.1 Pre-supplied input bundle

`ragged_offset` cannot be meaningfully randomized: its values must
be structurally consistent (monotonic, `ragged_offset[0] == 0`,
last element equal to the sized physical buffer). The harness
...
Note the value type is `shared_ptr<ITensor>` rather than
`unique_ptr<ITensor>` as in the original 0014 — this is required so
the bundle can thread a `ragged_offset` aux into a ragged primary's
constructor (see [§4.11.2](#4112-bundle-allocation)). It also lets
the same `ITensor` serve both execution paths via its built-in
host/device `MigratableMemory`.

The harness retains ownership of the pre-supplied bundle and the
per-path `GraphTensorBundle`s continue to own the non-pre-supplied
tensors. The harness's UID lookup is a pre-supplied-first
fallthrough: if a UID is present in the pre-supplied bundle, both
`GraphTensorBundle`s see the same `ITensor` via shared ownership,
and randomization is skipped for that UID.

**Strict requirement check.** Before allocation, the harness
verifies that every `ragged_offset` UID referenced by a
`TensorAttributes` is present in the pre-supplied bundle. Missing
`ragged_offset` UIDs fail setup with a clear error rather than
silently random-initializing a structurally invalid aux.
...
(via the same `ITensor`), guaranteeing byte-equal input values
across the GPU and CPU paths.
...
- Else: look up the `ragged_offset` UID in the pre-supplied bundle
  to obtain its `shared_ptr<TensorBase<IndexT>>` — the strict check
  in [§4.11.1](#4111-pre-supplied-input-bundle) guarantees its
  presence — read its host values, compute the physical element
  count, and allocate `make_shared<RaggedTensor<T>>(dims, strides,
...
valid size). Larger alignment values round the total buffer size up
to satisfy a trailing-alignment requirement. The plan layer
...
The `shared_ptr`-typed bundle storage is the principal harness-side
cost of this design relative to the original 0014; it is discussed
under [§6.1](#61-irregulartensort--raggedviewt-indext-split-the-original-0014-design).
...
`RaggedTensor<T>` per-batch over its `ragged_offset` ranges. This
fills every position in every batch's range, including any
trailing per-batch padding tails — which is harmless for the same
reason it was in 0014: neither GPU kernels nor CPU references read
padding positions, and identical seeds keep padding values
byte-equal between the two paths.

#### 4.11.4 Output validation in `verifyGraph`

The validation problem is unchanged from 0014: the CPU reference
writes only valid sequence positions of an output (those
`< seq_lens[b]`); the GPU kernel makes no contractual guarantees
about output padding. A naive full-buffer compare fails on padding.

**Recommended approach: sentinel-skip.** At harness init, fill each
ragged output's physical buffer (both bundles) with a sentinel the
CPU reference cannot legitimately produce (NaN for floats; a magic
number for ints). After execution the validator iterates the full
physical buffer; positions where the CPU side is bit-equal to the
sentinel are skipped (the reference didn't write there); other
positions use the existing tolerance check. This needs one
validator overload (`allCloseSkipSentinel(cpu, gpu, sentinel,
tolerance)`) and a small init-time branch.

Because `seq_lens` is no longer part of the ragged abstraction, the
seq_lens-aware-wrap fallback that 0014 carried isn't reachable from
the ragged primary at all — sentinel-skip is the only mechanism in
this design.
...
1. **`GraphTensorBundle` stores entries as `shared_ptr<ITensor>`,
   ...
   reference.
2. **Iteration walks per-batch ranges *including* padding tails.**
   `RaggedTensor`'s iterator does not skip positions
   `>= seq_lens[b]`. Ops that need to skip padding query `seq_lens`
   directly from the variant pack. This is by-design and is the
   primary trade-off this alternative makes against 0014 — it
   shifts a small amount of work into per-op CPU references in
   exchange for a much simpler SDK.
3. **Sentinel-skip validation requires per-CPU-reference
   ...
   sentinel. Low-precision floats (bf16, fp16, fp8) have a smaller
   sentinel space and harder unproducibility guarantees.
4. **Test authors must construct `ragged_offset` tensors by hand.**
   Every integration test of a ragged-consuming op must build a
   deliberate `ragged_offset` (and, when the op consumes
...
### 6.1 `IrregularTensor<T>` + `RaggedView<T, IndexT>` split (the original 0014 design)

The original 0014 design splits ragged-tensor support into two SDK
types: `IrregularTensor<T>` for owning storage with
`physicalElementCount ≠ prod(dims)`, and `RaggedView<T, IndexT>` as
a non-owning ragged-aware wrapper that attaches both
`ragged_offset` and (optionally) `seq_lens` at construction. The
plan layer assembles the view at execute time; ragged-aware
iteration uses `seq_lens` to bound per-batch walks.

The split design was originally chosen because:

- **Bundle-side aux ownership.** The bundle could store entries as
  `unique_ptr<ITensor>` without needing to thread aux refs into
  primary constructors; the view (assembled later) referenced the
  aux indirectly.
- **Deferred `seq_lens` attachment.** `seq_lens` could be attached
  to a ragged primary only at the plan layer, where node-level
  attributes are visible — the bundle layer (which sees only
  `TensorAttributes`) didn't need to discover it.
- **Output-validation flexibility.** The harness could attach
  `seq_lens` to a separate `RaggedView` for ragged-aware compare
  without mutating the bundle entry.

Once `seq_lens` is dropped from the SDK abstraction entirely (per
the rationale in [§6.2](#62-keeping-seq_lens-in-the-ragged-tensor-sdk-abstraction)),
every one of those motivations becomes either moot or much smaller:

- The bundle still has to thread `ragged_offset` into a primary's
  ctor, which is exactly the `shared_ptr` shift the split design
  was trying to avoid. The `shared_ptr` storage cost
  ([§4.11.2](#4112-bundle-allocation)) applies to both designs.
  The split's win on this axis disappears.
- "Deferred `seq_lens` attachment" is no longer a concept — the
  primary never holds `seq_lens` at any layer.

What the split design still costs:

- **Two new SDK types instead of one** (`IrregularTensor<T>` +
  `RaggedView<T, IndexT>`, plus the `ShallowTensor<T>`-as-underlying
  pattern at the plan layer that the recommended design here folds
  into `ShallowRaggedTensor<T>` directly).
- **A type that derives from `TensorBase<T>` but whose
  `getHostValue` / `setHostValue` are semantically inert**
  (`IrregularTensor`), motivating the additional `TypedTensor<T>`
  hierarchy refactor (0014's §7.1) to clean up.
- **Two distinct `elementCount()` contracts** in the new types,
  documented as a known limitation in 0014.

Given the symmetry of the bundle-side cost between the two
designs, the simpler one-type design recommended here is the
better choice.

### 6.2 Keeping `seq_lens` in the ragged-tensor SDK abstraction

This alternative — which the original 0014 takes — has the ragged
primary's view type hold a `shared_ptr` to `seq_lens` and provide
a `validSeqLen(b)` API plus seq_lens-aware iteration.
...
- CPU reference bodies don't need to take `seq_lens` as a separate
  parameter; they call `primary.validSeqLen(b)` and an
  early-out-on-`sq >= validSeqLen(b)` check is all that's needed.

**The case against (taken by this alternative):**

- **`seq_lens` is never used during input fill or output
  validation.** Randomization walks the physical buffer regardless
  (padding values are harmless because nobody reads them).
  Sentinel-skip validation doesn't consult `seq_lens` either. The
  only consumer is the CPU reference body — a single, narrow site.
- **CPU references already look up arbitrary inputs from the
  variant pack.** Querying a `seq_lens` tensor that's just another
  input is no more awkward than querying any other input. There is
  no concrete ergonomic gain from a `validSeqLen(b)` API over
  `seqLens.getHostValue({b})`.
- **Indexing is always semantically correct without `seq_lens`.**
  Walking each batch's full `ragged_offset` range only ever visits
  positions that belong to the right batch and to the tensor's
  owned memory. The cost of carrying `seq_lens` in the SDK is paid
  in every layer (frontend bundle setup, view ctor signatures,
  pre-supplied bundle strict checks, alternatives-section
  analysis) to buy what amounts to a per-batch loop bound in one
  CPU reference function body.
- **Significant complexity savings.** Dropping `seq_lens` from the
  SDK removes: the storage-vs-view split, the
  post-construction-attach-seq_lens problem, the `validSeqLen`
  virtual on `TensorBase`, the seq_lens-aware output-validation
  fallback, the strict-requirement check on `seq_lens` UIDs in the
  pre-supplied bundle, the `TypedTensor<T>` future-work refactor's
  primary motivation, and the related parts of the "shared
  elements" discussion in §4.5.
...
from user-supplied values and then constructing a `RaggedTensor<T>`
sized from `(ragged_offset, alignment)`. Collapses the user's API
to one call per `TensorAttributes`.
...
### 7.2 Ragged-aware multi-dim addressing on `RaggedTensor<T>`

`RaggedTensor<T>` currently inherits `getHostValue` / `setHostValue`
from `TensorBase<T>` and these methods use the padded strides, so a
multi-dim index `{b, sq, …}` reads from
`b * stride_0 + sq * stride_1 + …` — which is correct for
contiguous indexing into batch `b`'s range as long as the caller
respects `ragged_offset[b+1] - ragged_offset[b]` as the per-batch
extent. A small enhancement would be to override these methods to
translate `{b, sq, …}` indices to physical offsets using
`ragged_offset[b]` as the per-batch base, removing the responsibility
from the caller. Out of scope for this RFC but a natural
follow-up.

### 7.3 Sentinel selection per CPU reference
