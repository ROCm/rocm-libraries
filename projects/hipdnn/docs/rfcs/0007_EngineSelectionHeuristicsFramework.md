...
| **`SelectionHeuristic::ModeA`** | Engine-level prediction ranking: ranks candidates on the engine-level TFLOPS prediction alone, with no configuration prediction during ranking ([RFC 0019 §11.2](0019_UniversalHeuristicDescriptor.md#112-two-engine-selection-policies-rfc-0007)). Shipped as a **backend built-in** (`backend/src/heuristics/prediction/`, [§10.1](#101-heuristicpluginmanager)). Declines when no candidate yields a usable score. |
| **`SelectionHeuristic::ModeB`** | Configuration-level prediction ranking: prefers the best available **configuration**-level TFLOPS and falls back to the engine-level prediction where no configuration prediction is available ([RFC 0019 §11.2](0019_UniversalHeuristicDescriptor.md#112-two-engine-selection-policies-rfc-0007)). Same built-in as **`ModeA`** — one function table exposing **both** policy IDs ([§8.2](#82-plugin-module-metadata)) — and declines on the same condition. |
...
```c
/* Scoped host services for prediction-aware policies (host services version 1). */
typedef struct
{
