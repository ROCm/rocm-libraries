...
These are **examples**; each plugin chooses a **canonical UTF-8 policy name** (for documentation and user config). The **stable identifier** across the loader, registry, and C ABI is the **`int64_t`** produced by the **same `policyNameToId` hash** as computational engines, reported through **`hipdnnHeuristicPluginGetAllPolicyIds`** ([§8](#8-c-abi-for-heuristic-plugins)).
...
This draft standardizes four logical policies, each under a **well-known UTF-8 name** (the `SelectionHeuristic::` prefix is a **naming convention** in the string itself, not C++ language linkage or a shared type with the C++ `SelectionHeuristic` class). A policy's **canonical `int64_t` ID** is **`policyNameToId(name)`** (a plugin or built-in implementing it **must** report that exact value among the IDs it returns from **`hipdnnHeuristicPluginGetAllPolicyIds`**):
...
| **`SelectionHeuristic::ModeA`** | The quick policy: ranks candidates on the engine-level (L1) prediction in the request's **ranking metric** alone, and never asks for a configuration-level (L2) prediction — not even for the winner, whose kernel is chosen at plan build by the same metric ([RFC 0019 §11.2](0019_UniversalHeuristicDescriptor.md#112-two-engine-selection-policies-rfc-0007), Open Question 21 option (b)). Shipped as a **backend built-in** (`backend/src/heuristics/prediction/`, [§10.1](#101-heuristicpluginmanager)). Scored engines are ordered by value in the metric's direction, ties broken by static order; unscored or invalid engines follow in static order with one warning naming them and the metric. Declines ("no engine serves `<metric>` at L1") when no candidate is scored. |
| **`SelectionHeuristic::ModeB`** | The thorough policy: per engine, the configuration-level (L2) prediction in the ranking metric, falling back to the engine-level (L1) one where L2 has no usable answer ([RFC 0019 §11.2](0019_UniversalHeuristicDescriptor.md#112-two-engine-selection-policies-rfc-0007)). Same built-in as **`ModeA`** — one function table exposing **both** policy IDs ([§8.2](#82-plugin-module-metadata)) — with the same ordering and tail rules; declines with "no engine serves `<metric>` at L1 or L2". |

**Ranking metric.** Both prediction policies compare engines in one registered **ranking metric** ([RFC 0019 §4.4](0019_UniversalHeuristicDescriptor.md#44-ranking-metrics)): `tflops` (higher is better, the default) or `time` (milliseconds, lower is better). The metric is resolved on every **`EngineHeuristicDescriptor::finalize()`** with the same precedence as the policy order — the **`HIPDNN_HEUR_RANKING_METRIC`** environment variable, then the **`HIPDNN_ATTR_ENGINEHEUR_RANKING_METRIC_EXT`** attribute, then `tflops` — and an unregistered name is refused (`HIPDNN_STATUS_BAD_PARAM`) at set, or at finalize for the environment. The host passes it to policies in the host services table ([§8.9](#89-finalize-and-sorted-results)) and stamps it into every result configuration's **`EngineConfig.ranking_metric`**, so the chosen engine ranks its own catalog by the same metric at plan build ([RFC 0019 §11.4](0019_UniversalHeuristicDescriptor.md#114-selecting-a-uhd-by-metric)).
...
**Mandatory registration.** A shipped heuristic plugin (or built-in adapter) that implements **`SelectionHeuristic::StaticOrdering`** **must** report exactly **`policyNameToId("SelectionHeuristic::StaticOrdering")`** among the IDs from **`hipdnnHeuristicPluginGetAllPolicyIds`** — there is **no** alternate ID for that behavior, and the same holds for every other name in the table above. Optional **`hipdnnHeuristicPluginGetPolicyName`** ([§8.2](#82-plugin-module-metadata)) **should** report the same canonical UTF-8 string for that ID; when present, the host **validates at load time**, for **each** exposed ID, that **`policyNameToId(name) == policy_id`** and **rejects** the module on mismatch (same idea as keeping engine IDs and registered engine names consistent).

**Overlapping IDs at load.** **`HeuristicPluginManager`** **must** reject a heuristic module **any** of whose exposed policy IDs duplicates an **`int64_t`** already claimed by another **accepted** heuristic plugin or built-in ([§10.1](#101-heuristicpluginmanager), [§11](#11-versioning-and-compatibility-checks)). Uniqueness is **per ID, across plugins**: a module exposing several policies is checked once per ID, and one collision rejects the whole module, its other policies included. The built-ins are registered first, so a third-party module trying to claim one of their IDs will be rejected. This is **stricter than the engine side**, which drops only the duplicate engine — a module that cannot register its policies has nothing left to contribute.
...
**`StaticOrdering`** retains no override guarantee — it is a normal default-ordered fallback ([§5.3.2](#532-default-policy-order-user-strings)) and may be omitted or reordered by the user. Both are shipped through **`HeuristicPlugin::createBuiltIn(populateFunctionTable(), label)`** at **`HeuristicPluginManager`** construction time, as is the prediction built-in behind **`ModeA`** / **`ModeB`** ([§5.3.1](#531-well-known-policy-names-and-ids), [§10.1](#101-heuristicpluginmanager)); none has a separate `.so` and the user cannot unload them.
...
| `hipdnnHeuristicPluginGetAllPolicyIds(int64_t* policy_ids, uint32_t max_policies, uint32_t* num_policies)` | Enumerates **every** policy the module exposes — a module is **not** limited to one, and a first-party module may ship a family of related policies behind a single function table. Two-pass query/retrieve, the same shape as **`hipdnnEnginePluginGetAllEngineIds`**: **`max_policies == 0`** (with **`policy_ids`** optionally **NULL**) reports the total in **`*num_policies`**; a second call with real capacity fills the array and reports how many IDs were written. Each value **must** equal **`policyNameToId(canonical_utf8_name)`** for the policy it names (same hash as computational engine IDs—[§5.3](#53-ordered-policy-list-default-and-user-configuration), [RFC 0003](0003_EngineIdDesign.md)). The host matches these values against the resolved **`orderedPolicyIds`** after hashing user-supplied name strings. IDs are **unique within the module** and stable for the lifetime of the loaded library; the cross-module uniqueness rule of [§5.3.1](#531-well-known-policy-names-and-ids) applies **per exposed ID**, so a module exposing several policies must clear that check for **each** of them independently. |
| `hipdnnHeuristicPluginGetPolicyName(int64_t policy_id, const char** name)` | Optional. **NUL-terminated UTF-8** canonical name of **one** of the IDs **`GetAllPolicyIds`** reported (the same string the vendor tells users to put in **`HIPDNN_ATTR_ENGINEHEUR_POLICY_ORDER_EXT`**); returns **`HIPDNN_PLUGIN_STATUS_BAD_PARAM`** for an ID the module does not expose. For logging and enumeration; **not** required for matching if the IDs are computed correctly. When exported, the host **validates at load**, for **each** exposed ID, that **`policyNameToId(*name) == policy_id`** ([§5.3.1](#531-well-known-policy-names-and-ids), [§11](#11-versioning-and-compatibility-checks)). Omit from minimal plugins if the host derives display names from a static registry. |
...
**Lifecycle:** The backend **`EngineHeuristicDescriptor` owns** one **`hipdnnHeuristicPolicyDescriptor_t`** per entry in **`orderedPolicyIds`**. Objects are **created** when the descriptor’s policy list is established (on attribute set, bind, or lazily at first **`finalize()`**—implementation choice) and **destroyed** when the **`EngineHeuristicDescriptor`** is destroyed (**plugin policy descriptors and the hipDNN heuristic descriptor die together**). **`hipdnnHeuristicPolicyDescriptorCreate`** takes the **`hipdnnHeuristicHandle_t`** for that slot’s module; that **binds** the policy to the handle **before** **`Finalize`**, so selection code can treat the handle as the source of **device-properties** session state (**`SetDeviceProperties`**—[§8.6](#86-plugin-handle-lifecycle)). It also takes the **`int64_t` policy ID** for the slot, because a module may expose **several** policies ([§8.2](#82-plugin-module-metadata)): the handle names the module, the ID names **which** of its policies this descriptor executes. The ID **must** be one the module reported through **`hipdnnHeuristicPluginGetAllPolicyIds`**; anything else is **`HIPDNN_PLUGIN_STATUS_BAD_PARAM`**. Two slots naming two policies of one module therefore share a **`hipdnnHeuristicHandle_t`** and differ only in this argument.
...
**Optional prediction-aware finalize and config read-back (heuristic ABI 0.1.0).** A policy that ranks on **engine predictions** needs two things the two calls above cannot carry: a way to **ask the host** for a candidate’s prediction while it is deciding, and a way to hand back the exact **engine configuration** it decided on instead of an ID alone. Both are **optional exports**; a module that omits them is a perfectly valid heuristic plugin, and the host uses plain **`Finalize`** and ID-only results for it.

```c
/* Scoped host services for prediction-aware policies (host services version 2). */
typedef struct
{
    uint32_t version;     /* host services version; currently 2 */
    size_t   struct_size; /* size in bytes of this table as the host built it */
    void*    context;     /* opaque, borrowed callback context */
    hipdnnPluginStatus_t (*get_prediction)(
        void* context,
        int64_t engine_id,
        hipdnnEnginePredictionKind_t kind,
        hipdnnPluginConstData_t* prediction); /* borrowed, host-verified EnginePrediction */
    const char* ranking_metric; /* version 2: registered metric this finalize ranks by */
} hipdnnHeuristicHostCallbacks_t;

hipdnnPluginStatus_t hipdnnHeuristicPolicyFinalizeWithHost(
    hipdnnHeuristicPolicyDescriptor_t desc,
    const hipdnnHeuristicHostCallbacks_t* host,
    int32_t* out_applied);

hipdnnPluginStatus_t hipdnnHeuristicPolicyGetEngineConfig(
    hipdnnHeuristicPolicyDescriptor_t desc,
    int64_t engine_id,
    hipdnnPluginConstData_t* engine_config);
```

- **Host services table.** The **host** builds **`hipdnnHeuristicHostCallbacks_t`** and the plugin only reads it. **`version`** and **`struct_size`** are checked **before** any callback field, so a later host may **append** fields and a module written against version 1 still runs against the larger table. The table, its **`context`**, and every buffer a callback hands back are **borrowed for the duration of the call**: a plugin **must not** retain them, invoke callbacks asynchronously, or free host buffers, and copies anything it still needs after **`FinalizeWithHost`** returns. Calls are synchronous on the finalize thread, under the same **single-thread** contract as the rest of this section.
- **`get_prediction`.** Borrows one candidate’s verified **`EnginePrediction`** FlatBuffer for the requested **`hipdnnEnginePredictionKind_t`** layer. Only engine IDs supplied through **`SetEngineIds`** may be queried. Every prediction is asked in the finalize's **`ranking_metric`**: its **`value`** is a calibrated physical quantity in that registered metric's units — an absolute figure of merit, not an ordering key ([RFC 0019 §11.3](0019_UniversalHeuristicDescriptor.md#113-cross-engine-comparison)) — and better is higher or lower as the registry says. The host has already turned an answer in any other metric, or an **`AVAILABLE`** value the metric rejects (non-finite, negative throughput, non-positive time), into **`INVALID`**. The per-engine surface behind the callback belongs to the **engine** plugin ABI, not this one ([RFC 0017 §3](0017_UniversalKernelDescriptor.md#3-how-it-works)); the metric reaches it as **`EngineConfig.ranking_metric`** and the heuristic ABI only forwards the result.
- **`ranking_metric`.** Version 2 appends the registered metric name the policy ranks by (never NULL, borrowed for the call). A plugin reads it only when **`version >= 2`** and **`struct_size`** covers the field; a version 1 table predates metrics and means `tflops`. A name the plugin's registry does not know is a **`BAD_PARAM`**: it has no direction to rank by.
- **`FinalizeWithHost`.** Same two-phase design and the same **`out_applied`** meaning as **`Finalize`**, plus the scoped table. The host calls it **instead of** **`Finalize`** when the module exports it. A **NULL** **`host`** means predictions are unavailable for this finalize; a policy that can rank only on predictions then **declines** (**`out_applied == 0`**) rather than inventing an order, and the outer loop continues.
- **`GetEngineConfig`.** Valid after a successful finalize, for an engine ID the policy returned. The buffer is owned by the **policy descriptor** and lives only until that descriptor’s next mutation or its destruction, so the host verifies and copies it before the next plugin call; the config’s own engine ID **must** match a returned input engine. **`HIPDNN_PLUGIN_STATUS_NOT_APPLICABLE`** means this policy has no configuration for that engine — the legacy ID-only behavior — and is not an error. A configuration the policy **scored** must be returned with **every** scored knob setting preserved: this read-back is how a configuration-level decision reaches execution, and re-deriving or re-ranking it later would discard the prediction it was chosen on. The host rejects a configuration naming an unregistered **`ranking_metric`** and then stamps the finalize's metric into every result configuration, with or without a policy-supplied one.
...
- **Optional exports are the additive mechanism.** **`hipdnnHeuristicPolicyFinalizeWithHost`** and **`hipdnnHeuristicPolicyGetEngineConfig`** ([§8.9](#89-finalize-and-sorted-results)) arrived this way at heuristic ABI **0.1.0**: the host resolves each symbol, uses it when present, and falls back to **`Finalize`** and ID-only results when it is absent. An **absent optional symbol is not a load failure**, so a plugin built against an older header keeps loading unchanged. **`hipdnnHeuristicHostCallbacks_t`** is the one **host-built struct** the ABI does expose, and it evolves the same way a FlatBuffer does: fields are **appended**, never reordered or retyped, and its **`version`** / **`struct_size`** pair is what a plugin checks before reading a field — version 2 appended **`ranking_metric`** this way, and a plugin reading a version 1 table (which ends before it) treats the metric as `tflops`.
...
- Implements **`validateBeforeAdding`**-style checks before accepting a plugin—**parallel to `EnginePluginManager`**: **API major** match (via **`hipdnnHeuristicGetApiVersion`**); **unique** **`int64_t` policy ID** (via **`hipdnnHeuristicPluginGetAllPolicyIds`**, with every reported ID tracked in a set so a second module cannot register an ID already claimed; engines track the analogous map in `actionAfterAdding` but drop the engine rather than reject the plugin); optional **ID ↔ name** consistency when **`hipdnnHeuristicPluginGetPolicyName`** is exported; and any additional rules from [§11](#11-versioning-and-compatibility-checks). One plugin may export **multiple** policies; each policy ID is checked individually for global uniqueness.
...
- **Registers backend built-ins at construction.** Before any external `.so` discovery, the manager calls **`registerBuiltIns()`** (in `backend/src/heuristics/BuiltInHeuristics.cpp`) to register every shipped built-in heuristic. Today there are three registrations covering four policies — **`SelectionHeuristic::Config`** (in `backend/src/heuristics/config/`, driven by **`HIPDNN_HEUR_CONFIG_PATH`** — see [§5.3.5](#535-the-config-built-in-policy)), **`SelectionHeuristic::StaticOrdering`** (in `backend/src/heuristics/static_ordering/`), and the prediction built-in (in `backend/src/heuristics/prediction/`), whose one function table exposes **both** **`SelectionHeuristic::ModeA`** and **`SelectionHeuristic::ModeB`** through **`hipdnnHeuristicPluginGetAllPolicyIds`** ([§5.3.1](#531-well-known-policy-names-and-ids), [§8.2](#82-plugin-module-metadata)). A built-in implements the **same heuristic plugin C ABI shape** (a populated **`HeuristicPluginFunctionTable`**) as a third-party `.so` and is wrapped via **`HeuristicPlugin::createBuiltIn(populateFunctionTable(), source_label)`**, so the rest of the manager (lookup by **`int64_t` policy ID**, version validation, duplicate-ID rejection) treats it identically to a loaded module. There is **no** separate **`BaseHeuristicsPlugin`** library to ship and **no** override of **`loadPlugins`** to inject default paths. Built-ins always exist regardless of the caller's **`hipdnnPluginLoadingMode_ext_t`**, including **`HIPDNN_PLUGIN_LOADING_ABSOLUTE`**.
...
2. **Policy ID uniqueness:** Two loaded heuristic modules **must not** report the same **`int64_t`** among the IDs from **`hipdnnHeuristicPluginGetAllPolicyIds`**. Enforce with an “insert into a set, throw on duplicate” pattern. Engines use the same insert-is-the-check idea in **`actionAfterAdding`**, but log and drop the engine instead of throwing ([§5.3.1](#531-well-known-policy-names-and-ids)).
3. **Policy ID ↔ optional policy name:** If **`hipdnnHeuristicPluginGetPolicyName`** is provided, **`policyNameToId`** of the UTF-8 string it returns for an ID **must** equal that ID, for every ID the module exposes; otherwise the loader **rejects** the module (catches mistaken or overlapping well-known implementations early—[§5.3.1](#531-well-known-policy-names-and-ids)).
...
- **`HIPDNN_ATTR_ENGINEHEUR_RANKING_METRIC_EXT`** / **`HIPDNN_HEUR_RANKING_METRIC`:** The registered ranking metric the prediction policies order engines by (**`HIPDNN_TYPE_CHAR`**; `tflops` by default, or `time`), resolved env > attribute > default on every finalize and readable back as the effective metric ([§5.3.1](#531-well-known-policy-names-and-ids)). An unregistered name is **`HIPDNN_STATUS_BAD_PARAM`** — at set for the attribute, at finalize for the environment. The effective metric is stamped into every **`HIPDNN_ATTR_ENGINEHEUR_RESULTS`** configuration. The per-query counterparts are **`HIPDNN_ATTR_ENGINE_PREDICTION_METRIC_EXT`** (the metric **`HIPDNN_ATTR_ENGINE_PREDICTION_EXT`** answers in) and **`HIPDNN_ATTR_ENGINECFG_RANKING_METRIC_EXT`** (the configuration's **`EngineConfig.ranking_metric`**, read by its prediction and by plan build); the environment variable overrides neither.
...
**Policy IDs** (heuristic) and **engine IDs** are both **`int64_t`** name hashes but serve **different roles**: the heuristic policy IDs a module reports through **`hipdnnHeuristicPluginGetAllPolicyIds`** identify its **selection policies**; engine plugins expose **engine** IDs for execution. Each loaded **heuristic** module — and each **backend built-in** registered through the same C ABI shape — contributes one or more stable **`int64_t`** values through **`hipdnnHeuristicPluginGetAllPolicyIds`** ([§8.2](#82-plugin-module-metadata)). The **ordered list** the outer loop uses (**`orderedPolicyIds`**) is **`int64_t`**, derived from user-supplied **policy name** strings per [§5.3.3](#533-how-the-user-sets-orderedpolicyids). Applications and tools still need a **runtime view of which policies are actually available** for this **`hipdnnHandle`** (or process) — built-ins plus successfully loaded `.so` modules — analogous to **`getEngineInfos()`** (see [§10.2](#102-heuristicpluginresourcemanager)).
...
- **Backend / handle scope:** **`HeuristicPluginResourceManager`** exposes a query (exact name TBD) that returns **available heuristic policy metadata** — both built-ins and accepted external modules: at minimum the **`int64_t` policy ID**; optionally the **canonical policy name** (from **`hipdnnHeuristicPluginGetPolicyName`** if present, else from a host-maintained id→name map for well-known policies), **plugin implementation version**, install path (for `.so`-backed entries), and a flag distinguishing **built-in** from **loaded module** (mirroring **`getHeuristicPluginInfos()`** / **`getLoadedHeuristicPluginFiles`** in [§10.2](#102-heuristicpluginresourcemanager)). A thin **`hipdnn…_ext`** C wrapper is optional if the C API surface should stay symmetric with other handle queries.
...
- **ABI / loader tests** that load a minimal mock heuristic `.so`, verify **`hipdnnHeuristicGetApiVersion`**, **`hipdnnHeuristicPluginGetAllPolicyIds`** (each **`int64_t`** matches **`policyNameToId`** for the policy name it documents), **`HandleCreate` / `HandleDestroy`**, **`PolicyDescriptorCreate` / `Destroy`**, **`Finalize` / `GetSortedEngineIds`**, and reject wrong major versions; **negative tests** that a second module reporting a **duplicate** policy ID and a module whose optional **`hipdnnHeuristicPluginGetPolicyName`** does not hash to the policy ID it names are **rejected at load** (same spirit as **`EnginePluginManager`** duplicate engine IDs—[§5.3.1](#531-well-known-policy-names-and-ids), [§11](#11-versioning-and-compatibility-checks)).
...
- **Enumeration tests** that assert the handle-scoped **loaded heuristic policy** query ([§16.2](#162-loaded-heuristic-policy-enumeration)) matches modules that passed **`validateBeforeAdding`**-style checks, returns **`int64_t`** IDs consistent with **`hipdnnHeuristicPluginGetAllPolicyIds`**, and that optional frontend helpers return consistent metadata.

---

## 18. Risks and open questions

- **Policy list syntax:** Comma-separated env vars and string array attributes need clear rules for embedded commas and empty tokens; validate unknown **policy names** (no loaded module reporting a matching policy ID) strictly or leniently (skip vs error). If skipping unknown entries leaves no successful policy, **`finalize()`** fails (no backend fallback sort).
...
| **Heuristic C ABI** | `extern "C"` symbol set: module metadata (**`int64_t` policy IDs** via **`hipdnnHeuristicPluginGetAllPolicyIds`**, optional names via **`hipdnnHeuristicPluginGetPolicyName`**), **`hipdnnHeuristicHandle_t`** and **`hipdnnHeuristicPolicyDescriptor_t`** lifecycle, selection functions ([§8](#8-c-abi-for-heuristic-plugins)). |
...
| **Policy name** | UTF-8 string in **`HIPDNN_ATTR_ENGINEHEUR_POLICY_ORDER_EXT`**, env, or handle defaults; hashed with **`policyNameToId`** to form the **`int64_t`** used in **`orderedPolicyIds`** and matched against the IDs a module reports through **`hipdnnHeuristicPluginGetAllPolicyIds`**. |
...
| **Built-in heuristic** | Backend-shipped policy implemented as a populated **`HeuristicPluginFunctionTable`** (the heuristic plugin C ABI shape) and wrapped via **`HeuristicPlugin::createBuiltIn(...)`** at **`HeuristicPluginManager`** construction time. No `.so` is loaded. One table may expose several policy IDs. Today **`SelectionHeuristic::Config`**, **`SelectionHeuristic::StaticOrdering`**, and the prediction pair **`SelectionHeuristic::ModeA`** / **`SelectionHeuristic::ModeB`** (one table, both IDs) ship this way ([§5.3.1](#531-well-known-policy-names-and-ids), [§10.1](#101-heuristicpluginmanager), [§16.2](#162-loaded-heuristic-policy-enumeration)). |
