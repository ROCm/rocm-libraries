...
Provenance information is stored in a **separate companion file** (`{Name}.meta.json`), not in the graph JSON. This keeps the graph schema 1:1 with the hipDNN `graph.fbs` format and avoids polluting the computation definition with test infrastructure details. The test runner does not load this file — it is for humans, tooling, and traceability.

```
{Name}/
  {Name}.json              # Graph definition (unchanged)
  {Name}.tensor{uid}.bin   # Tensor data (unchanged)
  {Name}.meta.json         # Provenance metadata (not loaded by runner)
```
...
The `generator` and `reference_source` fields are mandatory — the pre-commit bundle verifier rejects bundles without them. Remaining fields are optional. Generator scripts populate the metadata file automatically.
...
| 2 | Connect tolerance lookup chain (TOML override -> per-operation default). Add pre-commit bundle verifier. | Old classes still active. Golden tests now use the same tolerance path as computed tests. |
