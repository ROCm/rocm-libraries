...
- **UMD (Universal Match Descriptor).** A matcher. It says when a kernel applies, as a graph
  pattern plus a declarative criteria expression, and it binds the named variables the
  launch step references.
- **UDD (Universal Dispatch Descriptor).** How to invoke a kernel: the dispatch application
  binary interface (ABI), meaning argument binding and ordering, grid, block, shared memory,
  and workspace.
- **UED (Universal Engine Descriptor).** One engine: a stable identity, the KMD fields it
  exposes as knobs, and its behavior and numerical notes. It carries no logic of its own. It
  **names** the engine's one heuristic (UHD) and one metadata schema (KMD) by id, because a
  single selector ranks all of the engine's kernels over one feature space. An engine is a
  named group of kernels.
- **UHD (Universal Heuristic Descriptor).** One kernel-selection model, one per engine.
  Given the kernels that fit a graph, the engine's **catalog** for that graph, it picks the
  best one for the problem, ranking on kernel metadata, problem shape, and device details.
- **KMD (Kernel Metadata Descriptor).** One schema per engine, shared across every kernel it
  owns: the variant fields each kernel carries, each with a type and an optional default
  (tile size, block size, and the like). Each UKD supplies concrete values. That completed
  tuple is the kernel's unique key in the catalog, the heuristic ranks the catalog on it, and
  matchers read the fields as `$kernel.<field>`.
- **UKD (Universal Kernel Descriptor).** One launchable kernel, carrying no logic of its
  own: a source plus concrete values for the fields the engine's KMD declares, either a
  compiled kernel or the details for building it ahead of time (AOT). It inherits everything
  else, matchers and dispatch from its KDP, heuristic and metadata schema from that pack's
  engine, and it applies only when **all** of its pack's matchers pass.
...
A UKD carries only source details and metadata values, inheriting how it applies, launches,
is ranked, and its schema from its KDP and that KDP's engine. The KDP's one UDD holds one or
more **Launches**, each a dispatch step paired at runtime with the UKD source that fills it:
...
shared by ID across packs; only the child kernels are unique to this pack.
...
graph dimension are two shapes, not two values, even with identical argument lists. Section 13's
worked example uses this case: two cohorts of one real kernel family, same five-argument ABI,
...
showing *accept*, so this section walks through one accept and two distinct declines.
...
`causal_bottom_right` or `sliding_window` UKD with no matcher change, but this pack's own §13.6
vector populates only one of them, `causal_top_left`; §13.4's Case C shows what happens to a graph
...
not applicable to it, per §13.1's matcher-plus-`$kernel.*` mechanism. The gap is easy to fix: the
...
**two KDPs** sharing one matcher and one engine (§13.6), each with its own UDD.
...
([Section 5](#5-matching-and-the-umd)): the kernel author's own docstring for
...
`value_or_default`, distinct from the literal one §13.3 uses.
...
| UHD + kernel selection | The heuristic format plus the generic selector that ranks the kernels matching a graph, the per-kernel scorer interface, the retraining pipeline, and whether a future selector may reason over the candidate set as a whole |
...
5. **Static versus dynamic shared memory:** a dispatch descriptor's `shared_mem_bytes` describes the
   ...
   ([Section 13.5](#135-dispatch-geometry-from-kernel) is such a kernel). Should the dispatch
...
- **SDPA:** scaled dot-product attention, the running example operation used throughout this
  document ([Section 13](#13-worked-example-sdpa-as-a-ukd) walks it as a worked example).
