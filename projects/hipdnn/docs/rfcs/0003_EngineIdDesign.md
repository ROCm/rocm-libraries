...
Plugins use the hash function internally to convert their engine names to ids so they can communicate via the existing API.  Dispatch addresses engines by `int64_t` only.

Plugins also report the names themselves, so that the host can display a name for an engine it was never compiled to know about.  Two channels carry the name:

- `hipdnnEnginePluginGetEngineName(int64_t engine_id, const char** name)` in `EnginePluginApi.h`, generated by `EnginePluginImpl.inl` from an optional static `getEngineName` member on the plugin's container type.  This is the authoritative channel.
- The optional `name` field on the `EngineDetails` FlatBuffers payload, which is graph-scoped and used as a fallback.

The static registry described below is therefore a compatibility fallback for engines built into the tree, not the source of truth, and a drop-in plugin needs no registry entry to be named correctly.
...
Adding an engine name to `EngineNames.hpp` is only for engines built into the tree.  A plugin
reports its own names, so a plugin author does not register anything and does not submit a PR
...
2. **Report the Name From the Plugin**: Implement the optional static `getEngineName` member on
   your container type.  `EnginePluginImpl.inl` turns it into the
   `hipdnnEnginePluginGetEngineName` entry point, and the host uses it in preference to the
   registry.  No entry in `EngineNames.hpp` is required and the host emits no warning for a name
   it has never seen.
...
- Once published, names should not be changed (breaks compatibility for anyone addressing the
  engine by name)

### Forward Compatibility

A name the host has never been compiled to know about is not a problem: the hash function generates
an ID for any string, and the plugin reports the name over the plugin API regardless.  This is what
allows a drop-in plugin pack to work against an already-shipped hipDNN without updating the shared
header.
