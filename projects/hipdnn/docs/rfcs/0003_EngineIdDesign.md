...
- `hipdnnEnginePluginGetEngineName(int64_t engine_id, const char** name)` in `EnginePluginApi.h`, generated by `EnginePluginImpl.inl` from an optional static `getEngineName` member on the plugin's container type.  This is the authoritative channel and the only one reachable without a plugin handle and a serialized graph.
- The optional `name` field on the `EngineDetails` FlatBuffers payload, which is graph-scoped and used as a fallback.

The backend resolves a display name by trying the entry point, then `EngineDetails.name`, then the static registry described below, then a hexadecimal rendering of the id.  The static registry is therefore a compatibility fallback for engines built into the tree, not the source of truth, and a drop-in plugin needs no registry entry to be named correctly.  The resolved name is surfaced through the `HIPDNN_ATTR_ENGINE_NAME_EXT` engine-descriptor attribute and through `hipdnnGetEngineInfo_ext`.
...
**Status: not implemented.**  Duplicate-id detection across engine plugins is an open gap.  The
in-process registrar in `EngineNames.hpp` rejects a duplicate or colliding registration, and the
heuristics plugin path validates policy ids at load, but nothing compares engine ids reported by
two different engine plugins.  Closing it needs a policy decision about which plugin wins.
