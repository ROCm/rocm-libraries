...
Algorithm: for each `op_chain` in `S`, compute its **safe rectangle** — the largest `io_dtypes × layouts` rectangle whose tuples are all in `S` and none in `U`. Group op_chains that share the same safe rectangle and emit one matcher per group with that rectangle and the group's op_chains. This guarantees coverage (every observed-supported tuple lies in its op_chain's safe rectangle) and safety (no rectangle contains a `U` tuple). Pure set operations over `std::set` / `std::map`; no trie, no token-splitting, no globbing.

**Worked example — gfx10 with a CBA carve-out.** Engine doesn't have CK fusion kernels: every `ConvFprop + Pointwise:*` tuple is in `U`. Plain `ConvFprop` / `ConvDgrad` / `ConvWgrad` are in `S` with full dtype/layout coverage and zero overlap with `U` → they share the same safe rectangle (all dtypes × all layouts), grouped into one matcher. The `ConvFprop + Pointwise:*` op_chains have empty safe rectangles (every observation is in `U`), so no matcher covers them; CBA tests run, return empty support, harness `GTEST_SKIP`s — no claim fires.

**Mixed-fixture case** (some tuples for an op_chain in `S`, some in `U`): the op_chain's safe rectangle shrinks to the largest sub-rectangle that excludes the `U` tuples — the matcher still gets emitted, just with reduced `io_dtypes` or `layouts`. Coverage of the supported combinations is preserved. The excluded combinations are listed on stderr ("supported: NCHW fp16/fp32/bf16; unsupported: NHWC fp16/fp32 — investigate") so the engineer can decide whether to add a `[[test_skips]]`.
