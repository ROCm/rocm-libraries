...
This index records current limitations without turning them into support guarantees or roadmap commitments. See [Testing Strategy](TESTING_STRATEGY.md) for the validation model, [Building](Building.md) for commands and sanitizer setup, and [CODEOWNERS](../../../.github/CODEOWNERS) for executable review ownership. Linked configuration and workflows remain authoritative where values or runner availability change.
