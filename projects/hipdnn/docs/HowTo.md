...
- Tune performance parameters (e.g., workspace size limits, tile sizes)
- Select algorithmic variants
- Control memory usage

Each knob has a unique string identifier (e.g., `"global.benchmarking"`), a type (integer, float, or string), constraints, and a default value.

#### Quick Start

**Step 1: Query available knobs for an engine**
...
// Query knobs for a specific engine
...
**Step 2: Set knob values when creating an execution plan**

```cpp
// Create knob settings
std::vector<KnobSetting> settings;
settings.emplace_back("global.benchmarking", 1);  // Enable benchmarking

// Create execution plan with custom settings
error = graph.create_execution_plan_ext(engineId, settings);
```
...
|------|------|---------|-------------|
| `global.benchmarking` | int64 | 0 | Enable benchmarking for optimal kernel selection |
| `global.workspace_size_limit` | int64 | Maximum | Limit workspace memory (provider-specific) |

> [!NOTE]
> Different engines and providers support different knobs. Always query available knobs for your specific engine using `get_knobs_for_engine()`.

#### Example: Enable Benchmarking
...
// Find benchmarking knob and check its default
for (const auto& knob : knobs) {
    if (knob.knobId() == "global.benchmarking") {
        std::cout << "Default: " << std::get<int64_t>(knob.defaultValue()) << "\n";
    }
}
...
// Create plan with benchmarking enabled
