...
class IntegrationBackendWrapperDynamicLoad : public testing::Test
{
protected:
    void SetUp() override
    {
    ...
    void expectAdditionalBackendApiForwardsToBackend(Backend& backend) const
    {
        ...
        EXPECT_EQ(
            backend.backendCreateDescriptor(HIPDNN_BACKEND_OPERATIONGRAPH_DESCRIPTOR, nullptr),
        ...
        EXPECT_STREQ(backend.getErrorString(HIPDNN_STATUS_SUCCESS),
                     _backend->getErrorString(HIPDNN_STATUS_SUCCESS));
...
TEST_F(IntegrationBackendWrapperDynamicLoad, SymbolResolutionCachesLoadedSymbols)
{
...
TEST_F(IntegrationBackendWrapperDynamicLoad, DynamicBackendWrapperAdditionalApiForwardsToBackend)
{
    SKIP_IF_NO_DEVICES();

    hipdnn_frontend::detail::HipdnnDynamicBackendWrapper backend(_backend->version());
