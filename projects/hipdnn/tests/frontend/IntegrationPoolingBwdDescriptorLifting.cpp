...
#include <hipdnn_frontend/node/PoolingBwdNode.hpp>
...
// -- Test constants --
constexpr int64_t K_TEST_DY_UID = 42;
constexpr int64_t K_TEST_DX_UID = 43;

constexpr std::array<int64_t, 4> K_TEST_DIMS = {1, 3, 16, 16};
constexpr std::array<int64_t, 4> K_TEST_STRIDES = {768, 256, 16, 1};

// Lifts a frontend graph via build_operation_graph(handle), then
// reconstructs it with fromBackendDescriptor() for verification.
class IntegrationPoolingBwdDescriptorLifting : public ::testing::Test
{
protected:
    void SetUp() override
    {
    ...
    /// Builds a standard PoolingBwd graph for round-trip testing.
    static std::shared_ptr<TestableGraph> buildGraph()
    {
        auto graph = std::make_shared<TestableGraph>();
        graph->set_name("PoolingBwdLiftingTestGraph")
            .set_compute_data_type(DataType::FLOAT)
            .set_intermediate_data_type(DataType::FLOAT)
            .set_io_data_type(DataType::FLOAT);

        auto dy = std::make_shared<TensorAttributes>();
        dy->set_uid(K_TEST_DY_UID)
            .set_name("dy")
            .set_data_type(DataType::FLOAT);
        dy->set_dim(toVec(K_TEST_DIMS)).set_stride(toVec(K_TEST_STRIDES));

        PoolingBwdAttributes attrs;
        ...
        auto dx = graph->pooling_bwd(
            dy,
            attrs);
        dx->set_uid(K_TEST_DX_UID)
            .set_output(true)
            .set_name("dx");
...
// Builds a standard PoolingBwd graph, lowers via build_operation_graph(handle),
...
TEST_F(IntegrationPoolingBwdDescriptorLifting, BasicPoolingBwdRoundTrip)
{
    ...
    auto* opNode = dynamic_cast<PoolingBwdNode*>(subNodes[0].get());
    ASSERT_NE(opNode, nullptr) << "Expected a PoolingBwdNode";
...
TEST_F(IntegrationPoolingBwdDescriptorLifting, PoolingBwdTensorSharingPreserved)
{
    auto originalGraph = buildGraph();
    ...
    EXPECT_EQ(tensorMap[K_TEST_DY_UID].get(),
              opNode->attributes.get_dy().get());
    // Verify dx tensor sharing
    EXPECT_EQ(opNode->attributes.get_dx()->get_uid(), K_TEST_DX_UID);
    EXPECT_EQ(tensorMap[K_TEST_DX_UID].get(),
              opNode->attributes.get_dx().get());
}

// Builds a PoolingBwd graph, serializes to binary, creates a backend descriptor
// from bytes (no handle, no finalize), calls fromBackendDescriptor(), and verifies
// all fields survive the FlatBuffer-direct path.
TEST_F(IntegrationPoolingBwdDescriptorLifting, PoolingBwdLiftWithoutFinalization)
{
    auto originalGraph = buildGraph();
