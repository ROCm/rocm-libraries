...
// -- Test constants for BN inference round-trip --

constexpr int64_t K_TENSOR_X_UID = 70;
constexpr std::array<int64_t, 4> K_TENSOR_X_DIMS = {1, 64, 32, 32};
constexpr std::array<int64_t, 4> K_TENSOR_X_STRIDES = {65536, 1024, 32, 1};
...
constexpr std::array<int64_t, 4> K_PER_CHANNEL_DIMS = {1, 64, 1, 1};
constexpr std::array<int64_t, 4> K_PER_CHANNEL_STRIDES = {64, 1, 1, 1};
...
// Builds a batchnorm inference graph via the frontend API, lowers it to the backend
...
// in the frontend.
TEST_F(IntegrationBatchnormInferenceDescriptorLowering, BatchnormInferenceGraphRoundTrip)
{
    ...
    EXPECT_EQ(yT->dims, toVec(K_TENSOR_Y_DIMS));
    EXPECT_EQ(yT->strides, toVec(K_TENSOR_Y_STRIDES));
...
TEST_F(IntegrationBatchnormInferenceDescriptorLowering, AutoAssignedUidsPreservedInRoundTrip)
{
    ...
    std::unordered_set<int64_t> nodeUids = {bnInf->x_tensor_uid,
                                            ...
                                            bnInf->y_tensor_uid};
    EXPECT_EQ(nodeUids.size(), 6u) << "BatchnormInference node tensor UIDs are not distinct";
}
