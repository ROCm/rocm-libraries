...
// -- Test constants for ConvWrwGraphRoundTrip --
// These use different values from the shared constants (different dims/strides/UIDs)
// and are specific to the conv wrw lowering tests.

constexpr int64_t K_TENSOR_X_UID = 40;
constexpr int64_t K_TENSOR_DY_UID = 41;
constexpr int64_t K_TENSOR_DW_UID = 42;
...
// in the frontend.
TEST_F(IntegrationConvolutionWgradDescriptorLowering, ConvWrwGraphRoundTrip)
{
    auto graph = std::make_shared<TestableGraph>();
    graph->set_name("TestConvWrwGraph")
    ...
    convAttrs.set_name("conv_wrw_op");
...
{
    auto graph = std::make_shared<TestableGraph>();
    graph->set_name("AutoUidWrwGraph")
        .set_io_data_type(DataType::FLOAT)
        .set_intermediate_data_type(DataType::FLOAT)
        .set_compute_data_type(DataType::FLOAT);
