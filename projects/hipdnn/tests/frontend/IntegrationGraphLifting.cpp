...
TEST_F(IntegrationGraphLifting, LayernormTrainingRoundTripViaCApi)
{
    auto graph = std::make_shared<TestableGraph>();
    graph->set_name("LayernormTrainingLiftingTest")
        .set_compute_data_type(DataType::FLOAT)
        .set_intermediate_data_type(DataType::FLOAT)
        .set_io_data_type(DataType::FLOAT);

    auto x = std::make_shared<TensorAttributes>();
    x->set_uid(ln_constants::K_LAYERNORM_TENSOR_X_UID).set_name("X").set_data_type(DataType::FLOAT);
    x->set_dim(toVec(ln_constants::K_LAYERNORM_TENSOR_X_DIMS)).set_stride(toVec(ln_constants::K_LAYERNORM_TENSOR_X_STRIDES));

    auto scale = std::make_shared<TensorAttributes>();
    scale->set_uid(ln_constants::K_LAYERNORM_TENSOR_SCALE_UID).set_name("SCALE").set_data_type(DataType::FLOAT);
    scale->set_dim(toVec(ln_constants::K_LAYERNORM_TENSOR_SCALE_DIMS))
        .set_stride(toVec(ln_constants::K_LAYERNORM_TENSOR_SCALE_STRIDES));

    auto bias = std::make_shared<TensorAttributes>();
    bias->set_uid(ln_constants::K_LAYERNORM_TENSOR_BIAS_UID).set_name("BIAS").set_data_type(DataType::FLOAT);
    bias->set_dim(toVec(ln_constants::K_LAYERNORM_TENSOR_BIAS_DIMS))
        .set_stride(toVec(ln_constants::K_LAYERNORM_TENSOR_BIAS_STRIDES));

    auto epsilon = std::make_shared<TensorAttributes>();
    epsilon->set_uid(ln_constants::K_LAYERNORM_TENSOR_EPSILON_UID)
        .set_name("EPSILON")
        .set_data_type(DataType::FLOAT)
        .set_dim(toVec(ln_constants::K_LAYERNORM_TENSOR_EPSILON_DIMS))
        .set_stride(toVec(ln_constants::K_LAYERNORM_TENSOR_EPSILON_STRIDES))
        .set_value(1e-5f);
    ...
    y->set_uid(ln_constants::K_LAYERNORM_TENSOR_Y_UID).set_output(true).set_name("Y");
    ASSERT_NE(mean, nullptr);
    ASSERT_NE(invVariance, nullptr);
    mean->set_uid(ln_constants::K_LAYERNORM_TENSOR_MEAN_UID).set_output(true).set_name("MEAN");
    invVariance->set_uid(ln_constants::K_LAYERNORM_TENSOR_INV_VARIANCE_UID)
        .set_output(true)
    ...
    // Verify tensors by UID (7 tensors: x, scale, bias, epsilon, y, mean, inv_variance)
    auto tensorMap = liftedGraph->getTensorsByUid();
    ASSERT_EQ(tensorMap.size(), 7u);

    ASSERT_NE(tensorMap.count(ln_constants::K_LAYERNORM_TENSOR_X_UID), 0u);
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_X_UID]->get_dim(), toVec(ln_constants::K_LAYERNORM_TENSOR_X_DIMS));
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_X_UID]->get_stride(),
              toVec(ln_constants::K_LAYERNORM_TENSOR_X_STRIDES));
    ...
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_Y_UID]->get_dim(), toVec(ln_constants::K_LAYERNORM_TENSOR_Y_DIMS));
    ...
    // Verify the lifted graph has 1 layernorm sub-node
    auto& subNodes = liftedGraph->getSubNodes();
    ASSERT_EQ(subNodes.size(), 1u);

    auto* lnNode = dynamic_cast<LayerNormNode*>(subNodes[0].get());
    ASSERT_NE(lnNode, nullptr);
    ...
    EXPECT_EQ(lnNode->attributes.get_scale()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_SCALE_UID);
    EXPECT_EQ(lnNode->attributes.get_bias()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_BIAS_UID);
    EXPECT_EQ(lnNode->attributes.get_epsilon()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_EPSILON_UID);
    EXPECT_EQ(lnNode->attributes.get_y()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_Y_UID);
    ASSERT_NE(lnNode->attributes.get_mean(), nullptr);
    EXPECT_EQ(lnNode->attributes.get_mean()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_MEAN_UID);
    ASSERT_NE(lnNode->attributes.get_inv_variance(), nullptr);
    EXPECT_EQ(lnNode->attributes.get_inv_variance()->get_uid(),
              ln_constants::K_LAYERNORM_TENSOR_INV_VARIANCE_UID);
}

// Builds a layernorm graph in INFERENCE mode (no mean/inv_variance), lowers
// via build_operation_graph(handle), lifts back with fromBackendDescriptor(),
// and verifies all operation attributes and that optional tensors are absent.
TEST_F(IntegrationGraphLifting, LayernormInferenceRoundTripViaCApi)
{
    auto graph = std::make_shared<TestableGraph>();
    ...
    LayernormAttributes layernormAttrs;
    layernormAttrs.set_name("layernorm_inference_op");
    ...
    // Mean and inv_variance should NOT be in the tensor map
    EXPECT_EQ(tensorMap.count(ln_constants::K_LAYERNORM_TENSOR_MEAN_UID), 0u);
    EXPECT_EQ(tensorMap.count(ln_constants::K_LAYERNORM_TENSOR_INV_VARIANCE_UID), 0u);
    ...
    EXPECT_EQ(lnNode->attributes.get_forward_phase(), NormFwdPhase::INFERENCE);
    EXPECT_EQ(lnNode->attributes.get_name(), "layernorm_inference_op");

    // Optional tensors should be null on the lifted node
    EXPECT_EQ(lnNode->attributes.get_mean(), nullptr);
    EXPECT_EQ(lnNode->attributes.get_inv_variance(), nullptr);
}

// Verifies that tensor objects are shared between the tensor map and the
// layernorm node attributes after lifting.
TEST_F(IntegrationGraphLifting, LayernormTensorSharingPreserved)
{
    auto graph = std::make_shared<TestableGraph>();
    ...
    LayernormAttributes layernormAttrs;
    layernormAttrs.set_name("layernorm_sharing_op");
    layernormAttrs.set_forward_phase(NormFwdPhase::TRAINING);
    layernormAttrs.set_epsilon(epsilon);
    ...
    // Verify tensor objects are shared (same pointer) between map and node
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_X_UID].get(), lnNode->attributes.get_x().get());
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_SCALE_UID].get(),
              lnNode->attributes.get_scale().get());
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_BIAS_UID].get(), lnNode->attributes.get_bias().get());
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_EPSILON_UID].get(),
              lnNode->attributes.get_epsilon().get());
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_Y_UID].get(), lnNode->attributes.get_y().get());
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_MEAN_UID].get(), lnNode->attributes.get_mean().get());
    EXPECT_EQ(tensorMap[ln_constants::K_LAYERNORM_TENSOR_INV_VARIANCE_UID].get(),
              lnNode->attributes.get_inv_variance().get());
}
...
TEST_F(IntegrationGraphLifting, LayernormLiftWithoutFinalization)
{
    auto graph = std::make_shared<TestableGraph>();
    ...
    LayernormAttributes layernormAttrs;
    layernormAttrs.set_name("layernorm_fb_op");
    layernormAttrs.set_forward_phase(NormFwdPhase::TRAINING);
    layernormAttrs.set_epsilon(epsilon);
    ...
    EXPECT_EQ(lnNode->attributes.get_forward_phase(), NormFwdPhase::TRAINING);
    EXPECT_EQ(lnNode->attributes.get_name(), "layernorm_fb_op");
    EXPECT_EQ(lnNode->attributes.get_x()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_X_UID);
    EXPECT_EQ(lnNode->attributes.get_scale()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_SCALE_UID);
    EXPECT_EQ(lnNode->attributes.get_bias()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_BIAS_UID);
    EXPECT_EQ(lnNode->attributes.get_epsilon()->get_uid(), ln_constants::K_LAYERNORM_TENSOR_EPSILON_UID);
...
TEST_F(IntegrationGraphLifting, LayernormDeserializeViaBackendWithHandle)
{
    auto graph = std::make_shared<TestableGraph>();
    graph->set_name("LayernormDeserializeTest")
        .set_compute_data_type(DataType::FLOAT)
        .set_intermediate_data_type(DataType::FLOAT)
        .set_io_data_type(DataType::FLOAT);
    ...
    LayernormAttributes layernormAttrs;
    layernormAttrs.set_name("layernorm_deserialize_op");
    layernormAttrs.set_forward_phase(NormFwdPhase::TRAINING);
    layernormAttrs.set_epsilon(epsilon);
    ...
    EXPECT_EQ(lnNode->attributes.get_forward_phase(), NormFwdPhase::TRAINING);
    EXPECT_EQ(lnNode->attributes.get_name(), "layernorm_deserialize_op");
