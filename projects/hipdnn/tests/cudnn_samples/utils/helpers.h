 ...
 *
 * Substituted for the hipDNN cuDNN-compatibility dry run. Only the prologue
 * differs from upstream: include block, handle helpers, arch predicates.
 */
...
// The corpus is not self-contained by its own include lines: upstream force-includes
// cudnn_frontend.h via an INTERFACE PCH. cudnnStatus_t/cudnnHandle_t are named here
// with no cuDNN include at all, so pull it in explicitly.
#include <cudnn.h>
...
#define CUDA_CHECK(status)                                                                 \
    {                                                                                      \
        ...
        {                                                                                  \
            ...
            err_msg << "CUDA Error: " << cudaGetErrorString(err) << " (" << err << ") at " \
                    << __FILE__ << ":" << __LINE__;                                        \
            FAIL(err_msg.str());                                                           \
        }                                                                                  \
    }
...
#define CUDNN_CHECK(status)                                                                  \
    {                                                                                        \
        ...
        {                                                                                    \
            std::stringstream err_msg;                                                       \
            err_msg << "cuDNN Error: " << cudnnGetErrorString(err) << " (" << err << ") at " \
                    << __FILE__ << ":" << __LINE__;                                          \
            FAIL(err_msg.str());                                                             \
        }                                                                                    \
    }

// CudnnHandleDeleter and create_cudnn_handle() are omitted: the shim's <cudnn.h>
// already defines both, and a second definition here is an ODR clash.

// Arch predicates are pinned, not derived. HIP's prop.major/minor would give 94 for
// gfx942 ("Hopper") and 115 for gfx1151 ("Blackwell").
...
inline bool is_blackwell_computing_arch()
{
    return false;
}

inline bool is_arch_supported_by_cudnn()
{
    return true;
}

// Upstream defaults to 6.0 and only overwrites on a recognised string, so a typo
// fails open. Reject unknown strings instead.
inline bool check_device_arch_newer_than(const std::string& arch)
{
    if(arch == "blackwell" || arch == "hopper")
    {
        return false;
    }
    if(arch == "ampere" || arch == "turing" || arch == "volta" || arch == "pascal")
    {
        return true;
    }
    throw std::runtime_error("check_device_arch_newer_than: unknown arch " + arch);
}
...
static void initHostImage(float* image, int64_t imageSize)
{
...
static void initHostImage(half* image, int64_t imageSize)
{
    static unsigned seed = 123456789;
    for(int64_t index = 0; index < imageSize; index++)
    {
...
static void initHostImage(int8_t* image, int64_t imageSize)
{
    static unsigned seed = 123456789;
    for(int64_t index = 0; index < imageSize; index++)
    {
...
static void initHostImage(uint8_t* image, int64_t imageSize)
{
    static unsigned seed = 123456789;
    for(int64_t index = 0; index < imageSize; index++)
    {
...
static void initHostImage(int32_t* image, int64_t imageSize)
{
    static unsigned seed = 123456789;
    for(int64_t index = 0; index < imageSize; index++)
    {
...
// Currently set to generate uniform integers [0,1]
static void initHostImage(int64_t* image, int64_t imageSize)
{
    static unsigned seed = 123456789;
    for(int64_t index = 0; index < imageSize; index++)
    {
...
static void initHostImage(bool* image, int64_t imageSize)
{
    static unsigned seed = 123456789;
    for(int64_t index = 0; index < imageSize; index++)
    {
...
template <typename T>
static void initImage(T* devPtr, size_t imageSize)
{
    if(imageSize == 0)
    {
        return;
    ...
    CUDA_CHECK(
        cudaMemcpy(devPtr, host.data(), sizeof(host[0]) * imageSize, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaDeviceSynchronize());
}

template <typename T>
static void fillImage(T* devPtr, size_t imageSize, T fillValue)
{
    if(imageSize == 0)
    {
        return;
...
public:
    ...
    explicit Surface(size_t size, T_ELEM fillValue)
        : size(size)
    {
        if(size == 0)
        {
        ...
        : size(other.size)
    {
        ...
        CUDA_CHECK(cudaMalloc(&devPtr, size * sizeof(T_ELEM)));
        CUDA_CHECK(
            cudaMemcpy(devPtr, other.devPtr, sizeof(devPtr[0]) * size, cudaMemcpyDeviceToDevice));
        CUDA_CHECK(cudaDeviceSynchronize());
    }
