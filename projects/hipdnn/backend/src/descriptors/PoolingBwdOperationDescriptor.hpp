...
class PoolingBwdOperationDescriptor
    : public HipdnnBackendDescriptorImpl<PoolingBwdOperationDescriptor>,
...
public:
    ...
    const hipdnn_data_sdk::data_objects::PoolingBwdAttributesT& getData() const
    {
        return _data;
    }
    ...
    // Creates a finalized PoolingBwdOperationDescriptor directly from a FlatBuffer NodeT.
    // Casts nodeT.attributes to PoolingBwdAttributes internally, then directly assigns
    // the data struct, looks up tensor descriptors from the tensor map, and calls finalize().
    static std::shared_ptr<PoolingBwdOperationDescriptor>
...
private:
    hipdnn_data_sdk::data_objects::PoolingBwdAttributesT _data;
