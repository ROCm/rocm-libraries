...
int main()
{
    ...
    std::cout << "[Step 2] Registering engine with UHD config..." << std::endl;
    ...
    // Load the model into the adapter (cached in registry)
    auto adapter = TreeDataAdapter::loadFromBuffer(modelBuffer.data(),
                                                   modelBuffer.size(),
                                                   entry.uhdConfig.featuresHash);
    if(!adapter)
    {
        std::cerr << "ERROR: Failed to load GBDT model" << std::endl;
        return 1;
    }
    ...
    std::cout << "  Device: cu_count=120, total_mem=32GB" << std::endl;
    std::cout << "  Query: batch=32, seqlen_q=2048, heads=32, hdim=128" << std::endl;
    std::cout << std::endl;

    // Step 4: Run selection
    std::cout << "[Step 4] Running UHD selection..." << std::endl;
