                          ...
                          hipdnnDataType_t computeType = HIPDNN_DATA_FLOAT)
{
    ...
    desc->setAttribute(
        HIPDNN_ATTR_OPERATION_POOLING_FORWARD_X, HIPDNN_TYPE_BACKEND_DESCRIPTOR, 1, &xDesc);
    desc->setAttribute(
        HIPDNN_ATTR_OPERATION_POOLING_FORWARD_Y, HIPDNN_TYPE_BACKEND_DESCRIPTOR, 1, &yDesc);
    ...
    auto windowSize = toVec(K_WINDOW_SIZE);
...
{
    auto xDesc = createFinalizedTensor(K_TENSOR_X_UID, toVec(K_TENSOR_X_DIMS), toVec(K_TENSOR_X_STRIDES));
    auto yDesc = createFinalizedTensor(K_TENSOR_Y_UID, toVec(K_TENSOR_Y_DIMS), toVec(K_TENSOR_Y_STRIDES));
...
TEST_F(TestGraphDescriptorPoolingFwd, PoolingFwdAttributesPreserved)
{
    ...
    opDesc->setAttribute(
        HIPDNN_ATTR_OPERATION_POOLING_FORWARD_X, HIPDNN_TYPE_BACKEND_DESCRIPTOR, 1, &xPtr);
    HipdnnBackendDescriptor* yPtr = yDesc.get();
    opDesc->setAttribute(
        HIPDNN_ATTR_OPERATION_POOLING_FORWARD_Y, HIPDNN_TYPE_BACKEND_DESCRIPTOR, 1, &yPtr);
    ...
    // Set operation name
    std::string opName = "test_poolingfwd";
