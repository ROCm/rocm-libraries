...
TEST_F(TestGraphDescriptorOps, SetOperationsMultipleBatches)
{
    ...
    // Set operations in separate setAttribute calls to verify accumulation
    HipdnnBackendDescriptor* op1 = conv1.convOp.get();
    ASSERT_NO_THROW(desc->setAttribute(
        HIPDNN_ATTR_OPERATIONGRAPH_OPS, HIPDNN_TYPE_BACKEND_DESCRIPTOR, 1, &op1));

    HipdnnBackendDescriptor* op2 = convOp2.get();
    ASSERT_NO_THROW(desc->setAttribute(
        HIPDNN_ATTR_OPERATIONGRAPH_OPS, HIPDNN_TYPE_BACKEND_DESCRIPTOR, 1, &op2));
