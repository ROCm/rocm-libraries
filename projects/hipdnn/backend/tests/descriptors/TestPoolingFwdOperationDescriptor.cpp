...
TEST_F(TestPoolingFwdOperationDescriptor, GetAttributeTensorDescriptor)
{
    ...
    std::unique_ptr<HipdnnBackendDescriptor> guardX(retrievedX);
}
...
TEST_F(TestPoolingFwdOperationDescriptor, BuildNodeProducesCorrectNodeT)
{
    ...
    ASSERT_EQ(poolAttrs->x_tensor_uid, K_TENSOR_X_UID);
    ASSERT_EQ(poolAttrs->y_tensor_uid, K_TENSOR_Y_UID);
    EXPECT_EQ(poolAttrs->pre_padding, toVec(K_PRE_PADDING));
    EXPECT_EQ(poolAttrs->stride, toVec(K_STRIDE));
    EXPECT_EQ(poolAttrs->window_size, toVec(K_WINDOW_SIZE));
...
{
    auto desc = getDescriptor();
    std::string name = "test_poolingfwd_op";
