...
protected:
    ...
    static hipdnn_data_sdk::data_objects::PoolingFwdAttributesT createStandardPoolingFwdAttrs()
    {
        ...
        attrs.window_size = toVec(K_POOL_FWD_WINDOW_SIZE);
...
TEST_F(TestPoolingFwdOperationFromNode, NodeFactoryDelegatesCorrectly)
{
    ...
    EXPECT_EQ(desc->getData().window_size, toVec(K_POOL_FWD_WINDOW_SIZE));
...
TEST_F(TestPoolingFwdOperationFromNode, BuildNodeRoundTrip)
{
    ...
    EXPECT_EQ(rebuiltAttrs->window_size, toVec(K_POOL_FWD_WINDOW_SIZE));
...
TEST_F(TestPoolingFwdOperationFromNode, GetAttributeWorksAfterFromNode)
{
    ...
    verifyTensorDescriptor(xScoped.get(), K_POOL_FWD_TENSOR_X_UID, HIPDNN_DATA_FLOAT,
                           {1, 3, 32, 32},
                           {3072, 1024, 32, 1});
    ...
    verifyTensorDescriptor(yScoped.get(), K_POOL_FWD_TENSOR_Y_UID, HIPDNN_DATA_FLOAT,
                           {1, 3, 16, 16},
                           {768, 256, 16, 1});
