...
def save_batchnorm_inference_execution(
    x_size: list[int],
    ...
    mean = TensorAttributes.random(
        min_val, max_val, intermediate_data_type, derived_sizes
    )
    inv_variance = TensorAttributes.random(
        min_val, max_val, intermediate_data_type, derived_sizes
    )
    scale = TensorAttributes.random(
        min_val, max_val, intermediate_data_type, derived_sizes
    )
    bias = TensorAttributes.random(
        min_val, max_val, intermediate_data_type, derived_sizes
    )
    ...
    graph = Graph(
        ...
        compute_data_type=io_data_type,
...
def main():
    ...
    parser.add_argument(
        "--min", default=0.1, type=float, help="minimum value in tensor"
    )
    parser.add_argument(
        "--max", default=1.0, type=float, help="maximum value in tensor"
    )
    parser.add_argument(
        "--seed", default=0, type=int, help="seed for random data in tensors"
    )
    parser.add_argument(
        "--gpu", default=False, type=bool, help="Use cuda or rocm backend"
    )
    ...
    save_batchnorm_inference_execution(
        args.size,
