...
public:
    ...
    Error pre_validate_node() const override
    {
        ...
        HIPDNN_RETURN_IF_TRUE(
            dxDims.empty(),
            ErrorCode::ATTRIBUTE_NOT_SET,
            "ConvolutionDgradNode: dx (gradient of input) dimensions must be set explicitly; they "
            "cannot be inferred from dy and w because the forward floor-division makes the input "
            "spatial size ambiguous under stride > 1 and the group count is not recoverable. Set "
            "dx "
            "dimensions to match the original forward input tensor x.");
    ...
    Error infer_properties_node() override
    {
        ...
        // dx dimensions are not inferable for dgrad (the forward floor-division makes the
        // input spatial size ambiguous under stride > 1 and the group count is not
        // recoverable), so they must be set explicitly. pre_validate_node enforces this;
        // this guard protects against infer being called without prior pre-validation.
        HIPDNN_RETURN_IF_TRUE(dx->get_dim().empty(),
                              ErrorCode::ATTRIBUTE_NOT_SET,
                              "ConvolutionDgradNode: dx dimensions must be set explicitly before "
                              "property inference");
