    ...
    graph::PoolingFwdAttributes& attributes)
{
    ...
    HIPDNN_CHECK_ERROR(unpackAndRegisterTensor(
        opDesc, HIPDNN_ATTR_OPERATION_POOLING_FORWARD_X, tensorMap, xTensor, "pooling X tensor"));
    attributes.set_x(xTensor);

    // Unpack y tensor
    std::shared_ptr<graph::TensorAttributes> yTensor;
    HIPDNN_CHECK_ERROR(unpackAndRegisterTensor(
        opDesc, HIPDNN_ATTR_OPERATION_POOLING_FORWARD_Y, tensorMap, yTensor, "pooling Y tensor"));
