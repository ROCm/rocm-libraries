...
protected:
    void SetUp() override
    {
        ...
        // The wide fixture path, the selected path and real backend use are exercised on
        // every Windows host, whatever its active code page: an ANSI-lossy host is where
        // the regression bites, and a UTF-8 host still proves the wide path is honoured.
        // The code page is recorded so a reader can tell the two runs apart.
