...
public:
    ...
    void injectValidCompiledPlan(int64_t engineId, int64_t workspaceSize, bool barred)
    {
        ...
        auto engineConfigHandle = reinterpret_cast<hipdnnBackendDescriptor_t>(
            0xE0 + engineId); // NOLINT(performance-no-int-to-ptr)
        auto executionPlanHandle = reinterpret_cast<hipdnnBackendDescriptor_t>(
            0xF0 + engineId); // NOLINT(performance-no-int-to-ptr)
