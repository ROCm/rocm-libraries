...
protected:
                         ...
                         DataType tolerance)
    {
        ...
        deviceBuffers.push_back(generateRandomDeviceBuffer(
            xTensor, static_cast<int>(xTensorUid), static_cast<DataType>(-1.0f), static_cast<DataType>(1.0f), testCase.conv.seed));

        PinnedTensor<DataType> wTensor(testCase.conv.wDims, _layout);
        const auto wTensorUid = tensorUid++;
        deviceBuffers.push_back(generateRandomDeviceBuffer(
            wTensor, static_cast<int>(wTensorUid), static_cast<DataType>(-1.0f), static_cast<DataType>(1.0f), testCase.conv.seed));
        ...
        if(testCase.doBias)
        {
            ...
            deviceBuffers.push_back(generateRandomDeviceBuffer(
                biasTensor, static_cast<int>(biasTensorUid), static_cast<DataType>(-1.0f), static_cast<DataType>(1.0f), testCase.conv.seed));
        ...
        CpuReferenceGraphExecutor().execute(fusionBuilder.GetBufferPointer(), fusionBuilder.GetSize(), variantPack);
...
class TestGpuMiopenConvFwdBiasActivExecuteGraphNchwFp32 : public ConvFwdBiasActivExecuteGraphBase<float>
{
public:
