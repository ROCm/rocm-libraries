...
def _ab(torch, op, call, ref_native, tag, warmup, iters, dtype):
    ...
    print(f"{tag}  {'OK ' if close else 'BAD'} maxerr={max_err:8.5f}  "
...
def _run_linear(torch, shapes, warmup, iters, dtype):
    ...
    for m, n, k in shapes:
        ...
        _ab(torch, "linear", lambda: F.linear(x, w), lambda: F.linear(x, w),
            tag, warmup, iters, dtype)
    print()
...
def main() -> int:
    ...
    ap.add_argument("--ops", default="linear,rmsnorm,sdpa",
                    help="comma-separated subset of linear,rmsnorm,sdpa")
    ap.add_argument("--dtype", default="bf16", choices=["bf16", "f16"])
    ap.add_argument("--warmup", type=int, default=int(os.environ.get("MB_WARMUP", "10")))
