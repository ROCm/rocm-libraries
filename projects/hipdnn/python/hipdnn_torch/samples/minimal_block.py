...
of the exact functionals hipdnn_torch intercepts -- ``F.rms_norm`` + ``nn.Linear``
(``F.linear``) + ``F.scaled_dot_product_attention`` -- with shapes the shipped
gfx1151 kernels can serve (heads=32, head_dim=64, hidden=2048, seqlen and feature
dims all multiples of 16). It then:
...
def build_block(torch):
    ...
    class Block(nn.Module):
        ...
        def __init__(self, heads=32, head_dim=64, mlp=8192, dtype=torch.bfloat16):
            ...
            self.n2 = nn.Parameter(torch.ones(hidden, dtype=dtype))
        ...
        def forward(self, x):
            b, s, _ = x.shape
            h = F.rms_norm(x, (self.hidden,), self.n1, eps=self.eps)
            ...
            x = x + self.proj(a)
            h = F.rms_norm(x, (self.hidden,), self.n2, eps=self.eps)
...
def main() -> int:
    if not hipdnn_torch.provider_ready():
        print("provider/torch not ready -- set HIPDNN_TORCH_PROVIDER_SO and run "
              "under a provider-compatible torch (see ../README.md).", file=sys.stderr)
        return 1
    ...
    print(f"block   = heads=32 head_dim=64 hidden={model.hidden} seq={seqlen} dtype={dtype}")
    print()
    ...
    rel = max_err / denom
    ok = rel < 5.0e-2
    print(f"parity native-vs-hipdnn: {'OK ' if ok else 'BAD'} "
