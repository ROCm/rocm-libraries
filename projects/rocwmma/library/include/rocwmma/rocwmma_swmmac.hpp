...
// ============================================================================
// MXFp4 — 软件 MXFP4 SWMMAC backend (INT4 HW + Q16/FP32 accum)
// A/B 复用 SwmmacI4 的寄存器布局 (<2xi32>/<4xi32>)
//
// Two accumulation paths:
//   Float path:  INT4 SWMMAC → INT32 → FP32 → float scale multiply
//   Q16  path:   INT4 SWMMAC → INT32 → int64_t → integer shift (lossless)
//
// Q16 uses UE8M0 exponent shift: e = (int)sA + (int)sB - 254.
// Scale = 2^e, applied as acc << e (or acc >> (-e)). No float rounding.
// ============================================================================
using SwmmacI64AccumT = __attribute__((__vector_size__(64))) int64_t; // <8 x int64_t>
...
struct MXFp4 {
    ...
    // --- Float path (backward compatible) ---
    __device__ static inline DRegsT exec(
        ARegsT const& a, BRegsT const& b,
        CRegsT const& c, int32_t sparse_idx = 0)
    {
    ...
    // --- Q16 fixed-point path (integer shift, no float rounding) ---
    // Single k-block: SWMMAC → INT32 → int64_t shift → accumulate.
    // e = (int)sA + (int)sB - 254 (UE8M0 combined exponent, bias 127+127)
    // Scale = 2^e, applied as exact integer << or >>
    __device__ static inline void accum_q16(
        SwmmacI64AccumT& total,       // in/out: running total in int64_t
        ARegsT const& a, BRegsT const& b,
        uint8_t sA, uint8_t sB)
    {
        using I4 = SwmmacInt4<true, true, true>;
        typename I4::CRegsT ci;
        for(int j=0;j<8;j++) ((int32_t*)&ci)[j] = 0;
        ci = I4::exec(a, b, ci, 0);
        int e = (int)sA + (int)sB - 254;
        if(e >= 0) {
            if(e > 31) e = 31;
            #pragma unroll
            for(int j=0;j<8;j++)
                ((int64_t*)&total)[j] += (int64_t)((int32_t*)&ci)[j] << e;
        } else {
            if(e < -31) e = -31;
            #pragma unroll
            for(int j=0;j<8;j++)
    ...
    __device__ static inline DRegsT q16_to_float(SwmmacI64AccumT const& res) {
        DRegsT r; for(int j=0;j<8;j++) ((float*)&r)[j] = (float)((int64_t*)&res)[j];
        return r;
    }
};
...
// FP8/BF8 backends
template <>
struct is_swmmac<SwmmacFp8<float8_t,  float8_t>>  : true_type {};
template <>
struct is_swmmac<SwmmacFp8<float8_t,  bfloat8_t>> : true_type {};
template <>
struct is_swmmac<SwmmacFp8<bfloat8_t, float8_t>>  : true_type {};
template <>
struct is_swmmac<SwmmacFp8<bfloat8_t, bfloat8_t>> : true_type {};

// FP16/BF16 backends
template <>
struct is_swmmac<SwmmacFp16> : true_type {};
template <>
struct is_swmmac<SwmmacBf16> : true_type {};
...
// Shared trait computation — InputTA/TB/ComputeT differ across backends
// INT4/INT8:   ComputeT = int32_t
// FP16/BF16/FP8: ComputeT = float32_t
template <typename ImplT, typename TA, typename TB, typename ComputeT_ = int32_t>
...
template <>
struct swmmac_traits<MXFp4>
    : swmmac_traits_base<MXFp4, int4_t, int4_t> {};

// FP8/BF8 traits — 4 combos, A/B packed as i32, accum as f32
template <>
struct swmmac_traits<SwmmacFp8<float8_t,  float8_t>>
    : swmmac_traits_base<SwmmacFp8<float8_t,  float8_t>,  float8_t,  float8_t,  float32_t>  {};
template <>
struct swmmac_traits<SwmmacFp8<float8_t,  bfloat8_t>>
    : swmmac_traits_base<SwmmacFp8<float8_t,  bfloat8_t>, float8_t,  bfloat8_t, float32_t> {};
template <>
struct swmmac_traits<SwmmacFp8<bfloat8_t, float8_t>>
    : swmmac_traits_base<SwmmacFp8<bfloat8_t, float8_t>,  bfloat8_t, float8_t,  float32_t>  {};
template <>
struct swmmac_traits<SwmmacFp8<bfloat8_t, bfloat8_t>>
    : swmmac_traits_base<SwmmacFp8<bfloat8_t, bfloat8_t>, bfloat8_t, bfloat8_t, float32_t> {};

// FP16 traits — A=<8×f16>, B=<16×f16>, C/D=<8×f32>
template <>
struct swmmac_traits<SwmmacFp16>
    : swmmac_traits_base<SwmmacFp16, float16_t, float16_t, float32_t> {};

// BF16 traits — A=<8×i16>, B=<16×i16>, C/D=<8×f32> (bf16 bitcast as i16)
template <>
struct swmmac_traits<SwmmacBf16>
    : swmmac_traits_base<SwmmacBf16, bfloat16_t, bfloat16_t, float32_t> {};
