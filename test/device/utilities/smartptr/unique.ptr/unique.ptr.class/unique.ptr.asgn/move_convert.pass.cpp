...
struct GenericConvertingDeleter {
  template <int OID>
  __device__ TEST_CONSTEXPR_CXX23 GenericConvertingDeleter(GenericConvertingDeleter<OID>) {}

  template <int OID>
  __device__ TEST_CONSTEXPR_CXX23 GenericConvertingDeleter& operator=(GenericConvertingDeleter<OID> const&) {
    return *this;
  }
...
struct TrackingDeleter {
  __device__ TrackingDeleter() : arg_type(&makeArgumentID<>()) {}

  __device__ TrackingDeleter(TrackingDeleter const&)
      : arg_type(&makeArgumentID<TrackingDeleter const&>()) {}

  __device__ TrackingDeleter(TrackingDeleter&&)
      : arg_type(&makeArgumentID<TrackingDeleter &&>()) {}

  template <class T, class = EnableIfSpecialization<TrackingDeleter, T> >
  __device__ TrackingDeleter(T&&) : arg_type(&makeArgumentID<T&&>()) {}

  __device__ TrackingDeleter& operator=(TrackingDeleter const&) {
    arg_type = &makeArgumentID<TrackingDeleter const&>();
    return *this;
  }

  __device__ TrackingDeleter& operator=(TrackingDeleter &&) {
    arg_type = &makeArgumentID<TrackingDeleter &&>();
    return *this;
  }

  template <class T, class = EnableIfSpecialization<TrackingDeleter, T> >
  __device__ TrackingDeleter& operator=(T&&) {
...
public:
  __device__ TypeID const* reset() const {
...
struct ConstTrackingDeleter {
  __device__ ConstTrackingDeleter() : arg_type(&makeArgumentID<>()) {}

  __device__ ConstTrackingDeleter(ConstTrackingDeleter const&)
      : arg_type(&makeArgumentID<ConstTrackingDeleter const&>()) {}

  __device__ ConstTrackingDeleter(ConstTrackingDeleter&&)
      : arg_type(&makeArgumentID<ConstTrackingDeleter &&>()) {}

  template <class T, class = EnableIfSpecialization<ConstTrackingDeleter, T> >
  __device__ ConstTrackingDeleter(T&&) : arg_type(&makeArgumentID<T&&>()) {}

  __device__ const ConstTrackingDeleter& operator=(ConstTrackingDeleter const&) const {
    arg_type = &makeArgumentID<ConstTrackingDeleter const&>();
    return *this;
  }

  __device__ const ConstTrackingDeleter& operator=(ConstTrackingDeleter &&) const {
    arg_type = &makeArgumentID<ConstTrackingDeleter &&>();
    return *this;
  }

  template <class T, class = EnableIfSpecialization<ConstTrackingDeleter, T> >
  __device__ const ConstTrackingDeleter& operator=(T&&) const {
    arg_type = &makeArgumentID<T&&>();
    return *this;
  }
...
__device__ bool checkArg(TrackingDeleter<ID> const& d) {
  return d.arg_type && *d.arg_type == makeArgumentID<ExpectT>();
}

template <class ExpectT, int ID>
__device__ bool checkArg(ConstTrackingDeleter<ID> const& d) {
  return d.arg_type && *d.arg_type == makeArgumentID<ExpectT>();
}

template <class From, bool AssignIsConst = false>
struct AssignDeleter {
  __device__ TEST_CONSTEXPR_CXX23 AssignDeleter()                     = default;
  __device__ TEST_CONSTEXPR_CXX23 AssignDeleter(AssignDeleter const&) = default;
  __device__ TEST_CONSTEXPR_CXX23 AssignDeleter(AssignDeleter&&)      = default;

  __device__ AssignDeleter& operator=(AssignDeleter const&) = delete;
  __device__ AssignDeleter& operator=(AssignDeleter &&) = delete;

  template <class T> __device__ AssignDeleter& operator=(T&&) && = delete;
  template <class T> __device__ AssignDeleter& operator=(T&&) const && = delete;

  template <class T, class = typename std::enable_if< std::is_same<T&&, From>::value && !AssignIsConst >::type>
  __device__ TEST_CONSTEXPR_CXX23 AssignDeleter& operator=(T&&) & {
    return *this;
  }

  template <class T, class = typename std::enable_if< std::is_same<T&&, From>::value && AssignIsConst >::type>
  __device__ TEST_CONSTEXPR_CXX23 const AssignDeleter& operator=(T&&) const& {
    return *this;
  }

  template <class T>
  __device__ TEST_CONSTEXPR_CXX23 void operator()(T) const {}
};

template <class VT, class DDest, class DSource>
__device__ TEST_CONSTEXPR_CXX23 void doDeleterTest() {
  using U1 = gpu::unique_ptr<VT, DDest>;
  using U2 = gpu::unique_ptr<VT, DSource>;
...
__device__ TEST_CONSTEXPR_CXX23 void test_sfinae() {
  ...
  { // Test that different non-reference deleter types are allowed so long
    ...
    using U1 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> >;
    using U2 = gpu::unique_ptr<VT, GenericConvertingDeleter<1> >;
    static_assert(std::is_assignable<U1, U2&&>::value, "");
  }
  { // Test that different non-reference deleter types are disallowed when
    // they cannot convert.
    using U1 = gpu::unique_ptr<VT, GenericDeleter<0> >;
    using U2 = gpu::unique_ptr<VT, GenericDeleter<1> >;
    static_assert(!std::is_assignable<U1, U2&&>::value, "");
  }
  { // Test that if the deleter assignment is not valid the assignment operator
    // SFINAEs.
    using U1 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> const& >;
    using U2 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> >;
    using U3 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> &>;
    using U4 = gpu::unique_ptr<VT, GenericConvertingDeleter<1> >;
    using U5 = gpu::unique_ptr<VT, GenericConvertingDeleter<1> const&>;
    static_assert(!std::is_assignable<U1, U2&&>::value, "");
    ...
    using U1C = gpu::unique_ptr<const VT, GenericConvertingDeleter<0> const&>;
  ...
  { // Test that if the deleter assignment is not valid the assignment operator
    ...
    using U1C = gpu::unique_ptr<const VT, GenericConvertingDeleter<0> &>;
    static_assert(std::is_nothrow_assignable<U1C, U1&&>::value, "");
  }
  { // Test that non-reference destination deleters can be assigned
    ...
    using U1 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> >;
    using U2 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> &>;
    using U3 = gpu::unique_ptr<VT, GenericConvertingDeleter<0> const &>;
    using U4 = gpu::unique_ptr<VT, GenericConvertingDeleter<1> >;
    using U5 = gpu::unique_ptr<VT, GenericConvertingDeleter<1> &>;
    using U6 = gpu::unique_ptr<VT, GenericConvertingDeleter<1> const&>;
    static_assert(std::is_assignable<U1, U2&&>::value, "");
    static_assert(std::is_assignable<U1, U3&&>::value, "");
    static_assert(std::is_assignable<U1, U4&&>::value, "");
    static_assert(std::is_assignable<U1, U5&&>::value, "");
...
__device__ TEST_CONSTEXPR_CXX23 void test_noexcept() {
  typedef typename std::conditional<IsArray, A[], A>::type VT;
  {
    typedef gpu::unique_ptr<const VT> APtr;
    typedef gpu::unique_ptr<VT> BPtr;
    static_assert(std::is_nothrow_assignable<APtr, BPtr>::value, "");
  }
  {
    typedef gpu::unique_ptr<const VT, CDeleter<const VT> > APtr;
    typedef gpu::unique_ptr<VT, CDeleter<VT> > BPtr;
    static_assert(std::is_nothrow_assignable<APtr, BPtr>::value, "");
  }
  {
    typedef gpu::unique_ptr<const VT, NCDeleter<const VT>&> APtr;
    typedef gpu::unique_ptr<VT, NCDeleter<const VT>&> BPtr;
    static_assert(std::is_nothrow_assignable<APtr, BPtr>::value, "");
  }
  {
    typedef gpu::unique_ptr<const VT, const NCConstDeleter<const VT>&> APtr;
    typedef gpu::unique_ptr<VT, const NCConstDeleter<const VT>&> BPtr;
    static_assert(std::is_nothrow_assignable<APtr, BPtr>::value, "");
  }
}

template <bool IsArray>
__device__ void test_deleter_value_category() {
  typedef typename std::conditional<IsArray, A[], A>::type VT;
  ...
  { // Test non-reference deleter conversions
    using U1 = gpu::unique_ptr<VT, TD1 >;
    using U2 = gpu::unique_ptr<VT, TD2 >;
    U1 u1;
    U2 u2;
  ...
  { // Test assignment to const&.
    using U1 = gpu::unique_ptr<VT, CD1 const& >;
    using U2 = gpu::unique_ptr<VT, CD2 >;
    U1 u1(nullptr, cd1);
    U2 u2;
    u1.get_deleter().reset();
    u1 = std::move(u2);
  ...
  { // Test assignment from const ref
    using U1 = gpu::unique_ptr<VT, TD1 >;
    using U2 = gpu::unique_ptr<VT, TD2 const& >;
    U1 u1;
    U2 u2(nullptr, d2);
    u1.get_deleter().reset();
    u1 = std::move(u2);
  ...
  { // Test assignment from const ref
    using U1 = gpu::unique_ptr<VT, CD1 const& >;
    using U2 = gpu::unique_ptr<VT, CD2 const& >;
    U1 u1(nullptr, cd1);
    U2 u2(nullptr, cd2);
    u1.get_deleter().reset();
    u1 = std::move(u2);
