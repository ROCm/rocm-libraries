...
__device__ TEST_CONSTEXPR_CXX23 void checkReferenceDeleter(LHS& lhs, RHS& rhs) {
...
template <class LHS, class RHS>
__device__ TEST_CONSTEXPR_CXX23 void checkDeleter(LHS& lhs, RHS& rhs, int LHSVal, int RHSVal) {
  assert(lhs.get_deleter().state() == LHSVal);
  assert(rhs.get_deleter().state() == RHSVal);
}

template <class LHS, class RHS>
__device__ TEST_CONSTEXPR_CXX23 void checkCtor(LHS& lhs, RHS& rhs, A* RHSVal) {
...
__device__ TEST_CONSTEXPR_CXX23 void checkNoneAlive() {
  if (!TEST_IS_CONSTANT_EVALUATED) {
    assert(A::count == 0);
    assert(B::count == 0);
  }
...
struct NCConvertingDeleter {
  __device__ TEST_CONSTEXPR_CXX23 NCConvertingDeleter()                      = default;
  __device__ NCConvertingDeleter(NCConvertingDeleter const&) = delete;
  __device__ TEST_CONSTEXPR_CXX23 NCConvertingDeleter(NCConvertingDeleter&&) = default;

  template <class U>
  __device__ TEST_CONSTEXPR_CXX23 NCConvertingDeleter(NCConvertingDeleter<U>&&) {}
...
struct NCGenericDeleter {
  __device__ TEST_CONSTEXPR_CXX23 NCGenericDeleter()                   = default;
  __device__ NCGenericDeleter(NCGenericDeleter const&) = delete;
  __device__ TEST_CONSTEXPR_CXX23 NCGenericDeleter(NCGenericDeleter&&) = default;
...
__device__ TEST_CONSTEXPR_CXX23 void test_noexcept() {
  {
    typedef gpu::unique_ptr<A> APtr;
    typedef gpu::unique_ptr<B> BPtr;
    static_assert(std::is_nothrow_constructible<APtr, BPtr>::value, "");
  }
  {
    typedef gpu::unique_ptr<A, Deleter<A> > APtr;
    typedef gpu::unique_ptr<B, Deleter<B> > BPtr;
    static_assert(std::is_nothrow_constructible<APtr, BPtr>::value, "");
  }
  {
    typedef gpu::unique_ptr<A, NCDeleter<A>&> APtr;
    typedef gpu::unique_ptr<B, NCDeleter<A>&> BPtr;
    static_assert(std::is_nothrow_constructible<APtr, BPtr>::value, "");
  }
  {
    typedef gpu::unique_ptr<A, const NCConstDeleter<A>&> APtr;
    typedef gpu::unique_ptr<B, const NCConstDeleter<A>&> BPtr;
    static_assert(std::is_nothrow_constructible<APtr, BPtr>::value, "");
  }
}
...
__device__ TEST_CONSTEXPR_CXX23 bool test() {
  ...
  {
    typedef gpu::unique_ptr<A, CDeleter<A> > APtr;
    typedef gpu::unique_ptr<B, CDeleter<B>&> BPtr;
    CDeleter<B> del(5);
    {
      BPtr b(new B, del);
      A* p = b.get();
