...
TEST_CONSTEXPR_CXX23 void test_sfinae() {
  { // Disallow copying
    using U1 = gpu::unique_ptr<A_h[], GenericConvertingDeleter<0> >;
    using U2 = gpu::unique_ptr<A_h[], GenericConvertingDeleter<1> >;
  ...
  { // Disallow illegal qualified conversions
    using U1 = gpu::unique_ptr_h<const A_h[]>;
    using U2 = gpu::unique_ptr_h<A_h[]>;
    static_assert(std::is_constructible<U1, U2&&>::value, "");
  ...
  { // Disallow base-to-derived conversions.
    using UA = gpu::unique_ptr<A_h[], GenericConvertingDeleter<0> >;
    using UB = gpu::unique_ptr<B_h[], GenericConvertingDeleter<1> >;
    static_assert(!std::is_constructible<UA, UB&&>::value, "");
  }
  { // Disallow invalid deleter initialization
    using U1 = gpu::unique_ptr<A_h[], GenericDeleter<0> >;
    using U2 = gpu::unique_ptr<A_h[], GenericDeleter<1> >;
    static_assert(!std::is_constructible<U1, U2&&>::value, "");
  }
  { // Disallow reference deleters with different qualifiers
    using U1 = gpu::unique_ptr<A_h[], Deleter<A_h[]>&>;
    using U2 = gpu::unique_ptr<A_h[], const Deleter<A_h[]>&>;
    static_assert(!std::is_constructible<U1, U2&&>::value, "");
    static_assert(!std::is_constructible<U2, U1&&>::value, "");
  }
  {
    using U1 = gpu::unique_ptr_h<A_h[]>;
    using U2 = gpu::unique_ptr_h<A_h>;
    static_assert(!std::is_constructible<U1, U2&&>::value, "");
    static_assert(!std::is_constructible<U2, U1&&>::value, "");
  }
