...
#
# Usage:
#   cd <repo-root>/dnn-providers/integration-tests/hipdnn_reference_data/SdpaFwd
...
# --- BF16 hd128, no mask, batch mode ---

OUTDIR="$SCRIPT_DIR/bf16/hd128_nomask_batch"
mkdir -p "$OUTDIR"

echo "=== Generating BF16 hd128 nomask batch bundles ==="

python3 "$GENERATOR" \
    --base-filename "$OUTDIR/Small" \
    --q-dims 2 4 256 128 --v-dims 2 4 256 128 \
    --seed 42

python3 "$GENERATOR" \
    --base-filename "$OUTDIR/Medium" \
    --q-dims 2 4 512 128 --v-dims 2 4 512 128 \
    --seed 42

python3 "$GENERATOR" \
    --base-filename "$OUTDIR/Gqa" \
