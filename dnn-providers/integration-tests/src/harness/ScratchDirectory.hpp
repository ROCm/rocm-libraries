...
///
/// ScopedDirectory refuses to adopt an existing directory, so a fixed name fails against any
/// leftover or concurrent copy of the suite. The counter walks past whatever is there, and
/// create_directory is atomic, so testing a name and taking it are one step. Retry rather
/// than clear: under `ctest -j` the taken name may belong to a live sibling process, and
/// removing it would delete that run's fixture mid-test.
///
/// `label` names the calling suite, so a directory left behind by a crash says which binary
/// and which suite made it.
///
    ...
    claimScratchDirectoryUnder(const std::filesystem::path& base, const std::string& label)
{
    ...
    prefix << "hipdnn_it_" << label << '_' << std::hex << s_session << '_';
