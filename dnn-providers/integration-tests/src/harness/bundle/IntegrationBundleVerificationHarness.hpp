...
public:
    ...
    // SupportObservationLog, read off the ranked list openGraph() already asked
    // for. Runs nothing and issues no verdict. A graph that would not open, or a
    // query that did not resolve, records UNKNOWN rather than nothing, so "we
    // asked and it broke" stays distinguishable from "nobody asked".
    //
    // Takes the session rather than opening its own graph: a second from_binary
    // would put the frontend Graph and a hipdnnHandle_t back into this
    // translation unit, which the unit-test binary links (see
    // tests/CMakeLists.txt) precisely because they are not here.
