...
void IntegrationBundleVerificationHarness::recordSupportObservations(const GraphSession& session)
{
    ...
    // The graph never opened, so there is no ranked list to read. Recorded as
    // UNKNOWN rather than skipped: "we asked and it broke" is a different fact from
    // "nobody asked", and a harvest that drops the first one publishes a matrix with
    // holes it cannot explain.
    if(!session.buildError.empty())
    {
...
{
    // Both harvest hooks sit above the buildError guard on purpose: a graph that
    // would not open is itself an observation (UNKNOWN), and returning early on it
    // would drop exactly the cells a harvest most needs to explain.
    if(TestConfig::get().writeSupportClaims())
    {
        recordSupportObservations(session);

        // SKIPPED, not PASSED: nothing ran, so the depth guard in TestBody() would
        // fire on every bundle. Not unverifiable() either — this bundle is fine, we
        // simply did not verify it, and the unverifiable report is for bundles that
        // cannot be verified at all.
        return VerificationOutcome::skipped(VerificationDepth::NOT_REACHED,
                                            "--write-support-claims: observed, not verified");
    }
