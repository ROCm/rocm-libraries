...
# specific plugin, and ``add_external_integration_test_target_all_engines()``
# for running against every engine the plugin exposes in one invocation.
#
...
#   Create a custom target that runs integration tests against every engine
#   exposed by a plugin, sequentially, in one process invocation::
#
...
#     )
#
#   Unlike ``add_external_integration_test_target()``, this helper does not
#   take an ``ENGINE_NAME``: the test binary enumerates the plugin's engines
#   at runtime and loops over them sequentially in a single ``--test-article``
#   invocation. It registers exactly one CTest entry (no
#   ``TEST_CATEGORIES_YAML`` category-suite fan-out is supported).
#
#   ``TARGET_NAME``
#     Name of the custom target to create.
#
...
#
#   ``TEST_CONFIG_DIR``
#     Optional directory of per-engine TOML configuration files (one named
#     ``<EngineName>.toml`` per engine). Passed via ``--test-config-dir`` to
#     the test binary, which resolves each engine's own file at runtime.
#
#   ``INSTALL_SUBDIR``
#     Optional. When provided, also stage an install-tree ``add_test()``
#     entry so this test appears in the installed ``CTestTestfile.cmake``
#     produced by ``install_provider_ctest_files(<subdir>)``. The value
#     must match the subdir passed to that helper. ``TEST_CONFIG_DIR`` (if
#     any) is installed alongside the CTestTestfile so it resolves relative
#     to ctest's working directory.
#
#   ``GTEST_FILTER``
#     Optional list of Google Test filter expressions. Each entry is joined
#     with ``:`` to form the final filter string passed via ``--gtest_filter``.
...
# Builds the build-tree command for an all-engines external integration test.
#
# Arguments:
#   out_var - Variable to receive the command list
# ~~~
macro(_build_external_integration_all_engines_command out_var)
    set(${out_var}
        $<TARGET_FILE:hipdnn_integration_tests>
        --test-article $<TARGET_FILE:${ARG_PLUGIN_TARGET}>
    )
    if(ARG_TEST_CONFIG_DIR)
        list(APPEND ${out_var} "--test-config-dir" "${ARG_TEST_CONFIG_DIR}")
    endif()
    if(ARG_GTEST_FILTER)
        list(JOIN ARG_GTEST_FILTER ":" _GTEST_FILTER_STR)
        list(APPEND ${out_var} "--gtest_filter=${_GTEST_FILTER_STR}")
...
# Stages the install-tree CTest entry for an all-engines external integration
# test.
#
# Uses the ARG_* variables parsed by
# add_external_integration_test_target_all_engines().
# ~~~
macro(_stage_external_integration_all_engines_install_test)
    set(_install_bin "")
    set(_install_plugin "")
    set(_install_config_dir "")
    if(ARG_INSTALL_SUBDIR)
        if(ARG_TEST_CONFIG_DIR)
            get_filename_component(_install_config_dir "${ARG_TEST_CONFIG_DIR}" NAME)
            install(DIRECTORY "${ARG_TEST_CONFIG_DIR}/"
                DESTINATION "${CMAKE_INSTALL_BINDIR}/${ARG_INSTALL_SUBDIR}/${_install_config_dir}"
            )
        endif()
        ...
        if(ARG_TEST_CONFIG_DIR)
            string(APPEND _install_cmd " \"--test-config-dir\" \"${_install_config_dir}\"")
        endif()
        if(ARG_GTEST_FILTER)
            string(APPEND _install_cmd " \"--gtest_filter=${_GTEST_FILTER_STR}\"")
        endif()
...
# Adds a custom target and CTest entry that runs integration tests against
# every engine exposed by a plugin, sequentially, in one process invocation.
# ~~~
function(add_external_integration_test_target_all_engines)
    cmake_parse_arguments(
        ARG
        ""
        "TARGET_NAME;PLUGIN_TARGET;INSTALL_SUBDIR;TEST_CONFIG_DIR"
        "GTEST_FILTER;ENVIRONMENT;ENVIRONMENT_MODIFICATION;FIXTURES_REQUIRED"
        ${ARGN}
    )
    ...
    _build_external_integration_all_engines_command(_CMD)

    add_custom_target(${ARG_TARGET_NAME}
        COMMAND ${_CMD}
        DEPENDS ${ARG_PLUGIN_TARGET} hipdnn_integration_tests
        COMMENT "Running integration tests for all engines of ${ARG_PLUGIN_TARGET}"
        USES_TERMINAL
        VERBATIM
    )

    set(_LABELS "integration_test;slow;external_integration_test;all_engines")
    add_test(NAME ${ARG_TARGET_NAME} COMMAND ${_CMD})
    set_tests_properties(${ARG_TARGET_NAME} PROPERTIES LABELS "${_LABELS}")
    if(ARG_ENVIRONMENT)
        set_tests_properties(${ARG_TARGET_NAME} PROPERTIES ENVIRONMENT "${ARG_ENVIRONMENT}")
    ...
    _stage_external_integration_all_engines_install_test()
endfunction()
