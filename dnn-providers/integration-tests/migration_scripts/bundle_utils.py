...
def _fmt_range(spec: dict) -> str:
    """Render one tensor's FillSpec compactly for the legend."""
...
def render_cases_md(topology_name: str, sweep_cases: list) -> str:
    ...
    for e in sweep_cases:
        ...
        if inputs:
            inp = " ".join(
                f"{uid}:{_fmt_range(spec)}" for uid, spec in sorted(inputs.items())
            )
        else:
            inp = "-"
