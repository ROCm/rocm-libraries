...
**Inputs:** AITER source files (`mha_bwd.h`, `mha_bwd.cu`) for the actual GPU kernel ABI structs. CK reference (`fmha_bwd.hpp`) provides semantic field reference only (single unified `fmha_bwd_args` struct for CPU-side launching — not the binary layout).
**Depends on:** Nothing.
...
- `AsmSdpaBwdPlanBuilder` added to the engine via `engine->addPlanBuilder()`; engine registered via `engineManager.addEngine()`
...
- Naive SDPA backward: recompute `S = softmax(Q * K^T * scale)`, compute `D = rowsum(dO * O)` (numerically preferred form, matches GPU kernel), then:
  - `dV = S^T * dO`
  - `dS = dO * V^T`
  - `dP = S * (dS - D)` (softmax backward)
  - `dQ = dP * K * scale`
  - `dK = dP^T * Q * scale`
...
**Requirements:**
- Forward pass first (CPU reference) to produce O and Stats (LSE) tensors. Note: `CpuFpReferenceSdpa::forward()` outputs O but does **not** output Stats (LSE). Test setup must either: (a) augment the CPU forward to optionally return LSE, or (b) provide a standalone LSE computation utility.
