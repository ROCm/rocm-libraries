...
// Batched matmul: A[B,M,K] x B[B,K,N] = C[B,M,N]
constexpr int64_t B = 2;
constexpr int64_t M = 4;
constexpr int64_t K = 8;
constexpr int64_t N = 5;

// Bias modes for batched matmul with output shape {B, M, N}.
enum class BiasMode {
  NoBias,
  BiasFullBroadcast, // Bias shape {1, 1, N} - broadcasts across B batches and M
                     // rows
  BiasRowBroadcast,  // Bias shape {B, 1, N} - different per batch, broadcasts
                     // across M rows
  BiasNoBroadcast    // Bias shape {B, M, N} - no broadcasting
};

struct BatchedMatmulTestCase {
  ...
  // {B, K, N} (row-major), but stride {N*K, 1, K} (column major).
  bool transposeB;

  BiasMode biasMode;

  friend std::ostream &operator<<(std::ostream &os,
                                  const BatchedMatmulTestCase &tc) {
    os << "B" << B << "M" << M << "K" << K << "N" << N;
    if (tc.transposeB) {
      os << "_TransposeB";
    }
    ...
    case BiasMode::BiasFullBroadcast:
      os << "_BiasFullBroadcast";
      break;
    case BiasMode::BiasRowBroadcast:
      os << "_BiasRowBroadcast";
      break;
...
class BatchedMatmulParameterizedTest
    : public ::testing::TestWithParam<BatchedMatmulTestCase> {};

TEST_P(BatchedMatmulParameterizedTest, Correctness) {
  ...
  // Initialize tensors.
  PinnedTensor<float> aTensor({B, M, K});
  // B tensor - logical dims are {B, K, N}, stride encodes the physical layout
  //  {K*N, N, 1} - (row major) when in standard layout
  //  {K*N, 1, K} - (column major) when transposed.
  PinnedTensor<float> bTensor(/*dims=*/{B, K, N},
                              /*strides=*/tc.transposeB
                                  ? std::vector<int64_t>{K * N, 1, K}
                                  : std::vector<int64_t>{K * N, N, 1});
  PinnedTensor<float> outTensor({B, M, N});
  ...
  // present.
  PinnedTensor<float> expectedOutput({B, M, N});
  ...
  matmulAttr.set_name("batched_matmul");
...
INSTANTIATE_TEST_SUITE_P(BatchedMatmul, BatchedMatmulParameterizedTest,
                         ::testing::ValuesIn(getBatchedMatmulTestCases()));
