...
TEST_F(TestGpuConvWrwPlan, CreatesPlanWithValidGraph)
{
    ...
    ConvWrwPlan(_handle, std::move(params), executionContext);
}
...
TEST_F(TestGpuConvWrwPlan, CreatesPlanWithWorkspaceLimit)
{
...
TEST_F(TestGpuConvWrwPlan, CreatesPlanWithZeroWorkspaceLimit)
{
    auto builder = hipdnn_test_sdk::utilities::createValidConvWrwGraph();
    hipdnn_plugin_sdk::GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());
    const auto& node = graph.getNode(0);
    ...
    auto plan = ConvWrwPlan(_handle, std::move(params), executionContext);
    EXPECT_EQ(plan.getWorkspaceSize(_handle), 0);
}

TEST_F(TestGpuConvWrwPlan, WorkspaceSizeRespectsLimit)
{
