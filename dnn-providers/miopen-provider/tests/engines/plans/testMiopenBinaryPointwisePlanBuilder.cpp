...
TEST_F(TestMiopenBinaryPointwisePlanBuilder, IsApplicableReturnsTrueForValidMinGraph)
{
    auto builder = createPointwiseGraph(hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MIN);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());

    EXPECT_TRUE(_planBuilder.isApplicable(*_dummyHandle, graph));
}

TEST_F(TestMiopenBinaryPointwisePlanBuilder, BuildPlanDoesNotThrowForValidMinGraph)
{
    auto builder = createPointwiseGraph(hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MIN);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());
...
TEST_F(TestMiopenBinaryPointwisePlanBuilder, IsApplicableReturnsTrueForMinHalfIoDtype)
{
    auto builder
        = createPointwiseGraphWithIoDtype(hipdnn_flatbuffers_sdk::data_objects::DataType::HALF,
                                          hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MIN);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());

    EXPECT_TRUE(_planBuilder.isApplicable(*_dummyHandle, graph));
}

TEST_F(TestMiopenBinaryPointwisePlanBuilder, IsApplicableReturnsTrueForMinRank1Tensor)
{
    const std::vector<int64_t> dims = {16};
    const std::vector<int64_t> strides = {1};
    auto builder = createPointwiseGraphWithDims(
        dims, strides, dims, strides, hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MIN);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());
...
TEST_F(TestMiopenBinaryPointwisePlanBuilder, IsApplicableReturnsTrueForValidMaxGraph)
{
    auto builder = createPointwiseGraph(hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MAX);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());

    EXPECT_TRUE(_planBuilder.isApplicable(*_dummyHandle, graph));
}

TEST_F(TestMiopenBinaryPointwisePlanBuilder, BuildPlanDoesNotThrowForValidMaxGraph)
{
    auto builder = createPointwiseGraph(hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MAX);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());
...
TEST_F(TestMiopenBinaryPointwisePlanBuilder, IsApplicableReturnsTrueForMaxHalfIoDtype)
{
    auto builder
        = createPointwiseGraphWithIoDtype(hipdnn_flatbuffers_sdk::data_objects::DataType::HALF,
                                          hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MAX);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());

    EXPECT_TRUE(_planBuilder.isApplicable(*_dummyHandle, graph));
}

TEST_F(TestMiopenBinaryPointwisePlanBuilder, IsApplicableReturnsTrueForMaxRank1Tensor)
{
    const std::vector<int64_t> dims = {16};
    const std::vector<int64_t> strides = {1};
    auto builder = createPointwiseGraphWithDims(
        dims, strides, dims, strides, hipdnn_flatbuffers_sdk::data_objects::PointwiseMode::MAX);
    const GraphWrapper graph(builder.GetBufferPointer(), builder.GetSize());
