...
SdpaSpec SdpaAdapter::buildSpec(const SdpaAttributes& sdpaAttr, const TensorMap& tensorMap) {
    ...
    if (sdpaAttr.stats_tensor_uid().has_value()) {
        throwBadParam("stats/LSE output not supported in M1 forward");
    }
    if (sdpaAttr.generate_stats().has_value() && sdpaAttr.generate_stats().value()) {
        throwBadParam("generate_stats (LSE output) not supported in M1 forward");
    }
