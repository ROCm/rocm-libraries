...
/// ``LaunchAbi::pack`` against the module's argSchema. The kernel's
/// 15-slot ABI is validated in the ctor:
...
class SdpaFwdPlan : public hipdnn_plugin_sdk::IPlan<::CkDslHandle> {
   public:
    ...
    /// Resolve Q/K/V/O device pointers from ``deviceBuffers`` by uid,
    /// pack the 15-slot argument buffer via ``LaunchAbi::pack``, and
