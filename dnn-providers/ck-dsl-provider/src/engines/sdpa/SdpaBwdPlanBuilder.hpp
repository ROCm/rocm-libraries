...
/// IPlanBuilder for the FMHA-backward kernel (and its LSE-prep kernel).
///
...
/// that node is an ``SdpaBackwardAttributes`` node, the adapter would
/// build a spec for it (matching dtype, shape constraints, supported
/// mask), and the DSL accepts the spec on the detected device arch.
/// Throws are caught + downgraded to ``false`` so the SDK can fall
/// through to other engines without surfacing an exception.
///
/// **buildPlan**: derives two cache keys via ``GraphSignature`` (one for
/// the bwd kernel, one for the LSE-prep kernel), calls
/// ``JitCache::getOrLoad`` for each with a loader that runs the
/// per-kernel payload + ``CompileServiceBridge::compile``, wraps the two
/// resulting ``HipModule`` instances in an ``SdpaBwdPlan``, and stores it
/// on the execution context via ``setPlan``.
class SdpaBwdPlanBuilder
    : public hipdnn_plugin_sdk::IPlanBuilder<::CkDslHandle, CkDslSettings, CkDslContext> {
   public:
    SdpaBwdPlanBuilder(CompileServiceBridge& bridge, JitCache& cache);
    ...
    /// Stable op-kind string for the backward kernel: the cache-key
    /// partition and the dispatch tag the Python compile_service sees.
    static constexpr const char* opKind() {
        return "sdpa_fmha_bwd";
    }

    /// Stable op-kind string for the LSE-prep kernel that precedes the
    /// backward launch.
    static constexpr const char* prepOpKind() {
        return "sdpa_lse_prep";
    }
