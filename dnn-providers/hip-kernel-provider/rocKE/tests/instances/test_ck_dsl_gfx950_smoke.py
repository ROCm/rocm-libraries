...
"""gfx950 GPU smoke tests for ck_dsl performance-sensitive paths.
...
class TestCkDslGfx950Smoke(unittest.TestCase):
    ...
    def tearDownClass(cls):
        if not cls.current_perf:
            return
        report_path = Path(os.environ.get("CK_DSL_GFX950_PERF_REPORT", _DEFAULT_REPORT))
        report_path.parent.mkdir(parents=True, exist_ok=True)
        payload = {
            "schema": "ck.dsl.gfx950_smoke_perf.current/v1",
            "baseline": str(
                Path(os.environ.get("CK_DSL_GFX950_PERF_BASELINE", _DEFAULT_BASELINE))
            ),
    ...
    def test_attention_decode_smoke(self):
        with tempfile.TemporaryDirectory(prefix="ckdsl_attn_gfx950_") as tmp:
            report = Path(tmp) / "attention.json"
            out = self._run(
                "-m",
                "ck_dsl.examples.gfx950.attention.parity_unified_attention",
                "--scenario",
                "decode_d128_b16",
                "--attempts",
                "2",
    ...
    def test_gemm_sweep_smoke(self):
        with tempfile.TemporaryDirectory(prefix="ckdsl_gemm_gfx950_") as tmp:
            report = Path(tmp) / "gemm.json"
            out = self._run(
                "-m",
                "ck_dsl.benchmark.gemm.fp16_rcr_sweep",
                "--arch",
                "gfx950",
                "--shape",
    ...
    def test_fused_moe_smoke(self):
        with tempfile.TemporaryDirectory(prefix="ckdsl_moe_gfx950_") as tmp:
            report = Path(tmp) / "moe.json"
            out = self._run(
                "-m",
                "ck_dsl.examples.gfx950.moe.fused_moe_e2e_perf",
                "--scenario",
        ...
        self.assertNotIn("FAIL", out)
        ck_dsl = rows[0]["results"]["ck_dsl"]
        self._record_and_compare(
            "fused_moe_small_T32_E4_K2_H128_I256",
            {
                "scenario": "small_T32_E4_K2_H128_I256",
                "ck_dsl_ms": float(ck_dsl["ms"]),
                "max_abs": float(ck_dsl["max_abs"]),
                "rel_max": float(ck_dsl["rel_max"]),
            },
        )
