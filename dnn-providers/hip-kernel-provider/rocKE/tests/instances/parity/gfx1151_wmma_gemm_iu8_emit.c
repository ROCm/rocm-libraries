 ...
 *
 * tests/parity/gfx1151_wmma_gemm_iu8_emit.c -- C-side emitter for the gfx1151
 * (RDNA3.5) native-integer WMMA GEMM (iu8) parity harness. Selects one of 6
 * sampled configs by argv[1] (0..5), builds it exactly as the Python emitter
 * gfx1151_wmma_gemm_iu8_emit.py does, and lowers to LLVM .ll text at
 ...
 *   (2) ckc_build_wmma_gemm_iu8(b, &spec, "gfx1151")  -> KernelDef
 ...
 * All 6 configs share the same (default) spec; M/N/K are runtime params. */
static int make_spec(int idx, ckc_wmma_gemm_iu8_spec_t *spec) {
    if (idx < 0 || idx > 5)
        return -1;
    *spec = ckc_wmma_gemm_iu8_spec_default();
    spec->name = "ck_dsl_wmma_gemm_iu8";
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    /* (2) build */
    ckc_kernel_def_t *kernel = ckc_build_wmma_gemm_iu8(&b, &spec, "gfx1151");
    if (kernel == NULL) {
