...
Unlike ``test_ck_dsl.py`` (build/IR/structure smoke tests) and
``test_ck_dsl_multiarch.py`` (pure-Python arch metadata), this module actually
...
breach):
  - ck_dsl.examples.common.ck_tile_parity         : elementwise(×12), reduce(×3),
                                             layernorm, rmsnorm, transpose2d,
                                             batched + grouped GEMM  (BOTH arches)
  - ck_dsl.examples.common.universal_gemm_verify : Universal GEMM vs A@B.T (BOTH)
  - ck_dsl.examples.common.bake_off_implicit_gemm + run_manifest --verify : conv (BOTH)
  - ck_dsl.examples.common.parity_extended_kernels: fmha/attention/sage/sparse/streamk/
...
def _detect_gpu_arch():
    """(has_gpu, gfx) via torch — imported FIRST so ck_dsl binds torch's HIP.

    Using torch (already required by the parity harnesses) avoids a HIP
    double-init: if ck_dsl's HIP is loaded before torch, torch.cuda can fail
...
class TestNumericVerification(unittest.TestCase):
    ...
    def test_core_parity(self):
        ...
        rc, out = self._run(
            "-m", "ck_dsl.examples.common.ck_tile_parity", "--arch", ARCH
        )
        self.assertEqual(
    ...
    def test_gemm_numeric(self):
        ...
        rc, out = self._run(
            "-m",
            "ck_dsl.examples.common.universal_gemm_verify",
            "--arch",
            ARCH,
            "--m",
            "512",
    ...
    def test_conv_numeric(self):
        ...
        d = tempfile.mkdtemp(prefix=f"ckdsl_conv_{ARCH}_")
        rc, out = self._run(
            "-m",
            "ck_dsl.examples.common.bake_off_implicit_gemm",
            "--arch",
            ARCH,
            "--output-dir",
            d,
    ...
    def test_extended_parity(self):
        ...
        rc, out = self._run(
            "-m",
            "ck_dsl.examples.common.parity_extended_kernels",
            "--arch",
            ARCH,
            timeout=420,
        )
    ...
    def test_wmma_gemm(self):
        ...
        rc, out = self._run(
            "-m",
            "ck_dsl.examples.gfx1151.wmma_gemm_verify",
            "--m",
            "128",
            "--n",
            "128",
    ...
    def test_rdna_core_parity(self):
        ...
        rc, out = self._run(
            "-m", "ck_dsl.examples.common.ck_tile_parity", "--arch", "gfx1151"
        )
        self.assertEqual(
    ...
    def test_elementwise_hip_numeric(self):
        ...
        rc, out = self._run(
            "-m",
            "ck_dsl.examples.common.elementwise_verify_hip",
            "--arch",
            ARCH,
            timeout=300,
        )
