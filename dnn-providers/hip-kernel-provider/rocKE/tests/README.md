...
> Notes: `instances/ck_dsl_ir_parity_harness.py` is a helper imported by the
> gate, not a `test_`-prefixed pytest module. The pybind **`ckc_engine` module
> has no default-pytest coverage**: the C-engine vs Python-engine equivalence is
> owned by the byte-identity gate (which builds `ckc_core` and byte-compares the
...
1. **Default runner** (`python tests/run_all.py`): relative-path guard -> byte-identity
   ...
   above) -> `ctest` **iff** the registered binaries (`ckc_ir_serialize_roundtrip`,
   `ckc_tiled_attention_2d_reentrancy`) are present in `--build-root`. This is the
...
3. **GPU / manual numeric lanes** (need a HIP device; skipped/not-collected
   otherwise): `differential/numeric.py`, `instances/test_ck_dsl_numeric.py`.
4. **Manual C++ demos / tools** (not built or registered by CMake/CTest --
   compile by hand against `libckc_core.a`): `core/ir_lower_cli.cpp`,
