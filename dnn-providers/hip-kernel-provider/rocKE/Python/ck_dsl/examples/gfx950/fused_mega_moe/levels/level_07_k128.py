...
AMAX_FLOOR = 1e-6
# L5: MI355X has 8 XCDs; the persistent-grid XCD remap groups consecutive
# logical tiles onto the same XCD slice for L2 reuse. 256 CUs total.
XCD_COUNT = 8
PERSISTENT_CTA_DEFAULT = 256
...
class FusedMegaKernelSpecFp8:
    ...
    * MFMA atom = fp8 ``16x16x32`` (e4m3); the 128-wide scale group = 4 atoms.
    ...
    block_size: int = 0
    dtype: str = "fp8e4m3"
    # L5: number of persistent workgroups to launch (capped at total_tiles).
    persistent_cta: int = PERSISTENT_CTA_DEFAULT
...
def moe_fused_mega_fp8_logical_grid(
    num_m_blocks: int, inter: int, spec: FusedMegaKernelSpecFp8
) -> Tuple[int, int]:
    """LOGICAL 2D tile space ``(grid_x, num_m_blocks)``.

    ``grid_x = ceil(inter / tile_n_inter)``. Canonical decode (I=7168,
    tile_n_inter=256): grid_x = 28; num_m_blocks = 8 -> 224 logical tiles.
    """
    sub_gu = spec.tile_n_inter
    gx = (inter + sub_gu - 1) // sub_gu
    return gx, num_m_blocks


def moe_fused_mega_fp8_grid(
    num_m_blocks: int, inter: int, spec: FusedMegaKernelSpecFp8
) -> Tuple[int, int, int]:
    """PERSISTENT 1D launch grid (L5).

    Instead of the natural 2D ``(grid_x, num_m_blocks, 1)`` grid the kernel now
    launches a fixed 1D persistent grid of ``num_cta`` workgroups; each CTA
    strides over the flattened logical tile ids (``total_tiles = grid_x *
    num_m_blocks``) and applies the XCD8 remap internally. ``num_cta`` is capped
    at ``total_tiles`` so we never launch idle workgroups, and (when it does not
    truncate the tile space) snapped down to a multiple of ``XCD_COUNT`` so the
    arithmetic XCD remap stays balanced.
    """
    gx, m_blocks = moe_fused_mega_fp8_logical_grid(num_m_blocks, inter, spec)
    total_tiles = gx * m_blocks
    num_cta = min(spec.persistent_cta, total_tiles)
    if num_cta >= XCD_COUNT and num_cta < total_tiles:
        num_cta = (num_cta // XCD_COUNT) * XCD_COUNT
    return (num_cta, 1, 1)


def moe_fused_mega_fp8_persistent_params(
    num_m_blocks: int, inter: int, spec: FusedMegaKernelSpecFp8
) -> Tuple[int, int, int]:
    """Return ``(num_cta, total_tiles, grid_x)`` for the persistent ABI scalars."""
    gx, m_blocks = moe_fused_mega_fp8_logical_grid(num_m_blocks, inter, spec)
    total_tiles = gx * m_blocks
    num_cta = moe_fused_mega_fp8_grid(num_m_blocks, inter, spec)[0]
    return num_cta, total_tiles, gx
    ...
    * INTERLEAVES the MFMAs across ni in hand-tuned asm wave-pair odd/even order
    ...
    with outer as (kg, outs):
        ...
        for kk in range(atoms_per_group):
            ...
            # Issue the NEXT atom's shared A load up front (in flight while the
            # current atom's MFMAs run).
            if not last:
                a_next = _a_at(kk + 1)
            # Wave-pair interleave: for each ni, emit gate then up, and splice
            # the next-atom B prefetch for ni right before consuming ni's MFMAs.
            for ni in range(nni):
                if not last:
...
def build_moe_fused_mega_gemm_fp8(
    ...
    # L5 persistent-grid ABI scalars: num_cta = launched workgroups, total_tiles
    # = grid_x * num_m_blocks (the LOGICAL 2D tile count), grid_x = inter split.
    num_cta = b.param("num_cta", I32)
    total_tiles = b.param("total_tiles", I32)
    grid_x = b.param("grid_x", I32)
    ...
    # The base param pointers (expert 0) -- the per-tile expert slice is folded
    # INSIDE the persistent loop (L5), since expert_idx now depends on the
    # remapped tile id rather than a static block_id.
    WGate_base = WGate
    WUp_base = WUp
    WDown_base = WDown
    WGateScale_base = WGateScale
    WUpScale_base = WUpScale
    WDownScale_base = WDownScale
    ...
    # ---- L5: persistent 1D grid + XCD8 remap ---------------------------
    # The launch grid is a fixed 1D persistent grid of ``num_cta`` workgroups.
    # Each CTA strides over the flattened LOGICAL tile space
    # ``tg in [pid, total_tiles, num_cta)`` (column-major: tg = mblk*grid_x + nx).
    # When ``total_tiles`` is a multiple of XCD_COUNT (the canonical 224 = 28*8
    # always is) the XCD8 remap ``tg' = (tg%8)*tgs_per_xcd + tg/8`` keeps
    # consecutive same-XCD tiles on the same inter-slice for L2 reuse; otherwise
    # it falls back to identity so the stride loop stays a bijection over tiles.
    c_xcd = b.const_i32(XCD_COUNT)
    tgs_per_xcd = b.div(total_tiles, c_xcd)
    rem = b.mod(total_tiles, c_xcd)
    do_remap = b.cmp_eq(rem, c0)
    pid = b.block_id_x()
    ploop = b.scf_for(pid, total_tiles, num_cta, iv_name="pid")
    with ploop as tg:
        tg_remap = b.add(b.mul(b.mod(tg, c_xcd), tgs_per_xcd), b.div(tg, c_xcd))
        tg_r = b.select(do_remap, tg_remap, tg)
        # Decode flattened tile id -> (m_block_idx, nx). Column-major over
        # grid_x: tg = m_block_idx * grid_x + nx.
        m_block_idx = b.div(tg_r, grid_x)
        nx = b.mod(tg_r, grid_x)
        expert_idx_l = b.global_load_i32(BlockExpertIds, m_block_idx)
        # Empty tail block (BlockExpertIds == -1) skips all work.
        with b.scf_if(b.cmp_ge(expert_idx_l, c0)):
            WGate_l = _b_base(WGate_base, stride_b_gate, expert_idx_l)
            WUp_l = _b_base(WUp_base, stride_b_up, expert_idx_l)
            WDown_l = _b_base(WDown_base, stride_b_down, expert_idx_l)
            WGateScale_l = _scale_base(
                WGateScale_base, stride_gate_scale_e, expert_idx_l
            )
            WUpScale_l = _scale_base(WUpScale_base, stride_up_scale_e, expert_idx_l)
            WDownScale_l = _scale_base(
                WDownScale_base, stride_down_scale_e, expert_idx_l
            )
            block_m_off = b.mul(m_block_idx, c_block_m)
            gu_n_off = b.mul(nx, c_block_n)
            _emit_body(
                expert_idx=expert_idx_l,
                WGate=WGate_l,
                WUp=WUp_l,
                WDown=WDown_l,
                WGateScale=WGateScale_l,
                WUpScale=WUpScale_l,
                WDownScale=WDownScale_l,
                block_m_off=block_m_off,
                gu_n_off=gu_n_off,
            )
        b.scf_yield()
