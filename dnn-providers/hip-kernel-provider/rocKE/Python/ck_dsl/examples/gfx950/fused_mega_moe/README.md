...
addressing contract pinned into assembly — not expressible from the `ck_dsl` ->
...
PYTHONPATH=$(pwd) $VENV -m ck_dsl.examples.gfx950.fused_mega_moe.reproduce_levels --levels 7,10,11
PYTHONPATH=$(pwd) $VENV -m ck_dsl.examples.gfx950.fused_mega_moe.reproduce_levels --no-perf
```
