...
"""Step 16: increase block_size by widening tile_n.

Validator forces block_size = warp_m*warp_n*64. With tile_m/n = 16 and
warp_tile = 16, only warp_m=warp_n=1 is legal. To get more lanes/WG we
must enlarge a tile dim. Strategy: bump tile_n to 32, 64; set warp_n
proportionally. More lanes -> fewer DTLA passes -> more in-flight VMEM.

Cost: fewer CTAs in grid_x = ceil(4096 / tile_n).
  tile_n=16: 256 CTAs, 64 lanes  -> 32 passes per A or B (block_k=1024)
  tile_n=32: 128 CTAs, 128 lanes -> 16 passes per operand   (2 WGs/CU on LDS budget)
  tile_n=64:  64 CTAs, 256 lanes ->  8 passes per operand   (LDS pressure)

At M=2, tile_m=16 over-allocates (only 2 rows used), but tile_n is real
work. Doubling tile_n doubles useful work per CTA -- halving CTA count
isn't a loss if the per-CTA issue rate goes up by 2×.
"""
...
from ck_dsl.instances.common.gemm_universal import (  # noqa: E402
    UniversalGemmSpec,
    TileSpec,
    TraitSpec,
    DataSpec,
...
def build(
    ...
    *,
    ...
    sub = ROOT / "build_more_lanes" / label
...
def main():
    results = []
    # config: (tile_m, tile_n, tile_k, warp_m, warp_n, warp_k)
    configs = [
        # baseline
        (16, 16, 1024, 1, 1, 1),
        # widen tile_n
        (16, 32, 1024, 1, 2, 1),
        (16, 32, 512, 1, 2, 1),  # might fit better with deeper N
        (16, 64, 1024, 1, 4, 1),
        (16, 64, 512, 1, 4, 1),
        # widen tile_m too (irrelevant work but more lanes; M=2 anyway)
        (32, 32, 1024, 2, 2, 1),
        (32, 32, 512, 2, 2, 1),
        # smaller tile_k with bigger N to cut LDS
        (16, 64, 256, 1, 4, 1),
    ]
    for tm, tn, tk, wm, wn, wk in configs:
        for wpe in [None, 4]:
            bs = wm * wn * wk * 64
            label = f"tm{tm}_tn{tn}_tk{tk}_w{wm}x{wn}x{wk}_bs{bs}_wpe{wpe}"
            print(f"\n=== {label} ===")
            try:
                art, mf = build(
                    label,
                ...
                print(f"  COMPILE-FAIL: {str(e)[:150]}")
    ...
    out = ROOT / "data" / "16_more_lanes.json"
    ...
    for r in ok[:8]:
        print(
            f"  {r['label']:<55} {r['ms_best'] * 1000:.2f}µs  ({r['pct_hbm']:.1f}% HBM)  {r['vs_rocblas']:.2f}x"
        )
