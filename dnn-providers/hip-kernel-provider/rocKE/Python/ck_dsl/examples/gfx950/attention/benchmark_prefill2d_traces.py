...
class CkDslComboBench:
    ...
    def _launcher(self, shape, problem, sliding_window: int):
        ...
        from ck_dsl.instances.common.attention_unified import _attn_signature
        from ck_dsl.runtime import KernelLauncher
    ...
    def benchmark(self, shape, data, *, warmup: int, iterations: int, attention_flops):
        from ck_dsl.instances.common.attention_unified import _attn_values
        from ck_dsl.runtime import LaunchConfig, synchronize_and_release, time_launches
        ...
        return {
            "framework": "ckdsl_ua_tiled_2d_combo",
            ...
            joined.append(
                {
                    ...
                    "ckdsl_tflops": ck.get("tflops", ""),
                    "speedup_triton_over_ckdsl": tri_ms / ck_ms,
                    "ckdsl_policy": ck.get("policy", ""),
                    "ckdsl_kernel_name": ck.get("kernel_name", ""),
                    "ckdsl_compile_backend": ck.get("compile_backend", ""),
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("/tmp/ckdsl_prefill2d_trace_combo.json"),
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("/tmp/ckdsl_prefill2d_trace_combo.csv"),
    )
    parser.add_argument(
        "--combined-csv",
        type=Path,
