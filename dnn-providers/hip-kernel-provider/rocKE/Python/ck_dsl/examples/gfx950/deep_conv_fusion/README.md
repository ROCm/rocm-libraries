...
A single-launch **deep-fusion** prototype authored in `ck_dsl` (Python → LLVM IR
...
- A working `ck_dsl` toolchain (comgr / HIP runtime) and `numpy`. The `--verify`
  and `--bench` paths launch on the GPU; building the HSACO alone does not.
- Run from the `ck_dsl` Python root with `PYTHONPATH` set so the package and the
...
`ck_dsl.instances.common.deep_fused_conv_pool` (arch-parametric) and is pinned to
gfx950 by `ck_dsl.instances.gfx950.deep_fused_conv_pool` (wave64, `32×32×16`
MFMA, kernel name `ck_dsl_gfx950_deep_fused_conv_pool`).
...
```bash
PYTHONPATH=$(pwd) $VENV -m ck_dsl.examples.gfx950.deep_conv_fusion.deep_fused_conv_pool_verify --verify
```
...
```bash
PYTHONPATH=$(pwd) $VENV -m ck_dsl.examples.gfx950.deep_conv_fusion.deep_fused_conv_pool_verify \
...
```bash
PYTHONPATH=$(pwd) $VENV -m ck_dsl.examples.gfx950.deep_conv_fusion.compare_pool_tile_configs
```
...
```bash
PYTHONPATH=$(pwd) $VENV -m ck_dsl.examples.gfx950.deep_conv_fusion.profile_best_config
# under a profiler, e.g.:
# rocprofv3 ... -- $VENV -m ck_dsl.examples.gfx950.deep_conv_fusion.profile_best_config
```
...
- **gfx950 only.** Both harnesses reject any non-gfx950 arch
  ...
  the historical kernel name `ck_dsl_gfx950_deep_fused_conv_pool`.
- **Arch-parametric body.** The kernel source is shared in
  `ck_dsl.instances.common.deep_fused_conv_pool` and is driven by the resolved MMA
