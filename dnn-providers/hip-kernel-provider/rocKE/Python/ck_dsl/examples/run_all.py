...
#
# run_all.py -- example-repeatability driver for the ck_dsl examples tree.
#
...
#   2. Its inputs must be seeded (the ck_dsl examples use np.random
...
#   python3 -m ck_dsl.examples.run_all --bless
#   # assert against goldens (CI / repeat):
#   python3 -m ck_dsl.examples.run_all --check
...
PYROOT = HERE.parent.parent  # python/ (holds ck_dsl)
...
# Each is a real ck_dsl example with a deterministic, seeded verify path.
REGISTRY: List[Example] = [
    # elementwise (HIP path, numpy-seeded add) -- fast, no torch dependency
    Example(
        name="elementwise_add_hip",
        module="ck_dsl.examples.common.elementwise_verify_hip",
    ...
    Example(
        ...
        module="ck_dsl.examples.common.universal_gemm_verify_hip",
    ...
    Example(
        ...
        module="ck_dsl.examples.common.fmha_fwd_verify_hip",
        argv=(
            "--arch",
            "gfx950",
            "--seqlen-q",
    ...
    Example(
        ...
        module="ck_dsl.examples.common.distribution_reduce_demo",
...
def run_one(ex: Example, *, bless: bool, live_arch: Optional[str]) -> RunResult:
    ...
    if proc.returncode != 0:
        # rc==2 in the ck_dsl examples means "validate rejected (unsupported
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print(f"ck_dsl examples {mode}  live_arch={live}  registry={len(REGISTRY)}")
