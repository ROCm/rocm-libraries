...
"""Driver for the BLOCK_N-widened WMMA FMHA kernel (``fmha_blockn``)."""
...
def verify_and_time(
    ...
    *,
    ...
    art = compile_kernel(build_wmma_fmha_blockn(cfg, arch=arch), arch=arch)
    ...
    grid = blockn_grid(cfg, seqlen_q=Sq, batch=B)
...
def _fmt(r):
    c = r["cfg"]
    return (
        f"bn{c.bn_tiles} | {'Y' if r['ok'] else 'N'} {r['max_abs']:.2e} "
...
def main():
    ...
    ap.add_argument("--bn", type=int, nargs="+", default=[1, 2, 4])
    args = ap.parse_args()
    ...
    for bn in args.bn:
        ...
        try:
            ...
            print(f"bn{bn}: BUILD/RUN FAIL: {e}")
            continue
        print(_fmt(r))
