...
CK Tile helper layer: 3D :func:`~ck_dsl.helpers.make_global_view` views drive
both the per-wave Q fragment (via :func:`~ck_dsl.helpers.load_wmma_fragment`) and
the cooperative K/V global reads; the shared K, transposed V, and per-wave P all
live in :func:`~ck_dsl.helpers.make_lds_view` LDS views; the matmuls are
:class:`~ck_dsl.helpers.WmmaAtom`; and the O epilogue uses
:func:`~ck_dsl.helpers.store_wmma_acc`.
"""
