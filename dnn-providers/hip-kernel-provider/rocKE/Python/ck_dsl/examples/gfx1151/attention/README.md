...
(`ck_dsl.instances.gfx1151.wmma_fmha_fwd`):
...
```bash
PYTHONPATH=python python3 -m ck_dsl.examples.gfx1151.attention.bench_v_staging \
...
```bash
PYTHONPATH=python python3 -m ck_dsl.examples.gfx1151.attention.tune \
...
```bash
PYTHONPATH=python python3 -m ck_dsl.examples.gfx1151.attention.mw_tune --waves 2 4
```
...
```bash
PYTHONPATH=python python3 -m ck_dsl.examples.gfx1151.attention.prod_tune \
...
All five kernels are built on the CK Tile helper layer (`ck_dsl.helpers`):
...
f32 path changed; the full `test_ck_dsl` suite stays green:
