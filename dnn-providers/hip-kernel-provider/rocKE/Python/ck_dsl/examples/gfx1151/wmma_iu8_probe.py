...
(:mod:`ck_dsl.instances.gfx1151.wmma_gemm_iu8`) and verifies ``C = A @ B.T``
...
Cross-build (this host has no gfx11 GPU; the Windows board runs it):
  ...
  set CK_DSL_HIP_LIB=C:\\Windows\\System32\\amdhip64_7.dll
  python -m ck_dsl.run_manifest C:\\ck_test\\iu8_probe\\<name>.hsaco \
...
def main() -> int:
    ...
    if args.emit_only:
        print(
            "[emit-only] board: run "
            f"`python -m ck_dsl.run_manifest {out / (art.kernel_name + '.hsaco')} "
