...
ck_dsl has **two engines**, each with a **front end** (the *builder*: spec → IR)
...
```mermaid
flowchart LR
  spec["spec / IRBuilder"] --> pybuild["Python builder<br/>ck_dsl.instances.*"]
  ...
  ser --> parse["ckc_ir_parse (C++)"]
  parse --> cpplow["C++ lowerer<br/>ckc_lower_kernel_to_llvm"]

  cbuild["C++ builder<br/>ckc_build_* (runtime, no Python)"] --> cir[("KernelDef (C++)")]
  cir --> cpplow
...
```mermaid
flowchart TD
  call["compile_kernel(kernel) /<br/>lower_kernel_to_llvm(kernel)"] --> rb{"resolve_backend()<br/>arg &gt; CK_DSL_BACKEND &gt; default (cpp)"}
  rb -->|python| py["native Python lowerer"]
  rb -->|cpp| chk{"ckc_engine importable?"}
  chk -->|yes| cpp["serialize → ckc_engine.lower_serialized_ir"]
  chk -->|no| fb["fallback → Python lowerer<br/>(recorded; CK_DSL_CPP_STRICT=1 to forbid)"]
...
- **Per process:** `export CK_DSL_BACKEND=cpp|python|both`.
- **Default:** `cpp` (the C++ engine), which **auto-falls back to the Python lowerer** if `ckc_engine` isn't built — set `CK_DSL_CPP_STRICT=1` to turn that fallback into an error.
...
> Match the **LLVM flavor** to your `comgr`: `CK_DSL_LLVM_FLAVOR=llvm22` for ROCm 7.2 (else it's auto-detected). See [`../development/setup_guide.md`](../development/setup_guide.md) and [`../reference/env_flags.md`](../reference/env_flags.md).
...
```mermaid
flowchart LR
  g["hipDNN graph"] --> prov["ck-dsl-provider (C++)"]
  prov --> fast["Fast: load prebuilt HSACO<br/>(CK_DSL_KERNEL_LIB_PATH)"]
  prov --> jit["JIT: comgr a shipped .ll"]
  prov --> irart["IR-artifact: ckc_ir_parse(shipped IR)<br/>→ C++ lower → comgr"]
  prov --> cjit["C-JIT: ckc_build_* → C++ lower → comgr<br/>(CK_DSL_C_JIT=1)"]
```
