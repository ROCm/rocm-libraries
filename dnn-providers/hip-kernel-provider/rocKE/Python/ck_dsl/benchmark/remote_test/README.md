# CK_DSL Remote Test Orchestrator

Thin layer that **builds CK_DSL example artifacts locally for multiple GPU
...
CK_DSL builds HSACO + a small manifest in pure Python on this machine; the
...
2. **Recommended — SSH alias.** Put the target in `~/.ssh/config`:
   ...
   ```bash
   export CKDSL_REMOTE_HOST=ckdsl-login
   # CKDSL_REMOTE_USER stays unset — ssh_config supplies it.
   # Optional: shared FS path on the login node, defaults to ~/ckdsl_remote
   export CKDSL_REMOTE_STAGE=/path/on/login-node/ckdsl_remote
   ```

3. **Fallback — explicit host + user** (useful for CI where ssh_config
   isn't convenient):
   ```bash
   export CKDSL_REMOTE_HOST=<slurm-login-host>
   export CKDSL_REMOTE_USER=<login-user>
   export CKDSL_REMOTE_KEY=~/.ssh/id_ed25519   # optional
   export CKDSL_REMOTE_STAGE=/path/on/login-node/ckdsl_remote   # optional
   ```
...
5. **Docker image on the compute node.** The verifier runs inside
   ...
   ```
   Leave `CKDSL_DOCKER_IMAGE` unset to run the verifier directly under
   ...
   `CKDSL_DOCKER_EXTRA_FLAGS` (space-separated, shlex-parsed). The
   default `.ckdsl_env` template (below) forwards Slurm's GPU-visibility
...
by sourcing them from a file you own — e.g. `~/.ckdsl_env`:

```bash
# ~/.ckdsl_env  (NOT checked in)
export CKDSL_REMOTE_HOST=ckdsl-login                                 # ssh alias
export CKDSL_DOCKER_IMAGE=rocm/composable_kernel:ck_ub24.04_rocm7.2
export CKDSL_DOCKER_EXTRA_FLAGS="-e HIP_VISIBLE_DEVICES \
  -e ROCR_VISIBLE_DEVICES -e GPU_DEVICE_ORDINAL --group-add render"
...
```bash
source ~/.ckdsl_env   # see "site env file" above
...
python -m ck_dsl.benchmark.remote_test.cli build --arch gfx942,gfx1151

# 3) Push + srun the manifest verifier on each arch
python -m ck_dsl.benchmark.remote_test.cli run   --arch gfx942,gfx1151
...
srun --partition=defq --constraint=GFX1151 --gres=gpu:1 --time=00:15:00 \
     ...
     bash -lc '
       docker run --rm --network=host \
         ...
         -w <stage>/ck_pkg $CKDSL_DOCKER_EXTRA_FLAGS \
         $CKDSL_DOCKER_IMAGE /bin/bash -lc "
           cd <stage>/ck_pkg
...
`docker run` wrapper is skipped if `CKDSL_DOCKER_IMAGE` is unset.
...
Both currently-wired examples (`ck_dsl.examples.gfx942.gemm_demo` and
`ck_dsl.examples.gfx1151.wmma_gemm_verify`) already satisfy this contract.
