...
Generalises :mod:`ck_dsl.instances.common.batched_gemm` to arbitrary
...
def is_valid_spec(
    ...
    underlying :func:`ck_dsl.instances.common.batched_gemm.is_valid_spec`, which
    sources the legal MFMA warp-tile atoms + LDS cap from
    :class:`ck_dsl.core.arch.ArchTarget`. A warp-tile atom that exists on
...
def build_batched_contraction(
    ...
    warp-tile atom against the :class:`ck_dsl.core.arch.ArchTarget`
...
# :func:`ck_dsl.helpers.tensor_view.view_from_transforms_descriptor`
...
def _build_contraction_descriptor(
    ...
    *lower* side; ck_dsl's :class:`Unmerge` matches that
    upper/lower split (the multi-to-flat direction is what
    ck_dsl names :class:`Merge`).
    """
