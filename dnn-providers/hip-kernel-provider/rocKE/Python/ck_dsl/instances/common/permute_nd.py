...
* Global loads / stores go through
  :class:`ck_dsl.helpers.tensor_view.TensorView`'s ``load_vec_at`` /
...
* Dtypes ``f16`` / ``bf16``. The CK Tile reference also handles
  ...
  ``load_vec`` dispatch in :mod:`ck_dsl.helpers.tensor_view`.
...
def is_valid_spec(spec: PermuteSpec, arch: str = "gfx950") -> Tuple[bool, str]:
    ...
    :class:`ck_dsl.core.arch.ArchTarget` (the supported launch sizes
