...
"""
ck_dsl-native, op-parameterized heuristics training-data generator.

This is the multi-op generalization of :mod:`ck_dsl.heuristics.gen_gemm_sweep_data`.
For a given op family it:
  ...
  3. Builds every ``(variant)`` to a cached HSACO (LLVM IR -> comgr), driving the
     same :mod:`ck_dsl.sweep` / :mod:`ck_dsl.sweep_bench` ecosystem GEMM uses.
  ...
  5. Writes a training parquet whose feature columns match the op's
     :mod:`ck_dsl.heuristics.feature_engine` engine, plus ``measured_tflops``,
...
Usage::
    ...
    python3 -m ck_dsl.heuristics.gen_sweep_data \\
        --op conv \\
        --out conv_training.parquet \\
        --cache-dir /tmp/ck_dsl_conv_cache \\
        --arch gfx950 \\
        --max-shapes 32

The ``gemm`` op keeps the original
``python3 -m ck_dsl.heuristics.gen_gemm_sweep_data ...`` entry point working as
...
def generate(
    ...
    **gemm_kwargs: object,
    ...
    :mod:`ck_dsl.heuristics.feature_engine` engine.
    """
    if op == "gemm":
