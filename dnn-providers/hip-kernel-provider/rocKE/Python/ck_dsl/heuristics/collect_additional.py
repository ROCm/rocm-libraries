...
SHAPES = [
    ...
    (1, 4096, 4096), (8, 4096, 4096), (32, 4096, 4096), (128, 4096, 4096),
    (1, 4096, 11008), (32, 4096, 11008), (1, 8192, 8192), (32, 8192, 8192),
    (1, 8192, 28672), (32, 8192, 28672), (256, 256, 8192), (8192, 8192, 256),
    (1024, 4096, 1024), (4096, 1024, 4096), (2048, 8192, 2048),
]
...
def main() -> int:
    ...
    if not exes:
        print(f"no benchmark binaries under {BIN_DIR} "
              f"(set CK_TILE_BIN_DIR)", file=sys.stderr)
    ...
    with OUT_LOG.open("w", encoding="utf-8") as log:
        ...
        for idx, (m, n, k) in enumerate(SHAPES, 1):
            ...
            print(f"Completed shape {idx}: M={m} N={n} K={k} ({len(exes)} kernels)",
                  file=sys.stderr)
