...
Worked implementation mirroring :mod:`ck_dsl.dispatch.gemm.bf16_rcr`. Two
``kind`` values are supported: ``rmsnorm`` (backed by
:mod:`ck_dsl.instances.common.rmsnorm2d`) and ``layernorm`` (backed by
:mod:`ck_dsl.instances.common.layernorm2d`).
...
:class:`ck_dsl.core.arch.ArchTarget`) rather than a GEMM tile/atom check. The
