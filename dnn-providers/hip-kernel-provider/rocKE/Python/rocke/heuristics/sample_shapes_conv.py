...
Reads one or more shape CSVs produced by generate_coverage_conv.py (or any
compatible source), deduplicates, buckets shapes by coverage dimensions, then
round-robins across buckets to hit a target count.  This preserves
representative coverage across all axes rather than accidentally dropping
entire corners by hand-thinning generator ranges.
...
Usage:
    # merge wide + edge outputs, sample to 2000
    python3 sample_shapes_conv.py \\
        --inputs wide_conv.csv edge_conv.csv \\
