...
const char* ckc_isa_datalayout_for_flavor(const ckc_isa_backend_t* be, ckc_llvm_flavor_t flavor)
{
    ...
    return (flavor == CKC_LLVM_FLAVOR_LLVM20) ? CKC_DATALAYOUT_LLVM20 : CKC_DATALAYOUT_LLVM22;
}
...
const char* ckc_isa_datalayout(const ckc_isa_backend_t* be)
{
     ...
     * callers use ckc_isa_datalayout_for_flavor. */
    return ckc_isa_datalayout_for_flavor(be, CKC_LLVM_FLAVOR_LLVM20);
}
...
{
    return ckc_isa_module_preamble_for_flavor(be, CKC_LLVM_FLAVOR_LLVM20, out);
}
...
int ckc_isa_encode_waitcnt(const ckc_isa_backend_t* be, int vmcnt, int expcnt, int lgkmcnt)
{
...
const ckc_wmma_spec_t* ckc_isa_resolve_wmma(const ckc_isa_backend_t* be, const char* op_name)
{
    if(be == NULL || !be->valid)
    {
        return NULL;
