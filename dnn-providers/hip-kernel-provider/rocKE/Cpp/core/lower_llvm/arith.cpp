...
static void _op_arith_fneg(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(
        L, "  %s = fneg %s %s", res->name, ckc_ll_llvm_type(L, v->type), ckc_ll_operand(L, v));
}
...
static void _op_arith_fabs(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    {
        ckc_ll_fail(
            L, CKC_ERR_NOTIMPL, "fabs: unsupported FP type %s", ty_name ? ty_name : "(null)");
    }
    char key[32];
...
static void _op_arith_fma(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    {
        ckc_ll_fail(
            L, CKC_ERR_NOTIMPL, "fma: unsupported FP type %s", ty_name ? ty_name : "(null)");
    }
    char key[32];
...
static void ll_fminmax(ckc_lower_t* L, const ckc_op_t* op, const char* intrin)
{
    ...
    {
        ckc_ll_fail(
            L, CKC_ERR_NOTIMPL, "%s: unsupported FP type %s", intrin, ty_name ? ty_name : "(null)");
    }
    char key[32];
    snprintf(key, sizeof key, "%s.%s", intrin, ty_name);
    ckc_ll_need(L, key);
 ...
 * is the _need() key. */
static void ll_math_f32_unary(
    ckc_lower_t* L, const ckc_op_t* op, const char* op_label, const char* key, const char* intrin)
{
    const ckc_value_t* res = ll_result(op);
    if(!ckc_ll_live(L) || !res)
    {
    ...
    {
        ckc_ll_fail(L, CKC_ERR_NOTIMPL, "math.%s currently supports f32", op_label);
    }
    ckc_ll_need(L, key);
    ckc_ll_emitf(
        L, "  %s = call float @llvm.%s(float %s)", res->name, intrin, ckc_ll_operand(L, v));
}
...
static void _op_gpu_thread_id(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_need(L, key);
    ckc_ll_emitf(L, "  %s = call i32 @llvm.amdgcn.workitem.id.%s()", res->name, axis);
}
...
static void _op_gpu_block_id(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_need(L, key);
    ckc_ll_emitf(L, "  %s = call i32 @llvm.amdgcn.workgroup.id.%s()", res->name, axis);
}
