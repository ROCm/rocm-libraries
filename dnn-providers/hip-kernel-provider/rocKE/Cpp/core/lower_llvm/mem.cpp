 ...
 * ck_dsl.core.lower_llvm: global memref loads/stores/atomics, smem
 ...
 *
 * Shared plumbing (ckc_ll_emit*, ckc_ll_fresh, ckc_ll_operand, ckc_ll_need,
 * ckc_ll_llvm_type, ckc_ll_smem_*, ...) is DEFINED IN BUCKET 0; this file only
 * calls it through ckc/lower_llvm_internal.h.
 */
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
static int64_t ll_attr_int(const ckc_op_t* op, const char* key, int64_t dflt)
{
    int64_t v = 0;
...
static const char* ll_attr_str(const ckc_op_t* op, const char* key, const char* dflt)
{
...
static const char* ll_smem_gidx(ckc_lower_t* L, const ckc_op_t* op, int lo, int hi)
{
    ...
    for(i = lo; i < hi; i++)
    {
        acc = ckc_arena_printf(&L->arena, "%s, i32 %s", acc, ckc_ll_operand(L, op->operands[i]));
    }
    return acc;
}
...
static void op_tile_smem_alloc(ckc_lower_t* L, const ckc_op_t* op)
{
...
static void op_memref_global_load(ckc_lower_t* L, const ckc_op_t* op)
{
...
static void op_memref_global_load_typed(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
...
static void op_memref_global_store(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
    ...
    ckc_ll_emitf(L,
                 ...
                 ckc_ll_operand(L, val),
                 gep,
                 (long long)align);
}

static void op_memref_global_store_typed(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
    const ckc_value_t* val = op->operands[2];
...
static void op_memref_global_atomic_add(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
    const ckc_value_t* val = op->operands[2];
...
static void op_memref_global_atomic_add_f32(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
    const ckc_value_t* val = op->operands[2];
...
static void op_memref_global_atomic_add_pk_bf16(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
    const ckc_value_t* val = op->operands[2];
    const char* gep;
    ckc_ll_need(L, "global.atomic.fadd.v2bf16");
    gep = ckc_ll_fresh(L, "gep");
    ckc_ll_emitf(L,
...
static void op_memref_global_load_vN(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
...
static void op_memref_global_store_vN(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* idx = op->operands[1];
    const ckc_value_t* val = op->operands[2];
...
static void op_memref_cooperative_global_store(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        const char* vi = ckc_ll_fresh(L, ckc_arena_printf(&L->arena, "coop_v%lld", (long long)i));
        const char* gep;
        ckc_ll_emitf(L,
                     "  %s = extractelement %s %s, i32 %lld",
                     ai,
                     addrs_ty,
                     ckc_ll_operand(L, addrs),
                     (long long)i);
        ckc_ll_emitf(L,
                     "  %s = extractelement %s %s, i32 %lld",
                     vi,
                     values_ty,
                     ckc_ll_operand(L, values),
                     (long long)i);
        gep = ckc_ll_fresh(L, ckc_arena_printf(&L->arena, "coop_gep%lld", (long long)i));
        ckc_ll_emitf(L,
                     "  %s = getelementptr inbounds %s, ptr addrspace(1) %s, %s %s",
                     gep,
                     elem_ty,
                     ...
                     addr_elem_ty,
                     ai);
        ckc_ll_emitf(L, "  store %s %s, ptr addrspace(1) %s, align 4", elem_ty, vi, gep);
    }
}
...
static void op_tile_smem_store(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L,
                 ...
                 ckc_ll_llvm_type(L, value->type),
                 ckc_ll_operand(L, value),
                 gep,
                 align);
}
...
static void op_tile_smem_load_v4(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L,
                 ...
                 agg_ty,
                 gname,
                 ckc_ll_operand(L, row),
                 ckc_ll_operand(L, col));
    for(i = 0; i < 4; i++)
    {
        const char* ep = ckc_ll_fresh(L, "smem.ep");
        const char* ld;
        ckc_ll_emitf(
            L, "  %s = getelementptr inbounds half, ptr addrspace(3) %s, i32 %d", ep, base, i);
        ld = ckc_ll_fresh(L, "smem.ld");
        ckc_ll_emitf(L, "  %s = load half, ptr addrspace(3) %s, align 2", ld, ep);
        elems[i] = ld;
    }
    prev = "undef";
    for(i = 0; i < 4; i++)
    {
        const char* tmp = (i == 3) ? ll_res(op) : ckc_ll_fresh(L, "vec");
        ckc_ll_emitf(
...
static void op_tile_smem_load_vN(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    if(vec == 1)
    {
        const char* scalar = ckc_ll_fresh(L, "smem.s");
        ckc_ll_emitf(L,
...
static void op_tile_smem_store_distributed(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        ckc_ll_emitf(L,
                     ...
                     gname,
                     i);
        ckc_ll_emitf(L, "  store %s %s, ptr addrspace(3) %s, align 2", elem_ty, ev, gep);
    }
}
...
static void op_tile_smem_store_vN_f32(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    if(vec == 1)
    {
        ...
        {
            const char* ext = ckc_ll_fresh(L, "v1ext");
            ckc_ll_emitf(L,
...
static void op_tile_global_ptr_add(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* ptr = op->operands[0];
    const ckc_value_t* off = op->operands[1];
    ...
    {
        off64 = ckc_ll_fresh(L, "goff64");
        ckc_ll_emitf(L, "  %s = zext i32 %s to i64", off64, ckc_ll_operand(L, off));
    }
    else if(strcmp(off_ty, "i64") == 0)
    {
        off64 = ckc_ll_operand(L, off);
    }
    else
    {
        ckc_ll_fail(
            L, CKC_ERR_VALUE, "tile.global_ptr_add offset must be i32 or i64, got %s", off_ty);
    }
    ckc_ll_emitf(L,
...
static void op_tile_buffer_rsrc(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    const char* nb_text;
    int word3;
    ckc_ll_need(L, "make.buffer.rsrc.p1");
    if(L->flavor == CKC_LLVM_FLAVOR_LLVM22)
    {
        const char* nb_ty = ckc_ll_llvm_type(L, num_bytes->type);
        const char* nb_arg;
        intrinsic = "llvm.amdgcn.make.buffer.rsrc.p8.p1";
        if(strcmp(nb_ty, "i64") == 0)
        {
            nb_arg = ckc_ll_operand(L, num_bytes);
        }
        else if(strcmp(nb_ty, "i32") == 0)
        {
            nb_arg = ckc_ll_fresh(L, "nb64");
            ckc_ll_emitf(L, "  %s = zext i32 %s to i64", nb_arg, ckc_ll_operand(L, num_bytes));
        }
        else
        {
            ckc_ll_fail(
                L, CKC_ERR_VALUE, "tile.buffer_rsrc num_bytes must be i32 or i64, got %s", nb_ty);
        }
...
static void op_tile_buffer_load_vN_f16(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    if(dwords == 1)
    {
        ...
        ckc_ll_emitf(L, "  %s = bitcast i32 %s to <2 x half>", ll_res(op), tmp);
    }
    else
    {
        const char* intr
...
static void op_tile_buffer_load_f16(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    const ckc_value_t* soffset = op->operands[2];
    const char* tmp;
    ckc_ll_need(L, "raw.ptr.buffer.load.i16");
    tmp = ckc_ll_fresh(L, "blu16");
    ckc_ll_emitf(L,
                 ...
                 ckc_ll_operand(L, voffset),
                 ckc_ll_operand(L, soffset));
    ckc_ll_emitf(L, "  %s = bitcast i16 %s to half", ll_res(op), tmp);
}
...
static void op_tile_buffer_store_vN_f16(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    {
        const char* bc;
        ckc_ll_need(L, "raw.ptr.buffer.store.i32");
        bc = ckc_ll_fresh(L, "bsbc");
        ckc_ll_emitf(L, "  %s = bitcast <2 x half> %s to i32", bc, ckc_ll_operand(L, val));
        ckc_ll_emitf(L,
...
static void op_tile_buffer_store_f16(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L, "  %s = bitcast half %s to i16", bc, ckc_ll_operand(L, val));
    ckc_ll_emitf(L,
...
static void op_tile_async_buffer_load_lds_addr(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(
        L, "  %s = inttoptr i64 %s to ptr addrspace(3)", ptr_name, ckc_ll_operand(L, lds_addr));
    ckc_ll_emitf(L,
                 ...
                 ckc_ll_operand(L, voff),
                 ckc_ll_operand(L, soff),
                 (long long)aux);
}
...
static void op_tile_global_load_lds(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    gep = ckc_ll_fresh(L, "gld_src");
    ckc_ll_emitf(L,
                 "  %s = getelementptr inbounds i8, ptr addrspace(1) %s, i32 %s",
                 gep,
                 ckc_ll_operand(L, src_ptr),
                 ckc_ll_operand(L, byte_off));
    ptr_name = ckc_ll_fresh(L, "lds_ptr");
    ckc_ll_emitf(
        L, "  %s = inttoptr i64 %s to ptr addrspace(3)", ptr_name, ckc_ll_operand(L, lds_addr));
    ckc_ll_emitf(L,
...
void ckc_ll_register_mem(void)
{
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_ALLOC, op_tile_smem_alloc);

    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_LOAD, op_memref_global_load);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_LOAD_TYPED, op_memref_global_load_typed);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_LOAD_VN, op_memref_global_load_vN);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_STORE, op_memref_global_store);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_STORE_TYPED, op_memref_global_store_typed);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_STORE_VN, op_memref_global_store_vN);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_ATOMIC_ADD, op_memref_global_atomic_add);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_ATOMIC_ADD_F32, op_memref_global_atomic_add_f32);
    ckc_ll_set_handler(CKC_OP_MEMREF_GLOBAL_ATOMIC_ADD_PK_BF16,
                       op_memref_global_atomic_add_pk_bf16);
    ckc_ll_set_handler(CKC_OP_MEMREF_COOPERATIVE_GLOBAL_STORE, op_memref_cooperative_global_store);

    ckc_ll_set_handler(CKC_OP_TILE_SMEM_STORE, op_tile_smem_store);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_STORE_VN, op_tile_smem_store_vN);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_STORE_VN_F32, op_tile_smem_store_vN_f32);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_STORE_DISTRIBUTED, op_tile_smem_store_distributed);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_LOAD_V4, op_tile_smem_load_v4);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_LOAD_VN, op_tile_smem_load_vN);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_LOAD_VN_F32, op_tile_smem_load_vN_f32);
    ckc_ll_set_handler(CKC_OP_TILE_LDS_ATOMIC_ADD, op_tile_lds_atomic_add);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_ADDR_OF, op_tile_smem_addr_of);
    ckc_ll_set_handler(CKC_OP_TILE_SMEM_PTR_ADD, op_tile_smem_ptr_add);
    ckc_ll_set_handler(CKC_OP_TILE_GLOBAL_PTR_ADD, op_tile_global_ptr_add);
    ...
    ckc_ll_set_handler(CKC_OP_TILE_ASYNC_BUFFER_LOAD_LDS, op_tile_async_buffer_load_lds);
    ckc_ll_set_handler(CKC_OP_TILE_ASYNC_BUFFER_LOAD_LDS_ADDR, op_tile_async_buffer_load_lds_addr);
    ckc_ll_set_handler(CKC_OP_TILE_GLOBAL_LOAD_LDS, op_tile_global_load_lds);
}
