...
static void _op_arith_trunc_f32_to_f16(ckc_lower_t* L, const ckc_op_t* op)
{
    const ckc_value_t* v = op->operands[0];
    ckc_ll_emitf(
        L, "  %s = fptrunc float %s to half", ckc_ll_result_name(op), ckc_ll_operand(L, v));
}
...
static void _op_arith_sitofp_f32(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L, "  %s = sitofp i32 %s to float", ckc_ll_result_name(op), ckc_ll_operand(L, v));
}
...
static void _op_arith_cvt_f32_to_i8_sat(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    ckc_ll_emitf(L, "  %s = call i32 @llvm.smax.i32(i32 -128, i32 %s)", smax_v, as_i32);
    ckc_ll_emitf(L, "  %s = call i32 @llvm.smin.i32(i32 127, i32 %s)", smin_v, smax_v);
...
static void _op_arith_cvt_pk_i8_f32x4(ckc_lower_t* L, const ckc_op_t* op)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        ckc_ll_emitf(L, "  %s = call i32 @llvm.smax.i32(i32 -128, i32 %s)", mx, ai);
        ckc_ll_emitf(L, "  %s = call i32 @llvm.smin.i32(i32 127, i32 %s)", mn, mx);
        clamped[i] = mn;
    }
...
void ckc_ll_register_convert(void)
{
    ...
    ckc_ll_set_handler(CKC_OP_ARITH_CVT_SCALEF32_PK_F32_FP8, _op_arith_cvt_scalef32_pk_f32_fp8);
    ckc_ll_set_handler(CKC_OP_ARITH_CVT_SCALEF32_PK_F32_BF8, _op_arith_cvt_scalef32_pk_f32_bf8);
