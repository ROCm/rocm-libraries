...
/*
 * bindings/ckc_engine_attention.cpp -- attention-family bindings for the
 * ckc_engine module. Kept in its own translation unit because the attention
 * instance headers share a set of struct tags (ckc_fmha_*, the tiled 2d/3d spec
...
std::string take_ll(ckc_status_t st, char* ll, const char* err, const char* fn)
{
    if(st != CKC_OK || !ll)
    {
        if(ll)
...
ckc_fmha_mask_mode_t mask_of(const py::dict& d, const char* key)
{
    std::string s;
    if(!a_str(d, key, s))
        return CKC_FMHA_MASK_NONE;
    if(s == "causal")
        return CKC_FMHA_MASK_CAUSAL;
    if(s == "sliding_window")
        return CKC_FMHA_MASK_SLIDING_WINDOW;
    if(s == "alibi")
        return CKC_FMHA_MASK_ALIBI;
    if(s == "custom")
        return CKC_FMHA_MASK_CUSTOM;
    return CKC_FMHA_MASK_NONE;
}

/* Build a ckc_fmha_common_spec_t from common sub-keys of the dict. The shape is
 * taken from head_size/num_query_heads/num_kv_heads (+ optional
 * block_size_q/block_size_k for the shape_make form). */
ckc_fmha_common_spec_t common_of(const py::dict& d, Store& st)
{
    ckc_fmha_shape_t shape;
    if(d.contains("block_size_q") || d.contains("block_size_k"))
        shape = ckc_fmha_shape_make(a_int(d, "head_size", 0),
                                    a_int(d, "num_query_heads", 0),
                                    a_int(d, "num_kv_heads", 0),
                                    a_int(d, "block_size_q", 16),
                                    a_int(d, "block_size_k", 64));
    else
        shape = ckc_fmha_shape_default(
            a_int(d, "head_size", 0), a_int(d, "num_query_heads", 0), a_int(d, "num_kv_heads", 0));
    ckc_fmha_common_spec_t c = ckc_fmha_common_spec_default(shape);
    std::string v;
    if(a_str(d, "dtype", v))
        c.dtype = st.keep(v);
...
ckc_kernel_def_t* au_make(const py::dict& d, Store& st, ckc_ir_builder_t* b)
{
    ckc_unified_attention_problem_t p = au_build(d, st);
    char kname[512];
    kname[0] = '\0';
    ckc_unified_attention_2d_scalar_kernel_name(&p, nullptr, kname, sizeof kname);
    ckc_ir_builder_init(b, kname);
    return ckc_build_unified_attention_2d_scalar(b, &p, nullptr);
}
...
std::string au_lower(const py::dict& d, const std::string& arch)
{
    Store st;
    ckc_ir_builder_t b;
    ckc_kernel_def_t* k = au_make(d, st, &b);
    if(!k || !ckc_ir_builder_ok(&b))
    {
        std::string m = std::string("attention_unified build failed: ") + ckc_ir_builder_error(&b);
    ...
    ckc_ir_builder_free(&b);
    return take_ll(s2, ll, err, "ckc_engine.attention_unified_lower_llvm");
}
std::string au_serialize(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = ser_kernel(k, "ckc_engine.attention_unified_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
ckc_fmha_fwd_fp8_spec_t fp8_build(const py::dict& d, Store& st, bool* persistent_unused)
{
    ...
    s.seqlen_k = a_int(d, "seqlen_k", s.seqlen_k);
    {
        ...
        if(a_str(d, "kv_dtype", kv))
            s.kv_dtype = (kv == "bf8e5m2" || kv == "bf8") ? CKC_KV_BF8_E5M2 : CKC_KV_FP8_E4M3;
    }
...
std::string fp8_serialize(const py::dict& d, const std::string& arch)
{
    ...
    if(!k)
    {
        ...
        throw std::runtime_error("ckc_engine.fmha_fwd_fp8_serialize_ir build failed");
    }
    std::string out = ser_kernel(k, "ckc_engine.fmha_fwd_fp8_serialize_ir");
    ckc_fmha_kernel_builder_free(&kb);
    return out;
}
std::vector<std::string> fp8_verify(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_fwd_fp8_verify build failed");
    }
    std::vector<std::string> out = ver_kernel(k);
    ckc_fmha_kernel_builder_free(&kb);
    return out;
...
{
    Store st;
    ckc_fmha_bwd_spec_t s = bwd_build(d, st);
    ckc_fmha_kernel_builder_t kb;
    ckc_kernel_def_t* k = ckc_build_fmha_bwd(&kb, &s, arch.empty() ? "gfx950" : arch.c_str());
    if(!k)
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_bwd_lower_llvm build failed");
    }
    char* ll = nullptr;
    char err[CKC_ERR_MSG_CAP];
    err[0] = '\0';
    ckc_status_t s2 = ckc_lower_kernel_to_llvm_ex(
        k, CKC_LLVM_FLAVOR_AUTO, arch.empty() ? "gfx950" : arch.c_str(), &ll, err, sizeof err);
    ckc_fmha_kernel_builder_free(&kb);
    return take_ll(s2, ll, err, "ckc_engine.fmha_bwd_lower_llvm");
}
std::string bwd_serialize(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_bwd_serialize_ir build failed");
    }
    std::string out = ser_kernel(k, "ckc_engine.fmha_bwd_serialize_ir");
    ckc_fmha_kernel_builder_free(&kb);
    return out;
}
std::vector<std::string> bwd_verify(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_bwd_verify build failed");
    }
    std::vector<std::string> out = ver_kernel(k);
    ckc_fmha_kernel_builder_free(&kb);
    return out;
...
ckc_fmha_head_grouping_spec_t hg_build(const py::dict& d, Store& st)
{
    ckc_fmha_common_spec_t c = common_of(d, st);
    return ckc_fmha_head_grouping_spec_make(c, a_int(d, "seqlen_q", 0), a_int(d, "seqlen_k", 0));
}
std::string hg_lower(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_head_grouping_lower_llvm build failed");
    }
    char* ll = nullptr;
    char err[CKC_ERR_MSG_CAP];
    err[0] = '\0';
    ckc_status_t s2 = ckc_lower_kernel_to_llvm_ex(
        k, CKC_LLVM_FLAVOR_AUTO, arch.empty() ? "gfx950" : arch.c_str(), &ll, err, sizeof err);
    ckc_fmha_kernel_builder_free(&kb);
    return take_ll(s2, ll, err, "ckc_engine.fmha_head_grouping_lower_llvm");
}
std::string hg_serialize(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_head_grouping_serialize_ir build failed");
    }
    std::string out = ser_kernel(k, "ckc_engine.fmha_head_grouping_serialize_ir");
    ckc_fmha_kernel_builder_free(&kb);
    return out;
}
std::vector<std::string> hg_verify(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.fmha_head_grouping_verify build failed");
    }
    std::vector<std::string> out = ver_kernel(k);
    ckc_fmha_kernel_builder_free(&kb);
    return out;
...
ckc_qk_scale_layout_t qkl_of(const std::string& s)
{
    return (s == "per_block") ? CKC_QK_SCALE_PER_BLOCK : CKC_QK_SCALE_PER_HEAD;
}
void fill_qk_scale(ckc_qk_scale_spec_t* q, const py::dict& d)
{
    std::string lay;
...
{
    if(s == "fp8_bf16")
        return CKC_SAGE_QUANT_FP8_BF16;
    if(s == "i8_fp8_bf16")
        return CKC_SAGE_QUANT_I8_FP8_BF16;
    if(s == "i4_fp8_bf16")
        return CKC_SAGE_QUANT_I4_FP8_BF16;
    return CKC_SAGE_QUANT_FP16_BF16;
}
...
std::string sage_serialize(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.sage_attention_serialize_ir build failed");
    }
    std::string out = ser_kernel(k, "ckc_engine.sage_attention_serialize_ir");
    ckc_fmha_kernel_builder_free(&kb);
    return out;
}
std::vector<std::string> sage_verify(const py::dict& d, const std::string& arch)
{
    ...
    {
        ckc_fmha_kernel_builder_free(&kb);
        throw std::runtime_error("ckc_engine.sage_attention_verify build failed");
    }
    std::vector<std::string> out = ver_kernel(k);
    ckc_fmha_kernel_builder_free(&kb);
    return out;
...
std::string t942_serialize(const py::dict& d, const std::string& arch)
{
    ...
    ckc_attention_tiled_2d_spec_t s = t2d_build(d, st);
    ckc_ir_builder_t b;
    ckc_kernel_def_t* k = ckc_build_unified_attention_2d_tiled_scalar_new(
    ...
    std::string out = ser_kernel(k, "ckc_engine.gfx942_attention_tiled_2d_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
std::string t950_serialize(const py::dict& d, const std::string& arch)
{
    ...
    ckc_attention_tiled_2d_spec_t s = t2d_build(d, st);
    ckc_ir_builder_t b;
    ckc_kernel_def_t* k = ckc_gfx950_build_unified_attention_2d_tiled_new(
    ...
    std::string out = ser_kernel(k, "ckc_engine.gfx950_attention_tiled_2d_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
ckc_attention_tiled_2d_spec_t fkv_build(const py::dict& d, Store& st)
{
    ckc_attention_tiled_2d_spec_t s = ckc_attention_tiled_2d_spec_default();
...
{
    Store st;
    ckc_attention_tiled_2d_spec_t s = fkv_build(d, st);
    ckc_ir_builder_t b;
    const char* a = arch.empty() ? "gfx950" : arch.c_str();
    ckc_ir_builder_init(&b, "attention_tiled_2d_fastkv_regp");
...
std::string fkv_serialize(const py::dict& d, const std::string& arch)
{
    ...
    ckc_ir_builder_t b;
    ckc_ir_builder_init(&b, "attention_tiled_2d_fastkv_regp");
    ckc_kernel_def_t* k = ckc_build_unified_attention_2d_fastkv_register_p(
        &b, &s, arch.empty() ? "gfx950" : arch.c_str());
    if(!k || !ckc_ir_builder_ok(&b))
    {
        std::string m = std::string("gfx950_attention_tiled_2d_fastkv_regp build failed: ")
...
std::string t3d942_serialize(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = ser_kernel(k, "ckc_engine.gfx942_attention_tiled_3d_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
...
std::string t3d950_serialize(const py::dict& d, const std::string& arch)
{
    ...
    std::string out = ser_kernel(k, "ckc_engine.gfx950_attention_tiled_3d_serialize_ir");
    ckc_ir_builder_free(&b);
    return out;
}
