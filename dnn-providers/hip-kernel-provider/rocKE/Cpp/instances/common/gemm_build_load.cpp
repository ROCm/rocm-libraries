...
void ckc_gemm_build_populate_ctx(ckc_gemm_build_ctx_t* ctx)
{
    ...
    if(spec->trait.chiplet_swizzle)
    {
        ...
        ckc_value_t* wgid_flat
            = ckc_b_add(b, ckc_b_mul(b, ckc_b_block_id_y(b), n_pid_n), ckc_b_block_id_x(b));
...
ckc_value_t* ckc_gemm_swz_col(ckc_gemm_build_ctx_t* ctx, ckc_value_t* col, ckc_value_t* row)
{
    ...
    return ckc_b_xor(
        b,
        col,
        ckc_b_shl(b, ckc_b_mod(b, ckc_b_lshr(b, row, ctx->c_swr), ctx->c_swmod), ctx->c_swl));
}
                              ...
                              ckc_value_t* parity_v)
{
    ...
    if(spec->trait.direct_to_lds)
    {
        ...
        {
            ckc_value_t* a_par_v
                = ckc_b_zext(b, ckc_b_mul(b, parity_v, ckc_b_const_i32(b, a_half_bytes)), I64);
            ckc_value_t* b_par_v
                = ckc_b_zext(b, ckc_b_mul(b, parity_v, ckc_b_const_i32(b, b_half_bytes)), I64);
        ...
        if(t->warp_m * t->warp_n * t->warp_k > 1)
        {
            ...
            ckc_value_t* wave_par_off
                = ckc_b_zext(b, ckc_b_mul(b, warp_id, ckc_b_const_i32(b, wave_bytes)), I64);
        ...
        for(int p = 0; p < ctx->dtl_a_passes; ++p)
        {
            ...
            ckc_value_t* a_row_term = ckc_b_mul(b, ckc_b_add(b, ctx->block_m_off, row), ctx->K);
        ...
        for(int p = 0; p < ctx->dtl_b_passes; ++p)
        {
            ...
            ckc_value_t* b_row_term = ckc_b_mul(b, ckc_b_add(b, ctx->block_n_off, row), ctx->K);
...
#define MASK_STORAGE(value, valid) ckc_b_select(b, (valid), (value), STORAGE_ZERO())
    ...
    {
        ckc_value_t* vec_idx
            = ckc_b_add(b, ckc_b_mul(b, ckc_b_const_i32(b, e), ctx->c_threads), ctx->tid);
        /* _vec_rc(vec_idx). */
    ...
    if(spec->trait.preshuffle_b)
    {
        ...
        for(int e = 0; e < ctx->b_vecs_per_thread; ++e)
        {
            ...
            {
                ckc_value_t* b_val
                    = ckc_b_global_load_vN(b, ctx->Bp, glob_off, ctx->storage_dtype, load_vec, 0);
                ckc_value_t* lidx[2] = {LD_ROW(row, ctx->block_n), ckc_gemm_swz_col(ctx, col, row)};
                ckc_tile_window_store_vec(b, &b_lds_tile, lidx, 2, b_val, load_vec);
            }
        }
                                          ...
                                          int frag_len)
{
    ...
    bool is_half
        = ckc_type_eq(ctx->storage_dtype, ckc_f16()) || ckc_type_eq(ctx->storage_dtype, ckc_bf16());
    ...
    {
        ckc_value_t* chunk = ckc_gemm_emit_smem_load(b,
                                                     src,
                                                     lds_row,
                                                     ckc_b_add(b, lds_col, ckc_b_const_i32(b, off)),
                                                     max_vec,
                                                     ctx->storage_dtype);
                              ...
                              ckc_value_t** out_accs)
{
    ...
    ckc_value_t* warp_m_off
        = ckc_b_mul(b, ctx->warp_m_idx, ckc_b_const_i32(b, ctx->mfmas_m * t->warp_tile_m));
    ckc_value_t* warp_n_off
        = ckc_b_mul(b, ctx->warp_n_idx, ckc_b_const_i32(b, ctx->mfmas_n * t->warp_tile_n));
    ...
    for(int kk = 0; kk < ctx->k_atoms; ++kk)
    {
        ...
        for(int mi = 0; mi < ctx->mfmas_m; ++mi)
        {
            ckc_value_t* atom_row
                = ckc_b_add(b, warp_m_off, ckc_b_const_i32(b, mi * t->warp_tile_m));
        ...
        {
            ckc_value_t* atom_row
                = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * t->warp_tile_n));
