...
static ckc_status_t ckc_x_make_lds_view(ckc_ir_builder_t* b,
...
static void ckc_x_tensor_view_load_vec_as_f32(ckc_ir_builder_t* b,
...
static const char* ckc_x_quant_canon(const char* qdtype)
{
    if(qdtype == NULL)
    {
        return NULL;
...
static ckc_value_t* ckc_x_quantize_scalar_f32(ckc_ir_builder_t* b,
                                              ckc_value_t* x_f32,
                                              ...
                                              const ckc_type_t* q_ty)
{
    ...
    (void)ckc_i_set_err(b, CKC_ERR_VALUE, "unreachable canon");
    return NULL;
}
...
static ckc_value_t* ckc_x_pack_quant_chunk_local_f32(ckc_ir_builder_t* b,
                                                     ckc_value_t* const* scaled_f32,
                                                     int n,
                                                     const ckc_type_t* q_ty,
                                                     const char* out_dtype)
{
    ...
    if((strcmp(out_dtype, "fp8e4m3") == 0 || strcmp(out_dtype, "bf8e5m2") == 0) && (n % 4 == 0))
    {
        ...
        {
            out_v = ckc_b_vec_concat(b, out_v, packed_chunks[i]);
        }
        return out_v;
    }
    ...
    /* i8 path (or VEC=2 fp8/bf8): per-element saturating cast + vec_pack. */
    {
        ...
        for(i = 0; i < n; ++i)
        {
            ...
            {
                qs[i] = ckc_b_cvt_f32_to_fp8(b, sf);
            }
            else if(strcmp(out_dtype, "bf8e5m2") == 0)
            {
                qs[i] = ckc_b_cvt_f32_to_bf8(b, sf);
            }
            else
            {
                (void)ckc_i_set_err(
                    b, CKC_ERR_VALUE, "unsupported out_dtype %s", out_dtype ? out_dtype : "<null>");
                return NULL;
            }
        }
...
static void ckc_x_store_packed_chunk_local(
    ckc_ir_builder_t* b, ckc_value_t* qy_ptr, ckc_value_t* byte_off, ckc_value_t* packed, int n)
{
...
static void ckc_arn_set_reason(char* reason, size_t reason_cap, const char* msg)
{
                                             ...
                                             size_t reason_cap)
{
    ...
    if(spec == NULL)
    {
        ckc_arn_set_reason(reason, reason_cap, "null spec");
        return false;
    }
    if(arch == NULL)
    {
    ...
    if(target == NULL)
    {
        snprintf(buf,
                 ...
                 arch ? arch : "None",
                 arch ? "'" : "");
        ckc_arn_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    if(spec->block_size > ckc_arch_max_threads_per_block(target))
    {
        snprintf(buf,
                 sizeof(buf),
                 "block_size %d > max_threads_per_block %d on %s",
                 spec->block_size,
                 ckc_arch_max_threads_per_block(target),
                 arch);
        ckc_arn_set_reason(reason, reason_cap, buf);
        return false;
    }
    ...
    ckc_arn_set_reason(reason, reason_cap, "");
    return true;
}
 ...
 * ===================================================================== */
typedef struct ckc_arn_pass1_ctx
{
    const ckc_add_rmsnorm2d_rdquant_spec_t* spec;
    int VEC;
    /* tile windows / views */
    const ckc_tile_window_t* bt_tile;
    const ckc_tensor_view_t* g_view;
    const ckc_tile_window_t* x_tile;
    /* running partials (nonlocal s_sq / s_amax_g) */
    ckc_value_t* s_sq;
    ckc_value_t* s_amax_g;
    /* cached x*gamma (grows by VEC each chunk) */
    ckc_value_t** cached_xg;
    int num_cached_xg;
    int cap_cached_xg;
} ckc_arn_pass1_ctx_t;

/* tree_reduce combiners (ckc_combine_fn): forward to fadd / fmax. */
static ckc_value_t* ckc_arn_fadd_cb(ckc_ir_builder_t* b, ckc_value_t* a, ckc_value_t* c, void* user)
{
    (void)user;
    return ckc_b_fadd(b, a, c);
}
static ckc_value_t* ckc_arn_fmax_cb(ckc_ir_builder_t* b, ckc_value_t* a, ckc_value_t* c, void* user)
{
    (void)user;
...
static void ckc_arn_pass1_body(
    ckc_ir_builder_t* b, ckc_value_t* n_off, ckc_value_t* const* a_scalars, int vec, void* user)
{
    ...
    ckc_value_t* chunk_xg[8];
    ckc_value_t* chunk_sq[8];
    ckc_value_t* chunk_abs_xg[8];
    ckc_value_t* sq_red;
    ckc_value_t* amax_red;
    ckc_value_t* gidx[1];
     ...
     * the SSA-id counter advances exactly as in Python. */
    {
        ...
        idx[1] = n_off;
        ckc_tile_window_load_vec_as_f32(b, c->bt_tile, idx, 2, VEC, b_scalars);
    }
    /* g_scalars = g_view.load_vec_as_f32(b, [n_off], n=VEC) */
    gidx[0] = n_off;
    ckc_x_tensor_view_load_vec_as_f32(b, c->g_view, gidx, 1, VEC, g_scalars);
    ...
    if(c->spec->save_residual)
    {
        ...
        idx[0] = ckc_b_const_i32(b, 0);
        idx[1] = n_off;
        ckc_tile_window_store_vec_from_f32(b, c->x_tile, idx, 2, chunk_x, VEC);
    }
}
                                                  ...
                                                  const char* arch)
{
    ...
    ckc_tensor_view_t a_view, b_view, g_view, qy_view, x_view;
    ckc_tile_window_t a_tile, bt_tile, qy_tile, x_tile;
    ckc_tensor_view_t lds_sum_view, lds_max_view;
    ckc_value_t* lds_sum;
    ckc_value_t* lds_max;
    ...
    ckc_value_t* rms_q;
    ckc_value_t* c_vec;
    ckc_value_t* row_base_byte_off;
    bool use_packed_store;
    int chunks;
    int k;

    ckc_arn_pass1_ctx_t p1;
    ckc_value_t** cached_xg;
    ...
    {
        (void)ckc_i_set_err(b, CKC_ERR_VALUE, "invalid add_rmsnorm2d_rdquant spec: %s", reason);
        return NULL;
    }
    ...
    ckc_x_make_naive_tensor_view_packed(&a_view, A, two_d_shape, 2, io_ty);
    /* b_view = make_naive_tensor_view_packed(Bp, (1, N), io_ty) */
    ckc_x_make_naive_tensor_view_packed(&b_view, Bp, two_d_shape, 2, io_ty);
    /* g_view = make_global_view(Gamma, (N,), io_ty) */
    ckc_make_global_view(&g_view, Gamma, one_d_shape, 1, io_ty, NULL);
    /* qy_view = make_naive_tensor_view_packed(QY, (1, N), q_ty) */
    ckc_x_make_naive_tensor_view_packed(&qy_view, QY, two_d_shape, 2, q_ty);
    ...
    /* a_tile = make_tile_window(a_view, (1, N), origin=(row, const_i32(0))) */
    {
        ...
        origin[0] = row;
        origin[1] = ckc_b_const_i32(b, 0);
        ckc_make_tile_window(&bt_tile, &b_view, lengths, origin, 2);
        origin[0] = row;
        origin[1] = ckc_b_const_i32(b, 0);
        ckc_make_tile_window(&qy_tile, &qy_view, lengths, origin, 2);
    }
    ...
    if(spec->save_residual)
    {
        ...
        lengths[0] = 1;
        lengths[1] = N;
        ckc_x_make_naive_tensor_view_packed(&x_view, X, two_d_shape, 2, io_ty);
        origin[0] = row;
        origin[1] = ckc_b_const_i32(b, 0);
        ckc_make_tile_window(&x_tile, &x_view, lengths, origin, 2);
    }
    ...
    lds_shape[0] = BS;
    ckc_x_make_lds_view(b, &lds_sum_view, ckc_f32(), lds_shape, 1, "lds_sum");
    ckc_x_make_lds_view(b, &lds_max_view, ckc_f32(), lds_shape, 1, "lds_max");
    ...
    (void)ckc_block_lds_reduce_pair(b,
                                    p1.s_sq,
                                    p1.s_amax_g,
                                    lds_sum,
                                    lds_max,
    ...
    inv_rms = ckc_b_rsqrt(b, ckc_b_fadd(b, mean_sq, eps_rms));

    /* amax_y = b.fmul(inv_rms, total_amax_g) */
    amax_y = ckc_b_fmul(b, inv_rms, total_amax_g);
    /* safe_amax = b.fmax(amax_y, eps_q) */
    safe_amax = ckc_b_fmax(b, amax_y, eps_q);
    ...
    for(k = 0; k < chunks; ++k)
    {
        ...
        if(use_packed_store)
        {
            ...
            ckc_value_t* scaled_f32[8];
            ckc_value_t* packed;
            ckc_value_t* byte_off;
            int i;
            for(i = 0; i < VEC; ++i)
            {
                scaled_f32[i] = ckc_b_fmul(b, cached_xg[k * VEC + i], rms_q);
            }
            /* packed = pack_quant_chunk_local_f32(b, scaled_f32, q_ty, out_dtype) */
            packed = ckc_x_pack_quant_chunk_local_f32(b, scaled_f32, VEC, q_ty, spec->out_dtype);
            /* byte_off = b.add(row_base_byte_off, n_off) */
            byte_off = ckc_b_add(b, row_base_byte_off, n_off);
            /* store_packed_chunk_local(b, QY, byte_off, packed, n=VEC) */
            ckc_x_store_packed_chunk_local(b, QY, byte_off, packed, VEC);
        }
        else
        {
            int i;
            for(i = 0; i < VEC; ++i)
            {
                ...
                idx[0] = ckc_b_const_i32(b, 0);
                idx[1] = col;
                ckc_tile_window_store_scalar(b, &qy_tile, idx, 2, q, 0);
            }
        }
    }

    /* _ = store_scalar_from_f32  # public-API touch (no IR) */
    (void)ckc_b_store_scalar_from_f32;
...
/* ===================================================================== *
 *  ckc_build_add_rmsnorm2d_rdquant_new -- init builder + build.
 * ===================================================================== */
...
/* ===================================================================== *
 *  ckc_add_rmsnorm2d_rdquant_lower_to_llvm -- build + lower to .ll.
                                                     ...
                                                     size_t err_cap)
{
    ...
    kernel = ckc_build_add_rmsnorm2d_rdquant_new(&b, spec, arch);
    if(kernel == NULL)
    {
