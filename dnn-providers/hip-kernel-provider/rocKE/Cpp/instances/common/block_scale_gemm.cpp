                                             ...
                                             const char* arch)
{
    ...
    if(spec == NULL)
    {
        return (ckc_kernel_def_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "build_block_scale_gemm: null spec");
    }
    if(arch == NULL)
    {
        arch = "gfx950";
    ...
    {
        return (ckc_kernel_def_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "block_scale_gemm: no MFMA atom for spec");
    }
    ...
    {
        return (ckc_kernel_def_t*)ckc_i_set_err(
            b, CKC_ERR_VALUE, "block_scale_gemm: bad storage dtype");
    }
    ...
    ckc_b_region_enter(b, outer.body);
    {
        ...
        group_acc = ckc_mfma_k_loop(b,
                                    gk,
                                    atom,
                                    ckc_bsg_load_a_in_group,
                                    ckc_bsg_load_b_in_group,
                                                ...
                                                size_t err_cap)
{
    ...
    if(kernel == NULL)
    {
        ...
        if(err != NULL && err_cap > 0)
        {
            ...
            ckc_i_write_reason(err, err_cap, (m != NULL) ? m : "build_block_scale_gemm failed");
        }
