...
void ckc_sage_mfma_prologue(ckc_sage_mfma_ctx_t* ctx)
{
    ...
    ctx->q_tile_base = ckc_b_mul(b, ctx->q_tile_idx, ckc_b_const_i32(b, CKC_MFMA_ATTN_BLOCK_M));
...
void ckc_sage_mfma_fold_scales(ckc_sage_mfma_ctx_t* ctx)
{
    ...
    {
        ctx->q_block_idx = ckc_sage_magic_div(b, ctx->q_tile_base, ctx->spec->q_scale.scale_block);
    }
    else
    {
    ...
    ctx->q_scale_v = ckc_b_load_q_scale_for_block(
    ...
    if(ctx->spec->k_scale.layout == CKC_QK_SCALE_PER_HEAD)
    {
        ...
        ctx->k_scale_const = ckc_b_load_k_scale_for_block(b,
                                                          ctx->k_scale_ptr,
                                                          &ctx->spec->k_scale,
                                                          ctx->batch_idx,
                                                          ctx->kv_head_idx,
    ...
    ckc_ir_builder_t* b, ckc_value_t* score_log2, ckc_value_t* kt, int row_in_atom, void* user)
{
    ...
    k_scale_v = ckc_b_load_k_scale_for_block(
