...
/*
 * instance_pooling.c -- C99 port of ck_dsl/instances/common/pooling.py.
 *
 * Ported symbols:
 *   PoolingProblem (+ Ho/Wo/total_out/short)   ckc_pooling_problem_*
 *   Pooling2DSpec (+ kernel_name)              ckc_pooling2d_spec_* / _kernel_name
 *   is_valid_spec                              ckc_pooling2d_is_valid_spec
 *   build_pooling2d                            ckc_build_pooling2d (+ _new)
 *   pooling2d_grid                             ckc_pooling2d_grid
 *   pooling2d_signature                        ckc_pooling2d_signature
 *   (build + lower convenience)                ckc_pooling2d_lower_to_llvm
 *
 ...
 * (helper_ck_dsl.helpers.distribution.{h,c}); the remaining five helpers are
...
/* NB: the rich named-coord ckc_tensor_descriptor (helpers.transforms.h, already
 * included above) and the simple shape/stride descriptor in
 * helpers.tensor_view.h share the struct tag ``ckc_tensor_descriptor`` but are
...
typedef struct ckc_buffer_view
{
    ckc_buffer_resource_t* rsrc;
    int rank;
    int shape[CKC_POOL_TV_MAX_RANK];
    int strides[CKC_POOL_TV_MAX_RANK]; /* packed row-major element strides     */
    const ckc_type_t* dtype;
} ckc_buffer_view_t;

/* helpers/tensor_view.py: TileWindow over a buffer view (lengths + origin). */
typedef struct ckc_buffer_window
{
    const ckc_buffer_view_t* view;
    int lengths[CKC_POOL_TV_MAX_RANK];
    ckc_value_t* origin[CKC_POOL_TV_MAX_RANK];
    int rank;
} ckc_buffer_window_t;

/* make_static_distributed_tensor(distribution, dtype): shared distribution-port
 * symbol (defined in helper_ck_dsl.helpers.distribution.c, declared in its
...
static ckc_buffer_view_t* ckc_make_buffer_view(ckc_ir_builder_t* b,
                                               ckc_buffer_resource_t* rsrc,
                                               const int* lengths,
                                               int n_lengths,
                                               const ckc_type_t* dtype);
...
static void* ckc_buffer_view_tile(ckc_ir_builder_t* b,
                                  ckc_buffer_view_t* view,
                                  const int* lengths,
                                  int n_lengths,
...
static void ckc_static_distributed_tensor_set(ckc_ir_builder_t* b,
...
static void ckc_store_tile(ckc_ir_builder_t* b,
                           void* window,
                           ckc_static_distributed_tensor_t* dt,
                           ckc_value_t* const* ps,
                           int n_ps);
...
{
    return p->N * ckc_pooling_problem_ho(p) * ckc_pooling_problem_wo(p) * p->C;
}
...
ckc_status_t ckc_pooling2d_kernel_name(const ckc_pooling2d_spec_t* spec, char* out, size_t out_cap)
{
    ...
    st = ckc_pooling_problem_short(&spec->problem, shortbuf, sizeof(shortbuf));
    if(st != CKC_OK)
    {
        return st;
    }
...
#define POOL_REASON(...)                               \
    ...
    Ho = ckc_pooling_problem_ho(p);
    Wo = ckc_pooling_problem_wo(p);
    if(Ho <= 0 || Wo <= 0)
    {
...
static ckc_tensor_descriptor_t* pool_make_input_descriptor(ckc_ir_builder_t* b,
                                                           const ckc_pooling_problem_t* p)
{
    int lengths[4];
    ...
    base = ckc_tensor_descriptor_naive(b, "X_nhwc", lengths, 4, /*strides*/ NULL, coord_names, 4);
    if(base == NULL)
    {
        return NULL;
    }
    ...
    chain[0] = e_h;
    chain[1] = e_w;
    return ckc_tensor_descriptor_transform(b, base, chain, 2);
}

/* _neutral_value(b, op): f32 neutral element. */
static ckc_value_t* pool_neutral_value(ckc_ir_builder_t* b, const char* op)
{
    if(strcmp(op, "max") == 0)
    {
        return ckc_b_const_f32(b, -INFINITY); /* float("-inf") */
    }
    /* sum / avg -> 0.0 */
...
pool_combine(ckc_ir_builder_t* b, const char* op, ckc_value_t* acc, ckc_value_t* x)
{
    if(strcmp(op, "max") == 0)
    {
        return ckc_b_fmax(b, acc, x);
    }
    /* sum / avg -> fadd */
    return ckc_b_fadd(b, acc, x);
}
...
static int ckc_pool_dtype_elem_bytes(const ckc_type_t* dtype)
{
                                               ...
                                               const ckc_type_t* dtype)
{
    ...
    if(b == NULL || rsrc == NULL)
        return NULL;
    if(n_lengths <= 0 || n_lengths > CKC_POOL_TV_MAX_RANK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "make_buffer_view: bad rank");
        return NULL;
    }
...
static void* ckc_buffer_view_tile(ckc_ir_builder_t* b,
                                  ...
                                  int n_origin)
{
    ckc_buffer_window_t* w;
    int i;
    if(b == NULL || view == NULL)
        return NULL;
    if(n_lengths != view->rank || n_origin != view->rank || n_lengths > CKC_POOL_TV_MAX_RANK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "TensorView.tile: rank mismatch");
        return NULL;
    }
    w = (ckc_buffer_window_t*)ckc_arena_calloc(&b->arena, sizeof(ckc_buffer_window_t));
    if(w == NULL)
        return NULL;
    w->view = view;
    w->rank = n_lengths;
...
static ckc_value_t* ckc_pool_buffer_offset(ckc_ir_builder_t* b,
                                           const ckc_buffer_view_t* v,
                                           ...
                                           int num_indices)
{
    ...
    for(i = 0; i < v->rank; ++i)
    {
        ...
        if(v->strides[i] == 1)
            ...
            term = ckc_b_mul(b, indices[i], ckc_b_const_i32(b, v->strides[i]));
...
static int ckc_pool_y_length(const ckc_tile_distribution_encoding_t* e, int yi)
{
...
static int ckc_pool_y_to_linear(const ckc_tile_distribution_encoding_t* e, const int* y, int n_y)
{
    int off = 0;
    int i;
    for(i = 0; i < n_y; ++i)
        off = off * ckc_pool_y_length(e, i) + y[i];
    return off;
}
                                              ...
                                              ckc_value_t* value)
{
    ...
    if(dt == NULL || dt->distribution == NULL || dt->distribution->encoding == NULL)
        return;
    off = ckc_pool_y_to_linear(dt->distribution->encoding, y, n_y);
...
static int ckc_pool_scalar_per_vector(const ckc_tile_distribution_encoding_t* e)
{
    int full_len;
    int spv;
...
static void ckc_store_tile(ckc_ir_builder_t* b,
                           ...
                           int n_ps)
{
    const ckc_buffer_window_t* w = (const ckc_buffer_window_t*)window;
    const ckc_tile_distribution_t* dist;
    const ckc_tile_distribution_encoding_t* enc;
    const ckc_type_t* dt_ty;
    const char* tyname;
    int spv;
    int num_x;
    int elem_bytes;
    ckc_value_t* ys[CKC_POOL_TV_MAX_RANK];
    ckc_value_t* x_coords[CKC_POOL_TV_MAX_RANK];
    ckc_value_t* gidx[CKC_POOL_TV_MAX_RANK];
    ckc_value_t* scalars[8];
    ckc_value_t* casts[8];
    ckc_value_t* off_elem;
    ...
    spv = ckc_pool_scalar_per_vector(enc);
    for(i = 0; i < enc->num_Y; ++i)
        ys[i] = ckc_b_const_i32(b, 0);
    if(!ckc_tile_distribution_calculate_x(b,
                                          dist,
                                          ys,
                                          enc->num_Y,
                                          /*ps*/ NULL,
    ...
    for(i = 0; i < spv; ++i)
    {
        int yfull[CKC_POOL_TV_MAX_RANK];
        int j;
        int off;
    ...
    for(i = 0; i < spv; ++i)
        casts[i] = ckc_b_cast_f32_to(b, scalars[i], dt_ty);
    ...
    {
        ckc_value_t* packed = ckc_b_vec_pack(b, casts, spv, dt_ty);
        if(num_x != w->rank)
            return;
        for(i = 0; i < num_x; ++i)
            gidx[i] = ckc_b_add(b, w->origin[i], x_coords[i]);
...
ckc_build_pooling2d(ckc_ir_builder_t* b, const ckc_pooling2d_spec_t* spec, const char* arch)
{
    return ckc::guard_builder(b, [&]() -> ckc_kernel_def_t* {
        ...
        {
            ckc_i_set_err(b, CKC_ERR_KEY, "pooling2d: unmerge_lower missing decoded coord");
            return NULL;
        }
                                     ...
                                     size_t* out_count)
{
    ...
    st = ckc_signature_builder_init(&sb, (ckc_arena_t*)arena);
    if(st != CKC_OK)
    {
        return st;
    }
...
/* ===================================================================== *
 * ckc_pooling2d_lower_to_llvm -- build + lower to .ll convenience.
                                         ...
                                         size_t err_cap)
{
    ...
    kernel = ckc_build_pooling2d_new(&b, spec, arch);
    if(kernel == NULL)
    {
