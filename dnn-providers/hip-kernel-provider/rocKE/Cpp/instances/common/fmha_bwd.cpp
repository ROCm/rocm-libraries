...
ckc_status_t ckc_fmha_bwd_kernel_name(const ckc_fmha_bwd_spec_t* spec, char* out, size_t out_cap)
{
    ...
    return ckc_kernel_name_join(spec->name, parts, 7, NULL, NULL, 0, out, out_cap, NULL);
}
                                     ...
                                     const char* arch)
{
    return ckc::guard_builder(ckc_fmha_kernel_builder_builder(kb), [&]() -> ckc_kernel_def_t* {
        ...
        if(!ckc_fmha_bwd_is_valid_spec(spec, arch, reason, sizeof(reason)))
        {
            ...
            {
                ckc_i_set_err(&kb->b, CKC_ERR_VALUE, "invalid fmha_bwd spec: %s", reason);
            }
            return NULL;
        }
                                    ...
                                    size_t* out_count)
{
    ...
    st = ckc_fmha_kernel_builder_init(kb, "ck_dsl_fmha_bwd_sig_probe", &spec->common);
    if(st != CKC_OK)
    {
        return st;
    }
