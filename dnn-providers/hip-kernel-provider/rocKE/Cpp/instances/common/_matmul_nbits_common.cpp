...
/*
 * C99 port of ck_dsl/instances/common/_matmul_nbits_common.py (Milestone-1
 * surface: spec, validator, signature, grid, 64-row outer-loop driver).
 *
 * See ckc/helper_ck_dsl.instances.common._matmul_nbits_common.h for the symbol
...
bool ckc_matmul_nbits_family_known(const char* family)
{
    ...
    for(f = ckc_matmul_nbits_families(); *f != NULL; ++f)
    {
                                           ...
                                           size_t reason_cap)
{
    ...
    if(spec == NULL)
    {
        CKC_NBITS_REJECT("spec is NULL");
    }
    if(arch == NULL)
    {
        arch = CKC_MATMUL_NBITS_V1_ARCH;
    }
    ...
    if(target == NULL)
    {
        CKC_NBITS_REJECT("'%s'", arch);
    }

    /* v1 contract gates. */
    if(!ckc_matmul_nbits_arch_supported(arch))
    {
        /* sorted(SUPPORTED_ARCHES) -> ['gfx1151', 'gfx1201']. */
        CKC_NBITS_REJECT("matmul_nbits v1 supports ['gfx1151', 'gfx1201'] only (got '%s')", arch);
    }
    if(!ckc_matmul_nbits_family_known(spec->family))
    {
        CKC_NBITS_REJECT("unknown family '%s'; expected one of ('large_n', 'skinny_n', "
                         "'decode_gemv')",
                         spec->family != NULL ? spec->family : "(null)");
    }
    if(spec->group_size != CKC_MATMUL_NBITS_V1_GROUP_SIZE)
    {
        CKC_NBITS_REJECT("matmul_nbits v1 fixes group_size=%d (got %d)",
                         CKC_MATMUL_NBITS_V1_GROUP_SIZE,
                         spec->group_size);
    }
    if(spec->zero_points)
    {
        CKC_NBITS_REJECT("matmul_nbits v1 is signed-symmetric int4 only; zero_points not "
                         "yet supported");
    }
    if(spec->packing == NULL || strcmp(spec->packing, "row_k_contiguous") != 0)
    {
        CKC_NBITS_REJECT("matmul_nbits v1 ships packing='row_k_contiguous' only (got '%s')",
                         spec->packing != NULL ? spec->packing : "(null)");
    }
    if(ckc_matmul_nbits_scale_wire_dtype(spec->scale_dtype, &scale_wire) != CKC_OK)
    {
        CKC_NBITS_REJECT("scale_dtype must be fp16 / fp32, got '%s'",
    ...
    {
        CKC_NBITS_REJECT("N / K must be positive (got N=%d, K=%d)", spec->N, spec->K);
    }
    if(spec->K % spec->group_size)
    {
        CKC_NBITS_REJECT("K (%d) must be divisible by group_size (%d)", spec->K, spec->group_size);
    }
    ...
    if(strcmp(family_str, "decode_gemv") != 0)
    {
        ...
        if(!ckc_archtarget_supports_dtype_combo(
               target, CKC__WMMA_AB_DTYPE, CKC__WMMA_AB_DTYPE, CKC__WMMA_C_DTYPE, mma_family))
        {
            CKC_NBITS_REJECT("unsupported matmul_nbits dtype fp16 on %s", arch);
        }
        if(ckc_archtarget_op_for_shape(target,
                                       mma_family,
                                       CKC__WMMA_AB_DTYPE,
                                       CKC__WMMA_AB_DTYPE,
                                       CKC__WMMA_C_DTYPE,
        ...
        {
            CKC_NBITS_REJECT("tile_m not divisible by warp_m * warp_tile_m");
        }
        if(t->tile_n % (t->warp_n * t->warp_tile_n))
        {
            CKC_NBITS_REJECT("tile_n not divisible by warp_n * warp_tile_n");
        }
        if(t->tile_k % t->warp_tile_k)
        {
            CKC_NBITS_REJECT("tile_k not divisible by warp_tile_k");
        }
        if(spec->N % t->tile_n)
        {
            CKC_NBITS_REJECT("N (%d) must be divisible by tile_n (%d)", spec->N, t->tile_n);
        }
        if(spec->K % t->tile_k)
        {
            CKC_NBITS_REJECT("K (%d) must be divisible by tile_k (%d)", spec->K, t->tile_k);
        }
    }

    /* block_size = warp_m * warp_n * warp_k * wave_size. */
    expected_bs = ckc_warp_tile_block_size(t->warp_m, t->warp_n, t->warp_k, spec->wave_size);
    ...
    {
        CKC_NBITS_REJECT("block_size %d > %d (hardware cap) on %s",
                         spec->block_size,
    ...
    {
        CKC_NBITS_REJECT("seq_len_tile must be positive (got %d)", spec->seq_len_tile);
    }
                                        ...
                                        size_t* out_count)
{
    ...
    ckc_signature_builder_ptr(&sb, "B", "i8", NULL);
    ckc_signature_builder_ptr(&sb, "Scales", scale_wire, NULL);
    ckc_signature_builder_ptr(&sb, "C", "f16", NULL);
