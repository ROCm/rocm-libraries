    ...
    for(mi = 0; mi < mfmas_m; ++mi)
    {
        for(ni = 0; ni < mfmas_n; ++ni)
        {
            ...
            for(i = 0; i < atom->c_per_lane; ++i)
            {
                ...
                row = ckc_b_add(
                    b, warp_m_off, ckc_b_add(b, ckc_b_const_i32(b, mi * atom->m), row_in));
                col = ckc_b_add(
...
{
    ckc_ir_builder_t* b = ctx->b;
    ckc_value_t* kbase = ckc_b_add(b, k_group_base, ckc_b_mul(b, ckc_b_const_i32(b, kk), c_atom_k));
                ...
                for(ni = 0; ni < nni; ++ni)
                {
                    ...
                    }
                    {
                        ...
                        gb_all[ni] = ckc_moe_fp8_dtla_read_b_fp8(ctx,
                                                                 ...
                                                                 dtla->warp_row_base,
                                                                 dtla->wave_size);
                        ub_all[ni] = ckc_moe_fp8_dtla_read_b_fp8(ctx,
                                                                 dtla->view,
                                                                 2 * pair + 1,
                                                                 dtla->lane,
                                                                 dtla->warp_row_base,
