    ...
    for(kk = 0; kk < k_atoms; ++kk)
    {
        ...
        for(mi = 0; mi < mfmas_m; ++mi)
        {
            ...
            ckc_value_t* a_row = ckc_b_add(
                ...
                warp_m_off,
                ckc_b_add(
                    b, ckc_b_const_i32(b, mi * t->warp_tile_m + a_row_parity_off), m_in_atom));
            if(a_par_row_v != NULL)
                a_row = ckc_b_add(b, a_row, a_par_row_v);
            a_rows[mi] = ckc_gemm_emit_smem_load(b,
                                                 A_src,
                                                 a_row,
                                                 ckc_gemm_swz_col(ctx, col_base, a_row),
                                                 ctx->a_per_lane,
                                                 ctx->storage_dtype);
        }
        ...
        for(ni = 0; ni < mfmas_n; ++ni)
        {
            ...
            ckc_value_t* b_row = ckc_b_add(
                ...
                warp_n_off,
                ckc_b_add(
                    b, ckc_b_const_i32(b, ni * t->warp_tile_n + b_row_parity_off), n_in_atom));
            if(b_par_row_v != NULL)
                b_row = ckc_b_add(b, b_row, b_par_row_v);
            b_cols[ni] = ckc_gemm_emit_smem_load(b,
                                                 B_src,
                                                 b_row,
                                                 ckc_gemm_swz_col(ctx, col_base, b_row),
                                                 ctx->b_per_lane,
                                                 ctx->storage_dtype);
        }
