                                            ...
                                            size_t reason_cap)
{
    ...
    if(l->swizzle != NULL)
    {
        CKC_LDS_REJECT("async LDS layout cannot use arbitrary per-lane swizzle; "
                                                      ...
                                                      size_t reason_cap)
{
    ...
    {
        CKC_LDS_REJECT("spec.lds_layout override is not supported in the C port "
...
#define CKC_CSPEC_REJECT(...)                          \
    ...
    {
        CKC_CSPEC_REJECT("tile_m %d not divisible by warp_m * warp_tile_m (%d * %d)",
                         s->tile_m,
                         s->warp_m,
                         s->warp_tile_m);
    }
    ...
    {
        CKC_CSPEC_REJECT("tile_n %d not divisible by warp_n * warp_tile_n (%d * %d)",
                         s->tile_n,
                         s->warp_n,
                         s->warp_tile_n);
    }
    ...
    {
        CKC_CSPEC_REJECT("tile_k %d not divisible by warp_tile_k %d", s->tile_k, s->warp_tile_k);
    }
     ...
     * ("async LDS layout must be packed: k_pad must be 0"), not the guard's. */
    {
        ...
        if(!ckc_implicit_gemm_conv_spec_effective_lds_layout(s, &layout, reason, reason_cap))
        {
            return false;
        }
        if(s->async_dma && !ckc_conv_lds_layout_validate_for_async(&layout, reason, reason_cap))
        {
            return false;
        }
    }
    ...
    {
        CKC_CSPEC_REJECT("async_dma requires lds_k_pad to be 0/None because "
       ...
       s->acc_epilogue.clamp_min > s->acc_epilogue.clamp_max)
    {
        ...
        CKC_CSPEC_REJECT("acc_epilogue clamp_min must be <= clamp_max (got %s > %s)", lo, hi);
    }
...
#define CKC_CONVVS_REJECT(...)                         \
    ...
    if(strcmp(family, "wmma") == 0)
    {
        ...
        {
            CKC_CONVVS_REJECT("WMMA conv supports only the 'mem' pipeline (got '%s') on %s",
                              s->pipeline ? s->pipeline : "",
                              arch);
        }
        if(!(s->epilogue && strcmp(s->epilogue, "default") == 0))
        {
            CKC_CONVVS_REJECT("WMMA conv supports only the 'default' epilogue (got '%s') on %s",
                              s->epilogue ? s->epilogue : "",
                              arch);
        }
        if(s->async_dma)
                                       ...
                                       const char* arch)
{
    ...
    op = ckc_archtarget_op_for_shape(target,
                                     ckc_conv_mma_family(arch),
                                     "f16",
                                     "f16",
                                     "fp32",
                                     spec->warp_tile_m,
