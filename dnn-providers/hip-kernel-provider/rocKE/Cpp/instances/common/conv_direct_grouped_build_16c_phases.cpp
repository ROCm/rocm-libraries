 ...
 * (ck_dsl/instances/common/conv_direct_grouped.py, lines 256-519 + 637-641).
 *
 * Implements exactly four phase functions over ckc_dconv_16c_ctx_t:
 *   ckc_dconv16c_prologue          (Python 256-355)
 *   ckc_dconv16c_load_weights      (Python 357-415)
 *   ckc_dconv16c_build_chunk_meta  (Python 444-473)
 *   ckc_dconv16c_build_descriptors (Python 475-519, 637-641)
 *
...
bool ckc_dconv16c_prologue(ckc_dconv_16c_ctx_t* ctx)
{
    ...
    const ckc_direct_conv_16c_spec_t* spec = ctx->spec;
    char reason[CKC_ERR_MSG_CAP];

    /* spec.validate() -- hard invariant assertions. */
    if(ckc_direct_conv_16c_validate(spec, reason, sizeof reason) != CKC_OK)
    {
        if(b->status == CKC_OK)
            b->status = CKC_ERR_VALUE; /* mirror Python AssertionError -> sticky error */
        return false;
    }

    /* ok, why = is_valid_spec_16c(spec, arch=arch); if not ok: raise ValueError */
    if(!ckc_direct_conv_16c_is_valid_spec(spec, ctx->arch, reason, sizeof reason))
    {
        if(b->status == CKC_OK)
    ...
    ctx->THREADS = ckc_direct_conv_16c_threads_per_block(spec);
    ...
    {
        if(b->status == CKC_OK)
            b->status = CKC_ERR_VALUE; /* "LDS row too small for one vec4 per thread" */
        return false;
    }
    ...
    ckc_attr_set_int(b, &b->kernel->attrs, "max_workgroup_size", ctx->THREADS);

    /* ---- params ---- */
    {
        const ckc_type_t* f16ptr = ckc_ptr_type(b, ckc_f16(), "global");
        ckc_param_opts_t ro;
        ckc_param_opts_t wo;
        ckc_param_opts_t none;

        /* A = b.param("A", PtrType(F16,"global"), noalias=True, readonly=True, align=16) */
        ro = (ckc_param_opts_t){0};
        ro.noalias = true;
        ro.noalias_set = true;
        ro.readonly = true;
        ...
        ro.align = 16;
        ro.align_set = true;
        ctx->A = ckc_b_param(b, "A", f16ptr, &ro);
        ctx->Bp = ckc_b_param(b, "B", f16ptr, &ro);

        /* D = b.param("D", PtrType(F16,"global"), noalias=True, writeonly=True, align=16) */
        wo = (ckc_param_opts_t){0};
        wo.noalias = true;
        wo.noalias_set = true;
        wo.writeonly = true;
        ...
        wo.align = 16;
        wo.align_set = true;
        ctx->D = ckc_b_param(b, "D", f16ptr, &wo);

        none = (ckc_param_opts_t){0};
        ctx->A_bytes = ckc_b_param(b, "A_bytes", ckc_i32(), &none);
        ctx->B_bytes = ckc_b_param(b, "B_bytes", ckc_i32(), &none);
        ctx->D_bytes = ckc_b_param(b, "D_bytes", ckc_i32(), &none);
    }

    /* ---- common SSA constants ---- */
    ctx->c0 = ckc_b_const_i32(b, 0);
    ctx->c_wave = ckc_b_const_i32(b, ctx->WAVE);
    ctx->c_BG = ckc_b_const_i32(b, ctx->BLOCK_GROUPS);
    ctx->c_BQ = ckc_b_const_i32(b, ctx->BLOCK_Q);
    ctx->c_cpg = ckc_b_const_i32(b, ctx->p.cpg);
    ctx->c_kpg = ckc_b_const_i32(b, ctx->p.kpg);
    ctx->c_W = ckc_b_const_i32(b, ctx->p.W);

    /* c_BG_cpg = b.const_i32(BLOCK_GROUPS * p.cpg) */
    ctx->c_BG_cpg = ckc_b_const_i32(b, ctx->BLOCK_GROUPS * ctx->p.cpg);

    /* ---- thread / wave / lane decode ---- */
    ctx->tid = ckc_b_thread_id_x(b);
    ctx->wave_id = ckc_b_div(b, ctx->tid, ctx->c_wave);
    ctx->lane = ckc_b_mod(b, ctx->tid, ctx->c_wave);
    ctx->c4 = ckc_b_div(b, ctx->lane, ckc_b_const_i32(b, 16)); /* 0..3 */
    ctx->q_in_lane = ckc_b_mod(b, ctx->lane, ckc_b_const_i32(b, 16)); /* 0..15 */
    ctx->s_lane_k32 = ckc_b_div(b, ctx->c4, ckc_b_const_i32(b, 2));
    ...
    {
        ckc_value_t* mod_c4_2 = ckc_b_mod(b, ctx->c4, ckc_b_const_i32(b, 2));
        ctx->ch_lane_k32 = ckc_b_mul(b, mod_c4_2, ckc_b_const_i32(b, 8));
    }
    ctx->ch_lane_k16 = ckc_b_mul(b, ctx->c4, ckc_b_const_i32(b, 4));
    ...
    ctx->g_tile = ctx->by;
    ctx->g = ckc_b_add(b, ckc_b_mul(b, ctx->g_tile, ctx->c_BG), ctx->wave_id);
    ctx->q_tile_start = ckc_b_mul(b, ctx->bx, ctx->c_BQ);
    ...
    ctx->lds_total_fp16 = ctx->PASSES * ctx->THREADS * ctx->LOAD_VEC;
    {
        ...
        ctx->A_smem = ckc_b_smem_alloc(b, ckc_f16(), shape, 2, "lds_a");
        if(spec->double_buffer)
            ctx->B_smem = ckc_b_smem_alloc(b, ckc_f16(), shape, 2, "lds_b");
        else
...
{
    ckc_ir_builder_t* b = ctx->b;
    int total_k = ckc_direct_conv_problem_total_k(&ctx->p);
    ...
    ctx->k_out_val = ckc_b_add(b, ckc_b_mul(b, ctx->g, ctx->c_kpg), ctx->q_in_lane);
    ...
    else
    {
        ...
        for(r_const = 0; r_const < ctx->p.KH; ++r_const)
        {
            ...
            {
                ckc_value_t* r_i = ckc_b_const_i32(b, r_const);
                ckc_value_t* s_i = ckc_b_const_i32(b, s_const);
                ...
                ctx->weights[ctx->n_weights++] = ckc_b_buffer_load_vN_f16(
 ...
 * ===================================================================== */
void ckc_dconv16c_build_chunk_meta(ckc_dconv_16c_ctx_t* ctx)
{
    ckc_ir_builder_t* b = ctx->b;
    static const char* const coord_names[3] = {"W_lds", "group_in_wg", "ch_block"};
    int lengths[3];
    int pass_idx;
    ckc_tensor_descriptor_t* naive;

    lengths[0] = ctx->LDS_W;
    lengths[1] = ctx->BLOCK_GROUPS;
    lengths[2] = 4;

    naive = ckc_tensor_descriptor_naive(b, "chunk_unmerge", lengths, 3, NULL, coord_names, 3);

    {
        const ckc_transform_t* xforms[1];
        xforms[0] = ckc_unmerge_magic(b, "chunk_idx", coord_names, 3, lengths);
        ctx->chunk_desc = ckc_tensor_descriptor_transform(b, naive, xforms, 1);
    }
    ...
    for(pass_idx = 0; pass_idx < ctx->PASSES; ++pass_idx)
    {
        ckc_value_t* chunk_idx
            = ckc_b_add(b, ctx->tid, ckc_b_const_i32(b, pass_idx * ctx->THREADS));
        ...
        ckc_value_t* ch_block = NULL;
        ckc_value_t* group_in_wg = NULL;
        ckc_value_t* W_lds = NULL;
        int n_out, i;

        in_values[0] = chunk_idx;
        n_out = ckc_tensor_descriptor_unmerge_lower(b,
                                                    ctx->chunk_desc,
                                                    in_names,
                                                    in_values,
                                                    1,
        ...
        ctx->chunk_meta[pass_idx].in_bounds
            = ckc_b_cmp_lt(b, chunk_idx, ckc_b_const_i32(b, ctx->NUM_VEC4));
        ctx->chunk_meta[pass_idx].abs_group
            = ckc_b_add(b, ckc_b_mul(b, ctx->g_tile, ctx->c_BG), group_in_wg);
        ctx->n_chunk_meta++;
    }
}
 ...
 * ===================================================================== */
void ckc_dconv16c_build_descriptors(ckc_dconv_16c_ctx_t* ctx)
{
    ckc_ir_builder_t* b = ctx->b;
    int total_c = ckc_direct_conv_problem_total_c(&ctx->p);
    int total_k = ckc_direct_conv_problem_total_k(&ctx->p);
    ...
    /* ---- input descriptor A[N,H,W,total_c] + 2 embeds ---- */
    {
        ...
        int a_lengths[4];
        ckc_tensor_descriptor_t* a_naive;
        ...
        a_lengths[2] = ctx->p.W;
        a_lengths[3] = total_c;
        a_naive = ckc_tensor_descriptor_naive(b, "A", a_lengths, 4, NULL, a_coords, 4);

        /* embed(upper=("y_iter",), into="h", strides=(1,), offset=-PAD, lo=0, hi=H) */
        {
            static const char* const h_upper[1] = {"y_iter"};
            int h_strides[1] = {1};
            xforms[0] = ckc_embed_bounded(b, h_upper, 1, "h", h_strides, -ctx->p.PAD, 0, ctx->p.H);
        }
         ...
         *       offset=-PAD, lo=0, hi=W) */
        {
            ...
            int w_strides[2] = {1, 1};
            xforms[1] = ckc_embed_bounded(b, w_upper, 2, "w", w_strides, -ctx->p.PAD, 0, ctx->p.W);
        }
    ...
    /* ---- output descriptor D[N,H,W,total_k] naive ---- */
    {
        ...
        d_lengths[2] = ctx->p.W;
        d_lengths[3] = total_k;
        ctx->d_desc = ckc_tensor_descriptor_naive(b, "D", d_lengths, 4, NULL, d_coords, 4);
    }
}
