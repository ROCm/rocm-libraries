 ...
 * (ck_dsl/instances/common/fused_moe.py, lines 449-573 + 587-708).
 *
 * Scope of THIS translation unit (both SwiGLU activation builders over the
 * shared ckc_moe_stream_ctx_t):
 *   ckc_moe_silu_mul_prologue         -- lines 491-523
 *   ckc_moe_silu_mul_body_scalar      -- lines 526-538 (VEC==1 fallback)
 *   ckc_moe_silu_mul_body_tile        -- lines 540-573 (CK-Tile dist path)
 *   ckc_moe_silu_mul_packed_prologue  -- lines 612-648
 *   ckc_moe_silu_mul_packed_body_scalar -- lines 658-670 (VEC==1 fallback)
 *   ckc_moe_silu_mul_packed_body_tile -- lines 674-708 (CK-Tile dist path)
 *
 * The builder-call sequence is byte-identical to the Python source so a lowered
 * .ll diffs clean. Peers resolved via their headers: ckc_moe_effective_vec,
 * ckc_moe_chunk_distribution, ckc_moe_silu_mul_f32 (internal header),
 * ckc_fused_moe_is_valid_spec, ckc_fused_moe_spec_elems_per_thread_inter,
 * ckc_io_ir_type, ckc_b_load_scalar_as_f32, ckc_b_store_scalar_from_f32.
 *
 * TILE PATH NOTE.
 ...
 *  ckc_ew_load_tile / ckc_ew_store_tile.
 * ===================================================================== */
...
static int ckc_moe_silu_load_tile(ckc_ir_builder_t* b,
 ...
 */
static void ckc_moe_silu_store_tile(ckc_ir_builder_t* b,
                                    const ckc_tile_window_t* window,
                                    const ckc_tile_distribution_t* dist,
                                    ckc_value_t* tid,
                                    int vec,
                                    ckc_value_t** storage)
{
    ...
    int ps_counts[1];
    ckc_value_t* x_coords[1];
    ckc_value_t* casts[CKC_TV_MAX_VEC];
    ckc_value_t* packed;
...
bool ckc_moe_silu_mul_prologue(ckc_moe_stream_ctx_t* ctx)
{
    ...
    /* ok, why = is_valid_spec(spec); if not ok: raise ValueError(...) */
    {
        ...
        {
            ckc_i_set_err(b, CKC_ERR_VALUE, "invalid fused_moe spec: %s", why);
            return false;
        }
    }
    ...
    ctx->p_inter = ckc_b_param(b, "intermediate", ckc_i32(), NULL);
    ...
    ctx->row_base = ckc_b_mul(b, ctx->bid, ckc_b_const_i32(b, ctx->N));
...
ckc_kernel_def_t* ckc_moe_silu_mul_body_scalar(ckc_moe_stream_ctx_t* ctx)
{
    ...
    for(k = 0; k < ctx->chunks; ++k)
    {
        ...
        ckc_value_t* g = ckc_b_load_scalar_as_f32(b, ctx->GateOut, off, dtype);
        /* u = load_scalar_as_f32(b, UpOut, off, dtype=dtype) */
        ckc_value_t* u = ckc_b_load_scalar_as_f32(b, ctx->UpOut, off, dtype);
        /* h = _silu_mul_f32(b, g, u, one_f32=one_f32, c_neg_log2e=c_neg_log2e) */
        ckc_value_t* h = ckc_moe_silu_mul_f32(b, g, u, ctx->one_f32, ctx->c_neg_log2e);
        /* store_scalar_from_f32(b, Hidden, off, h, dtype=dtype) */
        ckc_b_store_scalar_from_f32(b, ctx->Hidden, off, h, dtype);
    }
...
ckc_kernel_def_t* ckc_moe_silu_mul_body_tile(ckc_moe_stream_ctx_t* ctx)
{
    ...
    ctx->distribution = ckc_moe_chunk_distribution(b, BS, VEC);
    ...
    if(ckc_make_global_view(&ctx->gate_view, ctx->GateOut, shape1, 1, ctx->ty, NULL) != CKC_OK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "%s", "make_global_view(GateOut) failed");
        return NULL;
    }
    if(ckc_make_global_view(&ctx->up_view, ctx->UpOut, shape1, 1, ctx->ty, NULL) != CKC_OK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "%s", "make_global_view(UpOut) failed");
        return NULL;
    }
    if(ckc_make_global_view(&ctx->out_view, ctx->Hidden, shape1, 1, ctx->ty, NULL) != CKC_OK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "%s", "make_global_view(Hidden) failed");
        return NULL;
    }
    ...
    for(k = 0; k < ctx->chunks; ++k)
    {
        ...
        int lengths1[1];
        ckc_tile_window_t gate_tile, up_tile, out_tile;
        ckc_value_t* g_dt[CKC_TV_MAX_VEC];
        ckc_value_t* u_dt[CKC_TV_MAX_VEC];
        ckc_value_t* out_dt[CKC_TV_MAX_VEC];
        int y;
        ...
        if(!ckc_moe_silu_load_tile(b, &gate_tile, ctx->distribution, ctx->tid, VEC, g_dt))
        {
            return NULL;
        }
        /* u_dt = up_tile.load(b, distribution=distribution, ps=ps) */
        if(!ckc_moe_silu_load_tile(b, &up_tile, ctx->distribution, ctx->tid, VEC, u_dt))
        {
            return NULL;
        }
        ...
        for(y = 0; y < VEC; ++y)
        {
            out_dt[y] = ckc_moe_silu_mul_f32(b, g_dt[y], u_dt[y], ctx->one_f32, ctx->c_neg_log2e);
        }
        /* out_tile.store(b, out_dt, ps=ps) */
        ckc_moe_silu_store_tile(b, &out_tile, ctx->distribution, ctx->tid, VEC, out_dt);
...
ckc_kernel_def_t* ckc_moe_silu_mul_packed_body_scalar(ckc_moe_stream_ctx_t* ctx)
{
    ...
    for(k = 0; k < ctx->chunks; ++k)
    {
        ...
        ckc_value_t* u_off = ckc_b_add(b, ctx->up_base, i_col);
        /* o_off = b.add(out_base, i_col) */
        ckc_value_t* o_off = ckc_b_add(b, ctx->out_base, i_col);
        /* g = load_scalar_as_f32(b, GateUp, g_off, dtype=dtype) */
        ckc_value_t* g = ckc_b_load_scalar_as_f32(b, ctx->GateUp, g_off, dtype);
        /* u = load_scalar_as_f32(b, GateUp, u_off, dtype=dtype) */
        ckc_value_t* u = ckc_b_load_scalar_as_f32(b, ctx->GateUp, u_off, dtype);
        /* h = _silu_mul_f32(b, g, u, one_f32=one_f32, c_neg_log2e=c_neg_log2e) */
        ckc_value_t* h = ckc_moe_silu_mul_f32(b, g, u, ctx->one_f32, ctx->c_neg_log2e);
        /* store_scalar_from_f32(b, Hidden, o_off, h, dtype=dtype) */
        ckc_b_store_scalar_from_f32(b, ctx->Hidden, o_off, h, dtype);
    }
...
ckc_kernel_def_t* ckc_moe_silu_mul_packed_body_tile(ckc_moe_stream_ctx_t* ctx)
{
    ...
    shape1[0] = ctx->chunk_elems;
    if(ckc_make_global_view(&ctx->gate_view, ctx->GateUp, shape1, 1, ctx->ty, NULL) != CKC_OK)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "%s", "make_global_view(GateUp) failed");
        return NULL;
    }
    if(ckc_make_global_view(&ctx->out_view, ctx->Hidden, shape1, 1, ctx->ty, NULL) != CKC_OK)
    {
    ...
    {
        ckc_value_t* col_off;
        ckc_value_t* gate_origin[1];
        ckc_value_t* up_origin[1];
        ckc_value_t* out_origin[1];
        int lengths1[1];
        ckc_tile_window_t gate_tile, up_tile, out_tile;
        ckc_value_t* g_dt[CKC_TV_MAX_VEC];
        ckc_value_t* u_dt[CKC_TV_MAX_VEC];
