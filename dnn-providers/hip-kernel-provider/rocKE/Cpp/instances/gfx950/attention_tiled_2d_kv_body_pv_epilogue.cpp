 ...
 * ============================================================ */
static ckc_value_t* ckc__attn2d_mfma_16x16x16(
...
static ckc_value_t* ckc__attn2d_mfma_16x16x32(
    ckc_ir_builder_t* b, const ckc_type_t* dtype, ckc_value_t* a, ckc_value_t* bv, ckc_value_t* c)
{
    if(dtype != NULL && dtype->kind == CKC_TYPE_SCALAR)
    {
    ...
    if(ctx->USE_MFMA_32X32)
    {
        if(ctx->TRANSPOSED_QK_32X32)
        {
            ...
            for(n = 0; n < ctx->ACC_N_TILES; ++n)
            {
                ...
                (void)PT32_n;
                acc32 = ckc_gfx950_attn2d_apply_transposed_pv_regs(
                    ...
                    if(ctx->FP8_MFMA_PV)
                    {
                        ...
                        for(atom = 0; atom < ctx->M_ATOMS_PER_WARP; ++atom)
                        {
                            ckc_value_t* p_row = ckc_b_add(
                                ...
                                ctx->wave_row_base,
                                ckc_b_add(
                                    b, ckc_b_const_i32(b, (int64_t)(atom * 16)), ctx->lane_col_v));
                            ckc_value_t* pidx[2];
                            ckc_value_t* A_p8;
                            ckc_value_t* raw;
                            ckc_value_t* comps[4];
...
ckc_kernel_def_t* ckc_gfx950_attn2d_emit_epilogue(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
        ...
        if(ctx->TRANSPOSED_QK_32X32)
        {
            ...
            for(n = 0; n < ctx->ACC_N_TILES; ++n)
            {
                ...
                for(r = 0; r < ctx->REGS_PER_LANE; ++r)
                {
                    ...
                    ckc_b_global_store(
                        b, ctx->output, ckc_b_add(b, out_base_t, out_col_t), final_h, 2);
                    ckc_b_region_leave(b);
                }
            }
            return ctx->b->kernel;
    ...
    /* ---------------- striped epilogue (Python 3717-3817) ---------------- */
    {
        ...
        op_pos = ckc_b_add(
            b, ctx->qb_start_pos, ckc_b_div(b, OUT_ROW_BASE, ckc_b_const_i32(b, ctx->NQK)));
        {
            ckc_value_t* qh_mul = ckc_b_mul(b, ctx->kv_head_idx, ckc_b_const_i32(b, ctx->NQK));
        ...
        for(stripe = 0; stripe < ctx->OUT_STRIPES; ++stripe)
        {
            ...
            for(chunk = 0; chunk < OUT_CHUNKS_PER_THREAD; ++chunk)
            {
                ...
                ckc_b_region_enter(b, iff.then_region);
                ckc_b_global_store_vN(
                    b, ctx->output, ckc_b_add(b, out_base, out_col), v8h, OUT_VEC, 16);
                ckc_b_region_leave(b);
            }
