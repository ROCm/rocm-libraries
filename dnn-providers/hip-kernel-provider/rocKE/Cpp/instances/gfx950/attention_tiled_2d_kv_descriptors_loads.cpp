 ...
 *   of ck_dsl/instances/gfx950/attention_tiled_2d.py (arch gfx950).
 *
 * SCOPE (this translation unit).
 *   ckc_gfx950_attn2d_emit_preloop          -- the pre-loop section (Python lines
 ...
 *   ckc_gfx950_attn2d_fast_paged_kv_blocks  -- _fast_paged_kv_blocks (1550-1571):
 *     the per-tile two-logical-block block_tables lookup.
 *   ckc_gfx950_attn2d_fast_paged_kv_voff    -- _fast_paged_kv_voff (1573-1589):
 *     the per-call within-block byte voffset (+ optional i64 block base on
 *     I64_KV_ADDR).
 *   ckc_gfx950_attn2d_issue_k_load_runtime  -- _issue_k_load_runtime (1631-1689).
 *   ckc_gfx950_attn2d_issue_v_load_runtime  -- _issue_v_load_runtime (1691-1739).
 *   ckc_gfx950_attn2d_issue_k / _issue_v    -- the loader dispatch wrappers
 *     (2184-2227).
 *   ckc_gfx950_attn2d_read_k8_mfma_operand  -- _read_k8_mfma_operand (2229-2248).
 *
...
static int ckc__kv_halves_per_call(const ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    return ctx->THREADS * 8;
}

/* kv_calls_per_tile = (T * HD) // KV_HALVES_PER_CALL (Python 1415). */
static int ckc__kv_calls_per_tile(const ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    return (ctx->T * ctx->HD) / ckc__kv_halves_per_call(ctx);
}

/* bytes_per_call = KV_HALVES_PER_CALL * 2 (Python 1416). */
static int ckc__bytes_per_call(const ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    return ckc__kv_halves_per_call(ctx) * 2;
}

/* bytes_per_buf = T * HD * 2 (one [T, HD] working-dtype (bf16) slab) (Python 1464). */
static int ckc__bytes_per_buf(const ckc_gfx950_attn2d_build_ctx_t* ctx)
{
...
static ckc_value_t* ckc__kv_block_bytes_c(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
 ...
 * local). */
static ckc_value_t* ckc__lane_half_base(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    if(ctx->lane_half_base_v == NULL)
        ctx->lane_half_base_v = ckc_b_mul(ctx->b, ctx->tid, ckc_b_const_i32(ctx->b, 8));
...
static ckc_value_t* ckc__K_lds_addr(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
...
static ckc_value_t* ckc__V_lds_addr(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
...
static ckc_value_t* ckc__zero_soff(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
 ...
 * reuses it; cache. */
static ckc_value_t* ckc__wave_lds_offset_i64(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
...
void ckc_gfx950_attn2d_emit_preloop(ckc_gfx950_attn2d_build_ctx_t* ctx)
{
    ...
    if(ctx->FAST_PAGED_KV_DESC)
    {
        ...
        ckc_gfx950_attn2d_emit_fp8_async_preloop_setup(ctx);
        return;
    }
...
static void ckc__fast_paged_kv_voff_split(ckc_gfx950_attn2d_build_ctx_t* ctx,
                                          int call,
                                          ckc_value_t* block0,
                                          ckc_value_t* block1,
                                          ...
                                          ckc_value_t** out_voff)
{
    ...
    if(out_base_i64)
        *out_base_i64 = NULL;
    if(out_voff)
        *out_voff = ckc_b_add(b, block_b, within);
}
                                                  ...
                                                  ckc_value_t* block1)
{
    ...
    ckc__fast_paged_kv_voff_split(ctx, call, block0, block1, NULL, &voff);
    return voff;
}
                                            ...
                                            ckc_value_t* buf_idx)
{
    ...
    if(ctx->FAST_PAGED_KV_DESC)
        ckc_gfx950_attn2d_fast_paged_kv_blocks(ctx, kv_tile_idx, &fast_block0, &fast_block1);
    ...
    for(int call = 0; call < kv_calls_per_tile; ++call)
    {
        ...
        if(ctx->FAST_PAGED_KV_DESC)
        {
            ...
            ckc__fast_paged_kv_voff_split(ctx, call, fast_block0, fast_block1, &base_i64, &voff);
        ...
        else
        {
            ...
            else if(!ckc_transforms_descriptor_offset(
                               ...
                               ckc_value_t* buf_idx)
{
    if(ctx->FP8_MFMA_QK) /* K_FP8_MFMA == FP8_MFMA_QK */
        ckc_gfx950_attn2d_issue_k_fp8_mfma_async(ctx, tile_idx, buf_idx);
    else if(ctx->KV_FP8)
        ckc_gfx950_attn2d_issue_fp8_dequant_loads(ctx, tile_idx, buf_idx, false);
    else
        ckc_gfx950_attn2d_issue_k_load_runtime(ctx, tile_idx, buf_idx);
}
                               ...
                               ckc_value_t* buf_idx)
{
    if(ctx->FP8_MFMA_PV) /* PV_FP8_MFMA == FP8_MFMA_PV */
        ckc_gfx950_attn2d_issue_v_fp8_mfma_stripe(ctx, tile_idx);
    else if(ctx->KV_FP8)
        /* V is single-buffered: always pin slot 0 (Python 2225). */
        ckc_gfx950_attn2d_issue_fp8_dequant_loads(ctx, tile_idx, ckc_b_const_i32(ctx->b, 0), true);
    else
        ckc_gfx950_attn2d_issue_v_load_runtime(ctx, tile_idx, buf_idx);
}
                                                    ...
                                                    ckc_value_t* k_off)
{
    ...
    if(!ctx->FP8_MFMA_QK)
        return ckc_b_smem_load_vN(b, ctx->K_lds, idx, 3, ctx->dtype, 8);
    ...
    ckc_value_t* k_fp8 = ckc_b_smem_load_vN(b, ctx->K_lds, idx, 3, ckc_fp8e4m3(), 8);
    return ckc_dequant_fp8x8_to_dtype(b, k_fp8, ctx->k_scale_p, ctx->dtype);
}
