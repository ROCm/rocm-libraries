...
static ckc_value_t* dfcp_mma(
    ckc_ir_builder_t* b, const ckc_mma_op_t* op, ckc_value_t* a, ckc_value_t* bb, ckc_value_t* c)
{
...
static void dfcp_layout_coord0(ckc_ir_builder_t* b,
...
ckc_value_t* ckc_gfx1151_dfcp_pack_i4_codes_to_i32(ckc_ir_builder_t* b, ckc_value_t* codes)
{
    ...
    {
        ckc_value_t* nib
            = ckc_b_land(b, ckc_b_sext(b, ckc_b_vec_extract(b, codes, i), ckc_i32()), mask);
        if(i)
                                                               ...
                                                               int frag_len)
{
    ...
    ckc_value_t* c_sc = ckc_b_const_i32(b, c->X * c->C);
                                             ...
                                             size_t* out_count)
{
    ...
    for(int kk = 0; kk < k_atoms; ++kk)
    {
        ...
        for(int mi = 0; mi < mfmas_m; ++mi)
        {
            ...
            a_rows[mi]
                = ckc_gfx1151_dfcp_load_frag_iu8_from_lds(b, a_smem, a_row, atom_off, k_tile_base);
        }
        for(int ni = 0; ni < mfmas_n; ++ni)
        {
            ckc_value_t* atom_off = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * DFCP_WMMA));
            b_cols[ni]
                = ckc_gfx1151_dfcp_load_frag_iu8_from_lds(b, b_smem, b_col, atom_off, k_tile_base);
        }
        int flat = 0;
        for(int mi = 0; mi < mfmas_m; ++mi)
            for(int ni = 0; ni < mfmas_n; ++ni)
    ...
    for(int kk = 0; kk < k_atoms; ++kk)
    {
        ...
        for(int mi = 0; mi < mfmas_m; ++mi)
        {
            ...
            if(ctx->spec->static_direct_kmap)
                a_rows[mi] = ckc_gfx1151_dfcp_load_conv0_a_frag_from_footprint_iu8_static(
                    ctx, inp_smem, m_row, kk);
            else
                a_rows[mi] = ckc_gfx1151_dfcp_load_conv0_a_frag_from_footprint_iu8(
        ...
        for(int ni = 0; ni < mfmas_n; ++ni)
        {
            ckc_value_t* atom_off = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * DFCP_WMMA));
            b_cols[ni]
                = ckc_gfx1151_dfcp_load_frag_iu8_from_lds(b, w0_smem, b_col, atom_off, k_tile_base);
        }
        int flat = 0;
        for(int mi = 0; mi < mfmas_m; ++mi)
            for(int ni = 0; ni < mfmas_n; ++ni)
                                                  ...
                                                  size_t* out_count)
{
    ...
    if(prefetch_k)
    {
        for(int kk = 0; kk < k_atoms; ++kk)
        {
            ...
            {
                ckc_value_t* atom_off
                    = ckc_b_add(b, warp_m_off, ckc_b_const_i32(b, mi * DFCP_WMMA));
                a_all[kk][mi] = ckc_gfx1151_dfcp_load_frag_iu4_codes_from_lds(
        ...
        for(int kk = 0; kk < k_atoms; ++kk)
        {
            ...
            {
                ckc_value_t* atom_off
                    = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * DFCP_WMMA));
                b_all[kk][ni] = ckc_gfx1151_dfcp_load_frag_iu4_packed_from_lds(
    ...
    else
    {
        for(int kk = 0; kk < k_atoms; ++kk)
        {
            ...
            for(int mi = 0; mi < mfmas_m; ++mi)
            {
                ckc_value_t* atom_off
                    = ckc_b_add(b, warp_m_off, ckc_b_const_i32(b, mi * DFCP_WMMA));
                a_rows[mi] = ckc_gfx1151_dfcp_load_frag_iu4_codes_from_lds(
                    b, c0_smem, a_row, atom_off, k_tile_base);
            }
            for(int ni = 0; ni < mfmas_n; ++ni)
            {
                ckc_value_t* atom_off
                    = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * DFCP_WMMA));
                b_cols[ni] = ckc_gfx1151_dfcp_load_frag_iu4_packed_from_lds(
                    b, w1_smem, b_col, atom_off, k_tile_base);
            }
            int flat = 0;
            for(int mi = 0; mi < mfmas_m; ++mi)
    ...
    if(sched_fuse)
        ckc_gfx1151_dfcp_emit_wmma_k_sched(b, policy, n_ds * k_atoms, mfmas_m * mfmas_n * k_atoms);
    *out_count = (size_t)n_acc;
}
                                                         ...
                                                         size_t* out_count)
{
    ...
    for(int kk = 0; kk < k_atoms; ++kk)
    {
        ...
        {
            ckc_value_t* atom_off = ckc_b_add(b, warp_m_off, ckc_b_const_i32(b, mi * DFCP_WMMA));
            a_rows[mi] = ckc_gfx1151_dfcp_load_frag_iu4_packed_from_lds(
                b, c0_smem, a_row, atom_off, k_tile_base);
        }
        for(int ni = 0; ni < mfmas_n; ++ni)
        {
    ...
    if(prefetch_k)
    {
        for(int kk = 0; kk < k_atoms; ++kk)
        {
            ...
            for(int ni = 0; ni < mfmas_n; ++ni)
            {
                ckc_value_t* atom_off
                    = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * DFCP_WMMA));
                b_all[kk][ni] = ckc_gfx1151_dfcp_load_frag_iu8_from_lds(
                    b, w1_smem, b_col, atom_off, k_tile_base);
            }
        }
    }
    ...
    for(int kk = 0; kk < k_atoms; ++kk)
    {
        ...
        else
        {
            ...
            for(int ni = 0; ni < mfmas_n; ++ni)
            {
                ckc_value_t* atom_off
                    = ckc_b_add(b, warp_n_off, ckc_b_const_i32(b, ni * DFCP_WMMA));
                b_cols[ni] = ckc_gfx1151_dfcp_load_frag_iu8_from_lds(
                    b, w1_smem, b_col, atom_off, k_tile_base);
            }
        }
        int flat = 0;
    ...
    for(int mi = 0; mi < mfmas_m; ++mi)
    {
        for(int kk = 0; kk < mfmas_n; ++kk)
        {
            ...
            for(int s = 0; s < op0->c_frag_len; ++s)
                codes[s]
                    = ckc_gfx1151_dfcp_apply_code_fn(ctx, code_fn, ckc_b_vec_extract(b, acc, s));
            ...
            ckc_value_t* s0 = ckc_gfx1151_dfcp_pack_i4_codes_to_i32(
                b,
                ckc_b_bitcast(
                    b, ckc_b_vec_pack(b, p01, 2, ckc_i32()), ckc_vector_type(b, ckc_i8(), 8)));
            ckc_value_t* p23[2] = {ord2, ord3};
            ckc_value_t* s1 = ckc_gfx1151_dfcp_pack_i4_codes_to_i32(
                b,
                ckc_b_bitcast(
                    b, ckc_b_vec_pack(b, p23, 2, ckc_i32()), ckc_vector_type(b, ckc_i8(), 8)));
