...
ckc_kernel_def_t* ckc_gfx942_attn2d_emit_epilogue(ckc_gfx942_attn2d_build_ctx_t* ctx)
{
    ...
    ckc_value_t* lane_col
        = (ctx->lane_col_v != NULL) ? ctx->lane_col_v : ckc_b_mod(b, lane, ckc_b_const_i32(b, 16));
    ckc_value_t* lane_col32 = (ctx->lane_col32_v != NULL)
                                  ? ctx->lane_col32_v
    ...
    if(ctx->USE_MFMA_32X32)
    {
        if(ctx->TRANSPOSED_QK_32X32)
        {
            ...
            ckc_value_t* op_ok_qh = ckc_b_cmp_lt(b, op_qh_t, ckc_b_const_i32(b, NUM_QH));
            ...
            ckc_value_t* out_base_t
                = epilogue_q_desc_offset(ctx, op_token_t, op_qh_t, ckc_b_const_i32(b, 0));
            ...
            for(n = 0; n < ACC_N_TILES; ++n)
            {
                ...
                for(reg = 0; reg < REGS_PER_LANE; ++reg)
                {
                    ...
                    ckc_value_t* out_col_t
                        = ckc_b_add(b, out_col_base, ckc__mfma_32x32_c_row(b, lane, reg));
        ...
        ckc_value_t* OUT_ROW_BASE32 = ckc_b_div(b, tid, ckc_b_const_i32(b, OUT_THREADS_PER_ROW32));
        ...
        ckc_value_t* OUT_col_mod32 = ckc_b_mod(b, tid, ckc_b_const_i32(b, OUT_THREADS_PER_ROW32));
        ckc_value_t* OUT_col_base32
            = ckc_b_mul(b, OUT_col_mod32, ckc_b_const_i32(b, OUT_CHUNKS_PER_THREAD32 * OUT_VEC32));
        ckc_value_t* op_pos32_base
            = ckc_b_add(b, qb_start_pos, ckc_b_div(b, OUT_ROW_BASE32, ckc_b_const_i32(b, NQK)));
        ...
        ckc_value_t* op_qh32_mul = ckc_b_mul(b, kv_head_idx, ckc_b_const_i32(b, NQK));
        ckc_value_t* op_qh32_mod = ckc_b_mod(b, OUT_ROW_BASE32, ckc_b_const_i32(b, NQK));
        ckc_value_t* op_qh32_base = ckc_b_add(b, op_qh32_mul, op_qh32_mod);
        /* Python emits the pos cmp BEFORE the qh cmp; bind in order. */
        ckc_value_t* op_ok_pos32 = ckc_b_cmp_lt(b, op_pos32_base, cur_batch_q_len);
        ckc_value_t* op_ok_qh32 = ckc_b_cmp_lt(b, op_qh32_base, ckc_b_const_i32(b, NUM_QH));
        ...
        ckc_value_t* out_base32_base
            = epilogue_q_desc_offset(ctx, out_token32, op_qh32_base, ckc_b_const_i32(b, 0));
        ...
        for(n = 0; n < ACC_N_TILES; ++n)
        {
            ...
            for(reg = 0; reg < REGS_PER_LANE; ++reg)
            {
                ckc_value_t* row = ckc_b_add(b, wave_row_base, ckc__mfma_32x32_c_row(b, lane, reg));
            ...
            for(chunk = 0; chunk < OUT_CHUNKS_PER_THREAD32; ++chunk)
            {
                ...
                ckc_value_t* out_col = ckc_b_add(b, ckc_b_const_i32(b, n * 32), col_in_stripe);
    ...
    ckc_value_t* tid = ctx->tid;
    ckc_value_t* OUT_ROW_BASE = ckc_b_div(b, tid, ckc_b_const_i32(b, OUT_THREADS_PER_ROW));
    ...
    ckc_value_t* OUT_col_mod = ckc_b_mod(b, tid, ckc_b_const_i32(b, OUT_THREADS_PER_ROW));
    ckc_value_t* OUT_col_base_in_stripe
        = ckc_b_mul(b, OUT_col_mod, ckc_b_const_i32(b, OUT_CHUNKS_PER_THREAD * OUT_VEC));

    /* Compute (op_pos, op_qh, op_mask, out_base) once per CTA -- these depend
     * only on OUT_row, which is loop-invariant across stripes. */
    ckc_value_t* op_pos
        = ckc_b_add(b, qb_start_pos, ckc_b_div(b, OUT_ROW_BASE, ckc_b_const_i32(b, NQK)));
    ...
    ckc_value_t* out_base = epilogue_q_desc_offset(ctx, out_token, op_qh, out_dim0);
    ...
    for(stripe = 0; stripe < OUT_STRIPES; ++stripe)
    {
        ...
        for(chunk = 0; chunk < OUT_CHUNKS_PER_THREAD; ++chunk)
        {
            ...
            ckc_value_t* out_col
                = ckc_b_add(b, ckc_b_const_i32(b, stripe * OUT_STRIPE_COLS), col_in_stripe);
            ckc_if_t iff = ckc_b_scf_if(b, op_mask);
            ckc_b_region_enter(b, iff.then_region);
            {
                ckc_b_global_store_vN(b, output, ckc_b_add(b, out_base, out_col), v8h, OUT_VEC, 16);
            }
            ckc_b_region_leave(b);
        }
