...
#define ATTN3D_REASON(s)       \
    ...
    if(q_dtype != NULL && !ckc_attn3d_streq(q_dtype, "fp16") && !ckc_attn3d_streq(q_dtype, "bf16"))
    {
        snprintf(
    ...
    ckc_ir_builder_t* b, const ckc_unified_attention_reduce_tiled_spec_t* spec, const char* arch)
{
    ...
    if(spec == NULL)
    {
        ckc_i_set_err(b, CKC_ERR_VALUE, "build_unified_attention_reduce_tiled_gfx942: NULL spec");
        return NULL;
    }
    ...
    if(b->kernel != NULL)
    {
        ...
        {
            ckc_i_set_err(b, CKC_ERR_VALUE, "attn_reduce_tiled_gfx942: kernel_name encode failed");
            return NULL;
        }
        b->kernel->name = ckc_arena_strdup(&b->arena, name);
        if(b->kernel->name == NULL)
