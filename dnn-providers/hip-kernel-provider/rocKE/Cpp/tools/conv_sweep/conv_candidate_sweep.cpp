...
static rocke_implicit_gemm_conv_spec_t makeSpec(const ConvCase& cse, const Candidate& cand) {
    ...
    s.problem = rocke_conv_problem_make(
        ...
        for (int tn : kTileSizes)
            for (int tk : kTileSizes) {
                ...
                for (const char* pipe : kPipelines) {
                    ...
                    if (!rocke_implicit_gemm_conv_is_valid_spec(&spec, arch_bare.c_str(),
...
CompileResult compileCandidate(const ConvCase& cse, std::int64_t Ho, std::int64_t Wo,
                               const Candidate& cand, const std::string& arch_bare) {
    CompileResult cr;
    ...
    try {
        ...
        std::cerr << "[Sweep]   ERROR comgr compile FAILED (" << cr.comgr_ms
                  << "ms): " << e.what() << "\n";
        cr.is_error = true;
        return cr;
    }
...
bool preValidateCandidate(const CompileResult& cr) {
    ...
    if (hipModuleLoadData(&mod, cr.hsaco.data()) != hipSuccess) return false;
...
TimingResult execLaunchAndTime(const CompileResult& cr, const ConvCase& cse,
                               std::int64_t Ho, std::int64_t Wo, int timeoutS) {
    // Write HSACO to temp file.
    char tmppath[] = "/tmp/rocke_hsaco_XXXXXX";
    int tmpfd = mkstemp(tmppath);
    if (tmpfd < 0) return {std::nullopt, true};
    ...
    int pipefd[2];
    if (pipe(pipefd) != 0) { unlink(tmppath); return {std::nullopt, true}; }
    ...
    if (pid == 0) {
        ...
        execl(gHelperBin.c_str(), "rocke_kern_time",
              tmppath, cr.kernel_name.c_str(),
              s_gx.c_str(), s_gy.c_str(), s_gz.c_str(), s_bs.c_str(),
              s_a.c_str(), s_b.c_str(), s_d.c_str(),
              (char*)nullptr);
        // exec failed
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (w == pid) { finished = true; break; }
        if (w < 0) { finished = true; break; }
        usleep(10000);
    }
...
ShapeSweepResult runConvSweep(const ConvCase& cse, const hipDeviceProp_t& props,
    ...
    for (const auto& cand : candidates) {
        ...
        auto compileFut = std::async(std::launch::async,
                                     compileCandidate, cse, Ho, Wo, cand, arch_bare);

        if (compileFut.wait_for(compileTimeout) == std::future_status::timeout) {
            ++compileTimeouts;
            std::cerr << "[Sweep]   COMPILE_TIMEOUT: " << cse.name
        ...
        if (cr.is_error) {
            ++errors;
            std::cerr << "[Sweep]   COMPILE_ERROR: " << cse.name
                      << " tile(" << cand.tile_m << "," << cand.tile_n
                      << "," << cand.tile_k << ") pipeline=" << cand.pipeline
        ...
        if (!preValidateCandidate(cr)) {
            ++prevalidRejects;
            std::cerr << "[Sweep]   PREVALID_REJECT: " << cse.name
                      << " tile(" << cand.tile_m << "," << cand.tile_n
                      << "," << cand.tile_k << ") pipeline=" << cand.pipeline
        ...
        if (res.timed_out) {
            ++timeouts;
            std::cerr << "[Sweep]   EXEC_TIMEOUT: " << cse.name
                      << " tile(" << cand.tile_m << "," << cand.tile_n
                      << "," << cand.tile_k << ") pipeline=" << cand.pipeline
                      << " exceeded " << kExecTimeoutS << "s (killed)"
                      << " (compile: lower=" << cr.lower_ms << "ms comgr=" << cr.comgr_ms << "ms)\n";
            continue;
        }
        if (res.is_error) {
            ++errors;
            std::cerr << "[Sweep]   EXEC_ERROR: " << cse.name
                      << " tile(" << cand.tile_m << "," << cand.tile_n
                      << "," << cand.tile_k << ") pipeline=" << cand.pipeline << "\n";
            continue;
        }
        if (!res.latency_us) continue;
        ++ok;
