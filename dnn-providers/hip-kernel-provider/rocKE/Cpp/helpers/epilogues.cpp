...
ckc_value_t* ckc_warp_grid_warp_m_off(ckc_ir_builder_t* b, const ckc_warp_grid_t* grid)
{
    ...
    if(mfmas_m < 0)
        return NULL;
    return ckc_b_mul(b, grid->warp_m_idx, ckc_b_const_i32(b, (int64_t)mfmas_m * grid->warp_tile_m));
}
...
ckc_value_t* ckc_warp_grid_warp_n_off(ckc_ir_builder_t* b, const ckc_warp_grid_t* grid)
{
    ...
    if(mfmas_n < 0)
        return NULL;
    return ckc_b_mul(b, grid->warp_n_idx, ckc_b_const_i32(b, (int64_t)mfmas_n * grid->warp_tile_n));
}
        ...
        for(ni = 0; ni < mfmas_n; ++ni)
        {
            ...
            if(vec_in_acc)
            {
                ...
                if(atom->c_per_lane == 4)
                    ...
                    ckc_b_buffer_store_vN_f16(b, d_rsrc, safe, ckc_b_const_i32(b, 0), acc_h, 4);
                else
                {
                    ckc_i_set_err(b,
                                  CKC_ERR_VALUE,
            ...
            else
            {
                ...
                for(i = 0; i < atom->c_per_lane; ++i)
                {
                    ...
                    ckc_value_t* off_bytes;
                    ckc_value_t* safe;
                    if(ckc_epi_lane_to_output(b, atom, grid->lane, i, &row_off, &col_off) != 0)
                        return;
                    m_val = ckc_b_add(b, atom_m_off, row_off);
                    n_val = ckc_b_add(b, atom_n_off, col_off);
                    ok = ckc_direct_epilogue_bounds_check(b, m_val, n_val, bounds_m, bounds_n, 1);
                    off_elems = addr_fn(b, m_val, n_val, &valid, addr_user);
                    if(ok != NULL)
                        ok = (valid != NULL) ? ckc_b_land(b, ok, valid) : ok;
                    else
                                 ...
                                 ckc_value_t* bounds_n)
{
    ...
    if(ckc_make_lds_view(b, &c_view, ckc_f16(), lds_shape, 2, epi->smem_name_hint, NULL) != CKC_OK)
        return;
    c_smem = c_view.base;
