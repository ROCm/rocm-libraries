 ...
 *
 * ckc/lower_cktile.h -- C99 port of ck_dsl.core.lower_cktile.
 *
 * Lower a ck_dsl *instance spec* to CK Tile-style C++ source.
 *
 * Unlike the other lowerers in the C port (lower_hip / lower_llvm), this module
 * does NOT walk the post-IR ckc_kernel_def_t. It mirrors the Python module,
 ...
 * ckc/ir.h (they are inputs, not IR nodes), this header declares plain C99
 ...
 *
 * Emission uses ckc_strbuf for all text. The returned string is owned by the
 * caller-provided ckc_strbuf_t (use ckc_strbuf_cstr / ckc_strbuf_detach).
 *
 * Error model mirrors the rest of the C port: functions return a ckc_status_t
 * and write a human-readable message into `err` (CKC_ERR_MSG_CAP bytes). The
 * Python NotImplementedError / TypeError / ValueError cases map to
 * CKC_ERR_NOTIMPL / CKC_ERR_TYPE / CKC_ERR_VALUE respectively.
 */
#ifndef CKC_LOWER_CKTILE_H
#define CKC_LOWER_CKTILE_H
...
typedef struct ckc_cktile_tile_spec
{
    int tile_m;
    int tile_n;
    int tile_k;
...
typedef struct ckc_cktile_gemm_spec
{
    const char* name;
    ckc_cktile_tile_spec_t tile;
    ckc_cktile_trait_spec_t trait;
    ckc_cktile_data_spec_t data;
    bool batched;
    /* Optional precomputed mangled name; NULL => compute via kernel_name(). */
    const char* kernel_name;
} ckc_cktile_gemm_spec_t;

/* Mirror of conv_implicit_gemm.ConvProblem. */
typedef struct ckc_cktile_conv_problem
{
    int N, Hi, Wi, C, K, R, S;
    int sH, sW; /* default 1 */
    int pH, pW; /* default 0 */
    int dH, dW; /* default 1 */
} ckc_cktile_conv_problem_t;
...
typedef struct ckc_cktile_conv_spec
{
    const char* name; /* default "conv_igemm" */
    ckc_cktile_conv_problem_t problem;
    int tile_m;
    int tile_n;
    int tile_k;
    int warp_m;
    ...
    /* Optional precomputed mangled name; NULL => compute via kernel_name(). */
    const char* kernel_name;
} ckc_cktile_conv_spec_t;

/* Tagged dispatch input for lower_spec_to_cktile (the C analog of Python's
 * isinstance dispatch). */
typedef enum ckc_cktile_spec_kind
{
    CKC_CKTILE_SPEC_GEMM = 0,
    CKC_CKTILE_SPEC_CONV
} ckc_cktile_spec_kind_t;

typedef struct ckc_cktile_spec
{
    ckc_cktile_spec_kind_t kind;
    union
    {
        const ckc_cktile_gemm_spec_t* gemm;
        const ckc_cktile_conv_spec_t* conv;
    } u;
} ckc_cktile_spec_t;
 ...
 * Returns CKC_OK on success; CKC_ERR_NOTIMPL / CKC_ERR_VALUE on the Python
