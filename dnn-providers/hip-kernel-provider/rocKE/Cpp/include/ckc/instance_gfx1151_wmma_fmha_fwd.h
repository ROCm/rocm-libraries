 ...
 *
 * ckc/instance_gfx1151_wmma_fmha_fwd.h -- C99 port of the gfx1151 (RDNA3.5 /
 ...
 * ck_dsl.helpers.mfma_attention.mfma_attention_fwd_inner_body, which dispatches
 * to the WMMA wave32 path on an RDNA target (and the MFMA wave64 path on CDNA).
 *
 *   Python (wmma_fmha_fwd.py)             C99 (this header)
 *   ----------------------------------    --------------------------------------
 *   @dataclass WmmaFmhaFwdSpec            ckc_wmma_fmha_fwd_spec_t
 *     .kernel_name()                      ckc_wmma_fmha_fwd_kernel_name(...)
 *   is_valid_spec(spec, arch)             ckc_wmma_fmha_fwd_is_valid_spec(...)
 *   build_wmma_fmha_fwd(spec, arch)       ckc_build_wmma_fmha_fwd(...)
 *   wmma_fmha_fwd_grid(spec, sq, batch)   ckc_wmma_fmha_fwd_grid(...)
 *   (signature helper, parity)            ckc_wmma_fmha_fwd_signature(...)
 *   (build -> lower .ll convenience)      ckc_wmma_fmha_fwd_lower_to_llvm(...)
 *
 ...
 */
#ifndef CKC_INSTANCE_GFX1151_WMMA_FMHA_FWD_H
#define CKC_INSTANCE_GFX1151_WMMA_FMHA_FWD_H
...
#define CKC_WMMA_FMHA_FWD_OP_ID "wmma_f32_16x16x16_f16"
#define CKC_WMMA_FMHA_FWD_BLOCK_M 16 /* Q rows per wave per CTA            */
#define CKC_WMMA_FMHA_FWD_BLOCK_K 16 /* K positions per K-tile (WMMA N)    */
 ...
 *   name             : NULL => "ck_dsl_wmma_fmha_fwd".
 *
 * dtype is fp16 only (the Python __post_init__ rejects anything else). */
typedef struct ckc_wmma_fmha_fwd_spec
{
    int head_size;
    int num_query_heads;
 ...
 * "gfx1151". On CKC_OK *out_ll receives a malloc'd NUL-terminated string the
...
#endif /* CKC_INSTANCE_GFX1151_WMMA_FMHA_FWD_H */
