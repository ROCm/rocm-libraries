 ...
 *
 * ckc/instance_permute_nd.h -- C99 port of the N-D tensor permutation kernel
 * instance builder ck_dsl/instances/common/permute_nd.py.
 *
 ...
 *   -----------------------------------   --------------------------------------
 *   class PermuteSpec                     ckc_permute_spec_t
 *   PermuteSpec.rank / y_shape /          ckc_permute_rank / _y_shape /
 *     total_elements / vec_width            _total_elements / _vec_width
 *   PermuteSpec.kernel_name()             ckc_permute_kernel_name(...)
 *   is_valid_spec(spec, arch)             ckc_permute_is_valid_spec(...)
 *   build_permute(spec, arch)             ckc_build_permute(...)
 *   permute_grid(spec)                    ckc_permute_grid(...)
 *   permute_signature(spec)               ckc_permute_signature(...)
 *   (+ convenience: build -> lower .ll)   ckc_permute_lower_to_llvm(...)
 *
 ...
 */
#ifndef CKC_INSTANCE_PERMUTE_ND_H
#define CKC_INSTANCE_PERMUTE_ND_H
 ...
 * "f16" or "bf16". `name` defaults to "ck_dsl_permute" in the Python dataclass;
 * use ckc_permute_spec_default() to get that default. */
typedef struct ckc_permute_spec
{
 ...
 * extents into out (capacity CKC_PERMUTE_MAX_RANK) and returns the rank. */
 ...
 * or CKC_ERR_VALUE on an invalid (non-positive block_size etc.) spec. */
 ...
 * `arena` backs the builder's string/array storage. Returns CKC_OK, or
 * CKC_ERR_VALUE on a too-small out_cap / NULL args. */
...
#endif /* CKC_INSTANCE_PERMUTE_ND_H */
