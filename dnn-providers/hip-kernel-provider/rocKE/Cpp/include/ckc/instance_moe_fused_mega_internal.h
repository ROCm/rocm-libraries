 ...
 *
 * ckc/instance_moe_fused_mega_internal.h -- PRIVATE shared state + phase /
 * closure-function contract for the C99 port of build_moe_fused_mega_gemm
 * (ck_dsl/instances/common/moe_fused_mega.py, 740 LOC).
 *
 ...
 *   build-context struct, ckc_moe_mega_build_ctx_t, and turns `_emit_body` --
 ...
 *
 *   The public driver (ckc_build_moe_fused_mega_gemm in
 *   ckc/instance_moe_fused_mega.h) computes the prologue into the ctx in the
 ...
 *   calling ckc_moe_mega_emit_body(ctx) inside the scf_if.
 *
 ...
 *   `ckc_moe_mega_` prefix; the reused moe_gemm_fused helpers keep their own
 *   `ckc_moe_*` names (helper header) and are NOT re-declared here.
 *
 ...
 * ckc/instance_moe_fused_mega.h.
 */
#ifndef CKC_INSTANCE_MOE_FUSED_MEGA_INTERNAL_H
#define CKC_INSTANCE_MOE_FUSED_MEGA_INTERNAL_H
...
#define CKC_MOE_MEGA_MAX_ACCS 64
 ...
 * ===================================================================== */
typedef struct ckc_moe_mega_build_ctx
{
    /* =============================================================== *
    ...
    ckc_value_t* slot_size; /* Python `_slot_size` (unused but bound)          */
    ckc_value_t* tokens;
    ...
    const ckc_tensor_view_t* wd_view; /* make_global_view(WDown, (1,1,1),(1,N,1))*/
 ...
 * universal specs (gate+up and down) -> ValueError mapped to CKC_ERR_VALUE with
 ...
 * fields (u_gu/u_down/views) reference. Returns CKC_OK or the first error.
 *
 * On success the ctx is COMPLETE and ckc_moe_mega_emit_body(ctx) may be called
 ...
 *  (ckc_moe_mega_emit_body) and any unit test call them.
 * ===================================================================== */
 ...
 *  ckc_moe_mega_emit_body is the single entry the public driver calls inside
 ...
 *  explicit; a caller only needs ckc_moe_mega_emit_body.
 * ===================================================================== */
 ...
 * tile: build the down acc-init group, run ckc_moe_mega_emit_down_kloop_lds_a
 * (A=Hidden_smem, B=W_down at output-row ho / contraction-base gu_n_off),
 * b.sync(), then ckc_moe_emit_down_reduce_epilogue_atomic into Y. */
...
#endif /* CKC_INSTANCE_MOE_FUSED_MEGA_INTERNAL_H */
