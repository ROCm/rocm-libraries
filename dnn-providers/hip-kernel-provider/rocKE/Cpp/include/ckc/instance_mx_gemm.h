 ...
 *
 * ckc/instance_mx_gemm.h -- C99 port of the MX (microscaling) GEMM kernel
 * instance builder ck_dsl/instances/common/mx_gemm.py (CK Tile 42_mx_gemm
 * parity).
 *
 *   Python (mx_gemm.py)               C99 (this header)
 *   --------------------------------  -----------------------------------------
 *   class MxGemmSpec                  ckc_mx_gemm_spec_t
 *   MxGemmSpec.kernel_name()          ckc_mx_gemm_kernel_name(...)
 *   is_valid_spec(spec, arch)         ckc_mx_gemm_is_valid_spec(...)
 *   build_mx_gemm(spec, arch)         ckc_build_mx_gemm(...)
 *   mx_gemm_grid(spec)                ckc_mx_gemm_grid(...)
 *   (+ convenience: init-from-spec)   ckc_build_mx_gemm_new(...)
 *   (+ convenience: build -> lower)   ckc_mx_gemm_lower_to_llvm(...)
 *
 ...
 * defaults. In C the caller fills a ckc_mx_gemm_spec_t;
 * ckc_mx_gemm_spec_default() returns a struct with every field at the Python
 ...
 */
#ifndef CKC_INSTANCE_MX_GEMM_H
#define CKC_INSTANCE_MX_GEMM_H
...
typedef struct ckc_mx_gemm_spec
{
    int M;
    int N;
    int K;
 ...
 * "t{block_tile_m}x{block_tile_n}" via kernel_name_join. Returns CKC_OK or
