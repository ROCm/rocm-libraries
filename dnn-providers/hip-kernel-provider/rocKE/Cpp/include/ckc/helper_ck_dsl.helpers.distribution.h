 ...
 *
 * helper_ck_dsl.helpers.distribution.h -- C99 port of selected symbols from
 ...
 * function emits NO IR, it makes zero ckc_b_* builder calls; the "byte-identical
 ...
 * A ckc_make_tile_distribution_encoding() constructor runs that validation.
 *
 ...
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_DISTRIBUTION_H
#define CKC_HELPER_CK_DSL_HELPERS_DISTRIBUTION_H
 ...
 */
typedef struct ckc_tile_distribution_encoding
{
    const int* Rs; /* replication lengths (per R dim), arena-owned */
    int num_R;

    const ckc_h_row_t* Hs; /* hierarchical decomposition of each X dim     */
    int num_X;

    const ckc_p_seq_t* Ps; /* Ps2RHs_major/minor, one ckc_p_seq per P dim  */
    int num_P;
 ...
 * sticky error is set (CKC_ERR_VALUE) and NULL is returned.
 *
 ...
 * encoding->Hs[x_dim].count entries, one ckc_h_bucket_ref per H level. */
typedef struct ckc_tile_distribution
{
    const ckc_tile_distribution_encoding_t* encoding;
    const ckc_h_bucket_ref_t* const* contributors; /* [num_X][Hs[x].count]   */
 ...
 * unmapped, the builder's sticky error is set (CKC_ERR_VALUE) and NULL is
...
typedef struct ckc_static_distributed_tensor
{
    const ckc_tile_distribution_t* distribution;
    const ckc_type_t* dtype;
    ckc_value_t** storage;
    int num_storage;
} ckc_static_distributed_tensor_t;
 ...
 * when the reduce is single-warp). ``combine`` must be CKC_REDUCE_SUM or
 * CKC_REDUCE_MAX (Python raises ValueError otherwise). ``lds_buf`` / ``tid`` are
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_DISTRIBUTION_H */
