 ...
 *
 * ckc/instance_sage_attention_internal.h -- PRIVATE shared state + phase /
 * closure-function contract for the C99 port of _build_sage_mfma and
 * _build_sage_warp (ck_dsl/instances/common/sage_attention.py).
 *
 ...
 *   (ckc_sage_mfma_ctx_t / ckc_sage_warp_ctx_t) holding EXACTLY the variables the
 ...
 *   closure functions mirror the Python names with a `ckc_sage_mfma_` /
 *   `ckc_sage_warp_` prefix.
 *
 ...
 * ckc/instance_sage_attention.h.
 */
#ifndef CKC_INSTANCE_SAGE_ATTENTION_INTERNAL_H
#define CKC_INSTANCE_SAGE_ATTENTION_INTERNAL_H
...
#define CKC_SAGE_MAX_EPT 8

/* ===================================================================== *
 *  ckc_sage_mfma_ctx_t  --  shared state for _build_sage_mfma.
 *
...
typedef struct ckc_sage_mfma_ctx
{
     ...
     * These are read by ckc_sage_mfma_k_block_scale_transform. */
    ckc_value_t* c_block_k; /* const_i32(BLOCK_K); NULL on per_head path   */
...
/* ===================================================================== *
 *  ckc_sage_warp_ctx_t  --  shared state for _build_sage_warp.
 *
 ...
 * ckc_sage_codebook_i4_pair_to_f32, NOT this helper.) `out` capacity >= ept.
 * Unsupported quant_mode records CKC_ERR_VALUE on the builder. */
 ...
 * the per_block k_scale path; `user` is the ckc_sage_mfma_ctx_t*. Matches the
 * ckc_attn_score_transform_fn signature so it is passed straight into
 * ckc_mfma_attn_params_t.extra_score_transform. */
ckc_value_t* ckc_sage_mfma_k_block_scale_transform(
 ...
 * ckc_mfma_attn_params_t from ctx (wiring extra_score_transform =
 * ckc_sage_mfma_k_block_scale_transform with user=ctx when c_block_k != NULL);
 * call ckc_mfma_attention_fwd_inner_body; b.ret(). Returns the kernel
 ...
 * LDS (ckc_sage_stage_codebook_to_lds, n_entries per mode) and b.sync(); rewrite
 ...
 * ckc_sage_load_q_lane_f32 with ctx->Q/dtype/q_pointee. `user` == ctx. Matches
 * ckc_fmha_q_lane_loader_fn. */
 ...
 * ckc_sage_codebook_i4_pair_to_f32; else ckc_sage_load_kv_lane_f32 for K and V.
 * `user` == ctx. Matches ckc_fmha_kv_lane_loader_fn. */
 ...
 * ckc_fmha_warp_fwd_opts_t from ctx (wiring q_lane_loader / kv_lane_loader /
 * extra_score_transform to the three closures above with user=ctx); call
 * ckc_fmha_warp_fwd_inner_body; b.ret(). Returns the kernel (kb.kernel) on
...
#endif /* CKC_INSTANCE_SAGE_ATTENTION_INTERNAL_H */
