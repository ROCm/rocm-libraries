 ...
 *
 * ckc/instance_deep_fused_conv_pool_internal.h -- PRIVATE shared state + phase /
 * closure-function contract for the C99 port of build_deep_fused_conv_pool
 * (ck_dsl/instances/common/deep_fused_conv_pool.py, lines 1212-1401).
 *
 ...
 *   ckc_dfcp_build_ctx_t, which holds EXACTLY the variables the closures share.
 *   The public driver (ckc_build_deep_fused_conv_pool in
 *   ckc/instance_deep_fused_conv_pool.h) populates the ctx in the order the
 ...
 *   Phase functions mirror the Python closure names with a `ckc_dfcp_` prefix
 *   (the module-level emit/pure helpers keep their own `ckc_dfcp_*` names from
 ...
 * ckc/instance_deep_fused_conv_pool.h.
 */
#ifndef CKC_INSTANCE_DEEP_FUSED_CONV_POOL_INTERNAL_H
#define CKC_INSTANCE_DEEP_FUSED_CONV_POOL_INTERNAL_H
...
#define CKC_DFCP_MAX_ACCS 64

/* ===================================================================== *
 *  ckc_dfcp_build_ctx_t
 *
 ...
 * ===================================================================== */
typedef struct ckc_dfcp_build_ctx
{
    /* =============================================================== *
     * (A) BUILD-TIME CONSTANTS -- set once by ckc_build_deep_fused_conv_pool
    ...
    const ckc_implicit_gemm_conv_spec_t* conv_spec; /* spec.conv_spec()             */
    ...
    const ckc_mma_op_t* op; /* _resolve_conv_op(...)        */
     ...
     * per-warp tile count. */
    ckc_value_t* conv0_accs[CKC_DFCP_MAX_ACCS];
    size_t num_conv0_accs;
    ckc_value_t* conv1_accs[CKC_DFCP_MAX_ACCS];
 ...
 * ckc_dfcp_setup_input_footprint_cache, stores + returns ctx->input_cache. */
 ...
 * to ckc_dfcp_load_conv0_a_tile_from_input_cache. */
 ...
 * Delegates to ckc_dfcp_load_conv0_a_tile_specialized. */
 ...
 * Delegates to ckc_dfcp_load_conv0_a_operand_from_input_cache. */
...
#endif /* CKC_INSTANCE_DEEP_FUSED_CONV_POOL_INTERNAL_H */
