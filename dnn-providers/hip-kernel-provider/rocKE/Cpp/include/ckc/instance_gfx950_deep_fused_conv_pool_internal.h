 ...
 *
 * ckc/instance_gfx950_deep_fused_conv_pool_internal.h -- PRIVATE shared state +
 * phase / closure-function contract for the C99 port of the gfx950 (CDNA,
 ...
 *   The gfx950 module is a thin shim that pins MFMA geometry and re-exports the
 ...
 *   share. The gfx950 public driver
 *   (ckc_build_gfx950_deep_fused_conv_pool in
 *    ckc/instance_gfx950_deep_fused_conv_pool.h) stamps the MFMA geometry/name
 ...
 *   symbols carrying the ckc_gfx950_dfcp_ / gfx950_deep_fused_conv_pool prefix so
 *   the gfx950 shim translation units never clash with the common ones (or the
 *   parallel gfx1201 shim) when they land in one program. The gfx950 phase bodies
 *   are byte-faithful to the common closures driven by the MFMA-resolved op; they
 *   reuse the ported common emit/pure helpers (ckc_dfcp_* from
 ...
 *   ckc/helper_ck_dsl.helpers.loads.h, ckc_make_lds_view / ckc_tensor_view_t from
 *   ckc/helper_ck_dsl.helpers.tensor_view.h, and the full ckc_mma_op_t struct
 *   from ckc/arch_target.h via the helper alias).
 *
 * KEY GFX950 vs GFX1201 DIFFERENCE (maxpool routing).
 *   The MFMA 32x32 warp tile exposes a register-resident INTRA-LANE maxpool fast
 *   path (ckc_dfcp_maxpool_is_intra_lane + ckc_dfcp_emit_inline_maxpool_from_
 *   registers). For gfx950 this fast path is ACTIVE. The WMMA register-resident
 *   maxpool (ckc_dfcp_emit_wmma_maxpool_from_registers, gated by
 *   ckc_dfcp_maxpool_is_intra_lane_wmma) is INACTIVE here -- it is the gfx1201
 *   counterpart's path. When the intra-lane predicate is false the gfx950 path
 *   falls back to the generic cshuffle-LDS gather maxpool
 *   (ckc_dfcp_emit_inline_maxpool_from_cshuffle), exactly as in the common body.
 *
 ...
 * instance_gfx950_deep_fused_conv_pool_*.c translation units. Public callers use
 * ckc/instance_gfx950_deep_fused_conv_pool.h.
 */
#ifndef CKC_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_INTERNAL_H
#define CKC_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_INTERNAL_H
 ...
 * tile_n<=32, warp_tile 32x32) this is small (a handful for the intra-lane fast
 * path). 64 is generous headroom and matches the common / gfx1201 internal-header
 * convention. */
...
typedef struct ckc_gfx950_dfcp_build_ctx
{
     ...
     *     ckc_build_gfx950_deep_fused_conv_pool before calling
    ...
    /* ---- MFMA intra-lane register-resident decision (gfx950 maxpool routing) ----
     * The gfx950 MFMA path picks between the register-resident INTRA-LANE maxpool
     ...
     *   use_intra_lane_maxpool =
     *       _maxpool_is_intra_lane(common_spec, grid)
     * (This is the gfx950 ACTIVE fast path. The WMMA register-resident fast path
     *  -- _maxpool_is_intra_lane_wmma -- is INACTIVE here; it is the gfx1201
     *  counterpart's path, never selected for the MFMA 32x32 warp tile.) */
    bool use_intra_lane_maxpool;
 ...
 * MFMA intra-lane register-residency decision and the A-load routing flags. The
 * caller owns the conv_spec / op storage the ctx points at. Returns CKC_OK or a
 ...
 * ctx->defer), then route to the MFMA intra-lane register-resident maxpool
 * (ckc_dfcp_emit_inline_maxpool_from_registers when
 *  ctx->use_intra_lane_maxpool -- the gfx950 ACTIVE fast path) OR the generic
 * cshuffle-LDS maxpool (stage conv1 accs +
 * ckc_dfcp_emit_inline_maxpool_from_cshuffle). The WMMA register-resident maxpool
 * (ckc_dfcp_emit_wmma_maxpool_from_registers) is NOT reachable from this gfx950
 * shim. Stages accs/grid/resources onto ctx, then walks the phases in Python
 * order. */
...
#endif /* CKC_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_INTERNAL_H */
