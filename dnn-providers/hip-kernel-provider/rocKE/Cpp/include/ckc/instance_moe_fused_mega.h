 ...
 *
 * ckc/instance_moe_fused_mega.h -- PUBLIC C99 API for the single-launch
 ...
 *   -------------------------------------   --------------------------------------
 *   class FusedMegaKernelSpec (frozen)      ckc_moe_fused_mega_kernel_spec_t
 *   FusedMegaKernelSpec defaults+__post_init__  ckc_moe_fused_mega_kernel_spec_default()
 *                                               + ckc_moe_fused_mega_kernel_spec_finalize()
 *   spec.gate_up_universal_spec()           ckc_moe_fused_mega_gate_up_universal_spec(...)
 *   spec.down_universal_spec()              ckc_moe_fused_mega_down_universal_spec(...)
 ...
 *   build_moe_fused_mega_gemm(spec, arch)   ckc_build_moe_fused_mega_gemm(...)   <-- PRIMARY
 *   (+ convenience: build -> lower .ll)     ckc_moe_fused_mega_lower_to_llvm(...)
 *
 ...
 * In C the caller fills a ckc_moe_fused_mega_kernel_spec_t:
 *   ckc_moe_fused_mega_kernel_spec_default() returns every field at the Python
 *   default; the caller overrides what it cares about (at least `name`), then
 *   calls ckc_moe_fused_mega_kernel_spec_finalize() to run __post_init__.
 *
 ...
 * helper_ck_dsl.instances.common.moe_gemm_fused.h, and the tensor_view /
 ...
 * the sticky-error IRBuilder (ckc_b_*); an invalid spec yields NULL + CKC_ERR_VALUE
 * (ValueError "invalid fused-mega ... GEMM spec"); the convenience lower returns
 * a ckc_status_t.
 */
#ifndef CKC_INSTANCE_MOE_FUSED_MEGA_H
#define CKC_INSTANCE_MOE_FUSED_MEGA_H
 ...
 * *out_count. Returns CKC_OK, or CKC_ERR_VALUE if out_cap is too small. */
 ...
 *      -> NULL + CKC_ERR_VALUE "invalid fused-mega gate+up GEMM spec: <why>".
 *   2. u_down = spec.down_universal_spec(); is_valid_gemm_spec(u_down, arch)
 *      -> NULL + CKC_ERR_VALUE "invalid fused-mega down GEMM spec: <why>".
 *   3. Set builder attrs (max_workgroup_size, optional waves_per_eu).
 *   4. ckc_moe_mega_build_ctx_init(...) -- run the WHOLE prologue into the ctx
 *      (params -> geometry -> thread decode -> per-expert B byte bases -> LDS
 *      allocs -> views -> plans/operands -> acc inits -> down setup).
 *   5. scf_if(expert_idx >= 0) { ckc_moe_mega_emit_body(ctx); }  -- the empty
 *      tail block (BlockExpertIds == -1) skips all work.
 *   6. Return b->kernel.
 *
 * The prologue locals + the STAGE-1..5 body phases live in
 * ckc/instance_moe_fused_mega_internal.h (ckc_moe_mega_build_ctx_t +
 * ckc_moe_mega_emit_* prototypes); this entry owns the ctx + the u_gu/u_down
 * scratch for the duration of the build.
 *
 * NOTE: like the Python, this expects the builder created with the spec's
 * kernel_name(); use ckc_build_moe_fused_mega_gemm_new() for the convenience. */
 ...
 * and frees it with ckc_ir_builder_free(). Returns the kernel or NULL with b's
...
#endif /* CKC_INSTANCE_MOE_FUSED_MEGA_H */
