 ...
 *
 * ckc/helper_ck_dsl.helpers.fuse.h -- C99 port of a SUBSET of
 * ck_dsl/helpers/fuse.py (plus _MultiDEpilogue, which physically lives in
 * ck_dsl/instances/common/gemm_multi_d.py but is a FusedEpilogue subclass and
 * is ported here next to its base).
 *
 *   Python                                  C99 (this header)
 *   -------------------------------------   -------------------------------------
 *   dtype_to_ir(dtype)                      ckc_fuse_dtype_to_ir / _str / _b_str
 *   ir_dtype_zero(b, dtype)                 ckc_fuse_ir_dtype_zero
 *   ir_dtype_const(b, dtype, value)         ckc_fuse_ir_dtype_const
 *   ir_dtype_global_load(b, dtype, p, i)    ckc_fuse_ir_dtype_global_load
 *   class ResidualAdd(EpilogueOp)           ckc_residual_add_* (kind RESADD)
 *   class ResidualMul(EpilogueOp)           ckc_residual_mul_* (kind RESMUL)
 *   class EpilogueOp                        ckc_epilogue_op_t (tagged vtable)
 *   class FusedEpilogue                     ckc_fused_epilogue_t + ckc_fe_*
 *   class _MultiDEpilogue(FusedEpilogue)    ckc_multi_d_epilogue_t + ckc_mde_*
 *
 ...
 * error (ckc_i_set_err) and return NULL, matching the rest of the C port.
 *
 ...
 * "__N" and "__stride_m") is modelled by ckc_fe_params_t. declare_params /
 * record_runtime populate it; apply_* read it.
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_FUSE_H
#define CKC_HELPER_CK_DSL_HELPERS_FUSE_H
...
/* Builder-aware string variant: same mapping as ckc_fuse_dtype_to_ir_str, but
 * on an unsupported string records CKC_ERR_VALUE with the Python-matching
 ...
 *   F32  -> const_f32(0.0)
 * Any other type sets CKC_ERR_NOTIMPL (Python NotImplementedError). */
 ...
 * dispatch as ckc_fuse_ir_dtype_zero with `value` in place of 0.0; CKC_ERR_NOTIMPL
 ...
 * we pass align=0 = "use default" here too.) CKC_ERR_NOTIMPL otherwise. */
 ...
 * behaviour (ResidualAdd / ResidualMul); CKC_EOP_OTHER is the catch-all for any
 ...
 * callers building these from a string dtype should run ckc_fuse_dtype_to_ir_str
 ...
 * Written NUL-terminated into out (capacity out_cap). CKC_OK on success;
 * CKC_ERR_VALUE if the buffer is too small or the op kind has no ported tag. */
 ...
 * it into the chain's params under `op->param_name` (ckc_fe_declare_params does
...
typedef struct ckc_fe_params
{
    ckc_fe_param_entry_t entries[CKC_FE_MAX_PARAMS];
    size_t count;
} ckc_fe_params_t;
...
typedef struct ckc_fused_epilogue
{
 ...
 * the SSA values into _live_params keyed by op param_name. Returns CKC_OK / the
 ...
 * (the empty-chain case yields "id"). NUL-terminated into out/out_cap. CKC_OK
 * or CKC_ERR_VALUE (buffer too small / an op has no ported tag). */
 ...
 * CKC_EOP_OTHER has no ported element behaviour -> CKC_ERR_NOTIMPL. */
...
typedef enum ckc_mde_load_kind
{
    CKC_MDE_STOCK = 0,
    CKC_MDE_TILED,
    CKC_MDE_VECTOR
...
typedef struct ckc_multi_d_epilogue
{
    ckc_fused_epilogue_t base;
    ...
    ckc_epilogue_op_kind_t residual_kinds[CKC_FE_MAX_PARAMS];
    const ckc_type_t* residual_dtypes[CKC_FE_MAX_PARAMS];
    ckc_mde_load_kind_t load_kind;
} ckc_multi_d_epilogue_t;
 ...
 * VECTOR is the Python default). Returns CKC_OK, or CKC_ERR_VALUE if num_ops
 * exceeds CKC_FE_MAX_PARAMS. */
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_FUSE_H */
