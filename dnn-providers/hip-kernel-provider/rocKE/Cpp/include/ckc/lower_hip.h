 ...
 *
 * ckc/lower_hip.h -- PUBLIC API for the C99 port of
 * ck_dsl.core.lower_hip (lower_kernel_to_hip).
 *
 ...
 * the default gfx950 arch. The only entry point is ckc_lower_kernel_to_hip();
 * the rest of the file is shared internal plumbing in lower_hip_internal.h.
 *
 * Binds strictly to the FROZEN IR contract in ckc/ir.h. No new IR types are
 * introduced here -- only the arch seam (ckc_hip_arch_t) and the output struct
 * are HIP-lowerer-local.
 */
#ifndef CKC_LOWER_HIP_H
#define CKC_LOWER_HIP_H
 ...
 *
 * Resolve one from a gfx string with ckc_hip_arch_from_gfx(); the default is
...
/* Options for ckc_lower_kernel_to_hip (the Python keyword-only args). A *_set
 ...
 * true, arch=gfx950). */
typedef struct ckc_lower_hip_opts
{
    int launch_bounds;
...
extern const char* const CKC_HIP_PROLOGUE;
 ...
 * defaults. Returns CKC_OK on success; on failure returns a non-OK status and
 ...
 * yields CKC_ERR_NOTIMPL, mirroring Python NotImplementedError.
 *
