 ...
 *
 * ckc/instance_gfx950_attention_tiled_3d.h -- PUBLIC C99 port surface for the
 * gfx950 (CDNA4) WIDE-K tiled split-KV 3D attention kernels
 * (ck_dsl/instances/gfx950/attention_tiled_3d.py, 1164 LOC).
 *
 ...
 *   ------------------------------------------    -------------------------------------
 *   @dataclass UnifiedAttention3DTiledSpec        ckc_unified_attention_3d_tiled_spec_t
 *     (the dataclass + __post_init__ + @property)   + ckc_..._spec_default()
 *                                                   + ckc_..._spec_validate()
 *                                                   + the @property accessors
 *   @dataclass UnifiedAttentionReduceTiledSpec    ckc_unified_attention_reduce_tiled_spec_t
 *   supports_tiled_3d(...)                        ckc_gfx950_attention_tiled_3d_supports
 *   build_unified_attention_3d_tiled(spec, arch)  ckc_build_unified_attention_3d_tiled_gfx950
 *   build_unified_attention_reduce_tiled(...)     ckc_build_unified_attention_reduce_tiled_gfx950
 *   (build -> lower .ll convenience, NEW)         ckc_..._lower_to_llvm
 *
 ...
 * exports ckc_build_unified_attention_3d_tiled_gfx942; the shared common/
 * attention_unified files carry their own scalars). The spec STRUCT TAGS
 * (ckc_unified_attention_3d_tiled_spec_t / ckc_unified_attention_reduce_tiled_spec_t)
 ...
 *   ckc/helper_helper_ck_dsl.helpers.attention.h   -- apply_softcap_log2,
 *       binary_search_seq_idx, wave64_reduce_max, wave64_reduce_sum
 *   ckc/helper_ck_dsl.helpers.attention.h          -- warp_xor_reduce_sum,
 *       dequant_fp8x8_to_dtype (the ONE new helper this kernel threads), masks,
 *       safe_inv_l
 *   ckc/helper_ck_dsl.helpers.layouts.h            -- TransposeLdsReader (the
 *       gfx950-only PV transpose-read lane formulas)
 *   ckc/helper_ck_dsl.helpers.atoms.h              -- mfma_atom catalog
 ...
 *   ckc/helper_ck_dsl.helpers.transforms.h         -- TensorDescriptor naive/
 ...
 * kernel, ck_dsl.helpers.attention.dequant_fp8x8_to_dtype, ALREADY PORTED as
 * ckc_dequant_fp8x8_to_dtype in ckc/helper_ck_dsl.helpers.attention.h. The
 ...
 * ck_dsl.helpers.attention symbols the gfx942 sibling consumes; they are wired
 ...
 * (ck_dsl/instances/common/attention_arch.py): the tiled 3D kernel needs the
 ...
 * NULL no-op). The lower convenience returns a ckc_status_t and writes a
 ...
 */
#ifndef CKC_INSTANCE_GFX950_ATTENTION_TILED_3D_H
#define CKC_INSTANCE_GFX950_ATTENTION_TILED_3D_H
...
#define CKC_GFX950_ATTN_TILED_3D_MFMA_M 16
#define CKC_GFX950_ATTN_TILED_3D_MFMA_N 16
...
#define CKC_GFX950_ATTN_TILED_3D_QK_K_STEP 32
/* CDNA4 async global->LDS DMA delivers 4 DWORDS (8 halves = 16 bytes) per lane. */
#define CKC_GFX950_ATTN_TILED_3D_ASYNC_LDS_DWORDS 4
/* THREADS: single wave. */
#define CKC_GFX950_ATTN_TILED_3D_THREADS 64
/* rcp(ln2) folded into the score (exp2 domain). */
#define CKC_GFX950_ATTN_TILED_3D_RCP_LN2 1.4426950408889634
 ...
 * them (it defines CKC_INSTANCE_GFX942_ATTENTION_TILED_3D_H). When this header is
...
/* ------------------------------------------------------- reduce-kernel spec */
typedef struct ckc_unified_attention_reduce_tiled_spec
{
    int head_size;
    int num_query_heads;
    int num_kv_heads;
    ...
    bool has_waves_per_eu; /* Optional[int] None */
    int waves_per_eu;
} ckc_unified_attention_reduce_tiled_spec_t;

ckc_unified_attention_reduce_tiled_spec_t ckc_unified_attention_reduce_tiled_spec_default(void);

#endif /* !CKC_INSTANCE_GFX942_ATTENTION_TILED_3D_H (shared spec POD) */
 ...
 * ValueError text on b (CKC_ERR_VALUE) and returns false. Dead/NULL b is a
 * false no-op. */
 ...
 * caller owns b and frees it with ckc_ir_builder_free(). Returns the kernel
 ...
 * validated by ckc_gfx950_unified_attention_3d_tiled_spec_validate. */
 ...
 * ============================================================ *
 *
 * Build into an internally-owned builder, then lower to LLVM .ll text. On CKC_OK
...
#endif /* CKC_INSTANCE_GFX950_ATTENTION_TILED_3D_H */
