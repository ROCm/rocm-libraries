 ...
 *
 * ckc/helper_ck_dsl.helpers.loads.h -- C99 port of two loader symbols from
 * ck_dsl/helpers/loads.py:
 *
 *   Python                          C99 (this header)
 *   ------------------------------  -----------------------------------------
 *   CoalescedTileLoader (frozen dc) ckc_coalesced_tile_loader_t
 *     .choose_vec  (classmethod)      ckc_coalesced_tile_loader_choose_vec()
 *     .from_tile   (classmethod)      ckc_coalesced_tile_loader_from_tile()
 *     .vecs_per_thread (property)     ckc_coalesced_tile_loader_vecs_per_thread()
 *     .cols_per_vec    (property)     ckc_coalesced_tile_loader_cols_per_vec()
 *     .load        (method)          ckc_coalesced_tile_loader_load()
 *   AsyncTileLoader (frozen dc)     ckc_async_tile_loader_t
 *     .choose_dwords (classmethod)    ckc_async_tile_loader_choose_dwords()
 *     .from_tile     (classmethod)    ckc_async_tile_loader_from_tile()
 *     .halves_per_chunk (property)    ckc_async_tile_loader_halves_per_chunk()
 *     .bytes_per_chunk  (property)    ckc_async_tile_loader_bytes_per_chunk()
 *     .cols_per_chunk   (property)    ckc_async_tile_loader_cols_per_chunk()
 *     .wave_bytes       (property)    ckc_async_tile_loader_wave_bytes()
 *     .pass_bytes       (property)    ckc_async_tile_loader_pass_bytes()
 *     .bind          (method)         ckc_async_tile_loader_bind()
 *   AsyncTileLoaderSlot (frozen dc) ckc_async_tile_loader_slot_t
 *     .issue         (method)         ckc_async_tile_loader_slot_issue()
 *     .required_lds_bytes (method)    ckc_async_tile_loader_slot_required_lds_bytes()
 *
 ...
 * use a ckc_status_t out-param contract for the ValueError paths.
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_LOADS_H
#define CKC_HELPER_CK_DSL_HELPERS_LOADS_H
...
typedef ckc_value_t* (*ckc_loads_descriptor_fn)(
...
/* Value type mirroring ck_dsl.helpers.loads.CoalescedTileLoader (frozen
 * dataclass). Fields are 1:1 with the Python declaration order. */
typedef struct ckc_coalesced_tile_loader
{
    int tile_rows;
    int tile_cols;
    int block_size;
 ...
 * and returns CKC_OK; propagates CKC_ERR_VALUE from choose_vec. max_vec default
 ...
 *
 * Writes the result to *out and returns CKC_OK, or CKC_ERR_VALUE on the Python
...
/* Value type mirroring ck_dsl.helpers.loads.AsyncTileLoader (frozen dataclass).
 * Fields are 1:1 with the Python declaration order. */
typedef struct ckc_async_tile_loader
{
    int tile_rows;
    int tile_cols;
    int block_size;
...
typedef struct ckc_async_tile_loader_slot
{
    ckc_async_tile_loader_t loader; /* by value (frozen dataclass copy) */
    ckc_value_t* smem_dst;
    ckc_value_t* per_wave_lds_base; /* i64; lane 0 of the wave writes here */
} ckc_async_tile_loader_slot_t;

/* AsyncTileLoader.choose_dwords classmethod.
 *
 * Picks the widest `dwords` value (4, 3, or 1) that divides the tile evenly.
 * Writes it to *out and returns CKC_OK, or CKC_ERR_VALUE on the Python
 ...
 * success writes the loader to *out and returns CKC_OK; propagates
 * CKC_ERR_VALUE. wave_size default 64, max_dwords default 4. */
 ...
 * and returns CKC_OK. No-op returning b->status if `b` is already in error. */
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_LOADS_H */
