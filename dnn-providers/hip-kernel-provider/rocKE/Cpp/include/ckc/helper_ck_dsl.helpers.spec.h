 ...
 *
 * ckc/helper_ck_dsl.helpers.spec.h -- C99 port of three symbols from
 * ck_dsl/helpers/spec.py:
 *
 *   Python                              C99 (this header)
 *   ---------------------------------   ---------------------------------------
 *   WarpTileBlockSizeMixin              ckc_warp_tile_block_size() +
 *     ._init_block_size()                ckc_warp_tile_init_block_size()
 *   choose_load_vec(...)               ckc_choose_load_vec(...)
 *   kernel_name_join(...)              ckc_kernel_name_join(...)
 *
 * These three are pure compute / string helpers: NONE of them call the IR
 * builder (ckc_b_*). They are bit-for-bit value producers whose results are
 ...
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_SPEC_H
#define CKC_HELPER_CK_DSL_HELPERS_SPEC_H
 ...
 *
 *   - ckc_warp_tile_block_size() is the pure derive_block_size().
 *   - ckc_warp_tile_init_block_size() is the lazy "derive me" guard: it
...
typedef struct ckc_io_spec_rule
{
    const char* dtype;
    int block_size;
    int vec;
 ...
 * CKC_ERR_VALUE and leave *out_vec untouched. On success *out_vec holds the
 * chosen width and CKC_OK is returned. */
 ...
 * "global". On OOM the entry fields are left NULL; returns CKC_OK on success,
 * CKC_ERR_OOM on allocation failure. */
 ...
 * CKC_ERR_VALUE if either pointer is NULL. */
 ...
 * or a num_dims outside [1,3] is the Python ValueError -> CKC_ERR_VALUE (out is
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_SPEC_H */
