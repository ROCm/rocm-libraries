 ...
 * value struct, ``ckc_matmul_nbits_decode_gemv_spec_t``, carrying exactly the
 ...
 *   - ckc/ir.h                          (IRBuilder + Value API)
 *   - the i4 unpack helper: ck_dsl.helpers.i4_dequant.unpack_i4_byte_to_pair_f32
 *     -> ckc_unpack_i4_byte_to_pair_f32(b, byte, &lo, &hi). Declared (extern)
 ...
 */
#ifndef CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_DECODE_GEMV_H
#define CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_DECODE_GEMV_H
 ...
 * The decode body picks ckc_f16() for CKC_NBITS_SCALE_F16 and ckc_f32()
 * otherwise, mirroring:
...
/* build_decode_gemv_matmul_nbits(spec, arch="gfx1151") -> KernelDef.
 *
 * Builds the decode-GEMV IR into the supplied (already ckc_ir_builder_init'd)
...
/* ---- peer helper (ck_dsl.helpers.i4_dequant) ----
 *
...
void ckc_unpack_i4_byte_to_pair_f32(ckc_ir_builder_t* b,
...
#endif /* CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_DECODE_GEMV_H */
