 ...
 *
 * ckc/instance_moe_sorting.h -- C99 port of the MoE-sorting kernel instance
 * builders in ck_dsl/instances/common/moe_sorting.py (CK Tile 13_moe_sorting
 * parity).
 *
 ...
 *   ------------------------------------   -------------------------------------
 *   @dataclass(frozen) MoeSortingSpec       ckc_moe_sorting_spec_t
 *     .total_pairs (@property)              ckc_moe_sorting_spec_total_pairs
 *     .kernel_name(phase)                   ckc_moe_sorting_spec_kernel_name
 *   is_valid_spec(spec, arch)               ckc_moe_sorting_is_valid_spec
 *   build_moe_sort_histogram(spec, arch)    ckc_build_moe_sort_histogram
 *   build_moe_sort_scan(spec, arch)         ckc_build_moe_sort_scan
 *   build_moe_sort_scatter(spec, arch)      ckc_build_moe_sort_scatter
 *   build_moe_sort_persistent(spec, arch)   ckc_build_moe_sort_persistent
 *   moe_sort_*_grid(spec)                    ckc_moe_sort_*_grid
 *   moe_sort_*_signature(spec)               ckc_moe_sort_*_signature
 *   moe_sorting_workspace_bytes(spec)        ckc_moe_sorting_workspace_bytes
 *   (+ convenience: build -> lower .ll)      ckc_build_moe_sort_*_lower_to_llvm
 *
 * SPEC AS AN EXPLICIT C STRUCT. The frozen Python dataclass becomes a value
 * struct; ckc_moe_sorting_spec_default() installs the dataclass defaults
 * (block_size 256, name "ck_dsl_moe_sorting") so a caller overrides only the
 ...
 *   - ckc/helper_ck_dsl.helpers.scan.h       : ckc_lds_zero_i32 +
 *     ckc_block_exclusive_scan_i32 (NEWLY ported for this instance; the inline
 *     _wave_kogge_stone_scan_i32 stays a static helper inside the scan TU).
 *   - ckc/helper_ck_dsl.helpers.spec.h       : kernel_name_join, ceil_div_grid,
 *     SignatureBuilder, ckc_sig_entry_t.
 *   - ckc/helper_ck_dsl.helpers.transforms.h : unmerge_magic + CoordVar (the
 *     pair-index decode).
 *   - ckc/helper_ck_dsl.core.arch.h          : ArchTarget.from_gfx + .wave_size.
 *
 ...
 * ckc/instance_moe_sorting_internal.h (included only by the .c TUs).
 */
#ifndef CKC_INSTANCE_MOE_SORTING_H
#define CKC_INSTANCE_MOE_SORTING_H
 ...
 * ===================================================================== */
typedef struct ckc_moe_sorting_spec
{
    int tokens;
    int topk;
    int experts;
 ...
 * name "ck_dsl_moe_sorting") and the three required dims zeroed. Caller fills
 * tokens, topk, experts. */
 ...
 * terminated into out (capacity out_cap). Returns CKC_OK or CKC_ERR_VALUE. */
 ...
 *  BUILD ENTRIES
 *
 *  Each builds the IR into the supplied (already ckc_ir_builder_init'd with
 ...
 *  Returns CKC_OK or CKC_ERR_VALUE (NULL args / non-positive block_size).
 * ===================================================================== */
 ...
 *  *out_count. Strings live in `arena`. Returns CKC_OK or CKC_ERR_VALUE (NULL
...
#endif /* CKC_INSTANCE_MOE_SORTING_H */
