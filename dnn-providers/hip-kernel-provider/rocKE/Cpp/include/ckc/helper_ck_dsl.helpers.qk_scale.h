 ...
 *
 * ckc/helper_ck_dsl.helpers.qk_scale.h -- C99 port of ck_dsl.helpers.qk_scale.
 *
 ...
 *   - QkScaleLayout            -> ckc_qk_scale_layout_t
 *   - QkScaleSpec              -> ckc_qk_scale_spec_t (+ init/validate)
 *   - apply_qk_scales          -> ckc_b_apply_qk_scales
 *   - load_k_scale_for_block   -> ckc_b_load_k_scale_for_block
 *   - load_q_scale_for_block   -> ckc_b_load_q_scale_for_block
 *
 ...
 *
 *   * QkScaleSpec validation is exposed as ckc_qk_scale_spec_validate (returns
 ...
 *     model: an unsupported pointer or score type records CKC_ERR_VALUE + the
 *     Python-matching message on the builder and returns NULL.
 *   * An already-errored builder makes every ckc_b_* call a NULL no-op.
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_QK_SCALE_H
#define CKC_HELPER_CK_DSL_HELPERS_QK_SCALE_H
 ...
 * ckc_qk_scale_layout_name for any consumer that needs the exact Python text
 * (e.g. error messages). */
 ...
 * __post_init__ validation; call ckc_qk_scale_spec_validate for that. */
 ...
 * ptr<f32>; a non-typed-ptr / non-ptr<f32> operand records CKC_ERR_VALUE + the
 ...
 *
 * Symmetric to ckc_b_load_q_scale_for_block; used per K-iteration in the Sage
 ...
 * CKC_ERR_VALUE + the Python-matching message and returns NULL; otherwise
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_QK_SCALE_H */
