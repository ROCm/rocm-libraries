 ...
 *
 * ckc/instance_fmha_varlen.h -- C99 port of the variable-length FMHA forward
 * kernel instance builder ck_dsl/instances/common/fmha_varlen.py (CK Tile
 * 01_fmha varlen parity).
 *
 *   Python (fmha_varlen.py)               C99 (this header)
 *   -----------------------------------   --------------------------------------
 *   @dataclass FmhaFwdVarlenSpec          ckc_fmha_fwd_varlen_spec_t
 *     .kernel_name()                      ckc_fmha_fwd_varlen_kernel_name(...)
 *   is_valid_spec(spec, arch)             ckc_fmha_fwd_varlen_is_valid_spec(...)
 *   build_fmha_fwd_varlen(spec, arch)     ckc_build_fmha_fwd_varlen(...)
 *   fmha_fwd_varlen_grid(spec, total_q)   ckc_fmha_fwd_varlen_grid(...)
 *   (+ convenience: build -> lower .ll)   ckc_fmha_fwd_varlen_lower_to_llvm(...)
 *
 * SPEC AS AN EXPLICIT C STRUCT. The Python FmhaFwdVarlenSpec is a frozen value
 * type wrapping an FmhaCommonSpec (ported in
 * ckc/helper_ck_dsl.instances.common._fmha_common.h) plus the three varlen
 * scalars (max_seqlen_q, max_seqlen_k, batch) and the kernel-name prefix. The
 * caller fills a ckc_fmha_fwd_varlen_spec_t; ckc_fmha_fwd_varlen_spec_default()
 * returns a struct with the name prefix at the Python dataclass default.
 *
 * BUILDER-CALL FIDELITY. ckc_build_fmha_fwd_varlen reproduces the Python
 ...
 */
#ifndef CKC_INSTANCE_FMHA_VARLEN_H
#define CKC_INSTANCE_FMHA_VARLEN_H
 ...
 * NULL is treated as the dataclass default "ck_dsl_fmha_fwd_varlen". */
typedef struct ckc_fmha_fwd_varlen_spec
{
    ckc_fmha_common_spec_t common;
    int max_seqlen_q;
    int max_seqlen_k;
    int batch;
    const char* name; /* default "ck_dsl_fmha_fwd_varlen" */
} ckc_fmha_fwd_varlen_spec_t;
 ...
 * ckc_fmha_kernel_builder_free(out_kb). This mirrors the Python returning
...
void ckc_fmha_fwd_varlen_grid(const ckc_fmha_fwd_varlen_spec_t* spec, int total_q, int out_grid[3]);
...
#endif /* CKC_INSTANCE_FMHA_VARLEN_H */
