 ...
 *
 * ckc/instance_gfx950_deep_fused_conv_pool.h -- PUBLIC C99 API for the gfx950
 * (CDNA, wave64, MFMA 32x32x16) arch shim over the family-agnostic deep-fused
 * conv0 -> conv1 -> maxpool kernel, ported from
 *   ck_dsl/instances/gfx950/deep_fused_conv_pool.py  (67-LOC thin shim).
 *
 * WHAT THIS SHIM IS.
 ...
 *       name only,
 *     - sets the gfx950 kernel name ("ck_dsl_gfx950_deep_fused_conv_pool"),
 ...
 *   type (ckc_gfx950_deep_fused_conv_pool_spec_t) whose factory stamps the
 ...
 *   ---------------------------------------------   --------------------------------------
 *   Gfx950DeepFusedConvPoolSpec (frozen)            ckc_gfx950_deep_fused_conv_pool_spec_t
 *   make_deep_fused_conv_pool_spec(...)             ckc_gfx950_deep_fused_conv_pool_make_spec(...)
 *   (default-constructed gfx950 spec) ckc_gfx950_deep_fused_conv_pool_spec_default(...)
 *   is_valid_spec (re-export) ckc_gfx950_deep_fused_conv_pool_is_valid_spec(...)
 *   deep_fused_conv_pool_signature (re-export)      ckc_gfx950_deep_fused_conv_pool_signature(...)
 *   deep_fused_conv_pool_grid (re-export)           ckc_gfx950_deep_fused_conv_pool_grid(...)
 *   (kernel_name re-export) ckc_gfx950_deep_fused_conv_pool_kernel_name(...)
 *   build_deep_fused_conv_pool (re-export)          ckc_build_gfx950_deep_fused_conv_pool(...)
 *   (+ convenience: init+build)                     ckc_build_gfx950_deep_fused_conv_pool_new(...)
 *   (+ convenience: build -> lower .ll) ckc_gfx950_deep_fused_conv_pool_lower_to_llvm(...)
 *
 * SPEC TYPE NOTE (struct layout = common spec).
 *   ckc_gfx950_deep_fused_conv_pool_spec_t embeds the common
 ...
 *   build-context in ckc/instance_gfx950_deep_fused_conv_pool_internal.h
 *   (ckc_gfx950_dfcp_*); they are byte-faithful to the common closures driven by
 ...
 *   ckc_dfcp_maxpool_is_intra_lane); the WMMA register-resident path and the WMMA
 ...
 */
#ifndef CKC_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_H
#define CKC_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_H
 ...
 * fields to the common ckc_deep_fused_conv_pool_spec_t (embedded as `base`); the
 ...
 * ckc_make_deep_fused_conv_pool_spec(...) with name pinned to the gfx950 name,
 ...
 * shared build-context (ckc_gfx950_dfcp_build_ctx_t), then drives the wrapped
 * family-agnostic conv0 builder ckc_build_implicit_gemm_conv(...) with the nine
 ...
 * with ckc_ir_builder_free(). `b_unused` is named to flag that, like the Python,
 ...
 * lower to LLVM .ll text. `arch` NULL => "gfx950". On CKC_OK *out_ll receives a
...
#endif /* CKC_INSTANCE_GFX950_DEEP_FUSED_CONV_POOL_H */
