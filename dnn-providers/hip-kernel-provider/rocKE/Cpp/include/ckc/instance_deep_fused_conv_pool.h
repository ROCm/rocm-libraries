 ...
 *
 * ckc/instance_deep_fused_conv_pool.h -- PUBLIC C99 API for the family-agnostic
 * deep-fused conv0 -> conv1 -> maxpool kernel instance builder ported from
 * ck_dsl/instances/common/deep_fused_conv_pool.py (1494 LOC).
 *
 ...
 *   -------------------------------------   --------------------------------------
 *   FusedConvPoolProblem (frozen)           ckc_fused_conv_pool_problem_t  (helper hdr)
 *   DeepFusedConvPoolSpec (frozen)          ckc_deep_fused_conv_pool_spec_t (helper hdr)
 *   make_deep_fused_conv_pool_spec(...)     ckc_make_deep_fused_conv_pool_spec(...) (helper hdr)
 *   is_valid_spec(spec, arch)               ckc_deep_fused_conv_pool_is_valid_spec(...) (helper
 * hdr) deep_fused_conv_pool_signature(spec)    ckc_deep_fused_conv_pool_signature(...) (helper hdr)
 *   deep_fused_conv_pool_grid(spec)         ckc_deep_fused_conv_pool_grid(...) (helper hdr)
 *   build_deep_fused_conv_pool(spec, arch)  ckc_build_deep_fused_conv_pool(...)   <-- THIS HEADER
 *   (+ convenience: build -> lower .ll)     ckc_deep_fused_conv_pool_lower_to_llvm(...)
 *
 ...
 *     build entry forward-declares them via the opaque ckc_implicit_gemm_conv_spec_t
 *     + ckc_mma_op_t handles (see the internal header) and routes the override
 ...
 * the sticky-error IRBuilder (ckc_b_*); the validity gate (helper header)
 * returns a bool + reason; the convenience lower returns a ckc_status_t.
 */
#ifndef CKC_INSTANCE_DEEP_FUSED_CONV_POOL_H
#define CKC_INSTANCE_DEEP_FUSED_CONV_POOL_H
 ...
 *   1. ckc_deep_fused_conv_pool_is_valid_spec(spec, arch, ...) gate
 *      -> NULL + CKC_ERR_VALUE (ValueError "invalid deep fused conv/pool spec")
 *         when invalid.
 ...
 *      ckc_dfcp_extra_params phase) producing the W1 buffer resource.
 *   5. Build implicit_gemm_conv with an epilogue_override that:
 *        - stages conv0 accs to cshuffle LDS (sync=False)
 *          via ckc_dfcp_stage_accumulators_to_cshuffle_lds,
 *        - loads W1 weights to LDS (sync=False)
 *          via ckc_dfcp_load_conv1_weights_to_lds,
 *        - emits a single block-wide barrier (ckc_b_sync),
 *        - computes deferral via ckc_dfcp_epilogue_is_pool_deferrable,
 *        - emits conv1 1x1 GEMM via ckc_dfcp_emit_conv1_1x1
 *          (with deferred epilogue when pool-deferrable),
 *        - routes to the register-resident maxpool
 *          (ckc_dfcp_emit_inline_maxpool_from_registers for the MFMA-32x32 fast
 *           path gated by ckc_dfcp_maxpool_is_intra_lane, or
 *           ckc_dfcp_emit_wmma_maxpool_from_registers for the WMMA fast path
 *           gated by ckc_dfcp_maxpool_is_intra_lane_wmma) OR the generic
 ...
 *       ckc_dfcp_can_use_specialized_conv0_a_loader): input_cache_setup,
 ...
 * in ckc/instance_deep_fused_conv_pool_internal.h. The closures' captured locals
...
#endif /* CKC_INSTANCE_DEEP_FUSED_CONV_POOL_H */
