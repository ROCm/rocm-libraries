 ...
 *
 * ckc/instance_gfx1151_wmma_gemm_iu8.h -- C99 port of the gfx1151 (RDNA3.5)
 ...
 *   --------------------------------------   ------------------------------------
 *   class WmmaGemmIu8Spec                    ckc_wmma_gemm_iu8_spec_t
 *   WmmaGemmIu8Spec.block_size               ckc_wmma_gemm_iu8_block_size(spec)
 *   WmmaGemmIu8Spec.kernel_name()            ckc_wmma_gemm_iu8_kernel_name(...)
 *   is_valid_spec(spec, arch)                ckc_wmma_gemm_iu8_is_valid_spec(...)
 *   build_wmma_gemm_iu8(spec, arch)          ckc_build_wmma_gemm_iu8(...)
 *   wmma_gemm_iu8_grid(M, N)                 ckc_wmma_gemm_iu8_grid(...)
 *   (+ convenience: build -> lower .ll)      ckc_wmma_gemm_iu8_lower_to_llvm(...)
 *
 ...
 * field with a default; in C the caller fills a ckc_wmma_gemm_iu8_spec_t.
 * ckc_wmma_gemm_iu8_spec_default() returns a struct with the field set to the
 ...
 */
#ifndef CKC_INSTANCE_GFX1151_WMMA_GEMM_IU8_H
#define CKC_INSTANCE_GFX1151_WMMA_GEMM_IU8_H
...
typedef struct ckc_wmma_gemm_iu8_spec
{
    const char* name; /* default "ck_dsl_wmma_gemm_iu8" */
...
/* wmma_gemm_iu8_grid(M, N) -> ((M+15)//16, (N+15)//16, 1). Returns CKC_OK and
...
#endif /* CKC_INSTANCE_GFX1151_WMMA_GEMM_IU8_H */
