 ...
 *
 * ckc/helper_ck_dsl.instances.common._matmul_nbits_common.h -- C99 port of the
 * shared surface of the MatMulNBits gfx1151 instance family
 * (ck_dsl/instances/common/_matmul_nbits_common.py).
 *
 *   Python (_matmul_nbits_common.py)        C99 (this header)
 *   --------------------------------------  -------------------------------------
 *   MatMulNBitsFamily (Literal)             ckc_matmul_nbits_family_t (string set)
 *   FAMILIES                                ckc_matmul_nbits_families() / *_COUNT
 *   SUPPORTED_ARCHES                        ckc_matmul_nbits_arch_supported()
 *   V1_ARCH                                 CKC_MATMUL_NBITS_V1_ARCH
 *   V1_GROUP_SIZE                           CKC_MATMUL_NBITS_V1_GROUP_SIZE
 *   _scale_wire_dtype(scale_dtype)          ckc_matmul_nbits_scale_wire_dtype()
 *   class MatMulNBitsSpec                   ckc_matmul_nbits_spec_t
 *     .__post_init__ / _init_block_size      ckc_matmul_nbits_spec_finalize()
 *     .kernel_name()                         ckc_matmul_nbits_kernel_name()
 *   validate_common_spec(spec, arch)        ckc_matmul_nbits_validate_common_spec()
 *   matmul_nbits_signature(spec)            ckc_matmul_nbits_signature()
 *   matmul_nbits_grid(M, spec)              ckc_matmul_nbits_grid()
 *   matmul_nbits_outer_tiles(seq_len, spec) ckc_matmul_nbits_outer_tiles()
 *
 ...
 *
 * NONE of these symbols call the IR builder (ckc_b_*): they are pure value /
 * string producers. They reuse the canonical sibling helpers
 * (helper_ck_dsl.helpers.spec.h for derive_block_size / kernel_name_join, and
 * helper_ck_dsl.core.arch.h for ArchTarget) so the produced block_size, kernel
 * name, validity verdict, signature, and grid are byte-faithful to Python.
 *
 * Error model mirrors the rest of the C port: an out-param + ckc_status_t (or a
 * bool + reason buffer for the validity gate) stands in for the Python
 * `raise ValueError` / `(ok, reason)` tuple.
 */
#ifndef CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_COMMON_H
#define CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_COMMON_H
...
/* V1_ARCH = "gfx1151". */
#define CKC_MATMUL_NBITS_V1_ARCH "gfx1151"

/* V1_GROUP_SIZE = 32. */
#define CKC_MATMUL_NBITS_V1_GROUP_SIZE 32

/* FAMILIES = ("large_n", "skinny_n", "decode_gemv"). */
#define CKC_MATMUL_NBITS_FAMILIES_COUNT 3

/* SUPPORTED_ARCHES = frozenset({"gfx1151", "gfx1201"}). */
#define CKC_MATMUL_NBITS_SUPPORTED_ARCHES_COUNT 2
 ...
 * it; returns CKC_ERR_VALUE on an unsupported dtype and leaves *out untouched.
 * (`out` may be NULL to use the function as a pure validity probe.) */
ckc_status_t ckc_matmul_nbits_scale_wire_dtype(const char* scale_dtype, const char** out);
 ...
 * (ckc_gemm_tile_spec_t). String fields hold borrowed pointers compared by
 * strcmp, matching the Python str fields. block_size defaults to 0 and is
 * derived by ckc_matmul_nbits_spec_finalize() (the __post_init__ /
 * _init_block_size step). */
typedef struct ckc_matmul_nbits_spec
{
    const char* name;
    int N;
    int K;
...
ckc_matmul_nbits_spec_t ckc_matmul_nbits_spec_default(void);
 ...
 * the spec. */
void ckc_matmul_nbits_spec_finalize(ckc_matmul_nbits_spec_t* spec);
 ...
 * to skip the message. */
bool ckc_matmul_nbits_validate_common_spec(const ckc_matmul_nbits_spec_t* spec,
                                           const char* arch,
                                           char* reason,
                                           size_t reason_cap);
 ...
 * canonical ckc_signature_builder_* port (so the produced array is byte-faithful
 * to Python's List[dict]). On CKC_OK *out_items / *out_count expose the
 * arena-owned array (read-only for the arena's lifetime); on a bad scale_dtype
 * the underlying _scale_wire_dtype ValueError surfaces as CKC_ERR_VALUE. */
ckc_status_t ckc_matmul_nbits_signature(const ckc_matmul_nbits_spec_t* spec,
                                        ckc_arena_t* arena,
 ...
 * may be NULL to query *out_count only (pass out_cap=0). Returns CKC_OK, or
 * CKC_ERR_VALUE on seq_len < 0 (matching the Python ValueError) or when the
 * (non-NULL) arrays are too small for the produced count. */
ckc_status_t ckc_matmul_nbits_outer_tiles(int seq_len,
...
#endif /* CKC_HELPER_CK_DSL_INSTANCES_COMMON_MATMUL_NBITS_COMMON_H */
