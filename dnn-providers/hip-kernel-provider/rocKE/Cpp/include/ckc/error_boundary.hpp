...
auto guard_builder(ckc_ir_builder_t* b, Fn&& fn) -> decltype(fn())
{
    ...
    {
        ckc_i_set_err_msg(b, CKC_ERR_OOM, e.what());
        return static_cast<R>(nullptr);
    }
    catch(const std::exception& e)
    {
        ckc_i_set_err_msg(b, CKC_ERR_VALUE, e.what());
        return static_cast<R>(nullptr);
    }
    catch(...)
    {
        ckc_i_set_err_msg(b, CKC_ERR_VALUE, "unknown C++ exception at extern \"C\" boundary");
        return static_cast<R>(nullptr);
    }
}
