 ...
 *
 * ckc/helper_ck_dsl.helpers.rotary.h -- C99 port of ck_dsl.helpers.rotary.
 *
 ...
 *   * RotarySpec               -> ckc_rotary_spec_t + ckc_rotary_spec_init()
 *                                 (+ pair_count / stride_pos accessors)
 *   * pair_indices             -> ckc_rotary_pair_indices()
 *   * load_cos_sin             -> ckc_rotary_load_cos_sin()
 *   * apply_rotary_pair_f32    -> ckc_rotary_apply_pair_f32()
 *
 ...
 * pair_indices), the C spelling returns a ckc_status_t and writes nothing to
 ...
 * the matching CKC_ERR_VALUE on the sticky-error builder and returns NULL --
 ...
 */
#ifndef CKC_HELPER_CK_DSL_HELPERS_ROTARY_H
#define CKC_HELPER_CK_DSL_HELPERS_ROTARY_H
 ...
 * forms are recoverable via ckc_rotary_layout_name() for diagnostics. */
 ...
 * ckc_rotary_spec_init() so __post_init__'s validation runs; reading the
 ...
 *   * head_size must be positive and even, else CKC_ERR_VALUE;
 *   * layout must be CKC_ROTARY_INTERLEAVED or CKC_ROTARY_HALF, else
 *     CKC_ERR_VALUE.
 ...
 * CKC_ROTARY_HALF and table_stride_pos = 0. On failure *out is left untouched.
 * Returns CKC_OK or CKC_ERR_VALUE. */
 ...
 *
 * The three type checks set the matching CKC_ERR_VALUE on the builder (the
 * Python ValueError text is reproduced) and return CKC_ERR_VALUE with the
 * out-params untouched. On success *out_cos / *out_sin receive the two loaded
 * f32 Values and the call returns CKC_OK. NULL no-op (returns the sticky
 ...
 *
 * The two f32 type checks set CKC_ERR_VALUE (matching the Python text) and
 * return CKC_ERR_VALUE with the out-params untouched. On success *out_lo /
 * *out_hi receive the rotated f32 Values and the call returns CKC_OK. NULL
...
#endif /* CKC_HELPER_CK_DSL_HELPERS_ROTARY_H */
