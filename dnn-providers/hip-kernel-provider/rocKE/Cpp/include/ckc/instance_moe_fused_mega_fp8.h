 ...
 *
 * ckc/instance_moe_fused_mega_fp8.h -- C99 port of the single-launch fused-MoE
 ...
 *   ----------------------------------------   -----------------------------------
 *   @dataclass(frozen=True)                    ckc_fused_mega_kernel_spec_fp8_t
 *     FusedMegaKernelSpecFp8
 *   spec.* @property / methods                 ckc_fused_mega_fp8_spec_*(...)
 *     gate_up_atom / down_atom                 ckc_fused_mega_fp8_spec_{gate_up,down}_atom
 *     mfmas_m / mfmas_n /                       ckc_fused_mega_fp8_spec_mfmas_{m,n,m_down,n_down}
 *       mfmas_m_down / mfmas_n_down
 *     kernel_name()                            ckc_fused_mega_fp8_spec_kernel_name(...)
 *   moe_fused_mega_fp8_grid(...)               ckc_moe_fused_mega_fp8_grid(...)
 *   moe_fused_mega_fp8_persistent_grid(...)    ckc_moe_fused_mega_fp8_persistent_grid(...)
 *   moe_fused_mega_fp8_signature(spec, persi)  ckc_moe_fused_mega_fp8_signature(...)
 *   build_moe_fused_mega_gemm_fp8(spec, arch,  ckc_build_moe_fused_mega_gemm_fp8(...)
 *     persistent)
 *   (+ convenience: build -> lower .ll)        ckc_moe_fused_mega_fp8_lower_to_llvm(...)
 *
 ...
 * ckc_fused_mega_kernel_spec_fp8_t; ckc_fused_mega_kernel_spec_fp8_default()
 ...
 * ckc/instance_moe_fused_mega_fp8_internal.h; public callers only touch THIS
 * header.
 *
 ...
 * carried on a ckc_fused_mega_fp8_levers_t bundle: pass NULL to get the Python
 ...
 * ckc/helper_helpers.asm.h and selected only by the asm/cluster lever paths.
 *
 * Error model mirrors the rest of the C port: build/lower routes errors through
 * the sticky-error IRBuilder (ckc_b_*); the validity is enforced inside build
 * (Python raises ValueError; here the builder sticky error is set and NULL is
 * returned); the convenience lower returns a ckc_status_t.
 */
#ifndef CKC_INSTANCE_MOE_FUSED_MEGA_FP8_H
#define CKC_INSTANCE_MOE_FUSED_MEGA_FP8_H
 ...
 * ckc_fused_mega_fp8_levers_default()).
 *
...
typedef struct ckc_fused_mega_kernel_spec_fp8
{
 ...
 * out_grid[3]. Returns CKC_OK or CKC_ERR_VALUE on NULL. */
 ...
 *   launch grid = (P, 1, 1).
 * Pass p_cap<=0 to use CKC_MOE_FP8_PERSISTENT_P_CAP. Writes out_grid[3] = (P,1,1)
 ...
 * caller-supplied `arena` (its lifetime owns the strings + array). On CKC_OK
 ...
 *                 one-shot path.
 *   levers     NULL => ckc_fused_mega_fp8_levers_default() (golden-safe). */
...
#endif /* CKC_INSTANCE_MOE_FUSED_MEGA_FP8_H */
