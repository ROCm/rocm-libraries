...
SpatialSingleConfig defaultConfigSpatialSingle(int n,
                                                unsigned int hw,
                                                unsigned int nhw,
                                                bool isNHWC,
                                                bool isFpMix)
...
void getLocalConfigNHWC(size_t c,
                        size_t hw,
                        bool isFp32,
                        size_t vecSize,
                        size_t& xlocalsize,
...
void getHeuristicsConfigTuningNHWC(size_t n,
                                   size_t c,
                                   size_t hw,
                                   size_t& vecSize,
                                   size_t& xlocalsize)
{
    size_t c_next_pow2 =
        static_cast<size_t>(1) << static_cast<int>(std::ceil(std::log2(static_cast<double>(c))));
    if(c != c_next_pow2)
    {
        size_t max_modulo = 0;
...
size_t
    BatchnormBwdPlan::getWorkspaceSize([[maybe_unused]] const HipdnnEnginePluginHandle& handle) const
{
    return 0;
}
...
void BatchnormBwdPlan::executeSpatial(const HipdnnEnginePluginHandle& handle,
                                      const hipdnnPluginDeviceBuffer_t* deviceBuffers,
                                      uint32_t numDeviceBuffers) const
{
    ...
    auto xBuffer =
        hip_kernel_utils::findDeviceBuffer(_bwdParams.x()->uid(), deviceBuffers, numDeviceBuffers);
    auto dyBuffer =
        hip_kernel_utils::findDeviceBuffer(_bwdParams.dy()->uid(), deviceBuffers, numDeviceBuffers);
    auto dxBuffer =
        hip_kernel_utils::findDeviceBuffer(_bwdParams.dx()->uid(), deviceBuffers, numDeviceBuffers);
    ...
    if(!useMult)
    {
        ...
        auto singleCfg =
            defaultConfigSpatialSingle(d.n, d.hw, d.nhw, d.isLayoutNHWC, d.useFp16Mix);
    ...
    else
    {
        ...
        getSpatialMultipleConfig(sc, shw, d.isLayoutNHWC, d.useFp32, vecSize,
                                 xlocalsize, ylocalsize);

        if(!isSpatialMultipleApplicable(
               sn, sc, shw, d.isLayoutNHWC, d.useFp32, vecSize, stashValuesBwd, ylocalsize,
        ...
        int stashMethod = getStashMethod(d.isLayoutNHWC, d.useFp32, sc, sn, shw,
                                         stashValuesBwd, ylocalsize, zlocalsize, nelements);
        ...
        if(d.isLayoutNHWC && d.c % 2 == 0 && xlocalsize % 2 == 0)
        {
            ...
            if(ylocalsize_final == 0) ylocalsize_final = 1;
        }
        ...
        kernelDScaleDBias.SetGridSize(
            static_cast<unsigned int>(xgridsize / xlocalsize),
        ...
        kernelFinal.SetGridSize(static_cast<unsigned int>(xgridsize / xlocalsize_final),
                                1, 1);
...
void BatchnormBwdPlan::executePerActivation(const HipdnnEnginePluginHandle& handle,
                                            const hipdnnPluginDeviceBuffer_t* deviceBuffers,
                                            uint32_t numDeviceBuffers) const
{
    int device;
