 ...
 * the graph carries an identity.
 */
inline flatbuffers::FlatBufferBuilder
    buildPointwiseGraph(hipdnn_flatbuffers_sdk::data_objects::PointwiseMode operation
                        ...
                        bool binary = true)
{
    ...
    for(const auto uid : {INPUT_A_UID, INPUT_B_UID, OUTPUT_UID})
    {
        tensors.push_back(data_objects::CreateTensorAttributesDirect(
            builder, uid, nullptr, dataType, &strides, &dims, false));
    }
