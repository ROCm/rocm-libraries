...
namespace hip_kernel_provider::pass_by_value::test
{
...
// delivered value actually flows through. Together these catch a provider that bumps its API
// version to advertise pass-by-value support but silently mishandles the runtime scalar.
...
protected:
    ...
    // Batchnorm forward training WITH running stats: momentum drives the running-stats EMA
    // update, so it enters next_running_mean / next_running_variance.
    std::vector<float> runBnTrainingMomentum(PbvMode mode, float momentumValue)
    {
        ...
        Graph graphObj;
        graphObj.set_name("BnTrainingMomentumPbv");
        graphObj.set_intermediate_data_type(hipdnn_frontend::DataType::FLOAT)
            .set_compute_data_type(hipdnn_frontend::DataType::FLOAT)
            .set_io_data_type(hipdnn_frontend::DataType::FLOAT);
        ...
        auto biasTensorAttr = std::make_shared<TensorAttributes>(
            makeTensorAttributes("bias", derivedDims, generateStrides(derivedDims)));
        auto prevRunningMeanTensorAttr = std::make_shared<TensorAttributes>(
            makeTensorAttributes("prev_running_mean", derivedDims, generateStrides(derivedDims)));
        auto prevRunningVarianceTensorAttr
            = std::make_shared<TensorAttributes>(makeTensorAttributes(
                "prev_running_variance", derivedDims, generateStrides(derivedDims)));

        // Epsilon fixed as a baked compile-time constant so only momentum varies.
        auto epsilonTensorAttr = makeScalarTensor("epsilon", PbvMode::COMPILE_TIME, 1e-5f);
        auto momentumTensorAttr = makeScalarTensor("momentum", mode, momentumValue);
        ...
        invVarianceTensorAttr->set_output(true);
        invVarianceTensorAttr->set_data_type(hipdnn_frontend::DataType::FLOAT);
        nextMean->set_output(true);
        nextMean->set_data_type(hipdnn_frontend::DataType::FLOAT);
        nextVar->set_output(true);
        nextVar->set_data_type(hipdnn_frontend::DataType::FLOAT);

        return buildFillExecuteRead(graphObj,
                                    mode,
                                    {{momentumTensorAttr, momentumValue}},
                                    {prevRunningVarianceTensorAttr},
                                    {nextMean, nextVar},
                                    SEED);
    }

    // Layernorm forward (training): epsilon enters y and inv_variance.
    std::vector<float> runLayernormEpsilon(PbvMode mode, float epsilonValue)
    {
        constexpr unsigned int SEED = 23;
        const std::vector<int64_t> dims = {2, 8, 4, 4};
        // Normalize over the last dim; affine dims are 1 everywhere except that axis.
        const std::vector<int64_t> affineDims = {1, 1, 1, 4};

        Graph graphObj;
        graphObj.set_name("LayernormEpsilonPbv");
        graphObj.set_intermediate_data_type(hipdnn_frontend::DataType::FLOAT)
            .set_compute_data_type(hipdnn_frontend::DataType::FLOAT)
            .set_io_data_type(hipdnn_frontend::DataType::FLOAT);
        ...
        auto xTensorAttr = std::make_shared<TensorAttributes>(
            makeTensorAttributes("X", dims, generateStrides(dims, TensorLayout::NCHW.strideOrder)));
        auto scaleTensorAttr = std::make_shared<TensorAttributes>(makeTensorAttributes(
            "scale", affineDims, generateStrides(affineDims, TensorLayout::NCHW.strideOrder)));
        auto biasTensorAttr = std::make_shared<TensorAttributes>(makeTensorAttributes(
            "bias", affineDims, generateStrides(affineDims, TensorLayout::NCHW.strideOrder)));
        auto epsilonTensorAttr = makeScalarTensor("epsilon", mode, epsilonValue);
    ...
    // RMSnorm forward (training): epsilon enters y and inv_rms.
    std::vector<float> runRmsnormEpsilon(PbvMode mode, float epsilonValue)
    {
        ...
        Graph graphObj;
        graphObj.set_name("RmsnormEpsilonPbv");
        graphObj.set_intermediate_data_type(hipdnn_frontend::DataType::FLOAT)
            .set_compute_data_type(hipdnn_frontend::DataType::FLOAT)
            .set_io_data_type(hipdnn_frontend::DataType::FLOAT);
        ...
        invRmsAttr->set_data_type(hipdnn_frontend::DataType::FLOAT);

        return buildFillExecuteRead(
            graphObj, mode, {{epsilonTensorAttr, epsilonValue}}, {}, {invRmsAttr}, SEED);
    }
...
TEST_F(IntegrationGpuPassByValue, BatchnormTrainingMomentum)
{
    expectScalarFlowsThrough(runBnTrainingMomentum(PbvMode::COMPILE_TIME, 0.1f),
                             runBnTrainingMomentum(PbvMode::RUNTIME_USER, 0.1f),
                             runBnTrainingMomentum(PbvMode::RUNTIME_USER, 0.9f),
                             "batchnorm-training momentum");
}

TEST_F(IntegrationGpuPassByValue, LayernormEpsilon)
{
    expectScalarFlowsThrough(runLayernormEpsilon(PbvMode::COMPILE_TIME, 1e-3f),
                             runLayernormEpsilon(PbvMode::RUNTIME_USER, 1e-3f),
                             runLayernormEpsilon(PbvMode::RUNTIME_USER, 5.0f),
                             "layernorm epsilon");
}

TEST_F(IntegrationGpuPassByValue, RmsnormEpsilon)
{
    expectScalarFlowsThrough(runRmsnormEpsilon(PbvMode::COMPILE_TIME, 1e-3f),
                             runRmsnormEpsilon(PbvMode::RUNTIME_USER, 1e-3f),
                             runRmsnormEpsilon(PbvMode::RUNTIME_USER, 5.0f),
                             "rmsnorm epsilon");
}
