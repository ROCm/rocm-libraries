                                      ...
                                      float eps)
{
    ...
    for(unsigned int i = lid; i < INNER_SIZE; i += LOCAL_SIZE)
    {
        ...
        float tmp = hip_kernel_provider::to_float32<InputType>(x[idx]);
        pvar += tmp * tmp;
    }
    ...
    {
        size_t idx = o * INNER_SIZE * STRIDE + i * STRIDE + s;
        float y_val = hip_kernel_provider::to_float32<InputType>(x[idx]) * prstd
                      * hip_kernel_provider::to_float32<ScaleType>(weight[i]);
        if(bias != nullptr)
        {
            y_val += hip_kernel_provider::to_float32<ScaleType>(bias[i]);
        }
        y[idx] = hip_kernel_provider::from_float32<OutputType>(y_val);
    }
}
