...
HipFlash2Engine::HipFlash2Engine(int64_t engineId) : _id(engineId) {}
...
bool HipFlash2Engine::isApplicable(
    ...
    for (const auto& pb : _planBuilders) {
        if (pb->isApplicable(handle, opGraph)) return true;
    }
    return false;
}

void HipFlash2Engine::getDetails(
    Handle& handle, const hipdnn_flatbuffers_sdk::flatbuffer_utilities::IGraph& /*opGraph*/,
