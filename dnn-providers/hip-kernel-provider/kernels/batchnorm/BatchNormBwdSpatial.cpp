...
constexpr unsigned int segment = segtmp > HIP_PLUGIN_BN_NHW ? HIP_PLUGIN_BN_NHW : segtmp;
...
using fp_prec_read_vec_type =
    typename hip_kernel_plugin::mapped_vector_type<FLOAT_ACCUM, readSize>::type;
...
__device__ __forceinline__ FpPrecVecT vectorizedBwdActivation(FpPrecVecT const& dy,
...
__device__ __forceinline__ void
    accumulateMadVec(FLOAT_ACCUM& acc, FpPrecVecT const& a, FpPrecVecT const& b)
{
    constexpr auto vecSize = hip_kernel_plugin::mapped_vector_info<FpPrecVecT>::size;
    if constexpr(vecSize == 4)
    {
                             ...
                             FLOAT_ACCUM actBeta)
{
    ...
    constexpr unsigned int readUnrollHint = HIP_PLUGIN_BN_N > 768 ? 4 : 2;
    hip_kernel_plugin::static_unroll_count<unsigned int, 0, less4, grprd, readUnrollHint>{
        [&](unsigned int k) {
            ...
            if(l < less4)
            {
                ...
                fp_prec_read_vec_type dyvalue
                    = hip_kernel_plugin::cast<fp_prec_read_vec_type>(dyRead);
    ...
    if constexpr(rem4 > 0)
    {
        ...
        {
            fp_read_vec_type xread = *(reinterpret_cast<const fp_read_vec_type*>(x_in + index));
            fp_read_vec_type dyRead = *(reinterpret_cast<const fp_read_vec_type*>(dy_in + index));
    ...
    constexpr unsigned int writeUnrollHint = HIP_PLUGIN_BN_N > 768 ? 2 : 1;
    hip_kernel_plugin::static_unroll_count<unsigned int, 0, lessout, chunk, writeUnrollHint>{
        [&](unsigned int k) {
            ...
            if(l < lessout)
            {
                ...
                fp_prec_write_vec_type value1
                    = hip_kernel_plugin::cast<fp_prec_write_vec_type>(dyRead);
                ...
                fp_prec_write_vec_type tmp1
                    = hip_kernel_plugin::cast<fp_prec_write_vec_type>(
                          static_cast<FLOAT_ACCUM>(HIP_PLUGIN_BN_NHW))
                          * value1
