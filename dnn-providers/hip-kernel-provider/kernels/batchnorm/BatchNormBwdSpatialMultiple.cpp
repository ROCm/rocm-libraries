...
__device__ __forceinline__ void
    accumulateMadVal(fp_prec_c_type& a, fp_prec_ls_type const& b, fp_prec_ls_type const& c)
{
    if constexpr(vecSizeX == 1)
    {
...
{
    FpPrecVecType tmp1
        = hip_kernel_plugin::cast<FpPrecVecType>(static_cast<FLOAT_ACCUM>(nhw)) * value
                                   ...
                                   FLOAT_ACCUM actBeta)
{
    ...
    if(ygid * vecSizeY < HIP_PLUGIN_BN_HW && zgid < HIP_PLUGIN_BN_N)
    {
        ...
        unsigned int index_base = (zgid * HIP_PLUGIN_BN_N_ELEMENTS) * chw
                                  + ygid * (ystride / vecSizeX) * vecSizeY + xgid * (xstride);
        for(unsigned int nn = 0; nn < HIP_PLUGIN_BN_N_ELEMENTS; nn++)
        {
            ...
            fp_prec_ls_type value2 = hip_kernel_plugin::cast<fp_prec_ls_type>(
                *reinterpret_cast<const fp_ls_type*>(x_in + index));
            fp_prec_ls_type xhat = (value2 - mean) * invVar;

            value1 = bwdActivationVec(value1, xhat, pscale, pbias, actAlpha, actBeta);
                          ...
                          FLOAT_ACCUM actBeta)
{
    ...
    if(ygid * vecSizeY < HIP_PLUGIN_BN_HW && zgid < HIP_PLUGIN_BN_N)
    {
        ...
        for(unsigned int nn = 0; nn < HIP_PLUGIN_BN_N_ELEMENTS; nn++)
        {
            ...
            *reinterpret_cast<fp_ls_type*>(dx_out + index) = hip_kernel_plugin::cast<fp_ls_type>(
                batchBwdNormalization(value1,
                                      xhat,
