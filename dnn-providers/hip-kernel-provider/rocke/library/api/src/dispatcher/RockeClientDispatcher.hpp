...
// Owns the per-arch AOT catalog map and performs kernel selection:
...
// call to selectInstance/selectForArch for a given arch
...
// every arch query without any HIP or filesystem calls. This
...
{
public:
    // Production: empty catalog map, lazy per-arch load.
    ...
    // handle's stream, retrieve the per-arch catalog lazily, then select.
    // Never throws.
    ...
    // Test/explicit-arch seam: run translate + select using the catalog for
    // the given arch, bypassing HIP device detection.
...
private:
    // Get or lazily load the catalog for arch. Thread-safe via _catalogMutex.
    // Once loaded (or found absent), the result is cached in _catalogsByArch
    // for the lifetime of this dispatcher.
    const AotCatalog& catalogForArch(const std::string& arch) const;
    ...
    mutable std::map<std::string, AotCatalog> _catalogsByArch;
    mutable std::mutex _catalogMutex;

    // Non-empty only when constructed via the injection ctor. When set,
    // catalogForArch() returns this catalog for every arch query
