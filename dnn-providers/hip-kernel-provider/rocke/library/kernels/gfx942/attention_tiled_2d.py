...
def build_gfx942_4warp_gqa(
    ...
    *,
    ...
    C = b.param("output_ptr", PtrType(dtype, "global"), noalias=True, writeonly=True, align=16)
    Q = b.param("query_ptr", PtrType(dtype, "global"), noalias=True, readonly=True, align=16)
    Kp = b.param("key_cache_ptr", PtrType(dtype, "global"), noalias=True, readonly=True, align=16)
    Vp = b.param("value_cache_ptr", PtrType(dtype, "global"), noalias=True, readonly=True, align=16)
    b.param("sink_ptr", PtrType(dtype, "global"), readonly=True, align=16)
    ...
    with b.scf_if(b.cmp_ge(qbase, qlen)):  # padding q-block (AITER +num_seqs over-alloc) -> skip
        b.ret()

    V_lds = b.smem_alloc(dtype, [64, 256], name_hint="Vlds")
    q_desc = TensorDescriptor.naive("query_ptr", lengths=[1 << 30, H, HD], coord_names=("token", "head", "dim"))
    ...
    causal_t = b.div(b.add(b.add(context_off, qbase), b.const_i32(128 + BN - 1)), b.const_i32(BN))
    ...
    with loop as (kv, carry):
        ...
        for nt in range(NDdim):
            ...
            for kk in range(NKpv):
                va = b.vec_pack([b.vec_extract(b.smem_load_vN(V_lds, b.add(b.mul(b.const_i32(kk), b.const_i32(K)), b.add(b.mul(ld.k_blk, b.const_i32(APL)), b.const_i32(j))), b.add(b.mul(b.const_i32(nt), b.const_i32(32)), ld.m_in_atom), dtype=dtype, n=1), 0) for j in range(APL)], dtype)
