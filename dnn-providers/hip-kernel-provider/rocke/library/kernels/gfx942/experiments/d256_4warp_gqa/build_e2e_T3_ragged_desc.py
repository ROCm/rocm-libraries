...
from rocke.helpers.transforms import TensorDescriptor
...
BS = 64; BPT = BN//BS  # production block_size=64 (BPT=1)
NPHYS = 8            # per-seq physical blocks (max klen = NPHYS*BS = 512 @ BS=64)
H = 16; HKV = 2; GQAG = H//HKV
# ragged config (host sets these before build via globals)
NSEQ = 3; TOTALQ = 928; TOTALQB = 10

def build():
    b = IRBuilder("e2eRaggedDesc"); b.kernel.attrs["max_workgroup_size"] = 256
    # ---- production _attn_signature (18-arg paged ABI) ----
    C  = b.param("output_ptr", PtrType(F32, "global"), noalias=True, writeonly=True, align=16)
    Q  = b.param("query_ptr", PtrType(BF16, "global"), noalias=True, readonly=True, align=16)
    Kp = b.param("key_cache_ptr", PtrType(BF16, "global"), noalias=True, readonly=True, align=16)
    Vp = b.param("value_cache_ptr", PtrType(BF16, "global"), noalias=True, readonly=True, align=16)
    b.param("sink_ptr", PtrType(BF16, "global"), readonly=True, align=16)          # unused (D256 cohort)
    ...
    b.param("alibi_slopes_ptr", PtrType(F32, "global"), readonly=True, align=4)    # unused
    b.param("qq_bias_ptr", PtrType(F32, "global"), readonly=True, align=4)         # unused
    CUQ= b.param("query_start_len_ptr", PtrType(I32, "global"), readonly=True, align=16)
    scale_p = b.param("scale", F32)
    b.param("k_scale", F32); b.param("v_scale", F32); b.param("out_scale", F32); b.param("softcap", F32)
    ...
    # in-kernel CTA->seq via binary_search on cu_seqlens_q (production ABI; mirrors attention_tiled_2d.py:5590)
    ITERS = max(1, NSEQ.bit_length()) + 1
    sid = binary_search_seq_idx(b, CUQ, gqb, num_seqs_p, block_q=128, iterations=ITERS)  # num_seqs scalar in prod
    ...
    q_block_start = b.add(b.div(cu_q_start, b.const_i32(128)), sid)  # cu_q//BM + seq_idx (AITER find_seq_idx)
    ...
    q_desc = TensorDescriptor.naive("query_ptr", lengths=[1 << 30, H, HD], coord_names=("token", "head", "dim"))  # total_q unbounded -> descriptor, not buffer_rsrc
    ...
    def phys_key(kv, keytile):
        lblk = b.add(b.mul(sid, bt_stride_p), b.add(b.mul(kv, b.const_i32(BPT)), b.div(keytile, b.const_i32(BS))))
    ...
    sc = b.fmul(scale_p, b.const_f32(1.4426950408889634)); ninf = b.const_f32(-1e30); zf = b.const_f32(0.0)
    def bperm(v):
    ...
    with loop as (kv, carry):
        ...
        for h in range(NK):
            ...
            q_tok = b.add(b.add(qstart, wq), ld.n_in_atom)  # packed token index
            q_off, _ = q_desc.offset(b, token=q_tok, head=qhead, dim=koff)  # ELEMENT offset (descriptor)
            q = b.global_load_vN(Q, q_off, BF16, BPL, align=BPL*2)
...
if __name__ == "__main__":
    ...
    art = compile_kernel(build(), arch="gfx942"); open("/home/AMD/avirgoel/wk/eRagDesc.hsaco", "wb").write(art.hsaco); print("BUILT", art.kernel_name, "NSEQ", NSEQ, "TOTALQ", TOTALQ, "TOTALQB", TOTALQB, flush=True)
    ...
    L = KernelLauncher(hsaco=art.hsaco, kernel_name=art.kernel_name,
        signature=SignatureBuilder().ptr("output_ptr","f32").ptr("query_ptr","bf16").ptr("key_cache_ptr","bf16").ptr("value_cache_ptr","bf16").ptr("sink_ptr","bf16").ptr("block_tables_ptr","i32").ptr("seq_lens_ptr","i32").ptr("alibi_slopes_ptr","f32").ptr("qq_bias_ptr","f32").ptr("query_start_len_ptr","i32").scalar("scale","f32").scalar("k_scale","f32").scalar("v_scale","f32").scalar("out_scale","f32").scalar("softcap","f32").scalar("num_seqs","i32").scalar("block_table_stride","i32").build())
    _dummy = torch.zeros(1, device="cuda", dtype=torch.bfloat16); _df = torch.zeros(1, device="cuda", dtype=torch.float32)
    def run():
        L({"output_ptr":Cc,"query_ptr":Qp,"key_cache_ptr":Kk,"value_cache_ptr":Vv,"sink_ptr":_dummy,"block_tables_ptr":bt,"seq_lens_ptr":klt,"alibi_slopes_ptr":_df,"qq_bias_ptr":_df,"query_start_len_ptr":cuq,"scale":float(scale),"k_scale":1.0,"v_scale":1.0,"out_scale":1.0,"softcap":0.0,"num_seqs":int(NSEQ),"block_table_stride":int(NPHYS)}, config=LaunchConfig(grid=(TOTALQB*H,1,1), block=(256,1,1), stream=stream))
