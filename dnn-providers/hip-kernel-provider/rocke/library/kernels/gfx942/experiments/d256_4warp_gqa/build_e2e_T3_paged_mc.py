...
NSEQ=256  # distinct sequences (cache > L2)
def build():
    b=IRBuilder("e2eT3pgmcw4"); b.kernel.attrs["max_workgroup_size"]=256  # 4 warps
    ...
    seq=b.block_id_x()  # this CTA's sequence
    wq=b.mul(wid,b.const_i32(32))  # this warp's 32-query base
    V_lds=b.smem_alloc(BF16,[64,256],name_hint="Vlds")
    qr=b.buffer_rsrc(Q,b.const_i32(NSEQ*128*HD*2)); kr=b.buffer_rsrc(Kp,b.const_i32(NSEQ*NPHYS*BS*HD*2)); vr=b.buffer_rsrc(Vp,b.const_i32(NSEQ*NPHYS*BS*HD*2))
    ...
    with loop as (kv, carry):
        ...
        for h in range(NK):
            ...
            q=b.buffer_load_vN(qr,b.mul(b.add(b.mul(b.add(b.add(b.mul(seq,b.const_i32(128)),wq),ld.n_in_atom),b.const_i32(HD)),koff),b.const_i32(2)),b.const_i32(0),BF16,BPL)
    ...
    for nt in range(NDdim):
        for i in range(CPL):
            ...
            b.global_store(C,b.add(b.mul(b.add(b.add(b.mul(seq,b.const_i32(128)),wq),c),b.const_i32(HD)),dim),b.fmul(b.vec_extract(accs_f[nt],i),recip),align=4)
    b.ret(); return b.kernel
art=compile_kernel(build(),arch="gfx942"); open("/home/AMD/avirgoel/wk/e2eT3pgmc.hsaco","wb").write(art.hsaco); print("BUILT",art.kernel_name,flush=True)
torch.manual_seed(0)
Kk=torch.randn(NSEQ*NPHYS*BS,HD,device="cuda",dtype=torch.bfloat16)*0.3; Vv=torch.randn(NSEQ*NPHYS*BS,HD,device="cuda",dtype=torch.bfloat16)*0.3  # 64MB each > L2
Q=torch.randn(NSEQ*128,HD,device="cuda",dtype=torch.bfloat16)*0.3
# per-seq block table: each seq maps to its OWN distinct NPHYS physical blocks (scattered within)
bt=torch.stack([torch.randperm(NPHYS)+s*NPHYS for s in range(NSEQ)]).to(torch.int32).cuda().reshape(-1)
# logical K/V for seq 0 (correctness check on CTA 0)
bt0=bt.view(NSEQ,NPHYS)[0].long(); Klog=Kk.view(NSEQ*NPHYS,BS,HD)[bt0].reshape(NPHYS*BS,HD); Vlog=Vv.view(NSEQ*NPHYS,BS,HD)[bt0].reshape(NPHYS*BS,HD); Q0=Q[:128]
Cc=torch.zeros(NSEQ*128,HD,device="cuda",dtype=torch.float32)
...
L({"Q":Q,"K":Kk,"V":Vv,"C":Cc,"BT":bt},config=LaunchConfig(grid=(NSEQ,1,1),block=(256,1,1),stream=torch.cuda.current_stream().cuda_stream)); torch.cuda.synchronize()
S=(Q0.float()@Klog.float().T)*(1.0/math.sqrt(HD)); O=(torch.softmax(S,dim=-1)@Vlog.float())
err=(Cc[:128]-O).abs().max().item(); print(f"NUM_KV={NUM_KV} max_abs={err:.4e} {'CORRECT' if err<1e-1 else 'WRONG'}",flush=True)

import time
for _ in range(20): L({"Q":Q,"K":Kk,"V":Vv,"C":Cc,"BT":bt},config=LaunchConfig(grid=(NSEQ,1,1),block=(256,1,1),stream=torch.cuda.current_stream().cuda_stream))
torch.cuda.synchronize(); t0=time.perf_counter()
for _ in range(100): L({"Q":Q,"K":Kk,"V":Vv,"C":Cc,"BT":bt},config=LaunchConfig(grid=(NSEQ,1,1),block=(256,1,1),stream=torch.cuda.current_stream().cuda_stream))
torch.cuda.synchronize(); ms=(time.perf_counter()-t0)/100*1e3
flop=4*128*(NUM_KV*BN)*HD*NSEQ
print(f"TIMED paged-L2honest ms={ms:.5f} TFs={flop/(ms*1e-3)/1e12:.2f} (NSEQ={NSEQ}, cache={NSEQ*NPHYS*BS*HD*2/1e6:.0f}MB)",flush=True)
