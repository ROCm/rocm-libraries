...
NSEQ=64  # batched sequences (saturate + L2-honest)
def build():
    ...
    sh=b.block_id_x(); qhead=b.mod(sh,b.const_i32(H)); kvh=b.div(qhead,b.const_i32(GQAG)); seq=b.div(sh,b.const_i32(H))  # (seq,head)
    V_lds=b.smem_alloc(BF16,[64,256],name_hint="Vlds")
    qr=b.buffer_rsrc(Q,b.const_i32(NSEQ*128*H*HD*2)); kr=b.buffer_rsrc(Kp,b.const_i32(NSEQ*NPHYS*BS*HKV*HD*2)); vr=b.buffer_rsrc(Vp,b.const_i32(NSEQ*NPHYS*BS*HKV*HD*2))
    btr=b.buffer_rsrc(BT,b.const_i32(NSEQ*NPHYS*4))
    def phys_key(kv,keytile):  # logical key-in-tile -> physical key via block table
        lblk=b.add(b.mul(seq,b.const_i32(NPHYS)),b.add(b.mul(kv,b.const_i32(BPT)),b.div(keytile,b.const_i32(BS))))
    ...
    with loop as (kv, carry):
        ...
        for h in range(NK):
            ...
            qg=b.add(b.mul(seq,b.const_i32(128)),b.add(wq,ld.n_in_atom)); qi=b.add(b.mul(qg,b.const_i32(H)),qhead)
    ...
    for nt in range(NDdim):
        for i in range(CPL):
            ...
            oi=b.add(b.mul(b.add(b.mul(seq,b.const_i32(128)),b.add(wq,c)),b.const_i32(H)),qhead)
