...
NPHYS=NUM_KV*BPT  # physical blocks
def build():
    b=IRBuilder("e2eT3pgcvw4"); b.kernel.attrs["max_workgroup_size"]=256  # 4 warps
    ...
    KL=b.param("KL",PtrType(I32,"global"),noalias=True,readonly=True,align=16)  # varlen: seqlen_k
    ...
    qr=b.buffer_rsrc(Q,b.const_i32(128*HD*2)); kr=b.buffer_rsrc(Kp,b.const_i32(NPHYS*BS*HD*2)); vr=b.buffer_rsrc(Vp,b.const_i32(NPHYS*BS*HD*2))
    ...
    qbase=b.const_i32(NUM_KV*BN-128)  # causal: this 128-query block is the tail
    ...
    with loop as (kv, carry):
        ...
        for c in range(8):  # cooperative PAGED V load: logical key -> physical via block table
            ...
            pk=phys_key(kv,key); vsrc=b.mul(b.add(b.mul(pk,b.const_i32(HD)),hd),b.const_i32(2))
        ...
        for h in range(NK):
            ...
            q=b.buffer_load_vN(qr,b.mul(b.add(b.mul(b.add(wq,ld.n_in_atom),b.const_i32(HD)),koff),b.const_i32(2)),b.const_i32(0),BF16,BPL)
            for kt in range(NKEYT):
                kf=b.buffer_load_vN(kr,b.mul(b.add(b.mul(pk_kt[kt],b.const_i32(HD)),koff),b.const_i32(2)),b.const_i32(0),BF16,APL)
    ...
    for nt in range(NDdim):
        for i in range(CPL):
            ...
            b.global_store(C,b.add(b.mul(b.add(wq,c),b.const_i32(HD)),dim),b.fmul(b.vec_extract(accs_f[nt],i),recip),align=4)
    b.ret(); return b.kernel
art=compile_kernel(build(),arch="gfx942"); open("/home/AMD/avirgoel/wk/e2eT3pgcv.hsaco","wb").write(art.hsaco); print("BUILT",art.kernel_name,flush=True)
torch.manual_seed(0)
Kk=torch.randn(NPHYS*BS,HD,device="cuda",dtype=torch.bfloat16)*0.3; Vv=torch.randn(NPHYS*BS,HD,device="cuda",dtype=torch.bfloat16)*0.3
Q=torch.randn(128,HD,device="cuda",dtype=torch.bfloat16)*0.3
import os
bt=(torch.randperm(NPHYS) if os.environ.get("SCATTER") else torch.arange(NPHYS)).to(torch.int32).cuda()
Klog=Kk.view(NPHYS,BS,HD)[bt.long()].reshape(NPHYS*BS,HD); Vlog=Vv.view(NPHYS,BS,HD)[bt.long()].reshape(NPHYS*BS,HD)
Cc=torch.zeros(128,HD,device="cuda",dtype=torch.float32)
import os
...
L({"Q":Q,"K":Kk,"V":Vv,"C":Cc,"BT":bt,"KL":klt},config=LaunchConfig(grid=(1,1,1),block=(256,1,1),stream=torch.cuda.current_stream().cuda_stream)); torch.cuda.synchronize()
S=(Q.float()@Klog.float().T)*(1.0/math.sqrt(HD))
...
S=S.masked_fill(mask,float("-inf")); O=(torch.softmax(S,dim=-1)@Vlog.float())
