...
def build():
    b = IRBuilder("e2eRaggedBS"); b.kernel.attrs["max_workgroup_size"] = 256
    ...
    sid = binary_search_seq_idx(b, CUQ, gqb, b.const_i32(NSEQ), block_q=128, iterations=ITERS)  # num_seqs scalar in prod
...
if __name__ == "__main__":
    ...
    art = compile_kernel(build(), arch="gfx942"); open("/home/AMD/avirgoel/wk/eRagBS.hsaco", "wb").write(art.hsaco); print("BUILT", art.kernel_name, "NSEQ", NSEQ, "TOTALQ", TOTALQ, "TOTALQB", TOTALQB, flush=True)
    ...
    L = KernelLauncher(hsaco=art.hsaco, kernel_name=art.kernel_name,
        signature=SignatureBuilder().ptr("Q","bf16").ptr("K","bf16").ptr("V","bf16").ptr("C","f32").ptr("BT","i32").ptr("KL","i32").ptr("CUQ","i32").build())
    def run(): L({"Q":Qp,"K":Kk,"V":Vv,"C":Cc,"BT":bt,"KL":klt,"CUQ":cuq}, config=LaunchConfig(grid=(TOTALQB*H,1,1), block=(256,1,1), stream=stream))
