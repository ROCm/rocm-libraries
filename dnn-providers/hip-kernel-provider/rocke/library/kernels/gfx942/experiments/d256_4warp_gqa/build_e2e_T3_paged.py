...
def build():
    b=IRBuilder("e2eT3pgw4"); b.kernel.attrs["max_workgroup_size"]=256  # 4 warps
...
art=compile_kernel(build(),arch="gfx942"); open("/home/AMD/avirgoel/wk/e2eT3pg.hsaco","wb").write(art.hsaco); print("BUILT",art.kernel_name,flush=True)
torch.manual_seed(0)
...
L=KernelLauncher(hsaco=art.hsaco,kernel_name=art.kernel_name,signature=SignatureBuilder().ptr("Q","bf16").ptr("K","bf16").ptr("V","bf16").ptr("C","f32").ptr("BT","i32").build())
L({"Q":Q,"K":Kk,"V":Vv,"C":Cc,"BT":bt},config=LaunchConfig(grid=(1,1,1),block=(256,1,1),stream=torch.cuda.current_stream().cuda_stream)); torch.cuda.synchronize()
S=(Q.float()@Klog.float().T)*(1.0/math.sqrt(HD)); O=(torch.softmax(S,dim=-1)@Vlog.float())
