...
def build():
    b=IRBuilder("e2eT3w4w4"); b.kernel.attrs["max_workgroup_size"]=256  # 4 warps
    ...
    qr=b.buffer_rsrc(Q,b.const_i32(128*HD*2)); kr=b.buffer_rsrc(Kp,b.const_i32(NUM_KV*BN*HD*2)); vr=b.buffer_rsrc(Vp,b.const_i32(NUM_KV*BN*HD*2))
    ...
    with loop as (kv, carry):
        ...
        for c in range(8):  # cooperative V load: 256 threads x 8 = 16384 elems, ONCE, shared by 4 warps
...
art=compile_kernel(build(),arch="gfx942"); open("/home/AMD/avirgoel/wk/e2eT3w4.hsaco","wb").write(art.hsaco); print("BUILT",art.kernel_name,flush=True)
torch.manual_seed(0)
Q=torch.randn(128,HD,device="cuda",dtype=torch.bfloat16)*0.3; Kk=torch.randn(NUM_KV*BN,HD,device="cuda",dtype=torch.bfloat16)*0.3; Vv=torch.randn(NUM_KV*BN,HD,device="cuda",dtype=torch.bfloat16)*0.3
...
L({"Q":Q,"K":Kk,"V":Vv,"C":Cc},config=LaunchConfig(grid=(1,1,1),block=(256,1,1),stream=torch.cuda.current_stream().cuda_stream)); torch.cuda.synchronize()
