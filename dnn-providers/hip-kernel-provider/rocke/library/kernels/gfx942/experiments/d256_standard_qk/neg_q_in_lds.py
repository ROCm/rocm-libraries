...
def build():
    ...
    b=IRBuilder("stdqk_qlds"); b.kernel.attrs["max_workgroup_size"]=64
    ...
    Q_lds=b.smem_alloc(F16,[32,D],name_hint="Qlds")
    ...
    with b.scf_for(lane,b.const_i32(32*D//8),b.const_i32(64),iv_name="qi") as qi:
        base=b.mul(qi,b.const_i32(8)); qrow=b.div(base,b.const_i32(D)); dcol=b.mod(base,b.const_i32(D))
        gaddr=b.add(b.mul(b.add(b.add(hoq,qbase),qrow),b.const_i32(D)),dcol)
        b.smem_store_vN_f16(Q_lds,[qrow,dcol],b.global_load_vN_f16(Q,gaddr,8),8)
    b.sync()
    ...
    with kvloop as (kv,carry):
        ...
        def la(bb,kt): return bb.smem_load_vN_f16(Q_lds,ld.m_in_atom,bb.add(bb.mul(kt,bb.const_i32(at.k)),bb.mul(ld.k_blk,bb.const_i32(at.a_per_lane))),n=at.a_per_lane)
        ...
        b.smem_store_vN(m_lds,[row],mx,1); b.smem_store_vN(l_lds,[row],b.fadd(b.fmul(l_old,corr),ssum),1); b.smem_store_vN(c_lds,[row],corr,1); b.sync()
        new_acc=[None]*(ND*CPL)
        for nt in range(ND):
...
print(f"QLDS-CAUSAL SQ={SQ} H={H} blocks={NQB*H} max_abs_err={err:.4e}  {'CORRECT' if err<0.05 else 'WRONG'}")
