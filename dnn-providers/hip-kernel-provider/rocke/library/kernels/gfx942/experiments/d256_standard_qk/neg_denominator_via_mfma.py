...
def build():
    ...
    head=b.block_id_y(); hoq=b.mul(head,b.const_i32(SQ)); hok=b.mul(b.div(head,b.const_i32(GQ)),b.const_i32(SK))  # GQA: kv_head=q_head//GQ
...
# === direct bench vs AOTriton (torch SDPA flash backend on ROCm) ===
try:
    def aot():
        torch.nn.functional.scaled_dot_product_attention(q[None],k[None],v[None],is_causal=True,enable_gqa=True)
    aot(); torch.cuda.synchronize()
    ms_a=time_launches(aot,warmup=10,iters=50,stream=hs); synchronize_and_release(hs)
    tf_a=flop/(ms_a*1e-3)/1e12
    print(f"AOTriton(torch-flash) time={ms_a*1e3:.1f}us  TF/s={tf_a:.2f}")
    print(f"*** ck-DSL std-QK vs AOTriton: {ms_a/ms:.3f}x  ({'WIN' if ms<ms_a else 'trail'}) ***")
except Exception as e:
    print("aot bench failed:",str(e).splitlines()[0][:80])
