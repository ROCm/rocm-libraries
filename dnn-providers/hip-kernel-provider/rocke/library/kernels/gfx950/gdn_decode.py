...
the driver rejects any other negative value before launch. This code is a
lenient superset of that contract: it skips on ``read < 0 or write < 0``, so
on a path that bypasses the host guard (``validate_indices=False``) a stray
``-7`` is silently treated as a skip rather than caught. Callers should treat
``-1`` as the only sentinel; the wider test here is defence, not licence.
