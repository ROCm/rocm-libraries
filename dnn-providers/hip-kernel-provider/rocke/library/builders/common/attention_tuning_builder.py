...
"""Policy-free spec builders for unified-attention tuning.

The production builders in :mod:`attention_spec_builder` intentionally apply
shape/architecture heuristics.  A tuner needs the opposite contract: every
geometry and codegen choice is explicit, no ``_select_*``/``_enable_*`` helper
is consulted, and an invalid point is rejected rather than repaired.

Problem semantics (dtype, heads, masks, cache format and addressing width) are
still derived from :class:`UnifiedAttentionProblem`; those are correctness
properties, not tuning axes.
"""
...
def resolve_tile_policy(block_size: int, policy: str) -> int:
    """Resolve a finite dispatcher tile token without consulting a heuristic."""
...
def _semantic_fields(problem: UnifiedAttentionProblem, *, arch: str) -> dict:
...
def make_explicit_attention_2d_spec(
    ...
    *,
    ...
    """Construct one explicit 2D spec and reject unsupported geometry.

    Unlike ``_tiled_spec_from_problem``, this function never invokes a tuning
    predicate or the LDS fix-up ladder.
    """
    ...
    spec = spec_type(
        **_semantic_fields(problem, arch=arch),
