...
The indexer is a score-only kernel with no value tensor and no softmax state, so its on-chip structure is simpler than attention's, but its footprint is not automatically small and depends on the index-key storage layout that Section 3.1 leaves open. The index-query always carries all H_I indexer heads, so a resident whole index-query tile is Bq times H_I times D_I, which once cast to fp8 is Bq times 8 KiB for DeepSeek and Bq times 4 KiB for GLM. That is not small: a whole-tile index-query at Bq equal to 8 is 64 KiB on DeepSeek, the entire gfx942 LDS. The index-keys are streamed in tiles of Bk, and their tile cost carries the same H_I-fold swing as the cache, Bk times D_I under the shared layout and Bk times H_I times D_I under the per-head layout. Two tiling forms therefore bound the on-chip cost differently. A whole-tile form holds the full head-major query and key tiles resident and is only affordable on gfx950's 160 KB LDS. A head-streaming form holds one head's query slice of Bq times D_I and one head's key slice of Bk times D_I plus the running score at a time, a few kilobytes, and it is the form the gfx942 budget assumes. The tiling knobs are Bq, Bk, and whether heads are streamed, and the per-key work is a single narrow dot product plus a ReLU.

The load-bearing cost of the indexer is not its LDS, it is the index-key cache traffic. The indexer reads the whole index-key cache once per call, which is the compulsory O(Sk) read of the stage. The per-token footprint of that cache depends on an open question from Section 3.1, whether the index-keys are stored per head or shared across heads. If per head, the footprint is n_index_heads times index_head_dim fp8 bytes per token, which is 64 times 128 equals 8 KiB per token on DeepSeek and 32 times 128 equals 4 KiB per token on GLM, so a 32768-token context is on the order of 256 MiB and 128 MiB respectively. If shared, it is index_head_dim fp8 bytes per token, 128 bytes, an H_I-fold reduction (64x on DeepSeek, 32x on GLM). This difference is large enough that the storage form must be confirmed before the indexer budget is trusted, and it is the reason fp8 was chosen for this cache in the first place, since it halves that traffic against bf16. The index-query is negligible by comparison because it has no Sk axis.
...
| index_k tile, shared-key layout (on-chip) | Bk × D_I fp8 | Bk × 128 B | Bk × 128 B |
| index_k tile, per-head-key layout (on-chip) | Bk × H_I × D_I fp8 | Bk × 8 KiB | Bk × 4 KiB |
| index-key cache, per token, per-head form | H_I × D_I fp8 | 8 KiB | 4 KiB |
| index-key cache, per token, shared form | D_I fp8 | 128 B | 128 B |
| index-key cache traffic per call | above × Sk (compulsory O(Sk) read) | ~256 MiB at Sk=32768 (per-head) | ~128 MiB at Sk=32768 (per-head) |

The on-chip cost is layout-dependent rather than automatically small. Under the per-head key layout a whole-tile form does not fit gfx942 (the whole-tile index-query alone reaches the 64 KB cap at Bq=8) and the head-streaming form is required, whereas the shared layout fits comfortably. The gfx942 budget in Section 7.4 is therefore bounded on the per-head, head-streaming form, since that is the layout it has to survive. The cache-traffic row is the binding cost of the stage, and the per-head versus shared choice (Section 3.1) swings it H_I-fold, 64x on DeepSeek and 32x on GLM.
...
- Whether the index-keys are stored per head or shared across heads (Sections 3.1 and 7.1). This swings the index-key cache traffic H_I-fold, 64x on DeepSeek and 32x on GLM, and must be read from the model reference rather than assumed.
