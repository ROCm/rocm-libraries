...
"""Prefill benchmark: LIVE Triton vs FlyDSL variant sweep (gfx1250).

Compares aiter's Triton ``unified_attention`` (forced to the 2D kernel) against
the FlyDSL ``flash_attn_varlen_d192_gfx1250`` kernel on matching shapes.
...
Triton so you get the Triton baseline across the full set.

Run:

    export AITER_PATH=<path/to/aiter>   # e.g. /home/barkocot/aiter
...
The benchmark always compares every shape that FlyDSL supports.  Pass
``--skip-triton`` to measure FlyDSL latency only (no Triton reference,
...
def main() -> int:
    ...
    ap.add_argument(
        "--skip-triton",
        action="store_true",
        help="Skip Triton reference; measure FlyDSL latency only (no correctness check)",
    )
    ...
    for i, shape in enumerate(shapes, 1):
        ...
        if not fly_ok_for_shape:
            ...
            try:
                ...
                if tri_out is not None:
                    # Compare against Triton on the Q tokens only
                    # (FlyDSL output has head_dim_v=128, Triton may have head_dim=192
                    # with the first 128 dims meaningful — compare the overlap)
                    fly_v = fly_out
                    tri_v = tri_out[..., :HEAD_DIM_V]
                    fly_err = _compare(fly_v, tri_v)
        ...
        if tri_ms is not None and fly_ms is not None and fly_ms > 0:
            spd_str = f" spd={tri_ms / fly_ms:.2f}x"
        print(f"{tag} sw={sw} fp8={int(is_fp8)} {tri_str} fly={fly_status}{spd_str}")
