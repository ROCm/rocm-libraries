...
Usage:
    python library/benchmarks/gfx950/attention/prefill/benchmark_mla_prefill_live.py \\
        --shapes library/benchmarks/gfx950/attention/prefill/mla_prefill_shapes.json \\
...
def _run_variant_mfma_v2(
    ...
    # K/V in paged layout [nb, bs, 1, 256]
    K_paged, V_paged, seqlens_k = _expand_kv_paged(inputs, bs)
    num_blocks  = inputs["num_blocks"]
    block_table = inputs["block_table"]   # [1, nb] i32

    # Q: [sq, hq, 192] padded to [sq, hq, 256]
    q_raw = inputs["q"]  # [sq, hq, 192] bf16
    q_256 = torch.zeros(sq, hq, H, dtype=torch.bfloat16, device="cuda")
    q_256[:, :, :192] = q_raw

    # Output: [sq, hq, 256]; caller truncates to [:, :, :128]
    out_256 = torch.zeros(sq, hq, H, dtype=torch.bfloat16, device="cuda")

    cu_seqlens_q = torch.tensor([0, sq], dtype=torch.int32, device="cuda")
    scale_log2 = float(math.log2(inputs["scale"]))

    # Strides in elements for [sq, hq, H] layout
    stride_q_head  = H
    stride_q_token = hq * H
    stride_o_head  = H
    stride_o_token = hq * H

    # K/V cache strides for [nb, bs, 1, H] layout
    stride_block   = bs * H         # between blocks
    stride_page    = H              # between pages within a block
    stride_kv_head = H              # stride per KV head (H_k=1, so only offset 0)
    ...
    # Fenced warm-up launch to surface any async GPU errors (e.g. memory fault)
    # synchronously before entering the timed loop.
