...
* ``_mma_dst_frag_len`` — ``dst`` fragment length, resolved from the arch SSOT
  ``core.arch.target._MMA_FRAGMENT_INFO`` (no ir-side copy);
* ``_mma_dst_is_int`` — whether the atom produces i32, resolved from the JSON
  catalog ``dst`` dtype (``target._op_id_dst_dtype``);
...
from rocke.core.arch.target import _MMA_FRAGMENT_INFO, _op_id_dst_dtype
...
def _sizable_op_ids():
    """Op_ids the SSOT can actually size (positive ``dst`` frag length)."""
    return {
        op_id: info.dst.frag_len
        for op_id, info in _MMA_FRAGMENT_INFO.items()
        if info.dst.frag_len > 0
    }
...
class TestMmaFragTables(unittest.TestCase):
    def test_frag_lengths_are_positive(self):
        ...
        for op_id, frag in sizable.items():
            ...
            self.assertGreater(
                frag, 0, msg=f"dst.frag_len for {op_id!r} must be positive"
            )

    def test_accessor_matches_ssot_and_raises_on_unknown(self):
        for op_id, frag in _sizable_op_ids().items():
            self.assertEqual(_mma_dst_frag_len(op_id), frag)
        with self.assertRaises(ValueError):
            _mma_dst_frag_len("not_a_real_op_id")

    def test_int_acc_op_ids_have_frag_lengths(self):
        int_op_ids = [op for op, dtype in _op_id_dst_dtype().items() if dtype == "i32"]
        self.assertTrue(
            int_op_ids, "expected at least one i32-accumulator atom in the catalog"
        )
        for op_id in int_op_ids:
            self.assertTrue(
                _mma_dst_is_int(op_id),
                msg=f"{op_id!r} is i32 in the catalog but _mma_dst_is_int disagrees",
            )
            self.assertGreater(
                _mma_dst_frag_len(op_id),
                0,
    ...
    def test_known_frag_lengths(self):
        ...
        self.assertEqual(_mma_dst_frag_len("mfma_f32_16x16x16_f16"), 4)
        self.assertEqual(_mma_dst_frag_len("mfma_f32_32x32x8_f16"), 16)
        self.assertEqual(_mma_dst_frag_len("wmma_f32_16x16x16_f16"), 8)
