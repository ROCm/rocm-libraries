...
static rocke_value_t* frag_param(rocke_ir_builder_t* b, const char* name, const rocke_type_t* elem,
                                 bool readonly) {
...
static void wmma_scaled_formats(rocke_ir_builder_t* b, bool scale16, const char* op_id,
...
static void build_async_store(rocke_ir_builder_t* b) {
    ...
    for (int i = 0; i < 4; ++i)
        rocke_b_global_store_async_from_lds(b, out, lds_ptr, widths[i], widths[i],
                                            /*cachepolicy=*/3);
    rocke_b_ret(b);
}
...
int main(int argc, char** argv) {
    ...
    if (CONFIGS[idx].build) {
        ...
        wmma_scaled_formats(&b, CONFIGS[idx].scale16, CONFIGS[idx].op_id, CONFIGS[idx].scale_a,
                            CONFIGS[idx].scale_b);
    }
