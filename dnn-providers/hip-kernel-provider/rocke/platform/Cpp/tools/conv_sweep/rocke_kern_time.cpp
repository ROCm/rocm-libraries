...
int main(int argc, char** argv)
{
    ...
    if(he != hipSuccess)
    {
        printf("ERROR hipMalloc_A %s\n", hipGetErrorString(he));
        return 1;
    }
    he = hipMalloc(&devB, sz_b);
    if(he != hipSuccess)
    {
        printf("ERROR hipMalloc_B %s\n", hipGetErrorString(he));
        return 1;
    }
    he = hipMalloc(&devD, sz_d);
    if(he != hipSuccess)
    {
        printf("ERROR hipMalloc_D %s\n", hipGetErrorString(he));
        return 1;
    }
    ...
    if(he != hipSuccess)
    {
        printf("ERROR hipModuleLoadData %s\n", hipGetErrorString(he));
        return 1;
    }
    ...
    auto do_launch = [&]() -> hipError_t {
        return hipModuleLaunchKernel(fn, gx, gy, gz, block_size, 1, 1, 0, nullptr, nullptr, config);
    };
