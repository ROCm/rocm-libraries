...
static const char* mma_result_name(rocke_lower_t* L, const rocke_op_t* op) {
    if (op->num_results != 1) {
        rocke_ll_fail(L, ROCKE_ERR_VALUE, "%s: expected exactly one result, got %d",
...
static void _op_tile_mma(rocke_lower_t* L, const rocke_op_t* op) {
    ...
    if (strcmp(op_id, "mfma_f32_16x16x32_fp8") == 0) {
        ...
        if (L->backend && L->backend->kind == ROCKE_LL_ISA_RDNA) {
            ...
            rocke_ll_fail(L, ROCKE_ERR_NOTIMPL,
...
static void _emit_wmma_scale(rocke_lower_t* L, const rocke_op_t* op) {
    ...
    if (!L->backend || strcmp(L->backend->gfx, "gfx1250") != 0) {
        rocke_ll_fail(L, ROCKE_ERR_NOTIMPL, "%s is only available on gfx1250 (got %s)", op_name,
                      L->backend ? L->backend->gfx : "(unknown)");
    }
    if (L->flavor != ROCKE_LLVM_FLAVOR_LLVM23) {
        rocke_ll_fail(L, ROCKE_ERR_NOTIMPL, "%s requires llvm23 (ROCm 7.13+), got %s", op_name,
                      rocke_llvm_flavor_name(L->flavor));
    }
    if (op->num_operands != 5) {
        rocke_ll_fail(L, ROCKE_ERR_VALUE, "%s expects 5 operands, got %d", op_name,
                      op->num_operands);
    }
    ...
    if (strcmp(op->operands[3]->type->name, scale_ty) != 0 ||
        ...
        rocke_ll_fail(L, ROCKE_ERR_VALUE, "%s expects %s scale operands, got %s/%s", op_name,
                      scale_ty, op->operands[3]->type->name, op->operands[4]->type->name);
    }
    ...
    rocke_ll_emitf(
        ...
        mma_result_name(L, op), intrinsic, fmt_a, rocke_ll_operand(L, op->operands[0]), fmt_b,
        rocke_ll_operand(L, op->operands[1]), rocke_ll_operand(L, op->operands[2]), sa, scale_ty,
        rocke_ll_operand(L, op->operands[3]), sb, scale_ty, rocke_ll_operand(L, op->operands[4]));
}
...
static const _wmma_spec_t WMMA_SPECS[] = {
    ...
    {"wmma_f32_16x16x16_f16", "wmma.f32.16x16x16.f16",
     "llvm.amdgcn.wmma.f32.16x16x16.f16.v8f32.v16f16", "half", "half", 16, WMMA_SHAPE_AB_C},
    {"wmma_f32_16x16x16_bf16", "wmma.f32.16x16x16.bf16",
     "llvm.amdgcn.wmma.f32.16x16x16.bf16.v8f32.v16i16", "bfloat", "i16", 16, WMMA_SHAPE_AB_C},
    /* _RDNA_GFX12_WMMA (RDNA4, frag_width 8) */
    {"wmma_gfx12_f32_16x16x16_f16", "wmma.gfx12.f32.16x16x16.f16",
     "llvm.amdgcn.wmma.f32.16x16x16.f16.v8f32.v8f16", "half", "half", 8, WMMA_SHAPE_AB_C},
    {"wmma_gfx12_f32_16x16x16_bf16", "wmma.gfx12.f32.16x16x16.bf16",
     "llvm.amdgcn.wmma.f32.16x16x16.bf16.v8f32.v8i16", "bfloat", "i16", 8, WMMA_SHAPE_AB_C},
    ...
    {"wmma_gfx1250_f32_16x16x32_f16", "wmma.gfx1250.f32.16x16x32.f16",
     "llvm.amdgcn.wmma.f32.16x16x32.f16.v8f32.v16f16", "half", "half", 16, WMMA_SHAPE_GFX1250},
    {"wmma_gfx1250_f32_16x16x32_bf16", "wmma.gfx1250.f32.16x16x32.bf16",
     "llvm.amdgcn.wmma.f32.16x16x32.bf16.v8f32.v16bf16", "bfloat", "bfloat", 16,
     WMMA_SHAPE_GFX1250},
    /* _GFX1250_WMMA_FP8 (K=64). A/B arrive in SSA already as <8 x i32>. */
    {"wmma_gfx1250_f32_16x16x64_fp8_fp8", "wmma.gfx1250.f32.16x16x64.fp8.fp8",
     "llvm.amdgcn.wmma.f32.16x16x64.fp8.fp8.v8f32.v8i32", "i32", "i32", 8, WMMA_SHAPE_GFX1250_FP8},
    {"wmma_gfx1250_f32_16x16x64_fp8_bf8", "wmma.gfx1250.f32.16x16x64.fp8.bf8",
     "llvm.amdgcn.wmma.f32.16x16x64.fp8.bf8.v8f32.v8i32", "i32", "i32", 8, WMMA_SHAPE_GFX1250_FP8},
    {"wmma_gfx1250_f32_16x16x64_bf8_fp8", "wmma.gfx1250.f32.16x16x64.bf8.fp8",
     "llvm.amdgcn.wmma.f32.16x16x64.bf8.fp8.v8f32.v8i32", "i32", "i32", 8, WMMA_SHAPE_GFX1250_FP8},
    {"wmma_gfx1250_f32_16x16x64_bf8_bf8", "wmma.gfx1250.f32.16x16x64.bf8.bf8",
     "llvm.amdgcn.wmma.f32.16x16x64.bf8.bf8.v8f32.v8i32", "i32", "i32", 8, WMMA_SHAPE_GFX1250_FP8},
};
...
static void _emit_wmma_int(rocke_lower_t* L, const rocke_op_t* op, const _wmma_int_spec_t* spec) {
    ...
    rocke_ll_emitf(L,
                   ...
                   "i1 1, <%d x i32> %s, "
                   "<%d x i32> %s, i1 0)",
                   mma_result_name(L, op), spec->acc_vec, spec->intrinsic, spec->op_vec,
                   rocke_ll_operand(L, a), spec->op_vec, rocke_ll_operand(L, b), spec->acc_vec,
...
static void _emit_wmma(rocke_lower_t* L, const rocke_op_t* op, const char* op_id) {
    ...
    if (op->num_operands != 3) {
        rocke_ll_fail(L, ROCKE_ERR_VALUE, "%s expects 3 operands",
    ...
    if (spec == NULL) {
        ...
        rocke_ll_fail(L, ROCKE_ERR_NOTIMPL, "unsupported RDNA WMMA op 'tile.%s' for %s", op_id,
    ...
    if (strcmp(spec->call_elt, spec->ssa_elt) != 0) {
        ...
        rocke_ll_emitf(L, "  %s = bitcast <%d x %s> %s to <%d x %s>", a_cast, w, spec->ssa_elt,
                       a_arg, w, spec->call_elt);
        rocke_ll_emitf(L, "  %s = bitcast <%d x %s> %s to <%d x %s>", b_cast, w, spec->ssa_elt,
                       b_arg, w, spec->call_elt);
        a_arg = a_cast;
        b_arg = b_cast;
    }
    ...
    if (spec->shape == WMMA_SHAPE_GFX1250) {
        ...
        rocke_ll_emitf(L,
                       ...
                       "i16 0, <8 x float> %s, "
                       "i1 false, i1 false)",
                       mma_result_name(L, op), spec->intrinsic, w, spec->call_elt, a_arg, w,
                       spec->call_elt, b_arg, rocke_ll_operand(L, c));
        return;
    }
...
static const _mfma_spec_t MFMA_SPECS[] = {
    {"mfma_f32_16x16x16_f16", "mfma.f32.16x16x16f16", "llvm.amdgcn.mfma.f32.16x16x16f16",
     "<4 x half>", "<4 x float>", NULL},
    {"mfma_f32_16x16x32_f16", "mfma.f32.16x16x32.f16", "llvm.amdgcn.mfma.f32.16x16x32.f16",
     "<8 x half>", "<4 x float>", NULL},
    /* bf16 `_1k`: bitcast <4 x bfloat> -> <4 x i16> before the call. */
    {"mfma_f32_16x16x16_bf16", "mfma.f32.16x16x16bf16.1k", "llvm.amdgcn.mfma.f32.16x16x16bf16.1k",
     "<4 x bfloat>", "<4 x float>", "<4 x i16>"},
    {"mfma_f32_16x16x32_bf16", "mfma.f32.16x16x32.bf16", "llvm.amdgcn.mfma.f32.16x16x32.bf16",
     "<8 x bfloat>", "<4 x float>", NULL},
    {"mfma_f32_32x32x8_f16", "mfma.f32.32x32x8f16", "llvm.amdgcn.mfma.f32.32x32x8f16", "<4 x half>",
     "<16 x float>", NULL},
    /* bf16 `_1k`: bitcast <4 x bfloat> -> <4 x i16> exactly like 16x16x16. */
    {"mfma_f32_32x32x8_bf16", "mfma.f32.32x32x8bf16.1k", "llvm.amdgcn.mfma.f32.32x32x8bf16.1k",
     "<4 x bfloat>", "<16 x float>", "<4 x i16>"},
    /* fp32 (TF32-class) scalar atoms: A/B are single floats per lane. */
    {"mfma_f32_16x16x4_f32", "mfma.f32.16x16x4f32", "llvm.amdgcn.mfma.f32.16x16x4f32", "float",
     "<4 x float>", NULL},
    {"mfma_f32_32x32x2_f32", "mfma.f32.32x32x2f32", "llvm.amdgcn.mfma.f32.32x32x2f32", "float",
     "<16 x float>", NULL},
    {"mfma_f32_32x32x16_f16", "mfma.f32.32x32x16.f16", "llvm.amdgcn.mfma.f32.32x32x16.f16",
     "<8 x half>", "<16 x float>", NULL},
    {"mfma_f32_32x32x16_bf16", "mfma.f32.32x32x16.bf16", "llvm.amdgcn.mfma.f32.32x32x16.bf16",
     "<8 x bfloat>", "<16 x float>", NULL},
    {"mfma_f32_4x4x4_f16", "mfma.f32.4x4x4f16", "llvm.amdgcn.mfma.f32.4x4x4f16", "<4 x half>",
     "<4 x float>", NULL},
};
...
static void _emit_mfma(rocke_lower_t* L, const rocke_op_t* op, const _mfma_spec_t* spec) {
    ...
    if (spec->bitcast_to != NULL) {
        ...
        rocke_ll_emitf(L, "  %s = bitcast %s %s to %s", a_cast, spec->ab_ty, rocke_ll_operand(L, a),
                       spec->bitcast_to);
        rocke_ll_emitf(L, "  %s = bitcast %s %s to %s", b_cast, spec->ab_ty, rocke_ll_operand(L, b),
                       spec->bitcast_to);
        a_arg = a_cast;
        b_arg = b_cast;
    ...
    rocke_ll_emitf(L,
                   ...
                   mma_result_name(L, op), spec->acc_ty, spec->intrinsic, call_ty, a_arg, call_ty,
                   b_arg, spec->acc_ty, rocke_ll_operand(L, c));
}
...
void rocke_ll_lower_mfma_fp8_bf8(rocke_lower_t* L, const rocke_op_t* op, const char* dtype,
                                 int out_vec, const char* intrinsic) {
    const rocke_value_t *a, *b, *c;
    const char* ab_ty;
    ...
    rocke_ll_emitf(L,
                   ...
                   mma_result_name(L, op), out_vec, intrinsic, ab_ty, a_cast, ab_ty, b_cast,
                   out_vec, rocke_ll_operand(L, c));
}
...
static void _op_tile_mfma_scale_f32_16x16x128_f8f6f4(rocke_lower_t* L, const rocke_op_t* op) {
    ...
    if (strcmp(a_ty, "<8 x i32>") != 0) {
        rocke_ll_emitf(L, "  %s = bitcast %s %s to <8 x i32>", a_packed, a_ty,
                       rocke_ll_operand(L, a));
    } else {
        a_packed = rocke_ll_operand(L, a);
    }
    if (strcmp(b_ty, "<8 x i32>") != 0) {
        rocke_ll_emitf(L, "  %s = bitcast %s %s to <8 x i32>", b_packed, b_ty,
    ...
    rocke_ll_emitf(L,
                   ...
                   mma_result_name(L, op), a_packed, b_packed, rocke_ll_operand(L, c),
                   rocke_ll_operand(L, a_scale), rocke_ll_operand(L, b_scale));
}
...
static void _op_tile_mfma_f32_16x16x128_fp4(rocke_lower_t* L, const rocke_op_t* op) {
    ...
    rocke_ll_emitf(L,
                   ...
                   mma_result_name(L, op), a_cast, b_cast, rocke_ll_operand(L, c));
}
...
static void _op_tile_mfma_f32_16x16x96_fp6(rocke_lower_t* L, const rocke_op_t* op) {
    ...
    if (strcmp(a_ty, "<3 x i32>") != 0) {
        rocke_ll_emitf(L, "  %s = bitcast %s %s to <3 x i32>", a_cast, a_ty,
                       rocke_ll_operand(L, a));
    } else {
        a_cast = rocke_ll_operand(L, a);
    }
    if (strcmp(b_ty, "<3 x i32>") != 0) {
        rocke_ll_emitf(L, "  %s = bitcast %s %s to <3 x i32>", b_cast, b_ty,
                       rocke_ll_operand(L, b));
    } else {
        b_cast = rocke_ll_operand(L, b);
    }
...
static void _op_tile_register_p_from_qk_c(rocke_lower_t* L, const rocke_op_t* op) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        rocke_ll_emitf(L, "  %s = extractelement <16 x float> %s, i32 %d", e,
                       rocke_ll_operand(L, qk_c), i);
    ...
    for (i = 0; i < 8; i++) {
        ...
        rocke_ll_emitf(L, "  %s = insertelement <8 x %s> %s, %s %s, i32 %d", name, target_llvm,
                       prev, target_llvm, elems[i], i);
        prev = name;
    }
}
