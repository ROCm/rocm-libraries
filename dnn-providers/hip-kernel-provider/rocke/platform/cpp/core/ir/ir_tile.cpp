 ...
 * arch SSOT (core/arch/target): frag lengths from rocke_arch_mma_d_frag_len (the
 * _MMA_FRAGMENT_INFO projection) and the accumulator dtype from
 * rocke_arch_mma_op_id_d_dtype (the JSON catalog aggregation). This bucket keeps
...
static int rocke_mma_d_frag_len(const char* op_id)
{
    return rocke_arch_mma_d_frag_len(op_id);
}

/* True when op_id accumulates in i32 (integer WMMA), from the arch catalog SSOT
 * (target._op_id_d_dtype()[op_id] == "i32"). */
static bool rocke_mma_is_int_acc(const char* op_id)
{
    const char* d_dtype = rocke_arch_mma_op_id_d_dtype(op_id);
    return d_dtype != NULL && strcmp(d_dtype, ROCKE_DTYPE_I32) == 0;
}
                           ...
                           int num_extra)
{
    ...
    d_frag_len = rocke_mma_d_frag_len(op_id);
    if(d_frag_len <= 0)
    {
        return (rocke_value_t*)rocke_i_set_err(
    ...
    vt = rocke_vector_type(b, c_elem, d_frag_len);
    if(!vt)
    {
        return NULL;
    }
