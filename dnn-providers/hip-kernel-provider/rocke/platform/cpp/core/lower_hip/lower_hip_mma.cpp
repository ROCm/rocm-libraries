...
static rocke_status_t h_emit_gfx1250_scaled_wmma(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    if (!lw->arch.gfx || __builtin_strcmp(lw->arch.gfx, "gfx1250") != 0) {
        return rocke_h_fail(lw, ROCKE_ERR_NOTIMPL, "WMMA op '%s' is not available on %s", op_id,
    ...
    rocke_h_emitf(lw,
                  ...
                  rocke_h_name(lw, op->results[0]), builtin, fmt_a,
                  rocke_h_name(lw, op->operands[0]), fmt_b, rocke_h_name(lw, op->operands[1]),
                  rocke_h_name(lw, op->operands[2]), sa, rocke_h_name(lw, op->operands[3]), sb,
...
static rocke_status_t rocke_h_op_tile_inline_asm(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    return rocke_h_fail(lw, ROCKE_ERR_NOTIMPL, "no HIP lowering for op '%s'",
...
static rocke_status_t rocke_h_op_tile_readfirstlane(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "%s %s = __builtin_amdgcn_readfirstlane(%s);", ty, rocke_h_name(lw, r),
                  rocke_h_name(lw, v));
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_lane_id(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    const rocke_value_t* r = h_res(op);
    rocke_h_emitf(lw, "int %s = __builtin_amdgcn_mbcnt_hi(-1, __builtin_amdgcn_mbcnt_lo(-1, 0));",
                  rocke_h_name(lw, r));
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_ds_bpermute(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    const rocke_value_t* r = h_res(op);
    rocke_h_emitf(lw, "int %s = __builtin_amdgcn_ds_bpermute(%s, %s);", rocke_h_name(lw, r),
                  rocke_h_name(lw, addr), rocke_h_name(lw, data));
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_ds_bpermute_b64(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "int64_t %s = ((int64_t)(uint32_t)%s_phi << 32) | (uint32_t)%s_plo;", nice,
                  nice, nice);
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_ds_swizzle_xor(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "int %s = __builtin_amdgcn_ds_swizzle(%s, %d);", rocke_h_name(lw, r),
                  rocke_h_name(lw, data), offset);
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_mov_dpp(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "int %s = __builtin_amdgcn_update_dpp(%s, %s, %d, 15, 15, %d);",
                  rocke_h_name(lw, r), dn, dn, dpp_ctrl, bound_ctrl ? 1 : 0);
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_byte_perm(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "int %s = __builtin_amdgcn_perm(%s, %s, %uu);", rocke_h_name(lw, r),
                  rocke_h_name(lw, a), rocke_h_name(lw, b), (unsigned)(sel & 0xFFFFFFFF));
    return lw->status;
}
...
static rocke_status_t rocke_h_op_tile_perm_b32(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    const rocke_value_t* r = h_res(op);
    rocke_h_emitf(lw, "int %s = __builtin_amdgcn_perm(%s, %s, %s);", rocke_h_name(lw, r),
                  rocke_h_name(lw, src0), rocke_h_name(lw, src1), rocke_h_name(lw, sel));
    return lw->status;
}
...
static rocke_status_t h_ds_read_tr16(rocke_h_lowerer_t* lw, const rocke_op_t* op, const char* op_id,
                                     const char* raw_ty, const char* shim, int lanes, int bytes) {
    ...
    rocke_h_emitf(lw, "%s%d %s; __builtin_memcpy(&%s, &%s, %d);", vec_prefix, lanes, nice, nice,
                  raw_tmp, bytes);
    return lw->status;
}

static rocke_status_t rocke_h_op_tile_ds_read_tr16_b64(rocke_h_lowerer_t* lw,
                                                       const rocke_op_t* op) {
    return h_ds_read_tr16(lw, op, "ds_read_tr16_b64", "i16x4_raw", "_llvm_amdgcn_ds_read_tr16_b64",
                          4, 8);
}

static rocke_status_t rocke_h_op_tile_ds_read_tr16_b128(rocke_h_lowerer_t* lw,
                                                        const rocke_op_t* op) {
...
static rocke_status_t rocke_h_op_vector_bitcast(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    const char* rn = rocke_h_name(lw, r);
    rocke_h_emitf(lw, "%s %s; __builtin_memcpy(&%s, &%s, sizeof(%s));", tgt, rn, rn,
                  rocke_h_name(lw, v), tgt);
    return lw->status;
}
...
static rocke_status_t rocke_h_op_vector_extract(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "%s %s = %s[%d];", h_elem_scalar(v->type), rocke_h_name(lw, r),
                  rocke_h_name(lw, v), (int)i);
    return lw->status;
}
...
static rocke_status_t h_vec_minmax(rocke_h_lowerer_t* lw, const rocke_op_t* op, const char* cmp) {
    ...
    rocke_h_emitf(lw, "%s %s;", res_t, nice);
    for (i = 0; i < n; i++) {
        rocke_h_emitf(lw, "%s[%d] = (%s[%d] %s %s[%d]) ? %s[%d] : %s[%d];", nice, i, an, i, cmp, bn,
                      i, an, i, bn, i);
    }
    return lw->status;
}
...
static rocke_status_t rocke_h_op_vector_fma(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    rocke_h_emitf(lw, "%s %s;", res_t, nice);
    for (i = 0; i < n; i++) {
        rocke_h_emitf(lw, "%s[%d] = fmaf((float)%s[%d], (float)%s[%d], (float)%s[%d]);", nice, i,
                      an, i, bn, i, cn, i);
    }
    return lw->status;
}
...
static rocke_status_t rocke_h_op_scf_for(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    if (!iv_ty) {
        ...
        return rocke_h_fail(lw, ROCKE_ERR_KEY, "scf.for: no HIP type for iv_type '%s'",
    ...
    for (i = 0; i < meta_n && i < (int)num_iter; i++) {
        ...
        rocke_h_emitf(lw, "%s %s = %s;", h_for_cpp_type(lw, mtype), mnm,
                      rocke_h_name(lw, op->operands[3 + i]));
    }
    lo_n = rocke_h_name(lw, lower);
    up_n = rocke_h_name(lw, upper);
    st_n = rocke_h_name(lw, step);
    rocke_h_emitf(lw, "for(%s %s = %s; %s < %s; %s += %s) {", iv_ty, iv_name, lo_n, iv_name, up_n,
                  iv_name, st_n);
    rocke_h_push_indent(lw);
    rocke_h_lower_region(lw, op->regions[0]);
    rocke_h_pop_indent(lw);
...
static rocke_status_t rocke_h_op_scf_yield(rocke_h_lowerer_t* lw, const rocke_op_t* op) {
    ...
    if (op->num_operands != meta_n) {
        return rocke_h_fail(lw, ROCKE_ERR_VALUE, "scf.yield: %d values vs %d iter_args",
