 ...
 *
 * SCOPE (this TU only):
 *   - The shared spec PODs' _default() / _validate() (__post_init__) and every
 *     @property accessor (num_queries_per_kv, block_m=16, block_q, tile_size,
 *     dtype_ir, binary_search_iters, kernel_name via kernel_name_join). These
 ...
 *     in Python execution order. (build_unified_attention_3d_tiled 253-950 ;
 *     build_unified_attention_reduce_tiled 985-1164.)
 ...
 * emit_q_to_lds / emit_loop_init / emit_async_infra / emit_softmax_loop /
 ...
 * (QK_K_STEP=32, PV_K_STEP=32-or-16, ASYNC_LDS_DWORDS=4, HALVES_PER_LANE=8); no
 * wide-sync / invariant-hoist / wide-KV knobs; tile_size ignores the override;
 * the kernel_name carries no gfx942 suffix nor hoist/wkv parts; the segment
 * driver also calls emit_q_to_lds and emit_async_infra phases in Python order.
 *
                                          ...
                                          const char** reason)
{
    ...
    if(target == NULL)
    {
        if(reason != NULL)
        {
            snprintf(rbuf, rcap, "\"unknown gfx target '%s'\"", eff_arch);
            *reason = rbuf;
        }
        return false;
    }
...
/* ===================================================================== *
 *  SHARED SPEC POD -- _default / _validate / @property
 *
 *  These definitions are byte-identical across the gfx942 and gfx950 3D
 *  ports; when the gfx942 header is present it owns them (see the header's
    ...
    const rocke_unified_attention_3d_tiled_spec_t* s, char* buf, size_t cap)
{
     ...
     * source order. NOTE the gfx950 name has NO gfx942 suffix and NO hoist/wkv
...
#define ATTN3D950_REASON(s)    \
    ...
    {
        ATTN3D950_REASON("tiled 3D kernel needs num_queries_per_kv to divide BLOCK_M=16");
        return false;
    }
    ATTN3D950_REASON("supported");
    return true;
#undef ATTN3D950_REASON
}
...
/* ===================================================================== *
 *  PUBLIC build entry -- segment kernel (Python lines 253-950)
 *
