...
// ===========================================================================
// Descriptor closures for stride=1 dgrad
// ===========================================================================

struct dgrad_descriptor_ctx
{
    rocke_tensor_descriptor_t* desc;
    rocke_value_t* block_off_v; /* block_m_off_v or block_n_off_v */
    rocke_value_t* k_off_capture;
};

static rocke_value_t* _dgrad_dy_descriptor(rocke_ir_builder_t* b,
                                           rocke_value_t* row,
                                           rocke_value_t* col,
                                           rocke_value_t** out_valid,
                                           void* user)
{
    struct dgrad_descriptor_ctx* ctx = (struct dgrad_descriptor_ctx*)user;
    rocke_value_t* m_val = rocke_b_add(b, ctx->block_off_v, row);
    rocke_value_t* k_dg_val = rocke_b_add(b, ctx->k_off_capture, col);
    const char* names[] = {"m", "k_dg"};
    rocke_value_t* vals[] = {m_val, k_dg_val};
...
static rocke_value_t* _dgrad_w_descriptor(rocke_ir_builder_t* b,
                                          rocke_value_t* row,
                                          rocke_value_t* col,
                                          rocke_value_t** out_valid,
                                          void* user)
{
    struct dgrad_descriptor_ctx* ctx = (struct dgrad_descriptor_ctx*)user;
    rocke_value_t* c_val = rocke_b_add(b, ctx->block_off_v, row);
    rocke_value_t* k_dg_val = rocke_b_add(b, ctx->k_off_capture, col);
    const char* names[] = {"c", "k_dg"};
    rocke_value_t* vals[] = {c_val, k_dg_val};
    rocke_value_t* off = NULL;
    rocke_value_t* valid = NULL;
    rocke_transforms_descriptor_offset(b, ctx->desc, names, vals, 2, &off, &valid);
    if(out_valid)
...
// ===========================================================================
// Epilogue address closure for dX (used by direct / cshuffle epilogues)
// ===========================================================================
...
// ===========================================================================
// Split-K atomic epilogue (Python _emit_dgrad_split_k_epilogue)
// ===========================================================================

static void _emit_dgrad_split_k_epilogue(rocke_ir_builder_t* b,
                                         const rocke_dgrad_conv_spec_t* spec,
                                         ...
                                         int c_per_lane)
{
    ...
    rocke_value_t* dg_M_v = rocke_b_const_i32(b, dg_M);
    rocke_value_t* dg_N_v = rocke_b_const_i32(b, dg_N);
        ...
        for(int ni = 0; ni < mfmas_n; ni++)
        {
            ...
            if(is_fp32)
            {
                for(int i = 0; i < c_per_lane; i++)
                {
                    ...
                    rocke_value_t* c_off = rocke_b_add(b, rocke_b_mul(b, c_m, dg_N_v), c_n);
                    rocke_value_t* m_ok = rocke_b_cmp_lt(b, c_m, dg_M_v);
                    rocke_value_t* n_ok = rocke_b_cmp_lt(b, c_n, dg_N_v);
                    rocke_if_t if_ok = rocke_b_scf_if(b, rocke_b_land(b, m_ok, n_ok));
                    rocke_b_region_enter(b, if_ok.then_region);
                    {
                        rocke_b_global_atomic_add(
                            b, dx_ptr, c_off, rocke_b_vec_extract(b, acc, i), NULL);
                    }
                ...
                for(int i = 0; i < c_per_lane; i++)
                {
                    ...
                    rocke_b_region_enter(b, if_ok.then_region);
                    {
                        ...
                        rocke_value_t* c_off_even
                            = rocke_b_add(b, rocke_b_mul(b, c_m, dg_N_v), c_n_even);
                        ...
                        if(is_bf16)
                            rocke_b_global_atomic_add_pk_bf16(b, dx_ptr, c_off_even, vec, NULL);
                        else
                            rocke_b_global_atomic_add_pk_f16(b, dx_ptr, c_off_even, vec, NULL);
                    }
                    rocke_b_region_leave(b);
                }
            }
...
// ===========================================================================
// Direct epilogue (Python _emit_dgrad_direct_epilogue)
// ===========================================================================
...
// ===========================================================================
// CShuffleEpilogue (Python _emit_dgrad_cshuffle_epilogue)
// ===========================================================================
...
// ===========================================================================
// Stride=1 dgrad kernel builder
// (Python build_implicit_gemm_conv_dgrad, lines 833-1314)
// ===========================================================================

static rocke_kernel_def_t* _build_stride1_dgrad(rocke_ir_builder_t* b,
                                                const rocke_dgrad_conv_spec_t* spec,
                                                const char* arch)
{
    const rocke_conv_problem_t* p = &spec->problem;
    ...
    int dg_M = rocke_dgrad_conv_spec_dg_M(spec);
    int dg_N = rocke_dgrad_conv_spec_dg_N(spec);
    int dg_K = rocke_dgrad_conv_spec_dg_K(spec);
    ...
    rocke_value_t* c_dg_K = rocke_b_const_i32(b, dg_K);
    ...
    if(is_split_k)
    {
        ...
        rocke_value_t* c_ks = rocke_b_const_i32(b, dg_K_padded / spec->split_k);
        k_lo = rocke_b_to_sgpr_u32(b, rocke_b_mul(b, rocke_b_block_id_z(b), c_ks));
        k_hi = rocke_b_to_sgpr_u32(b, rocke_b_add(b, k_lo, c_ks));
    }
    else
    {
        k_lo = c0;
    ...
    {
        int num_pid_m = _ceil_div(dg_M, block_m);
        int num_pid_n = _ceil_div(dg_N, block_n);
        ...
        rocke_super_tile_swizzle_result_t swz = rocke_chiplet_aware_super_tile(
            b, wgid_flat, num_pid_m, num_pid_n,
            spec->chiplet_wgm, spec->chiplet_num_xcds, spec->chiplet_chunk_size);
        block_m_off_v = rocke_b_mul(b, swz.row, rocke_b_const_i32(b, block_m));
        block_n_off_v = rocke_b_mul(b, swz.col, rocke_b_const_i32(b, block_n));
    ...
    int b_shape[2] = {block_n, lds_layout.row_stride};
    rocke_value_t* A_smem = rocke_b_smem_alloc(b, rocke_f16(), a_shape, 2, "A_smem");
    rocke_value_t* B_smem = rocke_b_smem_alloc(b, rocke_f16(), b_shape, 2, "B_smem");
    bool double_buffer = (spec->pipeline != NULL && strcmp(spec->pipeline, "compv4") == 0)
                         || spec->async_dma || spec->unroll_k;
    rocke_value_t* A_smem2 = double_buffer
        ? rocke_b_smem_alloc(b, rocke_f16(), a_shape, 2, "A_smem2") : A_smem;
    rocke_value_t* B_smem2 = double_buffer
        ? rocke_b_smem_alloc(b, rocke_f16(), b_shape, 2, "B_smem2") : B_smem;
    (void)A_smem2;
    (void)B_smem2;
    ...
    // ---- descriptors (compile-time) ----
    rocke_tensor_descriptor_t* dY_desc = rocke_dgrad_make_dy_descriptor(b, p, spec->dtype_a);
    rocke_tensor_descriptor_t* W_desc = rocke_dgrad_make_w_descriptor(b, p, spec->dtype_b);
    ...
    // ---- descriptor closures ----
    struct dgrad_descriptor_ctx dy_ctx;
    dy_ctx.desc = dY_desc;
    dy_ctx.block_off_v = block_m_off_v;
    dy_ctx.k_off_capture = NULL;

    struct dgrad_descriptor_ctx w_ctx;
    w_ctx.desc = W_desc;
    w_ctx.block_off_v = block_n_off_v;
    w_ctx.k_off_capture = NULL;

    // ---- loaders ----
    rocke_async_tile_loader_t a_async_loader;
    rocke_async_tile_loader_t b_async_loader;
    rocke_coalesced_tile_loader_t a_sync_loader;
    rocke_coalesced_tile_loader_t b_sync_loader;
    if(spec->async_dma)
    {
        rocke_async_tile_loader_from_tile(
            block_m, block_k, block_size, spec->wave_size, 4, &a_async_loader);
        rocke_async_tile_loader_from_tile(
            block_n, block_k, block_size, spec->wave_size, 4, &b_async_loader);
    }
    else
    {
        rocke_coalesced_tile_loader_from_tile(
            block_m, block_k, block_size, 1, true, &a_sync_loader);
        rocke_coalesced_tile_loader_from_tile(
            block_n, block_k, block_size, 1, true, &b_sync_loader);
    }

    // ---- schedule ----
    rocke_schedule_policy_t schedule
        = rocke_schedule_policy_for_pipeline(b, spec->async_dma ? "async_dma" : spec->pipeline);
    rocke_schedule_policy_emit_prologue(&schedule, b);

    // ---- Emit MFMA phase helper (used by both sync and async K-loop paths) ----
    // Macro-like inline to avoid code duplication; relies on lexical scope.
    // Inputs: A_src, B_src (LDS buffers), acc_in[num_accs] → acc_out[num_accs]
#define EMIT_DGRAD_MFMA_PHASE(A_src_, B_src_, acc_in_, acc_out_)                              \
    do {                                                                                       \
        if(is_wmma)                                                                            \
        {                                                                                      \
            const rocke_arch_layout_map_t* _am = rocke_mmaop_a_layout(op, b);                 \
            const rocke_arch_layout_map_t* _bm = rocke_mmaop_b_layout(op, b);                 \
            rocke_value_t *_ar = NULL, *_ak = NULL, *_bk = NULL, *_bc = NULL;                 \
            rocke_arch_layout_map_coord(_am, b, lane, 0, &_ar, &_ak);                         \
            rocke_arch_layout_map_coord(_bm, b, lane, 0, &_bk, &_bc);                         \
            rocke_value_t* _wm = rocke_warp_grid_warp_m_off(b, &grid);                        \
            rocke_value_t* _wn = rocke_warp_grid_warp_n_off(b, &grid);                        \
            rocke_value_t* _ar2[ROCKE_CONV_MAX_ACCS];                                         \
            rocke_value_t* _bc2[ROCKE_CONV_MAX_ACCS];                                         \
            for(int _i = 0; _i < num_accs; _i++) (acc_out_)[_i] = (acc_in_)[_i];             \
            for(int _kk = 0; _kk < k_atoms; _kk++) {                                         \
                rocke_value_t* _ktb = rocke_b_const_i32(b, _kk * spec->warp_tile_k);         \
                for(int _mi = 0; _mi < mfmas_m; _mi++) {                                      \
                    rocke_value_t* _ar3 = rocke_b_add(b, _wm,                                 \
                        rocke_b_const_i32(b, _mi * spec->warp_tile_m));                       \
                    _ar2[_mi] = rocke_conv_emit_frag_smem_load(b, A_src_, _ar, _ak,           \
                        _ar3, _ktb, a_per_lane);                                              \
                }                                                                              \
                for(int _ni = 0; _ni < mfmas_n; _ni++) {                                      \
                    rocke_value_t* _br3 = rocke_b_add(b, _wn,                                 \
                        rocke_b_const_i32(b, _ni * spec->warp_tile_n));                       \
                    _bc2[_ni] = rocke_conv_emit_frag_smem_load(b, B_src_, _bc, _bk,           \
                        _br3, _ktb, b_per_lane);                                              \
                }                                                                              \
                int _f = 0;                                                                    \
                for(int _mi = 0; _mi < mfmas_m; _mi++)                                        \
                    for(int _ni = 0; _ni < mfmas_n; _ni++, _f++)                              \
                        (acc_out_)[_f] = rocke_b_mma(b, op->op_id,                            \
                            _ar2[_mi], _bc2[_ni], (acc_out_)[_f], NULL, 0);                   \
            }                                                                                  \
        }                                                                                      \
        else                                                                                   \
        {                                                                                      \
            rocke_lane_decode_t _dec = rocke_decode_mfma_lanes(b, atom, lane);                \
            rocke_value_t* _mi_a = _dec.m_in_atom;                                            \
            rocke_value_t* _ni_a = _dec.n_in_atom;                                            \
            rocke_value_t* _kb  = _dec.k_blk;                                                 \
            rocke_value_t* _wm  = rocke_warp_grid_warp_m_off(b, &grid);                       \
            rocke_value_t* _wn  = rocke_warp_grid_warp_n_off(b, &grid);                       \
            for(int _i = 0; _i < num_accs; _i++) (acc_out_)[_i] = (acc_in_)[_i];             \
            for(int _kk = 0; _kk < k_atoms; _kk++) {                                          \
                rocke_value_t* _cb_mul = rocke_b_mul(b, _kb,                                   \
                    rocke_b_const_i32(b, a_per_lane));                                         \
                rocke_value_t* _cb = rocke_b_add(b, _cb_mul,                                   \
                    rocke_b_const_i32(b, _kk * spec->warp_tile_k));                            \
                rocke_value_t* _ar[ROCKE_CONV_MAX_ACCS];                                       \
                rocke_value_t* _bc[ROCKE_CONV_MAX_ACCS];                                       \
                for(int _mi = 0; _mi < mfmas_m; _mi++) {                                       \
                    rocke_value_t* _r = rocke_b_add(b, _wm, rocke_b_add(b,                    \
                        rocke_b_const_i32(b, _mi * spec->warp_tile_m), _mi_a));               \
                    _ar[_mi] = rocke_conv_emit_smem_load(b, A_src_, _r, _cb, a_per_lane);     \
                }                                                                               \
                for(int _ni = 0; _ni < mfmas_n; _ni++) {                                       \
                    rocke_value_t* _r = rocke_b_add(b, _wn, rocke_b_add(b,                    \
                        rocke_b_const_i32(b, _ni * spec->warp_tile_n), _ni_a));               \
                    _bc[_ni] = rocke_conv_emit_smem_load(b, B_src_, _r, _cb, b_per_lane);     \
                }                                                                               \
                int _f = 0;                                                                     \
                for(int _mi = 0; _mi < mfmas_m; _mi++)                                         \
                    for(int _ni = 0; _ni < mfmas_n; _ni++, _f++)                               \
                        (acc_out_)[_f] = rocke_conv_emit_mfma(b, atom,                         \
                            _ar[_mi], _bc[_ni], (acc_out_)[_f]);                               \
    ...
    int num_final = num_accs;

    if(spec->async_dma)
    {
        // ---- async DMA path: SoftwarePipeline.run_ping_pong ----
        // double_buffer=True → nb=2, rotating=True, prefetch_depth=1
        // wait_vmcnt=True, sync_after_wait=True, sync_before_issue=True, overlap_vmcnt=True
        int dg_K_val = (k_hi != NULL) ? rocke_dgrad_conv_spec_dg_K_padded(spec) / spec->split_k
                                      : dg_K;
        int K_iters = _ceil_div(dg_K_val, block_k);

        rocke_value_t* buf_a[2] = {A_smem, A_smem2};
        rocke_value_t* buf_b[2] = {B_smem, B_smem2};

        rocke_value_t* state[ROCKE_CONV_MAX_ACCS];
        rocke_value_t* new_state[ROCKE_CONV_MAX_ACCS];
        for(int i = 0; i < num_accs; i++)
            state[i] = iter_args[i].init;

        if(K_iters > 0)
        {
            // Prologue: issue first load (mirrors Python: b.add(k_lo, b.const_i32(0*block_k)))
            dy_ctx.k_off_capture = rocke_b_add(b, k_lo, rocke_b_const_i32(b, 0));
            w_ctx.k_off_capture = dy_ctx.k_off_capture;
            {
                rocke_async_tile_loader_slot_t a_slot;
                rocke_async_tile_loader_bind(b, &a_async_loader, buf_a[0], warp_id, &a_slot);
                rocke_async_tile_loader_slot_issue(b, &a_slot, tid, dy_rsrc,
                    _dgrad_dy_descriptor, &dy_ctx, 0x7FFFFFFF, ROCKE_CACHE_STREAM);
                rocke_async_tile_loader_slot_t b_slot;
                rocke_async_tile_loader_bind(b, &b_async_loader, buf_b[0], warp_id, &b_slot);
                rocke_async_tile_loader_slot_issue(b, &b_slot, tid, w_rsrc,
                    _dgrad_w_descriptor, &w_ctx, 0x7FFFFFFF, ROCKE_CACHE_STREAM);
            }
            ...
            for(int it = 0; it < K_iters; it++)
            {
                int cur = it % 2;
                int issue_idx = it + 1;
                bool has_next = (issue_idx < K_iters);

                if(has_next)
                {
                    if(it > 0) rocke_b_sync_lds_only(b);
                    // mirrors Python: b.add(k_lo, b.const_i32(issue_idx * block_k))
                    dy_ctx.k_off_capture = rocke_b_add(b, k_lo, rocke_b_const_i32(b, issue_idx * block_k));
                    w_ctx.k_off_capture = dy_ctx.k_off_capture;
                    rocke_async_tile_loader_slot_t a_slot;
                    rocke_async_tile_loader_bind(b, &a_async_loader, buf_a[issue_idx % 2], warp_id, &a_slot);
                    rocke_async_tile_loader_slot_issue(b, &a_slot, tid, dy_rsrc,
                        _dgrad_dy_descriptor, &dy_ctx, 0x7FFFFFFF, ROCKE_CACHE_STREAM);
                    rocke_async_tile_loader_slot_t b_slot;
                    rocke_async_tile_loader_bind(b, &b_async_loader, buf_b[issue_idx % 2], warp_id, &b_slot);
                    rocke_async_tile_loader_slot_issue(b, &b_slot, tid, w_rsrc,
                        _dgrad_w_descriptor, &w_ctx, 0x7FFFFFFF, ROCKE_CACHE_STREAM);
                }

                rocke_b_s_waitcnt(b, has_next ? 1 : 0, -1, -1);
                if(has_next) rocke_b_sync_lds_only(b);
                else rocke_b_sync(b);

                rocke_schedule_policy_emit_compute_prologue(&schedule, b);

                EMIT_DGRAD_MFMA_PHASE(buf_a[cur], buf_b[cur], state, new_state);
                for(int i = 0; i < num_accs; i++) state[i] = new_state[i];

                rocke_schedule_policy_emit_compute_epilogue(&schedule, b);
            }
        }
        for(int i = 0; i < num_accs; i++) final_accs[i] = state[i];
        num_final = num_accs;
    }
    else
    {
    // ---- sync path: scf.for_iter ----
    rocke_value_t* k_upper = (k_hi != NULL) ? k_hi : c_dg_K;

    rocke_for_t for_op = rocke_b_scf_for_iter(
        b, k_lo, k_upper, c_block_k, iter_args, num_accs, "k0", false, true);
    ...
    {
        // emit_load_phase: set k_off_capture, load dY and W
        dy_ctx.k_off_capture = k0;
        w_ctx.k_off_capture = k0;
        rocke_coalesced_tile_loader_load(
            b, &a_sync_loader, tid, A_smem, _dgrad_dy_descriptor, &dy_ctx, dy_rsrc, NULL);
        rocke_coalesced_tile_loader_load(
            b, &b_sync_loader, tid, B_smem, _dgrad_w_descriptor, &w_ctx, w_rsrc, NULL);
        rocke_b_sync(b);

        // emit_mfma_phase
        rocke_value_t* new_accs[ROCKE_CONV_MAX_ACCS];
        EMIT_DGRAD_MFMA_PHASE(A_smem, B_smem, iter_vars, new_accs);
...
#undef EMIT_DGRAD_MFMA_PHASE
    ...
    else if(spec->epilogue && strcmp(spec->epilogue, "cshuffle") == 0)
    {
                                           ...
                                           void* user)
{
    ...
    rocke_value_t* k_out = rocke_b_div(b_, k_sub, ctx->ydot_times_xdot);
    rocke_value_t* yx_rem = rocke_b_mod(b_, k_sub, ctx->ydot_times_xdot);
                                              ...
                                              const char* arch)
{
    ...
    // K decomposition divisors (runtime)
    rocke_value_t* ydot_times_xdot = rocke_b_mul(b, rec_y_dot_slice, rec_x_dot_slice);
    ...
    // ---- loaders (sync only, load_vec=1) ----
    int threads = block_size;
    rocke_coalesced_tile_loader_t a_sync_loader;
    rocke_coalesced_tile_loader_t b_sync_loader;
    rocke_coalesced_tile_loader_from_tile(block_m, block_k, threads, 1, true, &a_sync_loader);
    rocke_coalesced_tile_loader_from_tile(block_n, block_k, threads, 1, true, &b_sync_loader);

    tilde_dy_ctx_t dy_tctx;
    dy_tctx.block_m_off = block_m_off_v;
    dy_tctx.k_off = NULL;
    dy_tctx.ydot_times_xdot = ydot_times_xdot;
    ...
    w_tctx.k_off = NULL;
    w_tctx.ydot_times_xdot = ydot_times_xdot;
                                                         ...
                                                         const char* arch)
{
    ...
    // Dispatch stride=1 vs tilde
    if(rocke_dgrad_conv_spec_is_strided(spec))
        return _build_tilde_dgrad(b, spec, arch);
    else
        return _build_stride1_dgrad(b, spec, arch);
}
