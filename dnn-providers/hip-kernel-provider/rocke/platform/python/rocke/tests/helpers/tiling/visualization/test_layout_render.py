...
def test_render_views_writes_all_panels(tmp_path):
    paths = lr.render_views(_mma().a_layout, axes=("M", "K"),
                            ...
                            dtype_bits=16, out_dir=str(tmp_path), name="v", combined=True)
    assert paths and all(os.path.getsize(p) > 0 for p in paths)
def test_register_and_logical_components_render(tmp_path):
    mma = _mma()
    rp = lr.RegisterFileComponent(dist=mma.a_layout, dims=("M", "K")).render(
        str(tmp_path / "rf.png"))
...
def test_logical_view_modes(tmp_path):
    ...
    b = lr.LogicalTileComponent(**base, mode="thread_tile", addr_fn=addr)
    c = lr.LogicalTileComponent(**base, mode="coalescing", addr_fn=addr)
    ...
    # coalescing auto-derives physical-vector run groups + a shade map.
    assert len(c.groups) >= 64 and c.shade_map is not None
...
def test_workflow_recipes_render(tmp_path):
    ...
    addr = lambda m, k: m * 16 + k
    w1 = lr.flow_mem_to_register(dist, dims=("M", "K"))
    w1.trace((3, 5))
    w2 = lr.flow_lds_to_register(mp, addr, read_dist=dist, dims=("M", "K"))
    w2.trace((3, 5))
...
def test_pipeline_traces_origin_across_stages(tmp_path):
    ...
    pipe = lr.Pipeline(stages=(
        ...
        lr.FlowStage("registers", lr.RegisterFileComponent(dist=dist, dims=("M", "K")),
                     transform="global load"),
        lr.FlowStage("lds", lr.LdsBankView(mp=mp, addr_fn=addr, dims=("M", "K")),
                     transform="LDS store"),
    ), title="A dataflow")
...
def test_lds_bank_view_renders_flow_and_thread(tmp_path):
    ...
    flow = lr.LdsBankView(mp=mp, addr_fn=addr, nbanks=32, label_by="flow", dims=("M", "K"))
    thread = lr.LdsBankView(mp=mp, addr_fn=addr, nbanks=32, label_by="thread")
