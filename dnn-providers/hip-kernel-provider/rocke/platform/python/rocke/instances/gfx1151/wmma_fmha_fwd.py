...
from ...core.ir import BF16, F16, F32, I32, IRBuilder, KernelDef, PtrType

# Element dtype normalization: the spec accepts fp16/f16/bf16; the inner body and
# the WMMA atom selection key on the canonical "f16"/"bf16" tokens. bf16 shares
# the f16 WMMA fragment layout on gfx1151 (same 16x16x16 lane math), so both flow
# through the identical inner body -- only the operand IR type and the WMMA
# intrinsic differ.
_DTYPE_CANON = {"fp16": "f16", "f16": "f16", "bf16": "bf16"}
_DTYPE_IR = {"f16": F16, "bf16": BF16}
...
_WMMA_OP_ID = "wmma_f32_16x16x16_f16"  # legacy default (f16)
_WMMA_OP_ID_BY_DTYPE = {
...
class WmmaFmhaFwdSpec:
    ...
    def __post_init__(self) -> None:
        if self.dtype not in _DTYPE_CANON:
            raise ValueError(
                f"WmmaFmhaFwdSpec supports {sorted(_DTYPE_CANON)}, got {self.dtype!r}"
            )
    ...
    @property
    def dtype_canon(self) -> str:
        """Canonical element dtype token ("f16" | "bf16")."""
        return _DTYPE_CANON[self.dtype]
...
def is_valid_spec(spec: WmmaFmhaFwdSpec, arch: str = "gfx1151") -> Tuple[bool, str]:
    ...
    try:
        ...
        return False, str(e)
    op_id = _WMMA_OP_ID_BY_DTYPE.get(_DTYPE_CANON.get(spec.dtype, ""), _WMMA_OP_ID)
...
def _declare_params(b: IRBuilder, elem=F16):
    ...
    kernel serves both MHA and GQA (kv_head stride differs). ``elem`` is the
    Q/K/V/O element IR type (F16 or BF16).
    """
...
def build_wmma_fmha_fwd(spec: WmmaFmhaFwdSpec, arch: str = "gfx1151") -> KernelDef:
    ...
    p = _declare_params(b, _DTYPE_IR[spec.dtype_canon])
