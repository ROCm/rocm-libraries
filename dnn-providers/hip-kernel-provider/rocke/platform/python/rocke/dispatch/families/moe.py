...
THE ONLY REGISTRY FOR THIS FAMILY
---------------------------------
There was a second candidate set under ``library/dispatch/moe`` claiming this
same ``family`` string and ``abi_version``, with its own request type, its own
spec dataclasses and its own copy of the band table; ``rocke-serve`` dispatched
through that one while everything else came here. Two registries answering to
one ``KernelId`` is not a divergence that tests can hold shut -- a request could
mint an id here and resolve a different kernel there -- so it was deleted and
serve now plans through this module. Anything that needs to dispatch MoE
imports from here.
...
class MoeRequest(OperatorRequest):
    ...
    activation: str = "silu"
    #: Compute units the caller intends to run on. Not a correctness input and
    #: not part of the compile identity -- it exists so a persistent-style
    #: launch can be sized by a caller that knows its own partition.
    num_sms: int = 120
