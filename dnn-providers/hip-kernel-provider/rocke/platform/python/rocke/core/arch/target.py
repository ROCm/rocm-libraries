...
class MmaOp:
    ...
    IDs including source and result dtypes and per-source scales. The backend
    maps this identity to an intrinsic and format selectors.
    ...
    def semantic_id(self) -> str:
        """Stable semantic identity, independent of backend packing and names."""
        fields = [self.family.replace("_", "."), f"{self.m}x{self.n}x{self.k}"]
        for i, src in enumerate(self.srcs):
            field = f"src{i}_{normalize_dtype(src.dtype)}"
            if src.scale is not None:
                field += f"_{src.scale.dtype}_b{src.scale.block_size}"
            fields.append(field)
        fields.append(f"dst_{normalize_dtype(self.dst.dtype)}")
        return ".".join(fields)
...
_MMA_FRAGMENT_INFO: Dict[str, _FragInfo] = {
    ...
    "wmma.scaled.16x16x128.src0_fp8e4m3_e8m0_b32.src1_fp8e4m3_e8m0_b32.src2_fp32.dst_fp32": _shared_src2_dst_frag_info(
        16,
        16,
        8,
        32,
    ...
    "wmma.scaled.16x16x128.src0_bf8e5m2_e8m0_b32.src1_bf8e5m2_e8m0_b32.src2_fp32.dst_fp32": _shared_src2_dst_frag_info(
        16,
        16,
        8,
        32,
    ...
    "wmma.scaled.16x16x128.src0_fp8e4m3_e8m0_b16.src1_fp8e4m3_e8m0_b16.src2_fp32.dst_fp32": _shared_src2_dst_frag_info(
        16,
        16,
        8,
        32,
    ...
    "wmma.scaled.16x16x128.src0_bf8e5m2_e8m0_b16.src1_bf8e5m2_e8m0_b16.src2_fp32.dst_fp32": _shared_src2_dst_frag_info(
        16,
        16,
        8,
        32,
...
class MmaCatalog:
    """Query concrete operand contracts; omitted scales are a wildcard."""
    ...
    def enumerate(
        ...
        *,
        ...
        src_scales: Optional[Tuple[Optional[MmaScaleOperand], ...]] = None,
        ...
        if src_scales is not None and len(src_scales) != 3:
            raise ValueError("src_scales must contain exactly 3 entries")
        ...
        for op in self._ops:
            ...
            if (
                ...
                continue
            if src_scales is not None and tuple(src.scale for src in op.srcs) != tuple(
                src_scales
            ):
                continue
    ...
    @staticmethod
    def _unique(ops: List[MmaOp]) -> Optional[MmaOp]:
        if len(ops) > 1:
...
def _build_mma_op(o: dict) -> MmaOp:
    ...
    if op.family == "wmma_scaled" and op.op_id != op.semantic_id():
        raise ValueError(
            f"scaled MMA catalog ID does not match its contract: {op.op_id}"
        )
    return op
