...
The A/B/C/D names introduced by this work apply to one MMA atom:

- MMA A and B are multiplicand inputs.
- MMA C is the accumulator input.
- MMA D is the produced result.

They do not rename operator-level tensors. In particular, CK-style Multi-D GEMM
uses ``D0`` through ``Dn`` for auxiliary epilogue inputs such as bias, residual,
or gate tensors. Those operands are unrelated to the D result of an individual
MMA instruction. Documentation and source comments must qualify these as
``MMA D`` and ``epilogue D`` respectively when both meanings are in scope.

Bare ``d`` is also already used as a head-dimension tile index in the shared
platform attention implementation. In code that now consumes MMA D metadata,
rename that index to ``head_tile_idx`` and its count to
``num_head_dim_tiles``. Qualify result metadata locals as
``mma_d_layout`` and ``mma_d_frag_len``. These are source-only renames and must
not change emitted IR.

### Planned file inventory

Implementation and source documentation:

- ``platform/python/rocke/helpers/mfma_attention.py``
- ``platform/cpp/helpers/mfma_attention.cpp``
- ``platform/python/rocke/core/arch/target.py``
- ``platform/python/rocke/helpers/atoms.py``
- ``platform/python/rocke/helpers/distribution.py``
- ``platform/python/rocke/instances/common/gemm_multi_d.py``
- ``platform/python/rocke/instances/common/gemm_multi_abd.py``

User-facing documentation:

- ``platform/dsl_docs/reference/glossary.md``
- ``platform/dsl_docs/reference/mfma_atom_catalog.md``
- ``platform/dsl_docs/instances/gemm.md``

No files under ``rocke/library/kernels`` or ``rocke/library/builders`` are in
scope. The architecture schema, public C atom fields, serialized
``tile.mma(A, B, C) -> D`` form, and ``unpack()`` signatures remain unchanged.

### Follow-up validation

1. Load ``ubuntu-24`` and ``rocm/10`` and record the resolved ROCm version.
2. Run repository pre-commit hooks.
3. Run the focused four-role, architecture-SSOT, fragment-table, and gfx12
   accumulator-layout tests.
4. Run the Python/C differential byte-identity gate for all 69 registered
   families and LLVM 20, 22, and 23.
5. Run the existing CTest suite.
6. Require ``git diff --check`` and verify that the follow-up diff contains no
   paths outside ``rocke/platform`` other than this plan document.

If generated bytes differ, the change is no longer terminology-only and must
stop for COMGR and target-specific runtime validation before publication.
