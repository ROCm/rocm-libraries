...
//
// M4 execute+numerics: for one NON-CAUSAL bf16 head_dim=128 prefill SDPA shape on gfx950,
// enumerate all applicable engines (no pin), then EXECUTE EACH ONE (pinned in turn) and check
// its on-device output against a single fp32 host non-causal-SDPA reference. Proves both
// FlydslAttention (raw-loaded flyDSL HSACO) AND ASM_SDPA_ENGINE (aiter fwd_hd128_bf16.co)
// actually RUN and compute correctly for the SAME problem — the substance of the bake-off.
//
// (Autotuned winner-selection is a separate step; here we just prove every ring member is
// numerically real. Non-causal is the flyDSL∩asm_sdpa common shape: the gfx950 asm_sdpa
// catalog has no causal kernel.)
...
// Host non-causal SDPA reference (fp32 math), physical BSHD layout.
...
int buildGraph(hipdnnHandle_t handle, const Shape& sh, float scale, int64_t pinnedEngine,
    ...
    if (pinnedEngine != 0) {
        graph->set_preferred_engine_id_ext(pinnedEngine);
    }
...
int main() {
    ...
    // ---- Enumerate all applicable engines (no pin) ----------------------------------------
    ...
    std::printf("\n[app] bake-off: %zu engine(s) enumerated for non-causal bf16 d128 (H=%d):\n",
                ranked.size(), sh.H);
    for (const auto id : ranked) {
        std::printf("        %-40s (id=%lld)\n",
                    hipdnn_data_sdk::utilities::engineNameOrHex(id).c_str(),
                    static_cast<long long>(id));
    }
    if (ranked.size() < 2) {
        std::fprintf(stderr, "[app] FAIL: expected >=2 engines, got %zu\n", ranked.size());
        return 5;
    }

    // ---- Execute EACH engine pinned, check numerics ---------------------------------------
    int failures = 0;
    for (const auto id : ranked) {
        const std::string name = hipdnn_data_sdk::utilities::engineNameOrHex(id);
        std::printf("\n[app] --- executing engine %s (id=%lld) ---\n", name.c_str(),
                    static_cast<long long>(id));
        ...
        bool present = false;
        for (const auto x : gr)
            if (x == id) present = true;
        if (!present) {
            std::fprintf(stderr, "[app]   pin failed: engine not in ranked set\n");
            ++failures;
            continue;
        }
        ...
        const bool ok = (maxAbsErr < 5e-2f) && std::isfinite(maxAbsErr);
        std::printf("[app]   workspace=%lld B, max_abs_err vs fp32 ref = %.3e  -> %s\n",
                    static_cast<long long>(wsBytes), maxAbsErr, ok ? "OK" : "FAIL");
        if (!ok) ++failures;
    }
    ...
    if (failures != 0) {
        std::fprintf(stderr, "\n[app] FAILED: %d engine(s) mismatched.\n", failures);
        return 6;
    }
    std::printf(
        "\n[app] SUCCESS: ALL %zu enumerated engines executed the SAME non-causal SDPA problem "
        "correctly on gfx950 (flyDSL + asm_sdpa bake-off, numerics verified).\n",
        ranked.size());
    return 0;
}
