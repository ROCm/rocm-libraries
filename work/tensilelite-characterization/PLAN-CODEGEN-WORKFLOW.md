...
- 2026-06-06 — **P4 round 6 — 78.18% → 78.78% (+0.60 pts, 11 candidates), `coverage/p4/master-baseline-R6.txt`.**
  ...
  — the arch-specific asm-capability arms were ALREADY covered by existing test_emit_<arch> + prior rounds.
  Real contributors: Subtile/SubtileGREmit −86, LibraryLogic −66, ClientWriter −52 (new ClientConfigIni
  suite, 53 tests via real ClientWriter + monkeypatched globalParameters), Solution −44 (sol_breadth),
  Activation −23. gsu3 covered 762 GSU lines in isolation but +0 whole-project (fully redundant w/ suite).
  Dropped rich_gfx950 (design-reject) + gwb2 + asmstore2 (verifier stable=false from coverage
  concurrency=multiprocessing non-determinism AND 0 named-target marginal; re-gate without them = identical
  78.78%, confirming 0 contribution). Round deltas now +0.36/+3.32/+2.82/+1.33/+1.50/+0.60. 3183 passed / 0 failed.
- 2026-06-06 — **P4 round 5 — 76.68% → 78.18% (+1.50 pts, 5 candidates), `coverage/p4/master-baseline-R5.txt`.**
  Final-cluster push via dynamic workflow `wf_dc503b3d-983`. 725 lines (11168→10443 miss). KW 1472→1217
  (the 222-line MFMA local-read pack scheduler), KWA 2974→2775 (fp8 global-read toF8 conversion + complex
  GEMM conjugate arms — complex DOES emit CPU-only), Solution 1253→1165 (auto-LRVW + UseSubtileImpl storeD),
  Subtile/Kernel 153→130. 4 candidates design-rejected (LocalSplitU-store, StreamK-fixup, GWB-atomic,
  ShiftVector — could not be turned on / cleared cutoff CPU-only; P5 ceiling evidence) and lsu_emit2 dropped
  (LocalRead.py coverage non-deterministic under concurrency=multiprocessing). 2953 passed / 0 failed.
