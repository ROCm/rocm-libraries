...
(wf/mutant-identity.py) - NOT mutmut ordinal IDs. Implements the fail-on-regression
...
tool then derives them via wf/mutant-identity.py so both forms are accepted.
...
def entry_key(e):
    ...
    if anchor is None or dhash is None:
        raise ValueError("entry missing line_anchor/diff_hash (and no diff to derive them): %r" % e)
...
def compare(baseline, current):
    ...
    result = {
        "ok": [], "regression": [], "reaudit": [], "absent": [],
        "reformat_suspect": [], "new": [], "other": [],
    }
...
def run(args):
    ...
    emit("other", "OTHER")
    print("")
    print("summary: ok=%d regression=%d reformat_suspect=%d reaudit=%d absent=%d new=%d other=%d (stage=%s)" % (
        len(res["ok"]), len(res["regression"]), len(res["reformat_suspect"]),
        len(res["reaudit"]), len(res["absent"]), len(res["new"]), len(res["other"]), args.stage))
...
def main(argv=None):
    p = argparse.ArgumentParser(description="CI mutation regression gate (stable identity)")
    p.add_argument("--baseline", required=True)
    p.add_argument("--current", required=True)
    p.add_argument("--stage", choices=("report-only", "fail-on-regression"), default="fail-on-regression")
    p.add_argument("--strict", action="store_true", help="also fail on equivalent->killed re-audit flags")
    p.add_argument("--fail-on-new", action="store_true", help="also fail on new (unbaselined) mutants")
