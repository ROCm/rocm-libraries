...
def score(importers_n, cyclomatic_n, loc_n, no_test_n):
    ...
    return (W_IMPORTERS * importers_n + W_CYCLOMATIC * cyclomatic_n
            + W_LOC * loc_n + W_NO_TEST * no_test_n)
...
DISPOSITIONS = {
    "CommonUtilities": (CERTIFIED, ["Tensile/Common/Utilities.py"],
        "slice-1 certified (survivors triaged, fresh-rerun certified)", "PILOT-BASELINE.md; PLAN slice 1"),
    "TensileLogic": (CERTIFIED, [
        "Tensile/TensileLogic/ValidChipId.py", "Tensile/TensileLogic/ValidMatrixInstruction.py",
        "Tensile/TensileLogic/ValidWorkGroup.py", "Tensile/TensileLogic/ValidWorkGroupMappingXCC.py"],
    ...
    "LibraryIO": (SCHEDULED, ["Tensile/LibraryIO.py"], "slice 2 (serialization/round-trip)", "PLAN slice table row 2"),
    "ValidParameters": (SCHEDULED, ["Tensile/Common/ValidParameters.py"], "slice 3 (parameter-validation gate)", "PLAN slice table row 3"),
    "Configuration": (SCHEDULED, ["Tensile/Configuration.py"], "slice 4 (config resolution)", "PLAN slice table row 4"),
    "Naming": (SCHEDULED, ["Tensile/SolutionStructs/Naming.py"], "slice 5 (kernel naming contracts)", "PLAN slice table row 5"),
    "SolutionStructsUtils": (SCHEDULED, ["Tensile/SolutionStructs/Utilities.py", "Tensile/SolutionStructs/LdsPadding.py"],
        "slice 6 (grouped: Utilities+LdsPadding)", "PLAN slice table row 6"),
    "CustomYamlLoader": (SCHEDULED, ["Tensile/CustomYamlLoader.py"], "slice 7 (leaf I/O)", "PLAN slice table row 7"),
    "SolutionClass": (SCHEDULED, ["Tensile/SolutionStructs/Solution.py"], "slice 10 (Solution.py covering dir; sub-sliced)", "PLAN slice table row 10"),
    "SolutionDerivationSweep": (SCHEDULED, ["Tensile/SolutionStructs/Solution.py"], "slice 10 (Solution.py covering dir; sub-sliced)", "PLAN slice table row 10"),
    "Activation": (DEF_HV, ["Tensile/Activation.py"], "deferred high-value next tranche (~156 char tests; plan number is a def-test count, NOT LOC)", "PLAN deferred list"),
    "TensileCreateLibraryRun": (DEF_HV, [], "deferred high-value next tranche (~90 char tests)", "PLAN deferred list"),
    "LibraryLogic": (DEF_HV, [], "deferred high-value next tranche (~56 char tests)", "PLAN deferred list"),
    "ProblemType": (DEF_HV, [], "deferred high-value next tranche (~52 char tests); NOTE: this dir imports SolutionStructs/Problem.py (slice 9 target) - its scheduled-vs-deferred status must be resolved in Phase-0 covering-set analysis for slice 9", "PLAN deferred list + slice 9 (Problem.py)"),
    "Validators": (DEF_HV, [], "deferred high-value next tranche (~41 char tests)", "PLAN deferred list"),
    "SolutionArms": (DEF_HV, [], "deferred high-value next tranche (~38 char tests)", "PLAN deferred list"),
    "CodegenResidue": (DEF_HV, [], "deferred high-value next tranche (_codegen family, ~242 char tests)", "PLAN deferred list (_codegen)"),
    "_codegen": (DEF_HV, [], "deferred high-value next tranche (~242 char tests)", "PLAN deferred list"),
}
...
DEFAULT_EVIDENCE = "PLAN: ranking will draw the scheduled/deferred cut once metrics are computed"


# --------------------------------------------------------------- metrics (read-only)
def _root():
    here = os.path.dirname(os.path.abspath(__file__))
    # wf/ -> tensilelite-characterization/ -> coverage-of-repo root is 3 up
...
def _dotted(module_path):
    return module_path[:-3].replace("/", ".") if module_path.endswith(".py") else module_path.replace("/", ".")
...
def build_rows(root, metrics=None):
    ...
    for d in dirs:
        if d in DISPOSITIONS:
            ...
            disp, mods, reason, ev = (OOS, [],
                "docs/meta only - no Python under the char dir (not a mutation target)",
                "read-only scan: no .py files in the dir")
        else:
            disp, mods, reason, ev = (DEFAULT_DISP, [], DEFAULT_REASON, DEFAULT_EVIDENCE)
        loc = loc_of(root, mods) if mods else None
        imp = importers_of(root, mods, py_files) if mods else None
        m = metrics.get(d, {})
        rows.append({
            "dir": d, "disposition": disp, "reason": reason, "evidence": ev,
            "modules": mods, "loc": loc, "importers": imp,
            "cyclomatic": m.get("cyclomatic"), "no_test_fraction": m.get("no_test_fraction"),
            "score": None,
        })
    _fill_scores(rows)
    return rows
...
def _fill_scores(rows):
    ...
    scored = [r for r in rows if None not in (r["importers"], r["cyclomatic"], r["loc"], r["no_test_fraction"])]
    if not scored:
        return
...
def render(rows):
    ...
    L.append("Read-only Phase-0 analysis (generated by `wf/rank-modules.py`). No mutation")
    L.append("testing was run; no production source edited.\n")
    L.append("## Ranking formula\n")
    L.append("All raw inputs are **min-max normalized to [0,1]** across candidate modules")
    ...
    L.append("  under `Tensile/` (incl. tests) that mention the module's dotted path. This")
    ...
    L.append("  `docker exec -w /work/%s tl-mut lizard <module.py>` (CCN sum)." % SRC_REL)
    L.append("- no_test_fraction: **PENDING** - `(1 - line_coverage)` from")
    L.append("  `docker exec -w /work/%s tl-mut pytest --cov=<module> --cov-report=term-missing <covering tests>`." % SRC_REL)
    L.append("")
    L.append("Because cyclomatic + no_test_fraction are PENDING, per-module `score` is")
    L.append("**PENDING** (not faked). Fill them and re-run `rank-modules.py` to publish the")
    L.append("computed order. The **slice-2..10 order is therefore interim/hand-ranked**")
    L.append("(from the plan) and marked pending metric completion.\n")
    L.append("## Disposition table (every characterization dir; exactly one disposition)\n")
    L.append("| char dir | disposition | LOC | importers | cyclomatic | no_test_frac | score | reason | evidence |")
    L.append("|---|---|---|---|---|---|---|---|---|")
    for r in rows:
        L.append("| %s | %s | %s | %s | %s | %s | %s | %s | %s |" % (
            r["dir"], r["disposition"], _cell(r["loc"]), _cell(r["importers"]),
            _cell(r["cyclomatic"]), _cell(r["no_test_fraction"]), _cell(r["score"]),
            r["reason"], r["evidence"]))
    L.append("")
    # disposition counts
    counts = {}
    ...
    L.append("Notes:")
    L.append("- `deferred-coverage-gap` is assigned only from measured `no_test_fraction`")
    L.append("  (PENDING), so no dir is placed there yet - it is defined and reserved.")
    L.append("- Dirs the plan did not explicitly place, that DO contain Python tests, are")
    L.append("  `deferred-high-value` marked PROVISIONAL until the ranking is computed (the")
    ...
    L.append("- Slices **8 (`BenchmarkSplitter.py`)** and **9 (`Problem.py`)** are Phase-2")
    L.append("  MODULE slices without a dedicated characterization dir (slice 8 uses")
    L.append("  `test_BenchmarkSplitter.py`; slice 9's covering set incl. the `ProblemType`")
    L.append("  dir is derived in Phase 0), so they do NOT appear as `scheduled` char-dir")
    L.append("  rows - tracked at module granularity in the plan slice table instead.")
    L.append("- The deferred-list counts in reasons (e.g. Activation ~156) are the plan's")
    L.append("  def-TEST counts, NOT LOC (the LOC column is the module's real line count).")
    return "\n".join(L) + "\n"


def main(argv=None):
    p = argparse.ArgumentParser(description="Phase-0 ranking + disposition generator (read-only)")
