...
check("g = 7" in keys("reformat_suspect"), "absent killed + new same-file survivor -> reformat_suspect")
check(len(res["regression"]) == 3, "exactly 3 regressions")

# killed -> timeout / suspicious also regressions
res2 = mod.compare(art(M(F, "t=1", "h_t", "killed"), M(F, "s=1", "h_s", "killed")),
                   art(M(F, "t=1", "h_t", "timeout"), M(F, "s=1", "h_s", "suspicious")))
check(len(res2["regression"]) == 2, "killed -> timeout and killed -> suspicious are regressions")
...
_mi_spec = _u.spec_from_file_location("mutant_identity", os.path.join(HERE, "..", "mutant-identity.py"))
_mi = _u.module_from_spec(_mi_spec); _mi_spec.loader.exec_module(_mi)
...
for st in ("equivalent", "pragma", "survivedd", "SURVIVED", "", None):
    ...
    check(len(r["regression"]) == 1, "killed -> %r is a regression (fail-closed)" % (st,))
...
check(len(mod.compare(dupbase, dup_ks)["regression"]) == 1
...
rc = art(M(F, "self.index = ( self.index + 1 ) % len(self.chars)", "h_r2", "survived"))  # respaced -> new key
res_r = mod.compare(rb, rc)
check(len(res_r["reformat_suspect"]) == 1 and len(res_r["absent"]) == 0,
...
check(len(res_a["absent"]) == 1 and len(res_a["reformat_suspect"]) == 0,
...
def cli(baseline, current, *extra):
    r = subprocess.run(
        [sys.executable, SUT, "--baseline", write(baseline), "--current", current if isinstance(current, str) else write(current)] + list(extra),
...
check(cli(clean_base, clean_cur, "--stage", "fail-on-regression") == 0, "CLI: killed->killed exit 0")
check(cli(clean_base, reg_cur, "--stage", "fail-on-regression") == 1, "CLI: killed->survived exit 1")
check(cli(clean_base, reg_cur, "--stage", "report-only") == 0, "CLI: report-only never fails (exit 0) even with a regression")
check(cli(eqbase, eqcur, "--stage", "fail-on-regression") == 0, "CLI: equivalent->killed is non-fatal by default (exit 0)")
check(cli(eqbase, eqcur, "--stage", "fail-on-regression", "--strict") == 1, "CLI: --strict fails on re-audit (exit 1)")
check(cli(newbase, newcur, "--stage", "fail-on-regression") == 0, "CLI: new mutant reported, not failed by default (exit 0)")
check(cli(newbase, newcur, "--stage", "fail-on-regression", "--fail-on-new") == 1, "CLI: --fail-on-new fails on new mutant (exit 1)")
check(cli(art(M(F, "a = 1", "h_a", "killed")), art(M(F, "a = 1", "h_a", "equivalent")), "--stage", "fail-on-regression") == 1,
      "CLI: killed->equivalent declassification exit 1")
check(cli(rb, rc, "--stage", "fail-on-regression") == 1, "CLI: reformat-masked regression exit 1")
