...
REGIONS = os.path.join(
    HERE, "..", "..", "coverage", "mutprod", "slices", "10-solution", "regions.json"
)
...
def base_doc():
    return {
        ...
        "regions": [
            {"id": "r01", "start_line": 61, "end_line": 261, "loc": 201,
             "same_level_fence": True, "covering_test_selection": ["t/a"]},
            {"id": "r02", "start_line": 262, "end_line": 396, "loc": 135,
             "same_level_fence": True, "covering_test_selection": ["t/b"]},
        ],
    }
...
ok("real regions.json validates clean") if not errs else bad("real regions.json errors: %r" % errs)

# --- proof region [61..261] present ---
ids = {r["id"]: r for r in real["regions"]}
r01 = ids.get("r01")
check(bool(r01) and r01["start_line"] == 61 and r01["end_line"] == 261, "proof region r01 [61..261] present")
...
d = base_doc(); d["regions"][1]["start_line"] = 60; d["regions"][1]["loc"] = 60 - 396  # unordered + will also flag loc
expect_error(d, "not ordered", "detects unordered regions")

d = base_doc(); d["regions"][1]["start_line"] = 200  # overlaps r01 (61..261)
d["regions"][1]["loc"] = d["regions"][1]["end_line"] - 200 + 1
expect_error(d, "overlaps", "detects overlapping regions")

d = base_doc(); d["regions"][0]["start_line"] = "61"
expect_error(d, "start_line must be an integer", "detects non-integer start_line")

d = base_doc(); d["regions"][0]["loc"] = 999
expect_error(d, "!= end-start+1", "detects loc inconsistent with span")

d = base_doc(); d["regions"][0]["loc"] = -1
expect_error(d, "loc must be a positive integer", "detects non-positive loc")

d = base_doc(); del d["regions"][0]["same_level_fence"]
expect_error(d, "same_level_fence", "detects missing same_level_fence")

d = base_doc(); d["regions"][0]["covering_test_selection"] = []
expect_error(d, "covering_test_selection is empty", "detects empty covering_test_selection")

d = base_doc(); d["regions"][1]["id"] = "r01"
expect_error(d, "duplicate id", "detects duplicate region id")

d = base_doc(); d["regions"][0]["end_line"] = 50  # end < start
...
d = base_doc(); d["regions"][0]["start_line"] = True
expect_error(d, "start_line must be an integer", "detects bool start_line (not just int)")

# negative / zero start
d = base_doc(); d["regions"][0]["start_line"] = 0
...
d = base_doc(); d["regions"][0]["covering_test_selection"] = "t/a"
...
d = base_doc(); d["regions"][0]["covering_test_selection"] = [1, ""]
expect_error(d, "non-string or empty member", "detects non-string/empty covering_test_selection member")

# structural: doc not a dict
check(bool(mod.validate_doc("nope")), "detects non-dict doc")
# structural: missing top-level key
d = base_doc(); del d["module"]
...
try:
    br = dict(r01); br["covering_test_selection"] = [1]
...
check(bool(cfg.get("only_mutate")) and isinstance(cfg["only_mutate"], list), "emit: only_mutate non-empty list")
check(bool(cfg.get("test_selection")) and isinstance(cfg["test_selection"], list), "emit: test_selection non-empty list")
check(cfg["only_mutate"] == ["Tensile/SolutionStructs/Solution.py"], "emit: only_mutate is the whole file (mutmut has no sub-file targeting)")
check("region" in cfg and cfg["region"]["start_line"] == 61 and cfg["region"]["end_line"] == 261, "emit: region metadata carried")
check("fence_gate" in cfg and "PragmaParseError" in cfg["fence_gate"], "emit: mandatory PragmaVisitor dry-parse gate hook present")
check("source_safety" in cfg and "diff --quiet" in cfg["source_safety"], "emit: source-safety/restore requirement present")
...
try:
    bad_reg = dict(r01); bad_reg["covering_test_selection"] = []
