...
REAL_DIFF = os.path.join(HERE, "..", "..", "coverage", "mutprod", "workflow", "diffs", "inc_7.diff")
...
check(mod.diff_hash(DIFF_A) == mod.diff_hash(DIFF_B),
...
check(mod.diff_hash(DIFF_A) != mod.diff_hash(DIFF_SEM),
...
check(mod.diff_hash(DIFF_INDENT) != mod.diff_hash(DIFF_INDENT_BASE),
...
check(mod.diff_hash(DIFF_WS) == mod.diff_hash(DIFF_INDENT_BASE),
...
check(all(not l.startswith("diff --git") and not l.startswith("index ")
...
check(canon == "-        self.index = (self.index + 1) % len(self.chars)\n"
               "+        self.index = (self.index - 1) % len(self.chars)",
      "canonical body is exactly the two changed lines (indentation preserved)")

# original_line + identity
check(mod.original_line(DIFF_A) == "self.index = (self.index + 1) % len(self.chars)",
...
check(ident["line_anchor"] == "self.index = (self.index + 1) % len(self.chars)",
...
check("line_number" not in dict(zip(("file", "line_anchor", "diff_hash"), mod.key_tuple(ident))),
      "CI key_tuple excludes line_number")
check(mod.key_tuple(ident) == ("Tensile/Common/Utilities.py",
                               "self.index = (self.index + 1) % len(self.chars)",
                               ident["diff_hash"]),
...
check(canon_dd == "--- header comment removed\n+++ header comment added",
      "hunk change lines starting with --/++ are kept, not dropped as headers (BUG-1)")
check(mod.diff_hash(DIFF_DASHDASH) != mod.diff_hash("@@ -1,1 +1,1 @@\n"),
...
DIFF_INS = "@@ -1,0 +2,1 @@\n+    logging.info('added')\n"
check(mod.original_line(DIFF_INS) == "logging.info('added')",
...
if os.path.isfile(REAL_DIFF):
    ...
    real2 = "# mutant xǁSpinnyThingǁincrement__mutmut_99\n" + real.replace("685ce17c5e5", "0000000")
    check(mod.diff_hash(real) == mod.diff_hash(real2),
