...
check(approx(mod.W_IMPORTERS + mod.W_CYCLOMATIC + mod.W_LOC + mod.W_NO_TEST, 1.0),
      "weights sum to 1.0")
...
check(approx(mod.score(0, 0, 0.5, 0), 0.075), "LOC contributes linearly (0.15*0.5), no log10")
check(approx(mod.score(0.5, 0.5, 0.5, 0.5), 0.5), "score is a convex combination (all 0.5 -> 0.5)")

# --- minmax_normalize ---
check(mod.minmax_normalize([10, 20, 30]) == [0.0, 0.5, 1.0], "minmax [10,20,30] -> [0,0.5,1]")
check(mod.minmax_normalize([5]) == [0.0], "minmax single element -> [0.0]")
check(mod.minmax_normalize([7, 7, 7]) == [0.0, 0.0, 0.0], "minmax all-equal (zero range) -> all 0.0")
check(all(0.0 <= v <= 1.0 for v in mod.minmax_normalize([3, 1, 4, 1, 5, 9, 2, 6])),
...
check(all(r["disposition"] in allowed for r in rows), "every disposition is one of the 5 allowed values")

# --- score/cyclomatic/no_test are PENDING (None), not faked, in this environment ---
check(all(r["score"] is None for r in rows), "every score is PENDING (not faked) while inputs are pending")
check(all(r["cyclomatic"] is None and r["no_test_fraction"] is None for r in rows),
...
check(li is not None and li["loc"] == 822 and isinstance(li["importers"], int),
...
check(disp_of.get("CommonUtilities") == mod.CERTIFIED and disp_of.get("TensileLogic") == mod.CERTIFIED,
      "slice-1 dirs certified")
check(disp_of.get("LibraryIO") == mod.SCHEDULED, "LibraryIO scheduled (slice 2)")
check(sum(1 for r in rows if r["disposition"] == mod.DEF_CG) == 0,
...
check(set(r["dir"] for r in scored) == {"LibraryIO", "Configuration", "ValidParameters"},
...
check(approx(mby["ValidParameters"]["score"], 0.80), "computed score matches formula (ValidParameters -> 0.80)")
check(mby["LibraryIO"]["score"] < mby["ValidParameters"]["score"],
      "ranking orders ValidParameters above LibraryIO")
check(mby.get("Naming") is not None and mby["Naming"]["score"] is None,
