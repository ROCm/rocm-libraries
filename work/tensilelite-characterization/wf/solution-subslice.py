...
def validate_doc(doc):
    ...
    for i, reg in enumerate(regions):
        ...
        if "same_level_fence" not in reg or not isinstance(reg["same_level_fence"], bool):
        ...
        if not isinstance(cts, list):
            ...
            errs.append("%s covering_test_selection is empty (config emit would be invalid)" % tag)
        elif any((not isinstance(x, str)) or (not x.strip()) for x in cts):
            errs.append("%s covering_test_selection has a non-string or empty member" % tag)
        # ordered by start_line and non-overlapping
        if i > 0:
            if start <= prev_start:
                errs.append("%s not ordered: start_line %d <= previous start_line %d" % (tag, start, prev_start))
            if start <= prev_end:
                errs.append("%s overlaps previous region (start %d <= previous end %d)" % (tag, start, prev_end))
        prev_start = start
        prev_end = end
...
def build_region_config(doc, reg):
    ...
    if not cts:
        raise ValueError("covering_test_selection is empty; cannot emit a valid test_selection")
    bad_members = [x for x in cts if (not isinstance(x, str)) or (not x.strip())]
    if bad_members:
        raise ValueError("covering_test_selection has non-string/empty members: %r" % bad_members)
    cfg = {
        "slice_id": "10-%s" % rid,
        "only_mutate": [module],
        "test_selection": list(cts),
        "out": "work/tensilelite-characterization/coverage/mutprod/slices/10-solution/regions/%s" % rid,
        "src_rel": src_rel,
        "container": container,
...
def cmd_validate(args):
    ...
    if unsafe:
        print("NOTE: %d region(s) NOT same-level-fenceable (need structural handling): %s"
...
def cmd_emit_config(args):
    ...
    errs = validate_doc(doc)
    if errs:
        sys.stderr.write("refusing to emit: regions.json is invalid (%d error(s)); run `validate` first:\n" % len(errs))
        for e in errs:
            sys.stderr.write("  - %s\n" % e)
        return 1
    try:
    ...
    if args.out and args.out != "-":
        with open(args.out, "w") as fh:
            fh.write(text)
        print("wrote %s (region %s, lines %d..%d)"
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Solution.py sub-slicing region model + config emitter")
    sub = p.add_subparsers(dest="cmd")
    pv = sub.add_parser("validate", help="validate a regions.json")
    pv.add_argument("--regions", required=True)
    pe = sub.add_parser("emit-config", help="emit a mutmut-slice.js config for one region")
