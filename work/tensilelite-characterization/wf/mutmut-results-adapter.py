...
def build_groups(survivors, opts=None):
    ...
    active, no_test = [], []
    for r in survivors:
        if not isinstance(r, dict) or any(r.get(k) is None for k in ("module", "mutant_id", "file", "line")):
            raise ValueError("survivor record missing required field {module,mutant_id,file,line}: %r" % (r,))
        try:
            int(r["line"])
        except (TypeError, ValueError):
            raise ValueError("survivor record has a non-integer line: %r" % (r,))
        if _is_no_test(r.get("status")):
            no_test.append({"module": r["module"], "mutant_id": r["mutant_id"], "file": r["file"], "line": r["line"]})
        else:
            active.append(r)
    ...
    for r in active:
        ...
        if f not in funcs_by_file:
            ...
            try:
                ...
                funcs_by_file[f] = None  # missing/unparseable -> module-level (never crash/hang)
        funcs = funcs_by_file[f]
        fn = (enclosing_for_line(funcs, int(r["line"])) if funcs is not None else None) or MODULE_LEVEL
        key = "%s::%s" % (r["module"], fn)
        if key not in groups_by_key:
            cd = pick_char_dir(char_dir_map, r["module"], fn, f, fallback)
            if cd is None:
                raise ValueError("no char_dir for group %s (provide charDirMap entry or defaultCharDir)" % key)
            g = {
                "module": r["module"],
                "function": fn,
    ...
    if len(active) != total_out:
        raise ValueError("survivor conservation failed: %d active in, %d placed" % (len(active), total_out))
    seen = set()
    for g in groups:
...
def _parse_args(argv):
    ...
    while i < len(argv):
        ...
        if a in ("-h", "--help"):
            ...
            i += 1
            continue
        if a not in ("--fixture", "--src-root", "--default-char-dir", "--test-dir-base"):
...
def main(argv):
    ...
    opts = {
        "srcRoot": o.get("--src-root") or (fixture.get("srcRoot") if isinstance(fixture, dict) else None),
        "testDirBase": o.get("--test-dir-base") or (fixture.get("testDirBase") if isinstance(fixture, dict) else None),
        "charDirMap": fixture.get("charDirMap") if isinstance(fixture, dict) else None,
        "defaultCharDir": dcd if dcd is not None else (fixture.get("defaultCharDir") if isinstance(fixture, dict) else None),
    }
    try:
