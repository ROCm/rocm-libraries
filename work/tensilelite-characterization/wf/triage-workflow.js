export const meta = {
  ...
  description:
    'Triage + kill the mutmut survivors of ONE slice. Parallel per-function triage authors add-only characterization tests (proposal-only, no source edits); a SINGLE serial agent kill-proofs them via wf/mutmut-verify.sh; one bounded repair round; a SINGLE serial agent applies any pragmas; then synthesis. Concurrency rule: only triage fans out — apply/verify/pragma are single serial actors. Per-slice output dir and pragma-stage pytest selection are supplied via args (not hard-coded), so mutmut-slice.js can reuse this per module.',
  phases: [
    { title: 'Triage' },
    { title: 'Verify' },
    { title: 'Repair' },
...
const DEFAULT_OUT = 'work/tensilelite-characterization/coverage/mutprod/workflow'
...
// wf/mutmut-results-adapter.py); host-FS access prefixes ${WT}/${SRC_REL}/.
