...
#
# Usage:
#   wf/slice-preflight.sh --slice <id> --module <path-or-name> \
#       [--out <dir>] [--container tl-mut] [--src projects/hipblaslt/tensilelite]
#
# Output artifact (on success only):
#   work/tensilelite-characterization/coverage/mutprod/slices/<slice>-<slug>/env.json
#
# Testability: source with MUTMUT_PREFLIGHT_LIB_ONLY=1 to expose the pure helpers
# (derive_slug, emit_json) without running main() — see wf/tests/preflight-selftest.sh.
...
data = {
    ...
    "generator": "work/tensilelite-characterization/wf/slice-preflight.sh",
}
...
if [[ -z "$OUT" ]]; then
  OUT="work/tensilelite-characterization/coverage/mutprod/slices/${SLICE}-${SLUG}"
fi
